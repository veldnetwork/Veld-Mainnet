(function () {
    ...
    function isConsolidation(entry, tx, address) {
        return !!(
            entry &&
            entry.type === 'self' &&
            Number(entry.net_veld) === 0 &&
            tx &&
            tx.txid === entry.txid &&
            tx.block_height === entry.block_height &&
            tx.coinbase === false &&
            Array.isArray(tx.vin) &&
            tx.vin.length > 1 &&
            Array.isArray(tx.vout) &&
            tx.vout.length === 1 &&
            tx.vout[0].address === address &&
                ...
                fetch(item.path, {
                    ...
                    .then(function (text) {
                        if (text.length > 4 * 1024 * 1024)
                            throw new Error('Details exceed display limit');
    ...
    function read(path) {
        ...
        if (++requested > MAX_REQUESTS)
            return Promise.reject(new Error('Display request limit reached'));
        var result = new Promise(function (resolve, reject) {
            pending.push({ path: path, resolve: resolve, reject: reject });
            drain();
        });
    ...
    function transaction(txid, height) {
        ...
        return read('/api/v1/transaction?height=' + height + '&txid=' + txid).then(function (tx) {
            if (tx.txid !== txid || tx.block_height !== height)
                throw new Error('Transaction reference changed');
            return tx;
        });
    }
    function historyEntry(address, txid, height) {
        ...
        function page(cursor, count) {
            ...
            return read(path).then(function (data) {
                ...
                var entry = data.entries.find(function (row) {
                    return row.txid === txid && row.block_height === height;
                });
                if (entry) return entry;
                var oldest = data.entries[data.entries.length - 1];
                if (!data.has_more || !oldest || oldest.block_height < height || count >= 10)
                    return null;
    ...
    function setFee(cell, entry, coinbase) {
        ...
        cell.title =
            fee == null
                ? 'The fee could not be verified from transaction history.'
                : 'Confirmed network fee';
        cell.style.color = fee == null ? 'var(--muted)' : 'var(--text)';
    }
    function blockRows() {
        var title =
            document.querySelector('.block-detail-header') || document.querySelector('.pheader');
        ...
        document.querySelectorAll('.tbl').forEach(function (table) {
            ...
            table.querySelectorAll('tbody tr').forEach(function (row, ordinal) {
                ...
                if (badge.textContent.trim() === 'COINBASE') {
                    setFee(cells[4], null, true);
                    return;
                }
                ...
                historyEntry(address, txid, height)
                    .then(function (entry) {
                        if (!row.isConnected || !entry) return;
                        setFee(cells[4], entry, false);
                        if (entry.type !== 'self' || badge.textContent.trim() !== 'TRANSFER')
                            return;
    ...
    var sourceLabels = {
        staking_distribution: 'Staking reward',
        endorsement_distribution: 'Validator reward',
        comine_distribution: 'Co-mining payout',
        stake_lock: 'Stake change',
        stake_unlock: 'Unstaked funds',
    };
    function recoverOutput(row, address) {
        var name = row.querySelector('.rl-name'),
            sub = row.querySelector('.rl-sub');
        if (
            !name ||
            !sub ||
            !/source unavailable|details unavailable|Loading output details|^Transfer received$/i.test(
                name.textContent,
            )
        )
            return;
        ...
        read('/api/v1/events/' + height)
            .then(function (data) {
                if (data.height !== height || !Array.isArray(data.events))
                    throw new Error('Block reference changed');
                var event = data.events.find(function (item) {
                    return item.txid === txid;
                });
                var output =
                    event &&
                    Array.isArray(event.credited) &&
                    event.credited.find(function (item) {
                        return item.address === address;
                    });
                if (!output) throw new Error('Output reference unavailable');
                if (!row.isConnected) return;
                if (event.type === 'coinbase')
                    name.textContent = output.protocol
                        ? 'Block reward allocation'
                        : 'Mining reward';
                else if (sourceLabels[event.type]) name.textContent = sourceLabels[event.type];
                else
                    return historyEntry(address, txid, height).then(function (entry) {
                        if (entry && entry.type === 'self')
                            return transaction(txid, height).then(function (tx) {
                                name.textContent = isConsolidation(entry, tx, address)
                                    ? 'Consolidated funds'
                                    : 'Self-transfer';
                            });
            ...
            .catch(function () {
                if (row.isConnected) name.textContent = 'Output details unavailable';
            });
    }
    function addressHistory(address) {
        ...
        function enhance() {
            ...
            rows.forEach(function (row) {
                ...
                var txid = link && link.getAttribute('href').split('/').pop();
                var entry = (window._expHist || []).find(function (item) {
                    return item.txid === txid;
                });
                if (!entry || entry.type !== 'self') return;
                var name = row.querySelector('.rl-name'),
                    value = row.querySelector('.rl-v'),
                    detail = row.querySelector('.rl-vs');
                if (name) name.textContent = 'Self-transfer';
                if (value) {
                    value.textContent = 'Same wallet';
                    value.style.color = 'var(--muted)';
                }
                if (detail) {
                    var fee = feeText(entry.fee_veld);
                    detail.textContent = fee == null ? 'Fee unavailable' : fee + ' VELD fee';
                }
                if (!checks.has(txid))
                    checks.set(
                        txid,
                        transaction(txid, entry.block_height)
                            .then(function (tx) {
                                return isConsolidation(entry, tx, address);
                            })
                            .catch(function () {
                                return false;
                            }),
                    );
                checks.get(txid).then(function (yes) {
                    if (yes && row.isConnected && name) name.textContent = 'Consolidation';
                });
            });
        }
        window.expHistRender = function () {
    ...
    function start() {
        ...
        if (match) {
            document.querySelectorAll('.card .row-list > a.rl-row').forEach(function (row, i) {
                if (i < 25) recoverOutput(row, match[1]);
            });
            addressHistory(match[1]);
        }
    }
    window.VeldTransactionDisplay = Object.freeze({
        feeText: feeText,
