    ...
    function paintExplorerDisplay() {
        if (location.pathname === '/mempool') {
            ...
            document.querySelectorAll('.tile').forEach((tile) => {
                ...
                if (
                    label &&
                    /^(Lifetime fees collected|Mempool fees)/i.test(label.textContent.trim())
                ) {
        ...
        if (location.pathname === '/mining') {
            document.querySelectorAll('.stat').forEach((stat) => {
                ...
                value.title =
                    'Exact block reward: 3.13926940 VELD. This display truncates to two decimals.';
            });
            document.querySelectorAll('.card').forEach((card) => {
                ...
                card.querySelectorAll('tbody tr').forEach((row) => {
                    ...
                    const color =
                        text === 'Miner' ? 'var(--em)' : text === 'Vault' ? 'var(--gold)' : '';
                    if (color) {
