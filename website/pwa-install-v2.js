(function () {
    ...
    function polishWalletUI() {
        ...
        document.querySelectorAll('#page-comining div').forEach(function (element) {
            ...
            if (value.indexOf('five winners below 1,000 eligible miners') !== -1) {
                element.textContent =
                    'Every 100 blocks, the pool is split equally between randomly drawn eligible miners.';
            }
        });
    ...
    function polishInstallUI() {
        ...
        if (banner) {
            ...
            if (title)
                title.textContent = isWallet ? 'Install Veld Wallet' : 'Install Veld Explorer';
        ...
        if (head) {
            ...
            if (heading)
                heading.textContent = isWallet ? 'Install Veld Wallet' : 'Install Veld Explorer';
            if (copy)
                copy.textContent = 'Use your browser menu to add the app in four quick steps.';
        }
    ...
    function ensureExplorerInstallSurface() {
        ...
        banner.innerHTML =
            '<span class="copy"><b>Install Veld Explorer</b><span>Open the app directly from your Home Screen</span></span><button type="button" class="install-btn" id="explorer-install-btn">Install</button><button type="button" class="dismiss-btn" id="explorer-install-dismiss" aria-label="Dismiss install prompt">&times;</button>';
        ...
        var icon = function (path) {
            return (
                '<span class="explorer-install-step-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">' +
                path +
                '</svg></span>'
            );
        };
        var step = function (iconPath, title, copy) {
            return (
                '<div class="explorer-install-step">' +
                icon(iconPath) +
                '<span class="explorer-install-step-copy"><b>' +
                title +
                '</b><span>' +
                copy +
                '</span></span></div>'
        ...
        sheet.innerHTML =
            '<div class="explorer-install-card"><div class="explorer-install-head"><div><h2 id="explorer-install-title">Install Veld Explorer</h2><p>Use your browser menu to add the app in four quick steps.</p></div><button type="button" class="sheet-close" aria-label="Close install instructions">&times;</button></div><div class="explorer-install-steps">' +
            step(
                '<path d="M12 3v12"/><path d="M8 7l4-4 4 4"/><path d="M8 12H6a2 2 0 0 0-2 2v5a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-5a2 2 0 0 0-2-2h-2"/>',
                'Open Share',
                'Tap the Share icon in your browser toolbar or menu.',
            ) +
            step(
                '<path d="m6 9 6 6 6-6"/>',
                'Show all actions',
                'Tap View More if the full action list is collapsed.',
            ) +
            step(
                '<rect x="4" y="4" width="16" height="16" rx="3"/><path d="M12 8v8M8 12h8"/>',
                'Add to Home Screen',
                'Choose Add to Home Screen from the action list.',
            ) +
            step(
                '<path d="m5 12 4 4L19 6"/>',
                'Confirm',
                'Tap Add. The app will open from your Home Screen.',
            ) +
            '</div></div>';
        ...
        sheet.querySelector('.sheet-close').addEventListener('click', function () {
            sheet.classList.remove('open');
        });
        ...
        if ('serviceWorker' in navigator)
            navigator.serviceWorker
                .register('/sw.js?ui=20260907-current-web', { scope: '/', updateViaCache: 'none' })
                .catch(function () {});
    }
    ...
    document.addEventListener('click', function (event) {
        ...
        if (
            ...
            event.target.closest('#pwa-install-btn,#explorer-install-btn')
        ) {
