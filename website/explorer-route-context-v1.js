(function () {
    ...
    document.documentElement.dataset.deviceLayout =
        compactLayout || touchLayout ? 'mobile' : 'desktop';
    ...
    function renderConsensusPhase(stats) {
        ...
        if (path === '/vault') {
            var reward = Number(stats.block_reward_veld || 3.1392694);
            var ordinaryShare = active ? 0.2 : 0.5;
            var inflow = 480 * reward * ordinaryShare + 480 * 0.01 * reward * (1 - ordinaryShare);
            setStat('Daily Inflow', '~' + inflow.toFixed(1), 'VELD/day into vault');
            setStat(
                'Daily Payout',
                active ? '~' + (inflow * 0.9).toFixed(1) : '0.0',
                active
                    ? 'VELD/day to stakers (90% inflow cap)'
                    : 'staking inactive; no distribution',
            );
            setStat(
                'Daily Retention',
                active ? '~' + (inflow * 0.1).toFixed(1) : '~' + inflow.toFixed(1),
            ...
            if (firstRow)
                firstRow.innerHTML =
                    '<td>Ordinary block reward share</td><td style="color:var(--em)">' +
                    (active ? '20%' : '50%') +
                    '</td><td style="color:var(--muted)">Every non-vault block</td>';
        }
        ...
        if (path === '/rules') {
            ...
            if (splitIntro)
                splitIntro.innerHTML =
                    'Ordinary blocks use the four-way split below. Every 100th block routes its full subsidy to the vault.';
            ...
            if (items)
                Array.from(items).forEach(function (item) {
                    ...
                    item.innerHTML =
                        '<strong>Cryptographic boundary.</strong> Native VELD transaction and finality signatures use ML-DSA-65. Native ordinary outputs currently use 160-bit HASH160 key commitments, so this is not an end-to-end post-quantum claim. btcVELD custody inherits Bitcoin Taproot\u2019s current secp256k1/Schnorr assumptions.';
                });
        }
    }
    ...
    function clarifyStakingMinimum() {
        if (window.location.pathname === '/staking') {
            document.querySelectorAll('table.tbl tbody tr').forEach(function (row) {
                ...
                if (
                    cells.length !== 3 ||
                    !/^Minimum (Ordinary )?Stake$/.test(cells[0].textContent.trim())
                )
                    return;
    ...
    function colorRewardBadge(badge) {
        ...
        var staking =
            /^(stake|unstake|stake lock|stake unlock|staking dist|staking rewards)$/.test(label) ||
            (label === 'payout' && badge.classList.contains('bk-badge'));
        var validator =
            /^(endorse|endorsement|endorse reward|validator|validator reg|validator dereg|val payout|validator payout)$/.test(
                label,
            );
        ...
        if (staking && badge.classList.contains('badge')) {
            ...
            if (heading)
                Array.from(heading.cells).forEach(function (cell, index) {
    ...
    function colorRewardBadges(root) {
        ...
        if (root.querySelectorAll)
            root.querySelectorAll('.badge,.op,.bk-badge').forEach(colorRewardBadge);
    }
    ...
    function markMempoolContext() {
        document
            .querySelectorAll('.nav-bar .nb-tab.active, .side-nav .sn-link.active')
...
(function () {
    ...
    function updateAge(el) {
        ...
        var seconds = observed
            ? (performance.now() - seen) / 1000
            : Date.now() / 1000 - Number(el.dataset.blockTime);
    ...
    function updateTipAge(hash) {
        ...
        if (el && typeof seen === 'number')
            el.textContent = '+' + displayAge((performance.now() - seen) / 1000);
    }
    ...
    function validate(data, height, hash) {
        if (
            !data ||
            !Number.isSafeInteger(data.tip_height) ||
            data.tip_height < height ||
            !/^[0-9a-f]{64}$/.test(data.tip_hash || '') ||
            data.start !== height ||
            data.end !== Math.max(0, height - 9) ||
            !Array.isArray(data.blocks) ||
            data.blocks.length !== data.start - data.end + 1
        )
            throw new Error('Invalid block page');
        data.blocks.forEach(function (block, index) {
            if (
                !block ||
                block.height !== height - index ||
                !/^[0-9a-f]{64}$/.test(block.hash || '') ||
                !Number.isFinite(block.time) ||
                block.time < 0
            )
                throw new Error('Invalid block');
            if (index && data.blocks[index - 1].prev_hash !== block.hash)
                throw new Error('Disconnected block page');
        });
        if (hash && data.blocks[0].hash !== hash) {
            var changed = new Error('Tip changed');
            changed.retryMs = 2000;
            throw changed;
        }
        return data.blocks;
    }
    function paint(blocks, observed) {
        ...
        blocks.forEach(function (block, index) {
            if (!firstSeen.has(block.hash))
                firstSeen.set(block.hash, observed && index === 0 ? performance.now() : null);
            ...
            slot.dataset.bh = block.height;
            slot.dataset.blockHash = block.hash;
            ...
            info.appendChild(
                textElement(
                    'div',
                    's',
                    shortHash(block.hash) + (block.miner ? ' · ' + shortAddr(block.miner) : ''),
                ),
            );
            link.appendChild(info);
            var right = textElement('div', 'right');
            var age = textElement('div', 'ago');
            age.dataset.blockTime = block.time;
            age.dataset.blockHash = block.hash;
            updateAge(age);
            ...
            right.appendChild(
                textElement(
                    'div',
                    'tx',
                    (block.tx_count || block.ntx || 0) +
                        ' tx · ' +
                        (block.size_kb || Math.round((block.size || 0) / 102.4) / 10) +
                        ' KB',
                ),
            );
    ...
    window.veldExplorerDashboard = {
        load: function (height, hash) {
            ...
            return fetch('/api/v1/blocks/' + height + '/10', {
                cache: 'no-store',
                signal: controller.signal,
            })
                .then(function (response) {
                    if (!response.ok) {
                        var error = new Error('Block page unavailable');
                    ...
                    var observed =
                        ...
                        ((tip.height === previous.height + 1 && tip.prev_hash === previous.hash) ||
                            (tip.height === previous.height &&
                                tip.hash !== previous.hash &&
                                tip.prev_hash === previous.prev_hash));
                    paint(blocks, observed);
                    state.loaded = key;
                    state.failures = 0;
                    state.retryAt = 0;
                })
                .catch(function (error) {
                    if (epoch !== state.epoch || (controller.signal.aborted && document.hidden))
                        return;
                    ...
                    state.retryAt =
                        ...
                        Math.max(
                            error.retryMs || 0,
                            Math.min(30000, 1000 * Math.pow(2, state.failures)),
                        );
                    state.loaded = '';
                    state.continuous = false;
                    if (list) {
                        var status = textElement(
                            'div',
                            'explorer-data-status',
                            'Recent blocks are temporarily unavailable. Retrying automatically.',
                        );
    ...
    document.addEventListener('visibilitychange', function () {
        if (document.hidden) pauseObservation();
    });
...
(function () {
    ...
    var badgeTypes = {
        btcveld_mint: ['Mint', 'gold'],
        btcveld_redeem: ['Redeem', 'gold'],
        btcveld_transfer: ['btcVELD', 'gold'],
        amm_op: ['AMM', 'blue'],
        stake_lock: ['Stake', 'veld-reward-stake'],
        stake_unlock: ['Unstake', 'veld-reward-stake'],
        endorsement: ['Endorse', 'veld-reward-validator'],
        validator_register: ['Validator', 'veld-reward-validator'],
        staking_distribution: ['Payout', 'veld-reward-stake'],
        endorsement_distribution: ['Val payout', 'veld-reward-validator'],
        anchor_post: ['Anchor', 'gold'],
        btc_header_relay: ['BTC hdr', 'gold'],
        near_miss: ['Near miss', 'veld-near-miss'],
    };
    var cache = new Map(),
        pending = new Map(),
        retries = new Map();
    var retryTimer = null,
        list = null;
    function visible() {
        return !document.hidden && (location.pathname === '/' || location.pathname === '/blocks');
    }
    function describe(row) {
        var match = /^\/block\/height\/(\d+)$/.exec(row.getAttribute('href') || '');
        var meta = row.querySelector('.info .s'),
            count = row.querySelector('.tx');
        ...
        return {
            row: row,
            height: height,
            key: height + ':' + meta.textContent.trim().split(/\s/)[0],
        };
    }
    function render(item, types) {
        ...
        types.forEach(function (type) {
            ...
            badge.className = 'bk-badge ' + badgeTypes[type][1];
            badge.textContent = badgeTypes[type][0];
    ...
    function refresh() {
        ...
        pending.forEach(function (controller, key) {
            if (!keys.has(key)) controller.abort();
        });
        retries.forEach(function (_, key) {
            if (!keys.has(key)) retries.delete(key);
        });
        items.forEach(function (item) {
            if (cache.has(item.key)) {
                render(item, cache.get(item.key));
                return;
            }
            if (pending.has(item.key)) return;
            if (Date.now() < (retries.get(item.key) || 0)) {
                scheduleRetry();
                return;
            }
            if (pending.size >= 2) return;
            var controller = new AbortController();
            pending.set(item.key, controller);
            var timer = setTimeout(function () {
                controller.abort();
            }, 8000);
            fetch('/api/v1/events/' + item.height, { cache: 'no-store', signal: controller.signal })
                ...
                .then(function (data) {
                    ...
                    if (!data || data.height !== item.height || !Array.isArray(data.events))
                    ...
                    var types = Array.from(
                        ...
                        return Object.prototype.hasOwnProperty.call(badgeTypes, type);
                    });
                    cache.set(item.key, types);
                    retries.delete(item.key);
                    if (cache.size > 128) cache.delete(cache.keys().next().value);
                    render(item, types);
                })
                .catch(function (error) {
                    retries.set(item.key, Date.now() + Math.max(5000, error.retryMs || 0));
                    scheduleRetry();
                })
                .finally(function () {
                    clearTimeout(timer);
    ...
    function start() {
        list = document.getElementById(
            location.pathname === '/' ? 'blocks-list' : 'blocks-container',
        );
        if (!list) return;
    ...
    function initialize() {
        ...
        var selection = links.map(function (link) {
            return {
                ...
                sticky: link.getAttribute('data-active-sticky'),
                current: link.getAttribute('aria-current'),
            };
        });
        var stateKey =
            'mempool-' + Date.now().toString(36) + '-' + Math.random().toString(36).slice(2);
        ...
        function restoreOrigin() {
            ...
            mempoolView = null;
            if (originDisplay)
                originView.style.setProperty('display', originDisplay, originPriority);
        ...
        function loadMempool(push) {
            ...
            fetch('/mempool', {
                ...
                .then(function (html) {
                    ...
                    if (!content || !/Mempool/i.test(page.title))
                        throw new Error('Invalid Mempool page');
                    // The existing document owns scripts and controls; the response supplies content.
                    content
                        .querySelectorAll('script,base,iframe,object,embed,link,meta')
                        .forEach(function (el) {
                            el.remove();
                        });
                    ...
                    if (push) {
                        if (!historyReady) {
                            history.replaceState(
                                { veldMempool: stateKey, view: 'origin', previous: history.state },
                                '',
                                originUrl,
                            );
                            historyReady = true;
                        ...
                        history.pushState(
                            { veldMempool: stateKey, view: 'mempool' },
                            '',
                            '/mempool',
                        );
                    }
        ...
        document.addEventListener('click', function (event) {
            ...
            var target =
                ...
                event.target.closest('.nav-bar a.nb-tab[href], .side-nav a.sn-link[href]');
            if (
                !target ||
            ...
            if (url.pathname === '/mempool' && !url.search && !url.hash) {
                ...
                event.preventDefault();
                if (mempoolView)
                    history.pushState({ veldMempool: stateKey, view: 'origin' }, '', originUrl);
                restoreOrigin();
            } else if (controller) {
                cancelRequest();
            }
