# Consensus security upgrade candidate

Base: `2bef06ec41ec5d52d9095d558c81c9fee06651a3` (Veld 3.0.5).
This contract supersedes the earlier decisions-only document. The four changes
are implemented in the isolated remediation candidate. The user selected
**provisional H=1920**; the candidate's default activation constant now encodes it.
A binary built from this candidate activates automatically at 1920. It is **not
qualified for deployment**: live-height runway, authenticated checkpoint, complete
replay/restart qualification and compatible adoption remain unverified. Nothing
has been deployed or activated on mainnet by this preparation.

## Shared activation and replay contract

H=1920 is a settlement/epoch boundary (four intervals of 480 in the mainnet
profile). There is no runtime or per-node override. Default module tests exercise
1920; an explicitly supplied test H=960 remains a regression control requiring
...
Before release activation, choose H and attest an authenticated pre-H checkpoint,
including existing exact durable floors and completed principal payouts. Perform
full canonical replay, restart, snapshot-import, and carrier-replacement
qualification on the release candidate, then coordinate compatible node, validator,
and wallet adoption. Old software is incompatible after H; this patch does not
establish fleet adoption or add a release-version rollout mechanism. No activation,
signing, publication, deployment, or live-network testing is part of this candidate.
...
Validation uses small module snapshots, direct dispatcher calls, ordinary local
controls, and exact pre-H output comparison. It does not constitute full-chain
fork convergence, crash/restart, real-wallet signing, release qualification, or
network adoption evidence. The accompanying remediation report records actual
commands, results, patch/tree hashes, independent-review corrections, and remaining
activation and qualification work. All four findings remain pending production
activation and that qualification, even when these local controls pass.
