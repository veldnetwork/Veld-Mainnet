...
class Payments:
    def __init__(
        ...
        require(
            fee_address != pool.pool_address and fee_script != pool_script, 'separate fee identity'
        )
        self.fee = units(str(fee_units))
        self.dust = units(str(dust_units))
    ...
    def apply(self, kind, value, sequence=0):
        ...
        if kind == 'payment_batch':
            self.batches[value['id']] = value
        elif kind == 'payment_intent':
            self.intents[value['id']] = dict(value, history_sequence=sequence)
        elif kind == 'payment_signed':
            self.intents[value['id']].update(value)
        elif kind in ('payment_state', 'payment_notice'):
            self.intents[value['id']].update(value)
    ...
    def record(self, kind, value):
        sequence = self.journal.append(kind, value)
        self.apply(kind, value, sequence)

    def deductions(self):
        result = {}
        for intent in self.intents.values():
            for account, amount in intent['deductions'].items():
                result[account] = result.get(account, 0) + units(amount)
        return result
    ...
    def public_summary(self):
        with self.lock:
            return self.public_overview.snapshot()
    ...
    def history(self, account, before=0):
        ...
        with self.lock:
            rows = sorted(
                (
                    p
                    for p in self.intents.values()
                    if account in p['deductions'] and (not before or p['history_sequence'] < before)
                ),
                key=lambda p: p['history_sequence'],
                reverse=True,
            )
            selected = rows[:20]
            return {
                'payments': [
                    {
                        ...
                        'amount_units': p['deductions'][account],
                        'state': p['state'],
                        'txid': p.get('txid'),
                    }
                    for p in selected
    ...
    def available(self):
        ...
        for income in self.pool.incomes.values():
            if income['state'] == 'available':
                for account, amount in income['credits'].items():
                    result.setdefault(account, []).append(Fraction(amount))
        result = {account: floor_total(values) for account, values in result.items()}
        for account, amount in self.deductions().items():
            result[account] = result.get(account, 0) - amount
        require(
            all(amount >= 0 for amount in result.values()),
            'income reorganization deficit: payments stopped for reconciliation',
        )
        return result
    ...
    def plan(self, now=None):
        ...
        with self.pool.lock, self.lock:
            ...
            require(
                not operator or not operator.faulted, 'operator journal failed; payments stopped'
            )
            ...
            if self.batches:
                ...
                require(
                    scope in ('account', 'payout_address'), 'unknown frozen payout threshold scope'
                )
                address_scope = scope == 'payout_address'
                if address_scope:
                    require(
                        set(batch.get('destinations', {})) == set(batch['quotas'])
                        and all(
                            self.pool.accounts[a]['address'] == destination
                            for a, destination in batch['destinations'].items()
                        ),
                ...
                for intent in self.intents.values():
                    if intent.get('batch') == batch['id']:
                        for account, amount in intent['deductions'].items():
                            spent[account] = spent.get(account, 0) + units(amount)
            ...
            if not quotas:
                prior = [i['created'] for i in self.intents.values()] + [
                    b['created'] for b in self.batches.values()
                ]
                if prior and now - max(prior) < units(policy['batch_seconds']):
                    return None
                minimum = units(policy['minimum_units'])
                address_scope = True
                ...
                batch = {
                    'created': now,
                    'quotas': {a: str(v) for a, v in sorted(quotas.items())},
                    'policy': 'operator-payment-policy-v3',
                    'threshold_scope': 'payout_address',
                ...
                batch['id'] = hashlib.sha256(encode(batch)).hexdigest()
                self.record('payment_batch', batch)
            require(
                all(available.get(a, 0) >= v for a, v in quotas.items()),
                'batch income no longer available',
            )
            allowed = {
                (i['txid'], o['vout'])
                for i in self.pool.incomes.values()
            ...
            for intent in self.intents.values():
                if intent.get('state') == 'confirmed':
                    ...
                    for out in tx['vout']:
                        if out['script_pubkey'] == self.pool_script:
                            allowed.add((intent['txid'], out['n']))
            reserved = {
                (i['txid'], units(i['vout']))
                for p in self.intents.values()
                for i in p['wire']['inputs']
            }
            pool_coins = self.node.call('listunspent', self.pool.pool_address)
            backed = sum(
                c['value_units']
                for c in pool_coins
                if (c['txid'], c['vout']) in allowed and (c['txid'], c['vout']) not in reserved
            )
            ...
            require(
                backed >= sum(available.values()),
                'miner liability backing deficit; restore signing records and reconcile',
            )
            selected = []
            pool_funding = 0
            for role, address, needed in [
                ('pool', self.pool.pool_address, sum(quotas.values())),
                ('fees', self.fee_address, self.fee),
            ]:
                gathered = 0
                for coin in sorted(
                    pool_coins if role == 'pool' else self.node.call('listunspent', address),
                    key=lambda c: (-c['value_units'], c['txid'], c['vout']),
                ):
                    identity = (coin['txid'], coin['vout'])
                    if identity in reserved or (role == 'pool' and identity not in allowed):
                        continue
                    require(
                        type(coin['value_units']) is int and coin['value_units'] > 0,
                        'funding amount',
                    )
                    if (
                        role == 'pool'
                        and coin['confirmations'] < 120
                        and identity[0] not in {p.get('txid') for p in self.intents.values()}
                    ):
                        continue
                    if len(selected) >= (63 if role == 'pool' else 64):
                        break
                    selected.append(
                        {
                            'txid': hex64(coin['txid']),
                            'vout': str(coin['vout']),
                            'units': str(coin['value_units']),
                            'role': role,
                        }
                    )
                    gathered += coin['value_units']
                    if gathered >= needed and (
                        role == 'pool' or gathered == needed or gathered - needed >= self.dust
                    ):
                        break
                if role == 'fees':
                    require(gathered >= needed, 'insufficient segregated operator fee funding')
                    require(
                        gathered == needed or gathered - needed >= self.dust,
                        'operator change below native dust policy',
                    )
                else:
                    if gathered < minimum:
                        return None  # Wait for confirmed change; keep the frozen batch.
                    pool_funding = gathered
            selected_groups = {}
            remaining = pool_funding
            for group, values in sorted(groups.items()):
                if len(selected_groups) >= 128 or remaining < minimum:
                    break
                amount = min(sum(values.values()), remaining)
                selected_groups[group] = amount
                remaining -= amount
            if 0 < remaining < self.dust:
                ...
                if selected_groups[group] < minimum:
                    del selected_groups[group]
            amounts = {}
            ...
            for account, amount in amounts.items():
                ...
                require(
                    address not in (self.pool.pool_address, self.fee_address),
                    'self payout is not permitted',
                )
                recipients[address] = recipients.get(address, 0) + amount
            wire = {
                'chain': self.node.genesis,
                'pool_script': self.pool_script,
                'fee_script': self.fee_script,
                'inputs': selected,
                'recipients': [
                    {'address': a, 'units': str(v)} for a, v in sorted(recipients.items())
                ],
                'fee_units': str(self.fee),
            }
            value = {
                'created': now,
                'batch': batch['id'],
                'deductions': {a: str(v) for a, v in sorted(amounts.items())},
                'wire': wire,
                'state': 'reserved',
            }
    ...
    def sign(self, identity):
        with self.pool.lock, self.lock:
            ...
            if 'signed_hex' in intent:
                return intent['txid']
            operator = getattr(self.pool, 'operator', None)
            require(
                not operator or not operator.faulted, 'operator journal failed; signing stopped'
            )
            require(
                not operator or not operator.settings['payments_paused'],
                'new payment signatures are paused',
            )
            ...
            for recipient in intent['wire']['recipients']:
                if len(recipient['address']) > 50:
                    ...
                    require(
                        ...
                        and active.get('isvalid') is True
                        and active.get('destination_type') == 'sha384-v1'
                        and active.get('active_for_next_block') is True,
                        'SHA-384 payout destination is not active',
                    )
            coins = {}
            for role, address in [('pool', self.pool.pool_address), ('fees', self.fee_address)]:
                for coin in self.node.call('listunspent', address):
                    coins[(role, coin['txid'], str(coin['vout']))] = coin
            for coin in intent['wire']['inputs']:
                current = coins.get((coin['role'], coin['txid'], coin['vout']))
                require(
                    current and current['value_units'] == units(coin['units']),
                    'reserved input no longer spendable',
                )
            response = subprocess.run(
                [self.binary, self.pool_seed, self.fee_seed],
                input=encode(intent['wire']) + b'\n',
                capture_output=True,
                timeout=60,
                check=False,
            )
            ...
            require(
                hashlib.sha256(hashlib.sha256(raw).digest()).hexdigest() == txid,
                'signed transaction identity',
            )
            self.record(
                'payment_signed',
                {'id': identity, 'txid': txid, 'signed_hex': fields[1], 'state': 'signed'},
            )
            return txid
    ...
    def reconcile(self, identity, broadcast=True):
        with self.lock:
            intent = self.intents[identity]
            if 'signed_hex' not in intent:
                return 'reserved'
            if intent['state'] == 'confirmed' and intent.get('block_hash'):
                height = self.node.call('getblockcount')
                if (
                    height >= intent['block_height']
                    and self.node.call('getblockhash', str(intent['block_height']))
                    == intent['block_hash']
                ):
                    return 'confirmed'
                self.record(
                    'payment_state', {'id': identity, 'state': 'signed', 'block_hash': None}
                )
            try:
                tx = (
                    self.node.call('gettransaction', intent['txid'], intent['block_hash'])
                    if intent.get('block_hash')
                    else self.node.call('getrawtransaction', intent['txid'])
                )
                confirmed = (
                    tx.get('confirmations', 0) > 0
                    and tx.get('block_hash')
                    and self.node.call('getblockhash', str(tx['block_height'])) == tx['block_hash']
                )
            except (Refused, Busy):
                confirmed = False
            if confirmed:
            ...
            operator = getattr(self.pool, 'operator', None)
            if not broadcast or (
                operator and (operator.faulted or operator.settings['payments_paused'])
            ):
                return 'signed'
            try:
