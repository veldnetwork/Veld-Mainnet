...
if os.name!='posix':raise unittest.SkipTest('Unix IPC service')
from pool.service import Server

class Capacity(unittest.TestCase):
    def test_short_saturation_drains_without_dropping_next_request(self):
        with tempfile.TemporaryDirectory() as d:
            release=threading.Event();full=threading.Event();lock=threading.Lock();active=[0];peak=[0]
            def health():
                with lock:
            ...
            path=str(Path(d)/'ipc');server=Server(path,SimpleNamespace(health=health))
            t=threading.Thread(target=server.serve_forever,kwargs={'poll_interval':.02});t.start()
            def request():
    ...
    def test_sustained_saturation_is_bounded_and_does_not_spawn_waiters(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            try:
                ...
                self.assertGreaterEqual(elapsed,.2);self.assertLess(elapsed,1)
