...
class CoreTests(unittest.TestCase):
    ...
    def test_restore_rejects_oversized_line_with_bounded_read(self):
        with tempfile.TemporaryDirectory() as directory:
            root=Path(directory);data=root/'data';anchor=root/'anchor/current.json'
            Journal(data,anchor).close()
            (data/'events.jsonl').write_bytes(b'x'*2048)
            with patch('pool.journal.MAX_EVENT',1024):
                with self.assertRaisesRegex(Refused,'damaged journal'):Journal(data,anchor)

    def test_restore_rejects_bad_or_unbounded_anchor(self):
        with tempfile.TemporaryDirectory() as directory:
            root=Path(directory);data=root/'data';anchor=root/'anchor/current.json'
            Journal(data,anchor).close()
            for value in ({'seq':True,'digest':'0'*64},{'seq':0,'digest':'bad'},
                          {'seq':0,'digest':'0'*64,'extra':0}):
                anchor.write_text(json.dumps(value),encoding='utf8')
                with self.assertRaises(Refused):Journal(data,anchor)
            anchor.write_bytes(b'x'*257)
            with self.assertRaisesRegex(Refused,'message limit'):Journal(data,anchor)
    ...
    def test_integer_balances_match_exact_reference_at_ambiguous_boundaries(self):
        ...
        for _ in range(1000):
            values=[Fraction(rng.randrange(1<<60),rng.randrange(1,1<<80)) for _ in range(rng.randrange(1,100))]
    ...
    def test_fractional_allocations_conserve_every_base_unit(self):
        ...
        for _ in range(1000):
            ...
            for account,value in amounts.items():
                self.assertEqual(value.numerator*1000000*total,
    ...
    def test_pplns_winner_once_cutoff_difficulty_boundary(self):
        target=f'{1<<248:064x}'
        easier=f'{1<<249:064x}'
        rows=[{'seq':i,'status':'verified','target':t,'account':a,'height':100+i}
              for i,t,a in [(1,target,'old'),(2,easier,'b'),(3,target,'winner'),(4,target,'late')]]
        result=pplns(rows,3,target)
        self.assertEqual(result,{'winner':Fraction(256),'b':Fraction(128),'old':Fraction(128)})
    ...
    def test_co_mining_window_is_separate_from_payment_membership(self):
        target=f'{1<<248:064x}'
        rows=[{'seq':i,'height':h,'status':'verified','target':target,'account':a}
              for i,h,a in [(1,99,'earlier'),(2,100,'a'),(3,199,'b'),(4,199,'late'),(5,200,'next')]]
        self.assertEqual(window_weights(rows,100,199,3),{'a':Fraction(256),'b':Fraction(256)})

    def test_income_without_beneficiaries_is_not_assigned(self):
        with self.assertRaises(Refused):allocate(1,{})
        self.assertEqual(allocate(0,{}),({},0))

    def test_journal_rebuilds_stale_database_without_reissuing_lease(self):
        with tempfile.TemporaryDirectory() as directory:
            root=Path(directory); anchor=root/'witness/current.json'; data=root/'data'
            journal=Journal(data,anchor)
            journal.append('lease',{'template':'x','end':'100'})
            journal.close()
            shutil.copyfile(data/'index.sqlite',root/'backup.sqlite')
            journal=Journal(data,anchor);journal.append('lease',{'template':'x','end':'200'});journal.close()
    ...
    def test_journal_rollback_is_refused_against_surviving_anchor(self):
        with tempfile.TemporaryDirectory() as directory:
            root=Path(directory); anchor=root/'witness/current.json'; data=root/'data'
            journal=Journal(data,anchor);journal.append('lease',{'end':'100'});journal.close()
            old=(data/'events.jsonl').read_bytes()
            journal=Journal(data,anchor);journal.append('lease',{'end':'200'});journal.close()
    ...
    def test_database_crash_after_durable_event_replays(self):
        with tempfile.TemporaryDirectory() as directory:
            root=Path(directory);data=root/'data';anchor=root/'witness/current.json'
            journal=Journal(data,anchor)
            original=journal._index
            def crash(*args):raise OSError('injected index crash')
            journal._index=crash
            with self.assertRaises(OSError):journal.append('intent',{'id':'same-payment'})
    ...
    def test_partial_event_refuses_instead_of_recycling_work(self):
        with tempfile.TemporaryDirectory() as directory:
            root=Path(directory);data=root/'data';anchor=root/'witness/current.json'
            journal=Journal(data,anchor);journal.close()
            with (data/'events.jsonl').open('ab') as file:file.write(b'{"seq":1')
            with self.assertRaisesRegex(Refused,'damaged'):Journal(data,anchor)
            gc.collect()
