...
class AdmissionTests(unittest.TestCase):
    def test_wide_address_requires_explicit_active_version_and_object_response(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            node = SimpleNamespace(
                check_chain=lambda: None, call=lambda *_: response[0], genesis='a' * 64
            )
            ...
            for value in [
                ...
                True,
                {'isvalid': True},
                {'isvalid': True, 'destination_type': 'sha384-v2', 'active_for_next_block': True},
    ...
    def test_worker_token_cannot_read_private_balance_or_payment_history(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            journal = Journal(root / 'events', root / 'anchor/current.json')
            node = SimpleNamespace(
                check_chain=lambda: None, call=lambda *_: {'isvalid': True}, genesis='a' * 64
            )
            pool = Coordinator(node, journal, None, 'pool', 'f' * 64)
            account = pool.register('x' * 30)
            with self.assertRaises(Refused):
                pool.account(account['account'], account['worker_token'])
            with self.assertRaises(Refused):
                pool.history(account['account'], account['worker_token'])
            self.assertEqual(
                pool.history(account['account'], account['view_token']),
                {'payments': [], 'next_before': 0},
            )
            health = pool.health()
        ...
        class Node:
            ...
            def call(self, method, *args):
                if method == 'validateaddress':
                    return {'isvalid': True}
                if method in ('getbestblockhash', 'getblockhash'):
                    return 'a' * 64
                raise AssertionError(method)

        verifier = SimpleNamespace(
            inspect=lambda *_: ('0' * 63 + '8', template_identity(bytes(88)), 'near_miss')
        )
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            journal = Journal(root / 'events', root / 'anchor/current.json')
            ...
            result = pool.submit(
                account['account'], account['worker_token'], work['lease'], '0000000000000000'
            )
    ...
    def test_failed_receipt_commit_releases_verification_slot(self):
        ...
        pool.admission = threading.BoundedSemaphore(1)
        pool.inflight = set()
        pool.receipts = {}
        pool.queue_capacity = 1
        pool.unverified_count = 0
        token = 'a' * 64
        ...
        with self.assertRaises(OSError):
            pool.submit('a', token, 'l', '0000000000000000')
    ...
    def test_disconnected_worker_receipt_is_retried_without_becoming_false_invalid(self):
        class Node:
            ...
            def call(self, method, *args):
                ...
                if method == 'getbestblockhash':
                    return ('a' if self.height == 0 else 'b') * 64
        ...
        class Verifier:
            ...
            def inspect(self, *_):
                self.calls += 1
                if self.calls == 1:
                    raise Busy('injected temporary resource failure')
    ...
    def test_durable_deferred_backlog_is_bounded_and_recovers_after_restart(self):
        ...
        class Verifier:
            ...
            def inspect(self, *_):
                if self.busy:
                    raise Busy('injected unavailable native verifier')
                return '0' * 63 + '2', template_identity(bytes(88)), 'none'
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            account = pool.register('x' * 30)
            work = pool.work(account['account'], account['worker_token'], 3)
            for n in range(2):
                with self.assertRaises(Busy):
                    pool.submit(
                        account['account'], account['worker_token'], work['lease'], f'{n:016x}'
                    )
            ...
            for _ in range(20):
                with self.assertRaisesRegex(Busy, 'backlog full'):
                    pool.submit(
                        account['account'],
                        account['worker_token'],
                        work['lease'],
                        '0000000000000002',
                    )
            self.assertEqual(journal.sequence, sequence)
            self.assertEqual(len(pool.receipts), 2)
            journal.close()
            journal = Journal(root / 'events', root / 'anchor/current.json')
            pool = Coordinator(Node(), journal, verifier, 'pool', 'f' * 64, capacity=2)
            ...
            self.assertEqual(
                pool.submit(
                    account['account'], account['worker_token'], work['lease'], '0000000000000002'
                ),
                {'status': 'verified'},
            )
            self.assertEqual(pool.unverified_count, 1)
            pool.retry_deferred()
            self.assertEqual(pool.unverified_count, 0)
            self.assertEqual(pool.verified_counts[account['account']], 3)
            journal.close()
    ...
    def test_accepted_block_is_recovered_after_crash_before_earned_record(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            node = SimpleNamespace(
                ...
                call=lambda method, *_: 10 if method == 'getblockcount' else 'b' * 64,
            )
            pool = Coordinator(node, journal, None, 'pool', 'f' * 64)
            pool.reconcile_solutions()
            pool.reconcile_solutions()
    ...
    def test_deferred_solution_renews_exact_candidate_and_keeps_nonce(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            def rpc(method, *args):
                return height[0] if method == 'getblockcount' else 'b' * 64

            def renew(template, address):
                calls.append(('renew', dict(template), address))
                return dict(template, work_token='fresh')

            def submit(template, nonce):
                calls.append(('submit', dict(template), nonce))
                height[0] = 10
            ...
            pool.record(
                'job',
                {'id': 'j', 'height': 10, 'node': {'work_token': 'expired', 'block_hex': '00'}},
            )
            pool.record(
                'solution',
                {
            ...
            self.assertEqual(len(pool.earnings), 1)
            journal.close()
