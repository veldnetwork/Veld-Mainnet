...
def pplns(receipts, cutoff, network_target, lookback=2, ordered=False):
    ...
    for receipt in (receipts if ordered else sorted(receipts, key=lambda value: value['seq'], reverse=True)):
...
def floor_total(values):
    ...
    lower=upper=0;inexact=False;scale=1<<256
    for denominator,numerator in groups.items():
        quotient,remainder=divmod(numerator*scale,denominator)
        lower+=quotient;upper+=quotient+bool(remainder);inexact|=bool(remainder)
    minimum=lower//scale
