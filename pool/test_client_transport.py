...
def main():
    ...
    parser.add_argument('--openssl',default='openssl');args=parser.parse_args()
    with tempfile.TemporaryDirectory() as temporary:
        ...
        for name in ('trusted','other','wrong-host'):
            ...
            subprocess.run([args.openssl,'req','-x509','-newkey','rsa:2048','-nodes','-days','1',
                '-keyout',str(root/(name+'.key')),'-out',str(root/(name+'.crt')),'-subj','/CN='+subject,
                '-addext','subjectAltName=DNS:'+subject+',IP:127.0.0.1'],capture_output=True,check=True)
        ...
        cases=[
            ...
            ('duplicate content length','trusted','trusted',normal.replace(b'Content-Length:',b'Content-Length: 22\r\nContent-Length:'),False,True),
            ('transfer encoding','trusted','trusted',normal.replace(b'Content-Type:',b'Transfer-Encoding: chunked\r\nContent-Type:'),False,True),
            ('oversized advertised body','trusted','trusted',normal.replace(str(len(body)).encode()+b'\r\n\r\n',b'99999999\r\n\r\n'),False,True),
                ...
                try:
                    ...
                    with context.wrap_socket(connection,server_side=True) as tls:
                        ...
                        while b'\r\n\r\n' not in request:
                            ...
                            if len(request)>16384:raise AssertionError('request framing')
                        requests.append(request);tls.sendall(response)
                except ssl.SSLError:pass # Expected on certificate rejection.
                except Exception as error:failures.append(repr(error))
                finally:listener.close()
            thread=threading.Thread(target=serve);thread.start()
            host='localhost' if certificate=='wrong-host' or name.startswith('localhost') else '127.0.0.1'
            try:
                result=subprocess.run([args.binary,'https://'+host+':'+str(listener.getsockname()[1]),str(root/(ca+'.crt'))],capture_output=True,text=True,timeout=25)
            except subprocess.TimeoutExpired as error:
                print('TIMEOUT',name,'requests',len(requests),'server failures',failures,'output',error.stdout,'errors',error.stderr,flush=True)
                raise
