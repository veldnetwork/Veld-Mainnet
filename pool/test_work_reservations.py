...
class Reservations(unittest.TestCase):
    def setUp(self):
        ...
        self.a = self.pool.register('x' * 30)
        self.b = self.pool.register('y' * 30)
    ...
    def test_every_returned_range_is_exactly_durable_and_disjoint(self):
        with ThreadPoolExecutor(max_workers=12) as ex:
            work = list(
                ex.map(
                    lambda i: self.work(self.a if i % 2 else self.b, 512 if i % 3 else 1024),
                    range(96),
                )
            )
        records = {
            e['payload']['id']: e['payload'] for e in self.j.events() if e['kind'] == 'lease'
        }
        intervals = []
        for w in work:
            p = records[w['lease']]
            start = int(w['start'], 16)
            end = start + int(w['count'])
            self.assertEqual((int(p['start']), int(p['end'])), (start, end))
            intervals.append((start, end))
        intervals.sort()
        self.assertTrue(all(a[1] <= b[0] for a, b in zip(intervals, intervals[1:])))
        marker = json.loads(self.j.anchor.read_text())
        self.assertEqual(marker['seq'], self.j.sequence)

    def test_cached_delivery_does_not_write_and_authentication_is_repeated(self):
        self.work()
        seq = self.j.sequence
        with patch.object(
            self.j, 'append_many', side_effect=AssertionError('unexpected durable write')
        ):
            self.work()
            self.assertEqual(self.j.sequence, seq)
            with self.assertRaises(Refused):
                self.pool.work(self.a['account'], self.b['worker_token'], 1024)
    ...
    def test_count_change_keeps_exact_nonoverlapping_reservations(self):
        a = self.work(count=1)
        b = self.work(count=4096)
        c = self.work(count=1)
        for w in (a, b, c):
            self.assertEqual(
                int(self.pool.leases[w['lease']]['end']) - int(w['start'], 16), int(w['count'])
            )
    ...
    def test_template_alias_and_restart_abandon_unused_ranges(self):
        one = self.work()
        first_end = int(self.pool.highwater[self.pool.jobs[self.pool.active[0]]['template']]['end'])
        ...
        self.pool = Coordinator(Node(), self.j, None, 'pool', 'f' * 64)
        three = self.work()
        self.assertGreaterEqual(int(three['start'], 16), high)
    ...
    def test_nonce_end_boundary_reserves_only_remaining_valid_ranges(self):
        self.work()
        job = self.pool.jobs[self.pool.active[0]]
        self.pool.lease_reserve.clear()
        self.pool.record(
            'lease',
            {
                'id': 'end-fixture',
        ...
        w = self.work(count=2)
        self.assertEqual(int(w['start'], 16), (1 << 64) - 3)
        with self.assertRaisesRegex(Refused, 'nonce space'):
            self.work(count=2)
        w = self.work(count=1)
        self.assertEqual(int(w['start'], 16), (1 << 64) - 1)

    def test_reserve_memory_is_bounded_and_eviction_never_reissues(self):
        for count in range(1, 81):
            self.work(count=count)
        self.assertLessEqual(len(self.pool.lease_reserve), 64)
        high = int(self.pool.highwater[self.pool.jobs[self.pool.active[0]]['template']]['end'])
        w = self.work(count=1)
        self.assertGreaterEqual(int(w['start'], 16), high)
    ...
    def test_group_is_old_format_and_uses_one_durability_barrier_set(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            try:
                ...
                self.assertEqual(seq, list(range(1, 9)))
                self.assertEqual(spy.call_count, 3 if os.name == 'posix' else 2)
                self.assertEqual([e['seq'] for e in j.events()], seq)
                self.assertEqual(json.loads(j.anchor.read_text())['seq'], 8)
            finally:
                j.close()
            j = Journal(root / 'data', root / 'anchor/current.json')
            self.assertEqual(j.sequence, 8)
            j.close()
        ...
        for fail_at in range(1, 4 if os.name == 'posix' else 3):
            with self.subTest(fail_at=fail_at), tempfile.TemporaryDirectory() as d:
                ...
                with patch('pool.journal.os.fsync', side_effect=fail):
                    with self.assertRaises(OSError):
                        j.append_many([('lease', {'end': str(i)}) for i in range(8)])
                self.assertIsNone(j.db)
                self.assertIsNone(j.log)
                with self.assertRaises(RuntimeError):
                    j.append('lease', {'end': '0'})
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            try:
                with self.assertRaises(ValueError):
                    j.append_many([('lease', {}), ('lease', {'bad': float('nan')})])
                self.assertEqual(j.sequence, 0)
                self.assertEqual((root / 'data/events.jsonl').read_bytes(), b'')
                with self.assertRaises(Refused):
                    j.append_many([('lease', {})] * 9)
            finally:
                j.close()
