...
class MaintenanceSupervision(unittest.TestCase):
    def test_complete_entry_stops_serving_and_closes_resources_on_unexpected_failure(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            p = Path(d)
            (p / 'ipc').mkdir(mode=0o700)
            cfg = {
                'rpc_url': 'unused',
                'rpc_token_file': 'unused',
                'genesis': 'a' * 64,
                'state_directory': str(p / 'data'),
                'rollback_anchor': str(p / 'anchor/current'),
                'native_binary': 'NO_PROCESS_ALLOWED',
                'pool_address': 'pool',
                'accounting_target': 'f' * 64,
                'socket': str(p / 'ipc/coordinator.sock'),
            }
            ...
            node = SimpleNamespace(
                ...
                call=lambda *a: {'window_blocks': 3, 'yield_blocks': 3},
            )
            ...
            try:
                with (
                    ...
                    patch.object(service, 'read_private', return_value=json.dumps(cfg).encode()),
                    patch.object(service, 'Node', return_value=node),
                    patch.object(service, 'Native', return_value=native),
                    patch.object(service, 'Journal', side_effect=journal),
                    patch.object(
                        service,
                        'maintain_once',
                        side_effect=sqlite3.OperationalError('injected SQLite fault'),
                    ),
                ):
            ...
            self.assertEqual(closed, ['native'])
            self.assertTrue(journals)
            self.assertTrue(all(j.db is None and j.log is None for j in journals))
            self.assertFalse(Path(cfg['socket']).exists())
            j = original(p / 'data', p / 'anchor/current')
