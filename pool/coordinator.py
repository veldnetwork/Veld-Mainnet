...
class Coordinator:
    def __init__(self, node, journal, verifier, pool_address, accounting_target, capacity=16):
        ...
        self.jobs = Records(journal, 'jobs')
        self.leases = Records(journal, 'leases')
        self.highwater = Records(journal, 'nonce_highwater')
        self.receipts = Records(journal, 'receipts')
        ...
        self.public_income = IncomeOverview()
    ...
    def apply(self, kind, value, seq):
        if kind == 'account':
            ...
            job['node'] = value['node']
            self.jobs[value['id']] = job
        elif kind == 'lease':
            self.leases[value['id']] = value
            end = max(
                int(self.highwater.get(value['template'], {'end': '0'})['end']), int(value['end'])
            )
            ...
            self.unverified_count += int(value['status'] in waiting) - int(
                receipt['status'] in waiting
            )
            self.verified_counts[account] = self.verified_counts.get(account, 0) + change
            if change == 1 and not self.replaying:
                self.public_work.add(receipt['target'])
            ...
            if solution is not None:
                solution['status'] = 'accounted'
                self.solutions[value['block']] = solution
        elif kind == 'solution':
            self.solutions[value['block']] = value
        elif kind == 'income':
            ...
            if value.get('category') == 'mining' and value['id'] in self.earnings:
                earned = self.earnings[value['id']]
                earned['status'] = 'accounted'
                self.earnings[value['id']] = earned
        elif kind == 'income_state':
            income = self.incomes[value['id']]
            old = dict(income)
            income['state'] = value['state']
            if value['state'] == 'available':
                income['available_once'] = True
    ...
    def unresolved(self, cutoff):
        return any(
            next(self.receipts.select(status=status, cutoff=cutoff), None) is not None
    ...
    def register(self, address):
        ...
        require(
            not self.payments or address != self.payments.fee_address,
            'operator fee identity cannot be a worker payout address',
        )
        ...
        require(
            isinstance(validation, dict) and validation.get('isvalid') is True,
            'invalid payout address',
        )
        if len(address) > 50:
            require(
                validation.get('destination_type') == 'sha384-v1'
                and validation.get('active_for_next_block') is True,
        ...
        with self.lock:
            ...
            value = {
                ...
                'address': address,
                'worker_hash': sha(worker.encode()),
                'view_hash': sha(view.encode()),
            }
    ...
    def authenticate(self, account, credential, role='worker'):
        require(
            isinstance(account, str) and isinstance(credential, str) and len(credential) == 64,
            'account authorization',
        )
        value = self.accounts.get(account)
        require(
            value is not None
            and hmac.compare_digest(value[role + '_hash'], sha(credential.encode())),
            'account authorization',
        )
        return value
    ...
    def work(self, account, credential, count=32):
        ...
        with self.lock:
            ...
            if self.active is None or self.active[1] < time.monotonic() + 2:
                ...
                if self.rewards:
                    if (
                        template['height'] != seal_height
                        or template['prev_block_hash'] != seal_parent
                    ):
                ...
                policy = (
                    self.operator.policy() if self.operator else {'fee_ppm': '0', 'revision': '0'}
                )
                job = {
                    'id': secrets.token_hex(16),
                    'template': identity,
                    ...
                    'node': template,
                    'policy': 'pplns-2-blocks-fee-0-v1'
                    if policy['fee_ppm'] == '0'
                    else 'pplns-share-policy-v2',
                    'fee_ppm': policy['fee_ppm'],
                    'operator_revision': policy['revision'],
                }
                self.record('job', job)
            ...
            if not reserve:
                ...
                reserve = [
                    {
                        ...
                        'start': str(start + i * count),
                        'end': str(start + (i + 1) * count),
                    }
                ...
                try:
                    for lease, sequence in zip(reserve, sequences):
                        self.apply('lease', lease, sequence)
                except BaseException:
                    self.journal.close()
                    raise
            lease = reserve.pop(0)
            ...
            if reserve:
                ...
                if len(self.lease_reserve) > 64:
                    self.lease_reserve.popitem(last=False)
            # An expensive durable write must not publish already expired work.
            if self.active[1] <= time.monotonic():
                raise Busy('work expired during durable reservation; retry')
            ...
            return {
                ...
                'header': job['node']['block_hex'][:176],
                'target': job['target'],
                'fee_ppm': job.get('fee_ppm', '0'),
                'policy_revision': job.get('operator_revision', '0'),
                'start': f'{start:016x}',
                'count': str(count),
                'ttl_ms': str(max(0, int((self.active[1] - time.monotonic()) * 1000))),
            }
        ...
        with self.lock:
            ...
            for income in self.incomes.values():
                ...
                if income['state'] == 'pending':
                    pending.append(credit)
                elif income['state'] == 'available':
            ...
            payment = (
                self.payments.summary(account)
                if self.payments
                else {'reserved_units': '0', 'paid_units': '0'}
            )
            pending_units = floor_total(pending)
            available_units = (
                floor_total(available) - int(payment['reserved_units']) - int(payment['paid_units'])
            )
            return {
                'account': account,
                ...
                'lottery_status': self.identity.status()
                if self.identity
                else 'Co-mining disabled; operator funding required.',
                'reward_windows': self.rewards.status() if self.rewards else {},
                'reward_totals': {
                    key: str(floor_total(value)) for key, value in categories.items()
                },
                **payment,
                'payment_status': (
                    ...
                    if self.operator and self.operator.settings['payments_paused']
                    else 'enabled'
                    if self.payments
                    else 'not enabled'
                ),
    ...
    def health(self):
        with self.lock:
            ...
            self.active_accounts = {
                key: seen for key, seen in self.active_accounts.items() if now - seen < 120
            }
            tip = getattr(self, 'income_tip', None)
            return {
                'version': VERSION,
                'status': 'Service responding'
                if time.monotonic() - self.last_healthy < 15
                else 'Reconciliation paused or starting',
                'test_candidate': True,
                'chain': self.node.genesis,
                ...
                'payments_enabled': self.payments is not None
                and not (self.operator and self.operator.settings['payments_paused']),
                'co_mining_enabled': self.identity is not None
                and not (self.operator and self.operator.settings['comining_paused']),
                'payment_policy': self.operator.policy()
                if self.operator
                else {
                ...
                'verification_queue': {
                    'waiting': self.unverified_count,
                    'capacity': self.queue_capacity,
                },
                'overview': {
                    'version': 1,
                    **self.public_income.snapshot(tip[0] if tip else 0),
                    'work': self.public_work.snapshot(),
                    'payments': self.payments.public_summary()
                    if self.payments
                    else {'paid_units': '0', 'confirmed_transactions': '0'},
                },
    ...
    def history(self, account, credential, before=0):
        ...
        with self.lock:
            return (
                self.payments.history(account, before)
                if self.payments
                else {'payments': [], 'next_before': 0}
            )
    ...
    def reconcile_income(self):
        with self.lock:
            ...
            extends = (
                last is not None
                and height >= last[0]
                and self.node.call('getblockhash', str(last[0])) == last[1]
            )
            for earned in self.earnings.select(status=''):
                block = earned['block']
                if extends and block in self.incomes and earned['height'] <= last[0]:
                    continue
                canonical = (
                    height >= earned['height']
                    and self.node.call('getblockhash', str(earned['height'])) == block
                )
                ...
                if self.unresolved(earned['cutoff']):
                    continue
                info = self.node.call('getblock', block)
                tx = self.node.call('gettransaction', info['tx'][0], block)
                require(
                    tx['coinbase'] is True and tx['block_hash'] == block, 'canonical miner receipt'
                )
                ...
                for receipt in expected:
                    ...
                    require(
                        len(matching) == 1
                        and matching[0]['address'] == self.pool_address
                        and matching[0]['value_units'] == receipt['value_units']
                        and matching[0]['script_pubkey'] == receipt['script_pubkey'],
                        'miner category receipt mismatch',
                    )
                    outputs.extend(matching)
                amount = sum(units(o['value_units']) for o in outputs)
                weights = pplns(
                    self.receipts.select(
                        status='verified', cutoff=earned['cutoff'], reverse=True, sequence=True
                    ),
                    earned['cutoff'],
                    self.jobs[earned['job']]['node']['target'],
                    ordered=True,
                    fee_policy=True,
                )
                ...
                self.record(
                    'income',
                    {
                        ...
                        'amount': str(amount),
                        'category': 'mining',
                        'policy': 'pplns-share-policy-v2',
                        'cutoff': earned['cutoff'],
            ...
            spendable = {
                (u['txid'], u['vout']): u for u in self.node.call('listunspent', self.pool_address)
            }
            ...
            def candidates():
                if not extends:
                    yield from self.incomes.values()
                    return
            ...
            for income in candidates():
                canonical = (
                    extends and income['height'] <= last[0] and income['state'] != 'orphaned'
                ) or (
                    height >= income['height']
                    and self.node.call('getblockhash', str(income['height']))
                    == income.get('block', income['id'])
                )
                if not canonical:
                    if income['state'] != 'orphaned':
                        self.record('income_state', {'id': income['id'], 'state': 'orphaned'})
                    continue
                if income['state'] == 'orphaned':
                    self.record('income_state', {'id': income['id'], 'state': 'pending'})
                    income['state'] = 'pending'
                if income['state'] != 'pending' or height - income['height'] + 1 < 120:
                    continue
                if income.get('available_once') or all(
                    (income['txid'], o['vout']) in spendable
                    and spendable[income['txid'], o['vout']]['value_units'] == units(o['units'])
                    for o in income['outputs']
                ):
                    self.record('income_state', {'id': income['id'], 'state': 'available'})
            # Never cache a prefix if the chain changed during reconciliation.
            require(
                self.node.call('getblockhash', str(height)) == tip,
                'chain changed during income reconciliation',
            )
    ...
    def reconcile_solutions(self, submit=True):
        with self.lock:
            ...
            for solution in self.solutions.select(status=''):
                if solution['block'] in self.earnings:
                    continue
    ...
    def retry_deferred(self):
        ...
        with self.lock:
            pending = next(
                (
                    r
                    for r in self.receipts.select(status='deferred', sequence=True)
                    if r['status'] == 'deferred' and r['identity'] not in self.inflight
                ),
                None,
            )
            if pending is None or not self.admission.acquire(blocking=False):
                return
    ...
    def submit(self, account, credential, lease_id, encoded_nonce):
        ...
        with self.lock:
            ...
            admitted_parent = (
                self.node.call('getbestblockhash') if not prior else prior.get('admitted_parent')
            )
            ...
            if not self.admission.acquire(blocking=False):
                raise Busy('verification queue full; retry same proof')
            self.inflight.add(identity)
            try:
                if not prior:
                    self.record(
    ...
    def _verify(self, identity, job, number):
        try:
            raw = bytearray.fromhex(job['node']['block_hex'])
            raw[80:88] = number.to_bytes(8, 'little')
            try:
                proof, native_identity, proof_kind = self.verifier.inspect(
                    bytes(raw[:88]), job['height']
                )
            except Busy:
                with self.lock:
            ...
            if int(proof, 16) >= int(job['target'], 16) and proof_kind != 'near_miss':
                status = 'invalid'
            elif (
                self.receipts[identity].get('admitted_parent') != job['node']['prev_block_hash']
                or self.node.call('getblockhash', str(job['height'] - 1))
                != job['node']['prev_block_hash']
            ):
                status = 'stale'
            else:
                status = 'verified' if int(proof, 16) < int(job['target'], 16) else 'near_miss'
            with self.lock:
                self.record(
                    'verification',
                    {
            ...
            if status == 'verified' and int(proof, 16) < int(job['node']['target'], 16):
                ...
                try:
                    ...
                    accepted = (
                        self.node.call('getblockcount') >= job['height']
                        and self.node.call('getblockhash', str(job['height'])) == block_hash
                    )
                if accepted:
                    with self.lock:
