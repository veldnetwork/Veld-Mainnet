...
from pool.qualification.isolation import (
    MARKER,
...
class IsolationTests(unittest.TestCase):
    ...
    def check(self, parent, current, links, routes=''):
        ...
        with (
            patch.dict(os.environ, {MARKER: parent}),
            patch('os.readlink', return_value=current),
    ...
    def test_same_or_missing_namespace_is_refused(self):
        for parent in ('', 'net:[2]'):
            with self.assertRaises(RuntimeError):
                self.check(parent, 'net:[2]', '[]')

    def test_external_interface_is_refused(self):
        with self.assertRaises(RuntimeError):
            self.check('net:[1]', 'net:[2]', '[{"ifname":"lo"},{"ifname":"eth0"}]')

    def test_non_loopback_route_is_refused(self):
        with self.assertRaises(RuntimeError):
            self.check('net:[1]', 'net:[2]', '[{"ifname":"lo"}]', 'default via 10.0.0.1 dev eth0\n')
