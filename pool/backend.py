...
_TEMPLATE_REASONS = frozenset(
    (
        ...
        'independent_validation_incomplete',
        'sync_incomplete',
        'snapshot_state_untrusted',
        'durable_state_unproven',
        'datadir_identity_unproven',
        'checkpoint_anchor_unproven',
        'tip_unknown',
        'runtime_closed',
        'peer_view_unsafe',
...
class NodeRefused(Refused):
    def __init__(self, method, error):
        ...
        for prefix, stage in (
            ('getblocktemplate refused: ', 'initial'),
            ...
            if error['code'] == -32010 and message.startswith(prefix):
                ...
                if reason in _TEMPLATE_REASONS:
                    self.diagnostic = 'template_' + stage + '_' + reason
                return
        if (
            error['code'] == -32603
            and message == 'getblocktemplate canonical builder binding or preflight failed'
        ):
...
class Node:
    ...
    def call(self, method, *parameters):
        ...
        require(
            ...
            'RPC credential unavailable',
        )
        with self.lock:
            self.counter += 1
            identity = self.counter
        request = encode(
            {'jsonrpc': '2.0', 'id': identity, 'method': method, 'params': list(parameters)}
        )
        ...
        try:
            ...
            if result.get('error') is not None:
                ...
                if (
                    ...
                    and error['code'] == -32010
                    and error['message'] in WORK_READINESS_REFUSALS
                ):
                ...
                if error['code'] == -32005 or (
                    error['code'] == -32010
                    and (
                        'local-work-unavailable' in error['message']
                        or 'capacity' in error['message']
                    )
                ):
    ...
    def require_transaction_index(self):
        ...
        require(
            isinstance(info, dict) and info.get('txindex_enabled') is True,
    ...
    def template(self, address, identity=None):
        self.check_chain()
        result = (
            self.call('getblocktemplate', address, hex64(identity))
            if identity is not None
            else self.call('getblocktemplate', address)
        )
        ...
        require(
            isinstance(encoded, str) and 184 <= len(encoded) <= 2 * MAX_BLOCK_BYTES,
            'template encoding',
        )
        try:
        ...
        hex64(result['target'])
        hex64(result['prev_block_hash'])
        require(type(result['height']) is int and result['height'] > 0, 'template height')
        require(
            type(result['work_ttl_ms']) is int and 0 < result['work_ttl_ms'] <= 10000,
            'template deadline',
        )
        require(
            result.get('reserved_miner_destination') is False,
            'protocol destination is not a pool identity',
        )
        require(
            isinstance(result.get('miner_receipts'), list) and len(result['miner_receipts']) <= 1,
            'canonical miner category',
        )
        return result
    ...
    def renew(self, template, address):
        ...
        header = bytes.fromhex(template['block_hex'][:176])
        require(header[80:88] == bytes(8), 'unsigned template nonce')
        identity = hashlib.sha256(hashlib.sha256(header).digest()).hexdigest()
        renewed = self.template(address, identity)
        require(
            renewed['block_hex'] == template['block_hex']
            and renewed['target'] == template['target']
            and renewed['height'] == template['height'],
            'renewal must preserve exact candidate',
        )
        return renewed
