...
def health():
    return dict(
        ...
        result=dict(
            active_accounts='2',
            verified_shares='123',
            reconciled_height='8570',
            chain='a' * 64,
            status='Service responding',
            payments_enabled=False,
            co_mining_enabled=False,
            worker_token='secret',
            accounts=['private'],
            private_endpoint='do-not-forward',
        ),
    )
...
class PublicStatusTests(unittest.TestCase):
    def test_strict_projection(self):
        ...
        for private in ('secret', 'private', 'worker_token'):
    ...
    def test_tls_endpoint_and_private_cors_boundary(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            bt = threading.Thread(target=backend)
            bt.start()
            ...
            def fetch(method, path, body=None):
                c = http.client.HTTPSConnection(
                    '127.0.0.1', server.server_port, context=client_ctx, timeout=5
                )
                c.request(
                    method,
                    path,
                    body,
                    {'Origin': 'https://explorer.veld.network', 'Content-Type': 'application/json'},
                )
                r = c.getresponse()
                result = (r.status, dict(r.getheaders()), json.loads(r.read()))
                c.close()
                return result
            ...
            try:
                ...
                self.assertEqual(
                    headers['Access-Control-Allow-Origin'], 'https://explorer.veld.network'
                )
                self.assertNotIn('Access-Control-Allow-Credentials', headers)
                self.assertEqual(value, public_status(health()))
                self.assertEqual(requests, [dict(action='health', payload={})])
                for path in (
                    '/v1/account',
                    '/v1/history',
                    '/v1/public?account=private',
                    '/admin',
                    '/jsonrpc',
                ):
                    code, headers, _ = fetch('GET', path)
                    self.assertEqual(code, 404)
                    self.assertNotIn('Access-Control-Allow-Origin', headers)
                code, headers, _ = fetch('POST', '/v1/health', '{}')
                self.assertEqual(code, 200)
                self.assertNotIn('Access-Control-Allow-Origin', headers)
