...
if os.name != 'posix':
    raise unittest.SkipTest('Linux coordinator IPC')
                ...
                with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as connection:
                    ...
                    connection.sendall(
                        encode(
                            dict(
                                action='work',
                                payload=dict(account='fixture', token='fixture', count='32'),
                            )
                        )
                        + b'\n'
                    )
    ...
    def test_fixed_diagnostics_never_echo_arbitrary_rpc_values(self):
        for prefix, stage in (
            ...
            for message in (prefix + 'secret-token', prefix + 'tip_unknown\nsecret-token'):
                self.assertEqual(
                    NodeRefused('getblocktemplate', dict(code=-32010, message=message)).diagnostic,
                    'node_rpc_refused',
                )
        self.assertEqual(
            NodeRefused(
                ...
                dict(
                    code=-32603,
                    message='getblocktemplate canonical builder binding or preflight failed',
                ),
            ).diagnostic,
            'template_builder_preflight_failed',
        )
        self.assertEqual(
            NodeRefused(
                'sendrawtransaction',
                dict(code=-32010, message='getblocktemplate refused: tip_unknown'),
            ).diagnostic,
            'node_rpc_refused',
        )
    ...
    def test_real_ipc_keeps_refusal_generic_and_operator_notice_bounded(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            def request(payload):
                with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as connection:
                    connection.connect(server.server_address)
                    connection.sendall(encode(payload) + b'\n')
                    return decode(connection.makefile('rb').readline())

            try:
                with (
                    contextlib.redirect_stderr(output),
                    patch('pool.service.time.monotonic', return_value=100),
                ):
                    for _ in range(5):
                        self.assertEqual(
                            request(
                                dict(
                                    action='work',
                                    payload=dict(
                                        account='private-account', token='private-token', count='32'
                                    ),
                                )
                            ),
                ...
                self.assertEqual(
                    output.getvalue(),
                    'pool request refused action=work reason=template_initial_tip_unknown\n',
                )
                with (
                    contextlib.redirect_stderr(output),
                    patch('pool.service.time.monotonic', return_value=131),
                ):
                ...
                for secret in ('private-account', 'private-token', 'secret-action'):
