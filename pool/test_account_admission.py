...
class AccountAdmissionTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                for index in range(10000):
                    pool.apply('account',{'id':f'{index:032x}','address':'x'*30,
                        'worker_hash':'a'*64,'view_hash':'b'*64},index+1)
    ...
    def test_global_budget_precedes_node_calls_and_persistence_and_refills(self):
        with tempfile.TemporaryDirectory() as directory,patch('pool.coordinator.time.monotonic',return_value=100) as clock:
            journal,pool,node,calls=self.fixture(Path(directory))
            try:
                for _ in range(16):pool.register('x'*30)
                sequence=journal.sequence
                for _ in range(100):
                    with self.assertRaises(Busy):pool.register('x'*30)
    ...
    def test_concurrent_sources_share_one_budget(self):
        with tempfile.TemporaryDirectory() as directory,patch('pool.coordinator.time.monotonic',return_value=100):
            journal,pool,node,calls=self.fixture(Path(directory))
            try:
                def request(index):
                    try:pool.register('x'*29+str(index%10));return True
    ...
    def test_old_credentials_and_liabilities_survive_projection_restore(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            pool.record('income',{'id':'receipt','category':'mining','state':'available',
            ...
            try:
                ...
                for account,amount in ((first,'123456789'),(second,'987654321')):
                    self.assertEqual(pool.authenticate(account['account'],account['worker_token'])['id'],account['account'])
                    self.assertEqual(pool.account(account['account'],account['view_token'])['available_units'],amount)
