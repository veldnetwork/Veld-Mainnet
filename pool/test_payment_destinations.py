...
class DestinationPaymentTests(unittest.TestCase):
    ...
    def test_pc_and_laptop_combined_balance_qualifies_one_wallet(self):
        with tempfile.TemporaryDirectory() as temp:
            node,pool,journal,payment=self.fixture(Path(temp),{'pc':81981122,'laptop':85117389})
            identity=payment.plan(now=100000)
            self.assertIsNotNone(identity)
            intent=payment.intents[identity]
            self.assertEqual(intent['wire']['recipients'],[{'address':'same-wallet','units':'167098511'}])
    ...
    def test_more_than_128_accounts_at_one_destination_still_make_one_output(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            identity=payment.plan(now=100000);self.assertIsNotNone(identity)
            self.assertEqual(payment.intents[identity]['wire']['recipients'],[{'address':'same-wallet','units':'120000000'}])
    ...
    def test_partial_input_batch_keeps_individual_deductions_and_same_destination(self):
        with tempfile.TemporaryDirectory() as temp:
            root=Path(temp);credits={f'worker{i:03}':100000000 for i in range(200)}
            ...
            pool.incomes={str(i):{'state':'available','txid':coin['txid'],'outputs':[{'vout':0}],
                'credits':{f'worker{i:03}':'100000000/1'}} for i,coin in enumerate(node.coins['pool'])}
            for _ in range(6):
                if payment.plan(now=100000) is None:break
            self.assertEqual(len(payment.intents),4)
            self.assertEqual(len(payment.batches),1)
            self.assertEqual(payment.deductions(),credits)
            self.assertTrue(all(len(i['wire']['inputs'])<=64 and len(i['wire']['recipients'])==1 for i in payment.intents.values()))

    def test_restart_never_reissues_the_same_combined_entitlement(self):
        with tempfile.TemporaryDirectory() as temp:
            root=Path(temp);node,pool,journal,payment=self.fixture(root,{'a':80000000,'b':90000000})
            identity=payment.plan(now=100000);self.assertIsNotNone(identity)
            raw=b'synthetic signed fixture';txid=hashlib.sha256(hashlib.sha256(raw).digest()).hexdigest()
            payment.record('payment_signed',{'id':identity,'txid':txid,'signed_hex':raw.hex(),'state':'signed'})
            journal.close();reopened=Journal(root/'payments',root/'anchor/current.json')
            self.addCleanup(reopened.close)
            restored=Payments(pool,reopened,'must-not-run','','','11','22','fees',100000,1000)
    ...
    def test_relayable_change_adjusts_destination_and_preserves_each_remaining_balance(self):
        with tempfile.TemporaryDirectory() as temp:
            node,pool,journal,payment=self.fixture(Path(temp),{'a':99999999,'b':99999999,'c':99999999})
            identity=payment.plan(now=100000);self.assertIsNotNone(identity)
            intent=payment.intents[identity]
            self.assertEqual(sum(int(v) for v in intent['deductions'].values()),299999000)
            self.assertEqual(intent['wire']['recipients'],[{'address':'same-wallet','units':'299999000'}])
            self.assertEqual(sum(payment.available().values()),997)

    def test_existing_daily_batch_and_interval_remain_frozen(self):
        with tempfile.TemporaryDirectory() as temp:
            node,pool,journal,payment=self.fixture(Path(temp),{'a':80000000,'b':90000000})
            payment.record('payment_batch',{'id':'old','created':100000,'quotas':{},'minimum_units':'100000000'})
    ...
    def test_legacy_frozen_quotas_keep_the_old_account_minimum(self):
        with tempfile.TemporaryDirectory() as temp:
            node,pool,journal,payment=self.fixture(Path(temp),{'a':80000000,'b':90000000})
            payment.record('payment_batch',{'id':'legacy','created':100000,
                'quotas':{'a':'80000000','b':'90000000'},'minimum_units':'100000000'})
            self.assertIsNone(payment.plan(now=100001))
            self.assertEqual(payment.intents,{})
            identity=payment.plan(now=186400)
            self.assertEqual(payment.intents[identity]['wire']['recipients'],
    ...
    def test_unknown_frozen_scope_fails_closed(self):
        with tempfile.TemporaryDirectory() as temp:
            node,pool,journal,payment=self.fixture(Path(temp),{'a':80000000,'b':90000000})
            payment.record('payment_batch',{'id':'future','created':100000,
                'quotas':{},'minimum_units':'100000000','threshold_scope':'unknown'})
