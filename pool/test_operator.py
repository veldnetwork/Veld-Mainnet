...
class Node:
    ...
    def __init__(self):
        self.coins = {
            'pool': [dict(txid='b' * 64, vout=0, value_units=300000000, confirmations=200)],
    ...
    def call(self, method, *args):
        ...
        if method == 'getblockcount':
            return 200
        if method in ('getblockhash', 'getbestblockhash'):
            return 'd' * 64
        if method == 'listunspent':
            return self.coins[args[0]]
        if method in ('gettransaction', 'getrawtransaction'):
            if not self.confirmed:
                raise Refused('fixture transaction not found')
...
def fixture(root, max_fee=0):
    node = Node()
    journal = Journal(root / 'work', root / 'anchor/work.json')
    verifier = SimpleNamespace(
        inspect=lambda *_: ('0' * 63 + '2', template_identity(bytes(88)), 'none')
    )
    ...
    pool.payments = Payments(
        pool, payment_journal, 'no-signer-in-fixture', '', '', '11', '22', 'fees', 100000, 1000
    )
    operator = Operator(pool, max_fee)
...
class OperatorTests(unittest.TestCase):
    ...
    def payload(self, **settings):
        ...
        return dict(
            ...
            revision=str(self.operator.revision),
            reason='Fixture operator change',
            settings=dict(self.operator.settings, **settings),
        )

    def apply(self, **settings):
        return self.operator.execute('settings', self.payload(**settings), 'operator:' + 'a' * 16)

    def restart(self):
        node, verifier = self.pool.node, self.pool.verifier
        self.pool.journal.close()
        journal = Journal(self.root / 'work', self.root / 'anchor/work.json')
        self.pool = Coordinator(node, journal, verifier, 'pool', 'f' * 64)
        self.pool.payments = Payments(
            self.pool, self.pj, 'no-signer-in-fixture', '', '', '11', '22', 'fees', 100000, 1000
        )
    ...
    def test_default_closed_and_restart_replay_idempotent_audit(self):
        ...
        value = self.payload(minimum_units='200000000')
        result = self.operator.execute('settings', value, 'operator:' + 'a' * 16)
        ...
        with self.assertRaisesRegex(Refused, 'different content'):
            self.operator.execute(
                'settings', dict(value, reason='different operator intent'), 'operator:' + 'a' * 16
            )
        self.restart()
        self.assertEqual(self.operator.settings['minimum_units'], '200000000')
        self.assertEqual(self.operator.revision, 1)
        self.assertEqual(len(self.operator.snapshot()['audit']), 1)
        with self.assertRaisesRegex(Refused, 'settings changed'):
            self.operator.execute(
                'settings', dict(self.payload(), revision='0'), 'operator:' + 'a' * 16
            )

    def test_rollback_refused_with_surviving_anchor(self):
        backup = self.root / 'old.jsonl'
        shutil.copy2(self.pool.journal.directory / 'events.jsonl', backup)
        self.apply(minimum_units='200000000')
        self.pool.journal.close()
        shutil.copy2(backup, self.root / 'work/events.jsonl')
        with self.assertRaises(Refused):
            Journal(self.root / 'work', self.root / 'anchor/work.json')
    ...
    def test_readiness_cannot_be_bypassed_or_turn_off_maturity(self):
        ...
        with self.assertRaisesRegex(Refused, 'fee funding'):
            self.apply(payments_paused=False)
        with self.assertRaisesRegex(Refused, 'signer'):
    ...
    def test_approved_zero_fee_ceiling_and_prospective_assigned_work(self):
        ...
        for work in (first, second):
            self.assertEqual(
                self.pool.submit(
                    account['account'], account['worker_token'], work['lease'], work['start']
                )['status'],
                'verified',
            )
        receipts = list(self.pool.receipts.values())
        self.assertEqual(sorted(r['fee_ppm'] for r in receipts), ['0', '10000'])
        weights = pplns(receipts, self.pool.journal.sequence, 'f' * 64, fee_policy=True)
        credit, fee = allocate_policy(10000, weights)
        self.assertEqual(credit[account['account']], 9950)
        self.assertEqual(fee, 50)
        self.assertEqual(sum(credit.values()) + fee, 10000)
        self.assertNotEqual(first['start'], second['start'])
        with self.assertRaisesRegex(Refused, 'ceiling'):
            Operator(self.pool, 0)
    ...
    def test_pause_is_serialized_and_does_not_hold_worker_lock(self):
        ...
        worker = threading.Thread(target=lambda: self.operator.run('payments', callback))
        worker.start()
        self.assertTrue(entered.wait(1))
        self.assertTrue(self.pool.lock.acquire(timeout=0.2))
        self.pool.lock.release()
        value = self.payload(payments_paused=True)

        def pause():
            self.operator.execute('settings', value, 'operator:' + 'a' * 16)
            paused.set()
    ...
    def test_pause_keeps_signed_bytes_and_observes_confirmation_without_broadcast(self):
        ...
        self.pool.incomes['fixture'] = {
            'id': 'd' * 64,
            'state': 'available',
            'txid': 'b' * 64,
            'height': 1,
            'outputs': [{'vout': 0, 'units': '300000000'}],
            'credits': {'a': '300000000/1'},
        }
        self.apply(payments_paused=False)
        identity = self.pool.payments.plan(now=100000)
        raw = b'fixture bytes, not a signed transaction'
        txid = hashlib.sha256(hashlib.sha256(raw).digest()).hexdigest()
        self.pool.payments.record(
            'payment_signed', dict(id=identity, txid=txid, signed_hex=raw.hex(), state='signed')
        )
        self.apply(payments_paused=True)
        self.assertEqual(self.pool.payments.reconcile(identity), 'signed')
        self.assertEqual(self.pool.node.broadcasts, [])
        self.assertEqual(self.pool.payments.summary('a')['reserved_units'], '300000000')
        self.pool.node.confirmed = True
        self.assertEqual(self.pool.payments.reconcile(identity), 'confirmed')
    ...
    def test_failed_audit_write_stops_new_payment_authority(self):
        self.apply(payments_paused=False)
        with patch.object(
            self.pool.journal, 'append', side_effect=OSError('injected disk failure')
        ):
            with self.assertRaises(OSError):
                self.apply(minimum_units='200000000')
        self.assertTrue(self.operator.faulted)
        with self.assertRaisesRegex(Refused, 'journal failed'):
            self.pool.payments.plan(now=100000)
        with self.assertRaisesRegex(Refused, 'journal failed'):
            self.operator.run('payments', lambda: self.fail('unsafe callback'))
    ...
    def test_queued_reconciliation_replays_after_restart_and_never_signs(self):
        ...
        self.assertEqual(self.pool.node.broadcasts, [])
        self.assertEqual(self.pool.payments.intents, {})
    ...
    def test_operator_reconcile_does_not_submit_retained_block_solution(self):
        ...
        self.pool.record(
            'solution',
            dict(block='e' * 64, height=201, job=job, nonce='0', cutoff=self.pool.journal.sequence),
        )
        request = dict(
            request_id='e' * 32, revision='0', reason='Read-only solution reconciliation'
        )
        self.operator.execute('reconcile', request, 'operator:' + 'a' * 16)
        with (
            patch.object(self.pool.node, 'renew', create=True) as renew,
            patch.object(self.pool.node, 'submit', create=True) as submit,
        ):
            self.operator.reconcile()
    ...
    def test_new_payout_threshold_and_frozen_batch_policy(self):
        ...
        self.pool.payments.record(
            'payment_batch',
            dict(id='prior', created=100000, quotas={'a': '300000000'}, minimum_units='100000000'),
        )
    ...
    def test_comining_readiness_requires_real_manifest_funds_and_excludes_liabilities(self):
        ...
        self.pool.node.coins['pool'] = [
            dict(txid=fund['txid'], vout=0, value_units=int(fund['units']), confirmations=200)
        ]
        ...
        state = dict(
            ...
            parent='d' * 64,
            stake_units='0',
            required_stake_units='100000000000',
            fee_units='100000',
            dust_units='1000',
            staking_active=True,
            eligible=False,
        ...
        self.pool.node.call = (
            lambda method, *args: state
            if method == 'getpoolidentitystate'
            else original(method, *args)
        )
        self.pool.identity = Identity(
            self.pool, journal, 'not-invoked', 'no-live-key', 'fixture-script', [fund]
        )
        self.apply(comining_paused=False)
        self.assertFalse(self.operator.settings['comining_paused'])
        self.assertEqual(self.pool.identity.intents, {})
        self.apply(comining_paused=True)
        self.pool.incomes['overlap'] = {'txid': fund['txid'], 'outputs': [{'vout': 0}]}
        with self.assertRaisesRegex(Refused, 'liabilities'):
            self.apply(comining_paused=False)
    ...
    def test_mixed_fee_allocations_match_independent_exact_reference(self):
        ...
        for _ in range(300):
            ...
            weights = {
                (str(i % 3), i * 100): Fraction(rng.randrange(1, 1 << 80), 7)
                for i in range(1, rng.randrange(2, 40))
            }
            credits, fee = allocate_policy(amount, weights)
            total = sum(weights.values())
            expected = {}
            for (account, ppm), weight in weights.items():
                expected[account] = expected.get(account, Fraction()) + Fraction(
                    amount
                ) * weight * (1000000 - ppm) / (total * 1000000)
            self.assertEqual(credits, expected)
            self.assertEqual(sum(credits.values()) + fee, amount)
    ...
    def test_operator_records_bind_chain_and_persist_initial_pause(self):
        ...
        with self.assertRaisesRegex(Refused, 'chain mismatch'):
            Operator(self.pool, 10000)
