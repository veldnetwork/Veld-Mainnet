...
class DashboardTests(unittest.TestCase):
    def setUp(self):
        ...
        self.a = self.pool.register('V' + 'a' * 33)
        self.b = self.pool.register('V' + 'b' * 33)
    ...
    def test_public_health_has_no_account_or_payment_credentials(self):
        ...
        self.pool.active_accounts = {
            self.a['account']: time.monotonic(),
            self.b['account']: time.monotonic() - 121,
        }
        self.pool.verified_counts = {self.a['account']: 3, self.b['account']: 7}
        value = self.pool.health()
        self.assertEqual(
            (value['active_accounts'], value['verified_shares'], value['reconciled_height']),
            ('1', '10', '9001'),
        )
        text = json.dumps(value)
        for account in (self.a, self.b):
            for key in ('account', 'worker_token', 'view_token'):
                self.assertNotIn(account[key], text)
        self.assertFalse(value['payments_enabled'])
        self.assertFalse(value['co_mining_enabled'])

    def test_rewards_are_private_and_use_only_canonical_earned_income(self):
        for i, (category, state) in enumerate(
            [
                ('mining', 'available'),
                ('comine_payout', 'pending'),
                ('staking_distribution', 'pending'),
                ('mining', 'orphaned'),
            ]
        ):
            self.pool.record(
        ...
        with self.assertRaises(Refused):
            self.pool.account(self.a['account'], self.a['worker_token'])
        with self.assertRaises(Refused):
            self.pool.account(self.a['account'], self.b['view_token'])
        result = self.pool.account(self.a['account'], self.a['view_token'])
        self.assertEqual(
            result['reward_totals'],
            dict(mining='100000001', comine_payout='100000001', staking_distribution='100000001'),
        )
        self.assertEqual(result['pending_units'], '200000002')
        self.assertEqual(result['available_units'], '100000001')
    ...
    def test_restart_does_not_invent_active_workers(self):
        ...
        self.assertEqual(
            restarted.account(self.a['account'], self.a['view_token'])['pending_units'], '0'
        )
