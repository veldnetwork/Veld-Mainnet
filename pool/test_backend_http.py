...
class BackendHttpTests(unittest.TestCase):
    ...
    def test_real_http_readiness_is_retryable_without_hiding_policy_refusals(self):
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_POST(self):
                ...
                state['calls'] += 1
                body = json.dumps(
                    {'id': request['id'], 'error': state['error'], 'result': {'ready': True}}
                ).encode()
        ...
        try:
            with tempfile.TemporaryDirectory() as directory:
                ...
                for message in cases:
                    ...
                    before = state['calls']
                    with self.subTest(message=message), self.assertRaises(Busy):
                        node.call('getblocktemplate', 'public-address-fixture')
                    self.assertEqual(
                        state['calls'], before + 1, 'adapter must leave retries to the worker'
                    )
                    state['error'] = None
                    self.assertEqual(
                        node.call('getblocktemplate', 'public-address-fixture'), {'ready': True}
                    )
                for stage in stages:
                    for reason in (
                        'role_denied',
                        ...
                        'runtime_closed',
                        'sync_incomplete extra',
                        'peer_view_unsafe extra',
                    ):
                        if (
                            stage
                            in (
                            ...
                            and reason == 'binding_mismatch'
                        ):
                            continue
                        state['error'] = {'code': -32010, 'message': stage + reason}
                        with self.subTest(stage=stage, reason=reason), self.assertRaises(Refused):
                            node.call('getblocktemplate', 'public-address-fixture')
                    for reason in ('sync_incomplete', 'peer_view_unsafe'):
                        ...
                        for method in ('sendrawtransaction', 'submitblock'):
                            ...
                            with self.assertRaises(Refused):
                                node.call(method, 'exact-fixture-bytes')
                            self.assertEqual(
                                state['calls'],
                                before + 1,
                                'writes must not acquire a new retry classification',
                            )
                ...
                for method in ('submitblock', 'sendrawtransaction', 'getblockcount'):
                    for stage in ('authorization refused', 'closed before publication'):
                        state['error'] = {
                            'code': -32010,
                            'message': 'getblocktemplate ' + stage + ': binding_mismatch',
                        }
                        before = state['calls']
                        with self.subTest(method=method, stage=stage), self.assertRaises(Refused):
                            node.call(method, 'exact-fixture-bytes')
                        self.assertEqual(state['calls'], before + 1)
                for code, message in (
                    (-32603, 'getblocktemplate authorization refused: binding_mismatch'),
                    ...
                    with self.subTest(code=code, message=message), self.assertRaises(Refused):
                        node.call('getblocktemplate', 'public-address-fixture')
                # Capacity is explicit. Do not make generic runtime closure,
                # token entropy failure or policy refusal silently retryable.
                state['error'] = {
                    'code': -32005,
                    'message': 'getblocktemplate authorization capacity; retry shortly',
                }
                before = state['calls']
                with self.assertRaises(Busy):
                    node.call('getblocktemplate', 'public-address-fixture')
    ...
    def test_invalid_http_never_becomes_a_result_or_an_automatic_write_retry(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            for error in (
                http.client.BadStatusLine('POST / HTTP/1.1\r\n'),
                ...
                with patch('pool.backend.http.client.HTTPConnection') as connection:
                    ...
                    with self.assertRaises(Busy):
                        node.call('sendrawtransaction', 'exact-disposable-test-bytes')
