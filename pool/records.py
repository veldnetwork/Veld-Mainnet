...
class Records(MutableMapping):
    def __init__(self, journal, bucket):
        ...
        with journal.lock:
            ...
            journal.db.execute(
                'CREATE INDEX IF NOT EXISTS pool_records_sequence ON pool_records(bucket,status,seq)'
            )
            journal.db.execute(
                'CREATE INDEX IF NOT EXISTS pool_records_height ON pool_records(bucket,height,seq)'
            )
    ...
    def __getitem__(self, key):
        with self.journal.lock:
            ...
            row = self.journal.db.execute(
                'SELECT body FROM pool_records WHERE bucket=? AND key=?', (self.bucket, key)
            ).fetchone()
        if row is None:
            raise KeyError(key)
    ...
    def __setitem__(self, key, value):
        ...
        require(
            type(seq) is int and type(height) is int and isinstance(status, str),
            'record index fields',
        )
        body = encode(value)
        require(len(body) <= MAX_EVENT, 'record limit')
        with self.journal.lock:
            self.journal.ensure_open()
            self.journal.db.execute(
                'INSERT INTO pool_records VALUES(?,?,?,?,?,?) '
    ...
    def __delitem__(self, key):
        with self.journal.lock:
            self.journal.ensure_open()
            row = self.journal.db.execute(
                'DELETE FROM pool_records WHERE bucket=? AND key=?', (self.bucket, key)
            )
            if not row.rowcount:
                raise KeyError(key)
    ...
    def __len__(self):
        with self.journal.lock:
            ...
            return self.journal.db.execute(
                'SELECT COUNT(*) FROM pool_records WHERE bucket=?', (self.bucket,)
            ).fetchone()[0]
    ...
    def __iter__(self):
        with self.journal.lock:
            ...
            cursor = self.journal.db.execute(
                'SELECT key FROM pool_records WHERE bucket=? ORDER BY key', (self.bucket,)
            )
            try:
                for row in cursor:
                    yield row[0]
    ...
    def select(
        self, *, status=None, cutoff=None, first=None, last=None, reverse=False, sequence=False
    ):
        sql = 'SELECT body FROM pool_records WHERE bucket=?'
        args = [self.bucket]
