...
def main():
    ...
    with tempfile.TemporaryFile(mode='w+') as log:
        ...
        try:
            ...
            try:
                native.hash(raw, int(height))
            except Busy:
                pass
            else:
                raise AssertionError('unbounded restart loop')
        finally:
            native.close()
    result = subprocess.run(
        [args.binary], input='x' * 10000 + '\n', text=True, capture_output=True, timeout=10
    )
    assert result.returncode == 2 and len(result.stdout) < 1024
    result = subprocess.run(
        [args.binary],
        input='scan 0 ' + header + ' ' + 'f' * 64 + ' ffffffffffffffff 2\n',
        text=True,
        capture_output=True,
        timeout=10,
    )
    ...
    print(
        'PASS real VeldHash before/after two verifier crashes, bounded restarts, oversized framing, nonce overflow'
    )
