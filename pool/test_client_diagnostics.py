...
def dummy_address():
    ...
    while value:value,remainder=divmod(value,58);out=alphabet[remainder]+out
    return out


def main():
    parser=argparse.ArgumentParser();parser.add_argument('--binary',required=True)
    parser.add_argument('--openssl',default='openssl');parser.add_argument('--expect-legacy',action='store_true')
    ...
    address=dummy_address();account='a'*32;worker_token='b'*64;view_token='c'*64
    ...
    if os.name=='nt':
        sid=json.loads(subprocess.check_output(['powershell.exe','-NoProfile','-Command',
            '[Security.Principal.WindowsIdentity]::GetCurrent().User.Value | ConvertTo-Json'],text=True,
            creationflags=subprocess.CREATE_NO_WINDOW))
    def protect(path,directory=False):
        if sid:
            subprocess.run(['icacls',str(path),'/inheritance:r','/grant:r',
                '*'+sid+':'+('(OI)(CI)F' if directory else 'F')],capture_output=True,check=True,
                creationflags=subprocess.CREATE_NO_WINDOW)
        else:path.chmod(0o700 if directory else 0o600)
    cases=[('work-refused','remote_refusal','work'),('registration-refused','remote_refusal','registration'),
    ...
    with tempfile.TemporaryDirectory(prefix='veld-native-diagnostics-') as temporary:
        ...
        for name in ('trusted','other'):
            subprocess.run([args.openssl,'req','-x509','-newkey','rsa:2048','-nodes','-days','1',
                '-keyout',str(root/(name+'.key')),'-out',str(root/(name+'.crt')),'-subj','/CN=localhost',
                '-addext','subjectAltName=DNS:localhost,IP:127.0.0.1'],capture_output=True,check=True)
        header=(Path(__file__).resolve().parents[1]/'tests/fixtures/veldhash-3.0.9-vectors.txt').read_text().splitlines()[0].split()[1]
        for name,code,stage in cases:
            private=root/name;private.mkdir();protect(private,True)
            if name=='state-write':
                state_dir=private/'state';state_dir.mkdir();protect(state_dir,True)
            ...
            class Handler(http.server.BaseHTTPRequestHandler):
                ...
                def do_POST(self):
                    length=int(self.headers['Content-Length']);assert 0<length<=16384
                    body=json.loads(self.rfile.read(length));action=self.path.rsplit('/',1)[-1]
                    with lock:seen.append(action);work_calls=seen.count('work')
                    if action=='register':
                        result=dict(version='veld-pool/1',account=account,worker_token=worker_token,view_token=view_token)
                    elif action=='account':
                        assert body==dict(account=account,token=view_token)
                        result=dict(account='0'*32 if name=='balance-identity' else account,address=address,
                            pending_units='0',available_units='0',reserved_units='0',paid_units='0',verified_shares='0')
                    elif action=='work':
                        assert body==dict(account=account,token=worker_token,count='1')
                        # Zero TTL performs no hashing. This is a diagnostic fixture.
                        result=dict(version='veld-pool/1',chain=genesis,lease='d'*32,header=header,
                            target='f'*64,height='0',start='0'*16,count='1',ttl_ms='0')
                        if name=='work-shape':result=[]
                        if name=='work-chain':result['chain']='0'*64
                        if name=='submit-refused':result['ttl_ms']='10000'
                        if name=='balance-identity':time.sleep(.15)
                    elif action=='submit':
                        assert name=='submit-refused' and body==dict(account=account,token=worker_token,lease='d'*32,nonce='0'*16)
                        result={}
                    ...
                    self.wfile.write(encoded);self.close_connection=True
            server=http.server.ThreadingHTTPServer(('127.0.0.1',0),Handler)
            context=ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)
            context.load_cert_chain(root/'trusted.crt',root/'trusted.key')
            server.socket=context.wrap_socket(server.socket,server_side=True)
            thread=threading.Thread(target=server.serve_forever,daemon=True);thread.start()
            config=dict(endpoint='https://127.0.0.1:'+str(server.server_port),ca_file=str(root/('other.crt' if name=='certificate' else 'trusted.crt')),
                genesis=genesis,payout_address=address,threads='2' if name=='work-refused' else '1',nonce_count='1',pause_ms='0',
                state_directory=str(private/'state'))
            path=private/'config.json';path.write_text(json.dumps(config));protect(path)
            try:
                proc=subprocess.run([args.binary,'--config',str(path),'--rounds','1'],capture_output=True,text=True,timeout=30)
            finally:server.shutdown();server.server_close();thread.join(timeout=5)
            logs=proc.stdout+proc.stderr
            assert all(secret not in logs for secret in (worker_token,view_token,private_sentinel)),(name,'private output')
            ...
            if args.expect_legacy:
                ...
                print('REPRODUCED legacy worker hides work refusal cause and stage');continue
            receipt=receipts[-1]
            assert receipt['failure_code']==(code or '') and receipt['failure_stage']==(stage or ''),(name,receipt)
            status_path=private/'state/pool-status.json'
            if stage not in ('registration','state'):
                state=json.loads(status_path.read_text())
                assert all(secret not in status_path.read_text() for secret in (worker_token,view_token,private_sentinel))
                assert state['active_workers']=='0' and state['hashes']==('1' if name=='submit-refused' else '0'),(name,state)
                if code:assert (state['failure_code'],state['failure_stage'])==(code,stage),(name,state)
                else:assert state['status']=='Stopped' and 'failure_code' not in state
