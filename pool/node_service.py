...
def deployment(binary):
    probe=subprocess.run([binary,'--deployment-info'],capture_output=True,check=True,timeout=15)
...
def validate_deployment(identity, config):
    require(identity.get('binary_role')=='node' and identity.get('fleet_no_mine') is False and
            identity.get('mining_rpc_methods_compiled') is True,'dedicated mining-capable backend required')
    ...
    require(identity.get('profile_id')==config['profile'] and rpc_genesis==config['genesis'],
            'backend profile/genesis mismatch')
    if config.get('runtime_network','mainnet')!='mainnet':
        require(identity.get('disposable') is True and identity.get('external_value') is False,
                'alternate transport requires an explicitly disposable artifact')

def configuration(value):
    value=dict(value)
    value.setdefault('runtime_network','mainnet')
    schema(value,('enabled','binary','datadir','passphrase_file','rpc_token_file','genesis',
                  'profile','p2p_port','rpc_port','peers','runtime_network'))
    require(value['enabled'] is True,'backend disabled; review the private network configuration first')
    hex64(value['genesis'])
    for field in ('binary','datadir','passphrase_file','rpc_token_file'):
        require(isinstance(value[field],str) and Path(value[field]).is_absolute(),'absolute '+field)
    ...
    require(isinstance(value['profile'],str) and 1<=len(value['profile'])<=80,'profile identity')
    require(value['runtime_network'] in ('mainnet','testnet','regtest'),'explicit runtime network')
    require(isinstance(value['peers'],list) and 1<=len(value['peers'])<=32,'explicit peer endpoints required')
    import ipaddress
    for peer in value['peers']:
        require(isinstance(peer,str) and peer.count(':')==1,'explicit IPv4 peer and port')
        host,port=peer.split(':')
        ipaddress.IPv4Address(host)
        require(port.isdecimal() and str(int(port))==port and 1024<int(port)<=65535,'peer port')
    return value
...
def command(config):
    ...
    return [config['binary'],'--no-prompt','--nomine','--full-ibd','--txindex',
            *network_arguments(config),
            '--datadir',config['datadir'],'--p2pport',str(config['p2p_port']),
            '--rpcport',str(config['rpc_port']),
...
def export_token(config):
    ...
    require(len(password)>=16 and '\x00' not in password and '\n' not in password,'backend passphrase unavailable')
    env=dict(os.environ);env['VELD_VAULT_PASSPHRASE']=password
    try:
        probe=subprocess.run([config['binary'],*network_arguments(config),'--datadir',config['datadir'],'--print-rpc-token'],
                             env=env,stdin=subprocess.DEVNULL,stdout=subprocess.PIPE,
                             stderr=subprocess.DEVNULL,timeout=30,check=True)
    finally:env.pop('VELD_VAULT_PASSPHRASE',None)
    token=probe.stdout.strip()
    require(len(token)==64 and all(c in b'0123456789abcdef' for c in token),'node RPC export refused')
...
def main():
    ...
    private_directory(config['datadir']);private_directory(Path(config['rpc_token_file']).parent)
    ...
    env.pop('VELD_VAULT_PASSPHRASE',None);password=''
    token_state=None
    rpc=Node('http://127.0.0.1:'+str(config['rpc_port']),config['rpc_token_file'],config['genesis'])
    try:
        while not stop.wait(1):
            ...
            info=source.stat();current=(info.st_dev,info.st_ino,info.st_mtime_ns,info.st_size)
            if token_state!=current:
                # No stale fallback if the encrypted token rotates unsuccessfully.
                Path(config['rpc_token_file']).unlink(missing_ok=True)
                export_token(config);token_state=current
            try:rpc.check_chain()
