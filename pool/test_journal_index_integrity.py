...
class JournalIndexIntegrity(unittest.TestCase):
    def test_cached_payload_rebuilt_from_unchanged_journal_and_anchor(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                journal.db.execute('SELECT body FROM events WHERE seq=1').fetchone()[0],
                encode(expected[0]),
            )
    ...
    def test_invalid_and_oversize_cached_bodies_are_not_recovery_input(self):
        for body in (b'{broken', 'text instead of blob', b'x' * 8193):
            with (
                self.subTest(body_type=type(body).__name__, length=len(body)),
                tempfile.TemporaryDirectory() as directory,
            ):
                root = Path(directory)
                journal = Journal(root / 'data', root / 'anchor/current')
                journal.append('intent', {'id': 'unchanged'})
                expected = list(journal.events())
                journal.close()
    ...
    def test_even_coherent_cache_forgery_after_open_cannot_change_replay(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            body = encode(forged)
            journal.db.execute(
                'UPDATE events SET digest=?,body=? WHERE seq=1',
    ...
    def test_nested_replay_and_append_preserve_snapshot_and_writer(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                [first['payload']['end']] + [e['payload']['end'] for e in replay], ['100', '200']
            )
            self.assertEqual([e['payload']['end'] for e in journal.events()], ['100', '200', '300'])
            journal.close()
    ...
    @unittest.skipUnless(
        os.name == 'posix', 'Linux permits replacement of an open journal pathname'
    )
    ...
    def test_signed_payment_commitment_survives_cache_corruption_without_resigning(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            expected = dict(payment.intents[identity])
            journal.close()
            with closing(sqlite3.connect(root / 'payments/index.sqlite')) as db, db:
                ...
                changed['payload']['signed_hex'] = '00'
                changed['payload']['state'] = 'reserved'
            ...
            with patch(
                ...
                self.assertEqual(
                    restored.summary('a'), {'paid_units': '0', 'reserved_units': '150000000'}
                )
