...
def dispatch(pool, request):
    ...
    if action == 'health':
        ...
        return pool.health()
    if action == 'register':
        schema(value, ('address',))
        return pool.register(value['address'])
    if action == 'work':
...
class Server(socketserver.ThreadingMixIn, socketserver.UnixStreamServer):
    ...
    def __init__(self, path, pool, operator_uid=None):
        if operator_uid is not None:
            self.request_queue_size = 4
        self.pool = pool
        self.operator_uid = operator_uid
        self.slots = threading.BoundedSemaphore(16 if operator_uid is None else 4)
        self.notice_lock = threading.Lock()
        self.last_refusal_notice = None
    ...
    def refusal_notice(self, action, error):
        ...
        print(
            'pool request refused action=' + action + ' reason=' + reason,
            file=sys.stderr,
            flush=True,
        )
    ...
    def process_request(self, request, address):
        ...
        if not self.slots.acquire(timeout=0.25 if self.operator_uid is None else 0):
            request.close()
            with self.notice_lock:
                now = time.monotonic()
                if self.last_capacity_notice is None or now - self.last_capacity_notice >= 30:
                    ...
                    print(
                        'pool IPC capacity saturated; request closed', file=sys.stderr, flush=True
                    )
            return
        try:
...
class Handler(socketserver.StreamRequestHandler):
    def handle(self):
        ...
        try:
            ...
            if self.server.operator_uid is None:
                result = dispatch(self.server.pool, request)
            else:
                schema(request, ('action', 'payload', 'actor'))
                operator = self.server.pool.operator
...
def maintain_once(pool, node, stop):
    ...
    def step(stage, action):
        try:
            ...
            if prior is None or prior[0] != detail or time.monotonic() - prior[1] >= 30:
                print(
                    stage + ' deferred: ' + detail + '; existing reservations retained',
                    file=sys.stderr,
                    flush=True,
                )
                notices[stage] = (detail, time.monotonic())
                pool.maintenance_notices = notices
            return False
    ...
    if safe and pool.identity:
        step(
            'pool identity',
            lambda: operator.run('comining', pool.identity.maintain)
            if operator
            else pool.identity.maintain(),
        )
    ...
    def payments():
        for identity in list(pool.payments.intents):
            if stop.is_set():
                return
            pool.payments.sign(identity)
            pool.payments.reconcile(identity)
        if not stop.is_set():
    ...
    if safe and pool.payments:
        if operator and operator.settings['payments_paused']:
            ...
            def observe_payments():
                for identity in list(pool.payments.intents):
                    pool.payments.reconcile(identity, broadcast=False)

            step('payment observations', observe_payments)
        else:
            step(
                'payment reconciliation',
                lambda: operator.run('payments', payments) if operator else payments(),
            )
    if operator:
        step('operator reconciliation', operator.reconcile)
    if safe:
...
def main():
    ...
    with (Path(config['state_directory']) / 'verifier.log').open('a') as log:
        ...
        pool = Coordinator(
            node, journal, native, config['pool_address'], config['accounting_target']
        )
        canonical_policy = node.call('getpoolidentitystate', config['pool_address'])
        pool.rewards = Rewards(
            pool, canonical_policy['window_blocks'], canonical_policy['yield_blocks']
        )
        ...
        if 'identity' in config:
            ...
            schema(
                ...
                (
                    ...
                    'seed',
                    'script',
                    'operator_funding_file',
                    'tier',
                ),
            )
            funding = decode(
                read_private(settings['operator_funding_file'], 2 * 1024 * 1024), 2 * 1024 * 1024
            )
        ...
        if 'payments' in config:
            ...
            policy_run = subprocess.run(
                [settings['native_binary'], '--policy'], capture_output=True, check=True, timeout=10
            )
            policy = decode(policy_run.stdout)
            schema(policy, ('chain', 'minimum_fee_units', 'dust_threshold_units'))
            require(
                policy['chain'] == node.genesis
                and units(settings['fee_units']) == units(policy['minimum_fee_units']),
                'payout native chain/fee policy',
            )
            pool.payments = Payments(
                ...
                settings['pool_seed'],
                settings['fee_seed'],
                settings['pool_script'],
                settings['fee_script'],
                settings['fee_address'],
                units(settings['fee_units']),
                units(policy['dust_threshold_units']),
            )
        operator_server = operator_thread = None
        if 'operator' in config:
            settings = config['operator']
            schema(settings, ('socket', 'uid', 'max_fee_ppm'))
            require(
                type(settings['uid']) is int
                and settings['uid'] > 0
                and settings['uid'] != os.geteuid(),
                'separate non-root operator UID required',
            )
            Operator(pool, units(settings['max_fee_ppm'], 100000))
            prepare_socket(settings['socket'])
            operator_server = Server(settings['socket'], pool, settings['uid'])
            operator_thread = threading.Thread(
                target=operator_server.serve_forever, kwargs={'poll_interval': 0.25}, daemon=True
            )
            operator_thread.start()
        else:
            require(
                not any(e['kind'].startswith('operator_') for e in journal.events()),
        ...
        def maintain():
            try:
                ...
                maintenance_failure.append(error)
                stop.set()
                server.shutdown()
        ...
        try:
            ...
            journal.close()
            if payment_journal:
                payment_journal.close()
            if identity_journal:
                identity_journal.close()
            Path(config['socket']).unlink(missing_ok=True)
        if maintenance_failure:
            raise RuntimeError(
                'pool maintenance failed; intact journal recovery required'
            ) from maintenance_failure[0]
