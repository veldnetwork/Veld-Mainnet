...
class Client:
    def __init__(self, url, ca, genesis):
        ...
        require(
            ...
            and self.endpoint.hostname
            and self.endpoint.path in ('', '/')
            and not self.endpoint.username
            and not self.endpoint.password
            and not self.endpoint.query
            and not self.endpoint.fragment,
            'TLS pool endpoint',
        )
    ...
    def call(self, action, payload):
        for attempt in range(8):
            try:
                ...
                if attempt == 7:
                    raise RuntimeError('pool busy after bounded retries') from error
                time.sleep(min(0.05 * (2**attempt), 1))

    def _call(self, action, payload):
        connection = http.client.HTTPSConnection(
            self.endpoint.hostname, self.endpoint.port or 443, context=self.context, timeout=20
        )
        try:
            connection.request(
                'POST', '/v1/' + action, encode(payload), {'Content-Type': 'application/json'}
            )
            response = connection.getresponse()
            result = decode(response.read(16385))
...
def main():
    ...
    account_path = Path(config['account_file'])
    account_path.parent.mkdir(parents=True, exist_ok=True)
    if account_path.exists():
        account = decode(account_path.read_bytes())
        require(
            account['endpoint'] == config['endpoint']
            and account['address'] == config['payout_address']
            and account['genesis'] == config['genesis'],
            'account configuration changed',
        )
    else:
        account = client.call('register', {'address': config['payout_address']})
        account.update(
            endpoint=config['endpoint'], address=config['payout_address'], genesis=config['genesis']
        )
    ...
    with account_path.with_suffix('.hash.log').open('a') as log:
        ...
        try:
            ...
            while not args.rounds or rounds < args.rounds:
                ...
                require(
                    lease['version'] == VERSION and lease['chain'] == config['genesis'],
                    'pool network/profile mismatch',
                )
                process.stdin.write(
                    f"scan {lease['height']} {lease['header']} {lease['target']} {lease['start']} {lease['count']}\n"
                )
                process.stdin.flush()
                for line in process.stdout:
                    if line.startswith('SHARE '):
                        fields = line.split()
                        require(len(fields) == 3, 'native share result')
                        response = client.call(
                            'submit', dict(base, lease=lease['lease'], nonce=fields[1])
                        )
                        print(
                            encode(
                                {
