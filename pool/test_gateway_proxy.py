...
def main():
    ...
    assert {
        x['ifname'] for x in json.loads(subprocess.check_output(['ip', '-j', 'link', 'show']))
    } == {'lo'}
    with tempfile.TemporaryDirectory(prefix='veld-proxy-reuse-') as temp:
        ...
        class IPC(socketserver.StreamRequestHandler):
            def handle(self):
                ...
                self.wfile.write(
                    json.dumps(dict(ok=True, result={'echo': payload['payload']})).encode() + b'\n'
                )
        ...
        report = dict(
            scope='native nginx/TLS/IPC; synthetic coordinator; loopback namespace only', cases=[]
        )
        try:
            for wrong_identity in (False, True):
                ...
                with (root / 'nginx-output.log').open('ab') as log:
                    ...
                    try:
                        for _ in range(50):
                            try:
                                status, _, _ = request('baseline', {})
                                break
                            except OSError:
                                time.sleep(0.05)
                        else:
                            raise AssertionError('private nginx start')
                        if wrong_identity:
                            assert status == 502
                        ...
                        for label in ('baseline', 'optimized'):
                            ...
                            report['cases'].append(
                                dict(
                                    mode=label,
                                    requests=32,
                                    upstream_tls_connections=connections,
                                    median_ms=statistics.median(times) * 1000,
                                )
                            )
                        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
                            responses = list(
                                executor.map(lambda i: request('optimized', {'n': i}), range(32))
                            )
