...
class Rewards:
    def __init__(self,pool,window_blocks,yield_blocks):
        ...
        self.seals={};self.quarantined={};self.observed={}
        for event in pool.journal.events():self.apply(event['kind'],event['payload'])

    def apply(self,kind,value):
        if kind=='reward_seal':self.seals[value['id']]=value
        elif kind=='reward_quarantine':self.quarantined[value['id']]=value
        elif kind=='reward_observed':self.observed[value['id']]=value

    def record(self,kind,value):
        self.pool.journal.append(kind,value);self.apply(kind,value)
    ...
    def seal(self,height,parent):
        ...
        for category,interval in self.intervals.items():
            if (height+1)%interval:continue
            draw=height+1;identity=category+':'+str(draw)+':'+hex64(parent)
    ...
    def reconcile(self):
        ...
        require(isinstance(page,dict) and isinstance(page.get('entries'),list) and
                len(page['entries'])<=50,'reward history shape')
        self.cursor=page['next_cursor'] if page.get('has_more') else ''
        for row in page['entries']:
            category=row.get('type')
            if category not in self.intervals:continue
            height=row['block_height'];require(type(height) is int and height>0,'reward height')
            block=hex64(self.node.call('getblockhash',str(height)))
            txid=hex64(row['txid']);identity=block+':'+txid
            if identity in self.pool.incomes:continue
            ...
            require(tx.get('block_hash')==block and tx.get('coinbase') is False,'reward inclusion')
            outputs=[o for o in tx['vout'] if o['address']==self.pool.pool_address and units(o['value_units'])>0]
            ...
            if seal is None:
                if identity not in self.quarantined:self.record('reward_quarantine',
                    {'id':identity,'block':block,'txid':txid,'amount':str(amount),'reason':'no pre-result contribution seal'})
                continue
            ...
            self.pool.record('income',{'id':identity,'block':block,'height':height,'txid':txid,
                ...
                'cutoff':seal['cutoff'],'credits':{a:text(v) for a,v in credits.items()},
                'operator_fee':text(fee),'state':'pending'})

    def status(self):
        return {'sealed_windows':str(len(self.seals)),
