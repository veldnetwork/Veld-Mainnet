...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='veld-gateway-latency-') as temporary:
        ...
        class IPC(socketserver.StreamRequestHandler):
            def handle(self):
                raw=self.rfile.readline(16385);body=json.loads(raw)
                assert set(body)=={'action','payload'}
                with lock:received.append(body)
                self.wfile.write(json.dumps(dict(ok=True,result={'echo':body['payload']})).encode()+b'\n')
        ...
        def request(name,body=b'{}',extra=None):
            ...
            conn=http.client.HTTPSConnection('127.0.0.1',server.server_port,context=trust,timeout=5)
            try:
                start=time.monotonic()
                conn.request('POST','/v1/work',body,{'Content-Type':'application/json',**(extra or {})})
                response=conn.getresponse();data=response.read(16385)
        ...
        try:
            ...
            kept=http.client.HTTPSConnection('127.0.0.1',services['candidate'][0].server_port,context=trust,timeout=5)
            warm=[];ports=[]
            try:
                for index in range(16):
                    start=time.monotonic();payload={'account':str(index%2),'token':'fixture-'+str(index)}
                    kept.request('POST','/v1/work',json.dumps(payload),{'Content-Type':'application/json','Connection':'keep-alive'})
                    ports.append(kept.sock.getsockname()[1])
                    response=kept.getresponse();value=json.loads(response.read())
                    assert response.status==200 and value['result']['echo']==payload
                    assert response.getheader('Connection')==('keep-alive' if index<15 else 'close')
                    warm.append(time.monotonic()-start)
                assert len(set(ports))==1 and kept.sock is None
            finally:kept.close()
            report['reuse']=dict(requests=16,connections=1,warm_median_ms=statistics.median(warm[1:])*1000)
            for fault in ('malformed','idle'):
                kept=http.client.HTTPSConnection('127.0.0.1',services['candidate'][0].server_port,context=trust,timeout=5)
                try:
                    kept.request('POST','/v1/work',b'{}',{'Content-Type':'application/json','Connection':'keep-alive'})
                    response=kept.getresponse();response.read();assert response.getheader('Connection')=='keep-alive'
                    if fault=='malformed':
                        kept.request('POST','/v1/work',b'{"x":1,"x":2}',{'Content-Type':'application/json','Connection':'keep-alive'})
                        response=kept.getresponse();response.read()
                        assert response.status==400 and response.getheader('Connection')=='close' and kept.sock is None
                    else:
                        time.sleep(2.2)
            ...
            for name,values in elapsed.items():
                report['cases'].append(dict(name=name,samples=len(values),median_ms=statistics.median(values)*1000,
                    p95_ms=sorted(values)[int(len(values)*.95)-1]*1000))
            ...
            assert all(status==200 for status,_,_ in responses), {'latency':report['cases'],
