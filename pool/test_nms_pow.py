...
def main():
    ...
    result = subprocess.run(
        [args.binary], input=request, text=True, capture_output=True, timeout=30, check=True
    )
    actual = result.stdout.splitlines()
    expected = [
        str(int(4 * target < space and target < proof <= 4 * target)) for target, proof in cases
    ]
