...
def main():
    ...
    keys = subprocess.check_output(
        [str(args.build / 'pool-lab-keys'), str(state / 'keys')], text=True
    )
    ...
    def client():
        return Node('http://127.0.0.1:34762', state / 'node/lab-rpc-token', genesis)
    ...
    report = dict(
        status='RUNNING',
        scope='native work authorization across actual canonical commits',
    ...
    def save():
        with lock:
            report.update(outcomes=dict(counts), failures=list(failures))
        ...
        while not stop.is_set():
            try:
                ...
                with lock:
                    ...
                    if transition_seen.is_set():
                        counts['usable_after_transition'] += 1
            except Busy as error:
                with lock:
                    counts['bounded_busy'] += 1
                    if str(error) == 'node canonical work transition; request fresh work':
                        counts['canonical_transition'] += 1
                        transition_seen.set()
            except Exception as error:
                with lock:
                    counts['fatal'] += 1
                    if len(failures) < 20:
                        failures.append(str(error)[:350])
            stop.wait(0.2)

    with (out / 'node.log').open('w') as log:
        process = subprocess.Popen(
            [str(args.build / 'pool-backend'), str(state / 'node'), '34761', '34762'],
            cwd=source,
        ...
        try:
            ...
            for index in range(192):
                mine_block(process, rpc, address, out / 'node.log')
                report['height'] = rpc.call('getblockcount')
                if index % 8 == 0:
                    save()
                with lock:
                    if counts['fatal']:
                        raise AssertionError('native work transition became a fatal refusal')
                    complete = (
                        counts['canonical_transition'] > 0
                        and counts['usable_after_transition'] >= 8
                    )
                if index >= 63 and complete:
                    break
            assert complete, (
                'required native transition was not observed; this run does not qualify the fix'
            )
            report['status'] = 'PASS'
            ...
            if process.poll() is None:
                ...
                try:
                    ...
                    process.wait()
            report.update(
                processes_stopped=process.poll() is not None
                and not any(t.is_alive() for t in threads),
                backend_sha256=hashlib.sha256(
                    (args.build / 'pool-backend').read_bytes()
                ).hexdigest(),
            )
            save()
