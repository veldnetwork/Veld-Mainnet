...
shutil.copytree(history / 'node', state / 'node')
shutil.copytree(history / 'node', state / 'observer')
shutil.copytree(history / 'keys', state / 'keys')
subprocess.run(['ip', 'link', 'set', 'lo', 'up'], check=True)
public = json.loads((args.public_identities or history / 'public-identities.json').read_text())
...
report = {
    'status': 'RUNNING',
    'scope': 'funded first-validator RPC/signature/admission/endorsement/restart gate',
    'activation_height': H,
    'private_test_boundary_only': True,
    'accelerated_clock': True,
    'consensus_or_economics_bypassed': False,
    'complete_validator_matrix': False,
...
def start(name, folder, p2p, rpcport):
    ...
    logs.append(log)
    p = subprocess.Popen(
        [str(build / 'pool-backend'), str(folder), str(p2p), str(rpcport)],
        cwd=src,
    ...
    processes.append(p)
    rpc = Node('http://127.0.0.1:' + str(rpcport), folder / 'lab-rpc-token', genesis)
...
def mine(address, producer=None, chain=None):
    producer = node if producer is None else producer
    chain = rpc if chain is None else chain
...
def balance():
    return sum(c['value_units'] for c in rpc.call('listunspent', addresses['pool']))
...
try:
    ...
    identity = (
        subprocess.run(
            [str(lab_sign), str(state / 'keys/pool.seed'), 'identity'],
            capture_output=True,
            check=True,
        )
        .stdout.decode()
    ...
    address, pubkey, script = identity
    assert address == addresses['pool'] and script == scripts['pool']
    checked(
        'zero aggregate ordinary stake before registration',
    ...
    while True:
        coins = sorted(
            rpc.call('listunspent', address), key=lambda c: (-c['value_units'], c['txid'])
        )
        if coins and coins[0]['value_units'] >= 10000 * 100000000 + 200000:
            break
        if len(coins) < 2:
            mine(address)
            continue
        ...
        manifest = state / 'authorized-inputs.json'
        manifest.write_bytes(
            encode({'chain': genesis, 'script': script, 'allowed_inputs': selected})
        )
        manifest.chmod(0o600)
        wire = {
            'chain': genesis,
            'action': 'consolidate',
            'height': str(rpc.call('getblockcount') + 1),
            'header': '',
            'stake_units': '0',
            'tier': '0',
            'inputs': selected,
        ...
        signed = (
            subprocess.run(
                [str(build / 'pool-identity'), str(state / 'keys/pool.seed'), str(manifest)],
                input=encode(wire) + b'\n',
                capture_output=True,
                check=True,
                timeout=120,
    ...
    while rpc.call('getblockcount') < H - 2:
        mine(addresses['fees'])
    try:
        rpc.call('prepareregistervalidator', address, pubkey)
    except Refused as error:
        checked('pre-activation aggregate gate retained', 'stak' in str(error).lower())
    else:
        raise AssertionError('pre-activation registration unexpectedly prepared')
    before_boundary = rpc.call('getstatedigest')
    save('pre-activation-state.json', before_boundary)
    mine(addresses['fees'])
    assert rpc.call('getblockcount') == H - 1
    ...
    ancestor = rpc.call('getbestblockhash')
    stop(node)
    ...
    checked(
        'closed pre-activation history replays identically on both branches',
        rpc.call('getbestblockhash') == ancestor == alternate.call('getbestblockhash')
        and rpc.call('getstatedigest') == alternate.call('getstatedigest'),
    )
    ...
    refused(
        'insufficient individual bond rejected without state change',
        sign(prepared, bond=str(10000 * 100000000 - 1))[1],
    )
    refused(
        'unauthorized funding signature rejected without state change',
        sign(prepared, key='fees')[1],
    )
    before_coins = rpc.call('listunspent', address)
    before = sum(c['value_units'] for c in before_coins)
    txid, raw = sign(prepared)
    ...
    registered = rpc.call('getvalidatorinfo', pubkey)
    validators = rpc.call('getvalidators')
    checked(
        'registration accepted at activation with zero ordinary stake',
        registered['registered']
        and registered['system_active']
        and validators['total_staked_veld'] == 0
        and validators['validator_count'] == 1
        and validators['validators'][0]['registered_height'] == H,
    )
    checked('bond and fee deducted exactly once', balance() == before - 10000 * 100000000 - 100000)
    checked(
        'registration alone pays no endorsement reward',
        rpc.call('getblockendorsements', str(H))['count'] == 0,
    )
    refused('spent bond inputs cannot be reused', sign(prepared, reduce='1')[1])
    checked(
        'duplicate registration preparation is idempotent',
        rpc.call('prepareregistervalidator', address, pubkey).get('status') == 'already_registered',
    )
    checked(
        'governance gate remains closed',
        rpc.call('getgovernanceinfo')['governance_active'] is False,
    )
    # Use the exact remote work grant / begin-signing / sink sequence.
    target = rpc.call('getblockcount')
    block = rpc.call('getblockhash', str(target))
    ...
    with (state / 'endorsement-intent.json').open('rb') as stream:
        os.fsync(stream.fileno())
    op = (
        subprocess.run(
            [str(lab_sign), str(state / 'keys/pool.seed'), 'endorse'],
            input=encode({'height': str(target), 'block': block}) + b'\n',
            capture_output=True,
            check=True,
            timeout=10,
        )
    ...
    endorse = rpc.call('preparerawop', address, op)
    end_id, end_raw = sign(endorse)
    assert (
        rpc.call('sendrawtransaction', end_raw, grant['binding'], grant['signing_token']) == end_id
    )
    mine(addresses['fees'])
    endorsements = rpc.call('getblockendorsements', str(target))
    checked(
        'first validator performs qualifying endorsement before finality quorum',
        endorsements['count'] == 1
        and endorsements['endorsements'][0]['address'] == address
        and rpc.call('getvalidators')['validator_count'] == 1,
    )
    save('endorsements.json', endorsements)
    save('validator-state.json', rpc.call('getvalidators'))
    tip = rpc.call('getbestblockhash')
    digest = rpc.call('getstatedigest')
    stop(node)
    node, rpc = start('node-restarted', state / 'node', 32761, 32762)
    checked(
        'restart preserves canonical registration and endorsement state',
        rpc.call('getbestblockhash') == tip
        and rpc.call('getstatedigest') == digest
        and rpc.call('getblockendorsements', str(target)) == endorsements,
    )
    while alternate.call('getblockcount') < H + 7:
        mine(addresses['fees'], fork, alternate)
    ...
    while rpc.call('getbestblockhash') != competing_tip:
        if time.monotonic() > until:
            raise RuntimeError('activation reorganization deadline')
        time.sleep(0.2)
    checked(
        'P2P reorganization removed the first registration branch',
        rpc.call('getblockhash', str(H)) != registered_hash
        and rpc.call('getblockhash', str(H + 1)) != tip,
    )
    ...
    save(
        'registration-rollback.json',
        {
            'original_coins': before_coins,
            'restored_coins': restored,
            'wallet': rpc.call('getbalance', address),
            'mempool': rpc.call('getrawmempool'),
            'validators': rpc.call('getvalidators'),
        },
    )
    checked(
        'registration rollback restores bond funding and registry together',
        all(
            c is not None
            and c['value_units'] == old['value_units']
            and c['script_pubkey_hex'] == script
            for old, c in zip(before_coins, restored)
        )
        and sum(c['value_units'] for c in restored) == before
        and rpc.call('getvalidators')['validator_count'] == 0
        and not rpc.call('getvalidatorinfo', pubkey)['registered'],
    )
    checked(
        'orphan registration input remains reserved while pending',
        txid in rpc.call('getrawmempool') and balance() < before,
    )
    checked(
        'registration rollback removes its qualifying endorsement',
        rpc.call('getblockendorsements', str(target))['count'] == 0,
    )
    checked(
        'reorganized state matches independently validated replacement branch',
    ...
    try:
        ...
        if txid not in rpc.call('getrawmempool'):
            raise
    inclusion = rpc.call('getblockcount') + 1
    mine(addresses['fees'])
    save(
        'replacement-registration.json',
        {
            'validator': rpc.call('getvalidatorinfo', pubkey),
            'validators': rpc.call('getvalidators'),
            'wallet': rpc.call('getbalance', address),
            'mempool': rpc.call('getrawmempool'),
            'expected_canonical_units': before - 10000 * 100000000 - 100000,
        },
    )
    checked(
        'same signed bond registers once on replacement history',
        rpc.call('getvalidatorinfo', pubkey)['registered']
        and rpc.call('getvalidators')['validators'][0]['registered_height'] == inclusion
        and canonical_balance() == before - 10000 * 100000000 - 100000,
    )
    checked(
        'reorganized registration still grants no automatic endorsement reward',
        rpc.call('getblockendorsements', str(inclusion))['count'] == 0,
    )
    tip = rpc.call('getbestblockhash')
    digest = rpc.call('getstatedigest')
    stop(node)
    ...
    checked(
        'restart preserves reorganized bond accounting',
        rpc.call('getbestblockhash') == tip
        and rpc.call('getstatedigest') == digest
        and canonical_balance() == before - 10000 * 100000000 - 100000,
    )
    ...
    while independent.call('getbestblockhash') != tip:
        ...
        report['independent_validation_progress'].append(
            {'height': height, 'seconds': now - progress.started}
        )
        save('progress.json', report)
        progress.observe(height, now)
        ...
        time.sleep(10)
    checked(
        'independent node agrees on replacement bond and removed endorsement state',
        independent.call('getstatedigest') == digest
        and independent.call('getblockendorsements', str(target))['count'] == 0,
    )
    report.update(
        status='PASS',
        height=rpc.call('getblockcount'),
        tip=tip,
        registered_txid=txid,
        endorsement_txid=end_id,
        reorganization_across_activation=True,
        replacement_registration_height=inclusion,
        remaining=[
            'funded below/at/above old floor matrix',
            'bond exit/slashing/yield settlement integration',
        ],
    )
    ...
    report['binary_sha256'] = {
        ...
        for n in ('pool-backend', 'pool-identity')
    }
    report['lab_sign_sha256'] = hashlib.sha256(lab_sign.read_bytes()).hexdigest()
