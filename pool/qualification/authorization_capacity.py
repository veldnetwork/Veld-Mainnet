...
report = dict(
    status='RUNNING',
    scope='native authorization capacity and expiry recovery',
...
with (args.output / 'node.log').open('w') as log:
    proc = subprocess.Popen(
        [str(args.build / 'pool-backend'), str(state / 'node'), '32861', '32862'],
    ...
    try:
        ...
        for trial in range(65):
            try:
                ...
                report.update(
                    refusal_type=type(error).__name__, reason=str(error), refusal_trial=trial + 1
                )
                assert trial == 64, 'unexpected capacity boundary'
                assert (
                    isinstance(error, Busy) if args.expect == 'busy' else isinstance(error, Refused)
                )
                break
        else:
            raise AssertionError('authorization capacity was not enforced')
        ...
        report.update(
            status='PASS',
            recovered_after_normal_expiry=True,
            chain_unchanged=True,
            interpretation='old defect reproduced'
            if args.expect == 'refused'
            else 'capacity correctly retryable; no work accepted while full',
        )
        ...
        report['backend_sha256'] = hashlib.sha256(
