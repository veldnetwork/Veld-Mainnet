...
def main():
    ...
    public = json.loads((original / 'public-identities.json').read_text())
    addresses = public['addresses']
    subprocess.run(['ip', 'link', 'set', 'lo', 'up'], check=True)
    genesis = 'd5f36a21eb02fca3c272c1cb87132a730b55f6821b454224adab5ad2865e87ee'
    report = dict(
        status='RUNNING',
        scope='unchanged seven-bond finality with actual pool worker carrier',
    ...
    def ready(rpc, p):
        ...
        while time.monotonic() < deadline:
            if p.poll() is not None:
                raise RuntimeError('backend exited during genuine replay')
            try:
                rpc.check_chain()
                return
    ...
    def sign(prepared, key):
        result = subprocess.run(
            [str(args.lab_signer), str(key), 'transaction'],
            input=encode(
                dict(
    ...
    try:
        node = start(
            'node', [str(build / 'pool-backend'), str(state / 'node'), '32501', '32502'], True
        )
        rpc = Node('http://127.0.0.1:32502', state / 'node/lab-rpc-token', genesis)
        ready(rpc, node)
        observer = start(
            'observer',
            [str(build / 'pool-backend'), str(state / 'observer'), '32511', '32512'],
            True,
        )
        independent = Node('http://127.0.0.1:32512', state / 'observer/lab-rpc-token', genesis)
        ready(independent, observer)
        observer.stdin.write('peer 32501\n')
        observer.stdin.flush()
        check(
            'naturally funded history has zero aggregate ordinary stake',
        ...
        for index in range(7):
            ...
            subprocess.run(
                [str(build / 'pool-lab-keys'), str(folder)], capture_output=True, check=True
            )
            key = folder / 'pool.seed'
            address, pubkey, script = subprocess.run(
                [str(args.lab_signer), str(key), 'identity'],
                capture_output=True,
                text=True,
                check=True,
            ).stdout.split()
            for attempt in range(1000):
                try:
                    prepared = rpc.call(
                        'preparerawtransaction', addresses['pool'], address, '10000.1'
                    )
                ...
                sign(compact, state / 'keys/pool.seed')
                consolidations += 1
                if consolidations % 20 == 0:
                    print('native funding consolidations', consolidations, flush=True)
            else:
                raise RuntimeError('bounded native funding consolidation deadline')
            ...
            check(
                'genuinely funded validator ' + str(index + 1) + ' registered', record['registered']
            )
            ...
            report['registrations'].append(
                dict(
                    ...
                    registration_txid=registration,
                    height=rpc.call('getblockcount'),
                    bond_units='1000000000000',
                )
            )
        registered = max(v['height'] for v in report['registrations'])
        snapshot = rpc.call('getfinalitysnapshot')['snapshot']
        check(
            'seven new bonds do not bypass registration maturity or warm-up', not snapshot['active']
        )
        while True:
            snapshot = rpc.call('getfinalitysnapshot')['snapshot']
            if snapshot and snapshot['active']:
                break
            mine()
            if rpc.call('getblockcount') % 100 == 0:
                print('native finality warm-up', rpc.call('getblockcount'), flush=True)
            if rpc.call('getblockcount') > registered + 1600:
                raise RuntimeError('unchanged finality warm-up deadline')
        check(
            'active snapshot preserves seven full bonds',
            len(snapshot['members']) == 7 and snapshot['total_weight'] == 70000 * 100000000,
        )
        check(
            'all seven registrations satisfied canonical maturity',
            all(
                snapshot['snapshot_height'] - v['registered_height'] >= 480
                for v in snapshot['members']
            ),
        )
        check(
            'aggregate ordinary stake remains zero',
            rpc.call('getvalidators')['total_staked_veld'] == 0,
        )
        ...
        snapshot = rpc.call('getfinalitysnapshot', str(target // 480))['snapshot']
        for phase in (1, 2):
            for index, member in enumerate(members[:5]):
                grant = rpc.call('getworkadmission', 'finality_vote', str(target), block)
                rpc.call(
                    'beginworksigning', 'finality_vote', grant['binding'], grant['signing_token']
                )
                intent = dict(
                ...
                intent_path = state / f'vote-{phase}-{index}.json'
                ...
                wire = (
                    subprocess.run(
                        [str(args.lab_signer), str(member['key']), 'finality'],
                ...
                check(
                    'native finality vote accepted phase '
                    + str(phase)
                    + ' signer '
                    + str(index + 1),
                    rpc.call('submitfinalityvote', wire, grant['binding'], grant['signing_token'])[
                        'accepted'
                    ],
                )
                qc = rpc.call('getfinalityqc', str(phase))['qc_hex']
                if index == 3:
                    check('four signatures do not form phase ' + str(phase) + ' quorum', not qc)
                if index == 4:
                    check('five signatures form phase ' + str(phase) + ' quorum', bool(qc))
        ...
        while not (state / 'coordinator.sock').exists():
            ...
            if time.monotonic() > until:
                raise RuntimeError('pool coordinator readiness')
            time.sleep(0.1)
        gateway = start(
            'gateway',
            [
                ...
                config(
                    ...
                    dict(
                        ...
                        coordinator_socket=settings['socket'],
                    ),
                ),
            ],
        )
        ...
        worker = start(
            'native-worker',
            [
                str(build / 'pool-client'),
                '--config',
                config(
                    'worker',
                    dict(
                        endpoint='https://localhost:32543',
                        ca_file=str(state / 'tls.crt'),
                        genesis=genesis,
                        payout_address=addresses['worker-a'],
                        state_directory=str(state / 'worker'),
                        threads='2',
                        nonce_count='8',
                        pause_ms='0',
                    ),
        ...
        while True:
            ...
            if finalized:
                break
            if time.monotonic() > until:
                raise RuntimeError('native pool QC-carrier deadline')
        ...
        check(
            'native pool worker carried the actual finality certificate',
            finalized['height'] == target and finalized['hash'] == block,
        )
        ...
        while independent.call('getbestblockhash') != tip:
            ...
            assert observer.poll() is None and node.poll() is None, (
                'independent finality node exited'
            )
            assert rpc.call('getbestblockhash') == tip, 'finality verification target changed'
            time.sleep(10)
        check(
            'independent node validated the same finality and state',
            independent.call('getstatedigest') == digest
            and independent.call('getfinalitysnapshot')['snapshot']['finalized'] == finalized,
        )
        exercise_equivocation(
            rpc=rpc,
        ...
        report.update(
            status='PASS',
            finalized=finalized,
            carrier_height=rpc.call('getblockcount'),
            consolidations=consolidations,
            worker=json.loads((state / 'worker/pool-status.json').read_text()),
        ...
        report['binary_sha256'] = {
            ...
            for n in ('pool-backend', 'pool-client', 'pool-work')
        }
        report['lab_signer_sha256'] = hashlib.sha256(args.lab_signer.read_bytes()).hexdigest()
        save()
