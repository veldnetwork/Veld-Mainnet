...
def main():
    ...
    for line in keys.splitlines():
        name, address, script = line.split()
        identities['addresses'][name] = address
        identities['scripts'][name] = script
    ...
    report = {
        ...
        'money_or_pow_bypassed': False,
        'pool_worker_evidence': False,
    ...
    def start(label):
        ...
        logs.append(log)
        p = subprocess.Popen(
            [str(args.build_directory / 'pool-backend'), str(state / 'node'), '32661', '32662'],
            cwd=source,
        ...
        while time.monotonic() < until:
            if p.poll() is not None:
                raise RuntimeError('history backend exited')
            try:
                rpc.check_chain()
                return p
    ...
    def stop():
        ...
        try:
            ...
            node.kill()
            node.wait()
            raise RuntimeError('history graceful shutdown deadline')
        if node.returncode:
            raise RuntimeError('history backend shutdown failed')
    ...
    try:
        ...
        with (out / 'blocks.jsonl').open('w') as receipts:
            for height in range(1, args.final_height + 1):
                ...
                if actual != height:
                    raise RuntimeError('funding history advanced beyond the acknowledged height')
                receipts.write(
                    json.dumps(
                        {
                            'height': height,
                ...
                if height % 100 == 0:
                    ...
                    save()
                    print('native history', height, flush=True)
                if height == 3266:
                    stop()
                    snapshot = Path(
                        tempfile.mkdtemp(prefix='veld-pool-history-comining-', dir='/var/tmp')
                    )
                    for directory in ('node', 'keys'):
                        shutil.copytree(state / directory, snapshot / directory)
                    shutil.copy2(
                        state / 'public-identities.json', snapshot / 'public-identities.json'
                    )
                    report['closed_snapshots']['comining'] = str(snapshot)
                    save()
                    node = start('node-resumed')
                if height == 6500 and args.final_height > 6500:
                    stop()
                    snapshot = Path(
                        tempfile.mkdtemp(prefix='veld-pool-history-validator-', dir='/var/tmp')
                    )
                    for directory in ('node', 'keys'):
                        shutil.copytree(state / directory, snapshot / directory)
                    shutil.copy2(
                    ...
                    report['closed_snapshots']['validator'] = str(snapshot)
                    save()
                    node = start('node-finality-funding')
        stop()
        report['closed_snapshots']['finality' if args.final_height > 6500 else 'validator'] = str(
            state
        )
        ...
        try:
            ...
            report['binary_sha256'] = hashlib.sha256(
                (args.build_directory / 'pool-backend').read_bytes()
            ).hexdigest()
            save()
