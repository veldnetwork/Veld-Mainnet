...
def main():
    ...
    stop=threading.Event();lock=threading.Lock();readers=[]
    report=dict(status='RUNNING',scope='native address-history concurrency and quiescent recovery',
                reads=0,transient_refusals=0,sticky=[],complete_pool_gate=False)
    log_path=out/'node.log'
    with log_path.open('w') as log:
        node=subprocess.Popen([str(args.backend),str(state/'node'),'32821','32822'],
        ...
        try:
            ...
            while True:
                ...
                except (OSError,ValueError,Busy):pass
                if time.monotonic()>until:raise RuntimeError('history-read backend deadline')
                time.sleep(.1)
            ...
            def reader():
                while not stop.is_set():
                    try:
                        ...
                        with lock:report['reads']+=1
                    except (OSError,Busy,Refused):
                        with lock:report['transient_refusals']+=1
                    stop.wait(.005)
            for _ in range(3):
                thread=threading.Thread(target=reader);readers.append(thread);thread.start()
            for _ in range(200):
                ...
                while True:
                    try:rpc.call('getaddresshistory',address,'50');break
                    except (Busy,Refused) as error:
                        if time.monotonic()>until:
                            report['sticky'].append(dict(height=height,error=str(error)))
                            break
                        time.sleep(.05)
                if report['sticky']:break
            stop.set()
            for thread in readers:thread.join(timeout=15)
    ...
    if report['status']!='PASS':raise RuntimeError('address history did not recover after completed publication')
