...
def acknowledgement(path, request_id):
    ...
    if (
        ...
        and parts[3].isascii()
        and parts[3].isdigit()
        and str(int(parts[3])) == parts[3]
        and 0 < int(parts[3]) < 2**64
        and len(parts[4]) == 64
        and all(c in '0123456789abcdef' for c in parts[4])
    ):
...
def mine_block(process, rpc, address, log_path, timeout=180):
    ...
    if not directory.is_dir():
        raise RuntimeError('native fixture directory missing')
        ...
        if receipt is not None:
            ...
            if result == 'MINED':
                if height <= before or rpc.call('getblockhash', str(height)) != block:
                    raise RuntimeError(
                        'native mining acknowledgement is not the expected canonical block'
                    )
                return height
            last_refusal = 'native command deferred'
            pending = False
        time.sleep(0.1 if pending else 0.5)
    # Do not enqueue another command or interpret a timeout as invalid work.
    raise RuntimeError(
        'native mining acknowledgement deadline: ' + (last_refusal or 'command still running')
    )
