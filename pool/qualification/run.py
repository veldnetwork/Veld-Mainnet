...
def validate_receipt(module, receipt):
    ...
    if not isinstance(receipt, dict):
        raise RuntimeError('native receipt must be an object')
    if module == 'pool.qualification.payment_fixture':
        if (
            ...
            or receipt['height'] < 120
            or not isinstance(receipt.get('fixture_path'), str)
            or not receipt['fixture_path']
        ):
            raise RuntimeError('native mature funding fixture is not safely prepared')
        return
    allowed = {'PASS'}
    if module == 'pool.qualification.validator_lifecycle':
        allowed.add('PASS_SCOPED_NATIVE_BOND_LIFECYCLE')
...
def main():
    ...
    parser.add_argument(
        '--through',
        choices=('focused', 'build', 'payments', 'complete'),
        default='complete',
    ...
    if args.service_roles and not service_identities_available():
        raise PermissionError(
            '--service-roles requires root to exercise distinct Linux process credentials'
        )
    ...
    def run(name, module, arguments, timeout=21600):
        ...
        with (out / 'logs' / (name + '.log')).open('w') as log:
            result = subprocess.run(
                command, cwd=SOURCE, stdout=log, stderr=subprocess.STDOUT, timeout=timeout
            )
        entry.update(
        ...
        if result.returncode:
            raise RuntimeError(name + ' failed; see raw log')
    ...
    try:
        run(
            'focused-tests',
            'unittest',
            ['discover', '-s', 'pool', '-t', '.', '-p', 'test_*.py'],
            600,
        )
        if args.through == 'focused':
            report['source_unchanged'] = before == hashes()
            if not report['source_unchanged']:
                raise RuntimeError('source changed during focused qualification')
            report['status'] = 'BLOCKED'
            report['blocked'].insert(
                0, 'development stop after focused tests; native stages not executed'
            )
        ...
        run(
            'native-authorization-capacity',
            'pool.qualification.authorization_capacity',
            ['--build', out / 'build/candidate', '--output', out / 'native-authorization-capacity'],
        )
        run(
            'native-template-transition',
            'pool.qualification.template_transition',
            ['--build', out / 'build/candidate', '--output', out / 'native-template-transition'],
        )
        run(
            'native-focused-regressions',
            'pool.qualification.native_regressions',
            [
                '--build-directory',
                out / 'build/candidate',
                '--output',
                out / 'native-focused-regressions',
            ],
        )
        run(
            'native-history-publication',
            'pool.qualification.history_publication',
            ['--build-directory', out / 'build/candidate', '--output', out / 'history-publication'],
        )
        run(
            'canonical-node-service',
            'pool.qualification.node_service',
            [
                '--build-directory',
                out / 'build/candidate',
                '--output',
                out / 'canonical-node-service',
            ],
        )
        if args.service_roles:
            run(
                'native-role-installation',
                'pool.qualification.role_install',
                [
                    '--build-directory',
                    out / 'build/candidate',
                    '--output',
                    out / 'native-role-installation',
                    '--systemd',
                ],
                25200,
            )
            ...
            report['blocked'].append(
                'distinct Linux service-user installation not selected; rerun as root with --service-roles'
            )
        if args.through != 'build':
            for variant in ('existing', 'candidate'):
                ...
                run(
                    variant + '-worker-fixture',
                    'pool.qualification.payment_fixture',
                    ['--build-directory', build, '--output', folder / 'worker-fixture'],
                )
                fixture = receipt(folder / 'worker-fixture/result.json')['fixture_path']
                run(
                    variant + '-immature-income-reorganization',
                    'pool.qualification.income_reorg',
                    [
                        '--fixture',
                        fixture,
                        '--build-directory',
                        build,
                        '--output',
                        folder / 'immature-income-reorganization',
                    ],
                )
                run(
                    variant + '-payment-recovery',
                    'pool.qualification.payment_matrix',
                    [
                        '--fixture',
                        fixture,
                        '--build-directory',
                        build,
                        '--output',
                        folder / 'payment-recovery',
                    ],
                )
                run(
                    variant + '-payment-reorganization',
                    'pool.qualification.payment_reorg',
                    [
                        '--fixture',
                        fixture,
                        '--build-directory',
                        build,
                        '--output',
                        folder / 'payment-reorganization',
                    ],
                )
                run(
                    variant + '-deep-reorganization-refusal',
                    'pool.qualification.payment_reorg',
                    [
                        '--fixture',
                        fixture,
                        ...
                        build,
                        '--output',
                        folder / 'deep-reorganization-refusal',
                        '--deep',
                    ],
                )
            ...
            for variant in ('existing', 'candidate'):
                run(
                    ...
                    [
                        ...
                        history['comining'],
                        '--output',
                        out / variant / 'comining',
                    ],
                )
                run(
                    variant + '-comining-reorganization',
                    'pool.qualification.comining',
                    [
                        '--build-directory',
                        out / 'build' / variant,
                        ...
                        history['comining'],
                        '--output',
                        out / variant / 'comining-reorganization',
                        '--reorganization',
                    ],
                )
            run(
                'funded-first-validator',
                'pool.qualification.validator',
                [
                    '--build-directory',
                    out / 'build/candidate',
                    '--history-state',
                    history['validator'],
                    '--output',
                    out / 'candidate/validator',
                ],
            )
            run(
                'funded-validator-floor-matrix',
                'pool.qualification.validator_matrix',
                [
                    '--build-directory',
                    out / 'build/candidate',
                    '--history-state',
                    history['finality'],
                    '--output',
                    out / 'candidate/validator-floor-matrix',
                ],
            )
            run(
                'funded-validator-full-lifecycle',
                'pool.qualification.validator_lifecycle',
                [
                    '--build-directory',
                    out / 'build/candidate',
                    '--matrix-output',
                    out / 'candidate/validator-floor-matrix',
                    '--output',
                    out / 'candidate/validator-lifecycle',
                ],
                86400,
            )
            run(
                'native-finality-pool-carrier',
                'pool.qualification.finality',
                [
                    '--build-directory',
                    out / 'build/candidate',
                    '--lab-signer',
                    out / 'build/candidate/pool-lab-sign',
                    '--history-state',
                    history['finality'],
                    '--output',
                    out / 'candidate/finality',
                ],
                86400,
            )
        report['source_unchanged'] = before == hashes()
        if not report['source_unchanged']:
            raise RuntimeError(
                'source changed during qualification; exact final candidate not qualified'
            )
        ...
        if args.through != 'complete':
            report['blocked'].insert(
                0, 'development stop point selected; later connected stages not executed'
            )
        ...
        save()
    print(
        'Connected Linux qualification finished; see the exact-source platform and artifact requirements.',
        flush=True,
    )
