...
def main():
    ...
    public=subprocess.check_output([str(args.build_directory/'pool-lab-keys'),str(state/'keys')],text=True)
    ...
    report=dict(status='RUNNING',scope='native canonical history read/commit interleaving',
                complete_pool_gate=False,argv=command,binary_sha256=hashlib.sha256(binary.read_bytes()).hexdigest())
    try:
        ...
        if result.returncode:raise RuntimeError('native history publication regression failed')
