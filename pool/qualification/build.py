...
BASE = [
    ...
    '-DVELD_TEST_HOOKS',
    '-DVELD_DSTATE_QUALIFICATION',
    '-DVELD_ASERT_TESTCHAIN',
    '-DVELD_PROTOCOL_UPGRADE_TEST_HEIGHT=3840',
]
...
def hashes():
    return {
        ...
        if p.is_file() and '__pycache__' not in p.parts and p.suffix not in ('.pyc', '.exe', '.o')
    }
...
def main():
    ...
    if link.is_file():
        ...
        if len(location) > 3 and location[0].isalpha() and location[1:3] == ':/':
            ...
            if not metadata.is_dir():
                raise ValueError('existing Git metadata is unavailable')
            git_env.update(GIT_DIR=str(metadata), GIT_WORK_TREE=str(SOURCE))

    def git(*args):
        return subprocess.check_output(
            ['git', *args], cwd=SOURCE, env=git_env, text=True, timeout=60
        ).strip()

    try:
        dirty_status = git('-c', 'core.fsmonitor=false', 'status', '--short')
        ...
        dirty_status = (
            'UNAVAILABLE: Git status exceeded 60 seconds; clean provenance is NOT established'
        )
    report = {
        'scope': 'isolated native qualification only',
        'status': 'RUNNING',
    ...
    def run(name, command, timeout=1800):
        ...
        if result.returncode:
            raise RuntimeError(name + ' failed; see recorded log')
    ...
    try:
        for name, command in [
            ...
            ('library-packages', ['dpkg-query', '--show', 'libssl-dev', 'libleveldb-dev']),
        ]:
            run(name, command)
        run('pqc-provenance', [sys.executable, 'scripts/verify-pqc-provenance.py'])
        objects = []
        for index, line in enumerate(
            (SOURCE / 'vendor/pqc/provenance/release-c-sources.txt').read_text().splitlines()
        ):
            if not line or line.startswith('#'):
                continue
            obj = str(out / 'obj' / f'pqc-{index}.o')
            objects.append(obj)
            run(
                'pqc-' + str(index),
                [
        ...
        for variant, height in [('existing', 0), ('candidate', 9500)]:
            ...
            for name, source, stateful in [
                ('pool-work', 'src/veld-pool-work.cpp', False),
                ('pool-client', 'src/veld-pool-client.cpp', False),
                ('pool-identity', 'src/veld-pool-identity.cpp', False),
                ('pool-payout', 'src/veld-pool-payout.cpp', False),
                ('pool-lab-keys', 'tests/pool_lab_keys.cpp', False),
                ('pool-lab-sign', 'tests/pool_lab_sign.cpp', False),
                ('pool-backend', 'tests/pool_native_backend.cpp', True),
            ]:
                flags = BASE + ['-DVELD_VALIDATOR_REGISTRATION_FORK_TEST_HEIGHT=' + str(height)]
                if stateful:
                    flags += ['-DVELD_USE_LEVELDB', '-DVELD_LIGHT_VERIFY']
                binary = target / name
                run(
                    variant + '-' + name,
                    [
                ...
                report['binaries'][str(binary.relative_to(out))] = hashlib.sha256(
                    binary.read_bytes()
                ).hexdigest()
                save()
        ...
        run(
            ...
            [
                ...
                '-pthread',
                *BASE,
                '-DVELD_VALIDATOR_REGISTRATION_FORK_TEST_HEIGHT=9500',
                '-DVELD_USE_LEVELDB',
                '-DVELD_LIGHT_VERIFY',
                '-Iinclude',
                '-Ivendor/pqc',
                ...
                *objects,
                ...
                '-lcrypto',
                '-o',
                str(out / 'candidate/veld-node'),
            ],
        )
        report['binaries']['candidate/veld-node'] = hashlib.sha256(
            (out / 'candidate/veld-node').read_bytes()
        ).hexdigest()
        run(
            'native-history-race',
            [
                ...
                *BASE,
                ...
                '-Iinclude',
                '-Ivendor/pqc',
                'tests/pool_history_publication_race.cpp',
                *objects,
                '-lleveldb',
                '-lssl',
                '-lcrypto',
                '-o',
                str(out / 'candidate/history-race'),
            ],
        )
        report['binaries']['candidate/history-race'] = hashlib.sha256(
            (out / 'candidate/history-race').read_bytes()
        ).hexdigest()
        for name, source in [
            ('download-continuation', 'tests/pool_download_continuation_tests.cpp'),
            ...
            report['binaries']['candidate/' + name] = hashlib.sha256(
                binary.read_bytes()
            ).hexdigest()
        report['source_unchanged'] = before == hashes()
        if not report['source_unchanged']:
            raise RuntimeError('source changed while building; fresh build required')
        report['status'] = 'PASS'
