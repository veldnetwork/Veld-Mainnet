...
old = pathlib.Path(fixture['state_directory'])
state = pathlib.Path(tempfile.mkdtemp(prefix='veld-pool-capacity-', dir='/var/tmp'))
out = args.output.resolve()
out.mkdir(parents=True, exist_ok=False)
(out / 'state-directory.txt').write_text(str(state) + '\n')
subprocess.run(['ip', 'link', 'set', 'lo', 'up'], check=True)
for name in ('node', 'observer', 'coordinator', 'work-anchor'):
    shutil.copytree(old / name, state / name)
settings = {
    k: (
        str(state) + v[len(str(old)) :]
        if isinstance(v, str) and v.startswith(str(old) + '/')
        else v
    )
...
report = {
    'status': 'RUNNING',
    'scope': '300 seconds of two native TLS workers under concurrent native validation',
    'production_capacity_claim': False,
    'profile': 'isolated ASERT full VeldHash, historical clock, unchanged monetary rules',
    ...
    try:
        return (
            int(
                next(
                    l
                    for l in pathlib.Path(f'/proc/{p.pid}/status').read_text().splitlines()
...
try:
    ...
    ready(rpc, node)
    observer = start(
        'observer', [str(build / 'pool-backend'), str(state / 'observer'), '32371', '32372'], True
    )
    independent = Node(
    ...
    client = Client('https://localhost:32443', str(state / 'tls.crt'), fixture['genesis'])
    time.sleep(0.5)
    first = rpc.call('getblockcount')
    node.stdin.write('clock ' + str(1767225600 + (first + 1) * 180) + '\n')
    node.stdin.flush()
            ...
            [
                ...
                config(
                    name,
                    {
                        ...
                        'threads': threads,
                        'nonce_count': str(args.nonce_count),
                        'pause_ms': pause,
                    },
                ),
            ],
    ...
    while time.monotonic() - start_time < 300:
        ...
        if time.monotonic() >= next_sample:
            ...
            queue = health['verification_queue']
            assert queue['waiting'] <= queue['capacity']
            max_queue = max(max_queue, queue['waiting'])
            usage = sum(rss(p) for p in processes)
            peak = max(peak, usage)
            report['samples'].append(
                {
                    'seconds': round(time.monotonic() - start_time, 2),
                    'height': height,
                    'observer_height': independent.call('getblockcount'),
                    'process_rss_bytes': usage,
                    'waiting': queue['waiting'],
                    'status': health['status'],
                }
            )
            save()
            print('native load height', height, 'queue', queue['waiting'], flush=True)
    ...
    while time.monotonic() < until:
        if independent.call('getbestblockhash') == tip:
            break
        time.sleep(0.5)
    else:
        raise RuntimeError('independent observer did not catch up after sustained load')
    assert independent.call('getstatedigest') == rpc.call('getstatedigest'), (
        'independent state mismatch'
    )
    ...
    accepted = sum(int(m['accepted']) for m in metrics.values())
    ordered = sorted(latencies)
    report.update(
        status='PASS',
        starting_height=first,
        ending_height=end,
        elapsed_seconds=elapsed,
        verified_shares=accepted,
        observed_verified_shares_per_second=accepted / elapsed,
        rpc_samples=len(latencies),
        rpc_timeouts=faults,
        rpc_median_seconds=statistics.median(latencies),
        rpc_p95_seconds=ordered[int(0.95 * (len(ordered) - 1))],
        peak_service_and_worker_rss_bytes=peak,
        max_unverified_queue=max_queue,
        independent_tip_and_state_match=True,
        workers=metrics,
        limitations=[
            'one development host with concurrent compilation',
            '300-second observation, not maximum sustainable public capacity',
            'RSS sample excludes transient signer/verifier child processes',
        ],
    )
