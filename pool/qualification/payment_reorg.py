...
def main():
    ...
    parser.add_argument(
        '--deep',
        action='store_true',
        help='verify that consensus refuses a fork beyond its reorganization limit',
    )
    ...
    report = dict(
        status='RUNNING',
        deep=args.deep,
        scope='native payment/funding reorganization',
    ...
    def ipc(action, payload):
        with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as c:
            ...
            c.sendall(encode(dict(action=action, payload=payload)) + b'\n')
            with c.makefile('rb') as stream:
                response = decode(stream.readline(16385))
    ...
    def balances():
        return {
            n: ipc('account', dict(account=a['account'], token=a['view_token']))
            for n, a in accounts.items()
        }

    def wallets(rpc):
        return {
            n: sum(u['value_units'] for u in rpc.call('listunspent', fixture['addresses'][n]))
            for n in accounts
        }
    ...
    def signed():
        ...
        return [
            ...
            if (e := json.loads(line))['kind'] == 'payment_signed'
        ]
    ...
    try:
        node = start(
            'node', [str(build / 'pool-backend'), str(state / 'node'), '32601', '32602'], True
        )
        rpc = Node('http://127.0.0.1:32602', state / 'node/lab-rpc-token', fixture['genesis'])
        ready(rpc, node)
        fork = start(
            'fork', [str(build / 'pool-backend'), str(state / 'fork'), '32611', '32612'], True
        )
        alternate = Node('http://127.0.0.1:32612', state / 'fork/lab-rpc-token', fixture['genesis'])
        ready(alternate, fork)
        common = 0 if args.deep else rpc.call('getblockcount')
        check(
            'branches start at the intended shared ancestor',
            rpc.call('getblockhash', str(common)) == alternate.call('getblockhash', str(common)),
        )
        service = start(
            'payment-service', [sys.executable, '-m', 'pool.service', '--config', str(config)]
        )
        ...
        for _ in range(30):
            ...
            raise RuntimeError('initial payment confirmation deadline')
        ...
        check('one signed economic payment before fork', len(original_signed) == 1)
        stop(service)
        ...
        check(
            'competing branch excludes the original payment',
            all(v == 0 for v in wallets(alternate).values()),
        )
        ...
        if args.deep:
            ...
            while 'reorg_beyond_max_depth' not in (out / 'node.log').read_text():
                if time.monotonic() > deadline:
                    raise RuntimeError('native deep-fork rejection evidence deadline')
                time.sleep(0.2)
            check(
                'consensus refused the over-depth funding fork',
                rpc.call('getbestblockhash') == old_tip,
            )
            check(
                'independent competing branch retains its separate state',
                alternate.call('getbestblockhash') == competing_tip,
            )
            service = start(
                'deep-fork-recovery',
            ...
            time.sleep(2)
            check(
                'refused deep fork leaves original recipient balances unchanged',
                wallets(rpc) == expected,
            )
            check('refused deep fork creates no extra signed payment', signed() == original_signed)
            report.update(
                status='PASS',
                consensus_deep_reorg_refused=True,
                original_height=old_height,
                competing_height=alternate.call('getblockcount'),
                recipient_wallet_units=wallets(rpc),
                separate_fork_wallet_units=wallets(alternate),
                txid=original_signed[0]['txid'],
        ...
        while rpc.call('getbestblockhash') != competing_tip:
            if time.monotonic() > deadline:
                raise RuntimeError('normal P2P branch reorganization deadline')
            time.sleep(0.2)
        check(
            'normal P2P reorganization replaced the paid branch',
            rpc.call('getblockhash', str(old_height)) != old_tip,
        )
        check(
            'independent nodes agree immediately after reorganization',
            rpc.call('getstatedigest') == alternate.call('getstatedigest'),
        )
        service = start(
            'reorg-recovery', [sys.executable, '-m', 'pool.service', '--config', str(config)]
        )
        service_ready(service)
        for _ in range(30):
            current = balances()
            ...
            if (
                all(int(current[n]['paid_units']) == expected[n] for n in accounts)
                and wallets(rpc) == expected
            ):
                break
            time.sleep(0.5)
            mine(node, rpc)
        else:
            raise RuntimeError('same-transaction recovery deadline')
        ...
        while alternate.call('getbestblockhash') != rpc.call('getbestblockhash'):
            if time.monotonic() > until:
                raise RuntimeError('independent recovered payment deadline')
            time.sleep(0.2)
        ...
        stop(service)
        service = start(
            'restarted-recovery', [sys.executable, '-m', 'pool.service', '--config', str(config)]
        )
        service_ready(service)
        time.sleep(2)
        check(
            'another restart creates no additional signed economic payment',
        ...
        report.update(
            ...
            txid=original_signed[0]['txid'],
            signed_bytes_sha256=hashlib.sha256(
                bytes.fromhex(original_signed[0]['signed_hex'])
            ).hexdigest(),
            entitlement_units=expected,
            recipient_wallet_units=wallets(rpc),
            independent_wallet_units=wallets(alternate),
        )
        ...
        report['binary_sha256'] = {
            ...
            for n in ('pool-backend', 'pool-payout')
        }
        save()
