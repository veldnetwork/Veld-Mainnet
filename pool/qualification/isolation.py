    ...
    for name in ('uid_map', 'gid_map'):
        try:
            with open('/proc/self/' + name) as stream:
                rows = stream.read(4096).splitlines()
        except OSError:
            return False
        try:
            mappings = [tuple(map(int, row.split())) for row in rows]
            if not all(
                any(len(row) == 3 and row[0] <= identity < row[0] + row[2] for row in mappings)
                for identity in (0, 61241, 61242, 61243)
            ):
                return False
        except ValueError:
            return False
...
def require_isolated_network():
    ...
    links = json.loads(
        subprocess.run(
            ['ip', '-j', 'link', 'show'], capture_output=True, text=True, check=True
        ).stdout
    )
