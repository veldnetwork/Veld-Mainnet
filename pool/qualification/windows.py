...
if os.name != 'nt':
    raise SystemExit('native Windows execution is required')
...
for name in ('objects', 'bin', 'logs'):
    (O / name).mkdir()
C = args.toolchain.resolve()
env = dict(
    os.environ, PATH=str(C / 'bin') + os.pathsep + os.environ['PATH'], PYTHONDONTWRITEBYTECODE='1'
)
...
def run(name, command, limit=1800):
    ...
    step = dict(name=name, command=list(map(str, command)), status='RUNNING')
    ...
    step.update(status='PASS' if result.returncode == 0 else 'FAILED', exit_code=result.returncode)
    save()
    if result.returncode:
        raise RuntimeError(name + ' failed; see exact log')
...
try:
    ...
    for n, line in enumerate(
        (S / 'vendor/pqc/provenance/release-c-sources.txt').read_text().splitlines()
    ):
    ...
    run(
        'resources',
        [C / 'bin/llvm-windres.exe', '-I.', 'resources/veld-node.rc', '-O', 'coff', '-o', resource],
    )
    defines = [
        '-DVELD_MAINNET_POW',
    ...
    archives = [
        str(C / 'lib' / n)
        for n in ('libc++.a', 'libc++abi.a', 'libunwind.a', 'libssl.a', 'libcrypto.a')
    ]
    libraries = [
        '-lws2_32',
        '-ladvapi32',
    ...
    for name, path, extra, more in [
        ...
        (
            'Veld Pool Qualification.exe',
            'src/veld-node-gui.cpp',
            [
                '-DVELD_GUI_TEST_INSTANCE',
                '-DVELD_POOL_GUI_QUALIFICATION',
                '-D_WIN32_WINNT=0x0A00',
                '-DWINVER=0x0A00',
                '-mwindows',
                '-municode',
            ],
            objects + [str(resource)],
        ),
        ('panel-regression.exe', 'tests/pool_panel_redraw_tests.cpp', [], objects),
        ('diagnostics.exe', 'tests/pool_client_diagnostics_tests.cpp', [], []),
        (
            'worker-ownership.exe',
            'tests/pool_gui_worker_ownership_tests.cpp',
            ['-municode'],
            objects,
        ),
        ...
        report['binaries'][binary.relative_to(O).as_posix()] = hashlib.sha256(
            binary.read_bytes()
        ).hexdigest()
        save()
    ...
    run(
        ...
        [O / 'bin/worker-ownership.exe', O / 'bin/veld-pool-client.exe'],
        180,
    )
    run(
        'native-transport-faults',
        [
            sys.executable,
            '-m',
            'pool.test_client_transport',
            '--binary',
            O / 'bin/transport.exe',
            '--openssl',
            C / 'bin/openssl.exe',
    ...
    (O / 'failure.txt').write_text(traceback.format_exc())
    raise
finally:
    save()
