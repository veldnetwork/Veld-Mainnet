...
def main():
    ...
    parser.add_argument(
        '--systemd',
        action='store_true',
        help='execute packaged sandbox settings in uniquely named transient test units',
    )
    parser.add_argument(
        '--maturity-timeout',
        type=int,
    ...
    if not 3600 <= args.maturity_timeout <= 43200:
        raise ValueError('maturity deadline must be between one and twelve hours')
    if os.geteuid() != 0:
        raise PermissionError('root is required to exercise distinct service UIDs')
    if not os.environ.get(MARKER):
        env = dict(os.environ)
        env[MARKER] = os.readlink('/proc/self/ns/net')
        raise SystemExit(
            subprocess.call(
                [
                    'unshare',
                    ...
                    sys.executable,
                    '-m',
                    'pool.qualification.role_install',
                    *sys.argv[1:],
                ],
                env=env,
            )
    ...
    for uid in (core, gateway, worker_uid):
        try:
            ...
            raise RuntimeError('test UID already belongs to a system user')
    ...
    root = Path(
        tempfile.mkdtemp(
            prefix='veld-pool-install-', dir='/var/lib' if args.systemd else '/var/tmp'
        )
    )
    root.chmod(0o755)
    ...
    def module(name):
        spec = importlib.util.spec_from_file_location(
            'pool_setup_' + name, source / 'pkg/pool' / (name + '.py')
        )
    ...
    package.mkdir()
    for name in ('bin', 'lib/pool', 'setup', 'third-party'):
    ...
    for file in (source / 'pool').glob('*.py'):
        if not file.name.startswith('test_'):
            shutil.copy2(file, package / 'lib/pool' / file.name)
    ...
    licenses = [
        p
        for p in (source / 'vendor/pqc').rglob('*')
        if p.is_file() and p.name in ('LICENSE', 'LICENSE.txt', 'COPYING')
    ]
    for index, file in enumerate(licenses):
        shutil.copy2(file, package / 'third-party' / f'{index}-{file.name}')
    (package / 'third-party/QUALIFICATION.txt').write_text(
        'Isolated native installation exercise; not a release artifact.\n'
    )
    (package / 'pool-sha256.txt').write_text(
        ''.join(
    ...
    assert identity['disposable'] and not identity['external_value']
    ...
    config_root, state = root / 'config', root / 'state'
    config_root.mkdir(mode=0o700)
    private = config_root / 'private'
    private.mkdir(mode=0o700)
    for name in ('pool', 'fees'):
        shutil.copy2(root / 'keys' / (name + '.seed'), private / (name + '.seed'))
    ...
    with (out / 'certificate.log').open('w') as log:
        subprocess.run(
            [
                ...
                '-nodes',
                '-keyout',
                str(tls / 'private-key.pem'),
                '-out',
                str(tls / 'certificate.pem'),
                '-days',
                '1',
                '-subj',
                '/CN=localhost',
    ...
    report = dict(
        status='RUNNING',
        scope='native Linux role separation, clean staging and side-by-side restart',
        complete_pool_gate=False,
        real_system_users_created=False,
        systemd_units_activated=args.systemd,
    ...
    def start(name, command, uid=0, cwd=source, stdin=False):
        ...
        if args.systemd and role in ('backend', 'coordinator', 'gateway'):
            ...
            text = installer.unit(role, cwd.parent, config_root, state, str(core), str(gateway))
            for other in ('backend', 'coordinator', 'gateway'):
                text = text.replace(
                    'veld-pool-' + other + '.service',
                    unit_prefix + '-' + other + '-v' + name.rsplit('v', 1)[1] + '.service',
                )
            properties = []
            section = ''
            ...
            subprocess.run(
                [
                    ...
                    *properties,
                    ...
                    '-m',
                    'pool.'
                    + {'backend': 'node_service', 'coordinator': 'service', 'gateway': 'gateway'}[
                        role
                    ],
                    '--config',
                    str(config_root / (role + '.json')),
                ],
                stdout=log,
                stderr=subprocess.STDOUT,
                check=True,
        ...
        p = subprocess.Popen(
            ...
            user=uid,
            group=uid,
            extra_groups=[gateway] if uid == core else [],
            umask=0o077,
    ...
    class UnitProcess:
        ...
        def __init__(self, unit, log):
            self.unit, self.log, self.returncode = unit, log, None
        ...
        def poll(self):
            ...
            if values.get('ActiveState') in ('active', 'activating', 'reloading', 'deactivating'):
                return None
            self.returncode = int(values.get('ExecMainStatus', '1' if result.returncode else '0'))
            return self.returncode

        def terminate(self):
            subprocess.run(
                ['systemctl', 'stop', self.unit], capture_output=True, check=True, timeout=180
            )

        def kill(self):
            subprocess.run(
                ['systemctl', 'kill', '--kill-whom=all', self.unit], capture_output=True, timeout=30
            )
    ...
    try:
        if args.systemd:
            ...
            for role in ('core', 'gateway'):
                ...
                unit = unit_prefix + '-' + role + '-identity.service'
                units.append(unit)
                ...
                subprocess.run(
                    [
                        ...
                        '/usr/bin/sleep',
                        str(identity_lease_seconds(args.maturity_timeout)),
                    ],
                    capture_output=True,
                    check=True,
                    timeout=30,
                ...
                allocated.append(account.pw_uid)
            core, gateway = allocated
            check(
                'temporary systemd identities are distinct and non-root',
                0 not in allocated and len(set(allocated)) == 2,
            )
            report['temporary_systemd_uids'] = allocated
        observer = start(
            'observer',
            [str(build / 'pool-backend'), str(root / 'observer'), '32111', '32112'],
            stdin=True,
        )
        independent = Node('http://127.0.0.1:32112', root / 'observer/lab-rpc-token', genesis)
        ready(independent, observer)
        prior_account = None
        prior_paid = 0
        for version in (1, 2, 3):
            prefix = root / ('installed-v' + str(version))
            installer.stage(
                package, prefix, config_root, state, 'veld-test-core', 'veld-test-gateway'
            )
            values = dict(
                prefix=str(prefix),
                state=str(state),
            ...
            backend = start(
                'backend-v' + str(version),
                [
                    sys.executable,
                    '-m',
                    'pool.node_service',
                    '--config',
                    str(config_root / 'backend.json'),
                ],
                core,
                prefix / 'lib',
            )
            rpc = Node('http://127.0.0.1:32102', state / 'rpc/token', genesis)
            ready(rpc, backend)
            if version == 1:
                ...
                with (out / 'operator-fee-work.log').open('w') as log:
                    ...
                    try:
                        candidate = rpc.template(addresses['fees'])
                        header = bytearray.fromhex(candidate['block_hex'])[:88]
                        for nonce in range(4096):
                            header[80:88] = nonce.to_bytes(8, 'little')
                            proof, _ = verifier.hash(bytes(header), candidate['height'])
                            if int(proof, 16) < int(candidate['target'], 16):
                                check(
                                    'operator fee float uses a separately earned canonical output',
                                    rpc.submit(candidate, nonce)['accepted'],
                                )
                                break
                        else:
                            raise RuntimeError('native operator fee funding deadline')
                    finally:
                        verifier.close()
            coordinator = start(
                'coordinator-v' + str(version),
                [
                    sys.executable,
                    '-m',
                    'pool.service',
            ...
            while not (state / 'ipc/coordinator.sock').exists():
                ...
                if time.monotonic() > until:
                    raise RuntimeError('owned IPC startup deadline')
                time.sleep(0.1)
            info = (state / 'ipc/coordinator.sock').stat()
            check(
                'v' + str(version) + ' IPC owner/group/mode enforce role access',
                info.st_uid == core and info.st_gid == gateway and info.st_mode & 0o777 == 0o660,
            )
            public = start(
                'gateway-v' + str(version),
                [
                    sys.executable,
                    '-m',
                    'pool.gateway',
                    '--config',
                    str(config_root / 'gateway.json'),
                ],
                gateway,
                prefix / 'lib',
            )
            ...
            denied = [
                ...
                private / 'pool.seed',
                private / 'fees.seed',
                config_root / 'coordinator.json',
            ]
            check_program = 'import sys\nfor p in sys.argv[1:]:\n try:\n  f=open(p,"rb");f.close();raise SystemExit(2)\n except PermissionError:pass\n'
            check(
                'v' + str(version) + ' gateway cannot read RPC or signing authority',
                subprocess.run(
                    [sys.executable, '-c', check_program, *map(str, denied)],
                    user=gateway,
                    group=gateway,
                    extra_groups=[],
                    capture_output=True,
            ...
            worker_root.mkdir(exist_ok=True, mode=0o700)
            os.chown(worker_root, worker_uid, worker_uid)
            shutil.copy2(tls / 'certificate.pem', worker_root / 'ca.pem')
            os.chown(worker_root / 'ca.pem', worker_uid, worker_uid)
            worker_config = worker_root / 'config.json'
            worker_config.write_bytes(
                encode(
                    dict(
                        endpoint='https://localhost:32143',
                        ca_file=str(worker_root / 'ca.pem'),
                        genesis=genesis,
                        payout_address=addresses['worker-a'],
                        state_directory=str(worker_root / 'account'),
                        threads='1',
                        nonce_count='8',
                        pause_ms='0',
                    )
            ...
            os.chown(worker_config, worker_uid, worker_uid)
            ...
            first = rpc.call('getblockcount')
            worker = start(
                'worker-v' + str(version),
                [str(prefix / 'bin/veld-pool-client'), '--config', str(worker_config)],
                worker_uid,
                prefix / 'lib',
            )
            ...
            while True:
                assert all(p.poll() is None for p in (backend, coordinator, public, worker)), (
                    'installed service exited'
                )
                height = rpc.call('getblockcount')
                status_path = worker_root / 'account/pool-status.json'
                try:
                    current = json.loads(status_path.read_text())
                except (OSError, json.JSONDecodeError):
                    current = {}
                if height >= first + (124 if version == 2 else 2) and (
                    version != 2 or int(current.get('paid_units', '0')) > 0
                ):
                    break
                if time.monotonic() > until:
                    raise RuntimeError('installed native worker deadline')
                if time.monotonic() >= next_progress:
                    print('installed worker v' + str(version) + ' height', height, flush=True)
            ...
            check(
                'v' + str(version) + ' installed worker mined accepted native blocks',
                int(status['accepted']) > 0,
            )
            if prior_account:
                check(
                    'upgrade preserved worker account and earned balances',
                    status['account'] == prior_account,
                )
            prior_account = status['account']
            tip = rpc.call('getbestblockhash')
            until = time.monotonic() + 180
            while independent.call('getbestblockhash') != tip:
                if time.monotonic() > until:
                    raise RuntimeError('independent installed-block validation deadline')
                time.sleep(0.2)
            check(
                'v' + str(version) + ' independently validated installed service state',
                independent.call('getstatedigest') == rpc.call('getstatedigest'),
            )
            if version >= 2:
                amount = int(status['paid_units'])
                actual = sum(
                    c['value_units'] for c in rpc.call('listunspent', addresses['worker-a'])
                )
                external = sum(
                    c['value_units'] for c in independent.call('listunspent', addresses['worker-a'])
                )
                check(
                    'v'
                    + str(version)
                    + ' installed signer paid exactly the recorded mature liability',
                    amount > 0 and actual == external == amount,
                )
                if version == 3:
                    check(
                        'post-payment upgrade did not repeat the economic payment',
                        amount == prior_paid,
                    )
                prior_paid = amount
                report['recipient_wallet_units'] = actual
                report['independent_wallet_units'] = external
            stop(public)
            stop(coordinator)
            stop(backend)
            check(
                'v' + str(version) + ' ordered shutdown removed API capabilities',
                not (state / 'rpc/token').exists()
                and not (state / 'ipc/coordinator.sock').exists(),
            )
        report.update(
            status='PASS',
            ...
            limitations=(
                []
                if args.systemd
                else ['systemd sandbox directives are staged but not activated in this namespace']
            )
        ...
        report['package_manifest_sha256'] = hashlib.sha256(
            (package / 'pool-sha256.txt').read_bytes()
        ).hexdigest()
        save()
