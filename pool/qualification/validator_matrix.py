...
def main():
    ...
    history = args.history_state.resolve()
    ...
    report = dict(
        status='RUNNING',
        scope='native funded former-floor matrix',
    ...
    def start(name, folder, p2p, port):
        ...
        logpaths[proc.pid] = out / (name + '.log')
        ...
        while True:
            ...
            try:
                ...
                if time.monotonic() > until:
                    raise TimeoutError('native replay readiness')
                time.sleep(0.25)
    ...
    def mine(proc, rpc):
        return mine_block(proc, rpc, addresses['fees'], logpaths[proc.pid])

    def sign(prepared, key):
        answer = (
            subprocess.run(
                [str(build / 'pool-lab-sign'), str(key), 'transaction'],
                input=encode(
                    dict(
    ...
    try:
        ...
        assert rpc.call('getblockcount') >= 9500, (
            'history must be after the isolated candidate boundary'
        )
        ...
        for attempt in range(1000):
            try:
                probe = rpc.call(
                    'preparerawtransaction', addresses['pool'], addresses['worker-a'], '25000'
                )
                if len(probe['inputs']) <= 128:
                    break
            ...
            pay(
                producer,
                rpc,
                rpc.call('prepareconsolidatetx', addresses['pool'], '128', '0'),
                state / 'keys/pool.seed',
            )
        else:
            raise TimeoutError('bounded genuine bond funding consolidation')
        report['funding_tip'] = rpc.call('getbestblockhash')
        report['funding_height'] = rpc.call('getblockcount')
        save()
        stop(producer)
        for name, stakes in [
            ('zero', ()),
            ...
            save()
            folder = state / name
            shutil.copytree(state / 'funding', folder / 'node')
            ...
            for index, amount in enumerate(stakes):
                ...
                subprocess.run(
                    [str(build / 'pool-lab-keys'), str(keydir)], capture_output=True, check=True
                )
                key = keydir / 'pool.seed'
                address, pubkey, script = subprocess.run(
                    [str(build / 'pool-lab-sign'), str(key), 'identity'],
                    capture_output=True,
                    text=True,
                    check=True,
                ).stdout.split()
                pay(
                    ...
                    rpc,
                    rpc.call(
                        'preparerawtransaction', addresses['pool'], address, str(amount) + '.1'
                    ),
                    state / 'keys/pool.seed',
                )
            ...
            record(
                case,
                'actual ordinary stake equals requested aggregate',
                Fraction(str(rpc.call('getvalidators')['total_staked_veld'])) == sum(stakes),
            )
            address = addresses['pool']
            key = state / 'keys/pool.seed'
            _, pubkey, _ = subprocess.run(
                [str(build / 'pool-lab-sign'), str(key), 'identity'],
                capture_output=True,
                text=True,
                check=True,
            ).stdout.split()
            before = funds(rpc, address)
            prepared = rpc.call('prepareregistervalidator', address, pubkey)
            registration = pay(producer, rpc, prepared, key)
            height = rpc.call('getblockcount')
            record(
                case,
                'funded individual bond registers regardless of former aggregate floor',
                rpc.call('getvalidatorinfo', pubkey)['registered']
                and rpc.call('getvalidators')['validator_count'] == 1,
            )
            record(
                case,
                'individual bond and transaction fee charged exactly once',
                before - funds(rpc, address) == 10000 * 100000000 + 100000,
            )
            record(
                case,
                'registration alone produces no endorsement or automatic payment',
                rpc.call('getblockendorsements', str(height))['count'] == 0,
            )
            record(
                case,
                'bond does not alter ordinary stake',
                Fraction(str(rpc.call('getvalidators')['total_staked_veld'])) == sum(stakes),
            )
            ...
            record(
                case,
                'custody principal is attributable to this validator',
                len(bond) == 1 and bond[0]['bond_veld'] == 10000 and bond[0]['principal_held'],
            )
            block = rpc.call('getblockhash', str(height))
            grant = rpc.call('getworkadmission', 'validator_endorsement', str(height), block)
            rpc.call(
                'beginworksigning',
                'validator_endorsement',
                grant['binding'],
                grant['signing_token'],
            )
            op = (
                subprocess.run(
                    [str(build / 'pool-lab-sign'), str(key), 'endorse'],
            ...
            assert (
                rpc.call('sendrawtransaction', raw, grant['binding'], grant['signing_token'])
                == txid
            )
            mine(producer, rpc)
            record(
                case,
                'one validator performs eligible endorsement without a finality set',
                rpc.call('getblockendorsements', str(height))['count'] == 1,
            )
            snapshot = rpc.call('getfinalitysnapshot').get('snapshot')
            record(
                case,
                'finality quorum and governance gates remain closed',
                not (snapshot and snapshot['active'])
                and not rpc.call('getgovernanceinfo')['governance_active'],
            )
            ...
            record(
                case,
                'restart preserves canonical bond stake and endorsement accounting',
            ...
            record(
                case,
                'independent node validates identical state and owned principal',
                independent.call('getstatedigest') == digest
                and independent.call('getbondvaultinfo') == rpc.call('getbondvaultinfo'),
            )
            case.update(
                status='PASS',
                registration_txid=registration,
                endorsement_txid=txid,
                height=rpc.call('getblockcount'),
                tip=tip,
                independent_state=digest,
            )
            save()
            stop(observer)
            stop(producer)
