...
keys = subprocess.run(
    [str(build / 'pool-lab-keys'), str(state / 'disposable-keys')],
    capture_output=True,
    text=True,
    check=True,
)
...
if args.shared_destination:
    addresses['worker-b'] = addresses['worker-a']
    scripts['worker-b'] = scripts['worker-a']
...
def services(generation):
    ...
    for name, threads, count, pause in [('worker-a', 2, 4, 30), ('worker-b', 1, 2, 120)]:
        ...
        workers.append(
            start(name + '-' + str(generation), [str(build / 'pool-client'), '--config', path])
        )
...
def accounts():
    ...
    for name in ('worker-a', 'worker-b'):
        ...
        result[name] = client.call(
            'account', {'account': data['account'], 'token': data['view_token']}
        )
    return result
...
report = {
    'status': 'RUNNING',
    'scope': 'native worker-to-wallet including restart; intermediate gate',
    'native_worker_executable': str(build / 'pool-client'),
...
try:
    ...
    with (out / 'fee-mining.log').open('w') as log:
        ...
        try:
            template = backend.template(addresses['fees'])
            ...
            for nonce in range(1000):
                ...
                if int(proof, 16) < int(template['target'], 16):
                    assert backend.submit(template, nonce)['accepted']
                    break
            else:
                raise RuntimeError('fee funding work bound')
        finally:
            verifier.close()
    ...
    for _ in range(100):
        ...
        try:
            independent.check_chain()
            break
        except (OSError, ValueError):
            time.sleep(0.1)
    else:
        raise RuntimeError('observer startup timeout')
    observer.stdin.write('peer 32361\n')
    observer.stdin.flush()
    settings = {
        'rpc_url': 'http://127.0.0.1:32362',
    ...
    while time.monotonic() < deadline:
        assert all(p.poll() is None for p in [coordinator, gateway, *workers]), (
            'service exited during mining'
        )
        ...
        if height != last:
            ...
            last = height
            if height % 10 == 0:
                print('maturity mining height', height, flush=True)
        if height >= 140:
            break
        time.sleep(0.15)
    else:
        raise RuntimeError('maturity timeout')
    ...
    before = accounts()
    (out / 'before-payments.json').write_text(json.dumps(before, indent=2) + '\n')
    assert all(int(a['available_units']) >= 100000000 for a in before.values()), (
        'both miners need mature balances'
    )
    for _ in range(240):
        if independent.call('getblockcount') == backend.call('getblockcount') and independent.call(
            'getbestblockhash'
        ) == backend.call('getbestblockhash'):
            break
        time.sleep(0.5)
    else:
        raise RuntimeError('independent fixture validation timeout')
    stop(gateway)
    stop(coordinator)
    fixture = {
        'settings': settings,
    ...
    report.update(
        status='PREPARED',
        height=height,
        scope='native mature pool-income fixture before any payout',
