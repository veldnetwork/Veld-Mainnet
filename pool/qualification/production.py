...
def verify_deployment_version(source, value):
    ...
    return verify(
        (source / 'include/core/version.h').read_text(encoding='utf-8'), json.dumps(value)
    )
...
def main():
    ...
    if os.name == 'nt':
        ...
        env['PATH'] = (
            str(args.toolchain / 'clang64/bin')
            + os.pathsep
            + str(args.toolchain / 'usr/bin')
            + os.pathsep
            + env['PATH']
        )
    ...
    try:
        if os.name == 'nt':
            ...
            for role in ('node', 'desktop', 'gui'):
                extra = (
                    ...
                    if role != 'gui'
                        ...
                        VELD_TRUSTED_NODE_BUILD=msys(output / 'node'),
                ...
                run(
                    ...
                    [
                        ...
                        'exec bash "$1" "$2" "$3"',
                        'production-build',
                        msys(SOURCE / 'build/mainnet-v2-windows.sh'),
                        role,
                        msys(output / role),
                    ],
                    extra,
            ...
            if args.component == 'pool':
                ...
                run(
                    ...
                    [
                        'bash',
                        SOURCE / 'build/mainnet-v2-linux.sh',
                        args.component,
                        output / args.component,
                    ],
                )
            binaries = (
                []
                if args.component == 'keygen'
                else [
                    output
                    / (
                        'pool/bin/veld-node'
                        if args.component == 'pool'
                        else 'fleet/bin/veld-node-fleet-no-mine'
                    )
                ]
            )
        deployments = {}
        for binary in binaries:
            ...
            values = [
                json.loads(line.removeprefix('VELD_DEPLOYMENT_INFO_V1_JSON '))
            ...
            assert (
                value['amm_flat_fee_activation_height'] == 9500 and value['amm_flat_fee_bps'] == 30
            )
