...
class MaintenanceTests(unittest.TestCase):
    def exercise(self,failed):
        ...
        pool=SimpleNamespace(lock=threading.RLock(),retry_deferred=action('work'),reconcile_solutions=action('solutions'),
            rewards=SimpleNamespace(reconcile=action('rewards')),reconcile_income=action('income'),
        ...
        maintain_once(pool,SimpleNamespace(check_chain=action('chain'),require_transaction_index=action('index')),threading.Event())
        return calls,pool
    ...
    def test_index_loss_pauses_identity_and_payments_without_stopping_work(self):
        ...
        for name in ('work','solutions','rewards','income'):self.assertIn(name,calls)
        for name in ('identity','sign','plan','reconcile'):self.assertNotIn(name,calls)
        self.assertEqual(pool.deferred_phases,['transaction index readiness'])
        calls,pool=self.exercise(set())
        for name in ('identity','sign','plan','reconcile'):self.assertIn(name,calls)
        self.assertEqual(pool.deferred_phases,[])

    def test_unavailable_index_refuses_startup_before_opening_economic_state(self):
        config={name:'fixture' for name in ('rpc_url','rpc_token_file','genesis','state_directory',
            'rollback_anchor','native_binary','pool_address','accounting_target','socket')}
        for enabled in ('identity','payments'):
            with self.subTest(enabled=enabled),patch('sys.argv',['service','--config','unused']),\
                 patch('pool.service.read_private',return_value=b'{}'),\
                 patch('pool.service.decode',return_value=dict(config,**{enabled:{}})),\
                 patch('pool.service.Node') as node,patch('pool.service.Journal') as journal,\
