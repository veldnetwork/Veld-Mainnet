...
@unittest.skipUnless(
    service_identities_available(),
    'requires mapped Linux service UIDs, not single-UID namespace root',
)
    ...
    def test_installed_admin_cannot_read_keys_and_gateway_cannot_reach_control_socket(self):
        ...
        for uid in (core, gateway, admin):
            try:
                ...
                self.fail(
                    'qualification UID is already assigned; select a different unused fixture range'
                )
        ...
        with tempfile.TemporaryDirectory(prefix='veld-admin-roles-') as directory:
            ...
            for name, data in configs.items():
                (config / (name + '.json')).write_bytes(encode(data))
            provision.layout(config, state, core, core, gateway, gateway, admin, admin)
            secret = config / 'private/pool.seed'
            secret.write_bytes(b'fixture-not-a-real-key')
            os.chown(secret, core, core)
            secret.chmod(0o600)
            password = 'separate-role-disposable-panel-passphrase'
            access = config / 'admin/access.json'
            access.write_bytes(encode(password_record(password)))
            subprocess.run(
                [
                    'openssl',
                    'req',
                    ...
                    '-keyout',
                    str(config / 'admin/private-key.pem'),
                    '-out',
                    str(config / 'admin/certificate.pem'),
                ],
                check=True,
                capture_output=True,
            )
            ...
            try:
                for name, argv, uid in [
                    ...
                    if name == 'core':
                        ...
                        while True:
                            self.assertIsNone(
                                processes[-1].poll(), 'private core exited during startup'
                            )
                            try:
                                with socket.socket(socket.AF_UNIX) as probe:
                                    probe.settimeout(0.2)
                                    probe.connect(configs['admin']['operator_socket'])
                                break
                            except OSError:
                                if time.monotonic() > deadline:
                                    self.fail('private core IPC readiness deadline')
                                time.sleep(0.02)
                context = ssl.create_default_context(cafile=str(config / 'admin/certificate.pem'))
                origin = 'https://127.0.0.1:' + str(port)
                deadline = time.monotonic() + 10
                while True:
                    try:
                        c = http.client.HTTPSConnection(
                            '127.0.0.1', port, context=context, timeout=1
                        )
                        c.request(
                            'POST',
                            '/api/login',
                ...
                value = dict(
                    request_id='a' * 32,
                    revision='0',
                    reason='Separate process role integration',
                    settings=dict(
                        minimum_units='200000000',
                        batch_seconds='86400',
                        fee_ppm='0',
                        payments_paused=True,
                        comining_paused=True,
                    ),
                )
                ...
                c.close()
                self.assertEqual(r.status, 200)
                self.assertEqual(reply['result']['status'], 'applied')
                for uid, path in (
                    (admin, secret),
                    (gateway, secret),
                    (gateway, access),
                    (admin, state / 'core/work/events.jsonl'),
                ):
                    ...
                    self.assertIn(b'PermissionError', check.stderr)
                check = subprocess.run(
                    [
                        sys.executable,
                        '-c',
