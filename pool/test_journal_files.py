...
class JournalFiles(unittest.TestCase):
    ...
    def test_public_or_symlinked_journal_directory_is_refused(self):
        with tempfile.TemporaryDirectory() as directory:
            root=Path(directory);unsafe=root/'unsafe';unsafe.mkdir(mode=0o755)
            with self.assertRaises(Refused):Journal(unsafe,root/'anchors/current')
            safe=root/'safe';safe.mkdir(mode=0o700);(root/'link').symlink_to(safe)
            with self.assertRaises(Refused):Journal(root/'link',root/'anchors/current')
