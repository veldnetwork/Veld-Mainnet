...
INTERLEAVED='  [rpc] sendrawtransaction REJECTED: utxo_missing_input_0 (txid=9eccMINED 3660 '+BLOCK+'\na133fe1c79ab...)\n'

class Fixture:
    def __init__(self,root,mode='mined'):
        self.args=['pool-backend',str(root),'30001','30002']
        self.root=root;self.mode=mode;self.commands=[];self.buffer=''
        self.stdin=self;self.height=3659
        self.log=root/'node.log';self.log.write_text('')
    def poll(self):return None
    def write(self,value):self.buffer+=value
    def flush(self):
        for command in self.buffer.splitlines():
            if not command.startswith('mine '):continue
            self.commands.append(command)
            fields=command.split();request=fields[-1]
            with self.log.open('a') as log:log.write(INTERLEAVED)
            if self.mode=='missing':continue
    ...
    def call(self,method,*args):
        if method=='getblockcount':return self.height
...
class ControlTests(unittest.TestCase):
    ...
    def test_log_alone_is_not_acknowledgement_and_never_queues_a_second_command(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with self.assertRaisesRegex(RuntimeError,'acknowledgement deadline'):
                mine_block(fixture,fixture,'fixture-address',fixture.log,timeout=.02)
            self.assertEqual(len(fixture.commands),1)
    ...
    def test_replaced_open_receipt_retries_without_enqueuing_more_work(self):
        with tempfile.TemporaryDirectory() as temp:
            fixture=Fixture(Path(temp));path=fixture.root/'lab-mining-ack.txt'
            real_open=os.open;replaced=[]
    ...
    def test_hardlinked_receipt_is_still_refused(self):
        with tempfile.TemporaryDirectory() as temp:
            path=Path(temp)/'lab-mining-ack.txt';identity='1'*32
    ...
    def test_symlink_receipt_is_still_refused(self):
        with tempfile.TemporaryDirectory() as temp:
            target=Path(temp)/'target';identity='1'*32
            target.write_text('V1 '+identity+' MINED 3660 '+BLOCK+'\n')
            path=Path(temp)/'lab-mining-ack.txt';path.symlink_to(target)
            with self.assertRaises(OSError):acknowledgement(path,identity)
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            for text in ['x'*257+'\n','V1 '+identity+' MINED 3660 '+BLOCK,
                ...
                with self.subTest(text=text[:40]):
                    ...
                    with self.assertRaises(RuntimeError):acknowledgement(path,identity)
