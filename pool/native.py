...
class Native:
    ...
    def _start(self):
        ...
        if len(self.starts) >= 3: raise Busy('native restart limit; retry later')
        self.starts.append(now)
        process = subprocess.Popen([self.binary], stdin=subprocess.PIPE, stdout=subprocess.PIPE,
    ...
    def request(self, line, timeout=10):
        require(isinstance(line,str) and len(line)<=512 and '\n' not in line and '\r' not in line, 'native request frame')
        if not self.lock.acquire(blocking=False): raise Busy('native verifier occupied')
        try:
            if self.closed: raise Busy('native verifier stopped')
            ...
            try: result = self.lines.get(timeout=timeout)
            except queue.Empty:
                self._stop()
    ...
    def inspect(self, header, height):
        ...
        require(len(result) == 4 and result[3] in ('block','near_miss','none'), 'native proof schema')
