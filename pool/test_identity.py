...
class IdentityTests(unittest.TestCase):
    def setUp(self):
        ...
        self.identity = Identity(
            self.pool, self.journal, 'native-identity', 'seed', 'script', self.funding
        )
    ...
    def call(self, method, *args):
        ...
        if method in ('gettransaction', 'getrawtransaction'):
            if args[0] not in self.txs:
                raise Refused('not included')
            return self.txs[args[0]]
        if method == 'getblockhash':
            return self.state['parent']
        if method == 'sendrawtransaction':
            self.raw.append(args[0])
            return hashlib.sha256(hashlib.sha256(bytes.fromhex(args[0])).digest()).hexdigest()
    ...
    def test_unlisted_coins_and_active_principal_are_never_selected(self):
        ...
        self.coins = [
            {'txid': 'e' * 64, 'vout': 0, 'value_units': 100000000000},
            {'txid': 'e' * 64, 'vout': 1, 'value_units': 1234567},
        ]
        self.assertEqual(
            self.identity.allowed(), [{'txid': 'e' * 64, 'vout': '1', 'units': '1234567'}]
        )
    ...
    def test_signed_bytes_reserved_before_broadcast_and_identical_retry(self):
        raw = b'focused-fixture'
        txid = hashlib.sha256(hashlib.sha256(raw).digest()).hexdigest()
        intent = {
            'id': 'i',
            'state': 'reserved',
            'parent': 'b' * 64,
            'wire': {'action': 'consolidate', 'inputs': self.funding},
        }
    ...
    def test_included_entry_and_pending_entry_do_not_sign_another(self):
        ...
        with patch.object(
            self.identity, 'sign', side_effect=AssertionError('unexpected signature')
        ):
    ...
    def test_slow_identity_lookup_does_not_hold_worker_lock(self):
        ...
        worker = threading.Thread(target=self.identity.maintain)
        worker.start()
        try:
            self.assertTrue(waiting.wait(2))
            acquired = self.pool.lock.acquire(timeout=0.1)
