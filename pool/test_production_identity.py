...
class ProductionIdentityTests(unittest.TestCase):
    def test_new_source_version_is_supported_and_mismatched_binary_is_refused(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            shutil.copy2(
                SOURCE / 'scripts/verify-release-version.py',
                source / 'scripts/verify-release-version.py',
            )
            ...
            self.assertEqual(
                verify_deployment_version(source, dict(client_version='9.8.7')), '9.8.7'
            )
            ...
            with self.assertRaises(ValueError):
                verify_deployment_version(source, dict(client_version='9.8.7'))
