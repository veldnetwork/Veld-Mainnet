...
namespace veld { namespace rpc_proxy {
namespace detail {
...
public:
    ...
    bool Parse(std::string& method) {
        method.clear();
        if (input_.empty() || input_.size() > kMaxBytes) return false;
        SkipWhitespace();
        if (!ParseObject(/*depth=*/1, /*root=*/true, method)) return false;
        SkipWhitespace();
...
private:
    ...
    // large allocation.
    bool ParseString(std::string* captured, bool& literal, bool& overflow,
                     size_t capture_limit) {
        if (!Consume('"')) return false;
        literal = true;
        overflow = false;
        if (captured) captured->clear();
        while (pos_ < input_.size()) {
            ...
            if (c == '\\') {
                ...
                if (e == 'u') {
                    ...
                    for (int i = 0; i < 4; ++i)
                        if (!HexDigit(input_[pos_++])) return false;
                } else if (e != '"' && e != '\\' && e != '/' &&
                           e != 'b' && e != 'f' && e != 'n' &&
                           e != 'r' && e != 't') {
                    return false;
    ...
    bool ParseValue(size_t depth) {
        if (++nodes_ > kMaxNodes || depth > kMaxDepth) return false;
        SkipWhitespace();
        if (pos_ >= input_.size()) return false;
        const char c = input_[pos_];
        if (c == '{') {
        ...
        if (c == 't') return ConsumeLiteral("true", 4);
        if (c == 'f') return ConsumeLiteral("false", 5);
        if (c == 'n') return ConsumeLiteral("null", 4);
        return ParseNumber();
    }
    ...
    bool ParseObject(size_t depth, bool root, std::string& method) {
        ...
        for (;;) {
            ...
            if (!ParseString(&key, key_literal, key_overflow, 16)) return false;
            SkipWhitespace();
            if (!Consume(':')) return false;
            SkipWhitespace();
            ...
            if (is_method) {
                if (method_seen_) return false; // reject duplicate authorization keys
                method_seen_ = true;
                bool value_literal = false, value_overflow = false;
                if (!ParseString(&method, value_literal, value_overflow, 128) ||
                    !value_literal || value_overflow || method.size() < 2)
                    return false;
...
inline std::string ExtractMethodFromJson(const std::string& json) {
    ...
    if (!parser.Parse(method)) return "";
    return method;
}

}} // namespace veld::rpc_proxy
