...
class PortMapper {
  public:
    ...
    std::string ExternalIp() const {
        std::lock_guard<std::mutex> lk(mu_);
        return mapping_.external_ip;
    }
    uint16_t ExternalPort() const {
        std::lock_guard<std::mutex> lk(mu_);
        return mapping_.external_port;
    }
    std::string Method() const {
        std::lock_guard<std::mutex> lk(mu_);
        return mapping_.method;
    }
    NatMapping Snapshot() const {
        std::lock_guard<std::mutex> lk(mu_);
        return mapping_;
    }
  ...
  private:
    ...
    static bool ValidateNatPmpAddressResponse_(const uint8_t* resp) {
        return resp[0] == 0 && resp[1] == 128 && get16(resp + 2) == 0 && IsNonzeroIpv4(resp + 8);
    }

    static bool ValidateNatPmpMapResponse_(const uint8_t* resp, uint16_t internal_port) {
        return resp[0] == 0 && resp[1] == 130 && get16(resp + 2) == 0 &&
               get16(resp + 8) == internal_port && get16(resp + 10) != 0 && get32(resp + 12) != 0;
    }

    static bool ValidatePcpMapResponse_(const uint8_t* resp, const uint8_t* nonce,
        ...
        if (resp[0] != 2 || resp[1] != 0x81 || resp[2] != 0 || resp[3] != 0 || get32(resp + 4) == 0)
            return false;
        const uint8_t* map = resp + 24;
        if (std::memcmp(map, nonce, 12) != 0 || map[12] != 6 || map[13] != 0 || map[14] != 0 ||
            map[15] != 0 || get16(map + 16) != internal_port || get16(map + 18) == 0)
            return false;
        ...
        for (size_t i = 0; i < 10; ++i)
            if (map[20 + i] != 0)
                return false;
        return map[30] == 0xff && map[31] == 0xff && IsNonzeroIpv4(map + 32);
    }
  ...
  public:
    ...
    static bool TestValidateNatPmpMapResponse(const uint8_t* resp, uint16_t internal_port) {
        return resp && ValidateNatPmpMapResponse_(resp, internal_port);
    }
    static bool TestValidatePcpMapResponse(const uint8_t* resp, const uint8_t* nonce,
                                           uint16_t internal_port) {
        return resp && nonce && ValidatePcpMapResponse_(resp, nonce, internal_port);
    }
  ...
  private:
    ...
    static bool DetectGateway(const std::string& localip, std::string& out) {
        ...
        while (std::getline(f, line)) {
            ...
            if (dest == "00000000" && gwhex.size() == 8) {
                ...
                unsigned char o0 = (g) & 0xFF, o1 = (g >> 8) & 0xFF, o2 = (g >> 16) & 0xFF,
                              o3 = (g >> 24) & 0xFF;
    ...
    bool TryNatPmp(const std::string& gw, uint16_t intport, NatMapping& out) {
        ...
        std::string extip;
        {
            ...
            if (SendRecvUdp(s, &ga, req, sizeof(req), resp, 12, /*retries*/ 3)) {
                if (ValidateNatPmpAddressResponse_(resp)) {
                    char b[INET_ADDRSTRLEN];
                    std::snprintf(b, sizeof(b), "%u.%u.%u.%u", resp[8], resp[9], resp[10],
                                  resp[11]);
                    extip = b;
                }
            }
        ...
        if (!ok)
            return false;
        if (!ValidateNatPmpMapResponse_(resp, intport))
            return false;
        out.ok = true;
        out.method = "NAT-PMP";
    ...
    // ----------------------------------------------------------------------
    bool TryPcp(const std::string& gw, const std::string& localip, uint16_t intport,
                NatMapping& out) {
        if (!pcp_nonce_ready_)
            return false;
        EnsureNet();
        ...
        {
            struct in_addr ia{};
            ::inet_pton(AF_INET, localip.c_str(), &ia);
            req[8 + 10] = 0xff;
            req[8 + 11] = 0xff;
        ...
        if (!ok)
            return false;
        if (!ValidatePcpMapResponse_(resp, pcp_nonce_, intport))
            return false;
    ...
    bool SendRecvUdp(SocketHandle s, struct sockaddr_in* ga, const uint8_t* req, size_t reqlen,
                     uint8_t* resp, size_t expectlen, int retries) {
        if (!ga || !req || !resp || reqlen == 0 || expectlen == 0 || expectlen > 2048)
            return false;
        ...
        if (::connect(s, reinterpret_cast<struct sockaddr*>(ga), sizeof(*ga)) != 0)
            return false;
        std::array<uint8_t, 2048> packet{};
        for (int attempt = 0; attempt < retries && running_.load(std::memory_order_acquire);
             ++attempt) {
            if (::send(s, reinterpret_cast<const char*>(req), static_cast<int>(reqlen), 0) < 0)
                return false;
            const int n = static_cast<int>(::recv(s, reinterpret_cast<char*>(packet.data()),
    ...
    bool TryUpnp(const std::string& localip, uint16_t intport, NatMapping& out) {
        ...
        if (!FetchIgdControlUrl(location, host, ctrl, svc))
            return false;
        // AddPortMapping
        std::string body = "<u:AddPortMapping xmlns:u=\"" + svc +
                           "\">"
                           ...
                           "</u:AddPortMapping>";
        std::string resp;
        if (!SoapCall(host, ctrl, svc, "AddPortMapping", body, resp))
            return false;
        ...
        {
            std::string gbody =
                "<u:GetExternalIPAddress xmlns:u=\"" + svc + "\"></u:GetExternalIPAddress>";
            std::string gresp;
    ...
    bool FetchIgdControlUrl(const std::string& location, std::string& host_out,
        ...
        uint16_t port = 80;
        if (!ParseHttpUrl(location, host, port, path))
            return false;
        std::string resp;
        if (!HttpGet(host, port, path, resp))
            return false;
    ...
    bool SoapCall(const std::string& hostport, const std::string& ctrl, const std::string& svc,
                  const std::string& action, const std::string& innerXml, std::string& resp) {
        ...
        std::string env = "<?xml version=\"1.0\"?>"
                          "<s:Envelope xmlns:s=\"http://schemas.xmlsoap.org/soap/envelope/\" "
                          "s:encodingStyle=\"http://schemas.xmlsoap.org/soap/encoding/\">"
                          "<s:Body>" +
                          innerXml + "</s:Body></s:Envelope>";
        std::string req = "POST " + ctrl +
                          " HTTP/1.1\r\n"
                          "HOST: " +
    ...
    bool HttpRaw(const std::string& host, uint16_t port, const std::string& req,
                 std::string& resp) {
        EnsureNet();
...
#ifdef _WIN32
        DWORD sndtv = 1500;
        ::setsockopt((SOCKET)s, SOL_SOCKET, SO_SNDTIMEO, (const char*)&sndtv, sizeof(sndtv));
#else
        struct timeval sndtv{1, 500000};
        ::setsockopt(s, SOL_SOCKET, SO_SNDTIMEO, (const char*)&sndtv, sizeof(sndtv));
#endif
        struct sockaddr_in a{};
        a.sin_family = AF_INET;
    ...
    static bool ParseHttpUrl(const std::string& url, std::string& host, uint16_t& port,
                             std::string& path) {
        std::string u = url;
        if (u.rfind("http://", 0) == 0)
            u = u.substr(7);
    ...
    void DeleteMappingBestEffort() {
        ...
        if (m.method == "NAT-PMP" || m.method == "PCP") {
                ...
                if (veld::compat::IsValidSocket(s)) {
                    ...
                    if (::inet_pton(AF_INET, gw.c_str(), &ga.sin_addr) == 1) {
                        ...
                        if (m.method == "NAT-PMP") {
                            ...
                            put32(req + 8, 0);
                            uint8_t resp[16] = {0};
                            SendRecvUdp(s, &ga, req, sizeof(req), resp, 16, 1);
                            ...
                            if (::inet_pton(AF_INET, localip.c_str(), &local_addr) == 1) {
                                req[18] = 0xff;
                                req[19] = 0xff;
                                ...
                                if (::inet_pton(AF_INET, m.external_ip.c_str(), &external_addr) ==
                                    1) {
                                    map[30] = 0xff;
                                    map[31] = 0xff;
                                    std::memcpy(map + 32, &external_addr, 4);
                                }
                                uint8_t resp[60] = {0};
                                SendRecvUdp(s, &ga, req, sizeof(req), resp, 60, 1);
                            }
                        }
            ...
            if (!host.empty()) {
                std::string body = "<u:DeletePortMapping xmlns:u=\"" + svc +
                                   "\">"
                                   "<NewRemoteHost></NewRemoteHost>"
                                   "<NewExternalPort>" +
                                   std::to_string(internal_port_) +
                                   "</NewExternalPort>"
    ...
    void RunLoop(uint16_t intport) {
        try {
            ...
            pcp_nonce_ready_ = veld::compat::SecureRandom(pcp_nonce_, sizeof(pcp_nonce_));
            ...
            if (!IsPrivateIp(localip)) {
                {
                    std::lock_guard<std::mutex> lk(mu_);
                    mapping_ = {true, "public", localip, intport, 0};
                }
            ...
            if (veld::DiagVerbose().load())
                std::cout << "  [nat] local=" << localip << (gw.empty() ? "" : (" gateway=" + gw))
                          << " port=" << intport << "\n";
            ...
            while (running_.load(std::memory_order_acquire)) {
                NatMapping m;
                bool ok = false;
                if (!gw.empty() && TryNatPmp(gw, intport, m))
                    ok = true;
                else if (!gw.empty() && TryPcp(gw, localip, intport, m))
                    ok = true;
#if defined(VELD_ENABLE_UPNP)
                else if (TryUpnp(localip, intport, m))
                    ok = true;
#endif
