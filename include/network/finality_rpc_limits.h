...
inline constexpr size_t kValidatorPubkeyHexChars = 2U * ::veld::dilithium::PUBKEY_BYTES;
...
inline constexpr size_t kMaxSnapshotMembers = static_cast<size_t>(qc::MAX_FINALITY_VALIDATOR_COUNT);
...
inline constexpr size_t kSnapshotMemberBytesWithoutIndex =
    (sizeof("{\"index\":") - 1) + (sizeof(",\"pubkey\":\"") - 1) + kValidatorPubkeyHexChars +
    (sizeof("\",\"commit\":\"") - 1) + kHashHexChars + (sizeof("\",\"address\":\"") - 1) +
    kMaxValidatorAddressChars + (sizeof("\",\"registered_height\":") - 1) +
    DecimalChars(UINT64_MAX) + (sizeof(",\"weight\":") - 1) + DecimalChars(qc::BOND_PER_KEY_UNITS) +
    (sizeof("}") - 1);
...
inline constexpr size_t kFinalizedRecordJsonMaxBytes =
    (sizeof("{\"epoch\":") - 1) + DecimalChars(UINT64_MAX) + (sizeof(",\"height\":") - 1) +
    DecimalChars(UINT64_MAX) + (sizeof(",\"hash\":\"") - 1) + kHashHexChars +
    (sizeof("\",\"round\":") - 1) + DecimalChars(UINT32_MAX) + (sizeof("}") - 1);
...
inline constexpr size_t kSnapshotResultJsonMaxBytes =
    ...
    (sizeof(",\"set_root\":\"") - 1) + kHashHexChars + (sizeof("\",\"total_weight\":") - 1) +
    DecimalChars(kMaxSnapshotTotalWeight) + (sizeof(",\"active\":") - 1) + (sizeof("false") - 1) +
    (sizeof(",\"members\":[") - 1) + kSnapshotMembersJsonMaxBytes +
    (sizeof("],\"finalized\":") - 1) + kFinalizedRecordJsonMaxBytes + (sizeof("}}") - 1);
...
inline constexpr size_t kValidatorRpcEnvelopeSuffixBytes = sizeof(",\"error\":null}") - 1;

inline constexpr size_t kSnapshotJsonResponseMaxBytes = kValidatorRpcEnvelopePrefixBytes +
                                                        kSnapshotResultJsonMaxBytes +
...
inline constexpr size_t kQcResultJsonMaxBytes =
    (sizeof("{\"qc_hex\":\"") - 1) + 2U * qc::MAX_FINALITY_QC_BYTES + (sizeof("\"}") - 1);
inline constexpr size_t kQcJsonResponseMaxBytes =
    kValidatorRpcEnvelopePrefixBytes + kQcResultJsonMaxBytes + kValidatorRpcEnvelopeSuffixBytes;
...
inline constexpr size_t kRpcHttpFixedHeaderBytes =
    ...
    (sizeof("Access-Control-Allow-Origin: https://wallet.veld.network\r\n"
     ...
     1) +
#endif
    (sizeof("Content-Length: ") - 1) + (sizeof("\r\nConnection: close\r\n\r\n") - 1);
...
inline constexpr ResponsePolicy kDefaultResponsePolicy{kDefaultRpcResponseBytes,
                                                       kDefaultRpcResponseBytes};
inline constexpr ResponsePolicy kSnapshotResponsePolicy{
    HttpResponseBytesFor(kSnapshotJsonResponseMaxBytes), kSnapshotJsonResponseMaxBytes};
inline constexpr ResponsePolicy kQcResponsePolicy{HttpResponseBytesFor(kQcJsonResponseMaxBytes),
...
inline bool AppendBoundedResponse(std::string& response, const char* bytes, size_t count,
                                  size_t limit) {
