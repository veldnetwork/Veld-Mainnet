...
inline std::string CompiledPublicNetworkIdentityText() {
    ...
    magic << "0x" << std::hex << std::nouppercase << std::setw(8) << std::setfill('0')
          << config.magic;
...
inline bool ValidateOrCreatePublicNetworkIdentity(const std::string& data_dir,
    ...
    auto read_exact = [&](bool* missing) -> bool {
        ...
        std::string why;
        const auto result = channel::secure_file::Read(marker.string(), bytes, &why, 4096,
        ...
        if (result != channel::secure_file::ReadResult::Ok) {
            if (error)
                *error = "cannot securely read network.identity: " + why;
            return false;
        }
    ...
    std::string why;
    const bool wrote = channel::secure_file::AtomicWriteNew(marker.string(), bytes, &why,
                                                            /*require_private_parent=*/true);
    channel::secure_file::WipeAndClear(bytes);
    if (!wrote) {
