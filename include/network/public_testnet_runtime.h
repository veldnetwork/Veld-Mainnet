...
inline constexpr const char* SESSION_MARKER_NAME =
    "public-testnet.session";
inline constexpr const char* SESSION_MARKER_HEADER =
    "VELD_PUBLIC_TESTNET_RUNTIME_V1";
inline constexpr const char* CLOCK_HIGH_WATER_NAME =
    "public-testnet.clock-high-water";
inline constexpr const char* CLOCK_HIGH_WATER_HEADER =
    "VELD_PUBLIC_TESTNET_CLOCK_HIGH_WATER_V1";
...
struct RuntimeLimits {
    ...
    bool TimePermitted(int64_t now_unix) const noexcept {
        return not_after_unix > 0 && now_unix >= 0 &&
               now_unix < not_after_unix;
    }
    ...
    bool CandidatePermitted(uint64_t candidate_height,
                            int64_t now_unix) const noexcept {
        return CandidateHeightPermitted(candidate_height) &&
               TimePermitted(now_unix);
    }
    ...
    bool ServicePermitted(uint64_t current_height,
                          int64_t now_unix) const noexcept {
        return not_after_height > 0 && current_height < not_after_height &&
               TimePermitted(now_unix);
    }
};
...
inline bool PermitOrLatchClosed(std::atomic<bool>& closed,
                                bool sampled_permitted) noexcept {
...
inline void LatchClosedAtDurableCanonicalHeight(
...
inline bool RestartBundleFreshAt(int64_t signed_observed_unix,
                                 int64_t valid_until_unix,
                                 int64_t local_now_unix,
                                 uint64_t authority_monotonic_seconds,
                                 uint64_t current_monotonic_seconds) noexcept {
    if (signed_observed_unix <= 0 ||
        valid_until_unix <= signed_observed_unix || local_now_unix < 0 ||
        local_now_unix >= valid_until_unix ||
    ...
    if (elapsed > static_cast<uint64_t>(INT64_MAX) ||
        ...
        return false;
    return signed_observed_unix + static_cast<int64_t>(elapsed) <
        valid_until_unix;
}
...
inline bool ParseCanonicalPositiveUint64(const std::string& value,
...
inline int64_t DaysFromCivil(int year, unsigned month,
...
inline bool ParseCanonicalUtc(const std::string& value,
    ...
    for (size_t i : {size_t{0}, size_t{1}, size_t{2}, size_t{3},
                     size_t{5}, size_t{6}, size_t{8}, size_t{9},
                     size_t{11}, size_t{12}, size_t{14}, size_t{15},
                     size_t{17}, size_t{18}}) {
...
inline constexpr const char* COMPILED_LEASE_NOT_AFTER_UTC =
...
inline bool CompiledRuntimeLimits(int64_t now_unix, RuntimeLimits& out,
    ...
    (void)now_unix; (void)out;
    if (error) *error =
        "the public-testnet lease is unavailable outside that profile";
    return false;
#else
    ...
    if (limits.not_after_height == 0 ||
        ...
        if (error) *error = "compiled public-testnet lease is malformed";
        return false;
    }
    ...
    const std::string identity =
        std::string("veld-public-testnet-compiled-lease-v1\n") +
    ...
    SHA256 hasher;
    hasher.update(reinterpret_cast<const uint8_t*>(identity.data()),
                  identity.size());
    limits.lease_identity_sha256 = BytesToHex(hasher.digest());
    if (!IsCanonicalSha256(limits.lease_identity_sha256)) {
        if (error) *error = "compiled public-testnet lease identity is invalid";
        return false;
    }
    out = std::move(limits);
...
inline bool ParseRuntimeLimits(const std::string& lease_identity_sha256,
                               ...
                               const std::string& not_after_utc,
                               int64_t now_unix,
                               RuntimeLimits& out,
                               std::string* error = nullptr) {
    ...
    if (!IsCanonicalSha256(lease_identity_sha256)) {
        if (error) *error =
            "--testnet-start-index-sha256 must be 64 lowercase nonzero hex characters";
        return false;
    }
    if (!ParseCanonicalPositiveUint64(
            not_after_height, parsed.not_after_height)) {
        if (error) *error =
            "START not_after_height must be a canonical positive decimal integer";
        return false;
    }
    if (!ParseCanonicalUtc(not_after_utc, parsed.not_after_unix)) {
        if (error) *error =
            "START not_after_utc must be canonical YYYY-MM-DDTHH:MM:SSZ in 2000..2099";
        return false;
    }
...
inline std::string CanonicalClockHighWater(const RuntimeLimits& limits,
...
inline bool ReadClockHighWater(const std::string& data_dir,
                               const RuntimeLimits& limits,
                               int64_t& observed_unix,
                               bool& missing,
                               std::string* error = nullptr) {
    ...
    if (result != channel::secure_file::ReadResult::Ok) {
        if (error) {
            *error = "cannot securely read public-testnet UTC high-water";
            if (!read_error.empty()) *error += ": " + read_error;
        }
        return false;
    }
    ...
    const std::string prefix = std::string(CLOCK_HIGH_WATER_HEADER) + "\n" +
        ...
        "not_after_unix=" + std::to_string(limits.not_after_unix) + "\n" +
        "observed_unix=";
    if (wire.size() <= prefix.size() + 1 ||
        wire.compare(0, prefix.size(), prefix) != 0 || wire.back() != '\n') {
        if (error) *error =
            "public-testnet UTC high-water is malformed or bound to another START index";
        return false;
    }
    const std::string decimal = wire.substr(
        prefix.size(), wire.size() - prefix.size() - 1);
    uint64_t parsed = 0;
    if (!ParseCanonicalPositiveUint64(decimal, parsed) ||
        parsed > static_cast<uint64_t>(INT64_MAX)) {
        if (error) *error =
            "public-testnet UTC high-water timestamp is not canonical";
        return false;
    }
    observed_unix = static_cast<int64_t>(parsed);
    if (wire != CanonicalClockHighWater(limits, observed_unix)) {
        if (error) *error =
            "public-testnet UTC high-water is not byte-exact canonical data";
        return false;
    }
    if (error) error->clear();
    return true;
...
inline bool CheckPersistedClockHighWater(const std::string& data_dir,
                                         const RuntimeLimits& limits,
                                         int64_t now_unix,
    ...
    int64_t persisted = 0;
    bool missing = false;
    if (!ReadClockHighWater(
            data_dir, limits, persisted, missing, error))
        return false;
    if (missing && require_existing) {
        if (error) *error =
            "existing public-testnet session is missing its UTC high-water";
        return false;
    }
    if (!missing && (now_unix < 0 || now_unix < persisted)) {
        if (error) *error =
            "local UTC is behind the owner-only public-testnet high-water; clock rollback is refused";
        return false;
    }
    if (error) error->clear();
    return true;
...
public:
    bool Initialize(const std::string& data_dir,
                    ...
                    std::string* error = nullptr) {
        return InitializeAt(
            data_dir, limits, now_unix,
            SuspendAwareMonotonicSeconds(), error);
    }

    bool InitializeAt(const std::string& data_dir,
                      const RuntimeLimits& limits,
                      int64_t now_unix,
                      uint64_t monotonic_now,
                      std::string* error = nullptr) {
        return InitializeAuthenticatedAt(
            data_dir, limits, now_unix, now_unix, monotonic_now, error);
    }
    ...
    bool InitializeAuthenticatedAt(const std::string& data_dir,
                      const RuntimeLimits& limits,
                      int64_t signed_observed_unix,
                      int64_t local_now_unix,
                      uint64_t monotonic_now,
                      std::string* error = nullptr) {
        ...
        if (initialized_) {
            if (error) *error =
                "public-testnet clock guard is startup-only and immutable";
            return false;
        }
        if (signed_observed_unix <= 0 || local_now_unix < 0 ||
            monotonic_now == INVALID_SUSPEND_AWARE_SECONDS) {
            if (error) *error =
                "public-testnet clock guard requires authenticated and local UTC";
            return false;
        }
        const int64_t effective_now =
            std::max(signed_observed_unix, local_now_unix);
        ...
        if (!channel::secure_file::EnsurePrivateDirectory(
                data_dir, &private_error)) {
            if (error) *error =
                "public-testnet clock datadir is unsafe: " + private_error;
            return false;
        }
        int64_t persisted = 0;
        bool missing = false;
        ...
        bool session_exists = false;
        {
            ...
            const auto session_result = channel::secure_file::Read(
                data_dir + "/" + SESSION_MARKER_NAME,
                session_bytes, &session_error, 4096,
                /*require_private_parent=*/true);
            if (session_result == channel::secure_file::ReadResult::Ok) {
                session_exists = true;
                channel::secure_file::WipeAndClear(session_bytes);
            } else if (session_result !=
                       channel::secure_file::ReadResult::NotFound) {
                channel::secure_file::WipeAndClear(session_bytes);
                if (error) {
                    *error =
                        "cannot securely inspect public-testnet session before clock initialization";
        ...
        if (!missing && effective_now < persisted) {
            if (error) *error =
                "authenticated/local UTC is behind the owner-only public-testnet high-water; rollback is refused";
            return false;
        }
    ...
    bool ObserveAt(int64_t now_unix,
                   uint64_t monotonic_now,
        ...
        try {
            ...
            if (!initialized_ || closed_) {
                if (error) *error =
                    "public-testnet clock guard is unavailable or latched closed";
                return false;
            }
            if (now_unix < 0 ||
                monotonic_now == INVALID_SUSPEND_AWARE_SECONDS ||
                monotonic_now < anchor_monotonic_seconds_ ||
                now_unix < last_wall_unix_) {
                closed_ = true;
                if (error) *error =
                    "public-testnet wall clock moved backward";
                return false;
            }
            const uint64_t elapsed =
                monotonic_now - anchor_monotonic_seconds_;
            const uint64_t wall_elapsed = static_cast<uint64_t>(
                now_unix - anchor_wall_unix_);
            if (elapsed > wall_elapsed &&
                elapsed - wall_elapsed >
                    CLOCK_CORRELATION_SLACK_SECONDS) {
                closed_ = true;
                if (error) *error =
                    "public-testnet wall clock stalled or rolled back relative to monotonic time";
                return false;
            }
            last_wall_unix_ = now_unix;
            if (elapsed > static_cast<uint64_t>(INT64_MAX) ||
                anchor_signed_unix_ >
                    INT64_MAX - static_cast<int64_t>(elapsed)) {
                closed_ = true;
                if (error) *error =
                    "public-testnet authenticated time arithmetic overflow";
                return false;
            }
            const int64_t signed_floor =
                anchor_signed_unix_ + static_cast<int64_t>(elapsed);
            ...
            if (!limits_.TimePermitted(effective_now)) {
                closed_ = true;
                if (error) *error =
                    "public-testnet not-after UTC has been reached";
                return false;
            }
            if (error) error->clear();
            return true;
            ...
            closed_ = true;
            if (error) *error =
                "public-testnet clock observation failed closed";
            return false;
        }
    }

private:
    bool PersistLocked(int64_t observed_unix,
                       std::string* error) {
        if (observed_unix <= persisted_wall_unix_) return true;
        const std::string wire = CanonicalClockHighWater(
            limits_, observed_unix);
        const std::string path = data_dir_ + "/" + CLOCK_HIGH_WATER_NAME;
        std::string write_error;
        if (!channel::secure_file::AtomicWriteText(
                path, wire, &write_error,
                /*require_private_parent=*/true)) {
            if (error) *error =
                "cannot atomically advance public-testnet UTC high-water: " +
                write_error;
            return false;
        }
        int64_t verified = 0;
        bool missing = false;
        if (!ReadClockHighWater(
                data_dir_, limits_, verified, missing, error) || missing ||
            verified < observed_unix) {
            if (error && error->empty()) *error =
                "advanced public-testnet UTC high-water did not verify";
            return false;
        }
...
inline bool LoadRuntimeLimitsFromStartIndex(
        const std::string& start_index_path,
        const std::string& expected_sha256,
        int64_t now_unix,
        RuntimeLimits& out,
        std::string* error = nullptr) {
    if (start_index_path.empty()) {
        if (error) *error = "--testnet-start-index requires an explicit file";
        return false;
    }
    ...
    std::vector<uint8_t> bytes;
    std::string read_error;
    const auto read = channel::secure_file::ReadExplicitImport(
        start_index_path, bytes, &read_error, 256u * 1024u);
    ...
    if (actual_sha256 != expected_sha256) {
        channel::secure_file::WipeAndClear(bytes);
        if (error) *error =
            "START index SHA-256 does not match the independently accepted digest";
        return false;
    }
    ...
    if (!parser.Parse(root, json_error)) {
        channel::secure_file::WipeAndClear(bytes);
        if (error) *error = "invalid START index JSON: " + json_error;
        return false;
    }
    ...
    if (!runtime_json::Canonicalize(root, canonical_json, json_error) ||
        ...
        channel::secure_file::WipeAndClear(bytes);
        if (error) *error =
            "START index is not canonical sorted, 2-space indented ASCII JSON with one LF";
        return false;
    }
    channel::secure_file::WipeAndClear(bytes);
    if (root.kind != runtime_json::Value::Kind::Object) {
        if (error) *error = "START index root must be an object";
        return false;
    }

    auto exact_string = [&](const runtime_json::Value& object,
                            const char* key, const std::string& expected) {
        const auto* value = object.Get(key);
        return value && value->kind == runtime_json::Value::Kind::String &&
               value->text == expected;
    };
    auto exact_bool = [&](const runtime_json::Value& object,
                          const char* key, bool expected) {
        const auto* value = object.Get(key);
        return value && value->kind == runtime_json::Value::Kind::Bool &&
               value->boolean == expected;
    };
    auto exact_number = [&](const runtime_json::Value& object,
                            const char* key, const char* expected) {
        const auto* value = object.Get(key);
        return value && value->kind == runtime_json::Value::Kind::Number &&
               value->text == expected;
    };
    auto exact_keys = [](const runtime_json::Value& object,
        ...
        for (const auto& [key, ignored] : object.object) {
            (void)ignored;
            if (!expected.count(key)) return false;
        }
        return true;
    };
    auto is_nonzero_hash = [](const std::string& value,
                              bool allow_source_width = false) {
    ...
    static const std::set<std::string> START_FIELDS = {
        "allowed_services", "completed_at_utc", "data_directory",
        "disposable", "display_name", "evidence", "external_value",
        "forbidden_services", "genesis_hash", "lifecycle", "network_magic",
        "not_after_height", "not_after_utc", "operator_set", "ports",
        "platform_scope", "profile_id", "release_version", "result", "role", "schema",
        "source_commit", "source_identity", "source_manifest_sha256",
        "source_tree", "statement", "warning_banner",
    };
    static const std::vector<std::string> ALLOWED_SERVICES = {
        "checkpoint", "core-node", "explorer", "miner", "oracle", "seed",
        "snapshot", "validator", "wallet",
    };
    static const std::vector<std::string> FORBIDDEN_SERVICES = {
        "bitcoin-anchor", "btcveld-custody", "btcveld-issuer-mint",
        "btcveld-peg", "btcveld-redeem", "btcveld-spv-mint",
        "btcveld-wrap", "doge-desk", "eth-desk", "ltc-desk", "swap-desk",
    };
    ...
    if (!exact_keys(root, START_FIELDS) ||
        ...
        !exact_string(root, "lifecycle", "START") ||
        !exact_string(root, "role", DEPLOYMENT_ROLE) ||
        ...
        !exact_string(root, "result", "PASS") ||
        !exact_bool(root, "disposable", true) ||
        !exact_bool(root, "external_value", false) ||
        !exact_number(root, "network_magic", "1381387332") ||
        !schema || schema->kind != runtime_json::Value::Kind::Number ||
        schema->text != "2" ||
        ...
        if (error) *error =
            "START index exact schema/identity/service policy does not match this disposable testnet binary";
        return false;
    }

    const auto* ports = root.Get("ports");
    static const std::set<std::string> PORT_FIELDS = {"p2p", "rpc"};
    if (!ports || !exact_keys(*ports, PORT_FIELDS) ||
        !exact_number(*ports, "p2p", "19333") ||
        !exact_number(*ports, "rpc", "19334")) {
        if (error) *error = "START index ports do not match this testnet binary";
        return false;
    }
    ...
    static const std::set<std::string> EVIDENCE_FIELDS = {
        ...
        "valueless_service_gate_report_sha256",
        "windows_client_manifest_sha256",
    };
    std::set<std::string> operator_and_evidence_hashes;
    if (!evidence || !exact_keys(*evidence, EVIDENCE_FIELDS)) {
        if (error) *error = "START index evidence set is not exact";
        return false;
    }
    for (const auto& [key, value] : evidence->object) {
        (void)key;
        if (value.kind != runtime_json::Value::Kind::String ||
            !is_nonzero_hash(value.text) ||
            !operator_and_evidence_hashes.insert(value.text).second) {
            if (error) *error = "START index evidence hashes are invalid or reused";
            return false;
        }
    }
    ...
    static const std::set<std::string> OPERATOR_FIELDS = {
        "attestation_sha256", "attestation_signature_sha256", "operator_id",
        "operator_public_key_sha256",
    };
    if (!operators || operators->kind != runtime_json::Value::Kind::Array ||
        operators->array.size() != 1) {
        if (error) *error =
            "START index must contain exactly one testnet owner/operator";
        return false;
    }
    ...
    for (const auto& op : operators->array) {
        if (!exact_keys(op, OPERATOR_FIELDS)) {
            if (error) *error = "START operator fields are not exact";
            return false;
        }
        const auto* id = op.Get("operator_id");
        const auto* public_key = op.Get("operator_public_key_sha256");
        bool safe_operator_id = id &&
            id->kind == runtime_json::Value::Kind::String &&
        ...
        if (!safe_operator_id || !operator_ids.insert(id->text).second ||
            !public_key || public_key->kind != runtime_json::Value::Kind::String ||
            !is_nonzero_hash(public_key->text) ||
            !operator_keys.insert(public_key->text).second) {
            if (error) *error = "START operator identities/keys are invalid or reused";
            return false;
        }
        for (const auto& [key, value] : op.object) {
            if (key == "operator_id") continue;
            if (value.kind != runtime_json::Value::Kind::String ||
                !is_nonzero_hash(value.text) ||
                !operator_and_evidence_hashes.insert(value.text).second) {
                if (error) *error = "START operator/evidence hashes are invalid or reused";
                return false;
            }
        }
    }
    if (operator_ids.size() != 1 || operator_keys.size() != 1) {
        if (error) *error =
            "START index testnet owner/operator is missing or ambiguous";
        return false;
    }
    ...
    if (!source_identity || source_identity->kind != runtime_json::Value::Kind::String ||
        (source_identity->text != "git-v1" &&
         source_identity->text != "source-manifest-v1") ||
        !source_commit || source_commit->kind != runtime_json::Value::Kind::String ||
        !is_nonzero_hash(source_commit->text, true) ||
        !source_tree || source_tree->kind != runtime_json::Value::Kind::String ||
        !is_nonzero_hash(source_tree->text, true) ||
        !source_manifest || source_manifest->kind != runtime_json::Value::Kind::String ||
        !is_nonzero_hash(source_manifest->text)) {
        if (error) *error = "START source identity/digests are incomplete";
        return false;
    }
    ...
    if (!height || height->kind != runtime_json::Value::Kind::Number ||
        !utc || utc->kind != runtime_json::Value::Kind::String ||
        !completed || completed->kind != runtime_json::Value::Kind::String) {
        if (error) *error =
            "START index completion/not-after fields are invalid";
        return false;
    }
    int64_t completed_unix = 0;
    if (!ParseCanonicalUtc(completed->text, completed_unix)) {
        if (error) *error = "START completed_at_utc is not canonical";
        return false;
    }
    RuntimeLimits parsed;
    if (!ParseRuntimeLimits(expected_sha256, height->text, utc->text,
                            now_unix, parsed, error))
        return false;
    if (parsed.not_after_height > static_cast<uint64_t>(INT64_MAX) ||
        completed_unix >= parsed.not_after_unix || completed_unix > now_unix) {
        if (error) *error =
            "START completion/not-after chronology or height range is invalid";
        return false;
    }
    out = std::move(parsed);
    if (error) error->clear();
...
inline bool BindOrVerifySession(const std::string& data_dir,
                                const RuntimeLimits& limits,
                                int64_t now_unix,
                                std::string* error = nullptr) {
#if !defined(VELD_PUBLIC_TESTNET)
    ...
    (void)now_unix;
    if (error) *error =
        "public-testnet runtime session is unavailable outside VELD_PUBLIC_TESTNET";
    return false;
#else
    if (!IsCanonicalSha256(limits.lease_identity_sha256) ||
        limits.not_after_height == 0 || limits.not_after_unix <= 0 ||
        limits.not_after_utc.empty() || now_unix < 0 ||
        now_unix >= limits.not_after_unix) {
        if (error) *error = "public-testnet runtime limits are malformed or expired";
        return false;
    }
    int64_t reparsed_unix = 0;
    if (!ParseCanonicalUtc(limits.not_after_utc, reparsed_unix) ||
        reparsed_unix != limits.not_after_unix) {
        if (error) *error = "public-testnet UTC/Unix cutoff binding is inconsistent";
        return false;
    }
    std::string private_error;
    if (!channel::secure_file::EnsurePrivateDirectory(
            data_dir, &private_error)) {
        if (error) *error =
            "public-testnet session datadir is unsafe: " + private_error;
        return false;
    }
    ...
    auto read_exact = [&](bool* missing) {
        ...
        if (result != channel::secure_file::ReadResult::Ok) {
            if (error) *error =
                "cannot securely read public-testnet session marker: " +
                read_error;
            return false;
        }
    ...
    if (read_exact(&missing)) {
        return CheckPersistedClockHighWater(
            data_dir, limits, now_unix,
    ...
    if (!CheckPersistedClockHighWater(
            data_dir, limits, now_unix,
            /*require_existing=*/true, error))
        return false;
    ...
    if (!channel::secure_file::AtomicWriteNew(
        ...
        if (error && error->empty()) {
            *error = "cannot create immutable public-testnet session marker: " +
                write_error;
        }
        return false;
    }
