...
private:
    ...
    bool ParseValue(JsonValue& out, std::string& error, size_t depth) {
        if (++nodes_ > kMaxNodes) return Fail(error, "too many JSON nodes");
        if (depth > kMaxDepth) return Fail(error, "JSON nesting too deep");
        SkipWhitespace();
        if (pos_ >= input_.size()) return Fail(error, "truncated JSON value");
        const char c = input_[pos_];
        if (c == '{') return ParseObject(out, error, depth + 1);
        if (c == '[') return ParseArray(out, error, depth + 1);
        if (c == '"') {
            out.kind = JsonValue::Kind::String;
            return ParseString(out.text, error, &out.string_had_escape);
        }
        if (c == 't' && Consume("true")) {
            out.kind = JsonValue::Kind::Bool; out.boolean = true; return true;
        }
        if (c == 'f' && Consume("false")) {
            out.kind = JsonValue::Kind::Bool; out.boolean = false; return true;
        }
        if (c == 'n' && Consume("null")) {
            out.kind = JsonValue::Kind::Null; return true;
        }
    ...
    bool ParseHex4(uint32_t& out, std::string& error) {
        if (pos_ + 4 > input_.size()) return Fail(error, "truncated unicode escape");
        out = 0;
        for (int i = 0; i < 4; ++i) {
            int n = HexNibble(input_[pos_++]);
            if (n < 0) return Fail(error, "invalid unicode escape");
    ...
    bool ParseNumber(JsonValue& out, std::string& error) {
        ...
        } else return Fail(error, "invalid JSON number");
        if (pos_ < input_.size() && input_[pos_] == '.') {
            ++pos_;
            size_t digits = pos_;
            while (pos_ < input_.size() &&
    ...
    bool ParseArray(JsonValue& out, std::string& error, size_t depth) {
        ++pos_; out.kind = JsonValue::Kind::Array; out.array.clear();
        SkipWhitespace();
        if (pos_ < input_.size() && input_[pos_] == ']') { ++pos_; return true; }
        for (;;) {
            JsonValue item;
            if (!ParseValue(item, error, depth)) return false;
            out.array.push_back(std::move(item));
            SkipWhitespace();
            if (pos_ >= input_.size()) return Fail(error, "unterminated JSON array");
            if (input_[pos_] == ']') { ++pos_; return true; }
            if (input_[pos_] != ',') return Fail(error, "expected JSON array comma");
            ++pos_; SkipWhitespace();
        }
    }

    bool ParseObject(JsonValue& out, std::string& error, size_t depth) {
        ++pos_; out.kind = JsonValue::Kind::Object; out.object.clear();
        SkipWhitespace();
        if (pos_ < input_.size() && input_[pos_] == '}') { ++pos_; return true; }
        for (;;) {
            std::string key;
            bool key_had_escape = false;
            if (!ParseString(key, error, &key_had_escape)) return false;
            ...
            if (input_[pos_] == '}') { ++pos_; return true; }
            if (input_[pos_] != ',') return Fail(error, "expected JSON object comma");
            ++pos_; SkipWhitespace();
        }
    }
};
...
inline bool IsLowerHex(const std::string& value, size_t exact_chars = 0) {
    if ((exact_chars && value.size() != exact_chars) || value.empty() ||
        (value.size() & 1U)) return false;
    for (char c : value)
...
inline bool IsNonzeroHash256(const std::string& value) {
    ...
    for (char c : value) if (c != '0') return true;
    return false;
}
