...
inline SocketHandle DialLocal(const std::string& ip, uint16_t port, int timeout_ms) {
    ...
    if (!veld::compat::IsValidSocket(s))
        return veld::compat::kInvalidSocket;
...
inline std::string ReadControlReply(SocketHandle s) {
    ...
    for (int i = 0; i < 64; ++i) {
        ...
        if (lastnl != std::string::npos) {
            ...
            if (out.size() >= line_start + 4 && out[line_start + 3] == ' ')
                break;
        }
    }
    return out;
...
template <typename Container> struct ScopedWipe {
    Container& value;
    ~ScopedWipe() {
        if (!value.empty())
...
template <size_t N> inline std::string Hex(const std::array<uint8_t, N>& bytes) {
...
template <size_t N>
inline bool DecodeExactHex(const std::string& encoded, std::array<uint8_t, N>& output) {
...
inline bool StrictControlLines(const std::string& reply, std::vector<std::string>& lines) {
    lines.clear();
    if (reply.empty() || reply.size() > 64U * 1024U || reply.size() < 2U ||
        reply.substr(reply.size() - 2U) != "\r\n")
        return false;
    size_t cursor = 0;
...
inline bool ProtocolSupportsSafeCookie(const std::string& reply) {
    ...
    for (size_t i = 1; i + 1U < lines.size(); ++i) {
        ...
        while (cursor < methods.size()) {
            ...
            const size_t token_end = comma == std::string::npos ? methods.size() : comma;
            if (token_end == cursor)
                return false;
            const std::string token = methods.substr(cursor, token_end - cursor);
...
inline bool ParseSafeCookieChallenge(const std::string& reply, std::array<uint8_t, 32>& server_hash,
                                     std::array<uint8_t, 32>& server_nonce) {
    std::vector<std::string> lines;
    if (!StrictControlLines(reply, lines) || lines.size() != 1U)
        return false;
    static constexpr char prefix[] = "250 AUTHCHALLENGE SERVERHASH=";
    const std::string& line = lines.front();
    if (line.rfind(prefix, 0) != 0)
        return false;
    const size_t hash_begin = sizeof(prefix) - 1U;
    const size_t nonce_label = line.find(" SERVERNONCE=", hash_begin);
    if (nonce_label == std::string::npos || line.find(' ', nonce_label + 1U) != std::string::npos)
        return false;
    const std::string encoded_hash = line.substr(hash_begin, nonce_label - hash_begin);
    const std::string encoded_nonce = line.substr(nonce_label + std::strlen(" SERVERNONCE="));
    return DecodeExactHex(encoded_hash, server_hash) && DecodeExactHex(encoded_nonce, server_nonce);
}

inline std::array<uint8_t, 32> SafeCookieHmac(const char* key,
                                              const std::array<uint8_t, 32>& cookie,
    ...
    return veld::wallet_crypto::HMAC_SHA256(reinterpret_cast<const uint8_t*>(key), std::strlen(key),
                                            message.data(), message.size());
}

inline bool ReadTrustedCookie(const std::string& data_directory, std::array<uint8_t, 32>& cookie,
                              std::string& error) {
    namespace fs = std::filesystem;
    const fs::path root(data_directory);
    if (data_directory.empty() || !root.is_absolute() || root.relative_path().empty()) {
    ...
    for (const auto& component : root.relative_path()) {
        current /= component;
        veld::channel::secure_file::WinHandle handle(
            ::CreateFileW(current.c_str(), FILE_READ_ATTRIBUTES | READ_CONTROL,
        ...
        if (current == root && !veld::channel::secure_file::HandleHasCurrentOwner(
                                   handle.value, owner.sid(), true, &error, "Tor data directory"))
            return false;
    }
    if (!opened_component)
        return false;
    ...
    for (const auto& component : root.relative_path()) {
        ...
        const int next =
            ::openat(directory, name.c_str(), O_RDONLY | O_DIRECTORY | O_NOFOLLOW | O_CLOEXEC);
        ::close(directory);
        directory = next;
        if (directory < 0) {
    ...
    if (::fstat(directory, &directory_status) != 0 || !S_ISDIR(directory_status.st_mode) ||
        directory_status.st_uid != ::geteuid() || (directory_status.st_mode & 0077) != 0) {
        ::close(directory);
        error = "Tor data directory must be owner-only";
        return false;
    }
    const int file =
        ::openat(directory, "control_auth_cookie", O_RDONLY | O_NOFOLLOW | O_CLOEXEC | O_NONBLOCK);
    ::close(directory);
    if (file < 0) {
    ...
    if (::fstat(file, &file_status) != 0 || !S_ISREG(file_status.st_mode) ||
        file_status.st_nlink != 1 || file_status.st_uid != ::geteuid() ||
    ...
    while (offset < cookie.size()) {
        const ssize_t count = ::read(file, cookie.data() + offset, cookie.size() - offset);
        if (count <= 0) {
            ::close(file);
    ...
    const auto result =
        veld::channel::secure_file::Read(cookie_path.string(), bytes, &error, cookie.size(), true);
    ScopedWipe<std::vector<uint8_t>> wipe_bytes{bytes};
    if (result != veld::channel::secure_file::ReadResult::Ok || bytes.size() != cookie.size()) {
...
inline SocketHandle Socks5Connect(const std::string& host, uint16_t port,
    ...
    if (!_tor::SendAll(s, (const char*)greet, 3)) {
        VELD_CLOSE_SOCKET(s);
        return veld::compat::kInvalidSocket;
    }
    uint8_t gresp[2] = {0};
    ...
    req.push_back((uint8_t)(port >> 8));
    req.push_back((uint8_t)(port & 0xFF));
    if (!_tor::SendAll(s, (const char*)req.data(), req.size())) {
        VELD_CLOSE_SOCKET(s);
        return veld::compat::kInvalidSocket;
    }
...
class TorController {
  public:
    ...
    bool Start(uint16_t p2p_port, const std::string& tor_data_directory,
               const std::string& control_ip = "127.0.0.1", uint16_t control_port = 9051) {
        ...
        if (!_tor::SendAll(s, cmd.c_str(), cmd.size())) {
            VELD_CLOSE_SOCKET(s);
            return false;
        }
  ...
  private:
    ...
    bool Authenticate(SocketHandle s, const std::string& tor_data_directory) {
        ...
        std::string proto;
        if (!_tor::SendAll(s, pi, std::strlen(pi)))
            return false;
        proto = _tor::ReadControlReply(s);
        if (!_tor::ProtocolSupportsSafeCookie(proto))
            return false;
        ...
        if (!_tor::ReadTrustedCookie(tor_data_directory, cookie, cookie_error) ||
            !veld::compat::SecureRandom(client_nonce.data(), client_nonce.size()))
            return false;

        std::string command = "AUTHCHALLENGE SAFECOOKIE " + _tor::Hex(client_nonce) + "\r\n";
        ...
        if (!_tor::ParseSafeCookieChallenge(challenge, server_hash, server_nonce))
            return false;
        if (have_last_server_nonce_ &&
            veld::compat::ConstantTimeEqual(last_server_nonce_.data(), server_nonce.data(),
                                            server_nonce.size()))
            return false;
        ...
        expected_server_hash = _tor::SafeCookieHmac(server_key, cookie, client_nonce, server_nonce);
        if (!veld::compat::ConstantTimeEqual(expected_server_hash.data(), server_hash.data(),
                                             server_hash.size()))
            return false;
        controller_hash = _tor::SafeCookieHmac(controller_key, cookie, client_nonce, server_nonce);
