...
class NodeServer {
  ...
  public:
    ...
    static bool PreferInboundForDuplicate(uint64_t local_nonce, uint64_t peer_nonce) {
    ...
    void SaveAnchors() {
        ...
        try {
            ...
            if (!f)
                return;
            f << "# veld outbound anchors — saved at Stop(), dialed at Start() before DNS "
                 "bootstrap\n";
            f << "# format: <ip> <port>\n";
  ...
  public:
    ...
    PeerHeightView GetPeerHeightView() const {
        ...
        uint64_t earliest_freshness_deadline = UINT64_MAX;
        {
                ...
                if (!live.inbound) {
                    ...
                    const uint64_t claim_deadline =
                        sync_claim->second.updated_at >
                            ...
                            ? UINT64_MAX
                            : sync_claim->second.updated_at + VERSION_HEIGHT_HINT_TTL_SECONDS + 1;
                    earliest_freshness_deadline =
                        std::min(earliest_freshness_deadline, claim_deadline);
                    auto outbound = selected_outbound.find(live.source_ip);
                    if (outbound == selected_outbound.end() ||
                        sync_claim->second.updated_at > outbound->second.updated_at ||
                        (sync_claim->second.updated_at == outbound->second.updated_at &&
                         live.connection_id > outbound->second.connection_id)) {
                        selected_outbound[live.source_ip] =
                            SelectedClaim{sync_claim->second.height, sync_claim->second.updated_at,
                                          live.connection_id};
                    }
                }
            ...
            if (outbound_claims.size() >= 2) {
                std::sort(outbound_claims.begin(), outbound_claims.end(), std::greater<uint64_t>());
    ...
    std::vector<PeerTipSnapshot>
    SnapshotPeerTips(bool configured_outbound_anchors_only = false) const {
        ...
        std::unordered_map<std::string, std::pair<uint64_t, PeerTipSnapshot>> selected;
        {
            ...
            for (const auto& live : live_sources) {
                ...
                if (chosen == selected.end() ||
                    ...
                    selected[live.source_ip] = {live.connection_id, std::move(candidate)};
                }
            }
        }
            ...
            for (const auto& live : live_sources) {
                ...
                if (chosen == selected.end() || it->second.updated_at > chosen->second.updated_at ||
                    ...
                    selected[live.source_ip] = SelectedSample{
                        it->second.delta_seconds, it->second.updated_at, live.connection_id};
                }
            }
  ...
  public:
    ...
    bool CurrentLosesDuplicateNonce_(const std::string& key, Connection& conn,
        ...
        const bool desired_inbound = PreferInboundForDuplicate(self_nonce_, peer_nonce);
    ...
    void TestRecordClockDrift(const std::shared_ptr<Connection>& conn, int64_t seconds,
        ...
        clock_drift_samples_[conn->Identity()] =
            ClockDriftSample{seconds, updated_at, conn->RemoteAddr(), conn->IsInbound()};
    }
  ...
  private:
    ...
    bool ReserveGetBlocksResponseBytes_(const std::string& ip, uint64_t bytes) {
        static const std::string GLOBAL_KEY = "*";
  ...
  private:
    ...
    void RecordPeerSyncHeight_(Connection& conn, uint64_t height) {
        ...
        if (source == peer_work_sources_.end() || !source->second.handshake_ready ||
            ...
            return;
        peer_sync_heights_[conn.Identity()] = PeerHeightClaim{peer_ip, height, PeerHeightNow_()};
    }
    ...
    StepResult PeerProtocolStep(PeerState& ps, Connection& conn, PeerManager& pm,
        ...
        if (msg.command == MessageType::VERSION) {
            ...
            conn.MarkAdvertisedServices(announced_services);
            {
...

                {
                    ...
                    const int64_t local_ts_signed = static_cast<int64_t>(std::time(nullptr));
                    ...
                    if (admissible_delta) {
                        ...
                        clock_drift_samples_[connection_id] =
                            ClockDriftSample{delta, now_mono, conn.RemoteAddr(), conn.IsInbound()};
                    }
                }
            ...
            for (uint16_t i = 0; i < count; ++i) {
                ...
                if (itype == InvType::BLOCK) {
                    ...
                    if (blk_opt.has_value()) {
                        ...
                        if (!TakeIpByteBudget(getdata_response_bytes_per_ip_, conn.RemoteAddr(),
                                              payload.size() + 24u,
                                              GETDATA_RESPONSE_BUDGET_PER_60S))
                            break;
                        if (!conn.Send(P2PMessage(magic_, MessageType::BLOCK, payload)))
        ...
        if (msg.command == MessageType::TIPSIG) {
            ...
            if (tipsig_source_colon != std::string::npos)
                tipsig_source_ip = tipsig_source_ip.substr(0, tipsig_source_colon);
            if (!TakeKeyCooldown(tipsig_processed_at_, tipsig_source_ip, 5, 600, 4096)) {
                return StepResult::Handled;
            }
        ...
        if (msg.command == MessageType::BLOCK) {
            ...
            const auto admission =
                chain_.AddBlockDirect(new_block, false, true, false,
    ...
    std::shared_ptr<mining::ExpensivePowBudget> PowBudgetForSource_(const std::string& source) {
        ...
        if (source_pow_budgets_.size() >= SOURCE_POW_BUDGET_CAP)
            return {};
        auto budget = std::make_shared<mining::ExpensivePowBudget>(1, 8, std::chrono::minutes(1));
