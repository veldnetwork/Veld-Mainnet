...
inline constexpr uint64_t G_RECV_INFLIGHT_PER_SOURCE_CAP = 32ull * 1024ull * 1024ull;
inline std::mutex& RecvInflightMutex() {
    static std::mutex m;
    return m;
}
inline std::unordered_map<std::string, uint64_t>& RecvInflightBySource() {
...
class Connection {
  public:
    ...
    static PayloadBounds BoundsForCommand(const std::string& command) {
        ...
        if (command == MessageType::VERACK || command == MessageType::GETADDR ||
            command == MessageType::MEMPOOL)
            return {0, 0};
        if (command == MessageType::PUNCHHELLO || command == MessageType::GETPUNCH)
        ...
        if (command == MessageType::SENDCMPCT)
            return {0, 16};
        if (command == MessageType::CMPCTBLOCK || command == MessageType::BLOCKTXN)
    ...
    static bool DecodeCanonicalHeader(const uint8_t header[24], uint32_t expected_magic,
                                      std::string& command, uint32_t& payload_len) {
        ...
        payload_len = (uint32_t)header[16] | ((uint32_t)header[17] << 8) |
                      ((uint32_t)header[18] << 16) | ((uint32_t)header[19] << 24);
        const PayloadBounds bounds = BoundsForCommand(command);
        return payload_len >= bounds.minimum && payload_len <= bounds.maximum &&
               payload_len <= MAX_MESSAGE_SIZE;
    }

    explicit Connection(SocketHandle fd, const std::string& remote_addr, uint16_t remote_port,
                        bool inbound = false)
    ...
    static constexpr size_t EVENT_SEND_PER_CONNECTION_CAP = 16ull * 1024ull * 1024ull;
    ...
    static uint64_t TestGlobalRecvInflightBytes() {
        return _detail::RecvInflightBytes().load(std::memory_order_acquire);
    }
    static uint64_t TestPerSourceRecvInflightBytes(const std::string& source) {
    ...
    void TestAgeIncompleteFrame(uint32_t seconds) {
        recv_frame_started_at_.store(std::chrono::steady_clock::now() -
    ...
    bool CanQueueEventLoopSend(size_t max_frame_bytes) const {
        ...
        if (max_frame_bytes > EVENT_SEND_PER_CONNECTION_CAP ||
            max_frame_bytes > _detail::G_SEND_QUEUED_CAP)
            return false;
        std::lock_guard<std::mutex> lock(event_send_mutex_);
        if (!connected_.load(std::memory_order_acquire) ||
            event_send_bytes_ > EVENT_SEND_PER_CONNECTION_CAP - max_frame_bytes ||
            event_send_queue_.size() >= EVENT_SEND_PER_CONNECTION_FRAME_CAP)
            return false;
        const uint64_t global = _detail::SendQueuedBytes().load(std::memory_order_relaxed);
        const uint64_t global_frames = _detail::SendQueuedFrames().load(std::memory_order_relaxed);
        ...
        bool fatal = false;
        {
            ...
            while (budget > 0 && !event_send_queue_.empty()) {
                ...
                if (event_send_offset_ >= front.size()) {
                    ...
                    _detail::SendQueuedFrames().fetch_sub(1, std::memory_order_relaxed);
                ...
                ssize_t sent = -1;
                {
                    ...
                    sent =
                        ::send(fd, reinterpret_cast<const char*>(front.data()) + event_send_offset_,
                               static_cast<int>(std::min<size_t>(want, INT_MAX)), MSG_NOSIGNAL);
#else
                    sent =
                        ::send(fd, reinterpret_cast<const char*>(front.data()) + event_send_offset_,
    ...
    bool Send(const std::vector<uint8_t>& data) {
        if (EventLoopIOEnabled())
            return QueueEventLoopSend_(data);
        ...
        while (total < data.size()) {
            ssize_t sent = -1;
            {
                std::shared_lock<std::shared_mutex> io_lock(fd_io_mutex_);
                const SocketHandle fd = fd_.load(std::memory_order_acquire);
                if (!connected_.load(std::memory_order_acquire) || !veld::compat::IsValidSocket(fd))
                    return false;
                sent =
                    ::send(fd, (const char*)data.data() + total, data.size() - total, MSG_NOSIGNAL);
            }
            if (sent < 0) {
                bool retry = false;
#ifdef _WIN32
    ...
    bool RecvExact(uint8_t* buf, size_t n, int timeout_ms = 30000) {
        ...
        while (total < n) {
            ...
            last_recv_at_.store(std::chrono::steady_clock::now(), std::memory_order_release);
        }
        return true;
    }
    ...
    std::optional<P2PMessage> RecvMessage(uint32_t expected_magic) {
        ...
        if (!DecodeCanonicalHeader(header, expected_magic, command, payload_len)) {
        ...
        struct InflightGuard {
            ...
            ~InflightGuard() {
                if (connection)
                    connection->ReleaseRecvInflight_();
            }
        } guard{this};
        if (payload_len > 0) {
            recv_frame_started_at_.store(std::chrono::steady_clock::now(),
        ...
        if (chk[0] != header[20] || chk[1] != header[21] || chk[2] != header[22] ||
            chk[3] != header[23]) {
            Close(CloseReason::InvalidFrame);
            return std::nullopt;
        }
    ...
    TryRecvResult TryRecvMessage(uint32_t expected_magic) {
...
#ifdef _WIN32
                WSAGetLastError();
            bool wouldblock = (err == WSAEWOULDBLOCK || err == WSAEINTR || err == WSAETIMEDOUT);
#else
                errno;
        ...
        recv_frame_started_at_.compare_exchange_strong(
            no_frame_started, std::chrono::steady_clock::now(), std::memory_order_acq_rel,
        ...
        if (rx_.state == RecvState::NeedHeader && rx_.bytes.size() >= 24) {
            ...
            std::string command;
            uint32_t payload_len = 0;
            if (!DecodeCanonicalHeader(h, expected_magic, command, payload_len)) {
                Close(CloseReason::InvalidFrame);
                out.status = TryRecvStatus::Error;
                return out;
            }
        ...
        if (rx_.state == RecvState::NeedPayload &&
            ...
            std::vector<uint8_t> payload(rx_.bytes.begin() + 24,
                                         rx_.bytes.begin() + 24 + rx_.pending_payload_len);
            const uint8_t* h = rx_.bytes.data();
            Hash256 chk = Hash256d(payload);
            if (chk[0] != h[20] || chk[1] != h[21] || chk[2] != h[22] || chk[3] != h[23]) {
                ReleaseRecvInflight_();
                Close(CloseReason::InvalidFrame);
                out.status = TryRecvStatus::Error;
                return out;
            }
            out.msg = P2PMessage(expected_magic, rx_.pending_command, std::move(payload));
    ...
    void Close(CloseReason reason = CloseReason::Unspecified) {
        ...
        const bool was_connected =
            connected_.compare_exchange_strong(expected, false, std::memory_order_acq_rel);
        ...
        if (!was_connected)
            return;
        connection_diagnostics::Record(identity_, reason, inbound_, bytes_sent_.load(),
                                       bytes_recv_.load());
        ...
        const SocketHandle fd =
            fd_.exchange(veld::compat::kInvalidSocket, std::memory_order_acq_rel);
    ...
    bool IsConnected() const {
        return connected_.load(std::memory_order_acquire) &&
               veld::compat::IsValidSocket(fd_.load(std::memory_order_acquire));
    }
    ...
    std::string DiagnosticId() const {
        return connection_diagnostics::Id(identity_);
    }
    ...
    bool TipReceived() const {
        return tip_received_.load(std::memory_order_acquire);
    }
    void MarkTipReceived() {
        tip_received_.store(true, std::memory_order_release);
    }

    bool VersionReceived() const {
        return version_received_.load(std::memory_order_acquire);
    }
    void MarkVersionReceived() {
        version_received_.store(true, std::memory_order_release);
    }
    bool HandshakeReady() const {
        return handshake_ready_.load(std::memory_order_acquire);
    }
    void MarkHandshakeReady() {
        handshake_ready_.store(true, std::memory_order_release);
    }
    ...
    uint64_t PeerNonce() const {
        return peer_nonce_.load(std::memory_order_acquire);
    }
...
#ifdef _WIN32
        u_long pending = 0;
        if (::ioctlsocket((SOCKET)fd, FIONREAD, &pending) != 0)
            return 0;
    ...
    bool RecvFrameAssemblyExpired(std::chrono::steady_clock::time_point now) const {
        const auto started = recv_frame_started_at_.load(std::memory_order_acquire);
        return started != std::chrono::steady_clock::time_point{} && now >= started &&
               std::chrono::duration_cast<std::chrono::seconds>(now - started).count() >=
                   static_cast<long long>(RECV_FRAME_ASSEMBLY_TIMEOUT_SECONDS);
    }
    void MarkRecvNow() {
        last_recv_at_.store(std::chrono::steady_clock::now(), std::memory_order_release);
    }
  ...
  private:
    ...
    bool QueueEventLoopSend_(const std::vector<uint8_t>& data) {
        if (data.empty())
            return true;
        if (data.size() > EVENT_SEND_PER_CONNECTION_CAP || data.size() > _detail::G_SEND_QUEUED_CAP)
            return false;
        std::lock_guard<std::mutex> lock(event_send_mutex_);
        if (!connected_.load(std::memory_order_acquire) || !EventLoopIOEnabled() ||
            event_send_bytes_ > EVENT_SEND_PER_CONNECTION_CAP - data.size() ||
            event_send_queue_.size() >= EVENT_SEND_PER_CONNECTION_FRAME_CAP)
            return false;
        const uint64_t n = static_cast<uint64_t>(data.size());
        const uint64_t prior = _detail::SendQueuedBytes().fetch_add(n, std::memory_order_relaxed);
        if (prior > _detail::G_SEND_QUEUED_CAP - n) {
            _detail::SendQueuedBytes().fetch_sub(n, std::memory_order_relaxed);
            return false;
        }
        const uint64_t prior_frames =
            _detail::SendQueuedFrames().fetch_add(1, std::memory_order_relaxed);
    ...
    void ReleaseQueuedSend_() {
        ...
        if (remaining != 0) {
            _detail::SendQueuedBytes().fetch_sub(remaining, std::memory_order_relaxed);
        }
        if (remaining_frames != 0) {
            _detail::SendQueuedFrames().fetch_sub(remaining_frames, std::memory_order_relaxed);
        }
    }
    ...
    bool ReserveRecvInflight_(uint64_t bytes) {
        if (bytes == 0)
            return true;
        const std::string source = remote_addr_.empty() ? std::string("<unknown>") : remote_addr_;
        std::lock_guard<std::mutex> lock(_detail::RecvInflightMutex());
        if (!connected_.load(std::memory_order_acquire))
            return false;
        const uint64_t total = _detail::RecvInflightBytes().load(std::memory_order_relaxed);
        auto& by_source = _detail::RecvInflightBySource();
        const auto found = by_source.find(source);
        const uint64_t source_total = found == by_source.end() ? 0 : found->second;
        if (bytes > _detail::G_RECV_INFLIGHT_CAP - std::min(total, _detail::G_RECV_INFLIGHT_CAP) ||
            bytes > _detail::G_RECV_INFLIGHT_PER_SOURCE_CAP -
                        std::min(source_total, _detail::G_RECV_INFLIGHT_PER_SOURCE_CAP))
            return false;
        try {
            if (found == by_source.end())
                by_source.emplace(source, bytes);
            else
                found->second += bytes;
        } catch (...) {
            return false;
        }
        _detail::RecvInflightBytes().store(total + bytes, std::memory_order_relaxed);
        rx_.inflight_accounted.fetch_add(bytes, std::memory_order_relaxed);
        return true;
    }
    ...
    void ReleaseRecvInflight_() {
        const std::string source = remote_addr_.empty() ? std::string("<unknown>") : remote_addr_;
        std::lock_guard<std::mutex> lock(_detail::RecvInflightMutex());
        const uint64_t accounted = rx_.inflight_accounted.exchange(0, std::memory_order_acq_rel);
        if (accounted != 0) {
            const uint64_t total = _detail::RecvInflightBytes().load(std::memory_order_relaxed);
            _detail::RecvInflightBytes().store(total >= accounted ? total - accounted : 0,
                                               std::memory_order_relaxed);
            auto& by_source = _detail::RecvInflightBySource();
            auto found = by_source.find(source);
            if (found != by_source.end()) {
                if (found->second > accounted)
                    found->second -= accounted;
                else
                    by_source.erase(found);
            }
        }
        recv_frame_started_at_.store(std::chrono::steady_clock::time_point{},
                                     std::memory_order_release);
    }
    ...
    std::atomic<std::chrono::steady_clock::time_point> recv_frame_started_at_{
...
class NodeServer {
  public:
    ...
    using PeerWorkViewTransitionFn = std::function<PeerWorkViewTransitionPermit()>;
      ...
      public:
        explicit PeerWorkViewWriteGuard_(NodeServer& owner) noexcept
            : PeerWorkViewWriteGuard_(
                  owner, std::unique_lock<std::mutex>(owner.peer_work_view_writer_mutex_)) {}
        ...
        PeerWorkViewWriteGuard_(NodeServer& owner,
            ...
            owner_.peer_work_view_write_pending_.store(true, std::memory_order_release);
            PeerWorkViewTransitionFn transition;
            {
                std::lock_guard<std::mutex> lock(owner_.peer_work_view_transition_mutex_);
            ...
            if (transition) {
                ...
                if (!transition_permit_) {
                    owner_.peer_work_view_sequencer_failed_.store(true, std::memory_order_release);
                }
            }
        ...
        PeerWorkViewWriteGuard_& operator=(const PeerWorkViewWriteGuard_&) = delete;
        ...
        ~PeerWorkViewWriteGuard_() noexcept {
            ...
            // post-mutation tuple.
            transition_permit_.reset();
            owner_.peer_work_view_write_pending_.store(false, std::memory_order_release);
        }
  ...
  public:
    ...
    using FinalityVotePrecheck = std::function<bool(const std::vector<uint8_t>&)>;
    ...
    static bool MarkMempoolInventoryRequestIfDue(PeerState& ps, PeerState::tp_t now) {
    ...
    void SetPeerWorkViewTransitionFn(PeerWorkViewTransitionFn fn) {
        ...
        std::lock_guard<std::mutex> callback_lock(peer_work_view_transition_mutex_);
        peer_work_view_transition_fn_ = std::move(fn);
        peer_work_view_sequencer_wired_.store(static_cast<bool>(peer_work_view_transition_fn_),
                                              std::memory_order_release);
    }

    void SetAdvertisedServices(uint64_t services) {
        local_services_.store(services | MessageType::NODE_FULL | MessageType::NODE_HOLE_PUNCH |
                                  MessageType::REQUIRED_NETWORK_IDENTITY_SERVICES,
                              std::memory_order_release);
    }

    uint64_t TopologyId() const {
        return self_nonce_;
    }

    std::string TopologyRole() const {
        const uint64_t services = local_services_.load(std::memory_order_acquire);
        if ((services & MessageType::NODE_FLEET) != 0)
            return "fleet";
        if ((services & MessageType::NODE_VALIDATOR) != 0)
            return "validator";
        if ((services & MessageType::NODE_MINER) != 0)
            return "miner";
        return "node";
    }

    static std::string ClassifyPeerTopologyRole(bool configured_fleet, uint64_t services) {
    ...
    bool Start(const std::function<bool()>& activation_guard = {}, bool accept_inbound = true) {
        ...
        if (running_.load(std::memory_order_acquire) || veld::compat::IsValidSocket(listen_fd_) ||
            accept_thread_.joinable() || !el_accept_threads_.empty() || !el_workers_.empty()) {
        ...
        if (env_eloop && env_eloop[0]) {
            if (std::strcmp(env_eloop, "1") == 0) {
                ...
                return false;
#else
                const char* unsafe = std::getenv("VELD_ALLOW_UNSAFE_THREAD_PER_PEER");
        ...
        if (veld::DiagVerbose().load()) {
            std::cerr << "  [p2p-policy] io="
                      ...
                      << " max_inbound=" << max_inbound_connections_ << " (" << inbound_source
                      << ")\n";
            std::cerr.flush();
        }
        ...
        if (self_nonce_ == 0) {
            ...
            if (fallback == 0)
                fallback = 0x5645'4c44'5f4e'4f4eULL;
        ...
        if (accept_inbound) {
            ...
            if (!veld::compat::IsValidSocket(listen_fd_))
                return false;
            ...
            if (::ioctlsocket((SOCKET)listen_fd_, FIONBIO, &listener_nonblocking) != 0) {
                VELD_CLOSE_SOCKET(listen_fd_);
                listen_fd_ = veld::compat::kInvalidSocket;
                return false;
            }
        ...
        if (accept_inbound) {
            ...
            local_listener_endpoint_.store((uint64_t(ntohs(bound.sin_port)) << 32) |
                                               bound.sin_addr.s_addr,
                                           std::memory_order_release);
        }
            ...
            if (use_event_loop_) {
                ...
                if (veld::DiagVerbose().load()) {
                    std::cerr << "  [event-loop] acceptors="
                              ...
                              << " workers=" << n_workers << " source=" << worker_source
                              << " hw=" << hw << " ingest_workers=1\n";
                    std::cerr.flush();
                }

                {
                    ...
                    for (auto& wp : el_workers_) {
                        ...
                        raw->thread =
                            std::thread(&NodeServer::EventLoopWorkerThreadEntry_, this, raw);
                        if (ShouldInjectStartFailure_(1))
                            throw std::runtime_error("injected event-worker start failure");
                    }
                }

                ingest_running_.store(true, std::memory_order_release);
                ingest_worker_thread_ = std::thread(&NodeServer::BlockIngestThreadEntry_, this);
                ...
                if (accept_inbound) {
                    for (int ai = 0; ai < ACCEPT_THREAD_COUNT; ++ai) {
                        el_accept_threads_.emplace_back(&NodeServer::AcceptThreadEntry_, this);
                        if (ai == 1 && ShouldInjectStartFailure_(3))
                            throw std::runtime_error("injected partial-acceptor start failure");
                    }
                }
            } else if (accept_inbound) {
                accept_thread_ = std::thread(&NodeServer::LegacyAcceptThreadEntry_, this);
            }
        } catch (const std::exception& e) {
            std::cerr << "  [p2p-policy] Start worker creation failed: " << e.what() << "\n";
    ...
    void EnablePortMapping() {
        port_mapper_.Start(port_);
    }
    bool PortMapped() const {
        return port_mapper_.Reachable();
    }
    ...
    void EnableHolePunch() {
        hole_punch_enabled_.store(true);
    }
    ...
    void HolePunchTick() {
        if (!hole_punch_enabled_.load())
            return;
        if (port_mapper_.Reachable())
        ...
        std::shared_ptr<Connection> coordinator;
        {
            ...
            for (auto& kv : peer_connections_) {
                if (kv.second && !kv.second->IsInbound() && HolePunchCapable_(*kv.second)) {
        ...
        if (have_hello) {
            ...
            coordinator->TrySend(
                P2PMessage(magic_, MessageType::PUNCHHELLO, std::move(payload)).Serialize());
        }
        if (send_get) {
            std::vector<uint8_t> payload;
            ...
            coordinator->TrySend(
                P2PMessage(magic_, MessageType::GETPUNCH, std::move(payload)).Serialize());
        }
        ...
        for (auto& candidate : candidates) {
            ...
            payload.insert(payload.end(), candidate.endpoint.begin(), candidate.endpoint.end());
            if (seed->TrySend(
                    P2PMessage(magic_, MessageType::PUNCHREQ, std::move(payload)).Serialize())) {
    ...
    void Stop() {
        ...
        // lease destructor and still leave a use-after-free window.
        {
            ...
            pending_dial_cv_.wait(
                dial_lock, [&] { return pending_dials_.load(std::memory_order_acquire) == 0; });
        }

        {
            ...
            for (auto& wp : el_workers_) {
                if (wp)
                    wp->running.store(false, std::memory_order_release);
            }
        }
        for (auto& wp : el_workers_) {
            if (wp && wp->thread.joinable())
                wp->thread.join();
        }
        {
            std::lock_guard<std::mutex> workers_lock(worker_registry_mutex_);
    ...
    static bool IsCanonicalIPv4Literal(const std::string& ip) {
        ...
        if (!::inet_ntop(AF_INET, &parsed, canonical, sizeof(canonical)))
            return false;
        return ip == canonical && ip != "0.0.0.0" && ip != "255.255.255.255";
    }

    static bool ParsePublicIPv4Endpoint(const std::string& endpoint, std::string& ip_out,
                                        uint16_t& port_out) {
        const size_t colon = endpoint.find(':');
        if (colon == std::string::npos || colon == 0 || colon != endpoint.rfind(':') ||
            colon + 1 >= endpoint.size())
            return false;
        const std::string ip = endpoint.substr(0, colon);
        if (!IsPublicRoutableIPv4(ip))
            return false;
        uint32_t port = 0;
    ...
    static bool ParseCanonicalOnionEndpoint(const std::string& endpoint, std::string& host_out,
                                            uint16_t& port_out) {
    ...
    class PendingDialLease {
      public:
        explicit PendingDialLease(NodeServer& owner) : owner_(&owner) {
            ...
            const uint32_t pending = owner.pending_dials_.load(std::memory_order_acquire);
            const uint32_t connected = owner.outbound_count_.load(std::memory_order_acquire);
            if (owner.running_.load(std::memory_order_acquire) && pending < MAX_PENDING_DIALS &&
        ...
        PendingDialLease(PendingDialLease&& other) noexcept : owner_(other.owner_) {
        ...
        ~PendingDialLease() {
            if (owner_) {
                ...
                if (owner_->pending_dials_.fetch_sub(1, std::memory_order_acq_rel) == 1) {
    ...
    bool IsLocalListenerEndpoint(const std::string& host, uint16_t port) const {
        ...
        if (listener == 0 || port != uint16_t(listener >> 32))
            return false;
        if (host.size() > 6 && host.compare(host.size() - 6, 6, ".onion") == 0)
            return host == OnionAddress();
        in_addr resolved{};
        if (::inet_pton(AF_INET, host.c_str(), &resolved) != 1)
            return false;
    ...
    bool ConnectTo(const std::string& host, uint16_t port, bool explicitly_trusted = false,
                   bool fleet_anchor = false) {
#ifdef VELD_LOCAL_TEST_NETWORK
        if (host != "127.0.0.1")
            return false; // before DNS, trust grants or socket creation
#endif
        if (host.empty() || port == 0)
            return false;
        if (fleet_anchor && (!explicitly_trusted || !IsCanonicalIPv4Literal(host)))
            return false;
        if (IsLocalListenerEndpoint(host, port))
            return false;
        if (explicitly_trusted) {
            if (fleet_anchor) {
                if (!AddFleetAnchorIp(host))
                    return false;
            } else
                AddTrustedIP(host);
        }
    ...
    bool ConnectToReserved_(const std::string& host, uint16_t port, bool explicitly_trusted = false,
                            bool fleet_anchor = false) {
#ifdef VELD_LOCAL_TEST_NETWORK
        if (host != "127.0.0.1")
            return false; // before DNS, trust grants or socket creation
        ...
        bool is_onion = (host.size() > 6 && host.compare(host.size() - 6, 6, ".onion") == 0);
        ...
        if (is_onion || tor_only_.load()) {
            if (is_onion && !tor_only_.load() && !tor_.Active())
                return false;
            SocketHandle ofd = Socks5Connect(host, port);
            if (!veld::compat::IsValidSocket(ofd))
                return false;
        ...
        for (auto* it = res; it; it = it->ai_next) {
            ...
            if (explicitly_trusted) {
                if (fleet_anchor)
                    AddFleetAnchorIp(std::string(ip_buf));
                else
                    AddTrustedIP(std::string(ip_buf));
            }
        ...
        ::WSAIoctl(fd, 0x98000004, &kav, sizeof(kav), nullptr, 0, &bytes_returned, nullptr,
                   nullptr);
#endif
        ...
        return FinishOutboundConnection(fd, resolved_ip.empty() ? host : resolved_ip, port);
    }
    ...
    bool FinishOutboundConnection(SocketHandle fd, const std::string& host, uint16_t port) {
        ...
        bool admitted = false;
        {
            ...
            admitted = RegisterAdmittedConnectionLocked_(key, conn, same_key);
            if (same_key)
                superseded_connections.push_back(same_key);
        }
        ...
        if (!conn->Send(initial_pm.BuildVersionMessage(chain_.Height(), self_nonce_))) {
        ...
        try {
            if (!SpawnTrackedPeerThread(
                    [this, conn, key]() { HandlePeer(conn, key, false, true); })) {
                FinalizePeerConnection_(key, conn);
                return false;
            }
    ...
    void SetSolutionCallback(
        std::function<void(uint64_t, const Hash256&, uint64_t, const std::vector<uint8_t>&)> cb) {
        solution_cb_ = cb;
    }
    void SetCOMineCallback(std::function<bool(uint64_t, const Hash256&, uint64_t, const Hash256&,
    ...
    void AddKnownPeer(const std::string& ip, uint16_t port) {
        ...
        if (known_peer_addrs_.size() >= PEER_CACHE_MAX_ENTRIES)
            return;
        ...
        if (subnet_it != known_peer_subnet_counts_.end() &&
            subnet_it->second >= ADDR_BOOK_PER_SUBNET_CAP)
            return;
    ...
    bool AddFleetAnchorIp(const std::string& ip) {
        if (!IsCanonicalIPv4Literal(ip))
            return false;
        std::lock_guard<std::mutex> lock(ban_mutex_);
        const auto inserted = fleet_anchor_ips_.insert(ip).second;
        if (inserted) {
            fleet_anchor_indices_[ip] = static_cast<uint32_t>(fleet_anchor_indices_.size() + 1);
        }
        trusted_ips_.insert(ip);
        return true;
    }
    ...
    size_t LoadBansFromFile() {
        ...
        while (std::getline(f, line)) {
            ...
            try {
                expiry_epoch = std::stoull(line.substr(sp + 1));
            } catch (...) {
                continue;
            }
        ...
        if (loaded > 0) {
            std::cout << "  [DoS] loaded " << loaded << " persistent ban(s) from " << ban_file_path_
                      << "\n";
        }
        return loaded;
    }
    ...
    size_t LoadPeerCache() {
        ...
        if (fsize > PEER_CACHE_MAX_FILE_BYTES) {
            std::cerr << "  [peer-cache] refusing to load oversized file (" << fsize << " bytes > "
                      << PEER_CACHE_MAX_FILE_BYTES << " cap) at " << peer_cache_path_ << "\n";
            std::cerr.flush();
            return 0;
        }
        ...
        while (std::getline(f, line)) {
            ...
            if (last_seen + PEER_CACHE_MAX_AGE_SECONDS < now_epoch)
                continue;
            if (last_seen > now_epoch + 3600)
                continue;
        ...
        if (loaded > 0) {
            std::cout << "  [peer-cache] loaded " << loaded << " known peer(s) from "
                      << peer_cache_path_ << "\n";
        }
        return loaded;
    }
    ...
    std::vector<std::pair<std::string, uint16_t>> LoadAnchors() {
        ...
        if (!out.empty()) {
            std::cout << "  [anchors] loaded " << out.size() << " outbound anchor(s) from "
                      << anchors_path_ << "\n";
        }
        return out;
    }
    ...
    void SaveAnchors() {
        ...
        std::vector<Candidate> cands;
        {
            ...
            for (const auto& [key, conn] : peer_connections_) {
                ...
                if (trusted_ips_.count(ip))
                    continue;
                if (fleet_anchor_ips_.count(ip))
                    continue;
                if (ip.empty() || ip == "0.0.0.0" || ip == "127.0.0.1")
                    continue;
        ...
        if (cands.size() > ANCHOR_COUNT)
            cands.resize(ANCHOR_COUNT);
  ...
  private:
    static bool ReplaceFileAtomically_(const std::string& tmp, const std::string& destination) {
#ifdef _WIN32
    ...
    void EnforceBanListCap_() {
        if (banned_ips_.size() <= MAX_BANNED_IPS)
            return;
        std::vector<std::pair<uint64_t, std::string>> by_expiry;
        by_expiry.reserve(banned_ips_.size());
        for (const auto& [ip, exp] : banned_ips_)
            by_expiry.emplace_back(exp, ip);
  ...
  public:
    void RecordViolation(const std::string& ip, uint32_t weight = 1, const char* tag = nullptr) {
        ...
        bool newly_banned = false;
        {
                ...
                if (it == violations_.end()) {
                    ...
                    while (!violation_order_.empty()) {
                        ...
                        if (violations_.size() < VIOLATION_TABLE_CAP ||
                            now <= oldest->second.last_touch + VIOLATION_TTL_SECONDS) {
                            break;
                        }
                        EraseViolationLocked_(oldest);
                    }
                    if (violations_.size() >= VIOLATION_TABLE_CAP && !violation_order_.empty()) {
                        auto oldest = violations_.find(violation_order_.front());
                        if (oldest != violations_.end())
                            EraseViolationLocked_(oldest);
                        else
                    ...
                    it = violations_.emplace(ip, ViolationEntry{0, now, order_it}).first;
                } else {
                    if (now > it->second.last_touch + VIOLATION_TTL_SECONDS)
                        it->second.count = 0;
                    violation_order_.splice(violation_order_.end(), violation_order_,
                                            it->second.order_it);
                    it->second.order_it = std::prev(violation_order_.end());
                }
                auto& entry = it->second;
                if (entry.count >= BAN_THRESHOLD || weight >= BAN_THRESHOLD - entry.count) {
            ...
            if (rate_it == violation_log_rate_.end()) {
                while (!violation_log_order_.empty()) {
                    auto oldest = violation_log_rate_.find(violation_log_order_.front());
                    if (oldest == violation_log_rate_.end()) {
                        violation_log_order_.pop_front();
                        continue;
                    }
                    if (now <= oldest->second.last_touch + VIOLATION_LOG_RETENTION_SECONDS)
                        break;
                ...
                rate_it = violation_log_rate_
                              .emplace(log_key, ViolationLogRateEntry{now, 0, now, order_it})
                              .first;
            } else {
                violation_log_order_.splice(violation_log_order_.end(), violation_log_order_,
                                            rate_it->second.order_it);
                rate_it->second.order_it = std::prev(violation_log_order_.end());
            ...
            if (emit_line) {
                const bool ingest_flow_control =
                    log_only && tag && std::string_view(tag) == "block_ingest_overflow";
                if (ingest_flow_control) {
                    std::cerr << "  [sync] ip=" << ip
                              << " Block validation queue busy; incoming block not queued. "
                                 "No peer penalty.\n";
                } else {
                    std::cerr << "  [violation] ip=" << ip << " tag=" << (tag ? tag : "untagged")
                              << " weight=" << weight << " count=" << count << "/" << BAN_THRESHOLD
                              << (log_only ? " (log-only)" : "") << "\n";
                }
            ...
            if (!log_only && count >= BAN_THRESHOLD && banned_ips_.find(ip) == banned_ips_.end()) {
                banned_ips_[ip] = now + 3600;
                EnforceBanListCap_();
                EraseViolationLocked_(ip);
                std::cout << "  [DoS] Banned peer " << ip << " for 1 hour"
                          << " (weight=" << weight << " final_tag=" << (tag ? tag : "untagged")
                          << ")\n";
                bans_dirty_ = true;
        ...
        if (newly_banned)
            RetireConnectionsForBannedIp_(ip);
    }

    struct RateLimitDecision {
        bool allow;
        bool record_violation;
    };
    ...
    void SetMinerScript(const std::vector<uint8_t>& script) {
        my_miner_script_ = script;
    }
    using ComineSigner = std::function<std::vector<uint8_t>(const std::vector<uint8_t>&)>;
    void SetCOMineIdentity(const std::vector<uint8_t>& pubkey, ComineSigner signer) {
        my_comine_pubkey_ = pubkey;
        my_comine_signer_ = std::move(signer);
    }
    void SetIBDComplete(bool v) {
        ibd_complete_flag_.store(v);
    }
    ...
    PeerHeightView GetPeerHeightView() const {
        ...
        uint64_t earliest_freshness_deadline = UINT64_MAX;
        {
            ...
            for (const auto& [connection_id, source] : peer_work_sources_) {
                if (!source.version_received)
                    continue;
                active_sources.push_back(
                    {connection_id, source.source_ip, source.handshake_ready, source.inbound});
            ...
            for (const auto& live : active_sources) {
                ...
                if (claimed == peer_heights_.end() || claimed->second.source_ip != live.source_ip)
                    continue;
                if (!live.handshake_ready)
                    continue;
                if (!live.inbound) {
                    auto sync_claim = peer_sync_heights_.find(live.connection_id);
                    if (sync_claim == peer_sync_heights_.end() ||
                        sync_claim->second.source_ip != live.source_ip ||
                        !VersionHeightHintFresh_(sync_claim->second.updated_at, height_now))
                        continue;
                }
                if (!VersionHeightHintFresh_(claimed->second.updated_at, height_now))
                    continue;
                ...
                if (chosen == selected.end() ||
                    ...
                    selected[live.source_ip] = SelectedClaim{
                        claimed->second.height, claimed->second.updated_at, live.connection_id};
                }
            }
        ...
        if (claims.size() >= (size_t)VERSION_FETCH_HINT_MIN_SUPPORT) {
            ...
            const uint64_t raw_hint = claims[VERSION_FETCH_HINT_MIN_SUPPORT - 1];
            const uint64_t local_height = chain_.Height();
            const uint64_t max_hint = local_height > UINT64_MAX - VERSION_HEIGHT_HINT_MAX_AHEAD
                                          ? UINT64_MAX
            ...
            version_fetch_hint_support =
                static_cast<size_t>(std::count(claims.begin(), claims.end(), raw_hint));
        }
        uint64_t freshness_valid_for_ms = UINT64_MAX;
        if (earliest_freshness_deadline != UINT64_MAX) {
            ...
            if (height_now == UINT64_MAX || earliest_freshness_deadline <= height_now ||
                earliest_freshness_deadline - height_now <= 1) {
                freshness_valid_for_ms = 0;
            } else {
                const uint64_t safe_seconds = earliest_freshness_deadline - height_now - 1;
                freshness_valid_for_ms =
                    safe_seconds > UINT64_MAX / 1000 ? UINT64_MAX : safe_seconds * 1000;
            }
        }
        ...
        const bool stable = (generation_before & 1U) == 0 &&
                            ...
                            !peer_work_view_write_pending_.load(std::memory_order_acquire) &&
                            !peer_work_view_sequencer_failed_.load(std::memory_order_acquire);
        const bool wired = peer_work_view_sequencer_wired_.load(std::memory_order_acquire);
        return PeerHeightView{verified_best,
                              version_fetch_hint,
    ...
    static std::optional<uint64_t> BoundPeerWorkLifetimeMs(const PeerHeightView& view,
                                                           uint64_t requested_ms,
                                                           uint64_t safety_margin_ms) noexcept {
        if (requested_ms == 0 || !view.work_sequencer_wired || !view.work_view_stable)
            return std::nullopt;
        ...
        const uint64_t bounded =
            std::min(requested_ms, view.freshness_valid_for_ms - safety_margin_ms);
    ...
    int GetConnectedAnchorCount() const {
        ...
        std::unordered_set<std::string> active_ips;
        {
            ...
            for (const auto& [_key, conn] : peer_connections_) {
                if (!conn || !conn->IsConnected() || !conn->HandshakeReady() || conn->IsInbound())
                    continue;
    ...
    uint64_t GetReceivedVersionCount() const {
        return received_version_count_.load();
    }
    uint64_t GetGenesisMatchCount() const {
        return genesis_match_count_.load();
    }
    uint64_t GetGenesisMismatchCount() const {
        return genesis_mismatch_count_.load();
    }
    ...
    std::vector<PeerTipSnapshot> SnapshotPeerTips(bool configured_outbound_anchors_only = false,
        ...
        std::vector<LiveTipSource> live_sources;
        {
            ...
            for (const auto& [_key, conn] : peer_connections_) {
                if (!conn || !conn->IsConnected() || !conn->HandshakeReady())
                    continue;
                if (configured_outbound_anchors_only &&
                    (conn->IsInbound() || configured_anchors.count(conn->RemoteAddr()) == 0))
                    continue;
                live_sources.push_back({conn->Identity(), conn->RemoteAddr(), conn->IsInbound(),
                                        conn->DiagnosticId()});
            }
        }
        ...
        std::unordered_map<std::string, std::pair<uint64_t, PeerTipSnapshot>> selected;
        {
            ...
            for (const auto& live : live_sources) {
                ...
                if (it == peer_tips_.end() || it->second.source_ip != live.source_ip ||
                    it->second.inbound != live.inbound)
                    continue;
                PeerTipSnapshot candidate{live.source_ip, it->second.hash, it->second.height,
                                          it->second.updated_at, live.diagnostic_id};
                ...
                if (chosen == selected.end() ||
                    ...
                    selected[source_key] = {live.connection_id, std::move(candidate)};
                }
            }
        }
        ...
        out.erase(std::remove_if(out.begin(), out.end(),
                                 [this](const auto& s) {
                                     const std::string active =
                                         chain_.GetBlockHashAtHeight(s.height);
    ...
    void ImportPeerTipSnapshots(const std::vector<PeerTipSnapshot>& snaps, int64_t now_s) {
        ...
        for (const auto& s : snaps) {
            if (s.ip.empty())
                continue;
            if (now_s - s.updated_at > PEER_TIP_HARD_TTL_S)
                continue;
            if (s.updated_at > now_s + 60)
                continue;
            auto known = CanonicalPeerEvidence_(s.hash);
            if (!known.has_value() || *known != s.height)
                continue;
            ...
            if (peer_tip_recovery_hints_.size() >= PEER_TIPS_CAP &&
                ...
                auto oldest = std::min_element(peer_tip_recovery_hints_.begin(),
            ...
            peer_tip_recovery_hints_[s.ip] = PeerTipRecoveryHint{s.hash, s.height, s.updated_at};
        }
    }
    void RecordPeerTip(Connection& conn, const Hash256& hash, uint64_t height, int64_t now_s) {
        if (!conn.IsConnected() || !conn.HandshakeReady() || conn.RemoteAddr().empty())
            return;
        if (HashIsZero(hash))
            return;
        ...
        height = *known;
        {
            ...
            for (const auto& [_key, live] : peer_connections_) {
                if (live && live.get() == &conn && live->IsConnected() && live->HandshakeReady()) {
                    exact_live = true;
                    break;
                }
            }
        ...
        if (peer_tips_.count(connection_id) == 0 && peer_tips_.size() >= PEER_TIPS_CAP) {
            auto oldest = std::min_element(peer_tips_.begin(), peer_tips_.end(),
                                           [](const auto& a, const auto& b) {
                                               return a.second.updated_at < b.second.updated_at;
                                           });
            if (oldest != peer_tips_.end())
                peer_tips_.erase(oldest);
        }
  ...
  private:
    ActiveClockDriftSnapshot
    CollectClockDriftSnapshot_(const std::unordered_set<std::string>* configured_anchors,
        ...
        for (const auto& [_key, conn] : peer_connections_) {
            if (!conn || !conn->IsConnected() || !conn->HandshakeReady())
                continue;
            if (outbound_only && conn->IsInbound())
                continue;
            if (configured_anchors && configured_anchors->count(conn->RemoteAddr()) == 0)
                continue;
            live_sources.push_back({conn->Identity(), conn->RemoteAddr(), conn->IsInbound()});
        }
        ...
        std::vector<int64_t> samples;
        {
            ...
            for (const auto& live : live_sources) {
                ...
                if (it == clock_drift_samples_.end())
                    continue;
                if (now < it->second.updated_at ||
                    now - it->second.updated_at > CLOCK_DRIFT_TTL_SECONDS)
                    continue;
                if (it->second.source_ip != live.source_ip || it->second.inbound != live.inbound)
                    continue;
                auto chosen = selected.find(live.source_ip);
                if (chosen == selected.end() || it->second.updated_at > chosen->second.updated_at ||
                    (it->second.updated_at == chosen->second.updated_at &&
                     live.connection_id > chosen->second.connection_id)) {
                    const uint64_t elapsed = now - it->second.updated_at;
                    if (it->second.peer_seconds > std::numeric_limits<uint64_t>::max() - elapsed)
                        continue;
                    const uint64_t peer_now = it->second.peer_seconds + elapsed;
                    const uint64_t local_now = static_cast<uint64_t>(wall_now);
                    const uint64_t distance =
                        peer_now >= local_now ? peer_now - local_now : local_now - peer_now;
                    const int64_t magnitude = static_cast<int64_t>(
                        std::min<uint64_t>(distance, std::numeric_limits<int64_t>::max()));
                    selected[live.source_ip] =
                        SelectedSample{peer_now >= local_now ? magnitude : -magnitude,
  ...
  public:
    ...
    static int InboundHandshakeDeadlineSeconds(const Connection& conn) {
        if (conn.VersionReceived())
            return STUCK_HANDSHAKE_GRACE_S;
        return conn.HasHandshakeProgress() ? PRE_VERSION_PROGRESS_GRACE_S : PRE_VERSION_GRACE_S;
    }

    static bool InboundHandshakeDeadlineExpired(const Connection& conn,
                                                std::chrono::steady_clock::time_point now) {
        if (!conn.IsInbound() || conn.TipReceived())
            return false;
        const auto age =
            std::chrono::duration_cast<std::chrono::seconds>(now - conn.AcceptedAt()).count();
        return age >= InboundHandshakeDeadlineSeconds(conn);
    }

    static bool ProtectInboundHandshakeFromIdleReap(const Connection& conn,
                                                    std::chrono::steady_clock::time_point now) {
    ...
    void DecrConnCount_(const std::shared_ptr<Connection>& c) {
        if (!c)
            return;
        if (c->IsInbound()) {
            if (inbound_count_.load() > 0)
                --inbound_count_;
        } else {
            outbound_connections_.erase(c->Identity());
            if (outbound_count_.load() > 0)
                --outbound_count_;
        }
    }
    ...
    bool RegisterAdmittedConnectionLocked_(const std::string& key,
                                           const std::shared_ptr<Connection>& conn,
                                           std::shared_ptr<Connection>& superseded,
                                           bool require_running = true) {
        if (!conn || (require_running && !running_.load(std::memory_order_acquire)))
            return false;
        auto existing = peer_connections_.find(key);
        if (require_running) {
            if (conn->IsInbound()) {
                uint32_t current = inbound_count_.load(std::memory_order_acquire);
                if (existing != peer_connections_.end() && existing->second &&
                    existing->second->IsInbound() && current > 0)
                    --current;
                if (current >= max_inbound_connections_)
                    return false;
            } else {
                uint32_t current = outbound_count_.load(std::memory_order_acquire);
                if (existing != peer_connections_.end() && existing->second &&
                    !existing->second->IsInbound() && current > 0)
                    --current;
                if (current >= MAX_OUTBOUND_CONNECTIONS)
                    return false;
            }
        }
        if (existing != peer_connections_.end()) {
            if (existing->second == conn)
                return true;
    ...
    void CleanupConnectionState_(const std::shared_ptr<Connection>& expected) {
        if (!expected)
            return;
        ...
        if (expected->VersionReceived())
            work_view_write = std::make_unique<PeerWorkViewWriteGuard_>(*this);
    ...
    void RetireDetachedConnection_(const std::shared_ptr<Connection>& expected) {
        if (!expected)
            return;
        expected->Close();
    ...
    bool FinalizePeerConnection_(const std::string& key,
                                 const std::shared_ptr<Connection>& expected) {
    ...
    bool CurrentLosesDuplicateNonce_(const std::string& key, Connection& conn,
                                     uint64_t peer_nonce) {
        if (peer_nonce == 0 || peer_nonce == self_nonce_)
            return false;
        conn.MarkPeerNonce(peer_nonce);

        std::vector<std::pair<std::string, std::shared_ptr<Connection>>> superseded;
        bool current_loses = false;
        {
            ...
            for (const auto& [other_key, other] : peer_connections_) {
                ...
                if (other->RemoteAddr() != conn.RemoteAddr())
                    continue;
                if (other->PeerNonce() != peer_nonce)
                    continue;
                if (other->IsInbound() != conn.IsInbound())
                    continue;
        ...
        if (current_loses) {
            std::cerr << "  [peer] duplicate node connection from " << conn.RemoteAddr()
                      << " (nonce=" << peer_nonce << ") — keeping deterministic existing leg\n";
            std::cerr.flush();
            return true;
        }
    ...
    bool PreferActualOutboundTipCopy_(const Connection& conn) const {
        if (!conn.IsInbound() || conn.PeerNonce() == 0)
            return false;
        std::lock_guard<std::mutex> lock(peers_mutex_);
        ...
        for (const auto& [identity, weak] : outbound_connections_) {
            ...
            if (!other || other.get() == &conn || !other->IsConnected() || other->IsInbound() ||
                !other->HandshakeReady())
                continue;
            if (other->RemoteAddr() == conn.RemoteAddr() && other->PeerNonce() == conn.PeerNonce())
                return true;
        }
        return false;
    }
    ...
    void ReapIdlePeers(uint32_t idle_threshold_secs = 90) {
        ...
        std::vector<std::string> closed_keys;
        {
            ...
            for (auto it = peer_connections_.begin(); it != peer_connections_.end();) {
                ...
                auto idle = std::chrono::duration_cast<std::chrono::seconds>(now - last).count();
        ...
        for (auto& c : detached_dead)
            RetireDetachedConnection_(c);
        if (!to_close.empty()) {
            std::cerr << "  [reaper] idle/frame-closed " << to_close.size() << " peer(s) (silent >"
                      << idle_threshold_secs << "s or incomplete frame >"
                      << Connection::RECV_FRAME_ASSEMBLY_TIMEOUT_SECONDS << "s):";
    ...
    void RotateOneRandomOutbound() {
        if (outbound_count_.load() < MAX_OUTBOUND_CONNECTIONS)
            return;
        ...
        std::cerr << "  [rotate] closed outbound " << victim.key << " (eclipse-defense rotation)\n";
        std::cerr.flush();
        ...
        std::pair<std::string, uint16_t> replacement{"", 0};
        {
            ...
            for (const auto& [ip, port] : known_peer_addrs_) {
                ...
                if (current_subnets.count(sn) == 0)
                    fresh.emplace_back(ip, port);
                else
                    any.emplace_back(ip, port);
            }
    ...
    std::vector<PeerStatsSnapshot> SnapshotPeerStats() const {
        ...
        for (const auto& [ip, info] : peer_stats_) {
            if (now_s > info.updated_at + PEER_STATS_HARD_TTL_S)
                continue;
            PeerStatsSnapshot s;
            s.ip = ip;
    ...
    void RecordPeerStats(const std::string& peer_ip, uint64_t mempool_size, uint32_t peer_count,
                         int64_t now_s) {
        if (peer_ip.empty())
            return;
        std::lock_guard<std::mutex> lock(peer_stats_mutex_);
        while (!peer_stats_order_.empty()) {
            ...
            if (now_s <= oldest->second.updated_at + PEER_STATS_HARD_TTL_S)
                break;
        ...
        if (it == peer_stats_.end()) {
            if (peer_stats_.size() >= PEER_STATS_CAP && !peer_stats_order_.empty()) {
                auto oldest = peer_stats_.find(peer_stats_order_.front());
                if (oldest != peer_stats_.end()) {
                    peer_stats_order_.erase(oldest->second.order_it);
                    peer_stats_.erase(oldest);
            ...
            it = peer_stats_.emplace(peer_ip, PeerStatsInfo{0, 0, 0, order_it}).first;
        } else {
            peer_stats_order_.splice(peer_stats_order_.end(), peer_stats_order_,
                                     it->second.order_it);
    ...
    void SetBlockAckCallback(BlockAckCallback cb) {
        on_block_ack_ = cb;
    }
    ...
    void BroadcastBlockBytesDirectExcept(const std::vector<uint8_t>& block_payload,
        ...
        std::vector<std::shared_ptr<Connection>> targets;
        {
            ...
            for (auto& [key, conn] : peer_connections_) {
                if (!conn->IsConnected())
                    continue;
                if (exclude_addrs.count(conn->RemoteAddr()))
                    continue;
                targets.push_back(conn);
            }
        }
    ...
    bool IsPeerConnected(const std::string& key, bool require_outbound = false) const {
        ...
        const std::string host = (colon == std::string::npos) ? key : key.substr(0, colon);
        std::unordered_set<std::string> connected_endpoints;
        {
            ...
            for (const auto& [stored_key, conn] : peer_connections_) {
                if (!conn || !conn->IsConnected())
                    continue;
                if (require_outbound && conn->IsInbound())
                    continue;
                if (stored_key == key)
                    return true;
        ...
        if (connected_endpoints.count(host))
            return true;
    ...
    size_t CountConnectedSeeds(const std::vector<std::string>& seeds) const {
        ...
        for (const auto& seed : seeds) {
            ...
            if (connected_endpoints.count(seed)) {
                ++connected;
                continue;
            }
            if (tor_only_.load())
            ...
            for (auto* it = res; it && !match; it = it->ai_next) {
                ...
                auto* sin = reinterpret_cast<struct sockaddr_in*>(it->ai_addr);
                ::inet_ntop(AF_INET, &sin->sin_addr, ip_buf, sizeof(ip_buf));
                if (connected_endpoints.count(std::string(ip_buf)))
                    match = true;
            }
            ::freeaddrinfo(res);
            if (match)
    ...
    std::vector<PeerInfo> GetPeerInfoList() const {
        ...
        for (const auto& [k, c] : peer_connections_) {
            ...
            const bool is_configured_fleet = configured_fleet.count(pi.ip) != 0;
            pi.role = ClassifyPeerTopologyRole(is_configured_fleet, pi.services);
    ...
    void TestDispatchPeerMessage(Connection& conn, const P2PMessage& msg) {
        ...
        (void)PeerProtocolStep(ps, conn, pm, msg,
                               conn.RemoteAddr() + ":" + std::to_string(conn.RemotePort()),
                               conn.IsInbound());
    }
    ...
    void TestDispatchPeerMessageWithState(PeerState& ps, Connection& conn, const P2PMessage& msg) {
        PeerManager pm(magic_, chain_.Height());
        (void)PeerProtocolStep(ps, conn, pm, msg,
                               conn.RemoteAddr() + ":" + std::to_string(conn.RemotePort()),
                               conn.IsInbound());
    ...
    std::shared_ptr<mining::ExpensivePowBudget> TestPowBudgetForSource(const std::string& source) {
        return PowBudgetForSource_(source);
    }

    void TestPrimePunchClientExchange(const std::shared_ptr<Connection>& coordinator,
    ...
    void TestSuppressPunchNetworkDials(bool suppress) {
        test_suppress_punch_network_dials_.store(suppress, std::memory_order_release);
    }
    ...
    bool TestTakePunchDialBudget() {
        return TakePunchDialBudget_();
    }
    ...
    // fork tree.
    void TestRecordAcceptedPeerBlockEvidence(const std::string& peer_ip, const Hash256& hash) {
        RecordVerifiedPeerHeight_(peer_ip, hash);
    }

    void TestRecordPeerTip(const std::shared_ptr<Connection>& conn, const Hash256& hash,
                           int64_t now_s) {
        if (!conn)
            return;
    ...
    void TestRememberValidatedDownload(Connection& connection, const Hash256& hash) {
    ...
    IngestEnqueueResult
    TestEnqueueBlockIngestFromConnection(Block blk, size_t wire_bytes,
                                         const std::string& source_key,
                                         const std::shared_ptr<Connection>& conn) {
        return EnqueueBlockIngest(std::move(blk), wire_bytes, source_key, conn);
    }
    ...
    bool TestRegisterProtectedBlockRequest(Connection& conn, const Hash256& block_hash,
                                           uint64_t now = 0) {
    ...
    void TestExpireProtectedBlockRequest(uint64_t connection_id) {
        ...
        if (it != protected_block_requests_.end())
    ...
    size_t TestPendingBlockIngestCount() {
        std::lock_guard<std::mutex> lk(ingest_mtx_);
        return PendingBlockCountLocked_(ingest_protected_lane_, ingest_normal_lane_);
    }
    ...
    TestBlockIngestView TestStartAndReleaseNextPendingBlock() {
        ...
        {
            std::lock_guard<std::mutex> lk(ingest_mtx_);
            if (!TakeNextBlockIngestLocked_(job))
                return {};
        }
        TestBlockIngestView view{true, job.protected_request, job.sender_source, job.block_hash,
    ...
    bool TestProcessOnePendingBlockIngest() {
        ...
        struct Guard {
            ...
            PendingBlockIngest* job;
            ~Guard() {
                self->ReleasePendingBlockAccounting_(*job);
            }
        } guard{this, &job};
    ...
    uint64_t TestBlockIngestConsensusCallCount() const {
        return test_block_ingest_consensus_calls_.load(std::memory_order_acquire);
    }
    void TestResetBlockIngestOutcomeCounters() noexcept {
        test_block_ingest_consensus_calls_.store(0, std::memory_order_release);
    ...
    uint64_t TestBlockIngestRelayCount() const noexcept {
        return test_block_ingest_relay_calls_.load(std::memory_order_acquire);
    }
    uint64_t TestBlockIngestPenaltyCount() const noexcept {
        return test_block_ingest_penalty_calls_.load(std::memory_order_acquire);
    }
    ...
    bool TestProtectedBlockQosRevoked(const std::string& source_ip) {
        ...
        return it != protected_block_qos_revoked_until_.end() && MonotonicSeconds() < it->second;
    }

    void TestApplyHardBlockReject(const std::string& source, uint64_t connection_id,
                                  bool protected_request, const std::string& tag) {
        PendingBlockIngest job;
    ...
    void TestSetEventLoopExceptionStage(int stage) {
        test_event_loop_exception_stage_.store(stage, std::memory_order_release);
    }
    uint64_t TestContainedNetworkExceptionCount() const {
        return test_contained_network_exceptions_.load(std::memory_order_acquire);
    }
    ...
    bool TestEnqueueFinalityVote(const std::vector<uint8_t>& wire, const std::string& source_ip,
    ...
    void TestRegisterAdmittedConnection(const std::string& key,
                                        const std::shared_ptr<Connection>& conn) {
        std::shared_ptr<Connection> superseded;
        {
            std::lock_guard<std::mutex> lock(peers_mutex_);
            (void)RegisterAdmittedConnectionLocked_(key, conn, superseded, false);
        }
        if (superseded)
            RetireDetachedConnection_(superseded);
    }

    bool TestRegisterProductionConnection(const std::string& key,
                                          const std::shared_ptr<Connection>& conn) {
        std::shared_ptr<Connection> superseded;
        bool admitted = false;
        {
            std::lock_guard<std::mutex> lock(peers_mutex_);
            admitted = RegisterAdmittedConnectionLocked_(key, conn, superseded, true);
        }
        if (superseded)
            RetireDetachedConnection_(superseded);
        if (!admitted && conn)
    ...
    bool TestFinalizePeerConnection(const std::string& key,
                                    const std::shared_ptr<Connection>& expected) {
        return FinalizePeerConnection_(key, expected);
    }

    bool TestResolveDuplicateDirection(const std::string& key, Connection& conn, uint64_t nonce) {
    ...
    bool TestMappedConnectionIs(const std::string& key,
    ...
    void TestTouchConnectionAccounting(const std::shared_ptr<Connection>& conn) {
        if (!conn)
            return;
    ...
    bool TestConnectionAccountingPresent(const std::shared_ptr<Connection>& conn) const {
        if (!conn)
            return false;
        std::lock_guard<std::mutex> lock(ban_mutex_);
        return msg_counts_.count(conn.get()) != 0 && getblocks_bytes_.count(conn.get()) != 0;
    }

    bool TestReservePendingGetData(const std::string& hash_hex, uint64_t now_ts) {
    ...
    void TestRecordClockDrift(const std::shared_ptr<Connection>& conn, int64_t seconds,
                              uint64_t updated_at) {
        if (!conn)
            return;
        ...
        clock_drift_samples_[conn->Identity()] =
            ClockDriftSample{static_cast<uint64_t>(wall + seconds), updated_at, conn->RemoteAddr(),
                             conn->IsInbound()};
    }

    void TestRecordVersionClaim(const std::shared_ptr<Connection>& conn, uint64_t height) {
        if (!conn)
            return;
        ...
        {
            std::lock_guard<std::mutex> lock(peer_heights_mutex_);
            const PeerHeightClaim claim{peer_ip, height, PeerHeightNow_()};
            peer_heights_[conn->Identity()] = claim;
            peer_sync_heights_[conn->Identity()] = claim;
            peer_work_sources_[conn->Identity()] =
                PeerWorkSource{peer_ip, conn->IsInbound(), true, conn->HandshakeReady()};
        }
    }

    void TestMarkPeerHandshakeReady(const std::shared_ptr<Connection>& conn) {
        if (!conn)
            return;
        PeerWorkViewWriteGuard_ work_view_write(*this);
        if (!work_view_write.MayPublish())
    ...
    void TestRecordVerifiedPeerHeight(const std::string& peer_ip, const Hash256& hash) {
        RecordVerifiedPeerHeight_(peer_ip, hash);
    }
    ...
    bool TestTakeInvItemWork(const std::string& ip, uint64_t count) {
        return TakeIpByteBudget(inv_items_per_ip_, ip, count, INV_ITEM_WORK_BUDGET_PER_60S);
    }
    bool TestTakeAddrItemWork(const std::string& ip, uint64_t count) {
        return TakeIpByteBudget(addr_items_per_ip_, ip, count, ADDR_ITEM_WORK_BUDGET_PER_60S);
    }
    ...
    bool TestTakeGetDataItemWork(const std::string& ip, uint64_t count) {
        return TakeIpByteBudget(getdata_items_per_ip_, ip, count, GETDATA_ITEM_WORK_BUDGET_PER_60S);
    }
    bool TestTakeGetBlocksRequest(const std::string& ip) {
        return TakeIpRateToken(getblocks_req_per_ip_, ip, GETBLOCKS_REQUESTS_PER_60S);
    }
    bool TestTakeGetDataResponseBytes(const std::string& ip, uint64_t bytes) {
        return TakeIpByteBudget(getdata_response_bytes_per_ip_, ip, bytes,
                                GETDATA_RESPONSE_BUDGET_PER_60S);
    }
    bool TestTakeGetBlocksResponseBytes(const std::string& ip, uint64_t bytes) {
    ...
    bool TestSendBoundedRecoveryGetData(Connection& source, const Hash256& hash) {
    ...
    bool TestVolatileRuntimeStateEmpty() {
        {
            ...
            if (!msg_counts_.empty() || !msg_windows_.empty() ||
                !msg_violations_in_window_.empty() || !getblocks_bytes_.empty() ||
                !getblocks_windows_.empty())
                return false;
        }
        {
            std::lock_guard<std::mutex> lock(peer_heights_mutex_);
            if (!peer_work_sources_.empty() || !peer_heights_.empty() ||
                !peer_sync_heights_.empty() || !peer_verified_heights_.empty())
                return false;
        }
        {
            std::lock_guard<std::mutex> lock(clock_drift_mutex_);
            if (!clock_drift_samples_.empty())
                return false;
        }
        {
            std::lock_guard<std::mutex> lock(peer_tips_mutex_);
        ...
        return ip_table_last_prune_.empty() && addr_dial_per_ip_.empty() &&
               ...
               solution_rx_per_ip_.empty() && recovery_getdata_global_.empty() &&
               tipsig_processed_at_.empty() && tipsig_getblocks_at_.empty() &&
               tipsig_getdata_at_.empty() && orphan_getblocks_at_.empty() &&
               orphan_parent_getdata_at_.empty() && accept_drop_log_at_.empty() &&
               tx_bytes_per_ip_.empty() && addr_items_per_ip_.empty() &&
               addr_items_global_.empty() && inv_items_per_ip_.empty() &&
               getdata_items_per_ip_.empty() && getdata_response_bytes_per_ip_.empty() &&
               getblocks_response_bytes_per_ip_.empty() &&
               getblocks_response_work_per_ip_.empty() &&
               getblocks_response_bytes_global_.empty() && getblocks_response_work_global_.empty();
    }
  ...
  private:
    ...
    std::atomic<uint64_t> local_services_{MessageType::NODE_FULL | MessageType::NODE_HOLE_PUNCH |
                                          MessageType::REQUIRED_NETWORK_IDENTITY_SERVICES};
    ...
    std::unordered_map<uint64_t, PunchClientCoordinator> punch_client_coordinators_;
    ...
    static bool PunchNonceEqual_(const PunchNonce& a, const PunchNonce& b) noexcept {
        uint8_t diff = 0;
        for (size_t i = 0; i < a.size(); ++i)
            diff |= a[i] ^ b[i];
        return diff == 0;
    }
    static void AppendPunchNonce_(std::vector<uint8_t>& out, const PunchNonce& nonce) {
        out.insert(out.end(), nonce.begin(), nonce.end());
    }
    static bool ReadPunchNonce_(const std::vector<uint8_t>& in, size_t& off, PunchNonce& out) {
        if (off > in.size() || in.size() - off < out.size())
            return false;
        std::copy(in.begin() + static_cast<std::ptrdiff_t>(off),
                  in.begin() + static_cast<std::ptrdiff_t>(off + out.size()), out.begin());
        off += out.size();
        return true;
    }
    ...
    bool TakePunchDialBudget_() {
        ...
        while (!punch_dial_attempts_.empty() && punch_dial_attempts_.front() + 60 <= now) {
    ...
    static constexpr size_t LAYER_C_BLOCK_INGEST_MAX_BYTES = 64ull * 1024ull * 1024ull;
    static constexpr size_t LAYER_C_BLOCK_INGEST_PROTECTED_CAP = 16;
    static constexpr size_t LAYER_C_BLOCK_INGEST_PROTECTED_MAX_BYTES = 16'000'000ull;
    ...
    static constexpr size_t LAYER_C_BLOCK_INGEST_NORMAL_MAX_BYTES =
        LAYER_C_BLOCK_INGEST_MAX_BYTES - LAYER_C_BLOCK_INGEST_PROTECTED_MAX_BYTES;
    ...
    std::unordered_map<uint64_t, ProtectedBlockRequestLease> protected_block_requests_;
    std::unordered_map<std::string, uint64_t> protected_block_qos_revoked_until_;
    ...
    static constexpr size_t FINALITY_VOTE_BYTE_CAP = 48ull * 1024ull * 1024ull;
    ...
    static_assert(P2P_FINALITY_VOTE_WIRE_BYTES * FINALITY_VOTE_JOB_CAP <= FINALITY_VOTE_BYTE_CAP,
    ...
    std::unordered_map<std::string, FinalityRawSourceQueue> finality_vote_raw_by_source_;
    std::deque<std::string> finality_vote_raw_rr_;
    std::unordered_map<std::string, FinalityPreparedLane> finality_vote_prepared_lanes_;
    ...
    std::deque<std::pair<std::string, uint64_t>> finality_vote_recent_invalid_order_;
    ...
    std::unordered_map<std::string, FinalityCompletedCandidate> finality_vote_completed_candidates_;
    std::deque<std::pair<std::string, uint64_t>> finality_vote_completed_candidate_order_;
    std::unordered_map<std::string, size_t> finality_vote_completed_lane_counts_;
    ...
    template <typename Fn> bool SpawnTrackedPeerThread(Fn&& fn) {
        ...
        if (peer_threads_.size() >= MAX_TRACKED_PEER_THREADS)
            return false;
    ...
    bool SpawnTrackedDial_(const std::string& host, uint16_t port, bool explicitly_trusted = false,
                           bool fleet_anchor = false) {
        if (IsLocalListenerEndpoint(host, port))
            return false;
        PendingDialLease lease(*this);
        if (!lease)
            return false;
        return SpawnTrackedPeerThread([this, host, port, explicitly_trusted, fleet_anchor,
                                       lease = std::move(lease)]() mutable {
            (void)lease; // RAII reservation lives through DNS/connect/setup.
            try {
                ConnectToReserved_(host, port, explicitly_trusted, fleet_anchor);
            } catch (...) {
            }
        });
    }
...
#ifdef VELD_TEST_HOOKS
        test_authorized_punch_dials_.fetch_add(1, std::memory_order_acq_rel);
        if (test_suppress_punch_network_dials_.load(std::memory_order_acquire)) {
            return true;
        }
#endif
    ...
    static std::string StripPort_(const std::string& addr) {
        if (!addr.empty() && addr.front() == '[') { // [v6]:port
            ...
            if (close != std::string::npos)
                return addr.substr(0, close + 1);
            return addr;
        }
        if (addr.find(':') != addr.rfind(':'))
            return addr; // bare IPv6
    ...
    bool CanDialOutboundIp(const std::string& target_ip) const {
        ...
        for (const auto& [k, c] : peer_connections_) {
            ...
            if (trusted_ips_.count(peer_ip))
                continue;
            if (peer_ip == target)
                ++per_ip;
            if (Subnet16Prefix(peer_ip) == target_subnet)
                ++per_subnet;
        }
        if (per_ip >= PER_IP_OUTBOUND_CAP)
            return false;
        if (per_subnet >= PER_SUBNET_OUTBOUND_CAP)
            return false;
        return true;
    }
    ...
    std::unordered_map<std::string, ViolationLogRateEntry> violation_log_rate_;
    std::list<std::string> violation_log_order_;
    uint64_t violation_log_global_window_{0};
    uint32_t violation_log_global_count_{0};

    using ViolationMap = std::unordered_map<std::string, ViolationEntry>;
    ...
    using ViolationLogRateMap = std::unordered_map<std::string, ViolationLogRateEntry>;
    void EraseViolationLogRateLocked_(ViolationLogRateMap::iterator it) {
        if (it == violation_log_rate_.end())
            return;
    ...
    std::unordered_map<const Connection*, uint32_t> msg_violations_in_window_;
    ...
    std::unordered_map<std::string, IpByteSlot> getdata_response_bytes_per_ip_;
    std::unordered_map<std::string, IpByteSlot> getblocks_response_bytes_per_ip_;
    std::unordered_map<std::string, IpByteSlot> getblocks_response_work_per_ip_;
    std::unordered_map<std::string, IpByteSlot> getblocks_response_bytes_global_;
    std::unordered_map<std::string, IpByteSlot> getblocks_response_work_global_;
    ...
    static constexpr uint64_t GETDATA_RESPONSE_BUDGET_PER_60S = 256ull * 1024ull * 1024ull;
    ...
    static constexpr uint64_t GETBLOCKS_RESPONSE_BYTES_PER_IP_PER_10M = 200ull * 1024ull * 1024ull;
    ...
    static constexpr uint64_t GETBLOCKS_RESPONSE_WORK_GLOBAL_PER_60S = 32'000;
    static constexpr uint32_t RECOVERY_GETDATA_GLOBAL_PER_60S = 256;

    bool TakeIpRateToken(std::unordered_map<std::string, IpRateSlot>& tbl, const std::string& ip,
                         uint32_t cap_per_60s) {
        ...
        if (slot.count >= cap_per_60s)
            return false;
        ++slot.count;
        return true;
    }

    bool TakeIpByteBudget(std::unordered_map<std::string, IpByteSlot>& tbl, const std::string& ip,
                          uint64_t bytes, uint64_t cap, uint64_t window_seconds = 60) {
        std::lock_guard<std::mutex> lk(ip_rate_mutex_);
        uint64_t now_s = MonotonicSeconds();
        ...
        if (bytes > cap || slot.bytes > cap - bytes)
            return false;
        slot.bytes += bytes;
        return true;
    }

    bool IpByteBudgetHasRoom_(std::unordered_map<std::string, IpByteSlot>& tbl,
                              const std::string& key, uint64_t cap, uint64_t window_seconds) {
        std::lock_guard<std::mutex> lk(ip_rate_mutex_);
    ...
    bool ReserveGetBlocksBodyWork_(const std::string& endpoint) {
        ...
        if (!IpByteBudgetHasRoom_(getblocks_response_bytes_per_ip_, ip,
                                  GETBLOCKS_RESPONSE_BYTES_PER_IP_PER_10M, 600) ||
            !IpByteBudgetHasRoom_(getblocks_response_bytes_global_, GLOBAL_KEY,
                                  GETBLOCKS_RESPONSE_BYTES_GLOBAL_PER_60S, 60)) {
            return false;
        }
        if (!TakeIpByteBudget(getblocks_response_work_per_ip_, ip, 1,
                              GETBLOCKS_RESPONSE_WORK_PER_IP_PER_10M, 600)) {
            return false;
        }
        return TakeIpByteBudget(getblocks_response_work_global_, GLOBAL_KEY, 1,
                                GETBLOCKS_RESPONSE_WORK_GLOBAL_PER_60S, 60);
    }

    bool ReserveGetBlocksResponseBytes_(const std::string& endpoint, uint64_t bytes) {
        const std::string ip = StripPort_(endpoint);
        static const std::string GLOBAL_KEY = "*";
        ...
        return TakeIpByteBudget(getblocks_response_bytes_global_, GLOBAL_KEY, bytes,
                                GETBLOCKS_RESPONSE_BYTES_GLOBAL_PER_60S, 60);
    }

    bool SendBoundedRecoveryGetData_(Connection& source, const Hash256& block_hash) {
        static const std::string GLOBAL_KEY = "*";
        ...
        PeerManager pm(magic_, chain_.Height());
        return source.Send(pm.BuildGetDataMessage({InvItem(InvType::BLOCK, block_hash)}));
    }

    bool TakeKeyCooldown(std::unordered_map<std::string, uint64_t>& tbl, const std::string& key,
                         uint64_t cooldown_seconds, uint64_t retention_seconds, size_t hard_cap) {
        if (key.empty() || hard_cap == 0)
            return false;
        std::lock_guard<std::mutex> lk(ip_rate_mutex_);
        const uint64_t now_s = MonotonicSeconds();
        auto found = tbl.find(key);
        if (found != tbl.end()) {
            if (now_s < found->second + cooldown_seconds)
                return false;
            found->second = now_s;
            return true;
        }
    ...
    bool MarkSolutionSeenIfNew_(const std::string& sol_hex) {
        ...
        if (solution_seen_.count(sol_hex))
            return true;
        solution_seen_[sol_hex] = now_s;
        solution_seen_fifo_.push_back(sol_hex);
        while (solution_seen_.size() > SOLUTION_SEEN_CACHE_CAP && !solution_seen_fifo_.empty()) {
    ...
    void RejectedBlockInsertLocked_(const std::string& hash_hex) {
        ...
        while (rejected_blocks_.size() > REJECT_CACHE_CAP && !rejected_blocks_fifo_.empty()) {
  ...
  private:
    ...
    bool ReservePendingGetData_(const std::string& hash_hex, uint64_t now_ts) {
        std::lock_guard<std::mutex> lock(pending_gd_mutex_);
        PrunePendingGetDataLocked_(now_ts);
        if (pending_getdata_.count(hash_hex) != 0)
            return false;
        if (pending_getdata_.size() >= PENDING_GETDATA_CAP)
            return false;
        pending_getdata_order_.push_back(hash_hex);
        auto order_it = std::prev(pending_getdata_order_.end());
        pending_getdata_.emplace(hash_hex, PendingGetDataEntry{now_ts, order_it});
        return true;
    }
    ...
    std::unordered_map<std::string, std::vector<OrphanBlockEntry>> orphan_pool_;
    ...
    std::function<void(uint64_t, const Hash256&, uint64_t, const std::vector<uint8_t>&)>
        solution_cb_;
    std::function<bool(uint64_t, const Hash256&, uint64_t, const Hash256&,
                       const std::vector<uint8_t>&)>
        comine_cb_;
    ...
    std::unordered_map<std::string, VerifiedPeerHeight> peer_verified_heights_;
...
#ifdef VELD_TEST_HOOKS
        const uint64_t forced = test_peer_height_now_.load(std::memory_order_acquire);
        if (forced != 0)
            return forced;
#endif
    ...
    static bool VersionHeightHintFresh_(uint64_t updated_at, uint64_t now) {
        return updated_at <= now && now - updated_at <= VERSION_HEIGHT_HINT_TTL_SECONDS;
    }
    ...
    PeerSyncProjection
    PeerSyncProjectionLocked_(uint64_t now, uint64_t replacement_id = 0,
        ...
        for (const auto& [id, source] : peer_work_sources_) {
            if (!source.version_received || !source.handshake_ready || source.inbound)
                continue;
            const auto version = peer_heights_.find(id);
            if (version == peer_heights_.end() || version->second.source_ip != source.source_ip)
                continue;
            const auto existing = peer_sync_heights_.find(id);
            const PeerHeightClaim* claim = replacement && id == replacement_id ? replacement
                                           : existing == peer_sync_heights_.end()
                                               ? nullptr
                                               : &existing->second;
            if (!claim || claim->source_ip != source.source_ip ||
                !VersionHeightHintFresh_(claim->updated_at, now))
                continue;
            const uint64_t deadline =
                claim->updated_at > UINT64_MAX - VERSION_HEIGHT_HINT_TTL_SECONDS - 1
                    ? UINT64_MAX
                    : claim->updated_at + VERSION_HEIGHT_HINT_TTL_SECONDS + 1;
            result.freshness_deadline = std::min(result.freshness_deadline, deadline);
            const auto previous = selected.find(source.source_ip);
            if (previous == selected.end() || claim->updated_at > previous->second.updated_at ||
                (claim->updated_at == previous->second.updated_at &&
                 id > previous->second.connection_id)) {
                selected[source.source_ip] = {claim->height, claim->updated_at, id};
            }
        }
    ...
    std::optional<uint64_t> CanonicalPeerEvidence_(const Hash256& hash) const {
        if (HashIsZero(hash))
            return std::nullopt;
    ...
    void RecordVerifiedPeerHeight_(const std::string& peer_ip, const Hash256& hash) {
        if (peer_ip.empty())
            return;
        ...
        const uint64_t verified_best = GetPeerHeightView().verified_height;
        {
            ...
            const bool active = std::any_of(
                peer_work_sources_.begin(), peer_work_sources_.end(), [&](const auto& entry) {
                    return entry.second.version_received && entry.second.handshake_ready &&
            ...
            if (current != peer_verified_heights_.end() &&
                (*derived_height < current->second.height ||
                 (*derived_height == current->second.height && hash == current->second.hash)))
                return;
            if (*derived_height <= verified_best) {
                ...
                peer_verified_heights_[peer_ip] = VerifiedPeerHeight{hash, *derived_height};
                return;
            }
        }
    ...
    void RecordPeerSyncHeight_(Connection& conn, uint64_t height) {
        if (!conn.IsConnected() || conn.RemoteAddr().empty())
            return;
        std::string peer_ip = conn.RemoteAddr();
        size_t colon = peer_ip.find_last_of(':');
        if (colon != std::string::npos)
        ...
        const PeerHeightClaim next{peer_ip, height, PeerHeightNow_()};
        {
            ...
            if (source == peer_work_sources_.end() || !source->second.version_received ||
            ...
            const auto after = PeerSyncProjectionLocked_(next.updated_at, conn.Identity(), &next);
            if (before.height == after.height && before.distinct_ips == after.distinct_ips &&
        ...
        std::lock_guard<std::mutex> lock(peer_heights_mutex_);
        const auto source = peer_work_sources_.find(conn.Identity());
        if (source == peer_work_sources_.end() || !source->second.handshake_ready ||
            source->second.source_ip != peer_ip)
            return;
        peer_sync_heights_[conn.Identity()] = next;
    }
    ...
    std::unordered_map<std::string, PeerTipRecoveryHint> peer_tip_recovery_hints_;
    ...
    void ProcessOrphanChain(const Hash256& parent_hash, PeerManager& pm, Connection& conn,
                            const std::string& key, uint32_t depth = 0) {
        if (depth >= 500 || peer_work_stop_.stop_requested())
            return;
        ...
        for (auto& entry : to_process) {
            if (peer_work_stop_.stop_requested())
                return;
            if (!entry.source_pow_budget)
                continue;
            Block& orphan = entry.block;
            const auto admission = chain_.AddBlockDirect(
                orphan, false, true, false,
                mining::PowAdmissionContext::Peer(entry.source, entry.source_pow_budget,
            ...
            if (admission.IsAccepted()) {
                ...
                if (h <= 10 || h % 100 == 0) {
                    if (!veld::net::g_suppress_sync.load() && !background_sync_mode_) {
                        std::lock_guard<std::mutex> lk(veld::net::g_stdout_mtx);
                        std::cout << "  [sync] height=" << h << "  supply=" << std::fixed
                                  << std::setprecision(2) << chain_.TotalSupplyVeld() << " VELD";
                        if (veld::DiagVerbose().load())
                            std::cout << " (reordered)";
                        std::cout << "\n";
                        std::cout.flush();
                ...
                if (on_block_)
                    on_block_(orphan, key);
                if (ibd_complete_flag_.load()) {
                    BroadcastMessage(
                        pm.BuildInvMessage({InvItem(InvType::BLOCK, orphan.GetHash())}), key);
                }
    ...
    void RecordValidatedDownload_(Connection& connection, const Hash256& hash) const {
    ...
    P2PMessage BuildChainLocatorGetBlocks(const Connection* connection = nullptr) const {
        ...
        try {
            extra.push_back(chain_.GetBlock(0).GetHash());
        } catch (...) {
        }
    ...
    void AcceptLoop() {
        while (running_) {
            ...
            std::vector<PeerFdEntry> peer_fds;
            {
                ...
                for (const auto& [k, c] : peer_connections_) {
                    if (!c || !c->IsConnected())
                        continue;
                    SocketHandle f = c->Fd();
                    if (!veld::compat::IsValidSocket(f))
                        continue;
            ...
            if (!died.empty()) {
                ...
                for (auto& c : died)
                    c->Close(Connection::CloseReason::PollError);
            }
            ...
            if (!SpawnTrackedPeerThread(
                    [this, conn, key]() { HandlePeer(conn, key, true, true); })) {
                FinalizePeerConnection_(key, conn);
            }
        }
    }
    ...
    StepResult PeerProtocolStep(PeerState& ps, Connection& conn, PeerManager& pm,
        ...
        if (background_sync_mode_ && !IsBackgroundValidationInboundCommand(msg.command)) {
        ...
        if (msg.command == MessageType::MEMPOOL) {
            ...
            auto txs = mempool_.GetBlockTransactions(500, MAX_BLOCK_SIZE, &chain_);
        ...
        if (msg.command == MessageType::VERSION) {
            ...
            if (msg.payload.size() < genesis_off + 32) {
                std::cerr << "  [peer] genesis-hash absent from VERSION by " << conn.RemoteAddr()
                          << " — dropping unbound peer\n";
                std::cerr.flush();
            ...
            for (int i = 0; i < 8; ++i)
                announced_services |= ((uint64_t)msg.payload[4 + i]) << (i * 8);
            if ((announced_services & MessageType::REQUIRED_NETWORK_IDENTITY_SERVICES) !=
                MessageType::REQUIRED_NETWORK_IDENTITY_SERVICES) {
                std::cerr << "  [peer] network-identity services mismatch from "
                          << conn.RemoteAddr()
                          << " — peer lacks required rolling-reserve semantics\n";
                std::cerr.flush();
                RecordViolation(conn.RemoteAddr(), 1, "network_identity_mismatch");
                return StepResult::DropPeer;
            }
            ...
            if (self_nonce_ != 0 && announced_nonce == self_nonce_) {
                std::cerr << "  [peer] self-connect detected from " << conn.RemoteAddr()
            ...
            }
            {
                ...
                if (mismatch) {
                    ...
                    std::cerr << "  [peer] genesis-hash MISMATCH from " << conn.RemoteAddr()
                              << " — their genesis " << oss.str() << "... != ours " << GENESIS_HASH
                              << " (first 8B). Dropping. Mismatched-genesis "
                              << "disconnect count: " << (genesis_mismatch_count_.fetch_add(1) + 1)
                              << "\n";
                    std::cerr.flush();
                    RecordViolation(conn.RemoteAddr(), 1, "genesis_mismatch");
                    return StepResult::DropPeer;
            ...
            if (announced_nonce != 0 && CurrentLosesDuplicateNonce_(key, conn, announced_nonce)) {
                return StepResult::DropPeer;
            }
            ...
            conn.MarkAdvertisedServices(announced_services);
            {
                ...
                if (their_height > MAX_PLAUSIBLE_HEIGHT) {
                    std::cerr << "  [peer] VERSION claims implausible height " << their_height
                              << " — clamping to 0\n";
                    their_height = 0;
                }
                {
                    ...
                    if (colon != std::string::npos)
                        peer_ip = peer_ip.substr(0, colon);
                    std::lock_guard<std::mutex> lock(peer_heights_mutex_);
                    const PeerHeightClaim claim{peer_ip, their_height, PeerHeightNow_()};
                    peer_heights_[conn.Identity()] = claim;
                    peer_sync_heights_[conn.Identity()] = claim;
                }
...

                {
                    ...
                    constexpr uint64_t MAX_CLOCK_SAMPLE_DELTA = 86400ull * 365ull;
                    bool admissible_delta = false;
                    if (local_ts_signed >= 0) {
                        const uint64_t local_ts = static_cast<uint64_t>(local_ts_signed);
                        if (peer_ts >= local_ts) {
                    ...
                    if (admissible_delta) {
                        ...
                        for (auto it = clock_drift_samples_.begin();
                            ...
                            if (now_mono > it->second.updated_at + CLOCK_DRIFT_TTL_SECONDS) {
                        ...
                        if (clock_drift_samples_.count(connection_id) == 0 &&
                            ...
                            auto oldest = std::min_element(
                                clock_drift_samples_.begin(), clock_drift_samples_.end(),
                                [](const auto& a, const auto& b) {
                                    return a.second.updated_at < b.second.updated_at;
                                });
                        ...
                        clock_drift_samples_[connection_id] = ClockDriftSample{
                            peer_ts, now_mono, conn.RemoteAddr(), conn.IsInbound()};
                    }
                }
                ...
                if (peer_proto_ver < PROTOCOL_VERSION) {
                    std::cerr << "[p2p] peer " << conn.RemoteAddr() << " uses protocol v"
                              << peer_proto_ver << " (we are v" << PROTOCOL_VERSION
            ...
            if (just_became_ready && ibd_complete_flag_.load()) {
                ...
                if (our_tip > ps.their_start_height) {
                    ...
                    for (uint64_t offset = 0; offset < n_to_send; ++offset) {
                        if (!conn.CanQueueEventLoopSend(static_cast<size_t>(MAX_BLOCK_SIZE) + 24u))
                            break;
                        if (!ReserveGetBlocksBodyWork_(conn.RemoteAddr()))
                            break;
                        Block blk;
                        try {
                        ...
                        if (bytes_sent + payload.size() > MAX_GETBLOCKS_RESPONSE_BYTES)
                            break;
                        P2PMessage first_inv;
                        size_t first_inv_bytes = 0;
                        if (!announced_first_body) {
                            first_inv =
                                pm.BuildInvMessage({InvItem(InvType::BLOCK, blk.GetHash())});
                            first_inv_bytes = first_inv.Serialize().size();
                        }
                        if (!ReserveGetBlocksResponseBytes_(conn.RemoteAddr(),
                                                            payload.size() + 24u + first_inv_bytes))
                            break;
                        if (!announced_first_body) {
                            if (!conn.TrySend(first_inv))
                                break;
                            announced_first_body = true;
                        }
                        if (!conn.TrySend(P2PMessage(magic_, MessageType::BLOCK, payload)))
                            break;
        ...
        if (msg.command == MessageType::VERACK) {
            ...
            if (ps.version_acked)
                return StepResult::Handled;
            const bool just_became_ready =
                !ps.handshake_done && ps.version_sent && ps.their_version;
            if (just_became_ready) {
                PeerWorkViewWriteGuard_ work_view_write(*this);
                if (!work_view_write.MayPublish())
                    return StepResult::DropPeer;
            ...
            if (just_became_ready) {
                ...
                if (ibd_complete_flag_.load()) {
                    ...
                    if (our_tip > ps.their_start_height) {
                        ...
                        bool announced_first_body = false;
                        for (uint64_t offset = 0; offset < n_to_send; ++offset) {
                            if (!conn.CanQueueEventLoopSend(static_cast<size_t>(MAX_BLOCK_SIZE) +
                                                            24u))
                                break;
                            if (!ReserveGetBlocksBodyWork_(conn.RemoteAddr()))
                                break;
                            ...
                            if (!ReserveGetBlocksResponseBytes_(
                                    conn.RemoteAddr(), payload.size() + 24u + first_inv_bytes))
                                break;
                            if (!announced_first_body) {
                                if (!conn.Send(first_inv))
                                    break;
                                announced_first_body = true;
                            }
                            if (!conn.Send(P2PMessage(magic_, MessageType::BLOCK, payload)))
                                break;
                            bytes_sent += payload.size();
                            ++sent_count;
                            if (sent_count % 100 == 0)
        ...
        if (msg.command == MessageType::COMINE) {
            ...
            uint64_t height = 0;
            for (int i = 0; i < 8; i++)
                height |= ((uint64_t)msg.payload[32 + i] << (i * 8));
            uint64_t best_nonce = 0;
            for (int i = 0; i < 8; i++)
                best_nonce |= ((uint64_t)msg.payload[40 + i] << (i * 8));
            Hash256 best_hash;
            ...
            if (script_len != 25)
                return StepResult::Handled;
            std::vector<uint8_t> miner_script(msg.payload.begin() + 81,
                                              msg.payload.begin() + 81 + script_len);
            if (miner_script[0] != 0x76 || miner_script[1] != 0xA9 || miner_script[2] != 0x14 ||
                miner_script[23] != 0x88 || miner_script[24] != 0xAC) {
                return StepResult::Handled;
            }
            size_t cursor = 81u + script_len;
            if (cursor + 2 > msg.payload.size())
                return StepResult::Handled;
            uint16_t pk_len =
                (uint16_t)msg.payload[cursor] | ((uint16_t)msg.payload[cursor + 1] << 8);
            cursor += 2;
            if (pk_len != 1952 || cursor + pk_len > msg.payload.size())
                return StepResult::Handled;
            std::vector<uint8_t> pubkey(msg.payload.begin() + cursor,
                                        msg.payload.begin() + cursor + pk_len);
            cursor += pk_len;
            if (cursor + 2 > msg.payload.size())
                return StepResult::Handled;
            uint16_t sig_len =
            ...
            if (sig_len == 0 || sig_len > 3309 || cursor + sig_len > msg.payload.size())
                return StepResult::Handled;
            std::vector<uint8_t> signature(msg.payload.begin() + cursor,
                                           msg.payload.begin() + cursor + sig_len);
            ...
            for (int i = 0; i < 8; i++)
                challenge.push_back((height >> (i * 8)) & 0xFF);
            for (int i = 0; i < 8; i++)
                challenge.push_back((best_nonce >> (i * 8)) & 0xFF);
        ...
        if (msg.command == MessageType::SOLUTION) {
            ...
            if (msg.payload.size() >= 49) {
                ...
                uint64_t nonce = 0;
                for (int i = 0; i < 8; i++)
                    nonce |= ((uint64_t)msg.payload[40 + i] << (i * 8));
                uint8_t script_len = msg.payload[48];
                if (msg.payload.size() >= 49u + script_len && script_len == 25) {
                    std::vector<uint8_t> solver_script(msg.payload.begin() + 49,
                                                       msg.payload.begin() + 49 + script_len);
                    ...
                    sol_id.insert(sol_id.end(), solver_script.begin(), solver_script.end());
                    const std::string sol_hex = HashToHex(Hash256d(sol_id));
                    if (MarkSolutionSeenIfNew_(sol_hex))
                        return StepResult::Handled;
                    if (!TakeIpRateToken(solution_rx_per_ip_, conn.RemoteAddr(), 32))
                        return StepResult::Handled;

                    if (solution_cb_)
                        solution_cb_(height, prev_hash, nonce, solver_script);
                    uint64_t our_height = chain_.Height();
                    if (height == our_height + 1 && ibd_complete_flag_.load() &&
                        my_miner_script_.size() > 0 && my_mining_height_.load() == height &&
                        my_comine_pubkey_.size() == 1952 && my_comine_signer_) {
                        PeerManager fresh_pm(magic_, chain_.Height());
                        Hash256 bh;
                        {
                            std::lock_guard<std::mutex> lk(best_hash_mutex_);
                            bh = my_best_hash_;
                        }
                        ...
                        for (int i = 0; i < 8; i++)
                            challenge.push_back((height >> (i * 8)) & 0xFF);
                        for (int i = 0; i < 8; i++)
                            challenge.push_back((bn >> (i * 8)) & 0xFF);
                        challenge.insert(challenge.end(), bh.begin(), bh.end());
                        challenge.insert(challenge.end(), my_miner_script_.begin(),
                                         my_miner_script_.end());
                        std::vector<uint8_t> sig;
                        try {
                            sig = my_comine_signer_(challenge);
                        } catch (...) {
                        }
                        if (!sig.empty()) {
                            conn.Send(fresh_pm.BuildCOMineMessage(prev_hash, height, bn, bh,
                                                                  my_miner_script_,
                                                                  my_comine_pubkey_, sig));
                        }
                    }
                    BroadcastMessage(msg, key);
                }
        ...
        if (msg.command == MessageType::TX) {
...

            {
                ...
                if (conn.mempool_budget_used + msg.payload.size() >
                    MEMPOOL_BYTES_PER_PEER_PER_MIN) {
            ...
            }
            {
                ...
                if (!TakeIpByteBudget(tx_bytes_per_ip_, ip_only, msg.payload.size(),
            ...
            if (consumed == 0 || consumed != msg.payload.size() || tx.Serialize() != msg.payload) {
            ...
            if (tx.IsValid()) {
                ...
                if (ExtractNmsFromTx(tx)) {
                    nms_pow_budget = PowBudgetForSource_(StripPort_(conn.RemoteAddr()));
                    if (!nms_pow_budget) {
                        RecordViolation(conn.RemoteAddr(), 0, "nms_pow_budget_unavailable");
                        return StepResult::Handled;
                    }
                }
                auto add_res =
                    mempool_.Add(tx, fee, (uint32_t)chain_.Height(), chain_, nms_pow_budget.get());
                if (add_res == Mempool::AddResult::ACCEPTED) {
                    if (on_tx_)
                        on_tx_(tx, key);
                    BroadcastMessage(pm.BuildInvMessage({InvItem(InvType::TX, tx.GetTxID())}), key);
                } else {
                    switch (add_res) {
                    case Mempool::AddResult::INVALID:
                        RecordViolation(conn.RemoteAddr(), MempoolRejectBanScore(add_res),
                                        "tx_mempool_invalid");
                        break;
                    ...
                    case Mempool::AddResult::MALFORMED_VALIDATOR_OP:
                        RecordViolation(conn.RemoteAddr(), 10, "tx_malformed_validator_op");
                        break;
                    case Mempool::AddResult::FEE_TOO_LOW:
                        RecordViolation(conn.RemoteAddr(), 5, "tx_mempool_fee_too_low");
                        break;
                    case Mempool::AddResult::DOUBLE_SPEND:
                        ...
                        RecordViolation(conn.RemoteAddr(), 0, "tx_double_spend");
                        break;
                    case Mempool::AddResult::STAKE_EXCEEDS_BALANCE:
                        RecordViolation(conn.RemoteAddr(), 5, "tx_stake_exceeds_balance");
                        break;
                    ...
                    case Mempool::AddResult::RUNTIME_ADMISSION_CLOSED:
                        RecordViolation(conn.RemoteAddr(), 0, "tx_mempool_race");
                        break;
                    case Mempool::AddResult::PUBLIC_TESTNET_EXTERNAL_VALUE_DISABLED:
                        RecordViolation(conn.RemoteAddr(), 0, "tx_external_value_disabled");
                        break;
        ...
        if (msg.command == MessageType::ADDR) {
            ...
            const size_t expected_size = 1u + 6u * static_cast<size_t>(adv_count_hdr);
            if (adv_count_hdr > MAX_ADDR_ENTRIES || msg.payload.size() != expected_size) {
                RecordViolation(conn.RemoteAddr(), 5, "addr_malformed");
                return StepResult::Handled;
            }
            const auto now_steady = std::chrono::steady_clock::now();
            const auto since_req =
                std::chrono::duration_cast<std::chrono::seconds>(now_steady - ps.last_getaddr_sent)
                    .count();
            const bool inside_req_window =
                (ps.last_getaddr_sent.time_since_epoch().count() != 0) && (since_req >= 0) &&
                (since_req <= (int64_t)ADDR_REQUEST_WINDOW_S) && !ps.addr_response_consumed;
            const bool greeting_ok = !ps.any_addr_received && ps.handshake_done &&
            ...
            if (inside_req_window)
                ps.addr_response_consumed = true;
            ps.any_addr_received = true;

            if (msg.payload.size() >= 1) {
                uint8_t count = msg.payload[0];
                if (!TakeIpByteBudget(addr_items_per_ip_, conn.RemoteAddr(), count,
                ...
                for (uint8_t i = 0; i < count && pos + 6 <= msg.payload.size(); ++i) {
                    uint32_t ip_raw = msg.payload[pos] | ((uint32_t)msg.payload[pos + 1] << 8) |
                    ...
                    if (dials_in_this_msg >= 4)
                        continue;
                    if (conn.addr_dial_count >= 16)
                        continue;
                    if (!TakeIpRateToken(addr_dial_per_ip_, sender_ip, 16))
                        continue;
                    {
                        std::lock_guard<std::mutex> lk(peers_mutex_);
                        ...
                        if (!peer_connections_.count(pkey) && !IsBanned(ip_str) &&
                            ...
                            const bool spawned = SpawnTrackedDial_(advertised_ip, port);
                            if (spawned) {
                                ++dials_in_this_msg;
        ...
        if (msg.command == MessageType::GETDATA) {
            ...
            if (!TakeIpByteBudget(getdata_items_per_ip_, conn.RemoteAddr(), count,
            ...
            for (uint16_t i = 0; i < count; ++i) {
                if (pos + 36 > msg.payload.size())
                    break;

                uint32_t type_raw =
                    (uint32_t)msg.payload[pos] | ((uint32_t)msg.payload[pos + 1] << 8) |
                    ((uint32_t)msg.payload[pos + 2] << 16) | ((uint32_t)msg.payload[pos + 3] << 24);
                pos += 4;

                Hash256 hash;
                std::copy(msg.payload.begin() + pos, msg.payload.begin() + pos + 32, hash.begin());
                pos += 32;
                ...
                if (itype == InvType::BLOCK) {
                    ...
                    if (!IpByteBudgetHasRoom_(getdata_response_bytes_per_ip_,
                        ...
                        !ReserveGetBlocksBodyWork_(conn.RemoteAddr()))
                        break;
                    ...
                    if (blk_opt.has_value()) {
                        ...
                        if (payload.size() >
                            GETDATA_RESPONSE_CAP -
                                std::min(getdata_response_bytes, GETDATA_RESPONSE_CAP)) {
                            break;
                        }
                        if (!TakeIpByteBudget(getdata_response_bytes_per_ip_,
                    ...
                    if (!conn.CanQueueEventLoopSend(Mempool::MAX_RELAY_TX_BYTES + 24u))
                        break;
                    auto entry = mempool_.Get(hash);
                    if (entry.has_value()) {
                        auto payload = entry->tx.Serialize();
                        if (payload.size() > GETDATA_RESPONSE_CAP - std::min(getdata_response_bytes,
                                                                             GETDATA_RESPONSE_CAP))
                            break;
                        if (!TakeIpByteBudget(getdata_response_bytes_per_ip_,
                                              StripPort_(conn.RemoteAddr()), payload.size() + 24u,
                                              GETDATA_RESPONSE_BUDGET_PER_60S))
                            break;
                        if (!conn.Send(P2PMessage(magic_, MessageType::TX, payload)))
                            break;
                        getdata_response_bytes += payload.size();
                    }
                }
        ...
        if (msg.command == MessageType::INV) {
            ...
            for (uint16_t i = 0; i < count; ++i) {
                ...
                if (itype == InvType::BLOCK) {
                    ...
                    auto known_height = chain_.GetKnownBlockHeightByHash(hash);
                    ...
                    const bool anchor_ibd_request = in_ibd && IsLiveConfiguredOutboundAnchor_(conn);
                    if ((!in_ibd || anchor_ibd_request) && !known_height.has_value()) {
                        ...
                        if (!rejected && ReservePendingGetData_(hash_hex, MonotonicSeconds())) {
                        ...
                        if (on_block_ack_)
                            on_block_ack_(hash, peer_ip);
                    }
                } else if (itype == InvType::TX) {
                    if (!mempool_.Contains(hash)) {
                        std::string hash_hex = HashToHex(hash);
                        if (ReservePendingGetData_(hash_hex, MonotonicSeconds())) {
            ...
            if (!want.empty() && !conn.Send(pm.BuildGetDataMessage(want))) {
                if (!protected_request_hash.empty())
                    CancelProtectedBlockRequest_(conn.Identity(), protected_request_hash);
        ...
        if (msg.command == MessageType::GETBLOCKS) {
            ...
            if (msg.payload.size() < 4 + 1 + 32 + 32) {
                ...
                if (!TakeIpRateToken(getblocks_req_per_ip_, conn.RemoteAddr(),
                ...
                for (uint8_t lc = 0; lc < locator_count; ++lc) {
                    if (pos + 32 > msg.payload.size())
                        break;
                    Hash256 locator;
                    std::copy(msg.payload.begin() + pos, msg.payload.begin() + pos + 32,
                              locator.begin());
                    pos += 32;
                    ...
                    start_height = 0;
                {
                    ...
                    uint64_t remaining_budget;
                    {
                        ...
                        remaining_budget =
                            (used >= GETBLOCKS_BUDGET_BYTES) ? 0 : (GETBLOCKS_BUDGET_BYTES - used);
                        remaining_budget =
                            std::min<uint64_t>(remaining_budget, MAX_GETBLOCKS_RESPONSE_BYTES);
                    }
                    if (remaining_budget == 0) {
                        ...
                        for (size_t offset = 0;
                             offset < MAX_GETBLOCKS_RESPONSE && send_from <= tip_height &&
                            ...
                            auto payload = blk.Serialize();
                            if (bytes_this_response + payload.size() > remaining_budget)
                                break;
                            P2PMessage first_inv;
                            ...
                            if (!ReserveGetBlocksResponseBytes_(
                                    conn.RemoteAddr(), payload.size() + 24u + first_inv_wire_bytes))
                                break;
                            if (!announced_first_body) {
                            ...
                            ++sent_count;
                            if (has_stop && blk.GetHash() == stop_hash)
                                break;
                        }
                        std::lock_guard<std::mutex> lk(ban_mutex_);
        ...
        if (msg.command == MessageType::TIPSIG) {
            ...
            uint32_t their_version = (uint32_t)msg.payload[0] | ((uint32_t)msg.payload[1] << 8) |
            ...
            Hash256 their_hash;
            std::copy(msg.payload.begin() + 12, msg.payload.begin() + 44, their_hash.begin());
            ...
            const auto known_tip_height = chain_.GetKnownBlockHeightByHash(their_hash);
            if (known_tip_height.has_value() && *known_tip_height != their_height) {
            ...
            if (divergent) {
                ...
                if (!already_have_their_tip) {
                    ...
                    if (walked) {
                        if (chain_.GetKnownBlockHeightByHash(fetch_target).has_value()) {
                    ...
                    {
                        const bool send_getdata =
                            TakeKeyCooldown(tipsig_getdata_at_, src_ip, 10, 600, 4096);
                        if (send_getdata) {
                            ...
                            if (fetch_queued && gap_broadcast) {
                                int64_t now_s =
                                    (int64_t)std::chrono::duration_cast<std::chrono::seconds>(
                                        ...
                                        .count();
                                int64_t last =
                                    last_tipsig_catchup_at_seconds_.load(std::memory_order_relaxed);
                                if (last == 0 || now_s - last >= 10) {
                                    last_tipsig_catchup_at_seconds_.store(
                                    ...
                                    if (veld::DiagVerbose().load() &&
                                        ...
                                        std::cout
                                            << "  [layer3] tip-pull CATCHUP-GETBLOCKS broadcast"
                            ...
                            if (veld::DiagVerbose().load() && fetch_queued &&
                                !veld::net::g_suppress_sync.load() && !background_sync_mode_) {
                                if (walked) {
                                    ...
                                    std::cout << "  [layer3] tip-pull GETDATA "
                                              << HashToHex(their_hash).substr(0, 16)
                                              << "... from announcer (gap="
                                              << (their_height - my_height_now) << " src=" << src_ip
                                              << " peer h=" << their_height << ")\n";
                                } else {
                                    std::cout << "  [layer3] tip-pull GETDATA "
                                              << HashToHex(their_hash).substr(0, 16) << "... from "
                                              << src_ip << " (peer h=" << their_height << ")\n";
                                }
                                std::cout.flush();
                            }
                        }
        ...
        if (msg.command == MessageType::BLOCK) {
            ...
            const std::string received_block_hash = HashToHex(new_block.GetHash());
            ...
            if (known_block.has_value()) {
                ...
                if (known_block->Serialize() != msg.payload) {
                    RecordViolation(conn.RemoteAddr(), 15, "block_known_body_mismatch");
            ...
            {
                std::lock_guard<std::mutex> rl(rejected_mutex_);
                rejected_cached = RejectedBlockSeenLocked_(received_block_hash);
            }
            if (rejected_cached) {
                ErasePendingGetData_(received_block_hash);
            ...
            if (use_event_loop_) {
                ...
                if (parent_known) {
                    std::shared_ptr<Connection> sender_sp;
                    {
                        std::lock_guard<std::mutex> lk(peers_mutex_);
                        auto it = peer_connections_.find(key);
                        if (it != peer_connections_.end() && it->second.get() == &conn) {
                            sender_sp = it->second;
                        }
                    }
                    const auto enq = EnqueueBlockIngest(std::move(new_block), msg.payload.size(),
                                                        key, sender_sp);
            ...
            ErasePendingGetData_(received_block_hash);
            auto source_pow_budget = PowBudgetForSource_(StripPort_(conn.RemoteAddr()));
            if (!source_pow_budget) {
                ...
                RecordViolation(conn.RemoteAddr(), 0, "pow_peer_budget_registry_exhausted");
                return StepResult::Handled;
            }
            const auto admission = chain_.AddBlockDirect(
                new_block, false, true, false,
                mining::PowAdmissionContext::Peer(StripPort_(conn.RemoteAddr()), source_pow_budget,
                                                  peer_work_stop_.get_token()));
            ...
            if (admission.IsAccepted()) {
                ...
                uint64_t h = chain_.Height();
                {
                    ...
                    int64_t now_s = (int64_t)std::chrono::duration_cast<std::chrono::seconds>(
                                        std::chrono::system_clock::now().time_since_epoch())
                                        .count();
                    RecordPeerTip(conn, new_block.GetHash(), new_block.height, now_s);
                }
                if ((h <= 10 || h % 100 == 0) && !veld::net::g_suppress_sync.load() &&
                    !background_sync_mode_) {
                    std::lock_guard<std::mutex> lk(veld::net::g_stdout_mtx);
                    std::cout << "  [sync] height=" << h << "  supply=" << std::fixed
                              << std::setprecision(2) << chain_.TotalSupplyVeld() << " VELD\n";
                ...
                if (!parent_known && !HashIsZero(new_block.header.prev_block_hash)) {
                    ...
                    auto remove_victim = [&](const OrphanRec& victim) {
                        ...
                        if (!victim.src_ip.empty()) {
                            ...
                            if (bit != orphan_bytes_by_ip_.end()) {
                                if (bit->second >= victim.size)
                                    bit->second -= victim.size;
                                else
                                    bit->second = 0;
                                if (bit->second == 0)
                                    orphan_bytes_by_ip_.erase(bit);
                    ...
                    size_t peer_count =
                        orphan_count_by_ip_.count(src_ip) ? orphan_count_by_ip_[src_ip] : 0;
                    size_t peer_bytes =
                        orphan_bytes_by_ip_.count(src_ip) ? orphan_bytes_by_ip_[src_ip] : 0;
                    if (peer_count >= MAX_ORPHAN_PER_PEER ||
                        ...
                        if (block_size <= MAX_ORPHAN_BYTES && orphan_count_ < MAX_ORPHAN_POOL &&
                            orphan_bytes_ + block_size <= MAX_ORPHAN_BYTES) {
                            ...
                            if (!already_present && bucket.size() < MAX_ORPHANS_PER_PARENT) {
                                bucket.push_back(
                                    OrphanBlockEntry{new_block, src_ip, source_pow_budget});
                                orphan_count_++;
                                orphan_bytes_ += block_size;
                                orphan_count_by_ip_[src_ip]++;
                                orphan_bytes_by_ip_[src_ip] += block_size;
                                orphan_fifo_.push_back({now_ts, prev_hex, bh, block_size, src_ip});
                                const bool send_getblocks =
                                    TakeKeyCooldown(orphan_getblocks_at_, src_ip, 10, 600, 4096);
                                if (send_getblocks)
                                    conn.Send(BuildChainLocatorGetBlocks(&conn));
                                ...
                                {
                                    std::string parent_key = src_ip + ":" + prev_hex;
                                    const bool send_parent_getdata = TakeKeyCooldown(
                                        orphan_parent_getdata_at_, parent_key, 10, 600, 4096);
                                    if (send_parent_getdata) {
                                        const bool queued = SendBoundedRecoveryGetData_(
                                            conn, new_block.header.prev_block_hash);
                                        if (veld::DiagVerbose().load() && queued &&
                                            !veld::net::g_suppress_sync.load() &&
                                            !background_sync_mode_) {
                                            std::cout << "  [sync-buffer] GETDATA "
                                                      << prev_hex.substr(0, 16)
                                                      << "... from announcer " << src_ip
                                                      << "; orphan child h~"
                                                      << (chain_.Height() + 1)
                                                      << " hash=" << HashToHex(bh).substr(0, 16)
                                                      << "...)\n";
                                            std::cout.flush();
                                        }
                                    }
                                }
                                if (veld::DiagVerbose().load() && orphan_count_ % 100 == 0 &&
                                    !veld::net::g_suppress_sync.load() && !background_sync_mode_) {
                                    std::lock_guard<std::mutex> lk(veld::net::g_stdout_mtx);
                                    std::cout << "  [sync] orphan pool: " << orphan_count_
                                              << " blocks buffered ("
                                              << (orphan_bytes_ / 1024 / 1024) << " MB)\n";
                                    std::cout.flush();
                                }
                            }
                        }
                    ...
                    const std::string tag = chain_.GetLastRejectTag();
                    CacheHeaderAuthenticatedBlockReject_(tag, HashToHex(new_block.GetHash()));
        ...
        const bool is_punch_command =
            msg.command == MessageType::PUNCHHELLO || msg.command == MessageType::GETPUNCH ||
            msg.command == MessageType::PUNCHREQ || msg.command == MessageType::PUNCHLIST ||
            msg.command == MessageType::PUNCHFWD;
        if (is_punch_command && !TakeIpRateToken(punch_control_rx_per_ip_, conn.RemoteAddr(), 32)) {
            return StepResult::Handled;
        }
        if (is_punch_command && (!ps.handshake_done || !HolePunchCapable_(conn))) {
            return StepResult::Handled;
        }
        const bool is_punch_client_action =
            msg.command == MessageType::PUNCHLIST || msg.command == MessageType::PUNCHFWD;
        if (is_punch_client_action && !hole_punch_enabled_.load(std::memory_order_acquire)) {
            return StepResult::Handled;
        }
        if (msg.command == MessageType::PUNCHHELLO) {
            size_t off = 0;
            PunchNonce nonce{};
            if (!ReadPunchNonce_(msg.payload, off, nonce) || off != msg.payload.size()) {
                return StepResult::Handled;
            }
            ...
            if (punchable_.size() < MAX_PUNCHABLE || punchable_.count(endpoint)) {
                punchable_[endpoint] = PunchRegistration{conn.Identity(), nonce, now + 600};
            }
            return StepResult::Handled;
        }
        if (msg.command == MessageType::GETPUNCH) {
            size_t off = 0;
            PunchNonce request_nonce{};
            if (!ReadPunchNonce_(msg.payload, off, request_nonce) || off != msg.payload.size()) {
                return StepResult::Handled;
            }
            std::vector<std::pair<std::string, PunchNonce>> offers;
            {
                const uint64_t now = MonotonicSeconds();
                std::lock_guard<std::mutex> lk(punchable_mutex_);
                for (auto it = punchable_.begin(); it != punchable_.end() && offers.size() < 16;) {
                    if (it->second.expires_at <= now) {
                        it = punchable_.erase(it);
                        continue;
                    }
                ...
                for (auto it = punch_seed_requests_.begin(); it != punch_seed_requests_.end();) {
                    if (it->second.expires_at <= now)
            ...
            conn.TrySend(
                P2PMessage(magic_, MessageType::PUNCHLIST, std::move(payload)).Serialize());
            return StepResult::Handled;
        }
        if (msg.command == MessageType::PUNCHREQ) {
            ...
            if (!ReadPunchNonce_(msg.payload, off, request_nonce) ||
                !ReadPunchNonce_(msg.payload, off, target_nonce) || off >= msg.payload.size()) {
                return StepResult::Handled;
            }
            const uint8_t len = msg.payload[off++];
            if (len == 0 || len > 64 || off + len != msg.payload.size())
                return StepResult::Handled;
            const std::string target(reinterpret_cast<const char*>(msg.payload.data() + off), len);
            ...
            if (!ParsePublicIPv4Endpoint(requester, requester_ip, requester_port))
                return StepResult::Handled;
            ...
            bool authorized = false;
            {
                ...
                if (request != punch_seed_requests_.end() && registration != punchable_.end() &&
                    request->second.requester_connection_id == conn.Identity() &&
                    request->second.expires_at > now && registration->second.expires_at > now &&
                    registration->second.connection_id == target_connection->Identity() &&
                    PunchNonceEqual_(request->second.request_nonce, request_nonce) &&
                    PunchNonceEqual_(registration->second.hello_nonce, target_nonce)) {
            ...
            if (authorized) {
                ...
                target_connection->TrySend(
                    P2PMessage(magic_, MessageType::PUNCHFWD, std::move(payload)).Serialize());
            }
            return StepResult::Handled;
        }
        if (msg.command == MessageType::PUNCHLIST) {
            ...
            for (uint8_t i = 0; i < count; ++i) {
                if (off >= msg.payload.size())
                    return StepResult::Handled;
                const uint8_t len = msg.payload[off++];
                if (len == 0 || len > 64 || off + len > msg.payload.size())
                    return StepResult::Handled;
                std::string endpoint(reinterpret_cast<const char*>(msg.payload.data() + off), len);
                off += len;
                PunchNonce target_nonce{};
                ...
                if (!ParsePublicIPv4Endpoint(endpoint, candidate_ip, candidate_port) ||
            ...
            for (const auto& item : parsed) {
                if (punch_candidates_.size() >= MAX_PUNCH_CANDIDATES)
                    break;
        ...
        if (msg.command == MessageType::PUNCHFWD) {
            size_t off = 0;
            PunchNonce hello_nonce{};
            if (!ReadPunchNonce_(msg.payload, off, hello_nonce) || off >= msg.payload.size()) {
                return StepResult::Handled;
            }
            const uint8_t len = msg.payload[off++];
            if (len == 0 || len > 64 || off + len != msg.payload.size())
                return StepResult::Handled;
            const std::string endpoint(reinterpret_cast<const char*>(msg.payload.data() + off),
                                       len);
            std::string ip;
            uint16_t port = 0;
            ...
            bool authorized = false;
            {
                ...
                if (state != punch_client_coordinators_.end()) {
                    ...
                    if (bound.get() == &conn && state->second.hello_live &&
                        ...
                        PunchNonceEqual_(state->second.hello_nonce, hello_nonce)) {
        ...
        if (msg.command == MessageType::ONIONADV) { // Tor: relay and dial an advertised .onion
            ...
            if (msg.payload.size() <= 80 &&
                TakeIpRateToken(onionadv_rx_per_ip_, conn.RemoteAddr(), 4)) {
                ...
                if (ParseCanonicalOnionEndpoint(a, onion, port)) {
                    ...
                    if (onion == own_onion && port == port_)
                        return StepResult::Handled;
                    if (TakeKeyCooldown(onionadv_relay_at_, a, 60, 600, 4096)) {
                        BroadcastMessage(msg, conn.RemoteAddr());
                    }
                    if (tor_.Active() || tor_only_.load(std::memory_order_acquire)) {
    ...
    bool RunPeerTimers(PeerState& ps, Connection& conn, PeerManager& pm) {
        ...
        if (!ps.handshake_done) {
            const auto age =
                std::chrono::duration_cast<std::chrono::seconds>(now - ps.conn_started).count();
            if (!conn.VersionReceived()) {
                const bool peer_bytes_observed = conn.HasHandshakeProgress();
                const int deadline =
                    peer_bytes_observed ? PRE_VERSION_PROGRESS_GRACE_S : PRE_VERSION_GRACE_S;
                if (age >= deadline)
                    return true;
        ...
        if (ps.handshake_done &&
            std::chrono::duration_cast<std::chrono::seconds>(now - ps.last_mempool_req).count() >=
                120) {
            P2PMessage mempool_req(magic_, MessageType::MEMPOOL);
    ...
    bool HandOffNewPeerToWorker(const std::string& key, std::shared_ptr<Connection> conn,
                                PeerStatePtr ps, bool needs_initial_version_send = false) {
        std::lock_guard<std::mutex> workers_lock(worker_registry_mutex_);
        if (!running_.load(std::memory_order_acquire) || el_workers_.empty() || !conn || !ps) {
            return false;
        }
        ...
        w.incoming.push_back({key, std::move(conn), std::move(ps), needs_initial_version_send});
        return true;
    }
    ...
    static bool FinalityClaimedKeys_(const std::vector<uint8_t>& wire, std::string& lane_out,
                                     std::string& candidate_out) {
        constexpr size_t PREFIX_BYTES = 4;
        constexpr size_t EPOCH_ROOT_PHASE_ROUND_BYTES = 8 + 32 + 1 + 4;
        constexpr size_t PUBKEY_OFFSET = 4 + 8 + 32 + 1 + 4 + 8 + 32 + 8 + 32;
        constexpr size_t TARGET_OFFSET = 4 + 8 + 32 + 1 + 4 + 8 + 32;
        ...
        const Hash256 key_hash =
            Hash256d(wire.data() + PUBKEY_OFFSET, ::veld::dilithium::PUBKEY_BYTES);
        lane_out.clear();
        lane_out.reserve(EPOCH_ROOT_PHASE_ROUND_BYTES + key_hash.size());
        lane_out.append(reinterpret_cast<const char*>(wire.data() + PREFIX_BYTES),
                        EPOCH_ROOT_PHASE_ROUND_BYTES);
        lane_out.append(reinterpret_cast<const char*>(key_hash.data()), key_hash.size());
        ...
        candidate_material.insert(candidate_material.end(), lane_out.begin(), lane_out.end());
        candidate_material.insert(
            candidate_material.end(), wire.begin() + static_cast<std::ptrdiff_t>(TARGET_OFFSET),
            wire.begin() + static_cast<std::ptrdiff_t>(TARGET_OFFSET + TARGET_BYTES));
        const Hash256 candidate_hash = Hash256d(candidate_material);
        candidate_out.assign(reinterpret_cast<const char*>(candidate_hash.data()),
                             candidate_hash.size());
        return true;
    }
    ...
    void PruneFinalityVoteRecentLocked_(uint64_t now) {
        while (!finality_vote_recent_invalid_order_.empty()) {
            ...
            if (it == finality_vote_recent_invalid_.end() || it->second != front.second) {
        ...
        while (!finality_vote_completed_candidate_order_.empty()) {
            const auto& front = finality_vote_completed_candidate_order_.front();
            ...
            if (now <= front.second + FINALITY_VOTE_RECENT_TTL_SECONDS)
                break;
            auto lane = finality_vote_completed_lane_counts_.find(it->second.lane_key);
            if (lane != finality_vote_completed_lane_counts_.end()) {
                if (lane->second > 1)
                    --lane->second;
                else
                    finality_vote_completed_lane_counts_.erase(lane);
            }
    ...
    static void InsertFinalityVoteRecentLocked_(Table& table, Order& order, const std::string& key,
                                                uint64_t now) {
        table[key] = now;
    ...
    bool InsertFinalityVoteCompletedCandidateLocked_(const FinalityVoteJob& job, uint64_t now) {
        ...
        try {
            auto [candidate, inserted] = finality_vote_completed_candidates_.emplace(
                job.candidate_id, FinalityCompletedCandidate{job.lane_key, now});
            (void)candidate;
            if (!inserted)
                return true;
            inserted_candidate = true;
            finality_vote_completed_candidate_order_.emplace_back(job.candidate_id, now);
            inserted_order = true;
            auto [lane, _inserted] =
                finality_vote_completed_lane_counts_.try_emplace(job.lane_key, 0);
            (void)_inserted;
            ++lane->second;
            incremented_lane = true;
        } catch (...) {
            if (incremented_lane) {
                auto lane = finality_vote_completed_lane_counts_.find(job.lane_key);
                if (lane != finality_vote_completed_lane_counts_.end()) {
                    if (lane->second > 1)
                        --lane->second;
                    else
            ...
            if (inserted_order && !finality_vote_completed_candidate_order_.empty() &&
                finality_vote_completed_candidate_order_.back().first == job.candidate_id)
        ...
        while (finality_vote_completed_candidates_.size() > FINALITY_VOTE_RECENT_CAP &&
               !finality_vote_completed_candidate_order_.empty()) {
            const auto oldest = std::move(finality_vote_completed_candidate_order_.front());
            finality_vote_completed_candidate_order_.pop_front();
    ...
    void ReleaseFinalityVoteJobLocked_(const FinalityVoteJob& job) {
        if (finality_vote_pending_ids_.erase(job.wire_id) == 0)
            return;
        ...
        if (lane != finality_vote_lane_counts_.end()) {
            if (lane->second > 1)
                ...
                finality_vote_lane_counts_.erase(lane);
        }
        ...
        if (source != finality_vote_source_counts_.end()) {
            if (source->second > 1)
                ...
                finality_vote_source_counts_.erase(source);
        }
    }
    ...
    bool EnqueueFinalityVote_(const std::vector<uint8_t>& wire, const std::string& source_ip,
                              const std::string& sender_key) {
        if (wire.size() != P2P_FINALITY_VOTE_WIRE_BYTES || source_ip.empty() ||
            IsBanned(source_ip) ||
            !TakeIpRateToken(finality_vote_rx_per_ip_, source_ip, FINALITY_VOTE_PER_IP_PER_60S))
            return false;

        const Hash256 digest = Hash256d(wire);
        const std::string wire_id(reinterpret_cast<const char*>(digest.data()), digest.size());
        std::string lane_key;
        std::string candidate_id;
        if (!FinalityClaimedKeys_(wire, lane_key, candidate_id))
            return false;

        FinalityVoteJob job{wire, source_ip, sender_key, wire_id, candidate_id, lane_key};
        std::lock_guard<std::mutex> lk(finality_vote_mtx_);
        const uint64_t now = MonotonicSeconds();
        ...
        const auto completed_lane_count = finality_vote_completed_lane_counts_.find(lane_key);
        const size_t pending_in_lane =
            lane_count == finality_vote_lane_counts_.end() ? 0 : lane_count->second;
        const size_t completed_in_lane =
            completed_lane_count == finality_vote_completed_lane_counts_.end()
                ? 0
                : completed_lane_count->second;
        if (pending_in_lane + completed_in_lane >= FINALITY_VOTE_PER_LANE_CAP)
            return false;
        if (finality_vote_pending_count_ >= FINALITY_VOTE_JOB_CAP ||
            wire.size() > FINALITY_VOTE_BYTE_CAP -
                              std::min(finality_vote_pending_bytes_, FINALITY_VOTE_BYTE_CAP))
            return false;
        const auto source_count = finality_vote_source_counts_.find(source_ip);
        if (source_count != finality_vote_source_counts_.end() &&
            source_count->second >= FINALITY_VOTE_PER_SOURCE_PENDING_CAP)
            return false;
        if (source_count == finality_vote_source_counts_.end() &&
            finality_vote_source_counts_.size() >= FINALITY_VOTE_SOURCE_TABLE_CAP)
            return false;
        ...
        try {
            auto [source_it, _new_source] = finality_vote_raw_by_source_.try_emplace(source_ip);
            auto [id_it, id_new] = finality_vote_pending_ids_.insert(wire_id);
            (void)id_it;
            if (!id_new)
                return false;
            inserted_id = true;
            ...
            if (source_it != finality_vote_raw_by_source_.end() && source_it->second.jobs.empty() &&
        ...
        for (;;) {
            FinalityVoteJob job;
            {
                ...
                finality_vote_raw_cv_.wait(lk, [this] {
                    return !finality_vote_running_.load(std::memory_order_acquire) ||
                ...
                while (!finality_vote_raw_rr_.empty() && !found) {
                    std::string source = std::move(finality_vote_raw_rr_.front());
                    finality_vote_raw_rr_.pop_front();
                    auto it = finality_vote_raw_by_source_.find(source);
                    if (it == finality_vote_raw_by_source_.end())
                        continue;
            ...
            bool eligible = false;
            try {
                eligible = finality_vote_precheck_ && finality_vote_precheck_(job.wire);
            } catch (...) {
                eligible = false;
            }
            ...
            bool ready = false;
            {
                ...
                const uint64_t now = MonotonicSeconds();
                PruneFinalityVoteRecentLocked_(now);
                if (!finality_vote_running_.load(std::memory_order_acquire) || !eligible ||
                    finality_vote_completed_candidates_.count(job.candidate_id) != 0) {
                ...
                try {
                    ...
                    if (lane != finality_vote_prepared_lanes_.end()) {
                        ...
                        if (!lane->second.in_flight && lane->second.jobs.empty())
            ...
            if (ready)
                finality_vote_ready_cv_.notify_one();
        }
    }
        ...
        for (;;) {
            FinalityVoteJob job;
            {
                ...
                while (!finality_vote_lane_rr_.empty() && !found) {
                    std::string lane_key = std::move(finality_vote_lane_rr_.front());
                    finality_vote_lane_rr_.pop_front();
                    auto it = finality_vote_prepared_lanes_.find(lane_key);
                    if (it == finality_vote_prepared_lanes_.end())
                        continue;
                    it->second.scheduled = false;
            ...
            FinalityVoteVerifyResult result = FinalityVoteVerifyResult::RejectedState;
            try {
                if (finality_vote_running_.load(std::memory_order_acquire) &&
            ...
            const bool accepted = result == FinalityVoteVerifyResult::AcceptedNew;
            const bool completed = accepted || result == FinalityVoteVerifyResult::AlreadyKnown ||
                                   result == FinalityVoteVerifyResult::EvidenceOnly;
            {
                ...
                if (result == FinalityVoteVerifyResult::InvalidSignature) {
                    try {
                        InsertFinalityVoteRecentLocked_(finality_vote_recent_invalid_,
                ...
                if (completed)
                    (void)InsertFinalityVoteCompletedCandidateLocked_(job, now);
                if (lane != finality_vote_prepared_lanes_.end()) {
            ...
            if (result == FinalityVoteVerifyResult::InvalidSignature)
                RecordViolation(job.source_ip, 20, "finvote_signature_invalid");
            if (accepted && running_.load(std::memory_order_acquire)) {
                BroadcastMessage(P2PMessage(magic_, MessageType::FINVOTE, job.wire),
                                 job.sender_key);
            }
        }
    }
    ...
    void FinalityVotePrefilterThreadEntry_() noexcept {
        while (finality_vote_running_.load(std::memory_order_acquire)) {
            try {
                ...
                LogNetworkThreadException_("FINVOTE prefilter worker", e.what());
            } catch (...) {
                NoteContainedNetworkException_();
                LogNetworkThreadException_("FINVOTE prefilter worker", "unknown exception");
            }
            if (finality_vote_running_.load(std::memory_order_acquire))
                std::this_thread::sleep_for(std::chrono::milliseconds(50));
        }
    ...
    void FinalityVoteCryptoThreadEntry_() noexcept {
        while (finality_vote_running_.load(std::memory_order_acquire)) {
            try {
                ...
                NoteContainedNetworkException_();
                LogNetworkThreadException_("FINVOTE crypto worker", e.what());
            } catch (...) {
                NoteContainedNetworkException_();
                LogNetworkThreadException_("FINVOTE crypto worker", "unknown exception");
            }
            if (finality_vote_running_.load(std::memory_order_acquire))
                std::this_thread::sleep_for(std::chrono::milliseconds(50));
        }
    ...
    bool StartFinalityVoteWorkers_() {
        ...
        try {
            const size_t workers = FinalityCryptoWorkerCount(std::thread::hardware_concurrency());
            finality_vote_crypto_threads_.clear();
            finality_vote_crypto_threads_.reserve(workers);
            finality_vote_prefilter_thread_ =
                std::thread(&NodeServer::FinalityVotePrefilterThreadEntry_, this);
...
#ifdef VELD_TEST_HOOKS
        int expected = stage;
        return test_start_failure_stage_.compare_exchange_strong(expected, 0,
...
#ifdef VELD_TEST_HOOKS
        test_suppress_punch_network_dials_.store(false, std::memory_order_release);
    ...
    void RollbackFailedStartLocked_() {
        ...
        for (auto& conn : to_close)
            conn->Close(Connection::CloseReason::NodeStop);
        for (auto& conn : to_close)
            CleanupConnectionState_(conn);
    ...
    static size_t PendingBlockCountLocked_(const BlockIngestLane& a, const BlockIngestLane& b) {
        return a.pending_jobs + b.pending_jobs;
    }

    void PruneProtectedBlockPolicyLocked_(uint64_t now) {
        for (auto it = protected_block_requests_.begin(); it != protected_block_requests_.end();) {
    ...
    bool IsLiveConfiguredOutboundAnchor_(Connection& conn) const {
        if (conn.IsInbound() || !conn.IsConnected() || !IsFleetAnchorIp_(conn.RemoteAddr())) {
            return false;
        }
        std::lock_guard<std::mutex> peers_lock(peers_mutex_);
        for (const auto& [_key, live] : peer_connections_) {
            if (live && live.get() == &conn && live->IsConnected() && !live->IsInbound()) {
                return true;
            }
        }
        return false;
    ...
    bool RegisterProtectedBlockRequest_(Connection& conn, const Hash256& block_hash,
                                        uint64_t now = 0) {
        ...
        auto revoked = protected_block_qos_revoked_until_.find(conn.RemoteAddr());
        if (revoked != protected_block_qos_revoked_until_.end() && now < revoked->second) {
            return false;
        }
        ...
        if (existing != protected_block_requests_.end()) {
            ...
            existing->second.expires_at = now + PROTECTED_BLOCK_REQUEST_TTL_SECONDS;
            return true;
        }
        if (protected_block_requests_.size() >= static_cast<size_t>(MAX_OUTBOUND_CONNECTIONS)) {
            return false;
        }
        protected_block_requests_.emplace(
            id, ProtectedBlockRequestLease{hash_hex, expected_prev,
                                           now + PROTECTED_BLOCK_REQUEST_TTL_SECONDS});
        return true;
    }

    void CancelProtectedBlockRequest_(uint64_t connection_id, const std::string& block_hash = {}) {
        if (connection_id == 0)
            return;
        std::lock_guard<std::mutex> lk(ingest_mtx_);
        auto it = protected_block_requests_.find(connection_id);
        if (it == protected_block_requests_.end())
            return;
        if (!block_hash.empty() && it->second.block_hash != block_hash)
            return;
    ...
    bool QueueBlockInLaneLocked_(BlockIngestLane& lane, PendingBlockIngest&& job, size_t lane_cap,
                                 size_t lane_max_bytes, size_t per_source_max_bytes) {
        auto found = lane.sources.find(job.sender_source);
        const size_t source_jobs = found == lane.sources.end() ? 0 : found->second.pending_jobs;
        const size_t source_bytes = found == lane.sources.end() ? 0 : found->second.pending_bytes;
        size_t source_cap = LAYER_C_BLOCK_INGEST_PER_SOURCE_CAP;
        if (&lane == &ingest_normal_lane_ && !ibd_complete_flag_.load()) {
            if (auto peer = job.sender_conn.lock(); peer && peer->HasIbdDownloadWindow()) {
        ...
        if (lane.pending_jobs >= lane_cap || lane.pending_bytes > lane_max_bytes - job.wire_bytes ||
            source_jobs >= source_cap || source_bytes > per_source_max_bytes - job.wire_bytes ||
            PendingBlockCountLocked_(ingest_protected_lane_, ingest_normal_lane_) >=
                LAYER_C_BLOCK_INGEST_CAP ||
            ingest_pending_bytes_ > LAYER_C_BLOCK_INGEST_MAX_BYTES - job.wire_bytes) {
            return false;
        }
        ...
        try {
            ...
            if (sit != lane.sources.end()) {
                ...
                if (added_rr) {
                    auto rr = std::find(lane.round_robin.begin(), lane.round_robin.end(), source);
                    if (rr != lane.round_robin.end())
                        lane.round_robin.erase(rr);
                    sit->second.in_round_robin = false;
                }
                if (inserted_source && sit->second.jobs.empty() && sit->second.pending_jobs == 0) {
    ...
    bool PopBlockFromLaneLocked_(BlockIngestLane& lane, PendingBlockIngest& out) {
    ...
    bool TakeNextBlockIngestLocked_(PendingBlockIngest& out) {
        ...
        if (!have_protected && !have_normal)
            return false;

        bool take_protected = have_protected && (ingest_protected_preempt_ || !have_normal ||
                                                 ingest_protected_streak_ < 3);
        if (take_protected) {
            ingest_protected_preempt_ = false;
            if (ingest_protected_streak_ < 3)
                ++ingest_protected_streak_;
            if (PopBlockFromLaneLocked_(ingest_protected_lane_, out))
                return true;
        }
        ingest_protected_streak_ = 0;
        if (PopBlockFromLaneLocked_(ingest_normal_lane_, out))
            return true;
        if (PopBlockFromLaneLocked_(ingest_protected_lane_, out)) {
    ...
    void ReleasePendingBlockAccountingLocked_(const PendingBlockIngest& job) {
        BlockIngestLane& lane =
            job.protected_request ? ingest_protected_lane_ : ingest_normal_lane_;
        if (lane.pending_hashes.erase(job.block_hash) == 0)
            return;
        auto sit = lane.sources.find(job.sender_source);
        if (sit != lane.sources.end()) {
            if (sit->second.pending_jobs > 0)
                --sit->second.pending_jobs;
    ...
    void RevokeProtectedBlockQos_(const PendingBlockIngest& rejected) {
        if (!rejected.protected_request || rejected.sender_connection_id == 0)
            return;
        std::lock_guard<std::mutex> lk(ingest_mtx_);
        const uint64_t now = MonotonicSeconds();
        PruneProtectedBlockPolicyLocked_(now);
        if (protected_block_qos_revoked_until_.size() >= PROTECTED_BLOCK_QOS_REVOKE_CAP &&
            protected_block_qos_revoked_until_.count(rejected.sender_source) == 0) {
            auto oldest =
                std::min_element(protected_block_qos_revoked_until_.begin(),
        ...
        for (auto& [source, sq] : ingest_protected_lane_.sources) {
            ...
            if (sq.jobs.empty() && sq.in_round_robin) {
                auto rr = std::find(ingest_protected_lane_.round_robin.begin(),
                                    ingest_protected_lane_.round_robin.end(), source);
    ...
    void RevokeMalformedProtectedBlockQos_(Connection& conn,
                                           const std::string& exact_block_hash = {}) {
        if (!IsLiveConfiguredOutboundAnchor_(conn))
            return;
        bool has_live_lease = false;
        {
            ...
            has_live_lease =
                ...
                (exact_block_hash.empty() || lease->second.block_hash == exact_block_hash);
        }
        if (!has_live_lease)
            return;
    ...
    static bool IsHardBlockReject_(const std::string& tag) {
        return tag == "pow_verify_failed" || tag == "intra_block_double_spend" ||
               tag == "consensus_tx_sig_or_value_invalid" ||
               tag == "consensus_tx_sig_or_value_invalid_v2" || tag == "signature_verify_fail" ||
               tag == "merkle_mismatch" || tag == "block_known_body_mismatch" ||
               tag == "block_too_large" || tag == "too_many_txs" || tag == "empty_transactions" ||
               tag == "tx0_not_coinbase" || tag == "extra_coinbase" ||
               tag == "coinbase_basic_invalid" || tag == "transaction_basic_invalid" ||
               tag == "duplicate_txid_in_block" || tag == "supply_cap_overflow_pre_pow" ||
               tag == "checkpoint_mismatch" || tag == "foreign_genesis" ||
               tag == "stake_lock_amount_out_of_range" || tag == "stake_lock_sum_overflow" ||
    ...
    static bool IsHeaderHashCacheableBlockReject_(const std::string& tag) {
        ...
        return tag == "pow_verify_failed" || tag == "checkpoint_mismatch" ||
               tag == "foreign_genesis";
    }

    void CacheHeaderAuthenticatedBlockReject_(const std::string& tag, const std::string& hash_hex) {
        if (!IsHeaderHashCacheableBlockReject_(tag))
            return;
        std::lock_guard<std::mutex> rl(rejected_mutex_);
        RejectedBlockInsertLocked_(hash_hex);
    }

    void HandleHardBlockReject_(const PendingBlockIngest& job, const std::string& tag) {
        // Credit the stored source even if it disconnected while VeldHash ran.
#ifdef VELD_TEST_HOOKS
        test_block_ingest_penalty_calls_.fetch_add(1, std::memory_order_acq_rel);
#endif
        RecordViolation(job.sender_source, 15, tag.c_str());
        RevokeProtectedBlockQos_(job);
    }

    std::shared_ptr<mining::ExpensivePowBudget> PowBudgetForSource_(const std::string& source) {
        if (source.empty() || source.size() > 255)
            return {};
        const uint64_t now = MonotonicSeconds();
        std::lock_guard<std::mutex> lk(source_pow_budget_mtx_);
        for (auto it = source_pow_budgets_.begin(); it != source_pow_budgets_.end();) {
            ...
            if (idle && stale)
                it = source_pow_budgets_.erase(it);
            else
                ++it;
        }
        ...
        if (source_pow_budgets_.size() >= SOURCE_POW_BUDGET_CAP)
            return {};
...
#endif
        source_pow_budgets_.emplace(source, SourcePowBudgetEntry{budget, now});
        return budget;
    }

    IngestEnqueueResult EnqueueBlockIngest(Block&& blk, size_t wire_bytes,
                                           const std::string& sender_key,
        ...
        bool anchor_outbound = sender_conn && IsLiveConfiguredOutboundAnchor_(*sender_conn);
        Hash256 current_tip{};
        ...
        bool queued = false;
        {
            ...
            const uint64_t now = MonotonicSeconds();
            PruneProtectedBlockPolicyLocked_(now);
            const uint64_t connection_id = sender_conn ? sender_conn->Identity() : 0;
            bool protected_request = false;
            ...
            if (connection_id != 0 && anchor_outbound && have_current_tip &&
                lease != protected_block_requests_.end() && now <= lease->second.expires_at &&
                ...
                (revoked == protected_block_qos_revoked_until_.end() || now >= revoked->second)) {
            ...
            if (protected_request) {
                const bool lane_was_idle = ingest_protected_lane_.pending_jobs == 0;
                queued = QueueBlockInLaneLocked_(
                    ingest_protected_lane_, std::move(e), LAYER_C_BLOCK_INGEST_PROTECTED_CAP,
                    LAYER_C_BLOCK_INGEST_PROTECTED_MAX_BYTES,
                    LAYER_C_BLOCK_INGEST_PROTECTED_PER_SOURCE_MAX_BYTES);
                if (queued) {
                    protected_block_requests_.erase(connection_id);
                    if (lane_was_idle)
                        ingest_protected_preempt_ = true;
                }
            } else {
                queued = QueueBlockInLaneLocked_(ingest_normal_lane_, std::move(e),
...
#ifdef VELD_TEST_HOOKS
        int expected = stage;
        return test_event_loop_exception_stage_.compare_exchange_strong(expected, 0,
                                                                        std::memory_order_acq_rel);
#else
        (void)stage;
        return false;
...
#ifdef VELD_TEST_HOOKS
        test_contained_network_exceptions_.fetch_add(1, std::memory_order_acq_rel);
#endif
    }

    static void LogNetworkThreadException_(const char* entry, const char* detail) noexcept {
        try {
    ...
    void FinalizePeerConnectionNoThrow_(const std::string& key,
                                        const std::shared_ptr<Connection>& conn,
    ...
    void RetireEventLoopPeerNoThrow_(EventLoopWorker* w, const std::string& key,
                                     const std::shared_ptr<Connection>& conn,
                                     const char* phase) noexcept {
        try {
            if (w) {
            ...
            NoteContainedNetworkException_();
            LogNetworkThreadException_("event-loop peer-state cleanup", "container exception");
        }
    ...
    void RetireEventLoopWorkerStateNoThrow_(EventLoopWorker* w) noexcept {
        ...
        try {
            while (!w->peer_states.empty()) {
                ...
                FinalizePeerConnectionNoThrow_(node.key(), node.mapped().conn,
                                               "event-loop shard recovery");
            }
        } catch (...) {
            NoteContainedNetworkException_();
            LogNetworkThreadException_("event-loop shard recovery", "peer-state cleanup exception");
        }
        ...
        try {
            ...
            while (!pending.empty()) {
                ...
                pending.pop_front();
                FinalizePeerConnectionNoThrow_(np.key, np.conn, "event-loop handoff recovery");
            }
        } catch (...) {
            NoteContainedNetworkException_();
    ...
    void EventLoopWorkerThreadEntry_(EventLoopWorker* w) noexcept {
        while (w && w->running.load(std::memory_order_acquire) &&
            ...
            try {
                ...
                NoteContainedNetworkException_();
                LogNetworkThreadException_("event-loop worker", "unknown exception");
            }
    ...
    void AcceptThreadEntry_() noexcept {
        while (running_.load(std::memory_order_acquire)) {
            try {
                ...
                NoteContainedNetworkException_();
                LogNetworkThreadException_("event-loop acceptor", "unknown exception");
            }
            if (running_.load(std::memory_order_acquire))
                std::this_thread::sleep_for(std::chrono::milliseconds(50));
        }
    ...
    void LegacyAcceptThreadEntry_() noexcept {
        while (running_.load(std::memory_order_acquire)) {
            try {
                ...
                NoteContainedNetworkException_();
                LogNetworkThreadException_("legacy acceptor", "unknown exception");
            }
            if (running_.load(std::memory_order_acquire))
                std::this_thread::sleep_for(std::chrono::milliseconds(50));
        }
    ...
    void BlockIngestThreadEntry_() noexcept {
        while (ingest_running_.load(std::memory_order_acquire)) {
            try {
                ...
                NoteContainedNetworkException_();
                LogNetworkThreadException_("block-ingest worker", "unknown exception");
            }
        ...
        while (w->running.load(std::memory_order_acquire) && running_) {
...

            {
                std::vector<EventLoopWorker::PendingNewPeer> drained;
                {
                    ...
                    const size_t count = std::min(MAX_NEW_PEERS_PER_TICK, w->incoming.size());
                ...
                for (auto& np : drained) {
                    try {
                        bool still_registered = false;
                        {
                            ...
                            still_registered =
                                it != peer_connections_.end() && it->second == np.conn;
                        }
                        if (!still_registered) {
                            RetireEventLoopPeerNoThrow_(w, np.key, np.conn,
                                                        "event-loop stale handoff");
                            continue;
                        }
                        if (np.needs_initial_version_send && np.conn) {
                            if (!np.conn->TrySend(
                                    pm.BuildVersionMessage(chain_.Height(), self_nonce_))) {
                        ...
                        NoteContainedNetworkException_();
                        LogNetworkThreadException_("event-loop peer handoff", e.what());
                        RetireEventLoopPeerNoThrow_(w, np.key, np.conn,
                                                    "event-loop handoff cleanup");
                    } catch (...) {
                        NoteContainedNetworkException_();
                        LogNetworkThreadException_("event-loop peer handoff", "unknown exception");
                        RetireEventLoopPeerNoThrow_(w, np.key, np.conn,
                                                    "event-loop handoff cleanup");
                    }
                }
            ...
            {
                std::vector<std::pair<std::string, std::shared_ptr<Connection>>> stale;
                {
                    std::lock_guard<std::mutex> lk(peers_mutex_);
                    for (const auto& [k, bound] : w->peer_states) {
                        auto it = peer_connections_.find(k);
                        if (it == peer_connections_.end() || !it->second ||
                            it->second != bound.conn || !bound.conn || !bound.conn->IsConnected()) {
                            stale.push_back({k, bound.conn});
                        }
                    }
                }
                for (const auto& [k, stale_conn] : stale) {
                    RetireEventLoopPeerNoThrow_(w, k, stale_conn, "event-loop stale peer");
                }
            }
            ...
            peer_fds.reserve(w->peer_states.size());
            {
                ...
                for (const auto& [k, bound] : w->peer_states) {
                    auto it = peer_connections_.find(k);
                    if (it == peer_connections_.end() || it->second != bound.conn)
                        continue;
                    auto& c = bound.conn;
                    if (!c || !c->IsConnected())
                        continue;
            ...
            std::sort(peer_fds.begin(), peer_fds.end(),
                      [](const PeerFdEntry& a, const PeerFdEntry& b) { return a.key < b.key; });
            ...
            for (const auto& e : peer_fds) {
                const short events =
                    static_cast<short>(POLLIN | (e.conn->HasPendingEventLoopSend() ? POLLOUT : 0));
                pfds.push_back({(SOCKET)e.fd, events, 0});
            }
            int pr = pfds.empty() ? 0 : ::WSAPoll(pfds.data(), (ULONG)pfds.size(), 50);
#else
            std::vector<struct pollfd> pfds;
            pfds.reserve(peer_fds.size());
            for (const auto& e : peer_fds) {
            ...
            int pr = pfds.empty() ? 0 : ::poll(pfds.data(), pfds.size(), 50);
#endif
            if (pfds.empty()) {
                std::this_thread::sleep_for(std::chrono::milliseconds(50));
                continue;
            ...
            const size_t io_start = pfds.empty() ? 0 : (w->io_cursor % pfds.size());
            if (!pfds.empty())
                w->io_cursor = (io_start + 1) % pfds.size();
            ...
            for (size_t off = 0; off < pfds.size(); ++off) {
                ...
                if ((rev & POLLOUT) && write_budget > 0 && entry.conn->HasPendingEventLoopSend()) {
                    try {
                        const size_t allowance = std::min(write_budget, MAX_WRITE_BYTES_PER_PEER);
                        ...
                        NoteContainedNetworkException_();
                        LogNetworkThreadException_("event-loop peer write", e.what());
                        RetireEventLoopPeerNoThrow_(w, entry.key, entry.conn,
                                                    "event-loop write cleanup");
                        peer_done[i] = true;
                        continue;
                    } catch (...) {
                        NoteContainedNetworkException_();
                        LogNetworkThreadException_("event-loop peer write", "unknown exception");
                        RetireEventLoopPeerNoThrow_(w, entry.key, entry.conn,
                                                    "event-loop write cleanup");
                        peer_done[i] = true;
                        continue;
            ...
            size_t recv_bytes = 0;
            for (int pass = 0;
                 pass < MAX_DRAIN_PASSES && recv_calls < MAX_RECV_CALLS_PER_TICK &&
                 messages < MAX_MESSAGES_PER_TICK && recv_bytes < MAX_RECV_BYTES_PER_TICK;
                 ++pass) {
                bool any_progress = false;
                for (size_t off = 0;
                     off < pfds.size() && recv_calls < MAX_RECV_CALLS_PER_TICK &&
                     messages < MAX_MESSAGES_PER_TICK && recv_bytes < MAX_RECV_BYTES_PER_TICK;
                     ++off) {
                    const size_t i = (io_start + off) % pfds.size();
                    if (peer_done[i])
                    ...
                    try {
                        ...
                        if (after_recv >= before_recv) {
                            ...
                            recv_bytes += static_cast<size_t>(
                                std::min<uint64_t>(delta, MAX_RECV_BYTES_PER_TICK - recv_bytes));
                        }
                        ...
                        if (!rl.allow)
                            continue;
                        if (!entry.ps->their_version && rr.msg.command != MessageType::VERSION &&
                        ...
                        if (ShouldInjectEventLoopException_(2))
                            throw std::runtime_error("injected peer-dispatch exception");
                        auto sr = PeerProtocolStep(*entry.ps, *entry.conn, pm, rr.msg, entry.key,
                        ...
                        NoteContainedNetworkException_();
                        LogNetworkThreadException_("event-loop peer dispatch", e.what());
                        RetireEventLoopPeerNoThrow_(w, entry.key, entry.conn,
                        ...
                        NoteContainedNetworkException_();
                        LogNetworkThreadException_("event-loop peer dispatch", "unknown exception");
                        RetireEventLoopPeerNoThrow_(w, entry.key, entry.conn,
                                                    "event-loop dispatch cleanup");
                        peer_done[i] = true;
                        continue;
            ...
            if (!died.empty()) {
                for (size_t i = 0; i < died_keys.size(); ++i) {
                    RetireEventLoopPeerNoThrow_(w, died_keys[i], died[i],
...

            {
                ...
                alive_fds.reserve(w->peer_states.size());
                {
                    ...
                    for (const auto& [k, bound] : w->peer_states) {
                        ...
                        if (!c || !c->IsConnected())
                            continue;
                        alive_fds.push_back({c->Fd(), c, k, bound.state});
                    }
                }
                ...
                const size_t timer_count = std::min(MAX_TIMERS_PER_TICK, alive_fds.size());
                const size_t timer_start =
                    alive_fds.empty() ? 0 : (w->timer_cursor % alive_fds.size());
                if (!alive_fds.empty()) {
                    w->timer_cursor = (timer_start + timer_count) % alive_fds.size();
                }
                for (size_t off = 0; off < timer_count; ++off) {
                    auto& e = alive_fds[(timer_start + off) % alive_fds.size()];
                    try {
                        if (ShouldInjectEventLoopException_(3))
                            throw std::runtime_error("injected peer-timer exception");
                        ...
                        NoteContainedNetworkException_();
                        LogNetworkThreadException_("event-loop peer timer", ex.what());
                        RetireEventLoopPeerNoThrow_(w, e.key, e.conn, "event-loop timer cleanup");
                    } catch (...) {
                        NoteContainedNetworkException_();
                        LogNetworkThreadException_("event-loop peer timer", "unknown exception");
                        RetireEventLoopPeerNoThrow_(w, e.key, e.conn, "event-loop timer cleanup");
                    }
                }
                if (!timer_died.empty()) {
                    for (size_t i = 0; i < timer_died_keys.size(); ++i) {
                        RetireEventLoopPeerNoThrow_(w, timer_died_keys[i], timer_died[i],
...
#ifdef VELD_TEST_HOOKS
        test_block_ingest_consensus_calls_.fetch_add(1, std::memory_order_acq_rel);
#endif
        if (!job.source_pow_budget)
            return;
        const auto admission = chain_.AddBlockDirect(
            job.new_block, false, true, false,
            mining::PowAdmissionContext::Peer(job.sender_source, job.source_pow_budget,
                                              peer_work_stop_.get_token()));
        if (admission.IsAccepted()) {
            ...
            if (auto sc = job.sender_conn.lock()) {
                ...
                int64_t now_s = (int64_t)std::chrono::duration_cast<std::chrono::seconds>(
                                    std::chrono::system_clock::now().time_since_epoch())
                                    .count();
                RecordPeerTip(*sc, job.new_block.GetHash(), job.new_block.height, now_s);
            }
            // Post-accept INV broadcast — sender NOT excluded so its INV
            // handler fires on_block_ack_ for layer-1 quorum tracking.
            if (ibd_complete_flag_.load()) {
#ifdef VELD_TEST_HOOKS
                test_block_ingest_relay_calls_.fetch_add(1, std::memory_order_acq_rel);
#endif
                PeerManager pm(magic_, chain_.Height());
                BroadcastMessage(
                    pm.BuildInvMessage({InvItem(InvType::BLOCK, job.new_block.GetHash())}), "");
            }
            if (on_block_)
                on_block_(job.new_block, job.sender_key);

            if (auto sc = job.sender_conn.lock()) {
                PeerManager pm2(magic_, chain_.Height());
                ProcessOrphanChain(job.new_block.GetHash(), pm2, *sc, job.sender_key);
            }
            return;
        }
    ...
    void BlockIngestWorker() {
        ...
        while (ingest_running_.load(std::memory_order_acquire)) {
            ...
            struct PendingAccountingGuard {
                ...
                ~PendingAccountingGuard() {
                    if (release)
                        release();
                }
            } accounting_guard{[this, &job] { ReleasePendingBlockAccounting_(job); }};

            try {
                ProcessBlockIngestJob_(job);
            } catch (const std::exception& e) {
                std::cerr << "  [block-ingest] consensus worker exception for "
                          << job.block_hash.substr(0, 16) << "...: " << e.what()
        ...
        while (running_) {
            ...
            std::vector<PeerFdEntry> peer_fds;
            {
                ...
                for (const auto& [k, c] : peer_connections_) {
                    ...
                    if (sit == peer_states_.end() || sit->second.conn != c)
                        continue;
            ...
            if (!died.empty()) {
                ...
                for (size_t i = 0; i < died_keys.size(); ++i) {
                    ...
                    if (sit != peer_states_.end() && sit->second.conn == died[i]) {
...

            {
                std::vector<PeerFdEntry> alive_fds;
                {
                    ...
                    for (const auto& [k, c] : peer_connections_) {
                        ...
                        if (sit == peer_states_.end() || sit->second.conn != c)
                            continue;
                        alive_fds.push_back({c->Fd(), c, k, sit->second.state});
                    }
                }
                std::vector<std::shared_ptr<Connection>> timer_died;
                std::vector<std::string> timer_died_keys;
                ...
                if (!timer_died.empty()) {
                    for (size_t i = 0; i < timer_died_keys.size(); ++i) {
                        ...
                        if (sit != peer_states_.end() && sit->second.conn == timer_died[i]) {
                            peer_states_.erase(sit);
                        }
                        FinalizePeerConnection_(timer_died_keys[i], timer_died[i]);
                    }
                }
            }
    ...
    std::shared_ptr<Connection> AdmitInboundConnection(SocketHandle client_fd,
                                                       ...
                                                       std::string& out_remote_addr) {
        {
            ...
            if (!is_trusted_early) {
                ...
                if (!TakeIpRateToken(accept_rate_per_ip_, ip_str, ACCEPT_PER_IP_PER_60S)) {
                    ...
                    const bool emit = TakeKeyCooldown(accept_drop_log_at_, ip_str, 60, 600, 4096);
                    if (emit) {
                        std::cerr << "  [accept-rate-limit] dropping fresh conn from " << ip_str
                                  << " — over " << ACCEPT_PER_IP_PER_60S << " accepts/60s\n";
                        std::cerr.flush();
                    }
                    return nullptr;
                }
...

        {
            ...
            if (::ioctlsocket((SOCKET)client_fd, FIONBIO, &accepted_blocking) != 0) {
                VELD_CLOSE_SOCKET(client_fd);
                return nullptr;
            }
#else
            ...
            if (accepted_flags == -1 ||
                ::fcntl(client_fd, F_SETFL, accepted_flags & ~O_NONBLOCK) == -1) {
                VELD_CLOSE_SOCKET(client_fd);
                return nullptr;
            }
#endif
            ...
            ::setsockopt(client_fd, IPPROTO_TCP, TCP_KEEPIDLE, (const char*)&keepidle,
                         sizeof(keepidle));
            ::setsockopt(client_fd, IPPROTO_TCP, TCP_KEEPINTVL, (const char*)&keepintvl,
                         sizeof(keepintvl));
            ::setsockopt(client_fd, IPPROTO_TCP, TCP_KEEPCNT, (const char*)&keepcnt,
                         sizeof(keepcnt));
#endif
#ifdef _WIN32
            ...
            DWORD bytes_returned = 0;
            ::WSAIoctl(client_fd, 0x98000004, &kav, sizeof(kav), nullptr, 0, &bytes_returned,
                       nullptr, nullptr);
#endif
        }
        ...
        }
        {
            ...
            for (const auto& [k, c] : peer_connections_) {
                ...
                if (c->RemoteAddr() == remote_addr) {
                    ++same_ip_count;
                    if (c->IsInbound() && c->IsConnected() && !c->VersionReceived()) {
                        ++same_ip_pre_version;
                    }
                }
            }
            ...
            const uint32_t replaced_inbound =
                (same_key_it != peer_connections_.end() && same_key_it->second &&
                 same_key_it->second->IsInbound())
                    ? 1u
                    : 0u;
            const uint32_t current_inbound = inbound_count_.load(std::memory_order_acquire);
            ...
            if (!ip_is_trusted && same_ip_pre_version >= PRE_VERSION_PER_IP_CAP) {
                reject = true;
            }
            ...
            if (!reject && !ip_is_trusted && same_ip_count >= PER_IP_CAP) {
                ...
                for (const auto& [k, c] : peer_connections_) {
                    if (k == key)
                        continue;
                    if (!c || c->RemoteAddr() != remote_addr)
                        continue;
                    int64_t sort_key = c->IsConnected() ? static_cast<int64_t>(c->BytesRecv()) : -1;
                ...
                for (const auto& [_sort, k] : closable) {
                    if (eviction_victims.size() >= need_to_close)
                        break;
                    auto it = peer_connections_.find(k);
                    if (it != peer_connections_.end() && it->second)
            ...
            if (!ip_is_trusted) {
                const size_t projected_ip =
                    same_ip_count - std::min(same_ip_count, eviction_victims.size());
                const size_t projected_subnet =
                    same_subnet_count - std::min(same_subnet_count, eviction_victims.size());
                if (projected_ip >= PER_IP_CAP || projected_subnet >= PER_SUBNET_CAP)
                    reject = true;
            }
            ...
            if (!reject) {
                ...
                admitted = RegisterAdmittedConnectionLocked_(key, conn, superseded_same_key);
            }
        }
        for (const auto& victim : eviction_victims)
        ...
        if (!eviction_victims.empty()) {
            std::cerr << "  [per-ip-cap] " << remote_addr << " was at " << observed_same_ip
    ...
    void AcceptOnePeerEventDriven(PeerManager&) {
        ...
        PeerManager local_pm(magic_, chain_.Height(),
                             local_services_.load(std::memory_order_acquire));
        if (!conn->Send(local_pm.BuildVersionMessage(chain_.Height(), self_nonce_))) {
            FinalizePeerConnection_(key, conn);
            return;
        }
    ...
    void HandlePeer(std::shared_ptr<Connection> conn, const std::string& key, bool inbound,
                    bool initial_version_sent = false) {
        PeerManager pm(magic_, chain_.Height(), local_services_.load(std::memory_order_acquire));
        ...
        (void)handshake_done;
        (void)getaddr_sent;
        (void)their_start_height;
        (void)last_ping;
        (void)last_getblocks;
        (void)conn_started;
        (void)last_mempool_req;
        while (conn->IsConnected() && running_) {
            if (RunPeerTimers(ps, *conn, pm))
                break;
            ...
            if (!their_version && msg.command != MessageType::VERSION &&
    ...
    static constexpr size_t MAX_GETBLOCKS_RESPONSE = IBD_GETBLOCKS_BATCH_BLOCKS;
