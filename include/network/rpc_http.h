...
inline ParseStatus ParseRequest(const std::string& raw, size_t max_header_bytes,
                                size_t max_body_bytes, Request* out) {
    if (!out || max_header_bytes < 16)
        return ParseStatus::BAD_REQUEST;
    const size_t head_end = raw.find("\r\n\r\n");
    if (head_end == std::string::npos) {
        return raw.size() >= max_header_bytes ? ParseStatus::HEADER_TOO_LARGE
                                              : ParseStatus::INCOMPLETE;
    }
    const size_t body_offset = head_end + 4;
    if (body_offset > max_header_bytes)
        return ParseStatus::HEADER_TOO_LARGE;
    if (raw.find('\0') != std::string::npos)
        return ParseStatus::BAD_REQUEST;

    const size_t first_eol = raw.find("\r\n");
    if (first_eol == std::string::npos || first_eol == 0 || first_eol > head_end)
        return ParseStatus::BAD_REQUEST;
    ...
    if ((parsed.version != "HTTP/1.0" && parsed.version != "HTTP/1.1") || parsed.target.empty() ||
        parsed.target.front() != '/')
        return ParseStatus::BAD_REQUEST;
...
inline const std::string* Header(const Request& request, const char* lower_name) {
...
class RpcHttpServer {
  public:
    RpcHttpServer(RpcServer& rpc, uint16_t port = 8334, const std::string& ui_html = "",
        ...
        : rpc_(rpc), port_(port), running_(false), listen_fd_(compat::kInvalidSocket),
    ...
    bool Start(const std::function<bool()>& activation_guard = {}) {
        ...
        if (const char* value = std::getenv("VELD_RPC_ALLOW_NO_AUTH")) {
            ...
            allow_test_no_auth = setting == "1" || setting == "true" || setting == "yes";
        }
#endif
        if (auth_token_.empty() && !allow_test_no_auth) {
            std::fprintf(stderr,
                         "  [rpc-http] REFUSING to start on port %u: no RPC auth token "
        ...
        const bool listener_ready =
            flags >= 0 && ::fcntl(listen_fd_, F_SETFL, flags | O_NONBLOCK) == 0;
#endif
        if (!listener_ready) {
            VELD_CLOSE_SOCKET(listen_fd_);
            listen_fd_ = compat::kInvalidSocket;
    ...
    const char* ShutdownPhase() const {
  ...
  private:
    ...
    int ReceiveUntil(SocketHandle fd, char* bytes, int size,
        ...
        while (running_.load(std::memory_order_acquire) &&
            ...
            if (!running_.load(std::memory_order_acquire))
                return 0;
            if (received >= 0 || !RetryableSocketWait())
                return received;
        }
        return 0;
    }
    ...
    bool SendAll(SocketHandle fd, const char* bytes, size_t size) {
        ...
        while (sent < size) {
            ...
            const int chunk =
                static_cast<int>(std::min<size_t>(remaining, static_cast<size_t>(INT_MAX)));
            const int wrote = ::send(fd, bytes + sent, chunk, MSG_NOSIGNAL);
            if (wrote < 0) {
                if (RetryableSocketWait())
                    continue;
                return false;
            }
            if (wrote == 0)
    ...
    void RecordRpcClient(const std::string& ip) {
        ...
        if (rpc_clients_.find(ip) == rpc_clients_.end() &&
            ...
            for (auto it = rpc_clients_.begin(); it != rpc_clients_.end();) {
                if (it->second < cutoff)
                    it = rpc_clients_.erase(it);
                else
                    ++it;
            }
            if (rpc_clients_.size() >= TRACKED_CLIENTS_MAX)
                return;
        }
        rpc_clients_[ip] = now;
    }
  ...
  private:
    ...
    static bool AuthFailureBlocked_(AuthFailure& failure, std::chrono::steady_clock::time_point now,
                                    int maximum) {
        if (failure.count == 0)
            return false;
        if (failure.first.time_since_epoch().count() == 0 || now - failure.first > BLOCK_SECS) {
            failure = AuthFailure{};
            return false;
        }
    ...
    bool IsRateLimited(const std::string& ip) {
        ...
        if (AuthFailureBlocked_(global_auth_fails_, now, MAX_GLOBAL_FAIL_AUTH))
            return true;
    ...
    static void RecordAuthFailure_(AuthFailure& failure,
                                   std::chrono::steady_clock::time_point now) {
        if (failure.count == 0 || failure.first.time_since_epoch().count() == 0 ||
    ...
    void RecordFailedAuth(const std::string& ip) {
        ...
        if (auth_fails_.find(ip) == auth_fails_.end() &&
            ...
            if (auth_fails_.size() >= TRACKED_CLIENTS_MAX)
                return;
        }
    ...
    static bool TakeBucketToken_(TokenBucket& bucket, std::chrono::steady_clock::time_point now) {
        if (bucket.last.time_since_epoch().count() == 0) {
            ...
            bucket.tokens = std::min(RATE_LIMIT_BURST, bucket.tokens + elapsed * RATE_LIMIT_RPS);
        }
        bucket.last = now;
    ...
    bool TakeRateToken(const std::string& ip) {
        ...
        if (!TakeBucketToken_(global_rate_bucket_, now))
            return false;
        ...
        if (existing == rate_buckets_.end() && rate_buckets_.size() >= RATE_MAP_MAX)
            return false;
    ...
    void AcceptLoop() {
        while (running_) {
            ...
            if (!running_.load(std::memory_order_acquire))
                break;
            if (poll_result <= 0 || !(ready.revents & POLLIN))
                continue;
            ...
            if (inflight_conns_.load(std::memory_order_acquire) >= MAX_INFLIGHT_RPC_CONNS) {
            ...
            const bool timeouts_ok =
                blocking_ok &&
                ::setsockopt(fd, SOL_SOCKET, SO_RCVTIMEO, (const char*)&to_ms, sizeof(to_ms)) ==
                    0 &&
                ::setsockopt(fd, SOL_SOCKET, SO_SNDTIMEO, (const char*)&to_ms, sizeof(to_ms)) == 0;
#else
            struct timeval tv {
                0, 100000
            };
            ...
            try {
                ...
                if (!worker_started)
                    inflight_conns_.fetch_sub(1, std::memory_order_acq_rel);
                std::cerr << "  [rpc-http] unexpected thread-spawn error: " << e.what() << "\n";
                std::cerr.flush();
                if (!worker_started)
                    CloseRequestSocket(fd);
            }
    ...
    void HandleConnection(SocketHandle fd) {
        ...
        if (header_end == std::string::npos) {
            SendEmptyError(fd, raw.size() >= MAX_HEADERS ? "431 Request Header Fields Too Large"
                                                         : "400 Bad Request");
            CloseRequestSocket(fd);
            return;
        }
        ...
        auto parse_status = rpc_http_detail::ParseRequest(raw, MAX_HEADERS, MAX_BODY, &parsed);
        if (parse_status == rpc_http_detail::ParseStatus::INCOMPLETE) {
            ...
            parse_status = rpc_http_detail::ParseRequest(raw, MAX_HEADERS, MAX_BODY, &parsed);
        }
        if (parse_status != rpc_http_detail::ParseStatus::OK) {
            const char* status = parse_status == rpc_http_detail::ParseStatus::HEADER_TOO_LARGE
        ...
        const std::string* host = rpc_http_detail::Header(parsed, "host");
        const std::string canonical_host_with_port = "127.0.0.1:" + std::to_string(port_);
        const bool canonical_host =
            host && (*host == "127.0.0.1" || *host == canonical_host_with_port);
        const std::string* origin = rpc_http_detail::Header(parsed, "origin");
        const std::string* fetch_site = rpc_http_detail::Header(parsed, "sec-fetch-site");
#ifdef VELD_PUBLIC_TESTNET
        const bool canonical_origin = !origin || *origin == "null";
#else
        const bool canonical_origin = !origin || *origin == "https://wallet.veld.network";
#endif
        const bool canonical_fetch_site = !fetch_site || *fetch_site == "same-origin" ||
                                          *fetch_site == "same-site" || *fetch_site == "none";
        if (parsed.target != "/" || !canonical_host || !canonical_origin || !canonical_fetch_site) {
        ...
        if (!auth_token_.empty() && method == "POST") {
            const std::string* authorization = rpc_http_detail::Header(parsed, "authorization");
            static const std::string prefix = "Bearer ";
            if (authorization && authorization->size() == prefix.size() + auth_token_.size() &&
        ...
        if (!authed) {
            if (!TakeRateToken(client_ip)) {
                std::cerr << "[rate-limit] 429 -> " << client_ip << " (" << method << ")\n";
                std::string err =
        ...
        if (!auth_token_.empty() && method != "OPTIONS" && method != "GET") {
            if (authed) {
                ...
                if (IsRateLimited(client_ip)) {
                    std::string err =
                        R"({"jsonrpc":"2.0","error":{"code":-32001,"message":"Too many failed attempts — try again later"},"id":null})";
                    std::string resp = "HTTP/1.1 429 Too Many Requests\r\n"
                                       "Retry-After: 300\r\n"
                                       "Content-Type: application/json\r\n" +
                                       cors + "Content-Length: " + std::to_string(err.size()) +
        ...
        if (method == "OPTIONS") {
            ...
            if (!ui_html_.empty()) {
                ...
                response = std::string("HTTP/1.1 200 OK\r\n") +
                           ...
                           "base-uri 'self'\r\n" +
                           cors + "Content-Length: " + std::to_string(html.size()) + "\r\n" +
                           "Connection: close\r\n\r\n" + html;
            } else {
            ...
            std::string result =
                body.empty()
                    ...
                    : rpc_.Handle(body);
            response = std::string("HTTP/1.1 200 OK\r\n") + "Content-Type: application/json\r\n" +
                       cors + "Content-Length: " + std::to_string(result.size()) + "\r\n" +
                       "Connection: close\r\n\r\n" + result;
        } else {
            std::string err =
                R"({"jsonrpc":"2.0","error":{"code":-32600,"message":"Method Not Allowed"},"id":null})";
