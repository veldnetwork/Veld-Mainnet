...
// transport constant.
constexpr size_t P2P_FINALITY_VOTE_WIRE_BYTES = ::veld::finality::wire::SIGNED_VOTE_BYTES;
...
constexpr uint64_t REQUIRED_NETWORK_IDENTITY_SERVICES = NODE_BTCVELD_RESERVE_V1;
#else
...
struct VersionPayload {
    ...
    VersionPayload()
        : version(PROTOCOL_VERSION), services(0x01), timestamp((uint64_t)std::time(nullptr)),
          user_agent(CLIENT_USER_AGENT), start_height(0), relay(true), nonce(0), genesis_hash{} {
...
struct Peer {
    ...
    Peer()
        : port(0), state(PeerState::DISCONNECTED), protocol_version(0), services(0), best_height(0),
          connected_time(0), last_seen(0), last_ping(0), ping_nonce(0), ping_latency_ms(0),
          is_inbound(false), ban_score(0) {}

    explicit Peer(const std::string& addr, uint16_t p = MAINNET_PORT)
        : address(addr), port(p), state(PeerState::DISCONNECTED), protocol_version(0), services(0),
          best_height(0), connected_time((uint64_t)std::time(nullptr)),
          last_seen((uint64_t)std::time(nullptr)), last_ping(0), ping_nonce(0), ping_latency_ms(0),
          is_inbound(false), ban_score(0) {}
    ...
    void AddBanScore(uint32_t score) {
        ...
        if (last_ban_event != std::chrono::steady_clock::time_point{}) {
            auto mins =
                std::chrono::duration_cast<std::chrono::minutes>(now - last_ban_event).count();
            if (mins > 0) {
    ...
    std::string GetInfo() const {
        ...
        oss << address << ":" << port << " [" << (is_inbound ? "in" : "out") << "]"
            << " height=" << best_height << " ping=" << ping_latency_ms << "ms"
            << " agent=" << user_agent;
        return oss.str();
    }
};

using OnBlockReceived =
    std::function<void(const std::string& peer_addr, const std::vector<uint8_t>& block_data)>;
using OnTxReceived =
    std::function<void(const std::string& peer_addr, const std::vector<uint8_t>& tx_data)>;
  ...
  public:
    PeerManager(uint32_t magic, uint64_t local_height,
        ...
        : magic_(magic), local_height_(local_height),
          local_services_(local_services | MessageType::NODE_FULL | MessageType::NODE_HOLE_PUNCH |
    ...
    bool AddPeer(const std::string& address, uint16_t port = MAINNET_PORT, bool inbound = false) {
        if (peers_.size() >= MAX_PEER_CONNECTIONS)
            return false;
    ...
    void DropPeer(const std::string& key, const std::string& reason = "") {
        ...
        if (on_peer_dropped_cb_)
            on_peer_dropped_cb_(key);
    }
    ...
    P2PMessage BuildGetDataMessage(const std::vector<InvItem>& items) const {
        return P2PMessage(magic_, MessageType::GETDATA, BuildInvMessage(items).payload);
    }

    P2PMessage BuildGetBlocksMessage(const Hash256& our_tip, const Hash256& stop_hash = {}) const {
        return BuildGetBlocksMessage(our_tip, {}, stop_hash);
    }

    P2PMessage BuildGetBlocksMessage(const Hash256& our_tip,
                                     const std::vector<Hash256>& extra_locators,
                                     const Hash256& stop_hash = {}) const {
        std::vector<uint8_t> payload;
    ...
    P2PMessage BuildSolutionMessage(const Hash256& prev_hash, uint64_t height, uint64_t nonce,
                                    const std::vector<uint8_t>& miner_script) const {
        std::vector<uint8_t> payload;
    ...
    P2PMessage BuildAddrMessage(const std::vector<std::pair<std::string, uint16_t>>& peers) const {
        ...
        for (const auto& p : peers) {
            struct in_addr addr{};
            if (::inet_pton(AF_INET, p.first.c_str(), &addr) != 1)
                continue;
            if (!looks_routable(addr.s_addr))
                continue;
    ...
    P2PMessage BuildCOMineMessage(const Hash256& prev_hash, uint64_t height, uint64_t best_nonce,
                                  const Hash256& best_hash,
                                  const std::vector<uint8_t>& miner_script,
                                  const std::vector<uint8_t>& comine_pubkey,
                                  const std::vector<uint8_t>& comine_sig) const {
        std::vector<uint8_t> payload;
        payload.insert(payload.end(), prev_hash.begin(), prev_hash.end());
        for (int i = 0; i < 8; ++i)
            payload.push_back((height >> (i * 8)) & 0xFF);
        for (int i = 0; i < 8; ++i)
            payload.push_back((best_nonce >> (i * 8)) & 0xFF);
    ...
    P2PMessage BuildFinalityVoteMessage(const std::vector<uint8_t>& canonical_vote) const {
    ...
    void BroadcastBlockInv(const Hash256& block_hash) {
        ...
        for (auto& [key, peer] : peers_) {
            if (!peer.IsConnected())
                continue;
            if (peer.known_inv.count(HashToHex(block_hash)))
                continue;
            SendMessage(key, msg);
        }
    }
    ...
    void BroadcastTxInv(const Hash256& tx_hash) {
        ...
        for (auto& [key, peer] : peers_) {
            if (!peer.IsConnected())
                continue;
            if (peer.known_inv.count(HashToHex(tx_hash)))
                continue;
            SendMessage(key, msg);
        }
    }
    ...
    void OnBlockReceived(OnBlockReceived cb) {
        on_block_received_cb_ = cb;
    }
    void OnTxReceived(OnTxReceived cb) {
        on_tx_received_cb_ = cb;
    }
    void OnPeerConnected(OnPeerConnected cb) {
        on_peer_connected_cb_ = cb;
    }
    void OnPeerDropped(OnPeerDropped cb) {
        on_peer_dropped_cb_ = cb;
    }
