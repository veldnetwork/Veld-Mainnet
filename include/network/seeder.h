...
struct PeerRecord {
    ...
    PeerRecord()
        : port(CompiledPublicP2PPort()), last_seen(0), first_seen(0), protocol_version(0),
          best_height(0), services(0x01), is_reachable(false), fail_count(0) {
        first_seen = veld::compat::MonotonicSeconds();
    }

    explicit PeerRecord(const std::string& ip_addr, uint16_t p = CompiledPublicP2PPort())
        : ip(ip_addr), port(p), last_seen(0), first_seen(veld::compat::MonotonicSeconds()),
          protocol_version(0), best_height(0), services(0x01), is_reachable(false), fail_count(0) {}

    bool IsGood() const {
        uint64_t now = veld::compat::MonotonicSeconds();
        return is_reachable && fail_count < 3 && (now - last_seen) < 24 * 3600;
    }
...
class PeerDB {
  public:
        ...
        if (!peers_.count(key)) {
            if (peers_.size() >= MAX_PEERDB_ENTRIES) {
                ...
                for (auto it = peers_.begin(); it != peers_.end(); ++it) {
                    if (worst == peers_.end() || it->second.fail_count > worst_fail ||
                        (it->second.fail_count == worst_fail && it->second.last_seen < worst_age)) {
                        worst = it;
    ...
    std::vector<PeerRecord> GetGoodPeers(size_t count = 25) const {
        ...
        std::mt19937_64 rng;
        {
            ...
            if (veld::compat::SecureRandom(seed_bytes, 8)) {
                uint64_t seed = 0;
                for (int i = 0; i < 8; ++i)
                    seed |= ((uint64_t)seed_bytes[i] << (i * 8));
                rng.seed(seed);
            } else {
                rng.seed(
        ...
        const size_t inspection_budget = std::min(good_keys_.size(), max_window);
        size_t cursor = good_keys_.empty() ? 0 : static_cast<size_t>(rng() % good_keys_.size());
        size_t inspected = 0;
        std::unordered_set<std::string> selected;
        selected.reserve(std::min(requested, inspection_budget));
        while (inspected < inspection_budget && !good_keys_.empty() && good.size() < requested) {
    ...
    static size_t MaxSelectionInspectionsForTesting(size_t count) {
        const size_t requested = std::min(count, MAX_PEERDB_ENTRIES);
        return requested > (MAX_PEERDB_ENTRIES / 8) ? MAX_PEERDB_ENTRIES
                                                    : std::max<size_t>(64, requested * 8);
    }
#endif
...
class NetworkCrawler {
  public:
    ...
    bool Start(const std::vector<std::string>& bootstrap_ips) {
        ...
        if (running_.load() || crawl_thread_.joinable())
            return false;
...

        {
            ...
            for (const auto& ip : bootstrap_ips) {
                if (to_crawl_.size() >= MAX_CRAWL_QUEUE_ENTRIES)
                    break;
    ...
    PeerRecord ProbePeer(const std::string& ip, uint16_t port) {
...
#endif
        if (::setsockopt(fd, SOL_SOCKET, SO_RCVTIMEO, reinterpret_cast<const char*>(&tv),
                         sizeof(tv)) != 0 ||
            ::setsockopt(fd, SOL_SOCKET, SO_SNDTIMEO, reinterpret_cast<const char*>(&tv),
                         sizeof(tv)) != 0) {
        ...
        if (connect_result == 0) {
            ...
            if (in_progress) {
                ...
                int waited_ms = 0;
                while (!stop_requested_.load() && waited_ms < CONNECT_BUDGET_MS) {
#ifdef _WIN32
                    WSAPOLLFD poll_fd{(SOCKET)fd, POLLOUT, 0};
                    const int poll_result = ::WSAPoll(&poll_fd, 1, CONNECT_SLICE_MS);
#else
                    struct pollfd poll_fd{fd, POLLOUT, 0};
                    const int poll_result = ::poll(&poll_fd, 1, CONNECT_SLICE_MS);
#endif
                    if (poll_result < 0) {
#ifndef _WIN32
                        if (errno == EINTR)
        ...
        uint8_t header[24] = {0};
        if (RecvExact(fd, header, sizeof(header)) && IsCanonicalVersionHeader(header)) {
            ...
            record.user_agent.assign(reinterpret_cast<const char*>(payload.data() + 29),
                                     user_agent_len);
        }
  ...
  private:
    ...
    bool SendExact(SocketHandle fd, const uint8_t* data, size_t size) const {
        ...
        while (sent < size && !stop_requested_.load() &&
            ...
            const int chunk = remaining > static_cast<size_t>(std::numeric_limits<int>::max())
                                  ? std::numeric_limits<int>::max()
                                  : static_cast<int>(remaining);
            const ssize_t n =
                ::send(fd, reinterpret_cast<const char*>(data + sent), chunk, MSG_NOSIGNAL);
            if (n > 0) {
    ...
    bool RecvExact(SocketHandle fd, uint8_t* data, size_t size) const {
        ...
        while (received < size && !stop_requested_.load() &&
            ...
            const int chunk = remaining > static_cast<size_t>(std::numeric_limits<int>::max())
                                  ? std::numeric_limits<int>::max()
                                  : static_cast<int>(remaining);
            const ssize_t n = ::recv(fd, reinterpret_cast<char*>(data + received), chunk, 0);
            if (n > 0) {
    ...
    bool IsCanonicalVersionHeader(const uint8_t* header) const {
        if (ReadLE32(header) != magic_)
            return false;
        static constexpr uint8_t version_command[12] = {'v', 'e', 'r', 's', 'i', 'o',
                                                        'n', 0,   0,   0,   0,   0};
    ...
    bool IsCanonicalVersionPayload(const uint8_t* header,
                                   const std::vector<uint8_t>& payload) const {
        if (payload.size() < 70)
            return false;
        const size_t user_agent_len = payload[28];
        if (payload.size() != 70 + user_agent_len)
            return false;
    ...
    void CrawlLoop() {
        while (running_) {
            ...
            uint16_t port;
            try {
                port = (uint16_t)std::stoi(key.substr(colon + 1));
            } catch (...) {
                continue;
            }
...
class DNSSeeder {
  public:
    DNSSeeder(PeerDB& db, uint16_t dns_port = 53)
        : db_(db), dns_port_(dns_port), running_(false), fd_(veld::compat::kInvalidSocket) {
    ...
    bool Start() {
        std::lock_guard<std::mutex> lifecycle_lock(lifecycle_mutex_);
        if (running_.load() || dns_thread_.joinable() || veld::compat::IsValidSocket(fd_)) {
            return false;
        }
        ...
        if (::setsockopt(fd_, SOL_SOCKET, SO_RCVTIMEO, reinterpret_cast<const char*>(&recv_timeout),
    ...
    static std::vector<uint8_t> BuildDNSResponse(const std::vector<uint8_t>& query,
    ...
    static std::vector<uint8_t> BuildBoundedDNSResponse(const std::vector<uint8_t>& query,
                                                        const std::vector<std::string>& ips) {
...
#ifdef VELD_TEST_HOOKS
    bool TakeDnsRateTokenForTesting(uint32_t client_ip_be, uint64_t monotonic_millis) {
        return TakeDnsRateTokenAt(client_ip_be, std::chrono::steady_clock::time_point(
  ...
  private:
    ...
    struct DnsRateBucket {
        ...
        DnsRateBucket(double initial_tokens, std::chrono::steady_clock::time_point now,
                      std::list<uint32_t>::iterator it)
            : tokens(initial_tokens), last_refill(now), last_seen(now), lru_it(it) {}
    };
    std::mutex dns_rate_mutex_;
    std::unordered_map<uint32_t, DnsRateBucket, DnsRateIpHash> dns_rate_map_;
    ...
    std::chrono::steady_clock::time_point dns_global_last_refill_{std::chrono::steady_clock::now()};

    bool TakeDnsRateToken(uint32_t client_ip_be) {
        return TakeDnsRateTokenAt(client_ip_be, std::chrono::steady_clock::now());
    }

    bool TakeDnsRateTokenAt(uint32_t client_ip_be, std::chrono::steady_clock::time_point now) {
        ...
        if (now >= dns_global_last_refill_) {
            global_elapsed = std::chrono::duration<double>(now - dns_global_last_refill_).count();
        }
        dns_global_tokens_ = std::min(
            DNS_GLOBAL_BURST, dns_global_tokens_ + global_elapsed * DNS_GLOBAL_REFILL_PER_SECOND);
        ...
        const auto idle_ttl = std::chrono::seconds(DNS_RATE_IDLE_TTL_SECONDS);
        while (!dns_rate_lru_.empty()) {
            ...
            if (now < oldest->second.last_seen || now - oldest->second.last_seen < idle_ttl) {
                break;
            }
            dns_rate_lru_.pop_front();
        ...
        if (found == dns_rate_map_.end()) {
            ...
            auto inserted =
                dns_rate_map_.emplace(std::piecewise_construct, std::forward_as_tuple(client_ip_be),
            ...
            dns_rate_lru_.splice(dns_rate_lru_.end(), dns_rate_lru_, found->second.lru_it);
    ...
    void ServeLoop() {
        ...
        while (running_) {
            ...
            ssize_t n =
                ::recvfrom(fd_, (char*)buf, sizeof(buf), 0, (struct sockaddr*)&client, &client_len);
            if (n < 12)
                continue;

            if (!IsSaneDnsQuery(buf, (size_t)n))
                continue;
            if (!TakeDnsRateToken(client.sin_addr.s_addr))
                continue;
            ...
            if (!response.empty()) {
                ...
                const ssize_t sent =
                    ::sendto(fd_, reinterpret_cast<const char*>(response.data()), response_size, 0,
...
class SeedNodeClient {
  public:
    ...
    ResolveSeedNodes(const std::vector<std::string>& seed_hostnames) {
        std::vector<std::string> ips;
        ips.reserve(MAX_RESOLVED_SEED_ADDRESSES);

        size_t hosts_visited = 0;
        for (const auto& hostname : seed_hostnames) {
            if (hosts_visited++ >= MAX_SEED_HOSTNAMES)
                break;
            if (ips.size() >= MAX_RESOLVED_SEED_ADDRESSES)
                break;
            ...
            for (auto* p = res;
                 p != nullptr && retained_for_host < MAX_RESOLVED_ADDRESSES_PER_HOST &&
...
class SeederService {
  public:
    SeederService(uint32_t magic, uint16_t dns_port = 5353)
        : db_(), crawler_(db_, magic), dns_(db_, dns_port), magic_(magic) {}

    bool Start(const std::vector<std::string>& bootstrap_ips) {
        if (!crawler_.Start(bootstrap_ips))
            return false;
        if (!dns_.Start()) {
            crawler_.Stop();
            return false;
