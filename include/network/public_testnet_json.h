  ...
  private:
    ...
    bool ParseArray(Value& out, std::string& error, size_t depth) {
        ...
        for (;;) {
            ...
            if (input_[pos_] != ',')
                return Fail(error, "missing JSON array comma");
            ++pos_;
            SkipWhitespace();
        }
    }
    bool ParseObject(Value& out, std::string& error, size_t depth) {
        ...
        for (;;) {
            ...
            if (input_[pos_] != ',')
                return Fail(error, "missing JSON object comma");
            ++pos_;
            SkipWhitespace();
        }
    }
...
inline bool AppendCanonicalString(const std::string& value, std::string& out, std::string& error) {
    ...
    for (size_t i = 0; i < value.size();) {
        ...
        if ((width == 3 && ((first == 0xe0 && cp < 0x800) || (first == 0xed && cp >= 0xd800))) ||
            (width == 4 && ((first == 0xf0 && cp < 0x10000) || (first == 0xf4 && cp > 0x10ffff))) ||
...
inline bool AppendCanonical(const Value& value, size_t depth, std::string& out,
                            std::string& error) {
    switch (value.kind) {
    ...
    case Value::Kind::Object: {
        ...
        for (const auto& [key, child] : value.object) {
            out.append((depth + 1) * 2, ' ');
            if (!AppendCanonicalString(key, out, error))
                return false;
            out += ": ";
...
inline bool Canonicalize(const Value& value, std::string& out, std::string& error) {
    out.clear();
    if (!AppendCanonical(value, 0, out, error))
        return false;
    out.push_back('\n');
    return true;
}
