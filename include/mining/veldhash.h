...
class ExpensivePowBudget {
  public:
    explicit ExpensivePowBudget(uint32_t max_in_flight, uint32_t max_external_starts_per_window = 0,
                                std::chrono::milliseconds external_window = std::chrono::minutes(1))
        : maximum_(max_in_flight == 0 ? 1 : max_in_flight),
          max_external_starts_(max_external_starts_per_window),
          external_window_(external_window.count() <= 0 ? std::chrono::minutes(1)
                                                        : external_window),
          external_window_started_(std::chrono::steady_clock::now()) {}

    std::optional<ExpensivePowLease> TryAcquire(ExpensivePowUse use) noexcept {
        attempts_.fetch_add(1, std::memory_order_relaxed);
        uint32_t observed = in_flight_.load(std::memory_order_relaxed);
        while (observed < maximum_) {
            if (in_flight_.compare_exchange_weak(observed, observed + 1, std::memory_order_acq_rel,
                ...
                while (peak < observed + 1 &&
                       !peak_.compare_exchange_weak(peak, observed + 1, std::memory_order_relaxed,
                                                    std::memory_order_relaxed)) {
                }
    ...
    ExpensivePowBudgetStats Stats() const noexcept {
        return {attempts_.load(std::memory_order_relaxed),
                admitted_.load(std::memory_order_relaxed), refused_.load(std::memory_order_relaxed),
                in_flight_.load(std::memory_order_relaxed), peak_.load(std::memory_order_relaxed)};
    }
  ...
  private:
    ...
    bool
    ReserveExternalStart_(ExpensivePowUse use,
        ...
        const bool external = use == ExpensivePowUse::PeerBlock ||
                              ...
                              use == ExpensivePowUse::PeerReorg || use == ExpensivePowUse::RpcReorg;
        if (!external || max_external_starts_ == 0)
            return true;
        ...
        if (external_starts_ >= max_external_starts_)
            return false;
        ++external_starts_;
...
class ExpensivePowCharge {
  ...
  private:
    friend class ExpensivePowLease;
    ExpensivePowCharge(ExpensivePowBudget* owner, std::chrono::steady_clock::time_point window)
...
inline ExpensivePowLease& ExpensivePowLease::operator=(ExpensivePowLease&& other) noexcept {
    if (this != &other) {
        if (owner_)
            owner_->Release();
        owner_ = other.owner_;
...
class LocalWorkLeaseHandoff {
  public:
    void Install(std::shared_ptr<void> owner, std::function<bool()> live) noexcept {
...
struct PowAdmissionContext {
    ...
    static PowAdmissionContext Peer(std::string source, std::shared_ptr<ExpensivePowBudget> budget,
                                    std::stop_token stop = {}) {
    ...
    static PowAdmissionContext
    RpcWork(std::string binding, std::string authorization,
            std::shared_ptr<work_admission::BlockTemplateAuthorizationClaim> claim) {
        ...
        out.local_work_handoff = std::make_shared<LocalWorkLeaseHandoff>();
        return out;
    }
    ...
    static PowAdmissionContext SynchronousGenerationWork(std::string binding) {
        PowAdmissionContext out = Internal();
    ...
    bool HasRequiredProvenance() const noexcept {
        if (source_identity.size() > 255)
            return false;
        if (origin == PowAdmissionOrigin::Unwired)
            return false;
        if (RequiresLocalWorkAdmission()) {
            if (origin != PowAdmissionOrigin::Rpc && origin != PowAdmissionOrigin::Internal)
                return false;
            if (work_binding.empty() || work_binding.size() > 512 || !local_work_handoff)
                return false;
            if (local_work_kind == LocalWorkKind::SubmitBlock) {
                if (work_authorization.empty() || work_authorization.size() > 128 ||
                    !block_template_authorization_claim)
                    return false;
            } else if (!work_authorization.empty() || block_template_authorization_claim) {
                return false;
            }
        ...
        if (origin == PowAdmissionOrigin::Peer)
            return !source_identity.empty() && static_cast<bool>(source_budget);
        return !source_budget && !peer_stop.stop_possible();
    }

    bool IsExternal() const noexcept {
        return origin == PowAdmissionOrigin::Peer || origin == PowAdmissionOrigin::Rpc;
    }
...
static_assert(INT64_MIN == (-INT64_MAX - 1), "VeldHash requires two's-complement int64_t");

inline constexpr unsigned VELD_FIXED_FRAC_BITS = 24;
inline constexpr int64_t VELD_FIXED_ONE = static_cast<int64_t>(uint64_t{1} << VELD_FIXED_FRAC_BITS);
...
inline int64_t VeldFixedAdd(int64_t a, int64_t b) {
    return VeldFixedClamp(static_cast<__int128>(a) + static_cast<__int128>(b));
}
...
inline int64_t VeldFixedFromInteger(int64_t value) {
    return VeldFixedClamp(static_cast<__int128>(value) * static_cast<__int128>(VELD_FIXED_ONE));
}

inline int64_t VeldFixedMul(int64_t a, int64_t b) {
    const __int128 product = static_cast<__int128>(a) * static_cast<__int128>(b);
    return VeldFixedClamp(product / static_cast<__int128>(VELD_FIXED_ONE));
}
...
inline uint64_t VeldIntegerSqrt128(unsigned __int128 value) {
    ...
    const unsigned top_bit =
        high != 0 ? 127u - static_cast<unsigned>(std::countl_zero(high))
    ...
    !defined(VELD_MINING_PORTABLE_DIVISION)
        ...
        if ((root >> 64) == 0 && high < divisor) {
            ...
            __asm__("div{q %4| %4}"
                    : "=a"(q), "=d"(remainder)
                    : "a"(static_cast<uint64_t>(value)), "d"(high), "r"(divisor)
                    : "cc");
            quotient = q;
        } else
#endif
        ...
        if (next >= root)
            return static_cast<uint64_t>(root);
        root = next;
    }
}
...
inline int64_t VeldFixedSqrt(int64_t nonnegative_q24) {
    if (nonnegative_q24 <= 0)
        return 0;
    const unsigned __int128 scaled =
        static_cast<unsigned __int128>(static_cast<uint64_t>(nonnegative_q24))
        << VELD_FIXED_FRAC_BITS;
    const uint64_t root = VeldIntegerSqrt128(scaled);
    return root > static_cast<uint64_t>(INT64_MAX) ? INT64_MAX : static_cast<int64_t>(root);
}
...
inline uint64_t VeldIntegerDeterminismKat() {
    ...
    auto normalize = [&](int64_t value) {
        while (value > three)
            value = VeldFixedSub(value, three_halves);
        while (value < VELD_FIXED_ONE)
            value = VeldFixedAdd(value, half);
        return value;
    };

    int64_t a = VeldFixedAdd(VeldFixedFromInteger(10), 1);
    int64_t b = VeldFixedClamp(static_cast<__int128>(VELD_FIXED_ONE) * 4 / 3);
    int64_t c = VeldFixedAdd(VeldFixedFromInteger(2), 3);
    int64_t s = VELD_FIXED_ONE;
    uint64_t mix = 0x9E3779B97F4A7C15ULL;
    for (int i = 0; i < 96; ++i) {
        const int64_t t1 = VeldFixedAdd(VeldFixedMul(a, b), s);
        const int64_t t2 = normalize(VeldFixedSqrt(t1 > 0 ? t1 : VELD_FIXED_ONE));
        const int64_t next_a = normalize(VeldFixedAdd(b, t2));
        const int64_t next_b = normalize(VeldFixedSub(c, VeldFixedSub(t1, t2)));
        const int64_t next_c = normalize(VeldFixedMul(VeldFixedAdd(t1, t2), half));
        const int64_t sum = VeldFixedAdd(VeldFixedAdd(a, b), c);
        const int64_t next_s = normalize(VeldFixedClamp(static_cast<__int128>(sum) / 3));
        s = next_s;
        a = next_a;
        b = next_b;
        c = next_c;
        mix ^=
            VeldFixedBits(a) + (VeldFixedBits(b) << 1) + (VeldFixedBits(c) >> 1) + VeldFixedBits(s);
        mix = (mix << 13) | (mix >> 51);
    }
    return mix;
}

inline constexpr uint64_t VELD_INTEGER_KAT_EXPECTED = 0x8a02b1c3b36ab8e8ULL;

inline void VeldIntegerDeterminismCheck() {
    const uint64_t got = VeldIntegerDeterminismKat();
    if (got != VELD_INTEGER_KAT_EXPECTED) {
        std::cerr << "  [FATAL INTEGER-KAT] expected=0x" << std::hex << VELD_INTEGER_KAT_EXPECTED
                  << " got=0x" << got << std::dec
  ...
  public:
    DatasetCache() {
        try {
            ...
            std::cerr << "\n  FATAL: failed to allocate " << (DATASET_SIZE / 1024 / 1024)
                      << " MB for the VeldHash dataset.\n"
                      << "  This node CANNOT validate blocks without it.\n"
                      << "  Provision a host with at least " << ((DATASET_SIZE / 1024 / 1024) + 256)
                      << " MB of free RAM.\n\n";
            std::cerr.flush();
            std::abort();
        }
    ...
    StatsSnapshot Stats() const noexcept {
        return {requests_.load(std::memory_order_relaxed), hits_.load(std::memory_order_relaxed),
                builds_.load(std::memory_order_relaxed), failures_.load(std::memory_order_relaxed)};
    }
    static constexpr size_t CapacityIdentities() noexcept {
        return 1;
    }
...
inline std::vector<VeldInstruction> GenerateProgram(uint64_t seed_a, uint64_t seed_b) {
...
#ifdef VELD_MAINNET_POW
    static const uint8_t OP_WEIGHTS[] = {12, 7, 12, 12, 4, 4,  4,  4, 8, 8, 12, 4,  8,  8,
                                         8,  4, 4,  16, 8, 10, 10, 8, 6, 6, 22, 22, 18, 18};
#else
    static const uint8_t OP_WEIGHTS[] = {16, 7, 16, 16, 4, 4, 4, 4, 8, 8, 16, 4, 8, 8,
                                         8,  4, 4,  16, 8, 0, 0, 0, 0, 0, 0,  0, 0, 0};
#endif
    ...
    for (uint32_t i = 0; i < PROGRAM_SIZE; ++i) {
        ...
        if (op == VeldOp::ISTORE)
            ++istores;
        if (op == VeldOp::IADD_D || op == VeldOp::IXOR_D || op == VeldOp::IMIX_D)
            ++dops;
        if (op == VeldOp::QADD_R || op == VeldOp::QMUL_R || op == VeldOp::QSUB_R ||
            op == VeldOp::QSQRT_R)
            ++fixed_ops;
    }
    uint32_t cursor = 0;
...
template <bool LightDataset> class VeldHashVMImpl {
  public:
    VeldHashVMImpl() {
    ...
    void SetDataset(const uint64_t* dataset) {
        dataset_ = dataset;
    }
#if defined(VELD_MAINNET_POW)
    void SetLightKey(const Hash256& seed) {
        std::memcpy(light_key_, seed.data(), 32);
    }
#endif
    ...
    void Initialize(const Hash256& seed) {
        ...
        VeldPRNG sp_prng(le64(seed.data()), le64(seed.data() + 8));
...
#endif
    inline void
    ExecuteInstruction(const VeldInstruction& instr, uint32_t& pc) {
        ...
        case VeldOp::IADD_RS:
            dst += src << (instr.mod & 0x3);
            break;
        case VeldOp::IADD_M:
            dst += scratchpad_[addr];
            break;
        case VeldOp::ISUB_R:
            dst -= src;
            break;
        ...
        case VeldOp::IROR_R:
            dst = rotr64(dst, src & 63);
            break;
        case VeldOp::IROL_R:
            dst = rotl64(dst, src & 63);
            break;
        case VeldOp::IXOR_R:
            dst ^= src;
            break;
        ...
        case VeldOp::IMIX_R: {
            const int64_t fd = VeldFixedAdd(
                VeldFixedMul(VeldFixedFromInteger(VeldFixedSignedHigh32(dst)), VELD_FIXED_ONE + 1),
        ...
        case VeldOp::IEXT_R: {
            const int64_t fd = VeldFixedMul(
                ...
                VeldFixedAdd(VeldFixedFromInteger(VeldFixedSignedHigh32(src)), VELD_FIXED_ONE));
            const uint64_t fbits = VeldFixedBits(fd);
        ...
        case VeldOp::IMIX_M: {
            ...
            const int64_t fd = VeldFixedSqrt(
                VeldFixedAbs(VeldFixedFromInteger(VeldFixedSignedHigh32(mem | 1ULL))));
            const uint64_t fbits = VeldFixedBits(fd);
...
#else
        case VeldOp::IMIX_R:
            dst = (dst + src) ^ rotr64(dst, 17);
            break;
        case VeldOp::IEXT_R:
            dst = dst * 0x9e3779b97f4a7c15ULL + src;
            break;
        case VeldOp::IMIX_M:
            dst ^= rotr64(scratchpad_[addr], dst & 63);
            break;
#endif
        ...
        case VeldOp::IADD_C:
            dst += (uint64_t)instr.imm;
            break;
        case VeldOp::IMUL_C:
            dst *= ((uint64_t)instr.imm | 1ULL);
            break;

        case VeldOp::QADD_R: {
            int64_t& fdst = fixed_regs_[instr.dst & (FIXED_REG_COUNT - 1)];
            const int64_t fsrc = fixed_regs_[instr.src & (FIXED_REG_COUNT - 1)];
            fdst = VeldFixedAdd(fdst, VeldFixedAdd(fsrc, VeldFixedFromInteger(static_cast<int64_t>(
        ...
        case VeldOp::QMUL_R: {
            int64_t& fdst = fixed_regs_[instr.dst & (FIXED_REG_COUNT - 1)];
            const int64_t fsrc = fixed_regs_[instr.src & (FIXED_REG_COUNT - 1)];
            fdst = VeldFixedMul(fdst, VeldFixedAdd(fsrc, VELD_FIXED_ONE + 1));
            const uint64_t fbits = VeldFixedBits(fdst);
        ...
        case VeldOp::QSUB_R: {
            int64_t& fdst = fixed_regs_[instr.dst & (FIXED_REG_COUNT - 1)];
            const int64_t fsrc = fixed_regs_[instr.src & (FIXED_REG_COUNT - 1)];
            const int64_t integer_term = VeldFixedFromInteger(VeldFixedSignedHigh32(src << 1));
            fdst = VeldFixedSub(fdst, VeldFixedSub(fsrc, integer_term));
            const uint64_t fbits = VeldFixedBits(fdst);
            dst += fbits;
            break;
        }
        case VeldOp::QSQRT_R: {
            int64_t& fdst = fixed_regs_[instr.dst & (FIXED_REG_COUNT - 1)];
            const int64_t integer_term =
                VeldFixedFromInteger(static_cast<int64_t>(((src >> 10) & 0xFFFFFFULL) + 1));
            fdst = VeldFixedSqrt(VeldFixedAdd(VeldFixedAbs(fdst), integer_term));
            const uint64_t fbits = VeldFixedBits(fdst);
...
inline Hash256 ComputeEpochSeed(const Hash256& prev_hash, uint64_t height,
    ...
    uint8_t epoch_le[8];
    for (int i = 0; i < 8; ++i)
        epoch_le[i] = (uint8_t)((epoch >> (i * 8)) & 0xFF);
    h.update(epoch_le, 8);
    uint8_t bits_le[4];
    for (int i = 0; i < 4; ++i)
        bits_le[i] = static_cast<uint8_t>(expected_target.bits >> (i * 8));
    h.update(bits_le, 4);
    return h.digest();
}

inline Hash256 ComputeEpochSeed(const Hash256& prev_hash, uint64_t height, uint32_t bits) {
...
inline Hash256 VeldHashWithDataset(const std::vector<uint8_t>& header_bytes, uint64_t block_height,
                                   const CanonicalPowTarget& expected_target,
    ...
    Hash256 ds_seed = ComputeEpochSeed(prev_hash, block_height, expected_target);
    DatasetHandle ds;
    if constexpr (LightDataset) {
...
inline Hash256 VeldHash(const std::vector<uint8_t>& header_bytes, uint64_t block_height,
                        const CanonicalPowTarget& expected_target) {
    return VeldHashWithDataset<VELD_DEFAULT_LIGHT_DATASET>(header_bytes, block_height,
                                                           expected_target);
}

inline Hash256 VeldHashForVerification(const std::vector<uint8_t>& header_bytes,
...
inline Hash256 VeldHash(const std::vector<uint8_t>& header_bytes, uint64_t block_height = 0) {
#ifdef VELD_MAINNET_POW
    if (header_bytes.size() != 88)
        throw std::invalid_argument("VeldHash: header_bytes must be exactly 88 bytes");
...
inline VeldHashResult VeldHashMine(std::vector<uint8_t> header_bytes, const Hash256& target,
...
inline std::string GetAlgorithmInfo() {
    ...
    oss << "Dataset:        " << (DATASET_SIZE / 1024 / 1024)
        << " MB (epoch seed from prev_hash + height/" << DATASET_EPOCH_BLOCKS << ")\n";
