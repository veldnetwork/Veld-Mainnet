...
inline bool ValidAddressOnlyDestination(const std::string& address, bool testnet) {
    if (address.empty() || address.size() > 96)
        return false;
    ...
    const bool version =
        payload.size() == 21
            ...
            : payload.size() == 49 &&
                  payload[0] == (testnet ? SHA384_DESTINATION_TESTNET : SHA384_DESTINATION_MAINNET);
    if (!version)
        return false;
...
inline bool AddressOnlyRpcSecret(const std::filesystem::path& directory, std::string& secret,
                                 std::string* error, bool create = true) {
    WipeString(secret);
#ifdef _WIN32
    using namespace channel::secure_file;
    if (!EnsurePrivateDirectory(directory.string(), error))
        return false;
    ...
    if (result != ReadResult::Ok) {
        if (!create) {
            if (error)
                *error = "address-only RPC credential is missing";
            return false;
        }
        uint8_t random[32]{};
        if (!compat::SecureRandom(random, sizeof(random))) {
            if (error)
                *error = "RPC credential random generation failed";
            return false;
        }
        DATA_BLOB clear{sizeof(random), random}, encoded{};
        const bool ok = CryptProtectData(&clear, L"Veld address-only RPC", nullptr, nullptr,
                                         nullptr, CRYPTPROTECT_UI_FORBIDDEN, &encoded) != FALSE;
        compat::SecureZero(random, sizeof(random));
        if (!ok) {
            if (error)
                *error = "Windows RPC credential protection failed";
            return false;
        }
        ...
        (void)written;
        if (Read(path, protected_bytes, error, 16384) != ReadResult::Ok)
            return false;
    }
    DATA_BLOB encoded{static_cast<DWORD>(protected_bytes.size()), protected_bytes.data()}, clear{};
    if (!CryptUnprotectData(&encoded, nullptr, nullptr, nullptr, nullptr, CRYPTPROTECT_UI_FORBIDDEN,
                            &clear)) {
        if (error)
            *error = "Windows cannot unlock the address-only RPC credential";
        return false;
    }
    if (clear.cbData == 32) {
        static constexpr char hex[] = "0123456789abcdef";
        for (DWORD i = 0; i < clear.cbData; ++i) {
            secret += hex[clear.pbData[i] >> 4];
            secret += hex[clear.pbData[i] & 15];
        }
    }
    ...
    if (secret.size() != 64) {
        if (error)
            *error = "invalid RPC credential length";
        return false;
    }
    return true;
#else
    ...
    if (!configured || std::string_view(configured).size() < 16) {
        if (error)
            *error =
                "set VELD_ADDRESS_RPC_PASSPHRASE to a private local RPC secret of at least 16 characters";
        return false;
    }
    secret = configured;
    return true;
