...
class MiningEngine {
  public:
    explicit MiningEngine(Blockchain& chain) : chain_(chain), running_(false) {}

    Block BuildCandidateBlock(const std::vector<Transaction>& mempool_txs,
                              const std::vector<uint8_t>& miner_script,
                              const std::string& miner_message = "") const {
        Block candidate;
    ...
    bool MineAndCommit(const std::vector<uint8_t>&, const std::vector<Transaction>& = {},
                       const std::string& = "", uint32_t = 0) {
        return false;
    }

    MiningResult Mine(Block&, std::atomic<bool>&, std::function<void(uint64_t)> = nullptr) {
        MiningResult result;
        result.found = false;
    ...
    static std::string FormatHashRate(double hashes_per_sec) {
        if (hashes_per_sec >= 1e12)
            return std::to_string(hashes_per_sec / 1e12) + " TH/s";
        if (hashes_per_sec >= 1e9)
            return std::to_string(hashes_per_sec / 1e9) + " GH/s";
        if (hashes_per_sec >= 1e6)
            return std::to_string(hashes_per_sec / 1e6) + " MH/s";
        if (hashes_per_sec >= 1e3)
            return std::to_string(hashes_per_sec / 1e3) + " KH/s";
