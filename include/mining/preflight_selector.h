    ...
    Preflight&& preflight)
{
    ...
    auto rebuild = [&](const std::vector<bool>& retained,
        ...
        rebuilt.transactions.reserve(
            1 + retained_count + mandatory_txs.size());
    ...
    if (!rebuild(result.retained_mempool, &result.candidate,
                 &result.retained_fees)) {
    ...
    for (const size_t index : stateful_indices) {
        ...
        ++result.preflight_calls;
        if (!preflight(trial_candidate)) continue;
