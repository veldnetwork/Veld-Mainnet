...
inline bool DecodeBase64Canonical(std::string_view input,
                                  std::vector<uint8_t>& output,
                                  size_t max_decoded_bytes) {
    output.clear();
    if (input.empty()) return true;
    if ((input.size() & 3u) != 0) return false;
    if (max_decoded_bytes > std::numeric_limits<size_t>::max() - 2) return false;
    const size_t max_quartets = (max_decoded_bytes + 2) / 3;
    if (max_quartets > std::numeric_limits<size_t>::max() / 4) return false;
    if (input.size() > max_quartets * 4) return false;
    ...
    for (size_t i = 0; i < input.size(); i += 4) {
        ...
        if (a < 0 || b < 0) { output.clear(); return false; }
        ...
        if (c < 0) { output.clear(); return false; }
        output.push_back(static_cast<uint8_t>((a << 2) | (b >> 4)));
        if (dch == '=') {
            if (!final || (c & 0x03) != 0) { output.clear(); return false; }
            output.push_back(static_cast<uint8_t>((b << 4) | (c >> 2)));
            continue;
        }

        const int d = value(static_cast<unsigned char>(dch));
        if (d < 0) { output.clear(); return false; }
        output.push_back(static_cast<uint8_t>((b << 4) | (c >> 2)));
