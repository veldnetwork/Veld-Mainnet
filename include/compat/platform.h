...
#  include <filesystem>
inline std::string VeldTmpDir(){ return std::filesystem::temp_directory_path().string() + "/"; }

namespace veld { namespace compat {
...
inline void UnsetEnv(const char* name) { _putenv_s(name, ""); }
...
inline std::string VeldTmpDir(){ return "/tmp/"; }
...
inline void UnsetEnv(const char* name) { ::unsetenv(name); }
