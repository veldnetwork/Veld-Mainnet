...
inline void AppendBoundedProcessOutput(ProcessResult& result, const char* data, size_t size,
                                       size_t maximum) {
    ...
    if (take != 0)
        result.output.append(data, take);
    if (take != size)
...
inline std::string TrustedSystemCurlExecutable() {
    ...
    const UINT windows_length =
        ::GetWindowsDirectoryA(windows_buffer.data(), static_cast<UINT>(windows_buffer.size()));
    const UINT system_length =
        ::GetSystemDirectoryA(system_buffer.data(), static_cast<UINT>(system_buffer.size()));
    if (windows_length == 0 || windows_length >= windows_buffer.size() || system_length == 0 ||
        system_length >= system_buffer.size()) {
        return {};
    }
    ...
    auto canonicalize = [](const std::string& path) -> std::string {
        if (path.empty())
            return {};
        std::vector<char> buffer(32768);
        const DWORD length = ::GetFullPathNameA(path.c_str(), static_cast<DWORD>(buffer.size()),
                                                buffer.data(), nullptr);
    ...
    const std::string windows_directory =
        canonicalize(std::string(windows_buffer.data(), static_cast<size_t>(windows_length)));
    const std::string system_directory =
        canonicalize(std::string(system_buffer.data(), static_cast<size_t>(system_length)));
    if (!is_absolute_drive_path(windows_directory) || !is_absolute_drive_path(system_directory)) {
        return {};
    }
    const size_t system_separator = system_directory.find_last_of("\\/");
    if (system_separator == std::string::npos ||
        !same_path(system_directory.substr(0, system_separator), windows_directory) ||
        !safe_component(windows_directory, true) || !safe_component(system_directory, true)) {
        return {};
    }

    const std::string candidate = canonicalize(system_directory + "\\curl.exe");
    if (!is_absolute_drive_path(candidate) || candidate.find_last_of("\\/") == std::string::npos ||
        !same_path(candidate.substr(0, candidate.find_last_of("\\/")), system_directory) ||
        !safe_component(candidate, false)) {
        return {};
    }

    HANDLE file =
        ::CreateFileA(candidate.c_str(), FILE_READ_ATTRIBUTES,
    ...
    const DWORD final_length =
        regular_nonempty ? ::GetFinalPathNameByHandleA(file, final_buffer.data(),
                                                       static_cast<DWORD>(final_buffer.size()),
    ...
    std::string final_path(final_buffer.data(), static_cast<size_t>(final_length));
    ...
    if (!same_path(final_path, candidate))
        return {};
    return candidate;
#else
    static constexpr const char* candidates[] = {"/usr/bin/curl", "/bin/curl",
                                                 "/usr/local/bin/curl"};
    static constexpr const char* canonical_allowlist[] = {"/usr/bin/curl", "/bin/curl",
                                                          "/usr/local/bin/curl"};
    ...
    for (const char* candidate : candidates) {
        ...
        while (component_start < canonical.size()) {
            ...
            if (separator == std::string::npos)
                break;
            if (!secure_root_owned_directory(canonical.substr(0, separator))) {
        ...
        struct stat status{};
        if (::lstat(canonical.c_str(), &status) != 0 || !S_ISREG(status.st_mode) ||
            status.st_uid != 0 || status.st_size <= 0 ||
...
inline std::string TrustedSystemTarExecutable() {
    ...
    if (windows_directory.empty() || system_directory.empty() || separator == std::string::npos ||
    ...
    if (candidate.empty() || !safe_component(candidate, false))
        return {};
    HANDLE file =
        ::CreateFileA(candidate.c_str(), FILE_READ_ATTRIBUTES,
                      FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, nullptr,
    ...
    for (const char* candidate : candidates) {
        ...
        const std::string canonical(resolved);
        std::free(resolved);
        if (canonical != "/usr/bin/tar" && canonical != "/bin/tar")
            continue;
        ...
        struct stat status{};
        if (parents_ok && ::lstat(canonical.c_str(), &status) == 0 && S_ISREG(status.st_mode) &&
            status.st_uid == 0 && status.st_size > 0 &&
            (status.st_mode & (S_IWGRP | S_IWOTH)) == 0 &&
            (status.st_mode & (S_IXUSR | S_IXGRP | S_IXOTH)) != 0 &&
            ::access(canonical.c_str(), X_OK) == 0)
...
inline ProcessResult RunProcess(const std::vector<std::string>& argv, bool capture_stdout = false,
    ...
    BOOL ok = CreateProcessA(nullptr, command.data(), nullptr, nullptr,
                             capture_stdout ? TRUE : FALSE, CREATE_NO_WINDOW, nullptr,
    ...
    if (capture_stdout) {
        ...
        while (ReadFile(read_h, buf, sizeof(buf), &n, nullptr) && n)
            AppendBoundedProcessOutput(result, buf, static_cast<size_t>(n), max_output_bytes);
        CloseHandle(read_h);
    }
...
inline BoundedFileResult RunProcessToBoundedFile(const std::vector<std::string>& argv,
                                                 const std::string& destination,
                                                 uint64_t maximum_bytes,
                                                 std::chrono::milliseconds deadline,
                                                 const std::string& working_dir = {}) {
    BoundedFileResult result;
    if (argv.empty() || destination.empty() || maximum_bytes == 0 || deadline.count() <= 0) {
        return result;
    }

    HANDLE file_h = ::CreateFileA(destination.c_str(), GENERIC_WRITE, 0, nullptr, CREATE_NEW,
    ...
    PROCESS_INFORMATION pi{};
    const BOOL spawned =
        ::CreateProcessA(nullptr, command.data(), nullptr, nullptr, TRUE, CREATE_NO_WINDOW, nullptr,
                         working_dir.empty() ? nullptr : working_dir.c_str(), &si, &pi);
    ::CloseHandle(write_h);
    write_h = nullptr;
    if (!spawned) {
    ...
    while (!(child_done && pipe_done)) {
        ...
        while (available != 0) {
            ...
            while (offset != received) {
                DWORD written = 0;
                if (!::WriteFile(file_h, buffer + offset, received - offset, &written, nullptr) ||
                    written == 0) {
                    result.status = BoundedFileStatus::WriteFailed;
                    break;
                }
...
inline bool RunDetached(const std::vector<std::string>& argv) {
    ...
    PROCESS_INFORMATION pi{};
    BOOL ok =
        CreateProcessA(nullptr, command.data(), nullptr, nullptr, FALSE,
...
inline BoundedFileResult RunProcessToBoundedFile(const std::vector<std::string>& argv,
    ...
    int file_fd = ::open(destination.c_str(),
    ...
    if (current_flags < 0 || ::fcntl(pipe_fds[0], F_SETFL, current_flags | O_NONBLOCK) != 0) {
        ::close(pipe_fds[0]);
        ::close(pipe_fds[1]);
        result.status = BoundedFileStatus::SpawnFailed;
        close_and_cleanup();
        ...
        if (polled > 0 && (pfd.revents & (POLLIN | POLLHUP))) {
            for (;;) {
                ...
                if (received > 0) {
                    ...
                    ssize_t offset = 0;
                    while (offset != received) {
                        const ssize_t written = ::write(file_fd, buffer + offset,
