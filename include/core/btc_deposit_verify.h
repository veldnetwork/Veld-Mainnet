...
inline bool ParseBtcTxOutputs(const uint8_t* p, size_t len, std::vector<BtcTxOut>& outs) {
    ...
    size_t off = 0;
    if (len < 4 + 1 + 1 + 4)
        return false; // version + min vin/vout + locktime
    off += 4;         // version
    uint64_t vin = 0;
    if (!ReadVarint(p, len, off, vin))
        return false;
    if (vin == 0 || vin > 100000)
        return false; // 0 == segwit-marker / non-standard
    for (uint64_t i = 0; i < vin; ++i) {
        if (off + 36 > len)
            return false;
        off += 36; // prevout
        uint64_t sl = 0;
        if (!ReadVarint(p, len, off, sl))
            return false;
        if (sl > len || off + sl + 4 > len)
            return false;
        off += sl + 4; // scriptSig + sequence
    }
    uint64_t vout = 0;
    if (!ReadVarint(p, len, off, vout))
        return false;
    ...
    if (off + 4 != len)
        return false; // locktime — and NOTHING after (rejects witness tails)
    return true;
}
...
inline bool ParseBtcTxInputPrevout(const uint8_t* p, size_t len, uint32_t vin_index,
    ...
    if (!ReadVarint(p, len, off, vin))
        return false;
    if (vin == 0 || vin > 100000 || vin_index >= vin)
        return false;
    for (uint64_t i = 0; i < vin; ++i) {
        if (off + 36 > len)
            return false;
...
inline bool ReadCanonicalVarint(const uint8_t* p, size_t len, size_t& off, uint64_t& out) {
    if (p == nullptr || off >= len)
        return false;
    const uint8_t prefix = p[off];
    if (!ReadVarint(p, len, off, out))
        return false;
    if ((prefix == 0xfd && out < 0xfd) || (prefix == 0xfe && out <= 0xffffULL) ||
        (prefix == 0xff && out <= 0xffffffffULL))
        return false;
    return true;
}

inline bool ParseWitnessAwareBtcTx(const uint8_t* p, size_t len, WitnessAwareBtcTx& parsed,
                                   size_t max_inputs = 100000, size_t max_outputs = 100000) {
    parsed = WitnessAwareBtcTx{};
    if (p == nullptr || len < 4 + 1 + 1 + 4 || max_inputs == 0 || max_outputs == 0)
        return false;
    ...
    uint64_t vin = 0;
    if (!ReadCanonicalVarint(p, len, off, vin) || vin == 0 || vin > max_inputs)
        return false;
    ...
    for (uint64_t i = 0; i < vin; ++i) {
        ...
        if (!ReadCanonicalVarint(p, len, off, script_len) || script_len > len - off ||
            len - off - script_len < 4)
            return false;
    ...
    uint64_t vout = 0;
    if (!ReadCanonicalVarint(p, len, off, vout) || vout == 0 || vout > max_outputs)
        return false;
    ...
    if (witness) {
        ...
        for (uint64_t i = 0; i < vin; ++i) {
            ...
            if (!ReadCanonicalVarint(p, len, off, item_count) || item_count > 100000)
                return false;
            if (item_count != 0)
                has_witness_stack = true;
            for (uint64_t item = 0; item < item_count; ++item) {
                uint64_t item_len = 0;
                if (!ReadCanonicalVarint(p, len, off, item_len) || item_len > len - off)
                    return false;
                off += static_cast<size_t>(item_len);
            }
        }
        if (!has_witness_stack)
            return false;
    }
...
inline bool ParseWitnessAwareBtcTx(const std::vector<uint8_t>& tx, WitnessAwareBtcTx& parsed,
                                   size_t max_inputs = 100000, size_t max_outputs = 100000) {
    return ParseWitnessAwareBtcTx(tx.data(), tx.size(), parsed, max_inputs, max_outputs);
}

inline bool ParseBtcTxPrevouts(const uint8_t* p, size_t len, std::vector<BtcPrevout>& prevouts) {
    prevouts.clear();
    size_t off = 0;
    if (p == nullptr || len < 5)
        return false;
    off += 4;
    uint64_t vin = 0;
    if (!ReadVarint(p, len, off, vin) || vin == 0 || vin > btcnull::MAX_MSPV_PARENT_COUNT)
        return false;
    ...
    for (uint64_t i = 0; i < vin; ++i) {
        ...
        prev.vout = rd_le32(p + off + 32);
        off += 36;
        if (HashIsZero(prev.txid) && prev.vout == UINT32_MAX)
            return false;
        uint64_t script_len = 0;
        if (!ReadVarint(p, len, off, script_len) || script_len > len || off + script_len + 4 > len)
            return false;
...
inline std::vector<uint8_t> ExtractOpReturn(const std::vector<uint8_t>& spk) {
    ...
    if (spk[1] < 0x4c) {
        ...
        if (spk.size() < 4)
            return {};
        dlen = (size_t)spk[2] | ((size_t)spk[3] << 8);
        i = 4;
    } else
        return {};
    if (i + dlen > spk.size())
...
inline std::string BtcDepositOutpointId(const H256& internal_txid, uint32_t vout) {
...
inline bool ParseC1FundingProof(const uint8_t* p, size_t len, H256& block_hash, uint64_t& dirs,
    ...
    if (p == nullptr || len < 4 + 32 + 4 + 1 + 4 + 10 || p[0] != 'C' || p[1] != 'F' ||
        p[2] != 'P' || p[3] != (btcnull::CUSTODY_LINEAGE_REQUIRED ? '2' : '1'))
        return false;
    size_t off = 4;
    std::memcpy(block_hash.data(), p + off, 32);
    off += 32;
    dirs = rd_le32(p + off);
    off += 4;
    ...
    const uint32_t tx_len = rd_le32(p + off);
    off += 4;
    ...
    if (btcnull::CUSTODY_LINEAGE_REQUIRED) {
        if (off + 2 > len)
            return false;
        const uint16_t parent_count =
            static_cast<uint16_t>(p[off]) | (static_cast<uint16_t>(p[off + 1]) << 8);
        off += 2;
        if (parent_count == 0 || parent_count > btcnull::MAX_MSPV_PARENT_COUNT)
            return false;
    ...
    return proof_len >= btcnull::MIN_PROOF_BYTES && proof_len <= btcnull::MAX_PROOF_BYTES &&
...
inline bool ParseC1FundingProof(const uint8_t* p, size_t len, H256& block_hash, uint64_t& dirs,
    ...
    return ParseC1FundingProof(p, len, block_hash, dirs, branch, legacy_tx, parents,
                               nullifier_proof);
}

inline C1FundingResult VerifyC1Funding(const BtcHeaderChain& chain, const uint8_t* payload,
                                       size_t len, const std::vector<uint8_t>& exact_script,
                                       uint64_t exact_amount, const std::string& exact_outpoint,
                                       uint32_t k_btc) {
    ...
    std::vector<std::vector<uint8_t>> parent_txs;
    btcnull::Proof nullifier_proof;
    if (!ParseC1FundingProof(payload, len, block_hash, dirs, branch, tx, parent_txs,
                             nullifier_proof)) {
        result.reason = btcnull::CUSTODY_LINEAGE_REQUIRED ? "malformed CFP2" : "malformed CFP1";
        return result;
    }
    if (tx.size() <= 64) {
    ...
    if (btcnull::CUSTODY_LINEAGE_REQUIRED) {
        ...
        for (size_t i = 0; i < prevouts.size(); ++i) {
            ...
            if (!ParseBtcTxOutputs(parent_txs[i].data(), parent_txs[i].size(), parent_outputs) ||
    ...
    for (size_t i = 0; i < outputs.size(); ++i) {
        if (outputs[i].spk == exact_script &&
            (btcnull::CUSTODY_LINEAGE_REQUIRED || outputs[i].value == exact_amount)) {
            ++matches;
    ...
    if (btcnull::CUSTODY_LINEAGE_REQUIRED &&
        (matched_value <= custody_inputs || matched_value - custody_inputs != exact_amount)) {
...
inline bool ParseMintSpvOp(const uint8_t* p, size_t len, H256& block_hash, uint64_t& dirs,
                           std::vector<H256>& branch, std::vector<uint8_t>& legacy_tx,
                           std::vector<std::vector<uint8_t>>& parent_txs,
                           btcnull::Proof& nullifier_proof) {
    block_hash = H256{};
    ...
    parent_txs.clear();
    nullifier_proof = btcnull::Proof{};
    if (p == nullptr || len < 4 + 32 + 4 + 1 + 4 + btcnull::MIN_PROOF_BYTES)
        return false;
    const uint8_t expected_version =
        btcnull::CUSTODY_LINEAGE_REQUIRED ? static_cast<uint8_t>('3') : static_cast<uint8_t>('2');
    if (p[0] != 'M' || p[1] != 'S' || p[2] != 'P' || p[3] != expected_version)
        return false;
    size_t off = 4;
    std::memcpy(block_hash.data(), p + off, 32);
    off += 32;
    ...
    const uint32_t tx_len = rd_le32(p + off);
    off += 4;
    if (tx_len == 0 || tx_len > btcnull::MAX_MSPV_STRIPPED_TX_BYTES || off + (size_t)tx_len > len)
        return false;
    legacy_tx.assign(p + off, p + off + tx_len);
    off += tx_len;
    if (!btcnull::CUSTODY_LINEAGE_REQUIRED) {
    ...
    const size_t proof_len = len - off;
    if (proof_len < btcnull::MIN_PROOF_BYTES || proof_len > btcnull::MAX_PROOF_BYTES)
        return false;
...
inline bool ParseMintSpvOp(const uint8_t* p, size_t len, H256& block_hash, uint64_t& dirs,
                           ...
                           btcnull::Proof& nullifier_proof) {
    std::vector<std::vector<uint8_t>> parents;
    return ParseMintSpvOp(p, len, block_hash, dirs, branch, legacy_tx, parents, nullifier_proof);
}
...
inline CustodyLineageResult
EvaluateCustodyInputLineage(const std::vector<uint8_t>& child_tx,
    ...
    for (size_t i = 0; i < prevouts.size(); ++i) {
        ...
        if (parent_output.spk == custody_spk) {
            if (result.custody_input_amount > UINT64_MAX - parent_output.value) {
...
// crediting.
inline DepositResult
VerifyDepositMint(const BtcHeaderChain& ch, const uint8_t* payload, size_t len,
                  const std::vector<uint8_t>& custody_spk, uint32_t k_btc, uint64_t current_supply,
    ...
    std::vector<std::vector<uint8_t>> parent_txs;
    btcnull::Proof nullifier_proof;
    if (!ParseMintSpvOp(payload, len, block_hash, dirs, branch, tx, parent_txs, nullifier_proof)) {
    ...
    if (tx.size() <= 64) {
        r.reason = "deposit tx implausibly short (merkle-ambiguity guard)";
        return r;
    }

    H256 txid = ::veld::Hash256d(tx);
    if (!ch.VerifyMerkle(block_hash, txid, branch, dirs)) {
        r.reason = "merkle proof invalid";
        return r;
    }
    if (!ch.IsFinalForExternalValue(block_hash, k_btc)) {
        r.reason = "block not deep and validator-observed on the Bitcoin best chain";
        return r;
    }

    std::vector<BtcTxOut> outs;
    if (!ParseBtcTxOutputs(tx.data(), tx.size(), outs)) {
        r.reason = "deposit tx unparseable";
        return r;
    }

    uint64_t custody_inputs = 0;
    if (btcnull::CUSTODY_LINEAGE_REQUIRED) {
        const CustodyLineageResult lineage =
            EvaluateCustodyInputLineage(tx, parent_txs, custody_spk);
        if (!lineage.ok) {
            r.reason = lineage.reason;
            return r;
        }
    ...
    if (custody_n != 1) {
        r.reason = "deposit must have exactly one custody output";
        return r;
    }
    ...
    for (const auto& o : outs) {
        ...
        if (d.size() > TAG.size() && std::equal(TAG.begin(), TAG.end(), d.begin())) {
            ++rec_n;
            recipient.assign(d.begin() + TAG.size(), d.end());
        }
    }
    if (rec_n != 1) {
        r.reason = "deposit must have exactly one btcVELD OP_RETURN";
        return r;
    }
    if (!valid_recipient(recipient)) {
        r.reason = "invalid recipient address";
        return r;
    }

    if (amount == 0 || (btcnull::CUSTODY_LINEAGE_REQUIRED && amount <= custody_inputs)) {
    ...
    if (current_supply > max_custody || net_new_amount > max_custody - current_supply) {
        r.reason = "over custody cap";
        return r;
    }
...
inline DepositResult
VerifyDepositMint(const BtcHeaderChain& ch, const uint8_t* payload, size_t len,
                  const std::vector<uint8_t>& custody_spk, uint32_t k_btc, uint64_t max_per_mint,
                  uint64_t current_supply, uint64_t max_custody,
                  const std::function<bool(const std::string&)>& valid_recipient) {
    DepositResult result = VerifyDepositMint(ch, payload, len, custody_spk, k_btc, current_supply,
                                             max_custody, valid_recipient);
