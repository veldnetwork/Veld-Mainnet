...
inline constexpr const char* BTCVELD_AMM_LAUNCH_AUTHORITY_ADDRESS = BTCVELD_ISSUER_ADDRESS;
...
struct AmmPool {
    ...
    int64_t locked_lp = 0;
    uint32_t fee_bps =
        BTCVELD_AMM_FEE_BPS; // consensus ceiling (1%); quote fee is direction/anchor aware
...
inline bool AmmSwapMinimumOutputSatisfied(bool veld_in, int64_t minimum_out, int64_t actual_out) {
    if (actual_out <= 0)
        return false;
    return veld_in ? minimum_out > 0 && actual_out >= minimum_out : minimum_out == 0;
}
...
inline bool ParseCanonicalAmmI64(const std::string& s, int64_t* out) {
    ...
    const uint64_t limit = negative ? (uint64_t)INT64_MAX + uint64_t{1} : (uint64_t)INT64_MAX;
    uint64_t value = 0;
    for (; pos < s.size(); ++pos) {
    ...
    if (negative) {
        *out = value == (uint64_t)INT64_MAX + uint64_t{1} ? INT64_MIN : -(int64_t)value;
    } else {
        *out = (int64_t)value;
    }
    return true;
...
inline bool AmmTransactionEnvelopeWithinBounds(const Transaction& tx) {
    if (tx.version != 1 || tx.locktime != 0 || tx.inputs.empty() ||
        tx.inputs.size() > MAX_AMM_FUNDING_INPUTS + 1 || tx.Serialize().size() > MAX_AMM_TX_BYTES)
        return false;
...
class AmmLedger {
  public:
    ...
    static const char* SwapRejectCodeName(SwapRejectCode code) {
        switch (code) {
        case SwapRejectCode::NONE:
            return "NONE";
        case SwapRejectCode::FEE_MODEL_NOT_ACTIVE:
            return "FEE_MODEL_NOT_ACTIVE";
        case SwapRejectCode::INVALID_ANCHOR:
            return "INVALID_ANCHOR";
        case SwapRejectCode::INVALID_STATE:
    ...
    struct SwapQuote {
        ...
        bool rebalances_anchor =
            false;           // healing: post-trade deviation <= pre-trade (four-band spec §1.2)
    ...
    static bool CheckedMulDivPositiveI64(int64_t a, int64_t b, int64_t divisor, int64_t* out) {
        if (!out || a <= 0 || b <= 0 || divisor <= 0)
            return false;
        const unsigned __int128 q =
            (unsigned __int128)(uint64_t)a * (uint64_t)b / (uint64_t)divisor;
        if (q == 0 || q > (unsigned __int128)INT64_MAX)
            return false;
        *out = (int64_t)q;
        return true;
    }
    ...
    static unsigned __int128 AnchorCrossDistance(int64_t veld, int64_t btcveld, int64_t anchor_veld,
                                                 int64_t anchor_btcveld) {
        unsigned __int128 lhs = (unsigned __int128)(uint64_t)veld * (uint64_t)anchor_btcveld;
        unsigned __int128 rhs = (unsigned __int128)(uint64_t)btcveld * (uint64_t)anchor_veld;
        return lhs >= rhs ? lhs - rhs : rhs - lhs;
    }

    // floor(10000*num/den), saturated at 10000, without overflowing u128.
    static uint32_t RatioBpsCapped(unsigned __int128 num, unsigned __int128 den) {
        if (den == 0 || num >= den)
            return FEE_DENOM;
    ...
    static uint32_t BandFeeBps(unsigned __int128 delta, unsigned __int128 den,
        ...
        auto pick = [&](uint8_t b, uint32_t fee) {
            if (band_index)
                *band_index = b;
            return fee;
        };
        if (den == 0)
            return pick(4, BTCVELD_AMM_BAND_FEE_BPS[3]);
        ...
        if (within(0))
            return pick(1, BTCVELD_AMM_BAND_FEE_BPS[0]); // D <= 5%
        if (within(1))
            return pick(2, BTCVELD_AMM_BAND_FEE_BPS[1]); // 5% < D <= 10%
        if (within(2))
            return pick(3, BTCVELD_AMM_BAND_FEE_BPS[2]); // 10% < D <= 20%
    ...
    static int64_t GrossSwapOut(int64_t rin, int64_t rout, int64_t amt_in) {
        if (rin <= 0 || rout <= 0 || amt_in <= 0)
            return -1;
        int64_t gross = (int64_t)((unsigned __int128)(uint64_t)rout * (uint64_t)amt_in /
                                  ((unsigned __int128)(uint64_t)rin + (uint64_t)amt_in));
        if (gross <= 0 || gross >= rout)
            return -1;
        return gross;
    }
    ...
    static int64_t SwapOut(int64_t rin, int64_t rout, int64_t amt_in, uint32_t fee_bps) {
        ...
        if (gross < 0)
            return -1;
        int64_t out =
            (int64_t)((unsigned __int128)(uint64_t)gross * (FEE_DENOM - fee_bps) / FEE_DENOM);
        if (out <= 0 || out >= rout)
            return -1;
        return out;
    }
    ...
    bool ApplyDStateQualificationLp(const std::string& pool_id, const std::string& address) {
        ...
        if (!pool.exists || pool.lp_supply == 0) {
            if (!pool.exists)
                CreatePool(pool_id, BTCVELD_AMM_FEE_BPS);
            const auto seed = AddLiquidity(pool_id, address, 40 * AMM_SEED_LOCK_VELD_UNITS,
                                           40 * AMM_SEED_LOCK_BTCVELD_SATS);
            if (seed.lp_minted <= LP_MIN_POSITION)
                return false;
            const auto trim = RemoveLiquidity(pool_id, address, seed.lp_minted);
            if (trim.veld_out < 0 || trim.btcveld_out < 0 || GetLp(pool_id, address) != 0)
                return false;
            pool = GetPool(pool_id);
        }
        if (GetLp(pool_id, address) != 0 || pool.reserve_veld <= 0 || pool.reserve_btcveld <= 0 ||
            pool.lp_supply <= 0)
            return false;
        const unsigned __int128 numerator = static_cast<unsigned __int128>(LP_MIN_POSITION) *
                                            static_cast<uint64_t>(pool.reserve_veld);
        const uint64_t denominator = static_cast<uint64_t>(pool.lp_supply);
        const uint64_t base = static_cast<uint64_t>((numerator + denominator - 1) / denominator);
        const uint64_t btc_min =
            static_cast<uint64_t>((static_cast<unsigned __int128>(LP_MIN_POSITION) *
                                       static_cast<uint64_t>(pool.reserve_btcveld) +
                                   denominator - 1) /
                                  denominator);
        const uint64_t veld_for_btc = static_cast<uint64_t>(
            (static_cast<unsigned __int128>(btc_min) * static_cast<uint64_t>(pool.reserve_veld) +
        ...
        if (veld <= static_cast<uint64_t>(INT64_MAX)) {
            const uint64_t btc = static_cast<uint64_t>(static_cast<unsigned __int128>(veld) *
                                                       static_cast<uint64_t>(pool.reserve_btcveld) /
                                                       static_cast<uint64_t>(pool.reserve_veld));
            if (btc == 0 || btc > static_cast<uint64_t>(INT64_MAX))
                return false;
            const auto added = AddLiquidity(pool_id, address, static_cast<int64_t>(veld),
    ...
    static int64_t LockedSeedUnits(int64_t g, int64_t V, int64_t B) {
        const int64_t by_veld =
            CeilDivU128((unsigned __int128)(uint64_t)g * (uint64_t)AMM_SEED_LOCK_VELD_UNITS,
                        (unsigned __int128)(uint64_t)V);
        const int64_t by_btc =
            CeilDivU128((unsigned __int128)(uint64_t)g * (uint64_t)AMM_SEED_LOCK_BTCVELD_SATS,
        ...
        if (by_btc > locked)
            locked = by_btc;
        if (locked > g)
            locked = g; // defensive; unreachable given admission
        return locked;
    }
    ...
    static bool IsCanonicalOpeningSeedRatio(int64_t veld_units, int64_t btcveld_sats) {
        if (veld_units <= 0 || btcveld_sats <= 0 ||
            btcveld_sats > INT64_MAX / BTCVELD_AMM_OPENING_VELD_UNITS_PER_BTCVELD_SAT)
            return false;
        return veld_units == btcveld_sats * BTCVELD_AMM_OPENING_VELD_UNITS_PER_BTCVELD_SAT;
    }
    ...
    static bool InitialSeedAuthorized(const std::string& user, uint64_t height) noexcept {
        return !MarketSeedAnchorActive(height) || user == BTCVELD_AMM_LAUNCH_AUTHORITY_ADDRESS;
    }

  private:
    static SeedQuote QuoteInitialSeedPolicy_(int64_t veld_units, int64_t btcveld_sats,
                                             bool market_anchor) {
        SeedQuote quote;
        if ((!market_anchor && !IsCanonicalOpeningSeedRatio(veld_units, btcveld_sats)) ||
            veld_units < AMM_SEED_LOCK_VELD_UNITS || btcveld_sats < AMM_SEED_LOCK_BTCVELD_SATS ||
            btcveld_sats > BTCVELD_AMM_MAX_POOL_BTCVELD_SATS ||
            static_cast<uint64_t>(veld_units) > MAX_SUPPLY_UNITS)
            return quote;
        quote.gross_lp =
            IsqrtU128((unsigned __int128)(uint64_t)veld_units * (uint64_t)btcveld_sats);
        if (quote.gross_lp <= MIN_LIQUIDITY)
            return SeedQuote{};
        quote.locked_lp = LockedSeedUnits(quote.gross_lp, veld_units, btcveld_sats);
  ...
  public:
    static SeedQuote QuoteInitialSeed(int64_t veld_units, int64_t btcveld_sats) {
    ...
    static constexpr const char* SEED_LIVENESS_POLICY_ID = "SEED_LIVENESS_PROBE_V1";
    ...
    static const char* SeedLivenessDirectionName(SeedLivenessDirection direction) {
        switch (direction) {
        case SeedLivenessDirection::B2V:
            return "B2V";
        case SeedLivenessDirection::V2B:
            return "V2B";
        case SeedLivenessDirection::NONE:
            return "NONE";
        }
        return "NONE";
    }
  ...
  private:
    ...
    static SeedLivenessProbe
    ProbeInitialSeedLivenessWithResources_(int64_t veld_units, int64_t btcveld_sats,
                                           uint64_t height, uint64_t continuation_units,
                                           int64_t remaining_btcveld_sats) {
        SeedLivenessProbe result;
        if (!QuoteInitialSeedPolicy_(veld_units, btcveld_sats, MarketSeedAnchorActive(height))
                 .valid ||
            veld_units <= 0 || (uint64_t)veld_units > MAX_SUPPLY_UNITS || btcveld_sats <= 0 ||
            btcveld_sats > BTCVELD_AMM_MAX_POOL_BTCVELD_SATS || !FourBandActive(height) ||
            !ammgate::SwapAllowed(height))
            return result;
        ...
        const unsigned __int128 initial_k =
            (unsigned __int128)(uint64_t)veld_units * (uint64_t)btcveld_sats;
        auto potentially_executable = [&](const SwapQuote& quote, SeedLivenessDirection direction) {
            if (quote.reject || quote.amount_out <= 0 || quote.post_reserve_veld <= 0 ||
                (uint64_t)quote.post_reserve_veld < DUST_THRESHOLD_UNITS ||
                (uint64_t)quote.post_reserve_veld > MAX_SUPPLY_UNITS ||
                quote.post_reserve_btcveld < OnChainTokenLedger::MIN_ACCOUNT_SATS ||
                quote.post_reserve_btcveld > BTCVELD_AMM_MAX_POOL_BTCVELD_SATS)
                return false;
            if ((unsigned __int128)(uint64_t)quote.post_reserve_veld *
                    (uint64_t)quote.post_reserve_btcveld <
                initial_k)
                return false;
        ...
        if (btcveld_sats < BTCVELD_AMM_MAX_POOL_BTCVELD_SATS &&
            ...
            remaining_btcveld_sats >= OnChainTokenLedger::MIN_ACCOUNT_SATS + 1) {
            const SwapQuote quote = QuoteSwapLocked(hypothetical, false, 1, height);
            if (potentially_executable(quote, SeedLivenessDirection::B2V)) {
                result.valid = true;
        ...
        if (remaining_btcveld_sats < OnChainTokenLedger::MIN_ACCOUNT_SATS ||
            continuation_units <= AMM_SEED_LIVENESS_TX_FEE_UNITS)
            return result;
        const uint64_t max_input = continuation_units - AMM_SEED_LIVENESS_TX_FEE_UNITS;
        if (SwapOut(veld_units, btcveld_sats, (int64_t)max_input, BTCVELD_AMM_FEE_MIN_BPS) <= 0)
            return result;
        uint64_t lo = 1;
        uint64_t hi = max_input;
        while (lo < hi) {
            const uint64_t mid = lo + (hi - lo) / 2;
            if (SwapOut(veld_units, btcveld_sats, (int64_t)mid, BTCVELD_AMM_FEE_MIN_BPS) > 0)
                hi = mid;
            else
                lo = mid + 1;
        }
        const SwapQuote quote = QuoteSwapLocked(hypothetical, true, (int64_t)lo, height);
  ...
  public:
    ...
    static SeedLivenessProbe ProbeInitialSeedLiveness(int64_t veld_units, int64_t btcveld_sats,
                                                      uint64_t height) {
        if (veld_units <= 0 || (uint64_t)veld_units > MAX_SUPPLY_UNITS)
            return SeedLivenessProbe{};
        const uint64_t total_headroom = MAX_SUPPLY_UNITS - (uint64_t)veld_units;
        if (total_headroom <= AMM_SEED_LIVENESS_TX_FEE_UNITS)
            return SeedLivenessProbe{};
        return ProbeInitialSeedLivenessWithResources_(
            veld_units, btcveld_sats, height, total_headroom - AMM_SEED_LIVENESS_TX_FEE_UNITS,
    ...
    static SeedLivenessProbe ProbeInitialSeedLivenessWithResources(int64_t veld_units,
                                                                   int64_t btcveld_sats,
                                                                   uint64_t height,
                                                                   uint64_t continuation_units,
                                                                   int64_t remaining_btcveld_sats) {
        return ProbeInitialSeedLivenessWithResources_(veld_units, btcveld_sats, height,
                                                      continuation_units, remaining_btcveld_sats);
    }

    static SeedQuote QuoteInitialSeedAtHeight(int64_t veld_units, int64_t btcveld_sats,
                                              uint64_t height,
                                              SeedLivenessProbe* probe_out = nullptr) {
        const SeedQuote quote =
            QuoteInitialSeedPolicy_(veld_units, btcveld_sats, MarketSeedAnchorActive(height));
        if (!quote.valid) {
            if (probe_out)
                *probe_out = SeedLivenessProbe{};
            return SeedQuote{};
        }
        const SeedLivenessProbe probe = ProbeInitialSeedLiveness(veld_units, btcveld_sats, height);
        if (probe_out)
            *probe_out = probe;
    ...
    AddResult AddLiquidity(const std::string& pool_id, const std::string& addr, int64_t d_veld,
                           int64_t d_btcveld_max) {
        std::lock_guard<std::mutex> lk(mu_);
        AmmPool* p = get(pool_id);
        if (!p || d_veld <= 0 || d_btcveld_max <= 0 || p->reserve_veld < 0 ||
        ...
        if (seed) {
            ...
            const uint64_t seed_height = BTCVELD_AMM_FOURBAND_ACTIVATION_HEIGHT;
            if (!InitialSeedAuthorized(addr, seed_height))
                return {-1, 0};
            const SeedQuote quote = QuoteInitialSeedAtHeight(d_veld, use_btc, seed_height);
            if (!quote.valid)
                return {-1, 0};
            ...
            if (p->reserve_veld <= 0 || p->reserve_btcveld <= 0 || p->lp_supply <= 0)
                return {-1, 0};
            if (!CheckedMulDivPositiveI64(d_veld, p->reserve_btcveld, p->reserve_veld, &use_btc) ||
                use_btc > d_btcveld_max)
                return {-1, 0};
            ...
            if (!CheckedMulDivPositiveI64(p->lp_supply, d_veld, p->reserve_veld,
                                          &minted_from_veld) ||
                !CheckedMulDivPositiveI64(p->lp_supply, use_btc, p->reserve_btcveld,
                                          &minted_from_btc))
                return {-1, 0};
        ...
        const unsigned __int128 lp_delta_u =
            (unsigned __int128)(uint64_t)minted + (seed ? (uint64_t)p->locked_lp : 0u);
        if (lp_delta_u > (unsigned __int128)INT64_MAX)
            return {-1, 0};
        ...
        if (d_veld > INT64_MAX - p->reserve_veld || use_btc > INT64_MAX - p->reserve_btcveld ||
            lp_delta > INT64_MAX - p->lp_supply || owner_lp < 0 || minted > INT64_MAX - owner_lp)
            return {-1, 0};
        p->reserve_veld += d_veld;
    ...
    RemoveResult RemoveLiquidity(const std::string& pool_id, const std::string& addr,
                                 int64_t d_lp) {
        std::lock_guard<std::mutex> lk(mu_);
        AmmPool* p = get(pool_id);
        if (!p || d_lp <= 0 || p->lp_supply <= 0 || d_lp > p->lp_supply || p->reserve_veld <= 0 ||
            p->reserve_btcveld <= 0)
            return {-1, 0};
        auto key = pool_id + ":" + addr;
        auto it = lp_.find(key);
        int64_t bal = (it != lp_.end()) ? it->second : 0;
        if (bal < d_lp)
            return {-1, 0};
        const int64_t residual = bal - d_lp;
        if (residual != 0 && residual < LP_MIN_POSITION)
            return {-1, 0};
        int64_t v = (int64_t)((unsigned __int128)(uint64_t)p->reserve_veld * (uint64_t)d_lp /
                              (uint64_t)p->lp_supply);
        int64_t b = (int64_t)((unsigned __int128)(uint64_t)p->reserve_btcveld * (uint64_t)d_lp /
                              (uint64_t)p->lp_supply);
        p->reserve_veld -= v;
        p->reserve_btcveld -= b;
        p->lp_supply -= d_lp;
    ...
    int64_t SwapAtHeight(const std::string& pool_id, bool veld_in, int64_t amt_in,
        ...
        if (quote.reject || out < 0)
            return -1; // FEE_SCHEDULE_NO_FIXED_POINT => refuse
        int64_t nrin = rin + amt_in, nrout = rout - out;
        if ((unsigned __int128)(uint64_t)nrin * (uint64_t)nrout < kb)
            return -1;
        if (veld_in) {
            p->reserve_veld = nrin;
            p->reserve_btcveld = nrout;
        } else {
            p->reserve_btcveld = nrin;
            p->reserve_veld = nrout;
        }
        return out;
    }
    ...
    int64_t Swap(const std::string& pool_id, bool veld_in, int64_t amt_in) {
        return SwapAtHeight(pool_id, veld_in, amt_in, BTCVELD_AMM_FOURBAND_ACTIVATION_HEIGHT);
    }

    // ---- increment-2: seed the pool UTXO, then TX-DRIVEN swaps ---------------
    void SeedPool(const std::string& pool_id, uint32_t fee_bps, int64_t veld, int64_t btcveld,
                  int64_t lp_supply, int64_t locked_lp, const Hash256& utxo_txid,
                  uint32_t utxo_vout) {
        std::lock_guard<std::mutex> lk(mu_);
        if (veld <= 0 || btcveld <= 0 || lp_supply <= 0 || locked_lp < MIN_LIQUIDITY ||
            locked_lp > lp_supply)
        ...
        for (const auto& [key, balance] : lp_) {
            if (key.rfind(prefix, 0) != 0)
                continue;
            if (balance <= 0)
                throw std::invalid_argument("invalid reconstructed AMM LP position");
            attributed += (uint64_t)balance;
            if (attributed > (unsigned __int128)(uint64_t)lp_supply)
                throw std::invalid_argument("reconstructed AMM LP attribution exceeds supply");
        }
        if (attributed != (unsigned __int128)(uint64_t)lp_supply)
            throw std::invalid_argument("reconstructed AMM LP attribution does not equal supply");
        AmmPool& p = pools_[pool_id];
        p = AmmPool{};
        p.exists = true;
        p.fee_bps = fee_bps == 0 ? BTCVELD_AMM_FEE_BPS
                                 : std::clamp<uint32_t>(fee_bps, BTCVELD_AMM_FEE_MIN_BPS,
                                                        BTCVELD_AMM_FEE_BPS);
        p.reserve_veld = veld;
        p.reserve_btcveld = btcveld;
        p.lp_supply = lp_supply;
        p.locked_lp = locked_lp;
        p.anchor_veld = veld;
        p.anchor_btcveld = btcveld;
        p.pool_txid = utxo_txid;
        p.pool_vout = utxo_vout;
        p.utxo_valid = true;
        p.veld_script = PoolVeldScript(pool_id);
        p.btcveld_addr = "AMM:" + pool_id;
    }
    ...
    bool ProcessBlock(const std::string& pool_id, const Block& block, OnChainTokenLedger& tokens,
        ...
        for (size_t i = 0; i < block.transactions.size(); ++i) {
            const AmmOpParseResult parsed = ParseAmmOpDetailed(block.transactions[i]);
            if (parsed.status == AmmOpParseStatus::INVALID)
                return false;
            if (parsed.status == AmmOpParseStatus::VALID) {
                if (op_index != block.transactions.size())
                    return false;
                op_index = i;
                sole_op = parsed.op;
            }
        ...
        if (has_op && !peg_gate.AmmAllowed())
            return false;

        auto current = pools_.find(pool_id);
        const bool pool_exists = current != pools_.end() && current->second.exists;
        const bool creation_allowed = PoolCreationAllowed(block.height, peg_gate);
        ...
        if (!has_op) {
            ...
            if (pool_exists || !creation_allowed)
                return true;
        ...
        try {
            ...
            if (it == pools_.end() || !it->second.exists)
                return true;
            if (!ApplyAmmOpLocked(pool_id, it->second, sole_op, block.transactions[op_index],
                                  tokens, false, block.height)) {
                rollback();
                return false;
            }
            return true;
    ...
    bool ValidateBlock(const std::string& pool_id, const Block& block, OnChainTokenLedger& tokens,
                       const BtcVeldPegGateState& peg_gate,
                       TokenValidationFrame token_frame = TokenValidationFrame::PARENT_STATE) {
        std::lock_guard<std::mutex> lk(mu_);
        ...
        if (parsed_amm_ops != 0 && !peg_gate.AmmAllowed())
            return false;
        ...
        if (parsed_amm_ops != 0 && token_frame == TokenValidationFrame::PARENT_STATE) {
        ...
        if (it != pools_.end() && it->second.exists) {
            ...
            if (!PoolCreationAllowed(block.height, peg_gate))
                return true;
        ...
        for (size_t tx_index = 0; tx_index < block.transactions.size(); ++tx_index) {
            ...
            if (prospective.utxo_valid) { // the committed pool UTXO may ONLY be spent at input[0]
                ...
                if (pos > 0)
                    return false; // pool spent in a non-canonical position
                pool_input_pos = pos;
            }
            const bool spends_pool = pool_input_pos == 0;
            const bool creates_pool =
                !tx.outputs.empty() && tx.outputs[0].script_pubkey == prospective.veld_script;
            const auto& parsed = parsed_ops[tx_index];
            const bool has_op = parsed.status == AmmOpParseStatus::VALID;
            if (!spends_pool && !creates_pool && !has_op)
                continue;
            if (spends_pool || creates_pool) {
                if (++covenant_touches != 1)
                    return false;
            }
            if (!has_op)
                return false; // covenant touched w/o an AMM op
            if (!spends_pool && !creates_pool)
                return false; // an orphan AMM marker would fail apply
            AmmPool pcopy = prospective;
            if (!ApplyAmmOpLocked(pool_id, pcopy, parsed.op, tx, *validation_tokens,
                                  /*dry_run=*/true, block.height))
                return false;
        }
        return true;
    }
    ...
    bool IsCommittedPoolOutpoint(const std::string& pool_id, const Hash256& txid,
                                 uint32_t vout) const {
        std::lock_guard<std::mutex> lk(mu_);
        auto it = pools_.find(pool_id);
        if (it == pools_.end() || !it->second.exists || !it->second.utxo_valid)
            return false;
    ...
    AmmPool GetPool(const std::string& pool_id) const {
        std::lock_guard<std::mutex> lk(mu_);
        auto it = pools_.find(pool_id);
        return it != pools_.end() ? it->second : AmmPool{};
    }
    ...
    SwapQuote QuoteSwapAtHeight(const std::string& pool_id, bool veld_in, int64_t amt_in,
    ...
    SwapQuote QuoteSwap(const std::string& pool_id, bool veld_in, int64_t amt_in) const {
        return QuoteSwapAtHeight(pool_id, veld_in, amt_in, BTCVELD_AMM_FOURBAND_ACTIVATION_HEIGHT);
    }
    int64_t GetLp(const std::string& pool_id, const std::string& addr) const {
        std::lock_guard<std::mutex> lk(mu_);
        auto it = lp_.find(pool_id + ":" + addr);
        return it != lp_.end() ? it->second : 0;
    }
#ifdef VELD_TEST_HOOKS
    ...
    Hash256 Digest() const {
        ...
        pk.reserve(pools_.size());
        for (auto& [k, _v] : pools_)
            pk.push_back(k);
        ...
        for (auto& k : l2) {
            sd::put_len_prefixed(body, k);
            sd::put_u64_le(body, (uint64_t)lp_.at(k));
        }
  ...
  private:
    ...
    static bool PoolCreationAllowed(uint64_t height, const BtcVeldPegGateState& peg_gate) {
        if (!peg_gate.AmmAllowed())
            return false;
        if (BTCVELD_ISSUER_ADDRESS[0] == '\0')
            return false;
        if (height < BTCVELD_ACTIVATION_HEIGHT)
            return false;
        if (BTCVELD_AMM_ACTIVATION_HEIGHT != 0 && height < BTCVELD_AMM_ACTIVATION_HEIGHT)
            return false;
    ...
    void MaybeCreatePool(const std::string& pool_id, uint64_t height,
        ...
        if (!PoolCreationAllowed(height, peg_gate))
            return;
    ...
    static SwapQuote QuoteSwapLocked(const AmmPool& p, bool veld_in, int64_t amt_in,
                                     uint64_t height) {
        SwapQuote q;
        q.fee_model_active = FourBandActive(height);
        ...
        if (!p.exists || p.reserve_veld <= 0 || p.reserve_btcveld <= 0 || amt_in <= 0) {
            q.reject = true;
            q.reject_code = SwapRejectCode::INVALID_STATE;
            q.fee_bps = 0;
            return q;
        ...
        if (FlatFeeActive(height)) {
            ...
            q.post_reserve_veld = veld_in ? p.reserve_veld + amt_in : p.reserve_veld - q.amount_out;
            q.post_reserve_btcveld =
                veld_in ? p.reserve_btcveld - q.amount_out : p.reserve_btcveld + amt_in;
        ...
        const unsigned __int128 before =
            AnchorCrossDistance(p.reserve_veld, p.reserve_btcveld, anchor_veld, anchor_btc);
        ...
        struct FeeEval {
            ...
            int64_t post_veld = 0;
            int64_t post_btc = 0;
            unsigned __int128 delta_after =
                0; // |R_v'*A_b - R_b'*A_v| of the fee-retained post-state
            unsigned __int128 den_after =
                0;                   // R_b'*A_v of the fee-retained post-state (band normalizer)
        ...
        auto evaluate = [&](uint32_t charged_bps) {
            ...
            if (e.out < 0)
                return e;
            e.post_veld = veld_in ? p.reserve_veld + amt_in : p.reserve_veld - e.out;
            e.post_btc = veld_in ? p.reserve_btcveld - e.out : p.reserve_btcveld + amt_in;
            if (e.post_veld <= 0 || e.post_btc <= 0) {
                e.out = -1;
                return e;
            }
            e.delta_after = AnchorCrossDistance(e.post_veld, e.post_btc, anchor_veld, anchor_btc);
            e.den_after = (unsigned __int128)(uint64_t)e.post_btc * (uint64_t)anchor_veld;
            ...
            e.rebalances = CompareU192(MulU128U64(e.delta_after, (uint64_t)p.reserve_btcveld),
        ...
        for (uint32_t cand : BTCVELD_AMM_BAND_FEE_BPS) {
            ...
            if (e.out < 0)
                continue; // output infeasible at this fee
        ...
        if (valid != 1) {
            ...
            q.reject_code = any_fee_output_feasible ? SwapRejectCode::FEE_SCHEDULE_NO_FIXED_POINT
        ...
        q.post_deviation_bps = RatioBpsCapped(chosen.delta_after, chosen.den_after);
    ...
    bool ApplyAmmOpLocked(const std::string& pool_id, AmmPool& p, const AmmOp& op,
                          ...
                          uint64_t height) {
        if (!ValidateAmmEnvelope(p, op, tx))
            return false;
        ...
        if (!IsCanonicalTokenCreditAddress(op.user))
            return false;
        if (op.action == "SWAP_V2B" || op.action == "SWAP_B2V")
            return ApplySwap(pool_id, p, op, tx, tokens, dry_run, height);
        if (op.action == "ADD")
            return ApplyAdd(pool_id, p, op, tx, tokens, dry_run, height);
        if (op.action == "REMOVE")
            return ApplyRemove(pool_id, p, op, tx, tokens, dry_run);
        return false;
    }
    ...
    bool ValidateAmmEnvelope(const AmmPool& p, const AmmOp& op, const Transaction& tx) const {
        if (!AmmTransactionEnvelopeWithinBounds(tx) || tx.outputs.empty())
            return false;

        const bool seed = op.action == "ADD" && (!p.utxo_valid || p.lp_supply == 0);
        const size_t pool_inputs = seed ? 0u : 1u;
        if (tx.inputs.size() <= pool_inputs ||
            tx.inputs.size() - pool_inputs > MAX_AMM_FUNDING_INPUTS)
            return false;

        const std::vector<uint8_t> expected_marker = BuildOpReturnScript(EncodeAmmOp(op));
        if (tx.outputs.back().value != 0 || tx.outputs.back().script_pubkey != expected_marker)
            return false;
        if (tx.outputs[0].script_pubkey != p.veld_script)
            return false;
        for (size_t i = 1; i + 1 < tx.outputs.size(); ++i) {
            const auto& script = tx.outputs[i].script_pubkey;
            if (script.empty() || script[0] == 0x6a || IsAmmPoolScript(script))
                return false;
        }
    ...
    bool SpendsPool(const AmmPool& p, const Transaction& tx) {
        return !tx.inputs.empty() && tx.inputs[0].prev_tx_hash == p.pool_txid &&
               tx.inputs[0].prev_out_index == p.pool_vout && tx.inputs[0].script_sig.empty() &&
               !tx.outputs.empty() && tx.outputs[0].script_pubkey == p.veld_script;
    }
    void ChainPool(AmmPool& p, const Transaction& tx) {
        p.pool_txid = tx.GetTxID();
        p.pool_vout = 0;
        p.utxo_valid = true;
    }

    bool ApplySwap(const std::string& pool_id, AmmPool& p, const AmmOp& op, const Transaction& tx,
                   OnChainTokenLedger& tokens, bool dry_run, uint64_t height) {
        ...
        if (!p.utxo_valid || op.amt <= 0 || op.extra < 0 || (v2b ? op.extra <= 0 : op.extra != 0) ||
            !SpendsPool(p, tx))
            return false;
        ...
        int64_t out = quote.amount_out;
        if (quote.reject || out < 0)
            return false; // FEE_SCHEDULE_NO_FIXED_POINT => block-invalid swap
        if (!AmmSwapMinimumOutputSatisfied(v2b, op.extra, out))
            return false;
        int64_t new_rveld = quote.post_reserve_veld;
        int64_t new_rbtc = quote.post_reserve_btcveld;
        if (new_rveld <= 0 || new_rbtc <= 0)
            return false;
        ...
        if ((uint64_t)new_rveld < DUST_THRESHOLD_UNITS ||
            (!v2b && (uint64_t)out < DUST_THRESHOLD_UNITS))
            return false;
        ...
        if (!ammgate::PoolReserveAllowed(height, p.reserve_btcveld, new_rbtc))
            return false;
        if ((unsigned __int128)(uint64_t)new_rveld * (uint64_t)new_rbtc < kb)
            return false; // k must not drop
        if (tx.outputs[0].value != (uint64_t)new_rveld)
            return false;
        if (v2b) {
            ...
            if (!AmmTxSignedBy(tx, op.user))
                return false; // user authorizes btcVELD spend
            ...
            if (user_script.empty() || tx.outputs.size() < 2 ||
                tx.outputs[1].value != (uint64_t)out || tx.outputs[1].script_pubkey != user_script)
                return false;
        ...
        if (dry_run)
            return true;
        if (v2b) {
            if (!tokens.AmmMove("btcVELD", p.btcveld_addr, op.user, out))
                return false;
        } else {
            if (!tokens.AmmMove("btcVELD", op.user, p.btcveld_addr, op.amt))
                return false;
        }
        p.reserve_veld = new_rveld;
        p.reserve_btcveld = new_rbtc;
        ChainPool(p, tx);
        return true;
    }
    ...
    bool ApplyAdd(const std::string& pool_id, AmmPool& p, const AmmOp& op, const Transaction& tx,
                  OnChainTokenLedger& tokens, bool dry_run, uint64_t height) {
        if (op.amt <= 0 || op.extra <= 0)
            return false;
        if (!AmmTxSignedBy(tx, op.user))
            return false; // user authorizes btcVELD deposit
        if (tx.outputs.empty() || tx.outputs[0].script_pubkey != p.veld_script)
            return false;
        const bool seed = (!p.utxo_valid || p.lp_supply == 0);
        int64_t use_btc = 0, minted = 0;
        if (seed) {
            if ((uint64_t)op.amt < DUST_THRESHOLD_UNITS)
                return false;
            if (!InitialSeedAuthorized(op.user, height))
                return false;
            use_btc = op.extra;
            const SeedQuote quote = QuoteInitialSeedAtHeight(op.amt, use_btc, height);
            if (!quote.valid)
                return false;
            const auto user_script = AddressToScript(op.user);
            if (user_script.empty() || tx.outputs.size() != 3 ||
                ...
                return false;
            const int64_t token_balance = tokens.GetBalance(BTCVELD_TOKEN_ID, op.user);
            if (token_balance < use_btc)
                return false;
            const SeedLivenessProbe seed_probe = ProbeInitialSeedLivenessWithResources(
                op.amt, use_btc, height, tx.outputs[1].value, token_balance - use_btc);
            if (!seed_probe.valid)
                return false;
            minted = quote.lp_minted;
            p.locked_lp = quote.locked_lp;
            if (tx.outputs[0].value != (uint64_t)op.amt)
                return false; // seed pool UTXO == d_veld
        } else {
            if (p.reserve_veld <= 0 || p.reserve_btcveld <= 0 || p.lp_supply <= 0)
                return false;
            if (!SpendsPool(p, tx))
                return false;
            if (!CheckedMulDivPositiveI64(op.amt, p.reserve_btcveld, p.reserve_veld, &use_btc) ||
                use_btc > op.extra)
                return false;
            ...
            int64_t minted_from_veld = 0;
            int64_t minted_from_btc = 0;
            if (!CheckedMulDivPositiveI64(p.lp_supply, op.amt, p.reserve_veld, &minted_from_veld) ||
                !CheckedMulDivPositiveI64(p.lp_supply, use_btc, p.reserve_btcveld,
                                          &minted_from_btc))
                return false;
            minted = std::min(minted_from_veld, minted_from_btc);
            if (op.amt > INT64_MAX - p.reserve_veld)
                return false;
            if (tx.outputs[0].value != (uint64_t)(p.reserve_veld + op.amt))
                return false;
        }
        // total supply grows by minted + the permanent lock (== g for a
        // seed).  Was MIN_LIQUIDITY; the lock generalises that constant.
        ...
        const int64_t owner_lp = lp_.count(lp_key) ? lp_.at(lp_key) : 0;
        if (owner_lp < 0 || minted > INT64_MAX - owner_lp)
            return false;
        if (owner_lp == 0 && minted > 0 && minted < LP_MIN_POSITION)
            return false;
        ...
        if (!ammgate::PoolReserveAllowed(height, seed ? 0 : p.reserve_btcveld,
                                         (seed ? 0 : p.reserve_btcveld) + use_btc))
            return false;
        if (!tokens.CanAmmMove("btcVELD", op.user, p.btcveld_addr, use_btc))
            return false;
        if (dry_run)
            return true;
        if (!tokens.AmmMove("btcVELD", op.user, p.btcveld_addr, use_btc))
            return false;
        if (seed) {
    ...
    bool ApplyRemove(const std::string& pool_id, AmmPool& p, const AmmOp& op, const Transaction& tx,
                     OnChainTokenLedger& tokens, bool dry_run) {
        // REMOVE has no secondary amount.  Keep the unused field canonical so
        // one LP burn has exactly one protocol encoding.
        if (op.amt <= 0 || op.extra != 0 || p.lp_supply <= 0 || op.amt > p.lp_supply ||
            p.reserve_veld <= 0 || p.reserve_btcveld <= 0 || !p.utxo_valid || !SpendsPool(p, tx))
            return false;
        if (!AmmTxSignedBy(tx, op.user))
            return false;
        ...
        if (residual != 0 && residual < LP_MIN_POSITION)
            return false;
        int64_t v = (int64_t)((unsigned __int128)(uint64_t)p.reserve_veld * (uint64_t)op.amt /
                              (uint64_t)p.lp_supply);
        int64_t b = (int64_t)((unsigned __int128)(uint64_t)p.reserve_btcveld * (uint64_t)op.amt /
                              (uint64_t)p.lp_supply);
        if (v <= 0 || (uint64_t)v < DUST_THRESHOLD_UNITS ||
            (uint64_t)(p.reserve_veld - v) < DUST_THRESHOLD_UNITS)
            return false;
        if (tx.outputs[0].value != (uint64_t)(p.reserve_veld - v))
            return false;
        const auto user_script = AddressToScript(op.user);
        ...
        if (user_script.empty() || tx.outputs.size() < 2 || tx.outputs[1].value != (uint64_t)v ||
            tx.outputs[1].script_pubkey != user_script)
            return false;
        if (b > 0 && !tokens.CanAmmMove("btcVELD", p.btcveld_addr, op.user, b))
            return false;
        if (dry_run)
            return true;
        if (b > 0 && !tokens.AmmMove("btcVELD", p.btcveld_addr, op.user, b))
            return false;
        p.reserve_veld -= v;
        p.reserve_btcveld -= b;
        p.lp_supply -= op.amt;
