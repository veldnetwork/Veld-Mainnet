...
inline bool ReadScriptNum(const std::vector<uint8_t>& v, int max_size, int64_t& out) {
    if ((int)v.size() > max_size) return false;
    if (v.empty()) { out = 0; return true; }
    ...
    if ((v.back() & 0x7f) == 0) {
        if (v.size() <= 1 || (v[v.size()-2] & 0x80) == 0) return false;
    }
    int64_t result = 0;
...
inline std::vector<uint8_t> EncodeScriptNum(int64_t value) {
    ...
    while (abs) { result.push_back(abs & 0xFF); abs >>= 8; }
    if (result.back() & 0x80) result.push_back(neg ? 0x80 : 0x00);
    else if (neg) result.back() |= 0x80;
    return result;
}
...
inline bool CheckTxLockTimes(const Transaction& tx, uint64_t height,
                             uint64_t mtp, ConfFn utxo_conf) {
    if (tx.locktime != 0) {
        bool all_final = true;
        for (const auto& in : tx.inputs)
            if (in.sequence != 0xFFFFFFFFu) { all_final = false; break; }
        if (!all_final) {
            uint64_t cmp = ((uint64_t)tx.locktime < LOCKTIME_THRESHOLD) ? height : mtp;
            if ((uint64_t)tx.locktime > cmp) return false;   // not yet final
        }
    }
    for (size_t i = 0; i < tx.inputs.size(); ++i) {
        uint32_t seq = tx.inputs[i].sequence;
        if (seq & 0x80000000u) continue;                     // relative lock disabled
        uint64_t out_h = 0, out_mtp = 0;
        if (!utxo_conf(i, out_h, out_mtp)) continue;
        const uint32_t TYPE = 0x00400000u, MASK = 0x0000FFFFu;
        uint32_t val = seq & MASK;
        if (seq & TYPE) {                                    // time domain (512 s units)
            if (mtp < out_mtp + (uint64_t)val * 512ull) return false;
        } else {                                             // height domain
            if (height < out_h + (uint64_t)val) return false;
        }
    }
    return true;
}
...
inline std::vector<uint8_t> Base58CheckDecode_(const std::string& address) {
    ...
    for (char c : address) {
        ...
        int carry = (int)pos;
        for (auto& d : digits) { carry += 58*(int)d; d = (uint8_t)(carry % 256); carry /= 256; }
        while (carry > 0) { digits.push_back((uint8_t)(carry % 256)); carry /= 256; }
    }
    dec.assign(leading, 0);
    for (auto it = digits.rbegin(); it != digits.rend(); ++it) dec.push_back(*it);

    if (dec.size() < 5) return {};
    std::vector<uint8_t> payload(dec.begin(), dec.end() - 4);
    std::array<uint8_t,4> chk = {dec[dec.size()-4], dec[dec.size()-3],
...
inline bool IsValidVeldAddress(const std::string& address, bool testnet = false) {
    ...
    if (v == VELD_P2SH_VERSION_MAINNET) return true;
#ifndef VELD_MAINNET_POW
    if (v == VELD_P2SH_VERSION_TESTNET) return true;
#endif
    return false;
}
...
inline std::vector<uint8_t> AddressToScript(const std::string& address) {
    ...
    if (ver != VELD_ADDR_VERSION_MAINNET) return {};
#else
    if (ver != VELD_ADDR_VERSION_MAINNET &&
        ver != VELD_ADDR_VERSION_TESTNET) return {};
#endif
...
inline std::string ScriptToAddress(const std::vector<uint8_t>& script, bool testnet = false) {
    ...
    for (auto it = digits.rbegin(); it != digits.rend(); ++it) addr += B58[*it];
    return addr;
}
...
public:
    ...
    bool Execute(
        ...
        const bool is_p2sh = ctx.covenants_active &&
                             script_pubkey.size() == 23 &&
...
private:
    ...
    static bool IsPushOnly(const std::vector<uint8_t>& s) {
        ...
        while (pc < s.size()) {
            ...
            if (op <= 0x4B) {
                const size_t n = op;
                if (n > s.size() - pc) return false;
                if (n == 1 &&
                    (s[pc] == 0x81 || (s[pc] >= 1 && s[pc] <= 16))) {
                ...
                const size_t n = s[pc++];
                if (n < 0x4C || n > s.size() - pc) return false;
                pc += n;
            }
            else if (op == 0x4D) {
                ...
                pc += 2;
                if (n <= 0xFF || n > s.size() - pc) return false;
                pc += n;
            }
            else if (op == 0x4F || (op >= 0x51 && op <= 0x60)) { /* OP_1NEGATE / OP_1..OP_16: single-byte numeric push */ }
            else return false;   // any executable opcode → not push-only
        }
        return true;
    }
    ...
    bool RunScript(
        ...
        auto fExec = [&]() {
            for (bool b : exec) if (!b) return false;
            return true;
        };
        const bool cov = ctx.covenants_active;

        while (pc < script.size()) {
            if (stack.size() + alt.size() > MAX_STACK_SIZE) return false;   // depth cap
            ...
            if (opcode <= 0x4B) {
                size_t n = opcode;
                if (pc + n > script.size()) return false;
                if (executing) stack.push_back(StackItem(script.begin()+pc, script.begin()+pc+n));
                pc += n; continue;
            }
            ...
            if (opcode > 0x60 && ++op_count > MAX_OPS_PER_SCRIPT) return false;
            ...
            if (op == OpCode::OP_IF || op == OpCode::OP_NOTIF) {
                ...
                if (executing) {
                    ...
                    if (!cond.empty() &&
                        !(cond.size() == 1 && cond[0] == 1)) return false;
                    val = !cond.empty();
                    stack.pop_back();
            ...
            if (op == OpCode::OP_ELSE) {
                if (!cov || exec.empty()) return false;
                exec.back() = !exec.back();
                continue;
            }
            ...
            if (!executing) continue;   // skip everything else in a dead branch
            ...
            case OpCode::OP_1: case OpCode::OP_2: case OpCode::OP_3:
            case OpCode::OP_4: case OpCode::OP_5: case OpCode::OP_6:
            case OpCode::OP_7: case OpCode::OP_8: case OpCode::OP_9:
            case OpCode::OP_10: case OpCode::OP_11: case OpCode::OP_12:
            case OpCode::OP_13: case OpCode::OP_14: case OpCode::OP_15:
            case OpCode::OP_16:
                if (!cov) return false;
            ...
            case OpCode::OP_2DROP:
                if (!cov || stack.size() < 2) return false;
            ...
            case OpCode::OP_SHA256: {
                if (!cov || stack.empty()) return false;
                auto top = stack.back(); stack.pop_back();
                SHA256 h; h.update(top.data(), top.size());
                Hash256 d = h.digest();
            ...
            case OpCode::OP_CHECKSIGVERIFY: {
                ...
                auto pubkey = stack.back(); stack.pop_back();
                auto sig    = stack.back(); stack.pop_back();
                if (!VerifySignature(sig, pubkey, tx, input_index)) return false;
                break;
            }
            ...
            case OpCode::OP_CHECKMULTISIGVERIFY: {
                ...
                if (!OpCheckMultisig(stack, tx, input_index, op_count)) {
                    if (op == OpCode::OP_CHECKMULTISIGVERIFY) return false;
            ...
            case OpCode::OP_CHECKLOCKTIMEVERIFY: {
                if (!cov || stack.empty()) return false;
                if (input_index >= tx.inputs.size()) return false;     // defensive bound
                int64_t want = 0;
                if (!ReadScriptNum(stack.back(), 5, want)) return false;
                ...
                if (want_time != lt_time) return false;
                if ((uint64_t)want > (uint64_t)tx.locktime) return false;
                // Input must be non-final, else locktime is disabled.
                if (tx.inputs[input_index].sequence == 0xFFFFFFFF) return false;
            ...
            case OpCode::OP_CHECKSEQUENCEVERIFY: {
                ...
                if (want_seq & 0x80000000UL) break;   // disable bit set → no-op (BIP112)
                uint32_t in_seq = tx.inputs[input_index].sequence;
                if (in_seq & 0x80000000UL) return false;       // spender disabled relative lock
                const uint32_t TYPE = 0x00400000UL;            // bit22 time/blocks flag
                const uint32_t MASK = 0x0000FFFFUL;            // low 16 bits = value
                if ((want_seq & TYPE) != (in_seq & TYPE)) return false;   // domains must match
                if ((in_seq & MASK) < (want_seq & MASK)) return false;    // input commits to >= the required lock
    ...
    bool OpCheckMultisig(std::vector<StackItem>& stack, const Transaction& tx,
                         uint32_t input_index, int& op_count) {
        if (stack.empty()) return false;
        int64_t nkeys = 0;
        if (!ReadScriptNum(stack.back(), 4, nkeys)) return false;
        stack.pop_back();
        if (nkeys < 0 || nkeys > 15) return false;
        if ((op_count += (int)nkeys) > MAX_OPS_PER_SCRIPT) return false;
        if ((int64_t)stack.size() < nkeys) return false;
        std::vector<StackItem> keys;
        for (int i = 0; i < nkeys; ++i) { keys.push_back(stack.back()); stack.pop_back(); }

        if (stack.empty()) return false;
        int64_t nsigs = 0;
        if (!ReadScriptNum(stack.back(), 4, nsigs)) return false;
        stack.pop_back();
        if (nsigs < 0 || nsigs > nkeys) return false;
        if ((int64_t)stack.size() < nsigs) return false;
        std::vector<StackItem> sigs;
        for (int i = 0; i < nsigs; ++i) { sigs.push_back(stack.back()); stack.pop_back(); }

        if (stack.empty()) return false;     // the historical dummy element
        if (!stack.back().empty()) return false;  // NULLDUMMY / TXID canonicality
        stack.pop_back();
        ...
        while (si < sigs.size()) {
            bool matched = false;
            while (ki < keys.size()) {
                if (VerifySignature(sigs[si], keys[ki], tx, input_index)) { ++ki; matched = true; break; }
                ++ki;
            }
            if (!matched) return false;
            ++si;
            if (keys.size() - ki < sigs.size() - si) return false;   // not enough keys left
        }
        return true;
    }
    ...
    bool VerifySignature(
        ...
        switch (scheme_id) {
        case 0x01: {
            if (sig.size() != 3311) return false;
            if (pubkey.size() != 1952) return false;
            if (sig.back() != 0x01) return false;

            Secp256k1SigDER raw_sig(sig.begin() + 1, sig.end() - 1);
            if (raw_sig.size() != 3309) return false;
            Secp256k1PubKey pk;
