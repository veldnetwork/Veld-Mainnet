...
enum class ScriptType { P2PKH, SHA384_KEY, P2PK, MULTISIG, OP_RETURN, UNKNOWN };

inline ScriptType DetectScriptType(const std::vector<uint8_t>& script) {
    if (IsSha384KeyScript(script))
        return ScriptType::SHA384_KEY;
...
inline std::vector<uint8_t> Base58CheckDecode_(const std::string& address) {
    if (address.size() < 25 || address.size() > 75)
        return {};
    ...
    if (h1[0] != chk[0] || h1[1] != chk[1] || h1[2] != chk[2] || h1[3] != chk[3])
        return {};
    if (payload.size() != 21 && payload.size() != 49)
        return {};
    return payload;
}
...
class TransactionScriptHashes {
  public:
    ...
    const Hash256& TemplateHash(uint32_t input_index) {
        if (input_index >= tx_.inputs.size())
            throw std::out_of_range("template input index");
        const auto cached = templates_.find(input_index);
        if (cached != templates_.end())
            return cached->second;
        if (prefix_.empty())
            BuildPrefix();
        auto preimage = prefix_;
        for (int i = 0; i < 4; ++i)
            preimage.push_back((input_index >> (i * 8)) & 0xff);
...
class ScriptInterpreter {
    ...
    bool Execute(const std::vector<uint8_t>& script_sig, const std::vector<uint8_t>& script_pubkey,
        ...
        if (!script_pubkey.empty() && script_pubkey[0] == SHA384_DESTINATION_OPCODE) {
            ...
            if (!pqc::ParseScriptSig(script_sig, signature, public_key) ||
                ...
                !MatchesSha384Key(script_pubkey, public_key, SHA384_DESTINATION_TEST_NETWORK))
                return false;
        ...
        if (is_p2sh) {
            ...
            if (!RunScript(script_sig, stack, alt, tx, input_index, ctx, hashes))
                return false;
            if (stack.empty())
                return false;
            ...
            if (!RunScript(script_pubkey, stack, alt, tx, input_index, ctx, hashes))
                return false;
            ...
            if (!RunScript(redeem, stack, alt, tx, input_index, ctx, hashes))
                return false;
  ...
  private:
    ...
    bool RunScript(const std::vector<uint8_t>& script, std::vector<StackItem>& stack,
                   ...
                   const ScriptContext& ctx, TransactionScriptHashes& hashes) {
        size_t pc = 0;
        int op_count = 0;
