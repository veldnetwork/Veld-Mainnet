...
inline bool StakeUnlockPreservesMinimum(uint64_t current_units,
                                        uint64_t mature_units,
                                        uint64_t requested_units,
                                        uint64_t effective_min_units) noexcept {
    if (mature_units > current_units) return false;
    const uint64_t applied =
        requested_units < mature_units ? requested_units : mature_units;
...
inline bool ParseCanonicalStakeOp(std::string_view payload,
                                  CanonicalStakeOp& out) noexcept {
    ...
    if (address_end == std::string_view::npos || address_end == 0) return false;
    ...
    const std::string_view amount_text = amount_end == std::string_view::npos
        ? payload : payload.substr(0, amount_end);
    if (!ParseCanonicalUint64Text(amount_text, parsed.amount_units)) return false;

    if (!is_lock) {
        if (amount_end != std::string_view::npos) return false;
        out = std::move(parsed);
        return true;
    }
    ...
    if (payload.size() != 2 || payload[0] != 'T' ||
        payload[1] < '1' || payload[1] > '4') {
        return false;
    }
