...
class MintNullifierTipRace : public std::runtime_error {
  public:
    explicit MintNullifierTipRace(const std::string& what) : std::runtime_error(what) {}
};

class MintNullifierIndexError : public std::runtime_error {
  public:
    explicit MintNullifierIndexError(const std::string& what) : std::runtime_error(what) {}
};
...
class MintNullifierIndex {
  public:
    ...
    static bool ValidateHeightKey(const std::string& key, uint64_t height,
        ...
        try {
            ...
            if (key[tx_pos + 10] != ':' || key[vout_pos + 10] != ':' ||
                !ParseFixedWidthUint32_(std::string_view(key).substr(tx_pos, 10)) ||
        ...
        try {
            ...
            for (const auto& t : transitions) {
                ...
                const std::string key =
                    Key(block.height, t.tx_index, t.marker_vout, block_hash_hex);
                ...
                if (!Decode(value, item) || item.height != block.height ||
                    ...
                    Key(item.height, item.transition.tx_index, item.transition.marker_vout,
        ...
        try {
            store_.Iterate(
                HeightPrefix(height), [&](const std::string& key, const std::string& value) {
                    Hash256 key_hash{};
                    Item item;
                    if (!ValidateHeightKey(key, height, &key_hash) || !Decode(value, item) ||
                        !ValidateDecodedItem_(item) || item.height != height ||
    ...
    Status BuildStatusFromQuiescentSample(const Blockchain& chain,
    ...
    bool PutAccepted(const Block& block, const std::vector<BtcVeldMintTransition>& transitions) {
        db::WriteBatch batch;
        if (!AppendAcceptedToBatch(batch, block, transitions))
            return false;
        return batch.IsEmpty() || store_.Write(batch);
    }

    Status BuildStatus(const Blockchain& chain, const std::string& target_outpoint,
        ...
        if (!chain.TryTip(start_tip))
            throw MintNullifierTipRace("btcVELD nullifier proof requested without a canonical tip");
        out.tip = start_tip.height;
        out.tip_hash = start_tip.GetHash();
        const bool genesis_empty = out.tip == 0 && expected.processed_height == 0 &&
                                   HashIsZero(expected.processed_block_hash) &&
                                   expected.count == 0 && expected.root == btcnull::EmptyRoot() &&
                                   expected.effect_count == 0 &&
                                   expected.effect_root == EmptyBtcVeldMintEffectCommitment();
        if (!genesis_empty &&
            (expected.processed_height != out.tip || expected.processed_block_hash != out.tip_hash))
            throw MintNullifierTipRace("btcVELD nullifier accumulator/tip race; retry");
        ...
        auto tip_still_matches = [&]() {
            Block now;
            return chain.TryTip(now) && now.height == out.tip && now.GetHash() == out.tip_hash;
        };
        try {
            store_.Iterate(PREFIX, [&](const std::string& key, const std::string& value) {
                Item item;
                if (!Decode(value, item))
                    throw MintNullifierIndexError("corrupt btcVELD nullifier-index value");
                if (Key(item.height, item.transition.tx_index, item.transition.marker_vout,
                        HashToHex(item.block_hash)) != key)
                    throw MintNullifierIndexError("btcVELD nullifier-index key/value mismatch");
                if (item.height > out.tip)
                    return true;
                ...
                if (item.transition.tx_index >= cached_canonical.transactions.size() ||
                    ...
                    item.transition.txid !=
                        HashToHex(
                            cached_canonical.transactions[item.transition.tx_index].GetTxID()))
                    throw MintNullifierIndexError("btcVELD nullifier index locator is corrupt");
                ...
                if (insertion) {
                    ...
                    const btcnull::InsertResult inserted =
                        btcnull::Insert(running_root, t.outpoint, insertion_proof);
                    if (!inserted.ok || inserted.new_root != t.new_root)
                        throw MintNullifierIndexError(
                            "btcVELD nullifier transition proof is corrupt");
                    if (!btcnull::UpdateWitnessAfterInsert(running_root, inserted.new_root,
                                                           target_outpoint, out.consumed, out.proof,
                                                           t.outpoint, insertion_proof))
                        throw MintNullifierIndexError(
                            "btcVELD nullifier target-witness update failed");
                    if (t.effect_kind == "C1_FUND") {
                        if (!funded_by_outpoint.emplace(t.outpoint, t.c1_allocation_id).second ||
                            !outpoint_by_allocation.emplace(t.c1_allocation_id, t.outpoint).second)
                            throw MintNullifierIndexError("duplicate unresolved C1 funding effect");
                    ...
                    const auto allocation = outpoint_by_allocation.find(t.c1_allocation_id);
                    ...
                    if (t.outpoint == target_outpoint) {
                        if (!out.consumed || out.minted || out.accepted_effect_kind != "C1_FUND" ||
                            out.c1_allocation_id != t.c1_allocation_id)
                            throw MintNullifierIndexError(
                ...
                if (running_effect_count == UINT64_MAX)
                    throw MintNullifierIndexError("btcVELD mint-effect transition count overflow");
                try {
                    running_effect_root =
                        ExtendBtcVeldMintEffectCommitment(running_effect_root, item.height, t);
            ...
            if (!tip_still_matches())
                throw MintNullifierTipRace(
                    "btcVELD nullifier canonical tip changed during proof rebuild");
            throw MintNullifierIndexError(std::string("btcVELD nullifier-index scan failed: ") +
                                          e.what());
        }
        ...
        auto recheck_locator = [&](const std::string& txid, uint64_t height,
                                   const Hash256& block_hash, uint32_t tx_index,
            ...
            if (!tip_still_matches() || final_carrier.GetHash() != block_hash ||
                tx_index >= final_carrier.transactions.size() ||
                marker_vout >= final_carrier.transactions[tx_index].outputs.size() ||
                HashToHex(final_carrier.transactions[tx_index].GetTxID()) != txid)
                throw MintNullifierTipRace(
                    "btcVELD mint-effect canonical carrier changed during proof rebuild");
        };
        recheck_locator(out.consumer_txid, out.consumer_block_height, out.consumer_block_hash,
                        out.consumer_tx_index, out.consumer_marker_vout);
        if (out.credit_txid != out.consumer_txid)
            recheck_locator(out.credit_txid, out.credit_block_height, out.credit_block_hash,
                            out.credit_tx_index, out.credit_marker_vout);
        ...
        if (running_count != expected.count || running_root != expected.root ||
            ...
            !btcnull::Verify(running_root, target_outpoint, out.consumed, out.proof) ||
            out.consumed != !out.consumer_txid.empty() || out.minted != !out.credit_txid.empty() ||
  ...
  private:
    ...
    static bool ValidateTransition_(const BtcVeldMintTransition& transition) noexcept {
        try {
            ...
            if (!insertion && !c1_mint)
                return false;
            if ((transition.effect_kind == "MINT" && !transition.c1_allocation_id.empty()) ||
                ((transition.effect_kind == "C1_FUND" || c1_mint) &&
                 !c1reserve::IsAllocationId(transition.c1_allocation_id)))
                return false;
            if (c1_mint)
                return transition.proof.empty() && transition.old_root == transition.new_root;
            if (transition.proof.empty() || transition.proof.size() > btcnull::MAX_PROOF_BYTES ||
                transition.old_root == transition.new_root)
                return false;
            btcnull::Proof proof;
            if (!btcnull::DecodeProof(transition.proof, proof))
                return false;
            const btcnull::InsertResult inserted =
                btcnull::Insert(transition.old_root, transition.outpoint, proof);
            return inserted.ok && inserted.new_root == transition.new_root;
        } catch (...) {
            return false;
        }
    }
    ...
    static std::string Key(uint64_t height, uint32_t tx_index, uint32_t marker_vout,
                           const std::string& block_hash_hex) {
        std::ostringstream out;
        out << PREFIX << std::setw(20) << std::setfill('0') << height << ':' << std::setw(10)
            << std::setfill('0') << tx_index << ':' << std::setw(10) << std::setfill('0')
            << marker_vout << ':' << block_hash_hex;
        return out.str();
    }
    ...
    static bool GetString(const std::string& in, size_t& pos, std::string& out, size_t max) {
        uint32_t n = 0;
    ...
    static std::string Encode(uint64_t height, const Hash256& block_hash,
        ...
        std::string out;
        out.reserve(1 + 8 + 32 + 4 + 4 + 4 + t.txid.size() + t.outpoint.size() +
                    t.effect_kind.size() + t.c1_allocation_id.size() + t.proof.size() + 64);
        out.push_back(static_cast<char>(2));
        PutU64(out, height);
        out.append(reinterpret_cast<const char*>(block_hash.data()), block_hash.size());
        ...
        out.append(reinterpret_cast<const char*>(t.proof.data()), t.proof.size());
        out.append(reinterpret_cast<const char*>(t.old_root.data()), t.old_root.size());
        out.append(reinterpret_cast<const char*>(t.new_root.data()), t.new_root.size());
        return out;
    }
    ...
    static bool Decode(const std::string& in, Item& item) {
        item = Item{};
        size_t pos = 0;
        if (in.empty() || static_cast<uint8_t>(in[pos++]) != 2 || !GetU64(in, pos, item.height) ||
            pos + item.block_hash.size() > in.size())
            return false;
        std::copy_n(reinterpret_cast<const uint8_t*>(in.data()) + pos, item.block_hash.size(),
                    item.block_hash.begin());
        ...
        if (!GetU32(in, pos, proof_len) || proof_len > btcnull::MAX_PROOF_BYTES ||
            pos + proof_len + 64 != in.size())
            return false;
        item.transition.proof.assign(reinterpret_cast<const uint8_t*>(in.data()) + pos,
