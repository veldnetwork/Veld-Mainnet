...
class StorageEngine {
  public:
    explicit StorageEngine(const std::string& data_dir, uint32_t network_magic)
        : data_dir_(data_dir), magic_(network_magic), current_file_number_(0),
    ...
    bool WriteBlock(const Block& block) {
        ...
        if (block_index_.count(hash_hex))
            return true;
        ...
        if (!ofs)
            throw std::runtime_error("Cannot open block file: " + block_file);
    ...
    std::optional<ChainState> ReadChainState() const {
        ...
        while (std::getline(ifs, line)) {
            if (line.rfind("height=", 0) == 0) {
                if (have_height ||
                    !ParseCanonicalUint64Text(std::string_view(line).substr(7), state.height))
                    return std::nullopt;
                have_height = true;
                ...
                if (have_supply ||
                    !ParseCanonicalUint64Text(std::string_view(line).substr(7),
                                              state.supply_units) ||
  ...
  private:
    ...
    std::string IndexPath() const {
        return data_dir_ + "/index/block_index.dat";
    }
    std::string ChainStatePath() const {
        return data_dir_ + "/chainstate/chain.dat";
    }
    std::string UTXOPath() const {
        return data_dir_ + "/chainstate/utxo.dat";
    }
    ...
    void FlushIndex() {
        ...
        for (const auto& [hash, entry] : block_index_) {
            ofs << entry.hash_hex << " " << entry.height << " " << entry.file_number << " "
                << entry.file_offset << " " << entry.block_size << " " << entry.timestamp << " "
                << entry.bits << "\n";
        }
    }
    ...
    void LoadIndex() {
        ...
        while (std::getline(ifs, line)) {
            ...
            BlockIndexEntry entry;
            ss >> entry.hash_hex >> entry.height >> entry.file_number >> entry.file_offset >>
                entry.block_size >> entry.timestamp >> entry.bits;
