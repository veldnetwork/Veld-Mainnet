...
struct MempoolEntry {
    ...
    MempoolEntry()
        : fee_units(0), fee_rate(0), added_time(0), tx_size_bytes(0), height_added(0),
          is_rbf(false), had_unconfirmed_parent(false), base_locking_validated(false),
          token_authorization_validated(false) {}

    MempoolEntry(const Transaction& t, uint64_t fee, uint32_t height)
        : tx(t), fee_units(fee), height_added(height), is_rbf(false), had_unconfirmed_parent(false),
          base_locking_validated(false), token_authorization_validated(false) {
...
class Mempool {
  public:
    ...
    static constexpr size_t MAX_TOKEN_TX_OUTPUTS = MAX_STANDARD_TRANSACTION_OUTPUTS;
    ...
    static const char* ResultToString(AddResult r) {
        ...
        case AddResult::DUPLICATE:
            return "duplicate";
        case AddResult::FEE_TOO_LOW:
            return "fee_too_low";
        case AddResult::INVALID:
            return "invalid";
        case AddResult::DEFERRED_LOCAL_WORK:
            return "deferred_local_work";
        case AddResult::EXPIRED_NMS:
            return "expired_nms";
        case AddResult::MISSING_INPUT:
            return "missing_input";
        case AddResult::FULL:
            return "full";
        case AddResult::DOUBLE_SPEND:
            return "double_spend";
        case AddResult::STAKE_ALREADY_PENDING:
            return "stake_already_pending";
        case AddResult::STAKE_EXCEEDS_BALANCE:
            return "stake_exceeds_balance";
        case AddResult::COINBASE_IMMATURE:
            return "coinbase_immature";
        case AddResult::VALIDATOR_STATE_COOLDOWN:
            return "validator_state_cooldown";
        case AddResult::MALFORMED_VALIDATOR_OP:
            return "malformed_validator_op";
    ...
    static bool ExtractOpReturnPayload(const std::vector<uint8_t>& spk, std::string& payload,
                                       bool& exact) {
        payload.clear();
        exact = false;
        ...
        if (spk[off] <= 75) {
            plen = spk[off++];
        } else if (spk[off] == 0x4C && spk.size() > off + 1) {
            off++;
            plen = spk[off++];
        } else if (spk[off] == 0x4D && spk.size() > off + 2) {
            off++;
            plen = (size_t)spk[off] | ((size_t)spk[off + 1] << 8);
            off += 2;
        } else {
            return false;
        }
        if (off + plen > spk.size())
            return false;
    ...
    static bool ExtractCanonicalStakeOp(const std::vector<uint8_t>& spk, CanonicalStakeOp& op,
        ...
        if (!is_stake_family || !exact || !IsCanonicalMarkerOpReturn(spk, payload))
            return false;
    ...
    static bool ExtractCanonicalTokenMarker(const Transaction& tx, bool& is_family,
                                            std::optional<TokenOpData>& token_op, bool& is_mspv,
        ...
        for (const auto& out : tx.outputs) {
            ...
            const bool reserve =
                btcveld::reserve::TRANSITION_V1_REQUIRED && payload.rfind("VELD_RSV", 0) == 0;
            if (!token && !mspv && !reserve)
                continue;
            is_family = true;
            if (++count != 1 || out.value != 0 || !exact ||
                ...
                return false;
            if (token) {
                if (payload.size() > MAX_TOKEN_OP_PAYLOAD_BYTES)
                    return false;
                ...
                const size_t direct_limit =
                    reserve ? MAX_TOKEN_OP_PAYLOAD_BYTES : btcnull::MAX_MSPV_OP_PAYLOAD_BYTES;
    ...
    static std::string TokenReplayReservationKey(const std::optional<TokenOpData>& token_op,
                                                 bool is_mspv,
                                                 const std::string& canonical_payload) {
        static constexpr const char* ROOT_CHILD = "btc-nullifier-root:one-unresolved-child";
        static constexpr const char* C1_CARRIER = "btc-c1-sequence:one-unresolved-carrier";
        if (!is_mspv) {
            if constexpr (btcveld::reserve::TRANSITION_V1_REQUIRED) {
                if (token_op && token_op->token_id == BTCVELD_TOKEN_ID &&
                    token_op->action == "MINT" &&
                    token_op->memo.rfind(btcveld::reserve::ISSUER_MEMO_PREFIX, 0) == 0) {
                    const std::vector<uint8_t> proof = HexToBytes(
                        token_op->memo.substr(std::strlen(btcveld::reserve::ISSUER_MEMO_PREFIX)));
            ...
            if (token_op && token_op->token_id == BTCVELD_TOKEN_ID &&
                (token_op->action == "RESERVE" || token_op->action == "CANCEL" ||
                 token_op->action == "EXPOSE"))
                return C1_CARRIER;
            if (token_op && token_op->token_id == BTCVELD_TOKEN_ID && token_op->action == "FUND")
                return ROOT_CHILD;
            std::string outpoint;
            btcnull::Proof nullifier_proof;
            std::string reservation_id;
            if (!token_op || token_op->token_id != BTCVELD_TOKEN_ID || token_op->action != "MINT" ||
                !btcnull::ParseIssuerMemo(token_op->memo, outpoint, nullifier_proof,
                                          reservation_id) ||
        ...
        if (canonical_payload.rfind(RESERVE_PREFIX, 0) == 0) {
            const std::vector<uint8_t> proof =
                HexToBytes(canonical_payload.substr(RESERVE_PREFIX.size()));
            btcveld::reserve::Claim claim;
            if (proof.empty() || !btcveld::reserve::DecodeProof(proof.data(), proof.size(), claim))
                return {};
            return "btc-reserve-transition:one-unresolved-child";
        }
        static const std::string MSPV_PREFIX = "VELD_MSPV|";
        if (canonical_payload.rfind(MSPV_PREFIX, 0) != 0)
            return {};
        const std::vector<uint8_t> proof = HexToBytes(canonical_payload.substr(MSPV_PREFIX.size()));
        if (proof.empty())
            return {};
        ...
        if (!btcspv::ParseMintSpvOp(proof.data(), proof.size(), block_hash, dirs, branch,
                                    legacy_tx))
            return {};
        std::vector<btcspv::BtcTxOut> outs;
        if (!btcspv::ParseBtcTxOutputs(legacy_tx.data(), legacy_tx.size(), outs))
            return {};
        ...
        if (!IsValidBtcOutpointId(btcspv::BtcDepositOutpointId(txid, custody_vout)))
            return {};
        return ROOT_CHILD;
    }

    static bool TokenTransactionEnvelopeWithinBounds(const Transaction& tx,
                                                     size_t serialized_size) {
        return tx.inputs.size() <= MAX_TOKEN_TX_INPUTS &&
               tx.outputs.size() <= MAX_TOKEN_TX_OUTPUTS && serialized_size <= MAX_TOKEN_TX_BYTES;
    }

    static std::pair<std::string, uint64_t>
    ExtractStakeLockAddrAmount(const std::vector<uint8_t>& spk) {
        CanonicalStakeOp op;
        bool is_stake_family = false;
        if (!ExtractCanonicalStakeOp(spk, op, is_stake_family) ||
    ...
    AddResult Add(const Transaction& tx, uint64_t fee_units, uint32_t current_height,
                  const Blockchain& chain,
                  mining::ExpensivePowBudget* source_pow_budget = nullptr) {
        return AddImpl_(tx, fee_units, current_height, chain, source_pow_budget, nullptr);
    }
  ...
  private:
    ...
    AddResult AddAuthorizedWork_(const Transaction& tx, uint64_t fee_units, uint32_t current_height,
                                 const Blockchain& chain, WorkSinkAuthorization authorization,
                                 mining::ExpensivePowBudget* source_pow_budget = nullptr) {
        if (!authorization)
            return AddResult::DEFERRED_LOCAL_WORK;
        return AddImpl_(tx, fee_units, current_height, chain, source_pow_budget, &authorization);
    }

    AddResult AddImpl_(const Transaction& tx, uint64_t fee_units, uint32_t current_height,
                       const Blockchain& chain, mining::ExpensivePowBudget* source_pow_budget,
                       WorkSinkAuthorization* work_authorization) {
        const uint64_t canonical_height = chain.Height();
        if (canonical_height == UINT64_MAX || !chain.RuntimeAdmissionPermits(canonical_height + 1))
        ...
        if (tx.outputs.size() > MAX_STANDARD_TRANSACTION_OUTPUTS)
            return AddResult::INVALID;
        if (TxComposesMultipleProtocols(tx) || TxHasInvalidTokenMarkerSet(tx))
            return AddResult::INVALID;
        ...
        if (!ExtractCanonicalTokenMarker(tx, token_family, token_op, token_is_mspv,
                                         &token_marker_payload))
            return AddResult::INVALID;
        const std::string token_replay_key =
            TokenReplayReservationKey(token_op, token_is_mspv, token_marker_payload);
        if (token_is_mspv && token_replay_key.empty())
            return AddResult::INVALID;
        if (token_op && token_op->token_id == BTCVELD_TOKEN_ID &&
            (token_op->action == "MINT" || token_op->action == "FUND") && token_replay_key.empty())
            return AddResult::INVALID;
        ...
        for (const auto& out : tx.outputs) {
            CanonicalStakeOp op;
            bool is_stake_family = false;
            const bool valid = ExtractCanonicalStakeOp(out.script_pubkey, op, is_stake_family);
            if (is_stake_family && !valid)
                return AddResult::INVALID;
            if (!valid)
                continue;
            if (AddressToScript(op.address).size() != 25)
                return AddResult::INVALID;
            if (op.amount_units == 0 || op.amount_units > MAX_STAKE_UNITS)
                return AddResult::INVALID;
        ...
        for (const auto& op : stake_ops) {
            if (op.action != CanonicalStakeOp::Action::UNLOCK)
                continue;
        ...
        if (!chain.StakeTransactionValid(tx, candidate_height))
            return AddResult::INVALID;
        const uint64_t effective_min_stake = chain.GetEffectiveMinStakeUnits(candidate_height);
        for (const auto& [address, requested] : unlock_claims) {
            const uint64_t current = chain.GetStakedForAddr(address);
            const uint64_t mature = chain.GetMatureStakeForAddr(address, candidate_height);
            ...
            if (current == 0 || mature == 0)
                return AddResult::INVALID;
            if (!StakeUnlockPreservesMinimum(current, mature, requested, effective_min_stake))
                return AddResult::INVALID;
        }
        ...
        // harmless and lock-correct — they only ever pass once we get here.)
        {
            ...
            if (tx_size_pre > 1024 * 1024)
                return AddResult::INVALID;
            if (token_family && !TokenTransactionEnvelopeWithinBounds(tx, tx_size_pre))
                return AddResult::INVALID;
            uint64_t fee_rate_pre = tx_size_pre > 0 ? (fee_units * 1000) / tx_size_pre : 0;
            if (fee_rate_pre < MIN_FEE_RATE)
                return AddResult::FEE_TOO_LOW;
            if (chain.Height() >= BATCH1_HARDENING_HEIGHT && tx.HasDustOutput(DUST_THRESHOLD_UNITS))
                return AddResult::INVALID;
            std::lock_guard<std::mutex> rl(recently_rejected_mtx_);
            if (recently_rejected_.count(key))
                return AddResult::INVALID;
        }
        ...
        const AmmOpParseResult amm_parsed = ParseAmmOpDetailed(tx);
        {
            ...
            if (amm_parsed.status == AmmOpParseStatus::VALID &&
                ...
                return AddResult::INVALID;
            if (amm_parsed.status == AmmOpParseStatus::VALID &&
                ((amm_parsed.op.action == "SWAP_V2B" && amm_parsed.op.extra <= 0) ||
                 (amm_parsed.op.action == "SWAP_B2V" && amm_parsed.op.extra != 0)))
                return AddResult::INVALID;
            if (amm_parsed.status == AmmOpParseStatus::VALID &&
...

        {
            ...
            if (!Blockchain::ValidateExternalProtocolCustodyOutputs(tx,
                                                                    (uint64_t)current_height + 1)) {
                return AddResult::INVALID;
            }
        }

        {
            if (!tx.inputs.empty() && !tx.outputs.empty()) {
                auto first_utxo =
                    chain.GetUTXO(tx.inputs[0].prev_tx_hash, tx.inputs[0].prev_out_index);
                if (first_utxo) {
                    const auto& input_script = first_utxo->script_pubkey;
                    bool homogeneous_inputs = true;
                    for (size_t i = 1; i < tx.inputs.size(); ++i) {
                        auto u =
                            chain.GetUTXO(tx.inputs[i].prev_tx_hash, tx.inputs[i].prev_out_index);
                    ...
                    if (homogeneous_inputs) {
                        ...
                        if (only_self_outputs && has_any_self_output && !has_op_return &&
                            tx.inputs.size() >= 2) {
                            ...
                            const char* decision =
                                (utxos_for_addr.size() < MIN_UTXOS_TO_CONSOLIDATE) ? "REJECT"
                                                                                   : "ACCEPT";
                            ...
                            for (const auto& out : tx.outputs) {
                                ...
                                if (off + len > out.script_pubkey.size())
                                    continue;
                            ...
                            std::cerr
                                << "[self-consolidate] decision=" << decision
                                << " txid=" << key.substr(0, 16)
                                << " from=" << (from_addr.empty() ? "?" : from_addr.substr(0, 14))
                                << " in=" << tx.inputs.size() << " out=" << tx.outputs.size()
                                << " utxos=" << utxos_for_addr.size()
                                << " floor=" << MIN_UTXOS_TO_CONSOLIDATE << " op_return='"
                                << op_return_preview << "'"
                                << "\n";
                            std::cerr.flush();
            ...
            if (nms_rec_for_admission) {
                {
                    ...
                    if (nms_payload_hashes_.size() >= MAX_NMS_RECORDS_PER_BLOCK) {
                ...
                {
                    std::string new_key = HashToHex(Hash256d(nms_rec_for_admission->raw));
                    std::lock_guard<std::mutex> lk(mutex_);
        ...
        {
            bool touches_amm_state = amm_parsed.status == AmmOpParseStatus::VALID;
            bool creates_pool_marker = false;
            for (const auto& out : tx.outputs) {
                if (!IsAmmPoolScript(out.script_pubkey))
                    continue;
            ...
            if (creates_pool_marker && amm_parsed.status != AmmOpParseStatus::VALID) {
                return AddResult::INVALID;
            }
            ...
            if (all_inputs_on_chain_pre) {
                ...
                if (!chain.ValidateTransactionLocking(tx, false, &permanent_failure)) {
                    std::cerr
                        << "  [mempool] reject: ValidateTransactionLocking failed (sig/script verify)\n";
                    std::cerr.flush();
                ...
                if (token_family) {
                    if (!chain.ValidateTokenMempoolCandidate(tx, (uint64_t)current_height + 1,
            ...
            if (touches_amm_state &&
                !chain.ValidateAmmMempoolCandidate(tx, (uint64_t)chain.Height() + 1)) {
                return AddResult::INVALID;
            }
        }
        ...
        if (nms_rec_for_admission) {
            ...
            const auto nms_validation = chain.ValidateNmsLocking(
                *nms_rec_for_admission, static_cast<uint64_t>(current_height) + 1,
                source_pow_budget);
            if (nms_validation != NmsValidationDisposition::Valid) {
                ...
                std::cerr << "  [mempool-nms] reject: authenticated NMS "
                          ...
                          << current_height
                          << " enclosing=" << (static_cast<uint64_t>(current_height) + 1) << "\n";
                std::cerr.flush();
                return AddResult::INVALID;
            }
        }
        ...
        const bool token_authorization_proven = token_family && !token_is_mspv &&
                                                token_op.has_value() && !token_op->from.empty() &&
        ...
        if (work_authorization) {
            ...
            bool claimed = false;
            try {
                claimed = work_authorization->claim();
            } catch (...) {
                claimed = false;
            }
            bool live = false;
            try {
                live = claimed && work_authorization->live();
            } catch (...) {
                live = false;
            }
            if (!live)
        ...
        if (amm_parsed.status == AmmOpParseStatus::VALID) {
            if (!pending_amm_tx_.empty() && entries_.count(pending_amm_tx_)) {
                return AddResult::DOUBLE_SPEND;
            }
            if (!pending_amm_tx_.empty())
                pending_amm_tx_.clear();
        ...
        for (size_t input_index = 0; input_index < tx.inputs.size(); ++input_index) {
            ...
            std::string spend_key =
                HashToHex(input.prev_tx_hash) + ":" + std::to_string(input.prev_out_index);
            ...
            if (utxo_opt) {
                ...
                if (!all_inputs_on_chain_pre) {
                    const auto input_result = chain.ValidateMempoolCanonicalInput(
                        tx, static_cast<uint32_t>(input_index), *utxo_opt, &script_hashes);
                    if (input_result ==
                        Blockchain::MempoolCanonicalInputResult::MISSING_OR_CHANGED) {
                        return AddResult::MISSING_INPUT;
                    }
                    if (input_result != Blockchain::MempoolCanonicalInputResult::VALID) {
                        return AddResult::INVALID;
                    }
                }
            ...
            if (input.prev_out_index >= pit->second.tx.outputs.size())
                return AddResult::INVALID;
            {
                const auto& parent_out = pit->second.tx.outputs[input.prev_out_index];
                ...
                if (IsAmmPoolScript(parent_out.script_pubkey))
                    return AddResult::INVALID;
                if (!Blockchain::VerifyInputAgainstScript(tx, static_cast<uint32_t>(input_index),
        ...
        if (amm_parsed.status == AmmOpParseStatus::VALID && !all_inputs_on_chain) {
            return AddResult::MISSING_INPUT;
        }
        ...
        // parent shape.
        if (tx.TotalOutput() > resolved_input_total)
            return AddResult::INVALID;
        ...
        if (token_family && all_inputs_on_chain && !token_policy_checked &&
            !chain.ValidateTokenMempoolCandidate(tx, (uint64_t)current_height + 1,
                                                 chain.ComputeNextBits())) {
            return AddResult::INVALID;
        }
        if (token_family && all_inputs_on_chain && !token_policy_checked)
        ...
        // LOCK/UNLOCK outputs.
        {
            ...
            for (const auto& op : stake_ops) {
                if (!verified_stakers.insert(op.address).second)
                    continue;
...

        {
            ...
            auto canonical_lower_hex = [](const std::string& value, size_t exact_size) {
                if (value.size() != exact_size)
                    return false;
            ...
            for (const auto& out : tx.outputs) {
                ...
                if (offset > out.script_pubkey.size() || len > out.script_pubkey.size() - offset)
                    continue;
                ...
                const bool validator_marker = data.compare(0, SLASH_PFX.size(), SLASH_PFX) == 0 ||
                ...
                if (data.compare(0, SLASH_PFX.size(), SLASH_PFX) == 0) {
                    ...
                    if (!canonical_lower_hex(parts[2], PUBKEY_HEX_LEN))
                        return AddResult::MALFORMED_VALIDATOR_OP;
                    if (!canonical_lower_hex(parts[4], 64) || !canonical_lower_hex(parts[6], 64))
                        return AddResult::MALFORMED_VALIDATOR_OP;
                    if (parts[4] == parts[6])
                        return AddResult::MALFORMED_VALIDATOR_OP;
                    if (!canonical_lower_hex(parts[5], 6618) ||
                        !canonical_lower_hex(parts[7], 6618) || parts[5] == parts[7])
                        return AddResult::MALFORMED_VALIDATOR_OP;
                    if (parts[8] != "v1")
                        return AddResult::MALFORMED_VALIDATOR_OP;
                ...
                if (data.size() != pfx_len + PUBKEY_HEX_LEN)
                    return AddResult::MALFORMED_VALIDATOR_OP;
                if (!canonical_lower_hex(data.substr(pfx_len), PUBKEY_HEX_LEN))
                    return AddResult::MALFORMED_VALIDATOR_OP;
            }
        }

        {
            ...
            if (pos > VAULT_DISTRIBUTION_INTERVAL - VALIDATOR_STATE_COOLDOWN) {
                ...
                for (const auto& out : tx.outputs) {
                    ...
                    if (offset + len > out.script_pubkey.size())
                        continue;
                    std::string data(out.script_pubkey.begin() + offset,
                                     out.script_pubkey.begin() + offset + len);
        ...
        for (const auto& op : stake_ops) {
            if (!stake_addresses_in_tx.insert(op.address).second)
                continue;
        ...
        for (const auto& op : stake_ops) {
            ...
            if (already_staked == 0)
                new_stakers_in_tx.insert(staker);
        ...
        if (!new_stakers_in_tx.empty()) {
            ...
            const size_t active_stakers = chain.GetActiveStakeAddressCount();
            if (active_stakers > MAX_VAULT_PAYOUT_STAKERS ||
                pending_new_stakers > MAX_VAULT_PAYOUT_STAKERS - active_stakers ||
                new_stakers_in_tx.size() >
                    MAX_VAULT_PAYOUT_STAKERS - active_stakers - pending_new_stakers) {
                return AddResult::INVALID;
            }
        }

        if (token_family && token_marker_count_ >= MAX_TOKEN_MEMPOOL_ENTRIES)
            return AddResult::FULL;
        ...
        // exits FULL.
        while (entries_.size() >= MAX_TX_COUNT || total_bytes_ + tx_size > MAX_MEMPOOL_BYTES) {
        ...
        entry.token_authorization_validated =
            token_family && !token_is_mspv && token_policy_checked && token_authorization_proven;
  ...
  public:
    ...
    template <typename UtxoCheckFn> size_t PurgeInvalidAgainst(UtxoCheckFn utxo_exists) {
        std::lock_guard<std::mutex> lock(mutex_);
    ...
    SummaryPage GetSummaryPage() const {
        ...
        for (const auto& entry : entries_) {
            ...
            if (selected.size() > MAX_SUMMARY_ROWS)
                selected.erase(selected.begin());
        }
        page.rows.reserve(selected.size());
        for (auto it = selected.rbegin(); it != selected.rend(); ++it) {
            const auto& [key, entry] = *it->second;
            TransactionSummary row{key,
                                   entry.fee_units,
                                   entry.tx.TotalOutput(),
                                   static_cast<size_t>(entry.tx_size_bytes),
                                   entry.tx.inputs.size(),
            ...
            for (const auto& output : entry.tx.outputs) {
                ...
                if (sp[off] <= 75)
                    ...
                    ++off;
                    length = sp[off] | (sp[off + 1] << 8);
                    off += 2;
                } else
                    continue;
                if (length > sp.size() - off)
                    continue;
                // Every Explorer category is identified by at most 32 bytes.
                row.op_return_prefixes.emplace_back(
                    sp.begin() + off, sp.begin() + off + std::min<size_t>(length, 32));
            }
    ...
    bool HasNmsSubmission(const std::vector<uint8_t>& script, const Hash256& parent) const {
        std::lock_guard<std::mutex> lock(mutex_);
    ...
    std::vector<Transaction> GetBlockTransactions(size_t max_count = MAX_TRANSACTIONS_PER_BLOCK,
        ...
        if (chain)
            PurgeInvalidAmmEntriesLocked(*chain,
        ...
        for (const auto& key : keys)
            result.push_back(entries_.at(key).tx);
    ...
    std::optional<Transaction> GetSpender(const Hash256& prev_tx_hash,
        ...
        const std::string spend_key =
            HashToHex(prev_tx_hash) + ":" + std::to_string(prev_out_index);
        auto sit = spent_by_tx_.find(spend_key);
        if (sit == spent_by_tx_.end())
            return std::nullopt;
        auto eit = entries_.find(sit->second);
        if (eit == entries_.end() || HashToHex(eit->second.tx.GetTxID()) != sit->second) {
            return std::nullopt;
        }
    ...
    GetBlockTransactionsWithFees(size_t max_count = MAX_TRANSACTIONS_PER_BLOCK,
                                 size_t max_bytes = MAX_BLOCK_SIZE,
                                 const Blockchain* chain = nullptr) {
        std::lock_guard<std::mutex> lock(mutex_);
    ...
    size_t RemoveStale(const Blockchain& chain) {
        ...
        for (auto& [key, entry] : entries_) {
            ...
            for (const auto& inp : entry.tx.inputs) {
                ...
                if (chain_utxo) {
                    ...
                    if (chain_utxo->is_coinbase &&
                        chain_utxo->block_height >= COINBASE_MATURITY_ACTIVATES_AT_HEIGHT &&
            ...
            if (!invalid && !entry.tx.IsCoinbase() && input_total < entry.tx.TotalOutput())
                invalid = true;
    ...
    size_t ExpireOld() {
        ...
        for (const auto& [key, entry] : entries_) {
            ...
            if (now >= entry.added_time && now - entry.added_time > TX_EXPIRY_SECONDS)
    ...
    std::optional<uint64_t> ResolveInputTotal(const Transaction& tx,
        ...
        for (const auto& inp : tx.inputs) {
            if (inp.IsCoinbase())
                return std::nullopt;
            uint64_t value = 0;
            auto canonical = chain.GetUTXO(inp.prev_tx_hash, inp.prev_out_index);
            if (canonical) {
  ...
  private:
    ...
    size_t PurgeInvalidAmmEntriesLocked(const Blockchain& chain, bool validate_tokens = true) {
        if (entries_.empty()) {
            pending_amm_tx_.clear();
            return 0;
        }
        ...
        for (const auto& [key, entry] : entries_) {
            for (const auto& inp : entry.tx.inputs) {
                ...
                if (entries_.count(parent))
                    children[parent].push_back(key);
            }
        }
        ...
        for (auto fit = fee_index_.rbegin(); fit != fee_index_.rend(); ++fit) {
            ...
            std::optional<TokenOpData> token_op;
            const bool token_shape_ok =
                ExtractCanonicalTokenMarker(tx, token_family, token_op, token_is_mspv);

            bool invalid = parsed.status == AmmOpParseStatus::INVALID ||
                           (token_family && !token_shape_ok) ||
                           (token_family &&
                            !TokenTransactionEnvelopeWithinBounds(tx, eit->second.tx_size_bytes)) ||
                           TxComposesMultipleProtocols(tx) || TxHasInvalidTokenMarkerSet(tx);
            ...
            if (has_pool_output && parsed.status != AmmOpParseStatus::VALID) {
                invalid = true;
            }
            ...
            for (const auto& inp : tx.inputs) {
                ...
                if (pit == entries_.end() || inp.prev_out_index >= pit->second.tx.outputs.size()) {
                    continue; // generic stale/orphan sweep owns this case
                }
                if (IsAmmPoolScript(pit->second.tx.outputs[inp.prev_out_index].script_pubkey)) {
            ...
            if (!invalid && touches &&
                (!chain.ValidateTransactionLocking(tx, false) ||
                 !chain.ValidateAmmMempoolCandidate(tx, next_height))) {
                invalid = true;
            }
        ...
        if (validate_tokens && !token_validation_txs.empty()) {
            const auto accepted = chain.FilterTokenMempoolCandidates(
                token_validation_txs, token_validation_authorized, next_height, next_bits);
            if (accepted.size() != token_validation_keys.size()) {
                ...
                dead.insert(token_validation_keys.begin(), token_validation_keys.end());
            } else {
                for (size_t i = 0; i < accepted.size(); ++i) {
                    if (!accepted[i])
                        dead.insert(token_validation_keys[i]);
                }
            }
        }
        ...
        if (!retained_amm.empty() && entries_.count(retained_amm)) {
            ...
            for (const auto& [key, entry] : entries_) {
                if (ParseAmmOpDetailed(entry.tx).status == AmmOpParseStatus::VALID) {
                    pending_amm_tx_ = key;
                    break;
                }
            }
    ...
    std::vector<std::string> SelectBlockEntryKeysLocked(size_t max_count, size_t max_bytes,
                                                        const Blockchain* chain) const {
        ...
        auto try_select = [&](const std::string& key) -> bool {
            ...
            if (bytes > max_bytes || entry.tx_size_bytes > max_bytes - bytes)
                return false;
            ...
            for (const auto& inp : entry.tx.inputs) {
                if (inp.IsCoinbase())
                    continue;
                if (chain && chain->GetUTXO(inp.prev_tx_hash, inp.prev_out_index))
                    continue;
                ...
                if (chain)
                    ready = false;
                else if (!selected.count(parent_key))
                    ready = false;
            }
            if (invalid || !ready)
                return false;
        ...
        if (chain) {
            const std::string reserved_amm_token_user =
                amm_selected ? pending_amm_token_user : std::string{};
            ...
            for (auto fit = fee_index_.rbegin(); fit != fee_index_.rend(); ++fit) {
                ...
                if (const auto nms = ExtractNmsFromTx(eit->second.tx)) {
                    ...
                    if (!chain->NmsMatchesNextBlockContext(*nms) ||
                        ...
                        candidate_nms_miners.count(nms_miner) || bytes > max_bytes ||
                        eit->second.tx_size_bytes > max_bytes - bytes)
                        continue;
                }
                const bool locking_ok =
                    eit->second.base_locking_validated
                        ? chain->ValidateCachedMempoolLockingContext(eit->second.tx)
                        : chain->ValidateTransactionLocking(eit->second.tx, false);
                if (!locking_ok)
                    continue;

                bool is_token = false, is_mspv = false;
                std::optional<TokenOpData> op;
                if (!ExtractCanonicalTokenMarker(eit->second.tx, is_token, op, is_mspv))
                    continue;
                if (!reserved_amm_token_user.empty() && op && op->token_id == BTCVELD_TOKEN_ID &&
                    op->from == reserved_amm_token_user &&
                    (op->action == "TRANSFER" || op->action == "REDEEM"))
                    continue;

                candidate_keys.push_back(key);
                if (!nms_miner.empty())
                    candidate_nms_miners.insert(nms_miner);
                ...
                candidate_token_authorized.push_back(is_token &&
                                                     eit->second.token_authorization_validated);
            }

            const auto choose = chain->SelectTokenAwareMempoolCandidates(
                candidate_txs, candidate_sizes, candidate_is_token, candidate_token_authorized,
                result.size(), bytes, max_count, max_bytes, (uint64_t)chain->Height() + 1,
                chain->ComputeNextBits());
            if (choose.size() != candidate_keys.size())
                return result;
        ...
        while (progressed && result.size() < max_count) {
            ...
            for (auto it = fee_index_.rbegin(); it != fee_index_.rend(); ++it) {
                ...
                if (!try_select(key))
                    continue;
                progressed = true;
                if (result.size() >= max_count || bytes >= max_bytes)
                    break;
            }
        }
        return result;
  ...
  public:
    ...
    std::vector<UTXO> GetPendingOutputsForScript(const std::vector<uint8_t>& script) const {
        std::lock_guard<std::mutex> lock(mutex_);
        std::vector<UTXO> out;
        ...
        for (const auto& [key, entry] : entries_) {
            for (const auto& out : entry.tx.outputs) {
                ...
                if (!ParseCanonicalStakeOp(data, op) ||
                    op.action != CanonicalStakeOp::Action::LOCK || op.address != address)
                    continue;
                if (total > UINT64_MAX - op.amount_units)
                    return UINT64_MAX;
    ...
    void InsertUncheckedForTest(const Transaction& tx, uint64_t fee_units = MIN_TX_FEE,
        ...
        for (const auto& input : tx.inputs) {
            ...
            const std::string spend_key =
                HashToHex(input.prev_tx_hash) + ":" + std::to_string(input.prev_out_index);
            if (!test_spends.insert(spend_key).second || spent_by_tx_.count(spend_key))
        ...
        entry.base_locking_validated = assume_base_locking_validated;
        total_bytes_ += entry.tx_size_bytes;
        fee_index_.insert({entry.fee_rate, key});
        entries_[key] = entry;
        bool token_family = false, token_is_mspv = false;
        ...
        (void)ExtractCanonicalTokenMarker(tx, token_family, token_op, token_is_mspv,
                                          &token_payload);
        if (token_family)
            ++token_marker_count_;
        const std::string replay_key =
            TokenReplayReservationKey(token_op, token_is_mspv, token_payload);
        if (!replay_key.empty())
  ...
  private:
    ...
    size_t RemoveWithDescendantsInternal(const std::vector<std::string>& roots) {
        if (roots.empty())
            return 0;
    ...
    void RemoveInternal(const std::string& key) {
        ...
        if (pending_amm_tx_ == key)
            pending_amm_tx_.clear();

        bool token_family = false, token_is_mspv = false;
        std::optional<TokenOpData> token_op;
        std::string token_payload;
        (void)ExtractCanonicalTokenMarker(entry.tx, token_family, token_op, token_is_mspv,
                                          &token_payload);
        ...
        if (!replay_key.empty()) {
            ...
            if (rit != pending_token_replay_counts_.end()) {
                if (rit->second <= 1)
                    pending_token_replay_counts_.erase(rit);
                else
                    --rit->second;
            }
        }
    ...
    bool Evict(uint64_t incoming_fee_rate) {
        ...
        if (lowest->first >= incoming_fee_rate)
            return false;
