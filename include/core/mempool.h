    ...
    AddResult AddImpl_(const Transaction& tx, uint64_t fee_units, uint32_t current_height,
        ...
        for (const auto& out : tx.outputs) {
            ...
            if (op.amount_units == 0 || op.amount_units > MAX_STAKE_UNITS)
                return AddResult::INVALID;
            if (op.action == CanonicalStakeOp::Action::LOCK && op.amount_units < MIN_STAKE_UNITS)
                return AddResult::INVALID;
        ...
        if (!chain.StakeTransactionValid(tx, candidate_height))
            return AddResult::INVALID;
        const uint64_t effective_min_stake = chain.GetEffectiveMinStakeUnits();
        ...
        bool token_policy_checked = false;
        {
            ...
            if (all_inputs_on_chain_pre) {
                if (!chain.ValidateTransactionLocking(tx, false)) {
                    std::cerr << "  [mempool] reject: ValidateTransactionLocking failed "
                                 "(sig/script verify)\n";
                    std::cerr.flush();
            ...
            if (utxo_opt) {
                ...
                if (!all_inputs_on_chain_pre) {
                    const auto input_result = chain.ValidateMempoolCanonicalInput(
                        tx, static_cast<uint32_t>(input_index), *utxo_opt);
                    if (input_result ==
