...
inline std::vector<uint8_t> EncodeBtcHeaderOp(const std::vector<std::array<uint8_t,80>>& hdrs) {
    ...
    for (size_t i = 0; i < n; ++i) p.insert(p.end(), hdrs[i].begin(), hdrs[i].end());
    return p;
}
...
inline bool IsBtcHeaderOp(const uint8_t* p, size_t len) {
    ...
    if (p[0]!='B'||p[1]!='H'||p[2]!='D'||p[3]!='R') return false;
...
inline int ApplyBtcHeaderOp(BtcHeaderChain& ch, const uint8_t* p, size_t len, uint32_t veld_time = 0) {
    if (!IsBtcHeaderOp(p, len)) return 0;
    size_t count = p[4];
    int accepted = 0;
    for (size_t i = 0; i < count; ++i)
        if (ch.SubmitHeader(p + 5 + i * 80, veld_time)) ++accepted;
    return accepted;
}
