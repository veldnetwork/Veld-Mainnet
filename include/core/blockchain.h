...
class Blockchain {
  public:
    ...
    struct BranchHeaderWindow {
        ...
        const BlockHeader* At(uint64_t height) const noexcept {
            if (ASERT_ACTIVATION_HEIGHT != 0 && height == ASERT_ACTIVATION_HEIGHT - 1 &&
                asert_reference)
    ...
    bool BuildBranchHeaderWindowNoLock_(const Hash256& parent_hash, BranchHeaderWindow& out) const {
        ...
        constexpr uint64_t reference_height = ASERT_ACTIVATION_HEIGHT - 1;
        const bool needs_reference =
            ASERT_ACTIVATION_HEIGHT != 0 && parent_height >= reference_height;
        if (needs_reference)
            stop_height = std::min(first, reference_height);
  ...
  public:
    ...
    bool NmsNeedsSubmission(const std::vector<uint8_t>& script, const Hash256& parent) const {
        std::shared_lock<std::shared_mutex> lock(chain_mutex_);
        ...
        if (chain_.size() % COMINE_WINDOW_BLOCKS == 0)
            return false;
    ...
    bool NmsMatchesNextBlockContext(const veld::NmsRecord& rec) const {
        std::shared_lock<std::shared_mutex> lock(chain_mutex_);
        if (chain_.empty() || rec.header.prev_block_hash != chain_.back().GetHash())
            return false;
        CanonicalPowTarget target;
        return DecodeExpectedVeldTarget(rec.header.bits, ComputeNextBitsAtLocked(chain_.size() - 1),
                                        target);
    }
    ...
    bool NmsHasExpiredCanonicalParent(const veld::NmsRecord& rec) const {
        ...
        if (!parent || chain_.empty() || *parent >= chain_.size() - 1)
            return false;
        CanonicalPowTarget target;
        return DecodeExpectedVeldTarget(rec.header.bits, ComputeNextBitsAtLocked(*parent), target);
    }
    ...
    ComputeExpectedEndorsementFlushOutputs(uint64_t boundary_height,
        ...
        for (uint64_t h = window_start; h <= boundary_height; ++h) {
            ...
            for (const auto& btx : history->marker_transactions) {
                ...
                if (!explicit_endorser.empty())
                    addr = explicit_endorser;
                if (addr.empty())
                    continue;
    ...
    ComputeExpectedPoolOutputs_LockHeld(const Hash256& prev_block_hash, uint64_t pool_balance_units,
        ...
        for (const auto& [script_hex, credit] : snapshot) {
            ...
            if (block_height >= BATCH2_HARDENING_HEIGHT) {
                if (!NmsBondSatisfied(script, block_height))
                    continue;
            }
...
#error "VELD_TEST_BRANCH_CONTEXT must never be enabled in a public release"
    ...
    MempoolCanonicalInputResult
    ValidateMempoolCanonicalInput(const Transaction& tx, uint32_t input_index,
    ...
    bool ValidateTransaction(const Transaction& tx, bool allow_coinbase = false,
        ...
        for (size_t i = 0; i < tx.inputs.size(); ++i) {
            ...
            if (IsSha384KeyScript(script_pubkey)) {
                if (!Sha384DestinationsActive(cov_height))
                    return false;
                if (!VerifyInputAgainstScript(tx, static_cast<uint32_t>(i), script_pubkey,
                                              cov_height))
    ...
    bool ValidateCoinbaseOutputs(const Block& block) const {
        ...
        const bool upgraded_fee_only =
            ProtocolUpgradeActive(block.height) && parent_supply == MAX_SUPPLY_UNITS;
        bool is_vault_block =
            !upgraded_fee_only && (block.height > 0 && block.height % VAULT_BLOCK_INTERVAL == 0);
        if (is_vault_block) {
    ...
    bool ValidateCoinbasePolicy(const Block& block,
        ...
        if (block.height == 0)
            return true;
        if (!ValidateCoinbaseOutputs(block) || !ValidateCanonicalCoinbaseSplit(block))
            return false;
        ...
        if ((ProtocolUpgradeActive(block.height) || path == CoinbaseAdmissionPath::Direct) &&
            ...
            return false;
        return ProtocolUpgradeActive(block.height) || path == CoinbaseAdmissionPath::Replay ||
               ValidateLegacyMinerCaps(block);
    }
    ...
    static Transaction
    BuildCanonicalCoinbase(uint64_t height, uint64_t parent_supply, uint64_t fees,
                           const std::vector<uint8_t>& miner_script,
                           const std::vector<std::vector<uint8_t>>& metadata = {}) {
        const auto allocation =
            ComputeCoinbaseAllocation(height, ExpectedBlockSubsidy(height), parent_supply, fees);
        if (!allocation)
            throw std::invalid_argument("invalid coinbase accounting");
    ...
    bool ValidateCanonicalCoinbaseSplit(const Block& block) const {
        ...
        for (size_t ti = 1; ti < block.transactions.size(); ++ti) {
            ...
            for (const auto& inp : tx.inputs) {
                ...
                if (utxo) {
                    if (utxo->value > MAX_SUPPLY_UNITS || tx_in > MAX_SUPPLY_UNITS - utxo->value)
                        return false;
                    tx_in += utxo->value;
                } else {
            ...
            if (this_tx_resolved && tx_in > tx_out) {
                ...
                if (actual_tx_fees > MAX_SUPPLY_UNITS - fee)
                    return false;
                actual_tx_fees += fee;
            }
        }
        ...
        if (total_supply_at_parent > MAX_SUPPLY_UNITS ||
            actual_tx_fees > MAX_SUPPLY_UNITS - effective_reward)
            return false;
        ...
        for (const auto& out : cb.outputs) {
            ...
            if (out.script_pubkey == vault_script) {
                ...
                other_is_canonical_p2pkh =
                    other_is_canonical_p2pkh && IsCanonicalKeyScriptAtHeight(s, block.height);
            }
        }

        const auto allocation =
            ComputeCoinbaseAllocation(block.height, base_reward, total_supply_at_parent, tx_fees);
        if (!allocation)
            return false;
...
#error "local admission timing is test-only"
        ...
        const auto admission_test_phase = [&](const char* phase) {
            if (local_work_required)
                std::cerr << "ADMISSION_TIME h=" << block.height << " phase=" << phase << " ms="
                          << std::chrono::duration_cast<std::chrono::milliseconds>(
                                 std::chrono::steady_clock::now() - admission_test_start)
                                 .count()
                          << std::endl;
        };
#else
        ...
        if (!skip_pow && !is_alt_chain && blk.height > 0 && !ValidateCoinbasePolicy(blk))
        ...
        if (blk.header.prev_block_hash == chain_.back().GetHash()) {
            ...
            if (!skip_pow || ProtocolUpgradeActive(blk.height)) {
                if (!ValidateCoinbasePolicy(blk))
                    return false;
            } else if (!ValidateLegacyMinerCaps(blk))
                return false;
    ...
    bool RollbackTip(bool allow_genesis = false) {
        ...
        try {
            if (chain_.empty())
                ...
                RebuildNmsTallyToHeight_(chain_.size() - 1);
        } catch (...) {
            durability_compromised_.store(true, std::memory_order_release);
  ...
  private:
    ...
    mutable std::unordered_map<std::string, std::shared_ptr<const SettlementHistory>>
        settlement_history_;
    mutable std::deque<std::string> settlement_history_order_;
    mutable size_t settlement_history_bytes_{0};

    std::shared_ptr<const SettlementHistory> SettlementHistoryNoLock_(uint64_t height) const {
        ...
        for (size_t i = 1; i < block.transactions.size(); ++i) {
            ...
            const bool marker =
                std::any_of(tx.outputs.begin(), tx.outputs.end(), [](const TxOutput& output) {
                    return output.script_pubkey.size() >= 2 && output.script_pubkey[0] == 0x6A;
                });
            if (marker) {
        ...
        if (facts->encoded_bytes > SETTLEMENT_HISTORY_BYTES)
            return facts;
        std::lock_guard<std::mutex> guard(settlement_history_mutex_);
        const auto existing = settlement_history_.find(key);
        if (existing != settlement_history_.end())
            return existing->second;
        while (!settlement_history_order_.empty() &&
               (settlement_history_.size() >= SETTLEMENT_HISTORY_ENTRIES ||
                settlement_history_bytes_ > SETTLEMENT_HISTORY_BYTES - facts->encoded_bytes)) {
    ...
    Block LoadCanonicalBlockNoLock_(uint64_t height, size_t body_budget = MAX_BLOCK_SIZE) const {
#ifdef VELD_TEST_HOOKS
  ...
  public:
    ...
    uint64_t GetEffectiveMinStakeUnits(uint64_t inclusion_height) const {
        return effective_min_stake_ ? effective_min_stake_(inclusion_height)
                                    : MinimumStakeAtHeight(inclusion_height);
    }
  ...
  private:
    ...
    ValidateBlockForReplay(const Block& blk, const mining::PowAdmissionContext& pow_admission,
        ...
        if (blk.height > 0 && !ValidateCoinbasePolicy(blk, CoinbaseAdmissionPath::Replay))
  ...
  public:
    ...
    ValidationResult Validate(const Transaction& tx) const {
        ...
        for (uint32_t i = 0; i < tx.inputs.size(); ++i) {
            ...
            bool script_valid = interpreter.Execute(input.script_sig, utxo->script_pubkey, tx, i,
                                                    sctx, &script_hashes);
