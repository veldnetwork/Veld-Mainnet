...
inline ChainWork AddChainWork(const ChainWork& accumulated, const ChainWork& increment) {
    ChainWork result;
    uint64_t carry = 0;
    for (size_t i = 0; i < result.w.size(); ++i) {
        const uint64_t sum = static_cast<uint64_t>(accumulated.w[i]) + increment.w[i] + carry;
        result.w[i] = static_cast<uint32_t>(sum);
        carry = sum >> 32;
    }
    if (carry != 0) {
        throw std::overflow_error("cumulative chainwork exceeded the full uint64-height domain");
    }
    return result;
}
