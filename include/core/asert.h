...
inline bool CalculateASERTBits(uint32_t anchor_bits, int64_t time_delta, uint64_t height_delta,
                               uint32_t spacing, uint32_t half_life, uint32_t& out_bits,
                               uint32_t limit_bits = VELD_POW_LIMIT_BITS) {
    CanonicalPowTarget anchor, limit;
    if (spacing == 0 || half_life == 0 || !DecodeCanonicalVeldTarget(anchor_bits, anchor) ||
        !DecodeCanonicalVeldTarget(limit_bits, limit) || anchor.value > limit.value)
        return false;

    // A 64-bit height times a 32-bit spacing, then 2^16, fits signed 128 bits.
    const __int128 delta =
        static_cast<__int128>(time_delta) -
        static_cast<__int128>(spacing) * (static_cast<__int128>(height_delta) + 1);
    const __int128 exponent = delta * 65536 / half_life; // truncate toward zero
    if (exponent > 512 * 65536) {
        out_bits = limit_bits;
        return true;
    }
    if (exponent < -512 * 65536) {
    ...
    const uint64_t factor =
        65536 + ((195766423245049ULL * fraction + 971821376ULL * fraction * fraction +
                  5127ULL * fraction * fraction * fraction + (1ULL << 47)) >>
                 48);
    ...
    for (int bit = 0; bit < 288; ++bit) {
        if ((product[bit / 32] & (uint32_t(1) << (bit % 32))) == 0)
            continue;
        ...
        if (destination >= 0)
            result.setbit(static_cast<int>(destination));
    }
    if (result.IsZero())
        result = btcspv::U256::FromU64(1);
