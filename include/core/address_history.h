...
inline std::string EntryType(const Transaction& tx, uint64_t input, int64_t net,
                             std::string_view marker, std::string_view funding_address = {}) {
    if (tx.IsCoinbase())
        return "coinbase";
...
inline bool AppendBlock(const Block& block, const OutputLookup& previous_output_lookup,
    ...
    for (size_t tx_pos = 0; tx_pos < block.transactions.size(); ++tx_pos) {
        ...
        const std::string funding_address = single_funding_script && funding_script
                                                ? ScriptToAddress(*funding_script)
                                                : std::string{};
        ...
        for (const auto& [script_hex, effect] : effects) {
            ...
            for (size_t i = 0; i < script_hex.size(); i += 2)
                script.push_back(
                    static_cast<uint8_t>((nibble(script_hex[i]) << 4) | nibble(script_hex[i + 1])));
            const std::string type = EntryType(tx, effect.input, net, marker, funding_address);
