...
struct U256 {
    ...
    U256 operator+(const U256& o) const {
        ...
        uint64_t carry = 0;
        for (int i = 0; i < 8; ++i) {
            uint64_t s = (uint64_t)w[i] + o.w[i] + carry;
            r.w[i] = (uint32_t)s;
            carry = s >> 32;
        }
        return r;
    ...
    U256 operator-(const U256& o) const {
        ...
        int64_t borrow = 0;
        for (int i = 0; i < 8; ++i) {
            int64_t d = (int64_t)w[i] - o.w[i] - borrow;
            if (d < 0) {
                d += (int64_t)1 << 32;
                borrow = 1;
            } else
    ...
    U256 mulU32(uint32_t m, bool* overflow = nullptr) const {
        ...
        uint64_t carry = 0;
        for (int i = 0; i < 8; ++i) {
            uint64_t p = (uint64_t)w[i] * m + carry;
            r.w[i] = (uint32_t)p;
            carry = p >> 32;
        }
        if (overflow)
    ...
    U256 divU32(uint32_t m, uint32_t* rem = nullptr) const {
        ...
        uint64_t r = 0;
        for (int i = 7; i >= 0; --i) {
            uint64_t cur = (r << 32) | w[i];
            q.w[i] = (uint32_t)(cur / m);
            r = cur % m;
        }
        if (rem)
            *rem = (uint32_t)r;
    ...
    U256 shlBits(unsigned n) const {
        ...
        for (int i = 7; i >= 0; --i) {
            ...
            uint64_t v = 0;
            if (src >= 0)
                v |= (uint64_t)w[src] << bitShift;
            if (bitShift && src - 1 >= 0)
                v |= (uint64_t)w[src - 1] >> (32 - bitShift);
            r.w[i] = (uint32_t)v;
        }
        return r;
    }
...
inline U256 CompactToTarget(uint32_t bits, bool* negative = nullptr, bool* overflow = nullptr) {
    ...
    if (size <= 3) {
        ...
        r = U256::FromU64(word);
        r = r.shlBits(8 * (size - 3));
    }
    if (negative)
        *negative = (word != 0) && ((bits & 0x00800000u) != 0);
    if (overflow)
        *overflow = (word != 0) &&
                    ((size > 34) || (word > 0xffu && size > 33) || (word > 0xffffu && size > 32));
    return r;
}
...
inline uint32_t TargetToCompact(const U256& t) {
    ...
    uint32_t compact;
    if (size <= 3)
        compact = (uint32_t)(t.low64() << (8 * (3 - size)));
    else
        compact = (uint32_t)(t.shrBits(8 * (size - 3)).low64());
...
inline bool CheckProofOfWork(const uint8_t hdr[80], uint32_t bits, const U256& pow_limit) {
    ...
    if (neg || over || target.IsZero() || target > pow_limit)
        return false;
...
inline U256 BlockWork(const U256& target) {
    ...
    if (tp1.IsZero())
        return U256::Zero(); // target == max → work 0 (never on mainnet)
...
inline uint32_t CalcNextBits(uint32_t old_bits, int64_t actual_timespan, const U256& pow_limit,
...
inline U256 MainnetPowLimit() {
    return CompactToTarget(0x1d00ffffu);
}
