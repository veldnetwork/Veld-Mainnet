...
#if defined(VELD_PUBLIC_RELEASE) || !defined(VELD_TEST_CHAIN_BUILD) ||                             \
    !defined(VELD_LOCAL_TEST_NETWORK)
...
inline bool BtcVeldRelayHex(const std::string& text, std::vector<uint8_t>& out) {
    out.clear();
    if (text.empty() || (text.size() & 1u))
        return false;
...
inline bool BtcVeldRelayPayloadShape(const std::string& op, std::string& family) {
    struct PrefixMagic {
        const char* prefix;
        const char* magic;
    };
    static constexpr PrefixMagic allowed[] = {
        ...
        {btcveld::reserve::PUBLIC_CARRIER_PREFIX, btcveld::reserve::PROOF_MAGIC},
#endif
    };
    ...
    if (!match || op.size() < 12)
        return false;
    const size_t op_max =
        family == "VELD_MSPV|" || family == "VELD_CIA1|" || family == "VELD_CST1|"
            ? btcnull::MAX_MSPV_OP_PAYLOAD_BYTES
            : (family == btcveld::reserve::PUBLIC_CARRIER_PREFIX
                   ? std::char_traits<char>::length(btcveld::reserve::PUBLIC_CARRIER_PREFIX) +
    ...
    if (!BtcVeldRelayHex(encoded, raw) || raw.size() < 4)
        return false;
    if (encoded != BytesToHex(raw))
        return false; // lowercase, one text form
    for (size_t i = 0; i < 4; ++i)
        if (raw[i] != static_cast<uint8_t>(match->magic[i]))
            return false;
    ...
    if (family == "VELD_CST1|") {
        ...
        return decoded && btcveld::proposed_custody::EncodeSettlement(*decoded) == raw &&
               ...
               btcveld::reserve::EncodeProof(claim) == decoded->proof && claim.mint_amount == 0 &&
    ...
    if (family == btcveld::reserve::PUBLIC_CARRIER_PREFIX) {
        btcveld::reserve::Claim claim;
        return btcveld::reserve::DecodeProof(raw.data(), raw.size(), claim) &&
               btcveld::reserve::EncodeProof(claim) == raw;
    }

    // ANCH/MSP3 share the fixed Merkle prefix.
    if (raw.size() < 4u + 32u + 4u + 1u)
        return false;
    const size_t branch_len = raw[40];
    if (branch_len > 32)
        return false;
    const size_t tx_offset = 41u + branch_len * 32u;
    if (tx_offset >= raw.size())
        return false;
    const uint32_t dirs = static_cast<uint32_t>(raw[36]) | (static_cast<uint32_t>(raw[37]) << 8) |
                          (static_cast<uint32_t>(raw[38]) << 16) |
                          (static_cast<uint32_t>(raw[39]) << 24);
    if (branch_len < 32 && (dirs >> branch_len) != 0)
        return false;
    if (family == "VELD_MSPV|") {
        if (tx_offset + 4u > raw.size())
            return false;
        ...
        if (!btcnull::CUSTODY_LINEAGE_REQUIRED) {
            ...
            return nullifier_len >= btcnull::MIN_PROOF_BYTES &&
                   ...
                   btcnull::DecodeProof(raw.data() + off, nullifier_len, nullifier);
        }
        if (off + 2 > raw.size())
            return false;
        const uint16_t parent_count =
            static_cast<uint16_t>(raw[off]) | (static_cast<uint16_t>(raw[off + 1]) << 8);
        off += 2;
...
inline std::string BtcVeldRelayTemplatePolicy(const Transaction& tx,
                                              const std::vector<uint8_t>& fee_key_p2pkh,
                                              std::string& family_out) {
    if (tx.inputs.empty())
        return "relay transaction has no inputs";
    int relay_markers = 0;
    ...
    for (size_t oi = 0; oi < tx.outputs.size(); ++oi) {
        ...
        if (!script.empty() && script[0] == 0x6a) {
            ...
            if (off >= script.size())
                return "relay OP_RETURN has no push";
            ...
            if (push <= 75) {
                ...
                pushed =
                    static_cast<size_t>(script[off]) | (static_cast<size_t>(script[off + 1]) << 8);
                off += 2;
                if (pushed <= 255)
