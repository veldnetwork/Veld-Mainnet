...
#else
constexpr uint64_t BTCVELD_SPV_ACTIVATION_HEIGHT =
    1; // fresh-genesis : ARMED (trust-min SPV mint from block 1)
constexpr uint64_t BTCVELD_ISSUER_MINT_SUNSET_HEIGHT =
    0; // 0 == issuer mint never sunset (custodial fallback stays)
...
#if defined(VELD_BTCVELD_REGTEST)
        std::vector<uint8_t> configured = BtcVeldHex_(VELD_L3_DISPOSABLE_BTCVELD_CUSTODY_SPK_HEX);
        const bool keyhash =
            configured.size() == 22 && configured[0] == 0x00 && configured[1] == 0x14;
        const bool taproot =
            configured.size() == 34 && configured[0] == 0x51 && configured[1] == 0x20;
        if (!keyhash && !taproot)
            configured.clear();
        return configured;
#else
...
inline btcspv::BtcCheckpoint BtcVeldCheckpoint() {
    ...
    cp.chain_work =
        btcspv::U256::Zero(); // relative fork-choice from the checkpoint; baseline irrelevant
#if defined(VELD_BTCVELD_REGTEST)
#if defined(VELD_FIRST_ACTIVATION_NETWORK)
#if !defined(VELD_LOCAL_TEST_NETWORK) || !defined(VELD_TEST_HOOKS) ||                              \
    !defined(VELD_TEST_CHAIN_BUILD) || !defined(VELD_REGTEST_FIXED_DIFF) ||                        \
...
#error "first activation Bitcoin checkpoint requires the isolated local regtest profile"
    ...
    cp.time = 1296688602;
    {
        ...
        for (size_t i = 0; i < 32 && i < hb.size(); ++i)
            cp.hash[i] = hb[i];
    }
    for (int i = 0; i < 10; ++i)
        cp.prev10_times[i] = cp.time;
#endif
#else
