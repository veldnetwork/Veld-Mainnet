...
struct Checkpoint {
    ...
    Checkpoint(uint64_t h, const Hash256& bh, std::vector<uint8_t> sig, uint64_t sat = 0)
        : height(h), block_hash(bh), signature(std::move(sig)), signed_at(sat) {}
};

inline Hash256 ComputeCheckpointDigest(uint64_t height, const Hash256& block_hash,
                                       uint64_t signed_at = 0) {
...
inline bool VerifyCheckpoint(const Checkpoint& cp) {
    if (cp.signature.empty())
        return false;
    if (HashIsZero(cp.block_hash))
        return false;
#ifdef VELD_MAINNET_POW
    if (cp.signed_at < GENESIS_TIME)
        return false;
#endif
    Secp256k1PubKey pubkey{};
    if (!LoadFleetCheckpointPubKey(pubkey))
        return false;
    Hash256 digest = ComputeCheckpointDigest(cp.height, cp.block_hash, cp.signed_at);
...
class CheckpointStore {
  public:
    ...
    std::vector<Checkpoint> SinceHeight(uint64_t min_height) const {
        ...
        for (auto it = by_height_.lower_bound(min_height); it != by_height_.end(); ++it) {
    ...
    std::vector<Checkpoint> UpToHeight(uint64_t max_height) const {
        std::shared_lock lock(mtx_);
        std::vector<Checkpoint> out;
        for (auto it = by_height_.begin(); it != by_height_.end() && it->first <= max_height;
             ++it) {
            out.push_back(it->second);
        }
        return out;
...
inline std::vector<Checkpoint> ParseCheckpointsJson(const std::string& json) {
    std::vector<Checkpoint> out;
    if (json.size() > kMaxCheckpointDocumentBytes)
        return out;
    ...
    while (cursor < json.size()) {
        ...
        if (obj_end == std::string::npos)
            break;
        const std::string_view obj(json.data() + obj_start, obj_end - obj_start + 1);
        cursor = obj_end + 1;
        ...
        auto find_in_obj = [&](const std::string& name, size_t max_length,
            ...
            if (pos == std::string_view::npos)
                return false;
            if (obj.find(needle, pos + needle.size()) != std::string_view::npos)
                return false;
            pos += needle.size();
            while (pos < obj.size() &&
            ...
            if (obj[pos] == '"') {
                ...
                if (end == std::string_view::npos || end - pos > max_length)
                    return false;
                ...
                while (end < obj.size() && (obj[end] == ' ' || obj[end] == '\t' ||
                                            obj[end] == '\n' || obj[end] == '\r'))
                    ++end;
                if (end >= obj.size() || (obj[end] != ',' && obj[end] != '}'))
                    return false;
            } else {
                size_t end = pos;
                while (end < obj.size() && (obj[end] >= '0' && obj[end] <= '9'))
                    ++end;
                if (end == pos || end - pos > max_length)
                    return false;
                value.assign(obj.data() + pos, end - pos);
                while (end < obj.size() && (obj[end] == ' ' || obj[end] == '\t' ||
                                            obj[end] == '\n' || obj[end] == '\r'))
        ...
        if (!find_in_obj("height", 20, h_str))
            continue;
        if (!find_in_obj("hash", 64, hash_hex))
            continue;
        if (!find_in_obj("sig", 6618, sig_hex))
            continue;
        Checkpoint cp;
        if (!ParseCanonicalUint64Text(h_str, cp.height))
            continue;
        ...
        if (!canonical_hex(hash_hex, 64) || !canonical_hex(sig_hex, 6618))
            continue;
        ...
        if (cp.signature.empty())
            continue;
        std::string sat_str;
        const bool has_signed_at = obj.find("\"signed_at\"") != std::string_view::npos;
        if (has_signed_at) {
            if (!find_in_obj("signed_at", 20, sat_str) ||
                !ParseCanonicalUint64Text(sat_str, cp.signed_at))
                continue;
        }
