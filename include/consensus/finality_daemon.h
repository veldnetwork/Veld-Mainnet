...
struct DaemonHooks {
    ...
    std::function<std::optional<DaemonWorkGrant>(const CheckpointRef&)>
                                                               authorize_work;
    ...
    std::function<bool(const SignedVote&, const DaemonWorkGrant&)>
                                                               gossip_vote;
    ...
    // precommit.
    std::function<std::optional<DecodedQc>(const EpochSnapshot&)>
                                                               fetch_prevote_qc;
...
public:
    DaemonGrantRelease(
            const std::function<void(const DaemonWorkGrant&)>& cancel,
...
public:
    FinalityDaemon(std::string pubkey_hex, dilithium::SecretKey sk,
                   uint32_t network_id, Hash256 genesis_hash,
                   DaemonHooks hooks)
        : voter_(std::move(pubkey_hex), sk),
        ...
        if (hooks_.load_journal) {
            if (auto j = hooks_.load_journal()) {
                if (j->version != 1 ||
                    (j->lock.held &&
                     (!IsScheduledCheckpoint(j->lock.target.height) ||
                    ...
                    (j->last_vote &&
                     (!IsScheduledCheckpoint(j->last_vote->target.height) ||
                      EpochOf(j->last_vote->target.height) !=
                          j->last_vote->epoch_id ||
                      CheckpointRound(j->last_vote->target.height) !=
                          j->last_vote->round)) ||
                    (j->last_vote &&
                     j->last_vote->pubkey_hex != voter_.pubkey_hex()) ||
                    (j->last_vote &&
                     j->last_vote->phase == Phase::PRECOMMIT &&
                     (!j->lock.held ||
                      j->lock.epoch_id != j->last_vote->epoch_id ||
    ...
    bool Tick() {
        ...
        if (!snap || !SnapshotQualifies(*snap) ||
            snap->epoch_id != EpochOf(target_h)) return false;
        ...
        if (!target_hash) return false;
        CheckpointRef target{target_h, *target_hash};
        if (EpochOf(target.height) != snap->epoch_id ||
            !InVoteWindow(target.height, tip)) return false;
        ...
        const auto finalized = hooks_.fetch_finalized
            ? hooks_.fetch_finalized() : std::nullopt;
        const CheckpointRef source = finalized ? finalized->target
                                               : CheckpointRef{};
        ...
        if (journal_.last_vote) {
            ...
            if (last.epoch_id != snap->epoch_id) {
                retained_last_snapshot =
                    hooks_.fetch_snapshot(last.epoch_id);
                if (!retained_last_snapshot ||
                    !SnapshotQualifies(*retained_last_snapshot) ||
            ...
            if (!VerifyVote(last, *last_snapshot,
                            network_id_, genesis_hash_)) {
                journal_invalid_ = true;
                return false;
            }

            const bool same_frame = last.epoch_id == snap->epoch_id &&
                                    last.round == round;
            if (!same_frame &&
                !ConsensusRoundNewer(snap->epoch_id, round,
                                     last.epoch_id, last.round))
                return false;

            const bool same_claim = same_frame &&
                                    last.source == source &&
                                    last.target == target;
            if (same_frame && !same_claim) return false;
            if (same_claim && last.phase == Phase::PRECOMMIT)
            {
                if (!journal_vote_delivered_) {
                    ...
                    journal_vote_delivered_ =
                        hooks_.gossip_vote(last, *grant);
                    if (journal_vote_delivered_) release.Complete();
            ...
            if (same_claim && last.phase == Phase::PREVOTE &&
                !(latest_prevote_qc_ &&
                  latest_prevote_qc_->qc.epoch_id == snap->epoch_id &&
        ...
        if (latest_prevote_qc_ &&
            ...
            auto pc = voter_.Precommit(*snap, round, source, target,
                                       *latest_prevote_qc_, network_id_,
                                       genesis_hash_);
            if (pc) {
                if (!grant->Live()) {
                    return false;
        ...
        auto pv = voter_.Prevote(*snap, round, source, target,
                                 /*unlock_proof*/ std::nullopt,
                                 extends_lock,
                                 network_id_, genesis_hash_);
        if (!pv) {
            return false;
...
private:
    ...
    uint64_t NewestVotableCheckpoint_(uint64_t tip) const {
        ...
        if (cp == 0) return 0;
        if (!IsScheduledCheckpoint(cp)) return 0;
        return cp;
    }
