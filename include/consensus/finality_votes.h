...
inline bool VoteClaimWellFormed(const SignedVote& v,
                                uint64_t snapshot_epoch,
                                const Hash256& snapshot_root) {
    return v.epoch_id == snapshot_epoch &&
           v.set_root == snapshot_root &&
           (v.phase == Phase::PREVOTE || v.phase == Phase::PRECOMMIT) &&
           IsScheduledCheckpoint(v.target.height) &&
           EpochOf(v.target.height) == v.epoch_id &&
           v.round == CheckpointRound(v.target.height) &&
           SourceRefWellFormed(v.source, v.target) &&
...
public:
    ...
    const SignedVote& Vote() const noexcept { return vote_; }
    uint64_t SnapshotEpoch() const noexcept { return snapshot_epoch_; }
    const Hash256& SnapshotRoot() const noexcept { return snapshot_root_; }
    const Hash256& MemberCommit() const noexcept { return member_commit_; }

private:
    AuthenticatedVote(const SignedVote& vote, uint64_t snapshot_epoch,
                      const Hash256& snapshot_root,
                      const Hash256& member_commit)
        : vote_(vote), snapshot_epoch_(snapshot_epoch),
          snapshot_root_(snapshot_root), member_commit_(member_commit) {}
    ...
    friend std::optional<AuthenticatedVote> AuthenticateVoteForMember(
        const SignedVote&, uint64_t, const Hash256&, const SnapshotEntry&,
...
inline bool HexToPublicKey(const std::string& hex,
                           ::veld::dilithium::PublicKey& out) {
...
inline std::optional<AuthenticatedVote> AuthenticateVoteForMember(
        const SignedVote& v, uint64_t snapshot_epoch,
    ...
    if (commit != member.pubkey_commit ||
        v.pubkey_hex != member.pubkey_hex)
        return std::nullopt;

    const std::vector<uint8_t> msg = VotePreimage(
        network_id, genesis_hash, v.epoch_id, v.set_root,
        v.phase, v.round, v.source, v.target);
    ::veld::dilithium::PublicKey pk{};
    if (!HexToPublicKey(v.pubkey_hex, pk) ||
        !::veld::dilithium::Verify(pk, msg, v.signature))
        return std::nullopt;
...
inline std::optional<AuthenticatedVote> AuthenticateVote(
        const SignedVote& v, const EpochSnapshot& s,
        uint32_t network_id, const Hash256& genesis_hash) {
    if (!VoteClaimWellFormed(v, s.epoch_id, s.root)) return std::nullopt;
    const size_t idx = SignerIndex(s, PubkeyCommit(v.pubkey_hex));
    if (idx == (size_t)-1) return std::nullopt;
    return AuthenticateVoteForMember(v, s.epoch_id, s.root, s.entries[idx],
...
inline bool VerifyVote(const SignedVote& v,
...
inline bool VoteMatchesCanonicalFrame(const SignedVote& v,
                                      const EpochSnapshot& s,
                                      uint64_t tip_height,
                                      const CheckpointRef& canonical_target,
                                      const FinalizedRecord& record) {
    if (!VoteClaimWellFormed(v, s.epoch_id, s.root) ||
        !InVoteWindow(v.target.height, tip_height) ||
        v.target != canonical_target)
        return false;
    if (record.IsNull()) return v.source.IsNull();
    return v.source == record.target &&
           v.target.height > record.target.height;
}
...
inline std::optional<QuorumCert> AssembleQc(const std::vector<SignedVote>& votes,
    ...
    if (!SnapshotWellFormed(s))   return std::nullopt;
    ...
    for (const auto& v : votes) {
        ...
        if (v.phase  != qc.phase  || v.round  != qc.round) return std::nullopt;
        if (v.source != qc.source || v.target != qc.target) return std::nullopt;
        if (!VerifyVote(v, s, network_id, genesis_hash))    return std::nullopt;
    ...
    if (!IsSupermajority(qc.weight, s.total_weight)) return std::nullopt;
    return qc;
}
...
inline bool IsEquivocation(const SignedVote& a, const SignedVote& b) {
    if (a.pubkey_hex != b.pubkey_hex) return false;
    if (a.epoch_id != b.epoch_id)     return false;
