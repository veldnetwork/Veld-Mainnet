...
constexpr const char* DOMAIN_EQUIVOCATION_PAIR =
    "VELD_FINALITY_EQUIVOCATION_PAIR_v1|";
constexpr const char* DOMAIN_EQUIVOCATION_JOURNAL =
    "VELD_FINALITY_EQUIVOCATION_JOURNAL_v1|";
...
public:
    ...
    ValidatedEquivocationEvidence& operator=(
        const ValidatedEquivocationEvidence&) = default;
    ValidatedEquivocationEvidence& operator=(
        ValidatedEquivocationEvidence&&) = default;

    const SignedVote& First() const noexcept { return first_; }
    const SignedVote& Second() const noexcept { return second_; }
    const Hash256& Id() const noexcept { return id_; }
    const Hash256& SignerCommit() const noexcept { return signer_commit_; }
    uint64_t Epoch() const noexcept { return first_.epoch_id; }

private:
    ValidatedEquivocationEvidence(SignedVote first, SignedVote second,
                                  const Hash256& id,
    ...
    static std::optional<ValidatedEquivocationEvidence> MakeCanonical(
            const SignedVote& a, const SignedVote& b) {
        SignedVote first = a;
        SignedVote second = b;
        if (std::lexicographical_compare(second.target.hash.begin(),
                                         second.target.hash.end(),
                                         first.target.hash.begin(),
                                         first.target.hash.end()))
            std::swap(first, second);

        const std::vector<uint8_t> first_wire =
            EncodeSignedVoteWire(first);
        const std::vector<uint8_t> second_wire =
            EncodeSignedVoteWire(second);
        ...
        const Hash256 id = state_digest::sha256_domain(
            DOMAIN_EQUIVOCATION_PAIR, body);
        return ValidatedEquivocationEvidence(
            std::move(first), std::move(second), id,
            PubkeyCommit(a.pubkey_hex));
    }

    friend std::optional<ValidatedEquivocationEvidence>
    ValidateAuthenticatedEquivocationPair(
        const AuthenticatedVote&, const AuthenticatedVote&, uint64_t);
    friend class FinalityEquivocationCollector;
};

static_assert(!std::is_default_constructible<
                  ValidatedEquivocationEvidence>::value,
              "raw votes must not mint finality slash evidence");
static_assert(!std::is_constructible<ValidatedEquivocationEvidence,
                  SignedVote, SignedVote, Hash256, Hash256>::value,
...
inline bool EquivocationClaimsWellFormed(const SignedVote& a,
                                         const SignedVote& b,
                                         uint64_t canonical_tip_height) {
    if (a.pubkey_hex != b.pubkey_hex ||
        a.epoch_id != b.epoch_id ||
        a.set_root != b.set_root ||
        a.phase != b.phase ||
        a.round != b.round)
        return false;
    ...
    if (a.target.height != b.target.height ||
        a.target.hash == b.target.hash)
        return false;

    if (!VoteClaimWellFormed(a, a.epoch_id, a.set_root) ||
        !VoteClaimWellFormed(b, b.epoch_id, b.set_root) ||
        a.target.height > canonical_tip_height ||
...
inline bool FinalityEvidenceVoteWellFormed(
        const SignedVote& vote, uint64_t snapshot_epoch,
        const Hash256& snapshot_root, uint64_t canonical_tip_height) {
    std::vector<uint8_t> canonical_pubkey;
    if (vote.pubkey_hex.size() != ::veld::dilithium::PUBKEY_BYTES * 2 ||
        ...
        return false;
    return canonical_tip_height - vote.target.height <=
...
ValidateAuthenticatedEquivocationPair(const AuthenticatedVote& a,
                                      const AuthenticatedVote& b,
                                      uint64_t canonical_tip_height) {
    if (a.SnapshotEpoch() != b.SnapshotEpoch() ||
        a.SnapshotRoot() != b.SnapshotRoot() ||
        a.MemberCommit() != b.MemberCommit() ||
        a.SnapshotEpoch() != a.Vote().epoch_id ||
        b.SnapshotEpoch() != b.Vote().epoch_id ||
        a.SnapshotRoot() != a.Vote().set_root ||
        b.SnapshotRoot() != b.Vote().set_root ||
        a.MemberCommit() != PubkeyCommit(a.Vote().pubkey_hex) ||
        b.MemberCommit() != PubkeyCommit(b.Vote().pubkey_hex) ||
        !EquivocationClaimsWellFormed(a.Vote(), b.Vote(),
...
ValidateEquivocationPair(const SignedVote& a, const SignedVote& b,
                         const EpochSnapshot& retained_snapshot,
                         uint64_t canonical_tip_height,
                         uint32_t network_id,
    ...
    if (!EquivocationClaimsWellFormed(a, b, canonical_tip_height) ||
        ...
        return std::nullopt;
    const auto authenticated_a = AuthenticateVote(
        a, retained_snapshot, network_id, genesis_hash);
    if (!authenticated_a) return std::nullopt;
    const auto authenticated_b = AuthenticateVote(
        b, retained_snapshot, network_id, genesis_hash);
    if (!authenticated_b) return std::nullopt;
    return ValidateAuthenticatedEquivocationPair(
        *authenticated_a, *authenticated_b, canonical_tip_height);
}
...
inline std::optional<ValidatedEquivocationEvidence>
ValidateEquivocationPairForRetainedMember(
        const SignedVote& a, const SignedVote& b, uint64_t snapshot_epoch,
        const Hash256& snapshot_root, const SnapshotEntry& retained_member,
        uint64_t canonical_tip_height, uint32_t network_id,
        const Hash256& genesis_hash) {
    if (!EquivocationClaimsWellFormed(a, b, canonical_tip_height))
        return std::nullopt;
    const auto authenticated_a = AuthenticateVoteForMember(
        a, snapshot_epoch, snapshot_root, retained_member,
        network_id, genesis_hash);
    if (!authenticated_a) return std::nullopt;
    const auto authenticated_b = AuthenticateVoteForMember(
        b, snapshot_epoch, snapshot_root, retained_member,
        network_id, genesis_hash);
    if (!authenticated_b) return std::nullopt;
    return ValidateAuthenticatedEquivocationPair(
        *authenticated_a, *authenticated_b, canonical_tip_height);
...
public:
    ...
    static constexpr size_t MAX_COMPLETED_PAIR_BYTES =
    ...
    static_assert(MAX_COMPLETED_PAIRS * PAIR_STORAGE_CHARGE_BYTES ==
                      MAX_COMPLETED_PAIR_BYTES,
    ...
    using PairValidator = std::function<
        std::optional<ValidatedEquivocationEvidence>(
    ...
    explicit FinalityEquivocationCollector(
            size_t pair_limit = MAX_COMPLETED_PAIRS,
    ...
    OfferResult Offer(const ValidatedEquivocationEvidence& evidence) {
        ...
        if (existing != records_.end()) {
            return existing->second.id == evidence.Id()
                       ? OfferResult::ALREADY_KNOWN
                       : OfferResult::OFFENSE_ALREADY_STORED;
        }
        if (records_.size() >= pair_limit_ ||
            PAIR_STORAGE_CHARGE_BYTES > byte_limit_ -
                std::min(bytes_charged_, byte_limit_))
    ...
    bool HasOffense(uint64_t epoch, const Hash256& signer_commit) const {
        std::lock_guard<std::mutex> lock(mutex_);
        return records_.find(OffenseKey{epoch, signer_commit}) !=
               records_.end();
    }
    ...
    std::vector<Summary> ListSummaries(size_t limit = 256) const {
        ...
        for (auto it = records_.rbegin();
             it != records_.rend() && out.size() < limit; ++it) {
    ...
    std::vector<Summary> ListSummaryPage(size_t offset,
                                         size_t limit) const {
        std::lock_guard<std::mutex> lock(mutex_);
        std::vector<Summary> out;
        if (offset >= records_.size() || limit == 0) return out;
    ...
    std::optional<ValidatedEquivocationEvidence> FindById(
            const Hash256& id) const {
        std::lock_guard<std::mutex> lock(mutex_);
        for (const auto& item : records_) {
            if (item.second.id == id) return EvidenceFromStored(item.second);
        }
        return std::nullopt;
    }
    ...
    std::vector<uint8_t> Encode() const {
        ...
        state_digest::put_u32_le(out,
            static_cast<uint32_t>(records_.size()));
        for (const auto& item : records_) {
            state_digest::put_bytes(out, item.second.first.data(),
                                    item.second.first.size());
            state_digest::put_bytes(out, item.second.second.data(),
                                    item.second.second.size());
        }
        const Hash256 checksum = state_digest::sha256_domain(
            DOMAIN_EQUIVOCATION_JOURNAL, out);
    ...
    bool Restore(const std::vector<uint8_t>& encoded,
        ...
        const uint32_t count =
            ...
            (static_cast<uint32_t>(encoded[6]) << 16) |
            (static_cast<uint32_t>(encoded[7]) << 24);
        if (count > pair_limit_ || count > MAX_COMPLETED_PAIRS ||
            static_cast<size_t>(count) >
                byte_limit_ / PAIR_STORAGE_CHARGE_BYTES)
            return false;
        const size_t expected = 8 + static_cast<size_t>(count) *
                                      2 * SIGNED_VOTE_WIRE_BYTES + 32;
        if (encoded.size() != expected) return false;

        const std::vector<uint8_t> body(encoded.begin(), encoded.end() - 32);
        const Hash256 expected_checksum = state_digest::sha256_domain(
            DOMAIN_EQUIVOCATION_JOURNAL, body);
        ...
        try {
            for (uint32_t i = 0; i < count; ++i) {
                ...
                StoredPair stored;
                if (!StoredPair::FromEvidence(*evidence, stored) ||
                    !std::equal(stored.first.begin(), stored.first.end(),
                                first_wire.begin()) ||
        ...
        if (offset + 32 != encoded.size()) return false;
...
private:
    ...
    struct StoredPair {
        ...
        static bool FromEvidence(const ValidatedEquivocationEvidence& evidence,
                                 StoredPair& out) {
            const std::vector<uint8_t> first_wire =
                EncodeSignedVoteWire(evidence.First());
            const std::vector<uint8_t> second_wire =
                EncodeSignedVoteWire(evidence.Second());
            if (first_wire.size() != out.first.size() ||
                second_wire.size() != out.second.size())
                return false;
            out.id = evidence.Id();
            ...
            std::copy(second_wire.begin(), second_wire.end(),
                      out.second.begin());
            return true;
        }
    ...
    static std::optional<ValidatedEquivocationEvidence> EvidenceFromStored(
            const StoredPair& stored) {
        const std::vector<uint8_t> first_wire(stored.first.begin(),
                                              stored.first.end());
        const std::vector<uint8_t> second_wire(stored.second.begin(),
                                               stored.second.end());
        const auto first = DecodeSignedVoteWire(first_wire);
        const auto second = DecodeSignedVoteWire(second_wire);
        if (!first || !second) return std::nullopt;
        const auto evidence =
            ValidatedEquivocationEvidence::MakeCanonical(*first, *second);
        if (!evidence || evidence->Id() != stored.id ||
            evidence->SignerCommit() != stored.signer_commit ||
            evidence->Epoch() != stored.epoch)
            return std::nullopt;
        return evidence;
    }

    static_assert(sizeof(StoredPair) + sizeof(OffenseKey) + 512 <=
                      PAIR_STORAGE_CHARGE_BYTES,
...
public:
    ...
    static_assert(MAX_PENDING_VOTES * PENDING_STORAGE_CHARGE_BYTES ==
                      MAX_PENDING_BYTES,
    ...
    explicit FinalityEquivocationDetector(
            size_t vote_limit = MAX_PENDING_VOTES,
    ...
    Observation ObserveAuthenticated(
            const AuthenticatedVote& authenticated,
        ...
        if (authenticated.SnapshotEpoch() != vote.epoch_id ||
            ...
            !FinalityEvidenceVoteWellFormed(
                vote, authenticated.SnapshotEpoch(),
        ...
        const FrameKey key{vote.epoch_id, vote.set_root,
                           static_cast<uint8_t>(vote.phase), vote.round,
                           vote.target.height, signer_commit};
        std::lock_guard<std::mutex> lock(mutex_);
        ...
        if (existing != pending_.end()) {
            ...
            const std::vector<uint8_t> first_wire(
                existing->second.wire.begin(), existing->second.wire.end());
            const auto first = DecodeSignedVoteWire(first_wire);
            if (!first || !FinalityEvidenceVoteWellFormed(
                              *first, vote.epoch_id, vote.set_root,
                              canonical_tip_height))
                return {ObserveResult::STORAGE_ERROR, std::nullopt};
            const auto first_authenticated = AuthenticateVoteForMember(
                *first, vote.epoch_id, vote.set_root, retained_member,
                network_id, genesis_hash);
            ...
            if (!evidence)
                return {ObserveResult::REJECTED, std::nullopt};
            return {ObserveResult::COMPLETED_EVIDENCE,
                    std::move(evidence)};
        }

        if (vote_limit_ == 0 ||
            PENDING_STORAGE_CHARGE_BYTES > byte_limit_) {
            return {ObserveResult::STORAGE_ERROR, std::nullopt};
        }
        ...
        try {
            ...
            if (inserted != pending_.end() &&
                inserted->second.sequence + 1 == next_sequence_)
    ...
    size_t EraseSignerEpoch(uint64_t epoch,
                            const Hash256& signer_commit) {
        std::lock_guard<std::mutex> lock(mutex_);
        size_t removed = 0;
        for (auto it = pending_.begin(); it != pending_.end();) {
            if (it->first.epoch == epoch &&
                it->first.signer_commit == signer_commit) {
...
private:
    struct FrameKey {
        ...
        bool operator<(const FrameKey& other) const {
            if (epoch != other.epoch) return epoch < other.epoch;
            if (set_root != other.set_root) return set_root < other.set_root;
    ...
    void TrimFifoLocked_() {
        while (!fifo_.empty()) {
            ...
            if (found != pending_.end() &&
                found->second.sequence == fifo_.front().second)
                break;
            fifo_.pop_front();
        }
    }
    ...
    static_assert(sizeof(StoredVote) + sizeof(FrameKey) + 512 <=
...
inline std::string ValidatorRegistry::BuildSlashEquivOp(
    ...
    return std::string(VAL_PREFIX) + "SLASH_EQUIV|" + a.pubkey_hex + "|" +
           ...
           std::to_string(static_cast<uint8_t>(a.phase)) + "|" +
           ...
           BytesToHex(a.signature) + "|" +
           std::to_string(b.source.height) + "|" + HashToHex(b.source.hash) + "|" +
           std::to_string(b.target.height) + "|" + HashToHex(b.target.hash) + "|" +
           BytesToHex(b.signature) + "|v1";
}
