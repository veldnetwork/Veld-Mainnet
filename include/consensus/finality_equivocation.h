...
class FinalityEquivocationCollector {
  public:
    ...
    bool RestoreJournalAtTip(const std::vector<uint8_t>& encoded,
                             const PairValidator& validate_pair, uint64_t canonical_tip,
                             uint32_t network_id, const Hash256& genesis_hash) {
        return RestoreJournal_(encoded, validate_pair, canonical_tip, network_id, genesis_hash);
    }

  private:
    bool RestoreJournal_(const std::vector<uint8_t>& encoded, const PairValidator& validate_pair,
                         std::optional<uint64_t> canonical_tip, uint32_t network_id,
                         const Hash256& genesis_hash) {
        ...
        try {
            for (uint32_t i = 0; i < count; ++i) {
                ...
                if (!first || !second)
                    return false;
                const OffenseKey key{first->epoch_id, PubkeyCommit(first->pubkey_hex)};
                ...
                if (canonical_tip && *canonical_tip > first->target.height &&
                    *canonical_tip - first->target.height > FINALITY_EQUIV_EVIDENCE_WINDOW) {
                    if (!EquivocationClaimsWellFormed(*first, *second, *canonical_tip) ||
                    ...
                    for (const auto* vote : {&*first, &*second}) {
                        if (!dilithium::Verify(public_key,
                                               VotePreimage(network_id, genesis_hash,
                                                            ...
                                                            vote->phase, vote->round, vote->source,
                                                            vote->target),
                                               vote->signature))
                            return false;
                    }
        ...
        bytes_charged_ = records_.size() * PAIR_STORAGE_CHARGE_BYTES;
        return true;
    }
