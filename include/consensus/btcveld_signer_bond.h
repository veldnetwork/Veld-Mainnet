...
struct SignerBond {
    ...
    bool active = false;
    uint64_t free() const {
        return bonded_sats > locked_sats ? bonded_sats - locked_sats : 0;
    }
};
...
class SignerBondCovenant {
  public:
    ...
    void BeginBlock(uint64_t height) {
        ...
        for (auto it = signers_.begin(); it != signers_.end();) {
            ...
            if (it->second.locked_sats == 0)
                it = signers_.erase(it);
            else
                ++it; // settlement-only bookkeeping for existing non-reserve requests
        }
    ...
    bool Register(const std::string& addr, uint64_t bond_sats) {
        std::lock_guard<std::recursive_mutex> lock(mutex_);
        if (state_migration_active_ || addr.empty() || bond_sats == 0)
            return false;
        SignerBond& s = signers_[addr];
        if (s.bonded_sats > UINT64_MAX - bond_sats)
            return false;
        s.veld_addr = addr;
        s.bonded_sats += bond_sats;
        return true;
    }
    bool Activate(const std::string& addr) {
        ...
        if (it == signers_.end() || it->second.bonded_sats == 0)
            return false;
    ...
    void AddRequest(const RedeemRequest& r, bool reserve_backed_without_bond = false) {
        std::lock_guard<std::recursive_mutex> lock(mutex_);
        RedeemRequest canonical = r;
        if (HashIsZero(canonical.request_id) || canonical.amount_sats == 0 ||
            canonical.dest_spk.empty() || canonical.veld_recipient.empty() ||
            canonical.deadline_height <= canonical.request_height ||
            canonical.status != ReqStatus::LOCKED_IN || !HashIsZero(canonical.fulfilled_txid)) {
            throw std::runtime_error("invalid btcVELD redemption request admission");
        }

        const H256 expected = RequestCommitment(canonical);
        if (!HashIsZero(canonical.request_commitment) && canonical.request_commitment != expected) {
            throw std::runtime_error("btcVELD redemption request commitment mismatch");
        }
        ...
        if (existing != requests_.end()) {
            ...
            throw std::runtime_error("conflicting btcVELD redemption request replay");
        }
        ...
        if (reserve_backed_without_bond) {
            auto inserted = requests_.emplace(canonical.request_id, std::move(canonical));
            if (!inserted.second)
                throw std::runtime_error("btcVELD reserve request insertion race");
            const H256 request_id = inserted.first->first;
            try {
                if (!reserve_backed_requests_.insert(request_id).second)
                    throw std::runtime_error("btcVELD reserve request accounting conflict");
            } catch (...) {
        ...
        if (!BuildLockPlan_(canonical.amount_sats, lock_plan)) {
            throw std::runtime_error("insufficient free signer bond for btcVELD redemption");
        }
        ...
        RollbackLockPlan_(lock_plan);
        throw std::runtime_error("btcVELD redemption request insertion race");
    }
    ...
    void SetStatus(const H256& id, ReqStatus st) {
        ...
        if (r.status == st)
            return;
        if (r.status == ReqStatus::FULFILLED || r.status == ReqStatus::DEFAULTED)
            return;
        if (st == ReqStatus::FULFILLED)
            throw std::runtime_error("use MarkFulfilled for btcVELD fulfillment");
    ...
    std::optional<RedeemRequest> FindOpenByCommitmentCopy(const H256& commitment) const {
        std::lock_guard<std::recursive_mutex> lock(mutex_);
        if (HashIsZero(commitment))
            return std::nullopt;
        const RedeemRequest* match = nullptr;
        for (const auto& [_id, request] : requests_) {
            const bool open =
                request.status == ReqStatus::LOCKED_IN || request.status == ReqStatus::ASSIGNED ||
                request.status == ReqStatus::SIGNED || request.status == ReqStatus::BROADCAST;
            if (!open || request.request_commitment != commitment)
                continue;
            if (match != nullptr)
                return std::nullopt; // commitments must be unique
            match = &request;
        }
        return match == nullptr ? std::nullopt : std::optional<RedeemRequest>(*match);
    }
    ...
    bool MarkFulfilled(const H256& id, const H256& payout_txid, uint64_t paid_sats,
        ...
        if (paid_sats != r.amount_sats || paid_spk != r.dest_spk)
            return false;
        ReleaseRequestLock_(id, r.amount_sats);
        r.status = ReqStatus::FULFILLED;
        r.fulfilled_txid = payout_txid;
        return true;
    }

    bool MarkAuthorizedReservePayout(const H256& id, const H256& payout_txid, uint64_t paid_sats,
                                     const std::vector<uint8_t>& paid_spk) {
        std::lock_guard<std::recursive_mutex> lock(mutex_);
    ...
    bool ResolveProvenPayout(const H256& id, const H256& payout_txid,
                             const std::vector<btcspv::BtcTxOut>& outs, SlashVerdict& out_v,
        ...
        auto it = requests_.find(id);
        if (it == requests_.end()) {
            out_v.diagnostic = "unknown request";
            return false;
        }
        RedeemRequest& r = it->second;
        if (r.status == ReqStatus::FULFILLED || r.status == ReqStatus::DEFAULTED) {
            out_v.diagnostic = "closed";
            return false;
        }
        ...
        if (REDEEM_PAYOUT_BINDING_REQUIRED &&
            (HashIsZero(request_commitment) || payout_btc_height == UINT64_MAX ||
             !spends_recognized_custody || custody_spk.empty() || proven_input_sats == 0)) {
        ...
        if (supplied_commitment != r.request_commitment) {
            out_v.diagnostic = "payout is not bound to the request commitment";
            return false;
        }
        if (!spends_recognized_custody) {
            out_v.diagnostic = "payout does not spend recognized custody";
            return false;
        }
        if (payout_btc_height <= r.btc_observed_height) {
            out_v.diagnostic = "payout predates the redemption request";
            return false;
        }
        if (IsConsumedFraudSpend(payout_txid)) {
            out_v.diagnostic = "custody transaction already classified as fraudulent";
            return false;
        }
        if (IsConsumedPayout(payout_txid)) {
            out_v.diagnostic = "payout transaction already consumed";
            return false;
        }
        ...
        if (!REDEEM_PAYOUT_BINDING_REQUIRED) {
            ...
            for (const auto& o : outs) {
                ...
                if (paid > UINT64_MAX - o.value) {
                    out_v.diagnostic = "payout amount overflow";
                    return false;
                }
                paid += o.value;
            }
            ...
            if (paid == r.amount_sats) {
                MarkFulfilled(id, payout_txid, paid, r.dest_spk);
                return true;
            }
        ...
        for (const auto& o : outs) {
            if (output_total > UINT64_MAX - o.value) {
            ...
            if (o.value == 0 && o.spk == expected_commitment) {
                ++commitment_outputs;
                continue;
            }
            if (!custody_spk.empty() && o.spk == custody_spk && o.value > 0) {
                ++change_outputs;
                continue;
            }
            canonical_shape = false;
        ...
        canonical_shape = canonical_shape && !custody_spk.empty() && r.dest_spk != custody_spk &&
                          proven_input_sats > 0 && destination_outputs == 1 &&
                          commitment_outputs == 1 && change_outputs <= 1 && paid == r.amount_sats &&
    ...
    SlashVerdict EvalNonPayment(const H256& id, uint64_t current_height,
                                uint64_t penalty_bps = 2000) const {
        std::lock_guard<std::recursive_mutex> lock(mutex_);
        SlashVerdict v;
        const auto it = requests_.find(id);
        if (it == requests_.end()) {
        ...
        if (r.status == ReqStatus::FULFILLED) {
            v.diagnostic = "already fulfilled — no slash";
            return v;
        }
        if (r.status == ReqStatus::DEFAULTED) {
            v.diagnostic = "already defaulted";
            return v;
        }
        if (r.status < ReqStatus::LOCKED_IN) {
            v.diagnostic = "not yet LOCKED_IN";
            return v;
        }
        if (current_height <= r.deadline_height) {
            v.diagnostic = "within honor window";
            return v;
        }
        v.slash = true;
        v.reason = SlashReason::NON_PAYMENT;
        v.compensate_sats = r.amount_sats;
        v.compensate_to = r.veld_recipient;
        const uint64_t penalty = PctOf(r.amount_sats, penalty_bps);
        v.slash_sats = r.amount_sats > UINT64_MAX - penalty ? UINT64_MAX : r.amount_sats + penalty;
        v.diagnostic = "deadline passed, no fulfilled payout";
        return v;
    }

    SlashVerdict EvalFraudulentSpend(uint64_t spent_sats, const std::vector<btcspv::BtcTxOut>& outs,
                                     const std::vector<uint8_t>& custody_spk = {},
                                     uint64_t total_input_sats = 0) const {
        std::lock_guard<std::recursive_mutex> lock(mutex_);
        SlashVerdict v;
        if (spent_sats == 0 || custody_spk.empty()) {
            v.diagnostic = "custody spend value/script unavailable";
            return v;
        }
        const uint64_t accounting_input = total_input_sats == 0 ? spent_sats : total_input_sats;
        if (accounting_input < spent_sats) {
            v.diagnostic = "custody value exceeds proven input value";
            return v;
        }
        ...
        for (const auto& o : outs) {
            ...
            bool matched = false;
            for (const auto& [id, r] : requests_) {
                const bool open = r.status == ReqStatus::LOCKED_IN ||
                                  r.status == ReqStatus::ASSIGNED ||
                                  r.status == ReqStatus::SIGNED || r.status == ReqStatus::BROADCAST;
                if (open && o.value == r.amount_sats && o.spk == r.dest_spk &&
                    matched_requests.insert(id).second) {
                    matched = true;
                    break;
                }
            }
        ...
        if (REDEEM_PAYOUT_BINDING_REQUIRED && marked_requests != matched_requests)
            complete = false;
        ...
        if (REDEEM_PAYOUT_BINDING_REQUIRED && matched_requests.size() > 1)
            complete = false;
        static constexpr uint64_t MAX_PROVEN_PAYOUT_FEE_SATS = 100'000;
        ...
        v.slash = true;
        v.reason = SlashReason::FRAUDULENT_SPEND;
    ...
    SlashVerdict EvalWrongPayout(const H256& id, uint64_t paid_sats,
        ...
        const RedeemRequest& r = it->second;
        if (paid_sats == r.amount_sats && paid_spk == r.dest_spk) {
            v.diagnostic = "payout matches the request — no slash";
            return v;
        }
        v.slash = true;
        v.reason = SlashReason::WRONG_PAYOUT;
        v.compensate_sats = r.amount_sats;
        v.compensate_to = r.veld_recipient;
    ...
    void ApplySlash(const SlashVerdict& v) {
        ...
        if (taken < want && !keys.empty()) {
            ...
            for (const auto& a : keys)
                if (signers_[a].bonded_sats > signers_[big].bonded_sats)
                    big = a;
            uint64_t d = std::min(rem, signers_[big].bonded_sats);
            signers_[big].bonded_sats -= d;
            taken += d;
        }
    ...
    void Reset() {
        ...
        consumed_payouts_.clear();
        consumed_fraud_spends_.clear();
        reserve_backed_requests_.clear();
        insurance_fund_sats_ = 0;
        archive_root_ = {};
    ...
    void RestoreState(const StateSnapshot& s) {
        ...
        consumed_payouts_ = s.consumed_payouts;
    ...
    H256 Digest() const {
        ...
        for (const auto& [addr, s] : signers_) {
            sd::put_len_prefixed(body, addr);
            sd::put_len_prefixed(body, s.veld_addr);
            sd::put_u64_le(body, s.bonded_sats);
            sd::put_u64_le(body, s.locked_sats);
            sd::put_u8(body, s.active ? 1 : 0);
        }
        sd::put_u32_le(body, static_cast<uint32_t>(requests_.size()));
        for (const auto& [id, r] : requests_) {
            sd::put_bytes(body, id.data(), id.size());
            sd::put_bytes(body, r.request_id.data(), r.request_id.size());
            sd::put_u64_le(body, r.amount_sats);
            sd::put_len_prefixed(body, r.dest_spk);
            sd::put_u64_le(body, r.request_height);
            sd::put_u64_le(body, r.deadline_height);
        ...
        for (const auto& payout : consumed_payouts_)
            sd::put_bytes(body, payout.data(), payout.size());
        sd::put_u32_le(body, static_cast<uint32_t>(consumed_fraud_spends_.size()));
        for (const auto& spend : consumed_fraud_spends_)
            sd::put_bytes(body, spend.data(), spend.size());
#if defined(VELD_PUBLIC_MAINNET) || defined(VELD_BTCVELD_REGTEST)
        sd::put_u32_le(body, static_cast<uint32_t>(reserve_backed_requests_.size()));
  ...
  private:
    ...
    static RedeemRequest DecodeRequest_(const std::string& body) {
        ...
        auto number = [&](size_t bytes) {
            ...
            for (size_t i = 0; i < bytes; ++i)
                value |= uint64_t(uint8_t(wire[i])) << (i * 8);
            return value;
        };
        auto hash = [&]() {
            H256 value{};
        ...
        RedeemRequest r;
        r.request_id = hash();
        r.amount_sats = number(8);
        const std::string destination = field();
        r.dest_spk.assign(destination.begin(), destination.end());
        r.request_height = number(8);
        r.deadline_height = number(8);
        r.status = static_cast<ReqStatus>(number(1));
        r.fulfilled_txid = hash();
        r.veld_recipient = field();
        r.request_commitment = hash();
    ...
    static uint64_t PctOf(uint64_t x, uint64_t bps) {
        const unsigned __int128 value = static_cast<unsigned __int128>(x) * bps / 10000;
        return value > UINT64_MAX ? UINT64_MAX : static_cast<uint64_t>(value);
    }

    static bool SameRequest_(const RedeemRequest& a, const RedeemRequest& b) {
        return a.request_id == b.request_id && a.amount_sats == b.amount_sats &&
               a.dest_spk == b.dest_spk && a.request_height == b.request_height &&
               a.deadline_height == b.deadline_height && a.status == b.status &&
               a.fulfilled_txid == b.fulfilled_txid && a.veld_recipient == b.veld_recipient &&
    ...
    void ReleaseLock(uint64_t amount_sats) {
        ...
        if (total_locked < amount_sats)
            throw std::runtime_error("btcVELD redemption lock accounting underflow");
    ...
    void ReleaseRequestLock_(const H256& id, uint64_t amount_sats) {
        if (reserve_backed_requests_.erase(id) != 0)
            return;
