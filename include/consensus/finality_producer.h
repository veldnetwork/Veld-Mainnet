...
public:
    ...
    Lock lock() const { std::lock_guard<std::mutex> g(mu_); return lock_; }
    std::string pubkey_hex() const {
        std::lock_guard<std::mutex> g(mu_); return pubkey_hex_;
    }
    void RestoreLock(const Lock& l) { std::lock_guard<std::mutex> g(mu_); lock_ = l; }
    ...
    // happen.
    std::optional<SignedVote> Prevote(const EpochSnapshot& s,
                                      uint32_t round,
        ...
        if (!MemberOf_(s))                             return std::nullopt;
        if (!IsScheduledCheckpoint(target.height))     return std::nullopt;
        if (round != CheckpointRound(target.height))    return std::nullopt;
        if (target.IsNull())                           return std::nullopt;
        if (!SourceRefWellFormed(source, target))        return std::nullopt;
        std::optional<QuorumCert> verified_unlock;
        if (unlock_proof) {
            DecodedQc proof = *unlock_proof;
            if (!VerifyDecodedQc(proof, s, network_id, genesis_hash) ||
                proof.qc.phase != Phase::PREVOTE) return std::nullopt;
        ...
        if (!LockedPrevoteAllowed(lock_, s.epoch_id, round, target,
                                  verified_unlock,
                                  extends_lock))
            return std::nullopt;
        return Sign_(s, Phase::PREVOTE, round, source, target,
    ...
    // call.
    std::optional<SignedVote> Precommit(const EpochSnapshot& s,
                                        uint32_t round,
                                        const CheckpointRef& source,
                                        const CheckpointRef& target,
                                        DecodedQc prevote,
        ...
        if (prevote_qc.phase != Phase::PREVOTE) return std::nullopt;
        if (prevote_qc.round != round)          return std::nullopt;
        if (prevote_qc.source != source)        return std::nullopt;
        if (prevote_qc.target != target)        return std::nullopt;
        ...
        if (lock_.held && lock_.target != target &&
            !ConsensusRoundNewer(s.epoch_id, round,
                                 lock_.epoch_id, lock_.round))
            return std::nullopt;

        auto v = Sign_(s, Phase::PRECOMMIT, round, source, target,
                       network_id, genesis_hash);
        if (v) {
...
private:
    ...
    std::optional<SignedVote> Sign_(const EpochSnapshot& s, Phase phase,
        ...
        const auto msg = VotePreimage(network_id, genesis_hash, v.epoch_id,
                                      v.set_root, phase, round, source, target);
#ifdef VELD_TEST_HOOKS
...
public:
    ...
    static constexpr size_t MAX_ACCOUNTED_VOTE_BYTES =
        sizeof(SignedVote) + 2u * ::veld::dilithium::PUBKEY_BYTES +
        ::veld::dilithium::SIG_MAX_BYTES;
    static constexpr size_t MAX_LIVE_ENVELOPE_VOTES =
        2u * MAX_FINALITY_VALIDATOR_COUNT;
    static_assert(MAX_LIVE_ENVELOPE_VOTES <= MAX_POOL_VOTES,
                  "finality pool cannot hold fault evidence plus two quorums");
    static_assert(MAX_LIVE_ENVELOPE_VOTES * MAX_ACCOUNTED_VOTE_BYTES <=
                      MAX_POOL_BYTES,
    ...
    // moves the cost.
    bool Offer(const SignedVote& v, const EpochSnapshot& s,
    ...
    bool OfferAuthenticated(const AuthenticatedVote& authenticated,
                            const EpochSnapshot& s) {
        const SignedVote& v = authenticated.Vote();
        if (authenticated.SnapshotEpoch() != s.epoch_id ||
            authenticated.SnapshotRoot() != s.root ||
            !VoteClaimWellFormed(v, s.epoch_id, s.root))
            return false;
        const size_t member_index =
            SignerIndex(s, authenticated.MemberCommit());
        if (member_index == (size_t)-1 ||
            s.entries[member_index].pubkey_hex != v.pubkey_hex)
            return false;
    ...
    bool OfferAuthenticatedCapacityFixture(const SignedVote& v,
                                           const EpochSnapshot& s) {
        if (!VoteClaimWellFormed(v, s.epoch_id, s.root)) return false;
...
private:
    bool StoreAuthenticatedVote_(const SignedVote& v,
                                 const EpochSnapshot& s) {
        ...
        if (have_epoch_ && v.epoch_id + 1 < highest_epoch_) return false;

        const SignerRoundKey srk{v.epoch_id, v.set_root, (uint8_t)v.phase,
                                 v.round, PubkeyCommit(v.pubkey_hex)};
        auto sr = signer_rounds_.find(srk);
        if (sr != signer_rounds_.end()) {
            if (SameVoteClaim_(sr->second.first, v)) return false;
        ...
        const Key k{v.epoch_id, v.set_root, (uint8_t)v.phase, v.round,
                    v.source, v.target};
        auto& bucket = pool_[k];
        bucket.push_back(v);
...
public:
    ...
    bool HasExactVote(const SignedVote& v) const {
        ...
        if (it == signer_rounds_.end()) return false;
        return SameSignedVote_(it->second.first, v) ||
               (it->second.conflict &&
                SameSignedVote_(*it->second.conflict, v));
    }
    ...
    bool HasVoteClaim(const SignedVote& v) const {
        ...
        return SameVoteClaim_(it->second.first, v) ||
               (it->second.conflict &&
                SameVoteClaim_(*it->second.conflict, v));
    }
    ...
    size_t RemoveSignerEpoch(uint64_t epoch,
        ...
        for (auto bucket = pool_.begin(); bucket != pool_.end();) {
            ...
            votes.erase(std::remove_if(
                votes.begin(), votes.end(), [&](const SignedVote& vote) {
                    return PubkeyCommit(vote.pubkey_hex) == signer_commit;
                }), votes.end());
            removed += before - votes.size();
            if (votes.empty()) bucket = pool_.erase(bucket);
            else ++bucket;
        }
        for (auto it = signer_rounds_.begin();
             it != signer_rounds_.end();) {
            if (it->first.epoch == epoch &&
                it->first.signer == signer_commit) {
                if (it->second.conflict) ++removed;
    ...
    std::optional<DecodedQc> TryAssemble(const EpochSnapshot& s,
                                         uint32_t network_id,
                                         const Hash256& genesis_hash,
                                         std::optional<Phase> phase = std::nullopt,
                                         const std::function<bool(
                                             const DecodedQc&)>& accept = {}) {
        std::lock_guard<std::mutex> g(mu_);
        for (auto& [k, votes] : pool_) {
            if (k.epoch != s.epoch_id) continue;
            if (phase && k.phase != (uint8_t)*phase) continue;
    ...
    std::optional<std::pair<SignedVote, SignedVote>> FindConflict() const {
        ...
        for (const auto& [k, votes] : signer_rounds_) {
            (void)k;
            if (votes.conflict &&
                IsEquivocation(votes.first, *votes.conflict))
    ...
    void Remove(const QuorumCert& qc) {
        std::lock_guard<std::mutex> g(mu_);
        const Key k{qc.epoch_id, qc.set_root, (uint8_t)qc.phase, qc.round,
                    qc.source, qc.target};
    ...
    void RemoveFinalizedClaim(const QuorumCert& qc) {
        if (qc.phase != Phase::PRECOMMIT) return;
        std::lock_guard<std::mutex> g(mu_);
        const Key precommit{qc.epoch_id, qc.set_root,
                            (uint8_t)Phase::PRECOMMIT, qc.round,
                            qc.source, qc.target};
        const Key prevote{qc.epoch_id, qc.set_root,
                          (uint8_t)Phase::PREVOTE, qc.round,
                          qc.source, qc.target};
...
private:
    struct Key {
        ...
        bool operator<(const Key& o) const {
            ...
            if (set_root != o.set_root) return set_root < o.set_root;
            if (phase != o.phase) return phase < o.phase;
            if (round != o.round) return round < o.round;
            if (source.height != o.source.height) return source.height < o.source.height;
            if (source.hash != o.source.hash) return source.hash < o.source.hash;
            if (target.height != o.target.height) return target.height < o.target.height;
    ...
    void RemoveBucketLocked_(const Key& k) {
        ...
        if (bucket == pool_.end()) return;
        for (const auto& v : bucket->second) {
            const SignerRoundKey srk{
                v.epoch_id, v.set_root, (uint8_t)v.phase, v.round,
                PubkeyCommit(v.pubkey_hex)};
            auto sr = signer_rounds_.find(srk);
    ...
    static bool SameVoteClaim_(const SignedVote& a, const SignedVote& b) {
        return a.epoch_id == b.epoch_id && a.set_root == b.set_root &&
    ...
    static std::optional<QuorumCert> AssembleVerifiedBucket_(
            const Key& k, const std::vector<SignedVote>& votes,
            const EpochSnapshot& s) {
        if (votes.empty() || !SnapshotWellFormed(s) ||
            k.epoch != s.epoch_id || k.set_root != s.root) return std::nullopt;
        QuorumCert qc;
        qc.epoch_id = k.epoch;
        ...
        for (const auto& v : votes) {
            ...
            if (idx == (size_t)-1) return std::nullopt;
            const uint8_t mask = (uint8_t)(1u << (idx & 7));
            if (qc.bitmap[idx >> 3] & mask) return std::nullopt;
    ...
    void PruneBelowEpochLocked_(uint64_t keep_from) {
        for (auto it = pool_.begin(); it != pool_.end(); ) {
            if (it->first.epoch < keep_from) it = pool_.erase(it);
            else ++it;
        }
        for (auto it = signer_rounds_.begin(); it != signer_rounds_.end(); ) {
            if (it->first.epoch < keep_from) it = signer_rounds_.erase(it);
            else ++it;
        }
        for (auto it = evidence_signers_.begin();
             it != evidence_signers_.end(); ) {
            if (it->first.epoch < keep_from) it = evidence_signers_.erase(it);
            else ++it;
        }
