...
inline void put_len_prefixed(std::vector<uint8_t>& buf,
...
inline Hash256 Compose(
        uint64_t height,
        const Hash256& best_block_hash,
        const Hash256& d_utxo,
        const Hash256& d_validators,
        const Hash256& d_staking,
        const Hash256& d_bondyield,
        const Hash256& d_nmstally,
        const Hash256& d_tokens,
        const Hash256& d_supply,
        const Hash256& d_governance,
        const Hash256& d_nms_extended) {
    std::vector<uint8_t> buf;
    static constexpr char DOMAIN[] = "VELD_STATE_DIGEST_v2|";
    put_bytes(buf, reinterpret_cast<const uint8_t*>(DOMAIN),
              sizeof(DOMAIN) - 1);
...
inline Hash256 ComposeV7(
        ...
        const Hash256& d_anchors,
        const Hash256& d_amm,
        const Hash256& d_finality,
        const Hash256& d_redeem_bond) {
    std::vector<uint8_t> buf;
...
namespace tags {
    ...
    inline constexpr const char* VALIDATORS   = "VELD_D_VALIDATORS_v5|"; // complete registry + admission config + exact endorsed hash
    inline constexpr const char* STAKING      = "VELD_D_STAKING_v3|"; // complete state + mutable validation config
    inline constexpr const char* BONDYIELD    = "VELD_D_BONDYIELD_v2|"; // complete ordered escrow tranche state
    inline constexpr const char* NMSTALLY     = "VELD_D_NMSTALLY_v1|";
    inline constexpr const char* TOKENS       = "VELD_D_TOKENS_v2|"; // includes ordered btcVELD tier-work window
    inline constexpr const char* TOKENS_RESERVE_V1 =
        "VELD_D_TOKENS_RESERVE_v1|";
    inline constexpr const char* SUPPLY       = "VELD_D_SUPPLY_v4|"; // accounting, mutable cap/activation config, bounded miner window
    ...
    inline constexpr const char* ANCHORS      = "VELD_D_ANCHORS_v7|"; // staged proof + full permanent BTC/finality floor
    inline constexpr const char* AMM          = "VELD_D_AMM_v3|"; // complete btcVELD AMM pool record + LP ledger
    inline constexpr const char* FINALITY     = "VELD_D_FINALITY_v1|"; // raw high-water + immutable activation/window inputs
    inline constexpr const char* REDEEM_BOND  = "VELD_D_REDEEM_BOND_v1|"; // complete signer-bond/redeem covenant snapshot
}
...
inline Hash256 FinalityDigest(uint64_t final_height,
