...
inline constexpr size_t OPEN_AUTH_PAYLOAD_BYTES = 4 + 1 + 32 + OPEN_AUTH_RECIPIENT_BYTES + 8;
inline constexpr size_t STANDARD_AUTH_PAYLOAD_BYTES = 4 + 1 + 32 + 32 + 8;
static_assert(1 + 2 + OPEN_AUTH_PAYLOAD_BYTES <= MAX_STANDARD_NULL_DATA_SCRIPT_BYTES,
              "OPEN RVS1 must be standard-relay compatible");
static_assert(1 + 2 + STANDARD_AUTH_PAYLOAD_BYTES <= MAX_STANDARD_NULL_DATA_SCRIPT_BYTES,
...
inline Hash256 EmptyTransitionCommitment() {
    ...
    return state_digest::sha256_domain("VELD/BTCVELD/RESERVE_TRANSITIONS/v1/EMPTY",
...
struct State {
    ...
    bool HasOutpoint() const {
        return status != Status::EMPTY && !HashIsZero(reserve_txid) && reserve_vout != NO_VOUT &&
               reserve_value_sats != 0;
    }

    bool Canonical() const {
        if (status != Status::EMPTY && status != Status::ACTIVE && status != Status::FROZEN)
            return false;
        if (HashIsZero(transition_commitment))
            return false;
        if (status == Status::EMPTY) {
            return HashIsZero(reserve_txid) && reserve_vout == NO_VOUT && reserve_value_sats == 0 &&
                   HashIsZero(reserve_bitcoin_block) && surplus_sats == 0 &&
    ...
    bool AccountingHolds(uint64_t circulating_supply) const {
        ...
        if (circulating_supply > UINT64_MAX - open_redemption_principal)
            return false;
        const uint64_t liabilities = circulating_supply + open_redemption_principal;
        if (liabilities > UINT64_MAX - surplus_sats)
            return false;
...
inline std::vector<uint8_t> EncodeState(const State& state) {
    ...
    sd::put_bytes(body, state.reserve_bitcoin_block.data(), state.reserve_bitcoin_block.size());
    sd::put_u64_le(body, state.transition_count);
    sd::put_bytes(body, state.transition_commitment.data(), state.transition_commitment.size());
...
inline Hash256 Digest(const State& state) {
    ...
    return state_digest::sha256_domain("VELD_D_BTCVELD_RESERVE_v1|", body);
}
...
inline bool DecodeState(const uint8_t* payload, size_t size, State& out) {
    ...
    if (!reader.U8(status) || status > static_cast<uint8_t>(Status::FROZEN) ||
        !reader.Hash(decoded.reserve_txid) || !reader.U32(decoded.reserve_vout) ||
        !reader.U64(decoded.reserve_value_sats) || !reader.Hash(decoded.reserve_bitcoin_block) ||
        !reader.U64(decoded.transition_count) || !reader.Hash(decoded.transition_commitment) ||
        !reader.U64(decoded.surplus_sats) || !reader.U64(decoded.open_redemption_principal) ||
        !reader.U64(decoded.processed_veld_height) ||
        !reader.Hash(decoded.processed_veld_block_hash) || !reader.Done())
        return false;
    decoded.status = static_cast<Status>(status);
    if (!decoded.Canonical() ||
        EncodeState(decoded) != std::vector<uint8_t>(payload, payload + size))
        return false;
    out = decoded;
    return true;
}
...
inline bool DecodeProof(const uint8_t* payload, size_t size, Claim& out) {
    ...
    uint8_t operation = 0;
    if (!reader.U8(operation) || !ValidOperation(operation))
        return false;
    out.operation = static_cast<Operation>(operation);
    if (!reader.Hash(out.network_binding) || !reader.Hash(out.prior_commitment) ||
        !reader.Hash(out.prior_reserve_txid) || !reader.U32(out.prior_reserve_vout) ||
        !reader.U64(out.prior_reserve_value) || !reader.U64(out.prior_transition_count) ||
        !reader.Hash(out.new_reserve_txid) || !reader.U32(out.new_reserve_vout) ||
        !reader.U64(out.new_reserve_value) || !reader.Hash(out.bitcoin_txid) ||
        !reader.Hash(out.bitcoin_block))
        return false;
    uint32_t directions = 0;
    uint8_t branch_size = 0;
    if (!reader.U32(directions) || !reader.U8(branch_size) || branch_size > 32 ||
    ...
    if (!reader.Hash(out.exact_commitment) || !reader.U64(out.mint_amount))
        return false;
    uint32_t tx_size = 0;
    if (!reader.U32(tx_size) || tx_size <= 64 || tx_size > btcnull::MAX_MSPV_STRIPPED_TX_BYTES ||
        !reader.Bytes(tx_size, out.bitcoin_tx))
        return false;
    uint16_t parent_count = 0;
    if (!reader.U16(parent_count) || parent_count == 0 || parent_count > MAX_DIRECT_INPUTS)
        return false;
    ...
    for (uint16_t i = 0; i < parent_count; ++i) {
        ...
        if (!reader.Bytes(parent_size, parent))
            return false;
    ...
    if (nullifier_size != 0) {
        if (nullifier_size < btcnull::MIN_PROOF_BYTES ||
            ...
            !btcnull::DecodeProof(reader.Current(), nullifier_size, out.nullifier_proof))
            return false;
        out.has_nullifier_proof = true;
        std::vector<uint8_t> ignored;
        if (!reader.Bytes(nullifier_size, ignored))
            return false;
    }
    return reader.Done();
}
...
inline bool ParseAuthMarker(const btcspv::BtcTxOut& output, AuthMarker& marker) {
    ...
    if ((bytes.size() != OPEN_AUTH_PAYLOAD_BYTES && bytes.size() != STANDARD_AUTH_PAYLOAD_BYTES) ||
        output.spk.size() != bytes.size() + 3 || output.spk[0] != 0x6a || output.spk[1] != 0x4c ||
    ...
    if (!ValidOperation(operation) || operation == static_cast<uint8_t>(Operation::FREEZE))
        return false;
    ...
    if (marker.operation == Operation::OPEN) {
        if (bytes.size() != OPEN_AUTH_PAYLOAD_BYTES)
            return false;
        marker.open_recipient.assign(bytes.begin() + 37,
        ...
        if (!IsCanonicalTokenCreditAddress(marker.open_recipient) ||
            ...
            return false;
        for (unsigned i = 0; i < 8; ++i)
            marker.mint_amount |= static_cast<uint64_t>(bytes[71 + i]) << (8 * i);
    } else {
        if (bytes.size() != STANDARD_AUTH_PAYLOAD_BYTES)
            return false;
        std::memcpy(marker.exact_commitment.data(), bytes.data() + 37, 32);
        for (unsigned i = 0; i < 8; ++i)
            marker.mint_amount |= static_cast<uint64_t>(bytes[69 + i]) << (8 * i);
    }
    return true;
}

inline std::vector<uint8_t> AuthScript(Operation operation, const Hash256& prior_commitment,
                                       const Hash256& exact_commitment, uint64_t mint_amount) {
    ...
    payload.insert(payload.end(), prior_commitment.begin(), prior_commitment.end());
    payload.insert(payload.end(), exact_commitment.begin(), exact_commitment.end());
    for (unsigned i = 0; i < 8; ++i)
        payload.push_back(static_cast<uint8_t>(mint_amount >> (8 * i)));
    std::vector<uint8_t> script{0x6a, 0x4c, static_cast<uint8_t>(payload.size())};
    script.insert(script.end(), payload.begin(), payload.end());
    if (script.size() > MAX_STANDARD_NULL_DATA_SCRIPT_BYTES)
        return {};
    return script;
}
...
inline std::vector<uint8_t> OpenAuthScript(const Hash256& prior_commitment,
                                           const std::string& recipient, uint64_t mint_amount) {
    if (recipient.size() != OPEN_AUTH_RECIPIENT_BYTES || !IsCanonicalTokenCreditAddress(recipient))
        return {};
    std::vector<uint8_t> payload(AUTH_MAGIC, AUTH_MAGIC + 4);
    ...
    for (unsigned i = 0; i < 8; ++i)
        payload.push_back(static_cast<uint8_t>(mint_amount >> (8 * i)));
    if (payload.size() != OPEN_AUTH_PAYLOAD_BYTES)
        return {};
    std::vector<uint8_t> script{0x6a, 0x4c, static_cast<uint8_t>(payload.size())};
    script.insert(script.end(), payload.begin(), payload.end());
    if (script.size() > MAX_STANDARD_NULL_DATA_SCRIPT_BYTES)
...
inline bool ExtractRecipient(const std::vector<btcspv::BtcTxOut>& outputs, std::string& recipient) {
    static const std::string TAG = "btcVELD:";
    size_t matches = 0;
    recipient.clear();
    for (const auto& output : outputs) {
        const std::vector<uint8_t> data = btcspv::ExtractOpReturn(output.spk);
        if (data.size() <= TAG.size() || !std::equal(TAG.begin(), TAG.end(), data.begin()))
            continue;
        ++matches;
        recipient.assign(data.begin() + static_cast<std::ptrdiff_t>(TAG.size()), data.end());
    }
    return matches == 1;
}

inline Hash256 OpenDepositCommitment(uint32_t reserve_vout, uint64_t reserve_value,
    ...
    return state_digest::sha256_domain("VELD/BTCVELD/PENDING_DEPOSIT/v1/OPEN", body);
}

inline Hash256 PendingDepositCommitment(const Hash256& txid, uint32_t vout, uint64_t value,
                                        const std::string& recipient) {
    std::vector<uint8_t> body;
    ...
    state_digest::put_len_prefixed(body, recipient);
    return state_digest::sha256_domain("VELD/BTCVELD/PENDING_DEPOSIT/v1", body);
}
...
inline Hash256 TransitionCommitment(const Claim& claim) {
    ...
    return state_digest::sha256_domain("VELD/BTCVELD/RESERVE_TRANSITION/v1", body);
}
...
inline bool ReadInputFacts(const Claim& claim, InputFacts& facts, std::string& reason) {
    facts = InputFacts{};
    btcspv::WitnessAwareBtcTx spend;
    if (!btcspv::ParseWitnessAwareBtcTx(claim.bitcoin_tx, spend, MAX_DIRECT_INPUTS)) {
        reason = "Bitcoin input serialization is non-canonical";
        return false;
    }
    facts.prevouts = spend.prevouts;
    if (facts.prevouts.size() != claim.direct_parents.size() || facts.prevouts.empty() ||
        facts.prevouts.size() > MAX_DIRECT_INPUTS) {
    ...
    for (size_t i = 0; i < facts.prevouts.size(); ++i) {
        if ((HashIsZero(facts.prevouts[i].txid) && facts.prevouts[i].vout == UINT32_MAX)) {
...
inline bool SumOutputs(const std::vector<btcspv::BtcTxOut>& outputs, uint64_t& total) {
    total = 0;
    for (const auto& output : outputs) {
        if (total > UINT64_MAX - output.value)
            return false;
        total += output.value;
    }
    return true;
...
inline std::vector<uint8_t> EncodeProof(const Claim& claim) {
    ...
    if (!detail::ValidOperation(operation) || claim.merkle_branch.size() > 32 ||
        ...
        claim.direct_parents.empty() || claim.direct_parents.size() > MAX_DIRECT_INPUTS)
        return {};

    size_t parent_total = 0;
    for (const auto& parent : claim.direct_parents) {
        if (parent.size() < 10 || parent.size() > btcnull::MAX_MSPV_PARENT_TX_BYTES ||
            parent_total > btcnull::MAX_MSPV_PARENT_TOTAL_BYTES - parent.size())
            return {};
    ...
    for (const Hash256& sibling : claim.merkle_branch)
        put_hash(sibling);
    ...
    Claim decoded;
    if (out.size() > MAX_PROOF_BYTES || !detail::DecodeProof(out.data(), out.size(), decoded))
        return {};
    return out;
}
...
using PayoutLookup = std::function<bool(const Hash256&, PayoutContext&)>;
...
inline SpendClassification
ClassifyBitcoinReserveSpend(const State& state, uint64_t circulating_supply,
    ...
    if (!state.Canonical() || custody_spk.empty() || !state.AccountingHolds(circulating_supply)) {
    ...
    if (!btcspv::ParseWitnessAwareBtcTx(bitcoin_tx, parsed_tx, MAX_DIRECT_INPUTS) ||
        !detail::ReadInputFacts(facts_claim, inputs, out.reason))
        return out;
    const std::vector<btcspv::BtcTxOut>& outputs = parsed_tx.outputs;
    uint64_t output_total = 0;
    if (!detail::SumOutputs(outputs, output_total) || output_total > inputs.input_total) {
    ...
    if (auth_count != 1 || null_data_output_count != 1 ||
        auth.prior_commitment != state.transition_commitment) {
        out.reason = "reserve spend must carry exactly one state-bound RVS1 null-data output";
        return out;
    }
    ...
    if (state.status == Status::EMPTY) {
        ...
        if (auth.operation != Operation::OPEN || current_inputs != 0 || other_custody_inputs != 0 ||
            custody_output_count != 1 || custody_output_value == 0 ||
            custody_output_value > BTCVELD_SPV_MAX_CUSTODY_SATS ||
            auth.mint_amount > custody_output_value || out.recipient.empty() ||
            !valid_recipient(out.recipient) || out.recipient.size() != OPEN_AUTH_RECIPIENT_BYTES) {
            out.reason = "EMPTY reserve transaction is not a canonical OPEN";
            return out;
        }
        out.exact_commitment =
            detail::OpenDepositCommitment(custody_output_vout, custody_output_value, out.recipient);
        out.new_reserve_txid = out.bitcoin_txid;
        out.new_reserve_vout = custody_output_vout;
        out.new_reserve_value = custody_output_value;
        out.pending_outpoint = btcspv::BtcDepositOutpointId(out.bitcoin_txid, custody_output_vout);
        out.disposition = SpendDisposition::AUTHORIZED_TRANSITION;
        return out;
    }

    if (current_inputs != 1 || current_input_index == std::numeric_limits<size_t>::max() ||
        auth.operation == Operation::OPEN || auth.operation == Operation::FREEZE) {
    ...
    if (auth.operation == Operation::DEPOSIT) {
        if (inputs.prevouts.size() != 2 || other_custody_inputs != 1 || custody_output_count != 1 ||
        ...
        if (!btcspv::ParseWitnessAwareBtcTx(direct_parents[pending_index], pending_parent) ||
            !detail::ExtractRecipient(pending_parent.outputs, out.recipient) ||
        ...
        for (const auto& output : pending_parent.outputs)
            if (output.spk == custody_spk)
                ++pending_custody_outputs;
        const auto& pending_prevout = inputs.prevouts[pending_index];
        const auto& pending_spent = inputs.spent_outputs[pending_index];
        const uint64_t increase = custody_output_value - state.reserve_value_sats;
        if (pending_custody_outputs != 1 || increase > pending_spent.value ||
            auth.mint_amount > increase ||
            auth.exact_commitment !=
                detail::PendingDepositCommitment(pending_prevout.txid, pending_prevout.vout,
        ...
        for (const auto& output : outputs) {
            ...
            if (output.spk == custody_spk ||
                (output.value == 0 && detail::ParseAuthMarker(output, ignored)))
                continue;
        ...
        out.new_reserve_vout = custody_output_vout;
        out.new_reserve_value = custody_output_value;
        out.pending_outpoint =
            btcspv::BtcDepositOutpointId(pending_prevout.txid, pending_prevout.vout);
    } else if (auth.operation == Operation::ROLLOVER) {
        if (other_custody_inputs != 0 || custody_output_count != 1 ||
            custody_output_value != state.reserve_value_sats || auth.mint_amount != 0 ||
            !HashIsZero(auth.exact_commitment)) {
        ...
        if (!payout_lookup || !payout_lookup(auth.exact_commitment, out.payout) ||
            !out.payout.present || out.payout.principal_sats == 0 ||
            out.payout.destination_spk.empty() || other_custody_inputs != 0 ||
            inputs.prevouts.size() < 2 || auth.mint_amount != 0 ||
        ...
        for (const auto& output : outputs) {
            if (output.value > 0 && output.spk == out.payout.destination_spk) {
                ++destination_count;
                destination_paid = output.value;
            }
        }
        const uint64_t successor = state.reserve_value_sats - out.payout.principal_sats;
        if (destination_count != 1 || destination_paid != out.payout.principal_sats ||
            (successor == 0 && custody_output_count != 0) ||
            (successor != 0 && (custody_output_count != 1 || custody_output_value != successor))) {
        ...
        if (other_custody_inputs != 0 || custody_output_count != 0 || auth.mint_amount != 0 ||
            !HashIsZero(auth.exact_commitment) || circulating_supply != 0 ||
...
inline Result Verify(const btcspv::BtcHeaderChain& headers, const State& state,
                     uint64_t circulating_supply, const uint8_t* payload, size_t payload_size,
                     const std::vector<uint8_t>& custody_spk, uint32_t finality_depth,
                     const std::function<bool(const std::string&)>& valid_recipient,
    ...
    if (!state.Canonical() || custody_spk.empty() || !detail::MatchesPrior(state, claim)) {
    ...
    btcspv::WitnessAwareBtcTx parsed_tx;
    if (!btcspv::ParseWitnessAwareBtcTx(claim.bitcoin_tx, parsed_tx, MAX_DIRECT_INPUTS)) {
        result.reason = "reserve Bitcoin transaction is non-canonical";
        return result;
    }
    const Hash256 txid = parsed_tx.txid;
    if (claim.bitcoin_txid != txid ||
        !headers.VerifyMerkle(claim.bitcoin_block, txid, claim.merkle_branch,
                              claim.merkle_directions) ||
        !headers.IsFinalForExternalValue(claim.bitcoin_block, finality_depth)) {
        result.reason = "reserve transaction is not final on the Bitcoin best chain";
        return result;
    }

    const PayoutLookup payout_lookup = [&payout](const Hash256& commitment, PayoutContext& found) {
        if (!payout.present || payout.request_commitment != commitment)
            return false;
        found = payout;
        return true;
    };
    const SpendClassification classification = ClassifyBitcoinReserveSpend(
        state, circulating_supply, claim.bitcoin_tx, claim.direct_parents, custody_spk,
        valid_recipient, payout_lookup);
    if (classification.disposition != SpendDisposition::AUTHORIZED_TRANSITION ||
    ...
    if (auth_count != 1 || null_data_output_count != 1 || auth.operation != claim.operation ||
        auth.prior_commitment != claim.prior_commitment ||
        (claim.operation == Operation::OPEN ? auth.open_recipient != classification.recipient
    ...
    const bool successor_expected =
        ...
        !(claim.operation == Operation::PAYOUT && claim.new_reserve_vout == NO_VOUT);
    if (successor_expected) {
        if (custody_output_count != 1 || claim.new_reserve_txid != txid ||
            claim.new_reserve_vout != custody_output_vout ||
            claim.new_reserve_value != custody_output_value || claim.new_reserve_vout == NO_VOUT ||
            claim.new_reserve_value == 0) {
            result.reason = "reserve successor output mismatch";
            return result;
        }
    } else if (custody_output_count != 0 || !HashIsZero(claim.new_reserve_txid) ||
               claim.new_reserve_vout != NO_VOUT || claim.new_reserve_value != 0) {
    ...
    if (claim.operation == Operation::OPEN) {
        ...
        if (current_inputs != 0 || other_custody_inputs != 0 ||
            claim.prior_reserve_vout != NO_VOUT || !HashIsZero(claim.prior_reserve_txid) ||
            claim.prior_reserve_value != 0 ||
            claim.new_reserve_value > BTCVELD_SPV_MAX_CUSTODY_SATS ||
            claim.mint_amount > claim.new_reserve_value || !claim.has_nullifier_proof ||
            result.recipient.empty() || !valid_recipient(result.recipient) ||
            claim.exact_commitment != detail::OpenDepositCommitment(claim.new_reserve_vout,
                                                                    claim.new_reserve_value,
                                                                    result.recipient)) {
            result.reason = "OPEN does not prove one genuine external deposit";
            return result;
        }
        result.pending_outpoint = btcspv::BtcDepositOutpointId(txid, claim.new_reserve_vout);
        ...
        if (claim.operation == Operation::DEPOSIT) {
            ...
            const size_t pending_index = current_input_index == 0 ? 1 : 0;
            btcspv::WitnessAwareBtcTx pending_parent;
            if (!btcspv::ParseWitnessAwareBtcTx(claim.direct_parents[pending_index],
                                                pending_parent) ||
                !detail::ExtractRecipient(pending_parent.outputs, result.recipient) ||
            ...
            const auto& pending_prevout = inputs.prevouts[pending_index];
            const auto& pending_spent = inputs.spent_outputs[pending_index];
            const uint64_t increase = claim.new_reserve_value - state.reserve_value_sats;
            if (pending_custody_outputs != 1 || increase > pending_spent.value ||
            ...
            result.pending_outpoint =
                btcspv::BtcDepositOutpointId(pending_prevout.txid, pending_prevout.vout);
            result.pending_value = pending_spent.value;
        } else if (claim.operation == Operation::ROLLOVER) {
            if (other_custody_inputs != 0 || claim.new_reserve_value != state.reserve_value_sats ||
            ...
            if (!payout.present || payout.principal_sats == 0 || payout.destination_spk.empty() ||
                other_custody_inputs != 0 || inputs.prevouts.size() < 2 || claim.mint_amount != 0 ||
                claim.has_nullifier_proof || claim.exact_commitment != payout.request_commitment ||
            ...
            uint64_t destination_paid = 0;
            for (const auto& output : outputs) {
                if (output.value > 0 && output.spk == payout.destination_spk) {
                    ++destination_count;
                    destination_paid = output.value;
                }
            }
            const uint64_t expected_successor = state.reserve_value_sats - payout.principal_sats;
            const bool terminal = expected_successor == 0;
            if (destination_count != 1 || destination_paid != payout.principal_sats ||
            ...
            if (other_custody_inputs != 0 || claim.mint_amount != 0 ||
                !HashIsZero(claim.exact_commitment) || claim.has_nullifier_proof) {
...
inline bool SetProcessed(State& state, uint64_t height, const Hash256& block_hash) {
    if (!state.Canonical() || HashIsZero(block_hash) || height < state.processed_veld_height)
        return false;
    if (height == state.processed_veld_height && !HashIsZero(state.processed_veld_block_hash) &&
...
inline bool ApplyAuthorized(State& state, const Result& verified,
                            uint64_t circulating_supply_before, uint64_t circulating_supply_after) {
    if (!verified.ok || verified.claim.operation == Operation::FREEZE ||
        !state.AccountingHolds(circulating_supply_before) ||
        !detail::MatchesPrior(state, verified.claim) || state.transition_count == UINT64_MAX)
        return false;
    const Claim& claim = verified.claim;
    State next = state;
    switch (claim.operation) {
    case Operation::OPEN:
        if (state.status != Status::EMPTY || circulating_supply_after < circulating_supply_before ||
            circulating_supply_after - circulating_supply_before != claim.mint_amount)
            return false;
    ...
    case Operation::DEPOSIT: {
        ...
        if (claim.mint_amount > increase ||
            next.surplus_sats > UINT64_MAX - (increase - claim.mint_amount))
            return false;
        next.reserve_txid = claim.new_reserve_txid;
        next.reserve_vout = claim.new_reserve_vout;
        next.reserve_value_sats = claim.new_reserve_value;
    ...
    case Operation::ROLLOVER:
        if (state.status != Status::ACTIVE || circulating_supply_after != circulating_supply_before)
            return false;
        next.reserve_txid = claim.new_reserve_txid;
        next.reserve_vout = claim.new_reserve_vout;
        next.reserve_value_sats = claim.new_reserve_value;
    ...
    case Operation::PAYOUT: {
        if (state.status != Status::ACTIVE ||
            circulating_supply_after != circulating_supply_before ||
            claim.new_reserve_value > state.reserve_value_sats ||
            state.open_redemption_principal < state.reserve_value_sats - claim.new_reserve_value ||
            verified.payout_principal_sats != state.reserve_value_sats - claim.new_reserve_value)
            return false;
        const uint64_t principal = state.reserve_value_sats - claim.new_reserve_value;
        next.open_redemption_principal -= principal;
        next.reserve_value_sats = claim.new_reserve_value;
        if (verified.terminal) {
            if (circulating_supply_after != 0 || next.open_redemption_principal != 0 ||
                next.surplus_sats != 0)
                return false;
    ...
    case Operation::CLOSE:
        if (state.status != Status::ACTIVE || circulating_supply_before != 0 ||
            circulating_supply_after != 0 || state.open_redemption_principal != 0 ||
            state.reserve_value_sats != state.surplus_sats)
            return false;
        next.status = Status::EMPTY;
        next.reserve_txid = Hash256{};
    ...
    if (!next.AccountingHolds(circulating_supply_after))
        return false;
    state = std::move(next);
    return true;
}

inline bool OpenRedemption(State& state, uint64_t amount, uint64_t circulating_supply_after_burn) {
    ...
    if (!next.AccountingHolds(circulating_supply_after_burn))
        return false;
    state = std::move(next);
    return true;
}

inline bool ResolveDefaultOrCompensation(State& state, uint64_t amount,
