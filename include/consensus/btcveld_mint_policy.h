...
inline bool BtcVeldMintPolicyOutpointId(const std::string& text) {
    ...
    btcnull::Proof proof;
    return btcnull::ParseIssuerMemo(text, outpoint, proof, reservation_request_id) &&
...
inline bool BtcVeldReserveMintPolicyMemo(const std::string& memo,
    ...
    if (proof.empty() || encoded != BytesToHex(proof) ||
        ...
        claim.mint_amount != expected_sats || expected_sats == 0 || !claim.has_nullifier_proof)
        return false;
    btcspv::WitnessAwareBtcTx bitcoin;
    if (!btcspv::ParseWitnessAwareBtcTx(claim.bitcoin_tx, bitcoin,
                                        btcveld::reserve::MAX_DIRECT_INPUTS) ||
        claim.bitcoin_txid != bitcoin.txid || claim.new_reserve_txid != claim.bitcoin_txid ||
        claim.new_reserve_vout == btcveld::reserve::NO_VOUT || claim.new_reserve_value == 0 ||
        claim.new_reserve_value > BTCVELD_SPV_MAX_CUSTODY_SATS || context == nullptr ||
        !context->prior_state.Canonical() ||
        !context->prior_state.AccountingHolds(context->circulating_supply) ||
        !btcveld::reserve::detail::MatchesPrior(context->prior_state, claim))
        return false;
    const auto classified = btcveld::reserve::ClassifyBitcoinReserveSpend(
        context->prior_state, context->circulating_supply, claim.bitcoin_tx, claim.direct_parents,
        BtcVeldCustodySpk(), [&expected_recipient](const std::string& candidate) {
            return candidate == expected_recipient && IsCanonicalTokenCreditAddress(candidate);
        });
    return classified.disposition == btcveld::reserve::SpendDisposition::AUTHORIZED_TRANSITION &&
...
inline std::string
BtcVeldMintTemplatePolicy(const Transaction& tx, const std::vector<uint8_t>& issuer_p2pkh,
                          std::string& from_out, std::string& to_out, uint64_t& sats_out,
                          std::string& memo_out,
                          const BtcVeldReserveMintPolicyContext* reserve_context = nullptr) {
    ...
    for (size_t oi = 0; oi < tx.outputs.size(); ++oi) {
        ...
        if (!s.empty() && s[0] == 0x6A) { // OP_RETURN data carrier
            ...
            size_t k = 1, pushed = 0;
            if (k >= s.size())
                return "OP_RETURN has no data push";
            const uint8_t push = s[k++];
            if (push <= 75) {
                pushed = push;
            } else if (push == 0x4C) {
                if (k >= s.size())
                    return "truncated OP_PUSHDATA1";
                pushed = s[k++];
                if (pushed <= 75)
                    return "non-canonical OP_PUSHDATA1";
            } else if (push == 0x4D) {
                if (k + 2 > s.size())
                    return "truncated OP_PUSHDATA2";
                pushed = static_cast<size_t>(s[k]) | (static_cast<size_t>(s[k + 1]) << 8);
                k += 2;
                if (pushed <= 255)
                    return "non-canonical OP_PUSHDATA2";
            } else {
            ...
            if (payload.rfind(kMint, 0) == 0) { // canonical mint marker AT START
                ...
                size_t p = 0, q;
                while ((q = payload.find('|', p)) != std::string::npos) {
                    f.push_back(payload.substr(p, q - p));
                    p = q + 1;
                }
                ...
                if (f.size() != 7)
                    return "MINT marker field count is not canonical";
                from_out = f[3];
                to_out = f[4];
                ...
                memo_out = f[6];
#if defined(VELD_PUBLIC_MAINNET) || defined(VELD_BTCVELD_REGTEST)
                if (!BtcVeldReserveMintPolicyMemo(memo_out, to_out, sats_out, reserve_context))
                    return "MINT memo or live reserve context is not canonical recipient-bound "
                           "RTP1";
#else
    ...
    if (mint_markers != 1)
        return "expected exactly one canonical MINT OP_RETURN, found " +
               std::to_string(mint_markers);
    if (other_markers != 0)
        return "tx carries " + std::to_string(other_markers) + " other VELD_ protocol marker(s)";
    return "";
}
...
inline std::string BtcVeldC1ReservationTemplatePolicy(const Transaction& tx,
    ...
    for (size_t oi = 0; oi < tx.outputs.size(); ++oi) {
        ...
        if (!script.empty() && script[0] == 0x6a) {
            ...
            if (push <= 75) {
                ...
                if (pos + 2 > script.size())
                    return "truncated OP_PUSHDATA2";
                pushed =
                    static_cast<size_t>(script[pos]) | (static_cast<size_t>(script[pos + 1]) << 8);
                pos += 2;
                if (pushed <= 255)
                    return "non-canonical OP_PUSHDATA2";
            } else {
            ...
            const bool token_c1 = payload.rfind("VELD_TOKEN|RESERVE|btcVELD|", 0) == 0 ||
            ...
            if (token_c1) {
                ...
                size_t begin = 0;
                size_t split = 0;
                while ((split = payload.find('|', begin)) != std::string::npos) {
                    fields.push_back(payload.substr(begin, split - begin));
                    begin = split + 1;
                }
                fields.push_back(payload.substr(begin));
                if (fields.size() != 7 || fields[0] != "VELD_TOKEN" || fields[2] != "btcVELD" ||
                    (fields[1] != "RESERVE" && fields[1] != "EXPOSE" && fields[1] != "CANCEL" &&
                     fields[1] != "FUND"))
                ...
                if (fields[5].empty() || (fields[5].size() > 1 && fields[5][0] == '0'))
                ...
                if (amount < static_cast<uint64_t>(c1reserve::MIN_SATS) ||
                    amount > static_cast<uint64_t>(BTCVELD_ISSUER_MAX_CUSTODY_SATS))
                    return "C1 amount outside consensus range";
                result_out.sats = amount;

                bool memo_ok = false;
                if (result_out.action == "RESERVE") {
                    memo_ok = c1reserve::ParseMemo(fields[6], result_out.allocation_id,
                                                   result_out.allocation_commitment);
                } else if (result_out.action == "EXPOSE") {
                    memo_ok = c1reserve::ParseExposureMemo(fields[6], result_out.allocation_id,
                                                           result_out.allocation_commitment);
                } else if (result_out.action == "CANCEL") {
                    memo_ok = c1reserve::ParseCancellationMemo(fields[6], result_out.allocation_id,
                                                               result_out.allocation_commitment);
                } else {
                    memo_ok = c1reserve::ParseFundingMemo(
                        fields[6], result_out.allocation_id, result_out.fund_script_pubkey_hex,
                        result_out.fund_commitment_blind_hex, result_out.fund_outpoint,
                    ...
                    if (memo_ok) {
                        ...
                        memo_ok = !funding_proof.empty() &&
                                  btcspv::ParseC1FundingProof(
                                      funding_proof.data(), funding_proof.size(), block_hash,
                                      merkle_dirs, merkle_branch, legacy_tx, nullifier_proof) &&
                                  legacy_tx.size() > 64;
                        std::vector<btcspv::BtcTxOut> outputs;
                        if (memo_ok)
                            memo_ok = btcspv::ParseBtcTxOutputs(legacy_tx.data(), legacy_tx.size(),
                                                                outputs);
                        ...
                        if (memo_ok && !exact_script.empty()) {
                            ...
                            memo_ok = exact_matches == 1 && btcspv::BtcDepositOutpointId(
                    ...
                    if (memo_ok) {
                        result_out.allocation_commitment =
                            c1reserve::AllocationCommitment(result_out.allocation_id, result_out.to,
                ...
                if (!memo_ok)
                    return result_out.action + " memo is not canonical C1 wire data";
            } else if (payload.rfind("VELD_", 0) == 0) {
                ++other_markers;
            } else {
