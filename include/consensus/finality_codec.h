...
constexpr size_t SIGNED_VOTE_WIRE_BYTES = ::veld::finality::wire::SIGNED_VOTE_BYTES;
...
constexpr size_t MAX_FINALITY_CARRIER_FRAGMENTS =
    (MAX_FINALITY_QC_BYTES + FINALITY_FRAGMENT_DATA_BYTES - 1) / FINALITY_FRAGMENT_DATA_BYTES;
static_assert(MAX_FINALITY_CARRIER_FRAGMENTS == ::veld::MAX_FINALITY_MARKER_OUTPUTS,
...
constexpr size_t FinalityQcWireSizeFor(size_t n) {
    return std::char_traits<char>::length(FIN_QC_BINARY_PREFIX) + 8 + 1 + 4 + 8 + 32 + 8 + 32 + 2 +
           ((n + 7) / 8) + 2 + FinalityQuorumCountFor(n) * ::veld::dilithium::SIG_MAX_BYTES;
}
static_assert(FinalityQcWireSizeFor(MAX_FINALITY_VALIDATOR_COUNT) <= MAX_FINALITY_QC_BYTES &&
                  FinalityQcWireSizeFor(MAX_FINALITY_VALIDATOR_COUNT + 1) > MAX_FINALITY_QC_BYTES,
...
inline bool HexToHash(const std::string& hex, Hash256& out) {
    ...
    if (!FromHexBytes(hex, b) || b.size() != 32)
        return false;
...
inline std::string EncodeQc(const QuorumCert& qc, const std::vector<std::vector<uint8_t>>& sigs) {
    if (qc.bitmap.empty() || qc.bitmap.size() > UINT16_MAX || sigs.empty() ||
        sigs.size() > UINT16_MAX)
        return {};
    for (const auto& sig : sigs)
        if (sig.size() != ::veld::dilithium::SIG_MAX_BYTES)
            return {};
    ...
    u8((uint8_t)qc.phase);
    u32(qc.round);
    u64(qc.source.height);
    bytes(qc.source.hash.data(), qc.source.hash.size());
    u64(qc.target.height);
    bytes(qc.target.hash.data(), qc.target.hash.size());
    u16((uint16_t)qc.bitmap.size());
    bytes(qc.bitmap.data(), qc.bitmap.size());
    u16((uint16_t)sigs.size());
    for (const auto& sig : sigs)
        bytes(sig.data(), sig.size());
    if (out.size() > MAX_FINALITY_QC_BYTES)
        return {};
    return out;
}
...
inline std::optional<DecodedQc> DecodeQc(const std::string& data) {
    DecodedQc d;
    if (data.size() > MAX_FINALITY_QC_BYTES || data.rfind(FIN_QC_BINARY_PREFIX, 0) != 0)
        return std::nullopt;
    ...
    auto u64 = [&](uint64_t& v) {
        ...
        v = 0;
        for (int i = 0; i < 8; ++i)
            v |= (uint64_t)(uint8_t)data[p++] << (8 * i);
        return true;
    };
    auto bytes = [&](uint8_t* out, size_t n) {
        if (!need(n))
            return false;
        std::memcpy(out, data.data() + p, n);
        p += n;
        return true;
    };
    uint8_t phase = 0;
    uint16_t bitmap_len = 0, sig_count = 0;
    if (!u64(d.qc.epoch_id) || !u8(phase) || (phase != 1 && phase != 2) || !u32(d.qc.round) ||
        !u64(d.qc.source.height) || !bytes(d.qc.source.hash.data(), d.qc.source.hash.size()) ||
        !u64(d.qc.target.height) || !bytes(d.qc.target.hash.data(), d.qc.target.hash.size()) ||
        !u16(bitmap_len) || bitmap_len == 0 || !need(bitmap_len))
        return std::nullopt;
    d.qc.phase = (Phase)phase;
    d.qc.bitmap.assign((const uint8_t*)data.data() + p,
                       (const uint8_t*)data.data() + p + bitmap_len);
    p += bitmap_len;
    if (!u16(sig_count) || sig_count == 0)
        return std::nullopt;
    const size_t sig_bytes = (size_t)sig_count * ::veld::dilithium::SIG_MAX_BYTES;
    if (!need(sig_bytes) || p + sig_bytes != data.size())
        return std::nullopt;
    d.sigs.reserve(sig_count);
    for (uint16_t i = 0; i < sig_count; ++i) {
        d.sigs.emplace_back((const uint8_t*)data.data() + p,
                            (const uint8_t*)data.data() + p + ::veld::dilithium::SIG_MAX_BYTES);
    ...
    if (!SourceRefWellFormed(d.qc.source, d.qc.target) ||
        ...
        d.qc.round != CheckpointRound(d.qc.target.height))
        return std::nullopt;
    return d;
}
...
inline bool VerifyDecodedQc(DecodedQc& d, const EpochSnapshot& s, uint32_t network_id,
                            const Hash256& genesis_hash) {
    if (!SnapshotWellFormed(s))
        return false;
    if (d.qc.epoch_id != s.epoch_id)
        return false;
    if (EpochOf(d.qc.target.height) != d.qc.epoch_id)
        return false;
    if (d.qc.round != CheckpointRound(d.qc.target.height))
        return false;
    if (d.qc.bitmap.size() != (s.entries.size() + 7) / 8)
        return false;
    if (!IsScheduledCheckpoint(d.qc.target.height))
        return false;
    if (!SourceRefWellFormed(d.qc.source, d.qc.target))
        return false;
    ...
    size_t sig_i = 0;
    const std::vector<uint8_t> msg = VotePreimage(network_id, genesis_hash, d.qc.epoch_id, s.root,
                                                  d.qc.phase, d.qc.round, d.qc.source, d.qc.target);

    for (size_t i = 0; i < s.entries.size(); ++i) {
        if (!(d.qc.bitmap[i >> 3] & (uint8_t)(1u << (i & 7))))
            continue;
        if (sig_i >= d.sigs.size())
            return false; // bitmap outruns sigs
        if (d.sigs[sig_i].size() != ::veld::dilithium::SIG_MAX_BYTES)
            return false;
        ::veld::dilithium::PublicKey pk{};
        if (!HexToPublicKey(s.entries[i].pubkey_hex, pk))
            return false;
        if (!::veld::dilithium::Verify(pk, msg, d.sigs[sig_i]))
            return false;
        ++sig_i;
    }
    if (sig_i != d.sigs.size())
        return false; // sigs outrun bitmap

    // Recompute; never trust a transmitted weight.
    const uint64_t w = BitmapWeight(s, d.qc.bitmap);
    if (!IsSupermajority(w, s.total_weight))
        return false;
...
inline std::vector<uint8_t> EncodeSignedVoteWire(const SignedVote& v) {
    ::veld::dilithium::PublicKey pk{};
    if (!HexToPublicKey(v.pubkey_hex, pk) || v.signature.size() != ::veld::dilithium::SIG_MAX_BYTES)
        return {};
...
inline std::optional<SignedVote> DecodeSignedVoteWire(const std::vector<uint8_t>& in) {
    constexpr size_t PREFIX = 4;
    constexpr size_t FIXED = SIGNED_VOTE_WIRE_BYTES;
    if (in.size() != FIXED || !std::equal(in.begin(), in.begin() + PREFIX, FIN_VOTE_BINARY_PREFIX))
        return std::nullopt;
    size_t p = PREFIX;
    auto u8 = [&]() { return in[p++]; };
    auto u32 = [&]() {
    ...
    SignedVote v;
    v.epoch_id = u64();
    std::copy_n(in.begin() + (ptrdiff_t)p, 32, v.set_root.begin());
    p += 32;
    ...
    v.round = u32();
    v.source.height = u64();
    std::copy_n(in.begin() + (ptrdiff_t)p, 32, v.source.hash.begin());
    p += 32;
    v.target.height = u64();
    std::copy_n(in.begin() + (ptrdiff_t)p, 32, v.target.hash.begin());
    p += 32;
    ...
    if (!SourceRefWellFormed(v.source, v.target) || !IsScheduledCheckpoint(v.target.height) ||
        EpochOf(v.target.height) != v.epoch_id)
        return std::nullopt;
    return v;
}
...
inline std::vector<std::string> EncodeFinalityCarrierPayloads(const std::string& wire) {
    constexpr size_t MAX_DIRECT_PAYLOAD = 32764; // OP_RETURN+PUSHDATA2 = 4
    if (wire.empty() || wire.size() > MAX_FINALITY_QC_BYTES || !DecodeQc(wire))
        return {};
    if (wire.size() <= MAX_DIRECT_PAYLOAD)
        return {wire};
    const size_t count =
        (wire.size() + FINALITY_FRAGMENT_DATA_BYTES - 1) / FINALITY_FRAGMENT_DATA_BYTES;
    if (count == 0 || count > MAX_FINALITY_CARRIER_FRAGMENTS || count > UINT16_MAX)
        return {};
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        auto u32 = [&](uint32_t v) {
            for (int n = 0; n < 4; ++n)
                f.push_back((char)(v >> (8 * n)));
        };
        u32((uint32_t)wire.size());
        u16((uint16_t)i);
        u16((uint16_t)count);
        const size_t off = i * FINALITY_FRAGMENT_DATA_BYTES;
        const size_t n = std::min(FINALITY_FRAGMENT_DATA_BYTES, wire.size() - off);
        f.append(wire.data() + off, n);
        if (f.size() > MAX_DIRECT_PAYLOAD)
            return {};
...
inline std::optional<QcFragment> DecodeQcFragment(const std::string& payload) {
    if (payload.rfind(FIN_QC_FRAGMENT_PREFIX, 0) != 0)
        return std::nullopt;
    size_t p = std::strlen(FIN_QC_FRAGMENT_PREFIX);
    constexpr size_t FIXED = 32 + 4 + 2 + 2;
    if (payload.size() < p + FIXED)
        return std::nullopt;
    QcFragment f;
    std::memcpy(f.id.data(), payload.data() + p, f.id.size());
    p += f.id.size();
    auto u16 = [&]() -> uint16_t {
        uint16_t v = (uint16_t)(uint8_t)payload[p] | ((uint16_t)(uint8_t)payload[p + 1] << 8);
        p += 2;
        return v;
    };
    auto u32 = [&]() -> uint32_t {
    ...
    f.index = u16();
    f.count = u16();
    if (f.total_bytes == 0 || f.total_bytes > MAX_FINALITY_QC_BYTES || f.count < 2 ||
        f.count > MAX_FINALITY_CARRIER_FRAGMENTS || f.index >= f.count)
        return std::nullopt;
    const size_t expected_count =
        (f.total_bytes + FINALITY_FRAGMENT_DATA_BYTES - 1) / FINALITY_FRAGMENT_DATA_BYTES;
    if (f.count != expected_count)
        return std::nullopt;
    const size_t expected_bytes =
        f.index + 1 == f.count ? f.total_bytes - (size_t)f.index * FINALITY_FRAGMENT_DATA_BYTES
                               : FINALITY_FRAGMENT_DATA_BYTES;
    if (payload.size() - p != expected_bytes)
        return std::nullopt;
...
inline FinParseResult ParseFinalityBlock(const BlockT& block, ParseOpReturnFn parse_op_return) {
    FinParseResult r;
    ...
    for (const auto& tx : block.transactions) {
        for (const auto& o : tx.outputs) {
            ...
            if (payload.rfind(FIN_QC_BINARY_PREFIX, 0) == 0) {
                ...
                if (!f || direct || fragments.size() >= MAX_FINALITY_CARRIER_FRAGMENTS) {
                    r.status = FinParseStatus::INVALID;
                    return r;
                }
    ...
    if (direct) {
        ...
        for (size_t i = 0; i < fragments.size(); ++i) {
            ...
            if (f.id != first.id || f.total_bytes != first.total_bytes || f.count != first.count ||
                f.index != i) {
                r.status = FinParseStatus::INVALID;
                return r;
            }
        ...
        if (wire.size() != first.total_bytes || FinalityWireId(wire) != first.id) {
            r.status = FinParseStatus::INVALID;
            return r;
        }
    } else {
