...
inline constexpr size_t MAX_MSPV_BINARY_PROOF_BYTES =
    4 + 32 + 4 + 1 + MAX_MSPV_MERKLE_BRANCH_HASHES * 32 + 4 +
    MAX_MSPV_STRIPPED_TX_BYTES + 2 +
    MAX_MSPV_PARENT_COUNT * 4 + MAX_MSPV_PARENT_TOTAL_BYTES +
    MAX_PROOF_BYTES;
...
inline constexpr const char* KEY_DOMAIN =
    "VELD_BTCVELD_MINT_NULLIFIER_KEY_v1|";
inline constexpr const char* EMPTY_LEAF_DOMAIN =
    "VELD_BTCVELD_MINT_NULLIFIER_EMPTY_LEAF_v1|";
inline constexpr const char* OCCUPIED_LEAF_DOMAIN =
    "VELD_BTCVELD_MINT_NULLIFIER_OCCUPIED_LEAF_v1|";
inline constexpr const char* NODE_DOMAIN =
    "VELD_BTCVELD_MINT_NULLIFIER_NODE_v1|";
...
inline bool BitmapBit(const std::array<uint8_t, BITMAP_BYTES>& bitmap,
                      size_t depth) {
    return ((bitmap[depth / 8] >> (7u - (depth % 8))) & 1u) != 0;
}

inline void SetBitmapBit(std::array<uint8_t, BITMAP_BYTES>& bitmap,
                         size_t depth, bool value) {
    const uint8_t mask = static_cast<uint8_t>(1u << (7u - (depth % 8)));
    if (value) bitmap[depth / 8] |= mask;
    else bitmap[depth / 8] &= static_cast<uint8_t>(~mask);
}

inline size_t BitmapPopcount(
        const std::array<uint8_t, BITMAP_BYTES>& bitmap) {
    size_t n = 0;
    for (uint8_t b : bitmap) {
        for (; b != 0; b &= static_cast<uint8_t>(b - 1)) ++n;
    }
    return n;
}
...
inline Hash256 EmptyRoot() { return EmptyHashes()[0]; }
...
inline bool DecodeProof(const uint8_t* data, size_t len, Proof& out) {
    ...
    if (data == nullptr || len < MIN_PROOF_BYTES || len > MAX_PROOF_BYTES ||
        ...
        return false;
    for (size_t i = 0; i < BITMAP_BYTES; ++i) out.bitmap[i] = data[i];
    const size_t count = BitmapPopcount(out.bitmap);
    if (len != BITMAP_BYTES + count * HASH_BYTES) return false;
    ...
    for (size_t depth = 0; depth < TREE_DEPTH; ++depth) {
        if (!BitmapBit(out.bitmap, depth)) continue;
        Hash256 sibling{};
        for (size_t j = 0; j < HASH_BYTES; ++j) sibling[j] = data[off++];
        ...
        if (sibling == empty[depth + 1]) return false;
...
inline bool IsCanonicalProof(const Proof& proof) {
    if (proof.siblings.size() != BitmapPopcount(proof.bitmap)) return false;
    const auto& empty = EmptyHashes();
    size_t i = 0;
    for (size_t depth = 0; depth < TREE_DEPTH; ++depth) {
        if (!BitmapBit(proof.bitmap, depth)) continue;
...
inline std::vector<uint8_t> EncodeProof(const Proof& proof) {
    if (!IsCanonicalProof(proof)) return {};
...
inline Proof EmptyProof() { return Proof{}; }

inline bool ExpandSiblings(const Proof& proof,
                           std::array<Hash256, TREE_DEPTH>& out) {
    if (!IsCanonicalProof(proof)) return false;
    const auto& empty = EmptyHashes();
    size_t i = 0;
    for (size_t depth = 0; depth < TREE_DEPTH; ++depth) {
        if (BitmapBit(proof.bitmap, depth)) out[depth] = proof.siblings[i++];
        else out[depth] = empty[depth + 1];
    }
    return i == proof.siblings.size();
}

inline Proof CompressSiblings(
        const std::array<Hash256, TREE_DEPTH>& siblings) {
    Proof out;
    const auto& empty = EmptyHashes();
    for (size_t depth = 0; depth < TREE_DEPTH; ++depth) {
        if (siblings[depth] == empty[depth + 1]) continue;
...
inline bool ComputePath(const Hash256& key, bool occupied, const Proof& proof,
    ...
    if (!ExpandSiblings(proof, siblings)) return false;
    path[TREE_DEPTH] = occupied ? OccupiedLeaf(key) : EmptyLeaf();
    for (size_t depth = TREE_DEPTH; depth-- > 0;) {
        path[depth] = Bit(key, depth)
            ? Node(siblings[depth], path[depth + 1])
            : Node(path[depth + 1], siblings[depth]);
    }
    return true;
}

inline bool Verify(const Hash256& root, const std::string& canonical_outpoint,
                   bool occupied, const Proof& proof) {
    std::array<Hash256, TREE_DEPTH + 1> path{};
...
inline InsertResult Insert(const Hash256& current_root,
    ...
    if (!ComputePath(key, false, nonmembership_proof, old_path) ||
        old_path[0] != current_root)
        return out;
    std::array<Hash256, TREE_DEPTH + 1> new_path{};
    if (!ComputePath(key, true, nonmembership_proof, new_path)) return out;
    out.ok = true;
...
inline bool UpdateWitnessAfterInsert(
        const Hash256& old_root, const Hash256& new_root,
    ...
    if (target_key == inserted_key) {
        if (target_occupied) return false; // stream attempted a duplicate insert
    ...
    size_t divergence = 0;
    while (divergence < TREE_DEPTH &&
           Bit(target_key, divergence) == Bit(inserted_key, divergence))
        ++divergence;
    if (divergence == TREE_DEPTH) return false; // SHA-256 key collision

    std::array<Hash256, TREE_DEPTH> target_siblings{};
    if (!ExpandSiblings(target_proof, target_siblings)) return false;
    std::array<Hash256, TREE_DEPTH + 1> inserted_new_path{};
    if (!ComputePath(inserted_key, true, insertion_nonmembership_proof,
                     inserted_new_path))
        return false;
...
inline std::string EncodeIssuerMemo(const std::string& canonical_outpoint,
                                    const Proof& proof) {
    const std::vector<uint8_t> bytes = EncodeProof(proof);
    if (bytes.empty()) return {};
    return std::string(ISSUER_MEMO_PREFIX) + canonical_outpoint + ";" +
           BytesToHex(bytes);
}
...
inline std::string EncodeReservedIssuerMemo(
    ...
    if (!lowercase_hex(request_id, 32) ||
        request_id.compare(0, 16, "0000000000000000") != 0 ||
        request_id == std::string(32, '0') ||
        script_pubkey_hex.rfind("5120", 0) != 0 ||
        !lowercase_hex(script_pubkey_hex, 68) ||
        !lowercase_hex(commitment_blind_hex, 64)) return {};
    return std::string(RESERVED_ISSUER_MEMO_PREFIX) + request_id + ";" +
           script_pubkey_hex + ";" + commitment_blind_hex + ";" +
           canonical_outpoint;
}

inline bool ParseIssuerMemo(const std::string& memo,
                            std::string& canonical_outpoint, Proof& proof) {
...
inline bool ParseIssuerMemo(const std::string& memo,
    ...
    if (request_split == std::string::npos) return false;
    reservation_request_id =
        memo.substr(prefix.size(), request_split - prefix.size());
    if (reservation_request_id.size() != 32) return false;
    for (char c : reservation_request_id)
        if (!((c >= '0' && c <= '9') || (c >= 'a' && c <= 'f')))
            return false;
    if (reservation_request_id.compare(
            0, 16, "0000000000000000") != 0 ||
        reservation_request_id == std::string(32, '0')) return false;
    const size_t script_split = memo.find(';', request_split + 1);
    if (script_split == std::string::npos ||
        script_split == request_split + 1) return false;
    reservation_script_pubkey_hex = memo.substr(
        request_split + 1, script_split - request_split - 1);
    if (reservation_script_pubkey_hex.size() != 68 ||
        reservation_script_pubkey_hex.rfind("5120", 0) != 0) return false;
    ...
    if (blind_split == std::string::npos || blind_split == script_split + 1)
        return false;
    reservation_commitment_blind_hex = memo.substr(
        script_split + 1, blind_split - script_split - 1);
    if (reservation_commitment_blind_hex.size() != 64) return false;
    ...
    if (memo.find(';', blind_split + 1) != std::string::npos)
        return false;
    canonical_outpoint =
        memo.substr(blind_split + 1);
    return EncodeReservedIssuerMemo(reservation_request_id,
                                    reservation_script_pubkey_hex,
                                    reservation_commitment_blind_hex,
                                    canonical_outpoint) == memo;
}
...
inline bool ParseIssuerMemo(const std::string& memo,
    ...
    return ParseIssuerMemo(memo, canonical_outpoint, proof,
                           reservation_request_id,
                           ignored_script_pubkey_hex,
                           ignored_commitment_blind_hex);
}
