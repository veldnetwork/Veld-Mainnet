...
static constexpr uint64_t BRONZE_ACTIVE = 7;
static constexpr uint64_t BRONZE_WINDOWS = 14;
static constexpr uint64_t SILVER_ACTIVE = 25;
static constexpr uint64_t SILVER_WINDOWS = 30;
static constexpr uint64_t GOLD_ACTIVE = 165;
static constexpr uint64_t GOLD_WINDOWS = 180;
static constexpr uint64_t PLATINUM_ACTIVE = 335;
static constexpr uint64_t PLATINUM_WINDOWS = 365;
static constexpr uint64_t DIAMOND_ACTIVE = 1000;
static constexpr uint64_t DIAMOND_WINDOWS = 1'095;
...
class TierEngine {
    ...
    TierInfo GetTier(const std::string& script_hex) const {
...

        {
            ...
            if (active >= DIAMOND_ACTIVE)
                return window_tier(5, DIAMOND_MULT, "Diamond", "1000 of last 1,095 days active",
                                   active, DIAMOND_WINDOWS, DIAMOND_ACTIVE, mined);
        }
        {
            uint64_t active = chain_.GetActiveWindowCount(script_hex, PLATINUM_WINDOWS, height);
            if (active >= PLATINUM_ACTIVE)
                return window_tier(4, PLATINUM_MULT, "Platinum", "335 of last 365 days active",
                                   active, PLATINUM_WINDOWS, PLATINUM_ACTIVE, mined);
        }
        {
            uint64_t active = chain_.GetActiveWindowCount(script_hex, GOLD_WINDOWS, height);
            if (active >= GOLD_ACTIVE)
                return window_tier(3, GOLD_MULT, "Gold", "165 of last 180 days active", active,
                                   GOLD_WINDOWS, GOLD_ACTIVE, mined);
        }
        {
            uint64_t active = chain_.GetActiveWindowCount(script_hex, SILVER_WINDOWS, height);
            if (active >= SILVER_ACTIVE)
                return window_tier(2, SILVER_MULT, "Silver", "25 of last 30 days active", active,
                                   SILVER_WINDOWS, SILVER_ACTIVE, mined);
        }
        {
            uint64_t active = chain_.GetActiveWindowCount(script_hex, BRONZE_WINDOWS, height);
            if (active >= BRONZE_ACTIVE)
                return window_tier(1, BRONZE_MULT, "Bronze", "7 of last 14 days active", active,
                                   BRONZE_WINDOWS, BRONZE_ACTIVE, mined);
        }
        TierInfo t;
        t.level = 0;
        t.multiplier = BASE_MULT;
        t.name = "";
  ...
  private:
    ...
    static TierInfo window_tier(int lvl, double mult, const char* name, const char* desc,
        ...
        t.windows_active = active;
        t.windows_total = total;
        t.windows_required = required;
        t.blocks_mined = mined;
        return t;
    }
};
