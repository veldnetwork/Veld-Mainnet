...
inline PayoutProof VerifyRedeemPayout(const btcspv::BtcHeaderChain& ch, const uint8_t* p,
                                      size_t len, uint32_t k_btc,
                                      const std::vector<uint8_t>& custody_spk = {}) {
    PayoutProof r;
    if (p == nullptr) {
        r.reason = "null payout proof";
        return r;
    }
    if (REDEEM_PAYOUT_BINDING_REQUIRED) {
        ...
        if (custody_spk.empty() || len < 4 + 32 + 32 + 32 + 4 + 1 + 4 + 10 + 4 + 10 + 4 + 4 ||
            std::memcmp(p, "PSP2", 4) != 0) {
            r.reason = "malformed PSP2 payout proof";
            return r;
        }
        size_t off = 4;
        std::memcpy(r.request_id.data(), p + off, 32);
        off += 32;
        std::memcpy(r.request_commitment.data(), p + off, 32);
        off += 32;
        H256 block_hash{};
        std::memcpy(block_hash.data(), p + off, 32);
        off += 32;
        const uint64_t dirs = btcspv::rd_le32(p + off);
        off += 4;
        ...
        const uint32_t tx_len = btcspv::rd_le32(p + off);
        off += 4;
        ...
        off += tx_len;
        const uint32_t parent_len = btcspv::rd_le32(p + off);
        off += 4;
        ...
        off += parent_len;
        const uint32_t custody_vout = btcspv::rd_le32(p + off);
        off += 4;
        const uint32_t payout_vin = btcspv::rd_le32(p + off);
        off += 4;
        ...
        if (!record) {
            r.reason = "PSP2 payout header unavailable";
            return r;
        }
        r.payout_btc_height = record->height;
        if (!btcspv::ParseBtcTxOutputs(payout_tx.data(), payout_tx.size(), r.outputs)) {
            r.reason = "PSP2 payout transaction is unparseable";
            return r;
        }
        std::vector<btcspv::BtcPrevout> payout_prevouts;
        if (!btcspv::ParseBtcTxPrevouts(payout_tx.data(), payout_tx.size(), payout_prevouts) ||
        ...
        if (!btcspv::ParseBtcTxOutputs(parent_tx.data(), parent_tx.size(), parent_outputs) ||
        ...
        if (payout_prevouts[0].txid != parent_txid || payout_prevouts[0].vout != custody_vout) {
    ...
    if (len < 4 + 32 + 32 + 4 + 1 || std::memcmp(p, "PSPV", 4) != 0) {
        r.reason = "malformed payout op";
        return r;
    }
    size_t off = 4;
    std::memcpy(r.request_id.data(), p + off, 32);
    ...
    std::memcpy(block_hash.data(), p + off, 32);
    off += 32;
    uint64_t dirs = btcspv::rd_le32(p + off);
    off += 4;
    uint8_t mlen = p[off++];
    ...
    // payload is hashed.
    if (mlen > 32) {
        r.reason = "bad merkle length";
        return r;
    }
    if (mlen < 32 && (dirs >> mlen) != 0) {
        r.reason = "non-canonical merkle direction bits";
        return r;
    }
    if (off + (size_t)mlen * 32 > len) {
        r.reason = "truncated merkle branch";
        return r;
    }
    std::vector<H256> branch;
    ...
    if (tx.size() <= 64) {
        r.reason = "payout tx implausibly short (merkle-ambiguity guard)";
        return r;
    }
    ...
    if (!ch.IsFinal(block_hash, k_btc)) {
        r.reason = "payout not final / not on best chain";
        return r;
    }
    if (!btcspv::ParseBtcTxOutputs(tx.data(), tx.size(), r.outputs)) {
        r.reason = "payout tx unparseable";
        return r;
    }
    r.ok = true;
    r.payout_txid = txid;
...
inline PayoutProof VerifyAndResolvePayout(SignerBondCovenant& cov, const btcspv::BtcHeaderChain& ch,
    ...
    fulfilled = cov.ResolveProvenPayout(
        r.request_id, r.payout_txid, r.outputs, out_slash, r.request_commitment,
        r.payout_btc_height, r.spends_recognized_custody, custody_spk, r.proven_input_sats);
    return r;
}
...
struct FraudProof {
    ...
    std::vector<btcspv::BtcTxOut>
        spend_outputs; // the spending tx's outputs (for the request check)
    H256 spend_block{};
...
inline FraudProof VerifyFraudulentSpend(const btcspv::BtcHeaderChain& ch, const uint8_t* p,
                                        size_t len, const std::vector<uint8_t>& custody_spk,
                                        uint32_t k_btc) {
    FraudProof r;
    ...
    if (REDEEM_PAYOUT_BINDING_REQUIRED) {
        ...
        if (len < 4 + 32 + 4 + 1 + 4 + 10 + 2 || std::memcmp(p, "FSP2", 4) != 0) {
            r.reason = "malformed FSP2 proof";
            return r;
        }
        size_t off = 4;
        std::memcpy(r.spend_block.data(), p + off, 32);
        off += 32;
        const uint64_t dirs = btcspv::rd_le32(p + off);
        off += 4;
        const uint8_t mlen = p[off++];
        ...
        const uint32_t spend_len = btcspv::rd_le32(p + off);
        off += 4;
        if (spend_len <= 64 || spend_len > btcnull::MAX_MSPV_STRIPPED_TX_BYTES ||
            off + spend_len + 2 > len) {
        ...
        if (!btcspv::ParseWitnessAwareBtcTx(r.spend_tx, spend, btcnull::MAX_MSPV_PARENT_COUNT)) {
        ...
        for (uint16_t i = 0; i < parent_count; ++i) {
            if (off + 4 > len) {
                r.reason = "truncated FSP2 parent";
                return r;
            }
            const uint32_t parent_len = btcspv::rd_le32(p + off);
            off += 4;
    ...
    auto rd32 = [&]() {
        uint32_t v = btcspv::rd_le32(p + off);
        off += 4;
        return v;
    };
    ...
    auto rd_merkle = [&](std::vector<H256>& br, uint64_t dirs) -> bool {
        if (!need(1))
            return false;
        uint8_t mlen = p[off++];
        if (mlen > 32 || !need((size_t)mlen * 32))
            return false;
        if (mlen < 32 && (dirs >> mlen) != 0)
            return false;
        for (int i = 0; i < mlen; i++) {
    ...
    if (len < 4 || std::memcmp(p, "FSPV", 4) != 0) {
        r.reason = "malformed fraud op";
        return r;
    }
    off = 4;
    ...
    H256 fblk;
    std::memcpy(fblk.data(), p + off, 32);
    off += 32;
    uint64_t fdirs = rd32();
    std::vector<H256> fbranch;
    if (!rd_merkle(fbranch, fdirs)) {
        r.reason = "bad F merkle";
        return r;
    }
    if (!need(4 + 4)) {
        r.reason = "trunc F vout/len";
        return r;
    }
    uint32_t custody_vout = rd32();
    uint32_t ftxlen = rd32();
    if (!need(ftxlen) || ftxlen <= 64) {
        r.reason = "bad F txlen";
        return r;
    }
    std::vector<uint8_t> ftx(p + off, p + off + ftxlen);
    off += ftxlen;
    H256 ftxid = ::veld::Hash256d(ftx);
    if (!ch.VerifyMerkle(fblk, ftxid, fbranch, fdirs)) {
        r.reason = "F merkle invalid";
        return r;
    }
    if (!ch.IsFinal(fblk, k_btc)) {
        r.reason = "F not final";
        return r;
    }
    std::vector<btcspv::BtcTxOut> fouts;
    if (!btcspv::ParseBtcTxOutputs(ftx.data(), ftx.size(), fouts)) {
        r.reason = "F unparseable";
        return r;
    }
    if (custody_vout >= fouts.size()) {
        r.reason = "custody_vout out of range";
        return r;
    }
    if (fouts[custody_vout].spk != custody_spk) {
        r.reason = "funding output is not the custody spk";
        return r;
    }
    ...
    H256 sblk;
    std::memcpy(sblk.data(), p + off, 32);
    off += 32;
    uint64_t sdirs = rd32();
    std::vector<H256> sbranch;
    if (!rd_merkle(sbranch, sdirs)) {
        r.reason = "bad S merkle";
        return r;
    }
    if (!need(4 + 4)) {
        r.reason = "trunc S vin/len";
        return r;
    }
    uint32_t spend_vin = rd32();
    uint32_t stxlen = rd32();
    if (off + stxlen != len || stxlen <= 64) {
        r.reason = "bad S txlen";
        return r;
    }
    std::vector<uint8_t> stx(p + off, p + off + stxlen);
    r.spend_txid = ::veld::Hash256d(stx);
    if (!ch.VerifyMerkle(sblk, r.spend_txid, sbranch, sdirs)) {
        r.reason = "S merkle invalid";
        return r;
    }
    if (!ch.IsFinal(sblk, k_btc)) {
        r.reason = "S not final";
        return r;
    }
    H256 prev_txid;
    uint32_t prev_vout;
    if (!btcspv::ParseBtcTxInputPrevout(stx.data(), stx.size(), spend_vin, prev_txid, prev_vout)) {
        r.reason = "S input parse";
        return r;
    }
    if (prev_txid != ftxid || prev_vout != custody_vout) {
        r.reason = "spend does not reference the custody output";
        return r;
    }
    if (!btcspv::ParseBtcTxOutputs(stx.data(), stx.size(), r.spend_outputs)) {
        r.reason = "S unparseable";
        return r;
    }
    r.ok = true;
    return r;
...
inline FraudProof VerifyAndResolveFraud(SignerBondCovenant& cov, const btcspv::BtcHeaderChain& ch,
    ...
    out_slash =
        cov.EvalFraudulentSpend(r.custody_value, r.spend_outputs, custody_spk, r.total_input_value);
