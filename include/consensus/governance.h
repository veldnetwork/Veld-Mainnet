...
class GovernanceEngine {
  public:
    ...
    StateSnapshot SnapshotState() const {
        std::lock_guard<std::mutex> lock(mutex_);
        return StateSnapshot{next_id_, proposals_, last_proposal_block_, last_vote_block_};
    }
    ...
    std::string ProposalToJSON(const Proposal& p) const {
        ...
        j << "{"
          ...
          << "\"timelock_until\":" << p.timelock_until << std::setprecision(4);
        j << "}";
        return j.str();
    }
    ...
    static std::string BuildVoteOp(uint64_t proposal_id, const std::string& voter,
        ...
        o << GOV_PREFIX << "V|" << proposal_id << "|" << voter << "|" << c << "|" << signed_height
          << "|" << pubkey_hex << "|" << sig_hex;
        return o.str();
    }
        ...
        for (size_t ti = 0; ti < block.transactions.size(); ++ti) {
            ...
            for (size_t oi = 0; oi < tx.outputs.size(); ++oi) {
                ...
                if (data.compare(0, GOV_PREFIX_LEN, GOV_PREFIX) != 0)
                    continue;
                if (ProcessOpLocked(data, block_height))
                    state_mutated = true;
            }
        }
  ...
  private:
    ...
    bool ApplyProposalFromChain(uint64_t block_height, ProposalType type, bool typed_marker,
        ...
        if (signed_height >= BATCH2_HARDENING_HEIGHT) {
            ...
            challenge = "GOV_PROPOSAL:" + (typed_marker ? ProposalTypeName(type) + ":" : "") +
                        title + "|" + description + ":@" + std::to_string(signed_height);
        }
    ...
    bool ApplyVoteFromChain(uint64_t block_height, uint64_t proposal_id, const std::string& voter,
        ...
        std::string challenge;
        if (signed_height >= BATCH2_HARDENING_HEIGHT) {
            challenge = GovChainIdPrefix() + "GOV_VOTE:" + std::to_string(proposal_id) + ":" +
                        choice_str + ":@" + std::to_string(signed_height);
        } else {
            challenge = "GOV_VOTE:" + std::to_string(proposal_id) + ":" + choice_str + ":@" +
