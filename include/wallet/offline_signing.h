...
inline constexpr size_t kCanonicalInputScriptBytes =
    3U + 1U + 3309U + 1U + 3U + 1952U;
...
inline constexpr const char* kIntentAuthorizationVersion =
    "VELD_INTENT_AUTHORIZATION_V1";
...
inline bool VerifyExactFeesOnlyEnvelope(
        const VerifiedPrepared& prepared,
    ...
    if (expected_change != 0 &&
        (outputs.front().value != expected_change ||
...
inline bool ExtractCanonicalOperationIdentity(
        const TxOutput& output, std::string& identity,
        std::string& error) {
    identity.clear();
    ...
    if (opcode <= 75U) {
        ...
        pushed = static_cast<size_t>(script[cursor]) |
                 (static_cast<size_t>(script[cursor + 1U]) << 8);
        cursor += 2U;
        if (pushed <= 255U) {
    ...
    if (pushed == 0U || cursor > script.size() ||
        pushed != script.size() - cursor) {
        error = "operation OP_RETURN push length or trailing bytes differ";
        return false;
    }
    identity.assign(
        script.begin() + static_cast<ptrdiff_t>(cursor), script.end());
    return true;
}
    ...
    for (size_t i = 0; i < parents.size(); ++i) {
        if (i < tx.inputs.size()) {
            preimage.insert(preimage.end(),
                            tx.inputs[i].prev_tx_hash.begin(),
...
inline std::string AuthorizerPubkeyDigest(const Secp256k1PubKey& pubkey) {
    return HashToHex(Hash256d(std::vector<uint8_t>(
        pubkey.begin(), pubkey.end())));
}

inline Hash256 IntentAuthorizationHash(
        const Intent& intent, const std::string& authorizer_pubkey_digest) {
    const std::string commitment = TaggedDigest(
        "VELD_OFFLINE_INTENT_AUTHORIZATION_V1", {
...
inline Intent MakeIntent(const Transaction& tx,
                         const std::vector<std::vector<uint8_t>>& parents,
                         std::string operation_type,
                         std::string intended_recipient,
    ...
    intent.expected_change_destination =
        std::move(expected_change_destination);
    ...
    intent.operation_identity_digest =
        OperationIdentityDigest(operation_identity);
    intent.intent_digest = IntentDigest(intent);
    return intent;
}

inline bool DecodeLowerHex(const std::string& hex, size_t max_bytes,
...
inline const btc_buy::JsonValue* SelectObject(
        const btc_buy::JsonValue& root, const char* nested) {
    if (root.kind != btc_buy::JsonValue::Kind::Object) return nullptr;
...
inline bool ParseStringField(const btc_buy::JsonValue& object,
                             const char* name, std::string& out) {
    const auto* field = object.Get(name);
    if (!field || field->kind != btc_buy::JsonValue::Kind::String ||
        field->string_had_escape) return false;
    out = field->text;
    return true;
}

inline bool ParseUintField(const btc_buy::JsonValue& object,
...
inline bool AuthenticatePrepared(const btc_buy::JsonValue& prepared,
    ...
    if (consumed != raw.size() || tx.Serialize() != raw || tx.version != 1 ||
        ...
        tx.outputs.empty()) {
        error = "unsigned transaction is non-canonical"; return false;
    }
    const auto* inputs = proposal->Get("inputs");
    if (!inputs || inputs->kind != btc_buy::JsonValue::Kind::Array ||
        inputs->array.size() != tx.inputs.size()) {
        error = "input evidence cardinality differs from transaction"; return false;
    }
    ...
    for (uint32_t i = 0; i < tx.inputs.size(); ++i) {
        ...
        if (!input.script_sig.empty() || input.sequence != 0xffffffffU ||
            ...
            error = "input is signed, non-final, or duplicated"; return false;
        }
        const auto& meta = inputs->array[i];
        if (meta.kind != btc_buy::JsonValue::Kind::Object) {
            error = "input evidence is not an object"; return false;
        }
        ...
        if (!ParseUintField(meta, "index", claimed_index) || claimed_index != i ||
            ...
            !btc_buy::IsLowerHex(claimed_script) ||
            !btc_buy::IsLowerHex(claimed_sighash, 64) ||
            !btc_buy::IsLowerHex(parent_hex)) {
            error = "input evidence is incomplete or non-canonical"; return false;
        }
        ...
        if (!DecodeLowerHex(parent_hex, kMaxParentTransactionBytes, parent_raw) ||
            ...
            error = "parent transaction or script exceeds policy"; return false;
        }
        Transaction parent;
        const size_t parent_consumed =
            Transaction::Deserialize(parent_raw, 0, parent);
        if (parent_consumed != parent_raw.size() ||
            parent.Serialize() != parent_raw ||
        ...
        if (input.prev_out_index >= parent.outputs.size()) {
            error = "referenced parent vout does not exist"; return false;
        }
        const auto& prevout = parent.outputs[input.prev_out_index];
        if (prevout.value != claimed_value ||
            prevout.script_pubkey != claimed_script_bytes ||
            prevout.script_pubkey != owned_script) {
            error = "authenticated parent value/script differs from proposal or key";
            return false;
        }
        if (verified.total_input >
            std::numeric_limits<uint64_t>::max() - prevout.value) {
            error = "input-total overflow"; return false;
        }
        verified.total_input += prevout.value;
        if (verified.total_input > MAX_SUPPLY_UNITS) {
            error = "input total exceeds network supply"; return false;
        }
        const std::string actual_sighash =
            BytesToHex(ComputeSighash(tx, i, prevout.script_pubkey));
        if (actual_sighash != claimed_sighash) {
            error = "input sighash differs from authenticated parent"; return false;
        }
    ...
    if (!ParseUintField(*proposal, "total_input", claimed_input) ||
        ...
        claimed_input != verified.total_input ||
        claimed_output != verified.total_output ||
        claimed_fee != verified.fee) {
        error = "proposal totals differ from authenticated transaction"; return false;
    }
    if (verified.fee > kHardAbsoluteFeeUnits) {
        error = "fee exceeds canonical absolute ceiling"; return false;
    }
    ...
    if (verified.signed_size == 0 ||
        verified.signed_size > kMaxUnsignedTransactionBytes ||
        verified.signed_size >
            std::numeric_limits<uint64_t>::max() /
                kHardFeeRateUnitsPerByte ||
        verified.fee > static_cast<uint64_t>(verified.signed_size) *
                           kHardFeeRateUnitsPerByte) {
        error = "fee rate exceeds canonical ceiling"; return false;
    }
    verified.fee_rate = (verified.fee + verified.signed_size - 1U) /
                        verified.signed_size;
...
inline bool ParseIntent(const btc_buy::JsonValue& root, Intent& out,
    ...
    if (!ParseStringField(*object, "version", intent.version) ||
        ...
        !ParseUintField(*object, "maximum_absolute_fee",
                        intent.maximum_absolute_fee) ||
        !ParseUintField(*object, "maximum_fee_rate",
                        intent.maximum_fee_rate) ||
        !ParseStringField(*object, "source_transactions_digest",
                          intent.source_transactions_digest) ||
        !ParseStringField(*object, "complete_output_digest",
                          intent.complete_output_digest) ||
        !ParseStringField(*object, "operation_identity_digest",
                          intent.operation_identity_digest) ||
        !ParseStringField(*object, "intent_digest", intent.intent_digest)) {
        error = "intent field types are non-canonical"; return false;
    }
    if (intent.version != kIntentVersion || intent.operation_type.empty() ||
        intent.operation_type.size() > 64U ||
        intent.intended_recipient.size() > 256U ||
        ...
        intent.intent_digest != IntentDigest(intent)) {
        error = "intent values or digest are non-canonical"; return false;
    }
    out = std::move(intent);
    return true;
}

inline bool ParseIntentAuthorization(
        const btc_buy::JsonValue& root, IntentAuthorization& out,
        std::string& error) {
    ...
    if (!signing_intent ||
        signing_intent->kind != btc_buy::JsonValue::Kind::Object ||
        !object || object->kind != btc_buy::JsonValue::Kind::Object ||
        object->object.size() != 6U) {
    ...
    if (!ParseStringField(*object, "version", auth.version) ||
        ...
        !ParseStringField(*object, "network_identity",
                          auth.network_identity) ||
        !ParseStringField(*object, "genesis_hash", auth.genesis_hash) ||
        !ParseStringField(*object, "authorizer_pubkey_digest",
                          auth.authorizer_pubkey_digest) ||
    ...
    if (auth.version != kIntentAuthorizationVersion ||
        auth.network_identity != DEPLOYMENT_PROFILE_ID ||
        auth.genesis_hash != GENESIS_HASH ||
...
inline bool VerifyIntentAuthorization(
        const Intent& intent, const IntentAuthorization& authorization,
    ...
    if (!DecodeLowerHex(authorization.signature_hex,
                        dilithium::SIG_MAX_BYTES, signature) ||
        !Verify(authorizer,
                IntentAuthorizationHash(intent, pubkey_digest), signature)) {
...
inline bool VerifyIntent(const Intent& intent,
                         const VerifiedPrepared& prepared,
                         const std::string& operation_type,
                         const std::string& intended_recipient,
                         uint64_t intended_amount,
                         const std::string& expected_change_destination,
                         uint64_t expected_change,
                         const std::string& operation_identity,
                         std::string& error) {
    if (intent.maximum_absolute_fee == 0 ||
        intent.maximum_absolute_fee > kHardAbsoluteFeeUnits ||
        intent.maximum_fee_rate == 0 ||
        intent.maximum_fee_rate > kHardFeeRateUnitsPerByte) {
        error = "intent fee ceilings are empty or exceed canonical policy";
        return false;
    }
    if (prepared.fee > intent.maximum_absolute_fee ||
        prepared.fee_rate > intent.maximum_fee_rate) {
        error = "prepared fee exceeds intent ceiling"; return false;
    }
    if (intent.operation_type != operation_type ||
        ...
        intent.expected_change != expected_change ||
        prepared.claimed_change != expected_change ||
        intent.source_transactions_digest !=
            prepared.source_transactions_digest ||
