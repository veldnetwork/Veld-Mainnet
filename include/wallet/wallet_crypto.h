    ...
    const uint8_t* msg, size_t mlen)
{
    ...
    if (klen > B) {
        ...
        auto d = h.digest(); memcpy(k, d.data(), 32);
    } else if (klen != 0) {
        memcpy(k, key, klen);
    }
    ...
    SHA256 outer; outer.update(opad, B); outer.update(ih.data(), 32);
    ...
    uint32_t iterations, uint32_t dklen)
{
    ...
    for (uint32_t blk = 1; dk.size() < dklen; ++blk) {
        ...
        if (slen != 0) memcpy(input.data(), salt, slen);
        input[slen+0] = (blk>>24)&0xFF; input[slen+1] = (blk>>16)&0xFF;
        input[slen+2] = (blk>> 8)&0xFF; input[slen+3] = (blk    )&0xFF;
...
inline std::vector<uint8_t> DeriveBrowserKeystoreKey(
    const std::string& password, const uint8_t* salt, size_t salt_len)
{
    ...
    return PBKDF2_SHA256(password, salt, salt_len,
                         BROWSER_KEYSTORE_PBKDF2_ITERS, 32);
}

struct ChaCha20 {
    uint32_t st[16]{};

    ~ChaCha20() { veld::compat::SecureZero(st, sizeof(st)); }
    ...
    void init(const uint8_t* key32, const uint8_t* nonce12, uint32_t ctr) {
        ...
        for (int i=0;i<8;++i) st[4+i]=le32(key32+i*4);
        st[12]=ctr;
        st[13]=le32(nonce12); st[14]=le32(nonce12+4); st[15]=le32(nonce12+8);
    }
    ...
    void block(uint8_t out[64]) {
        ...
        if (st[12] < prev) {
            throw std::runtime_error(
                "ChaCha20 block counter wrapped — refusing to reuse keystream "
    ...
    void keystream(const uint8_t* in, uint8_t* out, size_t len) {
        ...
        while (i < len) {
            ...
            for (size_t j=0; j<chunk; ++j) out[i+j] = in[i+j] ^ buf[j];
            i += chunk;
        }
    }
};
...
struct Poly1305 {
    ...
    void init(const uint8_t key[32]) {
        ...
        pad[0]=(uint32_t)key[16]|((uint32_t)key[17]<<8)|((uint32_t)key[18]<<16)|((uint32_t)key[19]<<24);
        pad[1]=(uint32_t)key[20]|((uint32_t)key[21]<<8)|((uint32_t)key[22]<<16)|((uint32_t)key[23]<<24);
        pad[2]=(uint32_t)key[24]|((uint32_t)key[25]<<8)|((uint32_t)key[26]<<16)|((uint32_t)key[27]<<24);
        pad[3]=(uint32_t)key[28]|((uint32_t)key[29]<<8)|((uint32_t)key[30]<<16)|((uint32_t)key[31]<<24);
    }
    ...
    void block(const uint8_t* m, size_t len, bool final_block) {
        ...
        uint32_t m0 = (uint32_t)buf[ 0]|((uint32_t)buf[ 1]<<8)|((uint32_t)buf[ 2]<<16)|((uint32_t)buf[ 3]<<24);
        uint32_t m1 = (uint32_t)buf[ 4]|((uint32_t)buf[ 5]<<8)|((uint32_t)buf[ 6]<<16)|((uint32_t)buf[ 7]<<24);
        uint32_t m2 = (uint32_t)buf[ 8]|((uint32_t)buf[ 9]<<8)|((uint32_t)buf[10]<<16)|((uint32_t)buf[11]<<24);
        uint32_t m3 = (uint32_t)buf[12]|((uint32_t)buf[13]<<8)|((uint32_t)buf[14]<<16)|((uint32_t)buf[15]<<24);
        uint32_t m4 = final_block ? 0 : (uint32_t)buf[16];

        uint64_t f;
        f = (uint64_t)h[0]+m0;           h[0]=(uint32_t)f;
        f = (uint64_t)h[1]+m1+(f>>32);   h[1]=(uint32_t)f;
        f = (uint64_t)h[2]+m2+(f>>32);   h[2]=(uint32_t)f;
        f = (uint64_t)h[3]+m3+(f>>32);   h[3]=(uint32_t)f;
        f = (uint64_t)h[4]+m4+(f>>32);   h[4]=(uint32_t)f;

        uint64_t s1=r[1]*5, s2=r[2]*5, s3=r[3]*5;
        uint64_t d0 = (uint64_t)h[0]*r[0] + (uint64_t)h[1]*s3 + (uint64_t)h[2]*s2 + (uint64_t)h[3]*s1;
        uint64_t d1 = (uint64_t)h[0]*r[1] + (uint64_t)h[1]*r[0] + (uint64_t)h[2]*s3 + (uint64_t)h[3]*s2;
        uint64_t d2 = (uint64_t)h[0]*r[2] + (uint64_t)h[1]*r[1] + (uint64_t)h[2]*r[0] + (uint64_t)h[3]*s3;
        uint64_t d3 = (uint64_t)h[0]*r[3] + (uint64_t)h[1]*r[2] + (uint64_t)h[2]*r[1] + (uint64_t)h[3]*r[0];
        uint64_t d4 = (uint64_t)h[4];

        uint32_t c;
        c=(uint32_t)(d0>>32); h[0]=(uint32_t)d0; d1+=c;
        c=(uint32_t)(d1>>32); h[1]=(uint32_t)d1; d2+=c;
        c=(uint32_t)(d2>>32); h[2]=(uint32_t)d2; d3+=c;
        c=(uint32_t)(d3>>32); h[3]=(uint32_t)d3; d4+=c;
        h[4]=(uint32_t)d4;

        c = (h[4]>>2)*5; h[4] &= 3;
        f = (uint64_t)h[0]+c;           h[0]=(uint32_t)f; c=(uint32_t)(f>>32);
        f = (uint64_t)h[1]+c;           h[1]=(uint32_t)f; c=(uint32_t)(f>>32);
        f = (uint64_t)h[2]+c;           h[2]=(uint32_t)f; c=(uint32_t)(f>>32);
        f = (uint64_t)h[3]+c;           h[3]=(uint32_t)f; c=(uint32_t)(f>>32);
        h[4]+=c;
    }
    ...
    std::array<uint8_t,16> final_tag() {
        uint32_t g[5];
        uint64_t f;
        f = (uint64_t)h[0]+5;           g[0]=(uint32_t)f;
        f = (uint64_t)h[1]+(f>>32);     g[1]=(uint32_t)f;
        f = (uint64_t)h[2]+(f>>32);     g[2]=(uint32_t)f;
        f = (uint64_t)h[3]+(f>>32);     g[3]=(uint32_t)f;
        f = (uint64_t)h[4]+(f>>32)-4;   g[4]=(uint32_t)f;
        ...
        for (int i=0;i<4;++i) {
            tag[i   ]=(h[0]>>(i*8))&0xFF; tag[i+ 4]=(h[1]>>(i*8))&0xFF;
            tag[i+ 8]=(h[2]>>(i*8))&0xFF; tag[i+12]=(h[3]>>(i*8))&0xFF;
        }
        return tag;
    }
};

inline std::array<uint8_t,16> poly1305_tag(
    const uint8_t poly_key[32],
    const uint8_t* cipher, size_t clen)
{
    Poly1305 p; p.init(poly_key);
...
inline CipherResult ChaCha20Poly1305_Encrypt(
    const uint8_t* key32, const uint8_t* nonce12,
    const uint8_t* plaintext, size_t plen)
{
    ChaCha20 c0; c0.init(key32, nonce12, 0);
    uint8_t poly_key[64]{}; uint8_t zeros[64]{};
    c0.keystream(zeros, poly_key, 64);

    CipherResult res;
    res.data.resize(plen);
    ChaCha20 enc; enc.init(key32, nonce12, 1);
    ...
    const uint8_t* expected_tag)
{
    ...
    for (int i=0;i<16;++i) diff |= (uint8_t)(tag[i] ^ expected_tag[i]);
    veld::compat::SecureZero(poly_key, sizeof(poly_key));
    veld::compat::SecureZero(zeros, sizeof(zeros));
    if (diff != 0) {
    ...
    ChaCha20 dec; dec.init(key32, nonce12, 1);
    dec.keystream(cipher, plain.data(), clen);
    veld::compat::SecureZero(tag.data(), tag.size());
    return plain;
}

inline CipherResult ChaCha20Poly1305_EVP_Encrypt(
    const uint8_t* key32, const uint8_t* nonce12,
    ...
    const uint8_t* plaintext, size_t plen)
{
    ...
    if (!::veld::vendored_crypto::chacha20_poly1305_encrypt(
            key32, nonce12, aad, aad_len,
            plaintext, plen, res.data.data(), res.tag.data())) {
        throw std::runtime_error("ChaCha20-Poly1305 encrypt failed");
    }
    return res;
}

inline std::vector<uint8_t> ChaCha20Poly1305_EVP_Decrypt(
    const uint8_t* key32, const uint8_t* nonce12,
    const uint8_t* aad, size_t aad_len,
    const uint8_t* cipher, size_t clen,
    const uint8_t* expected_tag)
{
    std::vector<uint8_t> plain(clen);
    if (!::veld::vendored_crypto::chacha20_poly1305_decrypt(
            key32, nonce12, aad, aad_len,
            cipher, clen, expected_tag, plain.data())) {
        if (!plain.empty()) veld::compat::SecureZero(plain.data(), plain.size());
        throw std::runtime_error("Wallet decryption failed — wrong password or file corrupted");
    }
    return plain;
}
...
inline void _salsa20_8(uint8_t B[64]) {
    ...
    uint32_t x[16], in[16];
    for (int i = 0; i < 16; ++i)
        in[i] = x[i] = (uint32_t)B[4*i] | ((uint32_t)B[4*i+1]<<8) | ((uint32_t)B[4*i+2]<<16) | ((uint32_t)B[4*i+3]<<24);
    for (int i = 8; i > 0; i -= 2) {
    ...
    for (int i = 0; i < 16; ++i) { uint32_t v = x[i] + in[i];
        B[4*i]=(uint8_t)v; B[4*i+1]=(uint8_t)(v>>8); B[4*i+2]=(uint8_t)(v>>16); B[4*i+3]=(uint8_t)(v>>24); }
...
inline void _scrypt_blockmix(const uint8_t* B, uint8_t* Y, size_t r) {
    uint8_t X[64]; std::memcpy(X, B + (2*r - 1)*64, 64);
...
inline void _scrypt_romix(uint8_t* B, size_t r, uint64_t N) {
    ...
    for (uint64_t i = 0; i < N; ++i) { std::memcpy(V.data() + (size_t)i*blen, B, blen); _scrypt_blockmix(B, Y.data(), r); std::memcpy(B, Y.data(), blen); }
    for (uint64_t i = 0; i < N; ++i) {
        uint64_t j = _scrypt_integerify(B, r) & (N - 1);     // N is a power of 2
        const uint8_t* Vj = V.data() + (size_t)j*blen;
        for (size_t k = 0; k < blen; ++k) B[k] ^= Vj[k];
        _scrypt_blockmix(B, Y.data(), r); std::memcpy(B, Y.data(), blen);
    }
}
inline std::vector<uint8_t> Scrypt(const std::string& P, const uint8_t* S, size_t slen,
    ...
    if (static_cast<uint64_t>(p)
        > std::numeric_limits<uint64_t>::max() / block_len64)
    ...
    if (block_len64 > std::numeric_limits<size_t>::max()
        ...
        throw std::length_error("scrypt parameters overflow addressable memory");
    std::vector<uint8_t> B = PBKDF2_SHA256(
        P, S, slen, 1, static_cast<uint32_t>(b_len64));
    ScopedByteWipe<std::vector<uint8_t>> wipe_b{B};
    for (uint32_t i = 0; i < p; ++i) _scrypt_romix(B.data() + (size_t)i * 128 * r, r, N);
...
inline std::vector<uint8_t> DeriveBrowserKeystoreKeyV3(
    const std::string& password, const uint8_t* salt, size_t salt_len)
{
    if (!salt || salt_len != BROWSER_KEYSTORE_SALT_BYTES)
        throw std::invalid_argument("browser keystore salt must be exactly 16 bytes");
    return Scrypt(password, salt, salt_len,
                  BROWSER_KEYSTORE_SCRYPT_N,
                  BROWSER_KEYSTORE_SCRYPT_R,
...
{
    return encrypted.size() >= 3
        && encrypted[0] == VELD_WALLET_MAGIC[0]
        && encrypted[1] == VELD_WALLET_MAGIC[1]
        && encrypted[2] == VELD_WALLET_VERSION_CURRENT;
}

inline std::vector<uint8_t> EncryptWallet(
    const std::string& plaintext, const std::string& password)
{
    ...
    auto key = Scrypt(password, salt, 32, VELD_WALLET_SCRYPT_N, VELD_WALLET_SCRYPT_R, VELD_WALLET_SCRYPT_P, 32);
    ...
    auto result = ChaCha20Poly1305_EVP_Encrypt(
        key.data(), nonce,
        aad, sizeof(aad),
...
inline std::string DecryptWallet(
    const std::vector<uint8_t>& encrypted, const std::string& password)
{
    if (encrypted.size() >= 63 &&
        encrypted[0] == VELD_WALLET_MAGIC[0] &&
        encrypted[1] == VELD_WALLET_MAGIC[1]) {
        ...
        switch (version) {
            ...
            default:
                throw std::runtime_error(
                    "Unknown wallet file version 0x" +
                    std::to_string(version) +
                    " — this binary does not know how to decrypt it");
        }
        auto key = use_scrypt
            ? Scrypt(password, salt, 32, scrypt_n, scrypt_r, scrypt_p, 32)
        ...
        if (use_evp_aad) {
            ...
            std::memcpy(aad + 3,  salt,  32);
            std::memcpy(aad + 35, nonce, 12);
            plain = ChaCha20Poly1305_EVP_Decrypt(key.data(), nonce,
                                                 aad, sizeof(aad),
                                                 ciph, clen, tag);
        } else {
    ...
    throw std::runtime_error(
        "Pre-v1 (magic-less) wallet format is deprecated because its AEAD "
