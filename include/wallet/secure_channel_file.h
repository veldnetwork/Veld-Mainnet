...
struct WinOwnerSecurity {
    ...
    PSID sid() {
        return token_user.empty() ? nullptr
                                  : reinterpret_cast<TOKEN_USER*>(token_user.data())->User.Sid;
    }
    PACL acl() {
        return acl_storage.empty() ? nullptr : reinterpret_cast<PACL>(acl_storage.data());
    }
    ...
    bool Initialize(std::string* error, DWORD ace_flags = 0) {
        ...
        if (!::GetTokenInformation(token.value, TokenUser, token_user.data(), bytes, &bytes) ||
            !::IsValidSid(sid())) {
...
inline bool ValidWindowsTarget(const std::filesystem::path& target, std::string* error) {
    if (!ValidTarget(target, error))
        return false;
    ...
    if (leaf.find(L':') != std::wstring::npos || leaf.back() == L'.' || leaf.back() == L' ') {
...
inline bool HandleHasCurrentOwner(HANDLE handle, PSID current_sid, bool require_private,
                                  std::string* error, const char* label) {
    PSID object_owner = nullptr;
    PACL returned_dacl = nullptr;
    WinLocalSecurityDescriptor holder;
    const SECURITY_INFORMATION requested =
        OWNER_SECURITY_INFORMATION | (require_private ? DACL_SECURITY_INFORMATION : 0);
    const DWORD status =
        ::GetSecurityInfo(handle, SE_FILE_OBJECT, requested, &object_owner, nullptr,
                          require_private ? &returned_dacl : nullptr, nullptr, &holder.value);
    if (status != ERROR_SUCCESS || !holder.value || !object_owner || !::IsValidSid(object_owner) ||
    ...
    DWORD revision = 0;
    if (!::GetSecurityDescriptorDacl(holder.value, &present, &dacl, &defaulted) || !present ||
        !dacl || dacl != returned_dacl || !::IsValidAcl(dacl) ||
        !::GetSecurityDescriptorControl(holder.value, &control, &revision) ||
        !(control & SE_DACL_PROTECTED)) {
        SetError(error, std::string(label) + " must have a present protected owner-only DACL");
        return false;
    }
    ...
    for (DWORD index = 0; index < dacl->AceCount; ++index) {
        ...
        if (header->AceType != ACCESS_ALLOWED_ACE_TYPE ||
            ...
            SetError(error, std::string(label) + " has an unsupported or non-owner allow ACE");
            return false;
        }
...
inline bool RegularSingleLink(HANDLE handle, BY_HANDLE_FILE_INFORMATION* out, std::string* error,
                              const char* label) {
    BY_HANDLE_FILE_INFORMATION info{};
    if (!::GetFileInformationByHandle(handle, &info) ||
        (info.dwFileAttributes & (FILE_ATTRIBUTE_DIRECTORY | FILE_ATTRIBUTE_REPARSE_POINT)) ||
        info.nNumberOfLinks != 1) {
        SetError(error, std::string(label) + " must be a regular, non-reparse, single-link file");
        return false;
    }
    if (out)
        *out = info;
...
inline bool FinalPathForHandle(HANDLE handle, std::filesystem::path& out, std::string* error) {
...
inline bool OpenParent(const std::filesystem::path& target, bool require_private_parent,
                       WinOwnerSecurity& owner, WinParent& out, std::string* error) {
    if (!ValidWindowsTarget(target, error))
        return false;
    const auto parent =
        target.parent_path().empty() ? std::filesystem::path(L".") : target.parent_path();
    WinHandle handle(::CreateFileW(parent.c_str(), FILE_READ_ATTRIBUTES | READ_CONTROL,
    ...
    if (!HandleHasCurrentOwner(handle.value, owner.sid(), require_private_parent, error,
                               "parent directory"))
        return false;
    if (!FinalPathForHandle(handle.value, out.canonical, error))
        return false;
...
inline ExistingTarget InspectExistingTarget(const std::filesystem::path& target,
                                            WinOwnerSecurity& owner, std::string* error) {
    WinHandle handle(::CreateFileW(target.c_str(), FILE_READ_ATTRIBUTES | READ_CONTROL,
                                   FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING,
    ...
    if (!RegularSingleLink(handle.value, nullptr, error, "existing target") ||
        !HandleHasCurrentOwner(handle.value, owner.sid(), false, error, "existing target"))
        return ExistingTarget::Unsafe;
    return ExistingTarget::Safe;
}

inline bool ValidatePrivateFilePath(const std::filesystem::path& target, WinOwnerSecurity& owner,
                                    std::string* error) {
    WinHandle handle(::CreateFileW(target.c_str(), FILE_READ_ATTRIBUTES | READ_CONTROL,
                                   FILE_SHARE_READ, nullptr, OPEN_EXISTING,
                                   FILE_ATTRIBUTE_NORMAL | FILE_FLAG_OPEN_REPARSE_POINT, nullptr));
    return handle && RegularSingleLink(handle.value, nullptr, error, "published file") &&
           HandleHasCurrentOwner(handle.value, owner.sid(), true, error, "published file");
}

inline bool CreateOwnerOnlyTemporary(const std::filesystem::path& target, WinOwnerSecurity& owner,
                                     std::filesystem::path& tmp, WinHandle& handle,
                                     std::string* error, const char* label) {
    for (unsigned i = 0; i < 128 && !handle; ++i) {
        tmp = target;
        tmp += L".tmp." + std::to_wstring(::GetCurrentProcessId()) + L"." + std::to_wstring(i);
        handle = WinHandle(::CreateFileW(
            tmp.c_str(), GENERIC_WRITE | FILE_READ_ATTRIBUTES | READ_CONTROL, 0, &owner.attributes,
            CREATE_NEW,
            FILE_ATTRIBUTE_NORMAL | FILE_FLAG_WRITE_THROUGH | FILE_FLAG_OPEN_REPARSE_POINT,
            nullptr));
        if (!handle && ::GetLastError() != ERROR_FILE_EXISTS)
            break;
    }
    if (!handle) {
    ...
    if (!RegularSingleLink(handle.value, nullptr, error, label) ||
        !HandleHasCurrentOwner(handle.value, owner.sid(), true, error, label)) {
...
inline bool WriteAndFlush(HANDLE handle, const uint8_t* data, size_t size, std::string* error,
                          const char* label) {
    ...
    if (!::FlushFileBuffers(handle) || !RegularSingleLink(handle, nullptr, error, label)) {
        if (error && error->empty())
            SetError(error, std::string(label) + " flush failed");
        return false;
    }
    return true;
}
...
inline bool AtomicWrite(const std::string& path, const uint8_t* data, size_t size,
    ...
    if (!owner.Initialize(error))
        return false;
    WinParent parent;
    if (!OpenParent(std::filesystem::path(path), require_private_parent, owner, parent, error))
        return false;
    ...
    WinHandle handle;
    if (!CreateOwnerOnlyTemporary(target, owner, tmp, handle, error, "temporary file"))
        return false;
    ...
    if (ok) {
        ...
        for (unsigned attempt = 0;; ++attempt) {
            ...
            if (ok)
                break;
            const DWORD code = ::GetLastError();
            if (attempt == 20 || (code != ERROR_SHARING_VIOLATION && code != ERROR_LOCK_VIOLATION &&
    ...
    if (ok)
        ok = ValidatePrivateFilePath(target, owner, error);
    if (!ok) {
        ::DeleteFileW(tmp.c_str());
        if (error && error->empty())
            SetError(error, "secure write/flush/replace failed");
    }
    return ok;
}
...
inline bool AtomicWriteNew(const std::string& path, const uint8_t* data, size_t size,
    ...
    std::filesystem::path tmp;
    WinHandle handle;
    if (!CreateOwnerOnlyTemporary(target, owner, tmp, handle, error, "immutable temporary file"))
        return false;
    bool ok = WriteAndFlush(handle.value, data, size, error, "immutable temporary file");
    const HANDLE raw = handle.release();
    if (!::CloseHandle(raw))
        ok = false;
    ...
    if (ok)
        ok = ::MoveFileExW(tmp.c_str(), target.c_str(), MOVEFILE_WRITE_THROUGH) != 0;
    if (ok)
        ok = ValidatePrivateFilePath(target, owner, error);
    if (!ok) {
        ::DeleteFileW(tmp.c_str());
...
inline ReadResult Read(const std::string& path, std::vector<uint8_t>& out,
    ...
    WinHandle handle(::CreateFileW(
        target.c_str(), GENERIC_READ | FILE_READ_ATTRIBUTES | READ_CONTROL, FILE_SHARE_READ,
    ...
    bool ok = RegularSingleLink(handle.value, &before, error, "private file") &&
              HandleHasCurrentOwner(handle.value, owner.sid(), true, error, "private file") &&
              ::GetFileSizeEx(handle.value, &n) && n.QuadPart >= 0 &&
              static_cast<unsigned long long>(n.QuadPart) <= max_size;
    if (ok)
        out.resize(static_cast<size_t>(n.QuadPart));
    size_t got = 0;
    while (ok && got < out.size()) {
        DWORD nr = 0;
        const DWORD want = static_cast<DWORD>(std::min<size_t>(out.size() - got, 1u << 30));
        ok = ::ReadFile(handle.value, out.data() + got, want, &nr, nullptr) && nr != 0;
        got += nr;
    }
    ...
    if (ok) {
        ok = RegularSingleLink(handle.value, &after, error, "private file") &&
             ::GetFileSizeEx(handle.value, &final_size) && final_size.QuadPart == n.QuadPart &&
...
inline bool EnsurePrivateDirectory(const std::string& path, std::string* error = nullptr) {
    ...
    WinHandle handle(
        ::CreateFileW(directory.c_str(), FILE_READ_ATTRIBUTES | READ_CONTROL | WRITE_DAC,
                      FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING,
    ...
    if (!handle || ::GetFileType(handle.value) != FILE_TYPE_DISK ||
        ...
        !(info.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) ||
        (info.dwFileAttributes & FILE_ATTRIBUTE_REPARSE_POINT) ||
        !HandleHasCurrentOwner(handle.value, owner.sid(), false, error, "private directory")) {
        if (error && error->empty())
            SetError(error, "directory must be real and owned by the current Windows user");
        return false;
    }
    const DWORD status =
        ::SetSecurityInfo(handle.value, SE_FILE_OBJECT,
                          DACL_SECURITY_INFORMATION | PROTECTED_DACL_SECURITY_INFORMATION, nullptr,
                          nullptr, owner.acl(), nullptr);
    if (status != ERROR_SUCCESS ||
        !HandleHasCurrentOwner(handle.value, owner.sid(), true, error, "private directory")) {
        if (error && error->empty())
...
inline bool OpenParent(const std::filesystem::path& target, bool require_private_parent,
    ...
    struct stat st {};
    const bool safe = ::fstat(fd, &st) == 0 && S_ISDIR(st.st_mode) && st.st_uid == ::geteuid() &&
...
inline bool AtomicWrite(const std::string& path, const uint8_t* data, size_t size,
    ...
    if (fd < 0) {
        SetError(error,
                 std::string("cannot create exclusive temporary file: ") + std::strerror(errno));
        ::close(parent.fd);
        return false;
    }
    ...
    if (ok)
        ok = ::fsync(parent.fd) == 0;
    if (!renamed)
        ::unlinkat(parent.fd, tmp.c_str(), 0);
    if (::close(parent.fd) != 0)
        ok = false;
    if (!ok)
        SetError(error, std::string("secure write/flush/rename failed: ") + std::strerror(errno));
    return ok;
}
...
inline bool AtomicWriteNew(const std::string& path, const uint8_t* data, size_t size,
    ...
    if (::fstatat(parent.fd, parent.base.c_str(), &existing, AT_SYMLINK_NOFOLLOW) == 0) {
        ::close(parent.fd);
        SetError(error, "immutable target already exists");
        return false;
    }
    if (errno != ENOENT) {
        ...
        ::close(parent.fd);
        SetError(error, std::string("cannot inspect immutable target: ") + std::strerror(saved));
        return false;
    }
    std::string tmp;
    int fd = -1;
    ...
    if (fd < 0) {
        SetError(error,
                 std::string("cannot create immutable temporary file: ") + std::strerror(errno));
        ::close(parent.fd);
        return false;
    }
    struct stat st {};
...
inline ReadResult Read(const std::string& path, std::vector<uint8_t>& out,
    ...
    if (!ok) {
        WipeAndClear(out);
        SetError(error,
                 "file must be regular, single-link, owner-only, owner-matched, and bounded");
        return ReadResult::Error;
    }
    return ReadResult::Ok;
}
...
inline ReadResult ReadExplicitImport(const std::string& path, std::vector<uint8_t>& out,
                                     std::string* error = nullptr,
                                     size_t max_size = 4u * 1024u * 1024u) {
    WipeAndClear(out);
#ifdef _WIN32
    const std::filesystem::path target(path);
    if (!ValidTarget(target, error))
        return ReadResult::Error;
    HANDLE h = ::CreateFileW(target.c_str(), GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
                             FILE_ATTRIBUTE_NORMAL | FILE_FLAG_OPEN_REPARSE_POINT, nullptr);
    ...
    LARGE_INTEGER size{};
    bool ok =
        ::GetFileInformationByHandle(h, &before) &&
        !(before.dwFileAttributes & (FILE_ATTRIBUTE_DIRECTORY | FILE_ATTRIBUTE_REPARSE_POINT)) &&
        before.nNumberOfLinks == 1 && ::GetFileSizeEx(h, &size) && size.QuadPart >= 0 &&
    ...
    LARGE_INTEGER final_size{};
    if (ok) {
        ok = ::GetFileInformationByHandle(h, &after) && ::GetFileSizeEx(h, &final_size) &&
             final_size.QuadPart == size.QuadPart &&
             after.dwVolumeSerialNumber == before.dwVolumeSerialNumber &&
             after.nFileIndexHigh == before.nFileIndexHigh &&
             after.nFileIndexLow == before.nFileIndexLow && after.nNumberOfLinks == 1;
    }
    if (!::CloseHandle(h))
        ok = false;
#else
    ...
    struct stat after {};
    if (ok) {
        ok = ::fstat(fd, &after) == 0 && S_ISREG(after.st_mode) && after.st_uid == before.st_uid &&
...
inline bool AtomicWriteNew(const std::string& path, const std::vector<uint8_t>& data,
                           std::string* error = nullptr, bool require_private_parent = true) {
    return AtomicWriteNew(path, data.data(), data.size(), error, require_private_parent);
}

inline bool AtomicWriteText(const std::string& path, const std::string& data,
                            std::string* error = nullptr, bool require_private_parent = true) {
    return AtomicWrite(path, reinterpret_cast<const uint8_t*>(data.data()), data.size(), error,
