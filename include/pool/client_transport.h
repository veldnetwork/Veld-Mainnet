...
inline const Json& Field(const Json& object, const char* name) {
    ...
    Require(value != nullptr, "missing pool field");
    return *value;
}
inline std::string Text(const Json& value) {
    Require(value.kind == Json::Kind::String, "pool string required");
    return value.text;
}
inline uint64_t Number(const std::string& text, uint64_t maximum = UINT64_MAX) {
    ...
    Require(result.ec == std::errc{} && result.ptr == text.data() + text.size() &&
                text == std::to_string(value) && value <= maximum,
            "pool integer encoding");
    return value;
}
...
struct Endpoint {
    ...
    explicit Endpoint(const std::string& url) {
        ...
        if (!authority.empty() && authority.back() == '/')
            authority.pop_back();
        Require(!authority.empty() &&
                    authority.find_first_of("/@?#%\\ \t\r\n") == std::string::npos,
                "pool endpoint authority");
        if (authority[0] == '[') {
            const auto close = authority.find(']');
            Require(close != std::string::npos, "IPv6 pool endpoint");
            host = authority.substr(1, close - 1);
            Require(close + 1 == authority.size() || authority[close + 1] == ':',
                    "pool endpoint port");
            port = close + 1 == authority.size() ? "443" : authority.substr(close + 2);
            Require(host.find_first_not_of("0123456789abcdefABCDEF:") == std::string::npos,
                    "IPv6 pool host");
        } else {
            const auto colon = authority.find(':');
            host = authority.substr(0, colon);
            port = colon == std::string::npos ? "443" : authority.substr(colon + 1);
            Require(host.find_first_not_of(
...
struct Connection {
    ...
    Connection(const Endpoint& endpoint, std::chrono::steady_clock::time_point deadline,
        ...
        for (auto candidate = addresses; candidate; candidate = candidate->ai_next)
        ...
        for (const auto candidate : ordered) {
            ...
            if (!compat::IsValidSocket(socket))
                continue;
#ifdef _WIN32
            ...
            if (connected) {
                ...
                if (static_cast<uint64_t>(socket) <= INT_MAX) {
                    family = candidate->ai_family;
                    return;
                }
            }
            VELD_CLOSE_SOCKET(socket);
...
class TlsClient {
  ...
  public:
    TlsClient(const std::string& endpoint, const std::string& ca) : endpoint_(endpoint) {
        ...
        Require(bool(context_), "TLS initialization failed");
        Require(SSL_CTX_set_min_proto_version(context_.get(), TLS1_2_VERSION) == 1,
                "TLS minimum version failed");
        ...
        if (!ca.empty()) {
            Require(SSL_CTX_load_verify_locations(context_.get(), ca.c_str(), nullptr) == 1,
                    "pool CA unavailable");
        } else {
#ifdef _WIN32
            HCERTSTORE roots =
            ...
            while ((certificate = CertEnumCertificatesInStore(roots, certificate)) != nullptr) {
                ...
                if (certificate->cbCertEncoded > 512 * 1024)
                    continue;
                const auto* bytes = certificate->pbCertEncoded;
                X509* parsed = d2i_X509(nullptr, &bytes, certificate->cbCertEncoded);
                if (parsed) {
                    count +=
                        X509_STORE_add_cert(SSL_CTX_get_cert_store(context_.get()), parsed) == 1;
                    X509_free(parsed);
                    ERR_clear_error();
                }
            }
            CertCloseStore(roots, 0);
            Require(count > 0, "Windows trust store has no usable roots");
#else
    ...
    Json Call(const std::string& action, const std::string& payload,
        ...
        Require(action == "register" || action == "work" || action == "submit" ||
                    action == "account",
                "pool action");
        ...
        auto ssl = session.get();
        Require(ssl != nullptr, "TLS session unavailable");
        Require(SSL_set_fd(ssl, static_cast<int>(connection.socket)) == 1,
                "TLS socket setup failed");
        ...
        Require(ip ? X509_VERIFY_PARAM_set1_ip_asc(parameters, endpoint_.host.c_str()) == 1
                   : SSL_set1_host(ssl, endpoint_.host.c_str()) == 1,
                "TLS peer identity setup failed");
        if (!ip)
            Require(SSL_set_tlsext_host_name(ssl, endpoint_.host.c_str()) == 1,
                    "TLS server name failed");
        ...
        for (;;) {
            ...
            Require(SSL_get_verify_result(ssl) == X509_V_OK,
                    "pool certificate verification failed");
            again(result);
        }
        Require(SSL_get_verify_result(ssl) == X509_V_OK, "pool certificate verification failed");
        std::unique_ptr<X509, decltype(&X509_free)> certificate(SSL_get1_peer_certificate(ssl),
                                                                X509_free);
        ...
        for (size_t offset = 0; offset < request.size();) {
            ERR_clear_error();
            const auto written =
                SSL_write(ssl, request.data() + offset, int(request.size() - offset));
            if (written > 0)
        ...
        std::string response;
        size_t body_at = std::string::npos, length = 0;
        int status = 0;
        std::array<char, 2048> bytes{};
        for (;;) {
            ERR_clear_error();
            const int count = SSL_read(ssl, bytes.data(), int(bytes.size()));
            if (count <= 0) {
                again(count);
                continue;
            }
            response.append(bytes.data(), size_t(count));
            Require(response.size() <= 24576, "pool response bound");
            if (body_at == std::string::npos) {
                const auto end = response.find("\r\n\r\n");
                Require(end != std::string::npos || response.size() <= 8192,
                        "pool HTTP header bound");
                if (end == std::string::npos)
                    continue;
                Require(end <= 8192 && response.rfind("HTTP/1.1 ", 0) == 0 && response.size() > 12,
                        "pool HTTP status");
                status = int(Number(response.substr(9, 3), 599));
                Require(status >= 100 && response[12] == ' ', "pool HTTP status framing");
                ...
                for (size_t pos = first + 2; pos < end;) {
                    ...
                    Require(next != std::string::npos && colon < next && colon > pos,
                            "pool HTTP header framing");
                    std::string name = response.substr(pos, colon - pos),
                                value = response.substr(colon + 1, next - colon - 1);
                    Require(name.find_first_not_of(
                                "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ-") ==
                                std::string::npos,
                            "pool HTTP header name");
                    std::transform(name.begin(), name.end(), name.begin(),
                                   [](unsigned char c) { return char(std::tolower(c)); });
                    while (!value.empty() && value.front() == ' ')
                        value.erase(0, 1);
                    Require(headers.emplace(name, value).second, "duplicate pool HTTP header");
                    pos = next + 2;
                }
                ...
                if (status == 429)
                    throw Retry("pool rate limit; retrying");
                if (status == 408 || status == 425 || status == 500 || status == 502 ||
                    status == 503 || status == 504 || (status >= 520 && status <= 527) ||
                    status == 530)
                    throw Retry("pool service temporarily unavailable");
                Require(!headers.contains("transfer-encoding") &&
                            headers.contains("content-length") &&
                            headers["content-type"] == "application/json",
                        "pool HTTP response schema");
                length = size_t(Number(headers["content-length"], 16384));
                Require(length > 0, "empty pool response");
                body_at = end + 4;
            }
            if (body_at != std::string::npos && response.size() >= body_at + length)
                break;
        }
        Require(response.size() == body_at + length, "trailing pool response bytes");
        const auto result = Parse(response.substr(body_at));
        const auto& ok = Field(result, "ok");
        Require(ok.kind == Json::Kind::Bool, "pool response status");
        ...
        const auto& value = Field(result, "result");
        Require(value.kind == Json::Kind::Object, "pool result schema");
        return value;
    }
};
} // namespace veld::pool
