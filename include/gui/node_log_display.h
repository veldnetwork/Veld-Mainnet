...
inline std::string DisplayLogLine(const std::string& line) {
    ...
    if (!event || event->kind != btc_buy::JsonValue::Kind::String || event->text != "connection" ||
        !reason || reason->kind != btc_buy::JsonValue::Kind::String)
        return line;
    if (reason->text == "connected")
        return "[network] Peer connected.";
    if (reason->text == "unspecified")
        return "[network] Peer connection closed.";
    if (reason->text == "remote_eof")
        return "[network] Peer closed its connection.";
    if (reason->text == "node_stop")
        return "[network] Peer disconnected for node shutdown.";
    for (const auto* known :
         {"receive_error", "send_error", "send_timeout", "invalid_frame", "receive_limit",
          "handshake_timeout", "idle_or_frame_timeout", "policy_rejection", "poll_error"}) {
        if (reason->text != known)
            continue;
