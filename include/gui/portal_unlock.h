...
inline std::string PortalBase64(const std::vector<uint8_t>& bytes) {
    ...
    if (!CryptBinaryToStringA(bytes.data(), static_cast<DWORD>(bytes.size()),
                              CRYPT_STRING_BASE64 | CRYPT_STRING_NOCRLF, value.data(), &size))
        return {};
...
inline bool PortalDecode(const std::string& value, size_t minimum, size_t maximum,
    ...
    while (padded.size() % 4)
        padded.push_back('=');
    DWORD size = 0;
    if (!CryptStringToBinaryA(padded.data(), static_cast<DWORD>(padded.size()), CRYPT_STRING_BASE64,
...
struct PortalUnlockPayload {
    ...
    bool Valid() const {
        ...
        return digest(identity) && digest(key_id) && PortalDecode(iv, 12, 12, decoded) &&
               PortalDecode(wrapped_key, 256, 256, decoded) &&
               PortalDecode(ciphertext, 17, 1040, decoded);
    }
    ...
    std::string Canonical() const {
        ...
        return "{\"ciphertext\":\"" + ciphertext + "\",\"identity\":\"" + identity +
               "\",\"iv\":\"" + iv + "\",\"key_id\":\"" + key_id + "\",\"wrapped_key\":\"" +
               wrapped_key + "\"}";
    }
};
...
class PortalUnlockKey {
    ...
    bool PublicInfo() {
        ...
        if (BCryptExportKey(key_, nullptr, BCRYPT_RSAPUBLIC_BLOB, blob.data(), size, &size, 0) <
                0 ||
        ...
        if (header.Magic != BCRYPT_RSAPUBLIC_MAGIC || header.BitLength != 2048 ||
            ...
            size != sizeof(header) + header.cbPublicExp + header.cbModulus)
            return false;
        exponent_ = PortalBase64(
            std::vector<uint8_t>(blob.begin() + sizeof(header), blob.begin() + sizeof(header) + 3));
        modulus_ =
            PortalBase64(std::vector<uint8_t>(blob.begin() + sizeof(header) + 3, blob.end()));
        if (exponent_ != "AQAB")
            return false;
  ...
  public:
    ...
    ~PortalUnlockKey() {
        ...
        if (rsa_)
            BCryptCloseAlgorithmProvider(rsa_, 0);
    }
    ...
    bool Ensure(const std::filesystem::path& path) {
        ...
        struct Attempt {
            ...
            ~Attempt() {
                if (ready)
                    return;
                if (owner.key_)
                    BCryptDestroyKey(owner.key_);
                owner.key_ = nullptr;
                owner.modulus_.clear();
                owner.exponent_.clear();
                owner.id_.clear();
            }
        } attempt{*this};
        if (!rsa_ && BCryptOpenAlgorithmProvider(&rsa_, BCRYPT_RSA_ALGORITHM, nullptr, 0) < 0)
            return false;
        HANDLE file =
        ...
        if (file != INVALID_HANDLE_VALUE) {
            ...
            const bool read =
                safe &&
                ReadFile(file, protected_bytes.data(), static_cast<DWORD>(protected_bytes.size()),
                         &count, nullptr) &&
                count == protected_bytes.size();
            CloseHandle(file);
            if (!read)
                return false;
            ...
            if (!CryptUnprotectData(&encrypted, nullptr, &entropy, nullptr, nullptr,
                ...
                return false;
            const bool imported = plain.cbData <= 4096 &&
                                  BCryptImportKeyPair(rsa_, nullptr, BCRYPT_RSAFULLPRIVATE_BLOB,
                                                      &key_, plain.pbData, plain.cbData, 0) >= 0;
            ...
            if (GetLastError() != ERROR_FILE_NOT_FOUND)
                return false;
            if (BCryptGenerateKeyPair(rsa_, &key_, 2048, 0) < 0 ||
                BCryptFinalizeKeyPair(key_, 0) < 0)
                return false;
            ULONG size = 0;
            if (BCryptExportKey(key_, nullptr, BCRYPT_RSAFULLPRIVATE_BLOB, nullptr, 0, &size, 0) <
                    0 ||
                size > 4096)
                return false;
            std::vector<uint8_t> bytes(size);
            if (BCryptExportKey(key_, nullptr, BCRYPT_RSAFULLPRIVATE_BLOB, bytes.data(), size,
                                &size, 0) < 0)
                return false;
            DATA_BLOB plain{size, bytes.data()}, encrypted{}, entropy = Entropy();
            const bool protected_ok =
                CryptProtectData(&plain, L"Veld remote unlock key", &entropy, nullptr, nullptr,
                                 CRYPTPROTECT_UI_FORBIDDEN, &encrypted) != FALSE;
            SecureZeroMemory(bytes.data(), bytes.size());
            if (!protected_ok)
                return false;
            const bool written =
                WriteStateFile(path, encrypted.pbData, encrypted.cbData, nullptr, false);
    ...
    bool Decrypt(const PortalUnlockPayload& payload, uint64_t device, const std::string& nonce,
        ...
        std::lock_guard<std::mutex> lock(mutex_);
        passphrase.clear();
        if (!key_ || !payload.Valid() || payload.key_id != id_ || payload.identity != identity ||
            !device)
            return false;
        std::vector<uint8_t> wrapped, iv, ciphertext, decoded_nonce;
        if (!PortalDecode(payload.wrapped_key, 256, 256, wrapped) ||
            !PortalDecode(payload.iv, 12, 12, iv) ||
            !PortalDecode(payload.ciphertext, 17, 1040, ciphertext) ||
            !PortalDecode(nonce, 16, 16, decoded_nonce))
            return false;
        ...
        bool ok = BCryptDecrypt(key_, wrapped.data(), static_cast<ULONG>(wrapped.size()), &padding,
                                nullptr, 0, secret.data(), static_cast<ULONG>(secret.size()), &size,
                                BCRYPT_PAD_OAEP) >= 0 &&
                  size == 32;
        BCRYPT_ALG_HANDLE aes = nullptr;
        BCRYPT_KEY_HANDLE aes_key = nullptr;
        if (ok)
            ok = BCryptOpenAlgorithmProvider(&aes, BCRYPT_AES_ALGORITHM, nullptr, 0) >= 0;
        if (ok)
            ok = BCryptSetProperty(
                     aes, BCRYPT_CHAINING_MODE,
                     reinterpret_cast<PUCHAR>(const_cast<wchar_t*>(BCRYPT_CHAIN_MODE_GCM)),
                     sizeof(BCRYPT_CHAIN_MODE_GCM), 0) >= 0;
        if (ok)
            ok = BCryptGenerateSymmetricKey(aes, &aes_key, nullptr, 0, secret.data(), 32, 0) >= 0;
        ...
        info.pbAuthData = reinterpret_cast<PUCHAR>(const_cast<char*>(context.data()));
        info.cbAuthData = static_cast<ULONG>(context.size());
        info.pbTag = ciphertext.data() + ciphertext.size() - 16;
        info.cbTag = 16;
        std::vector<uint8_t> plain(ciphertext.size() - 16);
        if (ok)
            ok = BCryptDecrypt(aes_key, ciphertext.data(), static_cast<ULONG>(plain.size()), &info,
                               nullptr, 0, plain.data(), static_cast<ULONG>(plain.size()), &size,
                               0) >= 0 &&
                 size == plain.size();
        if (aes_key)
        ...
        if (ok && std::find(plain.begin(), plain.end(), 0) == plain.end()) {
            const int chars = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS,
                                                  ...
                                                  static_cast<int>(plain.size()), nullptr, 0);
            if (chars > 0 && chars <= 512) {
                passphrase.resize(chars);
                ok = MultiByteToWideChar(
                         CP_UTF8, MB_ERR_INVALID_CHARS, reinterpret_cast<const char*>(plain.data()),
                         static_cast<int>(plain.size()), passphrase.data(), chars) == chars;
            } else
                ok = false;
        } else
            ok = false;
