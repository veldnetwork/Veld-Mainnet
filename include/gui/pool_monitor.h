...
class PoolMonitor {
  ...
  public:
    std::string Report(bool running, uint64_t configured, const pool::Json* status, uint64_t now,
                       double monotonic) {
        uint64_t active = 0, hashes = 0, accepted = 0, verified = 0, retries = 0, updated = 0;
        bool current = false;
        std::string state = running ? "unavailable" : "stopped";
        try {
            pool::Require(configured >= 1 && configured <= 64, "pool worker count");
            if (status) {
                auto number = [&](const char* field, uint64_t max = UINT64_MAX) {
                    return pool::Number(pool::Text(pool::Field(*status, field)), max);
                };
                updated = number("updated_at");
                current = updated <= now && now - updated <= 15;
                if (current) {
                    active = number("active_workers", configured);
                    hashes = number("hashes");
                    accepted = number("accepted");
                    verified = number("verified_shares");
                    retries = number("retry_count");
                    if (running)
                        state = status->Get("failure_code") ? "error"
                                : active                    ? "hashing"
                                : pool::Text(pool::Field(*status, "status")) == "Mining"
                                    ? "waiting"
                                    : "retrying";
                }
            }
            ...
            current = false;
            state = running ? "unavailable" : "stopped";
        }
        if (current && running) {
            if (sampled_ && hashes >= prior_hashes_ && monotonic > prior_time_) {
                ...
                rate_ = 0;
            prior_hashes_ = hashes;
            prior_time_ = monotonic;
            sampled_ = true;
        } else {
            sampled_ = false;
            rate_ = 0;
