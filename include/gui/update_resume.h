...
struct UpdateResume {
    ...
    bool Valid(uint64_t now) const {
        ...
        return created != 0 && now >= created && now - created <= 3600 && !data_directory.empty() &&
               data_directory.size() <= 32767 && data_directory.find(L'\0') == std::wstring::npos &&
               std::filesystem::path(data_directory).is_absolute() && digest(identity) &&
               digest(previous_manifest) && digest(target_manifest) &&
               previous_manifest != target_manifest && !passphrase.empty() &&
               passphrase.size() <= 4096 && passphrase.find(L'\0') == std::wstring::npos;
    }
};
...
inline bool SaveUpdateResume(const std::filesystem::path& path, const std::wstring& context,
                             const UpdateResume& resume, uint64_t now) {
    if (context.empty() || !resume.Valid(now))
        return false;
    size_t total = sizeof(uint64_t);
    for (const auto* field : {&resume.data_directory, &resume.identity, &resume.previous_manifest,
                              &resume.target_manifest, &resume.passphrase})
    ...
    auto put = [&](const void* source, size_t count) {
        std::memcpy(bytes.data() + at, source, count);
        at += count;
    };
    ...
    for (const auto* field : {&resume.data_directory, &resume.identity, &resume.previous_manifest,
        ...
        put(&chars, sizeof(chars));
        put(field->data(), chars * sizeof(wchar_t));
    }
    ...
    const bool protected_ok =
        CryptProtectData(&plain, L"Veld one-use update resume", &entropy, nullptr, nullptr,
                         CRYPTPROTECT_UI_FORBIDDEN, &sealed) != FALSE;
    SecureZeroMemory(bytes.data(), bytes.size());
    const bool saved =
        protected_ok && WriteStateFile(path, sealed.pbData, sealed.cbData, nullptr, false);
    if (sealed.pbData) {
...
inline bool ConsumeUpdateResume(const std::filesystem::path& path, const std::wstring& context,
                                UpdateResume& resume, uint64_t now) {
    resume.Wipe();
    namespace sf = channel::secure_file;
    sf::WinOwnerSecurity owner;
    sf::WinParent parent;
    if (context.empty() || !owner.Initialize(nullptr) ||
        !sf::OpenParent(path, false, owner, parent, nullptr))
        return false;
    ...
    if (!sf::RegularSingleLink(file.value, nullptr, nullptr, "update resume") ||
        ...
        !GetFileSizeEx(file.value, &size) || size.QuadPart <= 0 || size.QuadPart > 128 * 1024)
        return false;
    ...
    DATA_BLOB entropy{static_cast<DWORD>(context.size() * sizeof(wchar_t)),
                      reinterpret_cast<BYTE*>(const_cast<wchar_t*>(context.data()))};
    if (!CryptUnprotectData(&sealed, nullptr, &entropy, nullptr, nullptr, CRYPTPROTECT_UI_FORBIDDEN,
                            &plain))
        return false;
    size_t at = 0;
    auto take = [&](void* destination, size_t count) {
        if (at > plain.cbData || count > plain.cbData - at)
            return false;
        std::memcpy(destination, plain.pbData + at, count);
        at += count;
        return true;
    };
    bool ok = take(&resume.created, sizeof(resume.created));
    for (auto* field : {&resume.data_directory, &resume.identity, &resume.previous_manifest,
                        &resume.target_manifest, &resume.passphrase}) {
        uint32_t chars = 0;
        if (!ok || !take(&chars, sizeof(chars)) || chars > 32767 ||
            chars * sizeof(wchar_t) > plain.cbData - at) {
            ok = false;
            break;
        }
        field->resize(chars);
