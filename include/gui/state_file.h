...
inline bool WriteStateFile(const std::filesystem::path& path, const uint8_t* data, size_t size,
                           std::string* error = nullptr, bool replace = true) {
    namespace sf = channel::secure_file;
    sf::WinOwnerSecurity owner;
    sf::WinParent parent;
    if (!owner.Initialize(error) || !sf::OpenParent(path, false, owner, parent, error))
        return false;
    ...
    if (existing == sf::ExistingTarget::Unsafe ||
        (!replace && existing != sf::ExistingTarget::Missing))
        return false;
    std::filesystem::path pending;
    sf::WinHandle handle;
    if (!sf::CreateOwnerOnlyTemporary(target, owner, pending, handle, error, "GUI state"))
        return false;
    bool ok = sf::WriteAndFlush(handle.value, data, size, error, "GUI state");
    if (!::CloseHandle(handle.release()))
        ok = false;
    if (ok)
        ok = ::MoveFileExW(pending.c_str(), target.c_str(),
                           MOVEFILE_WRITE_THROUGH | (replace ? MOVEFILE_REPLACE_EXISTING : 0)) != 0;
    if (ok)
        ok = sf::ValidatePrivateFilePath(target, owner, error);
    if (!ok) {
...
inline bool WriteStateText(const std::filesystem::path& path, const std::string& text,
