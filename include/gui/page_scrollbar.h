...
class PageScrollBar {
    ...
    static void Paint(HWND window, HDC target) {
        ...
        if (GetScrollBarInfo(window, OBJID_CLIENT, &info)) {
            ...
            if (thumb.bottom > thumb.top) {
                brush = CreateSolidBrush(RGB(83, 89, 96));
                auto pen = CreatePen(PS_SOLID, 1, RGB(83, 89, 96));
                auto b = SelectObject(dc, brush), p = SelectObject(dc, pen);
                RoundRect(dc, thumb.left, thumb.top, thumb.right, thumb.bottom, inset * 2,
                          inset * 2);
                if (GetFocus() == window) {
                    RECT focus = thumb;
                    InflateRect(&focus, -1, -2);
            ...
            auto pen = CreatePen(PS_SOLID, 1, RGB(134, 141, 150));
            auto prior = SelectObject(dc, pen);
            const int x = bounds.right / 2, arrow = std::max<LONG>(2, bounds.right / 5);
            for (int down = 0; down < 2; ++down) {
                const int y =
                    down ? bounds.bottom - info.dxyLineButton / 2 : info.dxyLineButton / 2;
    ...
    static LRESULT CALLBACK Procedure(HWND window, UINT message, WPARAM wp, LPARAM lp, UINT_PTR,
                                      DWORD_PTR) {
        ...
        if (message == WM_PRINTCLIENT) {
            Paint(window, reinterpret_cast<HDC>(wp));
            return 0;
        }
        ...
        if (message == WM_MOUSEMOVE || message == WM_LBUTTONDOWN || message == WM_LBUTTONUP ||
            message == WM_KEYDOWN || message == WM_TIMER || message == WM_SETFOCUS ||
  ...
  public:
    ...
    void Update(const RECT& client, int content, int position, int scale) {
        ...
        RECT next{client.right - width - margin, margin, client.right - margin,
        ...
        if (content != content_ || client.bottom != page_ || position != position_) {
            ...
            info.nMin = 0;
            info.nMax = content - 1;
            info.nPage = std::max<LONG>(1, client.bottom);
            info.nPos = position;
