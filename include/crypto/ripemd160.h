...
static const uint32_t K_L[5] = {
    0x00000000, 0x5A827999, 0x6ED9EBA1, 0x8F1BBCDC, 0xA953FD4E
};
// Round constants — right
static const uint32_t K_R[5] = {
...
public:
    ...
    void Update(const uint8_t* data, size_t len) {
        for (size_t i = 0; i < len; ++i) {
            ...
            if (buf_pos_ == 64) { Transform(); buf_pos_ = 0; }
        }
        count_ += len * 8;
    }
...
private:
    ...
    void Transform() {
        ...
        uint32_t X[16];
        for (int i = 0; i < 16; ++i) {
            X[i] = (uint32_t)buf_[i*4]
                 | ((uint32_t)buf_[i*4+1] << 8)
                 | ((uint32_t)buf_[i*4+2] << 16)
                 | ((uint32_t)buf_[i*4+3] << 24);
        }
        ...
        for (int i = 0; i < 80; ++i) {
            ...
            switch (round) {
                case 0: fl = f(bl, cl, dl); break;
                case 1: fl = g(bl, cl, dl); break;
                case 2: fl = h(bl, cl, dl); break;
                case 3: fl = ii(bl, cl, dl); break;
                default: fl = j(bl, cl, dl); break;
            }

            // Right boolean function (rounds go j, ii, h, g, f)
            switch (round) {
                case 0: fr = j(br, cr, dr);  break;
                case 1: fr = ii(br, cr, dr); break;
                case 2: fr = h(br, cr, dr);  break;
                case 3: fr = g(br, cr, dr);  break;
                default: fr = f(br, cr, dr); break;
            }
...
template<size_t N>
inline Hash160 Hash160Compute(const std::array<uint8_t, N>& data) {
    return Hash160Compute(data.data(), data.size());
}
