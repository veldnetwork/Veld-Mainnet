...
inline Hash256 ReceiptDigest(const std::string& payload) {
    static constexpr uint8_t domain[] = {'V', 'E', 'L', 'D', '_', 'F', 'U', 'L', 'L',
                                         '_', 'I', 'B', 'D', '_', 'V', '1', 0};
...
inline Hash256 FleetReceiptDigest(const std::string& payload) {
    static constexpr uint8_t domain[] = {'V', 'E', 'L', 'D', '_', 'F', 'L', 'E', 'E',
                                         'T', '_', 'I', 'B', 'D', '_', 'V', '1', 0};
    std::vector<uint8_t> preimage;
    preimage.reserve(sizeof(domain) + payload.size());
    preimage.insert(preimage.end(), std::begin(domain), std::end(domain));
...
inline bool ReadProtectedFile(const std::string& path, size_t max_bytes, std::vector<uint8_t>& out,
                              std::string* error) {
    const auto result =
        channel::secure_file::Read(path, out, error, max_bytes, /*require_private_parent=*/true);
    return result == channel::secure_file::ReadResult::Ok;
}

inline std::string CanonicalPayload(uint64_t height, const std::string& tip_hash,
                                    int64_t completed_at, const std::string& miner_address,
...
inline bool WriteFullIbdReceipt(const std::string& datadir, const RealKeyPair& miner,
                                uint64_t height, const std::string& tip_hash,
                                std::string* error = nullptr) {
    if (height == 0 || !IsLowerHex(tip_hash, 64) || miner.address.empty()) {
        if (error)
            *error = "full-IBD receipt inputs are not canonical";
        return false;
    }
    ...
    if (revoke_ec) {
        if (error)
            *error = "cannot clear prior snapshot revocation after full IBD";
        return false;
    }

    std::vector<uint8_t> key_file;
    if (!ReadProtectedFile((std::filesystem::path(datadir) / "miner.key").string(), 64 * 1024,
                           key_file, error)) {
        return false;
    }

    const std::vector<uint8_t> pubkey(miner.public_key.begin(), miner.public_key.end());
    ...
    const std::string payload =
        CanonicalPayload(height, tip_hash, completed_at, miner.address, pubkey_hash, key_file_hash);
    ...
    try {
        ...
        if (error)
            *error = std::string("cannot sign full-IBD receipt: ") + e.what();
        return false;
    }
    const std::string body = payload + "miner_public_key=" + BytesToHex(pubkey) + "\n" +
                             "signature=" + BytesToHex(signature) + "\n";
    if (body.size() > 24 * 1024) {
        if (error)
            *error = "full-IBD receipt exceeds its fixed size bound";
        return false;
    }
    return channel::secure_file::AtomicWriteText(ReceiptPath(datadir), body, error,
                                                 /*require_private_parent=*/true);
}

inline bool VerifyFullIbdReceipt(const std::string& datadir, const Secp256k1PubKey& expected_pubkey,
                                 FullIbdReceipt& out, std::string* error = nullptr) {
    out = FullIbdReceipt{};
    std::error_code revoked_ec;
    const bool revoked = std::filesystem::exists(RevocationPath(datadir), revoked_ec);
    if (revoked || revoked_ec) {
        if (error)
            *error = revoked ? "snapshot eligibility was revoked after a failed verification"
    ...
    if (body.empty() || body.back() != '\n' || body.find('\r') != std::string::npos) {
        if (error)
            *error = "full-IBD receipt has non-canonical framing";
        return false;
    }
    ...
    while (pos < body.size()) {
        ...
        if (end == std::string::npos || end == pos) {
            if (error)
                *error = "full-IBD receipt has an empty or unterminated field";
            return false;
        }
        ...
        if (eq == std::string::npos || eq == 0 || eq + 1 == line.size() ||
            ...
            if (error)
                *error = "full-IBD receipt has a malformed or duplicate field";
            return false;
        }
        pos = end + 1;
    }
    ...
    static const std::array<const char*, 11> required = {
        ...
        "miner_address",    "miner_pubkey_hash",  "miner_key_file_hash",
        "validated_height", "validated_tip",      "completed_at",
        "miner_public_key", "signature"};
    if (fields.size() != required.size()) {
        if (error)
            *error = "full-IBD receipt has unknown or missing fields";
        return false;
    }
    for (const char* key : required) {
        if (fields.find(key) == fields.end()) {
            if (error)
                *error = "full-IBD receipt is missing a required field";
            return false;
        }
    }
    if (fields["schema"] != RECEIPT_SCHEMA ||
        fields["deployment_profile"] != DEPLOYMENT_PROFILE_ID ||
        fields["genesis"] != GENESIS_HASH || !IsLowerHex(fields["miner_pubkey_hash"], 64) ||
        ...
        if (error)
            *error = "full-IBD receipt identity or hexadecimal field is invalid";
        return false;
    }
    ...
    if (!ParseCanonicalUint64Text(fields["validated_height"], height) || height == 0 ||
        ...
        if (error)
            *error = "full-IBD receipt numeric field is invalid";
        return false;
    }
    ...
    if (pubkey_bytes.size() != Secp256k1PubKey{}.size() || signature.empty() ||
        ...
        if (error)
            *error = "full-IBD receipt signature material is malformed";
        return false;
    }
    ...
    const std::string payload =
        CanonicalPayload(height, fields["validated_tip"], completed_at, fields["miner_address"],
                         fields["miner_pubkey_hash"], fields["miner_key_file_hash"]);
    const std::string canonical_body = payload + "miner_public_key=" + fields["miner_public_key"] +
                                       "\n" + "signature=" + fields["signature"] + "\n";
    if (body != canonical_body || !Verify(pubkey, ReceiptDigest(payload), signature)) {
        if (error)
            *error = "full-IBD receipt signature or canonical encoding is invalid";
        return false;
    }
...
inline std::string FleetCanonicalPayload(uint64_t height, const std::string& tip_hash,
                                         int64_t completed_at, const std::string& fleet_address,
                                         const std::string& pubkey_hash,
                                         const std::string& key_file_hash,
...
inline bool WriteFleetIbdReceipt(const std::string& datadir, const RealKeyPair& fleet_identity,
                                 uint64_t height, const std::string& tip_hash,
                                 std::string* error = nullptr) {
    if (height == 0 || !IsLowerHex(tip_hash, 64) || fleet_identity.address.empty()) {
        if (error)
            *error = "fleet full-IBD receipt inputs are not canonical";
        return false;
    }
    std::error_code revoke_ec;
    std::filesystem::remove(RevocationPath(datadir), revoke_ec);
    if (revoke_ec) {
        if (error)
            *error = "cannot clear prior snapshot revocation after fleet full IBD";
        return false;
    }

    std::vector<uint8_t> key_file;
    if (!ReadProtectedFile(FleetKeyPath(datadir), 64 * 1024, key_file, error)) {
        return false;
    }
    std::vector<uint8_t> network_identity;
    if (!ReadProtectedFile((std::filesystem::path(datadir) / "network.identity").string(),
    ...
    const std::string pubkey_hash = HashToHex(Hash256d(pubkey));
    const std::string key_file_hash = HashToHex(Hash256d(key_file));
    const std::string network_identity_hash = HashToHex(Hash256d(network_identity));
    const int64_t completed_at = std::chrono::duration_cast<std::chrono::seconds>(
                                     std::chrono::system_clock::now().time_since_epoch())
                                     .count();
    const std::string payload =
        FleetCanonicalPayload(height, tip_hash, completed_at, fleet_identity.address, pubkey_hash,
                              key_file_hash, network_identity_hash);

    Secp256k1SigDER signature;
    try {
        signature = Sign(fleet_identity.private_key, FleetReceiptDigest(payload));
    } catch (const std::exception& e) {
        if (error) {
            *error = std::string("cannot sign fleet full-IBD receipt: ") + e.what();
        }
        return false;
    }
    const std::string body = payload + "fleet_public_key=" + BytesToHex(pubkey) + "\n" +
                             "signature=" + BytesToHex(signature) + "\n";
    if (body.size() > 24 * 1024) {
        if (error)
            *error = "fleet full-IBD receipt exceeds its fixed size bound";
        return false;
    }
    return channel::secure_file::AtomicWriteText(FleetReceiptPath(datadir), body, error,
                                                 /*require_private_parent=*/true);
}

inline bool VerifyFleetIbdReceipt(const std::string& datadir,
                                  const Secp256k1PubKey& expected_pubkey, FullIbdReceipt& out,
                                  std::string* error = nullptr) {
    out = FullIbdReceipt{};
    std::error_code revoked_ec;
    const bool revoked = std::filesystem::exists(RevocationPath(datadir), revoked_ec);
    ...
    std::vector<uint8_t> bytes;
    if (!ReadProtectedFile(FleetReceiptPath(datadir), 24 * 1024, bytes, error)) {
        return false;
    }
    const std::string body(bytes.begin(), bytes.end());
    if (body.empty() || body.back() != '\n' || body.find('\r') != std::string::npos) {
        if (error)
            *error = "fleet full-IBD receipt has non-canonical framing";
        return false;
    }
    ...
    if (fields.size() != required.size()) {
        if (error)
            *error = "fleet full-IBD receipt has unknown or missing fields";
        return false;
    }
    for (const char* key : required) {
        if (fields.find(key) == fields.end()) {
            if (error)
                *error = "fleet full-IBD receipt is missing a required field";
            return false;
        }
    }
    if (fields["schema"] != FLEET_RECEIPT_SCHEMA ||
        fields["deployment_profile"] != DEPLOYMENT_PROFILE_ID ||
        fields["genesis"] != GENESIS_HASH || !IsLowerHex(fields["network_identity_hash"], 64) ||
        !IsLowerHex(fields["fleet_pubkey_hash"], 64) ||
        !IsLowerHex(fields["fleet_key_file_hash"], 64) ||
        !IsLowerHex(fields["validated_tip"], 64) ||
        !IsLowerHex(fields["fleet_public_key"], Secp256k1PubKey{}.size() * 2) ||
        !IsLowerHex(fields["signature"], fields["signature"].size())) {
        if (error)
            *error = "fleet full-IBD receipt identity or hexadecimal field is invalid";
        return false;
    }
    ...
    if (!ParseCanonicalUint64Text(fields["validated_height"], height) || height == 0 ||
        ...
        completed_at_u64 > static_cast<uint64_t>(std::numeric_limits<int64_t>::max())) {
        if (error)
            *error = "fleet full-IBD receipt numeric field is invalid";
        return false;
    }
    const int64_t completed_at = static_cast<int64_t>(completed_at_u64);
    ...
    if (pubkey_bytes.size() != Secp256k1PubKey{}.size() || signature.empty() ||
        signature.size() > dilithium::SIG_MAX_BYTES) {
        if (error)
            *error = "fleet full-IBD receipt signature material is malformed";
        return false;
    }
    Secp256k1PubKey pubkey{};
    std::copy(pubkey_bytes.begin(), pubkey_bytes.end(), pubkey.begin());
    ...
    if (HashToHex(Hash256d(network_identity)) != fields["network_identity_hash"]) {
        if (error)
            *error = "fleet full-IBD receipt belongs to a different network identity";
        return false;
    }

    const std::string payload =
        FleetCanonicalPayload(height, fields["validated_tip"], completed_at,
                              fields["fleet_address"], fields["fleet_pubkey_hash"],
                              fields["fleet_key_file_hash"], fields["network_identity_hash"]);
    const std::string canonical_body = payload + "fleet_public_key=" + fields["fleet_public_key"] +
                                       "\n" + "signature=" + fields["signature"] + "\n";
    if (body != canonical_body || !Verify(pubkey, FleetReceiptDigest(payload), signature)) {
        if (error)
            *error = "fleet full-IBD receipt signature or canonical encoding is invalid";
        return false;
    }
