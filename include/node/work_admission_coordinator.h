...
public:
    ...
    public:
        ...
        const TokenBytes& bytes() const noexcept { return bytes_; }

        bool IsZero() const noexcept {
            uint8_t aggregate = 0;
            for (const uint8_t byte : bytes_) aggregate |= byte;
            return aggregate == 0;
        }
    ...
    public:
        ...
        Lease(Lease&& other) noexcept { MoveFrom_(std::move(other)); }
    ...
    private:
        ...
        Lease(const std::shared_ptr<SharedState>& state, uint64_t id,
              Path path, const Binding& binding, TimePoint deadline,
              uint64_t configuration_generation) noexcept
            : state_(state), id_(id), path_(path), binding_(binding),
              deadline_(deadline),
    ...
    AdmissionCoordinator()
        : AdmissionCoordinator(Limits{}, TokenMintFn{}, NowFn{}) {}

    explicit AdmissionCoordinator(Limits limits,
                                  TokenMintFn token_mint = TokenMintFn{},
                                  NowFn now = NowFn{});
    ...
    static bool BindCloseEpochSnapshot(Configuration& configuration,
                                       uint64_t before,
    ...
    CloseResult BeginClose(
        Refusal refusal = Refusal::RuntimeClosed) noexcept;
    ...
    CloseResult CancelAndClose(
        Refusal refusal = Refusal::RuntimeClosed) noexcept;

    LocalAttempt AcquireLocal(
        Path path, const Subject& subject,
    ...
    RemoteAttempt IssueRemoteSigningLease(
        Path path, const Subject& subject,
        const std::optional<Binding>& prior, bool require_prior,
        Duration requested_ttl) noexcept;
    ...
    RemoteConsumeAttempt ConsumeRemoteSigningLease(
        const RemoteToken& token, Path path, const Subject& subject,
    ...
    static const char* ErrorName(Error error) noexcept {
        switch (error) {
            ...
            case Error::Closing: return "closing";
            case Error::Busy: return "busy";
            case Error::Rejected: return "rejected";
            case Error::InvalidDuration: return "invalid_duration";
            ...
            case Error::PathPurposeMismatch: return "path_purpose_mismatch";
        }
        return "unknown";
    }
    ...
    static std::optional<RemoteToken> DecodeRemoteToken(
        ...
        return token.IsZero() ? std::nullopt
                              : std::optional<RemoteToken>(token);
    }
...
private:
    ...
    struct SharedState {
        explicit SharedState(Limits bounded_limits, TokenMintFn mint,
                             NowFn clock)
    ...
    static Refusal BasePrerequisiteRefusal_(
        const Prerequisites& prerequisites) noexcept;
    static Refusal NormalizeCloseRefusal_(Refusal refusal) noexcept;
    static bool ConfigurationEqual_(const Configuration& lhs,
    ...
    static void RememberSpentLocked_(SharedState& state,
    ...
    static bool TokenKnownLocked_(const SharedState& state,
                                  const TokenBytes& token) noexcept;
    static Decision EvaluateLocked_(
        const SharedState& state, Path path, const Subject& subject,
    ...
    static Record* FindRemoteRecordLocked_(SharedState& state,
                                           const TokenBytes& token) noexcept;
    static bool SpentTokenKnownLocked_(const SharedState& state,
                                       const TokenBytes& token) noexcept;
    static bool LeaseLive_(const std::shared_ptr<SharedState>& state,
                            uint64_t id, bool claim,
                            bool canonical_commit = false) noexcept;
    static void ReleaseLease_(const std::shared_ptr<SharedState>& state,
                              uint64_t id) noexcept;

    std::shared_ptr<SharedState> state_;
};

inline AdmissionCoordinator::Limits AdmissionCoordinator::NormalizeLimits_(
    ...
    limits.max_local_lease = clamp_duration(
        limits.max_local_lease, Duration{1});
    limits.max_remote_lease = clamp_duration(
        limits.max_remote_lease, Duration{1});
    limits.max_active_leases = std::max<size_t>(1, limits.max_active_leases);
    limits.max_spent_tokens = std::max(
        limits.max_active_leases, limits.max_spent_tokens);
    return limits;
}

inline AdmissionCoordinator::AdmissionCoordinator(
        Limits limits, TokenMintFn token_mint, NowFn now)
    : state_(std::make_shared<SharedState>(
          NormalizeLimits_(limits), std::move(token_mint), std::move(now))) {}
...
inline bool AdmissionCoordinator::BindCloseEpochSnapshot(
        Configuration& configuration, uint64_t before,
        uint64_t after) noexcept {
    configuration.close_epoch = before;
    if (before != 0 && before == after) return true;
    configuration.prerequisites.runtime_open = false;
    configuration.prerequisites.peer_view_safe = false;
    return false;
}

inline bool AdmissionCoordinator::PathMatchesPurpose_(
        Path path, Purpose purpose) noexcept {
    switch (path) {
...
inline Refusal AdmissionCoordinator::BasePrerequisiteRefusal_(
        const Prerequisites& p) noexcept {
    if (!p.node_running) return Refusal::NodeNotRunning;
    ...
    if (!p.sync_complete) return Refusal::SyncIncomplete;
    if (!p.snapshot_state_clean) return Refusal::SnapshotStateUntrusted;
    if (!p.durable_state_proven) return Refusal::DurableStateUnproven;
    if (!p.datadir_identity_valid) return Refusal::DatadirIdentityUnproven;
    if (!p.checkpoint_anchor_valid) return Refusal::CheckpointAnchorUnproven;
    if (!p.canonical_tip_known) return Refusal::TipUnknown;
    if (!p.runtime_open) return Refusal::RuntimeClosed;
    if (!p.peer_view_safe) return Refusal::PeerViewUnsafe;
    return Refusal::None;
}

inline Refusal AdmissionCoordinator::NormalizeCloseRefusal_(
        Refusal refusal) noexcept {
    return refusal == Refusal::None ? Refusal::RuntimeClosed : refusal;
}

inline bool AdmissionCoordinator::ConfigurationEqual_(
        const Configuration& lhs, const Configuration& rhs) noexcept {
    const auto& a = lhs.prerequisites;
    const auto& b = rhs.prerequisites;
    return lhs.close_epoch == rhs.close_epoch &&
        lhs.permitted_paths == rhs.permitted_paths &&
        a.node_running == b.node_running &&
        a.startup_replay_complete == b.startup_replay_complete &&
        a.independent_validation_complete ==
            b.independent_validation_complete &&
        a.sync_complete == b.sync_complete &&
        a.snapshot_state_clean == b.snapshot_state_clean &&
        ...
        a.canonical_tip_known == b.canonical_tip_known &&
        a.runtime_open == b.runtime_open &&
        a.peer_view_safe == b.peer_view_safe &&
        a.validation_generation == b.validation_generation &&
        a.network_magic == b.network_magic &&
        a.genesis_hash == b.genesis_hash &&
        a.profile_digest == b.profile_digest;
}

inline bool AdmissionCoordinator::TokenEqual_(
        const TokenBytes& lhs, const TokenBytes& rhs) noexcept {
...
inline void AdmissionCoordinator::AdvanceCloseEpochLocked_(
...
inline AdmissionCoordinator::TimePoint AdmissionCoordinator::NowLocked_(
        const SharedState& state) noexcept {
    try {
...
inline void AdmissionCoordinator::RememberSpentLocked_(
        SharedState& state, const TokenBytes& token, TimePoint now) noexcept {
    for (const auto& spent : state.spent_tokens) {
        if (TokenEqual_(spent.token, token)) return;
    }
    if (state.spent_tokens.size() >= state.limits.max_spent_tokens) return;
    const TimePoint forget_after = now == TimePoint::max()
        ? TimePoint::max()
        : now + state.limits.max_remote_lease;
    state.spent_tokens.push_back({token, forget_after});
}

inline void AdmissionCoordinator::FinalizeCloseLocked_(
        SharedState& state) noexcept {
    state.phase = Phase::Closed;
    state.close_deadline.reset();
}

inline void AdmissionCoordinator::PruneLocked_(
        SharedState& state, TimePoint now) noexcept {
    state.spent_tokens.erase(
        std::remove_if(state.spent_tokens.begin(), state.spent_tokens.end(),
            [&](const SpentToken& spent) {
    ...
    if (state.phase == Phase::Closing &&
        (state.records.empty() ||
         (state.close_deadline && *state.close_deadline <= now))) {
...
inline bool AdmissionCoordinator::TokenKnownLocked_(
        const SharedState& state, const TokenBytes& token) noexcept {
    for (const auto& record : state.records) {
        if (record.token && TokenEqual_(*record.token, token)) return true;
    }
    return SpentTokenKnownLocked_(state, token);
}

inline Decision AdmissionCoordinator::EvaluateLocked_(
        const SharedState& state, Path path, const Subject& subject,
        const std::optional<Binding>& prior, bool require_prior) noexcept {
...
inline AdmissionCoordinator::Record* AdmissionCoordinator::FindRecordLocked_(
        SharedState& state, uint64_t id) noexcept {
...
AdmissionCoordinator::FindRemoteRecordLocked_(
        SharedState& state, const TokenBytes& token) noexcept {
    for (auto& record : state.records) {
...
inline bool AdmissionCoordinator::SpentTokenKnownLocked_(
        const SharedState& state, const TokenBytes& token) noexcept {
    for (const auto& spent : state.spent_tokens)
...
inline AdmissionCoordinator::OpenResult AdmissionCoordinator::Open(
        const Configuration& configuration) noexcept {
    std::lock_guard<std::mutex> lock(state_->mutex);
    const uint64_t close_epoch =
        state_->close_epoch.load(std::memory_order_acquire);
    if (configuration.close_epoch == 0 ||
        state_->close_epoch_exhausted ||
        configuration.close_epoch != close_epoch) {
        const Refusal refusal = state_->refusal == Refusal::None
            ? Refusal::RuntimeClosed : state_->refusal;
    ...
    if (!state_->records.empty() &&
        !ConfigurationEqual_(state_->configuration, configuration))
        return {false, Error::Busy, state_->refusal};

    const bool configuration_changed = !state_->configured ||
        !ConfigurationEqual_(state_->configuration, configuration);
    ...
    const Refusal refusal = BasePrerequisiteRefusal_(
...
inline AdmissionCoordinator::CloseResult AdmissionCoordinator::BeginClose(
        Refusal refusal) noexcept {
    std::lock_guard<std::mutex> lock(state_->mutex);
    AdvanceCloseEpochLocked_(*state_);
    const TimePoint now = NowLocked_(*state_);
    ...
    const TimePoint absolute_deadline = now == TimePoint::max()
        ? TimePoint::max() : now + ABSOLUTE_MAX_LEASE;
...
AdmissionCoordinator::AcquireLocal(
        Path path, const Subject& subject,
        const std::optional<Binding>& prior, bool require_prior,
        Duration requested_ttl) noexcept {
    ...
    if (state_->phase != Phase::Open) {
        attempt.error = state_->phase == Phase::Closing
            ? Error::Closing : Error::Closed;
        attempt.decision.refusal = state_->configured
            ? state_->refusal : Refusal::Unwired;
        return attempt;
    }
    ...
    attempt.decision = EvaluateLocked_(
        *state_, path, subject, prior, require_prior);
    if (!attempt.decision.allowed || !attempt.decision.binding) {
        attempt.error = PathMatchesPurpose_(path, subject.purpose)
            ? Error::Rejected : Error::PathPurposeMismatch;
        return attempt;
    }
    ...
    const Duration ttl = std::min(
        {requested_ttl, state_->limits.max_local_lease,
         ABSOLUTE_MAX_LEASE});
    const TimePoint deadline = now + ttl;
    state_->records.push_back(Record{
        id, RecordKind::Local, path, subject, *attempt.decision.binding,
        deadline, state_->configuration_generation, false, std::nullopt});
    attempt.error = Error::None;
    attempt.lease = Lease(
        state_, id, path, *attempt.decision.binding, deadline,
        state_->configuration_generation);
    return attempt;
}

inline AdmissionCoordinator::RemoteAttempt
AdmissionCoordinator::IssueRemoteSigningLease(
        Path path, const Subject& subject,
        const std::optional<Binding>& prior, bool require_prior,
        Duration requested_ttl) noexcept {
    ...
    if (state_->records.size() >= state_->limits.max_active_leases ||
        state_->spent_tokens.size() + pending_remote >=
            state_->limits.max_spent_tokens) {
        attempt.error = Error::Capacity;
        attempt.decision.refusal = Refusal::RuntimeClosed;
        return attempt;
    }
    ...
    for (size_t i = 0; i < candidate_count; ++i) {
        ...
        if (!candidate.IsZero() &&
            !TokenKnownLocked_(*state_, candidates[i])) {
    ...
    const Duration ttl = std::min(
        {requested_ttl, state_->limits.max_remote_lease,
         ABSOLUTE_MAX_LEASE});
    const TimePoint deadline = now + ttl;
    state_->records.push_back(Record{
        id, RecordKind::RemotePending, path, subject,
        *attempt.decision.binding, deadline, state_->configuration_generation,
        false, *selected});
    attempt.error = Error::None;
    attempt.grant = RemoteGrant{
        RemoteToken::FromBytes(*selected), *attempt.decision.binding,
        deadline, ttl};
    return attempt;
}

inline AdmissionCoordinator::RemoteConsumeAttempt
AdmissionCoordinator::ConsumeRemoteSigningLease(
        const RemoteToken& token, Path path, const Subject& subject,
        const std::optional<Binding>& prior) noexcept {
    ...
    if (!record) {
        attempt.error = SpentTokenKnownLocked_(*state_, token.bytes())
    ...
    attempt.decision = EvaluateLocked_(
        *state_, path, subject, prior, true);
    if (!attempt.decision.allowed || !attempt.decision.binding ||
    ...
    attempt.error = Error::None;
    attempt.lease = Lease(state_, id, path, binding, deadline,
                          record->configuration_generation);
    return attempt;
}

inline bool AdmissionCoordinator::CancelRemoteSigningLease(
        const RemoteToken& token) noexcept {
    std::lock_guard<std::mutex> lock(state_->mutex);
    const TimePoint now = NowLocked_(*state_);
    PruneLocked_(*state_, now);
...
inline AdmissionCoordinator::Snapshot
AdmissionCoordinator::GetSnapshot() const noexcept {
    std::lock_guard<std::mutex> lock(state_->mutex);
    const TimePoint now = NowLocked_(*state_);
    PruneLocked_(*state_, now);
    Snapshot snapshot;
    ...
    for (const auto& record : state_->records)
        snapshot.pending_remote_tokens +=
            record.kind == RecordKind::RemotePending ? 1U : 0U;
...
inline bool AdmissionCoordinator::LeaseLive_(
        const std::shared_ptr<SharedState>& state, uint64_t id,
    ...
    if (!record || record->deadline <= now ||
        state->phase == Phase::Closed ||
        record->configuration_generation == 0 ||
        record->configuration_generation != state->configuration_generation)
        return false;
    if (claim) {
        if (record->sink_claimed) return false;
        if (canonical_commit) {
            if (record->path != Path::InternalMining &&
                record->path != Path::SubmitBlock &&
            ...
            for (const auto& live_record : state->records)
                close_deadline = std::max(
                    close_deadline, live_record.deadline);
            ...
            if (state->records.size() != 1) return false;
        }
...
inline void AdmissionCoordinator::ReleaseLease_(
        const std::shared_ptr<SharedState>& state, uint64_t id) noexcept {
    std::lock_guard<std::mutex> lock(state->mutex);
    const TimePoint now = NowLocked_(*state);
    PruneLocked_(*state, now);
    state->records.erase(
...
inline bool AdmissionCoordinator::Lease::IsLive() const noexcept {
    ...
    return id_ != 0 && state &&
        AdmissionCoordinator::LeaseLive_(state, id_, false);
}

inline bool AdmissionCoordinator::Lease::ClaimForSink() noexcept {
    const auto state = state_.lock();
    return id_ != 0 && state &&
        AdmissionCoordinator::LeaseLive_(state, id_, true);
}

inline bool AdmissionCoordinator::Lease::ClaimForCanonicalCommit() noexcept {
    const auto state = state_.lock();
    return id_ != 0 && state &&
        AdmissionCoordinator::LeaseLive_(state, id_, true, true);
}
...
inline void AdmissionCoordinator::Lease::Release() noexcept {
    ...
    state_.reset();
    if (state) AdmissionCoordinator::ReleaseLease_(state, id);
}
