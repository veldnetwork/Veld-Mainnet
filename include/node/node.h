...
MineOnly(Blockchain& chain, Mempool& mempool, const RealKeyPair& miner_keypair,
    ...
    Hash256 ds_seed_loop = mining::ComputeEpochSeed(candidate.header.prev_block_hash,
    ...
    auto worker_fn = [&](unsigned worker_id) {
        ...
        auto rebuild_local = [&]() {
            ...
            std::copy(candidate.header.prev_block_hash.begin(),
                      candidate.header.prev_block_hash.end(), tb.begin() + 4);
            std::copy(shared_merkle_root.begin(), shared_merkle_root.end(), tb.begin() + 36);
        ...
        while (!stop_local.load(std::memory_order_relaxed)) {
            ...
            Hash256 h;
            {
                ...
                Hash256 per_hash_seed = mining::ComputeEpochSeed(
                    candidate.header.prev_block_hash, candidate.height, candidate.header.bits);
                mining::DatasetHandle ds = mining::GlobalDataset().get_for_seed(per_hash_seed);
                if (!ds.get()) {
                    stop_local = true;
                    break;
                }
                ...
                if constexpr (OPTION_B_CONSENSUS_GATE_ENABLED) {
                    if (nms_cb && h < target_x4 && !(h < target)) {
                        ...
                        if (nms_broadcast_guard.compare_exchange_strong(expected, true)) {
                            ...
                            std::copy(shared_merkle_root.begin(), shared_merkle_root.end(),
            ...
            if (local_hashes > 0 && (local_hashes % REFRESH_EVERY_HASHES) == 0) {
                ...
                if (header_generation.load() == my_gen) {
                    ...
                    std::copy(candidate.header.merkle_root.begin(),
                              candidate.header.merkle_root.end(), shared_merkle_root.begin());
...
class VeldNode {
  public:
    ...
    explicit VeldNode(const NetworkConfig& config, const std::string& data_dir)
        : config_(config), data_dir_(EnsureDataDir(data_dir)), chain_(),
          ...
          explorer_(chain_, mempool_, CompiledPublicExplorerPort()), running_(false),
          mining_(false) {
    ...
    Block TestBuildDStateQualificationBlock(uint64_t expected_height,
        ...
        if (chain_.IsEmpty() || expected_height != chain_.Height() + 1)
            throw std::logic_error("D-STATE candidate height does not extend the live tip");
        if (miner_script.size() != 25 || miner_script[0] != 0x76 || miner_script[1] != 0xA9 ||
            ...
            throw std::logic_error("D-STATE candidate miner script is not canonical P2PKH");
        ...
        if (!valid_field(token_address) || !valid_field(lp_address))
            throw std::logic_error("D-STATE candidate carrier field is not canonical");
        ...
        if (capacity_carrier && (expected_height != 400 || token_address != "-" ||
            ...
            throw std::logic_error("D-STATE S6 capacity carrier has invalid scope or cardinality");
        }
        ...
        if (!candidate.success || candidate.block.height != expected_height)
            throw std::runtime_error("D-STATE production candidate construction failed: " +
                                     candidate.error);
        ...
        if (token_address != "-" || lp_address != "-" || capacity_carrier) {
            if (block.transactions.empty() || block.transactions.front().inputs.size() != 1 ||
                ...
                throw std::runtime_error("D-STATE production candidate lacks canonical coinbase");
    ...
    void Start() {
        ...
        if (tip && tip->height > 0 && background_validation_only_) {
            if (const auto prefix = ReadValidatedBackgroundPrefix_()) {
                if (prefix->first > tip->height) {
                    throw std::runtime_error("validated background prefix exceeds the durable tip");
                }
        ...
        rpc_.SetPeerInfo([this]() -> std::string {
            ...
            for (const auto& p : peers) {
                ...
                if (colon != std::string::npos)
                    ip_only = ip_only.substr(0, colon);
        ...
        rpc_.SetTxIndexLookupFn([this](const std::string& txid_hex) -> std::optional<uint64_t> {
            ...
            try {
                ...
                if (!v || v->empty())
                    return std::nullopt;
                uint64_t height = 0;
                if (!ParseMinerArchiveUint64_(*v, height) || height > chain_.Height())
                    return std::nullopt;
                const Block canonical = chain_.GetBlock(height);
                const bool exact =
                    std::any_of(canonical.transactions.begin(), canonical.transactions.end(),
                                [&txid_hex](const Transaction& tx) {
                                    return HashToHex(tx.GetTxID()) == txid_hex;
                                });
                return exact ? std::optional<uint64_t>(height) : std::nullopt;
            } catch (...) {
                return std::nullopt;
            }
        });
        ...
        rpc_.SetFlushTriggerFn([this](bool is_endorse) -> std::string {
            ...
            j << "\"height\":" << h << ",";
            if (!is_endorse) {
                j << "\"status\":\"disabled\",\"reason\":\"pool_flush_is_lottery_only_in_mining_"
                     "loop\"";
                j << "}";
                return j.str();
            }
            j << "\"status\":\"disabled\",\"reason\":\"endorse_flush_is_block_embedded_in_mining_"
                 "loop\"";
            j << "}";
            return j.str();
        });
        ...
        if (p2p_started) {
            ...
            throw std::runtime_error("FATAL: public-testnet P2P activation failed; a node without "
                                     "relay connectivity is not startable");
#endif
            if (!quiet_boot_)
        ...
        reorg_fixup_thread_ = std::thread([this]() {
            for (;;) {
                std::function<void()> task;
                {
                    ...
                    reorg_fixup_cv_.wait(
                        lk, [this] { return !reorg_fixup_q_.empty() || !running_.load(); });
  ...
  public:
    ...
    void LoadFinalityEvidence_() {
        ...
        const auto validate_pair =
            [this, tip](
                const fq::SignedVote& first,
                const fq::SignedVote& second) -> std::optional<fq::ValidatedEquivocationEvidence> {
    ...
    bool WriteValidatedBackgroundPrefix_(uint64_t height, const Hash256& tip_hash) {
        if (height == 0 || HashIsZero(tip_hash))
            return false;
        return DurableWriteControlFile_(ValidatedBackgroundPrefixPath_(),
                                        "schema=1\nheight=" + std::to_string(height) +
    ...
    void RevokeSnapshotValidation_(const std::string& reason) {
        ...
        for (const auto& path :
             {IndependentValidationRequirementPath_(), SnapshotReplayRequirementPath_()}) {
            std::error_code exists_ec;
            const bool present = std::filesystem::exists(path, exists_ec);
            if (exists_ec || !present)
                continue;
            try {
        ...
        const std::string revoked_path = snapshot_bootstrap::RevocationPath(data_dir_);
        if (!DurableWriteControlFile_(revoked_path,
                                      "schema=1\nreason=" + canonical_reason + "\n")) {
        ...
        std::filesystem::remove(snapshot_bootstrap::ReceiptPath(data_dir_), receipt_ec);
  ...
  private:
    ...
    std::atomic<bool> redeem_index_healthy_{
        true}; // N-02: false closes payout RPC until startup replay repairs the derived index
    ...
    void MaybeCaptureBackgroundValidationTarget_() {
        ...
        if (height > 0 &&
            (height <= 10 || height % 25 == 0 || height >= background_validation_target_height_) &&
            height > background_prefix_persisted_height_.load(std::memory_order_acquire) &&
            !chain_.IsEmpty()) {
    ...
    void LoadDurablePublicationRepair_() {
        ...
        if (pending) {
            ...
            if (!stored_tip || stored_tip->height != pending->height ||
                ...
                throw std::runtime_error("durable publication pending identity does not exactly "
                                         "match the authoritative DB tip");
            }
            const auto indexed = db_.GetHashAtHeight(pending->height);
            if (!indexed || *indexed != pending_hex) {
                throw std::runtime_error("durable publication pending identity does not exactly "
                                         "match the canonical height index");
            }
            ...
            if (consumed == 0 || consumed != bytes->size() || body.Serialize() != *bytes ||
                ...
                throw std::runtime_error("durable publication pending block body is non-canonical "
                                         "or does not match its identity");
            }
        }
    ...
    void FinishDurableCommitReplayRepair_() {
        ...
        if (expected_pending) {
            if (!pending || pending->height != expected_pending->height ||
                ...
                throw std::runtime_error("durable publication recovery identity disappeared or "
                                         "changed before replay completed");
            }
            const auto stored_tip = db_.ReadChainTipExact();
            const std::string pending_hex = HashToHex(pending->hash);
            if (!stored_tip || stored_tip->height != pending->height ||
            ...
            if (!selected) {
                ...
                if (incoming_t == current_t) {
                    if (!btcanchor::floor_store::ExactDuplicate(*candidate, *selected)) {
                        throw std::runtime_error("durable block reconstructs conflicting VLF1 "
                                                 "metadata at the current T");
                    }
  ...
  private:
    ...
    std::string AddressHistoryPageJson_(const std::string& address, size_t limit,
        ...
        const auto page = address_history::ReadPage(db_.GetIndexDB(), script, limit, cursor);
        if (!page || !address_history_ready_.load(std::memory_order_acquire) ||
    ...
    void ReplayChain(uint64_t target_height, bool verify_historical_pow = true,
        ...
        if (snapshot_requirement && !snapshot_fast_start_eligible_ && !verify_historical_pow) {
            throw std::runtime_error("ReplayChain: unqualified snapshot attempted to bypass "
                                     "historical PoW verification");
        }
                    ...
                    if (consumed != 0 && consumed == data->size() && gb.Serialize() == *data) {
                        ...
                        if (gb.GetHash() != ghash ||
                            !chain_.AddBlockDirect(gb, replay_genesis_skip_pow, false, false,
                                                   mining::PowAdmissionContext::Internal())) {
                            throw std::runtime_error("ReplayChain: stored genesis bytes do not "
                                                     "match the compiled genesis");
                        }
...
#if defined(VELD_ENABLE_SNAPSHOT_BOOTSTRAP)
        if (snapshot_requirement && (snapshot_requirement->first != target_height ||
    ...
    void BuildAndBroadcastNmsTx(const BlockHeader& nms_header) {
        ...
        for (const auto& u : utxos) {
            if (u.value < NMS_MIN_INPUT)
                continue;
            std::string ukey = HashToHex(u.tx_hash) + ":" + std::to_string(u.output_index);
            if (mempool_spent.count(ukey)) {
                ++skipped_pending;
                continue;
            }
            if (enforce_maturity && u.is_coinbase && u.block_height <= tip &&
        ...
        if (!input_utxo) {
            std::cerr << "  [nms] skip: no eligible UTXO (have " << utxos.size() << " total, "
                      << skipped_pending << " pending in mempool, " << skipped_immature
                      << " immature coinbase, rest below " << ((double)NMS_MIN_INPUT / VELD_UNITS)
                      << " VELD min)\n";
            std::cerr.flush();
            return;
        }
            ...
            try {
                ...
                if (tcp_server_) {
                    ...
                    if (mm > 0 && mt == 0) {
                        ...
                        if (now >= last_warn.load() + 30) {
                            last_warn.store(now);
                            std::cerr << "  [mining] REFUSING TO MINE: every peer that has "
                                         "handshaked with us ("
                                      << mm
                                      << " so far) has a different GENESIS_HASH. We're either on a "
                                      << "stale binary (download the latest from "
                                         "veld.network/downloads/) or "
                                      << "the network has bumped genesis since we last upgraded. "
                                         "Mining now "
                                      << "would just produce a private fork that nobody accepts. "
                                         "Will retry "
                                      << "every 5s; mining auto-resumes the moment we connect to a "
                                         "peer with "
                                      << "matching genesis.\n";
                            std::cerr.flush();
                        }
                ...
                if (settlement_probe.height > 0 &&
                    settlement_probe.height % COMINE_WINDOW_BLOCKS == 0) {
                    (void)BuildPoolPayoutTx(settlement_probe, AddressToScript(POOL_ADDRESS),
                                            mempool_);
                }
                std::vector<Transaction> mandatory_settlements =
                    std::move(settlement_probe.transactions);
