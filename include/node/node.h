            ...
            if (bracket != std::string::npos) {
                ...
                while (i < json.size() && json[i] != ']') {
                    ...
                    if (json[i] == '"') {
                        ...
                        while (!num.empty() && std::isspace(num.back()))
                            num.pop_back();
                        call.params.push_back(num);
                        i = end;
                    }

                    while (i < json.size() && (std::isspace(json[i]) || json[i] == ','))
                        ++i;
                }
            }
        }
...
inline bool IsProtocolSettlementHeight(uint64_t height) {
    return height > 0 &&
           ((height % COMINE_WINDOW_BLOCKS) == 0 || (height % VAULT_DISTRIBUTION_INTERVAL) == 0 ||
...
inline size_t
MaxCanonicalMiningCoinbaseSize(uint64_t height, const std::vector<uint8_t>& miner_script,
    ...
    if (outputs.empty())
        outputs.push_back({{}, 1});
    Transaction cb =
        Transaction::CreateProportionalCoinbase(outputs, "Veld block " + std::to_string(height));
...
inline bool ComputeMiningMempoolBudget(size_t max_coinbase_size,
                                       const std::vector<Transaction>& mandatory_txs,
                                       size_t* out_bytes, size_t* out_count) {
    if (!out_bytes || !out_count || mandatory_txs.size() > (size_t)MAX_TRANSACTIONS_PER_BLOCK - 1)
        return false;
    size_t fixed = 92;
    auto add_size = [&](size_t n) {
        if (n > std::numeric_limits<size_t>::max() - fixed)
            return false;
        fixed += n;
        return true;
    };
    if (!add_size(max_coinbase_size))
        return false;
    for (const auto& tx : mandatory_txs) {
        if (tx.IsCoinbase() || !tx.IsValid())
            return false;
        if (!add_size(tx.Serialize().size()))
            return false;
    }
    if (fixed > (size_t)MAX_BLOCK_SIZE)
        return false;
    *out_bytes = (size_t)MAX_BLOCK_SIZE - fixed;
    *out_count = (size_t)MAX_TRANSACTIONS_PER_BLOCK - 1 - mandatory_txs.size();
    return true;
}

inline MineBlockResult MineAndCommit(Blockchain& chain, Mempool& mempool,
                                     const RealKeyPair& miner_keypair, uint32_t bits_override = 0,
    ...
    if (!DecodeExpectedVeldTarget(candidate.header.bits, expected_bits, mining_target)) {
    ...
    if (IsProtocolSettlementHeight(candidate.height)) {
        result.error = "protocol settlement height requires the VeldNode mining builder";
        return result;
    }
    ...
    const size_t max_coinbase_size = MaxCanonicalMiningCoinbaseSize(
        candidate.height, miner_script, pool_script_const, vault_script, endorse_script_const);
    if (!ComputeMiningMempoolBudget(max_coinbase_size, {}, &mempool_byte_budget,
                                    &mempool_count_budget)) {
        result.error = "canonical coinbase exceeds the block-size envelope";
        return result;
    }
    auto mempool_raw =
        mempool.GetBlockTransactionsWithFees(mempool_count_budget, mempool_byte_budget, &chain);
    ...
    const bool is_pre_activation_sm = (supply_at_parent_sm < STAKING_UNLOCK_SUPPLY);
    ...
    Transaction coinbase =
        ProtocolUpgradeActive(candidate.height)
    ...
    auto global_pow_lease =
        mining::GlobalExpensivePowBudget().TryAcquire(mining::ExpensivePowUse::InternalMine);
    if (!global_pow_lease) {
    ...
    for (uint64_t nonce = 0;; ++nonce) {
        ...
        if (h < target) {
            ...
            if (veld::DiagVerbose().load())
                veld::vcerr() << "  [mine_debug h=" << candidate.height
                              << "] pow=" << HashToHex(h).substr(0, 32) << "... nonce=" << nonce
                              << " prev="
                              << HashToHex(candidate.header.prev_block_hash).substr(0, 16)
                              << "... merk="
                              << HashToHex(candidate.header.merkle_root).substr(0, 16)
    ...
    bool committed = chain.AddBlockDirect(candidate, false, false, false,
...
inline MineBlockResult
MineOnly(Blockchain& chain, Mempool& mempool, const RealKeyPair& miner_keypair,
         uint32_t bits_override = 0, std::atomic<bool>* stop = nullptr,
         const std::vector<uint8_t>& pool_script = {}, unsigned num_threads = 1,
         MiningProgressCb progress_cb = nullptr,
         const std::vector<std::pair<std::vector<uint8_t>, uint64_t>>& validator_cb_outputs = {},
         NmsBroadcastCb nms_cb = nullptr, std::atomic<uint64_t>* progress_counter = nullptr,
         const std::vector<Transaction>& mandatory_txs = {},
    ...
    auto vault_script = AddressToScript(VaultAddressAtHeight(candidate.height));
    auto miner_script = miner_keypair.script_override.empty() ? miner_keypair.GetP2PKHScript()
    ...
    const auto effective_pool_script =
        !shared_pool_script.empty() ? shared_pool_script : pool_script;
    const auto endorse_script = AddressToScript(ENDORSEMENT_POOL_ADDRESS);
    size_t mempool_byte_budget = 0, mempool_count_budget = 0;
    const size_t max_coinbase_size =
        MaxCanonicalMiningCoinbaseSize(candidate.height, miner_script, effective_pool_script,
                                       vault_script, endorse_script, coinbase_metadata_scripts);
    if (!ComputeMiningMempoolBudget(max_coinbase_size, mandatory_txs, &mempool_byte_budget,
                                    &mempool_count_budget)) {
        result.error = "mandatory settlements and coinbase exceed the block-size envelope";
        return result;
    }
    ...
    const bool is_pre_activation = (supply_at_parent_for_split < STAKING_UNLOCK_SUPPLY);
    ...
    auto build_coinbase = [&](uint64_t fees) {
        ...
        if (effective_reward == 0) {
            if (fees > 0) {
                    ...
                    coinbase_outputs[0].second += vault_cut;
                {
                    ...
                    if (!eps.empty() && endorse_cut > 0)
                        coinbase_outputs.push_back({eps, endorse_cut});
                    else
                        coinbase_outputs[0].second += endorse_cut;
                }
            } else {
                coinbase_outputs.push_back({vault_script, 0});
            }
        } else if (candidate.height > 0 && candidate.height % VAULT_BLOCK_INTERVAL == 0) {
        ...
        Transaction cb =
            ProtocolUpgradeActive(candidate.height)
                ? Blockchain::BuildCanonicalCoinbase(candidate.height, supply_at_parent_for_split,
                                                     fees, miner_script, coinbase_metadata_scripts)
                : Transaction::CreateProportionalCoinbase(
                      coinbase_outputs, "Veld block " + std::to_string(candidate.height));
    ...
    if (candidate_preflight) {
        auto selection = mining::SelectConstructivePreflightCandidate(
            candidate, mempool_with_fees, mandatory_txs, build_coinbase, candidate_preflight);
    ...
    if (veld::DiagVerbose().load())
        veld::vcerr() << "  [mineonly h=" << candidate.height
    ...
    for (uint64_t attempts = 1; attempts <= 1000000; ++attempts) {
        if (stop && stop->load()) {
            result.error = "test mining cancelled";
            return result;
        }
        const auto proof =
            mining::VeldHash(candidate.header.Serialize(), candidate.height, mining_target);
        if (!mining::g_veldhash_last_dataset_ok()) {
            result.error = "test dataset unavailable";
            return result;
        }
    ...
    auto rebuild_header = [&]() {
        ...
        std::copy(candidate.header.prev_block_hash.begin(), candidate.header.prev_block_hash.end(),
                  header_bytes.begin() + 4);
        std::copy(candidate.header.merkle_root.begin(), candidate.header.merkle_root.end(),
                  header_bytes.begin() + 36);
    ...
    const Hash256 ds_seed_loop = mining::ComputeEpochSeed(candidate.header.prev_block_hash,
                                                          candidate.height, candidate.header.bits);
#if !(defined(VELD_LOCAL_TEST_NETWORK) && defined(VELD_LIGHT_VERIFY))
    {
        mining::DatasetHandle warm = mining::GlobalDataset().get_for_seed(ds_seed_loop);
        if (!warm.get()) {
    ...
    auto worker_fn = [&](unsigned worker_id) {
        ...
        auto tw64 = [&](int offset, uint64_t v) {
            for (int i = 0; i < 8; ++i)
                tb[offset + i] = (uint8_t)((v >> (i * 8)) & 0xFF);
        };
        auto rebuild_local = [&]() {
        ...
        while (!stop_local.load(std::memory_order_relaxed)) {
...
#else
                mining::DatasetHandle ds = mining::GlobalDataset().get_for_seed(ds_seed_loop);
                if (!ds.get()) {
                    stop_local = true;
                    break;
                }
                ...
                if constexpr (OPTION_B_CONSENSUS_GATE_ENABLED) {
                    if (nms_cb && h < target_x4 && !(h < target)) {
                        ...
                        if (nms_broadcast_guard.compare_exchange_strong(expected, true)) {
                            if (veld::DiagVerbose().load())
                                veld::vcerr() << "  [nms-worker w=" << worker_id
            ...
            if (h < target) {
                if (!winner_found.exchange(true)) {
                    ...
                    winner_ts = ts_hashed;
                    std::copy(tb.begin() + 36, tb.begin() + 68, winner_merkle_root.begin());
                    if (veld::DiagVerbose().load())
                        veld::vcerr()
                            << "  [mine_debug h=" << candidate.height
                            << "] pow=" << HashToHex(h).substr(0, 32) << "... nonce=" << nonce
                            << " prev=" << HashToHex(candidate.header.prev_block_hash).substr(0, 16)
                            << "... merk=" << HashToHex(winner_merkle_root).substr(0, 16)
                            << "... bits=0x" << std::hex << candidate.header.bits << std::dec
                            << " ts=" << ts_hashed << " worker=" << worker_id << "/" << N << "\n";
                    std::cerr.flush();
                }
                stop_local = true;
                break;
...
class VeldNode {
  public:
    ...
    class PeerWorkViewTransitionPermit {
      public:
        PeerWorkViewTransitionPermit(VeldNode& owner,
                                     Blockchain::ConsensusTransitionGuard transition) noexcept
            : owner_(owner), transition_(std::move(transition)) {}
        PeerWorkViewTransitionPermit(const PeerWorkViewTransitionPermit&) = delete;
        PeerWorkViewTransitionPermit& operator=(const PeerWorkViewTransitionPermit&) = delete;
    ...
    class LocalSigningPermit {
      public:
        ...
        bool IsLive() const noexcept {
            return lease_ && std::chrono::steady_clock::now() < operation_deadline_ &&
                   lease_->IsLive();
        }
      ...
      private:
        ...
        explicit LocalSigningPermit(
            ...
            : lease_(std::move(lease)), operation_deadline_(operation_deadline) {}

        std::shared_ptr<work_admission::AdmissionCoordinator::Lease> lease_;
        std::chrono::steady_clock::time_point operation_deadline_{};
    };

    static constexpr std::chrono::milliseconds LOCAL_SIGNING_SAFETY_MARGIN{1000};
    static constexpr std::chrono::milliseconds REMOTE_SIGNING_SAFETY_MARGIN{2000};
    static constexpr std::chrono::milliseconds PEER_VIEW_EXPIRY_SAFETY_MARGIN{1000};

    static std::optional<uint64_t>
    BoundWallClockWorkLifetimeMs(int64_t now_unix, int64_t not_after_unix, uint64_t requested_ms,
                                 uint64_t safety_margin_ms) noexcept {
        if (requested_ms == 0 || now_unix < 0 || not_after_unix <= now_unix)
            return std::nullopt;
        const uint64_t remaining_seconds = static_cast<uint64_t>(not_after_unix - now_unix);
        if (remaining_seconds <= 1)
            return std::nullopt;
        const uint64_t conservative_seconds = remaining_seconds - 1;
        const uint64_t conservative_ms =
            conservative_seconds > UINT64_MAX / 1000 ? UINT64_MAX : conservative_seconds * 1000;
        if (conservative_ms <= safety_margin_ms)
            return std::nullopt;
        const uint64_t bounded = std::min(requested_ms, conservative_ms - safety_margin_ms);
        return bounded == 0 ? std::nullopt : std::optional<uint64_t>(bounded);
    }

    struct ActiveRemoteSigningLease {
        work_admission::Path path{work_admission::Path::ValidatorEndorsement};
        work_admission::Binding binding{};
        std::shared_ptr<work_admission::AdmissionCoordinator::Lease> lease;
        std::chrono::steady_clock::time_point operation_deadline{};

        bool OperationLive() const noexcept {
            return lease && std::chrono::steady_clock::now() < operation_deadline &&
                   lease->IsLive();
        }
    };
    ...
    static std::string EnsureDataDir(const std::string& d, uint32_t magic) {
        ...
        if (!channel::secure_file::EnsurePrivateDirectory(path, &identity_error) ||
            ...
            throw std::runtime_error("public datadir network identity check failed: " +
                                     identity_error);
        }
#endif
#if defined(VELD_ENABLE_SNAPSHOT_BOOTSTRAP)
    ...
    explicit VeldNode(const NetworkConfig& config, const std::string& data_dir)
        : config_(config), data_dir_(EnsureDataDir(data_dir, config.magic)), chain_(),
          work_admission_coordinator_(
              work_admission::AdmissionCoordinator::Limits{
                  std::chrono::milliseconds(5000), std::chrono::milliseconds(10000), 64, 128},
          ...
          block_template_authorizations_(
              ...
              [](work_admission::BlockTemplateAuthorizationStore::TokenBytes& token) {
                  return compat::SecureRandom(token.data(), token.size());
              }),
          mempool_(), staking_(), onchain_tokens_(),
          btc_headers_(BtcVeldCheckpoint(), BtcVeldPowLimit(), 2016, 1209600, BtcVeldNoRetarget()),
          tiers_(chain_, staking_), vault_(), governance_(chain_, staking_),
          storage_(data_dir_ + "/blocks", config.magic), db_(data_dir_ + "/db"),
        ...
        rpc_.SetWorkAdmissionFn(
            [this](work_admission::Path path, const work_admission::Subject& subject,
                   const std::optional<work_admission::Binding>& prior, bool require_prior) {
                return EvaluateWorkAdmission(path, subject, prior, require_prior);
            });
        rpc_.SetIssueBlockTemplateAuthorizationFn(
            [this](const work_admission::Subject& subject, const Hash256& template_identity) {
                return IssueBlockTemplateAuthorizationUnderTransition_(subject, template_identity);
            });
        rpc_.SetConsumeBlockTemplateAuthorizationFn(
            [this](const std::string& token, const work_admission::Binding& binding) {
                return ConsumeBlockTemplateAuthorizationUnderTransition_(token, binding);
            });
        rpc_.SetRemoteWorkGrantFn(
            [this](work_admission::Path path, const work_admission::Subject& subject) {
                return IssueRemoteWorkGrantUnderTransition_(path, subject);
            });
        rpc_.SetBeginRemoteSigningFn([this](work_admission::Path path, const std::string& binding,
                                            const std::string& token) {
            return BeginRemoteSigningUnderTransition_(path, binding, token);
        });
        rpc_.SetCancelRemoteSigningFn([this](const std::string& token) {
            return CancelRemoteSigningUnderTransition_(token);
        });
        rpc_.SetValidatorEndorsementSinkFn([this](const Transaction& tx, uint64_t fee,
                                                  const std::string& binding,
                                                  const std::string& token, bool rebroadcast) {
            return SubmitAuthorizedValidatorEndorsement_(tx, fee, binding, token, rebroadcast);
        });
        ...
        chain_.SetCanonicalWorkTransitionFn([this](const Block&) noexcept {
            const auto closed =
                work_admission_coordinator_.BeginClose(work_admission::Refusal::BindingMismatch);
        ...
        chain_.SetLocalWorkAdmissionPrepareFn(
            [this](const Block& block, const mining::PowAdmissionContext& context)
                -> std::optional<Blockchain::LocalWorkAdmissionTicket> {
                const auto path = WorkPathForLocalKind_(context.local_work_kind);
                const auto prior = work_admission::DecodeBinding(context.work_binding);
                Block tip;
                if (!path || !prior ||
                    work_admission::EncodeBinding(*prior) != context.work_binding ||
                    prior->subject.purpose != work_admission::Purpose::BlockProduction ||
                ...
                const Hash256 work_identity = block.header.GetTemplateWorkIdentity();
                ...
                const auto configuration =
                    WorkCoordinatorConfiguration_(subject.parent_height, true);
                const auto opened = OpenWorkCoordinator_(configuration);
                if (!opened.opened)
                    return std::nullopt;
                auto lease_ttl = BoundPeerWorkLifetime_(std::chrono::milliseconds(5000));
                if (!lease_ttl)
                    return std::nullopt;
                if (*path == work_admission::Path::SubmitBlock) {
                    const auto coordinator = work_admission_coordinator_.GetSnapshot();
                    if (coordinator.phase != work_admission::AdmissionCoordinator::Phase::Open ||
                        coordinator.configuration_generation == 0)
                        return std::nullopt;
                    const auto& authorization = context.block_template_authorization_claim;
                    if (!authorization->ClaimForTicket(*prior,
                ...
                auto attempt = work_admission_coordinator_.AcquireLocal(
                    *path, subject, normalized_prior, true, *lease_ttl);
                if (!attempt || !attempt.lease)
                    return std::nullopt;
                auto lease = std::make_shared<work_admission::AdmissionCoordinator::Lease>(
                    std::move(*attempt.lease));
                if (context.block_template_authorization_claim &&
                    !context.block_template_authorization_claim->IsClaimedAndLive())
                    return std::nullopt;
                ...
                auto authority = std::make_shared<PreparedTicketAuthority>(
                    PreparedTicketAuthority{lease, context.block_template_authorization_claim});
                ...
                ticket.claim_for_canonical_commit = [this, authority](
                    ...
                    if (authority->template_claim && !authority->template_claim->IsClaimedAndLive())
                        return false;
                    if (authority->lease->configuration_generation() != coordinator_generation ||
                        validation_generation_.load(std::memory_order_acquire) !=
                            validation_generation ||
                        issued.validation_generation != validation_generation ||
                ...
                ticket.validation_generation = binding.validation_generation;
                ticket.coordinator_generation = lease->configuration_generation();
                ...
                ticket.deadline =
                    authority->template_claim
                        ? std::min(lease->deadline(), authority->template_claim->deadline())
                        : lease->deadline();
                return ticket;
            });
        ...
        rpc_.SetFinalityEvidenceListFn([this](size_t offset, size_t limit) {
            return ListFinalityEquivocationEvidence(offset, limit);
        });
        rpc_.SetFinalityEvidenceFindFn([this](const Hash256& evidence_id) {
            return FindFinalityEquivocationEvidence(evidence_id);
        });
        rpc_.SetFinalitySlashPrepareFn([this](const Hash256& evidence_id) {
    ...
    void SetBackgroundValidationOnly(bool enabled) {
        if (running_.load(std::memory_order_acquire) || tcp_server_)
            throw std::logic_error("background validation mode must be selected before Start");
        background_validation_only_ = enabled;
    }
    void SetSnapshotQuarantineOnly(bool enabled) {
        if (running_.load(std::memory_order_acquire) || tcp_server_)
            throw std::logic_error("snapshot quarantine mode must be selected before Start");
        snapshot_quarantine_only_ = enabled;
    }
    bool SnapshotQuarantineOnly() const {
        return snapshot_quarantine_only_;
    }
    ...
    bool PersistFullIbdReceiptAtTip(const RealKeyPair& operational_identity, bool fleet,
                                    snapshot_bootstrap::FullIbdReceipt& out,
                                    std::string* error = nullptr) {
        try {
            auto transition = chain_.AcquireConsensusTransitionGuard();
            if (!ChainFullyValidated() || FailStopRequired() ||
                !AnchorSecurityImportSatisfiedLocked_() || IndependentValidationBase() ||
                chain_.IsEmpty()) {
                if (error)
                    *error = "canonical history is not fully verified";
                return false;
            }
            const auto* receipt_name = fleet ? snapshot_bootstrap::FLEET_RECEIPT_FILENAME
            ...
            if (recovery && recovery->phase == "complete") {
                ...
                CompleteSnapshotRecoveryLocked_(receipt_name, operational_identity.public_key);
                return fleet ? snapshot_bootstrap::VerifyFleetIbdReceipt(
                                   data_dir_, operational_identity.public_key, out, error)
            ...
            const bool written = fleet ? snapshot_bootstrap::WriteFleetIbdReceipt(
    ...
    bool PersistAddressOnlyIbdReceiptAtTip(const RealKeyPair& validation_identity,
        ...
        try {
            ...
            const auto recovery = snapshot_recovery::Load(data_dir_);
            if (recovery && recovery->phase == "complete") {
                CompleteSnapshotRecoveryLocked_(snapshot_bootstrap::ADDRESS_RECEIPT_FILENAME,
                                                validation_identity.public_key, payout_address);
                return snapshot_bootstrap::VerifyAddressIbdReceipt(
                    data_dir_, validation_identity.public_key, payout_address, out, error, false,
            ...
            identity.address = PubKeyToAddress(identity.public_key, false);
            const auto hash = HashToHex(tip.GetHash());
            if (!snapshot_bootstrap::WriteAddressIbdReceipt(data_dir_, identity, payout_address,
                                                            tip.height, hash, error,
                                                            config_.IsTestNetwork()))
                return false;
            CompleteSnapshotRecoveryLocked_(snapshot_bootstrap::ADDRESS_RECEIPT_FILENAME,
  ...
  private:
    void CompleteSnapshotRecoveryLocked_(const std::string& receipt_name,
        ...
        if (!SnapshotRecoveryPending())
            return;
        if (!IsIBDComplete() || !ChainFullyValidated() || !AnchorSecurityImportSatisfiedLocked_() ||
            FailStopRequired() || IndependentValidationBase())
            throw std::runtime_error(
                "snapshot recovery has not completed independent ordinary IBD");
        snapshot_bootstrap::FullIbdReceipt receipt;
        std::string error;
        const bool valid =
            receipt_name == snapshot_bootstrap::RECEIPT_FILENAME
                ? snapshot_bootstrap::VerifyFullIbdReceipt(data_dir_, expected_identity, receipt,
                                                           &error, true)
            : receipt_name == snapshot_bootstrap::FLEET_RECEIPT_FILENAME
                ? snapshot_bootstrap::VerifyFleetIbdReceipt(data_dir_, expected_identity, receipt,
                                                            &error, true)
                : receipt_name == snapshot_bootstrap::ADDRESS_RECEIPT_FILENAME &&
                      snapshot_bootstrap::VerifyAddressIbdReceipt(data_dir_, expected_identity,
                                                                  expected_payout, receipt, &error,
                                                                  true, config_.IsTestNetwork());
        ...
        if (!valid || receipt.height > chain_.Height() ||
            ...
            throw std::runtime_error("recovery receipt does not authenticate the rebuilt chain: " +
                                     error);
  ...
  private:
    bool AnchorSecurityImportSatisfiedLocked_() const {
        if (anchor_floor_security_uncertain_.load(std::memory_order_acquire) ||
            anchor_floor_repair_required_.load(std::memory_order_acquire))
            return false;
  ...
  public:
    ...
    bool SecurityFloorUncertain() const {
        return anchor_floor_security_uncertain_.load(std::memory_order_acquire) ||
               anchor_floor_repair_required_.load(std::memory_order_acquire);
    }
    ...
    bool DurableCommitFailStop() const {
        return durable_commit_fail_stop_.load(std::memory_order_acquire) ||
               durable_commit_repair_required_.load(std::memory_order_acquire) ||
    ...
    void SetPublicTestnetCompiledLease(const public_testnet::RuntimeLimits& lease) {
        if (running_.load(std::memory_order_acquire) || public_testnet_limits_) {
            throw std::runtime_error(
                "public-testnet runtime limits are startup-only and immutable");
        }
        const int64_t local_now = public_testnet::CurrentUnixTime();
        if (!lease.TimePermitted(local_now) || lease.not_after_height == 0 ||
            !public_testnet::IsCanonicalSha256(lease.lease_identity_sha256)) {
            throw std::runtime_error("public-testnet lease is malformed or already ended");
        }
        std::string error;
        auto clock_guard = std::make_unique<public_testnet::RuntimeClockGuard>();
        if (!clock_guard->Initialize(data_dir_, lease, local_now, &error)) {
            throw std::runtime_error("invalid public-testnet runtime clock: " + error);
        }
        ...
        chain_.SetRuntimeAdmissionFn([this](uint64_t candidate_height) {
            ...
            const bool time_open =
                public_testnet_clock_guard_ && public_testnet_clock_guard_->ObserveNow();
            if (!public_testnet::PermitOrLatchClosed(public_testnet_expired_, time_open))
                return false;
            return public_testnet_limits_->CandidateHeightPermitted(candidate_height);
        });
        rpc_.SetRuntimeAdmissionFn([this]() { return !PublicTestnetRuntimeStopRequired(); },
    ...
    bool PublicTestnetRestartAuthorityFreshNow() const noexcept {
        ...
        return public_testnet_limits_->TimePermitted(local_now) &&
               public_testnet_clock_guard_->ObserveAt(local_now, monotonic_now);
    }

    bool PublicTestnetRuntimeStopRequired() const noexcept {
        const bool sampled = public_testnet_limits_ && public_testnet_clock_guard_ &&
        ...
        return !public_testnet::PermitOrLatchClosed(public_testnet_expired_, sampled);
    }
#else
    bool PublicTestnetRuntimeStopRequired() const noexcept {
        return false;
    ...
    bool TestMainTokenConsensusDependenciesBound() const {
        return onchain_tokens_.TestHasBtcVeldConsensusDependencies(&btc_headers_, &bond_covenant_);
    }
    ...
    bool TestAnchorSecurityPinPermits(uint64_t height, const Hash256& hash) const {
        return EffectiveAnchorSecurityPinPermits_(height, hash);
    }
    bool TestAnchorSecurityReorgPermitted(uint64_t common_ancestor, uint64_t current_tip) const {
        return EffectiveAnchorSecurityReorgPermitted_(common_ancestor, current_tip);
    }
    std::string TestAnchorSecurityStatus() const {
        auto transition = chain_.AcquireConsensusTransitionGuard();
        return EffectiveAnchorSecurityStatusName_(EffectiveAnchorSecurityStatusNoTransition_());
    }
    ...
    size_t TestObservedAnchorSecurityFloorCount() const {
        std::lock_guard<std::mutex> lock(anchor_security_floors_mutex_);
        return static_cast<size_t>(std::count_if(effective_anchor_security_floors_.begin(),
    ...
    bool TestAllAnchorSecurityFloorsPermanentlyCovered() const {
        std::lock_guard<std::mutex> lock(anchor_security_floors_mutex_);
        return std::all_of(effective_anchor_security_floors_.begin(),
                           effective_anchor_security_floors_.end(),
    ...
    void
    TestSetAnchorSecurityPermanent(const btcanchor::AnchorSet::PermanentCheckpoint& permanent) {
    ...
    std::optional<btcanchor::floor_store::Record> TestAnchorLocalFloor() const {
        std::lock_guard<std::mutex> lock(anchor_floor_store_mutex_);
        return anchor_floor_store_.Current();
    }
    bool TestAnchorFloorRepairRequired() const {
        return anchor_floor_repair_required_.load(std::memory_order_acquire);
    }
    ...
    std::optional<db::VeldDB::DurablePublicationPending> TestDurablePublicationPending() {
    ...
    std::optional<std::string> TestReadDurableUTXO(const Hash256& tx_hash, uint32_t index) {
    ...
    bool TestSetCoinbaseAccountingParent(uint64_t supply) {
        auto transition = chain_.AcquireConsensusTransitionGuard();
        if (running_.load() || tcp_server_ || chain_.IsEmpty())
            return false;
        const Block tip = chain_.TipCopy();
        const uint64_t previous = chain_.TotalSupplyUnits();
        const auto durable = db_.ReadChainTipExact();
        if (supply < previous || supply > MAX_SUPPLY_UNITS || last_token_height_ != tip.height ||
            last_module_supply_ != previous ||
            staking_.SnapshotState().total_supply_units != previous || !durable ||
            durable->height != tip.height || durable->tip_hash != HashToHex(tip.GetHash()) ||
            durable->supply_units != previous || db_.ReadDurablePublicationPending() ||
            db_.UtxoRecoveryRequired() || chain_.ReorgRecoveryFramePending() ||
            DurableCommitFailStop() || FailStopRequired())
            return false;
    ...
    Block TestBuildDStateQualificationBlock(uint64_t expected_height,
                                            const std::vector<uint8_t>& miner_script,
                                            const std::string& token_address,
                                            const std::string& lp_address, uint64_t timestamp,
                                            uint64_t nonce, uint64_t capacity_token_accounts = 0,
                                            uint64_t capacity_lp_identities = 0) {
        if (chain_.IsEmpty() || expected_height != chain_.Height() + 1)
            throw std::logic_error("state-capacity candidate height does not extend the live tip");
        if (miner_script.size() != 25 || miner_script[0] != 0x76 || miner_script[1] != 0xA9 ||
            miner_script[2] != 0x14 || miner_script[23] != 0x88 || miner_script[24] != 0xAC)
            throw std::logic_error("state-capacity candidate miner script is not canonical P2PKH");
        ...
        if (!valid_field(token_address) || !valid_field(lp_address))
            throw std::logic_error("state-capacity candidate carrier field is not canonical");
        const bool capacity_carrier = capacity_token_accounts != 0 || capacity_lp_identities != 0;
        if (capacity_carrier && (expected_height != 400 || token_address != "-" ||
                                 lp_address != "-" || capacity_token_accounts == 0 ||
                                 capacity_token_accounts > 100'000 || capacity_lp_identities == 0 ||
        ...
        if (expected_height % COMINE_WINDOW_BLOCKS == 0) {
            (void)BuildPoolPayoutTx(settlement_probe, AddressToScript(POOL_ADDRESS), mempool_);
        }
        std::vector<Transaction> mandatory_settlements = std::move(settlement_probe.transactions);
        const auto finality_metadata = PendingFinalityCoinbaseMetadata_(expected_height);

        RealKeyPair qualification_miner;
        qualification_miner.script_override = miner_script;
        MineBlockResult candidate = MineOnly(
            chain_, mempool_, qualification_miner, 0, nullptr, AddressToScript(POOL_ADDRESS), 1,
            nullptr, {}, nullptr, nullptr, mandatory_settlements,
            [this](const Block& block) { return PreflightMiningCandidate_(block); },
            finality_metadata,
            /*qualification_candidate_only=*/true);
        if (!candidate.success || candidate.block.height != expected_height)
            throw std::runtime_error("state-capacity production candidate construction failed: " +
                                     candidate.error);

        Block& block = candidate.block;
        block.header.timestamp = timestamp;
        block.header.nonce = nonce;
        if (token_address != "-" || lp_address != "-" || capacity_carrier) {
            if (block.transactions.empty() || block.transactions.front().inputs.size() != 1 ||
            ...
            const std::string carrier =
                capacity_carrier
                    ? "dstate-replay:" + std::to_string(expected_height) +
                          "|VELD_DSTATE_S6_CAPACITY_V1|" + std::to_string(capacity_token_accounts) +
                          "|" + std::to_string(capacity_lp_identities)
                    : "dstate-replay:" + std::to_string(expected_height) + "|VELD_DSTATE_Q1|" +
                          token_address + "|" + lp_address;
            auto& coinbase = block.transactions.front();
            coinbase.inputs.front().script_sig.assign(carrier.begin(), carrier.end());
    ...
    bool TestIngestDStateQualificationFrame(const std::vector<uint8_t>& raw,
        ...
        if (expected_height == 0 && raw != CreateGenesisBlock().Serialize())
            return false;
        if (expected_height > 0) {
            const auto parent_height =
                chain_.GetKnownBlockHeightByHash(block.header.prev_block_hash);
            if (!parent_height || *parent_height == UINT64_MAX ||
                *parent_height + 1 != expected_height)
                return false;
        }
        return chain_.AddBlockDirect(block, false, false, false,
                                     mining::PowAdmissionContext::Internal());
    }
    ...
    DStateQualificationObservation TestObserveDStateQualificationState() {
        auto transition = chain_.AcquireConsensusTransitionGuard();
        DStateQualificationObservation out;
        out.height = chain_.Height();
        if (!chain_.IsEmpty())
            out.tip_hash = chain_.TipCopy().GetHash();
        ...
        out.consensus_state_digest =
            btcveld::reserve::TRANSITION_V1_REQUIRED
                ? state_digest::ComposeV8(
                      out.height, out.tip_hash, out.utxo_digest, validators_.ValidatorsDigest(),
                      staking_.StakingDigest(), validators_.BondYieldEscrowDigest(),
                      chain_.NmsTallyDigest(), out.token_digest, out.supply_digest,
                ...
                : state_digest::ComposeV7(
                      ...
                      btc_headers_.StateDigest(), anchors_.Digest(), out.amm_digest,
                      fin_state_.Digest(), bond_covenant_.Digest());
        out.miner_archive_ready = miner_archive_ready_.load(std::memory_order_acquire);
        out.durable_publication_pending = db_.ReadDurablePublicationPending().has_value();
        out.utxo_recovery_required = db_.UtxoRecoveryRequired();
        out.reorg_recovery_frame_pending = chain_.ReorgRecoveryFramePending();
        ...
        out.module_cursor_matches =
            last_token_height_ == out.height && last_module_supply_ == out.canonical_supply_units;
        return out;
    }
    ...
    void TestWireMinerArchiveLookup() {
        chain_.SetMinerArchiveLookup(
            [this](const std::string& script_hex) -> std::optional<Blockchain::MinerArchiveRecord> {
                return ReadMinerArchiveRecord_(script_hex);
            });
    }
    void TestWireMinerArchiveRollbackOnly() {
        chain_.SetOnRollback([this](const Blockchain::UTXODelta&, const Block& popped) {
    ...
    bool TestPutMinerArchiveRaw(const std::string& key, const std::string& value) {
        return db_.GetIndexDB().Put(key, value);
    }
    std::optional<std::string> TestGetMinerArchiveRaw(const std::string& key) {
    ...
    std::optional<std::vector<uint8_t>> TestCapturedReorgAnchorFloorForAbort() const {
        return reorg_anchor_floor_restore_wire_;
    }
    std::optional<std::vector<uint8_t>> TestCapturedReorgDurablePublicationForAbort() const {
    ...
    void TestConfigureWorkAdmissionProcess(const TestWorkAdmissionProcessState& state) {
        work_admission_coordinator_.CancelAndClose(work_admission::Refusal::Unwired);
        ClearActiveRemoteSigningLeases_();
        startup_replay_complete_.store(state.startup_replay_complete, std::memory_order_release);
        chain_fully_validated_.store(state.independent_validation_complete,
                                     std::memory_order_release);
        ibd_complete_.store(state.sync_complete, std::memory_order_release);
        snapshot_state_clean_.store(state.snapshot_state_clean, std::memory_order_release);
        durable_commit_fail_stop_.store(!state.durable_state_proven, std::memory_order_release);
        durable_commit_repair_required_.store(false, std::memory_order_release);
        reorg_publication_uncertain_.store(false, std::memory_order_release);
        datadir_identity_valid_.store(state.datadir_identity_valid, std::memory_order_release);
        checkpoint_anchor_valid_.store(state.checkpoint_anchor_valid, std::memory_order_release);
        background_validation_only_ = !state.role_permitted;
        test_work_force_tip_unknown_.store(!state.canonical_tip_known, std::memory_order_release);
        test_work_local_runtime_open_.store(state.runtime_open, std::memory_order_release);
        ...
        chain_.SetRuntimeAdmissionFn([](uint64_t) noexcept { return true; });
    ...
    void TestInstallWorkAdmissionProcessServer() {
        tcp_server_ = std::make_unique<net::NodeServer>(0, config_.magic, chain_, mempool_);
        tcp_server_->SetPeerWorkViewTransitionFn(
            [this]() -> net::NodeServer::PeerWorkViewTransitionPermit {
                (void)CloseWorkAdmissionBounded_(work_admission::Refusal::PeerViewUnsafe);
                try {
                    auto transition = chain_.AcquireConsensusTransitionGuard();
                    return net::NodeServer::PeerWorkViewTransitionPermit(
                        new PeerWorkViewTransitionPermit(*this, std::move(transition)));
                } catch (...) {
                    work_admission_coordinator_.CancelAndClose(work_admission::Refusal::Unwired);
                    return {};
                }
        ...
        rpc_.SetBlockBroadcast([this](const Block& block) {
            test_work_block_broadcast_calls_.fetch_add(1, std::memory_order_acq_rel);
            if (!tcp_server_)
                throw std::runtime_error("test process block transport unavailable");
    ...
    std::optional<work_admission::Subject> TestCurrentBlockProductionSubject() const {
        return CurrentBlockProductionSubject_();
    }

    work_admission::Decision
    TestEvaluateWorkAdmission(work_admission::Path path,
    ...
    void TestEnableWorkAdmissionMiningBarrier(bool enabled) noexcept {
        test_work_admission_mining_barrier_.store(enabled, std::memory_order_release);
    }

    void TestResetWorkAdmissionProcessCounters() noexcept {
        test_work_mining_admitted_calls_.store(0, std::memory_order_release);
        test_work_durable_writer_calls_.store(0, std::memory_order_release);
        test_work_block_broadcast_calls_.store(0, std::memory_order_release);
        test_work_tx_gossip_calls_.store(0, std::memory_order_release);
        test_work_finality_gossip_calls_.store(0, std::memory_order_release);
        test_work_finality_sink_calls_.store(0, std::memory_order_release);
        test_work_finality_active_calls_.store(0, std::memory_order_release);
        test_work_finality_verify_result_.store(UINT8_MAX, std::memory_order_release);
    ...
    uint64_t TestWorkMiningAdmittedCalls() const noexcept {
        return test_work_mining_admitted_calls_.load(std::memory_order_acquire);
    }
    uint64_t TestWorkDurableWriterCalls() const noexcept {
        return test_work_durable_writer_calls_.load(std::memory_order_acquire);
    }
    uint64_t TestWorkBlockBroadcastCalls() const noexcept {
        return test_work_block_broadcast_calls_.load(std::memory_order_acquire);
    }
    uint64_t TestWorkTxGossipCalls() const noexcept {
        return test_work_tx_gossip_calls_.load(std::memory_order_acquire);
    }
    uint64_t TestWorkFinalityGossipCalls() const noexcept {
        return test_work_finality_gossip_calls_.load(std::memory_order_acquire);
    }
    uint64_t TestWorkFinalitySinkCalls() const noexcept {
        return test_work_finality_sink_calls_.load(std::memory_order_acquire);
    }
    uint64_t TestWorkFinalityActiveCalls() const noexcept {
        return test_work_finality_active_calls_.load(std::memory_order_acquire);
    }
    uint8_t TestWorkFinalityVerifyResult() const noexcept {
        return test_work_finality_verify_result_.load(std::memory_order_acquire);
    }
    ...
    uint16_t TestLocalExplorerPort() const {
        return explorer_.Port();
    }
#endif
    StakingLedger& GetStaking() {
        return staking_;
    }
    ...
    ListFinalityEquivocationEvidence(size_t offset = 0, size_t limit = 100) const {
        namespace fq = ::veld::finality::qc;
        if (offset >= fq::FinalityEquivocationCollector::MAX_COMPLETED_PAIRS || limit == 0)
            return {};
    ...
    std::optional<std::string> PrepareFinalityEquivocationSlash(const Hash256& evidence_id) const {
        namespace fq = ::veld::finality::qc;
        ...
        if (!validators_.CanAcceptFinalityEquivocationEvidence(
                vote.pubkey_hex, vote.epoch_id, vote.set_root, static_cast<uint8_t>(vote.phase),
                vote.round, vote.target.height, tip + 1))
            return std::nullopt;
    ...
    void Start() {
        ...
        if (PublicSnapshotDatadirRefusal(&public_snapshot_marker)) {
            throw std::runtime_error("FATAL: public mainnet refuses a datadir marked as "
                                     "snapshot-imported (" +
        ...
        if (!genesis_pow.target_valid || !genesis_pow.dataset_ok || !genesis_pow.passed) {
            throw std::runtime_error("FATAL: compiled genesis failed strict VeldHash proof-of-work "
...
#endif
#if defined(VELD_ENABLE_SNAPSHOT_BOOTSTRAP)
        const bool snapshot_replay_pending = ReadSnapshotReplayRequirement_().has_value();
        const bool trusted_snapshot_replay =
            snapshot_replay_pending && snapshot_fast_start_eligible_ && !full_ibd_;
        snapshot_state_clean_.store(!trusted_snapshot_replay && !snapshot_replay_pending,
                                    std::memory_order_release);
#else
...
#if defined(VELD_ENABLE_SNAPSHOT_BOOTSTRAP)
        if (tip && tip->height > 0 && !snapshot_replay_pending && snapshot_fast_start_eligible_ &&
            snapshot_receipt_height_ > 0 && snapshot_receipt_height_ <= tip->height) {
            const auto receipt_hash = db_.GetHashAtHeight(snapshot_receipt_height_);
        ...
        if (tip && tip->height > 0 && background_validation_only_) {
            if (const auto prefix = ReadValidatedBackgroundPrefix_()) {
                const auto stored_hash = prefix->first <= tip->height
                                             ? db_.GetHashAtHeight(prefix->first)
                                             : std::nullopt;
                if (stored_hash && *stored_hash == prefix->second) {
                    trusted_local_pow_through = std::max(trusted_local_pow_through, prefix->first);
                } else {
                ...
                background_prefix_persisted_height_.store(prefix->first, std::memory_order_release);
        ...
        if (tip && tip->height > 0) {
            if (!quiet_boot_) {
                ...
                if (trusted_snapshot_replay) {
                    ...
                    std::cout << "; prior full IBD covers blocks 0-" << trusted_local_pow_through;
                    if (tip->height > trusted_local_pow_through) {
                        std::cout << ", checking " << (tip->height - trusted_local_pow_through)
                                  << " newer block(s)";
                    }
                }
                std::cout << " ...\n" << std::flush;
            ...
            chain_.AddBlockDirect(genesis, true, false, false,
                                  mining::PowAdmissionContext::Internal());
...
#endif
        checkpoint_anchor_valid_.store(AnchorSecurityImportSatisfied(), std::memory_order_release);
        startup_replay_complete_.store(true, std::memory_order_release);
        BumpValidationGeneration_();

        tcp_server_ = std::make_unique<net::NodeServer>(p2p_port_ ? p2p_port_ : config_.port,
                                                        config_.magic, chain_, mempool_);
        ...
        tcp_server_->SetAdvertisedServices(advertised_services_.load(std::memory_order_acquire));
        ...
        if constexpr (!OPTION_B_CONSENSUS_GATE_ENABLED) {
            tcp_server_->SetSolutionCallback([this](uint64_t height, const Hash256& prev_hash,
                ...
                pending_solutions_.erase(std::remove_if(pending_solutions_.begin(),
                                                        pending_solutions_.end(),
                                                        [&cutoff](const PendingSolution& s) {
                                                            return s.received_at < cutoff;
                                                        }),
                                         pending_solutions_.end());
            });
            ...
            tcp_server_->SetCOMineCallback([this](uint64_t height, const Hash256& prev_hash,
                ...
                for (auto& s : pending_solutions_)
                    if (s.prev_height == height && s.miner_script == script)
                        return false;
                PendingSolution sol;
                sol.prev_height = height;
                sol.prev_hash = prev_hash;
        ...
        tcp_server_->SetFinalityVotePrecheck([this](const std::vector<uint8_t>& wire) -> bool {
        ...
        tcp_server_->SetFinalityVoteVerifier(
            [this](const std::vector<uint8_t>& wire) -> net::NodeServer::FinalityVoteVerifyResult {
        ...
        rpc_.SetPeerCount(
            [this]() -> size_t { return tcp_server_ ? tcp_server_->ConnectedPeers() : 0; });
        rpc_.SetPeerInfo([this]() -> std::string {
            ...
            std::unordered_map<std::string, const veld::net::NodeServer::PeerTipSnapshot*>
                by_connection;
            by_connection.reserve(tip_snaps.size());
            for (const auto& s : tip_snaps)
                by_connection[s.connection_id] = &s;
        ...
        rpc_.SetBtcVeldMintProofFn(
            [this](const std::string& outpoint) { return BuildMintNullifierStatus_(outpoint); });
        rpc_.SetRtp1MintInspectionFn(
            [this](const std::string& raw) { return InspectRtp1MintJson_(raw); });
        ...
        rpc_.SetBtcVeldPegStatusFn([this]() -> BtcVeldPegStatus {
            ...
            const uint64_t candidate_height = out.tip == UINT64_MAX ? out.tip : out.tip + 1;
            const Block tip_block = chain_.TipCopy();
            const uint64_t candidate_time =
                tip_block.header.timestamp <= UINT64_MAX - TARGET_BLOCK_TIME
                    ? tip_block.header.timestamp + TARGET_BLOCK_TIME
                    : tip_block.header.timestamp;
            out.gate =
                PegGateForState_(fin_state_, candidate_height, &btc_headers_, candidate_time);
            const bool launch_profile_active =
                BTCVELD_ISSUER_ADDRESS[0] != '\0' && candidate_height >= BTCVELD_ACTIVATION_HEIGHT;
            if (!launch_profile_active)
                ...
                out.reason = "new_mint_exposure_paused_finality_stale_completion_open";
        ...
        rpc_.SetBtcVeldRedeemPageFn([this](const std::vector<std::string>& params) {
        ...
        rpc_.SetBtcHeaderInfoFn([this]() -> std::string {
            ...
            j << "{\"spv_active\":" << (active_candidate ? "true" : "false")
              ...
              << ",\"best_height\":" << bh << ",\"k_btc\":" << BTCVELD_SPV_K_BTC
              << ",\"activation_height\":" << BTCVELD_SPV_ACTIVATION_HEIGHT << "}";
            return j.str();
        });
        rpc_.SetBtcHeaderDigestFn([this]() -> Hash256 { return btc_headers_.StateDigest(); });
        ...
        if (config_.name == "Veld Regtest") {
            rpc_.SetGenerateFn([this](int n) { return GenerateBlocksForRpc(n, 0x207fffffu); });
        }
#endif
        ...
        rpc_.SetAnchorInfoFn([this]() -> std::string {
            ...
            const bool floor_exportable = anchors_.HasExportablePermanentFloor();
            const auto bootstrap_status = EffectiveAnchorSecurityStatusNoTransition_();
            std::ostringstream j;
            ...
            j << "{\"anchor_active\":" << (admission_live ? "true" : "false")
              ...
              << ",\"anchor_security_milestone\":"
              << (fin_state_.ever_promoted_anchor ? "true" : "false")
              << ",\"anchor_configured\":" << (configured ? "true" : "false")
              << ",\"finality_active\":" << (finality_active ? "true" : "false")
              << ",\"final_height\":" << fin << ",\"high_water\":" << hw
            ...
            j << ",\"bootstrap\":null";
            j << ",\"btc_observed_checkpoint_height\":" << btc_headers_.ObservedCheckpointHeight()
            ...
            for (size_t i = 0; i < pending_observations.size(); ++i) {
                ...
                const bool btc_final =
                    btc_headers_.IsFinal(pending.entry.btc_block_hash, BTCVELD_ANCHOR_BTC_CONFS);
                j << "{\"target_height\":" << pending.target_height
                  << ",\"proof_carrier_height\":" << pending.entry.carrying_veld_height
                  << ",\"proof_carrier_hash\":\"" << HashToHex(pending.entry.carrying_veld_hash)
                  << "\""
                  << ",\"btc_block_hash\":\"" << HashToHex(pending.entry.btc_block_hash) << "\""
                  << ",\"btc_final\":" << (btc_final ? "true" : "false") << "}";
            }
            j << "]";
            std::optional<btcanchor::floor_store::Record> local_floor;
            {
                std::lock_guard<std::mutex> floor_lock(anchor_floor_store_mutex_);
                local_floor = anchor_floor_store_.Current();
            }
            j << ",\"local_floor\":{"
              << "\"installed\":" << (local_floor ? "true" : "false") << ",\"repair_required\":"
              << (anchor_floor_repair_required_.load(std::memory_order_acquire) ? "true" : "false")
              << ",\"fail_stop\":"
              << (anchor_floor_security_uncertain_.load(std::memory_order_acquire) ? "true"
                                                                                   : "false");
            if (local_floor) {
                const auto& p = local_floor->checkpoint;
                j << ",\"target_height\":" << p.target_height
                  << ",\"proof_carrier_height\":" << p.entry.carrying_veld_height
                  << ",\"authorization_carrier_height\":" << p.authorization_record.carrier.height
                  << ",\"floor_digest\":\"" << HashToHex(local_floor->floor_digest) << "\"";
            } else {
            ...
            if (permanent) {
                ...
                if (!p.authorization_record.IsNull()) {
                    ...
                    j << ",\"authorization_record\":{\"epoch_id\":" << r.epoch_id
                      << ",\"finalized_height\":" << r.target.height << ",\"finalized_hash\":\""
                      << HashToHex(r.target.hash) << "\""
                      << ",\"set_root\":\"" << HashToHex(r.set_root) << "\""
                      << ",\"round\":" << r.round << ",\"phase\":" << static_cast<unsigned>(r.phase)
                      << ",\"cert_commit\":\"" << HashToHex(r.cert_commit) << "\""
                      << ",\"carrier_height\":" << r.carrier.height << ",\"carrier_hash\":\""
                      << HashToHex(r.carrier.hash) << "\""
        ...
        rpc_.SetAnchorDigestFn([this]() -> Hash256 { return anchors_.Digest(); });
        ...
        rpc_.SetRedeemBondDigestFn([this]() -> Hash256 { return bond_covenant_.Digest(); });
        ...
        rpc_.SetFinalitySnapshotFn([this](std::optional<uint64_t> requested_epoch) -> std::string {
            namespace fq = ::veld::finality::qc;
            auto transition = chain_.AcquireConsensusTransitionGuard();
            if (fin_state_.snapshots.empty())
                return "";
            auto selected = requested_epoch ? fin_state_.snapshots.find(*requested_epoch)
                                            : std::prev(fin_state_.snapshots.end());
            if (selected == fin_state_.snapshots.end())
                return "";
            ...
            if (s.entries.size() > fq::MAX_FINALITY_VALIDATOR_COUNT || !fq::SnapshotWellFormed(s))
                return "";
            ...
            for (size_t i = 0; i < s.entries.size(); ++i) {
                if (i)
                    j << ",";
                j << "{\"index\":" << i << ",\"pubkey\":\"" << s.entries[i].pubkey_hex << "\""
            ...
            if (fin_state_.record.IsNull()) {
                ...
                j << "{\"epoch\":" << r.epoch_id << ",\"height\":" << r.target.height
        ...
        rpc_.SetFinalityQcFn([this](uint8_t phase) -> std::string {
            ...
            for (auto it = fin_state_.snapshots.rbegin(); it != fin_state_.snapshots.rend(); ++it) {
                auto qc = fin_assembler_.TryAssemble(
                    it->second, fq::NETWORK_ID, GenesisHashBytes_(), (fq::Phase)phase,
                    [this, tip](const fq::DecodedQc& d) {
                        if (!fq::InVoteWindow(d.qc.target.height, tip))
                            return false;
                        try {
                            if (chain_.GetBlock(d.qc.target.height).GetHash() != d.qc.target.hash)
                                return false;
                        } catch (...) {
                            return false;
                        }
                        ...
                        return d.qc.source == fin_state_.record.target &&
                               d.qc.target.height > fin_state_.record.target.height;
                    });
                if (!qc)
                    continue;
                const std::string payload = fq::EncodeQc(qc->qc, qc->sigs);
                if (!payload.empty())
                    return fq::ToHexBytes(reinterpret_cast<const uint8_t*>(payload.data()),
                                          payload.size());
            }
            return "";
        });
        ...
        rpc_.SetModuleCursorFn(
            [this]() { return std::make_pair(last_token_height_, last_module_supply_); });
        rpc_.SetPowVerifyStatusFn([this]() -> std::string { return PowVerifyStatusJson(); });
        rpc_.SetAmm(&amm_);
        ...
        rpc_.SetMinerStatusFn([this]() -> std::string {
            ...
            std::ostringstream j;
            const bool mining_configured = mining_.load(std::memory_order_acquire);
            ...
            const bool mining_active =
                mining_work_state_.load(std::memory_order_acquire) == MiningWorkState::Hashing;
            j << std::fixed << std::setprecision(2);
            j << "{";
            j << "\"mining\":" << (mining_active ? "true" : "false") << ",";
            j << "\"mining_configured\":" << (mining_configured ? "true" : "false") << ",";
            j << "\"mining_ready\":" << (mining_ready ? "true" : "false") << ",";
        ...
        rpc_.SetTxIndexEnabledFn(
            [this]() -> bool { return txindex_enabled_ && txindex_operational_.load(); });

        rpc_.SetTxIndexLookupFn([this](const std::string& txid_hex, uint64_t tip,
            ...
            if (!txindex_operational_.load() || chain_.Height() != tip ||
                chain_.GetBlockHashAtHeight(tip) != tip_hash)
                return {};
            const auto marker = db_.GetIndexDB().Get("txi:_complete_v3");
            if (!marker || *marker != std::to_string(tip) + ":" + tip_hash)
                return {};
        ...
        auto address_history_page = [this](const std::string& address, size_t limit,
        ...
        rpc_.SetClearMiningHaltFn([this]() -> std::string {
            ...
            std::cerr << "  [layer1] OPERATOR clearmininghalt: dropped " << dropped
                      << " pending broadcast entries; mining halt cleared.\n";
            std::cerr.flush();
            return "ok";
        });
        ...
        rpc_.SetInvalidateBlockFn([this](const Hash256& h) -> std::string {
            if (!tcp_server_)
                return "tcp_server not initialised";
            tcp_server_->MarkBlockRejected(HashToHex(h));
            std::cerr << "  [recovery] invalidateblock " << HashToHex(h).substr(0, 16)
                      << "... — added to reject cache.\n";
            std::cerr.flush();
            return "ok";
        });
        ...
        rpc_.SetReconsiderBlockFn([this](const Hash256& h) -> std::string {
            ...
            std::cerr << "  [recovery] reconsiderblock " << HashToHex(h).substr(0, 16) << "... — "
        ...
        explorer_.SetPeerCountFn(
            [this]() -> size_t { return tcp_server_ ? tcp_server_->ConnectedPeers() : 0; });
        explorer_.SetPeerStatsFn(
            [this]() -> std::vector<veld::explorer::BlockExplorer::PeerStatsItem> {
        ...
        governance_.SetPersistence(
            [this](const std::string& k, const std::string& v) {
                try {
                    db_.GetIndexDB().Put(k, v);
                } catch (...) {
                }
            },
            [this](const std::string& k) {
                try {
                    db_.GetIndexDB().Delete(k);
                } catch (...) {
                }
            },
            [this](const std::string& prefix,
                ...
                try {
                    db_.GetIndexDB().Iterate(prefix, fn);
                } catch (...) {
                }
            });
        try {
        ...
        rpc_.SetTokenOpCallback([](const TokenOpData& op) {
            std::cerr << "  [TOKEN] REFUSED legacy privileged token op: " << op.action
                      << " (submit a funded, signed transaction)\n";
            std::cerr.flush();
        });
        ...
        if (txindex_enabled_) {
            try {
                ...
                const std::string tip_marker =
                    std::to_string(tip) + ":" + HashToHex(chain_.GetBlock(tip).GetHash());
                ...
                if (!complete || *complete != tip_marker) {
                    ...
                    std::cout << "  [txindex] checked backfill 0.." << tip << "\n";
                    {
                        ...
                        if (!db_.GetIndexDB().Write(invalidate))
                            throw std::runtime_error("txindex v3 invalidation write failed");
                    }
                    std::string cursor;
                    for (;;) {
                        ...
                        db_.GetIndexDB().IterateFrom(
                            "txi:", cursor, [&](const std::string& key, const std::string&) {
                                last_key = key;
                                ++scanned;
                                if (key.size() == 68 &&
                                    db::IsCanonicalHash256Text(std::string_view(key).substr(4))) {
                        ...
                        if (!stale_keys.empty()) {
                            ...
                            if (!db_.GetIndexDB().Write(erase))
                                throw std::runtime_error("txindex v3 stale-row deletion failed");
                        }
                        if (!page_full || last_key.empty())
                            break;
                    ...
                    for (uint64_t h = 0; h <= tip; ++h) {
                        ...
                        for (const auto& tx : b.transactions) {
                            if (!db_.GetIndexDB().Put("txi:" + HashToHex(tx.GetTxID()),
                                                      std::to_string(h)))
                                throw std::runtime_error(
                    ...
                    complete_batch.Put("txi:_synced", std::to_string(tip));
                    ...
                    std::cout << "  [txindex] checked backfill complete to h=" << tip << "\n";
                }
        ...
        if (p2p_started) {
            if (!quiet_boot_ && veld::DiagVerbose().load())
                std::cout << ((background_validation_only_ || snapshot_quarantine_only_)
                                  ? "  P2P validation sync is outbound-only\n"
                                  : "  P2P listening on port " + std::to_string(actual_port) +
                                        "\n");
        } else {
#ifdef VELD_PUBLIC_TESTNET
...
#endif
            if (!quiet_boot_)
                std::cout << "  P2P server: port " << actual_port
                          << " unavailable (OK in sandbox)\n";
        }
...
#endif
                if (!quiet_boot_ && veld::DiagVerbose().load())
                    std::cout << "  Explorer: http://localhost:" << CompiledPublicExplorerPort()
                              << "\n";
            } else {
#ifdef VELD_PUBLIC_TESTNET
    ...
    void Stop() {
        (void)CloseWorkAdmissionBounded_(work_admission::Refusal::NodeNotRunning);
        if (tcp_server_)
            tcp_server_->RequestWorkStop();
        {
            auto transition = chain_.AcquireConsensusTransitionGuard();
            work_admission_coordinator_.CancelAndClose(work_admission::Refusal::NodeNotRunning);
            startup_replay_complete_.store(false, std::memory_order_release);
            running_ = false;
            mining_ = false;
            BumpValidationGeneration_();
        ...
        if (prewarm_to_join.joinable())
            prewarm_to_join.join();
        ...
        if (mining_thread_.joinable())
            mining_thread_.join();
        if (pow_verify_thread_.joinable())
            pow_verify_thread_.join();
        // Join the reorg worker before chain and mempool destruction.
        NotifyReorgFixup_();
        if (reorg_fixup_thread_.joinable())
            reorg_fixup_thread_.join();
    }
...
#ifdef VELD_TEST_HOOKS
                        if (!ready && test_reorg_before_wait_)
                            test_reorg_before_wait_();
#endif
                        return ready;
                    });
                    if (reorg_fixup_q_.empty()) {
                        if (!running_.load())
                            return;
                        continue;
                    }
    ...
    void EnqueueReorgFixup(std::function<void()> fn) {
        {
            std::lock_guard<std::mutex> lk(reorg_fixup_mtx_);
            reorg_fixup_q_.push_back(std::move(fn));
        }
        reorg_fixup_cv_.notify_one();
    }

    void SetPoolKeypair(const RealKeyPair& kp) {
        pool_keypair_ = kp;
    }
    const RealKeyPair& GetPoolKeypair() const {
        return pool_keypair_;
    }
    void SetEndorsementPoolKeypair(const RealKeyPair& kp) {
        endorsement_pool_keypair_ = kp;
    }
    const RealKeyPair& GetEndorsementPoolKeypair() const {
        return endorsement_pool_keypair_;
    }
    ...
    bool IsMiningReady() const {
        return IsMiningConfigured() && ibd_complete_.load(std::memory_order_acquire) &&
               chain_fully_validated_.load(std::memory_order_acquire);
    }
    ...
    bool IsMiningActive() const {
        return mining_work_state_.load(std::memory_order_acquire) == MiningWorkState::Hashing;
    }
    const char* GetMiningWorkStateName() const {
        switch (mining_work_state_.load(std::memory_order_acquire)) {
        case MiningWorkState::Stopped:
            return "stopped";
        case MiningWorkState::Synchronizing:
            return "synchronizing";
        case MiningWorkState::WaitingForAnchor:
            return "waiting-for-anchor";
        case MiningWorkState::WorkAdmission:
            return "work-admission";
        case MiningWorkState::ClockDrift:
            return "clock-drift";
        case MiningWorkState::GenesisMismatch:
            return "genesis-mismatch";
        case MiningWorkState::WaitingForPeerTip:
            return "waiting-for-peer-tip";
        case MiningWorkState::BelowPeerFloor:
            return "below-peer-floor";
        case MiningWorkState::Propagation:
            return "propagation";
        case MiningWorkState::ReorgRebuild:
            return "reorg-rebuild";
        case MiningWorkState::Hashing:
            return "hashing";
        case MiningWorkState::Error:
            return "error";
        }
        return "error";
    }
        ...
        prewarm_thread_ = std::thread([this]() {
            try {
                ...
                while (config_.name != "Veld Regtest") {
                    ...
                    prewarm_cv_.wait_for(hold, std::chrono::milliseconds(500), [this]() {
...
#endif
                auto pow_lease =
                    mining::GlobalExpensivePowBudget().TryAcquire(mining::ExpensivePowUse::Prewarm);
                if (!pow_lease)
                    return;
                auto tip = chain_.GetBlock(h);
                Hash256 seed = mining::ComputeEpochSeed(tip.header.prev_block_hash, tip.height,
                                                        tip.header.bits);
                auto t0 = std::chrono::steady_clock::now();
                mining::DatasetHandle warm = mining::GlobalDataset().get_for_seed(seed);
                auto dt = std::chrono::duration_cast<std::chrono::milliseconds>(
                              std::chrono::steady_clock::now() - t0)
                              .count();
                if (warm.get()) {
                    std::cout << "  [prewarm] VeldHash dataset ready (" << dt
                              << " ms) — first-click Mine will be instant\n";
                    std::cout.flush();
                } else {
    ...
    bool BindGenerationIdentity(const RealKeyPair& miner_keypair, std::string* error = nullptr) {
    ...
    bool BindAddressGenerationIdentity(const std::string& address, std::string* error = nullptr) {
#ifdef VELD_FLEET_NO_MINE
        (void)address;
        if (error)
            *error = "VELD_FLEET_NO_MINE: address generation disabled";
        return false;
#else
        std::lock_guard<std::mutex> lock(generation_mutex_);
        if (mining_.load()) {
            if (error)
                *error = "stop mining before changing the payout address";
            return false;
        }
    ...
    bool StartAddressMining(const std::string& address, uint32_t bits_override = 0,
        ...
        (void)bits_override;
        if (error)
            *error = "VELD_FLEET_NO_MINE: address mining disabled";
        return false;
#else
        std::lock_guard<std::mutex> lock(generation_mutex_);
#ifdef VELD_PUBLIC_TESTNET
        if (PublicTestnetRuntimeStopRequired()) {
            if (error)
                *error = "PUBLIC TESTNET runtime lease is closed";
            return false;
        }
#endif
        if (mining_.load()) {
        ...
        if (mining_thread_.joinable())
            mining_thread_.join();
        if (!BindAddressGenerationIdentityLocked_(address, error))
            return false;
    ...
    void StartMining(const RealKeyPair& miner_keypair, uint32_t bits_override = 0) {
        ...
        if (!BindGenerationIdentityLocked_(miner_keypair, &identity_error)) {
            std::cerr << "  [mining] StartMining() refused: " << identity_error << "\n";
            std::cerr.flush();
            return;
        }
        mining_bits_ = bits_override;
    ...
    unsigned GetMiningThreads() const {
        return mining_threads_;
    }
    ...
    RpcServer::GenerateResult GenerateBlocksForRpc(int count, uint32_t bits_override = 0x207fffff) {
        RpcServer::GenerateResult generated;
        generated.height = chain_.Height();
#ifdef VELD_FLEET_NO_MINE
        (void)count;
        (void)bits_override;
        generated.error = "VELD_FLEET_NO_MINE: synchronous generation disabled";
        return generated;
#else
        std::lock_guard<std::mutex> lock(generation_mutex_);
        if (!generation_identity_bound_) {
            generated.error = "synchronous generation identity is not bound";
            return generated;
        }
        ...
        try {
            ...
            generated.height = chain_.Height();
            generated.generated =
                generated.height >= start_height ? generated.height - start_height : 0;
        ...
        for (size_t i = 0; i < results.size(); ++i) {
            ...
            if (result.success) {
                ...
                first_failure = result.error.empty() ? "MineBlocks returned an unspecified failure"
                                                     : result.error;
            }
        }
        generated.height = chain_.Height();

        if (results.size() != static_cast<size_t>(count) && first_failure.empty()) {
            first_failure = "MineBlocks returned an incomplete result vector";
        }
        if (generated.generated != static_cast<uint64_t>(count) && first_failure.empty()) {
            first_failure = "MineBlocks did not complete the requested count";
        }
        if (generated.height != start_height + generated.generated && first_failure.empty()) {
            first_failure = "canonical height change does not match successful generation results";
        }
    ...
    std::vector<MineBlockResult> MineBlocks(const RealKeyPair& keypair, int count,
        ...
        for (int i = 0; i < count; ++i) {
            ...
            const auto issued =
                work_subject ? EvaluateWorkAdmission(work_admission::Path::SynchronousGeneration,
                                                     *work_subject, std::nullopt, false)
                             : work_admission::Decision{false, work_admission::Refusal::TipUnknown,
                                                        std::nullopt};
            if (mining_.load(std::memory_order_acquire) || !issued.allowed || !issued.binding) {
                MineBlockResult denied{};
                denied.success = false;
                denied.error = "Synchronous mining refused: IBD/work admission is closed";
            ...
            auto finality_metadata = PendingFinalityCoinbaseMetadata_(settlement_probe.height);
            ...
            synchronous_watchdog = std::thread([&, issued_binding = *issued.binding]() {
                while (!synchronous_stop.load(std::memory_order_acquire)) {
                    bool safe = false;
                    try {
                        safe = running_.load(std::memory_order_acquire) &&
                               !mining_.load(std::memory_order_acquire) &&
                               MiningWorkStillSafe_(work_admission::Path::SynchronousGeneration,
                                                    issued_binding, true);
                    } catch (...) {
                        safe = false;
                    }
                    if (!safe) {
                        synchronous_safety_cancelled.store(true, std::memory_order_release);
                        synchronous_stop.store(true, std::memory_order_release);
                        break;
                    }
                    std::unique_lock<std::mutex> hold(synchronous_watchdog_mutex);
                    synchronous_watchdog_cv.wait_for(hold, std::chrono::milliseconds(500), [&]() {
                        return synchronous_stop.load(std::memory_order_acquire);
                    });
                }
            });
            ...
            try {
                r = MineOnly(
                    chain_, mempool_, keypair, bits_override, &synchronous_stop,
                    AddressToScript(POOL_ADDRESS), 1, nullptr, {}, nullptr, nullptr,
                    mandatory_settlements,
                    [this](const Block& candidate) { return PreflightMiningCandidate_(candidate); },
                    finality_metadata);
            } catch (...) {
            ...
            stop_synchronous_watchdog();
            if (synchronous_safety_cancelled.load(std::memory_order_acquire)) {
                r.success = false;
                r.error = "Synchronous mining cancelled: IBD/work admission closed";
            }
            if (r.success) {
                bool final_safe = !mining_.load(std::memory_order_acquire) &&
                                  MiningWorkStillSafe_(work_admission::Path::SynchronousGeneration,
                                                       *issued.binding, true);
                if (final_safe) {
                    final_safe = r.new_height > 0 && chain_.Height() == r.new_height - 1 &&
                                 chain_.GetBlockHashAtHeight(r.new_height - 1) ==
                                     HashToHex(r.block.header.prev_block_hash);
                }
                if (!final_safe) {
                    r.success = false;
                    r.error = "Synchronous mining discarded: work admission "
                              "closed before commit";
                }
            }
            if (r.success) {
                auto pow_context = mining::PowAdmissionContext::SynchronousGenerationWork(
                    work_admission::EncodeBinding(*issued.binding));
                const auto admission =
                    chain_.AddBlockDirect(r.block, false, false, false, pow_context);
                if (admission.IsDeferred()) {
                    r.success = false;
                    r.error = "Block commit deferred: local work admission unavailable";
                    ...
                    if (!pow_context.local_work_handoff->IsLive()) {
                        r.success = false;
                        r.error = "Block committed but local work publication closed";
                    } else {
...
#ifdef VELD_PUBLIC_TESTNET
        if (PublicTestnetRuntimeStopRequired()) {
            return JsonBuilder::RpcError(call.id, -32603,
    ...
    uint64_t EstimateFee(size_t tx_size_bytes = 250) const {
        ...
        uint64_t rate;
        if (mempool_size < 100)
            rate = Mempool::MIN_FEE_RATE;
        else if (mempool_size < 500)
            rate = Mempool::MIN_FEE_RATE * 2;
        else if (mempool_size < 2000)
            rate = Mempool::MIN_FEE_RATE * 5;
        else
            rate = Mempool::MIN_FEE_RATE * 10;
        return (rate * tx_size_bytes) / 1000;
    }

    SendResult Send(const RealKeyPair& sender, const std::string& recipient_address,
                    uint64_t amount_units, uint64_t fee_units = 0) {
        SendResult result{false, "", "", 0};

        if (fee_units == 0)
            fee_units = EstimateFee();
        ...
        if (!coins.sufficient) {
            result.error = "Insufficient funds: need " + std::to_string(total_needed) +
                           " units, have " +
        ...
        if (add_result != Mempool::AddResult::ACCEPTED) {
            ...
            case Mempool::AddResult::DUPLICATE:
                result.error = "Duplicate tx";
                break;
            case Mempool::AddResult::FEE_TOO_LOW:
                result.error = "Fee too low";
                break;
            case Mempool::AddResult::DOUBLE_SPEND:
                result.error = "Double spend";
                break;
            case Mempool::AddResult::FULL:
                result.error = "Mempool full";
                break;
            default:
                result.error = "Invalid tx";
                break;
            }
            return result;
        }
    ...
    Blockchain& GetChainMut() {
        return chain_;
    }
    Mempool& GetMempoolMut() {
        return mempool_;
    }
    bool MiningWorkStillSafeFromView_(const net::NodeServer::PeerHeightView& heights,
                                      uint64_t local_height, bool chain_empty) const {
        if (config_.name == "Veld Regtest")
            return true;
        return IsInitialDownloadAtTip(
            false, local_height, chain_empty, heights.distinct_version_ips, heights.verified_height,
            heights.distinct_outbound_sync_ips, heights.outbound_sync_height);
    }

    static std::optional<work_admission::Path>
    WorkPathForLocalKind_(mining::LocalWorkKind kind) noexcept {
        switch (kind) {
    ...
    WorkCoordinatorConfiguration_(uint64_t canonical_tip_height,
        ...
        const uint64_t close_epoch_before = work_admission_coordinator_.ObserveCloseEpoch();
        try {
#ifdef VELD_TEST_HOOKS
            canonical_tip_known = canonical_tip_known &&
                                  !test_work_force_tip_unknown_.load(std::memory_order_acquire);
#endif
            auto& prerequisites = configuration.prerequisites;
            prerequisites.role_permitted = true; // path-specific below
            prerequisites.node_running = running_.load(std::memory_order_acquire);
            ...
            prerequisites.sync_complete = regtest || ibd_complete_.load(std::memory_order_acquire);
            prerequisites.snapshot_state_clean =
                snapshot_state_clean_.load(std::memory_order_acquire);
            prerequisites.durable_state_proven =
                !FailStopRequired() &&
                !reorg_publication_uncertain_.load(std::memory_order_acquire);
            ...
            prerequisites.runtime_open = canonical_tip_known && canonical_tip_height != UINT64_MAX;
#ifdef VELD_PUBLIC_TESTNET
...
#endif
#ifdef VELD_TEST_HOOKS
            prerequisites.runtime_open =
                prerequisites.runtime_open &&
                test_work_local_runtime_open_.load(std::memory_order_acquire);
#endif
            if (regtest) {
                ...
                const bool freshness_window =
                    ...
                    heights.freshness_valid_for_ms >
                        static_cast<uint64_t>(PEER_VIEW_EXPIRY_SAFETY_MARGIN.count());
                prerequisites.peer_view_safe =
                    heights.work_sequencer_wired && heights.work_view_stable && freshness_window &&
                    MiningWorkStillSafeFromView_(heights, canonical_tip_height, false);
            }
            prerequisites.validation_generation =
                validation_generation_.load(std::memory_order_acquire);
            prerequisites.network_magic = config_.magic;
            prerequisites.genesis_hash = GenesisHashBytes_();
            prerequisites.profile_digest = Hash256d(std::string(DEPLOYMENT_PROFILE_ID));
...
#endif
            configuration
                .permitted_paths[static_cast<size_t>(work_admission::Path::InternalMining)] =
                block_work;
            configuration
                .permitted_paths[static_cast<size_t>(work_admission::Path::GetBlockTemplate)] =
                block_work;
            configuration.permitted_paths[static_cast<size_t>(work_admission::Path::SubmitBlock)] =
                block_work;
            configuration
                .permitted_paths[static_cast<size_t>(work_admission::Path::SynchronousGeneration)] =
                block_work;
            configuration
                .permitted_paths[static_cast<size_t>(work_admission::Path::ValidatorEndorsement)] =
                any_local_work;
            configuration.permitted_paths[static_cast<size_t>(work_admission::Path::FinalityVote)] =
                any_local_work;
        } catch (...) {
        ...
        const uint64_t close_epoch_after = work_admission_coordinator_.ObserveCloseEpoch();
    ...
    BoundPeerWorkLifetime_(std::chrono::milliseconds requested) const noexcept {
        if (requested <= std::chrono::milliseconds::zero())
            return std::nullopt;
        uint64_t bounded_ms = static_cast<uint64_t>(requested.count());
#ifdef VELD_PUBLIC_TESTNET
        if (!public_testnet_limits_ || !PublicTestnetRestartAuthorityFreshNow())
            return std::nullopt;
        const int64_t wall_now = public_testnet::CurrentUnixTime();
        const uint64_t wall_margin = static_cast<uint64_t>(PEER_VIEW_EXPIRY_SAFETY_MARGIN.count());
        const auto wall_bounded = BoundWallClockWorkLifetimeMs(
            wall_now, public_testnet_limits_->not_after_unix, bounded_ms, wall_margin);
        if (!wall_bounded)
            return std::nullopt;
        ...
        try {
            const auto heights = tcp_server_->GetPeerHeightView();
            const uint64_t margin = static_cast<uint64_t>(PEER_VIEW_EXPIRY_SAFETY_MARGIN.count());
            const auto peer_bounded =
                net::NodeServer::BoundPeerWorkLifetimeMs(heights, bounded_ms, margin);
            if (!peer_bounded)
                return std::nullopt;
            bounded_ms = *peer_bounded;
            if (bounded_ms == 0 ||
                bounded_ms > static_cast<uint64_t>(std::chrono::milliseconds::max().count()))
                return std::nullopt;
            return std::chrono::milliseconds(bounded_ms);
        } catch (...) {
            return std::nullopt;
    ...
    work_admission::AdmissionCoordinator::OpenResult OpenWorkCoordinator_(
        const work_admission::AdmissionCoordinator::Configuration& configuration) noexcept {
    ...
    IssueBlockTemplateAuthorizationUnderTransition_(const work_admission::Subject& subject,
                                                    const Hash256& template_identity) noexcept {
        RpcServer::BlockTemplateAuthorizationResult result;
        if (HashIsZero(template_identity)) {
            result.decision = {false, work_admission::Refusal::SubjectNotCanonical, std::nullopt};
            return result;
        }
        try {
            const auto gbt_decision = EvaluateWorkAdmission(work_admission::Path::GetBlockTemplate,
                                                            subject, std::nullopt, false);
            result.decision = EvaluateWorkAdmission(work_admission::Path::SubmitBlock, subject,
                                                    std::nullopt, false);
            ...
            if (!chain_.TryTip(tip) || subject.height != tip.height + 1 ||
                subject.parent_height != tip.height || subject.parent_hash != tip.GetHash()) {
                result.decision = {false, work_admission::Refusal::SubjectNotCanonical,
                                   std::nullopt};
                return result;
            }
            const auto configuration = WorkCoordinatorConfiguration_(tip.height, true);
            const auto opened = OpenWorkCoordinator_(configuration);
            if (!opened.opened) {
                result.decision = {false, opened.refusal, std::nullopt};
                return result;
            }
            const auto lifetime = BoundPeerWorkLifetime_(std::chrono::milliseconds(10000));
            if (!lifetime) {
                result.decision = {false, work_admission::Refusal::PeerViewUnsafe, std::nullopt};
                return result;
            }
            const auto coordinator = work_admission_coordinator_.GetSnapshot();
            if (coordinator.phase != work_admission::AdmissionCoordinator::Phase::Open ||
                coordinator.configuration_generation == 0) {
                result.decision = {false, work_admission::Refusal::RuntimeClosed, std::nullopt};
                return result;
            }
            ...
            auto issued = block_template_authorizations_.Issue(
                published_binding, coordinator.configuration_generation, *lifetime);
            ...
            result.ttl_ms = static_cast<uint64_t>(issued.authorization->ttl.count());
            return result;
        } catch (...) {
            result.decision = {false, work_admission::Refusal::Unwired, std::nullopt};
            result.token.clear();
            result.ttl_ms = 0;
            return result;
        }
    ...
    ConsumeBlockTemplateAuthorizationUnderTransition_(
        const std::string& token, const work_admission::Binding& binding) noexcept {
        try {
    ...
    RpcServer::RemoteWorkGrantResult
    IssueRemoteWorkGrantUnderTransition_(work_admission::Path path,
                                         const work_admission::Subject& subject) noexcept {
        RpcServer::RemoteWorkGrantResult result;
        result.decision = EvaluateWorkAdmission(path, subject, std::nullopt, false);
        if (!result.decision.allowed || !result.decision.binding)
            return result;
        try {
            Block tip;
            if (!chain_.TryTip(tip)) {
                result.decision = {false, work_admission::Refusal::TipUnknown, std::nullopt};
                return result;
            }
            const auto configuration = WorkCoordinatorConfiguration_(tip.height, true);
            const auto opened = OpenWorkCoordinator_(configuration);
            ...
            const auto lease_ttl = BoundPeerWorkLifetime_(std::chrono::milliseconds(10000));
            if (!lease_ttl) {
                result.decision = {false, work_admission::Refusal::PeerViewUnsafe, std::nullopt};
                return result;
            }
            auto issued = work_admission_coordinator_.IssueRemoteSigningLease(
                path, subject, std::nullopt, false, *lease_ttl);
            result.decision = issued.decision;
            if (!issued || !issued.grant)
                return result;
            result.token =
                work_admission::AdmissionCoordinator::EncodeRemoteToken(issued.grant->token);
            result.ttl_ms = static_cast<uint64_t>(issued.grant->ttl.count());
            return result;
        } catch (...) {
            result.decision = {false, work_admission::Refusal::Unwired, std::nullopt};
            result.token.clear();
    ...
    ConsumeRemoteWorkLeaseUnderTransition_(work_admission::Path path,
        ...
        const auto token = work_admission::AdmissionCoordinator::DecodeRemoteToken(token_text);
        if (!prior || !token || work_admission::EncodeBinding(*prior) != binding_text ||
            work_admission::AdmissionCoordinator::EncodeRemoteToken(*token) != token_text)
            return {};
        try {
            const auto snapshot = work_admission_coordinator_.GetSnapshot();
            const bool acquired_before_close =
                snapshot.phase == work_admission::AdmissionCoordinator::Phase::Closing;
            if (!acquired_before_close) {
                const auto decision = EvaluateWorkAdmission(path, subject, prior, true);
            ...
            auto consumed =
                work_admission_coordinator_.ConsumeRemoteSigningLease(*token, path, subject, prior);
            if (!consumed || !consumed.lease)
                return {};
            return std::make_shared<work_admission::AdmissionCoordinator::Lease>(
    ...
    void PruneActiveRemoteSigningLeases_() noexcept {
        std::vector<std::shared_ptr<work_admission::AdmissionCoordinator::Lease>> retired;
        const auto now = std::chrono::steady_clock::now();
        {
            std::lock_guard<std::mutex> lock(active_remote_signing_mu_);
            for (auto it = active_remote_signing_leases_.begin();
                 it != active_remote_signing_leases_.end();) {
                if (!it->second.lease || it->second.lease->deadline() <= now) {
    ...
    BeginRemoteSigningUnderTransition_(work_admission::Path path, const std::string& binding_text,
                                       const std::string& token_text) noexcept {
        RpcServer::RemoteSigningActivationResult out;
        try {
            const auto binding = work_admission::DecodeBinding(binding_text);
            if (!binding || work_admission::EncodeBinding(*binding) != binding_text) {
                out.reason = "invalid_binding";
                return out;
            }
            auto lease = ConsumeRemoteWorkLeaseUnderTransition_(path, binding->subject,
            ...
            const auto now = std::chrono::steady_clock::now();
            if (lease->deadline() <=
                now + REMOTE_SIGNING_SAFETY_MARGIN + std::chrono::milliseconds(1000)) {
                out.deferred = true;
                out.reason = "insufficient_signing_lease_time";
                return out;
            }
            const auto operation_deadline = lease->deadline() - REMOTE_SIGNING_SAFETY_MARGIN;
            ...
            PruneActiveRemoteSigningLeases_();
            {
                ...
                active_remote_signing_leases_.emplace(token_text, std::move(active));
            }
            const auto remaining =
                std::chrono::duration_cast<std::chrono::milliseconds>(operation_deadline - now);
            out.started = true;
    ...
    std::optional<ActiveRemoteSigningLease> TakeActiveRemoteSigningLeaseUnderTransition_(
        work_admission::Path path, const work_admission::Subject& subject,
        const std::string& binding_text, const std::string& token_text) noexcept {
        PruneActiveRemoteSigningLeases_();
        std::optional<ActiveRemoteSigningLease> active;
        {
            std::lock_guard<std::mutex> lock(active_remote_signing_mu_);
            const auto it = active_remote_signing_leases_.find(token_text);
            if (it == active_remote_signing_leases_.end() || it->second.path != path ||
                it->second.binding.subject != subject ||
                std::chrono::steady_clock::now() >= it->second.operation_deadline ||
                work_admission::EncodeBinding(it->second.binding) != binding_text)
                return {};
            active = std::move(it->second);
            active_remote_signing_leases_.erase(it);
        }
        if (!active || !active->OperationLive() || active->lease->binding().subject != subject ||
            work_admission::EncodeBinding(active->lease->binding()) != binding_text)
            return {};
        const auto snapshot = work_admission_coordinator_.GetSnapshot();
        if (snapshot.phase != work_admission::AdmissionCoordinator::Phase::Closing) {
            const auto decision = EvaluateWorkAdmission(path, subject, active->binding, true);
            if (!decision.allowed || !decision.binding || *decision.binding != active->binding)
                return {};
        }
        return active;
    }
    ...
    bool CancelRemoteSigningUnderTransition_(const std::string& token_text) noexcept {
        const auto token = work_admission::AdmissionCoordinator::DecodeRemoteToken(token_text);
        if (!token || work_admission::AdmissionCoordinator::EncodeRemoteToken(*token) != token_text)
            return false;
        PruneActiveRemoteSigningLeases_();
    ...
    AcquireLocalValidatorEndorsementPermit(const work_admission::Subject& subject) noexcept {
        try {
            auto transition = chain_.AcquireConsensusTransitionGuard();
            const auto decision = EvaluateWorkAdmission(work_admission::Path::ValidatorEndorsement,
                                                        subject, std::nullopt, false);
            if (!decision.allowed || !decision.binding)
                return std::nullopt;
            Block tip;
            if (!chain_.TryTip(tip))
                return std::nullopt;
            const auto configuration =
                WorkCoordinatorConfiguration_(tip.height, subject.parent_height == tip.height &&
                                                              subject.parent_hash == tip.GetHash());
            const auto opened = OpenWorkCoordinator_(configuration);
            if (!opened.opened)
                return std::nullopt;
            const auto lease_ttl = BoundPeerWorkLifetime_(std::chrono::milliseconds(5000));
            if (!lease_ttl)
                return std::nullopt;
            auto attempt = work_admission_coordinator_.AcquireLocal(
                work_admission::Path::ValidatorEndorsement, subject, decision.binding, true,
                *lease_ttl);
            if (!attempt || !attempt.lease)
                return std::nullopt;
            auto lease = std::make_shared<work_admission::AdmissionCoordinator::Lease>(
            ...
            const auto now = std::chrono::steady_clock::now();
            if (lease->deadline() <=
                now + LOCAL_SIGNING_SAFETY_MARGIN + std::chrono::milliseconds(250))
                return std::nullopt;
            const auto operation_deadline = lease->deadline() - LOCAL_SIGNING_SAFETY_MARGIN;
            return LocalSigningPermit(std::move(lease), operation_deadline);
        } catch (...) {
            return std::nullopt;
        }
    }

    RpcServer::AuthorizedWorkTxResult
    SubmitLocalValidatorEndorsement(const Transaction& tx, uint64_t fee,
        ...
        try {
            ...
            if (!rpc_detail::ExtractValidatorEndorsementTarget(tx, target) || !target) {
            ...
            const auto coordinator_snapshot = work_admission_coordinator_.GetSnapshot();
            if (coordinator_snapshot.phase !=
                work_admission::AdmissionCoordinator::Phase::Closing) {
                const auto decision =
                    EvaluateWorkAdmission(work_admission::Path::ValidatorEndorsement, subject,
            ...
            authorization.claim = [lease]() noexcept { return lease->ClaimForSink(); };
            authorization.live = operation_live;
            const auto add = mempool_.AddAuthorizedWork_(tx, fee, static_cast<uint32_t>(tip.height),
                                                         chain_, std::move(authorization));
            if (add != Mempool::AddResult::ACCEPTED) {
                out.deferred = add == Mempool::AddResult::DEFERRED_LOCAL_WORK;
...
#ifdef VELD_TEST_HOOKS
            test_work_tx_gossip_calls_.fetch_add(1, std::memory_order_acq_rel);
#endif
            tcp_server_->BroadcastTransaction(tx);
            out.accepted = true;
            return out;
    ...
    RpcServer::AuthorizedWorkTxResult SubmitAuthorizedValidatorEndorsement_(
        const Transaction& tx, uint64_t fee, const std::string& binding_text,
        const std::string& token_text, bool rebroadcast) noexcept {
        RpcServer::AuthorizedWorkTxResult out;
        try {
            ...
            auto active = TakeActiveRemoteSigningLeaseUnderTransition_(
                work_admission::Path::ValidatorEndorsement, subject, binding_text, token_text);
            ...
            if (rebroadcast) {
                const auto retained = mempool_.GetTransaction(HashToHex(tx.GetTxID()));
                if (!retained || retained->Serialize() != tx.Serialize()) {
                    out.reason = "endorsement_not_in_mempool";
                    return out;
                }
                if (!operation_live() || !lease->ClaimForSink() || !operation_live()) {
                    out.deferred = true;
                ...
                const auto add = mempool_.AddAuthorizedWork_(
                    tx, fee, static_cast<uint32_t>(tip.height), chain_, std::move(authorization));
                if (add != Mempool::AddResult::ACCEPTED) {
                    out.deferred = add == Mempool::AddResult::DEFERRED_LOCAL_WORK;
                    out.reason = Mempool::ResultToString(add);
                    return out;
    ...
    std::optional<work_admission::Subject> CurrentBlockProductionSubject_() const {
        Block tip;
    ...
    work_admission::Decision
    EvaluateWorkAdmission(work_admission::Path path, const work_admission::Subject& subject,
        ...
        try {
            const bool block_path = path == work_admission::Path::InternalMining ||
                                    path == work_admission::Path::GetBlockTemplate ||
                                    path == work_admission::Path::SubmitBlock ||
                                    path == work_admission::Path::SynchronousGeneration;
            if ((block_path && subject.purpose != work_admission::Purpose::BlockProduction) ||
            ...
            const bool subject_tip_matches = have_tip && subject.parent_height == tip.height &&
                                             subject.parent_hash == tip.GetHash();
            ...
            auto configuration = WorkCoordinatorConfiguration_(have_tip ? tip.height : UINT64_MAX,
                                                               subject_tip_matches);
            auto prerequisites = configuration.prerequisites;
            const size_t path_index = static_cast<size_t>(path);
            prerequisites.role_permitted = path_index < configuration.permitted_paths.size() &&
                                           configuration.permitted_paths[path_index];
            return work_admission::Evaluate(subject, prerequisites, prior, require_prior);
        } catch (...) {
            return deny(work_admission::Refusal::Unwired);
        }
    }

    bool MiningWorkStillSafe_(work_admission::Path path = work_admission::Path::InternalMining,
                              const std::optional<work_admission::Binding>& prior = std::nullopt,
                              bool require_prior = false) const {
        const auto subject = CurrentBlockProductionSubject_();
        return subject && EvaluateWorkAdmission(path, *subject, prior, require_prior).allowed;
    }

    bool CloseWorkAdmissionBounded_(work_admission::Refusal refusal) noexcept {
        auto closing = work_admission_coordinator_.BeginClose(refusal);
        if (closing.fully_closed)
            return true;
        ...
        while (std::chrono::steady_clock::now() < hard_stop) {
            const auto snapshot = work_admission_coordinator_.GetSnapshot();
            if (snapshot.phase == work_admission::AdmissionCoordinator::Phase::Closed)
                return true;
    ...
    void SetChainFullyValidatedLatch_(bool value) noexcept {
        if (chain_fully_validated_.load(std::memory_order_acquire) == value)
            return;
        (void)CloseWorkAdmissionBounded_(work_admission::Refusal::IndependentValidationIncomplete);
        try {
            auto transition = chain_.AcquireConsensusTransitionGuard();
            work_admission_coordinator_.CancelAndClose(
                work_admission::Refusal::IndependentValidationIncomplete);
            const bool previous = chain_fully_validated_.exchange(value, std::memory_order_acq_rel);
            if (previous != value)
                BumpValidationGeneration_();
        } catch (...) {
            work_admission_coordinator_.CancelAndClose(work_admission::Refusal::Unwired);
    ...
    void SetIbdCompleteLatch_(bool value) noexcept {
        if (ibd_complete_.load(std::memory_order_acquire) == value)
            return;
        (void)CloseWorkAdmissionBounded_(work_admission::Refusal::SyncIncomplete);
        try {
            auto transition = chain_.AcquireConsensusTransitionGuard();
            work_admission_coordinator_.CancelAndClose(work_admission::Refusal::SyncIncomplete);
            const bool previous = ibd_complete_.exchange(value, std::memory_order_acq_rel);
            if (previous != value)
                BumpValidationGeneration_();
        } catch (...) {
            work_admission_coordinator_.CancelAndClose(work_admission::Refusal::Unwired);
    ...
    void SetIBDComplete(bool v) {
        ...
        if (v && config_.name != "Veld Regtest") {
            const auto heights =
                tcp_server_ ? tcp_server_->GetPeerHeightView() : net::NodeServer::PeerHeightView{};
            const uint64_t local_height = chain_.Height();
            if (!tcp_server_ ||
                !MiningWorkStillSafeFromView_(heights, local_height, chain_.IsEmpty())) {
                SetIbdCompleteLatch_(false);
                std::cerr << "  [IBD] completion REFUSED: current peer-height "
                             "work admission is unsafe; verified peer height="
                          << heights.verified_height
                          << ", diagnostic VERSION fetch hint=" << heights.version_fetch_hint_height
                          << ", outbound sync floor=" << heights.outbound_sync_height
                          << ", local height=" << local_height
                          << " (ready distinct IPs=" << heights.distinct_version_ips
                          << ", outbound sync IPs=" << heights.distinct_outbound_sync_ips
        ...
        if (v && SnapshotRecoveryPending()) {
            ...
            try {
                RestoreRecoveredFinalityEvidence_();
            } catch (const std::exception& e) {
                SetIbdCompleteLatch_(false);
    ...
    void SetFullIbd(bool v) {
        full_ibd_ = v;
    }
    bool IsFullIbd() const {
        return full_ibd_;
    }
    ...
    bool PublicSnapshotDatadirRefusal(std::string* marker = nullptr) const {
        ...
        const std::array<std::pair<std::string, const char*>, 4> candidates{{
            {SnapshotReplayRequirementPath_(), ".snapshot-consensus-replay-required"},
            {IndependentValidationRequirementPath_(), ".background-chainstate-required"},
            {SnapshotRecoveryRequestPath_(), ".snapshot-recovery-requested"},
            {SnapshotRevocationMarkerPath_(), ".snapshot-fast-start-revoked"},
        }};
    ...
    void PrepareSnapshotCandidateReplay(uint64_t height, const std::string& tip_hash) {
        if (running_.load(std::memory_order_acquire) || tcp_server_ || height == 0 ||
            !SnapshotManifestIsHex64(tip_hash) ||
            !WriteSnapshotReplayRequirementAt_(SnapshotReplayRequirementPath_(), height,
                                               tip_hash)) {
            throw std::runtime_error("cannot durably prepare isolated snapshot candidate replay");
        }
        SetSnapshotFastStartEligible(true, height, tip_hash);
    }

    void SetSnapshotFastStartEligible(bool v, uint64_t validated_height = 0,
                                      const std::string& validated_tip = {}) {
        if (running_.load(std::memory_order_acquire))
            throw std::logic_error("snapshot fast-start eligibility must be set before Start");
        if (v && SnapshotRecoveryPending())
    ...
    Hash256 ConsensusStateDigestLocked_() const {
        ...
        if constexpr (btcveld::reserve::TRANSITION_V1_REQUIRED)
            return state_digest::ComposeV8(
                chain_.Height(), tip_hash, chain_.UtxoDigest(), validators_.ValidatorsDigest(),
                staking_.StakingDigest(), validators_.BondYieldEscrowDigest(),
                chain_.NmsTallyDigest(), onchain_tokens_.Digest(), chain_.SupplyDigest(),
                governance_.GovernanceDigest(), chain_.NmsExtendedDigest(),
                btc_headers_.StateDigest(), anchors_.Digest(), amm_.Digest(), fin_state_.Digest(),
                bond_covenant_.Digest());
        return state_digest::ComposeV7(
            chain_.Height(), tip_hash, chain_.UtxoDigest(), validators_.ValidatorsDigest(),
            staking_.StakingDigest(), validators_.BondYieldEscrowDigest(), chain_.NmsTallyDigest(),
            onchain_tokens_.Digest(), chain_.SupplyDigest(), governance_.GovernanceDigest(),
            chain_.NmsExtendedDigest(), btc_headers_.StateDigest(), anchors_.Digest(),
            amm_.Digest(), fin_state_.Digest(), bond_covenant_.Digest());
    }
  ...
  public:
    ...
    void SetIndependentValidationProgress(uint64_t height) {
        independent_validation_height_.store(height, std::memory_order_release);
    }
    bool FinalizeIndependentBackgroundValidation(const BackgroundValidationObservation& observation,
                                                 std::string* error = nullptr) {
        ...
        if (!base) {
            if (error)
                *error = "no independent validation requirement is active";
            return false;
        }
        const auto local_base_hash = db_.GetHashAtHeight(base->height);
        if (!local_base_hash || *local_base_hash != HashToHex(base->tip_hash)) {
            if (error)
                *error = "snapshot base is no longer on the local canonical chain";
            return false;
        }
        if (!observation.reached || observation.passed_target ||
            observation.height != base->height || observation.tip_hash != base->tip_hash ||
            observation.state_digest != base->state_digest) {
            if (error)
                *error = "background chainstate does not match the snapshot base";
            return false;
        }
        try {
            DurableRemoveControlFile_(IndependentValidationRequirementPath_());
        ...
        snapshot_background_verification_failed_.store(false, std::memory_order_release);
        independent_validation_height_.store(0, std::memory_order_release);
        SetChainFullyValidatedLatch_(true);
        snapshot_state_clean_.store(true, std::memory_order_release);
        BumpValidationGeneration_();
        return true;
    ...
    bool SnapshotBackgroundVerificationFailed() const {
        return snapshot_background_verification_failed_.load(std::memory_order_acquire);
    }
#endif
    void SyncTCPIBDFlag() {
        if (tcp_server_)
            tcp_server_->SetIBDComplete(ibd_complete_.load());
    }
    net::NodeServer::PeerHeightView GetPeerHeightView() const {
        if (tcp_server_)
            return tcp_server_->GetPeerHeightView();
        return {};
    }
    ...
    uint64_t GetReceivedVersionCount() const {
        if (tcp_server_)
            return tcp_server_->GetReceivedVersionCount();
        return 0;
    }
    bool IsIBDComplete() const {
        return ibd_complete_.load();
    }
    ...
    void TryBackfillFlushes() {
        ...
        auto try_flush = [&](const char* label, std::atomic<uint64_t>& last_h, uint64_t window) {
            if (h < last_h.load() + window)
                return;
            try {
                ...
                if (res == Mempool::AddResult::ACCEPTED) {
                    if (tcp_server_)
                        tcp_server_->BroadcastTransaction(tx);
                    last_h.store(h);
                    std::cout << "  [IBD-backfill] " << label << " broadcast at height " << h
                              << " — " << tx.inputs.size() << " inputs, " << tx.outputs.size()
                              << " payouts\n";
                    std::cout.flush();
                }
            } catch (...) {
    ...
    OnChainTokenLedger& GetTokens() {
        return onchain_tokens_;
    }
    AmmLedger& GetAmm() {
        return amm_;
    }
    ...
    void SetAdvertisedServices(uint64_t services) {
        ...
        if (tcp_server_)
            tcp_server_->SetAdvertisedServices(services);
    }
    ...
    void AddTrustedIP(const std::string& ip) {
        if (!tcp_server_ || ip.empty())
            return;
        tcp_server_->AddTrustedIP(ip);
    }

    bool AddFleetAnchorIp(const std::string& ip) {
        if (!tcp_server_ || !net::NodeServer::IsCanonicalIPv4Literal(ip))
            return false;
        ...
        if (!tcp_server_->AddFleetAnchorIp(ip))
            return false;
        // The local anchor is configured, but must never become its own peer.
        if (tcp_server_->IsLocalListenerEndpoint(ip, config_.port))
            return true;
    ...
    bool IsPeerConnected(const std::string& key, bool require_outbound = false) const {
        if (!tcp_server_)
            return false;
    ...
    std::string GetNodeInfo() const {
        ...
        oss << "Supply:     " << std::fixed << std::setprecision(8) << chain_.TotalSupplyVeld()
            << " / 21,000,000 VELD\n";
        oss << "Phase:      "
            << (!chain_.IsStakingActive() ? "Bootstrap (pre-staking)" : "Standard (staking active)")
            << "\n";
        oss << "Staking:    "
            << (chain_.IsStakingActive()
                    ? "Active"
                    : "Inactive (awaiting " +
                          std::to_string(
                              (uint64_t)(chain_.GetStakingActivationUnits() / VELD_UNITS)) +
                          " VELD)")
            << "\n";
        oss << "Mempool:    " << mempool_.Size() << " txs\n";
        oss << "Mining:     "
            << (!mining_.load()                                             ? "Stopped"
                : (!ibd_complete_.load() || !chain_fully_validated_.load()) ? "Sync (mining paused)"
                                                                            : "Active")
            << "\n";
        return oss.str();
    }
  ...
  public:
    ...
    void RecordPeerBlockAccepted() {
        ...
        last_peer_block_at_seconds_.store((int64_t)std::chrono::duration_cast<std::chrono::seconds>(
                                              std::chrono::steady_clock::now().time_since_epoch())
                                              .count(),
    ...
    void BroadcastSupervisorTick() {
...

        {
            ...
            for (auto it = pending_broadcasts_.begin(); it != pending_broadcasts_.end();) {
                ...
                try {
                    ...
                    if (at_height_hex.empty() || at_height_hex != HashToHex(it->block_hash)) {
                        reorged_out = true;
                    }
                } catch (...) {
                }
                ...
                if (tcp_server_ && tcp_server_->GetPeerVerifiedHeight() >= it->height) {
                    it = pending_broadcasts_.erase(it);
                    continue;
                }
                ...
                if (age_ms < LAYER1_SOFT_PACING_MS) {
                    ++it;
                    continue;
                }
        ...
        bool will_log_halt_clear = false;
        {
            ...
            for (const auto& e : pending_broadcasts_) {
                size_t need =
                    std::min<size_t>(LAYER1_ACK_QUORUM, std::max<size_t>(1, live_peers_now));
                if (e.acked_peers.size() >= need)
                    continue;
        ...
        for (const auto& pa : pushes) {
            ...
            std::cerr << "  [layer1] direct-push BLOCK h=" << pa.height << " to peers (excluded "
                      << pa.exclude_addrs.size() << " already-acked)\n";
            std::cerr.flush();
        }
        ...
        if (will_log_halt_set) {
            std::cerr << "  [layer1] HARD HALT mining: block h=" << halt_for_height << " ("
                      ...
                      << "...) failed to reach ACK quorum within " << (LAYER1_HARD_HALT_MS / 1000)
                      << "s. Will resume when "
        ...
        try {
            ...
            EnqueueReorgFixup([this]() {
                try {
                    ...
                    if (dropped > 0) {
                        std::cerr << "  [layer1] reorg-invalidate dropped " << dropped
                                  << " stale pending_broadcasts entr"
                                  << (dropped == 1 ? "y" : "ies") << " (now " << after_size
                                  << ")\n";
                        std::cerr.flush();
                    }
                } catch (const std::exception& e) {
            ...
            std::cerr << "  [layer1] reorg-invalidate dispatch exception: " << e.what() << "\n";
            std::cerr.flush();
        } catch (...) {
    ...
    void RotateOutboundPeers() {
        if (tcp_server_)
            tcp_server_->RotateOneRandomOutbound();
    }
    ...
    void PersistPeerTipsCache() {
        ...
        if (!out) {
            ...
            if (!warned.exchange(true)) {
                std::cerr << "  [peer-tips-cache] WARN: cannot open " << tmp
                          << " for write — peer-tips persistence disabled this cycle.\n";
                std::cerr.flush();
            }
            return;
        }
        ...
        for (const auto& s : snaps) {
            if (s.ip.empty())
                continue;
            body_ss << s.ip << '\t' << s.height << '\t' << HashToHex(s.hash) << '\t' << s.updated_at
                    << '\n';
        }
    ...
    void LoadPeerTipsCache() {
        ...
        if (trailer.rfind(sha_prefix, 0) == 0 && trailer.size() == sha_prefix.size() + 64) {
            std::string body =
                (prev_nl == std::string::npos) ? std::string() : content.substr(0, prev_nl + 1);
            SHA256 hasher;
            hasher.update(body);
            Hash256 body_hash = hasher.digest();
        ...
        if (!std::getline(sin, header_line))
            return;
        if (!header_line.empty() && header_line.back() == '\r')
            header_line.pop_back();
        if (header_line != PEER_TIPS_CACHE_HEADER_V1) {
            std::cerr << "  [peer-tips-cache] header mismatch (got '" << header_line.substr(0, 50)
  ...
  public:
    void RecordSelfMinedBlock(const Hash256& h) {
        ...
        last_self_mined_at_seconds_.store((int64_t)std::chrono::duration_cast<std::chrono::seconds>(
                                              std::chrono::steady_clock::now().time_since_epoch())
                                              .count(),
                                          std::memory_order_relaxed);
    ...
    bool BlockFloodGuardHolding() const {
        ...
        if (peer_acked_self_mined_.load(std::memory_order_acquire))
            return false;
        if (chain_.IsEmpty())
            return false;
        Hash256 our_tip;
        try {
            our_tip = chain_.Tip().GetHash();
        } catch (...) {
            return false;
        }
        std::lock_guard<std::mutex> lock(self_mined_mutex_);
        if (!(our_tip == last_self_mined_hash_))
            return false;
        if (tcp_server_) {
            uint64_t peer_verified = tcp_server_->GetPeerVerifiedHeight();
            if (peer_verified >= chain_.Height())
                return false;
        }
    ...
    bool ForkSuspected() const {
        if (!tcp_server_)
            return false;
        if (tcp_server_->ConnectedPeers() < 1)
            return false;
        ...
        if (local_blocks_since_peer_block_.load(std::memory_order_relaxed) <
            FORK_SELF_HEAL_THRESHOLD)
            return false;
    ...
    void TriggerTipReconcile() {
        ...
        if (last > 0 && now_s - last < TIP_RECONCILE_MIN_INTERVAL_S)
            return;
    ...
    void BroadcastTipsig() {
        if (tcp_server_)
            tcp_server_->BroadcastTipsig();
    }

    void ReapStuckHandshakes() {
        if (tcp_server_)
            tcp_server_->ReapStuckHandshakes();
    }

    void ReapIdlePeers() {
        if (tcp_server_)
            tcp_server_->ReapIdlePeers();
    }
  ...
  private:
    void OracleSyncCheckImpl_() {
        ...
        const std::string curl_executable = compat::TrustedSystemCurlExecutable();
        ...
        auto oracle = compat::RunProcess(
            {curl_executable, "--disable", "-fsS", "--max-time", "8", "--proto", "=https", "-H",
             "User-Agent: veld-node-oracle/1.0", "https://explorer.veld.network/api/stats"},
        ...
        uint64_t plausible_max = (our_height < 100) ? (our_height + 100'000) : (our_height * 10);
        if (oracle_height > plausible_max) {
            std::cerr << "  [oracle] suspicious oracle response: oracle_height=" << oracle_height
                      << " vs our_height=" << our_height
        ...
        if (!chain_.IsEmpty()) {
            try {
                our_hash_hex = HashToHex(chain_.Tip().GetHash());
            } catch (...) {
            }
        }
        ...
        if (log_now) {
            std::cerr
                << "  [oracle] DIVERGENCE: veld.network reports h=" << oracle_height
                << " hash=" << oracle_hash.substr(0, 16) << "...\n"
                << "  [oracle] our local view:        h=" << our_height << " hash="
                << (our_hash_hex.empty() ? "(empty)" : our_hash_hex.substr(0, 16) + "...") << "\n"
                << "  [oracle] "
                << (ahead ? "we are BEHIND oracle"
                          : "SAME height, DIFFERENT hash (likely on a fork)")
            ...
            std::cerr.flush();
            last_alarm_at_seconds.store((int64_t)std::chrono::duration_cast<std::chrono::seconds>(
                                            std::chrono::steady_clock::now().time_since_epoch())
                                            .count(),
                                        std::memory_order_relaxed);
        }
  ...
  private:
    void QuarantineDataDir_() {
        ...
        auto require_rename = [&](const std::string& dir) {
            ...
            if (ec)
                throw std::runtime_error("cannot inspect live data directory before quarantine: " +
                                         dir + ": " + ec.message());
            if (!present)
                return;
            ...
            if (ec || fs::exists(dir) || !fs::exists(target))
                throw std::runtime_error("failed to quarantine live directory " + dir +
                                         (ec ? ": " + ec.message() : std::string()));
            std::cerr << "  [recover] quarantined: " << dir << " -> " << target << "\n";
        };
        ...
        try {
            ...
            for (auto& ent : fs::directory_iterator(data_dir_)) {
                ...
                if (base != "blocks" && base != "db" && base != "index")
                    continue;
                ...
                for (size_t i = 2; i < vec.size(); ++i) {
                    ...
                    if (!ec) {
                        ...
                        std::cerr << "  [recover] WARN: could not prune "
                                  << vec[i].path().filename().string() << ": " << ec.message()
                                  << "\n";
                    }
                }
            }
        } catch (const std::exception& e) {
            std::cerr << "  [recover] WARN: quarantine prune failed: " << e.what()
    ...
    void PruneStaleQuarantines_() {
        ...
        try {
                    ...
                    try {
                        for (auto& f : fs::recursive_directory_iterator(vec[i].path())) {
                            ...
                            if (f.is_regular_file(ec))
                                bytes_before += fs::file_size(f.path(), ec);
                        }
                    } catch (...) {
                    }
                    std::error_code ec;
            ...
            if (total_pruned > 0) {
                std::cout << "  [startup] pruned " << total_pruned << " stale quarantine dirs ("
            ...
            if (all_quarantines.size() > MAX_QUARANTINE_DIRS_GLOBAL) {
                ...
                for (size_t i = MAX_QUARANTINE_DIRS_GLOBAL; i < all_quarantines.size(); ++i) {
                    ...
                    try {
                        for (auto& f :
                             fs::recursive_directory_iterator(all_quarantines[i].path())) {
                            std::error_code ec;
                            if (f.is_regular_file(ec))
                                bytes_before += fs::file_size(f.path(), ec);
                        }
                    ...
                    if (!ec) {
                        ++global_pruned;
                        global_bytes += bytes_before;
                    }
                }
                if (global_pruned > 0) {
                    std::cout << "  [startup] global quarantine cap: pruned " << global_pruned
                              << " more dirs (" << (global_bytes / (1024 * 1024)) << " MB freed)\n";
                }
            }
        } catch (const std::exception& e) {
            std::cerr << "  [startup] WARN: quarantine sweep failed: " << e.what() << "\n";
        }
    }
  ...
  public:
    ...
    void ValidateStoredChainOnly(uint64_t expected_height, const std::string& expected_tip_hash,
                                 bool verify_historical_pow = true) {
#if defined(VELD_ENABLE_SNAPSHOT_BOOTSTRAP)
        if (SnapshotRecoveryPending())
            throw std::runtime_error(
                "snapshot candidate validation is disabled during fresh recovery");
#endif
        if (expected_height == 0 || !SnapshotManifestIsHex64(expected_tip_hash)) {
            throw std::runtime_error("snapshot candidate has malformed signed tip identity");
        }
        PrepareAnchorSecurityBootstrap_();
        WireDB();
    ...
    bool DurableWriteControlFile_(const std::string& path, const std::string& body) const {
        ...
        bool ok = body.size() <= static_cast<size_t>(MAXDWORD) &&
                  ...
                  written == body.size() && ::FlushFileBuffers(h);
        ::CloseHandle(h);
        if (!ok) {
    ...
    std::optional<std::string> ReadSecureControlFile_(const std::string& path, size_t cap) const {
#if defined(_WIN32)
        ...
        DWORD got = 0;
        bool ok = ::ReadFile(h, body.data(), static_cast<DWORD>(body.size()), &got, nullptr) &&
                  got == body.size();
        ::CloseHandle(h);
        if (!ok)
            throw std::runtime_error("cannot read control file");
        return body;
#else
        struct stat lst {};
        ...
        if (!S_ISREG(lst.st_mode) || lst.st_uid != ::geteuid() || (lst.st_mode & 0077) != 0)
            throw std::runtime_error(
        ...
        if (fd < 0)
            throw std::runtime_error("cannot securely open control file");
        struct stat fst {};
        if (::fstat(fd, &fst) != 0 || !S_ISREG(fst.st_mode) || fst.st_dev != lst.st_dev ||
            fst.st_ino != lst.st_ino || fst.st_uid != ::geteuid() || (fst.st_mode & 0077) != 0 ||
    ...
    FinalityEvidencePersistStatus PersistFinalityEvidenceLocked_() {
        std::string body;
        try {
            const std::vector<uint8_t> encoded = fin_equivocation_collector_.Encode();
            if (encoded.size() >
                ::veld::finality::qc::FinalityEquivocationCollector::MAX_ENCODED_JOURNAL_BYTES)
    ...
    FinalityEvidenceValidator_(uint64_t tip) const {
        namespace fq = ::veld::finality::qc;
        return [this, tip](const fq::SignedVote& first, const fq::SignedVote& second)
                   -> std::optional<fq::ValidatedEquivocationEvidence> {
            if (first.epoch_id != second.epoch_id || first.set_root != second.set_root ||
            ...
            return fq::ValidateEquivocationPairForRetainedMember(
                first, second, retained->epoch, retained->root, member, tip, fq::NETWORK_ID,
                GenesisHashBytes_());
        };
    }
    ...
    void RestoreRecoveredFinalityEvidenceLocked_() {
        ...
        const auto body = ReadSecureControlFile_(
            path.string(), fq::FinalityEquivocationCollector::MAX_ENCODED_JOURNAL_BYTES);
        if (!body) {
        ...
        const uint64_t tip = chain_.Height();
        if (!recovered.RestoreJournalAtTip(std::vector<uint8_t>(body->begin(), body->end()),
                                           FinalityEvidenceValidator_(tip), tip, fq::NETWORK_ID,
                                           GenesisHashBytes_()))
            throw std::runtime_error(
                "preserved finality evidence does not authenticate against the rebuilt chain");
        recovered.PruneExpired(tip);
        for (const auto& summary :
             recovered.ListSummaries(fq::FinalityEquivocationCollector::MAX_COMPLETED_PAIRS)) {
            const auto evidence = recovered.FindById(summary.id);
            if (!evidence)
                throw std::runtime_error("recovered evidence disappeared");
    ...
    void LoadFinalityEvidence_() {
        ...
        const auto body =
            ReadSecureControlFile_(FinalityEvidenceJournalPath_(),
        ...
        if (!compat::RestrictFileToOwner(FinalityEvidenceJournalPath_()))
            throw std::runtime_error("finality equivocation journal is not owner-restricted");
#endif
        const std::vector<uint8_t> encoded(body->begin(), body->end());
        const uint64_t tip = chain_.Height();
        if (!fin_equivocation_collector_.RestoreJournalAtTip(
                encoded, FinalityEvidenceValidator_(tip), tip, fq::NETWORK_ID, GenesisHashBytes_()))
            throw std::runtime_error(
    ...
    void PruneFinalityEvidenceAfterDurableTip_(uint64_t tip) noexcept {
        try {
            ...
            const bool pruned = fin_equivocation_collector_.PruneExpired(tip) > 0;
            if (!pruned && !fin_equivocation_journal_uncertain_)
                return;
            ...
            if (persisted == FinalityEvidencePersistStatus::Committed) {
                ...
                if (persisted == FinalityEvidencePersistStatus::CommitUncertain)
                    fin_equivocation_journal_uncertain_ = true;
    ...
    bool PrecheckFinalityVoteWire_(const std::vector<uint8_t>& wire) const {
        ...
        auto transition = chain_.AcquireConsensusTransitionGuard();
        const uint64_t tip = chain_.Height();
        if (!fq::FinalityEvidenceVoteWellFormed(*vote, vote->epoch_id, vote->set_root, tip))
            return false;
        {
            std::lock_guard<std::mutex> evidence_gate(fin_equivocation_gate_mu_);
            if (!fin_equivocation_journal_uncertain_ &&
                fin_equivocation_collector_.HasOffense(vote->epoch_id,
                                                       fq::PubkeyCommit(vote->pubkey_hex)))
                return false;
        }
        return validators_
            .ResolveRetainedFinalityMember(vote->epoch_id, vote->set_root, vote->pubkey_hex)
            .has_value();
    }

    net::NodeServer::FinalityVoteVerifyResult
    VerifyFinalityVoteWire_(const std::vector<uint8_t>& wire,
        ...
        uint64_t initial_tip = 0;
        {
            ...
            if (!fq::FinalityEvidenceVoteWellFormed(*vote, vote->epoch_id, vote->set_root,
                                                    initial_tip))
                return Result::RejectedState;
        ...
        const fq::SnapshotEntry initial_member = EvidenceSnapshotEntry_(retained);
        const auto authenticated =
            fq::AuthenticateVoteForMember(*vote, retained.epoch, retained.root, initial_member,
        ...
        // the fixed transition -> evidence -> assembler lock order.
        {
            ...
            if (!fq::FinalityEvidenceVoteWellFormed(*vote, vote->epoch_id, vote->set_root, tip))
                return Result::RejectedState;
            const auto fresh_retained = validators_.ResolveRetainedFinalityMember(
                vote->epoch_id, vote->set_root, vote->pubkey_hex);
            if (!fresh_retained || fresh_retained->epoch != retained.epoch ||
                ...
                return Result::RejectedState;
            const fq::SnapshotEntry member = EvidenceSnapshotEntry_(*fresh_retained);
            ...
            if (fin_equivocation_collector_.HasOffense(vote->epoch_id, signer_commit)) {
                fin_equivocation_detector_.EraseSignerEpoch(vote->epoch_id, signer_commit);
                std::lock_guard<std::mutex> assembler_gate(fin_assembler_mu_);
            ...
            const auto observed = fin_equivocation_detector_.ObserveAuthenticated(
                *authenticated, member, tip, fq::NETWORK_ID, GenesisHashBytes_());
            if (observed.result == fq::FinalityEquivocationDetector::ObserveResult::REJECTED ||
                observed.result == fq::FinalityEquivocationDetector::ObserveResult::STORAGE_ERROR)
                return Result::RejectedState;
            if (observed.result ==
                fq::FinalityEquivocationDetector::ObserveResult::COMPLETED_EVIDENCE) {
                if (!observed.evidence)
                    return Result::RejectedState;
                const auto offer = fin_equivocation_collector_.Offer(*observed.evidence);
                if (offer == fq::FinalityEquivocationCollector::OfferResult::INSERTED) {
                    const auto persisted = PersistFinalityEvidenceLocked_();
                    if (persisted != FinalityEvidencePersistStatus::Committed) {
                        if (persisted == FinalityEvidencePersistStatus::NotCommitted) {
                            fin_equivocation_collector_.EraseById(observed.evidence->Id());
                        } else {
                            fin_equivocation_journal_uncertain_ = true;
                        }
                ...
                } else if (offer != fq::FinalityEquivocationCollector::OfferResult::ALREADY_KNOWN &&
                           offer != fq::FinalityEquivocationCollector::OfferResult::
            ...
            if (snapshot == fin_state_.snapshots.end() || snapshot->second.root != vote->set_root)
                return Result::EvidenceOnly;
            fq::CheckpointRef canonical_target;
            canonical_target.height = vote->target.height;
            try {
                canonical_target.hash = chain_.GetBlock(vote->target.height).GetHash();
            } catch (...) {
                return Result::EvidenceOnly;
            }
            if (!fq::VoteMatchesCanonicalFrame(*vote, snapshot->second, tip, canonical_target,
                                               fin_state_.record))
                return Result::EvidenceOnly;
            ...
            if (fin_assembler_.HasVoteClaim(*vote))
                return Result::AlreadyKnown;
            return fin_assembler_.OfferAuthenticated(*authenticated, snapshot->second)
                       ? Result::AcceptedNew
                       : Result::RejectedState;
        }
    }
    ...
    void DurableRemoveControlFile_(const std::string& path) const {
        ...
        int dfd = ::open(parent.c_str(), O_RDONLY | O_DIRECTORY | O_CLOEXEC);
        if (dfd < 0)
            throw std::runtime_error("cannot open datadir for control-file fsync");
        const bool ok = ::fsync(dfd) == 0;
        ::close(dfd);
        if (!ok)
            throw std::runtime_error("cannot fsync control-file removal");
#endif
    }
    ...
    bool SnapshotFastStartRevoked_() const {
        std::error_code ec;
        const bool exists = std::filesystem::exists(SnapshotRevocationMarkerPath_(), ec);
        return ec || exists;
    }

    std::string SnapshotRevocationMarkerPath_() const {
        return (std::filesystem::path(data_dir_) / ".snapshot-fast-start-revoked").string();
    }

#if defined(VELD_ENABLE_SNAPSHOT_BOOTSTRAP)
    bool WriteSnapshotReplayRequirementAt_(const std::string& path, uint64_t height,
                                           const std::string& tip_hash) {
        if (height == 0 || !SnapshotManifestIsHex64(tip_hash))
            return false;
    ...
    std::optional<std::pair<uint64_t, std::string>> ReadSnapshotReplayRequirement_() const {
        ...
        while (std::getline(in, line)) {
            ...
            if (eq == std::string::npos)
                throw std::runtime_error("snapshot consensus-replay requirement is malformed");
        ...
        if (schema_count != 1 || height_count != 1 || tip_count != 1 || schema != "1" || !digits ||
            height == 0 || !SnapshotManifestIsHex64(tip_hash)) {
            throw std::runtime_error("snapshot consensus-replay requirement is not canonical");
        }
    ...
    bool WriteValidatedBackgroundPrefix_(uint64_t height, const Hash256& tip_hash,
                                         std::string* error = nullptr) {
        if (height == 0 || HashIsZero(tip_hash)) {
            if (error)
                *error = "invalid verified height or tip";
            return false;
        }
        return channel::secure_file::AtomicWriteText(ValidatedBackgroundPrefixPath_(),
                                                     "schema=1\nheight=" + std::to_string(height) +
    ...
    std::optional<std::pair<uint64_t, std::string>> ReadValidatedBackgroundPrefix_() const {
        auto body = ReadSecureControlFile_(ValidatedBackgroundPrefixPath_(), 384);
        if (!body)
            return std::nullopt;
        std::istringstream in(*body);
        ...
        while (std::getline(in, line)) {
            ...
            if (eq == std::string::npos)
                throw std::runtime_error("validated background prefix is malformed");
            const std::string key = line.substr(0, eq);
            const std::string value = line.substr(eq + 1);
            if (key == "schema") {
                ++schema_count;
                ...
                throw std::runtime_error("validated background prefix has an unknown field");
            }
        }
        uint64_t height = 0;
        if (schema_count != 1 || height_count != 1 || tip_count != 1 || schema != "1" ||
            !ParseCanonicalUint64Text(height_text, height) || height == 0 ||
            !SnapshotManifestIsHex64(tip_hash)) {
            throw std::runtime_error("validated background prefix is not canonical");
        }
        return std::make_pair(height, tip_hash);
    }

    bool WriteIndependentValidationRequirement_(const SnapshotValidationBase& base) {
        if (base.height == 0 || HashIsZero(base.tip_hash) || HashIsZero(base.state_digest)) {
            return false;
        }
        return DurableWriteControlFile_(IndependentValidationRequirementPath_(),
    ...
    std::optional<SnapshotValidationBase> ReadIndependentValidationRequirement_() const {
        auto body = ReadSecureControlFile_(IndependentValidationRequirementPath_(), 512);
        if (!body)
            return std::nullopt;
        std::istringstream in(*body);
        std::string line, schema, height_text, tip_hash, state_digest;
        size_t schema_count = 0, height_count = 0, tip_count = 0, digest_count = 0;
        while (std::getline(in, line)) {
            if (!line.empty() && line.back() == '\r')
                line.pop_back();
            const auto eq = line.find('=');
            if (eq == std::string::npos)
                throw std::runtime_error("background chainstate requirement is malformed");
            const std::string key = line.substr(0, eq);
            const std::string value = line.substr(eq + 1);
            if (key == "schema") {
                ++schema_count;
                ...
                throw std::runtime_error("background chainstate requirement has an unknown field");
            }
        }
        uint64_t height = 0;
        const bool height_ok = ParseCanonicalUint64Text(height_text, height) && height > 0;
        if (schema_count != 1 || height_count != 1 || tip_count != 1 || digest_count != 1 ||
            schema != "1" || !height_ok || !SnapshotManifestIsHex64(tip_hash) ||
            !SnapshotManifestIsHex64(state_digest)) {
            throw std::runtime_error("background chainstate requirement is not canonical");
        }
    ...
    bool RequestSnapshotRecoveryOnRestart(const std::string& reason) {
        ...
        return DurableWriteControlFile_(SnapshotRecoveryRequestPath_(),
    ...
    bool SnapshotRecoveryRequestedAtStartup_() const {
        ...
        while (std::getline(in, line)) {
            ...
            if (key == "schema") {
                ...
                throw std::runtime_error("snapshot recovery request has unknown field");
        }
    ...
    void RevokeSnapshotValidation_(const std::string& reason) {
        ...
        snapshot_state_clean_.store(false, std::memory_order_release);
        BumpValidationGeneration_();
        snapshot_background_verification_failed_.store(true, std::memory_order_release);
    ...
    void FailPowVerify_(uint64_t h, const char* why, bool mandatory_snapshot_check = false) {
        ...
        if (mandatory_snapshot_check)
            RevokeSnapshotValidation_("snapshot-background-pow-failed");
#else
    ...
    void StartBackgroundPowVerification(uint64_t from_height, std::string marker_path = "",
                                        bool mandatory_snapshot_check = false) {
        if (pow_verify_target_.load() != 0)
            return; // already running / ran
        uint64_t tip = chain_.Height();
        if (from_height < 1)
            from_height = 1;
        ...
        pow_verify_thread_ = std::thread([this, from_height, tip, marker_path,
                                          mandatory_snapshot_check]() {
            std::cerr << "  [pow-verify] background PoW re-verification " << from_height << ".."
                      << tip
#if defined(VELD_ENABLE_SNAPSHOT_BOOTSTRAP)
                      << (mandatory_snapshot_check ? " (mandatory snapshot validation)\n"
            ...
            for (uint64_t h = from_height; h <= tip && running_.load(); ++h) {
                ...
                if (!hash_opt) {
                    FailPowVerify_(h, "canonical height index missing", mandatory_snapshot_check);
                    return;
                }
                auto data = db_.ReadBlock(HexToHash(*hash_opt));
                if (!data) {
                    FailPowVerify_(h, "block data missing", mandatory_snapshot_check);
                    return;
                }
                Block blk;
                const size_t consumed = Block::Deserialize(*data, 0, blk);
                if (consumed == 0 || consumed != data->size() || blk.Serialize() != *data) {
                    FailPowVerify_(h, "non-canonical serialized block", mandatory_snapshot_check);
                    return;
                }
                blk.height = h;
                uint32_t want = chain_.ComputeNextBitsAt(h - 1);
                if (want == 0 || blk.header.bits != want) {
                    FailPowVerify_(h, "bits != expected difficulty", mandatory_snapshot_check);
                    return;
                }
...
#if defined(VELD_ENABLE_SNAPSHOT_BOOTSTRAP)
                        if (mandatory_snapshot_check) {
                            FailPowVerify_(h, "VeldHash dataset unavailable", true);
                        } else {
                            pow_verify_dataset_unavailable_.store(true, std::memory_order_release);
                            pow_verify_from_.store(h, std::memory_order_release);
                            pow_verify_target_.store(0, std::memory_order_release);
                            std::cerr << "  [pow-verify] PAUSED at block " << h
            ...
            if (!marker_path.empty()) {
                try {
                    ...
                    std::cerr << "  [pow-verify] WARN: verification completed but "
                              ...
                              << e.what()
                              << "; verification will repeat on the "
                                 "next start.\n";
                }
            }
    ...
    std::string PowVerifyStatusJson() const {
        ...
        if (independent_base) {
            const bool failed =
                snapshot_background_verification_failed_.load(std::memory_order_acquire);
            std::ostringstream j;
            j << "{\"mode\":\"independent-background-ibd\""
              << ",\"running\":" << (failed ? "false" : "true")
              << ",\"failed\":" << (failed ? "true" : "false") << ",\"diagnostic_complete\":false"
              << ",\"verified_height\":"
              << independent_validation_height_.load(std::memory_order_acquire)
              << ",\"target_height\":" << independent_base->height << "}";
            return j.str();
        }
#endif
        uint64_t tgt = pow_verify_target_.load();
        bool running = tgt != 0 && !pow_verify_done_.load() && !pow_verify_failed_.load() &&
        ...
        j << "{\"running\":" << (running ? "true" : "false")
          ...
          << ",\"diagnostic_complete\":"
          << (pow_verify_done_.load() && !pow_verify_failed_.load() ? "true" : "false")
          << ",\"verified_height\":" << pow_verified_height_.load() << ",\"target_height\":" << tgt
          << "}";
        return j.str();
    }
    ...
    size_t LoadCheckpointsFromUrl() {
        ...
        return 0;
#endif
        constexpr const char* CHECKPOINTS_URL = "https://veld.network/downloads/checkpoints.json";
        uint8_t nonce[16]{};
        if (!compat::SecureRandom(nonce, sizeof(nonce)))
            return 0;
#if defined(_WIN32)
        const std::string tmp =
            data_dir_ + "\\.checkpoints.json.fetch." + BytesToHex(nonce, sizeof(nonce));
#else
        const std::string tmp =
            data_dir_ + "/.checkpoints.json.fetch." + BytesToHex(nonce, sizeof(nonce));
#endif
        ...
        const auto transfer = compat::RunProcessToBoundedFile(
            {curl_executable, "--disable", "--fail", "--silent", "--show-error",
             "--connect-timeout", "10", "--max-time", "30", "--max-filesize",
             std::to_string(kMaxCheckpointDocumentBytes), "--location", "--max-redirs", "0",
             "--proto", "=https", "--proto-redir", "=https", CHECKPOINTS_URL},
        ...
        try {
            ...
            if (size_error || length == 0 || length > kMaxCheckpointDocumentBytes) {
                return 0;
            }
            ...
            if (!in || static_cast<size_t>(in.gcount()) != body.size())
                return 0;
            char unexpected = 0;
            if (in.get(unexpected))
                return 0;
        } catch (...) {
            return 0;
        }
        ...
        if (added > 0 || rejected > 0) {
            std::cerr << "  [checkpoint-fetch] parsed=" << parsed.size() << " new=" << added
                ...
                try {
                    ...
                    if (our_block.GetHash() != cp.block_hash) {
                        std::cerr << "  [checkpoint-mainchain-mismatch] "
                                  ...
                                  << " checkpoint says " << HashToHex(cp.block_hash).substr(0, 16)
  ...
  private:
    ...
    void WireFinalityVoteRpcSink_() {
        ...
        rpc_.SetFinalityVoteSink([this](const std::string& vote_json,
                                        const std::string& binding_text,
                                        const std::string& token_text) -> bool {
            namespace fq = ::veld::finality::qc;
            using Result = net::NodeServer::FinalityVoteVerifyResult;
#ifdef VELD_TEST_HOOKS
            test_work_finality_sink_calls_.fetch_add(1, std::memory_order_acq_rel);
#endif
            ...
            if (!fq::FromHexBytes(vote_json, wire))
                return false;
            const auto vote = fq::DecodeSignedVoteWire(wire);
            if (!vote)
                return false;
            ...
            auto active = TakeActiveRemoteSigningLeaseUnderTransition_(
                work_admission::Path::FinalityVote, subject, binding_text, token_text);
            if (!active || !active->OperationLive() || !active->lease->ClaimForSink() ||
                !active->OperationLive())
                return false;
#ifdef VELD_TEST_HOOKS
            test_work_finality_active_calls_.fetch_add(1, std::memory_order_acq_rel);
#endif
            ...
            const Result result =
                VerifyFinalityVoteWire_(wire, /*caller_holds_consensus_transition=*/true);
#ifdef VELD_TEST_HOOKS
            test_work_finality_verify_result_.store(static_cast<uint8_t>(result),
                                                    std::memory_order_release);
#endif
            if (result == Result::AcceptedNew && tcp_server_ && active->OperationLive()) {
#ifdef VELD_TEST_HOOKS
                test_work_finality_gossip_calls_.fetch_add(1, std::memory_order_acq_rel);
#endif
                auto pm = tcp_server_->GetMessageBuilder();
                tcp_server_->BroadcastMessage(pm.BuildFinalityVoteMessage(wire));
            }
            return result == Result::AcceptedNew || result == Result::AlreadyKnown ||
                   result == Result::EvidenceOnly;
        });
    }

    bool ValidateGenerationIdentityLocked_(const RealKeyPair& miner_keypair,
        ...
        try {
            const auto derived_public = DerivePublicKey(miner_keypair.private_key);
            ...
            if (miner_keypair.script_override.empty()) {
                ...
                const auto override_script = AddressToScript(miner_keypair.address);
                if (override_script.empty() || override_script != miner_keypair.script_override)
            ...
            const auto payout = miner_keypair.script_override.empty()
                                    ...
                                    : miner_keypair.script_override;
            if (IsSha384KeyScript(payout) && !Sha384DestinationsActive(chain_.Height() + 1))
                return refuse("SHA-384 mining destination is not active at the next block height");
        } catch (const std::exception& e) {
            return refuse(std::string("generation identity validation failed: ") + e.what());
        } catch (...) {
    ...
    bool BindAddressGenerationIdentityLocked_(const std::string& address, std::string* error) {
        if (!mining::ValidAddressOnlyDestination(address, config_.IsTestNetwork())) {
            if (error)
                *error = "invalid address-only payout for this network";
            return false;
        }
        const auto script = AddressToScript(address);
        if (IsSha384KeyScript(script) && !Sha384DestinationsActive(chain_.Height() + 1)) {
            if (error)
                *error = "SHA-384 mining destination is not active at the next block height";
            return false;
        }
        RealKeyPair recipient;
        recipient.private_key.fill(0);
        recipient.public_key.fill(0);
        recipient.address = address;
        recipient.testnet = config_.IsTestNetwork();
    ...
    bool BindGenerationIdentityLocked_(const RealKeyPair& miner_keypair, std::string* error) {
    ...
    void StopMiningLocked_() {
        ...
        mining_work_state_.store(MiningWorkState::Stopped, std::memory_order_release);
    }
    ...
    work_admission::BlockTemplateAuthorizationStore block_template_authorizations_;
    mutable std::mutex active_remote_signing_mu_;
    std::unordered_map<std::string, ActiveRemoteSigningLease> active_remote_signing_leases_;
#ifdef VELD_PUBLIC_TESTNET
    std::optional<public_testnet::RuntimeLimits> public_testnet_limits_;
    mutable std::unique_ptr<public_testnet::RuntimeClockGuard> public_testnet_clock_guard_;
    ...
    std::atomic<uint64_t> btc_header_tip_{
        0}; // btcVELD relay: race-free snapshot of btc_headers_.BestHeight() read by the RPC thread (single writer = block path)
    std::atomic<bool> btc_relay_fresh_snapshot_{!btcspv::EXTERNAL_VALUE_FRESHNESS_REQUIRED};
    ...
    std::vector<btcanchor::floor_store::Record> anchor_floor_inherited_records_;
    std::vector<btcanchor::floor_store::Record> anchor_floor_validation_records_;
    ...
    std::vector<EffectiveAnchorSecurityFloor> effective_anchor_security_floors_;
    ...
    std::optional<db::VeldDB::DurablePublicationPending> startup_durable_publication_recovery_;
    std::optional<db::VeldDB::ReorgUtxoPending> startup_reorg_utxo_recovery_;
    ...
    ::veld::finality::qc::FinalityEquivocationDetector fin_equivocation_detector_;
    ::veld::finality::qc::FinalityEquivocationCollector fin_equivocation_collector_;
    ...
    std::atomic<uint64_t> final_height_{
        0}; // Layer-3 finalized high-water-mark (single writer = the serialized block path via UpdateFinality_; read by RPC)
    ...
    static constexpr uint64_t PEG_GATE_FINALITY_EVER_ACTIVE_BIT_ = uint64_t{1} << 63;
    std::atomic<uint64_t> peg_gate_snapshot_{0};
    std::atomic<bool> redeem_index_healthy_{
        true}; // false closes payout RPC until startup replay repairs the derived index
    std::atomic<bool> mint_nullifier_index_healthy_{
        true}; // false pauses new mint proof production; transfers/redeems remain live
    btcveld::SupplySnapshotPublisher
        btcveld_supply_snapshot_; // coherent (token supply, canonical tip, hash) RPC tuple
    ...
    void MaybeCaptureBackgroundValidationTarget_() {
        if (!background_validation_only_ || background_validation_target_height_ == 0) {
            return;
        }
        ...
        const auto saved_height =
            background_prefix_persisted_height_.load(std::memory_order_acquire);
        const bool replaced_prefix =
            saved_height > 0 && (height < saved_height || chain_.GetBlock(saved_height).GetHash() !=
            ...
            try {
                ...
                if (!background_prefix_write_warning_logged_.exchange(true))
                    std::cerr << "  [background-ibd] WARN: could not retire "
                                 "replaced restart progress: "
                              << error.what() << '\n';
            }
        }
        if (height > 0 &&
            (replaced_prefix || ((height <= 10 || height % 25 == 0 ||
            ...
            if (WriteValidatedBackgroundPrefix_(height, tip_hash, &error)) {
                background_prefix_persisted_height_.store(height, std::memory_order_release);
                background_prefix_persisted_tip_ = tip_hash;
                if (background_prefix_write_warning_logged_.exchange(false,
        ...
        if (!chain_.IsEmpty())
            observation.tip_hash = chain_.TipCopy().GetHash();
        if (height == background_validation_target_height_) {
    ...
    void NotifyPrewarm_() {
#ifdef VELD_TEST_HOOKS
        if (test_prewarm_before_notify_)
            test_prewarm_before_notify_();
#endif
...
#ifdef VELD_TEST_HOOKS
        if (test_reorg_before_notify_)
            test_reorg_before_notify_();
#endif
        std::lock_guard<std::mutex> lock(reorg_fixup_mtx_);
    ...
    std::atomic<MiningWorkState> mining_work_state_{MiningWorkState::Stopped};
    ...
    void EnsureAnchorSecurityDirectory_() const {
        ...
        if (!channel::secure_file::EnsurePrivateDirectory(AnchorSecurityDirectory_(), &dir_error)) {
            throw std::runtime_error("anchor security directory is not a private owner-only "
                                     "directory: " +
                                     dir_error);
        }
    }
    ...
    static const std::vector<uint8_t>& AnchorFloorUncertainMarker_() {
        static const std::vector<uint8_t> marker{'V', 'L', 'F', '1', '-', 'U', 'N', 'C',
                                                 'E', 'R', 'T', 'A', 'I', 'N', '\n'};
        return marker;
    }

    static const std::vector<uint8_t>& AnchorFloorClearMarker_() {
        static const std::vector<uint8_t> marker{'V', 'L', 'F', '1', '-', 'C',
                                                 'L', 'E', 'A', 'R', '\n'};
        return marker;
    }
    ...
    bool AnchorFloorRepairBlocksSnapshot_() const {
        return anchor_floor_repair_required_.load(std::memory_order_acquire) ||
               anchor_floor_security_uncertain_.load(std::memory_order_acquire);
    }
    ...
    EffectiveAnchorSecurityPins_(const EffectiveAnchorSecurityFloor& floor) {
        return {
            {{floor.target_height, floor.target_hash},
             {floor.proof_carrier_height, floor.proof_carrier_hash},
             {floor.authorization_record.carrier.height, floor.authorization_record.carrier.hash}}};
    }

    static EffectiveAnchorSecurityFloor
    EffectiveLocalFloorFromRecord_(const btcanchor::floor_store::Record& record,
        ...
        incoming.proof_carrier_height = checkpoint.entry.carrying_veld_height;
        incoming.proof_carrier_hash = checkpoint.entry.carrying_veld_hash;
        ...
        incoming.exact_reconstruction_observed = exact_reconstruction_observed;
        return incoming;
    }

    void
    RequireEffectiveAnchorFloorCompatibilityLocked_(const EffectiveAnchorSecurityFloor& incoming,
        ...
        for (const auto& prior : effective_anchor_security_floors_) {
            ...
            if (prior.target_height == incoming.target_height) {
                ...
                checkpoint.entry.carrying_veld_height = incoming.proof_carrier_height;
                checkpoint.entry.carrying_veld_hash = incoming.proof_carrier_hash;
                ...
                checkpoint.entry.authorization_finality_digest =
                    ::veld::finality::qc::RecordDigest(incoming.authorization_record);
                checkpoint.authorization_record = incoming.authorization_record;
                if (!EffectiveAnchorSecurityPermanentExactlyMatches_(prior, checkpoint)) {
                    throw std::runtime_error(
                        std::string(source_name) +
    ...
    void RequireAnchorLocalFloorCompatibility_(const btcanchor::floor_store::Record& record) const {
        const auto incoming = EffectiveLocalFloorFromRecord_(
            record, EffectiveAnchorSecurityFloor::Source::LocalObserved,
    ...
    bool AllRecoveredLocalFloorsExactlyObserved_() const {
        ...
        for (const auto& floor : effective_anchor_security_floors_) {
            if ((floor.source == EffectiveAnchorSecurityFloor::Source::LocalObserved ||
                 floor.source == EffectiveAnchorSecurityFloor::Source::LocalRecovery) &&
    ...
    void InstallAnchorLocalFloor_(const btcanchor::floor_store::Record& record,
                                  bool exact_reconstruction_observed) {
        if (record.network_magic != config_.magic ||
            record.genesis_hash != CreateGenesisBlock().GetHash())
            throw std::runtime_error("local anchor floor belongs to another network/genesis");
        ...
        auto local = std::find_if(
            effective_anchor_security_floors_.begin(), effective_anchor_security_floors_.end(),
            [](const EffectiveAnchorSecurityFloor& floor) {
                return floor.source == EffectiveAnchorSecurityFloor::Source::LocalObserved;
            });
        if (local != effective_anchor_security_floors_.end()) {
            if (local->target_height > incoming.target_height)
                throw std::logic_error("attempted to roll back installed local anchor floor");
            if (local->target_height == incoming.target_height) {
                local->exact_reconstruction_observed =
                    local->exact_reconstruction_observed || exact_reconstruction_observed;
                return;
            }
    ...
    void InstallAnchorRecoveryFloor_(const btcanchor::floor_store::Record& record) {
        auto incoming = EffectiveLocalFloorFromRecord_(
            record, EffectiveAnchorSecurityFloor::Source::LocalRecovery,
            /*exact_reconstruction_observed=*/false);
        std::lock_guard<std::mutex> lock(anchor_security_floors_mutex_);
        RequireEffectiveAnchorFloorCompatibilityLocked_(incoming, "recovered VLF1 copy");
        const auto duplicate = std::find_if(
            effective_anchor_security_floors_.begin(), effective_anchor_security_floors_.end(),
            [&incoming](const EffectiveAnchorSecurityFloor& prior) {
                return prior.source == EffectiveAnchorSecurityFloor::Source::LocalRecovery &&
    ...
    void InheritAnchorLocalFloorForValidation_(
        ...
        if (running_.load(std::memory_order_acquire) || anchor_floor_prepared_)
            throw std::logic_error("snapshot floor inheritance is startup-only");
        if (records.empty())
            return;
        std::string error;
        for (const auto& record : records) {
            if (!btcanchor::floor_store::Validate(record, &error) ||
                ...
                record.genesis_hash != CreateGenesisBlock().GetHash())
                throw std::runtime_error("cannot inherit invalid snapshot anchor floor: " + error);
        }
        anchor_floor_inherited_records_ = records;
    }

    void InheritAnchorLocalFloorForValidation_(const btcanchor::floor_store::Record& record) {
        InheritAnchorLocalFloorForValidation_(std::vector<btcanchor::floor_store::Record>{record});
    }

    bool WriteAnchorFloorUncertaintyMarker_(bool uncertain, std::string* error) {
        const auto& marker = uncertain ? AnchorFloorUncertainMarker_() : AnchorFloorClearMarker_();
        if (!::veld::channel::secure_file::AtomicWrite(AnchorFloorUncertaintyPath_(), marker, error,
                                                       /*require_private_parent=*/true))
            return false;
        anchor_floor_repair_required_.store(uncertain, std::memory_order_release);
        return true;
    }
...
#endif
            {
                cleared = db_.ClearDurablePublicationPending(block.height, block.GetHash());
            }
        } catch (const std::exception& e) {
            FailStopDurableCommitInvariant_(std::string("VDP1 clear threw for durable h=") +
                                            std::to_string(block.height) + ": " + e.what());
            return false;
        } catch (...) {
            FailStopDurableCommitInvariant_("VDP1 clear threw an unknown exception for durable h=" +
                                            std::to_string(block.height));
            return false;
        }
        if (!cleared) {
            FailStopDurableCommitInvariant_(
                "VDP1 identity-checked clear failed for durable h=" + std::to_string(block.height) +
    ...
    void LoadDurablePublicationRepair_() {
        ...
        if (pending) {
            ...
            if (!bytes) {
                throw std::runtime_error("durable publication pending block body is missing");
            }
            Block body;
            body.height = pending->height;
            const size_t consumed = Block::Deserialize(*bytes, 0, body);
            if (consumed == 0 || consumed != bytes->size() || body.Serialize() != *bytes ||
                body.GetHash() != pending->hash) {
                throw std::runtime_error(
                    "durable publication pending block body is non-canonical or does not match its identity");
            }
        }
        durable_commit_repair_required_.store(pending.has_value() || utxo_recovery,
                                              std::memory_order_release);
    }
    ...
    void FinishDurableCommitReplayRepair_() {
        ...
        if (startup_reorg_utxo_recovery_) {
            ...
            try {
                completed = db_.CompleteReorgUtxoRecoveryAfterReplay(expected);
            } catch (const db::DurableWriteUncertain& e) {
                FailStopDurableCommitInvariant_(
                    std::string("VUR1 startup acknowledgement is uncertain: ") + e.what());
                throw;
            }
            if (!completed) {
                FailStopDurableCommitInvariant_(
        ...
        if (expected_pending) {
            ...
            if (!stored_tip || stored_tip->height != pending->height ||
                ...
                chain_.Height() != pending->height || chain_.TipCopy().GetHash() != pending->hash) {
                throw std::runtime_error(
...
#endif
                {
                    cleared = db_.ClearDurablePublicationPending(pending->height, pending->hash);
                }
            } catch (const std::exception& e) {
                FailStopDurableCommitInvariant_(
                    std::string("VDP1 clear threw after exact replay: ") + e.what());
                throw;
            } catch (...) {
                FailStopDurableCommitInvariant_(
    ...
    void FailStopAnchorFloorPersistence_(const std::string& why) {
        ...
        const bool sentinel_ok = WriteAnchorFloorUncertaintyMarker_(true, &sentinel_error);
        anchor_floor_security_uncertain_.store(true, std::memory_order_release);
        ...
        NotifyPrewarm_();
        if (tcp_server_)
            tcp_server_->SetIBDComplete(false);
        ...
        if (!sentinel_ok)
            std::cerr << " Uncertainty sentinel also failed: " << sentinel_error;
        std::cerr << "\n";
        std::cerr.flush();
    }
    ...
    void FailStopDurableCommitInvariant_(const std::string& why) {
        ...
        work_admission_coordinator_.CancelAndClose(work_admission::Refusal::DurableStateUnproven);
        durable_commit_fail_stop_.store(true, std::memory_order_release);
        durable_commit_repair_required_.store(true, std::memory_order_release);
        if (ibd_complete_.exchange(false, std::memory_order_acq_rel))
            BumpValidationGeneration_();
        mining_.store(false, std::memory_order_release);
        running_.store(false, std::memory_order_release);
    ...
    void LoadAnchorFloorSecurity_(const Hash256& genesis_hash) {
        ...
        const auto marker_read =
            ::veld::channel::secure_file::Read(AnchorFloorUncertaintyPath_(), marker, &error, 64,
                                               /*require_private_parent=*/true);
        if (marker_read == ::veld::channel::secure_file::ReadResult::Error) {
            throw std::runtime_error("cannot securely read anchor-floor uncertainty marker: " +
                                     error);
        }
        if (marker_read == ::veld::channel::secure_file::ReadResult::Ok) {
            if (marker == AnchorFloorUncertainMarker_()) {
                anchor_floor_repair_required_.store(true, std::memory_order_release);
            } else if (marker == AnchorFloorClearMarker_()) {
                anchor_floor_repair_required_.store(false, std::memory_order_release);
            } else {
                throw std::runtime_error("anchor-floor uncertainty marker is malformed");
            }
        }
        ...
        if (!anchor_floor_inherited_records_.empty()) {
            ...
            const auto external_read = ::veld::channel::secure_file::Read(
                AnchorFloorPersistentPath_(), external_wire, &error,
                btcanchor::floor_store::VLF1_ENCODED_SIZE,
                /*require_private_parent=*/true);
            if (external_read == ::veld::channel::secure_file::ReadResult::Error) {
                throw std::runtime_error("cannot securely read external local anchor floor: " +
                                         error);
            }
            if (external_read == ::veld::channel::secure_file::ReadResult::Ok) {
                const auto decoded = btcanchor::floor_store::Decode(external_wire, &error);
                if (!decoded || decoded->network_magic != config_.magic ||
                    decoded->genesis_hash != genesis_hash) {
                    throw std::runtime_error("external local anchor floor rejected: " + error);
                }
            ...
            if (db_wire) {
                const auto decoded = btcanchor::floor_store::Decode(*db_wire, &error);
                if (!decoded || decoded->network_magic != config_.magic ||
                    decoded->genesis_hash != genesis_hash) {
                    throw std::runtime_error("authoritative DB local anchor floor rejected: " +
                                             error);
                }
                learned.push_back(*decoded);
            }
        ...
        const auto retained = snapshot_recovery::ReadFloors(data_dir_, config_.magic, genesis_hash);
        learned.insert(learned.end(), retained.begin(), retained.end());
        if (learned.empty())
            return;
        std::sort(
            learned.begin(), learned.end(),
            [](const btcanchor::floor_store::Record& a, const btcanchor::floor_store::Record& b) {
        ...
        for (const auto& record : learned) {
            ...
            if (merged == btcanchor::floor_store::MergeResult::Rejected ||
                ...
                throw std::runtime_error(
                    "local anchor floor copies are not one monotonic history: " + error);
            }
        }
        if (!rebuilt.Current())
        ...
        if (learned.size() == 1) {
            const auto selected_wire = btcanchor::floor_store::Encode(*rebuilt.Current(), &error);
            if (!selected_wire)
                throw std::runtime_error("selected local anchor floor could not be encoded: " +
                                         error);
            const auto mirrored =
                rebuilt.MergeAndPersist(AnchorFloorPersistentPath_(), *rebuilt.Current(), &error);
            if (mirrored == btcanchor::floor_store::MergeResult::Rejected ||
                mirrored == btcanchor::floor_store::MergeResult::IoError) {
                throw std::runtime_error("cannot repair external local anchor floor mirror: " +
                                         error);
            }
            if (!db_.WriteAnchorSecurityFloor(*selected_wire)) {
                throw std::runtime_error("cannot repair authoritative DB local anchor floor copy");
            }
        }

        std::lock_guard<std::mutex> store_lock(anchor_floor_store_mutex_);
    ...
    AnchorFloorWireForDurableCommit_(bool include_new_permanent) {
        ...
        if (include_new_permanent && anchors_.HasExportablePermanentFloor()) {
            const auto permanent = anchors_.Permanent();
            if (!permanent)
                throw std::logic_error("exportable anchor floor has no permanent checkpoint");
            std::string error;
            const auto candidate = btcanchor::floor_store::Make(
                config_.magic, CreateGenesisBlock().GetHash(), *permanent, &error);
            if (!candidate)
                throw std::runtime_error("durable block cannot form its exact VLF1 floor: " +
                                         error);
            ...
            if (!selected) {
                ...
                const uint64_t incoming_t = candidate->checkpoint.target_height;
                const uint64_t current_t = selected->checkpoint.target_height;
                if (incoming_t == current_t) {
                    if (!btcanchor::floor_store::ExactDuplicate(*candidate, *selected)) {
                        throw std::runtime_error(
                    ...
                    if (trial.Merge(*selected, &error) ==
                        ...
                        trial.Merge(*candidate, &error) ==
                            btcanchor::floor_store::MergeResult::Rejected) {
                        throw std::runtime_error(
                            "durable block cannot monotonically advance VLF1: " + error);
                    }
                    RequireAnchorLocalFloorCompatibility_(*candidate);
                    selected = *candidate;
                }
        ...
        if (recovery_count > 0 && !AllRecoveredLocalFloorsExactlyObserved_())
            return std::nullopt;
        std::string error;
        const auto wire = btcanchor::floor_store::Encode(*selected, &error);
        if (!wire)
            throw std::runtime_error("durable VLF1 copy could not be encoded: " + error);
        return *wire;
    }
    ...
    bool PersistObservedAnchorFloorAfterDurable_(bool startup_replay) {
        if (!anchors_.HasExportablePermanentFloor())
            return true;
        const auto permanent = anchors_.Permanent();
        if (!permanent)
            return true;
        ...
        const auto candidate = btcanchor::floor_store::Make(
            config_.magic, CreateGenesisBlock().GetHash(), *permanent, &error);
        const auto fail = [this, startup_replay](const std::string& message) -> bool {
            if (startup_replay)
                throw std::runtime_error("startup replay could not repair local anchor floor: " +
                                         message);
            FailStopAnchorFloorPersistence_(message);
            return false;
        };
        if (!candidate)
            return fail("exportable permanent could not form VLF1: " + error);
        ...
        const bool preserve_recovery_witness =
            recovery_count > 0 && !AllRecoveredLocalFloorsExactlyObserved_();

        {
            ...
            if (current) {
                const uint64_t incoming_t = candidate->checkpoint.target_height;
                const uint64_t current_t = current->checkpoint.target_height;
                if (incoming_t < current_t) {
                ...
                if (incoming_t == current_t &&
                    !btcanchor::floor_store::ExactDuplicate(*candidate, *current)) {
                    return fail("reconstructed permanent conflicts with VLF1 at "
                                "the same target T");
                }
            }
...
#endif
            {
                result = anchor_floor_store_.MergeAndPersist(AnchorFloorPersistentPath_(),
                                                             *candidate, &error);
            }
        ...
        try {
            InstallAnchorLocalFloor_(*candidate,
                                     /*exact_reconstruction_observed=*/true);
        } catch (const std::exception& e) {
            return fail(std::string("persisted VLF1 conflicts with installed security floors: ") +
                        e.what());
        }
        {
            std::lock_guard<std::mutex> store_lock(anchor_floor_store_mutex_);
            if (anchor_floor_validation_records_.size() > anchor_floor_recovery_record_count_) {
                anchor_floor_validation_records_.resize(anchor_floor_recovery_record_count_);
            }
    ...
    void FinishAnchorFloorReplayRepair_() {
        if (!anchor_floor_repair_required_.load(std::memory_order_acquire))
            return;
        const auto permanent = anchors_.Permanent();
        std::optional<btcanchor::floor_store::Record> current;
        {
        ...
        if (!permanent || !current ||
            !EffectiveAnchorSecurityPermanentExactlyMatches_(
                EffectiveAnchorSecurityFloor{EffectiveAnchorSecurityFloor::Source::LocalObserved,
                                             {},
                                             ...
                                             true},
                *permanent)) {
            throw std::runtime_error("startup replay did not reconstruct the uncertain VLF1 floor");
        }
        std::string error;
        if (!WriteAnchorFloorUncertaintyMarker_(false, &error))
            throw std::runtime_error("startup replay repaired VLF1 but could not clear its "
                                     "uncertainty marker: " +
                                     error);
        anchor_floor_security_uncertain_.store(false, std::memory_order_release);
    }
    ...
    void PrepareAnchorSecurityBootstrap_() {
        ...
        if (anchor_floor_prepared_)
            return;
        if (running_.load(std::memory_order_acquire) || tcp_server_ || mining_thread_.joinable() ||
            pow_verify_thread_.joinable()) {
            throw std::runtime_error("anchor WS preparation must complete before node startup");
        }

        const Hash256 genesis_hash = CreateGenesisBlock().GetHash();
        if (HashIsZero(genesis_hash))
            throw std::runtime_error("compiled genesis hash is null during anchor WS preparation");
    ...
    bool EffectiveAnchorSecurityPinPermits_(uint64_t height, const Hash256& hash) const {
        if ((!startup_replay_active_.load(std::memory_order_acquire) && DurableCommitFailStop()) ||
            anchor_floor_security_uncertain_.load(std::memory_order_acquire))
            return false;
        std::lock_guard<std::mutex> lock(anchor_security_floors_mutex_);
        for (const auto& floor : effective_anchor_security_floors_) {
            if (height == floor.target_height && hash != floor.target_hash)
                return false;
            if (height == floor.proof_carrier_height && hash != floor.proof_carrier_hash)
                return false;
    ...
    bool EffectiveAnchorSecurityReorgPermitted_(uint64_t common_ancestor_height,
        ...
        std::lock_guard<std::mutex> lock(anchor_security_floors_mutex_);
        for (const auto& floor : effective_anchor_security_floors_) {
            const uint64_t carrier = floor.authorization_record.carrier.height;
            ...
            if (current_tip_height >= carrier && common_ancestor_height < carrier)
                return false;
        }
        return true;
    }
    ...
    static bool EffectiveAnchorSecurityPermanentExactlyMatches_(
        ...
        const auto expected_record_digest =
            ::veld::finality::qc::RecordDigest(floor.authorization_record);
        const auto& entry = permanent.entry;
        return entry.veld_block_hash == floor.target_hash &&
               entry.carrying_veld_height == floor.proof_carrier_height &&
               entry.carrying_veld_hash == floor.proof_carrier_hash &&
               entry.btc_block_hash == floor.btc_block_hash && entry.btc_txid == floor.btc_txid &&
               entry.authorization_veld_height == floor.authorization_record.carrier.height &&
               entry.authorization_veld_hash == floor.authorization_record.carrier.hash &&
               entry.authorization_finality_digest == expected_record_digest &&
               ::veld::finality::qc::RecordDigest(permanent.authorization_record) ==
                   expected_record_digest;
    }

    void ObserveExactAnchorSecurityReconstruction_(uint64_t applied_height) {
        const auto permanent = anchors_.Permanent();
        if (!permanent)
            return;
        std::lock_guard<std::mutex> lock(anchor_security_floors_mutex_);
        for (auto& floor : effective_anchor_security_floors_) {
            if (floor.exact_reconstruction_observed ||
                applied_height != floor.authorization_record.carrier.height)
                continue;
            if (EffectiveAnchorSecurityPermanentExactlyMatches_(floor, *permanent)) {
    ...
    void RestoreExactAnchorSecurityReconstructions_(const std::vector<bool>& observed) {
        std::lock_guard<std::mutex> lock(anchor_security_floors_mutex_);
        if (observed.size() != effective_anchor_security_floors_.size())
            throw std::logic_error("anchor security reconstruction snapshot shape changed");
        for (size_t i = 0; i < observed.size(); ++i)
            effective_anchor_security_floors_[i].exact_reconstruction_observed = observed[i];
    }

    bool EffectiveAnchorSecurityPermanentCovers_(const EffectiveAnchorSecurityFloor& floor) const {
        const auto permanent = anchors_.Permanent();
        if (!permanent)
            return false;
        if (permanent->target_height > floor.target_height) {
        ...
        return EffectiveAnchorSecurityPermanentExactlyMatches_(floor, *permanent);
    }

    EffectiveAnchorSecurityStatus EffectiveAnchorSecurityStatusNoTransition_() const {
        if (anchor_floor_security_uncertain_.load(std::memory_order_acquire))
        ...
        const uint64_t tip = chain_.Height();
        const auto reached_pin_matches = [this, tip](uint64_t height, const Hash256& expected) {
            if (tip < height)
                return true;
            return chain_.GetBlockHashAtHeight(height) == HashToHex(expected);
        };
        bool pending = false;
        for (const auto& floor : effective_anchor_security_floors_) {
            if (!reached_pin_matches(floor.target_height, floor.target_hash) ||
                !reached_pin_matches(floor.proof_carrier_height, floor.proof_carrier_hash) ||
                !reached_pin_matches(floor.authorization_record.carrier.height,
    ...
    static const char* EffectiveAnchorSecurityStatusName_(EffectiveAnchorSecurityStatus status) {
        switch (status) {
        case EffectiveAnchorSecurityStatus::None:
            return "none";
        case EffectiveAnchorSecurityStatus::Pending:
            return "pending";
        case EffectiveAnchorSecurityStatus::Satisfied:
            return "satisfied";
        case EffectiveAnchorSecurityStatus::Mismatched:
            return "mismatched";
        }
        return "mismatched";
    }

    void VerifyEffectiveAnchorSecurityStoredPins_(uint64_t stored_tip_height) {
        std::lock_guard<std::mutex> lock(anchor_security_floors_mutex_);
        for (const auto& floor : effective_anchor_security_floors_) {
            ...
            for (const auto& pin : pins) {
                if (pin.first > stored_tip_height)
                    continue;
                ...
                if (!stored || *stored == HashToHex(pin.second))
                    continue;
                throw std::runtime_error(
    ...
    void RequireEffectiveAnchorSecurityAfterReplay_(uint64_t replayed_tip_height) const {
        std::lock_guard<std::mutex> lock(anchor_security_floors_mutex_);
        for (const auto& floor : effective_anchor_security_floors_) {
            if (replayed_tip_height < floor.authorization_record.carrier.height)
                continue;
            if (!EffectiveAnchorSecurityPermanentCovers_(floor)) {
                throw std::runtime_error(
    ...
    std::string BtcVeldSupplySnapshotJson_() const {
        ...
        std::ostringstream j;
        j << "{\"supply_sats\":" << snapshot->supply_sats << ",\"tip\":" << snapshot->tip
    ...
    SerializeUTXODelta_(const Blockchain::UTXODelta& delta) {
        ...
        for (const auto& entry : delta) {
            ...
            if (entry.value)
                value = SerializeUTXO(*entry.value);
    ...
    std::string InspectRtp1MintJson_(const std::string& encoded) {
        ...
        uint64_t amount = 0;
        const auto refusal =
            BtcVeldMintTemplatePolicy(tx, issuer_script, from, to, amount, memo, &context);
        ...
        if (accepted.size() != 1 || !accepted[0])
            throw std::runtime_error(
                "RTP1 template does not pass current token and Bitcoin validation");
        const auto proof =
            HexToBytes(memo.substr(std::strlen(btcveld::reserve::ISSUER_MEMO_PREFIX)));
        ...
        return JB::Object(
            {{"version", JB::Number(uint64_t(1))},
             {"proof_version", JB::String("RTP1")},
             {"tip", JB::Number(tip.height)},
             {"tip_hash", JB::String(HashToHex(tip.GetHash()))},
             ...
             {"issuer", JB::String(from)},
             {"recipient", JB::String(to)},
             {"sats", JB::Number(amount)},
             {"fee_units", JB::Number(input_total - output_total)},
             ...
             {"reserve_prior_state_hex",
              JB::String(BytesToHex(btcveld::reserve::EncodeState(prior)))},
    ...
    BtcVeldMintProofStatus BuildMintNullifierStatus_(const std::string& outpoint) {
        ...
        for (int attempt = 0; attempt < 3; ++attempt) {
            try {
                ...
                return index.BuildStatusFromQuiescentSample(chain_, outpoint, [this] {
                    ...
                    if (!mint_nullifier_index_healthy_.load(std::memory_order_acquire))
                        throw std::runtime_error("btcVELD nullifier proof index is degraded; "
                ...
                mint_nullifier_index_healthy_.store(false, std::memory_order_release);
                throw;
    ...
    std::string BuildRedeemPageJson_(const std::vector<std::string>& params) {
        ...
        if (!redeem_index_healthy_.load(std::memory_order_acquire))
            throw std::runtime_error("btcVELD redeem index is degraded; restart/replay required");
        ...
        if (params.size() >= 2 && !params[1].empty()) {
            try {
                ...
                throw std::invalid_argument("getbtcveldredeems limit must be 1..1000");
            }
        }
        ...
        for (int attempt = 0; attempt < 3; ++attempt) {
            ...
            const auto commitment = onchain_tokens_.GetBtcVeldRedeemCommitment();
            uint64_t fin = FinalHeight();
            Block current_tip;
            const bool genesis_empty_commitment =
                page.tip == 0 && commitment.processed_height == 0 &&
                HashIsZero(commitment.processed_block_hash) && commitment.count == 0 &&
            ...
            if (!commitment_at_page_tip || !chain_.TryTip(current_tip) ||
                current_tip.height != page.tip || current_tip.GetHash() != page.tip_hash ||
                fin > page.tip)
                continue;

            std::ostringstream j;
            j << "{\"tip\":" << page.tip << ",\"tip_hash\":\"" << HashToHex(page.tip_hash) << "\""
              << ",\"final_height\":" << fin << ",\"redeem_count\":" << commitment.count
              << ",\"redeem_root\":\"" << HashToHex(commitment.root) << "\""
              << ",\"cursor\":\"" << OnChainTokenLedger::JsonEscape(cursor) << "\""
              << ",\"next_cursor\":\""
              << OnChainTokenLedger::JsonEscape(page.next_cursor.empty() ? cursor
                                                                         : page.next_cursor)
              << "\""
              << ",\"has_more\":" << (page.has_more ? "true" : "false")
              << ",\"page_limit\":" << limit << ",\"redeems\":[";
            bool first = true;
            for (const auto& indexed : page.items) {
                if (!first)
                    j << ',';
                std::string item = onchain_tokens_.TransferToJSON(indexed.record);
                if (!item.empty() && item.back() == '}')
                    item.pop_back();
                j << item << ",\"block_hash\":\"" << HashToHex(indexed.block_hash) << "\"}";
                first = false;
            }
            j << "]}";
            return j.str();
        }
        throw std::runtime_error("chain changed while reading btcVELD redeem page; retry");
    }
    ...
    static bool FeedBtcHeaders_(btcspv::BtcHeaderChain& ch, const Block& b) {
        if (!BtcVeldSpvActive(b.height))
            return true;
        for (const auto& tx : b.transactions)
            for (const auto& out : tx.outputs) {
                std::string d = ParseOpReturn(out.script_pubkey);
                if (d.rfind("VELD_BHDR|", 0) != 0)
                    continue;
                ...
                const int accepted = btcspv::ApplyBtcHeaderOp(ch, raw.data(), raw.size(),
                                                              (uint32_t)b.header.timestamp);
    ...
    static bool
    FeedAnchors_(btcanchor::AnchorSet& aset, const btcspv::BtcHeaderChain& btc, const Block& b,
                 ::veld::finality::qc::FinalityState& finality_state,
                 const std::function<bool(uint64_t, Hash256&)>& resolve_applied_ancestor) {
        ...
        if (!finality_state.AnchorWarmupComplete() || finality_state.record.IsNull() ||
            !BtcVeldAnchorActive(finality_state.record.target.height))
            return true;
        for (const auto& tx : b.transactions)
            for (const auto& out : tx.outputs) {
                std::string d = ParseOpReturn(out.script_pubkey);
                if (d.rfind("VELD_ANCHOR|", 0) != 0)
                    continue;
                std::vector<uint8_t> raw = BtcVeldHex_(d.c_str() + 12);
                if (raw.empty())
                    continue;
                btcanchor::AnchorResult r =
                    btcanchor::VerifyAnchor(btc, raw.data(), raw.size(), BTCVELD_ANCHOR_BTC_CONFS);
                if (!r.ok)
                    continue;
                ...
                if (r.veld_height > fin)
                    continue;
                if (!BtcVeldAnchorTargetValid(finality_state.record, r.veld_height,
                                              r.veld_block_hash, b.height))
                    continue;
                ...
                if (!btcanchor::MatchesAppliedVeldBranch(r, b, resolve_applied_ancestor))
                    continue;
                ...
                if (!BtcVeldAnchorTargetInWindow(r.veld_height, b.height))
                    continue;
                ...
                // partition locks.
                (void)aset.Stage(r.veld_height, r.veld_block_hash, b.height, b.GetHash(),
                                 r.btc_block_hash, r.btc_txid);
            }
        return true;
    }
    ...
    static bool PromoteFinalizedAnchors_(
        btcanchor::AnchorSet& aset, btcspv::BtcHeaderChain& btc,
        ::veld::finality::qc::FinalityState& finality_state,
        const std::function<bool(uint64_t, Hash256&)>& resolve_applied_ancestor) {
        const auto& authorization = finality_state.AnchorAuthorizationRecord();
        if (authorization.IsNull() || !BtcVeldAnchorActive(authorization.target.height))
            return true;
        const auto promotion = aset.PromoteFinalized(
            ...
            [&btc](const btcspv::H256& btc_block_hash) {
                return btc.IsFinal(btc_block_hash, BTCVELD_ANCHOR_BTC_CONFS);
            },
            authorization);
        if (!promotion.ok)
            return false;
        if (promotion.promoted != 0) {
            ...
            if (!permanent || !btc.PromoteObservedCheckpoint(permanent->entry.btc_block_hash))
                return false;
    ...
    void RestoreAllModules(const AllModuleSnapshots& s) {
        ...
        btc_relay_fresh_snapshot_.store(s.btc_relay_fresh, std::memory_order_release);
        peg_gate_snapshot_.store(s.peg_gate_snapshot, std::memory_order_release);
        last_pool_flush_height_.store(s.last_pool_flush_height, std::memory_order_release);
        RestoreExactAnchorSecurityReconstructions_(s.anchor_security_exact_observed);
    }
    ...
    void CaptureModuleCheckpoint_(uint64_t height, const Hash256& hash, uint64_t supply,
                                  bool force = false) {
        if (!force && (height % MAX_REORG_DEPTH) != 0)
            return;
        InstallModuleCheckpoint_(height, hash, supply, SnapshotAllModules());
    }

    void InstallModuleCheckpoint_(uint64_t height, const Hash256& hash, uint64_t supply,
                                  const AllModuleSnapshots& modules) {
        for (auto it = module_checkpoints_.begin(); it != module_checkpoints_.end();) {
            if (it->height == height && it->hash == hash)
                it = module_checkpoints_.erase(it);
            else
                ++it;
        }
        module_checkpoints_.push_back(ModuleCheckpoint{height, hash, supply, modules});
        while (module_checkpoints_.size() > MODULE_CHECKPOINT_RETAIN)
            module_checkpoints_.pop_front();
    }

    const ModuleCheckpoint*
    FindCanonicalModuleCheckpoint_(uint64_t max_height,
                                   bool chain_mutex_already_held = false) const {
        for (auto it = module_checkpoints_.rbegin(); it != module_checkpoints_.rend(); ++it) {
    ...
    std::optional<std::vector<uint8_t>> reorg_durable_publication_restore_wire_;
    std::optional<db::VeldDB::ReorgUtxoPending> reorg_utxo_pending_;
    ...
    std::optional<db::VeldDB::ReorgUtxoPending> reorg_derived_index_frame_;
    ...
    std::vector<ReorgBlockPublicationStage> reorg_block_publication_stages_;
    ...
    std::vector<std::pair<std::string, uint64_t>> reorg_displaced_txindex_rows_;
    ...
    std::vector<::veld::finality::qc::QuorumCert> reorg_finality_retirements_;
    void CaptureReorgAnchorFloorForAbort_() {
        if (reorg_anchor_floor_restore_captured_)
            return;
        ...
        if (db_wire) {
            if (db_wire->empty() || db_wire->size() > db::VeldDB::MAX_ANCHOR_SECURITY_FLOOR_BYTES) {
                throw std::runtime_error("pre-reorg DB anchor floor exceeds its strict size bound");
            }
            std::string error;
            const auto decoded = btcanchor::floor_store::Decode(*db_wire, &error);
            if (!decoded || decoded->network_magic != config_.magic ||
                ...
                throw std::runtime_error(
                    "pre-reorg DB anchor floor is malformed or belongs to another chain: " + error);
            }
        }
        reorg_anchor_floor_restore_wire_ = std::move(db_wire);
        reorg_durable_publication_restore_wire_ = db_.ReadDurablePublicationPendingWire();
    ...
    void CaptureReorgDisplacedTxIndexRows_(uint64_t ancestor_height, const Hash256& ancestor_hash,
                                           const Block& old_tip) {
        reorg_displaced_txindex_rows_.clear();
        if (!txindex_enabled_)
            return;
        if (old_tip.height <= ancestor_height)
            throw std::runtime_error("reorg displaced txindex suffix is empty");
        ...
        for (uint64_t height = ancestor_height + 1;; ++height) {
            ...
            if (!indexed || !db::IsCanonicalHash256Text(*indexed))
                throw std::runtime_error("cannot stage displaced txindex: invalid old height row");
            const Hash256 hash = HexToHash(*indexed);
            const auto bytes = db_.ReadBlock(hash);
            if (!bytes)
                throw std::runtime_error("cannot stage displaced txindex: old block body missing");
            Block old_block;
            old_block.height = height;
            const size_t consumed = Block::Deserialize(*bytes, 0, old_block);
            if (consumed == 0 || consumed != bytes->size() || old_block.Serialize() != *bytes ||
                old_block.GetHash() != hash || old_block.header.prev_block_hash != prior) {
                throw std::runtime_error(
                    "cannot stage displaced txindex: old suffix is non-canonical");
            }
            for (const auto& tx : old_block.transactions) {
                reorg_displaced_txindex_rows_.emplace_back(HashToHex(tx.GetTxID()), height);
            }
            prior = hash;
            if (height == old_tip.height)
                break;
            if (height == UINT64_MAX)
                throw std::runtime_error("cannot stage displaced txindex: height overflow");
        }
        if (prior != old_tip.GetHash())
            throw std::runtime_error("displaced txindex suffix does not end at the old tip");
    }
    ...
    static const Hash256& GenesisHashBytes_() {
        static const Hash256 h = [] {
            ...
            for (size_t i = 0; i < 32 && GENESIS_HASH[2 * i] && GENESIS_HASH[2 * i + 1]; ++i)
                out[i] = (uint8_t)((nib(GENESIS_HASH[2 * i]) << 4) | nib(GENESIS_HASH[2 * i + 1]));
            return out;
        }();
        return h;
    }
    ...
    static bool ParseSignedVoteJson_(const std::string& j, ::veld::finality::qc::SignedVote& out) {
        namespace fq = ::veld::finality::qc;
        ...
        if (round > UINT32_MAX)
            return false;
        if (!num("src_h", src_h) || !num("tgt_h", tgt_h))
            return false;
        if (!str("set_root", set_root) || !str("src_hash", src_hash) ||
            !str("tgt_hash", tgt_hash) || !str("pubkey", pubkey) || !str("sig", sig))
            return false;
        if (pubkey.size() != 3904 || sig.size() != 6618)
            return false;

        if (!hex_to_hash(set_root, out.set_root))
            return false;
        if (!hex_to_hash(src_hash, out.source.hash))
            return false;
        if (!hex_to_hash(tgt_hash, out.target.hash))
            return false;
        out.epoch_id = epoch;
    ...
    bool MaybeSnapshotEpochFor_(uint64_t h, ValidatorRegistry& registry,
        ...
        const fq::EpochSnapshot snapshot = fq::BuildEpochSnapshot(registry, next_epoch, h);
        return registry.RecordFinalitySnapshot(snapshot) && state.OnEpochBoundary(snapshot);
    }
    ...
    bool FeedFinalityCerts_(const Block& block, bool chain_mutex_already_held) {
        namespace fq = ::veld::finality::qc;
        auto r = fq::ParseFinalityBlock(
            block, [](const std::vector<uint8_t>& spk) { return ::veld::ParseOpReturn(spk); });
        if (r.status == fq::FinParseStatus::NONE)
            return true;
        if (r.status != fq::FinParseStatus::VALID || r.decoded.qc.phase != fq::Phase::PRECOMMIT ||
            !fq::InVoteWindow(r.decoded.qc.target.height, block.height))
            return false;

        auto it = fin_state_.snapshots.find(r.decoded.qc.epoch_id);
        if (it == fin_state_.snapshots.end())
            return false;
        ...
        if (!fq::VerifyDecodedQc(r.decoded, it->second, fq::NETWORK_ID, GenesisHashBytes_()))
            return false;
        ...
        if (canonical_target != r.decoded.qc.target.hash)
            return false;

        fq::CheckpointRef carrier;
        carrier.height = block.height;
        carrier.hash = block.GetHash();
        if (!fin_state_.OnCertificate(r.decoded.qc, carrier, canonical_target))
            return false;
        if (!validators_.RecordFinalityQc(r.decoded.qc, it->second, block.height))
            return false;
        return true;
    }
    ...
    bool DecodeDurableFinalityCarrier_(const Block& block,
    ...
    void RetireDurableFinalityCarrier_(const Block& block) noexcept {
        try {
            ...
            std::cerr << "  [finality] durable carrier vote retirement deferred: " << e.what()
                      << "\n";
            std::cerr.flush();
        } catch (...) {
    ...
    void StageReorgFinalityCarrier_(const Block& block) noexcept {
        try {
            ...
            std::cerr << "  [finality] reorg carrier retirement staging deferred: " << e.what()
                      << "\n";
            std::cerr.flush();
        } catch (...) {
    ...
    void RetireCompletedReorgFinalityCarriers_() noexcept {
        try {
            ...
            std::cerr << "  [finality] completed-reorg vote retirement deferred: " << e.what()
                      << "\n";
            std::cerr.flush();
        } catch (...) {
    ...
    void UpdateFinality_(uint64_t h, bool chain_mutex_already_held) {
        (void)h;
        (void)chain_mutex_already_held;
        final_height_.store(fin_state_.FinalizedHeight(), std::memory_order_release);
    }
    ...
    std::vector<std::vector<uint8_t>> PendingFinalityCoinbaseMetadata_(uint64_t candidate_height) {
        auto transition = chain_.AcquireConsensusTransitionGuard();
    ...
    PendingFinalityCoinbaseMetadataUnderTransition_(uint64_t candidate_height) {
        namespace fq = ::veld::finality::qc;
        std::vector<std::vector<uint8_t>> out;
        if (!fin_state_.FinalityActive())
            return out;

        std::lock_guard<std::mutex> g(fin_assembler_mu_);
        for (auto it = fin_state_.snapshots.rbegin(); it != fin_state_.snapshots.rend(); ++it) {
            auto cert = fin_assembler_.TryAssemble(
                it->second, fq::NETWORK_ID, GenesisHashBytes_(), fq::Phase::PRECOMMIT,
                [this, candidate_height](const fq::DecodedQc& d) {
                    if (!fq::InVoteWindow(d.qc.target.height, candidate_height))
                        return false;
                    try {
                        if (chain_.GetBlock(d.qc.target.height).GetHash() != d.qc.target.hash)
                            return false;
    ...
    static BtcVeldPegGateState PegGateForState_(const ::veld::finality::qc::FinalityState& state,
                                                uint64_t current_height,
                                                const btcspv::BtcHeaderChain* btc_headers = nullptr,
                                                uint64_t candidate_timestamp = 0) {
        const bool launch_active =
            BTCVELD_ISSUER_ADDRESS[0] != '\0' && current_height >= BTCVELD_ACTIVATION_HEIGHT;
        const bool finality_live =
            state.finality_ever_active &&
            ::veld::finality::qc::FinalityLive(current_height, state.FinalizedHeight());
        ...
        BtcVeldPegGateState gate =
            DeriveBtcVeldPegGate(launch_active, state.finality_ever_active, finality_live,
                                 /*amm_continues_during_later_stall=*/true);
        if (gate.mint_live && btcspv::EXTERNAL_VALUE_FRESHNESS_REQUIRED &&
            (!btc_headers || !btc_headers->ExternalValueFresh(candidate_timestamp)))
    ...
    static bool ValidateAmmBlockWithModulePrefix_(
        ...
        const btcspv::BtcHeaderChain& btc_parent, const btcanchor::AnchorSet& anchors_parent,
        const ::veld::finality::qc::FinalityState& finality_parent, const Block& block,
        const std::function<bool(uint64_t, Hash256&)>& resolve_applied_ancestor) {
        try {
            btcspv::BtcHeaderChain btc_preview(BtcVeldCheckpoint(), BtcVeldPowLimit(), 2016,
                                               1209600, BtcVeldNoRetarget());
            ...
            if (!FeedBtcHeaders_(btc_preview, block))
                return false;
            if (!FeedAnchors_(anchors_preview, btc_preview, block, finality_preview,
                              resolve_applied_ancestor))
                return false;
            const BtcVeldPegGateState peg_gate = PegGateForState_(
                finality_preview, block.height, &btc_preview, block.header.timestamp);
            ...
            tokens_preview.SetBtcVeldRedeemCovenant(tokens_parent.GetBtcVeldRedeemCovenant());
            if (!tokens_preview.ProcessBlock(block, peg_gate))
                return false;

            return amm_parent.ValidateBlock("VELD:btcVELD", block, tokens_preview, peg_gate,
  ...
  public:
...
#endif
    bool TestPrecheckFinalityVoteWire(const std::vector<uint8_t>& wire) const {
        return PrecheckFinalityVoteWire_(wire);
    }

    net::NodeServer::FinalityVoteVerifyResult
    TestVerifyFinalityVoteWire(const std::vector<uint8_t>& wire) {
        return VerifyFinalityVoteWire_(wire);
    }
    ...
    net::NodeServer::FinalityVoteVerifyResult
    TestVerifyFinalityVoteWireWithHeldTransition(const std::vector<uint8_t>& wire) {
        auto transition = chain_.AcquireConsensusTransitionGuard();
        return VerifyFinalityVoteWire_(wire, /*caller_holds_consensus_transition=*/true);
    }

    void TestSetFinalityEvidencePersistOverride(
        std::function<FinalityEvidencePersistStatus(const std::string&)> override_fn) {
        std::lock_guard<std::mutex> gate(fin_equivocation_gate_mu_);
        fin_equivocation_persist_override_ = std::move(override_fn);
    }

    void TestLoadFinalityEvidence() {
        LoadFinalityEvidence_();
    }
    ...
    bool TestInstallFinalityEvidenceSnapshot(const ::veld::finality::qc::EpochSnapshot& snapshot) {
        if (!validators_.RecordFinalitySnapshot(snapshot))
            return false;
        auto transition = chain_.AcquireConsensusTransitionGuard();
    ...
    void TestPoisonFinalityReplayState() {
        ...
        peg_gate_snapshot_.store(PEG_GATE_FINALITY_EVER_ACTIVE_BIT_ | uint64_t{123},
                                 std::memory_order_release);
    }
    ...
    void TestActivatePegValidatorFloorWithoutCertificate() {
        ...
        peg_gate_snapshot_.store(PEG_GATE_FINALITY_EVER_ACTIVE_BIT_, std::memory_order_release);
    }

    bool TestFinalityReplayStateIsCold() const {
        return fin_state_.consecutive_qualified_epochs == 0 && !fin_state_.finality_ever_active &&
               !fin_state_.ever_promoted_anchor && fin_state_.snapshots.empty() &&
               fin_state_.record.IsNull() && final_height_.load(std::memory_order_acquire) == 0 &&
    ...
    static bool TestValidateAmmBlockWithModulePrefix(
        ...
        return ValidateAmmBlockWithModulePrefix_(amm_parent, tokens_parent, btc_parent,
                                                 anchors_parent, finality_parent, block, resolver);
    }
    ...
    bool TestInstallAnchorFinalityParent(const ::veld::finality::qc::FinalityState& parent) {
        auto transition = chain_.AcquireConsensusTransitionGuard();
        ...
        const uint64_t frame =
            ...
            (fin_state_.finality_ever_active ? PEG_GATE_FINALITY_EVER_ACTIVE_BIT_ : 0);
    ...
    TestAnchorFinalityStatus TestReadAnchorFinalityStatus() const {
        auto transition = chain_.AcquireConsensusTransitionGuard();
        return TestAnchorFinalityStatus{anchors_.SnapshotState(), fin_state_, anchors_.Digest(),
                                        fin_state_.Digest()};
    }
    ...
    bool TestCaptureAnchorFinalityCheckpoint() {
        auto transition = chain_.AcquireConsensusTransitionGuard();
        const Block tip = chain_.TipCopy();
        if (tip.height != last_token_height_)
            return false;
        CaptureModuleCheckpoint_(tip.height, tip.GetHash(), chain_.TotalSupplyUnits(),
                                 /*force=*/true);
        return true;
    }
  ...
  private:
    ...
    BtcVeldPegGateState PegGateForHeight_(uint64_t current_height) const {
        const uint64_t frame = peg_gate_snapshot_.load(std::memory_order_acquire);
        const bool finality_ever_active = (frame & PEG_GATE_FINALITY_EVER_ACTIVE_BIT_) != 0;
        const uint64_t finalized = frame & ~PEG_GATE_FINALITY_EVER_ACTIVE_BIT_;
        const bool launch_active =
            BTCVELD_ISSUER_ADDRESS[0] != '\0' && current_height >= BTCVELD_ACTIVATION_HEIGHT;
        const bool finality_live =
            finality_ever_active && ::veld::finality::qc::FinalityLive(current_height, finalized);
        BtcVeldPegGateState gate =
            DeriveBtcVeldPegGate(launch_active, finality_ever_active, finality_live,
                                 /*amm_continues_during_later_stall=*/true);
        if (gate.mint_live && !btc_relay_fresh_snapshot_.load(std::memory_order_acquire))
            gate.mint_live = false;
        return gate;
    }
    ...
    bool FeedRedeemCovenant_(btcveld::SignerBondCovenant& cov, OnChainTokenLedger& led,
        ...
        if (!BtcVeldRedeemActive(b.height) && !btcveld::reserve::TRANSITION_V1_REQUIRED)
            return true;
        // (a) signer bonding ops: "VELD_SBOND|register|<addr>|<bond_sats>"
        for (const auto& tx : b.transactions)
            for (const auto& out : tx.outputs) {
                std::string d = ParseOpReturn(out.script_pubkey);
                if (d.rfind("VELD_SBOND|", 0) != 0)
                    continue;
                if (SecurityStateMigrationActive(b.height))
                    return false;
                std::vector<std::string> f;
                {
                    std::string cur;
                    for (char c : d.substr(11)) {
                        if (c == '|') {
                            f.push_back(cur);
                            cur.clear();
                        } else
                ...
                if (f.size() == 3 && f[0] == "register" && !f[1].empty()) {
                    uint64_t amt = 0;
        ...
        for (const auto& payout : led.LastBlockReservePayoutsCopy()) {
            if (!cov.MarkAuthorizedReservePayout(payout.request_id, payout.payout_txid,
        ...
        for (const auto& r : led.LastBlockRedeems()) {
            ...
            if (idb.size() == 32)
                std::memcpy(req.request_id.data(), idb.data(), 32);
            req.amount_sats = (uint64_t)r.amount;
            req.dest_spk =
                BtcVeldHex_(r.memo.c_str()); // dest BTC scriptPubKey (hex in the redeem memo)
            ...
            req.status =
                btcveld::ReqStatus::LOCKED_IN; // accepted burn is on-chain; SLA is the honor window
            req.request_commitment = btcveld::SignerBondCovenant::RequestCommitment(req);
            cov.AddRequest(req,
        ...
        for (const auto& tx : b.transactions)
            for (const auto& out : tx.outputs) {
                ...
                const auto payout = btcveld::VerifyAndResolvePayout(
                    cov, btc, proof.data(), proof.size(), BTCVELD_SPV_K_BTC, fulfilled, sv,
                    BtcVeldCustodySpk());
                if (!payout.ok || (!fulfilled && !sv.slash))
                    return false;
                if (sv.slash) {
                    cov.ApplySlash(sv); // proven-but-wrong payout
                    if (sv.compensate_sats && !sv.compensate_to.empty())
                        if (!led.CompensateMint(sv.compensate_to, sv.compensate_sats, b.height,
                                                HashToHex(b.GetHash())))
            ...
            for (const auto& out : tx.outputs) {
                ...
                if constexpr (btcveld::reserve::TRANSITION_V1_REQUIRED) {
                    const auto fraud = btcveld::VerifyFraudulentSpend(
                        btc, proof.data(), proof.size(), BtcVeldCustodySpk(), BTCVELD_SPV_K_BTC);
                    if (!fraud.ok || cov.IsConsumedPayout(fraud.spend_txid) ||
                        cov.IsConsumedFraudSpend(fraud.spend_txid))
                        return false;
                    const auto classification =
                        led.ClassifyBtcVeldReserveSpend(fraud.spend_tx, fraud.direct_parents);
                    if (classification.disposition ==
                        ...
                        if (classification.operation == btcveld::reserve::Operation::PAYOUT) {
                            BtcVeldReservePayoutTransition applied;
                            if (!led.ApplyFsp2AuthorizedReservePayout(
                                    fraud.spend_block, fraud.spend_merkle_directions,
                                    fraud.spend_merkle_branch, fraud.spend_tx, fraud.direct_parents,
                                    applied) ||
                                !cov.MarkAuthorizedReservePayout(
                                    applied.request_id, applied.payout_txid, applied.principal_sats,
                    ...
                    if (classification.disposition !=
                        btcveld::reserve::SpendDisposition::UNAUTHORIZED_SPEND)
                        return false;
                    btcveld::SlashVerdict classified;
                    classified.slash = true;
                    classified.reason = btcveld::SlashReason::FRAUDULENT_SPEND;
                    classified.slash_sats = std::min(fraud.custody_value, cov.TotalActiveBond());
                    classified.diagnostic = "current canonical reserve spent without a valid "
                                            "state-bound reserve transition";
                    if (!cov.ConsumeFraudSpend(fraud.spend_txid) ||
                        !led.FreezeBtcVeldReserve(fraud.spend_txid, fraud.spend_block))
                        return false;
                ...
                btcveld::SlashVerdict sv;
                const auto fraud =
                    btcveld::VerifyAndResolveFraud(cov, btc, proof.data(), proof.size(),
                                                   BtcVeldCustodySpk(), BTCVELD_SPV_K_BTC, sv);
                if (!fraud.ok || !sv.slash)
                    return false;
                cov.ApplySlash(sv);
            }
        ...
        for (const auto& id : cov.DueForDefault(b.height)) {
            ...
            if (v.slash) {
                ...
                if (v.compensate_sats && !v.compensate_to.empty())
                    if (!led.CompensateMint(v.compensate_to, v.compensate_sats, b.height,
                                            HashToHex(b.GetHash())))
    ...
    static std::string DStateQualificationAddress_(uint64_t index) {
        ...
        for (size_t i = 0; i < 8; ++i)
            payload[12 + i] = static_cast<uint8_t>(index >> (56 - 8 * i));
        const Hash256 mix = Hash256d(std::string("dstate-p2pkh:161:") + std::to_string(index));
    ...
    static bool ParseDStateQualificationCount_(const std::string& text, uint64_t& out) {
    ...
    bool ApplyDStateQualificationCapacity_(uint64_t token_accounts, uint64_t lp_identities) {
        if (token_accounts == 0 || token_accounts > 100'000 || lp_identities == 0 ||
        ...
        if (token_before.tokens.size() > 1 ||
            (!token_before.tokens.empty() && token == token_before.tokens.end()) ||
            (token != token_before.tokens.end() &&
             (token->second.id != BTCVELD_TOKEN_ID || token->second.name != "Wrapped Bitcoin" ||
            ...
            (!token_before.supply.empty() && supply == token_before.supply.end()) ||
        ...
        if (pool != amm_before.pools.end()) {
            ...
            if (!p.exists || p.reserve_veld != 0 || p.reserve_btcveld != 0 || p.lp_supply != 0 ||
                p.locked_lp != 0 || p.anchor_veld != 0 || p.anchor_btcveld != 0 || p.utxo_valid ||
        ...
        for (uint64_t i = 0; i < token_accounts; ++i) {
            if (!onchain_tokens_.ApplyDStateQualificationCredit(DStateQualificationAddress_(i)))
                return false;
        }
        for (uint64_t i = 0; i < 2'000; ++i) {
            TokenTransferRecord record{};
            record.txid = HashToHex(Hash256d("dstate-history:" + std::to_string(i)));
            record.token_id = BTCVELD_TOKEN_ID;
            record.from = DStateQualificationAddress_(i % token_accounts);
            record.to = DStateQualificationAddress_((i + 1) % token_accounts);
            record.amount = OnChainTokenLedger::MIN_ACCOUNT_SATS;
            record.block_height = i + 1;
            record.timestamp = static_cast<std::time_t>(1'700'000'000 + i);
            record.memo = "dstate-bounded-history";
            if (!onchain_tokens_.ApplyDStateQualificationHistoryRecord(record))
                return false;
        }
        for (uint64_t i = 0; i < lp_identities; ++i) {
            if (!amm_.ApplyDStateQualificationLp("VELD:btcVELD",
        ...
        return token_after.balances.size() == token_accounts &&
               token_after.history.size() == 2'000 && final_supply != token_after.supply.end() &&
               final_supply->second ==
                   static_cast<int64_t>(token_accounts) * OnChainTokenLedger::MIN_ACCOUNT_SATS &&
               amm_after.lp.size() == lp_identities &&
               std::all_of(amm_after.lp.begin(), amm_after.lp.end(), [](const auto& entry) {
                   return entry.second >= AmmLedger::LP_MIN_POSITION;
               });
    }
    ...
    bool ApplyDStateQualificationCarrier_(const Block& block) {
        ...
        if (block.transactions.empty() || block.transactions.front().inputs.size() != 1 ||
            !block.transactions.front().IsCoinbase())
            return false;
        const auto& script_sig = block.transactions.front().inputs.front().script_sig;
        const std::string payload(script_sig.begin(), script_sig.end());
        if (payload.rfind(envelope_prefix, 0) != 0)
            return true;
        if (block.height == 0)
            return false;
        ...
        if (payload.rfind(exact_prefix, 0) != 0)
            return false;
        const std::string body = payload.substr(exact_prefix.size());

        static constexpr const char* capacity_prefix = "VELD_DSTATE_S6_CAPACITY_V1|";
        if (body.rfind(capacity_prefix, 0) == 0) {
            if (block.height != 400)
                return false;
            const std::string fields = body.substr(std::char_traits<char>::length(capacity_prefix));
            const size_t split = fields.find('|');
            if (split == std::string::npos || fields.find('|', split + 1) != std::string::npos)
                return false;
            uint64_t token_accounts = 0, lp_identities = 0;
            if (!ParseDStateQualificationCount_(fields.substr(0, split), token_accounts) ||
                !ParseDStateQualificationCount_(fields.substr(split + 1), lp_identities))
                return false;
            return ApplyDStateQualificationCapacity_(token_accounts, lp_identities);
        }

        static constexpr const char* q1_prefix = "VELD_DSTATE_Q1|";
        if (body.rfind(q1_prefix, 0) != 0)
            return false;
        const std::string fields = body.substr(std::char_traits<char>::length(q1_prefix));
        const size_t split = fields.find('|');
        if (split == std::string::npos || fields.find('|', split + 1) != std::string::npos)
            return false;
        const std::pair<std::string, std::string> carrier{fields.substr(0, split),
        ...
        if (!valid_field(carrier.first) || !valid_field(carrier.second) ||
            ...
            std::cerr << "  [dstate] invalid qualification carrier fields at h=" << block.height
                      << "\n";
            return false;
        }
        if (carrier.first != "-" &&
            !onchain_tokens_.ApplyDStateQualificationCredit(carrier.first)) {
            std::cerr << "  [dstate] token qualification credit rejected at h=" << block.height
                      << "\n";
            return false;
        }
        if (carrier.second != "-" &&
            !amm_.ApplyDStateQualificationLp("VELD:btcVELD", carrier.second)) {
            std::cerr << "  [dstate] AMM qualification LP rejected at h=" << block.height << "\n";
            return false;
        }
        return true;
    }
    ...
    bool ApplyBlockModules_(const Block& block, uint64_t total_supply_after,
                            bool persist_governance, bool publish_rpc_snapshots,
                            bool chain_mutex_already_held) {
#if defined(VELD_TEST_HOOKS) && defined(VELD_DSTATE_QUALIFICATION)
        const auto dstate_reject = [&block](const char* module) {
            std::cerr << "  [dstate] module rejected h=" << block.height << " module=" << module
                      << "\n";
            std::cerr.flush();
            return false;
        };
        ...
        if (publish_rpc_snapshots) {
            ...
            const uint64_t next_timestamp = block.header.timestamp <= UINT64_MAX - TARGET_BLOCK_TIME
                                                ? block.header.timestamp + TARGET_BLOCK_TIME
                                                : block.header.timestamp;
            btc_relay_fresh_snapshot_.store(btc_headers_.ExternalValueFresh(next_timestamp),
                                            std::memory_order_release);
        }
        ...
        if (!FeedAnchors_(anchors_, btc_headers_, block, fin_state_,
                          [this, chain_mutex_already_held](uint64_t height, Hash256& out) -> bool {
                              try {
        ...
        const BtcVeldPegGateState peg_gate =
            PegGateForState_(fin_state_, block.height, &btc_headers_, block.header.timestamp);
        ...
        if (!FeedRedeemCovenant_(bond_covenant_, onchain_tokens_, btc_headers_, block)) {
#if defined(VELD_TEST_HOOKS) && defined(VELD_DSTATE_QUALIFICATION)
        ...
        if (!amm_.ProcessBlock("VELD:btcVELD", block, onchain_tokens_, peg_gate)) {
#if defined(VELD_TEST_HOOKS) && defined(VELD_DSTATE_QUALIFICATION)
        ...
        if (!staking_.ProcessBlock(block))
            return false;
        validators_.SetTotalStaked(staking_.GetTotalStake());
        if (!validators_.ProcessBlock(
                block, [this](const std::string& addr) { return staking_.GetStake(addr); },
    ...
    static std::string CanonicalDerivedIndexTipMarker_(uint64_t height, const Hash256& hash) {
        return std::to_string(height) + ":" + HashToHex(hash);
    }

    bool ResetDerivedIndexPrefixForReplay_(const char* row_prefix, const char* completion_key,
                                           const char* label) {
        try {
            ...
            if (!db_.GetIndexDB().Write(invalidate))
                throw std::runtime_error(std::string(label) + " completion invalidation failed");
            ...
            for (;;) {
                ...
                std::string last_key;
                bool page_full = false;
                db_.GetIndexDB().IterateFrom(row_prefix, cursor,
                ...
                if (!stale.empty()) {
                    ...
                    for (const auto& key : stale)
                        erase.Delete(key);
                    if (!db_.GetIndexDB().Write(erase))
                        throw std::runtime_error(std::string(label) + " stale-row deletion failed");
                }
                if (!page_full)
                    break;
            ...
            std::cerr << "  [" << label << "] replay reset failed: " << e.what()
    ...
    void PrepareCanonicalDerivedIndexesForReplay_(uint64_t target_height,
                                                  const Hash256& target_hash) {
        const std::string expected = CanonicalDerivedIndexTipMarker_(target_height, target_hash);
        const auto prepare_one = [this, &expected](const char* marker_key, const char* row_prefix,
            ...
            try {
                ...
                std::cerr << "  [" << label << "] completion-marker read failed: " << e.what()
            ...
            if (ResetDerivedIndexPrefixForReplay_(row_prefix, marker_key, label)) {
                // Writable during offline replay, but no completion marker is
                // published until the complete canonical tip is verified.
                healthy.store(true, std::memory_order_release);
            }
        };
        prepare_one(RedeemObligationIndex::CLEANUP_COMPLETE_KEY, RedeemObligationIndex::PREFIX,
                    "redeem-index", redeem_index_healthy_);
        prepare_one(MintNullifierIndex::CLEANUP_COMPLETE_KEY, MintNullifierIndex::PREFIX,
                    "mint-nullifier-index", mint_nullifier_index_healthy_);
    }

    void FinishCanonicalDerivedIndexesAfterReplay_(uint64_t target_height,
                                                   const Hash256& target_hash) {
        const std::string marker = CanonicalDerivedIndexTipMarker_(target_height, target_hash);
        const bool redeem_ok = redeem_index_healthy_.load(std::memory_order_acquire);
        const bool mint_ok = mint_nullifier_index_healthy_.load(std::memory_order_acquire);
        db::WriteBatch complete;
        if (redeem_ok)
            complete.Put(RedeemObligationIndex::CLEANUP_COMPLETE_KEY, marker);
        if (mint_ok)
        ...
        try {
            if (!db_.GetIndexDB().Write(complete))
                throw std::runtime_error("derived replay completion batch returned false");
        } catch (const std::exception& e) {
            if (redeem_ok)
                redeem_index_healthy_.store(false, std::memory_order_release);
            if (mint_ok)
                mint_nullifier_index_healthy_.store(false, std::memory_order_release);
            std::cerr << "  [derived-index] replay completion failed: " << e.what()
                      << "; affected indexes remain closed\n";
            std::cerr.flush();
        } catch (...) {
            if (redeem_ok)
                redeem_index_healthy_.store(false, std::memory_order_release);
    ...
    bool CanonicalDerivedIndexMarkersMatch_(uint64_t height, const Hash256& hash) {
        try {
            const std::string expected = CanonicalDerivedIndexTipMarker_(height, hash);
            const auto redeem = db_.GetIndexDB().Get(RedeemObligationIndex::CLEANUP_COMPLETE_KEY);
            const auto mint = db_.GetIndexDB().Get(MintNullifierIndex::CLEANUP_COMPLETE_KEY);
            return redeem && mint && *redeem == expected && *mint == expected;
        } catch (...) {
            return false;
        }
    }

    void PersistCanonicalDerivedIndexes_(const std::vector<ReorgBlockPublicationStage>& stages,
                                         bool from_reorg = false, bool publish_txindex = true,
        ...
        for (const auto& stage : stages) {
            ...
            if (block.GetHash() != stage.hash)
                throw std::runtime_error("derived-index stage no longer names the canonical block");
        ...
        if (txindex_enabled_ && publish_txindex) {
            ...
            try {
                db::WriteBatch batch;
                if (from_reorg) {
                    for (const auto& [txid, old_height] : reorg_displaced_txindex_rows_) {
                        const std::string key = "txi:" + txid;
                        const auto current = db_.GetIndexDB().Get(key);
                        if (current && *current == std::to_string(old_height)) {
                            batch.Delete(key);
                        }
                    }
                }
                for (const auto& block : blocks) {
                    for (const auto& tx : block.transactions) {
                        batch.Put("txi:" + HashToHex(tx.GetTxID()), std::to_string(block.height));
                    }
                }
                ...
                batch.Put("txi:_complete_v3",
                          std::to_string(tip.height) + ":" + HashToHex(tip.GetHash()));
                ok = db_.GetIndexDB().Write(batch);
            } catch (...) {
                ok = false;
            }
            if (!ok && txindex_operational_.exchange(false, std::memory_order_acq_rel)) {
                std::cerr << "  [txindex] DISABLED: final canonical batch "
                             "publication failed at height "
                          << blocks.back().height << "; restart with --txindex to rebuild\n";
                std::cerr.flush();
            }
        }
        ...
        if (publish_completion_markers) {
            if (!redeem_index_healthy_.load(std::memory_order_acquire) ||
                !mint_nullifier_index_healthy_.load(std::memory_order_acquire))
                throw std::runtime_error(
                    "derived index is already unhealthy; full replay required");

            if (from_reorg) {
                if (!reorg_derived_index_frame_)
                    throw std::runtime_error("reorg derived-index cleanup frame is missing");
                ...
                if (stages.front().height != frame.ancestor_height + 1 ||
                    ...
                    !CanonicalDerivedIndexMarkersMatch_(frame.old_height, frame.old_hash))
                    throw std::runtime_error("reorg derived-index parent/frame marker mismatch");
                for (size_t i = 1; i < stages.size(); ++i) {
                    if (stages[i].height != stages[i - 1].height + 1 ||
                        blocks[i].header.prev_block_hash != stages[i - 1].hash)
                        throw std::runtime_error("reorg derived-index suffix is non-contiguous");
                }
                const uint64_t cleanup_tip = std::max(frame.old_height, frame.new_height);
                for (uint64_t height = frame.ancestor_height + 1; height <= cleanup_tip; ++height) {
                    std::optional<Hash256> canonical;
                    if (height <= frame.new_height) {
                        const size_t offset =
                            static_cast<size_t>(height - (frame.ancestor_height + 1));
                        if (offset >= stages.size() || stages[offset].height != height)
                            throw std::runtime_error("reorg derived-index cleanup height gap");
                        canonical = stages[offset].hash;
                    }
                    if (!redeem_index.AppendCanonicalHeightCleanupToBatch(derived_batch, height,
                                                                          canonical) ||
                        !mint_index.AppendCanonicalHeightCleanupToBatch(derived_batch, height,
                                                                        canonical))
                        throw std::runtime_error(
                ...
                if (stages.size() != 1)
                    throw std::runtime_error("linear derived-index parent marker mismatch");
                if (stages.front().height == 0) {
                    const auto redeem_marker =
                        db_.GetIndexDB().Get(RedeemObligationIndex::CLEANUP_COMPLETE_KEY);
                    const auto mint_marker =
                        db_.GetIndexDB().Get(MintNullifierIndex::CLEANUP_COMPLETE_KEY);
                    bool stale_redeem_row = false;
                    bool stale_mint_row = false;
                    db_.GetIndexDB().Iterate(RedeemObligationIndex::PREFIX,
                                             [&](const std::string&, const std::string&) {
                                                 stale_redeem_row = true;
                                                 return false;
                                             });
                    db_.GetIndexDB().Iterate(MintNullifierIndex::PREFIX,
                                             [&](const std::string&, const std::string&) {
                                                 stale_mint_row = true;
                                                 return false;
                                             });
                    if (stages.front().hash != CreateGenesisBlock().GetHash() ||
                        blocks.front().header.prev_block_hash != Hash256{} || redeem_marker ||
                        mint_marker || stale_redeem_row || stale_mint_row)
                        throw std::runtime_error("genesis derived-index initialization mismatch");
                } else if (!CanonicalDerivedIndexMarkersMatch_(
                               stages.front().height - 1, blocks.front().header.prev_block_hash)) {
                    throw std::runtime_error("linear derived-index parent marker mismatch");
                }
            }
        }
        ...
        for (size_t i = 0; i < stages.size(); ++i) {
            if (!redeem_index.AppendAcceptedToBatch(derived_batch, blocks[i], stages[i].redeems) ||
                !mint_index.AppendAcceptedToBatch(derived_batch, blocks[i],
                                                  stages[i].mint_transitions))
                throw std::runtime_error("derived-index accepted-row validation failed at h=" +
                                         std::to_string(stages[i].height));
        }

        if (publish_completion_markers) {
            const std::string marker =
                CanonicalDerivedIndexTipMarker_(stages.back().height, stages.back().hash);
            derived_batch.Put(RedeemObligationIndex::CLEANUP_COMPLETE_KEY, marker);
            derived_batch.Put(MintNullifierIndex::CLEANUP_COMPLETE_KEY, marker);
        }
        if (!derived_batch.IsEmpty() && !db_.GetIndexDB().Write(derived_batch))
            throw std::runtime_error("atomic redeem/nullifier derived-index batch returned false");
    }

    static bool AdvanceModuleSupply_(const Block& block, uint64_t& running_supply) noexcept {
    ...
    bool PreflightBlockModules_(const Block& block, uint64_t total_supply_after,
        ...
        if (dstate_memory_observer_ && (block.height % MAX_REORG_DEPTH) == 0)
            dstate_memory_observer_();
#endif
        try {
            const bool ok =
                ApplyBlockModules_(block, total_supply_after,
                                   /*persist_governance=*/false,
                                   /*publish_rpc_snapshots=*/false, chain_mutex_already_held);
    ...
    bool PreflightMiningCandidateUnderTransition_(const Block& block) {
        Block tip;
        if (!chain_.TryTip(tip) || block.height != tip.height + 1 ||
            block.header.prev_block_hash != tip.GetHash())
            return false;
        if (!chain_.ValidateMiningCoinbase(block))
            return false;
        uint64_t projected_supply = chain_.TotalSupplyUnits();
        if (!Blockchain::AdvanceCanonicalSupply(block, projected_supply))
            return false;
        return PreflightBlockModules_(block, projected_supply,
    ...
    void BuildAltOverlay(uint64_t anc_height) {
        const ModuleCheckpoint* checkpoint =
            FindCanonicalModuleCheckpoint_(anc_height, /*chain_mutex_already_held=*/true);
        if (!checkpoint) {
            throw std::runtime_error("no hash-bound module checkpoint covers reorg ancestor " +
        ...
        bond_covenant_alt_ =
            std::make_unique<btcveld::SignerBondCovenant>(); // fork-aware redeem covenant (fresh)
        WireRedeemArchive_(*bond_covenant_alt_);
        anchors_alt_ = std::make_unique<btcanchor::AnchorSet>();
        fin_state_alt_ =
            std::make_unique<::veld::finality::qc::FinalityState>(checkpoint->modules.fin_state);
        onchain_tokens_alt_->SetBtcHeaderChain(btc_headers_alt_.get()); // fork-aware SPV mint gate
        onchain_tokens_alt_->SetBtcVeldRedeemCovenant(bond_covenant_alt_.get());
    ...
    bool ResolveAltHash_(uint64_t height, Hash256& out) const {
        ...
        if (it != alt_chain_hashes_.end()) {
            out = it->second;
            return true;
        }
        if (height > alt_common_ancestor_height_)
            return false;
        try {
    ...
    bool FeedFinalityCertsAlt_(const Block& block) {
        namespace fq = ::veld::finality::qc;
        if (!fin_state_alt_ || !validators_alt_)
            return false;
        auto parsed = fq::ParseFinalityBlock(
            block, [](const std::vector<uint8_t>& spk) { return ::veld::ParseOpReturn(spk); });
        if (parsed.status == fq::FinParseStatus::NONE)
            return true;
        if (parsed.status != fq::FinParseStatus::VALID ||
            parsed.decoded.qc.phase != fq::Phase::PRECOMMIT ||
            !fq::InVoteWindow(parsed.decoded.qc.target.height, block.height))
            return false;
        auto snapshot = fin_state_alt_->snapshots.find(parsed.decoded.qc.epoch_id);
        if (snapshot == fin_state_alt_->snapshots.end())
            return false;
        if (!fq::VerifyDecodedQc(parsed.decoded, snapshot->second, fq::NETWORK_ID,
                                 GenesisHashBytes_()))
            return false;
        Hash256 canonical{};
        if (!ResolveAltHash_(parsed.decoded.qc.target.height, canonical) ||
            canonical != parsed.decoded.qc.target.hash)
            return false;
        fq::CheckpointRef carrier{block.height, block.GetHash()};
        if (!fin_state_alt_->OnCertificate(parsed.decoded.qc, carrier, canonical))
            return false;
        return validators_alt_->RecordFinalityQc(parsed.decoded.qc, snapshot->second, block.height);
    }

    bool AltProcessOne(const Block& b) {
        if (!validators_alt_ || !staking_alt_ || !onchain_tokens_alt_ || !amm_alt_ ||
            !btc_headers_alt_ || !bond_covenant_alt_ || !anchors_alt_ || !fin_state_alt_)
            return false;
        alt_chain_hashes_[b.height] = b.GetHash();
        if (!ValidatorRegistry::HasValidRegisterMultiplicity(b))
            return false;
        bond_covenant_alt_->BeginBlock(b.height);
        if (!AdvanceModuleSupply_(b, alt_running_supply_))
            return false;
        ...
        if (!FeedBtcHeaders_(*btc_headers_alt_, b))
            return false; // fork-aware BTC header view (before the mint gate)
        if (!FeedAnchors_(*anchors_alt_, *btc_headers_alt_, b, *fin_state_alt_,
                          [this](uint64_t h, Hash256& out) { return ResolveAltHash_(h, out); }))
            return false;
        const BtcVeldPegGateState peg_gate =
            PegGateForState_(*fin_state_alt_, b.height, btc_headers_alt_.get(), b.header.timestamp);
        if (!onchain_tokens_alt_->ProcessBlock(b, peg_gate))
            return false;
        if (!FeedRedeemCovenant_(*bond_covenant_alt_, *onchain_tokens_alt_, *btc_headers_alt_, b))
            return false;
        if (!amm_alt_->ProcessBlock("VELD:btcVELD", b, *onchain_tokens_alt_, peg_gate))
            return false;
        staking_alt_->SetTotalSupply(alt_running_supply_);
        if (!staking_alt_->ProcessBlock(b))
            return false;
        validators_alt_->SetTotalStaked(staking_alt_->GetTotalStake());
        if (!validators_alt_->ProcessBlock(
                b, [this](const std::string& a) { return staking_alt_->GetStake(a); },
                [this](const std::string& a, uint64_t hh) {
                    staking_alt_->ApplySlashBondLockup(a, hh);
                }))
            return false;
        if (!FeedFinalityCertsAlt_(b))
            return false;
        if (!PromoteFinalizedAnchors_(
                *anchors_alt_, *btc_headers_alt_, *fin_state_alt_,
                [this](uint64_t h, Hash256& out) { return ResolveAltHash_(h, out); }))
            return false;
        if (!MaybeSnapshotEpochFor_(b.height, *validators_alt_, *fin_state_alt_))
            return false;
        return true;
    }
    ...
    bool RebuildAddressHistoryIndex_() {
        ...
        try {
            ...
            if (!chain_.IsEmpty())
                tip = std::make_pair(chain_.Height(), chain_.TipCopy().GetHash());
            const bool ok = address_history::Rebuild(
                db_.GetIndexDB(), tip, [this](uint64_t height) { return chain_.GetBlock(height); },
    ...
    bool AdvanceAddressHistoryIndex_(const Block& block) {
        const bool was_ready = address_history_ready_.load(std::memory_order_acquire);
        CloseAddressHistory_();
        try {
            if (!was_ready)
                return RebuildAddressHistoryIndex_();
            const bool ok = address_history::Advance(db_.GetIndexDB(), block);
            if (ok)
                OpenAddressHistory_();
            return ok;
        } catch (...) {
    ...
    bool RollbackAddressHistoryIndex_(const Block& popped) {
        ...
        CloseAddressHistory_();
        try {
            const bool ok = address_history::Rollback(db_.GetIndexDB(), popped);
            if (ok)
                OpenAddressHistory_();
            return ok;
        } catch (...) {
    ...
    std::string AddressHistoryPageJson_(const std::string& address, size_t limit,
                                        const std::string& cursor) {
        if (!address_history_ready_.load(std::memory_order_acquire))
            throw std::runtime_error("address history index is rebuilding; retry shortly");
        uint32_t active = address_history_queries_.load(std::memory_order_relaxed);
        do {
            if (active >= 4)
                throw std::runtime_error("address history is busy; retry shortly");
...
#error "address-history interleave hook is test-only"
...
#endif
            if (!address_history::MarkersMatch(db_.GetIndexDB(), canonical_tip.height,
                                               canonical_tip.GetHash())) {
                CloseAddressHistory_();
                throw std::runtime_error("address history index is stale; rebuild required");
            }
            const uint64_t revision = address_history_revision_.load(std::memory_order_acquire);
            const auto snapshot =
                address_history::ReadPage(db_.GetIndexDB(), script, limit, cursor);
            if (!snapshot || !address_history_ready_.load(std::memory_order_acquire) ||
                address_history_revision_.load(std::memory_order_acquire) != revision ||
                !address_history::MarkersMatch(db_.GetIndexDB(), canonical_tip.height,
                                               canonical_tip.GetHash()))
                throw std::runtime_error("address history changed while reading; retry shortly");
            return snapshot;
        }();
        ...
        for (const auto& entry : page->entries) {
            rows.push_back(JsonBuilder::Object({
                ...
                {"net_veld",
                 JsonBuilder::Float(static_cast<double>(entry.net_units) / VELD_UNITS, 8)},
                {"fee_veld",
                 JsonBuilder::Float(static_cast<double>(entry.fee_units) / VELD_UNITS, 8)},
    ...
    static constexpr const char* MINER_ARCHIVE_COUNT_PREFIX_ = "miner:count:";
    static constexpr const char* MINER_ARCHIVE_LAST_PREFIX_ = "miner:last:";
    static constexpr const char* MINER_ARCHIVE_UNDO_PREFIX_ = "miner:undo:";
    static constexpr const char* MINER_ARCHIVE_HEIGHT_KEY_ = "miner:archive:tip_height";
    static constexpr const char* MINER_ARCHIVE_HASH_KEY_ = "miner:archive:tip_hash";
    ...
    static bool ParseMinerArchiveUint64_(const std::string& text, uint64_t& out) {
        if (text.empty() || (text.size() > 1 && text.front() == '0'))
            return false;
        uint64_t value = 0;
    ...
    static bool IsCanonicalMinerArchiveScript_(const std::string& script_hex) {
    ...
    static std::string MinerArchiveUndoHeightPrefix_(uint64_t height) {
        ...
        out << MINER_ARCHIVE_UNDO_PREFIX_ << std::setw(20) << std::setfill('0') << height << ':';
        return out.str();
    }

    static std::string MinerArchiveUndoKey_(uint64_t height, const std::string& script_hex) {
        return MinerArchiveUndoHeightPrefix_(height) + script_hex;
    }

    static std::string EncodeMinerArchiveUndo_(const Blockchain::MinerArchiveRecord& prior) {
        return std::to_string(prior.blocks_mined) + "," + std::to_string(prior.last_block_mined);
    }

    static bool ParseMinerArchiveUndo_(const std::string& text,
        ...
        if (!ParseMinerArchiveUint64_(text.substr(0, comma), count) ||
            !ParseMinerArchiveUint64_(text.substr(comma + 1), last) || (count == 0 && last != 0))
            return false;
        prior = Blockchain::MinerArchiveRecord{count, last};
        return true;
    }

    bool ReadMinerArchiveRecordRaw_(const std::string& script_hex,
                                    Blockchain::MinerArchiveRecord& record) {
        const auto count_raw =
            db_.GetIndexDB().Get(std::string(MINER_ARCHIVE_COUNT_PREFIX_) + script_hex);
        const auto last_raw =
            db_.GetIndexDB().Get(std::string(MINER_ARCHIVE_LAST_PREFIX_) + script_hex);
        if (!count_raw && !last_raw) {
            record = Blockchain::MinerArchiveRecord{};
            return true;
        }
        return count_raw && last_raw && ParseMinerArchiveUint64_(*count_raw, record.blocks_mined) &&
               ParseMinerArchiveUint64_(*last_raw, record.last_block_mined) &&
    ...
    bool DeleteMinerArchivePrefixBatched_(const std::string& prefix) {
        ...
        for (;;) {
            ...
            db_.GetIndexDB().IterateFrom(
                prefix, cursor, [&](const std::string& key, const std::string&) {
                    keys.push_back(key);
                    return keys.size() < MINER_ARCHIVE_REBUILD_BATCH_BLOCKS_;
                });
            if (keys.empty())
                return true;
            cursor = keys.back();
    ...
    ReadMinerArchiveRecord_(const std::string& script_hex) {
        if (!miner_archive_ready_.load(std::memory_order_acquire))
            return std::nullopt;
        const uint64_t revision = miner_archive_revision_.load(std::memory_order_acquire);
    ...
    std::optional<Hash256> MinerArchiveLogicalDigest_() {
        ...
        const uint64_t revision = miner_archive_revision_.load(std::memory_order_acquire);
        try {
            const auto height_raw = db_.GetIndexDB().Get(MINER_ARCHIVE_HEIGHT_KEY_);
            const auto hash_raw = db_.GetIndexDB().Get(MINER_ARCHIVE_HASH_KEY_);
            uint64_t tip_height = 0;
            if (!height_raw || !hash_raw || !ParseMinerArchiveUint64_(*height_raw, tip_height) ||
            ...
            db_.GetIndexDB().Iterate(MINER_ARCHIVE_LAST_PREFIX_,
                                     [&](const std::string&, const std::string&) {
            ...
            for (;;) {
                ...
                db_.GetIndexDB().IterateFrom(MINER_ARCHIVE_COUNT_PREFIX_, count_cursor,
                                             [&](const std::string& key, const std::string& value) {
                                                 counts.emplace_back(key, value);
                                                 return counts.size() <
                                                        MINER_ARCHIVE_REBUILD_BATCH_BLOCKS_;
                                             });
                db_.GetIndexDB().IterateFrom(MINER_ARCHIVE_LAST_PREFIX_, last_cursor,
                                             [&](const std::string& key, const std::string& value) {
                ...
                if (counts.size() != lasts.size())
                    throw std::runtime_error("miner archive row-page mismatch");
                ...
                for (size_t i = 0; i < counts.size(); ++i) {
                    ...
                    if (!IsCanonicalMinerArchiveScript_(script) ||
                        ...
                        throw std::runtime_error("miner archive row-key mismatch");
                    uint64_t count = 0, last = 0;
                    if (!ParseMinerArchiveUint64_(counts[i].second, count) ||
                        !ParseMinerArchiveUint64_(lasts[i].second, last) || count == 0 ||
                        last > tip_height)
                        throw std::runtime_error("invalid miner archive row");
    ...
    bool MinerArchiveMarkersMatch_(uint64_t height, const Hash256& hash) {
    ...
    bool RebuildMinerArchiveIndex_() {
        {
            ...
            try {
                ...
                const bool ok = miner_archive::Rebuild(
                    db_.GetIndexDB(), tip, [this](uint64_t h) { return chain_.GetBlock(h); },
        ...
        try {
            ...
            if (!db_.GetIndexDB().Write(invalidate) ||
                !DeleteMinerArchivePrefixBatched_(MINER_ARCHIVE_COUNT_PREFIX_) ||
                !DeleteMinerArchivePrefixBatched_(MINER_ARCHIVE_LAST_PREFIX_) ||
                !DeleteMinerArchivePrefixBatched_(MINER_ARCHIVE_UNDO_PREFIX_))
                return false;
            ...
            const Hash256 tip_hash = chain_.TipCopy().GetHash();
            std::unordered_map<std::string, Blockchain::MinerArchiveRecord> pending;
            ...
            auto flush = [&]() -> bool {
                ...
                for (const auto& script : scripts) {
                    ...
                    page.Put(std::string(MINER_ARCHIVE_COUNT_PREFIX_) + script,
                             std::to_string(record.blocks_mined));
                    page.Put(std::string(MINER_ARCHIVE_LAST_PREFIX_) + script,
                             std::to_string(record.last_block_mined));
                }
                for (const auto& [key, value] : pending_undo)
                    page.Put(key, value);
                if (!db_.GetIndexDB().Write(page))
                    return false;
                pending.clear();
                pending_undo.clear();
                blocks_buffered = 0;
            ...
            for (uint64_t h = 0; h <= tip_height; ++h) {
                ...
                for (const auto& script : Blockchain::MinerScriptsForArchive(block)) {
                    ...
                    if (found == pending.end()) {
                        ...
                        if (!ReadMinerArchiveRecordRaw_(script, current))
                            return false;
                    ...
                    if (tip_height - h <= MAX_REORG_DEPTH) {
                        pending_undo.emplace_back(MinerArchiveUndoKey_(h, script),
                                                  EncodeMinerArchiveUndo_(prior));
                    }
                }
                ++blocks_buffered;
                if (blocks_buffered >= MINER_ARCHIVE_REBUILD_BATCH_BLOCKS_ && !flush())
                    return false;
            }
            if (!flush())
                return false;
            if (chain_.Height() != tip_height || chain_.TipCopy().GetHash() != tip_hash)
                return false;

            veld::db::WriteBatch complete;
            complete.Put(MINER_ARCHIVE_HEIGHT_KEY_, std::to_string(tip_height));
    ...
    bool AdvanceMinerArchiveIndex_(const Block& block) {
        {
            const bool was_ready = miner_archive_ready_.load(std::memory_order_acquire);
            CloseMinerArchive_();
            if (!was_ready)
                return RebuildMinerArchiveIndex_();
            const auto result = miner_archive::Advance(db_.GetIndexDB(), block);
        ...
        try {
            ...
            if (block.height == 0) {
                ...
                parent_matches = ParseMinerArchiveUint64_(*stored_height, parsed_height) &&
            ...
            veld::db::WriteBatch batch;
            for (const auto& script : Blockchain::MinerScriptsForArchive(block)) {
                const std::string count_key = std::string(MINER_ARCHIVE_COUNT_PREFIX_) + script;
                const std::string last_key = std::string(MINER_ARCHIVE_LAST_PREFIX_) + script;
                auto count_raw = db_.GetIndexDB().Get(count_key);
                auto last_raw = db_.GetIndexDB().Get(last_key);
                Blockchain::MinerArchiveRecord prior;
                if (count_raw || last_raw) {
                    if (!count_raw || !last_raw ||
                        !ParseMinerArchiveUint64_(*count_raw, prior.blocks_mined) ||
                        !ParseMinerArchiveUint64_(*last_raw, prior.last_block_mined) ||
                        prior.blocks_mined == 0 || prior.blocks_mined == UINT64_MAX ||
            ...
            if (block.height > MAX_REORG_DEPTH) {
                const std::string expired =
                    MinerArchiveUndoHeightPrefix_(block.height - MAX_REORG_DEPTH - 1);
                db_.GetIndexDB().Iterate(expired, [&](const std::string& key, const std::string&) {
                    batch.Delete(key);
                    return true;
                });
            }
            batch.Put(MINER_ARCHIVE_HEIGHT_KEY_, std::to_string(block.height));
            batch.Put(MINER_ARCHIVE_HASH_KEY_, HashToHex(block.GetHash()));
    ...
    bool RollbackMinerArchiveIndex_(const Block& popped) {
        {
            if (!miner_archive_ready_.load(std::memory_order_acquire))
                return false;
            CloseMinerArchive_();
            const bool ok = miner_archive::Rollback(db_.GetIndexDB(), popped);
            if (ok)
                OpenMinerArchive_();
            return ok;
        }
        ...
        try {
            ...
            for (const auto& script : Blockchain::MinerScriptsForArchive(popped)) {
                const std::string count_key = std::string(MINER_ARCHIVE_COUNT_PREFIX_) + script;
                const std::string last_key = std::string(MINER_ARCHIVE_LAST_PREFIX_) + script;
                const std::string undo_key = MinerArchiveUndoKey_(popped.height, script);
                auto count_raw = db_.GetIndexDB().Get(count_key);
                auto last_raw = db_.GetIndexDB().Get(last_key);
            ...
            if (popped.height == 0) {
                ...
                batch.Put(MINER_ARCHIVE_HEIGHT_KEY_, std::to_string(popped.height - 1));
                batch.Put(MINER_ARCHIVE_HASH_KEY_, HashToHex(popped.header.prev_block_hash));
            }
            const bool ok = db_.GetIndexDB().Write(batch);
            if (ok)
                OpenMinerArchive_();
    ...
    void WireDB() {
        if (!anchor_floor_prepared_)
            throw std::logic_error("WireDB called before anchor security bootstrap preparation");
        ...
        chain_.SetHistoricalBlockLoader(
            [this](const Hash256& hash) -> std::optional<std::vector<uint8_t>> {
                return db_.ReadBlock(hash);
            });
        chain_.SetDurableBlockBodyWriter(
            [this](const Hash256& hash, const std::vector<uint8_t>& bytes) -> bool {
#ifdef VELD_TEST_HOOKS
                test_work_durable_writer_calls_.fetch_add(1, std::memory_order_acq_rel);
#endif
                return db_.WriteBlock(hash, bytes);
            });
        chain_.SetDurableBlockBodyEraser(
            [this](const Hash256& hash) -> bool { return db_.DeleteNonCanonicalBlock(hash); });
        chain_.SetMinerArchiveLookup(
            [this](const std::string& script_hex) -> std::optional<Blockchain::MinerArchiveRecord> {
                return ReadMinerArchiveRecord_(script_hex);
            });
        ...
        rpc_.SetMiningTemplateBuilderFn([this](const std::vector<uint8_t>& script) {
            ...
            const auto metadata =
                PendingFinalityCoinbaseMetadataUnderTransition_(settlements.height);
            auto result = MineOnly(
                chain_, mempool_, recipient, 0, nullptr, AddressToScript(POOL_ADDRESS), 1, nullptr,
                {}, nullptr, nullptr, settlements.transactions,
        ...
        chain_.SetAcceptedEndorsementQuery([this](const std::string& address, uint64_t height,
                                                  const std::string& block_hash_hex,
                                                  const std::string& sig_hex) -> bool {
            const auto* ov = Blockchain::alt_engine_overlay_;
            const ValidatorRegistry& vr = (ov && ov->validators) ? *ov->validators : validators_;
            return vr.HasAcceptedEndorsement(address, height, block_hash_hex, sig_hex);
        });

        chain_.SetStakeSnapshot([this](uint64_t query_height) -> std::map<std::string, uint64_t> {
            const auto* ov = Blockchain::alt_engine_overlay_;
            const StakingLedger& sl = (ov && ov->staking) ? *ov->staking : staking_;
        ...
        chain_.SetBondSettlementsFn(
            [this](uint64_t boundary_height)
                ...
                const auto* ov = Blockchain::alt_engine_overlay_;
                const ValidatorRegistry& vr =
                    (ov && ov->validators) ? *ov->validators : validators_;
                std::vector<std::tuple<std::string, int, uint64_t, std::string>> out;
                for (const auto& s : vr.GetBondSettlements(boundary_height)) {
                    out.emplace_back(s.address,
                                     s.kind == BondSettlement::SLASH_EQUIVOCATION
                                         ? 2
                                         : (s.kind == BondSettlement::SLASH_CONFISCATE ? 1 : 0),
                                     s.bond_units, s.slasher_address);
                }
                return out;
            });

        chain_.SetBondYieldWeightFn([this](uint64_t boundary_height) -> uint64_t {
            const auto* ov = Blockchain::alt_engine_overlay_;
            const ValidatorRegistry& vr = (ov && ov->validators) ? *ov->validators : validators_;
        ...
        chain_.SetCheckpointAtOrBelow(
            [](uint64_t max_height, uint64_t& out_height, Hash256& out_hash) -> bool {
        ...
        chain_.SetOnReorg([this](const Blockchain::UTXODelta& delta, uint64_t ancestor_height,
            ...
            try {
                if (reorg_utxo_pending_ || reorg_derived_index_frame_ ||
                    reorg_transaction_modules_ || !reorg_block_publication_stages_.empty() ||
                    !reorg_displaced_txindex_rows_.empty()) {
                    FailStopDurableCommitInvariant_(
                        "a new reorg entered with stale staged durable state");
                    throw std::runtime_error("stale reorg durable transaction state");
                }
                ...
                reorg_finality_retirements_.clear();
                reorg_block_publication_stages_.clear();
                CaptureReorgDisplacedTxIndexRows_(ancestor_height, ancestor_hash, old_tip);
                ...
                std::cerr << "  [reorg-leveldb] bounded writes=" << durable_delta.size() << " — "
                          << (ok ? "ok" : "FAILED") << "\n";
                std::cerr.flush();
                if (!ok) {
                    throw std::runtime_error("retained reorg UTXO transaction returned false");
                }
            } catch (const db::DurableWriteUncertain& e) {
                reorg_publication_uncertain_.store(true, std::memory_order_release);
                FailStopDurableCommitInvariant_(
                    std::string("VUR1 begin durability is uncertain: ") + e.what());
                std::cerr << "  [reorg-leveldb] durability uncertain: " << e.what() << "\n";
                std::cerr.flush();
        ...
        chain_.SetOnReorgAbort([this](const Blockchain::UTXODelta& delta, const Block& old_tip,
                                      uint64_t old_supply,
            ...
            try {
                ...
                if (!reorg_utxo_pending_) {
                    ...
                    const bool untouched = !current && tip && tip->height == old_tip.height &&
                    ...
                    if (untouched) {
                        ...
                        miner_archive_ready_.store(
                            MinerArchiveMarkersMatch_(old_tip.height, old_tip.GetHash()),
                            std::memory_order_release);
                    }
                    return untouched;
                }
                ...
                if (expected.old_height != old_tip.height ||
                    expected.old_hash != old_tip.GetHash() || expected.old_supply != old_supply) {
                    FailStopDurableCommitInvariant_(
                        "VUR1 abort callback old-frame identity mismatch");
                    return false;
                }
                const bool restored = db_.AbortReorgUTXO(SerializeUTXODelta_(delta), expected);
                if (!restored) {
                    FailStopDurableCommitInvariant_("VUR1 identity-checked abort returned false");
                    return false;
                }
                ...
                FailStopDurableCommitInvariant_(
                    std::string("VUR1 abort durability is uncertain: ") + e.what());
                return false;
            } catch (const std::exception& e) {
                FailStopDurableCommitInvariant_(std::string("VUR1 abort failed: ") + e.what());
                return false;
            } catch (...) {
                FailStopDurableCommitInvariant_("VUR1 abort failed with an unknown exception");
                return false;
            }
        });

        chain_.SetReorgPublicationUncertainFn([this]() -> bool {
            return reorg_publication_uncertain_.load(std::memory_order_acquire);
        });

        chain_.SetOnOrphanedTxs([this](const std::vector<Transaction>& orphan_txs) {
            if (reorg_publication_uncertain_.load(std::memory_order_acquire) ||
                FailStopRequired()) {
                return;
            }
            ...
            try {
                EnqueueReorgFixup([this, orphans_owned]() {
                    try {
                        ...
                        size_t purged = mempool_.PurgeInvalidAgainst(
                            [this](const Hash256& tx_hash, uint32_t idx) -> bool {
                        ...
                        for (const auto& tx : *orphans_owned) {
                            ...
                            try {
                                ...
                                auto input_total = mempool_.ResolveInputTotal(tx, chain_);
                                if (!input_total) {
                                    ++dropped;
                                    continue;
                                }
                            ...
                            auto res = mempool_.Add(tx, fee, (uint32_t)chain_.Height(), chain_);
                            if (res == Mempool::AddResult::ACCEPTED) {
                                ++reinjected;
                            } else {
                                ++dropped;
                        ...
                        if (purged > 0 || reinjected > 0 || dropped > 0) {
                            std::cerr << "  [reorg-mempool] re-injected " << reinjected
                                      << " orphan TX(s), dropped " << dropped << ", purged "
                                      << purged << " (post-reorg)\n";
                            std::cerr.flush();
                        }
                    } catch (const std::exception& e) {
                        std::cerr << "  [reorg-mempool] re-inject exception: " << e.what() << "\n";
                        std::cerr.flush();
                    } catch (...) {
                ...
                std::cerr << "  [reorg-mempool] re-inject dispatch: " << e.what() << "\n";
                std::cerr.flush();
            } catch (...) {
        ...
        chain_.SetOnRollback([this](const Blockchain::UTXODelta& delta, const Block& popped) {
            const auto durable_delta = SerializeUTXODelta_(delta);
            const bool ok = db_.ApplyUTXODelta(durable_delta);
            std::cerr << "  [rollback-leveldb] applied " << durable_delta.size()
            ...
            if (!ok) {
                ...
                // rejected block.
                throw std::runtime_error("leveldb UTXO rollback failed; restart required");
            }
        ...
        chain_.SetAmmMempoolValidator([this](const Transaction& tx, uint64_t candidate_height) {
            ...
            candidate.transactions.push_back(tx);
            return amm_.ValidateBlock("VELD:btcVELD", candidate, onchain_tokens_,
                                      PegGateForHeight_(candidate_height));
        });
        chain_.SetTokenMempoolValidator([this](const Transaction& tx, uint64_t candidate_height,
                                               uint32_t candidate_bits) {
            return onchain_tokens_.ValidateMempoolCandidate(tx, candidate_height, candidate_bits,
                                                            PegGateForHeight_(candidate_height));
        });
        chain_.SetTokenMempoolBatchFilter(
            [this](const std::vector<Transaction>& candidates,
                ...
                return onchain_tokens_.FilterMempoolCandidates(
                    ...
                    PegGateForHeight_(candidate_height), token_authorization_prevalidated);
            });
        chain_.SetTokenAwareMempoolSelector(
            [this](const std::vector<const Transaction*>& candidates,
                   ...
                   size_t initial_bytes, size_t max_count, size_t max_bytes,
                   uint64_t candidate_height, uint32_t candidate_bits) {
                return onchain_tokens_.SelectResourceFeasibleMempoolCandidates(
                    ...
                    candidate_bits, PegGateForHeight_(candidate_height));
            });
        chain_.amm_pool_input_check_ = [this](const std::vector<uint8_t>& spk, const Hash256& txid,
                                              uint32_t vout) -> int {
            if (!IsAmmPoolScript(spk))
                return 0;
        ...
        chain_.amm_block_validator_ = [this](const Block& b) {
            const auto* ov = Blockchain::alt_engine_overlay_; // fork-aware during reorg eval
            AmmLedger& a = (ov && ov->amm) ? *ov->amm : amm_;
            OnChainTokenLedger& t = (ov && ov->tokens) ? *ov->tokens : onchain_tokens_;
            if (ov) {
                if (!btc_headers_alt_ || !anchors_alt_ || !fin_state_alt_)
                    return false;
                return ValidateAmmBlockWithModulePrefix_(
                    a, t, *btc_headers_alt_, *anchors_alt_, *fin_state_alt_, b,
                    [this](uint64_t h, Hash256& out) { return ResolveAltHash_(h, out); });
            }
            return ValidateAmmBlockWithModulePrefix_(
                a, t, btc_headers_, anchors_, fin_state_, b, [this](uint64_t h, Hash256& out) {
                    try {
        ...
        chain_.anchor_gate_ = [this](uint64_t h, const Hash256& block_hash) -> bool {
            return EffectiveAnchorSecurityPinPermits_(h, block_hash) &&
                   anchors_.Allows(h, block_hash);
        };
        chain_.anchor_reorg_gate_ = [this](uint64_t common_ancestor_height,
                                           uint64_t current_tip_height) -> bool {
            return EffectiveAnchorSecurityReorgPermitted_(common_ancestor_height,
                                                          current_tip_height) &&
                   anchors_.PermanentReorgPermitted(common_ancestor_height);
        };
        ...
        chain_.finality_reorg_gate_ = [this](uint64_t common_ancestor_height) -> bool {
        ...
        chain_.SetOnCommit([this](const Block& block,
                                  const std::vector<std::pair<Hash256, uint32_t>>& spent,
                                  const std::vector<UTXO>& created, bool from_reorg) -> bool {
            ...
            const bool publishes_final_frame = !from_reorg || block.height == chain_.Height();
            ...
            if (from_reorg && reorg_publication_uncertain_.load(std::memory_order_acquire)) {
                return true;
            }
            ...
            try {
                ...
                if (block.height <= last_token_height_ && last_token_height_ > 0) {
                    ...
                    struct RebuildFlag {
                        std::atomic<bool>& f;
                        RebuildFlag(std::atomic<bool>& x) : f(x) {
                            f.store(true, std::memory_order_release);
                        }
                        ~RebuildFlag() {
                            f.store(false, std::memory_order_release);
                        }
                    } _rbf{reorg_rebuild_active_};
                    const ModuleCheckpoint* checkpoint =
                    ...
                    if (!checkpoint) {
                        ...
                        for (uint64_t h = checkpoint->height + 1; h < block.height; ++h) {
                            try {
                                ...
                                std::cerr << "  [replay] exception at h=" << h << ": " << e.what()
                                          << "\n";
                                std::cerr.flush();
                                replay_ok = false;
                                break;
                            } catch (...) {
                                std::cerr << "  [replay] unknown exception at h=" << h << "\n";
                                std::cerr.flush();
                                replay_ok = false;
                                break;
                            }
                ...
                if (!replay_ok) {
                    if (from_reorg && reorg_transaction_modules_) {
                        ...
                        if (pre_reorg_modules)
                            RestoreAllModules(*pre_reorg_modules);
                ...
                if (!pre_reorg_modules && block.height > 0 &&
                    ...
                    throw std::runtime_error("non-contiguous module apply height: prior=" +
                                             std::to_string(callback_entry_height) +
                ...
                if ((!from_reorg || block.height == chain_.Height()) &&
                    ...
                    throw std::runtime_error("module/canonical supply mismatch at h=" +
                                             std::to_string(block.height));
                }
                ...
                if (block.transactions.size() > 1) {
                    uint64_t cb_total = 0;
                    for (auto& out : block.transactions[0].outputs)
                        cb_total += out.value;
                ...
                try {
                    ...
                    current_publication_stage.module_supply = module_supply_after;
                    ...
                    current_publication_stage.redeems = onchain_tokens_.LastBlockRedeemsCopy();
                    current_publication_stage.mint_transitions =
                        onchain_tokens_.LastBlockMintTransitionsCopy();
                    if (from_reorg && (block.height % MAX_REORG_DEPTH) == 0) {
                        current_publication_stage.boundary_checkpoint = SnapshotAllModules();
                    }
                    if (from_reorg) {
                        reorg_block_publication_stages_.push_back(current_publication_stage);
#if defined(VELD_TEST_HOOKS) && defined(VELD_DSTATE_QUALIFICATION)
                    ...
                    std::cerr << "  [module] apply failed at h=" << block.height << ": " << e.what()
                ...
                try {
                    if (from_reorg) {
                        ...
                        if (reorg_utxo_pending_->new_height != block.height ||
                            ...
                            reorg_utxo_pending_->new_supply != module_supply_after) {
                            throw std::runtime_error(
                        ...
                        for (const auto& stage : reorg_block_publication_stages_) {
                            suffix.emplace_back(stage.height, stage.hash, stage.bits);
                        }
                        persist_ok = db_.CommitReorgMetadata(suffix, *reorg_utxo_pending_,
                                                             durable_anchor_floor_ptr);
                    } else {
                        persist_ok = db_.CommitBlock(
                            block.GetHash(), block.Serialize(), block.height, module_supply_after,
                            block.header.bits, spent, new_utxos, durable_anchor_floor_ptr);
                    }
                } catch (const db::DurableWriteUncertain& e) {
                    ...
                    if (!from_reorg) {
                        ...
                        FailStopDurableCommitInvariant_(
                            "linear canonical persistence failed before publication at h=" +
                            std::to_string(block.height) + ": unknown persistence exception");
                    }
                    return false;
                }
                if (!persist_ok) {
                    consecutive_commit_failures_.fetch_add(1, std::memory_order_relaxed);
                    if (from_reorg && reorg_transaction_modules_)
                        RestoreReorgTransactionModules_();
                    else if (callback_entry_modules)
                        RestoreAllModules(*callback_entry_modules);
                ...
                public_testnet::LatchClosedAtDurableCanonicalHeight(
                    ...
                    public_testnet_limits_ ? &public_testnet_limits_.value() : nullptr,
                    block.height);
#endif
                ...
                try {
                    ...
                    {
                        const auto mark =
                            [this, &canonical_durable_marker_ok, &marker_failure_height,
                             &marker_failure_hash](uint64_t height, const Hash256& hash) {
                    ...
                    FailStopDurableCommitInvariant_(
                        "canonical durable-body marker threw at h=" + std::to_string(block.height) +
                        " hash=" + HashToHex(block.GetHash()).substr(0, 16) + ": " + e.what());
                } catch (...) {
                    canonical_durable_marker_threw = true;
                    FailStopDurableCommitInvariant_(
                        "canonical durable-body marker threw at h=" +
                        std::to_string(marker_failure_height) +
                        " hash=" + HashToHex(marker_failure_hash).substr(0, 16));
                }
                if (!canonical_durable_marker_ok && !canonical_durable_marker_threw) {
                    ...
                    FailStopDurableCommitInvariant_(
                        "canonical durable-body marker rejected h=" + std::to_string(block.height) +
                        " hash=" + HashToHex(block.GetHash()).substr(0, 16));
                }
                ...
                if (publishes_final_frame && !PersistObservedAnchorFloorAfterDurable_(
                    ...
                    if (from_reorg) {
                        for (const auto& stage : reorg_block_publication_stages_) {
                            if (stage.boundary_checkpoint) {
                                InstallModuleCheckpoint_(stage.height, stage.hash,
                                                         stage.module_supply,
                                                         *stage.boundary_checkpoint);
                            }
                        }
                    }
                    CaptureModuleCheckpoint_(block.height, block.GetHash(), module_supply_after,
                ...
                if (publishes_final_frame) {
                    try {
                        if (from_reorg) {
                            PersistCanonicalDerivedIndexes_(reorg_block_publication_stages_,
                                                            /*from_reorg=*/true);
                        } else {
                            PersistCanonicalDerivedIndexes_(
                                std::vector<ReorgBlockPublicationStage>{current_publication_stage});
                        }
                    } catch (const std::exception& e) {
                        txindex_operational_.store(false, std::memory_order_release);
                        redeem_index_healthy_.store(false, std::memory_order_release);
                        mint_nullifier_index_healthy_.store(false, std::memory_order_release);
                        std::cerr << "  [derived-index] final publication failed: " << e.what()
                                  << "; indexes closed until replay\n";
                        std::cerr.flush();
                    }
                }
                ...
                if (btcveld::ShouldPublishSupplySnapshot(from_reorg, block.height, chain_.Height()))
                ...
                if (publishes_final_frame && (block.height % BLOCKS_PER_DAY) == 0)
                    governance_.PruneOrphanedKV();

                if (publishes_final_frame) {
                    const auto publish_vault_entry =
                        [this](const ReorgBlockPublicationStage& stage) {
                            if (stage.vault_fees > 0) {
                                vault_.Deposit(stage.vault_fees,
                                               "fees block " + std::to_string(stage.height));
                            }
                            if (stage.vault_reserve_payout > 0) {
                                vault_.Deposit(stage.vault_reserve_payout,
                ...
                if (from_reorg)
                    miner_archive_ready_.store(false, std::memory_order_release);
                const bool archive_ok =
                    from_reorg
                        ? (block.height == chain_.Height() ? RebuildMinerArchiveIndex_() : true)
                        : AdvanceMinerArchiveIndex_(block);
                if (!archive_ok) {
                    miner_archive_ready_.store(false, std::memory_order_release);
                    std::cerr << "  [miner-archive] index unavailable at h=" << block.height
                ...
                const bool history_ok =
                    from_reorg
                        ? (block.height == chain_.Height() ? RebuildAddressHistoryIndex_() : true)
                        : AdvanceAddressHistoryIndex_(block);
                if (!history_ok) {
                    address_history_ready_.store(false, std::memory_order_release);
                    std::cerr << "  [address-history] index unavailable at h=" << block.height
                              << "; bounded history disabled until rebuild\n";
                    std::cerr.flush();
                }

                if (publishes_final_frame && ibd_complete_.load() && block.height > 0 &&
                ...
                std::cerr << "  [ERROR] on_commit_ exception at height " << block.height << ": "
                          << e.what() << "\n";
                ...
                if (!durable_publication_cleared && !DurableCommitFailStop()) {
                    FailStopDurableCommitInvariant_(
                ...
                std::cerr << "  [ERROR] on_commit_ unknown exception at height " << block.height
                          << "\n";
                if (!durable_commit_done) {
                    if (from_reorg && reorg_transaction_modules_)
                        RestoreReorgTransactionModules_();
    ...
    void ReplayChain(uint64_t target_height, bool verify_historical_pow = true,
        ...
        if (startup_replay_active_.exchange(true, std::memory_order_acq_rel)) {
        ...
        if (snapshot_requirement) {
            ...
            if (prior_snapshot_base &&
                (prior_snapshot_base->height != snapshot_requirement->first ||
                 HashToHex(prior_snapshot_base->tip_hash) != snapshot_requirement->second)) {
                throw std::runtime_error(
                    "ReplayChain: independent validation base conflicts with the signed snapshot handoff");
            }
        }
        const bool local_pow_cache_permitted =
            !snapshot_requirement && (snapshot_fast_start_eligible_ || background_validation_only_);
        ...
        if (trusted_pow_through > target_height) {
            throw std::runtime_error("ReplayChain: local PoW cache exceeds the durable chain tip");
        }
        if (snapshot_requirement && !snapshot_fast_start_eligible_ && !verify_historical_pow) {
            throw std::runtime_error(
...
#else
        const uint64_t deferred_pow_through = verify_historical_pow ? 0 : target_height;
        ...
        PrepareCanonicalDerivedIndexesForReplay_(target_height, replay_target_hash);
...

        {
            ...
            if (genesis_hash_hex) {
                ...
                if (*genesis_hash_hex != expected_genesis_hex) {
                    ...
                    std::cerr
                        << "  [recover] miner on an orphan chain forever and silently fork.\n";
        ...
        uint64_t running_supply = 0;
        {
            ...
            if (genesis_hash_hex) {
                ...
                if (data) {
                    ...
                    if (consumed != 0 && consumed == data->size() && gb.Serialize() == *data) {
                        gb.height = 0;
...
#endif
                        if (gb.GetHash() != ghash ||
                            !chain_.AddBlockDirect(gb, replay_genesis_skip_pow, false, false,
                                                   mining::PowAdmissionContext::Internal())) {
                            throw std::runtime_error(
                        ...
                        if (!ApplyBlockModules_(gb, running_supply,
                                                /*persist_governance=*/false,
                                                /*publish_rpc_snapshots=*/true,
                                                /*chain_mutex_already_held=*/false)) {
                            throw std::runtime_error(
                        ...
                        CaptureModuleCheckpoint_(0, ghash, running_supply, /*force=*/true);
                        last_token_height_ = 0;
                    ...
                    throw std::runtime_error("ReplayChain: stored genesis block data is missing");
                }
            }
        }
        ...
        for (const auto& [cp_h, cp_hex] : Blockchain::AllCheckpointPins()) {
            if (cp_h == 0 || cp_h > target_height)
                continue;
            auto stored = db_.GetHashAtHeight(cp_h);
            if (!stored || *stored == cp_hex)
                continue;
            ...
            QuarantineDataDir_();
            throw std::runtime_error(
                "VeldNode: stored chain fails checkpoint anchor at height " + std::to_string(cp_h) +
        ...
        if (target_height > 0 && chain_.IsEmpty()) {
            throw std::runtime_error("ReplayChain: nonzero stored tip has no canonical genesis");
        }
        ...
        for (uint64_t h = 1; h <= target_height; ++h) {
            ...
            if (!hash_opt) {
                ...
                if (!recovered.empty()) {
                    std::cerr << "  [replay] hash index gap at h=" << h
                              << " — repaired from reverse-index (" << recovered.substr(0, 16)
                              << "...)\n";
            ...
            if (!data) {
                throw std::runtime_error("ReplayChain: block data missing at height " +
                                         std::to_string(h));
            }
            Block blk;
            blk.height = h;
            ...
            if (!chain_.AddBlockDirect(blk, /*skip_pow=*/false, /*skip_scripts=*/false,
                                       /*skip_pow_hash_only=*/
            ...
            if (!AdvanceModuleSupply_(blk, running_supply) ||
                running_supply != chain_.TotalSupplyUnits()) {
                throw std::runtime_error("ReplayChain: canonical supply mismatch at height " +
                                         std::to_string(h));
            }
            if (!ApplyBlockModules_(blk, running_supply,
                                    ...
                                    /*publish_rpc_snapshots=*/true,
                                    /*chain_mutex_already_held=*/false)) {
                throw std::runtime_error("ReplayChain: stateful module rejected block at height " +
                                         std::to_string(h));
            }
            try {
                ...
                stage.mint_transitions = onchain_tokens_.LastBlockMintTransitionsCopy();
                ...
                PersistCanonicalDerivedIndexes_({stage}, /*from_reorg=*/false,
                ...
                redeem_index_healthy_.store(false, std::memory_order_release);
                mint_nullifier_index_healthy_.store(false, std::memory_order_release);
                std::cerr << "  [derived-index] replay publication failed at h=" << h << ": "
                          << e.what() << "; indexes closed until next rebuild\n";
                std::cerr.flush();
            }
            ...
            if (snapshot_requirement && h == snapshot_requirement->first) {
                ...
                if (prior_snapshot_base && prior_snapshot_base->state_digest != base.state_digest) {
                    throw std::runtime_error(
        ...
        if (chain_.Height() != target_height || running_supply != chain_.TotalSupplyUnits()) {
            throw std::runtime_error("ReplayChain: did not reach the complete stored tip");
        }
        const std::string replayed_tip =
            chain_.IsEmpty() ? std::string() : HashToHex(chain_.TipCopy().GetHash());
        auto stored_tip = db_.ReadChainTip();
        ...
        if (stored_tip->supply_units != running_supply) {
            if (replayed_tip.empty() ||
                !db_.WriteChainTip(HexToHash(replayed_tip), target_height, running_supply)) {
                throw std::runtime_error(
        ...
        if (target_height > 0 || durable_commit_repair_required_.load(std::memory_order_acquire) ||
            db_.UtxoRecoveryRequired()) {
            try {
                auto utxo_kvs = chain_.SnapshotUTXOsForLevelDB_NoLock();
                bool rebuilt = db_.RebuildUTXOsFromSnapshot(utxo_kvs);
                if (veld::DiagVerbose().load()) {
                    veld::vcerr() << "  [replay-leveldb-sync] rewrote " << utxo_kvs.size()
                ...
                std::cerr << "  [replay-leveldb-sync] exception: " << e.what() << "\n";
                std::cerr.flush();
                throw;
            } catch (...) {
                std::cerr << "  [replay-leveldb-sync] unknown exception\n";
                std::cerr.flush();
                throw;
            }
        }
        ...
        FinishCanonicalDerivedIndexesAfterReplay_(target_height, replay_target_hash);
        ...
        if (body_prune.deleted > 0 || veld::DiagVerbose().load()) {
            std::cerr << "  [replay-body-cache] canonical=" << body_prune.canonical
                      << " pruned=" << body_prune.deleted << "\n";
            std::cerr.flush();
        }
        ...
        const uint64_t next_timestamp = tip_timestamp <= UINT64_MAX - TARGET_BLOCK_TIME
                                            ? tip_timestamp + TARGET_BLOCK_TIME
                                            : tip_timestamp;
        btc_relay_fresh_snapshot_.store(btc_headers_.ExternalValueFresh(next_timestamp),
                                        std::memory_order_release);
        ...
        if (independent_snapshot_validation) {
            ...
            if (!prior_snapshot_base && !WriteIndependentValidationRequirement_(base)) {
                throw std::runtime_error(
        ...
        if (chain_.Height() > 0) {
            std::cout << "  Chain "
                      << (verify_historical_pow ? "fully consensus-replayed"
                                                : "state-replayed (independent IBD pending)")
                      << " from disk: height=" << chain_.Height()
                      << " supply=" << chain_.TotalSupplyVeld() << " VELD\n";
            ...
            if (snapshot_requirement) {
                if (verify_historical_pow) {
                    ...
                    if (std::filesystem::exists(IndependentValidationRequirementPath_(),
    ...
    ComputeExpectedPoolPayoutOutputs(const Hash256& prev_block_hash, uint64_t pool_balance_units,
                                     uint64_t block_height) const {
        return chain_.ComputeExpectedPoolOutputs(prev_block_hash, pool_balance_units, block_height);
    }
    ...
    void BuildAndBroadcastNmsTx(const BlockHeader& nms_header) {
        ...
        auto miner_script = miner_keypair_.script_override.empty() ? miner_keypair_.GetP2PKHScript()
        ...
        if (!mining::NeedsNmsSubmission(chain_, mempool_, miner_script, nms_header.prev_block_hash))
            return;
        ...
        try {
            ...
            tip = chain_.Height();
            funding =
                rpc_.ComputeWalletState(ScriptToAddress(miner_script, config_.IsTestNetwork()));
        } catch (const std::exception& error) {
            std::cerr << "  [nms] skip: funding state unavailable: " << error.what() << "\n";
            std::cerr.flush();
            return;
        }
        const UTXO* input_utxo =
            mining::SelectNmsFundingOutput(funding.selectable, tip, funding.spendable_units,
    ...
    bool BuildPoolPayoutTx(Block& block, const std::vector<uint8_t>& pool_script, Mempool&) {
        ...
        auto payouts = ComputeExpectedPoolPayoutOutputs(block.header.prev_block_hash, total_pool,
                                                        block.height);
        if (payouts.empty())
            return false;

        std::sort(pool_utxos.begin(), pool_utxos.end(), [](const UTXO& a, const UTXO& b) {
    ...
    bool BuildEndorsementFlushTx(Block& block) {
        ...
        auto payouts = chain_.ComputeExpectedEndorsementFlushOutputs(block.height, total_pool, 0);
        if (payouts.empty())
            return false;

        std::sort(ep_utxos.begin(), ep_utxos.end(), [](const UTXO& a, const UTXO& b) {
            if (a.tx_hash != b.tx_hash)
                return a.tx_hash < b.tx_hash;
            return a.output_index < b.output_index;
        });
    ...
    bool BuildVaultDistributionTx(Block& block) {
        ...
        if (total_vault <= fee_reserve)
            return false;

        auto snapshot = staking_.GetWeightedStakeSnapshot(block.height - 1);

        uint64_t prev_cycle_inflow = chain_.ComputeVaultInflowSinceLastDistribution(block.height);

        auto payouts = chain_.ComputeExpectedVaultDistribution(
            block.height, total_vault, fee_reserve, prev_cycle_inflow, snapshot);
        if (payouts.empty())
            return false;
    ...
    bool BuildBondSettlementTx(Block& block) {
        ...
        auto payouts = chain_.ComputeExpectedBondMovements(block.height, total_sv, fee_reserve);
        if (payouts.empty())
            return false;

        std::sort(sv_utxos.begin(), sv_utxos.end(), [](const UTXO& a, const UTXO& b) {
            if (a.tx_hash != b.tx_hash)
                return a.tx_hash < b.tx_hash;
            return a.output_index < b.output_index;
        });
    ...
    bool BuildBondYieldSettlementTx(Block& block) {
        ...
        auto payouts =
            chain_.ComputeExpectedBondYieldSettlement(block.height, total_esc, fee_reserve);
        if (payouts.empty())
            return false;

        std::sort(esc_utxos.begin(), esc_utxos.end(), [](const UTXO& a, const UTXO& b) {
            if (a.tx_hash != b.tx_hash)
                return a.tx_hash < b.tx_hash;
            return a.output_index < b.output_index;
        });
    ...
    Transaction BuildEndorsementFlushStandalone(uint64_t current_height) {
        Transaction tx;
        if (endorsement_pool_keypair_.public_key[0] == 0)
            return tx;
        ...
        for (const auto& u : ep_utxos_all) {
            ...
            if (ep_mempool_spent.count(outpoint))
                continue;
        ...
        try {
            ...
            rs << std::fixed << std::setprecision(12) << per_endorsement_veld;
    ...
    std::string GetPoolInfoJSON() const {
        ...
        uint64_t blocks_until_payout =
            (COMINE_WINDOW_BLOCKS == 0) ? 0 : (COMINE_WINDOW_BLOCKS - blocks_in_window);
        ...
        for (const auto& [script_hex, count] : nms_snap) {
            if (count == 0)
                continue;
            if (script_hex == vault_hex || script_hex == pool_hex)
                continue;
            auto script_bytes = HexToBytes(script_hex);
            std::string addr = ScriptToAddress(script_bytes);
            if (addr.empty() || addr == VAULT_ADDRESS || addr == POOL_ADDRESS)
                continue;
    ...
    bool VerifyNearMiss(const Block& solved_block, uint64_t comine_nonce,
        ...
        auto pow_lease =
            mining::GlobalExpensivePowBudget().TryAcquire(mining::ExpensivePowUse::NearMiss);
        if (!pow_lease)
            return false;
        ...
        if (actual_hash != comine_hash)
            return false;

        uint32_t bits = solved_block.header.bits;
        Hash256 target_arr{};
        if (!BlockHeader::DecodeBits(bits, target_arr))
            return false;
    ...
    void LoadCominePool() {
        ...
        while (std::getline(ss, line)) {
            if (line.substr(0, 11) == "pool_units=")
                try {
                    comine_pool_.pool_units = std::stoull(line.substr(11));
                } catch (...) {
                }
            else if (line.substr(0, 13) == "window_start=")
                try {
                    comine_pool_.window_start = std::stoull(line.substr(13));
                } catch (...) {
                }
                ...
                if (colon != std::string::npos) {
                    ...
                    double count = 0.0;
                    try {
                        count = std::stod(rest.substr(colon + 1));
                    } catch (...) {
                        continue;
                    }
                    ...
                    if (key.size() == 50) {
                        ...
                        if (valid)
                            comine_pool_.tallies[key].script = script;
                    }
                }
            }
        }
        std::cout << "  [Pool] Loaded co-miner pool state: " << comine_pool_.pool_units
                  << " units accumulated, " << comine_pool_.tallies.size() << " miners tallied\n";
    }
        ...
        while (mining_.load() && running_.load()) {
            try {
                ...
                qualification_pre_hash_barrier =
                    test_work_admission_mining_barrier_.load(std::memory_order_acquire);
#endif
                if (tcp_server_ && config_.name != "Veld Regtest" &&
                    !qualification_pre_hash_barrier) {
                    int anchors = tcp_server_->GetConnectedAnchorCount();
                    if (anchors <= 0) {
                        mining_work_state_.store(MiningWorkState::WaitingForAnchor,
                                                 std::memory_order_release);
                        hashrate_.store(0.0, std::memory_order_release);
                ...
                const auto round_admission =
                    round_subject ? EvaluateWorkAdmission(work_admission::Path::InternalMining,
                ...
                const work_admission::Binding round_binding = *round_admission.binding;
                ...
                if (test_work_admission_mining_barrier_.load(std::memory_order_acquire)) {
                    test_work_mining_admitted_calls_.fetch_add(1, std::memory_order_acq_rel);
                    mining_work_state_.store(MiningWorkState::Hashing, std::memory_order_release);
                    hashrate_.store(0.0, std::memory_order_release);
                    std::this_thread::sleep_for(std::chrono::milliseconds(1));
                    continue;
                }
                ...
                if (tcp_server_) {
                    const auto clock = tcp_server_->GetActiveClockDriftSnapshot();
                    const auto diagnostic_clock = tcp_server_->GetDiagnosticClockDriftSnapshot();
                    const int64_t hard_abs =
                        clock.median_seconds < 0 ? -clock.median_seconds : clock.median_seconds;
                    const bool authoritative_halt = clock.distinct_ip_count >= 3 && hard_abs >= 600;
                    const int64_t diagnostic_abs = diagnostic_clock.median_seconds < 0
                                                       ? -diagnostic_clock.median_seconds
                                                       : diagnostic_clock.median_seconds;
                    if (!authoritative_halt && diagnostic_clock.distinct_ip_count >= 3 &&
                        diagnostic_abs >= 600) {
                        ...
                        if (now >= last_diagnostic_warn.load() + 30) {
                            ...
                            std::cerr << "  [mining] CLOCK DIAGNOSTIC ONLY: all-active-peer "
                                         "median differs by "
                                      << diagnostic_abs
                                      << "s (n=" << diagnostic_clock.distinct_ip_count
                        ...
                        if (adrift >= 600) {
                            ...
                            if (now >= last_warn.load() + 30) {
                                ...
                                std::cerr << "  [mining] REFUSING TO MINE: local clock is "
                                          << adrift << "s " << (drift > 0 ? "behind" : "ahead of")
                ...
                if (tcp_server_) {
                    ...
                    if (mm > 0 && mt == 0) {
                        mining_work_state_.store(MiningWorkState::GenesisMismatch,
                                                 std::memory_order_release);
                        hashrate_.store(0.0, std::memory_order_release);
                        static std::atomic<uint64_t> last_warn{0};
                        uint64_t now = (uint64_t)std::time(nullptr);
                        if (now >= last_warn.load() + 30) {
                            last_warn.store(now);
                            std::cerr
                                << "  [mining] REFUSING TO MINE: every peer that has handshaked with us ("
                ...
                if (tcp_server_ && config_.name != "Veld Regtest") {
                    ...
                    const auto bootstrap_peer_heights = tcp_server_->GetPeerHeightView();
                    const uint64_t bootstrap_local_height = chain_.Height();
                    bool genesis_bootstrap_safe =
                        bootstrap_local_height == 0 &&
                        MiningWorkStillSafeFromView_(bootstrap_peer_heights, bootstrap_local_height,
                                                     chain_.IsEmpty());
                    if (genesis_bootstrap_safe) {
                        ...
                        for (const auto& t : tips) {
                            if (now_s - t.updated_at > MINING_SAFETY_TIP_TTL_S)
                                continue;
                            if (HashIsZero(t.hash))
                                continue;
                            ...
                            if (active_at_height.empty() || active_at_height != HashToHex(t.hash))
                                continue;
                            ++fresh;
                        ...
                        const auto safety_peer_heights = tcp_server_->GetPeerHeightView();
                        const uint64_t verified_peer_height = safety_peer_heights.verified_height;
                        if (!MiningWorkStillSafeFromView_(safety_peer_heights, our_height,
                                                          chain_.IsEmpty())) {
                            unsafe = true;
                        ...
                        if (!unsafe && peer_count_ok && !peers_confirm_our_tip &&
                            local_blocks_since_peer_block_.load(std::memory_order_relaxed) >=
                                MAX_DARK_SELF_MINED) {
                            unsafe = true;
                        ...
                        if (unsafe) {
                            MiningWorkState work_state = MiningWorkState::WorkAdmission;
                            ...
                            mining_work_state_.store(work_state, std::memory_order_release);
                            hashrate_.store(0.0, std::memory_order_release);
                            static std::atomic<uint64_t> last_warn{0};
                            uint64_t now = (uint64_t)std::time(nullptr);
                            if (now >= last_warn.load() + 15) {
                                ...
                                std::cerr << "  [mining] PAUSING: " << reason
                                          << " (peers=" << connected << " fresh-tips=" << fresh
                ...
                const uint64_t round_total_base = total_hashes_.load(std::memory_order_acquire);
                mining_work_state_.store(MiningWorkState::Hashing, std::memory_order_release);
                std::thread mirror([this, &stop_now, round_total_base, round_binding]() {
                    ...
                    while (stop_now.load() == false && running_.load()) {
                        ...
                        // every hash.
                        if (!MiningWorkStillSafe_(work_admission::Path::InternalMining,
                                                  round_binding, true)) {
                        ...
                        const double rate_elapsed =
                            std::chrono::duration<double>(rate_now - rate_sample_ts).count();
                        if (rate_elapsed >= 1.0) {
                            const uint64_t completed_hashes = cur * 32ULL;
                            total_hashes_.store(round_total_base + completed_hashes,
                                                std::memory_order_release);
                            const uint64_t progress_delta =
                                cur >= rate_sample_progress ? cur - rate_sample_progress : 0;
                            hashrate_.store(static_cast<double>(progress_delta * 32ULL) /
                                                rate_elapsed,
                                            std::memory_order_release);
                        ...
                        if (cur != last_seen) {
                            ...
                            if (std::chrono::duration_cast<std::chrono::seconds>(stall).count() >=
                                WATCHDOG_STALL_SECONDS) {
                                std::cerr << "  [watchdog] mining stalled >"
                                          << WATCHDOG_STALL_SECONDS
                ...
                uint32_t effective_bits =
                    mining_bits_ ? mining_bits_ : chain_.ComputeNextBitsAtTip();
                ...
                if constexpr (OPTION_B_CONSENSUS_GATE_ENABLED) {
                    nms_cb = [this](const BlockHeader& hdr) {
                        try {
                            this->BuildAndBroadcastNmsTx(hdr);
                        } catch (...) {
                        }
                    };
                }
                ...
                auto result = MineOnly(
                    chain_, mempool_, miner_keypair_, effective_bits, &stop_now,
                    AddressToScript(POOL_ADDRESS), mining_threads_, progress_cb, {}, nms_cb,
                    &miner_progress_counter_, mandatory_settlements,
                    [this](const Block& candidate) { return PreflightMiningCandidate_(candidate); },
                    finality_metadata);
                if (veld::DiagVerbose().load())
                    veld::vcerr() << "  [post-mine h=" << result.new_height
                                  << "] success=" << result.success
                                  << " chain_tip=" << chain_.Height()
                                  << " expected=" << (result.new_height ? result.new_height - 1 : 0)
                                  << " mining=" << mining_.load() << " running=" << running_.load()
                                  << "\n";
                std::cerr.flush();
                ...
                if (!mining_.load() || !running_.load())
                    break;
                if (!result.success)
                    continue;

                if (chain_.Height() != result.new_height - 1)
                    continue;

                if (tcp_server_) {
                    auto pm = tcp_server_->GetMessageBuilder();
                    tcp_server_->BroadcastMessage(
                        pm.BuildSolutionMessage(result.block.header.prev_block_hash,
                                                result.new_height, result.block.header.nonce,
                ...
                size_t n_cominers = 0;
                if constexpr (!OPTION_B_CONSENSUS_GATE_ENABLED) {
                    auto peer_solutions = CollectPeerSolutions(result.new_height,
...

                {
                    ...
                    if (!is_vault_block)
                        comine_pool_.pool_units += comine_cut;
                }
                ...
                if (effective_reward == 0) {
                    ...
                    const uint64_t pre_vault = effective_reward - pre_miner + block_tx_fees;
                ...
                if (needs_rebuild) {
                    ...
                    for (const auto& script : finality_metadata)
                        result.block.transactions[0].outputs.push_back(TxOutput(0, script));
                    ...
                    if (veld::DiagVerbose().load())
                        veld::vcerr() << "  [remine-entry h=" << result.new_height
                    ...
                    if (!MiningWorkStillSafe_(work_admission::Path::InternalMining, round_binding,
                                              true)) {
                        continue;
                    }
                    auto remine_pow_lease = mining::GlobalExpensivePowBudget().TryAcquire(
                    ...
                    for (uint32_t wi = 0; wi < workers_to_spawn; ++wi) {
                        rm_workers.emplace_back([&, wi]() {
                            ...
                            while (!rm_winner_found.load() &&
                                   !rm_stop_workers.load(std::memory_order_acquire) &&
                                ...
                                for (uint64_t n = base; n < base + STRIDE; ++n) {
                                    ...
                                    if (++local_hashes_since_bump >= 32) {
                                        miner_progress_counter_.fetch_add(
                    ...
                    while (!rm_winner_found.load() && mining_.load() && running_.load()) {
                        ...
                        if (!MiningWorkStillSafe_(work_admission::Path::InternalMining,
                                                  round_binding, true)) {
                            rm_abort_for_safety.store(true, std::memory_order_release);
                            rm_stop_workers.store(true, std::memory_order_release);
                            break;
                        }
                        std::this_thread::sleep_for(std::chrono::milliseconds(500));
                        auto elapsed = std::chrono::duration<double>(
                                           std::chrono::steady_clock::now() - remine_t0)
                                           .count();
                        uint64_t iters = rm_total_iters.load();
                        if (veld::DiagVerbose().load())
                            veld::vcerr()
                                << "  [remine h=" << result.new_height << "] iters=" << iters
                    ...
                    rm_stop_workers.store(true, std::memory_order_release);
                    for (auto& t : rm_workers)
                        if (t.joinable())
                            t.join();
                    if (rm_abort_for_safety.load(std::memory_order_acquire)) {
                        continue;
                    }
                    ...
                    if (remined) {
                        ...
                        if (veld::DiagVerbose().load())
                            veld::vcerr()
                                << "  [remine h=" << result.new_height << "] SOLVED after "
                                << remine_iters << " iters (" << (int)(elapsed * 1000) << "ms)\n";
                        std::cerr.flush();
                    }
                    if (!remined) {
                        if (veld::DiagVerbose().load())
                            veld::vcerr() << "  [remine-exit h=" << result.new_height
                ...
                if (veld::DiagVerbose().load())
                    veld::vcerr() << "  [commit h=" << result.new_height
                                  << "] txs_in_block=" << result.block.transactions.size()
                                  << " needs_rebuild=" << (needs_rebuild ? "y" : "n") << "\n";
                std::cerr.flush();
                {
                    auto verify_pow_lease = mining::GlobalExpensivePowBudget().TryAcquire(
                        mining::ExpensivePowUse::InternalMine);
                    if (!verify_pow_lease)
                        continue;
                ...
                const bool parent_is_current = result.new_height > 0 &&
                ...
                if (!parent_is_current || !mining_.load(std::memory_order_acquire) ||
                    !running_.load(std::memory_order_acquire) ||
                    !MiningWorkStillSafe_(work_admission::Path::InternalMining, round_binding,
                                          true)) {
                    std::cerr << "  [mining] DISCARDING solved block " << result.new_height
                ...
                auto pow_context = mining::PowAdmissionContext::InternalMiningWork(
                    work_admission::EncodeBinding(round_binding));
                const auto block_admission =
                    chain_.AddBlockDirect(result.block, false, false, false, pow_context);
                if (block_admission.IsAccepted()) {
                    RecordLocalBlockMined();
                    session_blocks_mined_.fetch_add(1, std::memory_order_relaxed);
                    ...
                    if (pow_context.local_work_handoff->IsLive()) {
                        ...
                        std::string t =
                            _tier.level > 0 ? " [Tier " + std::to_string(_tier.level) + "]" : "";
                        std::cout << "  Block " << result.new_height << " mined | " << std::fixed
                                  << std::setprecision(3) << result.elapsed_ms
                                  << "ms | supply=" << std::fixed << std::setprecision(2)
                    ...
                    if (chain_.Height() >= result.new_height) {
                        ...
                        std::cerr << "  [Mining] Block " << result.new_height
                                  << " rejected by chain: " << Blockchain::GetLastRejectTag()
                                  << "\n";
                    }
                }
            } catch (const std::exception& e) {
                mining_work_state_.store(MiningWorkState::Error, std::memory_order_release);
                hashrate_.store(0.0, std::memory_order_release);
    ...
    std::vector<PendingSolution> CollectPeerSolutions(uint64_t prev_height,
        ...
        for (auto it = pending_solutions_.begin(); it != pending_solutions_.end();) {
            auto age =
                std::chrono::duration_cast<std::chrono::seconds>(now - it->received_at).count();
            if (age > 30) {
...
inline void RunIntegrationTests() {
    ...
    {
        std::vector<uint8_t> empty_sha = {0xe3, 0xb0, 0xc4, 0x42, 0x98, 0xfc, 0x1c, 0x14,
...

    {
        ...
        if (result.success) {
            check("Mined block matches chain tip", chain.Tip().GetHash() == result.block.GetHash());
        }
    }
    ...
    {
        RpcCall c1 = RpcCall::Parse(
            R"({"jsonrpc":"2.0","id":"1","method":"validateaddress","params":["VTestAddress123"]})");
        check("RPC parses method correctly", c1.method == "validateaddress");
        check("RPC parses string param correctly",
              !c1.params.empty() && c1.params[0] == "VTestAddress123");

        RpcCall c2 =
            RpcCall::Parse(R"({"jsonrpc":"2.0","id":"2","method":"getblockcount","params":[]})");
        check("RPC parses empty params", c2.params.empty());

        RpcCall c3 = RpcCall::Parse(
            R"({"jsonrpc":"2.0","id":"3","method":"gettxout","params":["abc123",0]})");
...

    {
        ...
        check("Staking inactive in Bootstrap",
...

    {
        ...
        check("Supply set to just below threshold", chain.TotalSupplyUnits() == just_below);
        ...
        check("Staking becomes active after transition",
              chain.TotalSupplyUnits() >= STAKING_ACTIVATION_SUPPLY);
        check("Miner cap drops to 10% in Standard phase", true);
        check("Min miners rises to 10 in Standard phase", true);
...

    {
        ...
        check("PQ: different privkeys → different pubkeys", kp1.public_key != kp2.public_key);
        check("PQ: pubkey length is 1952 bytes (ML-DSA-65)", kp1.public_key.size() == 1952);
        check("PQ: address starts with V (mainnet)", !kp1.address.empty() && kp1.address[0] == 'V');
        ...
        check("EC: Verify accepts own signature", Verify(kp1.public_key, msg_hash, sig));

        check("EC: Verify rejects wrong key", !Verify(kp2.public_key, msg_hash, sig));
    ...
    {
        std::string test_dir =
            VeldTmpDir() + "veld_persist_" + std::to_string((uint64_t)std::time(nullptr));
...

        {
            ...
            check("Persistence: chain height survives restart", node2.GetChain().Height() == 3);
            check("Persistence: supply survives restart", node2.GetChain().TotalSupplyUnits() > 0);
            ...
            check("Persistence: can mine after reload", !results.empty() && results[0].success);
            check("Persistence: height = 4 after reload + 1 block", node2.GetChain().Height() == 4);
    ...
    //   - Rejection of wrong-key signatures
    {
        ...
        bool valid =
            interp.Execute(spend_tx.inputs[0].script_sig, alice.GetP2PKHScript(), spend_tx, 0);
        ...
        bool tampered_valid =
            interp2.Execute(tampered.inputs[0].script_sig, alice.GetP2PKHScript(), tampered, 0);
        ...
        bool wrong_key_valid = interp3.Execute(wrong_key_tx.inputs[0].script_sig,
                                               alice.GetP2PKHScript(), wrong_key_tx, 0);
...

    {
        ...
        for (uint64_t h = 1; h <= fork_chain.Height(); ++h) {
            ...
            blk.height = h;
            bool accepted = reorg_chain.AddBlockDirect(blk, false, false, false,
                                                       mining::PowAdmissionContext::Internal());
