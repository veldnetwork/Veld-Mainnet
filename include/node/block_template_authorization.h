...
class BlockTemplateAuthorizationClaim {
  public:
    ...
    BlockTemplateAuthorizationClaim(const BlockTemplateAuthorizationClaim&) = delete;
    BlockTemplateAuthorizationClaim& operator=(const BlockTemplateAuthorizationClaim&) = delete;
    ...
    bool ClaimForTicket(const Binding& expected_binding,
        ...
        if (binding_ != expected_binding ||
            coordinator_generation_ != expected_coordinator_generation || Now_() >= deadline_)
            return false;
        bool expected = false;
    ...
    bool IsLive() const noexcept {
        return !claimed_.load(std::memory_order_acquire) && Now_() < deadline_;
    }
  ...
  private:
    ...
    BlockTemplateAuthorizationClaim(const Binding& binding, uint64_t coordinator_generation,
                                    Clock::time_point deadline, NowFn now)
        : binding_(binding), coordinator_generation_(coordinator_generation), deadline_(deadline),
          now_(std::move(now)) {}
...
class BlockTemplateAuthorizationStore {
  public:
    ...
    struct ConsumeResult {
        ...
        explicit operator bool() const noexcept {
            return error == Error::None && static_cast<bool>(authorization);
        }
    };
    ...
    BlockTemplateAuthorizationStore()
        : BlockTemplateAuthorizationStore(Limits{}, TokenMintFn{}, NowFn{}) {}

    explicit BlockTemplateAuthorizationStore(Limits limits, TokenMintFn mint = TokenMintFn{},
                                             NowFn now = NowFn{})
        : limits_(NormalizeLimits_(limits)), token_mint_(std::move(mint)), now_(std::move(now)) {}

    BlockTemplateAuthorizationStore(const BlockTemplateAuthorizationStore&) = delete;
    BlockTemplateAuthorizationStore& operator=(const BlockTemplateAuthorizationStore&) = delete;

    IssueResult Issue(const Binding& binding, uint64_t coordinator_generation,
                      Duration requested_lifetime) noexcept {
        IssueResult result;
        try {
            if (!ValidIssuedBinding_(binding) || coordinator_generation == 0 ||
            ...
            const TimePoint sampled_now = injected_clock ? SampleInjectedNow_() : TimePoint{};
            std::lock_guard<std::mutex> lock(mutex_);
            const TimePoint now = injected_clock ? sampled_now : Clock::now();
            PruneLocked_(now);
            ...
            PruneLocked_(now);
            if (active_.size() >= limits_.max_active || spent_.size() >= limits_.max_spent) {
            ...
            for (size_t i = 0; i < candidate_count; ++i) {
                if (!TokenIsZero_(candidates[i]) && !TokenKnownLocked_(candidates[i])) {
            ...
            const Duration ttl =
                std::min({requested_lifetime, limits_.max_lifetime, ABSOLUTE_MAX_LIFETIME});
            ...
            Issued published{EncodeToken_(*selected), binding, coordinator_generation, deadline,
                             ttl};
            active_.push_back(Record{*selected, binding, coordinator_generation, deadline});
            try {
    ...
    ConsumeResult Consume(const std::string& token_text, const Binding& binding,
                          uint64_t coordinator_generation) noexcept {
        ConsumeResult result;
        try {
            const auto token = DecodeToken_(token_text);
            if (!token || !ValidIssuedBinding_(binding) || coordinator_generation == 0) {
                result.error = Error::Invalid;
                return result;
            }
            ...
            for (auto it = active_.begin(); it != active_.end(); ++it) {
                ...
                std::shared_ptr<BlockTemplateAuthorizationClaim> consumed(
                    new BlockTemplateAuthorizationClaim(it->binding, it->coordinator_generation,
            ...
            result.error = SpentKnownLocked_(*token) ? Error::TokenConsumed : Error::TokenUnknown;
            return result;
        } catch (...) {
            result.error = Error::Capacity;
            result.authorization.reset();
  ...
  private:
    ...
    static Limits NormalizeLimits_(Limits limits) noexcept {
        ...
        limits.max_lifetime = std::min(limits.max_lifetime, ABSOLUTE_MAX_LIFETIME);
    ...
    static bool ValidIssuedBinding_(const Binding& binding) noexcept {
        ...
        return binding.version == 1 && subject.purpose == Purpose::BlockProduction &&
               subject.height > 0 && subject.parent_height != UINT64_MAX &&
               subject.height == subject.parent_height + 1 && !HashIsZero(subject.parent_hash) &&
               !HashIsZero(subject.target_hash) && binding.validation_generation > 0 &&
               binding.network_magic != 0 && !HashIsZero(binding.genesis_hash) &&
               !HashIsZero(binding.profile_digest);
    }

    static bool SameAuthorityDomain_(const Binding& lhs, const Binding& rhs) noexcept {
        return lhs.validation_generation == rhs.validation_generation &&
               lhs.network_magic == rhs.network_magic && lhs.genesis_hash == rhs.genesis_hash &&
               lhs.profile_digest == rhs.profile_digest;
    }

    static bool TokenIsZero_(const TokenBytes& token) noexcept {
        uint8_t aggregate = 0;
        for (const uint8_t byte : token)
            aggregate |= byte;
        return aggregate == 0;
    }
    ...
    static std::optional<TokenBytes> DecodeToken_(const std::string& encoded) noexcept {
        ...
        return TokenIsZero_(token) ? std::nullopt : std::optional<TokenBytes>(token);
    }

    TimePoint SampleInjectedNow_() const noexcept {
        try {
            return now_ ? now_() : TimePoint::max();
        } catch (...) {
    ...
    bool TokenKnownLocked_(const TokenBytes& token) const noexcept {
        for (const auto& record : active_)
            if (TokenEqual_(record.token, token))
                return true;
    ...
    bool RememberSpentLocked_(const TokenBytes& token, TimePoint forget_after) const noexcept {
        if (SpentKnownLocked_(token) || spent_.size() >= limits_.max_spent)
            return true;
        try {
    ...
    void PruneLocked_(TimePoint now) const noexcept {
        spent_.erase(std::remove_if(spent_.begin(), spent_.end(),
                                    [&](const Spent& spent) { return spent.forget_after <= now; }),
                     spent_.end());
        for (auto it = active_.begin(); it != active_.end();) {
            if (it->deadline > now) {
                ++it;
