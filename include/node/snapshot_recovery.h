...
inline constexpr std::array<const char*, 11> NAMES{
    {"db", "blocks", "index", ".snapshot-handoff", ".background-chainstate-required",
     "background-ibd", "background-chainstate", "full-ibd.receipt", "fleet-full-ibd.receipt",
     "finality-equivocation.evj", "address-only-full-ibd.receipt"}};

inline void Require(bool value, const std::string& message) {
    if (!value)
        throw std::runtime_error("snapshot recovery: " + message);
}
...
inline int RejectionExitCode(const fs::path& root) noexcept {
    try {
        return HasReason(root / REQUEST) ? 75 : 76;
    } catch (...) {
        return 76;
    }
}
...
inline Floors Normalize(Floors records, uint32_t magic, const Hash256& genesis) {
    ...
    for (const auto& record : records)
        Require(btcanchor::floor_store::Validate(record, &error) && record.network_magic == magic &&
                    record.genesis_hash == genesis,
    ...
    records.erase(
        std::unique(records.begin(), records.end(), btcanchor::floor_store::ExactDuplicate),
        records.end());
...
inline Floors ReadFloors(const fs::path& root, uint32_t magic, const Hash256& genesis) {
    ...
    for (size_t at = 4; at < wire->size(); at += btcanchor::floor_store::VLF1_ENCODED_SIZE) {
        ...
        const auto record = btcanchor::floor_store::Decode(
            std::vector<uint8_t>(wire->begin() + at,
                                 wire->begin() + at + btcanchor::floor_store::VLF1_ENCODED_SIZE),
            &error);
...
inline void PreserveFloors(const fs::path& root, const Floors& records) {
    std::string error;
    Require(channel::secure_file::EnsurePrivateDirectory((root / "security").string(), &error),
            error);
...
inline Floors CaptureLocalState(const fs::path& root, uint32_t magic, const Hash256& genesis) {
    ...
    if (Exists(root / "security")) {
        ...
        if (const auto wire = Read(root / "security/anchor-floor.vlf1",
...
inline std::optional<Transaction> Load(const fs::path& root) {
    ...
    const std::array<const char*, 6> names{
        {"schema=", "id=", "phase=", "present=", "receipt=", "receipt_hash="}};
    std::string line;
    for (size_t i = 0; i < names.size(); ++i) {
        Require(bool(std::getline(input, line)) && line.starts_with(names[i]),
                "invalid recovery journal field");
    ...
    Require(values[0] == "1" && LowerHex(values[1], 24) &&
                ParseCanonicalUint64Text(values[3], mask) && mask < (1U << NAMES.size()),
            "invalid recovery journal identity");
    Transaction t{values[1], values[2], static_cast<unsigned>(mask), values[4], values[5]};
    Require(t.phase == "quarantine" || t.phase == "ibd" || t.phase == "complete",
            "unknown recovery phase");
    Require(t.phase == "complete"
                ? ((t.receipt == "full-ibd.receipt" || t.receipt == "fleet-full-ibd.receipt" ||
                    t.receipt == "address-only-full-ibd.receipt") &&
                   LowerHex(t.receipt_hash, 64))
                : (t.receipt.empty() && t.receipt_hash.empty()),
...
inline ImportTransaction LoadImport(const fs::path& root) {
    ...
    const std::array<const char*, 5> names{
        {"schema=", "stage=", "quarantine=", "next=", "present="}};
    std::array<std::string, 5> values;
    std::string line;
    for (size_t i = 0; i < names.size(); ++i) {
        Require(bool(std::getline(in, line)) && line.starts_with(names[i]),
                "malformed import journal");
        values[i] = line.substr(std::strlen(names[i]));
    }
    ...
    Require(!std::getline(in, line) && values[0] == "1" &&
                ...
                values[2].starts_with(".snapshot-preimport-") &&
                LowerHex(values[2].substr(20), 24) && ParseCanonicalUint64Text(values[3], next) &&
                next <= 5 && ParseCanonicalUint64Text(values[4], present) && present < 4,
            "invalid import journal identity");
    ImportTransaction t{values[1], values[2], static_cast<unsigned>(next),
...
inline void ResumeImport(const fs::path& root) {
    ...
    Require(Exists(root / "db") && Exists(root / ".background-chainstate-required") &&
                Exists(root / ".snapshot-handoff"),
            "import publication is incomplete");
    Remove(root / IMPORT);
}
...
inline void PublishImport(const fs::path& root, const fs::path& scratch) {
    ...
    const auto relative = fs::absolute(scratch).lexically_normal().lexically_relative(
    ...
    Require(relative.parent_path().empty() && stage.starts_with(".snapshot-stage-") &&
                LowerHex(stage.substr(16), 32),
            "import staging is not an owned direct child");
    ...
    Require(!Exists(root / ".background-chainstate-required"),
            "independent validation is already pending");
    std::array<uint8_t, 12> random{};
    Require(compat::SecureRandom(random.data(), random.size()),
            "import identity generation failed");
    ImportTransaction t{stage, ".snapshot-preimport-" + BytesToHex(random.data(), random.size()), 0,
                        0};
    if (Exists(root / "db")) {
        CheckTree(root / "db");
        t.present |= 1;
    }
    if (Exists(root / ".snapshot-handoff")) {
        CheckTree(root / ".snapshot-handoff");
        t.present |= 2;
    }
...
inline bool Prepare(const fs::path& root, uint32_t magic, const Hash256& genesis) {
    ...
    if (!transaction) {
        ...
        std::array<uint8_t, 12> random{};
        Require(compat::SecureRandom(random.data(), random.size()),
                "recovery identity generation failed");
        Transaction next{BytesToHex(random.data(), random.size()), "quarantine", 0, {}, {}};
        for (size_t i = 0; i < NAMES.size(); ++i) {
            if (Exists(root / NAMES[i])) {
                CheckTree(root / NAMES[i]);
                next.present |= 1U << i;
            }
        }
...
inline void Complete(const fs::path& root, const std::string& receipt_name) {
    auto t = Load(root);
    Require(t && (t->phase == "ibd" || t->phase == "complete"),
            "completion requires fresh IBD phase");
    Require(receipt_name == "full-ibd.receipt" || receipt_name == "fleet-full-ibd.receipt" ||
                receipt_name == "address-only-full-ibd.receipt",
            "unknown receipt role");
    if (t->phase == "complete") {
    ...
    t->phase = "complete";
    t->receipt = receipt_name;
    t->receipt_hash = HashToHex(Hash256d(*receipt));
