...
inline bool IsInitialDownloadAtTip(bool regtest, uint64_t local_height, bool chain_empty,
                                   size_t current_version_peers, uint64_t verified_peer_height,
                                   size_t current_outbound_sync_peers,
                                   uint64_t outbound_sync_height) {
    if (regtest)
        return true;
    if (current_version_peers == 0)
        return false;
    ...
    if (current_outbound_sync_peers < 2)
        return false;
    if (local_height < outbound_sync_height)
        return false;
    if (local_height < verified_peer_height)
        return false;
    if (local_height > 0)
        return true;
...
inline unsigned DefaultMiningThreads(unsigned hardware_threads) {
    if (hardware_threads == 0)
        hardware_threads = 1;
    const unsigned physical_est =
        hardware_threads == 1 ? 1 : (hardware_threads / 2) + (hardware_threads % 2);
