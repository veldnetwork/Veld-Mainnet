...
def bip340_sign(secret: int, message: bytes) -> bytes:
    ...
    nonce = int.from_bytes(V.tagged_hash(
        "BIP0340/nonce", normalized.to_bytes(32, "big") + public_key + message),
        "big") % V.N
    assert nonce
    ...
    challenge = int.from_bytes(V.tagged_hash(
        "BIP0340/challenge", r + public_key + message), "big") % V.N
...
def account_fixture(slot: int) -> tuple[int, bytes, str]:
    ...
    payload = (
        V.XPUB_VERSION + b"\x03" + hashlib.sha256(
            f"parent-{slot}".encode()).digest()[:4] +
        (0x80000000).to_bytes(4, "big") + chain + compressed)
    expression = (
        f"[{fingerprint.hex()}/86h/0h/0h]{base58check(payload)}/0/*")
...
def ckd_private(secret: int, chain: bytes, index: int) -> tuple[int, bytes]:
    ...
    compressed = bytes([2 | (point[1] & 1)]) + point[0].to_bytes(32, "big")
    derived = hmac.new(
        chain, compressed + index.to_bytes(4, "big"), hashlib.sha512).digest()
...
def boundary_fixture(
    accounts: list[tuple[int, bytes, str]], expressions: list[str], index: int,
    ...
    keys, script, control, script_pubkey = V.descriptor_boundary_material(
        expressions, index)
...
def funding_tx(script_pubkey: bytes, amount: int, tag: int) -> bytes:
    return (
        (2).to_bytes(4, "little") + b"\x01" + bytes([tag]) * 32 +
        (0).to_bytes(4, "little") + b"\x01\x00" +
        (0xFFFFFFFE).to_bytes(4, "little") + b"\x01" +
        amount.to_bytes(8, "little") + V.compact_size(len(script_pubkey)) +
        script_pubkey + (tag).to_bytes(4, "little")
    )


def spend_tx(prev_txid: str, prev_vout: int, amount_out: int, locktime: int,
             witness: list[bytes]) -> bytes:
    txin = (
        bytes.fromhex(prev_txid)[::-1] + prev_vout.to_bytes(4, "little") +
        b"\x00" + (0xFFFFFFFD).to_bytes(4, "little"))
    ...
    return (
        (2).to_bytes(4, "little") + b"\x00\x01\x01" + txin +
        b"\x01" + txout + witness_raw + locktime.to_bytes(4, "little")
    )
...
def mine_header(previous_hash: str, merkle_internal: bytes, height: int) -> bytes:
    prefix = (
        (4).to_bytes(4, "little") + bytes.fromhex(previous_hash)[::-1] +
        merkle_internal + (1_788_112_800 + height).to_bytes(4, "little") +
...
def signed_spend(funding: V.Transaction, amount: int, secrets: list[int],
                 script: bytes, control: bytes, script_pubkey: bytes,
                 selected_slots: list[int], locktime: int) -> bytes:
    blank_witness = [b""] * 5 + [script, control]
    provisional = V.parse_transaction(spend_tx(
        funding.txid, 0, amount - 1_000 - locktime, locktime, blank_witness))
    ...
    return spend_tx(
        funding.txid, 0, amount - 1_000 - locktime, locktime,
...
def build_fixture(root: Path) -> tuple[Path, Path]:
    ...
    payload = (
        f"tr({V.NUMS_INTERNAL_KEY.hex()},multi_a(3," +
        ",".join(expressions) + "))")
    ...
    manifest: dict[str, object] = {
        "version": 1, "descriptor": descriptor,
        "descriptor_sha256": hashlib.sha256(descriptor.encode()).hexdigest(),
        "range": [0, 10_999], "script_pubkeys": scripts,
    }
    drill_material = []
    for position, index in enumerate((0, 1000, 10_999), 1):
        secrets, script, control, script_pubkey = boundary_fixture(
            accounts, expressions, index)
        ...
        success_raw = signed_spend(
            funding, amount, secrets, script, control, script_pubkey,
            [0, 1, 2], 10 + position)
        ...
        common = {
            "schema": 1, "result": "PASS", "release_version": "2.8.0",
            "source_commit": source_commit, "source_tree": source_tree,
            "completed_at_utc": "2026-08-30T18:00:21Z",
            "bitcoin_network": "main",
            "descriptor_sha256": manifest["descriptor_sha256"],
            "descriptor_index": index, "script_pubkey_hex": script_pubkey.hex(),
            "funded_sats": amount, "funding_txid": funding.txid,
            "funding_vout": 0, "funding_raw_transaction_hex": funding_raw.hex(),
            "funding_txoutproof_hex": "00",
            "funding_block_hash": "0" * 64, "funding_confirmations": 0,
            "spend_input_index": 0, "tapleaf_script_hex": script.hex(),
            "control_block_hex": control.hex(),
        }
        success_report = {
            **common, "statement": "veld-mainnet-custody-3of5-success-v1",
            "signer_ids": [f"signer-{slot + 1}" for slot in success_slots],
            "signer_slots": success_slots, "signature_sha256": success_hashes,
            "spend_txid": success.txid,
            "spend_raw_transaction_hex": success_raw.hex(),
            "spend_txoutproof_hex": "00",
            "spend_block_hash": "0" * 64, "spend_confirmations": 0,
            "testmempoolaccept_allowed": True,
            "testmempoolaccept_raw_json": json.dumps([{
                "allowed": True, "txid": success.txid, "wtxid": success.wtxid,
        ...
        reject_report = {
            **common, "statement": "veld-mainnet-custody-2of5-rejection-v1",
            "signer_ids": [f"signer-{slot + 1}" for slot in reject_slots],
            "signer_slots": reject_slots, "signature_sha256": reject_hashes,
            "rejection_txid": reject.txid, "rejection_wtxid": reject.wtxid,
        ...
        reject_report["testmempoolaccept_raw_json"] = json.dumps([{
            "allowed": False, "reject-reason": reject_report["reject_reason"],
            "txid": reject.txid, "wtxid": reject.wtxid,
        }], sort_keys=True, separators=(",", ":")) + "\n"
        drill_material.append((
            index, amount, funding, success, success_report, reject,
            reject_report))
    ...
    for offset in range(11):
        ...
        merkle_internal = (
            bytes.fromhex(transaction.txid)[::-1] if transaction is not None
        ...
        if transaction is not None:
            ...
            proofs[transaction.txid] = (
                proof_hex, block_hash, 11 - offset)
        previous_hash = block_hash
    for index, _amount, funding, success, success_report, _reject, reject_report \
            in drill_material:
    ...
    chain_evidence = {
        ...
        "getblockchaininfo": {
            "chain": "main", "blocks": 110, "headers": 110,
            "bestblockhash": previous_hash, "initialblockdownload": False,
        },
        "headers": headers,
        "txoutproofs": [
            {
    ...
    consensus_manifest = {
        **manifest, "range": [0, 999], "script_pubkeys": scripts[:1000],
    }
    consensus_path = root / "custody-consensus-manifest.json"
    consensus_sha = write_json(consensus_path, consensus_manifest)
    drills = []
    for (index, amount, _funding, success, success_report, reject,
         reject_report) in drill_material:
        ...
        drills.append({
            "descriptor_index": index, "funded_sats": amount,
    ...
    parent = {
        ...
        "result": "PASS", "release_version": "2.8.0",
        "source_commit": source_commit, "source_tree": source_tree,
        "completed_at_utc": "2026-08-30T18:00:30Z",
        "bitcoin_network": "main",
        "descriptor_sha256": manifest["descriptor_sha256"],
        "manifest_sha256": manifest_sha, "script_range": [0, 10_999],
...
class FundedCustodyTests(unittest.TestCase):
    ...
    def validate(self) -> None:
        V.validate(
            self.parent, self.manifest, self.consensus_manifest, self.root,
            [f"signer-{slot + 1}" for slot in range(5)], TEST_TARGET)
    ...
    def test_fixture_easy_headers_fail_release_mainnet_floor(self) -> None:
        with self.assertRaisesRegex(
                V.ValidationError, r"compact target|proof of work.*floor"):
            V.validate(
                self.parent, self.manifest, self.consensus_manifest, self.root,
    ...
    def test_broken_best_chain_link_is_rejected(self) -> None:
        def mutate(evidence):
            raw = bytearray.fromhex(evidence["headers"][5][
    ...
    def test_two_signature_report_cannot_claim_three(self) -> None:
        self.mutate_subreport(
            "two_of_five_rejection",
            lambda report: report["signer_slots"].append(2))
        with self.assertRaises(V.ValidationError):
            self.validate()

    def test_success_txid_substitution_rejected(self) -> None:
        self.mutate_subreport(
            "three_of_five_success",
            lambda report: report.__setitem__("spend_txid", "f" * 64))
    ...
    def test_cross_boundary_report_reuse_rejected(self) -> None:
        ...
        second["three_of_five_success_report_path"] = \
            first["three_of_five_success_report_path"]
        second["three_of_five_success_report_sha256"] = \
            first["three_of_five_success_report_sha256"]
        write_json(self.parent, parent)
    ...
    def test_descriptor_key_order_substitution_is_rejected(self) -> None:
        ...
        manifest["descriptor_sha256"] = hashlib.sha256(
            manifest["descriptor"].encode()).hexdigest()
        manifest_sha = write_json(self.manifest, manifest)
    ...
    def test_missing_or_spent_reject_reason_is_not_threshold_evidence(self) -> None:
        def mutate(report):
            ...
            report["testmempoolaccept_raw_json"] = json.dumps(
    ...
    def test_symlinked_subreport_is_refused_before_resolution(self) -> None:
        parent = json.loads(self.parent.read_text(encoding="utf-8"))
        path = self.root / parent["threshold_spend_drills"][0][
