    ...
    if args.role != 'gui':
        ...
        for flag in ('--version', '--deployment-info'):
            result = subprocess.run(
                [str(args.binary.resolve()), flag],
                cwd=args.binary.resolve().parent,
                env=env,
                capture_output=True,
                text=True,
                timeout=30,
            ...
            if result.returncode != 0 or not result.stdout.strip():
                raise RuntimeError(
                    f'{flag} failed with Windows-only PATH: exit {result.returncode}'
                )
            checks.append(flag)
    print(
        json.dumps(
            dict(
                result='PASS_WINDOWS_SYSTEM_RUNTIME',
                role=args.role,
                imports=imports,
                clean_path_checks=checks,
            )
        )
    )
