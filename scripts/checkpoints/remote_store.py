...
class Store:
    def __init__(self, config, verifier=None):
        ...
        require(
            not self.state.is_symlink() and not self.feed.parent.is_symlink(),
    ...
    def verify(self, raw):
        ...
        try:
            ...
            result = subprocess.run(
                [self.tool, 'verify', str(target), str(len(records))],
                capture_output=True,
                timeout=20,
            )
            require(result.returncode == 0, 'Checkpoint signature verification failed')
            require(
                json.loads(result.stdout)['verified'] == len(records),
                'Checkpoint verification count mismatch',
            )
        finally:
    ...
    def transaction_path(self, transaction):
        require(
            isinstance(transaction, str) and re.fullmatch('[0-9a-f]{32}', transaction),
    ...
    def install(self, request):
        ...
        if backup.exists():
            require(read_regular(backup) == old, 'Backup identity mismatch')
        else:
        ...
        record = dict(
            ...
            checkpoint_height=after[-1]['height'],
    ...
    def finish(self, transaction, commit):
        ...
        if commit:
            require(
                record['phase'] in ('prepared', 'installed', 'committed'),
            ...
            require(
                record['phase'] in ('prepared', 'installed', 'rolled_back'),
                'Transaction cannot be rolled back',
            )
            require(
                sha(raw) in (record['before_sha256'], record['after_sha256']),
                'Rollback refuses unrelated feed change',
            )
            old = read_regular(self.state / ('feed-' + record['before_sha256'] + '.json'))
            require(sha(old) == record['before_sha256'], 'Rollback backup mismatch')
            self.verifier(old)
            if sha(raw) != record['before_sha256']:
                atomic_write(self.feed, old, 0o644)
    ...
    def health(self):
        ...
        require(
            0 <= time.time() - status['last_checked_at'] <= 3 * 3600,
            'Protected checkpoint publisher has not checked in for three hours',
        )
...
def main():
    ...
    with (store.state / 'publisher.lock').open('a+b') as lock:
        ...
        request = (
            {}
            if command in ('read', 'health')
            else json.loads(sys.stdin.buffer.read(2 * MAX_BYTES + 1))
        )
        if command == 'read':
            result = store.read()
            ...
            result = store.finish(request['transaction'], command == 'commit')
        elif command == 'checked':
            result = store.checked(request['tip'])
        elif command == 'health':
            result = store.health()
        else:
            raise ValueError('Unknown checkpoint publication operation')
