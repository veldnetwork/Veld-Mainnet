...
def main():
    ...
    if action == 'public-feed':
        ...
        print(
            json.dumps(
                dict(
                    sha256=sha(result.stdout), document_b64=base64.b64encode(result.stdout).decode()
                )
            )
        )
        return
    if action == 'observe':
        ...
        require(
            ...
            and len(heights) <= 16
            and all(type(h) is int and 0 <= h < 2**64 for h in heights),
            'Historical query range rejected',
        )
        result = subprocess.run(
            [
                '/usr/bin/bash',
                '/opt/veld-checkpoints/remote-read.sh',
                config['host'],
                ','.join(map(str, heights)) or '-',
            ],
            capture_output=True,
            timeout=80,
        )
    ...
    with (store.state / 'publisher.lock').open('a+b') as lock:
        ...
        if action == 'read':
            require(not request, 'Unexpected read fields')
            result = store.read()
        elif action == 'install':
            require(
                set(request) == {'transaction', 'base_sha256', 'document_b64'},
            ...
            require(set(request) == {'tip'}, 'Unexpected observation status fields')
