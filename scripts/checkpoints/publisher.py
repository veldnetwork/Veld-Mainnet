...
class Publisher:
    def __init__(self, config):
        ...
        require(
            sha(self.tool.read_bytes()) == config['tool_sha256'], 'Checkpoint tool identity changed'
        )
    ...
    def gateway(self, host, action, **values):
        ...
        args = [
            ...
            '-o',
            'UserKnownHostsFile=' + self.config['known_hosts'],
            '-o',
            'ConnectTimeout=12',
            '-o',
            ...
            '-i',
            str(self.credentials / 'fleet-ssh-key'),
            'veld-checkpoint-peer@' + address,
        ]
        response = subprocess.run(
            args,
            input=json.dumps(dict(action=action, **values)).encode(),
            capture_output=True,
            timeout=100,
        )
        if response.returncode:
            atomic_write(
                self.run_dir
                / ('gateway-' + host + '-' + action + '-' + str(time.time_ns()) + '.log'),
                response.stderr[:16384],
            )
    ...
    def observe(self, heights):
        for attempt in range(3):
            with ThreadPoolExecutor(max_workers=3) as pool:
                rows = list(
                    pool.map(
                        lambda host: self.gateway(host, 'observe', heights=sorted(set(heights))),
                        self.config['hosts'],
                    )
                )
            self.counter += 1
    ...
    def verify(self, raw, name):
        ...
        result = subprocess.run(
            [str(self.tool), 'verify', str(path), str(len(records))],
            capture_output=True,
            timeout=20,
        )
        require(
            result.returncode == 0 and json.loads(result.stdout)['verified'] == len(records),
            'Checkpoint signature validation failed',
        )
        return records
    ...
    def sign(self, height, block_hash):
        ...
        if candidate.exists():
            ...
            require(
                record['height'] == height and record['hash'] == block_hash,
                'Previously signed checkpoint conflicts with candidate',
            )
            return record
        password = bytearray((self.credentials / 'checkpoint-password').read_bytes())
        require(
            password and b'\n' not in password and b'\r' not in password,
            'Checkpoint credential format rejected',
        )
        password.append(10)
        try:
            result = subprocess.run(
                [
                    str(self.tool),
                    'sign',
                    str(self.credentials / 'checkpoint-keystore'),
                    str(height),
                    block_hash,
                ],
                input=password,
        ...
        require(
            record['height'] == height and record['hash'] == block_hash,
            'Signer returned a different checkpoint',
        )
    ...
    def finish(self, status, records, rows, **extra):
        ...
        result = dict(
            ...
            checkpoint_height=records[-1]['height'],
            checkpoint_hash=records[-1]['hash'],
            observed_tip=tip,
    ...
    def run(self):
        ...
        if origin['pending'] and sha(raw) != origin['sha256']:
            ...
            require(
                sha(raw) == pending['before_sha256']
                and origin['sha256'] == pending['after_sha256'],
                'Unresolved publication has unexpected public contents',
            )
            self.gateway(
                self.config['publisher_host'], 'rollback', transaction=pending['transaction']
            )
            origin = self.gateway(self.config['publisher_host'], 'read')
            raw = self.public_feed()
        ...
        require(
            base64.b64decode(origin['document_b64'], validate=True) == raw,
            'Origin response differs from public feed',
        )
        history = self.state / 'history.json'
        if history.exists():
            preserves(parse(history.read_bytes()), records)
        ...
        if origin['pending']:
            ...
            if origin['sha256'] == pending['before_sha256']:
                ...
                require(
                    origin['sha256'] == pending['after_sha256'],
                    'Unresolved publication does not match feed',
                )
                bind_candidate(records[-1]['height'], records[-1]['hash'], rows)
                self.gateway(
                    self.config['publisher_host'], 'commit', transaction=pending['transaction']
                )
                return self.finish(
                    'recovered_publication', records, rows, transaction=pending['transaction']
                )
        ...
        bind_records(records[-1:], rows)
        bind_candidate(height, block_hash, rows)
        ...
        atomic_json(
            self.run_dir / 'publication-plan.json',
            dict(transaction=transaction, base_sha256=sha(raw), after_sha256=sha(new)),
        )
        try:
            installed = self.gateway(
                self.config['publisher_host'],
                'install',
                transaction=transaction,
                base_sha256=sha(raw),
                document_b64=base64.b64encode(new).decode(),
            )
            ...
            if any(p['transaction'] == transaction for p in current['pending']):
                rolled_back = self.gateway(
                    self.config['publisher_host'], 'rollback', transaction=transaction
                )
...
def main():
    ...
    with (publisher.state / 'publisher.lock').open('a+b') as lock:
        ...
        if args.preflight:
            ...
            print(
                json.dumps(
                    dict(status='preflight_pass', tip=rows[0]['height'], feed_sha256=sha(raw))
                )
            )
        else:
            publisher.run()
