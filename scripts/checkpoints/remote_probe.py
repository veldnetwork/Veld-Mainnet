...
def unit():
    raw = subprocess.check_output(
        ['systemctl', 'show', 'veld-node.service', '--property=MainPID,ActiveState'], text=True
    )
...
data = [
    args[i + 1] if a == '--datadir' else a.split('=', 1)[1]
    for i, a in enumerate(args)
    if a == '--datadir' or a.startswith('--datadir=')
]
assert len(data) == 1
...
state = rpc('getstatedigest')
peers = rpc('getpeerinfo')
fresh = [p for p in peers if not p.get('inbound') and 0 <= p.get('peer_tip_age_s', 99999) <= 120]
matching = [
    p
    for p in fresh
    if p.get('peer_height') == chain['blocks']
    and p.get('peer_tip_hash') == chain['best_block_hash']
]
stats = Path('/proc/' + pid + '/stat').read_text().rsplit(')', 1)[1].split()
uptime = float(Path('/proc/uptime').read_text().split()[0]) - int(stats[19]) / os.sysconf(
    'SC_CLK_TCK'
)
out = dict(
    host=host,
    ...
    height=chain['blocks'],
    tip=chain['best_block_hash'],
    ibd_complete=chain['ibd_complete'],
    historical_validated=daemon['historical_validated'],
    active=before['ActiveState'] == 'active',
    pid=pid,
    uptime_seconds=uptime,
    fresh_outbound=len({p['ip'] for p in fresh}),
    matching_outbound=len({p['ip'] for p in matching}),
    state_height=state['height'],
    state_tip=state['tip_hash'],
    state_digest=state['digest'],
    clock_synchronized=subprocess.check_output(
        ['timedatectl', 'show', '-p', 'NTPSynchronized', '--value'], text=True
    ).strip()
    == 'yes',
