...
def main():
    ...
    for height, header, proof in rows:
        ...
        commands += [
            f'scan {height} {header} ' + 'f' * 64 + f' {nonce:016x} 1',
    ...
    assert result.stdout.splitlines() == expected, (
        'scan/light workspace state or known hash changed'
    )
    print(
        'PASS 28 exact full-size scan vectors interleaved with light verification, including maximum nonce'
    )
