...
assert(
    extract('injectSignatures').includes(
...
for (const [name, firstAsync] of Object.entries(asynchronousEntries)) {
    ...
    assert(
        body.indexOf('_veldAssertActiveSignerSeed(') < body.indexOf(firstAsync),
        name + ': capture before async',
    );
}
for (const name of [
    'signAndBroadcast',
    ...
    const body = extract(name);
    assert(
        /veldCrypto\.injectSignatures\([^\n;]+,\s*signerGeneration\)/.test(body),
        name + ': signer token',
    );
    assert(
        /_veldBroadcastExactSigned\(signedHex,\s*(?:seed|keyHex),\s*signerGeneration\)/.test(body),
...
assert(
    extract('_bvAttachAdmission').includes(
        'veldCrypto.sign(seed,veldCrypto.sha256d(messageHex),signerGeneration)',
    ),
);
...
assert(
    extract('bvDoRemoveLpConfirmed').includes(
        '_veldAssertActiveSignerSeed(seed,approved.signerGeneration)',
    ),
);
...
const fixtureToken = 'ab'.repeat(32),
    generation = 41,
    fixtureId = '42'.repeat(32);
let currentGeneration = generation;
const ownerHash = '19'.repeat(20),
    ownerScript = '76a914' + ownerHash + '88ac';
...
const context = vm.createContext({
    VELD_BROWSER_SELF_CUSTODY: true,
    VELD_TRUSTED_LOCAL_SIGNER: true,
    ...
    _veldAutoLockSigner() {
        throw new Error('ordinary fixture unexpectedly expired');
    },
    ...
    _veldAuthenticatePreparedPrevouts: async () => {
        events.push('parents');
    },
    ...
    rpc: async (method) => {
        if (method === 'preparerawtransaction') {
            events.push('prepare');
            return prepared;
        }
...
(async () => {
    ...
    assert.equal(
        context._veldAssertActiveSignerSeed(fixtureToken.toUpperCase(), generation),
        generation,
    );
    const signed = await context.injectSignatures(
        ordinaryBytes,
        prepared.inputs,
        fixtureToken,
    ...
    await context.injectSignatures(
        ordinaryBytes,
        [prepared.inputs[0], { sigless: true }],
        fixtureToken,
        null,
        generation,
    );
    ...
    assert.equal(
        await context._veldBroadcastExactSigned(signed, fixtureToken, generation),
        fixtureId,
    );

    for (const explicitGeneration of [undefined, generation]) {
        events.length = 0;
        const result = await context.signAndBroadcast(
    ...
    context._setCurrentAddr = (address) => {
        ++addressChecks;
        return address === context.currentAddr;
    };
    context.__veldKey.set = (token) => {
        assert.equal(token, fixtureToken);
        ++activations;
        ++currentGeneration;
    };
    context.__veldKey.clear = () => {
        ++clears;
        ++currentGeneration;
    };
