...
int main() {
    ...
    try {
        ...
        for (size_t i = 0; i < 9; ++i)
            server.RecordViolation("192.0.2.1", 0, "block_ingest_overflow");
        std::cerr.rdbuf(previous);
        const auto text = captured.str();
        if (std::count(text.begin(), text.end(), '\n') != 5 ||
            text.find("[violation]") != std::string::npos ||
        ...
        if (server.TestViolationTableSize() != 0 || server.TestViolationScore("192.0.2.1") != 0 ||
            server.TestBanPersistCount() != 0 || server.TestBansDirty() ||
            server.TestViolationLogRateTableSize() != 1)
        ...
        std::cerr.rdbuf(previous);
        if (server.TestViolationScore("192.0.2.2") != 1 ||
            captured.str().find("[violation]") == std::string::npos)
            throw std::runtime_error("unrelated diagnostic or scoring changed");
        std::cout
            << "PASS queue message, per-source throttle, zero penalty, unchanged other diagnostics; no network started\n";
