#define main original_signing_policy_main
#include "signing_policy_tests.cpp"
#undef main
...
reserve::Claim MakeOpenClaim(const reserve::State& prior, uint64_t mint_amount,
                             const std::string& tag, const std::string& recipient = RECIPIENT) {
    const uint64_t reserve_value = MINT_SATS;
    const std::vector<uint8_t> parent =
        BitcoinTx({{TaggedHash(tag + "-funding"), 0}}, {{reserve_value + 1'000, {0x51}}});
    reserve::Claim claim;
    ...
    claim.bitcoin_tx = BitcoinTx({{Hash256d(parent), 0}},
...
reserve::Claim MakeCloseClaim(const reserve::State& prior,
                              const std::vector<uint8_t>& reserve_parent, const std::string& tag) {
    reserve::Claim claim;
    ...
    claim.bitcoin_tx =
        BitcoinTx({{prior.reserve_txid, prior.reserve_vout}},
                  {{0, reserve::detail::AuthScript(reserve::Operation::CLOSE,
                                                   claim.prior_commitment, ZeroHash(), 0)}});
    claim.bitcoin_txid = Hash256d(claim.bitcoin_tx);
    return claim;
}

reserve::SpendClassification Classify(const reserve::State& state, const reserve::Claim& claim) {
    return reserve::ClassifyBitcoinReserveSpend(
        state, 0, claim.bitcoin_tx, claim.direct_parents, BtcVeldCustodySpk(),
...
reserve::Result Authorized(const reserve::Claim& claim,
    ...
    result.ok = classified.disposition == reserve::SpendDisposition::AUTHORIZED_TRANSITION;
    result.claim = claim;
...
bool IssuerAccepts(const reserve::Claim& claim, const std::vector<uint8_t>& issuer_script,
                   const reserve::State& expected_prior, uint64_t expected_supply = 0,
    ...
    const Transaction carrier =
        Carrier(MintPayload(recipient, claim.mint_amount, memo), issuer_script);
    std::string from, to, decoded;
    uint64_t amount = 0;
    const BtcVeldReserveMintPolicyContext context{expected_prior, expected_supply};
    return BtcVeldMintTemplatePolicy(carrier, issuer_script, from, to, amount, decoded, &context)
        .empty();
}

bool RelayAccepts(const reserve::Claim& claim, const std::vector<uint8_t>& fee_script) {
    const std::vector<uint8_t> proof = reserve::EncodeProof(claim);
    const Transaction carrier =
        Carrier(std::string(reserve::PUBLIC_CARRIER_PREFIX) + BytesToHex(proof), fee_script);
    std::string family;
...
int main() {
    try {
        ...
        const reserve::Claim first = MakeOpenClaim(genesis, MINT_SATS, "first-open");
        ...
        CHECK(IssuerAccepts(first, issuer_script, genesis));
        {
            ...
            const Transaction carrier =
                Carrier(MintPayload(RECIPIENT, first.mint_amount,
                                    std::string(reserve::ISSUER_MEMO_PREFIX) + BytesToHex(proof)),
                        issuer_script);
            std::string from, to, memo;
            uint64_t amount = 0;
            CHECK(
                !BtcVeldMintTemplatePolicy(carrier, issuer_script, from, to, amount, memo).empty());
        }
        ...
        for (unsigned cycle = 0; cycle < 3; ++cycle) {
            ...
            const reserve::Claim opened = MakeOpenClaim(closed, 0, tag + "-open");
            const auto open_classified = Classify(closed, opened);
            CHECK(open_classified.disposition == reserve::SpendDisposition::AUTHORIZED_TRANSITION);
            ...
            CHECK(reserve::ApplyAuthorized(closed, Authorized(opened, open_classified), 0, 0));
            ...
            const reserve::Claim close = MakeCloseClaim(closed, opened.bitcoin_tx, tag + "-close");
            const auto close_classified = Classify(closed, close);
            CHECK(close_classified.disposition == reserve::SpendDisposition::AUTHORIZED_TRANSITION);
            const uint64_t before_close_count = closed.transition_count;
            const Hash256 before_close_root = closed.transition_commitment;
            CHECK(reserve::ApplyAuthorized(closed, Authorized(close, close_classified), 0, 0));
        ...
        const std::vector<uint8_t> prior_bytes = reserve::EncodeState(reopen_prior);
        reserve::State decoded_prior;
        CHECK(reserve::DecodeState(prior_bytes.data(), prior_bytes.size(), decoded_prior));
        ...
        CHECK(!reserve::DecodeState(noncanonical_prior.data(), noncanonical_prior.size(),
                                    decoded_prior));
        const reserve::Claim reopen = MakeOpenClaim(reopen_prior, MINT_SATS, "canonical-reopen");
        ...
        CHECK(reserve::DecodeProof(canonical_bytes.data(), canonical_bytes.size(), decoded));
        ...
        CHECK(!IssuerAccepts(earlier_count, issuer_script, reopen_prior));
        reserve::Claim earlier_root = reopen;
        earlier_root.prior_commitment = genesis.transition_commitment;
        earlier_root.bitcoin_tx =
            MakeOpenClaim(reserve::State{}, MINT_SATS, "earlier-root").bitcoin_tx;
        ...
        CHECK(!IssuerAccepts(earlier_root, issuer_script, reopen_prior));

        reserve::Claim malformed_prior = reopen;
        malformed_prior.prior_reserve_txid = TaggedHash("not-empty-prior");
        CHECK(!IssuerAccepts(malformed_prior, issuer_script, reopen_prior));
        reserve::Claim overflow_prior = reopen;
        overflow_prior.prior_transition_count = UINT64_MAX;
        CHECK(!IssuerAccepts(overflow_prior, issuer_script, reopen_prior));
        reserve::State false_genesis;
        false_genesis.transition_commitment = TaggedHash("false-genesis-root");
        const reserve::Claim false_first = MakeOpenClaim(false_genesis, MINT_SATS, "false-first");
        CHECK(!IssuerAccepts(false_first, issuer_script, genesis));
        const reserve::Claim invalid_recipient = MakeOpenClaim(
            reopen_prior, MINT_SATS, "invalid-recipient", "not-a-canonical-veld-address");
        CHECK(!IssuerAccepts(invalid_recipient, issuer_script, reopen_prior, 0,
        ...
        const reserve::Claim competing = MakeOpenClaim(reopen_prior, MINT_SATS, "competing-reopen");
        CHECK(Classify(reopen_prior, competing).disposition ==
              reserve::SpendDisposition::AUTHORIZED_TRANSITION);
        CHECK(RelayAccepts(competing, fee_script));
        CHECK(IssuerAccepts(competing, issuer_script, reopen_prior));
        reserve::State advanced = reopen_prior;
        const auto reopen_classified = Classify(advanced, reopen);
        CHECK(reserve::ApplyAuthorized(advanced, Authorized(reopen, reopen_classified), 0,
                                       MINT_SATS));
        CHECK(!reserve::detail::MatchesPrior(advanced, competing));
        CHECK(Classify(advanced, competing).disposition !=
              reserve::SpendDisposition::AUTHORIZED_TRANSITION);
        CHECK(!IssuerAccepts(competing, issuer_script, advanced, MINT_SATS));
        ...
        reserve::Result rejected = Authorized(earlier_count, Classify(reopen_prior, earlier_count));
        rejected.ok = true; // exercise ApplyAuthorized's own live-state guard
        CHECK(!reserve::ApplyAuthorized(advanced, rejected, MINT_SATS, 2 * MINT_SATS));
        CHECK(reserve::EncodeState(advanced) == state_before);
        CHECK(!IssuerAccepts(reopen, issuer_script, reopen_prior, 0,
                             "VV6pcrLQvxq7uBZEFtc4qxCizQ26azxTtK"));

        const Transaction old_mnp =
            Carrier(MintPayload(RECIPIENT, MINT_SATS, std::string(64, '0') + ":0"), issuer_script);
        std::string from, to, memo;
        uint64_t amount = 0;
        CHECK(!BtcVeldMintTemplatePolicy(old_mnp, issuer_script, from, to, amount, memo).empty());
        std::string family;
        CHECK(!BtcVeldRelayTemplatePolicy(Carrier("VELD_MSPV|4d53503200", fee_script), fee_script,
                                          family)
                   .empty());

        std::cout << "PASS reopen_parity_tests checks=" << g_checks
                  << " cycles=3 final_prior_count=" << reopen_prior.transition_count << '\n';
        return 0;
    } catch (const std::exception& error) {
        std::cerr << "FAIL reopen_parity_tests: " << error.what() << '\n';
        return 1;
    }
}
