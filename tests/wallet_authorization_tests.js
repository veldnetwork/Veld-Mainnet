...
function element() {
    return {
        ...
        set innerHTML(_) {
            throw new Error('numeric display must never parse HTML');
        },
    };
}
...
let transaction = {
    inputs: [{}, {}],
    outputs: [{ value_units_str: '1000', script_pubkey_hex: ownerScript }],
};
let signed = 0,
    authenticated = 0,
    failBroadcast = false;
const prep = {
    unsigned_tx_hex: 'ordinary-parser-fixture',
    inputs: [{}, {}],
    inputs_consolidated: 99999,
    verified_consolidation_inputs: 99999,
    verified_consolidation_output_units: '99999',
};
...
const call = (budget) =>
    signing.signAndBroadcast(
        'prepareconsolidatetx',
        ['fixture-owner'],
        'inert-seed-placeholder',
        null,
        null,
        null,
        null,
...
(async () => {
    ...
    for (const candidate of [
        ...
        {
            ...valid,
            outputs: [{ value_units_str: '1000', script_pubkey_hex: 'ordinary-other-script' }],
        },
    ...
    vm.runInContext(
        ['_veldAutomaticConsolidationBudget', 'autoConsolidateRun', 'autoConsolidateMaybe']
            .map(extract)
            .join('\n'),
        signing,
    );
    ...
    console.log(
        'PASS: countdown text nodes; owned UTXO reduction; local progress; concurrent fee reservation; failed-broadcast accounting; scheduler cannot renew authority',
    );
})().catch((error) => {
    console.error(error);
    process.exitCode = 1;
