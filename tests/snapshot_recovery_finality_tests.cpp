...
void Mine(VeldNode& node, const RealKeyPair& key) {
    std::string error;
    Check(node.BindGenerationIdentity(key, &error), "generation identity");
...
void Install(VeldNode& node, const fq::EpochSnapshot& snapshot) {
    ...
    record.pubkey_hex = snapshot.entries[0].pubkey_hex;
    state.validators[record.pubkey_hex] = record;
    node.GetValidators().RestoreState(state);
    Check(node.TestInstallFinalityEvidenceSnapshot(snapshot), "retained membership fixture");
}
int main(int argc, char** argv) {
    try {
        Check(argc == 2, "explicit disposable directory required");
        const auto root = fs::absolute(argv[1]);
        Check(!fs::exists(root), "preserve previous fixtures");
        auto config = RegtestConfig();
        config.port = 31141;
        auto key = GenerateKeyPair(true);
        key.address = PubKeyToAddress(key.public_key, false);
        key.script_override = AddressToScript(key.address);
        const auto signer = dilithium::Generate();
        fq::SnapshotEntry member;
        member.pubkey_hex = BytesToHex(signer.public_key.data(), signer.public_key.size());
        ...
        snapshot.entries = {member};
        snapshot.total_weight = member.weight;
        snapshot.root = fq::SnapshotRoot(snapshot.entries, 0, 0, snapshot.total_weight);
        Hash256 genesis{};
        const auto genesis_wire = HexToBytes(GENESIS_HASH);
        ...
        fq::SignedVote a;
        a.epoch_id = 0;
        a.set_root = snapshot.root;
        a.phase = fq::Phase::PREVOTE;
        a.round = fq::CheckpointRound(20);
        a.target.height = 20;
        a.target.hash.fill(0x31);
        a.pubkey_hex = member.pubkey_hex;
        auto b = a;
        b.target.hash.fill(0x32);
        const auto sign = [&](fq::SignedVote& vote) {
            vote.signature = dilithium::Sign(
                signer.secret_key,
                fq::VotePreimage(fq::NETWORK_ID, genesis, vote.epoch_id, vote.set_root, vote.phase,
        ...
        Check(original.Offer(*evidence) == fq::FinalityEquivocationCollector::OfferResult::INSERTED,
              "completed evidence retained");
        const auto wire = original.Encode();
        {
            VeldNode before(config, root.string());
            before.SetQuietBoot(true);
            before.Start();
            Mine(before, key);
            Install(before, snapshot);
            sr::Write(root / "finality-equivocation.evj", wire);
            before.TestLoadFinalityEvidence();
            Check(before.TestFinalityEvidenceCount() == 1,
                  "legitimate nonempty journal loads before recovery");
            before.RejectIndependentBackgroundValidation("independent-tip-mismatch");
            Check(sr::RejectionExitCode(root) == 75,
                  "background rejection requests recovery restart");
            before.Stop();
        }
        {
            VeldNode recovered(config, root.string());
            recovered.SetQuietBoot(true);
            recovered.SetFullIbd(true);
            recovered.Start();
            Check(recovered.GetChain().Height() == 0 && recovered.IsRunning(),
                  "fresh sync can start without later validator history");
            Check(recovered.TestFinalityEvidenceCount() == 0,
                  "unverified old evidence is not active");
            const auto t = sr::Load(root);
            Check(t.has_value(), "recovery journal retained");
            const auto saved = root / (".snapshot-rejected-" + t->id) / "finality-equivocation.evj";
            Check(sr::Read(saved, wire.size()) == wire,
                  "original signed evidence preserved byte for byte");
            recovered.SetIBDComplete(true);
            Check(!recovered.IsIBDComplete(),
                  "unreconstructed evidence prevents premature promotion");
            Mine(recovered, key);
            Install(recovered, snapshot);
            recovered.SetIBDComplete(true);
            Check(recovered.IsIBDComplete(), "restored membership permits completion");
            Check(recovered.TestFinalityEvidenceCount() == 1,
                  "evidence restored after authentication");
            Check(sr::Read(root / "finality-equivocation.evj", wire.size()) == wire,
                  "restored evidence durably published");
            std::string error;
            Check(channel::secure_file::AtomicWriteText(
                      (root / "miner.key").string(), "disposable receipt binding\n", &error, true),
                  "role binding");
            Check(snapshot_bootstrap::WriteFullIbdReceipt(
                      root.string(), key, 20, HashToHex(recovered.GetChain().TipCopy().GetHash()),
                      &error),
                  "real signed recovery receipt");
            snapshot_bootstrap::FullIbdReceipt parsed_receipt;
            Check(!snapshot_bootstrap::VerifyFullIbdReceipt(root.string(), key.public_key,
                                                            parsed_receipt, &error),
                  "default receipt verification cannot bypass revocation");
            Check(snapshot_bootstrap::VerifyFullIbdReceipt(root.string(), key.public_key,
                                                           parsed_receipt, &error, true),
            ...
            auto refused_completion = [&](const Secp256k1PubKey& identity) {
                bool refused = false;
                try {
                    recovered.CompleteSnapshotRecovery(snapshot_bootstrap::RECEIPT_FILENAME,
                                                       identity);
                } catch (const std::runtime_error&) {
                    refused = true;
                }
                Check(refused && sr::Pending(root),
                      "invalid receipt cannot retire recovery barriers");
            };
            auto other_identity = key.public_key;
            other_identity[0] ^= 1;
            refused_completion(other_identity);
            sr::Write(receipt_path, "truncated receipt\n");
            refused_completion(key.public_key);
            Check(snapshot_bootstrap::WriteFullIbdReceipt(
                      root.string(), key, 19, HashToHex(recovered.GetChain().TipCopy().GetHash()),
                      &error),
                  "signed wrong-height fixture");
            refused_completion(key.public_key);
            ...
            Check(sr::Load(root)->phase == "complete",
                  "receipt-bound completion was not committed before cleanup failure");
#else
            ...
            auto committed = *sr::Load(root);
            committed.phase = "complete";
            ...
            Check(!sr::Pending(root),
                  "recovery completes only after evidence and receipt persistence");
            Check(sr::Read(saved, wire.size()) == wire, "completion retains original evidence");
            recovered.TestLoadFinalityEvidence();
            Check(recovered.TestFinalityEvidenceCount() == 1,
                  "restored journal authenticates on reload");
            recovered.Stop();
        }
