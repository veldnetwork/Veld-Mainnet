...
int main() {
    try {
        ...
        for (const uint64_t height : {2879ULL, 2880ULL, 3839ULL, 3840ULL, 3841ULL, 3900ULL}) {
            ...
            for (const uint64_t amount :
                 {0ULL, 499ULL * VELD_UNITS, 500ULL * VELD_UNITS, 1000ULL * VELD_UNITS - 1,
                  1000ULL * VELD_UNITS, 1000ULL * VELD_UNITS + 1}) {
                stake = amount;
                const bool eligible = amount >= 1000 * VELD_UNITS;
                Check(chain.NmsBondSatisfied(script, height) == eligible,
                      "co-mining stake verdict at height " + std::to_string(height) + " for " +
                          std::to_string(amount) + " units");
                const auto outputs =
                    chain.ComputeExpectedPoolOutputs(previous, pool_amount, height);
