...
for (const t of vectors) {
    ...
    const p = {
        ...
        anchor_veld: av,
        anchor_btcveld: ab,
        fee_model: h >= 100 ? c.BV_FLAT_FEE_MODEL : c.BV_FEE_MODEL,
        quote_height: h,
        fee_model_active: true,
    };
    const q = c.bvQuote(p, !!d, a),
        n = t.native;
    ...
    if (!pq.reject && pn[3] > 0) {
        const got = [
            ...
            pq.postV,
            pq.postB,
            Number(pq.rebalances),
        ].map(String);
        assert.deepEqual(got, pn.slice(1, 9).map(String));
    }
    assert(
        pub.bvQuote(
            { ...pp, fee_model: h >= 9500 ? pub.BV_FEE_MODEL : pub.BV_FLAT_FEE_MODEL },
            !!d,
            a,
        ).reject,
    );
...
console.log(
    JSON.stringify({
        status: 'PASS',
        wallet_native_cases: count,
        full_scripts_parsed: scriptCount,
        scope: 'actual embedded JS quote functions; no GUI or signing E2E',
    }),
);
