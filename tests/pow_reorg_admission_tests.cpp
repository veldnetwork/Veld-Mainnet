...
int main() {
    ...
    if (HashToHex(main_blocks[2].GetHash()) > HashToHex(fork[2].GetHash()))
        main_blocks.swap(fork);
    for (size_t i = 0; i < 3; ++i) {
        auto admitted = chain.AddBlockDirect(main_blocks[i], false, true, false,
    ...
    chain.SetDurableBlockBodyWriter([&](const Hash256& hash, const std::vector<uint8_t>& raw) {
    ...
    chain.SetHistoricalBlockLoader([&](const Hash256& hash) -> std::optional<std::vector<uint8_t>> {
        auto it = durable_bodies.find(HashToHex(hash));
        if (it == durable_bodies.end())
            return std::nullopt;
        return it->second;
    });
    ...
    chain.SetOnCommit([&](const Block& committed, const std::vector<std::pair<Hash256, uint32_t>>&,
    ...
    // retryable, with no durable side write before the successful retry.
    {
        ...
        for (size_t i = 0; i < 3; ++i) {
            auto mined = MineAndCommit(dataset_chain, dataset_mempool, dataset_miner);
            if (!mined.success)
                throw std::runtime_error("dataset fixture mining failed");
        }
        size_t dataset_writer_calls = 0;
        dataset_chain.SetDurableBlockBodyWriter([&](const Hash256&, const std::vector<uint8_t>&) {
        ...
        const auto dataset_deferred = dataset_chain.AddBlockDirect(
            dataset_candidate, false, true, false, PowAdmissionContext::Internal());
        Check(dataset_deferred.IsDeferred() &&
                  Blockchain::GetLastRejectTag() == "pow_reorg_dataset_unavailable",
              "replay dataset refusal returns deferred tri-state");
        Check(dataset_chain.VolatileSideQuarantineCount() == 1 &&
                  dataset_chain.BadAltTipCount() == 0 && dataset_writer_calls == 0,
        ...
        for (const auto& tip : dataset_chain.GetChainTips()) {
            if (tip.hash == dataset_hash)
                deferred_tip_exposed = true;
        }
        Check(!dataset_chain.GetBlockByHash(dataset_hash).has_value() &&
                  !dataset_chain.GetKnownBlockHeightByHash(dataset_hash).has_value() &&
                  !dataset_chain.HasBlockAtHeight(dataset_candidate.height, dataset_hash_hex) &&
    ...
    for (size_t i = 0; i < fork.size(); ++i) {
        ...
        const auto admission = chain.AddBlockDirect(fork[i], false, true, false, context);
        observe_peer_boundary(admission);
        if (admission.IsDeferred()) {
            const std::string tag = Blockchain::GetLastRejectTag();
            saw_deferred = tag == "pow_peer_reorg_budget_exhausted" ||
    ...
    Check(published_reorg_blocks == 0, "deferred reorg invokes no durable publication callback");
    Check(durable_side_writes == 3 && peer_credit_events == 3 && relay_events == 3 &&
              on_block_events == 3,
    ...
    Check(chain.BadAltTipCount() == 0, "deferred branch is not consensus-blacklisted");
    Check(chain.SideBranchHeaderCount() >= 4 && chain.SideBranchPowAdmissionCount() >= 4,
    ...
    const size_t effects_before_retry =
        durable_side_writes + peer_credit_events + relay_events + on_block_events;
    const auto immediate_retry = chain.AddBlockDirect(fork.back(), false, true, false, context_c);
    observe_peer_boundary(immediate_retry);
    Check(immediate_retry.IsDeferred() && Blockchain::GetLastRejectTag() == "reorg_retry_backoff",
          "exact side-body retry is bounded before state rollback");
    Check(durable_side_writes + peer_credit_events + relay_events + on_block_events ==
    ...
    Check(chain.BadAltTipCount() == 0 && chain.Tip().GetHash() == original_tip &&
              published_reorg_blocks == 0,
          "second transient refusal remains non-publishing and non-blacklisting");

    std::this_thread::sleep_for(std::chrono::milliseconds(1'100));
    const auto pre_refill_retry = chain.AddBlockDirect(fork.back(), false, true, false, context_c);
    observe_peer_boundary(pre_refill_retry);
    Check(pre_refill_retry.IsDeferred() &&
              Blockchain::GetLastRejectTag() == "pow_peer_reorg_budget_exhausted",
    ...
    std::this_thread::sleep_for(retry_window + std::chrono::milliseconds(100));
    reject_body_writes_once.insert(HashToHex(fork.back().GetHash()));
    const auto persist_deferred = chain.AddBlockDirect(fork.back(), false, true, false, context_c);
    observe_peer_boundary(persist_deferred);
    Check(persist_deferred.IsDeferred() &&
              Blockchain::GetLastRejectTag() == "reorg_candidate_body_persist_failed",
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(1'100));
    const auto completed = chain.AddBlockDirect(fork.back(), false, true, false, context_c);
    observe_peer_boundary(completed);
    Check(completed.IsAccepted(), "retained side branch succeeds after bounded refill");
    Check(chain.Height() == 4 && chain.Tip().GetHash() == fork.back().GetHash(),
    ...
    Check(durable_side_writes == fork.size() && peer_credit_events == fork.size() &&
              relay_events == fork.size() && on_block_events == fork.size() &&
              chain.VolatileSideQuarantineCount() == 0,
          "winning volatile tip is durably published only after complete replay");
    Check(chain.BadAltTipCount() == 0, "successful retry leaves no false bad-tip residue");
    Check(source_c->Stats().attempts == 0,
          "completed retry still charges original per-block sources");
    Check(chain.SideBranchPowAdmissionCount() <= chain.SideBranchHeaderCount(),
          "admission provenance cardinality stays side-index bounded");

    const size_t effects_before_known =
        durable_side_writes + peer_credit_events + relay_events + on_block_events;
    const auto known_retransmit = chain.AddBlockDirect(fork.back(), false, true, false, context_c);
