...
#define CHECK(expr)                                                                                \
    do {                                                                                           \
        ++g_checks;                                                                                \
        if (!(expr)) {                                                                             \
            throw std::runtime_error(std::string("check failed: ") + #expr + " at line " +         \
  ...
  public:
    ScopedTempDir() {
        ...
        path_ = std::filesystem::temp_directory_path() /
                ("veld-security-test-token-dependencies-" + std::to_string(suffix));
...
int main() {
    try {
        ...
        CHECK(standalone.TestHasBtcVeldConsensusDependencies(nullptr, nullptr));

        ScopedTempDir data_dir;
        veld::VeldNode node(veld::RegtestConfig(), data_dir.path().string());
        ...
        const auto header_digest_before = tokens.TestBoundBtcHeaderDigest();
        ...
        CHECK(node.TestMainBtcHeaderChain().StateDigest() != header_digest_before);
        CHECK(tokens.TestBoundBtcHeaderDigest() == node.TestMainBtcHeaderChain().StateDigest());

        const auto covenant_digest_before = tokens.TestBoundBtcVeldRedeemCovenantDigest();
        auto covenant_state = node.TestMainBtcVeldRedeemCovenant().SnapshotState();
        covenant_state.insurance_fund_sats = 17;
        node.TestMainBtcVeldRedeemCovenant().RestoreState(covenant_state);
        CHECK(node.TestMainBtcVeldRedeemCovenant().Digest() != covenant_digest_before);
        ...
        (void)tokens.BuildPostBlockPreview(invalid_block, closed_gate, preview);
        CHECK(preview.TestHasBtcVeldConsensusDependencies(&node.TestMainBtcHeaderChain(),
        ...
        veld::btcspv::BtcHeaderChain preview_headers(veld::BtcVeldCheckpoint(),
                                                     veld::BtcVeldPowLimit(), 2016, 1209600,
                                                     veld::BtcVeldNoRetarget());
        ...
        CHECK(preview.TestHasBtcVeldConsensusDependencies(&preview_headers, &preview_covenant));
        CHECK(!preview.TestHasBtcVeldConsensusDependencies(&node.TestMainBtcHeaderChain(),
                                                           &node.TestMainBtcVeldRedeemCovenant()));

        std::cout << "PASS node_token_dependency_tests checks=" << g_checks << '\n';
        return 0;
    } catch (const std::exception& error) {
        std::cerr << "FAIL node_token_dependency_tests: " << error.what() << '\n';
        return 1;
    }
}
