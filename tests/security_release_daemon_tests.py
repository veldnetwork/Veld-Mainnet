...
def main():
    ...
    result_path = build / ('daemon-results' + suffix + '.json')
    ...
    report = {
        ...
        'binaries': {
            ...
            for name in (
                'release-integration-node.exe',
                'veld-validator.exe',
                'veld-node.exe',
                'regtest-node-helper.exe',
                'bitcoin-cli-local.exe',
            )
        },
    ...
    def start_validators(phase):
        for index in range(1, 8):
            ...
            cmd = [
                ...
                str(node.directory),
                '--bitcoin-cli',
                str(build / 'bitcoin-cli-local.exe'),
            ]
    ...
    def finalize(target=None):
        height = node.control('status')['height']
        ...
        while time.monotonic() < deadline:
            assert all(proc.poll() is None for proc, _, _ in processes), (
                'validator exited; inspect private test logs'
            )
            ...
            if (
                ...
                and len(raw) >= offset + 8
                and int.from_bytes(raw[offset : offset + 8], 'little') == target
            ):
                break
            node.control('maintain')
            time.sleep(1)
        else:
            raise TimeoutError('standalone validators did not assemble a precommit certificate')
        node.generate(1)
        peg = node.rpc('getpeginfo')
    ...
    try:
        node = Node(
            build / 'release-integration-node.exe', run / ('node-daemon-integration' + suffix)
        )
        node.control('import_history', name='final-history.bin')
        node.control('synchronize')
        wait_admission(node, require_peer=False)
        keys = subprocess.run(
            [str(build / 'release-identities.exe'), str(run), str(node.directory), str(private)],
            env=env,
            capture_output=True,
            text=True,
            check=True,
        ...
        bitcoin_dir = run / ('bd' + str(a.attempt))
        assert not bitcoin_dir.exists()
        ...
        env.update(
            VELD_TEST_BITCOIN_CLI=str(run.parent / 'first-activation-tools/bitcoin-cli.exe'),
            VELD_TEST_BITCOIN_DATADIR=str(bitcoin_dir),
            VELD_TEST_BITCOIN_RPC_PORT=str(core.port),
            VELD_TEST_BITCOIN_OBSERVATIONS=str(build / ('bitcoin-observations' + suffix + '.log')),
        )
        start_validators('initial')
        finalize(3360)
        ...
        node.control('fa_anchor', proof=wire.hex())
        node.generate(1)
        ...
        assert (
            anchor_state['btc_observed_checkpoint_height']
        ...
        report['checks'][
            'daemon_processes_independently_query_real_bitcoin_core_before_finalizing_observation'
        ] = True
        report['observation_count'] = len(observations)
        report['anchor'] = anchor_state
        ...
        journal_hashes = {
            path.parent.name: hashlib.sha256(path.read_bytes()).hexdigest() for path in journals
        }
        stop_validators()
        start_validators('restarted')
        time.sleep(8)
        assert all(proc.poll() is None for proc, _, _ in processes)
        report['checks']['all_seven_daemons_restart_from_durable_journals'] = True
        finalize()
        assert any(
            hashlib.sha256(path.read_bytes()).hexdigest() != journal_hashes[path.parent.name]
        ...
        report['final_state'] = node.control('status')
        ...
        if core and hasattr(core, 'process'):
            try:
                ...
                if core.process.poll() is None:
                    ...
                    core.process.wait(timeout=30)
                if hasattr(core, 'log') and not core.log.closed:
                    core.log.close()
        if node:
            node.stop()
        if node and (node.directory / 'rpc.token').exists():
            (node.directory / 'rpc.token').unlink()
        env.pop('VELD_VAULT_PASSPHRASE', None)
        report['stopped'] = (
            not processes
            and (node is None or node.process.poll() is not None)
            and (core is None or core.process.poll() is not None)
        )
