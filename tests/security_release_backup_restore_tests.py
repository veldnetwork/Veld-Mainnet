...
binary = build / 'release-integration-node.exe'
result_path = build / ('backup-restore-results' + suffix + '.json')
...
request = {k: v for k, v in prior['request'].items() if k != 'request_id'}
report = {
    'status': 'running',
    'checks': {},
...
nodes = []
try:
    source = Node(binary, run / 'node-upgrade-recovery', '-backup-source')
    nodes.append(source)
    state = source.control('status')
    assert source.control('fa_request', request_id=request_id) == request
    source.stop()
    backup = run / ('upgrade-recovery-cold-backup' + suffix)
    before = inventory(source.directory)
    receipt = private_copy(source.directory, backup, run)
    assert inventory(backup) == before
    report['checks']['cold_backup_preserves_every_byte_and_windows_acl'] = True
    report['copy_evidence'] = [receipt]
    restored_dir = run / ('node-upgrade-restored' + suffix)
    report['copy_evidence'].append(private_copy(backup, restored_dir, run))
    restored = Node(binary, restored_dir)
    nodes.append(restored)
    assert (
        restored.ready['height'] == state['height'] and restored.ready['digest'] == state['digest']
    )
    ...
    peer_dir = run / ('node-upgrade-peer' + suffix)
    report['copy_evidence'].append(private_copy(backup, peer_dir, run))
    peer = Node(binary, peer_dir)
    nodes.append(peer)
