...
def main():
    ...
    try:
        source = Node(binary, run / ('node-sync-source' + suffix))
        nodes.append(source)
        ...
        fresh = Node(binary, run / ('node-sync-fresh' + suffix))
        nodes.append(fresh)
        ...
        while time.monotonic() - start < a.timeout_seconds:
            source.control('maintain')
            fresh.control('maintain')
            ...
            if state['height'] == source_status['height']:
                assert (
                    state['tip'] == source_status['tip']
                    and state['digest'] == source_status['digest']
                )
                break
            if time.monotonic() - last_progress >= 60:
                row = {
                    ...
                    'height': state['height'],
                    'target_height': source_status['height'],
                    'peer_ips': state['ready_peer_ips'],
                    'admission': state['admission'],
                }
                report['progress'].append(row)
                save()
                print('SYNC ' + json.dumps(row), flush=True)
                last_progress = time.monotonic()
            if time.monotonic() - last_advance > 600:
                raise TimeoutError(
                    'P2P synchronization made no progress for 10 minutes at ' + str(last_height)
                )
            time.sleep(5)
        else:
            raise TimeoutError('complete P2P download deadline exceeded')
        ...
        assert (
            archive['migration_active']
            and archive['hot_requests'] == 0
        ...
        fresh.stop()
        report['checks']['clean_shutdown_after_full_download'] = True
        restarted = Node(binary, fresh.directory, '-clean-restart')
        ...
        restarted.stop()
        recovered = Node(binary, fresh.directory, '-crash-restart')
        ...
        before = hashes(fresh.directory)
        private_copy(fresh.directory, backup, run)
        ...
        report['final_state'] = source_status
