...
def main():
    ...
    report = {
        ...
        "checks": {},
        "native_issuance_tested": False,
        "production_access": False,
        "independent_bitcoin_process": bool(args.core),
    ...
    report["source_sha256"] = {
        ...
        for path in [
            ...
            for name in (
                "rtp1_service.py",
                "rtp1_service_runtime.py",
                "rtp1_backing_evidence.py",
                "rtp1_mint_policy.py",
                "veld_signerd.py",
                "veld_wt_reserve.py",
                "rpc_url_policy.py",
        ...
        + [
            ...
            source / "tests/rtp1_service_process_checks.py",
            source / "tests/rtp1_descriptor_process_checks.py",
            source / "swap/veld_custody_binding.py",
        ]
    }
    core = None
    try:
        with (
            tempfile.TemporaryDirectory(prefix="rtp1-backing-check-") as temporary,
            ...
            issuer = re.search(
                r"^address:\s+(\S+)\s*$", run([args.keygen, "show", key]), re.M
            ).group(1)
            ...
            if args.core:
                ...
                transaction = core.send(
                    [funding], [{address: btc(auth["sats"])}, {"data": auth["payload"]}]
                )
                core.mine(140)
            ...
            payload = {
                "issuer_script_hex": prepared["inputs"][0]["prev_script_hex"],
            ...
            facts = evidence.validate_backing_facts(
                json.loads(run([args.keygen, "inspect-rtp1-backing-stdin"], json.dumps(payload)))
            )
            ...
            run(
                [args.keygen, "inspect-rtp1-backing-stdin"],
                json.dumps(payload)[:-1] + ',"extra":1}',
                expected=2,
            )
            ...
            parent_request = {
                "issuer_script_hex": payload["issuer_script_hex"],
                "unsigned_tx_hex": payload["unsigned_tx_hex"],
                "parent_transactions": [row["parent_tx_hex"] for row in prepared["inputs"]],
            }
            signing_evidence = json.loads(
                run([args.keygen, "prepare-signing-stdin"], json.dumps(parent_request))
            )
            ...
            run(
                [
                    ...
                    signing_evidence["operation_identity_digest"],
            ...
            assert "PASS signed-inputs=" in run(
                [args.fixture, "--verify-signed", issuer, prepared_path, signed]
            )
            ...
            if core:
                policy = {
                    ...
                    "expected_genesis": core.rpc("getblockhash", [0]),
                    "custody_script_hex": auth["custody_script_hex"],
                ...
                if args.service_lifecycle:
                    ...
                    report["service_lifecycle"] = exercise(
                        root,
                        run,
                        args.keygen,
                        args.fixture,
