...
check(
    "$role != keygen && $role != validator && $role != gui" in windows,
...
version_match = re.search(r'CLIENT_VERSION\s*=\s*"([^"]+)"', version_header)
check(version_match is not None, "core version declaration is missing")
check(
    version.stdout.strip() == f"Veld Operator {version_match.group(1)}",
    "operator and core versions differ",
)
...
check(
    info["client_version"] == version_match.group(1),
    "operator deployment identity and core versions differ",
)
check(
...
print(f"PASS release_controller_oracle_tests checks={checks}")
