...
const quick = element(quickTag),
    submit = element(submitTag);
const walletPage = element(),
    stakingPage = element(),
    stakingNav = element();
...
const context = vm.createContext({
    ...
    __opLock() {
        ++transactionCalls;
        throw new Error('Submission reached operation lock');
    },
    rpc() {
        ++transactionCalls;
        throw new Error('Unexpected transaction RPC');
    },
});
vm.runInContext(
    ['nav', 'setStakingActivationUi', 'stakingActivationErrorText', 'doStake']
        .map(extract)
        .join('\n'),
    context,
);
const handlers = [...source.matchAll(/h9c6994df: (function\(event\)\{[^\n]+\})\s*,/g)].map(
    (match) => vm.runInContext('(' + match[1] + ')', context),
);
...
console.log(
    'PASS wallet stake navigation: initial state, ' +
        pageLoads +
