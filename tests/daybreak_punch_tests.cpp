...
std::shared_ptr<Connection> MakeConnection(const std::string& ip,
                                           uint16_t port,
                                           bool capable = true) {
    ...
    conn->MarkAdvertisedServices(
        veld::net::MessageType::NODE_FULL |
...
template <size_t N>
std::vector<uint8_t> Bytes(const std::array<uint8_t, N>& value) {
    return std::vector<uint8_t>(value.begin(), value.end());
}

template <size_t N>
void Append(std::vector<uint8_t>& out,
            const std::array<uint8_t, N>& value) {
    out.insert(out.end(), value.begin(), value.end());
}

void AppendEndpoint(std::vector<uint8_t>& out,
...
int main() {
    ...
    const auto hello_bounds =
        Connection::BoundsForCommand(veld::net::MessageType::PUNCHHELLO);
    const auto get_bounds =
        Connection::BoundsForCommand(veld::net::MessageType::GETPUNCH);
    const auto req_bounds =
        Connection::BoundsForCommand(veld::net::MessageType::PUNCHREQ);
    const auto fwd_bounds =
        Connection::BoundsForCommand(veld::net::MessageType::PUNCHFWD);
    const auto list_bounds =
        Connection::BoundsForCommand(veld::net::MessageType::PUNCHLIST);
    ...
    const std::array<uint8_t, 16> target_nonce{
        0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
        0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f};
    const std::array<uint8_t, 16> request_nonce{
        0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27,
        0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f};
    const std::array<uint8_t, 16> other_nonce{
    ...
    server.TestDispatchPeerMessageWithState(
        target_state, *target,
        P2PMessage(veld::MAINNET_MAGIC,
                   veld::net::MessageType::PUNCHHELLO,
                   Bytes(target_nonce)));
    ...
    server.TestDispatchPeerMessageWithState(
        requester_state, *requester,
        P2PMessage(veld::MAINNET_MAGIC,
                   veld::net::MessageType::PUNCHREQ, req));
    ...
    server.TestPrimePunchClientExchange(coordinator, other_nonce,
                                        request_nonce, now + 60, now + 60);
    ...
    server.TestDispatchPeerMessageWithState(
        ...
        P2PMessage(veld::MAINNET_MAGIC,
                   veld::net::MessageType::PUNCHLIST, list));
    ...
    server.TestDispatchPeerMessageWithState(
        coordinator_state, *coordinator,
        P2PMessage(veld::MAINNET_MAGIC,
                   veld::net::MessageType::PUNCHFWD, fwd));
    ...
    server.TestPrimePunchClientExchange(coordinator, target_nonce,
                                        request_nonce, now + 60, now + 60,
                                        true, false);
    server.TestDispatchPeerMessageWithState(
        coordinator_state, *coordinator,
    ...
    NodeServer budget_server(0, veld::MAINNET_MAGIC,
                             budget_chain, budget_mempool);
    for (size_t i = 0; i < 8; ++i)
        CHECK(budget_server.TestTakePunchDialBudget());
    CHECK(!budget_server.TestTakePunchDialBudget());

    std::cout << "PASS daybreak_punch_tests checks=" << checks
              << " authorized_dials="
              << server.TestAuthorizedPunchDialCount() << '\n';
    return 0;
}
