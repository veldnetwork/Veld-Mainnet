...
#error "Work-admission process fixture requires VELD_TEST_HOOKS"
#endif
#if defined(VELD_PUBLIC_RELEASE) || defined(VELD_PUBLIC_MAINNET)
#error "Work-admission process fixture must never compile in a public profile"
#endif
...
inline veld::SignedInput CountedBuildScriptSig(const veld::Secp256k1PrivKey& private_key,
    ...
    return veld::BuildScriptSig(private_key, public_key, transaction, input_index, previous_script);
}
...
static_assert(std::string_view(DEPLOYMENT_PROFILE_ID) != "veld-public-mainnet-v2",
...
void Check(bool condition, const std::string& label) {
    ++checks;
    if (!condition)
        throw std::runtime_error("FAIL: " + label);
}
...
std::string RpcRequest(const std::string& method, const std::vector<std::string>& params = {}) {
...
std::string EncodeSubmitBindingForBlock(const work_admission::Binding& issued,
                                        const Block& candidate) {
    auto bound = issued;
    bound.subject.target_hash = candidate.header.GetTemplateWorkIdentity();
...
std::optional<PublishedBlockTemplate> ParsePublishedBlockTemplate(const std::string& response) {
    ...
    if (!rpc_error || rpc_error->kind != btc_buy::JsonValue::Kind::Null || !result ||
    ...
    if (!block_hex || !binding || !token || block_hex->kind != btc_buy::JsonValue::Kind::String ||
        binding->kind != btc_buy::JsonValue::Kind::String ||
        token->kind != btc_buy::JsonValue::Kind::String || token->text.size() != 64)
        return std::nullopt;
    ...
    if (consumed == 0 || consumed != bytes.size() || block.Serialize() != bytes || !decoded ||
        work_admission::EncodeBinding(*decoded) != binding->text)
        return std::nullopt;
    block.height = decoded->subject.height;
    if (decoded->subject.target_hash != block.header.GetTemplateWorkIdentity())
        return std::nullopt;
    return PublishedBlockTemplate{std::move(block), binding->text, token->text};
}
...
void SolvePublishedBlock(Block& block) {
    ...
    for (uint64_t nonce = 0; nonce < 5'000'000; ++nonce) {
        ...
        if (mining::VeldHash(block.header.Serialize(), block.height) < target) {
            Check(Blockchain::VerifyBlockPoW(block), "published block has valid authoritative PoW");
            return;
        }
    }
...
void WaitForTemplateTimestamp(const Blockchain& chain) {
    ...
    throw std::runtime_error("wall clock did not advance beyond template parent MTP");
}
...
PeerFixture InstallSafePeers(VeldNode& node, uint64_t height, uint16_t port_base) {
    auto& server = node.TestWorkAdmissionProcessServer();
    server.TestSetPeerHeightClock(100);
    PeerFixture peers{MakeConnection("10.71.0.1", port_base),
    ...
    Check(server.TestVerifiedPeerEvidenceCount() == 2 && view.verified_height == height,
          "two distinct peers publish exact canonical-tip observations");
    return peers;
}

void InstallTrustedChain(Blockchain& chain, Mempool& mempool, const RealKeyPair& miner,
                         uint64_t tip_height) {
    Check(chain
    ...
    for (uint64_t height = 1; height <= tip_height; ++height) {
        ...
        Check(mined.success && mined.block.height == height, "fresh fixture block constructed");
        Check(chain
                  .AddBlockDirect(mined.block, true, true, false,
                                  mining::PowAdmissionContext::Internal())
                  .IsAccepted(),
...
VeldNode::TestWorkAdmissionProcessState StateForRow(const std::string& row) {
    auto state = ReadyState();
    if (row == "node_running")
        state.node_running = false;
    else if (row == "startup_replay")
        state.startup_replay_complete = false;
    else if (row == "independent_validation")
        state.independent_validation_complete = false;
    else if (row == "sync_ibd")
        state.sync_complete = false;
    else if (row == "snapshot_clean")
        state.snapshot_state_clean = false;
    else if (row == "durable_state")
        state.durable_state_proven = false;
    else if (row == "datadir_identity")
        state.datadir_identity_valid = false;
    else if (row == "checkpoint_anchor")
        state.checkpoint_anchor_valid = false;
    else if (row == "tip_known")
        state.canonical_tip_known = false;
    else if (row == "runtime")
        state.runtime_open = false;
    else if (row == "role_profile")
...
bool IsStartupListenerUnavailableRow(const std::string& row) {
    return row == "default_unknown" || row == "node_running" || row == "startup_replay" ||
           row == "datadir_identity";
}
...
class LoopbackRpcShim {
  public:
    using ResponseFilter = std::function<std::string(const std::string&, std::string)>;

    explicit LoopbackRpcShim(VeldNode& node, ResponseFilter response_filter = {})
        ...
        int one = 1;
        ::setsockopt(listener_, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<const char*>(&one),
                     sizeof(one));
        sockaddr_in address{};
  ...
  private:
    static bool SendAll(compat::SocketHandle fd, const std::string& bytes) {
        ...
        while (offset < bytes.size()) {
            const int sent =
                ::send(fd, bytes.data() + offset, static_cast<int>(bytes.size() - offset), 0);
            if (sent <= 0)
                return false;
    ...
    void Run() noexcept {
        while (!stop_.load(std::memory_order_acquire)) {
            ...
            const auto fd = ::accept(listener_, reinterpret_cast<sockaddr*>(&peer), &length);
            if (!compat::IsValidSocket(fd))
                continue;
                ...
                if (body_start == std::string::npos) {
                    ...
                    if (body_start != std::string::npos) {
                        ...
                        if (marker != std::string::npos) {
                            ...
                            content_length =
                                static_cast<size_t>(std::stoull(request.substr(value)));
                        }
                        body_start += 4;
                    }
                }
            ...
            try {
                ...
                const std::string request_body = request.substr(body_start, content_length);
                response_body = node_.TestHandleWorkAdmissionRpc(request_body);
                if (response_filter_)
                    response_body = response_filter_(request_body, std::move(response_body));
...
std::string RpcResultEnvelope(const std::string& result) {
    return "{\"jsonrpc\":\"2.0\",\"result\":" + result + ",\"error\":null,\"id\":1}";
}

MaliciousValidatorProposal BuildMaliciousRegistrationProposal(const ValidatorKey& validator,
                                                              const std::string& operation,
                                                              ValidatorValueAttack attack) {
    const std::vector<uint8_t> from_script = AddressToScript(validator.address);
    const std::vector<uint8_t> vault_script = AddressToScript(STAKE_VAULT_ADDRESS);
    const std::vector<uint8_t> operation_script = BuildOpReturnScript(operation);
    Check(!from_script.empty() && !vault_script.empty() && !operation_script.empty(),
    ...
    auto add_parent = [&](uint64_t value) {
        ...
        proposal.parents.push_back(
            FakeAuthenticatedParent{HashToHex(parent_hash), value, Hex(parent_raw)});
        TxInput input;
    ...
    proposal.transaction.outputs.emplace_back(MIN_VALIDATOR_STAKE, vault_script);
    if (attack == ValidatorValueAttack::RegistrationOutputWrap) {
        ...
        const uint64_t malicious_change =
            std::numeric_limits<uint64_t>::max() - MIN_VALIDATOR_STAKE + 2U;
        add_parent(MIN_TX_FEE + 1U);
        proposal.transaction.outputs.emplace_back(malicious_change, from_script);
        ...
        const uint64_t oversized_parent = std::numeric_limits<uint64_t>::max() - MAX_SUPPLY_UNITS +
                                          1U + MIN_VALIDATOR_STAKE + MIN_TX_FEE;
    ...
    for (uint32_t i = 0; i < proposal.transaction.inputs.size(); ++i) {
        if (i)
            inputs << ',';
        const Hash256 sighash = ComputeSighash(proposal.transaction, i, from_script);
        inputs << "{\"index\":" << i << ",\"sighash_hex\":\""
               << bytes_to_hex(sighash.data(), sighash.size()) << "\",\"prev_script_hex\":\""
               << from_script_hex << "\"}";
    }
    inputs << ']';

    std::ostringstream result;
    result << "{\"unsigned_tx_hex\":\"" << Hex(proposal.transaction.Serialize())
           << "\",\"inputs\":" << inputs.str()
           << ",\"total_input\":" << proposal.claimed_total_input
           << ",\"total_output\":" << proposal.claimed_total_output << ",\"fee\":" << MIN_TX_FEE
           << ",\"change\":" << proposal.claimed_change << '}';
    proposal.response = RpcResultEnvelope(result.str());
    return proposal;
}

std::string AuthenticatedParentRpcResponse(const FakeAuthenticatedParent& parent,
                                           const std::vector<uint8_t>& from_script,
                                           bool transaction) {
    std::ostringstream result;
    if (transaction) {
        result << "{\"raw_hex\":\"" << parent.raw_hex << "\",\"txid\":\"" << parent.txid << "\"}";
    } else {
        result << "{\"txid\":\"" << parent.txid << "\",\"vout\":0,\"value_units\":" << parent.value
               << ",\"script_pubkey_hex\":\"" << Hex(from_script) << "\",\"block_height\":1}";
    }
    return RpcResultEnvelope(result.str());
}

void ExerciseStandaloneValidatorValueBounds(VeldNode& node, const ValidatorKey& validator) {
    const std::string host = "127.0.0.1";
    const std::string operation = ValidatorRegistry::BuildRegisterOp(validator.pubkey_hex);
    const std::vector<uint8_t> from_script = AddressToScript(validator.address);

    for (const ValidatorValueAttack attack : {ValidatorValueAttack::RegistrationOutputWrap,
                                              ValidatorValueAttack::AuthenticatedInputWrap}) {
        const MaliciousValidatorProposal proposal =
            BuildMaliciousRegistrationProposal(validator, operation, attack);
        if (attack == ValidatorValueAttack::RegistrationOutputWrap) {
            Check(proposal.parents.size() == 1 && proposal.parents[0].value == MIN_TX_FEE + 1U &&
                      proposal.transaction.outputs.size() == 3 &&
                      proposal.transaction.outputs[0].value == MIN_VALIDATOR_STAKE &&
            ...
            Check(wrapped_output == 1U, "registration malicious outputs wrap to one unit");
        } else {
            Check(proposal.parents.size() == 2 && proposal.parents[0].value == MAX_SUPPLY_UNITS &&
        ...
        LoopbackRpcShim malicious_rpc(node, [&](const std::string& request, std::string response) {
            if (request.find("\"method\":\"prepareregistervalidator\"") != std::string::npos) {
                prepare_responses.fetch_add(1, std::memory_order_acq_rel);
                return proposal.response;
            }
            const bool gettxout = request.find("\"method\":\"gettxout\"") != std::string::npos;
            const bool gettransaction =
                request.find("\"method\":\"gettransaction\"") != std::string::npos;
            for (const auto& parent : proposal.parents) {
                if (request.find(parent.txid) == std::string::npos)
                    continue;
                if (gettxout) {
                    gettxout_responses.fetch_add(1, std::memory_order_acq_rel);
                    return AuthenticatedParentRpcResponse(parent, from_script, false);
                }
                if (gettransaction) {
                    gettransaction_responses.fetch_add(1, std::memory_order_acq_rel);
                    return AuthenticatedParentRpcResponse(parent, from_script, true);
                }
            }
            if (request.find("\"method\":\"sendrawtransaction\"") != std::string::npos)
                send_requests.fetch_add(1, std::memory_order_acq_rel);
            return response;
        });

        const uint64_t script_sigs_before =
            validator_value_bound_test::build_script_sig_calls.load(std::memory_order_acquire);
        const size_t mempool_before = node.GetMempool().Size();
        const uint64_t gossip_before = node.TestWorkTxGossipCalls();
        Check(!broadcast_op_return(host, malicious_rpc.port(), validator.address, operation,
                                   validator),
        ...
        if (attack == ValidatorValueAttack::RegistrationOutputWrap) {
            Check(gettxout_responses.load(std::memory_order_acquire) == 0 &&
                      gettransaction_responses.load(std::memory_order_acquire) == 0,
                  "oversized output is refused before parent lookups");
        } else {
            Check(gettxout_responses.load(std::memory_order_acquire) == 2 &&
                      gettransaction_responses.load(std::memory_order_acquire) == 2,
                  "oversized input is refused after both parents authenticate");
        }
        Check(validator_value_bound_test::build_script_sig_calls.load(std::memory_order_acquire) ==
                  script_sigs_before,
...
finality::qc::EpochSnapshot MakeSnapshot(const dilithium::PublicKey& voter_public) {
    ...
    snapshot.root = fq::SnapshotRoot(snapshot.entries, snapshot.epoch_id, snapshot.snapshot_height,
                                     snapshot.total_weight);
    Check(fq::SnapshotQualifies(snapshot), "finality snapshot qualifies");
    return snapshot;
}

void AssertFinalityVoteAdmissibleBeforeNodeSink(VeldNode& node,
                                                const finality::qc::EpochSnapshot& snapshot,
                                                const finality::qc::SignedVote& vote) {
    namespace fq = finality::qc;
    const uint64_t tip = node.GetChain().Height();
    Check(fq::FinalityEvidenceVoteWellFormed(vote, snapshot.epoch_id, snapshot.root, tip),
    ...
    Check(retained.has_value(), "finality signer resolves through production retained membership");
    const auto member =
        std::find_if(snapshot.entries.begin(), snapshot.entries.end(),
                     [&](const auto& entry) { return entry.pubkey_hex == vote.pubkey_hex; });
    Check(member != snapshot.entries.end(), "finality signer exists in exact installed snapshot");
    Check(fq::AuthenticateVoteForMember(vote, snapshot.epoch_id, snapshot.root, *member,
                                        fq::NETWORK_ID, CompiledGenesisBytes())
              .has_value(),
          "finality vote authenticates under production network/genesis domain");
    fq::CheckpointRef canonical_target{vote.target.height,
                                       node.GetChain().GetBlock(vote.target.height).GetHash()};
    Check(
        fq::VoteMatchesCanonicalFrame(vote, snapshot, tip, canonical_target, fq::FinalizedRecord{}),
        "finality vote matches production canonical frame");
}

void ExerciseStandaloneEndorsement(VeldNode& node, LoopbackRpcShim& shim, const RealKeyPair& signer,
    ...
    const auto grant =
        fetch_work_admission(host, shim.port(), work_admission::Purpose::ValidatorEndorsement,
                             tip.height, tip.GetHash());
    Check(static_cast<bool>(grant) == expect_open, "standalone fetch_work_admission follows tuple");
    if (!grant)
        return;
    ...
    const std::string signature =
        sign_block(signer.private_key, tip.height, HashToHex(tip.GetHash()));
    ...
    const std::string operation =
        ValidatorRegistry::BuildEndorseOp(tip.height, HashToHex(tip.GetHash()), signature);
    const size_t mempool_before = node.GetMempool().Size();
    const uint64_t gossip_before = node.TestWorkTxGossipCalls();
    std::string prep_error;
    btc_buy::JsonValue prep_root;
    const auto* prep_result =
        strict_rpc_result(json_rpc(host, shim.port(), "preparerawop",
                                   "[\"" + validator.address + "\",\"" + operation + "\"]"),
                          prep_root, prep_error);
    uint64_t prepared_change = 0;
    uint64_t prepared_output_total = 0;
    Check(prep_result && json_u64(*prep_result, "change", prepared_change) &&
              json_u64(*prep_result, "total_output", prepared_output_total) &&
              prepared_change > 0 && prepared_output_total == prepared_change,
          "standalone production proposal has authenticated nonzero change "
          "included in its complete output total");
    Check(broadcast_op_return(host, shim.port(), validator.address, operation, validator, &*grant),
          "standalone endorsement traverses preparerawop/authenticated parents/"
          "sign/sendrawtransaction sink");
    counters.standalone_mempool = static_cast<uint64_t>(node.GetMempool().Size() - mempool_before);
    counters.standalone_gossip = node.TestWorkTxGossipCalls() - gossip_before;
    Check(counters.standalone_mempool == 1 && counters.standalone_gossip == 1,
          "standalone bound endorsement reaches one real mempool/gossip sink");

    // Regression for the transaction preparer contract: `total_output` is the
    ...
    LoopbackRpcShim altered_total_rpc(node, [&](const std::string& request, std::string response) {
        if (request.find("\"method\":\"preparerawop\"") == std::string::npos)
            return response;
        ...
        while (value_end < response.size() && response[value_end] >= '0' &&
    ...
    const auto altered_grant = fetch_work_admission(host, altered_total_rpc.port(),
                                                    work_admission::Purpose::ValidatorEndorsement,
                                                    tip.height, tip.GetHash());
    Check(altered_grant.has_value(), "altered-total regression obtains a fresh one-use grant");
    WorkGrantCancelGuard altered_release(host, altered_total_rpc.port(), *altered_grant);
    const size_t altered_mempool_before = node.GetMempool().Size();
    const uint64_t altered_gossip_before = node.TestWorkTxGossipCalls();
    Check(!broadcast_op_return(host, altered_total_rpc.port(), validator.address, operation,
                               validator, &*altered_grant),
...
void ExerciseFinalityDaemon(VeldNode& node, LoopbackRpcShim& shim,
                            const std::filesystem::path& artifact_dir, bool expect_open,
                            RowCounters& counters) {
    namespace fq = finality::qc;
    ...
    for (const auto& member : snapshot.entries) {
        node.GetValidators().TestInjectValidatorBond(member.pubkey_hex, member.address,
                                                     member.weight);
    }
    Check(node.TestInstallFinalityEvidenceSnapshot(snapshot),
          "finality production verifier retains exact epoch membership");
    const std::string pubkey = BytesToHex(keypair.public_key.data(), keypair.public_key.size());
    fq::DaemonHooks hooks;
    hooks.fetch_snapshot = [snapshot](uint64_t epoch) -> std::optional<fq::EpochSnapshot> {
        return epoch == snapshot.epoch_id ? std::optional<fq::EpochSnapshot>(snapshot)
                                          : std::nullopt;
    };
    hooks.fetch_tip_height = [&] { return node.GetChain().Height(); };
    hooks.fetch_block_hash = [&](uint64_t height) -> std::optional<Hash256> {
        if (height > node.GetChain().Height())
            return std::nullopt;
        return node.GetChain().GetBlock(height).GetHash();
    };
    hooks.authorize_work =
        [&](const fq::CheckpointRef& target) -> std::optional<fq::DaemonWorkGrant> {
        auto grant =
            fetch_work_admission("127.0.0.1", shim.port(), work_admission::Purpose::FinalityVote,
                                 target.height, target.hash);
        if (!grant)
            return std::nullopt;
        return fq::DaemonWorkGrant{grant->binding, grant->token, grant->deadline};
    };
    hooks.cancel_work = [&](const fq::DaemonWorkGrant& grant) {
        (void)cancel_work_signing("127.0.0.1", shim.port(), grant.token);
    };
    hooks.gossip_vote = [&](const fq::SignedVote& vote, const fq::DaemonWorkGrant& grant) {
        AssertFinalityVoteAdmissibleBeforeNodeSink(node, snapshot, vote);
        WorkAdmissionGrant transport{grant.binding, grant.token, grant.deadline};
        ...
        Check(transport.Live(), "finality transport grant remains live at submission");
        Check(validate_work_binding(transport.binding, work_admission::Purpose::FinalityVote,
                                    vote.target.height, vote.target.hash),
              "finality transport binding matches exact signed target");
        const bool submitted = submit_finality_vote("127.0.0.1", shim.port(), vote, transport);
        if (submitted)
            ++counters.finality_gossip;
        return submitted;
    };
    hooks.persist_journal = [&](const fq::DaemonJournal&) {
        ...
        out << "WORK-ADMISSION-FINALITY-JOURNAL\n";
        out.flush();
        return out.good();
    };
    hooks.load_journal = []() -> std::optional<fq::DaemonJournal> { return std::nullopt; };
    hooks.fetch_finalized = []() -> std::optional<fq::FinalizedRecord> { return std::nullopt; };
    hooks.fetch_prevote_qc = [](const fq::EpochSnapshot&) -> std::optional<fq::DecodedQc> {
        return std::nullopt;
    };
    hooks.authorize_target = [](const fq::CheckpointRef&) { return true; };

    fq::FinalityVoter::TestResetSignatureCount();
    fq::FinalityDaemon daemon(pubkey, keypair.secret_key, fq::NETWORK_ID, CompiledGenesisBytes(),
                              std::move(hooks));
    ...
    Check(
        acted == expect_open,
        "FinalityDaemon Tick follows tuple (sink=" + std::to_string(counters.finality_sink_calls) +
            ", active=" + std::to_string(counters.finality_active_calls) +
            ", verify=" + std::to_string(counters.finality_verify_result) + ")");
...
Transaction MakeEndorsementTransaction(VeldNode& node, const RealKeyPair& signer,
                                       uint64_t target_height, const Hash256& target_hash) {
    UTXO funding;
    funding.tx_hash =
        Hash256d(std::string("WORK-ADMISSION-INPROC-FUNDING-") + HashToHex(target_hash));
    ...
    const Hash256 message = ValidatorRegistry::BuildEndorseMessage(target_height, target_hash);
    ...
    tx.inputs[0].script_sig = signer.SignInput(tx, 0, funding.script_pubkey).script_sig;
    return tx;
}

void ExerciseInProcessEndorsement(VeldNode& node, const RealKeyPair& signer,
                                  const std::filesystem::path& artifact_dir, bool expect_open,
                                  RowCounters& counters) {
    const Block tip = node.GetChain().TipCopy();
    ...
    Check(static_cast<bool>(permit) == expect_open, "in-process pre-journal permit follows tuple");
    if (!permit)
        return;
    ...
    guard.load(journal.string());
    const std::string pubkey = BytesToHex(signer.public_key.data(), signer.public_key.size());
    Check(guard.record(std::to_string(tip.height) + ":" + pubkey, HashToHex(tip.GetHash())),
          "in-process actual anti-equivocation record persists");
    ++counters.inproc_journals;
    const Transaction tx = MakeEndorsementTransaction(node, signer, tip.height, tip.GetHash());
    ++counters.inproc_signatures;
    const size_t before = node.GetMempool().Size();
    const auto result = node.SubmitLocalValidatorEndorsement(tx, MIN_TX_FEE, std::move(*permit));
    Check(result.accepted, "in-process endorsement reaches real mempool sink");
    counters.inproc_mempool = static_cast<uint64_t>(node.GetMempool().Size() - before);
...
void AssertNoRetainedWork(VeldNode& node, bool allow_mempool) {
    ...
    Check(snapshot.active_leases == 0 && snapshot.pending_remote_tokens == 0,
          "no coordinator lease or pending bearer retained");
    Check(node.TestActiveRemoteSigningLeaseCount() == 0, "no active remote signing lease retained");
    Check(node.TestPendingBroadcastCount() == 0, "no pending block broadcast retained");
    Check(node.TestCachedTemplateCount() == 0, "no pending block-template authorization retained");
    if (!allow_mempool)
        Check(node.GetMempool().IsEmpty(), "closed row retains no mempool work");
}

bool WaitForCoordinatorClosing(VeldNode& node, std::chrono::milliseconds timeout) {
...
void RethrowThreadFailure(const std::exception_ptr& failure, const char* label) {
    if (!failure)
        return;
    try {
...
void RunSubmitRaceRow(const std::string& row, const std::filesystem::path& artifact_dir) {
#if defined(VELD_PUBLIC_TESTNET)
    (void)row;
    (void)artifact_dir;
    ...
    Check(acquired_first || row == "race_submit_close_first", "recognized submit race row");
    ...
    const uint64_t peer_generation_before =
        node.TestWorkAdmissionProcessServer().GetPeerHeightView().work_generation;
    ...
    const auto decision =
        node.TestEvaluateWorkAdmission(work_admission::Path::SubmitBlock, *subject);
    Check(decision.allowed && decision.binding, "submit race issues real bound work decision");
    WaitForTemplateTimestamp(node.GetChain());
    auto candidate = ParsePublishedBlockTemplate(
        node.TestHandleWorkAdmissionRpc(RpcRequest("getblocktemplate", {miner.address})));
    Check(candidate.has_value(), "submit race obtains a server-authorized real template");
    SolvePublishedBlock(candidate->block);
    const std::string request = RpcRequest(
        "submitblock", {Hex(candidate->block.Serialize()), candidate->binding, candidate->token});
    ...
    if (!acquired_first) {
        (void)node.TestWorkAdmissionProcessServer().TestFinalizePeerConnection("unused",
                                                                               peers.first);
        ...
        if (entered.wait_for(std::chrono::seconds(3)) != std::future_status::ready) {
            ...
            submit_thread.join();
            throw std::runtime_error("submit acquired-first never reached real pre-commit barrier");
        }
        ...
        close_blocked_before_release = !close_done.load(std::memory_order_acquire);
        Check(close_blocked_before_release && !submit_done.load(std::memory_order_acquire),
    ...
    const auto published_peer_view = node.TestWorkAdmissionProcessServer().GetPeerHeightView();
    ...
    if (acquired_first) {
        ...
        Check(barrier_calls.load(std::memory_order_acquire) == 0 && add_calls == 0 &&
                  durable_calls == 0 && broadcasts == 0,
    ...
    const auto snapshot = node.TestWorkAdmissionCoordinatorSnapshot();
    std::cout << "WORK_ADMISSION_ROW_JSON {\"row\":\"" << JsonEscape(row)
              ...
              << ",\"close_blocked_before_release\":" << (close_blocked_before_release ? 1 : 0)
              ...
              << ",\"active_remote_leases\":" << node.TestActiveRemoteSigningLeaseCount()
              << ",\"pending_broadcasts\":" << node.TestPendingBroadcastCount() << "}" << std::endl;
#endif
}

void RunFinalityRaceRow(const std::string& row, const std::filesystem::path& artifact_dir) {
#if defined(VELD_PUBLIC_TESTNET)
    (void)row;
    (void)artifact_dir;
    ...
    Check(acquired_first || row == "race_finality_close_first", "recognized finality race row");
    const RealKeyPair miner = GenerateKeyPair(false);
    ...
    hooks.authorize_work =
        ...
        auto grant =
            fetch_work_admission("127.0.0.1", rpc.port(), work_admission::Purpose::FinalityVote,
                                 target.height, target.hash);
        if (!grant)
            return std::nullopt;
        active_observed.store(node.TestActiveRemoteSigningLeaseCount(), std::memory_order_release);
        if (acquired_first) {
            entered_promise.set_value();
            release.wait();
        }
    ...
    hooks.gossip_vote = [&](const fq::SignedVote& vote, const fq::DaemonWorkGrant& grant) {
        ...
        Check(transport.Live(), "finality race transport grant remains live at submission");
        Check(validate_work_binding(transport.binding, work_admission::Purpose::FinalityVote,
                                    vote.target.height, vote.target.hash),
              "finality race binding matches exact signed target");
        const bool submitted = submit_finality_vote("127.0.0.1", rpc.port(), vote, transport);
        if (submitted)
    ...
    hooks.persist_journal = [&](const fq::DaemonJournal&) {
        ...
        std::ofstream out(artifact_dir / "finality-race.journal",
                          std::ios::binary | std::ios::trunc);
        out << "WORK-ADMISSION-FINALITY-RACE-JOURNAL\n";
        out.flush();
        return out.good();
    };
    hooks.load_journal = []() -> std::optional<fq::DaemonJournal> { return std::nullopt; };
    ...
    if (!acquired_first) {
        ...
        if (entered.wait_for(std::chrono::seconds(3)) != std::future_status::ready) {
            release_promise.set_value();
            tick_thread.join();
            throw std::runtime_error("finality acquired-first never reached active-lease barrier");
        }
        ...
        std::this_thread::sleep_for(std::chrono::milliseconds(25));
        close_blocked_before_release = !close_done.load(std::memory_order_acquire);
        Check(close_blocked_before_release && !tick_done.load(std::memory_order_acquire),
    ...
    if (acquired_first) {
        ...
        Check(authorize_calls.load() == 1 && journal_calls.load() == 1 && signatures == 1 &&
                  gossip_calls.load() == 1 && sink_calls == 1 && active_calls == 1 &&
        ...
        Check(!tick_acted.load(std::memory_order_acquire) && authorize_calls.load() == 1 &&
                  active_observed.load() == 0 && journal_calls.load() == 0 && signatures == 0 &&
                  gossip_calls.load() == 0 && sink_calls == 0 && active_calls == 0 &&
                  verify_result == UINT8_MAX && node_gossip == 0 && assembler == 0,
    ...
    std::cout << "WORK_ADMISSION_ROW_JSON {\"row\":\"" << JsonEscape(row)
              ...
              << journal_calls.load() << ",\"signature_calls\":" << signatures
              << ",\"gossip_calls\":" << gossip_calls.load() << ",\"sink_calls\":" << sink_calls
              << ",\"active_calls\":" << active_calls << ",\"verify_result\":" << verify_result
              << ",\"node_gossip_calls\":" << node_gossip << ",\"assembler_calls\":" << assembler
              << ",\"cached_templates\":0,\"pending_tokens\":" << coordinator.pending_remote_tokens
...
void RunGenericRow(const std::string& row, const std::filesystem::path& artifact_dir) {
#if defined(VELD_PUBLIC_TESTNET)
    (void)row;
    (void)artifact_dir;
    ...
    VeldNode finality_node(MainnetConfig(), (artifact_dir / "finality-node").string());
    InstallTrustedChain(finality_node.GetChainMut(), finality_node.GetMempoolMut(), miner, 21);
    finality_node.TestInstallWorkAdmissionProcessServer();
    auto finality_peers = InstallSafePeers(finality_node, finality_node.GetChain().Height(), 37201);

    node.TestConfigureWorkAdmissionProcess(ReadyState());
    finality_node.TestConfigureWorkAdmissionProcess(ReadyState());
    const auto subject = node.TestCurrentBlockProductionSubject();
    Check(subject.has_value(), "canonical block subject available");
    auto stale_submit = node.TestEvaluateWorkAdmission(work_admission::Path::SubmitBlock, *subject);
    Check(stale_submit.allowed && stale_submit.binding, "open setup issues real submit binding");
    ...
    if (row == "peer_view" || row == "peer_version_loss") {
        (void)node.TestWorkAdmissionProcessServer().TestFinalizePeerConnection("unused",
                                                                               peers.first);
        (void)finality_node.TestWorkAdmissionProcessServer().TestFinalizePeerConnection(
            "unused", finality_peers.first);
    }
    if (row == "peer_version_expiry") {
        const uint64_t expired = 100 + net::NodeServer::TestVersionHeightHintTtlSeconds() + 1;
        node.TestWorkAdmissionProcessServer().TestSetPeerHeightClock(expired);
        finality_node.TestWorkAdmissionProcessServer().TestSetPeerHeightClock(expired);
    }

    std::string binding = EncodeSubmitBindingForBlock(*stale_submit.binding, candidate.block);
    std::string submit_hex = Hex(candidate.block.Serialize());
    std::string submit_token(64, 'a');
    if (expect_open) {
        const auto current =
            node.TestEvaluateWorkAdmission(work_admission::Path::SubmitBlock, *subject);
        Check(current.allowed && current.binding, "open row reissues current submit binding");
        binding = EncodeSubmitBindingForBlock(*current.binding, candidate.block);
    }
    ...
    counters.internal_progress = node.GetMiningProgressCounter() - progress_before;
    ...
    if (expect_open)
        WaitForTemplateTimestamp(node.GetChain());
    const std::string gbt =
        node.TestHandleWorkAdmissionRpc(RpcRequest("getblocktemplate", {miner.address}));
    counters.gbt_templates = gbt.find("\\\"block_hex\\\"") != std::string::npos ||
                             gbt.find("\"block_hex\"") != std::string::npos;
    Check((counters.gbt_templates == 1) == expect_open, "real getblocktemplate follows tuple");
    if (expect_open) {
        auto published = ParsePublishedBlockTemplate(gbt);
        Check(published.has_value(), "open row receives a server-issued template capability");
    ...
    counters.submit_durable_calls = node.TestWorkDurableWriterCalls() - durable_before;
    ...
    Check((counters.block_broadcasts == 1) == expect_open, "submitblock publication follows tuple");

    LoopbackRpcShim validator_rpc(node);
    LoopbackRpcShim finality_rpc(finality_node);
    ExerciseInProcessEndorsement(node, miner, artifact_dir, expect_open, counters);
    ExerciseStandaloneEndorsement(node, validator_rpc, miner, artifact_dir, expect_open, counters);
    ExerciseFinalityDaemon(finality_node, finality_rpc, artifact_dir, expect_open, counters);

    if (closed) {
        Check(counters.inproc_journals == 0 && counters.inproc_signatures == 0 &&
                  counters.inproc_mempool == 0 && counters.inproc_gossip == 0 &&
                  counters.standalone_journals == 0 && counters.standalone_signatures == 0 &&
                  counters.standalone_mempool == 0 && counters.standalone_gossip == 0 &&
                  counters.finality_journals == 0 && counters.finality_signatures == 0 &&
                  counters.finality_gossip == 0 && counters.finality_sink_calls == 0 &&
                  counters.finality_active_calls == 0 &&
                  counters.finality_verify_result == UINT8_MAX &&
                  counters.finality_assembler == 0 && counters.finality_node_gossip == 0,
              "closed tuple emits zero journal/sign/mempool/gossip effects");

        // The work-admission predicate controls only locally produced/signed work.
        ...
        {
            Blockchain::TestResetAddBlockDirectCalls();
            const uint64_t p2p_durable_before = node.TestWorkDurableWriterCalls();
            ...
            const auto queued = server.TestEnqueueBlockIngest(
                candidate.block, candidate.block.SerializedSize(), "198.51.100.7");
            ...
            counters.p2p_durable_calls = node.TestWorkDurableWriterCalls() - p2p_durable_before;
            counters.p2p_tip_advanced = node.GetChain().Height() == p2p_height_before + 1;
            ...
            if (IsGlobalTerminalSafetyRow(row)) {
                ...
                Check(counters.p2p_durable_calls == 0 && counters.p2p_tip_advanced == 0 &&
                          counters.p2p_relay_calls == 0 && counters.p2p_penalty_calls == 0,
                ...
                Check(counters.p2p_add_calls == 1 && counters.p2p_durable_calls == 1 &&
                          counters.p2p_tip_advanced == 1 && counters.p2p_relay_calls == 1 &&
        ...
        Check(
            counters.inproc_journals == 1 && counters.inproc_signatures == 1 &&
                counters.inproc_mempool == 1 && counters.inproc_gossip == 1 &&
                counters.standalone_journals == 1 && counters.standalone_signatures == 1 &&
                counters.standalone_mempool == 1 && counters.standalone_gossip == 1 &&
                counters.finality_journals == 1 && counters.finality_signatures == 1 &&
                counters.finality_gossip == 1 && counters.finality_sink_calls == 1 &&
                counters.finality_active_calls == 1 &&
                counters.finality_verify_result ==
                    static_cast<uint8_t>(net::NodeServer::FinalityVoteVerifyResult::AcceptedNew) &&
                counters.finality_assembler == 1 && counters.finality_node_gossip == 1,
    ...
    std::cout << "WORK_ADMISSION_ROW_JSON {\"row\":\"" << JsonEscape(row)
              << "\",\"closed\":" << (closed ? "true" : "false") << ",\"checks\":" << checks
              ...
              << ",\"p2p_global_runtime_closed\":" << counters.p2p_global_runtime_closed
              << ",\"p2p_relay_calls\":" << counters.p2p_relay_calls
              << ",\"p2p_penalty_calls\":" << counters.p2p_penalty_calls << ",\"p2p_reject_tag\":\""
              << JsonEscape(counters.p2p_reject_tag) << "\""
              << ",\"p2p_disposition\":\""
              << (IsGlobalTerminalSafetyRow(row) ? "terminal_global_safety_refusal"
...
void RunPublicTestnetExpiryRow(const std::filesystem::path& artifact_dir) {
#if !defined(VELD_PUBLIC_TESTNET)
    (void)artifact_dir;
    ...
    Check(now >= 1788112800, "wall clock is beyond immutable public-testnet expiry");
    ...
    Check(counters.internal_mining_admitted == 0 && counters.internal_hashes == 0,
    ...
    prior.subject.target_hash = candidate.block.header.GetTemplateWorkIdentity();
    ...
    Blockchain::TestResetAddBlockDirectCalls();
    (void)node.TestHandleWorkAdmissionRpc(
        RpcRequest("submitblock", {Hex(candidate.block.Serialize()),
                                   work_admission::EncodeBinding(prior), std::string(64, 'a')}));
    Check(Blockchain::TestAddBlockDirectCalls() == 0 && node.TestWorkDurableWriterCalls() == 0,
          "expired public-testnet submit reaches no chain/durable sink");
    AssertNoRetainedWork(node, false);
    std::cout << "WORK_ADMISSION_ROW_JSON {\"row\":\"public_testnet_expiry\","
                 "\"closed\":true,\"checks\":"
              << checks
...
int main(int argc, char** argv) {
    try {
        ...
        else if (row == "race_submit_close_first" || row == "race_submit_acquired_first")
            RunSubmitRaceRow(row, artifact_dir);
        else if (row == "race_finality_close_first" || row == "race_finality_acquired_first")
