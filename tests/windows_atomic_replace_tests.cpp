...
int main() {
    ...
    try {
        ...
        std::string error;
        check(EnsurePrivateDirectory(root.string(), &error), "private directory");
        const auto file = root / L"status.json";
        check(AtomicWriteText(file.string(), "old", &error, true), "initial write");
        auto locked = [&]() {
            return CreateFileW(file.c_str(), GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
                               FILE_ATTRIBUTE_NORMAL, nullptr);
        };
        HANDLE handle = locked();
        check(handle != INVALID_HANDLE_VALUE, "reader handle");
        std::thread reader([handle] {
            Sleep(250);
            CloseHandle(handle);
        });
        const bool replaced = AtomicWriteText(file.string(), "new", &error, true);
        reader.join();
        check(replaced, "transient sharing lock must recover");
        std::vector<uint8_t> bytes;
        check(Read(file.string(), bytes, &error, 32, true) == ReadResult::Ok &&
                  std::string(bytes.begin(), bytes.end()) == "new",
              "exact bytes after retry");
        handle = locked();
        check(handle != INVALID_HANDLE_VALUE, "persistent reader handle");
        ...
        check(Read(file.string(), bytes, &error, 32, true) == ReadResult::Ok &&
                  std::string(bytes.begin(), bytes.end()) == "new",
              "failed replacement preserves previous bytes");
        size_t files = 0;
        for (const auto& entry : std::filesystem::directory_iterator(root)) {
            ++files;
            check(entry.path() == file, "temporary file leaked");
        }
