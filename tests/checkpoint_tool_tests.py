...
with tempfile.TemporaryDirectory() as folder:
    ...
    def verify(value, expected, count=1):
        ...
        result = subprocess.run(
            [str(tool), 'verify', str(path), str(count)], capture_output=True, timeout=15
        )
        assert (result.returncode == 0) == expected, (
            'Native checkpoint verification result mismatch'
        )
        checks += 1
    ...
    for field, value in [
        ('height', 2801),
        ('hash', 'a' * 64),
        ('signed_at', fixture[0]['signed_at'] + 1),
        ('sig', '0' * 6618),
    ]:
    ...
    result = subprocess.run(
        [str(tool), 'identity', str(path)],
        input=b'test-password-must-not-appear\n',
        capture_output=True,
        timeout=15,
    )
    assert (
        result.returncode != 0
        and b'test-password-must-not-appear' not in result.stdout + result.stderr
    )
    checks += 1
