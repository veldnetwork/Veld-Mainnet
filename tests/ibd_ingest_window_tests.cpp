...
int main() {
    ...
    auto enqueue = [&](const std::shared_ptr<net::Connection>& peer, size_t bytes = 256) {
        ...
        return server.TestEnqueueBlockIngestFromConnection(std::move(block), bytes,
                                                           peer->RemoteAddr() + ":32001", peer);
    };
    ...
    auto outbound = MakeInertPeer("192.0.2.50");
    outbound->MarkHandshakeReady();
    two_only(outbound);
    auto inbound = MakeInertPeer("192.0.2.51", true);
    inbound->MarkHandshakeReady();
    inbound->NoteIbdGetBlocksRequest();
    two_only(inbound);
    auto incomplete = MakeInertPeer("192.0.2.52");
    incomplete->NoteIbdGetBlocksRequest();
    two_only(incomplete);
    outbound->NoteIbdGetBlocksRequest();
    outbound->TestExpireIbdDownloadWindow();
    two_only(outbound);

    server.TestClearPendingBlockIngest();
    outbound->NoteIbdGetBlocksRequest();
    for (unsigned i = 0; i < 32; ++i)
        Check(enqueue(outbound) == Result::Queued, "one requested IBD batch is retained");
    Check(enqueue(outbound) == Result::Full,
          "second unsolicited batch cannot expand the source queue");
    auto alias = MakeInertPeer("192.0.2.50");
    alias->MarkHandshakeReady();
    alias->NoteIbdGetBlocksRequest();
    ...
    Check(enqueue(outbound, MAX_BLOCK_SIZE) == Result::Full,
          "IBD does not expand the existing source byte ceiling");
    Check(server.TestPendingBlockIngestBytes() == 2 * MAX_BLOCK_SIZE,
          "byte accounting remains exact");

    server.SetIBDComplete(true);
    two_only(outbound);
    server.SetIBDComplete(false);
    server.TestClearPendingBlockIngest();
    ...
    for (unsigned ip = 60; ip < 68; ++ip) {
        ...
        peer->NoteIbdGetBlocksRequest();
        peers.push_back(peer);
        for (unsigned i = 0; i < 32; ++i)
            if (enqueue(peer) == Result::Queued)
                ++queued;
    }
    ...
    std::cout << "IBD_INGEST_WINDOW: PASS (" << checks
              << " checks); inert queues, no network or PoW claim\n";
}
