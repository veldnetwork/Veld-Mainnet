...
constexpr const char* RECIPIENT =
    "VUjD1JoewGkiGxRqJ52FkK1UiMotjsp9Tg";
constexpr const char* RECIPIENT_2 =
...
std::vector<uint8_t> MakeBitcoinTx(
...
std::vector<uint8_t> MakeWitnessBitcoinTx(
        const std::vector<std::pair<Hash256, uint32_t>>& inputs,
        const std::vector<btcspv::BtcTxOut>& outputs) {
    ...
    btcspv::WitnessAwareBtcTx parsed;
    if (!btcspv::ParseWitnessAwareBtcTx(tx, parsed) ||
        !parsed.has_witness || parsed.txid != Hash256d(stripped) ||
        parsed.txid == Hash256d(tx))
        throw std::runtime_error("witness fixture txid mismatch");
    return tx;
}

std::vector<uint8_t> MakeFundingParent(
        uint64_t value, const std::vector<uint8_t>& script,
        const std::string& recipient = {}) {
    static uint64_t sequence = 1;
    const Hash256 fake = TaggedHash("external-funding-" +
                                    std::to_string(sequence++));
    std::vector<btcspv::BtcTxOut> outputs{{value, script}};
    if (!recipient.empty())
        outputs.push_back({0, BitcoinOpReturn(
            std::string("btcVELD:") + recipient)});
...
struct BitcoinHistory {
    ...
    BitcoinHistory()
        : chain(BtcVeldCheckpoint(), BtcVeldPowLimit(), 2016, 1209600,
                BtcVeldNoRetarget()) {
    ...
    Hash256 MineOn(const Hash256& parent, const Hash256& merkle) {
        ...
        for (uint32_t nonce = 0;; ++nonce) {
            ...
            if (btcspv::CheckProofOfWork(header.data(), 0x207fffffu,
                                         BtcVeldPowLimit())) {
                mined = true;
                break;
            }
            if (nonce == UINT32_MAX)
                throw std::runtime_error("daybreak Bitcoin header nonce exhausted");
        }
        CHECK(mined);
    ...
    Hash256 MineDummyOn(const Hash256& parent) {
        return MineOn(parent, TaggedHash(
            "dummy-bitcoin-block-" + std::to_string(++dummy_counter)));
    }

    void Bury(const Hash256& block, uint32_t depth = BTCVELD_SPV_K_BTC) {
        CHECK(chain.Has(block));
        Hash256 parent = chain.BestTip();
        for (uint32_t i = 0; i < depth; ++i) parent = MineDummyOn(parent);
    ...
    void ReorgFrom(const Hash256& fork_parent) {
        ...
        for (uint32_t i = 0; i < required; ++i) cursor = MineDummyOn(cursor);
...
reserve::Claim PriorClaim(const reserve::State& state,
...
void FinalizeFixture(BitcoinHistory& bitcoin, TransitionFixture& fixture,
    ...
    if (burial != 0) bitcoin.Bury(fixture.claim.bitcoin_block, burial);
    fixture.proof = reserve::EncodeProof(fixture.claim);
    CHECK(!fixture.proof.empty());
}

TransitionFixture MakeOpen(BitcoinHistory& bitcoin,
                           const reserve::State& state,
                           uint64_t reserve_value, uint64_t mint_amount,
                           const std::string& recipient,
                           const btcnull::Proof& witness,
                           uint32_t burial = BTCVELD_SPV_K_BTC,
    ...
    fixture.claim.exact_commitment = reserve::detail::OpenDepositCommitment(
        0, reserve_value, recipient);
    ...
    const std::vector<std::pair<Hash256, uint32_t>> inputs{
        {BitcoinTxid(source), 0}};
    const std::vector<btcspv::BtcTxOut> outputs{
        {reserve_value, BtcVeldCustodySpk()},
        {0, reserve::detail::OpenAuthScript(
            state.transition_commitment, recipient, mint_amount)}};
    fixture.bitcoin_tx = witness_serialization
        ? MakeWitnessBitcoinTx(inputs, outputs)
...
std::vector<uint8_t> MakePendingDeposit(uint64_t value,
                                        const std::string& recipient,
                                        bool witness_serialization = false) {
    ...
    const std::vector<std::pair<Hash256, uint32_t>> inputs{
        {BitcoinTxid(source), 0}};
    const std::vector<btcspv::BtcTxOut> outputs{
        {value, BtcVeldCustodySpk()},
        {0, BitcoinOpReturn(std::string("btcVELD:") + recipient)}};
    return witness_serialization
        ? MakeWitnessBitcoinTx(inputs, outputs)
        : MakeBitcoinTx(inputs, outputs);
}

TransitionFixture MakeDeposit(BitcoinHistory& bitcoin,
                              const reserve::State& state,
                              const std::vector<uint8_t>& current_reserve_tx,
                              const std::vector<uint8_t>& pending_tx,
                              uint64_t successor_value, uint64_t mint_amount,
                              const btcnull::Proof& witness,
                              bool witness_serialization = false) {
    TransitionFixture fixture;
    ...
    const std::vector<std::pair<Hash256, uint32_t>> inputs{
        {state.reserve_txid, state.reserve_vout}, {pending.txid, 0}};
    const std::vector<btcspv::BtcTxOut> outputs{
        {successor_value, BtcVeldCustodySpk()},
        {0, reserve::detail::AuthScript(
            reserve::Operation::DEPOSIT, state.transition_commitment,
...
TransitionFixture MakeRollover(BitcoinHistory& bitcoin,
                               const reserve::State& state,
                               const std::vector<uint8_t>& current_reserve_tx,
                               bool witness_serialization = false) {
    TransitionFixture fixture;
    ...
    const std::vector<btcspv::BtcTxOut> outputs{
        ...
        {0, reserve::detail::AuthScript(
            reserve::Operation::ROLLOVER, state.transition_commitment,
            Hash256{}, 0)}};
    fixture.bitcoin_tx = witness_serialization
        ? MakeWitnessBitcoinTx(inputs, outputs)
        : MakeBitcoinTx(inputs, outputs);
...
TransitionFixture MakePayout(BitcoinHistory& bitcoin,
                             const reserve::State& state,
                             const std::vector<uint8_t>& current_reserve_tx,
                             uint64_t principal,
    ...
    const uint64_t successor_output =
        !include_external_fee_input && successor >= 100
            ? successor - 100 : successor;
    if (successor != 0)
        outputs.push_back({successor_output, BtcVeldCustodySpk()});
    outputs.push_back({principal, destination});
    outputs.push_back({0, reserve::detail::AuthScript(
        reserve::Operation::PAYOUT, state.transition_commitment,
        request_commitment, 0)});
    fixture.bitcoin_tx = witness_serialization
        ? MakeWitnessBitcoinTx(inputs, outputs)
        : MakeBitcoinTx(inputs, outputs);
    fixture.claim.new_reserve_txid = successor == 0
        ? Hash256{} : BitcoinTxid(fixture.bitcoin_tx);
    FinalizeFixture(bitcoin, fixture);
    return fixture;
}
...
std::vector<uint8_t> Fsp2Proof(const TransitionFixture& fixture) {
    ...
    proof.insert(proof.end(), fixture.bitcoin_tx.begin(),
                 fixture.bitcoin_tx.end());
...
TransitionFixture MakeClose(BitcoinHistory& bitcoin,
                            const reserve::State& state,
                            const std::vector<uint8_t>& current_reserve_tx) {
    TransitionFixture fixture;
    ...
    fixture.bitcoin_tx = MakeBitcoinTx(
        ...
        {{state.reserve_value_sats, {0x51}},
         {0, reserve::detail::AuthScript(
             reserve::Operation::CLOSE, state.transition_commitment,
             Hash256{}, 0)}});
    FinalizeFixture(bitcoin, fixture);
    return fixture;
}
...
struct TestLedger {
    ...
    btcnull::Proof WitnessFor(const std::string& outpoint) const {
        ...
        for (const auto& event : nullifier_events) {
            CHECK(btcnull::UpdateWitnessAfterInsert(
                event.old_root, event.new_root, outpoint, occupied, proof,
                event.outpoint, event.proof));
        }
        CHECK(!occupied);
    ...
    Hash256 CompleteDigest() const {
        ...
        return state_digest::sha256_domain(
            "VELD/DAYBREAK/PURPOSE_BUILT_COMPLETE_STATE/v1", body);
    }
    ...
    bool Apply(BitcoinHistory& bitcoin, const TransitionFixture& fixture) {
        ...
        const auto verified = reserve::Verify(
            bitcoin.chain, state, supply, fixture.proof.data(),
            fixture.proof.size(), BtcVeldCustodySpk(), BTCVELD_SPV_K_BTC,
        ...
        if (supply > UINT64_MAX - verified.claim.mint_amount) return false;
        ...
        const bool consumes_deposit =
            verified.claim.operation == reserve::Operation::OPEN ||
            verified.claim.operation == reserve::Operation::DEPOSIT;
        if (consumes_deposit) {
            insertion = btcnull::Insert(nullifier_root,
        ...
        state = next_state;
        if (consumes_deposit) {
            nullifier_events.push_back(NullifierEvent{
                insertion.old_root, insertion.new_root,
                verified.pending_outpoint, verified.claim.nullifier_proof});
        ...
        CHECK(reserve::SetProcessed(state, veld_height,
                                    TaggedHash("veld-block-" +
                                               std::to_string(veld_height))));
        CHECK(state.AccountingHolds(supply));
        return true;
    }

    bool OpenRedemption(uint64_t amount) {
        if (amount > supply) return false;
        reserve::State next = state;
        if (!reserve::OpenRedemption(next, amount, supply - amount)) return false;
        state = next;
        supply -= amount;
    ...
    bool Compensate(uint64_t amount) {
        reserve::State next = state;
        if (!reserve::ResolveDefaultOrCompensation(
                next, amount, supply + amount)) return false;
        state = next;
        supply += amount;
        return state.AccountingHolds(supply);
    }
...
reserve::SpendClassification Classify(
        const TestLedger& ledger, const std::vector<uint8_t>& tx,
        const std::vector<std::vector<uint8_t>>& parents,
        const reserve::PayoutContext& payout = {}) {
    reserve::PayoutLookup lookup;
    if (payout.present) {
        lookup = [&payout](const Hash256& commitment,
                           reserve::PayoutContext& found) {
            if (commitment != payout.request_commitment) return false;
            found = payout;
            return true;
        };
    }
...
Transaction ReserveCarrier(const TransitionFixture& fixture) {
    Transaction tx;
    tx.inputs.push_back(TxInput::Coinbase("daybreak-reserve-carrier"));
    const std::string payload =
        std::string(reserve::PUBLIC_CARRIER_PREFIX) +
        BytesToHex(fixture.proof);
...
Transaction IssuerReserveCarrier(const TransitionFixture& fixture,
    ...
    Transaction tx;
    tx.inputs.push_back(TxInput::Coinbase("daybreak-reserve-issuer"));
    TokenOpData op;
    op.action = "MINT";
    ...
    op.memo = std::string(reserve::ISSUER_MEMO_PREFIX) +
              BytesToHex(fixture.proof);
...
Transaction RawCarrier(const std::string& payload) {
    Transaction tx;
    tx.inputs.push_back(TxInput::Coinbase("daybreak-raw-carrier"));
    tx.outputs.emplace_back(0, BuildOpReturnScript(payload));
    return tx;
}

Transaction SignedTokenCarrier(const TokenOpData& op,
                               const RealKeyPair& signer,
                               const std::string& nonce) {
    Transaction tx;
    TxInput input;
    input.prev_tx_hash = TaggedHash("daybreak-token-input-" + nonce);
    input.prev_out_index = 0;
    tx.inputs.push_back(input);
    tx.outputs.emplace_back(0, BuildOpReturnScript(EncodeTokenOp(op)));
    tx.inputs[0].script_sig =
        signer.SignInput(tx, 0, signer.GetP2PKHScript()).script_sig;
    tx.InvalidateTxIDCache();
    CHECK(TxVerifiedSignedBy(tx, signer.address));
    return tx;
}

btcveld::RedeemRequest RequestFromRedeem(
    ...
    request.request_commitment =
        btcveld::SignerBondCovenant::RequestCommitment(request);
    return request;
}

Block TransactionsBlock(uint64_t height,
                        const std::vector<Transaction>& transactions,
                        const std::string& branch = "canonical") {
    Block block;
    block.height = height;
    block.header.version = 1;
    block.header.prev_block_hash = TaggedHash(
        "production-ledger-parent-" + branch + "-" +
...
Block ReserveBlock(uint64_t height,
                   const std::vector<TransitionFixture>& fixtures) {
...
Hash256 CompleteNodeEnvelope(
        const OnChainTokenLedger& tokens,
        const btcspv::BtcHeaderChain& bitcoin,
        const btcveld::SignerBondCovenant& covenant,
        uint64_t height, const Hash256& tip) {
    const Hash256 zero = TaggedHash("daybreak-empty-component");
    return state_digest::ComposeV8(
        height, tip,
        zero, zero, zero, zero, zero,
        tokens.Digest(), zero, zero, zero,
        bitcoin.StateDigest(), zero, zero,
...
int main() {
    try {
        ...
        CHECK(CompleteNodeEnvelope(
                  fresh_tokens_a, fresh_bitcoin_a.chain,
                  fresh_covenant_a, 0, Hash256{}) ==
              CompleteNodeEnvelope(
                  fresh_tokens_b, fresh_bitcoin_b.chain,
                  fresh_covenant_b, 0, Hash256{}));
        ...
        auto open = MakeOpen(bitcoin, ledger.state, 100000, 100000,
                             RECIPIENT, btcnull::EmptyProof());
        reserve::Claim decoded_open;
        CHECK(reserve::DecodeProof(open.proof.data(), open.proof.size(),
                                   decoded_open));
        CHECK(reserve::EncodeProof(decoded_open) == open.proof);
        std::vector<btcspv::BtcTxOut> open_outputs;
        CHECK(btcspv::ParseBtcTxOutputs(
            open.bitcoin_tx.data(), open.bitcoin_tx.size(), open_outputs));
        CHECK(open_outputs.size() == 2);
        CHECK(open_outputs[1].spk.size() == 82);
        CHECK(open_outputs[1].spk.size() <=
              reserve::MAX_STANDARD_NULL_DATA_SCRIPT_BYTES);
        reserve::detail::AuthMarker open_marker;
        CHECK(reserve::detail::ParseAuthMarker(
            open_outputs[1], open_marker));
        CHECK(open_marker.operation == reserve::Operation::OPEN);
        CHECK(open_marker.open_recipient == RECIPIENT);
        CHECK(reserve::detail::AuthScript(
                  reserve::Operation::OPEN,
                  ledger.state.transition_commitment,
                  open.claim.exact_commitment, 100000).empty());
        auto legacy_open_auth = reserve::detail::AuthScript(
            reserve::Operation::ROLLOVER,
            ledger.state.transition_commitment,
            open.claim.exact_commitment, 100000);
        CHECK(legacy_open_auth.size() == 80);
        legacy_open_auth[7] =
            static_cast<uint8_t>(reserve::Operation::OPEN);
        CHECK(!reserve::detail::ParseAuthMarker(
            btcspv::BtcTxOut{0, legacy_open_auth}, open_marker));
        auto trailing_open_auth = open_outputs[1];
        trailing_open_auth.spk.push_back(0);
        CHECK(!reserve::detail::ParseAuthMarker(
            trailing_open_auth, open_marker));
        const auto nonstandard_open = MakeBitcoinTx(
            {{Hash256d(open.parents[0]), 0}},
        ...
        const auto public_result = reserve::Verify(
            bitcoin.chain, ledger.state, ledger.supply,
            open.proof.data(), open.proof.size(), BtcVeldCustodySpk(),
            BTCVELD_SPV_K_BTC,
            [](const std::string& a) { return a == RECIPIENT; });
        const auto issuer_result = reserve::Verify(
            bitcoin.chain, ledger.state, ledger.supply,
            open.proof.data(), open.proof.size(), BtcVeldCustodySpk(),
            BTCVELD_SPV_K_BTC,
            [](const std::string& a) { return a == RECIPIENT; });
        ...
        CHECK(public_result.claim.bitcoin_txid ==
              issuer_result.claim.bitcoin_txid);
        ...
        auto rollover = MakeRollover(bitcoin, ledger.state,
                                     ledger.current_reserve_tx);
        CHECK(Classify(ledger, rollover.bitcoin_tx, rollover.parents)
                  .disposition ==
              reserve::SpendDisposition::AUTHORIZED_TRANSITION);
        ...
        const auto split = MakeBitcoinTx(
            {{current_txid, current_vout}},
            {{50000, BtcVeldCustodySpk()}, {50000, BtcVeldCustodySpk()},
             {0, reserve::detail::AuthScript(
                 reserve::Operation::ROLLOVER,
                 represented_base.state.transition_commitment,
                 Hash256{}, 0)}});
        ...
        const auto merge = MakeBitcoinTx(
            {{current_txid, current_vout}, {Hash256d(external), 0}},
            {{100500, BtcVeldCustodySpk()},
             {0, reserve::detail::AuthScript(
                 reserve::Operation::ROLLOVER,
                 represented_base.state.transition_commitment,
                 Hash256{}, 0)}});
        CHECK(Classify(represented_base, merge,
                       {current_parent, external}).disposition ==
              reserve::SpendDisposition::UNAUTHORIZED_SPEND);

        std::vector<uint8_t> laundering_parent = MakeBitcoinTx(
            {{current_txid, current_vout}}, {{99500, {0x51}}});
        auto laundering_class = Classify(
            represented_base, laundering_parent, {current_parent});
        CHECK(laundering_class.disposition ==
              reserve::SpendDisposition::UNAUTHORIZED_SPEND);
        TestLedger frozen = represented_base;
        CHECK(reserve::ApplyFreeze(
            frozen.state, Hash256d(laundering_parent),
            TaggedHash("laundering-spend-block")));
        const uint64_t frozen_supply = frozen.supply;
        for (int hop = 0; hop < 6; ++hop) {
            const auto next = MakeBitcoinTx(
                {{Hash256d(laundering_parent), 0}},
                {{static_cast<uint64_t>(99000 - hop * 100), {0x51}}});
            laundering_parent = next;
        }
        const auto return_to_custody = MakeBitcoinTx(
            {{Hash256d(laundering_parent), 0}},
            {{98000, BtcVeldCustodySpk()},
             {0, BitcoinOpReturn(std::string("btcVELD:") + RECIPIENT_2)}});
        CHECK(Classify(frozen, return_to_custody,
                       {laundering_parent}).disposition ==
              reserve::SpendDisposition::NOT_CURRENT_RESERVE);
        std::vector<uint8_t> cycle_parent = return_to_custody;
        for (int cycle = 0; cycle < 3; ++cycle) {
            const auto ordinary_again = MakeBitcoinTx(
                {{Hash256d(cycle_parent), 0}},
                {{static_cast<uint64_t>(97500 - cycle * 500), {0x51}}});
            const auto custody_again = MakeBitcoinTx(
                {{Hash256d(ordinary_again), 0}},
                {{static_cast<uint64_t>(97000 - cycle * 500),
                  BtcVeldCustodySpk()},
                 {0, BitcoinOpReturn(
                         std::string("btcVELD:") + RECIPIENT_2)}});
            CHECK(Classify(frozen, custody_again,
                           {ordinary_again}).disposition ==
                  reserve::SpendDisposition::NOT_CURRENT_RESERVE);
        ...
        const auto mixed_return = MakeBitcoinTx(
            {{Hash256d(laundering_parent), 0}, {Hash256d(mixed_new), 0}},
            {{102000, BtcVeldCustodySpk()},
             {0, BitcoinOpReturn(std::string("btcVELD:") + RECIPIENT)}});
        CHECK(Classify(frozen, mixed_return,
                       {laundering_parent, mixed_new}).disposition ==
              reserve::SpendDisposition::NOT_CURRENT_RESERVE);
        CHECK(frozen.supply == frozen_supply);
        ...
        CHECK(Classify(represented_base, merge,
                       {current_parent, external}).mint_amount == 0);
        ...
        const std::string pending_outpoint = btcspv::BtcDepositOutpointId(
            Hash256d(pending), 0);
        auto pending_witness = ledger.WitnessFor(pending_outpoint);
        auto deposit = MakeDeposit(bitcoin, ledger.state,
                                   ledger.current_reserve_tx, pending,
        ...
        const std::string surplus_outpoint = btcspv::BtcDepositOutpointId(
            Hash256d(pending_surplus), 0);
        auto surplus_witness = ledger.WitnessFor(surplus_outpoint);
        auto deposit_surplus = MakeDeposit(
            bitcoin, ledger.state, ledger.current_reserve_tx,
        ...
        CHECK(replay.state.transition_commitment ==
              ledger.state.transition_commitment);
        ...
        for (const auto& fixture : canonical_history) {
            ...
            if (!production.ProcessBlock(block, peg_gate)) {
                const auto diagnostic = reserve::Verify(
                    ...
                    fixture.proof.data(), fixture.proof.size(),
                    BtcVeldCustodySpk(), BTCVELD_SPV_K_BTC,
                    [](const std::string& address) {
                        return address == RECIPIENT || address == RECIPIENT_2;
                    }, fixture.payout);
                const auto accumulator =
                    production.GetBtcVeldMintAccumulator();
                const auto insertion = diagnostic.ok
                    ? btcnull::Insert(accumulator.root,
                                      diagnostic.pending_outpoint,
                                      diagnostic.claim.nullifier_proof)
                    : btcnull::InsertResult{};
                throw std::runtime_error(
                    "production ledger rejected canonical fixture " +
                    std::to_string(production_index) + ": " +
                    diagnostic.reason + " verify=" +
                    (diagnostic.ok ? "ok" : "fail") + " nullifier=" +
                    (insertion.ok ? "ok" : "fail"));
            }
            ++production_index;
        }
        CHECK(production.GetSupply(BTCVELD_TOKEN_ID) ==
              static_cast<int64_t>(ledger.supply));
        CHECK(production.GetBtcVeldReserveState().reserve_value_sats ==
              ledger.state.reserve_value_sats);
        CHECK(production.GetBtcVeldReserveState().surplus_sats ==
              ledger.state.surplus_sats);
        ...
        production_height = 1;
        for (const auto& fixture : canonical_history)
            CHECK(production_replay.ProcessBlock(
                ReserveBlock(production_height++, {fixture}), peg_gate));
        ...
        const auto production_replay_state =
            production_replay.GetBtcVeldReserveState();
        CHECK(CompleteNodeEnvelope(
                  production, bitcoin.chain, production_covenant_a,
                  production_state.processed_veld_height,
                  production_state.processed_veld_block_hash) ==
              CompleteNodeEnvelope(
                  production_replay, bitcoin.chain,
                  production_covenant_b,
        ...
        CHECK(public_interface.ValidateMempoolCandidate(
            public_open, 1, 0x207fffffu, peg_gate));
        ...
        const auto redirected = issuer_interface.FilterMempoolCandidates(
            {IssuerReserveCarrier(open, RECIPIENT_2)}, 1,
            0x207fffffu, peg_gate, {true});
        ...
        const auto amount_mismatch = issuer_interface.FilterMempoolCandidates(
            {IssuerReserveCarrier(wrong_amount)}, 1,
            0x207fffffu, peg_gate, {true});
        CHECK(amount_mismatch.size() == 1 && !amount_mismatch[0]);

        auto preview_next = MakeRollover(
            bitcoin, ledger.state, ledger.current_reserve_tx);
        const Transaction preview_tx = ReserveCarrier(preview_next);
        CHECK(production.ValidateMempoolCandidate(
            preview_tx, production_height, 0x207fffffu, peg_gate));
        OnChainTokenLedger post_block_preview;
        CHECK(production.BuildPostBlockPreview(
            ReserveBlock(production_height, {preview_next}), peg_gate,
            post_block_preview));
        CHECK(post_block_preview.GetBtcVeldReserveState()
                  .transition_commitment ==
              reserve::detail::TransitionCommitment(preview_next.claim));
        CHECK(production.Digest() == production_digest);

        const Block two_carrier_block = ReserveBlock(
            production_height, {preview_next, preview_next});
        CHECK(!production.ProcessBlock(two_carrier_block, peg_gate));
        CHECK(production.Digest() == production_digest);

        production.Reset();
        CHECK(production.GetSupply(BTCVELD_TOKEN_ID) == 0);
        CHECK(production.GetBtcVeldReserveState().status ==
              reserve::Status::EMPTY);
        production.SetBtcHeaderChain(&bitcoin.chain);
        production_height = 1;
        for (const auto& fixture : canonical_history)
            CHECK(production.ProcessBlock(
                ReserveBlock(production_height++, {fixture}), peg_gate));
        CHECK(production.Digest() == production_digest);
        ...
        auto competing_a = MakeRollover(
            bitcoin, competing_parent.state,
            competing_parent.current_reserve_tx);
        auto competing_b = MakeRollover(
            bitcoin, competing_parent.state,
            competing_parent.current_reserve_tx);
        ...
        const bool batch_ok = two_in_one.Apply(bitcoin, competing_a) &&
                              two_in_one.Apply(bitcoin, competing_b);
        CHECK(!batch_ok);
        ...
        const auto valid_classification = Classify(
            fsp_first, competing_a.bitcoin_tx, competing_a.parents);
        CHECK(valid_classification.disposition ==
              reserve::SpendDisposition::AUTHORIZED_TRANSITION);
        ...
        CHECK(Classify(fsp_first, competing_a.bitcoin_tx,
                       competing_a.parents).disposition ==
              reserve::SpendDisposition::NOT_CURRENT_RESERVE);
        ...
        const Hash256 overwide_commitment =
            TaggedHash("overwide-fsp2-payout");
        ...
        std::vector<std::vector<uint8_t>> overwide_parents{
            overwide.current_reserve_tx};
        ...
        const std::vector<uint8_t> overwide_tx = MakeBitcoinTx(
            overwide_inputs,
            {{overwide.state.reserve_value_sats - 1000,
              BtcVeldCustodySpk()},
             {1000, overwide_destination},
             {0, reserve::detail::AuthScript(
                     reserve::Operation::PAYOUT,
                     overwide.state.transition_commitment,
                     overwide_commitment, 0)}});
        CHECK(overwide_parents.size() == reserve::MAX_DIRECT_INPUTS + 1);
        CHECK(Classify(overwide, overwide_tx, overwide_parents,
                       overwide_context).disposition ==
              reserve::SpendDisposition::UNAUTHORIZED_SPEND);

        TestLedger fraud_first = competing_parent;
        const auto theft = MakeBitcoinTx(
            {{fraud_first.state.reserve_txid,
              fraud_first.state.reserve_vout}},
            {{fraud_first.state.reserve_value_sats - 100, {0x51}}});
        CHECK(Classify(fraud_first, theft,
                       {fraud_first.current_reserve_tx}).disposition ==
              reserve::SpendDisposition::UNAUTHORIZED_SPEND);
        ...
        const auto predecessor_result = reserve::Verify(
            bitcoin.chain, predecessor_parent.state,
            predecessor_parent.supply, competing_a.proof.data(),
            competing_a.proof.size(), BtcVeldCustodySpk(),
            BTCVELD_SPV_K_BTC,
            [](const std::string&) { return true; });
        CHECK(predecessor_result.ok);
        reserve::State post_predecessor = predecessor_parent.state;
        CHECK(reserve::ApplyAuthorized(
            post_predecessor, predecessor_result,
            predecessor_parent.supply, predecessor_parent.supply));
        auto successor = MakeRollover(
            bitcoin, post_predecessor, competing_a.bitcoin_tx);
        ...
        CHECK(veld_reorg_parent.CompleteDigest() ==
              independent_alt.CompleteDigest());
        ...
        auto reorg_open = MakeOpen(
            reorg_bitcoin, reorg_ledger.state, 50000, 50000,
            RECIPIENT, btcnull::EmptyProof());
        CHECK(reorg_ledger.Apply(reorg_bitcoin, reorg_open));
        CHECK(reorg_bitcoin.chain.IsFinalForExternalValue(
        ...
        CHECK(!reorg_bitcoin.chain.IsFinalForExternalValue(
            ...
            BTCVELD_SPV_K_BTC));
        CHECK(reserve::ApplyFreeze(
            reorg_ledger.state, reorg_ledger.state.reserve_txid,
            reorg_ledger.state.reserve_bitcoin_block));
        ...
        const Hash256 production_reorg_fork =
            production_reorg_bitcoin.chain.BestTip();
        TestLedger production_reorg_model;
        auto production_reorg_open = MakeOpen(
            production_reorg_bitcoin, production_reorg_model.state,
            45000, 45000, RECIPIENT, btcnull::EmptyProof());
        OnChainTokenLedger production_reorg;
        production_reorg.SetBtcHeaderChain(&production_reorg_bitcoin.chain);
        CHECK(production_reorg.ProcessBlock(
            ReserveBlock(1, {production_reorg_open}), peg_gate));
        production_reorg_bitcoin.ReorgFrom(production_reorg_fork);
        CHECK(production_reorg.ProcessBlock(
            TransactionsBlock(2, {}, "bitcoin-reorg"), peg_gate));
        CHECK(production_reorg.GetBtcVeldReserveState().status ==
              reserve::Status::FROZEN);
        CHECK(production_reorg.GetBtcVeldReserveState().AccountingHolds(
            static_cast<uint64_t>(
                production_reorg.GetSupply(BTCVELD_TOKEN_ID))));
        ...
        TestLedger terminal;
        auto terminal_open = MakeOpen(
            terminal_bitcoin, terminal.state, 40000, 40000,
            RECIPIENT, btcnull::EmptyProof());
        ...
        auto terminal_payout = MakePayout(
            terminal_bitcoin, terminal.state, terminal.current_reserve_tx,
            40000, terminal_commitment, destination, true);
        std::vector<btcspv::BtcTxOut> payout_outputs;
        CHECK(btcspv::ParseBtcTxOutputs(
            terminal_payout.bitcoin_tx.data(),
            terminal_payout.bitcoin_tx.size(), payout_outputs));
        CHECK(std::count_if(
                  payout_outputs.begin(), payout_outputs.end(),
                  [](const btcspv::BtcTxOut& output) {
                      return !output.spk.empty() && output.spk.front() == 0x6a;
                  }) == 1);
        CHECK(payout_outputs.back().spk.size() <=
              reserve::MAX_STANDARD_NULL_DATA_SCRIPT_BYTES);
        std::vector<btcspv::BtcPrevout> payout_prevouts;
        CHECK(btcspv::ParseBtcTxPrevouts(
            terminal_payout.bitcoin_tx.data(),
        ...
        payout_outputs.insert(
            ...
            btcspv::BtcTxOut{
                0, btcveld::RedeemRequestCommitmentScript(
                       terminal_commitment)});
        const auto nonstandard_payout = MakeBitcoinTx(
            payout_inputs, payout_outputs);
        CHECK(Classify(
                  terminal, nonstandard_payout, terminal_payout.parents,
                  terminal_payout.payout).disposition ==
              reserve::SpendDisposition::UNAUTHORIZED_SPEND);
        ...
        auto surplus_open = MakeOpen(
            close_bitcoin, close_ledger.state, 5000, 0,
            RECIPIENT, btcnull::EmptyProof());
        CHECK(close_ledger.Apply(close_bitcoin, surplus_open));
        CHECK(close_ledger.state.surplus_sats == 5000);
        auto close = MakeClose(close_bitcoin, close_ledger.state,
                               close_ledger.current_reserve_tx);
        ...
        auto fee_open = MakeOpen(
            fee_bitcoin, fee_ledger.state, 30000, 30000,
            RECIPIENT, btcnull::EmptyProof());
        CHECK(fee_ledger.Apply(fee_bitcoin, fee_open));
        CHECK(fee_ledger.OpenRedemption(10000));
        auto reserve_fee = MakePayout(
            fee_bitcoin, fee_ledger.state, fee_ledger.current_reserve_tx,
        ...
        auto first_a = MakeOpen(
            first_open_bitcoin, first_open_ledger.state, 20000, 20000,
            RECIPIENT, btcnull::EmptyProof());
        auto first_b = MakeOpen(
            first_open_bitcoin, first_open_ledger.state, 21000, 21000,
        ...
        auto rbf_original = MakeOpen(
            rbf_bitcoin, rbf_ledger.state, 12000, 12000,
            RECIPIENT, btcnull::EmptyProof(), 0);
        ...
        auto rbf_replacement = MakeOpen(
            rbf_bitcoin, rbf_ledger.state, 11000, 11000,
            RECIPIENT_2, btcnull::EmptyProof());
        ...
        const std::vector<uint8_t> old_issuer{
            'M','N','P','1',';','d','e','a','d'};
        const std::vector<uint8_t> old_public{
            'M','S','P','3',0,0,0,0};
        reserve::Claim rejected_old;
        CHECK(!reserve::DecodeProof(old_issuer.data(), old_issuer.size(),
                                    rejected_old));
        CHECK(!reserve::DecodeProof(old_public.data(), old_public.size(),
                                    rejected_old));
        ...
        CHECK(!legacy_public.ProcessBlock(
            TransactionsBlock(1, {RawCarrier("VELD_MSPV|00")},
                              "legacy-public"), peg_gate));
        ...
        CHECK(!legacy_issuer.ProcessBlock(
            TransactionsBlock(
                1, {RawCarrier(EncodeTokenOp(legacy_issuer_op))},
                "legacy-issuer"), peg_gate));
        ...
        auto lifecycle_open = MakeOpen(
            lifecycle_bitcoin, lifecycle_empty, 40000, 40000,
        ...
        CHECK(lifecycle_tokens.ProcessBlock(
            ReserveBlock(1, {lifecycle_open}), peg_gate));

        const std::vector<uint8_t> lifecycle_destination =
            lifecycle_user.GetP2PKHScript();
        ...
        const Transaction lifecycle_redeem = SignedTokenCarrier(
            lifecycle_redeem_op, lifecycle_user, "fulfilled");
        CHECK(lifecycle_tokens.ProcessBlock(
            TransactionsBlock(2, {lifecycle_redeem}, "redeem"), peg_gate));
        const auto lifecycle_redeems =
            lifecycle_tokens.LastBlockRedeemsCopy();
        CHECK(lifecycle_redeems.size() == 1);
        const auto lifecycle_request = RequestFromRedeem(
            lifecycle_redeems[0], lifecycle_redeem,
            lifecycle_bitcoin.chain.BestHeight());
        lifecycle_covenant.AddRequest(
            lifecycle_request, /*reserve_backed_without_bond=*/true);
        lifecycle_tokens.SetBtcVeldRedeemCovenant(&lifecycle_covenant);
        CHECK(lifecycle_tokens.GetSupply(BTCVELD_TOKEN_ID) == 0);
        CHECK(lifecycle_tokens.GetBtcVeldReserveState()
                  .open_redemption_principal == 40000);
        ...
        const auto before_payout_covenant =
            lifecycle_covenant.SnapshotState();

        auto lifecycle_payout = MakePayout(
            lifecycle_bitcoin,
            lifecycle_tokens.GetBtcVeldReserveState(),
            lifecycle_open.bitcoin_tx, 40000,
            lifecycle_request.request_commitment,
            lifecycle_destination, true);
        CHECK(lifecycle_tokens.ProcessBlock(
            ReserveBlock(3, {lifecycle_payout}), peg_gate));
        const auto staged_payouts =
            lifecycle_tokens.LastBlockReservePayoutsCopy();
        CHECK(staged_payouts.size() == 1);
        CHECK(staged_payouts[0].request_id ==
              lifecycle_request.request_id);
        CHECK(staged_payouts[0].principal_sats == 40000);
        CHECK(!lifecycle_covenant.IsConsumedPayout(
            lifecycle_payout.claim.bitcoin_txid));
        CHECK(lifecycle_covenant.MarkAuthorizedReservePayout(
            staged_payouts[0].request_id, staged_payouts[0].payout_txid,
            staged_payouts[0].principal_sats,
            staged_payouts[0].destination_spk));
        CHECK(lifecycle_covenant.IsConsumedPayout(
            lifecycle_payout.claim.bitcoin_txid));
        const auto fulfilled_snapshot = lifecycle_covenant.SnapshotState();
        CHECK(fulfilled_snapshot.requests.at(lifecycle_request.request_id)
                  .status == btcveld::ReqStatus::FULFILLED);
        CHECK(lifecycle_tokens.GetBtcVeldReserveState().status ==
              reserve::Status::EMPTY);
        CHECK(lifecycle_tokens.GetBtcVeldReserveState().AccountingHolds(0));
        ...
        CHECK(lifecycle_tokens.ProcessBlock(
            TransactionsBlock(3, {}, "fsp2-first"), peg_gate));
        const std::vector<uint8_t> fsp2_bytes = Fsp2Proof(lifecycle_payout);
        const auto fsp2 = btcveld::VerifyFraudulentSpend(
            lifecycle_bitcoin.chain, fsp2_bytes.data(), fsp2_bytes.size(),
            BtcVeldCustodySpk(), BTCVELD_SPV_K_BTC);
        CHECK(fsp2.ok);
        CHECK(fsp2.spend_txid == lifecycle_payout.claim.bitcoin_txid);
        const auto fsp2_classification =
            lifecycle_tokens.ClassifyBtcVeldReserveSpend(
                fsp2.spend_tx, fsp2.direct_parents);
        CHECK(fsp2_classification.disposition ==
              reserve::SpendDisposition::AUTHORIZED_TRANSITION);
        ...
        CHECK(lifecycle_tokens.ApplyFsp2AuthorizedReservePayout(
            fsp2.spend_block, fsp2.spend_merkle_directions,
            fsp2.spend_merkle_branch, fsp2.spend_tx,
            fsp2.direct_parents, fsp2_applied));
        CHECK(lifecycle_covenant.MarkAuthorizedReservePayout(
            fsp2_applied.request_id, fsp2_applied.payout_txid,
            fsp2_applied.principal_sats, fsp2_applied.destination_spk));
        CHECK(lifecycle_tokens.GetBtcVeldReserveState().status ==
              reserve::Status::EMPTY);
        CHECK(lifecycle_tokens.GetBtcVeldReserveState().AccountingHolds(0));
        CHECK(lifecycle_covenant.DueForDefault(
            lifecycle_request.deadline_height + 1).empty());
        CHECK(!lifecycle_covenant.EvalNonPayment(
            lifecycle_request.request_id,
            lifecycle_request.deadline_height + 1).slash);
        ...
        auto edge_open_a = MakeOpen(
            edge_bitcoin_a, edge_empty_a, 50000, 50000,
            edge_user_a.address, btcnull::EmptyProof());
        ...
        CHECK(edge_tokens_a.ProcessBlock(
            ReserveBlock(1, {edge_open_a}), peg_gate));
        ...
        const Transaction edge_redeem_a = SignedTokenCarrier(
            edge_redeem_op_a, edge_user_a, "edge-a");
        CHECK(edge_tokens_a.ProcessBlock(
            TransactionsBlock(2, {edge_redeem_a}, "edge-a"), peg_gate));
        const auto edge_redeems_a = edge_tokens_a.LastBlockRedeemsCopy();
        CHECK(edge_redeems_a.size() == 1);
        const auto edge_request_a = RequestFromRedeem(
            edge_redeems_a[0], edge_redeem_a,
            edge_bitcoin_a.chain.BestHeight());
        edge_covenant_a.AddRequest(
            edge_request_a, /*reserve_backed_without_bond=*/true);
        const reserve::State edge_prior_a =
            edge_tokens_a.GetBtcVeldReserveState();
        auto edge_rollover_a = MakeRollover(
            edge_bitcoin_a, edge_prior_a, edge_open_a.bitcoin_tx);
        const auto edge_rollover_verified = reserve::Verify(
            edge_bitcoin_a.chain, edge_prior_a, 40000,
            edge_rollover_a.proof.data(), edge_rollover_a.proof.size(),
            BtcVeldCustodySpk(), BTCVELD_SPV_K_BTC,
            [](const std::string&) { return true; });
        CHECK(edge_rollover_verified.ok);
        reserve::State edge_after_rollover_a = edge_prior_a;
        CHECK(reserve::ApplyAuthorized(
            edge_after_rollover_a, edge_rollover_verified, 40000, 40000));
        auto edge_successor_payout_a = MakePayout(
            edge_bitcoin_a, edge_after_rollover_a,
            edge_rollover_a.bitcoin_tx, 10000,
            edge_request_a.request_commitment,
            edge_user_a.GetP2PKHScript(), true);
        const std::vector<uint8_t> edge_fsp_bytes_a =
            Fsp2Proof(edge_successor_payout_a);
        const auto edge_fsp_a = btcveld::VerifyFraudulentSpend(
            edge_bitcoin_a.chain, edge_fsp_bytes_a.data(),
            edge_fsp_bytes_a.size(), BtcVeldCustodySpk(),
            BTCVELD_SPV_K_BTC);
        CHECK(edge_fsp_a.ok);
        CHECK(edge_tokens_a.ProcessBlock(
            ReserveBlock(3, {edge_rollover_a}), peg_gate));
        ...
        CHECK(!edge_tokens_a.ApplyFsp2AuthorizedReservePayout(
            ...
            edge_fsp_a.spend_merkle_directions,
            edge_fsp_a.spend_merkle_branch, edge_fsp_a.spend_tx,
            edge_fsp_a.direct_parents, rejected_second_edge));
        CHECK(!edge_tokens_a.FreezeBtcVeldReserve(
            TaggedHash("rtp1-then-fsp-freeze"),
            edge_fsp_a.spend_block));
        ...
        auto edge_open_b = MakeOpen(
            edge_bitcoin_b, edge_empty_b, 50000, 50000,
            edge_user_b.address, btcnull::EmptyProof());
        ...
        CHECK(edge_tokens_b.ProcessBlock(
            ReserveBlock(1, {edge_open_b}), peg_gate));
        ...
        const Transaction edge_redeem_b = SignedTokenCarrier(
            edge_redeem_op_b, edge_user_b, "edge-b");
        CHECK(edge_tokens_b.ProcessBlock(
            TransactionsBlock(2, {edge_redeem_b}, "edge-b"), peg_gate));
        const auto edge_redeems_b = edge_tokens_b.LastBlockRedeemsCopy();
        CHECK(edge_redeems_b.size() == 1);
        const auto edge_request_b = RequestFromRedeem(
            edge_redeems_b[0], edge_redeem_b,
            edge_bitcoin_b.chain.BestHeight());
        edge_covenant_b.AddRequest(
            edge_request_b, /*reserve_backed_without_bond=*/true);
        auto edge_payout_b = MakePayout(
            edge_bitcoin_b, edge_tokens_b.GetBtcVeldReserveState(),
            edge_open_b.bitcoin_tx, 10000,
            edge_request_b.request_commitment,
            edge_user_b.GetP2PKHScript(), true);
        CHECK(edge_tokens_b.ProcessBlock(
            TransactionsBlock(3, {}, "edge-b-fsp"), peg_gate));
        const std::vector<uint8_t> edge_fsp_bytes_b = Fsp2Proof(edge_payout_b);
        const auto edge_fsp_b = btcveld::VerifyFraudulentSpend(
            edge_bitcoin_b.chain, edge_fsp_bytes_b.data(),
            edge_fsp_bytes_b.size(), BtcVeldCustodySpk(),
            BTCVELD_SPV_K_BTC);
        CHECK(edge_fsp_b.ok);
        BtcVeldReservePayoutTransition edge_applied_b;
        CHECK(edge_tokens_b.ApplyFsp2AuthorizedReservePayout(
            edge_fsp_b.spend_block,
            edge_fsp_b.spend_merkle_directions,
            edge_fsp_b.spend_merkle_branch, edge_fsp_b.spend_tx,
            edge_fsp_b.direct_parents, edge_applied_b));
        CHECK(edge_covenant_b.MarkAuthorizedReservePayout(
            edge_applied_b.request_id, edge_applied_b.payout_txid,
            edge_applied_b.principal_sats,
            edge_applied_b.destination_spk));
        CHECK(!edge_tokens_b.FreezeBtcVeldReserve(
            TaggedHash("fsp-payout-then-fsp-freeze"),
            edge_fsp_b.spend_block));
        ...
        auto default_open = MakeOpen(
            default_bitcoin, default_empty, 50000, 50000,
        ...
        CHECK(default_tokens.ProcessBlock(
            ReserveBlock(1, {default_open}), peg_gate));
        ...
        const Transaction default_redeem = SignedTokenCarrier(
            default_redeem_op, default_user, "defaulted");
        CHECK(default_tokens.ProcessBlock(
            TransactionsBlock(2, {default_redeem}, "default"), peg_gate));
        const auto default_redeems = default_tokens.LastBlockRedeemsCopy();
        CHECK(default_redeems.size() == 1);
        const auto default_request = RequestFromRedeem(
            default_redeems[0], default_redeem,
            default_bitcoin.chain.BestHeight());
        default_covenant.AddRequest(
            default_request, /*reserve_backed_without_bond=*/true);
        CHECK(default_tokens.GetSupply(BTCVELD_TOKEN_ID) == 40000);
        CHECK(default_tokens.GetBtcVeldReserveState()
                  .open_redemption_principal == 10000);
        CHECK(default_tokens.GetBtcVeldReserveState().AccountingHolds(40000));
        const uint64_t default_height =
            default_request.deadline_height + 1;
        ...
        const auto default_verdict = default_covenant.EvalNonPayment(
            default_request.request_id, default_height,
            BTCVELD_NONPAY_PENALTY_BPS);
        ...
        const auto default_covenant_snapshot =
            default_covenant.SnapshotState();
        const Hash256 before_default_token_digest = default_tokens.Digest();
        const Hash256 before_default_covenant_digest =
            default_covenant.Digest();
        auto apply_default = [&]() {
            default_covenant.ApplySlash(default_verdict);
            CHECK(default_tokens.CompensateMint(
                default_verdict.compensate_to,
                default_verdict.compensate_sats, default_height,
                "daybreak-default"));
            default_covenant.SetStatus(
                default_request.request_id,
                btcveld::ReqStatus::DEFAULTED);
        };
        apply_default();
        const Hash256 after_default_token_digest = default_tokens.Digest();
        const Hash256 after_default_covenant_digest =
            default_covenant.Digest();
        CHECK(default_tokens.GetSupply(BTCVELD_TOKEN_ID) == 50000);
        CHECK(default_tokens.GetBalance(
            BTCVELD_TOKEN_ID, default_user.address) == 50000);
        CHECK(default_tokens.GetBtcVeldReserveState()
                  .open_redemption_principal == 0);
        CHECK(default_tokens.GetBtcVeldReserveState().AccountingHolds(50000));
        CHECK(default_covenant.SnapshotState().requests.at(
                  default_request.request_id).status ==
              btcveld::ReqStatus::DEFAULTED);
        CHECK(!default_covenant.EvalNonPayment(
            default_request.request_id, default_height + 1).slash);
        ...
        auto witness_open = MakeOpen(
            witness_bitcoin, witness_ledger.state, 100000, 100000,
            RECIPIENT, btcnull::EmptyProof(), BTCVELD_SPV_K_BTC, true);
        btcspv::WitnessAwareBtcTx parsed_witness_open;
        CHECK(btcspv::ParseWitnessAwareBtcTx(
            witness_open.bitcoin_tx, parsed_witness_open,
        ...
        CHECK(!btcspv::ParseBtcTxOutputs(
            witness_open.bitcoin_tx.data(), witness_open.bitcoin_tx.size(),
            legacy_only_outputs));
        const auto witness_public = reserve::Verify(
            witness_bitcoin.chain, witness_ledger.state, witness_ledger.supply,
            witness_open.proof.data(), witness_open.proof.size(),
            BtcVeldCustodySpk(), BTCVELD_SPV_K_BTC,
            [](const std::string& address) { return address == RECIPIENT; });
        ...
        CHECK(witness_public.claim.bitcoin_txid ==
              witness_issuer.claim.bitcoin_txid);
        CHECK(witness_ledger.Apply(witness_bitcoin, witness_open));

        const auto witness_pending = MakePendingDeposit(
            50000, RECIPIENT, true);
        btcspv::WitnessAwareBtcTx parsed_witness_pending;
        CHECK(btcspv::ParseWitnessAwareBtcTx(
            witness_pending, parsed_witness_pending));
        const std::string witness_pending_outpoint =
            btcspv::BtcDepositOutpointId(parsed_witness_pending.txid, 0);
        auto witness_deposit = MakeDeposit(
            witness_bitcoin, witness_ledger.state,
            witness_ledger.current_reserve_tx, witness_pending,
            150000, 40000,
            witness_ledger.WitnessFor(witness_pending_outpoint), true);
        const auto witness_deposit_classification = Classify(
            witness_ledger, witness_deposit.bitcoin_tx,
            witness_deposit.parents);
        if (witness_deposit_classification.disposition !=
                reserve::SpendDisposition::AUTHORIZED_TRANSITION)
            throw std::runtime_error(
                "witness DEPOSIT classification failed: " +
        ...
        auto witness_rollover = MakeRollover(
            witness_bitcoin, witness_ledger.state,
        ...
        auto witness_payout = MakePayout(
            witness_bitcoin, witness_ledger.state,
            witness_ledger.current_reserve_tx, 10000,
            TaggedHash("witness-payout-request"), {0x51}, true, true);
        CHECK(Classify(witness_ledger, witness_payout.bitcoin_tx,
                       witness_payout.parents,
                       witness_payout.payout).disposition ==
              reserve::SpendDisposition::AUTHORIZED_TRANSITION);
        const std::vector<uint8_t> witness_fsp2 = Fsp2Proof(witness_payout);
        const auto witness_fraud = btcveld::VerifyFraudulentSpend(
            witness_bitcoin.chain, witness_fsp2.data(), witness_fsp2.size(),
            BtcVeldCustodySpk(), BTCVELD_SPV_K_BTC);
        ...
        const auto stripped_witness_open = MakeBitcoinTx(
            {{parsed_witness_open.prevouts[0].txid,
              parsed_witness_open.prevouts[0].vout}},
        ...
        CHECK(!btcspv::ParseWitnessAwareBtcTx(
            unknown_witness_flag, malformed_witness));
        std::vector<uint8_t> superfluous_witness;
        superfluous_witness.insert(
            superfluous_witness.end(), stripped_witness_open.begin(),
            stripped_witness_open.begin() + 4);
        superfluous_witness.push_back(0x00);
        superfluous_witness.push_back(0x01);
        superfluous_witness.insert(
            superfluous_witness.end(), stripped_witness_open.begin() + 4,
            stripped_witness_open.end() - 4);
        superfluous_witness.push_back(0x00);
        superfluous_witness.insert(
            superfluous_witness.end(), stripped_witness_open.end() - 4,
            stripped_witness_open.end());
        CHECK(!btcspv::ParseWitnessAwareBtcTx(
            superfluous_witness, malformed_witness));
        auto truncated_witness = witness_open.bitcoin_tx;
        truncated_witness.pop_back();
        CHECK(!btcspv::ParseWitnessAwareBtcTx(
            truncated_witness, malformed_witness));
        auto trailing_witness = witness_open.bitcoin_tx;
        trailing_witness.push_back(0x00);
        CHECK(!btcspv::ParseWitnessAwareBtcTx(
            trailing_witness, malformed_witness));
        std::vector<uint8_t> noncanonical_vin;
        noncanonical_vin.insert(noncanonical_vin.end(),
                                stripped_witness_open.begin(),
                                stripped_witness_open.begin() + 4);
        noncanonical_vin.insert(noncanonical_vin.end(), {0xfd, 0x01, 0x00});
        noncanonical_vin.insert(noncanonical_vin.end(),
                                stripped_witness_open.begin() + 5,
                                stripped_witness_open.end());
        CHECK(!btcspv::ParseWitnessAwareBtcTx(
            noncanonical_vin, malformed_witness));
        ...
        std::cout << "PASS daybreak_reserve_tests checks=" << g_checks
                  ...
                  << " canonical_digest="
                  << HashToHex(ledger.CompleteDigest())
                  << " production_token_digest="
                  << HashToHex(production.Digest()) << "\n";
        return 0;
    } catch (const std::exception& error) {
        std::cerr << "FAIL daybreak_reserve_tests: " << error.what() << "\n";
        return 1;
    }
}
