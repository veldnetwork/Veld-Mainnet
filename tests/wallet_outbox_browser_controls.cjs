...
const sourcePath =
    process.env.VELD_WALLET_TEST_SOURCE || path.join(__dirname, '../include/network/ui_desktop.h');
...
const functions = [
    ...
    '_veldJournal',
    '_veldJournalOperation',
    '_veldGetSingleCanonicalOpReturnPayload',
    '_veldRecoverLegacyTransactions',
    '_veldJournalSign',
    '_veldJournalBroadcast',
    '_veldRetryJournalTransaction',
    '_veldBroadcastExactSigned',
    'injectSignatures',
    'rpc',
];
const constants = source.slice(
    source.indexOf('  var K256 = ['),
    source.indexOf('  function sha256Compress('),
);
...
async function launch() {
    browserContext = await (browserName === 'WebKit' ? webkit : chromium).launchPersistentContext(
        profile,
        {
            headless: true,
            executablePath: process.env.VELD_TEST_BROWSER_EXECUTABLE || undefined,
            args:
...
(async () => {
    ...
    let first = await p.evaluate(async () => {
        ...
        const signed = await injectSignatures(
            unsigned,
            metadata,
            __veldKey.get(),
            null,
        ...
        if (before.payload.signed !== signed || before.record.state !== 'ready')
            throw Error('Signed bytes were not committed');
        unavailable = true;
        try {
            await _veldBroadcastExactSigned(signed, __veldKey.get(), generation);
        } catch (e) {
        ...
        return {
            ...
            signatureCalls,
            attempt: sends[0],
            claims: Array.from(await _veldJournal().guards(owner)),
            state: (await _veldJournal().get(txid)).record.state,
    ...
    results.push({
        case: 'persist-before-delivery-and-uncertain-response',
        ms: first.ms,
        result: 'PASS',
    });
    ...
    const resumed = await p.evaluate(async (txid) => {
        ...
        return {
            ...
            guardCount: (await _veldJournal().guards(owner)).size,
        };
    }, first.txid);
    ...
    const blocked = await second.evaluate(async () => {
        try {
            ...
            return { message: error.message, signatureCalls };
        }
    ...
    const accounting = await p.evaluate(async (txid) => {
        selected = [
            { txid: '12'.repeat(32), vout: 0, value_units: 200000000 },
            { txid: '12'.repeat(32), vout: 1, value_units: 300000000 },
        ];
        nodeSpendable = 5;
        ...
        selected.unshift({ txid: '12'.repeat(32), vout: 0, value_units: 200000000 });
        nodeSpendable = 5;
        ...
        return {
            ...
            confirmed,
            restored,
            pending: (await _veldJournal().pending(owner)).length,
        };
    }, first.txid);
    ...
    const interruption = await p.evaluate(async () => {
        ...
        try {
            await _veldJournal().sign(bytes, metadata, owner, async () => {
                throw Error('Ordinary cancellation');
            });
        } catch (e) {
            if (!e.message.includes('cancellation')) throw e;
        }
        ...
        return {
            held,
            interrupted: pending.some((row) => row.interrupted),
            signed: saved.payload.signed,
            signatureCalls,
        };
    });
    ...
    const legacy = await p.evaluate(async () => {
        ...
        localStorage.setItem(
            'veld_pending_txs',
            JSON.stringify([{ txid, from_addr: owner, ts: 42 }]),
        );
        const previous = _veldRpcTransport;
        _veldRpcTransport = async (method, params) =>
            method === 'gettransactionrecent' ? { raw_hex: raw, txid } : previous(method, params);
        ...
        return {
            raw: recovered.payload.signed,
            txid: recovered.record.txid,
            guards: (await _veldJournal().guards(owner)).size,
        };
    });
    ...
    const disconnected = await p.evaluate(async () => {
        ...
        localStorage.setItem(
            'veld_pending_txs',
            JSON.stringify([{ txid: '99'.repeat(32), from_addr: owner, ts: 41 }]),
        );
        let message = '';
        try {
            await injectSignatures(
                ordinaryTransaction(4, 15),
                metadata,
                __veldKey.get(),
                null,
                generation,
    ...
    results.push({
        case: 'unavailable-older-record-retains-evidence-and-stops-signing',
        result: 'PASS',
    });
    const storageFailure = await p.evaluate(async () => {
        ...
        IDBObjectStore.prototype.add = function (...args) {
            if (this.name === 'payloads')
                throw new DOMException(
                    'Fixture storage capacity unavailable',
                    'QuotaExceededError',
                );
        ...
        return {
            initial,
            after: (await _veldJournal().guards(owner)).size,
            message,
            signatures: signatureCalls - before,
        };
    });
    ...
    const interruptedWrite = await p.evaluate(async () => {
        ...
        IDBObjectStore.prototype.put = function (...args) {
            if (this.name === 'payloads')
                throw new DOMException(
                    'Fixture signed-record write unavailable',
                    'QuotaExceededError',
                );
        ...
        let returned = false,
            message = '';
        try {
            await injectSignatures(unsigned, metadata, __veldKey.get(), null, generation);
            returned = true;
        } catch (error) {
            message = error.message;
        } finally {
        ...
        const recovered = await injectSignatures(
            unsigned,
            metadata,
            __veldKey.get(),
            null,
        ...
        return {
            ...
            held: saved.record.inputs.length,
            unpublished: saved.payload.signed === '',
            recovered: !!recovered,
        };
    });
    ...
    results.push({
        case: 'signed-record-write-failure-keeps-reservation-and-releases-no-bytes',
        result: 'PASS',
    });
    const large = await p.evaluate(async () => {
        ...
        for (let i = 5; i < count + 5; i++)
            unsigned += '12'.repeat(32) + i.toString(16).padStart(2, '0') + '00000000ffffffff';
        unsigned +=
            '01' + '0f00000000000000' + '19' + '76a914' + '19'.repeat(20) + '88ac' + '00000000';
        const items = Array.from({ length: count }, () => ({
            sighash_hex: 'inert-reviewed-input',
        }));
        const build = buildScriptSig;
        buildScriptSig = () => {
            signatureCalls++;
            return '00'.repeat(5269);
        };
        const start = performance.now();
        let signed;
        try {
            signed = await injectSignatures(unsigned, items, __veldKey.get(), null, generation);
        } finally {
            buildScriptSig = build;
        }
        const saved = await _veldJournal().get(veldCrypto.sha256d(signed));
        return {
            bytes: signed.length / 2,
            ms: performance.now() - start,
            inputs: saved.record.inputs.length,
    ...
    const storage = await p.evaluate(async () => {
        if (!navigator.storage || !navigator.storage.estimate)
            return { usage: null, quota: null, measurement: 'not exposed by this engine' };
    ...
    console.log(
        JSON.stringify(
            {
                status: 'PASS',
                sourceSHA256: hash(Buffer.from(source)),
                sourcePath,
                browser: browserName,
                version,
                profile,
                ...
                storage,
                realKeys: 0,
                cryptographicSignatures: 0,
                productionRequests: 0,
                limit: 'Disposable browser-engine controls; installed iOS PWA, storage deletion and downgrade remain separate.',
            },
            null,
            2,
        ),
