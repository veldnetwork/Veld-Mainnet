...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='veld-signing-safety-') as temp:
        ...
        def run(name, mode, path, expected=0):
            r = subprocess.run([str(binary), mode, str(path)], env=env,
                               text=True, capture_output=True, timeout=15)
            results.append(dict(name=name, status='PASS' if r.returncode == expected else 'FAIL',
                                expected_exit=expected, exit_code=r.returncode,
        ...
        for name, body in {
            ...
            file = root/name; file.write_text(body, encoding='ascii')
        ...
        linked = root/'hardlink'; os.link(path, linked)
        run('hard-link-alias', 'write-a', linked, 3)
        os.unlink(linked)

        for label, mode in [('journal','hold'),('identity','identity-hold')]:
            live = root/(label+'-live')
            child = subprocess.Popen([str(binary),mode,str(live)], env=env,
                                     stdin=subprocess.PIPE,stdout=subprocess.PIPE,
                                     stderr=subprocess.PIPE,text=True)
            try:
                assert child.stdout.readline().strip() == 'AUTHORIZED'
                alias = live.parent/'.'/live.name
                run(label+'-second-process', 'write-b' if label=='journal' else 'identity-b',
                    alias if label=='journal' else root/'other-datadir', 3)
                child.kill(); child.wait(timeout=10)
                run(label+'-crash-exact-retry', 'write-a' if label=='journal' else 'identity-a',live)
                run(label+'-crash-reorg-refused', 'write-b' if label=='journal' else 'identity-b',live,3)
            finally:
        ...
        if os.name != 'nt':
            symlink = root/'symlink'; symlink.symlink_to(path)
            ...
            r = subprocess.run([str(binary),'write-a',str(root/'io-failure')],
                               env=env,capture_output=True,text=True,preexec_fn=limit,timeout=15)
            results.append(dict(name='partial-write-refused',status='PASS' if r.returncode==3 else 'FAIL',
    ...
    report = dict(status='PASS' if all(r['status']=='PASS' for r in results) else 'FAIL',
                  platform=os.name, binary=str(binary), cases=results,
