...
wallet = (
    args.wallet_source.read_text(encoding="utf-8")
...
assert (
    redeems.index("AcquireConsensusTransitionGuard()")
    < redeems.index("index.ReadPage(")
    < redeems.index("FinalHeight()")
)
assert (
...
history = region(
    explorer, 'if (resource == "blocks" && parts.size() == 5)', 'if (resource == "blocks")'
)
...
callers = {
    "autoConsolidateRun": "_veldAutomaticConsolidationBudget()",
...
for name, budget in callers.items():
    ...
    print("CHECKED: " + name)
print(
    "PASS: launcher-independent recovery, finality publication/retirement, cache classification, bounded Explorer routes, both consolidation callers"
)
