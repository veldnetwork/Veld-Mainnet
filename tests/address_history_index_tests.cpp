...
class MemoryStore final : public veld::db::KVStore {
  public:
    ...
    void IterateFrom(const std::string& prefix, const std::string& start_after,
                     std::function<bool(const std::string&, const std::string&)> fn) override {
        auto it =
            start_after.empty() ? values_.lower_bound(prefix) : values_.upper_bound(start_after);
    ...
    std::string FirstKey(const std::string& prefix) const {
        ...
        return it != values_.end() && it->first.rfind(prefix, 0) == 0 ? it->first : std::string{};
    }
...
veld::Transaction Coinbase(const std::vector<uint8_t>& script, uint64_t value,
...
int main() {
    ...
    Transaction genesis_coinbase = Coinbase(alice_script, 100 * VELD_UNITS, "genesis");
    ...
    Block b1 = MakeBlock(1, b0.GetHash(), {Coinbase(miner_script, 2 * VELD_UNITS, "one"), spend});
    Block b2 = MakeBlock(2, b1.GetHash(), {Coinbase(alice_script, 2 * VELD_UNITS, "two")});
    ...
    const bool rebuilt = Rebuild(
        ...
        [&](uint64_t height, const Hash256& hash) { return height == 2 && hash == b2.GetHash(); });
    ...
    Check(MarkersMatch(store, 2, b2.GetHash()), "rebuild publishes exact canonical marker");
    ...
    Check(alice_page && alice_page->has_more && !alice_page->next_cursor.empty(),
    ...
    Check(alice_page && alice_page->entries[0].block_height == 2 &&
              ...
              alice_page->entries[0].net_units == static_cast<int64_t>(2 * VELD_UNITS),
          "newest mining reward is first");
    Check(alice_page && alice_page->entries[1].block_height == 1 &&
              alice_page->entries[1].type == "sent" &&
              alice_page->entries[1].net_units == -static_cast<int64_t>(10 * VELD_UNITS) &&
              alice_page->entries[1].fee_units == VELD_UNITS,
          "spend binds exact debit and fee");
    Check(store.iterate_from_callbacks == 3, "page reads no more than limit plus one index row");
    Check(block_loads == loads_before_query, "history query never invokes the block loader");

    store.iterate_from_callbacks = 0;
    auto alice_next = ReadPage(store, alice_script, 2, alice_page ? alice_page->next_cursor : "");
    Check(alice_next && alice_next->entries.size() == 1 && alice_next->entries[0].block_height == 0,
          "cursor continues strictly after the prior row");
    Check(alice_next && !alice_next->has_more && alice_next->next_cursor.empty(),
          "final page has no continuation cursor");
    Check(store.iterate_from_callbacks == 1, "final page reads only the remaining index row");

    auto bob_page = ReadPage(store, bob_script, 50, "");
    Check(bob_page && bob_page->entries.size() == 1 && bob_page->entries[0].type == "received" &&
              bob_page->entries[0].net_units == static_cast<int64_t>(10 * VELD_UNITS),
    ...
    Check(!ReadPage(store, alice_script, 1, RowPrefixForScript(bob_script) + "cursor"),
          "cross-address cursor fails closed");
    Check(!ReadPage(store, alice_script, 1, std::string(MAX_CURSOR_SIZE + 1, 'x')),
          "oversized cursor fails closed");

    Check(Rollback(store, b2), "tip rollback succeeds");
    Check(MarkersMatch(store, 1, b1.GetHash()), "rollback restores exact parent marker");
    alice_page = ReadPage(store, alice_script, 50, "");
    Check(alice_page && alice_page->entries.size() == 2 && alice_page->entries[0].block_height == 1,
          "rollback removes every row created by the popped block");

    Block alternate2 =
        MakeBlock(2, b1.GetHash(), {Coinbase(bob_script, 3 * VELD_UNITS, "alternate")});
    Check(Advance(store, alternate2), "linear advance after rollback succeeds");
    bob_page = ReadPage(store, bob_script, 50, "");
    Check(bob_page && bob_page->entries.size() == 2 && bob_page->entries[0].block_height == 2 &&
    ...
    Check(!MarkersMatch(store, 2, alternate2.GetHash()), "missing completion marker is detected");
    ...
    for (size_t i = 0; i < pools.size(); ++i) {
        const auto found = std::find_if(
            reward_page->entries.begin(), reward_page->entries.end(),
    ...
    Check(EntryType(spend, 0, static_cast<int64_t>(VELD_UNITS), "VELD_DIST|STAKING|fixture") ==
              "received",
          "an ordinary sender cannot label a transfer as a staking reward");

    std::cout << (failures == 0 ? "PASS " : "FAIL ")
              << "address_history_index_tests checks=" << checks << " block_loads=" << block_loads
              << "\n";
