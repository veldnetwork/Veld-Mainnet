...
int main(int argc, char** argv) {
    ...
    struct Wipe {
        std::string& s;
        ~Wipe() {
            veld::compat::SecureZero(s.data(), s.size());
        }
    } wipe{pass};
    std::string error;
    if (!veld::channel::secure_file::EnsurePrivateDirectory(target.string(), &error))
        return 4;
    for (size_t i = 0; i < 11; ++i) {
        ...
        if (i >= 8 && !std::filesystem::exists(path)) {
            ...
            const bool ok =
                veld::channel::secure_file::AtomicWrite(path.string(), bytes, &error, true);
            veld::compat::SecureZero(bytes.data(), bytes.size());
            if (!ok)
                return 5;
        }
        if (std::filesystem::file_size(path) != 32)
            return 6;
        ValidatorKey key;
        ...
        if (!key.HasExactIdentityBinding())
            return 8;
        if (i && !key.Save((target / ("validator-" + std::to_string(i) + ".key")).string(), pass))
            return 9;
        std::cout << key.address << "\n";
        veld::compat::SecureZero(key.privkey.data(), key.privkey.size());
    }
    if (node != run) {
        std::ifstream stream(node / "test-rpc-token");
        std::string token;
        stream >> token;
        if (token.size() != 64)
            return 10;
        ...
        if (!veld::channel::secure_file::AtomicWrite((node / "rpc.token").string(), encrypted,
                                                     &error, true))
            return 11;
