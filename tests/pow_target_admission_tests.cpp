...
void ExpectError(uint32_t bits, veld::CompactTargetError expected, const char* name) {
    veld::CanonicalPowTarget decoded;
    veld::CompactTargetError error = veld::CompactTargetError::None;
    Check(!veld::DecodeCanonicalVeldTarget(bits, decoded, &error) && error == expected, name);
}
...
struct FakeNmsChain {
    ...
    std::optional<uint64_t> GetHeightByHashLocked(const veld::Hash256& hash) const {
        if (hash == parent)
...
int main() {
    ...
    ExpectError(0x20ffffffu, CompactTargetError::Negative, "0x20ffffff alias rejected");
    ExpectError(0x1e800001u, CompactTargetError::Negative, "negative compact rejected");
    ExpectError(0x00000000u, CompactTargetError::Zero, "zero compact rejected");
    ExpectError(0x23000001u, CompactTargetError::Overflow, "overflow compact rejected");
    ExpectError(0x2100ffffu, CompactTargetError::AboveLimit, "above-limit compact rejected");
    ...
    Check(DecodeExpectedVeldTarget(GENESIS_BITS, GENESIS_BITS, internal_target),
          "internal miner accepts exact canonical branch bits");
    Check(!DecodeExpectedVeldTarget(VELD_POW_LIMIT_BITS, GENESIS_BITS, internal_target),
    ...
    Check(ValidateNms(nms, nms_chain, 11), "NMS accepts exact next-block canonical parent");
    Check(!ValidateNms(nms, nms_chain, 12), "NMS rejects historical-parent dataset rotation");
    nms.header.bits = 0x20ffffffu;
    nms.raw = EncodeNmsPayload(nms.header);
    Check(!ValidateNms(nms, nms_chain, 11), "NMS rejects compact alias before work");
    nms.header.bits = GENESIS_BITS;
    nms.raw = EncodeNmsPayload(nms.header);
    ExpensivePowBudget nms_defer_budget(1, 1, std::chrono::minutes(1));
    auto consume_nms_start = nms_defer_budget.TryAcquire(ExpensivePowUse::PeerNms);
    consume_nms_start.reset();
    bool nms_deferred = false;
    Check(!ValidateNms(nms, nms_chain, 11, &nms_defer_budget, &nms_deferred,
                       ExpensivePowUse::PeerNms) &&
              nms_deferred,
          "NMS budget refusal is deferred local work, not invalid consensus");
    Check(ValidateNmsWithDisposition(nms, nms_chain, 11, &nms_defer_budget,
                                     ExpensivePowUse::PeerNms) ==
    ...
    Hash256 seed = ComputeEpochSeed(limit.bytes, DATASET_EPOCH_BLOCKS + 1, limit);
    for (int i = 0; i < 8; ++i) {
        {
            DatasetHandle held = GlobalDataset().get_for_seed(seed);
            Check(held.get() != nullptr, "canonical alternating request obtains dataset");
        }
        auto before_bad = GlobalDataset().Stats();
        Hash256 rejected = VeldHash(HeaderWithBits(0x20ffffffu), DATASET_EPOCH_BLOCKS + 1);
        auto after_bad = GlobalDataset().Stats();
        bool sentinel = true;
        for (uint8_t byte : rejected)
            sentinel &= byte == 0xff;
        Check(sentinel && !g_veldhash_last_dataset_ok(), "alias rejected before hashing");
        Check(after_bad.requests == before_bad.requests && after_bad.builds == before_bad.builds,
    ...
    Check(obtained.load() == workers.size(), "all concurrent identical requests obtain dataset");
    Check(singleflight.builds == 1 && singleflight.hits == workers.size() - 1,
          "concurrent identical requests perform one dataset build");
    Check(DatasetCache::CapacityIdentities() == 1, "dataset identity cache is bounded");
    ...
    ExpensivePowBudget rate_budget(1, 2, std::chrono::minutes(1));
    ...
    Check(!rate_three.has_value(), "sequential external work budget fails closed");
    Check(rate_budget.TryAcquire(ExpensivePowUse::InternalMine).has_value(),
    ...
    auto peer_context = PowAdmissionContext::Peer("2001:db8::17", owned_source_budget);
    owned_source_budget.reset();
    Check(peer_context.HasRequiredProvenance() && !retained_source.expired(),
    ...
    Check(PowAdmissionContext::Rpc("submitblock").ReorgUse() == ExpensivePowUse::RpcReorg,
          "RPC reorg remains globally external");
    Check(PowAdmissionContext::Internal().ReorgUse() == ExpensivePowUse::InternalReorg,
          "internal reorg has an explicit non-peer classification");
    Check(!PowAdmissionContext::Peer("", retained_source.lock()).HasRequiredProvenance() &&
              !PowAdmissionContext::Peer("198.51.100.7", {}).HasRequiredProvenance(),
    ...
    ExpensivePowBudget reorg_rate_budget(1, 1, std::chrono::minutes(1));
    auto peer_reorg = reorg_rate_budget.TryAcquire(ExpensivePowUse::PeerReorg);
    Check(peer_reorg.has_value(), "first peer reorg enters external budget");
    peer_reorg.reset();
    Check(!reorg_rate_budget.TryAcquire(ExpensivePowUse::RpcReorg).has_value(),
          "peer and RPC reorg share the external rate envelope");
    Check(reorg_rate_budget.TryAcquire(ExpensivePowUse::InternalReorg).has_value(),
          "external reorg exhaustion preserves internal recovery lane");

    auto global_first = GlobalExpensivePowBudget().TryAcquire(ExpensivePowUse::PeerBlock);
    auto global_second = GlobalExpensivePowBudget().TryAcquire(ExpensivePowUse::InternalMine);
    auto global_third = GlobalExpensivePowBudget().TryAcquire(ExpensivePowUse::RpcSubmit);
    Check(global_first.has_value() && global_second.has_value() && !global_third.has_value(),
