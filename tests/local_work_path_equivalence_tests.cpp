...
int main(int argc, char** argv) {
    try {
        ...
        Check(
            !RpcSucceeded(random_token_refusal) &&
                random_token_refusal.find(attacker_token) == std::string::npos &&
                Blockchain::TestAddBlockDirectCalls() == 0 &&
                node.TestWorkDurableWriterCalls() == 0 && node.TestWorkBlockBroadcastCalls() == 0 &&
                node.TestCachedTemplateCount() == 1 && Same(before_refusals, Observe(node)),
        ...
        Check(
            !RpcSucceeded(genuine_token_mismatch) &&
        ...
        submit_b.join();
        Check(
            !simultaneous_failure_a && !simultaneous_failure_b &&
                (RpcSucceeded(simultaneous_a) != RpcSucceeded(simultaneous_b)) &&
                Blockchain::TestAddBlockDirectCalls() == 1 &&
                node.TestWorkDurableWriterCalls() == 1 && node.TestWorkBlockBroadcastCalls() == 1 &&
                node.GetChain().Height() == 3 && node.TestCachedTemplateCount() == 0 &&
