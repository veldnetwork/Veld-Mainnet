...
int main(int argc, char** argv) {
    ...
    try {
        if (argc != 2)
            throw std::runtime_error("supply a new disposable fixture directory");
        fs::path base = argv[1];
        if (fs::exists(base))
            throw std::runtime_error("fixture path already exists");
        ...
        for (const auto* marker : {sr::REQUEST, sr::REVOKED, sr::JOURNAL, sr::IMPORT}) {
            ...
            if (!refused || !fs::is_directory(root / "db") || !fs::exists(root / marker))
                throw std::runtime_error("disabled build bypassed recovery obligation");
            ++checks;
        }
