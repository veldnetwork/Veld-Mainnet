...
isolated = [
    "-DVELD_FIRST_ACTIVATION_NETWORK",
    "-include",
    str(a.fixture_header.resolve()),
    "-include",
    str(root / "tests/security_first_activation_profile.h"),
]
cases = [
    ("public_10000_bond", public, 10000, 3840, True),
    (
        "existing_regtest_10000_finality",
        ["-include", str(root / "tests/security_state_migration_profile.h")],
        10000,
        2880,
        True,
    ),
...
for name, flags, bond, height, allowed in cases:
    cmd = [
        ...
        "-fsyntax-only",
        "-I" + str(root / "include"),
        "-DEXPECT_BOND=" + str(bond),
        "-DEXPECT_MIGRATION_HEIGHT=" + str(height),
        *flags,
        str(source),
    ]
    ...
    if not allowed:
        passed = passed and ("isolated" in r.stderr or "public releases" in r.stderr)
...
cmd = [
    ...
    "-I" + str(root / "include"),
    "-DEXPECT_BOND=50",
    "-DEXPECT_MIGRATION_HEIGHT=3360",
    *isolated,
    str(source),
    "-o",
    str(out / "checkpoint-contract.exe"),
]
