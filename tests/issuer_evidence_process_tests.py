...
def main():
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='issuer-evidence-') as temporary:
            ...
            address = re.search(
                r'^address:\s+(\S+)\s*$', run([args.keygen, 'show', key]).stdout, re.MULTILINE
            ).group(1)
            ...
            for name, command, operation, recipient, amount, digest, prepared_name in rows:
                ...
                built = json.loads(
                    run([args.keygen, 'prepare-signing-stdin'], data=json.dumps(payload)).stdout
                )
                if args.expected_genesis is not None:
                    assert built['genesis_hash'] == args.expected_genesis, (
                        'native evidence differs from the independent RPC genesis pin'
                    )
                assert built['operation_identity_digest'] == digest
                assert built['prepared'] == {
                    k: v for k, v in prepared.items() if not k.startswith('reserve_prior_')
                }
                chain = {
                    'profile_id': 'offline-fixture',
                    'consensus_build_profile': 'offline-fixture',
                    'disposable': True,
                ...
                raw_by_id = {
                    hashlib.sha256(hashlib.sha256(bytes.fromhex(p)).digest()).hexdigest(): p
                    for p in parents
                }
                resolved = [
                    {
                        'txid': hashlib.sha256(
                            hashlib.sha256(bytes.fromhex(meta['parent_tx_hex'])).digest()
                        ).hexdigest(),
                        'value_units': meta['value'],
                        'script_pubkey_hex': meta['prev_script_hex'],
                        'block_height': 1,
                    }
                ...
                def rpc(method, params):
                    if method == 'getnetworkinfo':
                        return dict(chain)
                    if method == 'getcompiledgenesis':
                        return chain['genesis_hash']
                    if method == 'getblockhash':
                        return (
                            chain['genesis_hash'] if params[0] == 0 else chain['launch_block_hash']
                        )
                    if method == 'gettransaction':
                        assert params[1] == 1
                        return {
                            'txid': params[0],
                            'raw_hex': raw_by_id[params[0]],
                            'block_height': 1,
                            'block_hash': chain['launch_block_hash'],
                        }
                    raise AssertionError(method)

                context = {
                    k: v for k, v in prepared.items() if k.startswith('reserve_prior_')
                } or None
                ...
                assert (
                    'PASS signed-inputs='
                    in run([args.fixture, '--verify-signed', address, evidence_path, signed]).stdout
                )
                carrier = {
                    'issuer_script_hex': script,
                    'unsigned_tx_hex': prepared['unsigned_tx_hex'],
                ...
                assert len(bytes.fromhex(carrier['signed_tx_hex'])) == len(
                    bytes.fromhex(carrier['unsigned_tx_hex'])
                ) + 5271 * len(prepared['inputs'])
                checked = json.loads(
                    run(
                        [args.keygen, 'verify-signed-carrier-stdin'], data=json.dumps(carrier)
                    ).stdout
                )
                ...
                damaged[60] ^= 1
                run(
                    [args.keygen, 'verify-signed-carrier-stdin'],
                    data=json.dumps(dict(carrier, signed_tx_hex=damaged.hex())),
                    expected=2,
                )
                run(
                    [args.keygen, 'verify-signed-carrier-stdin'],
                    data=json.dumps(
                        dict(carrier, unsigned_tx_hex=carrier['unsigned_tx_hex'] + '00')
                    ),
                    expected=2,
                )
                bad = dict(
                    payload,
                    parent_transactions=list(reversed(parents)) if len(parents) > 1 else ['00'],
                )
                run([args.keygen, 'prepare-signing-stdin'], data=json.dumps(bad), expected=2)
                run(
                    [args.keygen, 'prepare-signing-stdin'],
                    data=json.dumps(payload)[:-1] + ',"extra":0}',
                    expected=2,
                )
                if os.name == 'posix':
                    ...
                    password.write_text(env['VELD_VAULT_PASSPHRASE'])
                    password.chmod(0o600)
                    with mock.patch.multiple(
                        service,
                        PREVOUT_STATEF=str(journal),
                        SIGNING_STAGE_DIR=str(stage),
                        KEYGEN=str(args.keygen),
                        KEYFILE=str(key),
                        ...
                        gates = []
                        actual = service.sign_or_recover_staged_carrier(
                            state,
                            owner,
                            prepared['unsigned_tx_hex'],
                            script,
                            'isolated issuer signature',
                            build_evidence=lambda: evidence,
                            revalidate=lambda: gates.append(True),
                        )
                        assert len(gates) >= 3
                        assert actual == service.sign_or_recover_staged_carrier(
                            state, owner, prepared['unsigned_tx_hex'], script, 'recovery'
                        )
                        ...
                        service.mark_issuer_prevout_signature(
                            ...
                            prepared['unsigned_tx_hex'],
                            hashlib.sha256(
                                hashlib.sha256(bytes.fromhex(actual)).digest()
                            ).hexdigest(),
                        )
                ...
                report['checks'].append(
                    {
                        'operation': name,
                        'native_evidence_parity': True,
                        'independent_genesis_pin_checked': args.expected_genesis is not None,
                        'actual_signature_verified': True,
                        'wrong_parent_refused': True,
                        'native_committed_carrier_verified': True,
                        'forged_signature_refused': True,
