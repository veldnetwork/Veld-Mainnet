...
int main() {
    char mode;
    while (std::cin >> mode) {
        int64_t v, b, av, ab, amount;
        unsigned direction;
        uint64_t height;
        if (!(std::cin >> v >> b >> av >> ab >> amount >> direction >> height))
            return 2;
        AmmLedger ledger;
        ...
        if (mode == 'S') {
            ...
            if (out != q.amount_out || (q.reject && before != after))
                return 3;
            ...
            AmmLedger restored;
            restored.RestoreState(ledger.SnapshotState());
            if (restored.Digest() != after)
                return 4;
            restored.RestoreState(snapshot);
            if (restored.Digest() != before)
                return 5;
            if (restored.SwapAtHeight("VELD:btcVELD", direction != 0, amount, height) != out ||
                restored.Digest() != after)
                return 6;
        } else if (mode != 'Q')
            return 2;
        if (mode == 'Q' && before != ledger.Digest())
            return 7;
        std::cout << q.reject << ' ' << unsigned(q.band) << ' ' << q.fee_bps << ' ' << q.amount_out
                  << ' ' << q.gross_out << ' ' << q.fee_out << ' ' << q.post_reserve_veld << ' '
                  << q.post_reserve_btcveld << ' ' << q.rebalances_anchor << ' '
                  << AmmLedger::SwapRejectCodeName(q.reject_code) << '\n';
    }
}
