...
void* operator new(std::size_t size) {
    if (allocation_probe::fail_next.exchange(0,
...
work_admission::Binding TemplateBinding(uint64_t generation = 17,
                                        uint8_t identity = 0x51) {
...
struct ReentrantCopyMint {
    ...
    explicit ReentrantCopyMint(std::shared_ptr<CopyReentryState> value)
        : state(std::move(value)) {}
    ReentrantCopyMint(const ReentrantCopyMint& other)
        : state(other.state) {
...
struct ReentrantCopyClock {
    ...
    ReentrantCopyClock(std::shared_ptr<CopyReentryState> value,
                       Store::TimePoint current)
        : state(std::move(value)), now(current) {}
    ReentrantCopyClock(const ReentrantCopyClock& other)
        : state(other.state), now(other.now) {
        if (state && state->armed && state->store) {
            (void)state->store->GetSnapshot();
            state->reentries.fetch_add(1, std::memory_order_acq_rel);
        }
    }
    Store::TimePoint operator()() const { return now; }
};
...
int main() {
    try {
        ...
        auto issued = fixture.store.Issue(
            binding, 23, std::chrono::milliseconds(5000));
        Check(issued && issued.authorization &&
                  issued.authorization->token.size() == 64 &&
                  issued.authorization->binding == binding &&
                  issued.authorization->coordinator_generation == 23 &&
                  issued.authorization->ttl ==
        ...
        Check(!mismatch &&
                  mismatch.error == Store::Error::BindingMismatch &&
                  fixture.store.GetSnapshot().active == 1,
              "caller-edited template identity does not burn authorization");

        std::string unknown_token(64, 'a');
        auto unknown = fixture.store.Consume(
            unknown_token, binding, 23);
        ...
        Fixture concurrent;
        auto concurrent_issue = concurrent.store.Issue(
            TemplateBinding(27, 0x59), 29,
            std::chrono::milliseconds(1000));
        Check(concurrent_issue && concurrent_issue.authorization,
              "concurrent bearer issued");
        ...
        std::thread consumer_a([&] {
            consume_start.wait();
            consume_a = concurrent.store.Consume(
                concurrent_issue.authorization->token,
                concurrent_issue.authorization->binding, 29);
        });
        std::thread consumer_b([&] {
            consume_start.wait();
            consume_b = concurrent.store.Consume(
                concurrent_issue.authorization->token,
                concurrent_issue.authorization->binding, 29);
        });
        ...
        Check(static_cast<unsigned>(static_cast<bool>(consume_a)) +
                  static_cast<unsigned>(static_cast<bool>(consume_b)) == 1 &&
                  concurrent.store.GetSnapshot().active == 0,
              "simultaneous bearer reservation has exactly one winner");
        const std::shared_ptr<Claim> shared_claim = consume_a
            ? consume_a.authorization : consume_b.authorization;
        ...
        std::thread claimant_a([&] {
            claim_start.wait();
            if (shared_claim->ClaimForTicket(
                    concurrent_issue.authorization->binding, 29))
        ...
        auto expiring = fixture.store.Issue(
            TemplateBinding(17, 0x53), 23,
            std::chrono::milliseconds(5));
        Check(expiring && expiring.authorization,
              "short-lived authorization issued");
        const std::string expiring_token = expiring.authorization->token;
        auto expiring_claim = fixture.store.Consume(
            expiring_token, expiring.authorization->binding, 23);
        ...
        Check(!expiring_claim.authorization->ClaimForTicket(
                  expiring.authorization->binding, 23) &&
                  !expiring_claim.authorization->IsClaimedAndLive(),
              "claim fails at the exact absolute expiry boundary");

        auto clock_failure = fixture.store.Issue(
            TemplateBinding(17, 0x54), 23,
            std::chrono::milliseconds(100));
        Check(clock_failure && clock_failure.authorization,
              "clock-failure claim issued while clock is healthy");
        auto clock_claim = fixture.store.Consume(
            clock_failure.authorization->token,
        ...
        Check(!clock_claim.authorization->ClaimForTicket(
                  clock_failure.authorization->binding, 23),
              "injected clock exception fails claim closed");
        fixture.clock_throws = false;

        Fixture bounded(Store::Limits{
            std::chrono::milliseconds(1000), 2, 4});
        auto first = bounded.store.Issue(
            TemplateBinding(31, 0x61), 41,
            std::chrono::milliseconds(1000));
        auto second = bounded.store.Issue(
            TemplateBinding(31, 0x62), 41,
            std::chrono::milliseconds(1000));
        auto over_cap = bounded.store.Issue(
            TemplateBinding(31, 0x63), 41,
            std::chrono::milliseconds(1000));
        Check(first && second && !over_cap &&
                  over_cap.error == Store::Error::Capacity &&
                  bounded.store.GetSnapshot().active == 2,
              "active template authorization cardinality is bounded");
        auto released = bounded.store.Consume(
            first.authorization->token,
            first.authorization->binding, 41);
        auto after_release = bounded.store.Issue(
            TemplateBinding(31, 0x63), 41,
            std::chrono::milliseconds(1000));
        Check(released && after_release &&
                  bounded.store.GetSnapshot().active == 2,
              "capacity recovers after exact one-use reservation");
        bounded.store.CancelAll();
        Check(bounded.store.GetSnapshot().active == 0 &&
                  !bounded.store.Consume(
                      first.authorization->token,
                      first.authorization->binding, 41),
              "shutdown cancellation retires every pending authorization");

        Fixture generations;
        auto old_generation = generations.store.Issue(
            TemplateBinding(51, 0x71), 61,
            std::chrono::milliseconds(1000));
        auto new_generation = generations.store.Issue(
            TemplateBinding(52, 0x72), 62,
            std::chrono::milliseconds(1000));
        Check(old_generation && new_generation &&
                  generations.store.GetSnapshot().active == 1 &&
                  !generations.store.Consume(
                      old_generation.authorization->token,
                      old_generation.authorization->binding, 61),
              "new authority epoch retires stale template records before cap");

        Store unwired;
        auto no_mint = unwired.Issue(
            TemplateBinding(), 23, std::chrono::milliseconds(10));
        Check(!no_mint &&
                  no_mint.error == Store::Error::TokenMintUnavailable,
              "missing production token mint fails closed");
        Store mint_failure(
            Store::Limits{},
            [](Store::TokenBytes&) { return false; });
        auto failed_mint = mint_failure.Issue(
            TemplateBinding(), 23, std::chrono::milliseconds(10));
        Check(!failed_mint &&
                  failed_mint.error == Store::Error::TokenMintFailed,
              "failed production token mint fails closed");
        Store::TokenBytes collision_token{};
        collision_token.fill(0x7c);
        Store collisions(
            Store::Limits{},
            [collision_token](Store::TokenBytes& token) {
                token = collision_token;
                return true;
            });
        auto collision_first = collisions.Issue(
            TemplateBinding(71, 0x73), 81,
            std::chrono::milliseconds(1000));
        auto collision_second = collisions.Issue(
            TemplateBinding(71, 0x74), 81,
            std::chrono::milliseconds(1000));
        ...
        Check(!collisions.Consume(
                  std::string(63, 'a'),
                  collision_first.authorization->binding, 81) &&
                  !collisions.Consume(
                      std::string(64, 'A'),
                      collision_first.authorization->binding, 81) &&
                  collisions.GetSnapshot().active == 1,
        ...
        Store::TimePoint reentrant_now =
            Store::TimePoint{} + std::chrono::seconds(200);
        Store reentrant(
            Store::Limits{},
        ...
        auto reentrant_issue = reentrant.Issue(
            TemplateBinding(81, 0x79), 91,
            std::chrono::milliseconds(100));
        Check(reentrant_issue && reentered_clock &&
                  reentrant.GetSnapshot().active == 1,
              "injected clock may re-enter store without mutex deadlock");

        auto mint_copy_state = std::make_shared<CopyReentryState>();
        Store mint_copy_reentrant(
            Store::Limits{}, ReentrantCopyMint{mint_copy_state});
        mint_copy_state->store = &mint_copy_reentrant;
        mint_copy_state->armed = true;
        auto mint_copy_issue = mint_copy_reentrant.Issue(
            TemplateBinding(83, 0x7b), 93,
            std::chrono::milliseconds(100));
        Check(mint_copy_issue &&
                  mint_copy_state->reentries.load(
        ...
        Store clock_copy_reentrant(
            ...
            ReentrantCopyClock{
                clock_copy_state,
                Store::TimePoint{} + std::chrono::seconds(300)});
        clock_copy_state->store = &clock_copy_reentrant;
        clock_copy_state->armed = true;
        auto clock_copy_issue = clock_copy_reentrant.Issue(
            TemplateBinding(84, 0x7c), 94,
            std::chrono::milliseconds(100));
        ...
        auto clock_copy_claim = clock_copy_reentrant.Consume(
            clock_copy_issue.authorization->token,
            clock_copy_issue.authorization->binding, 94);
        Check(clock_copy_claim &&
                  clock_copy_state->reentries.load(
                      std::memory_order_acquire) > 0,
        ...
        auto issue_oom = allocation_safe.store.Issue(
            allocation_binding, 92, std::chrono::milliseconds(100));
        ...
        auto allocation_issue = allocation_safe.store.Issue(
            allocation_binding, 92, std::chrono::milliseconds(100));
        ...
        auto consume_oom = allocation_safe.store.Consume(
            allocation_issue.authorization->token,
        ...
        Check(static_cast<bool>(allocation_safe.store.Consume(
                  allocation_issue.authorization->token,
                  allocation_issue.authorization->binding, 92)),
              "authorization remains exactly consumable after allocation recovery");

        Fixture expiry_capacity(Store::Limits{
            std::chrono::milliseconds(5), 1, 2});
        auto capacity_expiring = expiry_capacity.store.Issue(
            TemplateBinding(91, 0x75), 101,
            std::chrono::milliseconds(5));
        Check(capacity_expiring &&
                  !expiry_capacity.store.Issue(
                      TemplateBinding(91, 0x76), 101,
                      std::chrono::milliseconds(5)),
              "full active set refuses N plus one without eviction");
        expiry_capacity.now += std::chrono::milliseconds(5);
        auto capacity_recovered = expiry_capacity.store.Issue(
            TemplateBinding(91, 0x76), 101,
            std::chrono::milliseconds(5));
        Check(capacity_recovered &&
                  expiry_capacity.store.GetSnapshot().active == 1,
              "capacity recovers exactly at authorization expiry");

        std::cout
            << "PASS daybreak_block_template_authorization_tests checks="
            << checks
