...
def public_custody(core, root):
    ...
    for i in range(5):
        ...
        for secret, target in ((False, public), (True, private)):
            ...
            (row,) = [
                row
                for row in rows
                if row["active"] and not row["internal"] and row["desc"].startswith("tr(")
            ]
    ...
    assert core.rpc(
        "importdescriptors",
        [[{"desc": descriptor, "timestamp": "now", "range": [0, 999]}]],
    ...
    for i in range(5):
        ...
        assert core.rpc(
            "importdescriptors",
            [[{"desc": member + "#" + checksum, "timestamp": "now", "range": [0, 999]}]],
            wallet="independent-fixture-" + str(i),
        )[0]["success"]
    private.clear()
    document = {
        "version": 1,
...
class Services:
    ...
    def reconnect(cls, source, root, keygen, core, node):
        ...
        code_changes = {}
        for directory, cfg, filename in (
            (self.issuer, icfg, "signer-config.json"),
            (self.witness, self.wcfg, "watchtowerd.conf.json"),
        ):
            assert (directory / "rtp1-migration.json").exists()
            assert (
                hashlib.sha256((directory / "veld-keygen").read_bytes()).digest()
                == hashlib.sha256(keygen.read_bytes()).digest()
            )
            if "keygen" in cfg:
            ...
            cfg["btc"]["cli_base"] = [
                str(core.binary.parent / "bitcoin-cli"),
                "-regtest",
                "-datadir=" + str(core.directory),
                "-rpcport=" + str(core.port),
            ]
            ...
            for path in (source / "swap").glob("*.py"):
                ...
                code_changes[path.name] = {
                    "before": digest,
                    "after": hashlib.sha256(new).hexdigest(),
                }
        (root / "service-code-continuation.json").write_text(canonical(code_changes))
        self.command = [
            str(root / "operator-python"),
            str(self.issuer / "veld_signerd.py"),
            "--capability",
            "mint",
        ]
        self.sequence = (
            int(json.loads((self.issuer / "signer-heartbeat.json").read_text())["seq"])
            if (self.issuer / "signer-heartbeat.json").exists()
            else 0
        )
        return self
    ...
    def __init__(self, source, root, keygen, core, node, binding):
        ...
        chain = {
            ...
            for key in (
                "profile_id",
                "consensus_build_profile",
                "disposable",
                "external_value",
                "fixed_difficulty_regtest",
            )
        }
        chain.update(
            genesis_hash=node.rpc("getcompiledgenesis"),
            launch_block_hash=node.rpc("getblockhash", [1]),
        )
        ...
        self.config = {
            ...
            "issuer": self.address,
            "witness_id": "disposable-independent-witness",
            "custody_descriptor_sha256": binding["descriptor_sha256"],
            "custody_manifest_sha256": binding["manifest_sha256"],
            "bitcoin_genesis": core.rpc("getblockhash", [0]),
            "bitcoin_network": "regtest",
            "custody_script_hex": binding["script"],
        ...
        rpc = {
            "url": "http://127.0.0.1:" + str(node.rpc_port),
            "token_file": str(node.directory / "test-rpc-token"),
            "expected_chain": chain,
        }
        common = {
            "veld_rpc": rpc,
        ...
        self.wcfg = dict(
            common,
            production=True,
            state_dir=str(self.witness),
            keygen=str(keygen),
            rtp1_witness_public_key_file=str(self.witness / "watchtower-beat-pubkey.hex"),
            signer={
                "beat_keyfile": str(self.witness / "witness.key"),
                "beat_passfile": str(self.witness / "passphrase.txt"),
            },
        )
        ...
        icfg = dict(
            common,
            authority_state_activation_marker=str(self.issuer / "signer-authority-state.json"),
            mint_authority={
                "topology": "single-active-shared-witness",
                "signer_id": "disposable-issuer",
                "active_signer_id": "disposable-issuer",
        ...
        (self.witness / "mint-reservations.json").write_text(
            canonical(witness_module._empty_ledger())
        )
        (self.witness / "mint-reservation-tombstones.jsonl").write_text(
            witness_module._empty_terminal_log_text()
        )
        allocation = {
            "version": 5,
            "initial_descriptor_index": 1000,
            "capacity_policy_sha256": "ab" * 32,
            "capacity_policy_sequence": 0,
            "public_descriptor_range_end": 999,
    ...
    def heartbeat(self, reserve_sats):
        ...
        cfg["signer"] = dict(
            cfg["signer"],
            ssh_target="disposable-local-receiver",
            receiver_command=[
                str(self.root / "operator-python"),
                str(self.issuer / "veld_wt_recv.py"),
            ],
        )
        path = self.witness / "native-watchtower.json"
        path.write_text(canonical(cfg))
        output = self.run(
            [self.root / "operator-python", self.witness / "veld_watchtowerd.py", path, "--once"]
        )
        assert "SOLVENT beat" in output, (
            "actual watchtower did not deliver a verified solvency heartbeat: "
            + output[-600:]
            + self.last_command_stderr[-1600:]
        )
        ...
        assert (
            json.loads((self.witness / "watchtower-current-beat.json").read_text())["seq"]
            == beat["seq"]
        )
    ...
    def confirmed(self, request):
        ...
        result = runtime.confirmation(
            self.config,
            SimpleNamespace(call=self.node.rpc),
            SimpleNamespace(call=lambda method, *args: self.core.rpc(method, args)),
            row,
        )
