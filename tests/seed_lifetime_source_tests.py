...
def main() -> int:
    if len(sys.argv) != 2:
        raise SystemExit("usage: seed_lifetime_source_tests.py SRC")
    source = Path(sys.argv[1]).read_text(encoding="utf-8")

    require(
        "void Clear() noexcept" in source,
        "sensitive wrappers must expose explicit no-throw clearing",
    )
    require("~SensitiveString() { Clear(); }" in source, "sensitive strings must remain RAII-wiped")
    require("~LockedSeed() {\n        Clear();" in source, "locked seeds must remain RAII-wiped")
    ...
    require(
        "LockedSeed seed;" in import_flow,
        "imported decoded seed must live only in locked RAII storage",
    )
    require(
        "std::array<uint8_t, 32> seed{}" not in import_flow,
    ...
    require(
        "plaintext.value.reserve(" in command, "plaintext must reserve before appending secret hex"
    )

    print(f"PASS seed_lifetime_source_tests checks={checks}")
    return 0
