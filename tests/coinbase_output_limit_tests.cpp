...
int main() {
    ...
    try {
        ...
        for (uint64_t height : {3359ULL, 3360ULL, 3361ULL, 3400ULL}) {
            for (uint64_t supply :
                 {STAKING_UNLOCK_SUPPLY, MAX_SUPPLY_UNITS - 1, MAX_SUPPLY_UNITS}) {
                chain.SetTotalSupplyForTesting(supply);
                for (size_t count :
                     {0U, 1U, 195U, 196U, 197U, 199U, 200U, 201U, 234U, 235U, 236U, 240U}) {
                    ...
                    block.transactions.push_back(Blockchain::BuildCanonicalCoinbase(
                        height, supply, 0, owner.GetP2PKHScript(), metadata));
                    ...
                    check(direct == (canonical && (height >= 3360 || old_ingress)),
                          "direct output envelope");
                    check(replay == canonical, "historical or upgraded replay envelope");
                    if (height >= 3360)
                        check(direct == replay, "activated paths disagree");
                    if (!metadata.empty()) {
                        auto bad = block;
                        ++bad.transactions[0].outputs.back().value;
                        check(!chain.ValidateCoinbasePolicy(bad),
                              "value-bearing finality output accepted");
                        check(!chain.ValidateCoinbasePolicy(
                                  bad, Blockchain::CoinbaseAdmissionPath::Replay),
                              "replay accepted value-bearing metadata");
                    }
                }
            }
        }
