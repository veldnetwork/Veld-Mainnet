...
int main() {
    ...
    for (const bool queued : {false, true}) {
        for (const bool already_waiting : {false, true}) {
            ...
            const P2PMessage message(MAINNET_MAGIC, MessageType::BLOCK, block.Serialize());
            ...
            bool expected_boundary;
            {
                ...
                const bool returned = dispatch.wait_for(300ms) == std::future_status::ready;
            ...
            if (!expected_boundary || server.TestViolationScore("198.51.100.81") != 0 ||
                server.TestIsBlockRejected(block.GetHash()) ||
                server.TestPendingBlockIngestCount() != 0 || server.ClearOrphanPool() != 0) {
    ...
    std::cout
        << "PASS: late and waiting peer work cancels in direct and queued modes without a validity decision\n";
}
