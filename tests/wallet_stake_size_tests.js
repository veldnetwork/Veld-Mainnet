...
async function runFixture(context) {
    ...
    function transaction(count) {
        const varint =
            count < 253
                ...
                : 'fd' +
                  (count & 255).toString(16).padStart(2, '0') +
                  (count >> 8).toString(16).padStart(2, '0');
    ...
    for (const count of [1, 21, 150, 197, 198, 319]) {
        const hex = transaction(count),
            inputs = Array.from({ length: count }, () => ({}));
        const expected = hex.length / 2 + count * 5271;
        if (expected <= 1048576) {
            check(
                context._veldAssertPreparedRelaySize(hex, inputs) === expected,
                'signed size mismatch',
            );
            rows.push({ inputs: count, signed_bytes: expected, accepted: true });
        } else {
            await rejects(
                () => context._veldAssertPreparedRelaySize(hex, inputs),
                /Combine my outputs/,
            );
    ...
    let signed = 0,
        ...
        hashed = 0,
        broadcast = 0;
    const seed = '23'.repeat(32),
        txid = '34'.repeat(32);
    let prepared = {
        unsigned_tx_hex: transaction(319),
        inputs: Array.from({ length: 319 }, () => ({})),
    };
    ...
    const stake = () =>
        context.signAndBroadcast(
            'preparestake',
            ['disposable-fixture', '500', '1'],
            seed,
            null,
            null,
            null,
    ...
    check(
        signed === 0 && authenticated === 0 && hashed === 0 && broadcast === 0,
    ...
    check(
        (await stake()).txid === txid && signed === 1 && broadcast === 1,
    ...
    for (const bad of [null, {}, '', '0', 'zz', 'ab'.repeat(1048577)]) {
        ...
        check(
            before === [hashed, broadcast].join(','),
            'invalid bytes reached hashing or broadcast',
        );
    }
    check(
        (await context._veldBroadcastExactSigned('AB'.repeat(1048576), seed, 7)) === txid,
        'valid maximum-size hex rejected',
    );
    await rejects(
        () => context._veldBroadcastExactSigned('ab', '45'.repeat(32), 7),
        /Wallet locked or changed/,
    );
    context.rpc = async () => '56'.repeat(32);
    await rejects(
        () => context._veldBroadcastExactSigned('ab', seed, 7),
        /different from the signed bytes/,
    );
