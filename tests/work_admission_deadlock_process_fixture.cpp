...
#error "work-admission deadlock process fixture requires VELD_TEST_HOOKS"
#endif
#if defined(VELD_PUBLIC_RELEASE) || defined(VELD_PUBLIC_MAINNET)
#error "work-admission deadlock fixture must never compile in a public profile"
#endif
...
void InstallGenesis(VeldNode& node) {
    ...
    const auto result = node.GetChainMut().AddBlockDirect(
        genesis, true, true, false,
        mining::PowAdmissionContext::Internal());
    Check(result.IsAccepted(), "trusted fixture genesis installed");
    Check(node.GetChain().Height() == 0,
          "fresh production-profile node begins at height zero");
...
ExactTipPeers InstallExactTipEvidence(VeldNode& node, uint16_t base_port) {
    ...
    ExactTipPeers peers{
        MakeConnection("10.81.0.1", base_port),
        MakeConnection("10.81.0.2", static_cast<uint16_t>(base_port + 1))};
    Check(peers.first && peers.second,
          "two outbound peer observations allocated");
...
EqualityView Observe(const VeldNode& node) {
    ...
    Check(tips.size() == 1 && tips.front().status == "active" &&
              tips.front().height == out.height &&
              tips.front().hash == out.tip,
          "node exposes exactly one active canonical tip");
    out.work = tips.front().cumulative_work;
    return out;
}

void PropagateToObserver(VeldNode& observer, const Block& block,
                         const char* source) {
    ...
    const auto queued = server.TestEnqueueBlockIngest(
        block, block.SerializedSize(), source);
    ...
    Check(server.TestBlockIngestRelayCount() == 1 &&
              server.TestBlockIngestPenaltyCount() == 0,
    ...
    try {
        if (argc != 2)
            throw std::invalid_argument(
                "usage: work_admission_deadlock_process_fixture <artifact-root>");
        const std::filesystem::path root(argv[1]);
        std::filesystem::create_directories(root);
        compat::InitNetwork();

        const auto config = MainnetConfig();
        Check(config.kind == NetworkKind::Mainnet &&
                  config.name == "Veld Mainnet" &&
                  config.magic == MAINNET_MAGIC,
              "all objects use the production network configuration");

        auto miner_node = std::make_unique<VeldNode>(
            config, (root / "miner").string());
        auto observer_a = std::make_unique<VeldNode>(
            config, (root / "observer-a").string());
        auto observer_b = std::make_unique<VeldNode>(
            config, (root / "observer-b").string());
        ...
        Check(subject && subject->height == 1 &&
                  subject->parent_height == 0 &&
                  subject->parent_hash ==
                      miner_node->GetChain().TipCopy().GetHash(),
              "production local-work subject is bound to height one");
        const auto decision = miner_node->TestEvaluateWorkAdmission(
        ...
        auto solved = MineOnly(
            miner_node->GetChainMut(), miner_node->GetMempoolMut(), miner_key);
        ...
        std::cout << "WORK_ADMISSION_DEADLOCK_ARMED verified_peers=2 height=0"
                  << std::endl;
        const auto admission = miner_node->GetChainMut().AddBlockDirect(
            solved.block, false, false, false, context);
        Check(admission.IsAccepted(),
              "exact production AddBlockDirect local-work path returns");
        Check(miner_node->GetChain().Height() == 1 &&
                  miner_node->GetChain().TipCopy().GetHash() ==
                      solved.block.GetHash(),
              "local solved block commits height zero to one");
        Check(context.local_work_handoff &&
        ...
        const auto coordinator =
            miner_node->TestWorkAdmissionCoordinatorSnapshot();
        Check(coordinator.active_leases == 0,
              "local-work ticket releases after propagation");
        ...
        const auto equal = [&](const EqualityView& other) {
            return other.height == miner_view.height &&
                   other.tip == miner_view.tip &&
                   other.work == miner_view.work &&
                   other.supply == miner_view.supply &&
        ...
        Check(miner_node->GetMempool().IsEmpty() &&
                  observer_a->GetMempool().IsEmpty() &&
        ...
        std::cout << "WORK_ADMISSION_DEADLOCK_JSON {\"status\":\"pass\","
                  ...
                  << "\"state_digest\":\""
                  << HashToHex(miner_view.state) << "\","
                  << "\"observer_accepts\":2,\"observer_relays\":2,"
                  << "\"shutdown\":true,\"checks\":" << checks << "}"
                  << std::endl;
        return 0;
