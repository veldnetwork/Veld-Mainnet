...
int main() {
    ...
    size_t rows = 0;
    for (uint64_t h : {1ULL, 99ULL, 100ULL, 479ULL, 480ULL, 2879ULL, 2880ULL, 2881ULL, 3299ULL,
                       3300ULL, 3359ULL, 3360ULL, 3361ULL, 3399ULL, 3400ULL, 3839ULL, 3840ULL}) {
        for (uint64_t supply :
             {STAKING_UNLOCK_SUPPLY - 1, STAKING_UNLOCK_SUPPLY, MAX_SUPPLY_UNITS - 3 * r,
              MAX_SUPPLY_UNITS - 17, MAX_SUPPLY_UNITS - 1, MAX_SUPPLY_UNITS}) {
            for (uint64_t fee :
                 {uint64_t(0), uint64_t(1), uint64_t(9), uint64_t(10), MIN_TX_FEE, 5 * r, 6 * r - 1,
                  6 * r, 6 * r + 1, 30 * r - 1, 30 * r, 30 * r + 1}) {
                ...
                const uint64_t reward =
                    std::min(Blockchain::ExpectedBlockSubsidy(h), MAX_SUPPLY_UNITS - supply);
                uint64_t miner = 0, pool = 0, vault = 0, endorse = 0;
                if (!reward) {
                    vault = fee * 4 / 10;
                    endorse = fee / 10;
                    miner = fee - vault - endorse;
                } else if (h % 100 == 0)
                    vault = reward + fee;
                else if (supply < STAKING_UNLOCK_SUPPLY) {
                    miner = reward / 2;
                    vault = reward - miner + fee;
                } else {
                    pool = reward / 5;
                    vault = reward / 5 + fee;
                    endorse = reward / 10;
                    miner = reward - pool - (vault - fee) - endorse;
                }
                Block original;
                original.height = h;
                original.transactions.push_back(Transaction::CreateProportionalCoinbase(
                    {{owner.GetP2PKHScript(), miner},
                     {AddressToScript(PoolAddressAtHeight(h)), pool},
                     {AddressToScript(VaultAddressAtHeight(h)), vault},
                     {AddressToScript(EndorsementPoolAddressAtHeight(h)), endorse}},
                    "compatibility"));
                if (fee) {
                    Transaction tx;
                    TxInput in;
                ...
                for (unsigned variant = 0; variant < 5; ++variant) {
                    ...
                    if (variant == 2)
                        outputs.clear();
                    if (variant == 3)
                        outputs.emplace_back(0, owner.GetP2PKHScript());
                    if (variant == 4)
                        outputs[0].value = UINT64_MAX;
                    ...
                    const bool replay =
                        chain.ValidateCoinbasePolicy(b, Blockchain::CoinbaseAdmissionPath::Replay);
#endif
                    std::cout << h << ',' << supply << ',' << fee << ',' << variant << ',' << shape
                              << ',' << exact << ',' << direct << ',' << replay << '\n';
                    ++rows;
                }
            }
