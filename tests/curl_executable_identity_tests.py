...
def main() -> None:
    ...
    check(
        "GetWindowsDirectoryA" in resolver
        and "GetSystemDirectoryA" in resolver
        and "GetFinalPathNameByHandleA" in resolver,
        "Windows curl identity is not bound to the canonical system path",
    )
    check(
        "FILE_ATTRIBUTE_REPARSE_POINT" in resolver and "FILE_FLAG_OPEN_REPARSE_POINT" in resolver,
        "Windows curl identity does not reject reparse substitution",
    )
    check(
        '"/usr/bin/curl"' in resolver
        and '"/bin/curl"' in resolver
        and '"/usr/local/bin/curl"' in resolver
        and "::realpath" in resolver,
        "POSIX curl resolver lacks a fixed canonical system allowlist",
    )
    check(
        "status.st_uid != 0" in resolver
        and "status.st_size <= 0" in resolver
        and "S_IWGRP | S_IWOTH" in resolver
        and "::access(canonical.c_str(), X_OK)" in resolver,
        "POSIX curl resolver lacks root-owner/write/execute checks",
    )
    check(
        "PATH" not in resolver and "getenv" not in resolver,
    ...
    oracle_resolve = node.index("compat::TrustedSystemCurlExecutable()", oracle_start)
    ...
    checkpoint_resolve = node.index("compat::TrustedSystemCurlExecutable()", checkpoint_start)
    checkpoint_refuse = node.index("if (curl_executable.empty())", checkpoint_resolve)
    checkpoint_launch = node.index("compat::RunProcessToBoundedFile(", checkpoint_refuse)
    check(
        oracle_resolve < oracle_refuse < oracle_launch
        and checkpoint_resolve < checkpoint_refuse < checkpoint_launch,
        "a node curl sink can launch before trusted-resolution refusal",
    )
    check(
        '{curl_executable, "--disable", "-fsS"' in node
        and '{curl_executable, "--disable", "--fail", "--silent"' in node
        and '{"curl"' not in node,
        "node retains an unqualified curl child or lost config disabling",
    )

    with tempfile.TemporaryDirectory(prefix="veld-test-curl-identity-build-") as tmp:
        binary = Path(tmp) / (
            "curl-identity-tests.exe" if os.name == "nt" else "curl-identity-tests"
        )
        command = [
            compiler(),
            "-std=c++20",
        ...
        check(build.returncode == 0, f"curl identity fixture compile failed: {build.stderr}")
        run = subprocess.run([str(binary)], cwd=ROOT, text=True, capture_output=True, timeout=30)
        check(
            run.returncode == 0,
            f"curl identity fixture failed: stdout={run.stdout!r} stderr={run.stderr!r}",
        )
        check(
            "PASS curl_executable_identity_tests" in run.stdout and "trusted=" in run.stdout,
        ...
        if os.name != "nt" and hasattr(os, "geteuid") and os.geteuid() == 0:
            ...
            check(chroot is not None, "root curl-absence fixture has no chroot executable")
            empty_root = Path(tmp) / "empty-root"
            empty_root.mkdir()
            jailed_binary = empty_root / "identity-tests"
            shutil.copy2(binary, jailed_binary)
            unavailable = subprocess.run(
                [chroot, str(empty_root), "/identity-tests", "--expect-unavailable"],
                cwd=ROOT,
                text=True,
                capture_output=True,
                timeout=30,
            ...
            check(
                ...
                and "trusted-system-curl-unavailable-fail-closed" in unavailable.stdout,
