...
Hash256 Hash(unsigned char value) {
    Hash256 h{};
    h.fill(value);
    return h;
}
...
btcanchor::floor_store::Record Floor(uint64_t target, unsigned char id) {
    ...
    auto& e = p.entry;
    e.veld_block_hash = Hash(id);
    e.carrying_veld_height = target + 1;
    e.carrying_veld_hash = Hash(id + 1);
    e.btc_block_hash = Hash(id + 2);
    e.btc_txid = Hash(id + 3);
    auto& f = p.authorization_record;
    f.target.height =
        ((target + 1) / finality::qc::CHECKPOINT_INTERVAL + 1) * finality::qc::CHECKPOINT_INTERVAL;
    f.target.hash = Hash(id + 4);
    f.epoch_id = finality::qc::EpochOf(f.target.height);
    f.round = finality::qc::CheckpointRound(f.target.height);
    f.phase = finality::qc::Phase::PRECOMMIT;
    f.set_root = Hash(id + 5);
    f.cert_commit = Hash(id + 6);
    f.carrier.height = f.target.height + 1;
    f.carrier.hash = Hash(id + 7);
    ...
    sr::Require(record.has_value(), error);
    return *record;
}
...
void PutIndex(const fs::path& root, const std::string& key, const std::string& value) {
    ...
    Check(store.Put(key, value), "durable fixture key");
}
void PutDbFloor(const fs::path& root, const btcanchor::floor_store::Record& record) {
    const auto wire = btcanchor::floor_store::Encode(record);
    Check(bool(wire), "floor encoding");
...
void PutExternalFloor(const fs::path& root, const btcanchor::floor_store::Record& record) {
    std::string error;
    Check(channel::secure_file::EnsurePrivateDirectory((root / "security").string(), &error),
          "private security dir");
    const auto wire = btcanchor::floor_store::Encode(record);
    Check(bool(wire), "floor encoding");
...
int main(int argc, char** argv) {
    try {
        ...
        fs::create_directories(base);
        {
            ...
            Check(channel::secure_file::EnsurePrivateDirectory(fresh.string(), &directory_error),
                  "private bootstrap fixture");
            Check(channel::secure_file::EnsurePrivateDirectory((fresh / "db").string(),
                                                               &directory_error),
                  "private empty storage directory");
            ...
            for (const auto* name :
                 {"db/blocks", "db/utxo", "db/index", "db/.snapshot-consensus-replay-required",
                  ".snapshot-handoff", ".background-chainstate-required", "background-ibd",
                  "background-chainstate", sr::REQUEST, sr::REVOKED, sr::JOURNAL, sr::IMPORT}) {
                const auto path = fresh / name;
                sr::Write(path, "retained local state\n");
                Check(sr::HasLocalChainState(fresh),
                      "existing state blocks another snapshot import");
                Check(sr::Read(path, 64) ==
        ...
        for (unsigned sources = 0; sources < 5; ++sources) {
            ...
            if (sources == 1 || sources >= 3)
                PutExternalFloor(root, lower);
            if (sources == 2 || sources == 3)
                PutDbFloor(root, lower);
            if (sources == 4)
                PutDbFloor(root, higher);
            ...
            Check(sr::RejectionExitCode(root) == 75,
                  "durable rejection requests automatic restart");
            Check(sr::Prepare(root, magic, genesis), "recovery starts");
            const auto t = sr::Load(root);
            Check(t && t->phase == "ibd", "fresh IBD journal");
            const auto rejected = root / (".snapshot-rejected-" + t->id);
            Check(fs::is_directory(rejected / "db") && !fs::exists(root / "db"),
                  "rejected database quarantined");
            const auto floors = sr::ReadFloors(root, magic, genesis);
            Check(floors.size() == (sources == 0   ? 0
                                    : sources == 4 ? 2
                                                   : 1),
                  "all distinct floor copies survive");
            Check(sr::Read(root / "miner.key", 128).has_value() &&
                      sr::Read(root / "network.identity", 128).has_value(),
            ...
            {
                db::LevelDBStore fresh((root / "db/index").string(), false);
                Check(fresh.Get("fixture:new-progress") == "17", "fresh progress retained");
            }
            Check(sr::Load(root)->id == t->id, "one quarantine generation");
            Refuses([&] { sr::Complete(root, "full-ibd.receipt"); },
                    "missing receipt cannot clear recovery");
            Check(sr::Pending(root), "failed completion preserves barrier");
        }
        for (const auto* key :
             {db::VeldDB::DURABLE_PUBLICATION_PENDING_KEY, db::VeldDB::REORG_UTXO_PENDING_KEY,
              "reorg:utxo:rebuilding", "pending:commit"}) {
            const auto root = New(base, "repair-" + std::to_string(checks));
            PutIndex(root, key, "retained local obligation");
            sr::Request(root, "independent-tip-mismatch");
            Refuses([&] { sr::Prepare(root, magic, genesis); }, "local repair blocks replacement");
            Check(fs::is_directory(root / "db") && !sr::Exists(root / sr::JOURNAL),
                  "repair source remains in place");
            db::LevelDBStore store((root / "db/index").string(), false);
            Check(store.Get(key) == "retained local obligation", "repair identity unchanged");
        }
        {
            const auto root = New(base, "conflicting-floors");
            PutExternalFloor(root, lower);
            auto other = lower;
            other.checkpoint.authorization_record.cert_commit = Hash(88);
            other.checkpoint.entry.authorization_finality_digest =
                finality::qc::RecordDigest(other.checkpoint.authorization_record);
            other.floor_digest = btcanchor::floor_store::ComputeFloorDigest(other);
            PutDbFloor(root, other);
            sr::Request(root, "independent-tip-mismatch");
            Refuses([&] { sr::Prepare(root, magic, genesis); },
                    "same target metadata conflict refuses replacement");
        ...
        for (const auto* body :
             {"", "schema=1\nreason=bad reason\n", "schema=1\nreason=ok\nextra=1\n"}) {
            const auto root = New(base, "malformed-" + std::to_string(checks));
            sr::Write(root / sr::REQUEST, body);
            ...
            Check(fs::is_directory(root / "db"),
                  "malformed request does not mutate chain namespace");
        }
        {
            const auto root = New(base, "uncertain-floor");
            PutExternalFloor(root, lower);
            sr::Write(root / "security/anchor-floor.uncertain", "VLF1-UNCERTAIN\n");
            sr::Request(root, "independent-tip-mismatch");
            Refuses([&] { sr::Prepare(root, magic, genesis); },
                    "uncertain floor cannot be erased by recovery");
        }
        {
            const auto root = New(base, "healthy");
            PutIndex(root, "fixture:healthy", "unchanged");
            Check(!sr::Prepare(root, magic, genesis), "healthy startup does not enter recovery");
            Check(sr::RejectionExitCode(root) == 76,
                  "absent rejection cannot authorize recovery restart");
            db::LevelDBStore store((root / "db/index").string(), false);
