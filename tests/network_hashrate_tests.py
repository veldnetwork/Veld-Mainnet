...
class HashrateTests(unittest.TestCase):
    def test_exact_variable_difficulty_and_elapsed_time(self):
        blocks=[dict(time=900,bits=0x1e00ffff),dict(time=360,bits=0x1f00ffff),dict(time=0,bits=0x1f00ffff)]
        work=((1<<256)//((0xffff<<(8*27))+1))+((1<<256)//((0xffff<<(8*28))+1))
        rate,count,seconds=stats.observed_hashrate(blocks)
        self.assertEqual((count,seconds),(2,900));self.assertAlmostEqual(rate,float(Fraction(work,900)))
        blocks[0]['time']=1800;self.assertAlmostEqual(stats.observed_hashrate(blocks)[0],rate/2)
    ...
    def test_native_estimate_is_preserved_without_body_reads(self):
        value=dict(height=8570,best_block_hash='a'*64,hashrate_method='canonical_work_over_observed_time',
                   hashrate_sample_height=8570,hashrate=1234.5,hashrate_window_blocks=144,hashrate_window_seconds=25920)
        paths=[]
        def read(path):paths.append(path);return value
        self.assertEqual(stats.StatsCollector(read).collect(),value);self.assertEqual(paths,['/api/stats'])
        for field,bad in [('hashrate',math.inf),('hashrate_window_blocks',145),('hashrate_sample_height',8569)]:
            broken=dict(value);broken[field]=bad
            with self.assertRaises(ValueError):stats.StatsCollector(lambda _:broken).collect()
