...
vm.runInContext(
    fs.readFileSync(path.join(__dirname, '../website/explorer-transactions-v1.js'), 'utf8'),
    context,
);
...
const tx = {
    ...
    vin: [
        ...
        { txid: '3'.repeat(64), vout: 0 },
    ],
    vout: [{ address, value: 56.5058492 }],
};
...
for (const change of [
    ...
    { vout: [{ address: 'another-wallet', value: 1 }] },
    { vout: [{ address, value: 0 }] },
]) {
...
const script = fs.readFileSync(
    path.join(__dirname, '../website/explorer-transactions-v1.js'),
    'utf8',
);
const proxy = fs.readFileSync(
    path.join(__dirname, '../pkg/reverse-proxy/veld-explorer-transaction-data.conf'),
    'utf8',
);
...
(async () => {
    for (const version of [1, 2]) {
        ...
        const history = vm.createContext({
            addressPattern: /^V[1-9A-HJ-NP-Za-km-z]{25,40}$/,
            read: async (url) => {
                if (++calls === 1)
                    return {
                        entries: [{ txid: '3'.repeat(64), block_height: 4406 }],
                        has_more: true,
                        next_cursor: cursor,
                    };
