...
uint64_t ParseU64(const char* text) {
    ...
    if (used != value.size()) throw std::runtime_error("non-canonical integer");
...
int EmitMode(int argc, char** argv) {
    ...
    if (bytes > 64U * 1024U * 1024U ||
        delay_ms > 60U * 1000U || exit_code > 125) {
        return 2;
    }
...
int FetchMode(int argc, char** argv) {
    ...
    if (url.rfind("http://127.0.0.1:", 0) != 0 || maximum == 0 ||
        deadline_ms == 0 || deadline_ms > 60U * 1000U) {
        return 2;
    }
    const uint64_t curl_seconds = deadline_ms / 1000U + 2U;
    const BoundedFileResult result = veld::compat::RunProcessToBoundedFile(
        {"curl", "--fail", "--silent", "--show-error",
         "--connect-timeout", "2", "--max-time", std::to_string(curl_seconds),
         "--max-filesize", std::to_string(maximum), "--location",
         "--max-redirs", "0", "--proto", "=http,https",
         "--proto-redir", "=https", url},
...
int main(int argc, char** argv) try {
    if (argc > 1 && std::string(argv[1]) == "--emit") return EmitMode(argc, argv);
    if (argc > 1 && std::string(argv[1]) == "--fetch") return FetchMode(argc, argv);
    if (argc != 1) return 2;
    ...
    const fs::path root = fs::temp_directory_path() /
                          ("veld-f8-bounded-" + std::to_string(nonce));
    ...
    auto result = veld::compat::RunProcessToBoundedFile(
        {self, "--emit", "8192", "0", "0"}, exact.string(), 8192,
        std::chrono::seconds(5));
    test.Check(result.status == BoundedFileStatus::Success,
               "exact maximum transfer failed");
    ...
    result = veld::compat::RunProcessToBoundedFile(
        {self, "--emit", "8193", "0", "0"}, oversized.string(), 8192,
        std::chrono::seconds(5));
    ...
    result = veld::compat::RunProcessToBoundedFile(
        {self, "--emit", "1", "2000", "0"}, slow.string(), 8192,
        std::chrono::milliseconds(120));
    const auto elapsed = std::chrono::steady_clock::now() - before;
    test.Check(result.status == BoundedFileStatus::DeadlineExceeded,
               "slow stream did not hit parent deadline");
    test.Check(elapsed < std::chrono::seconds(1),
               "deadline did not terminate child promptly");
    test.Check(!fs::exists(slow), "deadline partial file survived");

    const fs::path failed = root / "failed.bin";
    result = veld::compat::RunProcessToBoundedFile(
        {self, "--emit", "1024", "0", "7"}, failed.string(), 8192,
        std::chrono::seconds(5));
    test.Check(result.status == BoundedFileStatus::ChildFailed,
               "child failure was accepted");
    ...
    result = veld::compat::RunProcessToBoundedFile(
        {self, "--emit", "1", "0", "0"}, existing.string(), 8192,
        std::chrono::seconds(5));
    ...
    result = veld::compat::RunProcessToBoundedFile(
        {self, "--emit", "1", "0", "0"}, missing_parent.string(), 8192,
        std::chrono::seconds(5));
    test.Check(result.status == BoundedFileStatus::OpenFailed,
               "unwritable destination was accepted");
    test.Check(!fs::exists(missing_parent), "open failure left a partial file");

    const std::string parser_suffix =
        "\",\"height\":1,\"hash\":\"" + std::string(64, '0') +
        "\",\"sig\":\"" + std::string(6618, '0') + "\"}";
    std::string maximum_document = "{\"padding\":\"";
    test.Check(maximum_document.size() + parser_suffix.size() <
                   veld::kMaxCheckpointDocumentBytes,
               "checkpoint parser fixture exceeds shared cap");
    maximum_document.append(
        veld::kMaxCheckpointDocumentBytes - maximum_document.size() -
            parser_suffix.size(),
        'P');
    ...
    const size_t parser_allocation =
        g_largest_allocation.load(std::memory_order_relaxed);
    test.Check(parsed.size() == 1,
               "checkpoint parser exact-cap fixture was not exercised");
    test.Check(parser_allocation < 64U * 1024U,
               "checkpoint parser copied a complete object or oversized field");

    std::cout << "PASS daybreak_bounded_download_tests checks=" << test.checks
              << "\n";
    return 0;
} catch (const std::exception& e) {
    std::cerr << "FAIL daybreak_bounded_download_tests: " << e.what() << "\n";
    return 1;
}
