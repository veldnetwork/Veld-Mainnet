...
from security_state_migration_network_tests import (
    Node,
    free_port,
    wait_admission,
    wait_equal,
...
def internal(display):
    return bytes.fromhex(display)[::-1].hex()
...
class Bitcoin:
    def __init__(self, binary, directory):
        ...
        for _ in range(100):
            if self.process.poll() is not None:
                raise RuntimeError("Bitcoin Core startup failed; see local log")
            try:
            ...
            raise TimeoutError("Bitcoin Core RPC startup")
    ...
    def rpc(self, method, params=(), wallet=None):
        ...
        headers = {
            ...
            "Authorization": "Basic " + base64.b64encode(self.cookie.strip().encode()).decode(),
        }
        conn.request(
            "POST",
            ("/wallet/" + (wallet or "disposable")) if self.wallet else "/",
            json.dumps({"jsonrpc": "2.0", "id": 1, "method": method, "params": list(params)}),
            headers,
        )
        ...
        if value.get("error"):
            raise RuntimeError(f"Bitcoin RPC {method}: {value['error']}")
        return value["result"]

    def mine(self, count=3):
        return self.rpc(
            "generatetoaddress", [count, self.rpc("getnewaddress", ["mining", "bech32"])]
        )

    def fund(self, sats, legacy=False):
        address = self.rpc(
            "getnewaddress", ["external-fee-or-deposit", "legacy" if legacy else "bech32"]
        )
    ...
    def send(self, inputs, outputs):
        raw = self.rpc(
            ...
            [[{"txid": x["txid"], "vout": x["vout"]} for x in inputs], outputs],
        )
    ...
    def proof(self, transaction):
        ...
        return {
            "txid": internal(transaction["txid"]),
            "rawtx": transaction["raw"],
            "block": internal(transaction["block"]),
            "branch": branch,
            "directions": original_index,
...
def compile_node(directory, script, compiler, checkpoint):
    ...
    if not public.exists():
        ...
        command = [
            ...
            "-O2",
            "-Iinclude",
            "tests/security_first_activation_identity.cpp",
        ...
        addresses = subprocess.check_output(
            [str(helper), str(directory / "disposable-identities")], env=env, text=True
        ).splitlines()
    ...
    header.write_text(
        '#define VELD_FIRST_ACTIVATION_CUSTODY_SPK_HEX "'
        + script
        + '"\n'
        + '#define VELD_FIRST_ACTIVATION_AUTHORITY_ADDRESS "'
        + addresses[0]
        + '"\n'
        + '#define VELD_FIRST_ACTIVATION_BTC_CHECKPOINT_HEIGHT 2016u\n'
        + '#define VELD_FIRST_ACTIVATION_BTC_CHECKPOINT_BITS 0x'
        + checkpoint["bits"]
        + 'u\n'
        + '#define VELD_FIRST_ACTIVATION_BTC_CHECKPOINT_TIME '
        + str(checkpoint["time"])
        + 'u\n'
        + '#define VELD_FIRST_ACTIVATION_BTC_CHECKPOINT_HASH "'
        + internal(checkpoint["hash"])
        + '"\n'
        + '#define VELD_FIRST_ACTIVATION_BTC_CHECKPOINT_PREV10 {'
        + ','.join(str(t) + 'u' for t in checkpoint["prev10"])
        + '}\n'
    )
    ...
    command = [
        ...
        "-O2",
        "-pthread",
        "-DVELD_FIRST_ACTIVATION_NETWORK=1",
        "-DVELD_FIRST_ACTIVATION_MIGRATION_HEIGHT=3360",
        "-DVELD_LIGHT_VERIFY=1",
        "-DVELD_USE_LEVELDB=1",
        "-include",
        str(header),
        "-Iinclude",
        "-Ivendor/pqclean/crypto_sign/ml-dsa-65/clean",
        "-Ivendor/pqclean/common",
        "tests/security_state_migration_node.cpp",
        *[str(x) for x in objects],
        "-lleveldb",
        "-lws2_32",
        "-lbcrypt",
    ...
    if process.returncode:
        raise RuntimeError("first activation build failed; see build.log")
    return binary
...
def relay(a, core):
    ...
    for start in range(best + 1, tip + 1, 100):
        headers = [
            core.rpc("getblockheader", [core.rpc("getblockhash", [h]), False])
            for h in range(start, min(start + 100, tip + 1))
        ]
...
def observe_bitcoin(a, core):
    ...
    payload = (
        b"VELD_ANCHOR:"
        + finalized["height"].to_bytes(8, "little")
        + bytes.fromhex(finalized["hash"])
    )
    ...
    wire = (
        b"ANCH"
        + bytes.fromhex(proof["block"])
        + proof["directions"].to_bytes(4, "little")
        + bytes([len(proof["branch"])])
        + b"".join(bytes.fromhex(x) for x in proof["branch"])
        + bytes.fromhex(anchor["raw"])
    )
    a.control("fa_anchor", proof=wire.hex())
    a.generate(1)
    info = a.rpc("getanchorinfo")
    ...
    info = a.rpc("getanchorinfo")
    assert (
        info["anchor_security_milestone"]
        and info["btc_observed_checkpoint_height"]
        >= core.rpc("getblockheader", [anchor["block"]])["height"]
    ), info
    return info
...
def accounting(a, expected_supply, expected_reserve, expected_open=0):
    peg = a.rpc("getpeginfo")
    coherent = a.rpc("getbtcveldsupply")
    assert peg["reserve_accounting_holds"], peg
    assert (
        peg["supply_sats"] == expected_supply and peg["reserve_value_sats"] == expected_reserve
    ), peg
...
def scenario(a, core, custody, report, binary, directory, nodes):
    ...
    b = Node(binary, directory / "node-b")
    nodes.append(b)
    b.control("connect", port=a.p2p)
    wait_admission(a)
    wait_admission(b)
    a.control("synchronize")
    b.control("synchronize")
    a.generate(3)
    wait_equal(a, b)
    b.stop()
    ...
    for index in range(1, 8):
        a.control("fund", to=index, amount="60")
        a.generate(1)
    for index in range(1, 8):
        a.control("register", wallet=index)
        a.generate(1)
    ...
    opened = core.send(
        [core.fund(2001000)], [{custody["address"]: btc(2000000)}, {"data": auth["payload"]}]
    )
    ...
    for direction, amount in [("v2b", 100000000), ("b2v", 2000)]:
        ...
        assert (
            after["reserve_veld"] * after["reserve_btcveld"]
            >= before["reserve_veld"] * before["reserve_btcveld"]
        )
    ...
    fields = {
        "wallet": 0,
        "operation": "DEPOSIT",
        "reserve_value": 2500000,
        "mint_amount": 500000,
        "pending_txid": internal(pending["txid"]),
        "pending_vout": 0,
        "pending_value": 501000,
    }
    auth = a.control("fa_auth", **fields)
    deposited = core.send(
        [dict(opened, vout=0), dict(pending, vout=0)],
    ...
    destination = core.rpc(
        "getnewaddress", ["redemption-destination", "bech32"], wallet="recipient"
    )
    destination_script = core.rpc("getaddressinfo", [destination], wallet="recipient")[
        "scriptPubKey"
    ]
    ...
    fields = {
        "wallet": 0,
        "operation": "PAYOUT",
        "reserve_value": 2400000,
        "mint_amount": 0,
        "request_id": request_id,
    }
    auth = a.control("fa_auth", **fields)
    paid = core.send(
        [dict(deposited, vout=0), core.fund(1000)],
    ...
    assert fulfilled["status"] == 5 and fulfilled["fulfilled_txid"] == internal(paid["txid"]), (
        fulfilled
    )
    assert round(core.rpc("gettxout", [paid["txid"], 1])["value"] * 100000000) == 100000
    assert (
        round(core.rpc("getbalances", wallet="recipient")["mine"]["trusted"] * 100000000) == 100000
    )
    ...
    final = a.control("status")
    report["final_state"] = final
    a.control("export_history", name="final-history.bin")
    a.stop()
    restarted = Node(binary, a.directory, "-restart")
    nodes.append(restarted)
    assert (
        restarted.ready["digest"] == final["digest"]
        and restarted.ready["height"] == final["height"]
    )
    ...
    fresh = Node(binary, directory / "node-fresh")
    nodes.append(fresh)
    replay = fresh.control("import_history", name="final-history.bin")
    assert replay["digest"] == final["digest"] and replay["imported_height"] == final["height"], (
        replay
    )
    fresh.control("connect", port=restarted.p2p)
    wait_equal(fresh, restarted)
...
def main():
    ...
    report = {
        "status": "running",
        "original_upgrade_height": 2880,
        "test_migration_height": 3360,
        "checks": {},
        "profile": {
            "bond_veld": 50,
            "public_bond_veld": 10000,
            "validator_count": 7,
            "registration_maturity": 480,
            "finality_warmup_epochs": 2,
            "anchor_warmup_epochs": 3,
            "quorum": 5,
            "bitcoin": "regtest",
            "transport": "loopback",
            "production_migration_activation": 0,
        },
    }
    core = None
    nodes = []
    try:
        ...
        if fixture.exists():
            custody = json.loads(fixture.read_text())
        else:
            address = core.rpc("getnewaddress", ["reserve-custody", "bech32"])
            custody = {
                "address": address,
                "script": core.rpc("getaddressinfo", [address])["scriptPubKey"],
            }
        ...
        checkpoint["prev10"] = [
            core.rpc("getblockheader", [core.rpc("getblockhash", [h])])["time"]
            for h in range(2006, 2016)
        ]
        ...
        if args.prepare_only:
            ...
            return
        a = Node(binary, directory / "node-a")
        nodes.append(a)
        ...
        if core:
            core.stop()
        report["stopped"] = all(n.process.poll() is not None for n in nodes) and (
            core is None or core.process.poll() is not None
        )
        (directory / "first-activation-results.json").write_text(
