...
inline void Check(bool value, const std::string& text) {
    ++checks;
    if (!value)
...
inline void Coherent(VeldNode& node, uint64_t supply) {
    ...
    Check(state.canonical_supply_units == supply && state.module_cursor_matches,
          "supply/module cursor differs");
    ...
    Check(!state.durable_publication_pending && !state.utxo_recovery_required &&
              ...
              !state.durable_commit_fail_stop && !state.fail_stop_required &&
              state.miner_archive_ready,
          "unresolved durability/archive state");
    Check(node.TestGetMinerArchiveRaw("chain:supply") ==
              std::optional<std::string>(std::to_string(supply)),
          "durable supply differs");
    Check(node.TestGetMinerArchiveRaw("chain:height") ==
              std::optional<std::string>(std::to_string(state.height)),
          "durable height differs");
    Check(node.TestGetMinerArchiveRaw("chain:tip") ==
              std::optional<std::string>(HashToHex(state.tip_hash)),
          "durable hash differs");
}
inline void Equal(VeldNode& a, VeldNode& b) {
    ...
    Check(x.tip_hash == y.tip_hash && x.utxo_digest == y.utxo_digest &&
              x.supply_digest == y.supply_digest &&
          ...
          "history state differs");
    Coherent(a, x.canonical_supply_units);
    Coherent(b, x.canonical_supply_units);
}
inline Block Build(VeldNode& node, const RealKeyPair& owner, uint64_t nonce) {
    ...
    const auto block = node.TestBuildDStateQualificationBlock(
        node.GetChain().Height() + 1, owner.GetP2PKHScript(), "-", "-",
    ...
    Check(before.consensus_state_digest == after.consensus_state_digest &&
              before.utxo_digest == after.utxo_digest &&
              before.supply_digest == after.supply_digest &&
              before.module_cursor_matches == after.module_cursor_matches,
          "construction mutated parent");
    return block;
}
inline void Fee(VeldNode& node, const RealKeyPair& owner, uint64_t fee) {
    ...
    for (const auto& coin : coins) {
        if (coin.is_coinbase &&
            node.GetChain().Height() + 1 < coin.block_height + COINBASE_MATURITY)
            continue;
        TxInput in;
    ...
    Check(node.GetMempoolMut().Add(tx, fee, node.GetChain().Height(), node.GetChain()) ==
              Mempool::AddResult::ACCEPTED,
          "authenticated fee transaction refused");
}
inline void Seed(VeldNode& node, uint64_t headroom) {
    const auto before = node.ConsensusStateDigest();
    Check(!node.TestSetCoinbaseAccountingParent(MAX_SUPPLY_UNITS + 1),
          "above-cap parent installed");
    Check(node.ConsensusStateDigest() == before, "invalid seed mutated parent");
    Check(node.TestSetCoinbaseAccountingParent(MAX_SUPPLY_UNITS - headroom),
          "coherent accounting parent refused");
...
inline void Malformed(VeldNode& node, const Block& canonical) {
    ...
    for (unsigned variant = 0; variant < 4; ++variant) {
        auto bad = canonical;
        auto& cb = bad.transactions[0];
        if (variant == 0)
            ++cb.outputs[0].value;
        if (variant == 1)
            cb.outputs.emplace_back(0, AddressToScript(VaultAddressAtHeight(bad.height)));
        if (variant == 2)
            cb.outputs.clear();
        if (variant == 3)
            cb.outputs.emplace_back(0, BuildOpReturnScript("VELD_FINALITY|invalid-certificate"));
        cb.InvalidateTxIDCache();
        bad.UpdateMerkleRoot();
        Check(!node.TestIngestDStateQualificationFrame(bad.Serialize(), bad.height),
              "malformed full admission accepted");
