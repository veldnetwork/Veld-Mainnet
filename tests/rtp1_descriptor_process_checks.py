...
def exercise(root, core):
    ...
    for index in range(5):
        ...
        candidates = [row["desc"] for row in entries if row.get("active") is True and
            row.get("internal") is False and row["desc"].startswith("tr(")]
    ...
    config = {"custody_descriptor_sha256": document["descriptor_sha256"],
        "custody_manifest_sha256": hashlib.sha256(raw).hexdigest(), "custody_script_hex": scripts[0],
        "bitcoin_network": "regtest"}
    cfg = {"custody_spk_manifest_file": str(manifest)}
    peg = {"active": True, "spv_active": True, "custody_descriptor_sha256": config["custody_descriptor_sha256"],
        "custody_manifest_sha256": config["custody_manifest_sha256"], "custody_descriptor_range": [0, 999],
        "spv_custody_descriptor_index": 0, "spv_custody_spk_hex": scripts[0]}
    def native(method, params):
        assert method == "getpeginfo" and params == []
        return peg
    def bitcoin(method, *params):
        return core.rpc(method, [json.loads(p) if type(p) is str and p.startswith("[") else p for p in params])
    check_descriptor(config, cfg, SimpleNamespace(call=native), SimpleNamespace(call=bitcoin))
    try:
        binding.verify_core_derivation(bitcoin, binding.load_manifest(str(manifest),
    ...
    try:
        ...
        raise AssertionError("hash-pinned but incorrect final script passed independent Core derivation")
    return {"all_1000_scripts_derived_by_bitcoin_core": True,
        "native_peg_identity_simulated": True, "five_disposable_wallets_created": True,
