...
void SignAndQueue(VeldNode& node, Transaction& tx, const RealKeyPair& key) {
    for (size_t i = 0; i < tx.inputs.size(); ++i)
        tx.inputs[i].script_sig =
            key.SignInput(tx, static_cast<uint32_t>(i), key.GetP2PKHScript()).script_sig;
    Check(tx.Serialize().size() < 1024 * 1024, "fixture transaction exceeds mempool envelope");
    const auto result = node.GetMempoolMut().Add(
        tx, MIN_TX_FEE, static_cast<uint32_t>(node.GetChain().Height()), node.GetChain());
...
void Consolidate(VeldNode& node, const RealKeyPair& key) {
    ...
    std::sort(coins.begin(), coins.end(), [](const auto& a, const auto& b) {
        return a.value > b.value || (a.value == b.value && UTXOKey(a.tx_hash, a.output_index) <
                                                               UTXOKey(b.tx_hash, b.output_index));
    });
    if (coins.size() < 128)
        return;
    Transaction tx;
...
Transaction Register(VeldNode& node, const RealKeyPair& key) {
    ...
    tx.outputs.emplace_back(0, BuildOpReturnScript(std::string(ValidatorRegistry::VAL_PREFIX) +
                                                   "REGISTER|" + KeyHex(key)));
...
void BondAccounting(VeldNode& node, const RealKeyPair& key, const Transaction& tx,
    ...
    Check(output && output->value == record.bond_units &&
              output->script_pubkey == AddressToScript(STAKE_VAULT_ADDRESS),
          "vault output and bond principal differ");
    Check(node.TestReadDurableUTXO(tx.GetTxID(), 1).has_value(), "bond output was not durable");
    for (const auto& input : tx.inputs) {
        Check(!node.GetChain().GetUTXO(input.prev_tx_hash, input.prev_out_index),
              "spent bond input remains live");
        Check(!node.TestReadDurableUTXO(input.prev_tx_hash, input.prev_out_index),
              "spent bond input remains durable");
    }
...
int main(int argc, char** argv) {
    try {
        ...
        for (uint64_t h = 1; h <= parent_height; ++h) {
            if (h % 128 == 0)
                Consolidate(*accepted, owner);
            auto block = Build(*accepted, owner, h);
            Admit(*accepted, block);
            history.push_back(block);
            if (h % 480 == 0)
                std::cout << "PROGRESS naturally funded durable height=" << h << std::endl;
        }
        Check(accepted->GetValidators().SnapshotState().validators.empty(),
              "fixture unexpectedly registered a validator");
        ...
        Check(std::any_of(candidate.transactions.begin(), candidate.transactions.end(),
                          [&](const auto& included) { return included.GetTxID() == tx.GetTxID(); }),
              "builder omitted funded registration");

        size_t writes = 0;
        const auto before_failure = branch->TestObserveDStateQualificationState();
        branch->GetChainMut().SetDurableBlockBodyWriter(
        ...
        for (const auto& input : tx.inputs) {
            Check(branch->GetChain().GetUTXO(input.prev_tx_hash, input.prev_out_index).has_value(),
                  "refusal consumed funding input");
            Check(branch->TestReadDurableUTXO(input.prev_tx_hash, input.prev_out_index).has_value(),
                  "refusal consumed durable input");
        }
        branch.reset();
        branch = Reopen(root / "branch", parent_height);
        Equal(*accepted, *branch);
        std::cout
            << "PASS failed persistence leaves attributable funds and state unchanged after restart"
            << std::endl;
        ...
        for (const auto& block : history)
            Admit(*independent, block);
        Admit(*independent, candidate);
        accepted.reset();
        accepted = Reopen(root / "accepted", candidate.height);
        Equal(*accepted, *independent);
        BondAccounting(*accepted, owner, tx, owner_before, candidate.height, supply_before);
        std::cout << "PASS accepted bond accounting, independent admission and durable restart"
                  << std::endl;
        ...
        Check(disconnected > 0 && applied > 0,
              "longer branch did not exercise actual disconnect/apply");
        Equal(*accepted, *branch);
        Check(!accepted->GetValidators().SnapshotState().validators.contains(KeyHex(owner)),
              "orphaned bond record survived reorganization");
        Check(!accepted->GetChain().GetUTXO(tx.GetTxID(), 1),
              "orphaned vault output survived reorganization");
        Check(Balance(*accepted, owner) == owner_before,
              "reorganization did not restore owner funds");
        for (const auto& input : tx.inputs)
            Check(
                accepted->TestReadDurableUTXO(input.prev_tx_hash, input.prev_out_index).has_value(),
                "reorganization did not restore durable funding input");
