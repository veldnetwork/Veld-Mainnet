...
int main(int argc, char** argv) {
    ...
    const auto numeric = [](const char* value) {
        return value && *value &&
               std::all_of(value, value + std::strlen(value),
                           [](unsigned char c) { return c >= '0' && c <= '9'; });
    };
    const int duration = argc > 1 && numeric(argv[1])
                             ? std::stoi(argv[1])
    ...
    if (!chain.AddBlockDirect(genesis, true, false, false, mining::PowAdmissionContext::Internal())
             .IsAccepted())
        return 2;
    Mempool mempool;
    net::NodeServer verifier(0, MAINNET_MAGIC, chain, mempool);
    if (!verifier.Start({}, false))
        return 3; // actual outbound-only background protocol
    ...
    if (!verifier.ConnectTo("127.0.0.1", ntohs(address.sin_port)))
        return 5;
    const auto peer = ::accept(listener, nullptr, nullptr);
    VELD_CLOSE_SOCKET(listener);
    if (!compat::IsValidSocket(peer))
        return 6;
#ifdef _WIN32
    DWORD timeout = 500;
    ...
    if (!send_message(P2PMessage(MAINNET_MAGIC, MessageType::VERSION, version.Serialize())))
        return 7;
    if (!bad_identity && !send_message(P2PMessage(MAINNET_MAGIC, MessageType::VERACK)))
        return 7;
    PeerManager manager(MAINNET_MAGIC, 0);
    if (!bad_identity)
        send_message(manager.BuildTipsigMessage(0, genesis_hash));
    ...
    while (Clock::now() - start < std::chrono::seconds(duration)) {
        ...
        if (got > 0)
            buffered.insert(buffered.end(), incoming, incoming + got);
        while (buffered.size() >= 24) {
            uint32_t length = 0;
            for (int i = 0; i < 4; ++i)
                length |= uint32_t(buffered[16 + i]) << (8 * i);
            if (length > 1024 * 1024)
                return 8;
            if (buffered.size() < 24 + length)
                break;
            const std::string command(
                reinterpret_cast<const char*>(buffered.data() + 4),
            ...
            if (command == MessageType::TIPSIG) {
                ...
                if (tips > 0) {
                    const auto gap =
                        std::chrono::duration_cast<std::chrono::seconds>(Clock::now() - last_tip)
                            .count();
            ...
            if (command == MessageType::PING)
                send_message(P2PMessage(
                    MAINNET_MAGIC, MessageType::PONG,
        ...
        if (Clock::now() - last_tip > std::chrono::seconds(90))
            break;
    }
    ...
    const auto stop_ms =
        std::chrono::duration_cast<std::chrono::milliseconds>(Clock::now() - stop).count();
    const bool pass = (bad_identity ? (!connected && tips == 0)
                                    : (connected && valid && tips >= (duration >= 90 ? 4 : 1))) &&
                      stop_ms < 2000;
    std::cout << "background=" << true << " tips=" << tips << " valid=" << valid
              << " connected=" << connected << " stop_ms=" << stop_ms << " pass=" << pass
              << std::endl;
    return pass ? 0 : 1;
}
