...
Block SolvedHeightOne(Blockchain& chain, Mempool& mempool) {
    ...
    Check(solved.success && solved.block.height == 1, "fixture height-one candidate solved");
    return solved.block;
}
...
Snapshot Observe(const Blockchain& chain) {
    return Snapshot{chain.Height(), chain.TipCopy().GetHash(), chain.TotalSupplyUnits(),
                    chain.UtxoDigest()};
}

bool Same(const Snapshot& a, const Snapshot& b) {
    return a.height == b.height && a.tip == b.tip && a.supply == b.supply && a.utxo == b.utxo;
}
...
using TicketMutation = std::function<void(Blockchain::LocalWorkAdmissionTicket&)>;

Blockchain::LocalWorkAdmissionTicket MakeTicket(const Block& block,
    ...
    ticket.claim_for_canonical_commit =
        [control, identity](uint64_t coordinator_generation, uint64_t validation_generation,
                            uint32_t network_magic, const Hash256& genesis_hash,
            ...
            if (!control->live.load(std::memory_order_acquire) ||
                ...
                network_magic != identity.network_magic || genesis_hash != identity.genesis_hash ||
                profile_digest != identity.profile_digest)
                return false;
            return !control->claimed.exchange(true, std::memory_order_acq_rel);
        };
    ticket.live = [control] { return control->live.load(std::memory_order_acquire); };
    ...
    ticket.work_identity = context.local_work_kind == mining::LocalWorkKind::SubmitBlock
    ...
    ticket.deadline = std::chrono::steady_clock::now() + std::chrono::seconds(10);
    if (mutate)
        mutate(ticket);
    return ticket;
}

void RunRefusal(const char* label, TicketMutation mutate, bool starts_live = true,
                bool starts_claimed = false, bool expect_claim = false) {
    Blockchain chain;
    Mempool mempool;
    ...
    auto context =
        mining::PowAdmissionContext::InternalMiningWork(std::string("ticket-refusal-") + label);
    ...
    chain.SetLocalWorkAdmissionPrepareFn(
        [control, identity, mutate](const Block& block,
                                    const mining::PowAdmissionContext& prepared_context)
            -> std::optional<Blockchain::LocalWorkAdmissionTicket> {
            return MakeTicket(block, prepared_context, control, identity, mutate);
        });
    const auto result = chain.AddBlockDirect(candidate, false, false, false, context);
    ...
    Check(mempool.IsEmpty(), "ticket refusal leaves mempool unchanged");
    Check(control->claim_calls.load(std::memory_order_acquire) == (expect_claim ? 1U : 0U),
...
int main() {
    RunRefusal("candidate hash mismatch",
               [](auto& ticket) { ticket.candidate_hash = Filled(0x31); });
    RunRefusal("candidate parent mismatch",
               [](auto& ticket) { ticket.parent_hash = Filled(0x32); });
    RunRefusal("work binding mismatch", [](auto& ticket) { ticket.work_binding += "-altered"; });
    RunRefusal("work source mismatch",
               [](auto& ticket) { ticket.source = mining::LocalWorkKind::SynchronousGeneration; });
    RunRefusal("internal work identity mismatch",
               [](auto& ticket) { ticket.work_identity = Filled(0x33); });
    RunRefusal("expired ticket", [](auto& ticket) {
        ticket.deadline = std::chrono::steady_clock::now() - std::chrono::milliseconds(1);
    });
    RunRefusal("shutdown canceled ticket", {}, false);
    RunRefusal("reused ticket", {}, true, true, true);
    RunRefusal(
        "validation generation mismatch", [](auto& ticket) { ++ticket.validation_generation; },
        true, false, true);
    RunRefusal(
        "coordinator generation mismatch", [](auto& ticket) { ++ticket.coordinator_generation; },
        true, false, true);
    RunRefusal(
        "network identity mismatch", [](auto& ticket) { ++ticket.network_magic; }, true, false,
        true);
    RunRefusal(
        "genesis identity mismatch", [](auto& ticket) { ticket.genesis_hash = Filled(0x34); }, true,
        false, true);
    RunRefusal(
        "profile identity mismatch", [](auto& ticket) { ticket.profile_digest = Filled(0x35); },
        true, false, true);

    {
        ...
        Block candidate = SolvedHeightOne(chain, mempool);
        const Snapshot before = Observe(chain);
        auto context = mining::PowAdmissionContext::InternalMiningWork("ticket-prepare-exception");
        chain.SetLocalWorkAdmissionPrepareFn(
        ...
        const auto result = chain.AddBlockDirect(candidate, false, false, false, context);
        Check(result.IsDeferred() && Same(before, Observe(chain)) && mempool.IsEmpty(),
...

    {
        ...
        InstallGenesis(chain);
        Block candidate = SolvedHeightOne(chain, mempool);
        auto context = mining::PowAdmissionContext::InternalMiningWork("ticket-claim-exception");
        auto control = std::make_shared<TicketControl>();
        const TicketIdentity identity;
        chain.SetLocalWorkAdmissionPrepareFn(
            [control, identity](const Block& block,
                                const mining::PowAdmissionContext& prepared_context)
                -> std::optional<Blockchain::LocalWorkAdmissionTicket> {
                auto ticket = MakeTicket(block, prepared_context, control, identity);
                ticket.claim_for_canonical_commit = [](uint64_t, uint64_t, uint32_t, const Hash256&,
                                                       const Hash256&) -> bool {
...

    {
        ...
        Check(local.GetHash() != competing.GetHash(), "tip-change candidates are distinct");
        auto context = mining::PowAdmissionContext::InternalMiningWork("ticket-tip-change");
        auto control = std::make_shared<TicketControl>();
        const TicketIdentity identity;
        bool competitor_committed = false;
        chain.SetLocalWorkAdmissionPrepareFn(
            [&](const Block& block, const mining::PowAdmissionContext& prepared_context)
                -> std::optional<Blockchain::LocalWorkAdmissionTicket> {
                competitor_committed = chain
                                           .AddBlockDirect(competing, false, false, false,
                                                           mining::PowAdmissionContext::Internal())
                                           .IsAccepted();
                return MakeTicket(block, prepared_context, control, identity);
            });
        const auto result = chain.AddBlockDirect(local, false, false, false, context);
        Check(competitor_committed && result.IsDeferred(),
              "canonical tip change between prepare and claim refuses local work");
        Check(chain.Height() == 1 && chain.TipCopy().GetHash() == competing.GetHash() &&
                  !chain.IsCanonicalBlock(local.GetHash()) && control->claim_calls.load() == 0,
...

    {
        ...
        Check(first.GetHash() != second.GetHash(), "simultaneous candidates are distinct");
        ...
        chain.SetLocalWorkAdmissionPrepareFn(
            [&](const Block& block, const mining::PowAdmissionContext& prepared_context)
                ...
                ticket.claim_for_canonical_commit =
                    [control, identity,
                     &claims](uint64_t coordinator_generation, uint64_t validation_generation,
                              uint32_t network_magic, const Hash256& genesis_hash,
                              const Hash256& profile_digest) {
                        if (coordinator_generation != identity.coordinator_generation ||
                            validation_generation != identity.validation_generation ||
        ...
        auto first_context =
            mining::PowAdmissionContext::InternalMiningWork("ticket-simultaneous-first");
        auto second_context =
            mining::PowAdmissionContext::InternalMiningWork("ticket-simultaneous-second");
        Blockchain::BlockAdmissionResult first_result(false);
        Blockchain::BlockAdmissionResult second_result(false);
        std::thread first_thread([&] {
            first_result = chain.AddBlockDirect(first, false, false, false, first_context);
        });
        std::thread second_thread([&] {
            second_result = chain.AddBlockDirect(second, false, false, false, second_context);
        });
        {
            std::unique_lock<std::mutex> lock(barrier_mutex);
            Check(barrier_cv.wait_for(lock, std::chrono::seconds(3), [&] { return prepared == 2; }),
...

    {
        ...
        auto context = mining::PowAdmissionContext::InternalMiningWork("ticket-shutdown-pending");
        Blockchain::BlockAdmissionResult result(false);
        std::thread pending(
            [&] { result = chain.AddBlockDirect(candidate, false, false, false, context); });
        {
            std::unique_lock<std::mutex> lock(pending_mutex);
            Check(pending_cv.wait_for(lock, std::chrono::seconds(3), [&] { return entered; }),
    ...
    std::cout << "PASS local_work_ticket_tests checks=" << checks
