...
vm.runInContext(
    ['encryptNodePassphrase', 'canonicalPayload', 'commandEnvelope', 'normalizeEcdsaSignature']
        .map((name) => extractFunction(script, name))
        .join('\n'),
    context,
);
(async () => {
    const payload = await context.encryptNodePassphrase(
        'Portal test 日本語 é passphrase',
        publicKey,
        17,
        nonce,
    );
    ...
    fs.writeFileSync(
        path.join(root, 'ciphertext.txt'),
        ['ciphertext', 'identity', 'iv', 'key_id', 'wrapped_key']
            .map((k) => payload[k])
            .concat(nonce)
            .join('\n') + '\n',
    );
    ...
    const commandKey = {
        ...
        id: Buffer.from(
            await webcrypto.subtle.digest(
                'SHA-256',
                new TextEncoder().encode(`VELD_PORTAL_KEY_V1\n${jwk.x}\n${jwk.y}`),
            ),
        ).toString('hex'),
    };
    ...
    const command = {
        ...
        action: 'node.signin',
        payload,
        key_id: commandKey.id,
    };
    const signature = await webcrypto.subtle.sign(
        { name: 'ECDSA', hash: 'SHA-256' },
        pair.privateKey,
        new TextEncoder().encode(context.commandEnvelope(command)),
    );
    command.signature = b64url(context.normalizeEcdsaSignature(signature));
    fs.writeFileSync(
        path.join(root, 'signed-command.json'),
        JSON.stringify({
            portal_protocol: 4,
            device_id: 17,
            paired: true,
    ...
    for (const action of [
        ...
        const signature = await webcrypto.subtle.sign(
            { name: 'ECDSA', hash: 'SHA-256' },
            pair.privateKey,
            new TextEncoder().encode(context.commandEnvelope(control)),
        );
        control.signature = b64url(context.normalizeEcdsaSignature(signature));
        fs.writeFileSync(
            path.join(root, action + '.json'),
            JSON.stringify({
                portal_protocol: 4,
                device_id: 17,
                paired: true,
    ...
    for (const mode of ['full', 'snapshot']) {
        const sync = {
            ...
            action: 'sync.mode',
            payload: { mode },
            nonce: b64url(webcrypto.getRandomValues(new Uint8Array(16))),
        };
        let envelope;
        if (mode === 'full') {
            assert.equal(
                context.canonicalPayload(sync.action, sync.payload),
                JSON.stringify({ mode }),
            );
        ...
        fs.writeFileSync(
            path.join(root, 'sync.mode-' + mode + '.json'),
            JSON.stringify({
                portal_protocol: 4,
                device_id: 17,
                paired: true,
    ...
    fs.writeFileSync(
        path.join(root, 'other-pairing.json'),
        JSON.stringify({
            portal_protocol: 4,
            device_id: 17,
            paired: true,
    ...
    for (const value of ['', 'x'.repeat(1025), 'hello\0world']) {
        await assert.rejects(
            context.encryptNodePassphrase(value, publicKey, 17, nonce),
            /valid node passphrase/,
        );
    }
    console.log(output.trim());
    console.log(
        'PASS: shipping browser encryptor and native decryptor agree; invalid input rejected; full sync signed; snapshot refused by browser; legacy snapshot native confirmation fixture retained',
    );
    if (process.argv[4])
        console.log(
            execFileSync(path.resolve(process.argv[4]), [root], {
                encoding: 'utf8',
                timeout: 30000,
            }).trim(),
        );
