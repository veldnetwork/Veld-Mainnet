...
int main() {
    ...
    TempDirectory temp{
        ...
        ("veld-daybreak-identity-" + std::to_string(nonce))};
    std::string error;
    CHECK(veld::channel::secure_file::EnsurePrivateDirectory(
        temp.path.string(), &error));
    CHECK(veld::ValidateOrCreatePublicNetworkIdentity(
        temp.path.string(), &error));

    std::vector<uint8_t> bytes;
    CHECK(veld::channel::secure_file::Read(
              (temp.path / "network.identity").string(), bytes, &error,
              4096, true) ==
    ...
    CHECK(created.find("profile_id=veld-public-mainnet-v2\n") !=
          std::string::npos);
    CHECK(created.find(
              "btcveld_reserve_semantics=rolling-outpoint-v1\n") !=
          std::string::npos);
    ...
    legacy.replace(legacy.find(current_header), current_header.size(),
                   legacy_header);
    const std::string current_profile = "veld-public-mainnet-v2";
    const std::string legacy_profile = "veld-public-mainnet-v1";
    legacy.replace(legacy.find(current_profile), current_profile.size(),
                   legacy_profile);
    CHECK(veld::channel::secure_file::AtomicWriteText(
        (temp.path / "network.identity").string(), legacy, &error, true));
    error.clear();
    CHECK(!veld::ValidateOrCreatePublicNetworkIdentity(
        temp.path.string(), &error));
    CHECK(error.find("does not exactly match") != std::string::npos);

    TempDirectory missing{
        std::filesystem::temp_directory_path() /
        ("veld-daybreak-identity-missing-" + std::to_string(nonce))};
    CHECK(veld::channel::secure_file::EnsurePrivateDirectory(
        missing.path.string(), &error));
    const std::string occupied = "historical-state";
    CHECK(veld::channel::secure_file::AtomicWriteText(
        (missing.path / "blocks.dat").string(), occupied, &error, true));
    error.clear();
    CHECK(!veld::ValidateOrCreatePublicNetworkIdentity(
        missing.path.string(), &error));
    CHECK(error.find("absent from a nonempty public datadir") !=
          std::string::npos);

    std::cout << "PASS daybreak_datadir_identity_tests checks=" << checks
