...
class Node:
    def __init__(self, binary, directory, suffix=""):
        ...
        self.log = open(
            directory.parent / (directory.name + suffix + ".log"), "w", encoding="utf-8"
        )
    ...
    def event(self, prefix, timeout=120):
        ...
        while time.monotonic() < deadline:
            item = self.events.get(timeout=max(0.1, deadline - time.monotonic()))
    ...
    def control(self, command, **fields):
        self.process.stdin.write(
            json.dumps(dict(command=command, **fields), ensure_ascii=True) + "\n"
        )
    ...
    def rpc(self, method, params=(), authenticated=True, timeout=240):
        ...
        conn.request(
            "POST",
            "/",
            json.dumps(dict(jsonrpc="2.0", id=1, method=method, params=list(params))),
            headers,
        )
    ...
    def generate(self, count):
        ...
        for attempt in range(max(20, count * 2)):
            ...
            if height == target:
                return {"generated": count, "height": height}
            try:
...
def wait_equal(a, b, timeout=90):
    ...
    while time.monotonic() < deadline:
        ...
        if time.monotonic() >= redial_at:
            if aa["ready_peer_ips"] == 0:
                a.control("connect", port=b.p2p)
            if bb["ready_peer_ips"] == 0:
...
def advance(a, b, height):
    ...
    while previous < height:
        ...
        print(
            ...
            + str(previous)
            + " "
            + ("peers_equal=true" if b is not None else "local_full_validation=true"),
            flush=True,
        )
    return state
...
def full_scenario(a, b, binary, directory, nodes, report):
    ...
    try:
        a.control(
            "propose",
            wallet=1,
            title="Ordinary proposal while locked",
            description="Governance gate control.",
        )
        raise AssertionError("four validators unexpectedly permitted governance")
    except RuntimeError as error:
        if "Governance is locked" not in str(error):
            raise
    ...
    b = Node(binary, directory / "node-b-prefix")
    nodes.append(b)
    imported = b.control("import_history", name="prefix-history.bin")
    assert (
        imported["imported_height"] == parent["height"] and imported["digest"] == parent["digest"]
    )
    checks["full_local_prefix_validation"] = True
    b.stop()
    shutil.copytree(b.directory, directory / "node-fork")
    b = Node(binary, b.directory, "-preactivation-restart")
    nodes.append(b)
    b.control("connect", port=a.p2p)
    ...
    a.control(
        ...
        wallet=1,
        title=legacy_title,
        description="Preserve the recorded proposal across migration.",
    )
    ...
    a.control(
        ...
        wallet=3,
        title=new_title,
        description="UTF-8 and delimiter text retain their signed field boundaries.",
    )
    ...
    assert (
        ...
        and endorsements["endorsements"][0]["address"] == a.ready["addresses"][4]
    ), endorsements
    ...
    b.stop()
    b = Node(binary, b.directory, "-postactivation-restart")
    nodes.append(b)
    ...
    fork = Node(binary, directory / "node-fork")
    nodes.append(fork)
    ...
    a.control("connect", port=fork.p2p)
    b.control("connect", port=fork.p2p)
    ...
    fresh = Node(binary, directory / "node-fresh-ibd")
    nodes.append(fresh)
    fork.control("export_history", name="final-history.bin")
    replayed = fresh.control("import_history", name="final-history.bin")
    assert (
        replayed["digest"] == forked["digest"] and replayed["imported_height"] == forked["height"]
    )
    ...
    report["genesis_to_tip_p2p_ibd"] = (
        "NOT RUN: unchanged eight-PoW-checks-per-minute source limit; local full validation plus migration P2P suffix tested"
    )
...
def main():
    ...
    try:
        a = Node(args.binary.resolve(), directory / "node-a")
        nodes.append(a)
        ...
        report = {
            "status": "smoke_pass",
            "activation_height": 2880,
            "height": state["height"],
            "digest": state["digest"],
            "p2p": "loopback",
            "rpc_auth": "required",
            "mainnet_activation": 0,
            "checks": {
                "authenticated_rpc": True,
                "explorer_http": True,
                "initial_p2p_convergence": True,
            },
