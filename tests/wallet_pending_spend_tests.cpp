...
Transaction Prepare(RpcServer& rpc, const RealKeyPair& owner, const std::string& recipient,
...
void Accepted(RpcServer& rpc, const Transaction& tx) {
    ...
    if (const auto* error = response.Get("error"))
        if (const auto* message = error->Get("message"))
            std::cerr << message->text << '\n';
    Check(!Failed(response) && response.Get("result") &&
              response.Get("result")->text == HashToHex(tx.GetTxID()),
          "signed RPC broadcast refused");
}
...
int main(int argc, char** argv) {
    try {
        ...
        Check((results[0] == Mempool::AddResult::ACCEPTED) !=
                  (results[1] == Mempool::AddResult::ACCEPTED),
              "racing spends admitted twice");
        Check(results[0] == Mempool::AddResult::DOUBLE_SPEND ||
                  results[1] == Mempool::AddResult::DOUBLE_SPEND,
              "race did not reject conflicting input");
        Check(rpc.ComputeWalletState(owner.address).spendable_units == 0,
              "pending sends leave no spendable inputs");
        const auto tip_before = chain.TipCopy().GetHash();
        for (unsigned mode = 0; mode < 2; ++mode) {
            const auto block = Candidate(chain, recipient, {first, conflict}, 100 + mode);
            Check(!chain
                       .AddBlockDirect(block, true, bool(mode), false,
                                       mining::PowAdmissionContext::Internal())
                       .IsAccepted(),
                  "block included conflicting spends");
        ...
        Check(
            chain.AddBlockDirect(block, true, true, false, mining::PowAdmissionContext::Internal())
                .IsAccepted(),
            "valid confirmation failed");
        pool.Remove(HashToHex(first.GetTxID()));
        Check(!chain.ValidateTransactionLocking(conflict, false),
              "confirmed input can be spent again");
