...
std::shared_ptr<veld::net::Connection> MakeConnection(const std::string& address, uint16_t port) {
    const auto fd = ::socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    if (!veld::compat::IsValidSocket(fd))
        return {};
    return std::make_shared<veld::net::Connection>(fd, address, port, true);
}

veld::UTXO InstallNmsInput(veld::Blockchain& chain, const veld::RealKeyPair& signer, uint8_t tag) {
    veld::UTXO input;
...
veld::Transaction BuildNmsTransaction(veld::Blockchain& chain, const veld::RealKeyPair& signer,
                                      const veld::UTXO& parent, uint64_t nonce,
    ...
    tx.outputs.emplace_back(0, BuildNmsOpReturnScript(EncodeNmsPayload(claimed)));
    tx.outputs.emplace_back(parent.value - (2 * MIN_TX_FEE), miner_script);
    tx.inputs[0].script_sig = signer.SignInput(tx, 0, parent.script_pubkey).script_sig;
    return tx;
}

void AttachNmsTransaction(veld::Block& block, const veld::Transaction& tx) {
    const auto vault_script = veld::AddressToScript(veld::VaultAddressAtHeight(block.height));
...
void Init(veld::Blockchain& chain) {
    ...
    const auto result = chain.AddBlockDirect(genesis, true, false, false,
                                             veld::mining::PowAdmissionContext::Internal());
    if (!result.IsAccepted())
        throw std::runtime_error("test genesis rejected");
}

veld::Block BuildCandidate(veld::Blockchain& chain, const std::vector<uint8_t>& miner_script,
                           uint64_t timestamp) {
    using namespace veld;
    Block block;
    ...
    const uint64_t remaining = MAX_SUPPLY_UNITS > supply ? MAX_SUPPLY_UNITS - supply : 0;
    ...
    if (block.height > 0 && block.height % VAULT_BLOCK_INTERVAL == 0) {
        ...
        const uint64_t miner_cut = effective - pool_cut - vault_cut - endorse_cut;
        outputs.push_back({miner_script, miner_cut});
        outputs.push_back({AddressToScript(PoolAddressAtHeight(block.height)), pool_cut});
        outputs.push_back({vault_script, vault_cut});
        outputs.push_back(
            {AddressToScript(EndorsementPoolAddressAtHeight(block.height)), endorse_cut});
    }
...
veld::Block Append(veld::Blockchain& chain, const std::vector<uint8_t>& miner_script,
                   uint64_t timestamp) {
    auto block = BuildCandidate(chain, miner_script, timestamp);
    const auto result = chain.AddBlockDirect(block, false, false, false,
                                             veld::mining::PowAdmissionContext::Internal());
    if (!result.IsAccepted()) {
        throw std::runtime_error("candidate rejected: " + veld::Blockchain::GetLastRejectTag());
    }
    return block;
}
...
void Replay(veld::Blockchain& chain, const veld::Block& source) {
    ...
    const auto result = chain.AddBlockDirect(block, false, false, false,
                                             veld::mining::PowAdmissionContext::Internal());
    if (!result.IsAccepted()) {
        throw std::runtime_error("replay rejected: " + veld::Blockchain::GetLastRejectTag());
    }
}
...
int main() {
    ...
    for (const auto& block : main_suffix)
        Replay(recipient, block);
    ...
    recipient.SetDurableBlockBodyWriter([&](const Hash256& hash, const std::vector<uint8_t>& raw) {
        ++durable_writes;
        durable[HashToHex(hash)] = raw;
        return true;
    });
    recipient.SetHistoricalBlockLoader(
        [&](const Hash256& hash) -> std::optional<std::vector<uint8_t>> {
    ...
    for (size_t i = 0; i < 5; ++i) {
        ...
        const auto result =
            recipient.AddBlockDirect(block, false, true, false, PowAdmissionContext::Internal());
        observe(result);
        Check(result.IsAccepted(), "correct lower-work side prefix is contextually accepted");
    }
    Check(durable_writes == 5 && peer_credits == 5 && relays == 5 && on_block == 5 &&
    ...
    const size_t effects_before_invalid = durable_writes + peer_credits + relays + on_block;
    const auto invalid_result = recipient.AddBlockDirect(invalid_state, false, true, false,
    ...
    Check(durable_writes + peer_credits + relays + on_block == effects_before_invalid &&
              recipient.VolatileSideQuarantineCount() == 0,
    ...
    Blockchain::TestResetVerifyBlockPowCalls();
    const size_t effects_before_wrong = durable_writes + peer_credits + relays + on_block;
    const auto wrong_result =
        recipient.AddBlockDirect(wrong_bits, false, true, false, PowAdmissionContext::Internal());
    ...
    Check(durable_writes + peer_credits + relays + on_block == effects_before_wrong,
          "wrong-bit side block reaches no durable or peer effect");

    auto correct_boundary = side_suffix[5];
    const auto correct_result = recipient.AddBlockDirect(correct_boundary, false, true, false,
                                                         PowAdmissionContext::Internal());
    observe(correct_result);
    Check(correct_result.IsAccepted(),
          "correct varying-LWMA side bits pass full contextual validation");
    Check(durable_writes == 6 && peer_credits == 6 && relays == 6 && on_block == 6 &&
              recipient.VolatileSideQuarantineCount() == 0,
          "correct boundary side block is promoted before one peer effect");

    const size_t effects_before_known = durable_writes + peer_credits + relays + on_block;
    const auto known_result = recipient.AddBlockDirect(correct_boundary, false, true, false,
                                                       PowAdmissionContext::Internal());
    ...
    Check(durable_writes + peer_credits + relays + on_block == effects_before_known,
          "known promoted retransmit repeats no writer or peer effect");

    Block next_main =
        BuildCandidate(recipient, main_script, recipient.TipCopy().header.timestamp + 15);
    ...
    const auto checkpoint_result =
        recipient.AddBlockDirect(next_main, false, true, false, PowAdmissionContext::Internal());
    ...
    const auto anchor_result =
        recipient.AddBlockDirect(next_main, false, true, false, PowAdmissionContext::Internal());
    Check(!anchor_result.IsAccepted() && Blockchain::GetLastRejectTag() == "anchor_conflict" &&
              Blockchain::TestVerifyBlockPowCalls() == 0,
          "anchor conflict rejects before VeldHash");
    recipient.anchor_gate_ = {};

    Blockchain::TestResetVerifyBlockPowCalls();
    const auto unwired_result = recipient.AddBlockDirect(next_main, false, true, false);
    Check(!unwired_result.IsAccepted() && !unwired_result.IsDeferred() &&
              Blockchain::GetLastRejectTag() == "pow_admission_context_unavailable" &&
              Blockchain::TestVerifyBlockPowCalls() == 0,
    ...
    forged_spend.outputs.emplace_back(forged_parent.value, MinerScript(0xB0));
    ...
    const std::string bad_signature_hex = HashToHex(bad_signature_hash);
    const size_t writes_before_bad_signature = durable_writes;
    auto bad_signature_budget = std::make_shared<ExpensivePowBudget>(1, 8, std::chrono::minutes(1));
    const auto bad_signature_context =
        PowAdmissionContext::Peer("198.51.100.44", bad_signature_budget);
    const auto bad_signature_result =
        recipient.AddBlockDirect(bad_signature, false, true, false, bad_signature_context);
    Check(!bad_signature_result.IsAccepted() && !bad_signature_result.IsDeferred() &&
              Blockchain::GetLastRejectTag() == "consensus_tx_sig_or_value_invalid_v2",
    ...
    for (const auto& tip : recipient.GetChainTips()) {
        if (tip.hash == bad_signature_hash)
            bad_signature_tip_exposed = true;
    }
    Check(durable_writes == writes_before_bad_signature &&
              !recipient.GetBlockByHash(bad_signature_hash).has_value() &&
              !recipient.GetKnownBlockHeightByHash(bad_signature_hash).has_value() &&
              !recipient.HasBlockAtHeight(bad_signature.height, bad_signature_hex) &&
              !bad_signature_tip_exposed,
          "late-invalid canonical candidate has no durable or public residue");

    const auto bad_signature_retry =
        recipient.AddBlockDirect(bad_signature, false, true, false, bad_signature_context);
    Check(!bad_signature_retry.IsAccepted() && !bad_signature_retry.IsDeferred() &&
              Blockchain::GetLastRejectTag() == "consensus_tx_sig_or_value_invalid_v2" &&
              durable_writes == writes_before_bad_signature,
          "late-invalid retransmit is revalidated and never cached as a duplicate");

    Block valid_after_reject =
        BuildCandidate(recipient, main_script, recipient.TipCopy().header.timestamp + 15);
    const auto valid_after_reject_result =
        recipient.AddBlockDirect(valid_after_reject, false, true, false, bad_signature_context);
    Check(valid_after_reject_result.IsAccepted() &&
              durable_writes == writes_before_bad_signature + 1 &&
              recipient.GetBlockByHash(valid_after_reject.GetHash()).has_value() &&
              recipient.GetKnownBlockHeightByHash(valid_after_reject.GetHash()).has_value(),
    ...
    const auto nms_parent = InstallNmsInput(nms_block_chain, nms_signer, 0xC1);
    const auto nms_tx = BuildNmsTransaction(nms_block_chain, nms_signer, nms_parent, 0xC101);
    Block nms_candidate = BuildCandidate(nms_block_chain, nms_signer.GetP2PKHScript(),
                                         nms_block_chain.TipCopy().header.timestamp + 1);
    AttachNmsTransaction(nms_candidate, nms_tx);
    size_t nms_block_writes = 0;
    nms_block_chain.SetDurableBlockBodyWriter([&](const Hash256&, const std::vector<uint8_t>&) {
        ++nms_block_writes;
        return true;
    });
    auto nms_block_budget =
        std::make_shared<ExpensivePowBudget>(1, 2, std::chrono::milliseconds(100));
    auto preconsume_nms_start = nms_block_budget->TryAcquire(ExpensivePowUse::PeerNms);
    preconsume_nms_start.reset();
    const auto nms_block_context = PowAdmissionContext::Peer("198.51.100.61", nms_block_budget);
    const auto nms_block_deferred =
        nms_block_chain.AddBlockDirect(nms_candidate, false, true, false, nms_block_context);
    Check(nms_block_deferred.IsDeferred() &&
              Blockchain::GetLastRejectTag() == "nms_local_work_deferred" &&
              nms_block_writes == 0 && nms_block_chain.Height() == 0,
          "canonical NMS extension defers without persistence or tip change");
    std::this_thread::sleep_for(std::chrono::milliseconds(120));
    const auto nms_block_retry =
        nms_block_chain.AddBlockDirect(nms_candidate, false, true, false, nms_block_context);
    Check(nms_block_retry.IsAccepted() && nms_block_writes == 1 && nms_block_chain.Height() == 1,
    ...
    const auto pool_parent = InstallNmsInput(nms_pool_chain, pool_signer, 0xC2);
    const auto pool_tx = BuildNmsTransaction(nms_pool_chain, pool_signer, pool_parent, 0xC201);
    ExpensivePowBudget blocked_pool_budget(1);
    auto held_pool_lease = blocked_pool_budget.TryAcquire(ExpensivePowUse::PeerNms);
    Check(nms_pool.Add(pool_tx, MIN_TX_FEE, static_cast<uint32_t>(nms_pool_chain.Height()),
                       nms_pool_chain,
                       &blocked_pool_budget) == Mempool::AddResult::DEFERRED_LOCAL_WORK &&
              !nms_pool.Contains(pool_tx.GetTxID()),
          "mempool exposes NMS local-work deferral without insertion");
    held_pool_lease.reset();
    Check(nms_pool.Add(pool_tx, MIN_TX_FEE, static_cast<uint32_t>(nms_pool_chain.Height()),
                       nms_pool_chain, &blocked_pool_budget) == Mempool::AddResult::ACCEPTED &&
              nms_pool.Contains(pool_tx.GetTxID()),
          "mempool exact retry succeeds after local capacity returns");
    const auto invalid_parent = InstallNmsInput(nms_pool_chain, pool_signer, 0xC3);
    const auto invalid_nms_tx =
        BuildNmsTransaction(nms_pool_chain, pool_signer, invalid_parent, 0xC301, 0x20ffffffu);
    Check(nms_pool.Add(invalid_nms_tx, MIN_TX_FEE, static_cast<uint32_t>(nms_pool_chain.Height()),
                       nms_pool_chain, &blocked_pool_budget) == Mempool::AddResult::INVALID &&
              net::NodeServer::MempoolRejectBanScore(Mempool::AddResult::INVALID) == 10,
    ...
    const auto p2p_parent = InstallNmsInput(p2p_chain, p2p_signer, 0xC4);
    const auto p2p_tx = BuildNmsTransaction(p2p_chain, p2p_signer, p2p_parent, 0xC401);
    ...
    if (p2p_conn) {
        ...
        auto held_p2p_lease = source_budget->TryAcquire(ExpensivePowUse::PeerNms);
        const P2PMessage p2p_message(MAINNET_MAGIC, MessageType::TX, p2p_tx.Serialize());
        ...
        Check(!p2p_pool.Contains(p2p_tx.GetTxID()) &&
                  ...
                  net::NodeServer::MempoolRejectBanScore(Mempool::AddResult::DEFERRED_LOCAL_WORK) ==
                      0,
              "P2P NMS deferral has no insertion ban score or ban");
        held_p2p_lease.reset();
        p2p_server.TestDispatchPeerMessage(*p2p_conn, p2p_message);
        Check(p2p_pool.Contains(p2p_tx.GetTxID()) &&
                  source_budget == p2p_server.TestPowBudgetForSource(p2p_source),
              "P2P exact retry preserves source budget and is admitted");

        const auto p2p_invalid_parent = InstallNmsInput(p2p_chain, p2p_signer, 0xC5);
        const auto p2p_invalid_tx =
            BuildNmsTransaction(p2p_chain, p2p_signer, p2p_invalid_parent, 0xC501, 0x20ffffffu);
        const P2PMessage p2p_invalid_message(MAINNET_MAGIC, MessageType::TX,
                                             p2p_invalid_tx.Serialize());
        p2p_server.TestDispatchPeerMessage(*p2p_conn, p2p_invalid_message);
    ...
    std::cerr << "FAIL checks=" << checks << " failures=" << failures << "\n";
    return 1;
}
