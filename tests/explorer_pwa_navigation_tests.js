...
function fixture(options = {}) {
    ...
    function element(href, active = false) {
        ...
        return {
            href: href ? new URL(href, location).href : '',
            target: '',
            classList: {
    ...
    const origin = element(),
        nav = element(),
        more = element();
    const home = element('/', true),
        mempool = element('/mempool'),
        mining = element('/mining');
    ...
    const document = {
        ...
        querySelector: (selector) =>
            selector === 'body > .wrap' ? origin : selector === '.nav-bar' ? nav : null,
    ...
    const history = {
        state: null,
        entries: [{ url: location.href, state: null }],
        index: 0,
        ...
        move(delta) {
            ...
            location.href = new URL(entry.url, location).href;
            windowEvents.popstate({ state: entry.state });
        },
    };
    const context = vm.createContext({
        ...
        DOMParser: class {
            parseFromString() {
                ...
                parsedContent.querySelectorAll = (selector) =>
                    selector === '*' ? [injectedLink] : [discardedScript];
                return {
                    title: options.badTitle ? 'Error' : 'Veld · Mempool',
...
(async () => {
    assert.equal(
        fixture({ standalone: false }).events.click,
        undefined,
        'Browser tabs retain native navigation',
    );
    assert.equal(
        fixture({ url: 'https://explorer.test/mempool' }).events.click,
        undefined,
        'Direct Mempool loads initialize normally',
    );
    const f = fixture(),
        nav = f.nav;
    assert.equal(
        f.click(f.mempool, { ctrlKey: true }).defaultPrevented,
        false,
        'Modified clicks retain their browser behavior',
    );
    assert.equal(
        f.click(f.mining).defaultPrevented,
        false,
        'Other routes retain native navigation',
    );
    ...
    for (const response of [
        { ok: false },
        { url: 'https://other.test/mempool' },
        { url: 'https://explorer.test/error' },
        { headers: { get: () => 'application/json' } },
        { text: async () => 'x'.repeat(2 * 1024 * 1024 + 1) },
    ]) {
        const bad = fixture();
        bad.click(bad.mempool);
        await bad.finish(response);
        assert.deepEqual(
            bad.assignments,
            ['/mempool'],
            'Invalid responses fall back to ordinary navigation',
        );
    ...
    cancelled.click(cancelled.mempool);
    cancelled.click(cancelled.home);
    await flush();
    assert.equal(cancelled.requests[0].settings.signal.aborted, true);
    assert.equal(
        cancelled.assignments.length,
        0,
        'Cancelling navigation cannot redirect afterward',
    );
    const timedOut = fixture();
    timedOut.click(timedOut.mempool);
    const timer = [...timedOut.timers.values()][0];
    assert.equal(timer.ms, 15000);
    timer.handler();
    await flush();
    assert.deepEqual(timedOut.assignments, ['/mempool'], 'Timeout returns to the normal page path');
    console.log(
        'PASS Explorer PWA navigation: mounted controls, fresh content, history, modifiers, bounded requests, cancellation, sanitization and failure fallback',
    );
})().catch((error) => {
    console.error(error);
    process.exitCode = 1;
