...
inline size_t test_transport_lookups = 0;
inline std::string TestDisabledCurlLookup() {
    ++test_transport_lookups;
    return {}; // cannot invoke curl, even if a policy check regresses
}
}
#define TrustedSystemCurlExecutable TestDisabledCurlLookup
#include "node/node.h"
