...
int main() try {
    ...
    for (unsigned attempt = 0; attempt < 8 && !refreshed; ++attempt) {
        ...
        auto search = std::async(std::launch::async, [&] {
            return MineOnly(chain, pool, owner, 0, &stop, {}, 1, nullptr, {}, nullptr,
                            nullptr, {}, [&](const Block& candidate) {
                Check(candidate.transactions.size() == 1, "original candidate must predate claim");
                built.store(true);
                return true;
            });
        });
        ...
        if (!built.load()) { stop.store(true); search.get(); throw std::runtime_error("candidate timeout"); }
        ...
        if (search.wait_for(std::chrono::seconds(15)) != std::future_status::ready) {
            stop.store(true); search.get(); throw std::runtime_error("new claim never refreshed active mining");
        }
        const auto old_work = search.get();
        if (old_work.success) continue;
        ...
        Check(chain.AddBlockDirect(rebuilt.block, false, false, false,
        ...
        const auto payouts = chain.ComputeExpectedPoolOutputs(
            chain.TipCopy().GetHash(), 100 * VELD_UNITS, 5000);
        uint64_t paid = 0, carried = 0;
        for (const auto& [recipient, value] : payouts) {
            if (recipient == script) paid += value;
            else if (recipient == AddressToScript(POOL_ADDRESS)) carried += value;
            else Check(false, "unexpected payout recipient");
        }
    ...
    std::cout << "PASS active mining refresh, signed claim inclusion, confirmed eligibility and sole-participant payout\n";
    return 0;
