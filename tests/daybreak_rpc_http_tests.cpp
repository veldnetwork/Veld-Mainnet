...
std::string Request(uint16_t port, const std::string& method,
                    ...
                    const std::string& forwarded_identity,
...
int main() {
    ...
    const std::filesystem::path storage_path =
        ...
        ("veld-daybreak-rpc-http-test-" + suffix);

    {
        veld::Blockchain chain;
        veld::Mempool mempool;
        veld::StorageEngine storage(storage_path.string(),
                                    veld::MAINNET_MAGIC);
        veld::RpcServer rpc(chain, mempool, storage);
        const std::string token = "daybreak-test-token";
        ...
        for (int i = 0; i < 10; ++i) {
            const std::string spoof =
                "198.51.100." + std::to_string(i + 1);
            const std::string response = Request(
                port, "POST", "wrong-" + std::to_string(i), spoof, body);
        ...
        CHECK(Status(Request(port, "POST", "wrong-eleven",
                             "203.0.113.250", body)) == 429);
        CHECK(server.TestAuthIdentityCount() == 1);
        CHECK(Status(Request(port, "POST", token,
                             "192.0.2.77", body)) == 200);
        ...
        const std::string removed_history_response = Request(
            port, "POST", token, "192.0.2.78", removed_history_body);
        CHECK(Status(removed_history_response) == 200);
        CHECK(removed_history_response.find("\"code\":-32601") !=
              std::string::npos);
        CHECK(removed_history_response.find("Method not found") !=
              std::string::npos);
        CHECK(Status(Request(port, "POST", "wrong-after-success",
                             "198.18.0.1", body)) == 401);
        CHECK(server.TestAuthIdentityCount() == 1);
        server.Stop();
    ...
    std::cout << "PASS daybreak_rpc_http_tests checks=" << checks
