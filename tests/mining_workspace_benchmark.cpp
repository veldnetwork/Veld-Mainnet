...
int main() {
    ...
    mining::VeldIntegerDeterminismCheck();
    ...
    for (unsigned i = 4; i < 68; ++i)
        header[i] = uint8_t(i * 11 + 5);
    for (unsigned i = 0; i < 4; ++i)
        header[76 + i] = uint8_t(GENESIS_BITS >> (8 * i));
    ...
    for (unsigned trial = 0; trial < 8; ++trial) {
        ...
        for (unsigned order = 0; order < 2; ++order) {
            const bool reuse = (trial + order) % 2;
            SHA256 checksum;
            const auto start = Clock::now();
            for (unsigned n = 0; n < 256; ++n) {
                for (unsigned j = 0; j < 8; ++j)
                    header[80 + j] = uint8_t(uint64_t(n) >> (8 * j));
                const auto hash = mining::VeldHashWithDataset<false>(header, 2880, target,
                                                                     reuse ? &workspace : nullptr);
                if (!mining::g_veldhash_last_dataset_ok())
                    return 1;
            ...
            if (!order)
                ...
                return 1;
            std::cout << std::fixed << std::setprecision(6) << "{\"trial\":" << trial
                      << ",\"reuse\":" << (reuse ? "true" : "false")
                      << ",\"hashes\":256,\"seconds\":" << seconds
                      << ",\"hashrate_hs\":" << 256 / seconds << ",\"checksum\":\""
                      << HashToHex(digest) << "\"}\n"
                      << std::flush;
        }
    }
