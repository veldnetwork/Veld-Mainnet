...
int main() {
    ...
    auto ordinary =
        std::async(std::launch::async, [&] { service.TestHandleReceipt(receiver, deadline); });
    ...
    assert(::send(sender, request.data(), static_cast<int>(request.size()), 0) ==
    ...
    receiver = idle_pair.first;
    sender = idle_pair.second;
    service.TestTrackReceiptSocket(receiver, "127.0.0.1");
