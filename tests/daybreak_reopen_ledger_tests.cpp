...
#define VELD_L3_DISPOSABLE_BTCVELD_AUTHORITY_ADDRESS \
    "VZqR3pfH6br3Aqatjkwn8XCvtFaaNqa3mj"
#define VELD_L3_DISPOSABLE_BTCVELD_CUSTODY_SPK_HEX \
    "00141111111111111111111111111111111111111111"

#define main daybreak_original_reserve_main
#include "daybreak_reserve_tests.cpp"
#undef main
...
Transaction SignedIssuerReserveCarrier(const TransitionFixture& fixture,
    ...
    tx.inputs[0].script_sig =
        issuer.SignInput(tx, 0, issuer.GetP2PKHScript()).script_sig;
...
int main() {
    try {
        CHECK(std::string(BTCVELD_ISSUER_ADDRESS) ==
              "VZqR3pfH6br3Aqatjkwn8XCvtFaaNqa3mj");
        ...
        TestLedger model;
        const auto first_open = MakeOpen(
            bitcoin, model.state, 10'000, 0, RECIPIENT,
            btcnull::EmptyProof());
        CHECK(model.Apply(bitcoin, first_open));
        const auto close = MakeClose(
            bitcoin, model.state, model.current_reserve_tx);
        CHECK(model.Apply(bitcoin, close));
        CHECK(model.state.status == reserve::Status::EMPTY);
        CHECK(model.state.transition_count == 2);
        CHECK(model.state.transition_commitment !=
              reserve::EmptyTransitionCommitment());

        auto reopen = MakeOpen(
            bitcoin, model.state, 20'000, 20'000, RECIPIENT,
            btcnull::EmptyProof());
        ...
        for (OnChainTokenLedger* ledger : {&public_ledger, &issuer_ledger}) {
            CHECK(ledger->ProcessBlock(
                ReserveBlock(1, {first_open}), peg_gate));
            CHECK(ledger->ProcessBlock(
                ReserveBlock(2, {close}), peg_gate));
            CHECK(ledger->GetBtcVeldReserveState().status ==
        ...
        const Transaction issuer_carrier =
            SignedIssuerReserveCarrier(reopen, issuer);
        ...
        const std::string public_payload(
            public_payload_bytes.begin(), public_payload_bytes.end());
        const auto issuer_payload_bytes =
            btcspv::ExtractOpReturn(issuer_carrier.outputs[0].script_pubkey);
        const auto issuer_operation = DecodeTokenOp(std::string(
            issuer_payload_bytes.begin(), issuer_payload_bytes.end()));
        CHECK(public_payload ==
              std::string(reserve::PUBLIC_CARRIER_PREFIX) +
                  BytesToHex(canonical_reopen));
        CHECK(issuer_operation.has_value());
        CHECK(issuer_operation->memo ==
              std::string(reserve::ISSUER_MEMO_PREFIX) +
                  BytesToHex(canonical_reopen));
        const auto direct_reopen = reserve::Verify(
            bitcoin.chain, public_ledger.GetBtcVeldReserveState(),
            static_cast<uint64_t>(
                public_ledger.GetSupply(BTCVELD_TOKEN_ID)),
            canonical_reopen.data(), canonical_reopen.size(),
        ...
        if (!direct_reopen.ok)
            throw std::runtime_error(
                "direct post-CLOSE re-OPEN verification failed: " +
                direct_reopen.reason);
        CHECK(direct_reopen.recipient == RECIPIENT);
        const auto public_before_reopen_accumulator =
            public_ledger.GetBtcVeldMintAccumulator();
        CHECK(public_before_reopen_accumulator.root == model.nullifier_root);
        CHECK(public_before_reopen_accumulator.count ==
              model.nullifier_events.size());
        CHECK(btcnull::Insert(
                  public_before_reopen_accumulator.root,
                  direct_reopen.pending_outpoint,
                  direct_reopen.claim.nullifier_proof).ok);
        CHECK(public_ledger.ValidateMempoolCandidate(
            public_carrier, 3, 0x207fffffu, peg_gate));
        CHECK(issuer_ledger.ValidateMempoolCandidate(
            issuer_carrier, 3, 0x207fffffu, peg_gate));
        CHECK(public_ledger.ProcessBlock(
            TransactionsBlock(3, {public_carrier}, "public-reopen"),
            peg_gate));
        CHECK(issuer_ledger.ProcessBlock(
            TransactionsBlock(3, {issuer_carrier}, "issuer-reopen"),
            peg_gate));
        ...
        CHECK(issuer_state.reserve_value_sats ==
              public_state.reserve_value_sats);
        ...
        CHECK(issuer_state.open_redemption_principal ==
              public_state.open_redemption_principal);
        CHECK(issuer_state.transition_commitment ==
              public_state.transition_commitment);
        CHECK(issuer_state.reserve_bitcoin_block ==
              public_state.reserve_bitcoin_block);
        CHECK(issuer_state.processed_veld_height ==
              public_state.processed_veld_height);
        ...
        CHECK(public_state.reserve_value_sats ==
              static_cast<uint64_t>(public_ledger.GetSupply(BTCVELD_TOKEN_ID)) +
                  public_state.open_redemption_principal +
                  public_state.surplus_sats);
        const auto public_accumulator =
            public_ledger.GetBtcVeldMintAccumulator();
        const auto issuer_accumulator =
            issuer_ledger.GetBtcVeldMintAccumulator();
        ...
        auto competing = MakeOpen(
            bitcoin, model.state, 21'000, 21'000, RECIPIENT_2,
            btcnull::EmptyProof());
        const std::string competing_outpoint = btcspv::BtcDepositOutpointId(
            Hash256d(competing.bitcoin_tx),
            competing.claim.new_reserve_vout);
        competing.claim.nullifier_proof =
            model.WitnessFor(competing_outpoint);
        ...
        const auto public_state_before =
            public_ledger.GetBtcVeldReserveState();
        const auto issuer_state_before =
            issuer_ledger.GetBtcVeldReserveState();
        const int64_t public_supply_before =
            public_ledger.GetSupply(BTCVELD_TOKEN_ID);
        const int64_t issuer_supply_before =
            issuer_ledger.GetSupply(BTCVELD_TOKEN_ID);
        CHECK(!public_ledger.ProcessBlock(
            ReserveBlock(4, {competing}), peg_gate));
        CHECK(!issuer_ledger.ProcessBlock(
            TransactionsBlock(
                4, {SignedIssuerReserveCarrier(competing, issuer)},
                "issuer-stale-reopen"),
            peg_gate));
        ...
        CHECK(public_ledger.GetSupply(BTCVELD_TOKEN_ID) ==
              public_supply_before);
        CHECK(issuer_ledger.GetSupply(BTCVELD_TOKEN_ID) ==
              issuer_supply_before);
        CHECK(public_ledger.GetBtcVeldMintAccumulator().root ==
              public_accumulator.root);
        CHECK(issuer_ledger.GetBtcVeldMintAccumulator().root ==
              issuer_accumulator.root);

        std::cout << "PASS daybreak_reopen_ledger_tests checks=" << g_checks
                  << " transition_count=" << public_state.transition_count
                  << " supply=20000" << '\n';
        return 0;
    } catch (const std::exception& error) {
        std::cerr << "FAIL daybreak_reopen_ledger_tests: "
                  << error.what() << '\n';
        return 1;
    }
}
