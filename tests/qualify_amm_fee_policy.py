...
def main():
    ...
    args=p.parse_args();out=pathlib.Path(args.out);out.mkdir(exist_ok=False)
    root=pathlib.Path(__file__).resolve().parents[1];rng=random.Random(3200919)
    cases=[]
    ...
    def run(label,exe):
        ...
        if r.returncode:raise RuntimeError((label,r.returncode,r.stderr[-1000:]))
        rows=[s.split() for s in r.stdout.splitlines()];assert len(rows)==len(lines)
        return [[int(x) if i<9 else x for i,x in enumerate(row)] for row in rows]
    baseline=run('baseline',args.baseline);public=run('public',args.public);candidate=run('candidate',args.candidate)
    ...
    for i,t in enumerate(cases+states):
        ...
        if h<9000:
            ...
            assert public[i]==candidate[i],('public activation mismatch',t,public[i],candidate[i])
            public_flat+=1
        if h<100:
            assert n==baseline[i],('historical drift',t,n,baseline[i]);history+=1
        else:
        ...
        if h>0 and v>0 and b>0 and av>0 and ab>0 and a>0 and max(v,b)+a<2**63:
            ...
            if (rout*a//(rin+a))*9970//10000>0:
                js.append(dict(input=[str(x) if j<5 else x for j,x in enumerate(t)],native=[str(x) if j in (3,4,5,6,7) else x for j,x in enumerate(n)],public=[str(x) if j in (3,4,5,6,7) else x for j,x in enumerate(public[i])]))
    (out/'wallet-vectors.json').write_text(json.dumps(js))
    cmd=[args.node,str(root/'tests/amm_fee_wallet_tests.js'),str(root/'include/network/ui_desktop.h'),str(out/'wallet-vectors.json')]
    r=subprocess.run(cmd,text=True,capture_output=True,timeout=90)
    (out/'wallet.stdout').write_text(r.stdout);(out/'wallet.stderr').write_text(r.stderr)
