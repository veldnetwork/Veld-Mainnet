...
int main(int argc, char** argv) {
    ...
    try {
        ...
        std::string error;
        Require(
            btc_buy::StrictJsonParser(frame.data(), 2 * 1024 * 1024, true).Parse(request, error),
            "lab request JSON");
        if (action == "endorse") {
            ...
            std::cout << ValidatorRegistry::BuildEndorseOp(height, hash, BytesToHex(signature),
                                                           key.address, height + 1)
                      << '\n';
            return 0;
        }
        if (action == "finality") {
            namespace fq = veld::finality::qc;
            Fields(request, {"epoch", "set_root", "phase", "target_height", "target_hash",
            ...
            auto hash = [&](const char* field) {
                ...
                Require(value.size() == 64 &&
                        ...
                        "canonical finality hash");
                return HexToHash(value);
            };
            fq::SignedVote vote;
            vote.epoch_id = Amount(*request.Get("epoch"), UINT64_MAX - 1);
            vote.set_root = hash("set_root");
            const auto phase = Amount(*request.Get("phase"), 2);
            Require(phase >= 1, "finality phase");
            vote.phase = static_cast<fq::Phase>(phase);
            vote.target = {Amount(*request.Get("target_height"), UINT64_MAX - 1),
                           hash("target_hash")};
            vote.source = {Amount(*request.Get("source_height"), UINT64_MAX - 1),
                           hash("source_hash")};
            vote.round = fq::CheckpointRound(vote.target.height);
            Require(fq::IsScheduledCheckpoint(vote.target.height) &&
                        fq::EpochOf(vote.target.height) == vote.epoch_id &&
                        fq::SourceRefWellFormed(vote.source, vote.target),
                    "finality checkpoint context");
            vote.pubkey_hex = BytesToHex(key.public_key.data(), key.public_key.size());
            const auto genesis_bytes = HexToBytes(GENESIS_HASH);
            Hash256 genesis{};
            ...
            struct Wipe {
                dilithium::SecretKey& value;
                ~Wipe() {
                    compat::SecureZero(value.data(), value.size());
                }
            } wipe{secret};
            dilithium::PublicKey derived{};
            Require(veld_mldsa65_keypair_from_seed(key.private_key.data(), derived.data(),
                                                   secret.data()) == 0 &&
                        derived == key.public_key,
                    "disposable seed expansion");
            vote.signature = dilithium::Sign(
                secret, fq::VotePreimage(fq::NETWORK_ID, genesis, vote.epoch_id, vote.set_root,
                                         vote.phase, vote.round, vote.source, vote.target));
            const auto wire = fq::EncodeSignedVoteWire(vote);
            Require(!wire.empty(), "canonical finality wire");
        ...
        const auto unsigned_bytes = HexToBytes(encoded);
        Transaction tx;
        Require(Transaction::Deserialize(unsigned_bytes, 0, tx) == unsigned_bytes.size(),
        ...
        if (!bond.empty()) {
            ...
            Require(tx.outputs.size() == 3 &&
                        tx.outputs[0].script_pubkey == AddressToScript(STAKE_VAULT_ADDRESS) &&
                        tx.outputs[1].script_pubkey == key.GetP2PKHScript() &&
                        amount <= tx.outputs[0].value,
                    "adversarial bond fixture shape");
            tx.outputs[1].value += tx.outputs[0].value - amount;
        ...
        if (reduce) {
            Require(tx.outputs.size() == 3 && tx.outputs[1].script_pubkey == key.GetP2PKHScript() &&
                    ...
                    "adversarial spent-input fixture shape");
            tx.outputs[1].value -= reduce;
        }
        std::vector<std::vector<uint8_t>> signatures;
        for (uint32_t i = 0; i < tx.inputs.size(); ++i)
            signatures.push_back(key.SignInput(tx, i, key.GetP2PKHScript()).script_sig);
        for (size_t i = 0; i < signatures.size(); ++i)
            tx.inputs[i].script_sig = std::move(signatures[i]);
        tx.InvalidateTxIDCache();
        const auto raw = tx.Serialize();
        Require(raw.size() <= 1024 * 1024, "native transaction size bound");
