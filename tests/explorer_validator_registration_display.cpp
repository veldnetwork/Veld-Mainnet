...
int main(int argc, char** argv) {
    ...
    request.method = "GET";
    request.path = "/validators";
    request.path_parts = {"validators"};
    ...
    for (auto stake : {uint64_t{0}, VALIDATOR_UNLOCK_STAKED - 1}) {
        ...
        assert(response.status_code == 200 &&
               response.body.find("Validator Registration") != std::string::npos);
    ...
    request.path = "/rules";
    request.path_parts = {"rules"};
    auto rules = service.Route(request);
    ...
    if (argc == 2) {
        std::ofstream(std::filesystem::path(argv[1]) / "validator-open.html") << open.body;
    }
    std::cout
        << "PASS real Explorer registration display: unknown, zero and below-floor, open with zero validators, and unchanged 9500 boundary\n";
}
