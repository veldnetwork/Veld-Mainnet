...
int main(int argc, char** argv) {
    ...
    try {
        if (argc != 2)
            throw std::runtime_error("new disposable directory required");
        fs::path root = argv[1];
        if (fs::exists(root))
            throw std::runtime_error("preserve prior fixture");
        std::string error;
        if (!veld::channel::secure_file::EnsurePrivateDirectory(root.string(), &error))
            throw std::runtime_error(error);
        fs::create_directories(root / "db");
        fs::create_directories(root / "blocks");
        {
            veld::db::LevelDBStore store((root / "db/index").string(), true);
            if (!store.Put("fixture:rejected-history", "retained"))
                throw std::runtime_error("database fixture write");
        }
        const auto write = [&](const char* name, const char* body) {
            if (!veld::channel::secure_file::AtomicWriteText((root / name).string(), body, &error,
                                                             true))
                throw std::runtime_error(error);
        };
        ...
        for (const auto& item : fs::directory_iterator(root))
            if (item.path().filename().string().starts_with(".snapshot-rejected-")) {
                ...
                if (store.Get("fixture:rejected-history") != "retained")
                    throw std::runtime_error("rejected history not preserved");
            }
        if (quarantines != 1 || fs::exists(root / "db/index"))
            throw std::runtime_error(
                "recovery request left rejected database in the active namespace");
        if (!fs::exists(root / ".snapshot-recovery-requested"))
            throw std::runtime_error("recovery request retired before IBD");
