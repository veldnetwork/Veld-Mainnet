#!/usr/bin/env python3
"""Parent process for the Work-admission real-entrypoint matrix.
...
def run_matrix(fixture: Path, output: Path, timeout: int) -> list[dict]:
    ...
    for row in ROWS:
        ...
        records = [
            line[len("WORK_ADMISSION_ROW_JSON ") :]
            for line in stdout.splitlines()
            if line.startswith("WORK_ADMISSION_ROW_JSON ")
        ]
        if len(records) != 1:
...
def main() -> int:
    ...
    summary = {
        "schema": "VELD_WORK_ADMISSION_PROCESS_MATRIX_V1",
        ...
        "public_testnet_terminal_exception": {
            "not_admission_gate_induced": True,
            "reason": (
    ...
    print(
        "PASS work_admission_process_tests "
...
if __name__ == "__main__":
    try:
        ...
        print(f"FAIL work_admission_process_tests: {exc}", file=sys.stderr)
        raise SystemExit(1)
