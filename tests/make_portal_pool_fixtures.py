...
def create(root):
    ...
    n = signer.public_key().public_numbers()
    x = b64(n.x.to_bytes(32, 'big'))
    y = b64(n.y.to_bytes(32, 'big'))
    kid = hashlib.sha256(f'VELD_PORTAL_KEY_V1\n{x}\n{y}'.encode()).hexdigest()
    now = int(time.time())
    for seq, (name, action) in enumerate(
        (
            ('pool.start', 'pool.start'),
            ('pool.start-ready', 'pool.start'),
            ('pool.start-actual', 'pool.start'),
            ('pool.stop', 'pool.stop'),
            ('pool.restart', 'pool.start'),
            ('pool.final-stop', 'pool.stop'),
        ),
        1,
    ):
        ...
        command = dict(
            ...
            payload={},
            key_id=kid,
            signature=b64(r.to_bytes(32, 'big') + min(s, order - s).to_bytes(32, 'big')),
        )
        reply = dict(
            portal_protocol=4,
            device_id=17,
            ...
            pair_expires=0,
            report_interval=5,
            command_key=dict(id=kid, x=x, y=y),
            command=command,
        )
    ...
    version = int(
        re.search(
            r'VELD_ADDR_VERSION_MAINNET\s*=\s*(0x[0-9a-fA-F]+)',
            (source / 'include/core/script.h').read_text(),
        ).group(1),
        16,
    )
    ...
    while number:
        ...
        address = alphabet[r] + address
    (root / 'client.json').write_text(
        json.dumps(
            dict(
                endpoint='https://pool-fixture.invalid',
