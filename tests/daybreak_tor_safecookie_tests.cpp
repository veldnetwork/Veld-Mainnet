...
std::array<uint8_t, 32> IndependentHmac(
...
public:
    FakeServer(Scenario scenario, const std::array<uint8_t, 32>& cookie,
        ...
        if (::bind(listener_, reinterpret_cast<sockaddr*>(&address),
                   sizeof(address)) != 0 || ::listen(listener_, 1) != 0) {
    ...
    ~FakeServer() {
        if (compat::IsValidSocket(listener_)) VELD_CLOSE_SOCKET(listener_);
...
private:
    void Serve() {
        ...
        static constexpr char challenge_prefix[] =
        ...
        result_.saw_challenge = net::_tor::DecodeExactHex(
            challenge.substr(sizeof(challenge_prefix) - 1U, 64U),
            client_nonce);
        ...
        auto server_hash = IndependentHmac(
            server_key, cookie_, client_nonce, server_nonce_);
        if (scenario_ == Scenario::WrongServerHash) server_hash[0] ^= 0x80U;
        const std::string reply =
            "250 AUTHCHALLENGE SERVERHASH=" + net::_tor::Hex(server_hash) +
        ...
        if (scenario_ == Scenario::WrongServerHash ||
            scenario_ == Scenario::ReplayedServerNonce) {
            VELD_CLOSE_SOCKET(client);
            return;
        }
        ...
        if (authenticate.rfind(authenticate_prefix, 0) != 0 ||
            authenticate.size() !=
                sizeof(authenticate_prefix) - 1U + 64U + 2U) {
            VELD_CLOSE_SOCKET(client);
            return;
        }
        std::array<uint8_t, 32> supplied_controller_hash{};
        if (!net::_tor::DecodeExactHex(
                authenticate.substr(sizeof(authenticate_prefix) - 1U, 64U),
        ...
        const auto expected_controller_hash = IndependentHmac(
            controller_key, cookie_, client_nonce, server_nonce_);
        result_.controller_hash_valid = compat::ConstantTimeEqual(
            supplied_controller_hash.data(), expected_controller_hash.data(),
...
fs::path MakeCookieDirectory(const fs::path& base, const std::string& name,
    ...
    std::string error;
    if (!channel::secure_file::EnsurePrivateDirectory(directory.string(),
                                                       &error))
        throw std::runtime_error("private Tor test directory: " + error);
    if (!bytes.empty() && !channel::secure_file::AtomicWriteNew(
            (directory / "control_auth_cookie").string(), bytes, &error,
            true))
        throw std::runtime_error("Tor test cookie: " + error);
    return directory;
}

bool RunStart(net::TorController& controller, FakeServer& server,
              const fs::path& directory) {
    return controller.Start(28333, directory.string(), "127.0.0.1",
                            server.port());
}
...
int main(int argc, char** argv) {
    try {
        ...
        const fs::path correct_dir = MakeCookieDirectory(
            base, "correct", cookie_bytes);
        ...
        const fs::path short_dir = MakeCookieDirectory(
            base, "short", std::vector<uint8_t>(31U, 0x41U));
        net::TorController short_controller;
        FakeServer short_server(Scenario::StopAfterProtocol, cookie,
                                server_nonce);
        ...
        FakeServer wrong_hash_server(Scenario::WrongServerHash, cookie,
                                     server_nonce);
        CHECK(!RunStart(wrong_hash_controller, wrong_hash_server,
                        correct_dir));
        ...
        FakeServer malformed_server(Scenario::MalformedProtocol, cookie,
                                    server_nonce);
        ...
        FakeServer trailing_method_server(
            Scenario::TrailingEmptyMethod, cookie, server_nonce);
        CHECK(!RunStart(trailing_method_controller, trailing_method_server,
                        correct_dir));
        ...
        FakeServer prime_server(Scenario::AuthenticateThenRejectOnion, cookie,
                                server_nonce);
        ...
        FakeServer replay_server(Scenario::ReplayedServerNonce, cookie,
                                 server_nonce);
        ...
        const fs::path outside_root = MakeCookieDirectory(
            base, "outside", cookie_bytes);
        bool link_created = false;
#ifdef _WIN32
        link_created = ::CreateHardLinkW(
            (link_root / "control_auth_cookie").c_str(),
            (outside_root / "control_auth_cookie").c_str(), nullptr) != 0;
#else
        std::error_code link_error;
        fs::create_symlink(outside_root / "control_auth_cookie",
                           link_root / "control_auth_cookie", link_error);
        link_created = !link_error;
#endif
        CHECK(link_created);
        net::TorController link_controller;
        FakeServer link_server(Scenario::StopAfterProtocol, cookie,
                               server_nonce);
        ...
        const fs::path junction_target = MakeCookieDirectory(
            base, "junction-target", cookie_bytes);
        ...
        FakeServer junction_server(Scenario::StopAfterProtocol, cookie,
                                   server_nonce);
        ...
        FakeServer fifo_server(Scenario::StopAfterProtocol, cookie,
                               server_nonce);
        ...
        std::cout << "PASS daybreak_tor_safecookie_tests checks="
                  << g_checks << "\n";
        return 0;
    } catch (const std::exception& error) {
        std::cerr << "FAIL daybreak_tor_safecookie_tests: "
                  << error.what() << "\n";
        return 1;
    }
}
