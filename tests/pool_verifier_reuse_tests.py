...
def main():
    ...
    for _ in range(2):
        for height, header, proof in rows:
            ...
            value = int(proof, 16)
            kind = ('block' if value < target else 'near_miss'
                    if target < value <= 4 * target < 1 << 256 else 'none')
    ...
    print(f'PASS {len(commands)} native requests: 28 frozen vectors, repeat, inspect, rejection and recovery')
