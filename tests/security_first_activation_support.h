...
class FirstActivationSupport {
    ...
    std::optional<fa_fq::EpochSnapshot> Snapshot(uint64_t epoch) {
        ...
        if (!fa_fq::SnapshotWellFormed(out))
            throw std::runtime_error("noncanonical RPC snapshot");
        return out;
    }
    void StartVoters() {
        if (!daemons_.empty())
        ...
        for (size_t index = 1; index < wallets_.size(); ++index) {
            ...
            const std::string public_key =
                BytesToHex(wallet.public_key.data(), wallet.public_key.size());
            const auto path =
                directory_ / ("finality-" + std::string(GENESIS_HASH).substr(0, 16) + "-" +
                              HashToHex(fa_fq::PubkeyCommit(public_key)) + ".hex");
            ...
            hooks.fetch_block_hash = [this](uint64_t h) -> std::optional<Hash256> {
                if (h > node_.GetChain().Height())
                    return std::nullopt;
                return node_.GetChain().GetBlock(h).GetHash();
            };
            hooks.authorize_work =
                [this](
                    const fa_fq::CheckpointRef& target) -> std::optional<fa_fq::DaemonWorkGrant> {
                const auto grant = Result(
                    node_, "getworkadmission",
                    {"finality_vote", std::to_string(target.height), HashToHex(target.hash)});
                fa_fq::DaemonWorkGrant out;
                out.binding = Field(grant, "binding").text;
                out.token = Field(grant, "signing_token").text;
                const auto activation =
                    Result(node_, "beginworksigning", {"finality_vote", out.binding, out.token});
                out.deadline = std::chrono::steady_clock::now() +
                               std::chrono::milliseconds(Number(Field(activation, "ttl_ms")));
                return out;
            };
            hooks.cancel_work = [this](const fa_fq::DaemonWorkGrant& grant) {
                (void)Result(node_, "cancelworksigning", {grant.token});
            };
            hooks.gossip_vote = [this](const fa_fq::SignedVote& vote,
                                       const fa_fq::DaemonWorkGrant& grant) {
                const auto snapshot = Snapshot(vote.epoch_id);
                if (!snapshot ||
                    !fa_fq::VerifyVote(vote, *snapshot, fa_fq::NETWORK_ID, HexToHash(GENESIS_HASH)))
                    throw std::runtime_error(
                        "disposable vote does not verify against the node's compiled domain");
                const auto accepted = Result(
                    node_, "submitfinalityvote",
                    {BytesToHex(fa_fq::EncodeSignedVoteWire(vote)), grant.binding, grant.token});
            ...
            hooks.persist_journal = [path](const fa_fq::DaemonJournal& journal) {
                ...
                if (bytes.empty())
                    return false;
                std::string error;
                return channel::secure_file::AtomicWriteText(path.string(), BytesToHex(bytes),
                                                             &error, true);
            };
            hooks.load_journal = [path]() -> std::optional<fa_fq::DaemonJournal> {
                if (!std::filesystem::exists(path))
                    return std::nullopt;
                if (std::filesystem::file_size(path) > 100000)
                    throw std::runtime_error("oversized finality journal");
                std::ifstream input(path);
                std::string hex;
                input >> hex;
                auto journal = DecodeJournal(HexToBytes(hex));
                if (!input || !journal)
                    throw std::runtime_error("invalid finality journal");
                return journal;
            };
            hooks.fetch_finalized = [this]() -> std::optional<fa_fq::FinalizedRecord> {
                ...
                if (snapshot.kind == Value::Kind::Null)
                    return std::nullopt;
                const auto& record = Field(snapshot, "finalized");
                if (record.kind == Value::Kind::Null)
                    return std::nullopt;
            ...
            hooks.fetch_prevote_qc =
                [this](const fa_fq::EpochSnapshot&) -> std::optional<fa_fq::DecodedQc> {
            ...
            if (veld_mldsa65_keypair_from_seed(wallet.private_key.data(), expanded_public.data(),
                                               expanded_secret.data()) != 0 ||
                !std::equal(expanded_public.begin(), expanded_public.end(),
                            wallet.public_key.begin()))
                throw std::runtime_error("finality identity expansion mismatch");
            daemons_.push_back(std::make_unique<fa_fq::FinalityDaemon>(
                public_key, expanded_secret, fa_fq::NETWORK_ID, HexToHash(GENESIS_HASH),
                std::move(hooks)));
    ...
    btcveld::reserve::Claim Claim(const Value& command) {
        ...
        if (operation == "OPEN")
            claim.operation = r::Operation::OPEN;
        else if (operation == "DEPOSIT")
            claim.operation = r::Operation::DEPOSIT;
        else if (operation == "PAYOUT")
            claim.operation = r::Operation::PAYOUT;
        else
            throw std::runtime_error("unsupported first activation operation");
        ...
        if (operation == "OPEN")
            claim.exact_commitment =
                r::detail::OpenDepositCommitment(0, claim.new_reserve_value, recipient);
        if (operation == "DEPOSIT")
            claim.exact_commitment = r::detail::PendingDepositCommitment(
                HexToHash(Field(command, "pending_txid").text),
                uint32_t(Number(Field(command, "pending_vout"))),
                Number(Field(command, "pending_value")), recipient);
        if (operation == "PAYOUT") {
            const auto request = node_.TestMainBtcVeldRedeemCovenant().GetCopy(
                HexToHash(Field(command, "request_id").text));
            if (!request)
                throw std::runtime_error("unknown on-chain redeem request");
            claim.exact_commitment = request->request_commitment;
            if (claim.new_reserve_value == 0)
                claim.new_reserve_vout = r::NO_VOUT;
        }
        return claim;
    }

  public:
    FirstActivationSupport(VeldNode& node, std::vector<RealKeyPair>& wallets,
    ...
    std::string Handle(const std::string& op, const Value& command) {
        ...
        if (op == "fa_finality") {
            ...
            return JsonBuilder::Object(
                {{"votes_sent", JsonBuilder::Number(uint64_t(sent))},
                 ...
                 {"verification_result",
                  JsonBuilder::Number(uint64_t(node_.TestWorkFinalityVerifyResult()))},
                 {"precommit_qc_bytes",
                  JsonBuilder::Number(uint64_t(Field(qc, "qc_hex").text.size() / 2))}});
        }
        if (op == "fa_anchor") {
            const auto raw = HexToBytes(Field(command, "proof").text);
            if (raw.empty() || raw.size() > 12000)
                throw std::runtime_error("anchor proof fixture size");
            return SignAndSend(node_, wallet, "preparerawop",
                               {wallet.address, "VELD_ANCHOR|" + BytesToHex(raw)});
        }
        if (op == "fa_request") {
            const auto request = node_.TestMainBtcVeldRedeemCovenant().GetCopy(
                HexToHash(Field(command, "request_id").text));
            if (!request)
                throw std::runtime_error("request absent from hot and archived state");
            return JsonBuilder::Object(
                {{"status", JsonBuilder::Number(uint64_t(request->status))},
        ...
        if (op == "fa_archive_status") {
            ...
            return JsonBuilder::Object(
                {{"migration_active", JsonBuilder::Bool(state.state_migration_active)},
                 ...
                 {"hot_payout_identities",
                  JsonBuilder::Number(uint64_t(state.consumed_payouts.size()))},
        ...
        if (op == "fa_headers") {
            ...
            for (const auto& item : Field(command, "headers").array) {
                ...
                if (raw.size() != 80 || headers.size() >= 100)
                    throw std::runtime_error("header fixture bound");
                std::array<uint8_t, 80> header{};
                std::copy(raw.begin(), raw.end(), header.begin());
                headers.push_back(header);
            }
            return SignAndSend(
                node_, wallet, "preparerawop",
                {wallet.address, "VELD_BHDR|" + BytesToHex(btcspv::EncodeBtcHeaderOp(headers))});
        }
        if (op == "fa_auth" || op == "fa_reserve") {
            ...
            if (op == "fa_auth") {
                const auto script =
                    claim.operation == r::Operation::OPEN
                        ? r::detail::OpenAuthScript(claim.prior_commitment, wallet.address,
                                                    claim.mint_amount)
                        : r::detail::AuthScript(claim.operation, claim.prior_commitment,
                                                claim.exact_commitment, claim.mint_amount);
                if (script.empty())
                    throw std::runtime_error("cannot construct standard reserve authorization");
                return JsonBuilder::Object({{"script", JsonBuilder::String(BytesToHex(script))},
                                            {"payload", JsonBuilder::String(BytesToHex(
                                                            btcspv::ExtractOpReturn(script)))}});
            }
            claim.bitcoin_tx = HexToBytes(Field(command, "rawtx").text);
            claim.bitcoin_txid = HexToHash(Field(command, "txid").text);
            claim.new_reserve_txid =
                claim.new_reserve_vout == r::NO_VOUT ? Hash256{} : claim.bitcoin_txid;
            claim.bitcoin_block = HexToHash(Field(command, "block").text);
            claim.merkle_directions = Number(Field(command, "directions"));
            for (const auto& item : Field(command, "branch").array)
                claim.merkle_branch.push_back(HexToHash(item.text));
            for (const auto& item : Field(command, "parents").array)
                claim.direct_parents.push_back(HexToBytes(item.text));
            if (claim.mint_amount) {
                claim.has_nullifier_proof = true;
                if (!btcnull::DecodeProof(HexToBytes(Field(command, "nullifier_proof").text),
                                          claim.nullifier_proof))
                    throw std::runtime_error("invalid RPC nullifier witness");
            }
            const auto proof = r::EncodeProof(claim);
            if (proof.empty())
                throw std::runtime_error("reserve fixture encoding refused");
            return SignAndSend(node_, wallet, "preparerawop",
                               {wallet.address, "VELD_RSV1|" + BytesToHex(proof)});
        }
        if (op == "fa_transaction") {
            const auto method = Field(command, "method").text;
            const std::set<std::string> allowed = {"prepareammseed",     "prepareammadd",
                                                   "prepareammswap",     "prepareammremove",
                                                   "preparetokenredeem", "preparetokentransfer"};
            if (!allowed.count(method))
                throw std::runtime_error("unsupported first activation transaction");
            std::vector<std::string> params{wallet.address};
            for (const auto& p : Field(command, "params").array)
...
FirstActivationSupport::EncodeJournal(const fa_fq::DaemonJournal& journal) {
    ...
    if (journal.last_vote) {
        ...
        if (vote.size() != fa_fq::SIGNED_VOTE_WIRE_BYTES)
            return {};
...
inline std::optional<fa_fq::DaemonJournal>
FirstActivationSupport::DecodeJournal(const std::vector<uint8_t>& in) {
    constexpr size_t BASE = 4 + 4 + 1 + 8 + 4 + 8 + 32 + 1;
    if (in.size() != BASE + 32 && in.size() != BASE + fa_fq::SIGNED_VOTE_WIRE_BYTES + 32)
        return std::nullopt;
