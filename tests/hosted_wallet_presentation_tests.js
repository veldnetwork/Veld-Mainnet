...
const root = path.resolve(__dirname, '..'),
    dir = path.join(root, 'website', 'hosted-wallet');
...
for (const match of overlay.matchAll(/^sub_filter '([^']*)' '([^']*)';$/gm)) {
    assert(
        ...
        'Only static insertion/label/theme metadata rules are permitted',
    );
    rendered = rendered.split(match[1]).join(match[2]);
    count++;
}
assert.equal(count, 4);
assert(
    overlay.includes('content="#070B08"><meta name="color-scheme" content="dark light">'),
    'Legacy wallet chrome starts dark before scripts load',
);
const scripts = (html) =>
    [...html.matchAll(/<script\b[^>]*>([\s\S]*?)<\/script>/gi)].map((m) => m[1]).filter(Boolean);
assert.deepEqual(
    scripts(rendered),
    scripts(native),
    'Hosted overlay must never rewrite native JavaScript',
);
for (const text of scripts(rendered)) new vm.Script(text);
for (const [name, expected] of Object.entries(
    JSON.parse(fs.readFileSync(path.join(dir, 'asset-sha256.json'), 'utf8')),
)) {
...
for (const state of ['loading', 'complete']) {
    ...
    assert.equal(calls, state === 'complete' ? 1 : 0);
    if (callback) callback();
    assert.equal(calls, 1);
}
vm.runInNewContext(bootstrap, { window: {}, document: { readyState: 'complete' } });
console.log(
    'PASS hosted wallet static overlay, native script parity, captured assets and navigation bootstrap',
);
