...
Child Spawn(const std::wstring& executable, const std::vector<std::wstring>& arguments,
            HANDLE input) {
...
void Drain(Child& child, std::string& output) {
    for (;;) {
        DWORD available = 0;
        if (!::PeekNamedPipe(child.output.value, nullptr, 0, nullptr, &available, nullptr) ||
            available == 0)
            return;
        std::vector<char> bytes(available);
        DWORD count = 0;
        if (!::ReadFile(child.output.value, bytes.data(), available, &count, nullptr))
            return;
        output.append(bytes.data(), count);
    }
}

DWORD WaitForExit(Child& child, std::string& output, DWORD timeout_ms = 30000) {
...
std::string CaptureProcessCommandLine(DWORD pid) {
    ...
    const std::wstring powershell = std::wstring(system_directory.data(), length) +
                                    L"\\WindowsPowerShell\\v1.0\\powershell.exe";
    const std::wstring expression =
        L"$p=Get-CimInstance Win32_Process -Filter 'ProcessId = " + std::to_wstring(pid) +
    ...
    Child inspector = Spawn(
        powershell, {L"-NoProfile", L"-NonInteractive", L"-Command", expression}, null_input.value);
    std::string output;
...
RunResult RunProtectedPipe(const std::wstring& executable, const fs::path& output_path,
                           const std::optional<std::string>& input, bool inspect_command = false) {
    SECURITY_ATTRIBUTES security{};
    security.nLength = sizeof(security);
    security.bInheritHandle = TRUE;
    ...
    const uint64_t numeric_handle =
        static_cast<uint64_t>(reinterpret_cast<uintptr_t>(seed_read.value));
    Child child = Spawn(executable,
                        {L"from-seed", L"--out", output_path.wstring(), L"--seed-input-handle",
                         std::to_wstring(numeric_handle)},
                        seed_read.value);
    seed_read = Handle{};
    ...
    if (input.has_value() && !input->empty()) {
        DWORD written = 0;
        if (!::WriteFile(seed_write.value, input->data(), static_cast<DWORD>(input->size()),
                         &written, nullptr) ||
            written != input->size()) {
...
RunResult RunProtectedHandleArgument(const std::wstring& executable, const fs::path& output_path,
                                     HANDLE candidate) {
    Handle null_input(::CreateFileW(L"NUL", GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE,
                                    nullptr, OPEN_EXISTING, 0, nullptr));
    if (null_input.value == INVALID_HANDLE_VALUE)
        throw std::runtime_error("cannot open NUL");
    const uint64_t numeric_handle = static_cast<uint64_t>(reinterpret_cast<uintptr_t>(candidate));
    Child child = Spawn(executable,
                        {L"from-seed", L"--out", output_path.wstring(), L"--seed-input-handle",
                         std::to_wstring(numeric_handle)},
                        null_input.value);
...
int wmain(int argc, wchar_t** argv) {
    try {
        ...
        const std::string canary = "000102030405060708090a0b0c0d0e0f"
                                   "101112131415161718191a1b1c1d1e1f";
        CHECK(!EnvironmentContains(canary));

        const fs::path success_path = output_dir / L"seed-success.key";
        const RunResult success = RunProtectedPipe(executable, success_path, canary, true);
        ...
        const RunResult regular_result =
            RunProtectedHandleArgument(executable, regular_output, regular.value);
        CHECK(regular_result.exit_code == 2);
        CHECK(!fs::exists(regular_output));
        CHECK(regular_result.output.find("inherited anonymous pipe") != std::string::npos);
        regular = Handle{};
        ...
        if (!::CreatePipe(&noninherit_read_raw, &noninherit_write_raw, &inheritable, 0))
        ...
        if (!::SetHandleInformation(noninherit_read.value, HANDLE_FLAG_INHERIT, 0))
            throw std::runtime_error("cannot protect negative pipe fixture");
        noninherit_write = Handle{};
        const fs::path noninherit_output = output_dir / L"noninherit-handle.key";
        const RunResult noninherit_result =
            RunProtectedHandleArgument(executable, noninherit_output, noninherit_read.value);
        CHECK(noninherit_result.exit_code == 2);
        CHECK(!fs::exists(noninherit_output));
        CHECK(noninherit_result.output.find("inherited anonymous pipe") != std::string::npos);

        const fs::path duplicate_output = output_dir / L"duplicate-handle.key";
        const RunResult duplicate_handle = RunClosedStdin(
            executable, {L"from-seed", L"--out", duplicate_output.wstring(), L"--seed-input-handle",
                         L"1", L"--seed-input-handle", L"1"});
        CHECK(duplicate_handle.exit_code == 2);
        CHECK(!fs::exists(duplicate_output));
        CHECK(duplicate_handle.output.find("argument value suppressed") != std::string::npos);

        const fs::path noncanonical_output = output_dir / L"noncanonical-handle.key";
        const RunResult noncanonical_handle =
            RunClosedStdin(executable, {L"from-seed", L"--out", noncanonical_output.wstring(),
                                        L"--seed-input-handle", L"00"});
        CHECK(noncanonical_handle.exit_code == 2);
        CHECK(!fs::exists(noncanonical_output));
        CHECK(noncanonical_handle.output.find("argument value suppressed") != std::string::npos);

        const RunResult redirected_eof = RunClosedStdin(
            executable, {L"from-seed", L"--out", (output_dir / L"redirected.key").wstring()});
        CHECK(redirected_eof.exit_code == 2);
        CHECK(redirected_eof.output.find("hidden interactive terminal") != std::string::npos);

        const std::string legacy_canary = "LEGACY_TEST_CANARY_NOT_A_SEED";
        const RunResult positional = RunClosedStdin(
            executable, {L"from-seed", std::wstring(legacy_canary.begin(), legacy_canary.end()),
        ...
        const RunResult named =
            RunClosedStdin(executable, {L"from-seed", L"--seed=LEGACY_TEST_CANARY_NOT_A_SEED",
                                        L"--out", (output_dir / L"legacy-named.key").wstring()});
        CHECK(named.exit_code == 2);
        CHECK(named.output.find(legacy_canary) == std::string::npos);
        CHECK(named.output.find("argument value suppressed") != std::string::npos);
        ...
        std::cout << "PASS seed_import_windows_process_tests checks=" << g_checks << "\n";
        return 0;
    } catch (const std::exception& error) {
        std::cerr << "FAIL seed_import_windows_process_tests: " << error.what() << "\n";
        return 1;
    }
}
