...
int main() try {
    ...
    const std::vector<std::pair<std::string,std::string>> cases={
        ...
        {"pool CA unavailable","trust_store"},{"pool refused request","remote_refusal"},
        {"duplicate pool HTTP header","http_response"},{"pool JSON response","response_schema"},
        {"pool job network or version mismatch","work_identity"},
        {"pool balance identity","account_identity"},{"private pool state write failed","private_state"},
        {"pool refused request with SECRET","local_failure"},{std::string(100000,'Z'),"local_failure"}};
    for(const auto& [message,code]:cases) {
        const auto failure=ClassifyFailure(std::runtime_error(message));
        if(failure.code!=code)throw std::runtime_error("wrong bounded category");
    }
    ...
    if(untrusted.find("SECRET")!=std::string::npos || untrusted.find("TOKEN")!=std::string::npos || untrusted.size()>100)
