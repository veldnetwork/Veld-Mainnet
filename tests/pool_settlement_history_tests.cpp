...
Block Child(const Block& parent, const char* label) {
    ...
    for (int i = 0; i < 3; ++i) cb.outputs.emplace_back(1, std::vector<uint8_t>{0x51});
    b.transactions.push_back(cb);
    b.UpdateMerkleRoot();
    return b;
}
void Admit(Blockchain& chain, const Block& b) {
    Check(chain.AddBlockDirect(b, true, true, false,
        mining::PowAdmissionContext::Internal()).IsAccepted(), "synthetic summary fixture admission");
}
int main() {
    try {
        Blockchain chain;
        auto b = CreateGenesisBlock(); Admit(chain, b);
        b = Child(b, "a"); Admit(chain, b);
        const auto parent = b;
        b = Child(b, "a");
        for (uint32_t index = 1; index <= 3; ++index) {
            ...
            tx.outputs.emplace_back(index == 2 ? 1 : 0, index == 2 ? std::vector<uint8_t>{0x51}
                : std::vector<uint8_t>{0x6A, 1, static_cast<uint8_t>(index)});
            b.transactions.push_back(tx);
        }
        b.UpdateMerkleRoot(); Admit(chain, b);
        const auto summary = chain.TestSettlementHistory(2);
        Check(summary == std::vector<std::vector<uint8_t>>{
            b.transactions[0].Serialize(), b.transactions[1].Serialize(), b.transactions[3].Serialize()},
        ...
        b = Child(chain.TipCopy(), "replacement"); Admit(chain, b);
        chain.TestResetBlockBodyLookupCount();
        ...
        std::cout << "PASS immutable settlement facts, exact bytes, bounds, inflow and fork identity; synthetic fixture\n";
