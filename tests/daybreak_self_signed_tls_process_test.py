...
TEST_BINARY = ROOT / "tests" / "daybreak_desktop_rpc_transport_tests.exe"


def main() -> int:
    if not OPENSSL.is_file() or not TEST_BINARY.is_file():
        raise RuntimeError("required local test dependency is missing")
    with tempfile.TemporaryDirectory(prefix="veld-daybreak-self-signed-") as temp:
        temp_path = Path(temp)
        ...
        env["DAYBREAK_SELF_SIGNED_TLS_PORT"] = str(listener.getsockname()[1])
        ...
        print("PASS daybreak_self_signed_tls_process_test accepted=1 rejected=1")
    return 0
