...
static TestResponse RequestWithBody(uint16_t port, const std::string& path,
    ...
    while (offset < wire.size()) {
        const int sent =
            ::send(fd, wire.data() + offset, static_cast<int>(wire.size() - offset), 0);
        if (sent <= 0) {
            VELD_CLOSE_SOCKET(fd);
            return {};
        }
    ...
    TestResponse result;
    try {
        result.status = std::stoi(response.substr(first_space + 1, 3));
    } catch (...) {
        return {};
    }
    const auto separator = response.find("\r\n\r\n");
    if (separator != std::string::npos)
        result.body = response.substr(separator + 4);
    return result;
}

static int Request(uint16_t port, const std::string& path, const std::string& headers = {}) {
    return RequestWithBody(port, path, headers).status;
}

static std::string ProxyHeaders(const std::string& client, const std::string& token) {
...
static veld::Block RewardBlock(veld::Blockchain& chain, const std::vector<uint8_t>& miner_script) {
    ...
    const uint64_t effective = std::min(Blockchain::ExpectedBlockSubsidy(block.height),
...
int main() {
    ...
    const auto directory =
        std::filesystem::temp_directory_path() / ("veld-explorer-proxy-" + std::to_string(nonce));
    std::string error;
    check(veld::channel::secure_file::EnsurePrivateDirectory(directory.string(), &error));
    ...
    check(veld::channel::secure_file::AtomicWriteNew(
        token_path.string(), reinterpret_cast<const uint8_t*>(token_file_bytes.data()),
    ...
    check(veld::channel::secure_file::AtomicWriteNew(
        ...
        reinterpret_cast<const uint8_t*>(proxy_config.data()), proxy_config.size(), &error, true));

    veld::Blockchain chain;
    veld::Mempool mempool;
    check(chain
              .AddBlockDirect(veld::CreateGenesisBlock(), true, true, false,
    ...
    const auto block_page = RequestWithBody(port, "/api/v1/blocks/latest/25", page_headers);
    ...
    const std::string stats_guard = "fetch('/api/stats').then(function(r){if(!r.ok)";
    const auto first_stats_guard = landing_html.body.find(stats_guard);
    check(first_stats_guard != std::string::npos);
    check(landing_html.body.find(stats_guard, first_stats_guard + stats_guard.size()) !=
          std::string::npos);
    ...
    const auto rich_before = RequestWithBody(port, "/api/v1/richlist", page_headers);
    ...
    const uint64_t miner_reward = (veld::Blockchain::ExpectedBlockSubsidy(1) * 50) / 100;
    auto reward = RewardBlock(chain, miner_script);
    check(chain
              .AddBlockDirect(reward, true, true, false,
                              veld::mining::PowAdmissionContext::Internal())
              .IsAccepted());
    std::ostringstream exact_balance;
    exact_balance << std::fixed << std::setprecision(8)
                  << static_cast<double>(miner_reward) / veld::VELD_UNITS;
    const auto rich_after = RequestWithBody(port, "/api/v1/richlist", page_headers);
    ...
    check(rich_after.body.find("\"address\":\"" + std::string(veld::POOL_ADDRESS) +
                               "\",\"balance_veld\":" + exact_balance.str()) != std::string::npos);
    ...
    check(Request(port, "/notfound", ProxyHeaders("198.51.100.9", std::string(64, '0'))) == 403);
    check(Request(port, "/notfound",
                  ProxyHeaders("198.51.100.10", token) + "X-Veld-Client-IP: 198.51.100.11\r\n") ==
          403);
    for (size_t i = 5; i < 60; ++i)
        check(Request(port, "/notfound", ProxyHeaders("198.51.100.9", std::string(64, '0'))) ==
              403);
    check(Request(port, "/notfound", ProxyHeaders("198.51.100.9", std::string(64, '0'))) == 429);
    ...
    check(Request(port, "/api/txhistory?address=invalid", ProxyHeaders("198.51.100.20", token)) ==
          404);
    ...
    check(Request(port, "/address/invalid", ProxyHeaders("::ffff:198.51.100.30", token)) == 429);
    check(Request(port, "/address/invalid", ProxyHeaders("198.51.100.31", token)) == 200);
    ...
    check(global_explorer.ConfigureTrustedProxy("127.0.0.1", token_path.string(), &error));
    check(global_explorer.Start());
    std::this_thread::sleep_for(std::chrono::milliseconds(20));
    const std::array<std::string, 5> routes = {"/api/blocks", "/api/supply", "/api/validators",
                                               "/api/topology", "/api/not-a-route"};
    for (size_t i = 0; i < 6000; ++i) {
        const std::string client =
            "198.18." + std::to_string(i / 256) + "." + std::to_string(i % 256);
        const int status =
            Request(global_port, routes[i % routes.size()], ProxyHeaders(client, token));
        check(status != 0 && status != 429 && status != 503);
    }
    check(Request(global_port, "/api/topology", ProxyHeaders("198.19.0.1", token)) == 429);
    ...
    check(history_explorer.ConfigureTrustedProxy("127.0.0.1", token_path.string(), &error));
    check(history_explorer.Start());
    std::this_thread::sleep_for(std::chrono::milliseconds(20));
    for (size_t i = 0; i < 60; ++i) {
        const std::string client = "203.0.113." + std::to_string(i + 1);
        check(Request(history_port, "/address/invalid", ProxyHeaders(client, token)) == 200);
    }
    check(Request(history_port, "/address/invalid", ProxyHeaders("203.0.113.200", token)) == 429);
    history_explorer.Stop();

    std::error_code cleanup_error;
    std::filesystem::remove_all(directory, cleanup_error);
    check(!cleanup_error);
    std::cout << "PASS explorer_proxy_process_tests checks=" << checks
