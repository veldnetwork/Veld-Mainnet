...
int main(int argc, char** argv) {
    ...
    if (entries.size() != 1 || entries[0].height != 2800)
        return 3;
    auto checkpoint = entries[0];
#if !defined(VELD_PUBLIC_MAINNET)
    if (veld::VerifyCheckpoint(checkpoint))
        return 4;
    ...
    if (!store.Add(checkpoint) || !store.Get(2800).has_value())
        return 7;
    for (int change = 0; change < 5; ++change) {
        ...
        case 3:
            ...
            break;
        case 4:
            changed.signed_at = veld::GENESIS_TIME - 1;
            break;
        }
        if (veld::VerifyCheckpoint(changed) || store.Add(changed))
            return 8 + change;
    }
    if (store.Size() != 1 || store.Latest()->block_hash != checkpoint.block_hash)
        return 13;
