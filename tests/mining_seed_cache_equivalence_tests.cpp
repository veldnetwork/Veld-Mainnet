...
int main() try {
    ...
    for (size_t i = 36; i < 68; ++i)
        header[i] = static_cast<uint8_t>(i * 7 + 3);
    ...
    const auto cached = ComputeEpochSeed(parent, height, target.bits);
    {
        ...
        if (!warm)
            throw std::runtime_error("dataset fixture unavailable");
    }
    ...
    for (uint64_t n = 0; n < 12; ++n) {
        ...
        const auto canonical =
            VeldHashWithDataset<false>(header, height, target, &canonical_workspace);
        ...
        if (!dataset)
            throw std::runtime_error("cached dataset unavailable");
        ...
        if (canonical != optimized || cached != ComputeEpochSeed(parent, height, target.bits))
