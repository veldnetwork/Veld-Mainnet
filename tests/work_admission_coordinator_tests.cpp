...
Subject SubjectFor(Path path) {
    if (path == Path::FinalityVote)
        return FinalitySubject();
...
AdmissionCoordinator::TokenMintFn
CounterMint(const std::shared_ptr<std::atomic<uint64_t>>& counter) {
...
int main() {
    ...
    auto default_attempt = unwired.AcquireLocal(Path::SubmitBlock, BlockSubject(), std::nullopt,
    ...
    const std::vector<std::pair<Refusal, PrerequisiteMutator>> false_prerequisites{
    ...
    const std::vector<Path> every_path{Path::InternalMining,       Path::GetBlockTemplate,
                                       Path::SubmitBlock,          Path::SynchronousGeneration,
                                       Path::ValidatorEndorsement, Path::FinalityVote};
    for (const auto& [expected, mutate] : false_prerequisites) {
        Coordinator matrix(limits, CounterMint(std::make_shared<std::atomic<uint64_t>>(0)), now);
        ...
        for (const Path path : every_path) {
            const auto attempt = matrix.AcquireLocal(path, SubjectFor(path), std::nullopt, false,
                                                     Coordinator::Duration{10});
            Check(!attempt && attempt.decision.refusal == expected,
                  "false prerequisite closes every production path");
        }
        Check(matrix.GetSnapshot().active_leases == 0, "false prerequisite retains no cached work");
    }
    ...
    Check(coordinator.GetSnapshot().phase == Coordinator::Phase::Open, "open phase visible");

    auto invalid_duration = coordinator.AcquireLocal(
        Path::SubmitBlock, BlockSubject(), std::nullopt, false, Coordinator::Duration::zero());
    Check(!invalid_duration && invalid_duration.error == Coordinator::Error::InvalidDuration,
          "zero lease duration rejected");

    auto wrong_path = coordinator.AcquireLocal(Path::FinalityVote, BlockSubject(), std::nullopt,
                                               false, Coordinator::Duration{10});
    Check(!wrong_path && wrong_path.error == Coordinator::Error::PathPurposeMismatch,
          "path and purpose must match");

    auto role_configuration = ReadyConfiguration(coordinator);
    role_configuration.permitted_paths[static_cast<size_t>(Path::SubmitBlock)] = false;
    Check(coordinator.Open(role_configuration).opened,
          "configuration may intentionally deny one role path");
    auto denied_role = coordinator.AcquireLocal(Path::SubmitBlock, BlockSubject(), std::nullopt,
                                                false, Coordinator::Duration{10});
    ...
    auto issued = coordinator.AcquireLocal(Path::SubmitBlock, BlockSubject(), std::nullopt, false,
                                           Coordinator::Duration{10000});
    Check(issued && issued.lease->IsLive(), "valid local lease issued");
    Check(issued.lease->deadline() == Coordinator::TimePoint(Coordinator::Duration{1020}),
    ...
    Check(coordinator.Open(changed_configuration).error == Coordinator::Error::Busy,
          "changed open state cannot replace a live lease");
    const Binding first_binding = issued.lease->binding();
    issued.lease->Release();
    Check(coordinator.GetSnapshot().active_leases == 0, "explicit release removes local lease");

    auto missing_prior = coordinator.AcquireLocal(Path::SubmitBlock, BlockSubject(), std::nullopt,
                                                  true, Coordinator::Duration{10});
    Check(!missing_prior && missing_prior.decision.refusal == Refusal::BindingMissing,
          "required prior binding re-evaluated at acquisition");
    Binding stale = first_binding;
    ++stale.validation_generation;
    auto stale_prior = coordinator.AcquireLocal(Path::SubmitBlock, BlockSubject(), stale, true,
                                                Coordinator::Duration{10});
    Check(!stale_prior && stale_prior.decision.refusal == Refusal::BindingMismatch,
          "stale generation binding rejected at acquisition");

    try {
        auto exceptional = coordinator.AcquireLocal(Path::InternalMining, BlockSubject(),
                                                    std::nullopt, false, Coordinator::Duration{10});
        Check(exceptional && exceptional.lease->ClaimForSink(), "exception fixture acquired lease");
        throw std::runtime_error("fixture");
    } catch (const std::runtime_error&) {
    }
    Check(coordinator.GetSnapshot().active_leases == 0, "lease releases during stack unwinding");
    ...
    auto no_mint_remote = no_mint.IssueRemoteSigningLease(
        Path::FinalityVote, FinalitySubject(), std::nullopt, false, Coordinator::Duration{10});
    Check(!no_mint_remote && no_mint_remote.error == Coordinator::Error::TokenMintUnavailable,
    ...
    Check(!block_remote && block_remote.error == Coordinator::Error::PathPurposeMismatch,
          "remote signing tokens restricted to validator paths");

    const Subject finality = FinalitySubject();
    auto remote = coordinator.IssueRemoteSigningLease(Path::FinalityVote, finality, std::nullopt,
                                                      false, Coordinator::Duration{10000});
    Check(remote && remote.grant->deadline == Coordinator::TimePoint(Coordinator::Duration{1080}),
    ...
    auto forged =
        coordinator.ConsumeRemoteSigningLease(Coordinator::RemoteToken::FromBytes(forged_bytes),
    ...
    auto wrong_subject = coordinator.ConsumeRemoteSigningLease(
        remote.grant->token, Path::FinalityVote, changed_target, remote.grant->binding);
    Check(!wrong_subject && wrong_subject.error == Coordinator::Error::PathPurposeMismatch,
    ...
    auto consumed = coordinator.ConsumeRemoteSigningLease(remote.grant->token, Path::FinalityVote,
                                                          finality, remote.grant->binding);
    Check(consumed && consumed.lease->IsLive(), "exact remote capability converts to local lease");
    ...
    Check(consumed.lease->ClaimForSink(), "consumed remote capability claims sink once");
    Check(!consumed.lease->ClaimForSink(), "consumed remote sink claim cannot repeat");
    ...
    Check(cancellable && coordinator.CancelRemoteSigningLease(cancellable.grant->token),
          "pending remote capability can be released explicitly");
    auto cancelled_use = coordinator.ConsumeRemoteSigningLease(
        cancellable.grant->token, Path::FinalityVote, finality, cancellable.grant->binding);
    Check(!cancelled_use && cancelled_use.error == Coordinator::Error::TokenConsumed,
          "cancelled remote capability remains replay-proof");

    auto expiring = coordinator.IssueRemoteSigningLease(Path::FinalityVote, finality, std::nullopt,
                                                        false, Coordinator::Duration{5});
    ...
    auto expired_use = coordinator.ConsumeRemoteSigningLease(
        expiring.grant->token, Path::FinalityVote, finality, expiring.grant->binding);
    Check(!expired_use, "expired remote capability rejected");

    auto emergency = coordinator.AcquireLocal(Path::SynchronousGeneration, BlockSubject(),
                                              std::nullopt, false, Coordinator::Duration{10});
    Check(emergency && emergency.lease->IsLive(), "emergency cancellation fixture acquired");
    const auto cancelled = coordinator.CancelAndClose(Refusal::DurableStateUnproven);
    Check(cancelled.fully_closed && cancelled.refusal == Refusal::DurableStateUnproven,
          "emergency close completes immediately");
    Check(!emergency.lease->IsLive() && !emergency.lease->ClaimForSink(),
          "emergency close revokes outstanding local lease");
    Check(coordinator.GetSnapshot().active_leases == 0, "emergency close retains no cached work");

    {
        Coordinator canonical(limits, CounterMint(std::make_shared<std::atomic<uint64_t>>(0)), now);
        Check(canonical.Open(ReadyConfiguration(canonical)).opened,
              "canonical-claim coordinator opens");
        const uint64_t initial_generation = canonical.GetSnapshot().configuration_generation;
        const uint64_t initial_close_epoch = canonical.GetSnapshot().close_epoch;
        auto sole = canonical.AcquireLocal(Path::InternalMining, BlockSubject(), std::nullopt,
                                           false, Coordinator::Duration{10});
        Check(sole && sole.lease->configuration_generation() == initial_generation,
              "local ticket captures coordinator configuration generation");
        Check(sole.lease->ClaimForCanonicalCommit() && sole.lease->IsLive() &&
                  canonical.GetSnapshot().close_epoch == initial_close_epoch + 1,
        ...
        auto after_claim = canonical.AcquireLocal(Path::SubmitBlock, BlockSubject(), std::nullopt,
                                                  false, Coordinator::Duration{10});
        Check(!after_claim, "canonical claim closes every competing acquisition");
        sole.lease->Release();
        Check(canonical.GetSnapshot().phase == Coordinator::Phase::Closed,
              "canonical claim closes fully after propagation lease release");
        Check(canonical.Open(ReadyConfiguration(canonical)).opened &&
                  canonical.GetSnapshot().configuration_generation > initial_generation,
              "closed-to-open transition advances the coordinator epoch");
        const uint64_t reopened_generation = canonical.GetSnapshot().configuration_generation;
        Check(canonical.Open(ReadyConfiguration(canonical)).opened &&
                  canonical.GetSnapshot().configuration_generation == reopened_generation,
        ...
        auto first = canonical.AcquireLocal(Path::InternalMining, BlockSubject(), std::nullopt,
                                            false, Coordinator::Duration{10});
        auto competitor = canonical.AcquireLocal(Path::SubmitBlock, BlockSubject(), std::nullopt,
                                                 false, Coordinator::Duration{10});
        Check(first && competitor, "two prepared local tickets may coexist before canonical claim");
        ...
        auto pending = canonical.AcquireLocal(Path::SynchronousGeneration, BlockSubject(),
                                              std::nullopt, false, Coordinator::Duration{10});
        const uint64_t pending_generation = pending.lease->configuration_generation();
        canonical.CancelAndClose(Refusal::NodeNotRunning);
        Check(!pending.lease->IsLive() && !pending.lease->ClaimForCanonicalCommit() &&
                  canonical.GetSnapshot().configuration_generation > pending_generation,
    ...
    for (const Path path : {Path::InternalMining, Path::SubmitBlock, Path::SynchronousGeneration}) {
        Coordinator equivalent(limits, CounterMint(std::make_shared<std::atomic<uint64_t>>(0)),
                               now);
        Check(equivalent.Open(ReadyConfiguration(equivalent)).opened,
              "equivalent block-production path coordinator opens");
        auto ticket = equivalent.AcquireLocal(path, BlockSubject(), std::nullopt, false,
                                              Coordinator::Duration{10});
    ...
    std::cout << "PASS work_admission_coordinator_tests checks=" << checks << "\n";
    return 0;
}
