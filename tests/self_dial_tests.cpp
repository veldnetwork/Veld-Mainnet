...
std::string RestrictAddressFamilies() {
    ...
    sock_fprog program{static_cast<unsigned short>(std::size(filter)),
    ...
    Check(::prctl(PR_SET_SECCOMP, SECCOMP_MODE_FILTER, &program) == 0,
          "apply fleet address-family restriction");
...
int main() {
    try {
        ...
        InitChain(chain);
        InitChain(other_chain);
        InitChain(background_chain);
        ...
        if (!local_ip.empty()) {
            Check(node.IsLocalListenerEndpoint(local_ip, port),
                  "recognize assigned interface address");
        ...
        Check(!node.ConnectTo("127.0.0.1", port, true, true),
              "anchor authority cannot bypass self exclusion");
        std::atomic<bool> rejected{true};
        std::vector<std::thread> retries;
        for (int i = 0; i < 8; ++i)
            retries.emplace_back([&] {
                for (int j = 0; j < 8; ++j)
                    if (node.ConnectTo("127.0.0.1", port))
                        rejected.store(false);
            });
        for (auto& thread : retries)
            thread.join();
        ...
        Check(node.GetPeerInfoList().front().connection_id == connection,
              "remote retry retains connection identity");
        ...
        Check(!background.IsLocalListenerEndpoint("127.0.0.1", port),
              "background chainstate has no own listener");
        Check(background.ConnectTo("127.0.0.1", port),
              "background chainstate can reach local foreground node");
        Wait([&] { return background.VersionReadyPeers() == 1; },
             "background connection completes handshake");
        background.Stop();
        ...
        Check(node.GetPeerInfoList().front().connection_id != connection,
              "reconnect has a new identity");
        other.Stop();
        node.Stop();
        Check(!node.IsLocalListenerEndpoint("127.0.0.1", port),
              "stopped node owns no active listener");
        Check(node.Start(), "restart local listener");
        Check(node.IsLocalListenerEndpoint("127.0.0.1", node.TestListeningPort()),
              "restart publishes the actual new port");
        Check(!node.ConnectTo("localhost", node.TestListeningPort()),
              "resolved self exclusion survives restart");
        node.Stop();
