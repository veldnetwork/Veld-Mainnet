...
source = (
    Path(sys.argv[1])
    if len(sys.argv) > 1
    else Path(__file__).resolve().parents[1] / 'src/veld-miner-portal.py'
)
spec = importlib.util.spec_from_file_location('portal_pool_test', source)
...
for patch in (
    ...
    try:
        p.validate_pool_snapshot(dict(valid, **patch))
    except ValueError:
        pass
    else:
        raise AssertionError(patch)
assert (
    p.validate_pool_snapshot(
        dict(valid, running=False, state='stopped', active_workers=0, hashrate=0)
    )['state']
    == 'stopped'
)
with tempfile.TemporaryDirectory() as td:
    ...
    report = dict(
        ...
        warning='',
        snapshot=p.validate_snapshot({'pool': valid}),
    )
    ...
    p.present_device(row)
    assert row['snapshot']['pool'] == valid
    assert row['height'] is None and row['hashrate'] is None and row['daemon_version'] is None
print(
    'PASS pool report schema, secret refusal, backward compatibility, stored pool state without invented node validation'
)
