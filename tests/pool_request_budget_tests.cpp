...
int main() {
    ...
    try {
        ...
        auto work = [&] {
            return std::make_unique<RequestBudget::Permit>(budget, true, Clock::now() + 1s);
        };
        ...
        try {
            RequestBudget::Permit extra(budget, true, start + 40ms);
        } catch (const Retry&) {
            timeout = true;
        }
        if (!timeout || Clock::now() - start > 1s)
            throw std::runtime_error("bounded queue deadline");
        a.reset();
        a = work();
        a.reset();
        b.reset();
        ...
        proof.reset();
        try {
            RequestBudget::Permit unwind(budget, true, Clock::now() + 1s);
            throw 1;
        } catch (int) {
        }
        ...
        auto maximum = [](auto& value, unsigned count) {
            auto old = value.load();
            while (old < count && !value.compare_exchange_weak(old, count)) {
            };
        };
        ...
        std::cout
            << "PASS reserved proof capacity, bounded deadline, exception recovery, 256 concurrent admissions\n";
