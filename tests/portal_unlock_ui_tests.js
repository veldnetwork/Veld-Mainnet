...
async function main() {
    ...
    const id = hex(
        new Uint8Array(
            await webcrypto.subtle.digest(
                'SHA-256',
                new TextEncoder().encode(`VELD_PORTAL_UNLOCK_KEY_V1\n${jwk.n}\n${jwk.e}`),
            ),
        ),
    );
    ...
    let device = {
        ...
        online: true,
        command_sequence: 0,
        snapshot: { unlock_key: unlock, remote_control: true },
    };
    const context = vm.createContext({
        ...
        snap: (d) => d.snapshot,
        current: () => device,
        openCommandDb: async () => db,
        commandStore: 'keys',
        commandKey: async () => ({ pair, id: 'b'.repeat(64) }),
        ensureDeviceCommandKey: async () => {},
        api: async (url, method, body) => {
            requests.push(JSON.parse(JSON.stringify({ url, method, body })));
        },
        toast(message) {
    ...
    vm.runInContext(
        'let nodeSignInTarget=null,nodeSignInBusy=false,actionQueue=Promise.resolve();',
        context,
    );
    vm.runInContext(
        [
            'encryptNodePassphrase',
            'pinUnlockKey',
            'openNodeSignIn',
            'closeNodeSignIn',
            'submitNodeSignIn',
    ...
    assert.match(
        context.portalControlStatus({ snapshot: { pairing_control: true } }),
        /Pairing is finishing/,
    );
    assert.match(
        context.portalControlStatus({ snapshot: { unlock_key: unlock } }),
        /one update on the PC/,
    );
    assert.match(context.portalControlStatus({ snapshot: {} }), /one update on the PC/);
    assert.deepEqual(Object.keys(requests[0].body.payload).sort(), [
        'ciphertext',
        'identity',
        'iv',
        'key_id',
    ...
    const payload = requests[0].body.payload,
        command = requests[0].body;
    const raw = await webcrypto.subtle.decrypt(
        { name: 'RSA-OAEP' },
        rsa.privateKey,
        Buffer.from(payload.wrapped_key, 'base64url'),
    );
    ...
    console.log(
        'PASS: remote sign-in UI encryption, key pinning, cancellation, duplicate submission, queued device binding, offline handling',
    );
}
main().catch((error) => {
    console.error(error);
