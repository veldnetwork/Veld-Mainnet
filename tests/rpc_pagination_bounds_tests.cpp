...
constexpr bool HasRange(size_t total, size_t page, size_t width, size_t begin, size_t end,
                        size_t pages) {
    const auto r = BoundedPageRange(total, page, width);
    return r.begin == begin && r.end == end && r.total_pages == pages && r.begin <= r.end &&
           r.end <= total;
}
...
static_assert(HasRange(kLargest, kLargest, kLargest, kLargest, kLargest, 1));
