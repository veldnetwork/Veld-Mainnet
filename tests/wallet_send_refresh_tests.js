...
const names = [
    'sendSourceAddress',
    'refreshWalletAfterBroadcast',
    'onFromChange',
    '_veldAssertActiveSignerSeed',
    '_veldKeyCommitmentToScriptHex',
    'signAndBroadcast',
    '_veldConsolidationBudget',
    '_veldConsolidationProgress',
];
...
function answer(batch, balance, outputs = []) {
    batch
        .find((r) => r.method === 'getbalance')
        .resolve({ balance_veld: 600, spendable_veld: balance });
...
(async () => {
    ...
    answer(currentRequests, 300, [{ txid: 'remaining' }]);
    await current;
    answer(oldRequests, 600, [{ txid: 'already-spent' }]);
    await old;
    ...
    answer(requests.splice(0), 8);
    await switched;
    answer(previousWallet, 300);
    await pending;
    assert.equal(element('s-avail').textContent, '8.00 VELD');
    const failure = context.onFromChange();
    requests.splice(0).forEach((r) => r.reject(new Error('offline')));
    await failure;
    assert.equal(element('s-avail').textContent, 'Unavailable');
    assert.equal(context.window._sUtxoData.length, 0);
    const zero = context.onFromChange();
    answer(requests.splice(0), 0);
    await zero;
    ...
    Object.assign(context, {
        ...
        veldCrypto: {
            injectSignatures: async () => 'inert-test-bytes',
            sha256d: () => '12'.repeat(32),
        },
        _veldBroadcastExactSigned: async () => '12'.repeat(32),
    });
    const call = context.signAndBroadcast(
    ...
    assert.equal(element('s-avail').textContent, 'Updating…');
    answer(requests.splice(0), 100, [{ txid: 'second-send-remaining' }]);
    await settle();
    ...
    console.log(
        'PASS: send success, cross-tab updates, zero balances, failures, and stale response ordering',
    );
})().catch((error) => {
    console.error(error);
    process.exitCode = 1;
