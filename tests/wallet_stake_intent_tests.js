...
const owner = 'ordinary-fixture-owner',
    txid = 'c'.repeat(64);
const flush = async () => {
...
function fixture(options = {}) {
    ...
    const context = vm.createContext({
        ...
        _veldStakeBalanceRevision: 0,
        __opLocks: {},
        currentAddr: owner,
        window: {},
        VELD_MIN_TX_FEE_UNITS: 100000,
        stakingMinVeld: 500,
        stakingMaxVeld: 10000,
        ...
        document: {
            ...
            querySelector: (selector) => {
                ...
                if (selector === 'button[data-act-click="haa7ba916"]')
                    return element('unstake-button');
        ...
        rpc: async (method, params) => {
            ...
            if (method === 'getstake')
                return { staked_veld: 1000, blocks_until_unlock: 10, unlock_height: 6000 };
            if (method === 'getbalance')
                return { balance_veld: 1800, spendable_veld: 525.26, staked_veld: 1000 };
...
(async () => {
    ...
    assert.equal(f.context._veldStakeFormAddress(), owner);
    ++checks;
    f.element('sk-addr').style.display = '';
    assert.equal(f.context._veldStakeFormAddress(), 'previous-wallet');
    ++checks;
    f.element('sk-addr').style.display = 'none';
    assert.equal(f.context._veldStakeAvailableUnits({ spendable_veld: 525.26 }), 52525900000n);
    ++checks;
    assert.equal(f.context._veldStakeAvailableUnits({ spendable_veld: 0.0005 }), 0n);
    ++checks;
    await f.context.onStakeAddrChange();
    assert.equal(f.element('sk-balance').textContent, '525.259');
    assert.equal(f.element('sk-current-stake').textContent, '1000.00');
    ++checks;
    ...
    assert.equal(f.pending[0].args[9], 2);
    ++checks;
    ...
    assert.equal(f.element('stake-button').disabled, true);
    ++checks;
    ...
    assert.equal(f.element('stake-button').disabled, false);
    ++checks;
    ...
    f = fixture({
        rpc: async (method) =>
            method === 'getstake'
                ? { staked_veld: 0 }
                : { balance_veld: 1000, spendable_veld: 499 },
    });
    await f.context.doStake();
    assert.equal(f.pending.length, 0);
    assert.equal(f.context.__opLocks.stake, false);
    assert.equal(f.element('stake-button').textContent, 'Stake VELD');
    assert.match(f.element('stake-msg').innerHTML, /available to stake after the network fee/);
    ++checks;
    ...
    ++checks;
    await f.context.onStakeAddrChange();
    assert.equal(f.element('sk-balance').textContent, '—');
    ++checks;
