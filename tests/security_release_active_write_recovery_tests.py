...
def main():
    ...
    result = out / 'active-write-recovery-results.json'
    assert not result.exists()
    report = {
        'status': 'running',
        'checks': {},
        'scope': 'abrupt process termination after observed block commits during an unfinished generation RPC',
    }
    binary = out / 'release-integration-node.exe'
    report['binary_sha256'] = hashlib.sha256(binary.read_bytes()).hexdigest()
    directory = run / 'node-active-write-recovery'
    ...
    try:
        ...
        initial = node.rpc('getblockchaininfo')
        start = initial['blocks']
        request_id = json.loads((run / 'first-activation-results.json').read_text())['request'][
            'request_id'
        ]
        request = node.control('fa_request', request_id=request_id)
        archive = node.control('fa_archive_status')
        peg = node.rpc('getpeginfo')
        proc = psutil.Process(node.process.pid)
        before = proc.io_counters().write_bytes
        ...
        assert observed is not None, (
            'no committed progress observed while generation remained active'
        )
        ...
        try:
            ...
            raise AssertionError('interrupted generation unexpectedly returned normally')
        except (ConnectionError, OSError) as error:
            report['interrupted_rpc_type'] = type(error).__name__
        report['interruption'] = {
            'starting_height': start,
            'observed_committed_height': observed['blocks'],
            'observed_committed_hash': observed['best_block_hash'],
            'write_bytes_before': before,
            'write_bytes_at_interruption': writes,
        ...
        assert (
            recovered.rpc('getblockhash', [str(observed['blocks'])]) == observed['best_block_hash']
        )
        ...
        assert (
            actual['reserve_accounting_holds']
            and actual['supply_sats'] == peg['supply_sats'] == 2400000
        )
        report['checks']['recovered_prefix_preserves_financial_state_and_authenticated_archive'] = (
            True
        )
        wait_admission(recovered, require_peer=False)
        recovered.generate(1)
        ...
        report['recovered_state'] = recovered.control('status')
        ...
        report['stopped'] = all(
            n is None or n.process.poll() is not None for n in (node, recovered)
        )
