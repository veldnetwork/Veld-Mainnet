...
parser.add_argument(
    '--launcher', type=Path, default=Path(__file__).resolve().parents[1] / 'pkg/Start Mining.bat'
)
...
''',
    ...
    for child_code, expected_exit, expected_count in [
        (75, 76, 4),
        (0, 0, 1),
        (76, 76, 1),
        (77, 77, 1),
        ...
        (root / 'fixture-child.cmd').write_text(
            '''@echo off
...
exit /b '''
        ...
        assert (result.returncode, calls) == (expected_exit, expected_count), (
            result.stdout + result.stderr
        )
        print(
            f'PASS launcher child_exit={child_code} launches={calls} final_exit={result.returncode}'
        )
