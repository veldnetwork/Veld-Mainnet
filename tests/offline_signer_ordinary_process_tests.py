...
def main():
    ...
    report = {'status': 'running', 'started_utc': datetime.now(timezone.utc).isoformat(),
        ...
        'platform': os.name, 'disposable_credentials_only': True, 'network_access': False,
        'checks': [], 'limits': ['Offline signing only; no funded mint, witness reservation, managed worker, or live-chain clearance.']}
    try:
        with tempfile.TemporaryDirectory(prefix='veld-ordinary-signing-') as temporary:
            ...
            def run(arguments, expected=0):
                ...
                assert value.returncode == expected, (Path(arguments[0]).name, arguments[1], value.returncode,
                    value.stderr[-400:])
            ...
            for name, command, operation, recipient, amount, digest, prepared_name in rows:
                ...
                authorized = run([args.keygen, 'authorize-intent', keyfile, prepared,
                ...
                report['checks'].append({'operation': name, 'detached_intent_authorized': True,
                    'actual_input_signatures_verified': True, 'input_evidence_retained': True})
