...
int main(int argc, char** argv) {
    if (argc != 4 || std::string(argv[1]) != "getblockheader" || std::string(argv[3]) != "true")
        return 2;
    ...
    if (!executable || !directory || !port || !observations)
        return 4;
    const auto result = veld::compat::RunProcess(
        {executable, "-regtest", std::string("-datadir=") + directory,
         std::string("-rpcport=") + port, "-rpcconnect=127.0.0.1", "getblockheader", hash, "true"},
        true, {}, false, 256u * 1024u);
    if (result.output_truncated)
        return 5;
    // One append per adapter invocation; only the public hash and return code.
    std::ofstream log(observations, std::ios::app);
    log << hash << " " << result.exit_code << "\n";
