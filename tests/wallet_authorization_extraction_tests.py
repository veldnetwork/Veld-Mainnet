...
with tempfile.TemporaryDirectory(prefix="veld-caller-checks-") as directory:
    ...
    def run(source, expected, caller=None):
        ...
        result = subprocess.run(
            [
                sys.executable,
                "-B",
                str(ROOT / "tests/wallet_authorization_wiring_tests.py"),
                "--wallet-source",
                str(fixture),
            ],
            env=env,
    ...
    run(
        wallet.replace(
            ...
            "/* Renamed surrounding comment: } function ignored() { */",
        ),
        True,
    )
    ...
    for caller, budget in (
        ...
        run(
            wallet[:at]
            + "inputs_consolidated"
            + wallet[at + len("verified_consolidation_inputs") :],
            False,
            caller,
        )
