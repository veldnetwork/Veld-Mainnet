...
const asset = fs.readFileSync(
    process.argv[2] || path.join(__dirname, '../website/explorer-route-context-v1.js'),
    'utf8',
);
const start = asset.indexOf(
...
function fixture(route) {
    ...
    class Element {
        ...
        querySelector(selector) {
            assert.equal(selector, '.bk-badges');
            return this.children.find((child) => child.className === 'bk-badges') || null;
        }
    }
    function row(height, hash = 'a'.repeat(8) + '…' + 'b'.repeat(6), count = 2) {
        const heading = new Element('#' + height),
            meta = new Element(hash + ' · Vminer'),
            tx = new Element(count + ' tx · 5.7 KB');
        return {
            heading,
            meta,
            tx,
            ...
            querySelector: (selector) =>
                ({ '.info .h': heading, '.info .s': meta, '.tx': tx })[selector],
            labels: () =>
                heading.querySelector('.bk-badges')?.children.map((child) => child.textContent) ||
                [],
            colors: () =>
                heading.querySelector('.bk-badges')?.children.map((child) => child.className) || [],
        };
    }
    let rows = [row(5002), row(4997), row(4996), row(4995, undefined, 1)];
    const list = {
        querySelectorAll: (selector) => {
            assert.equal(selector, 'a.bk');
            return rows;
        },
        contains: (item) => rows.includes(item),
    };
    const document = {
        hidden: false,
        readyState: 'complete',
        getElementById: (id) => {
            assert.equal(id, route === '/' ? 'blocks-list' : 'blocks-container');
            return list;
        },
    ...
    return {
        ...
        async reply(index, types, status = 200, height, retryAfter) {
            ...
            const blockHeight =
                height === undefined ? Number(request.url.split('/').pop()) : height;
            request.resolve({
                ok: status === 200,
                status,
...
(async () => {
    for (const route of ['/', '/blocks']) {
        ...
        const replacements = f.rows.map((item, index) =>
            f.row([5002, 4997, 4996, 4995][index], undefined, index === 3 ? 1 : 2),
        );
        f.replace(replacements);
        assert.deepEqual(
            f.rows[0].labels(),
            ['Near miss'],
            'Refresh retains cached labels on newly rendered rows',
        );
        assert.deepEqual(f.rows[1].labels(), ['Stake']);
    ...
    assert.deepEqual(
        retry.rows[1].labels(),
        ['Stake'],
        'One unavailable event feed must not hide other block labels',
    );
    ...
    assert.deepEqual(limited.rows[0].labels(), [
        'Mint',
        'Redeem',
        'btcVELD',
        'AMM',
    ...
    assert.deepEqual(
        page.rows[0].labels(),
        [],
        'A previous page response cannot label the new page',
    );
    ...
    assert.equal(
        ...
        4,
        'A replacement block at the same height must fetch its own events',
    );
    ...
    paused.events.visibilitychange();
    await flush();
    ...
    assert.deepEqual(
        paused.rows[0].labels(),
        [],
        'A mismatched event height cannot be labeled or cached',
    );
    console.log(
        'PASS shared Home/Blocks badges, near misses, colors, deduplication, refresh, pagination, reorg, retry and PWA suspension',
    );
})().catch((error) => {
    console.error(error);
    process.exitCode = 1;
