...
SOURCE = ROOT / "tests" / "daybreak_curl_executable_identity_tests.cpp"
checks = 0
...
def main() -> None:
    ...
    check("PATH" not in resolver and "getenv" not in resolver,
    ...
    with tempfile.TemporaryDirectory(prefix="veld-f8-curl-identity-build-") as tmp:
        ...
        check("PASS daybreak_curl_executable_identity_tests" in run.stdout and
    ...
    print(f"PASS daybreak_curl_executable_identity_tests.py checks={checks}")
