...
int main(int argc, char** argv) {
    ...
    try {
        if (argc != 4)
            throw std::runtime_error("use NEW_LAB_DIRECTORY P2P_PORT RPC_PORT");
        compat::InitNetwork();
        ...
        if (!compat::SecureRandom(entropy.data(), entropy.size()))
            throw std::runtime_error("randomness");
        const std::string token = BytesToHex(entropy);
        if (!channel::secure_file::AtomicWriteText((directory / "lab-rpc-token").string(), token,
                                                   &error, true))
            throw std::runtime_error(error);
        RpcHttpServer http(node.GetRPC(), config.rpc_port, "", token);
        if (!http.Start())
            throw std::runtime_error("RPC start");
        ...
        while (std::getline(std::cin, command)) {
            ...
            if (command.rfind("clock ", 0) == 0) {
                ...
                if (hex.size() != 2 * fq::SIGNED_VOTE_WIRE_BYTES || !fq::FromHexBytes(hex, wire) ||
                    !fq::DecodeSignedVoteWire(wire))
                    throw std::runtime_error("lab finality wire schema");
                auto* server = node.GetTCPServer();
                if (!server)
                    throw std::runtime_error("lab peer unavailable");
                ...
                if ((!request_id.empty() &&
                     (request_id.size() != 32 ||
                      request_id.find_first_not_of("0123456789abcdef") != std::string::npos)) ||
                    (request >> extra))
                    throw std::runtime_error("fixture mining command schema");
                ...
                if (!request_id.empty()) {
                    ...
                    if (mined.size() == 1 && mined[0].success)
                        acknowledgement += "MINED " + std::to_string(mined[0].new_height) + " " +
                                           HashToHex(mined[0].block.GetHash()) + "\n";
                    else
                        acknowledgement += "DEFERRED\n";
                    if (!channel::secure_file::AtomicWriteText(
                            (directory / "lab-mining-ack.txt").string(), acknowledgement, &error,
                            true))
                        throw std::runtime_error(
                            "fixture mining acknowledgement persistence failed");
                }
                if (mined.size() != 1 || !mined[0].success) {
                    // A normal work-admission cancellation is retryable by the
                    // lab driver. It must not terminate a healthy full node.
                    std::cout << "MINE_DEFERRED "
                              << (mined.empty() ? std::string("no result") : mined[0].error)
                              << " tag=" << Blockchain::GetLastRejectTag() << std::endl;
                } else
                    std::cout << "MINED " << mined[0].new_height << ' '
