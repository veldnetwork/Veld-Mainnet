...
Transaction Spend(const Transaction& parent, uint32_t vout, const std::vector<TxOutput>& outputs,
...
std::string Proposal(const Transaction& tx, const std::vector<Transaction>& parents,
                     const std::vector<uint8_t>& owned, uint64_t claimed_input,
                     uint64_t claimed_output, uint64_t claimed_fee, uint64_t claimed_change,
                     bool omit_first_parent = false, uint64_t first_claimed_value = UINT64_MAX) {
    std::string inputs = "[";
    ...
    inputs += "]";
    return "{" + std::string("\"unsigned_tx_hex\":") + Q(BytesToHex(tx.Serialize())) + "," +
           "\"inputs\":" + inputs + "," + "\"total_input\":" + std::to_string(claimed_input) + "," +
...
bool Authenticate(const std::string& json, const std::vector<uint8_t>& owned,
    ...
    std::string error;
    btc_buy::StrictJsonParser parser(json, offline_signing::kMaxPreparedJsonBytes, true);
...
std::string IntentJson(const offline_signing::Intent& intent) {
    return "{" + std::string("\"version\":") + Q(intent.version) + "," +
...
bool VerifyAgainst(const offline_signing::VerifiedPrepared& verified,
                   const offline_signing::Intent& intent, uint64_t expected_change = 100000) {
    std::string error;
    return offline_signing::VerifyIntent(intent, verified, "VELD_BHDR|", "", 0, "issuer-address",
...
int main() {
    try {
        ...
        const Transaction valid_tx =
            Spend(parent, 0, {TxOutput(100000, owned), TxOutput(0, marker)});
        const std::string valid_json =
            Proposal(valid_tx, {parent}, owned, 200000, 100000, 100000, 100000);
        ...
        const auto canonical_intent =
            offline_signing::MakeIntent(valid_tx, {parent.Serialize()}, "VELD_BHDR|", "", 0,
        ...
        CHECK(offline_signing::VerifyExactFeesOnlyEnvelope(valid, owned, envelope_error));
        auto split_change = valid;
        split_change.tx.outputs = {TxOutput(50000, owned), TxOutput(50000, owned),
                                   TxOutput(0, marker)};
        CHECK(!offline_signing::VerifyExactFeesOnlyEnvelope(split_change, owned, envelope_error));
        auto marker_first = valid;
        marker_first.tx.outputs = {TxOutput(0, marker), TxOutput(100000, owned)};
        CHECK(!offline_signing::VerifyExactFeesOnlyEnvelope(marker_first, owned, envelope_error));
        auto extra_output = valid;
        extra_output.tx.outputs.insert(extra_output.tx.outputs.begin() + 1,
                                       TxOutput(0, std::vector<uint8_t>{0x51}));
        CHECK(!offline_signing::VerifyExactFeesOnlyEnvelope(extra_output, owned, envelope_error));
        auto wrong_fee = valid;
        wrong_fee.fee = MIN_TX_FEE - 1;
        CHECK(!offline_signing::VerifyExactFeesOnlyEnvelope(wrong_fee, owned, envelope_error));
        auto wrong_change_claim = valid;
        wrong_change_claim.claimed_change--;
        CHECK(!offline_signing::VerifyExactFeesOnlyEnvelope(wrong_change_claim, owned,
                                                            envelope_error));
        ...
        authorization.signature_hex = BytesToHex(
            Sign(authorizer_seed, offline_signing::IntentAuthorizationHash(
                                      canonical_intent, authorization.authorizer_pubkey_digest)));
        std::string authorization_error;
        CHECK(offline_signing::VerifyIntentAuthorization(canonical_intent, authorization,
                                                         authorizer_pubkey, authorization_error));
        ...
        CHECK(!offline_signing::VerifyIntentAuthorization(wrong_authorized_intent, authorization,
                                                          authorizer_pubkey, authorization_error));
        auto altered_authorization = authorization;
        altered_authorization.signature_hex[0] =
            altered_authorization.signature_hex[0] == '0' ? '1' : '0';
        CHECK(!offline_signing::VerifyIntentAuthorization(canonical_intent, altered_authorization,
                                                          authorizer_pubkey, authorization_error));
        auto other_seed = authorizer_seed;
        other_seed[0] ^= 0x5a;
        CHECK(!offline_signing::VerifyIntentAuthorization(
            canonical_intent, authorization, DerivePublicKey(other_seed), authorization_error));
        ...
        CHECK(!Authenticate(
            Proposal(valid_tx, {wrong_parent}, owned, 200000, 100000, 100000, 100000), owned));
        // Incorrect caller-supplied value.
        CHECK(!Authenticate(
            Proposal(valid_tx, {parent}, owned, 200000, 100000, 100000, 100000, false, 199999),
            owned));
        // Missing parent.
        CHECK(!Authenticate(
            Proposal(valid_tx, {parent}, owned, 200000, 100000, 100000, 100000, true), owned));
        // Wrong vout.
        const Transaction wrong_vout =
            Spend(parent, 1, {TxOutput(100000, owned), TxOutput(0, marker)});
        CHECK(!Authenticate(Proposal(wrong_vout, {parent}, owned, 200000, 100000, 100000, 100000),
                            owned));
        ...
        const auto twin_intent =
            offline_signing::MakeIntent(twin_original, {twin_parent.Serialize()}, "VELD_BHDR|", "",
                                        0, "issuer-address", 100000, ValidBhdrIdentity());
        offline_signing::VerifiedPrepared twin_verified;
        CHECK(Authenticate(
            Proposal(twin_switched, {twin_parent}, owned, 200000, 100000, 100000, 100000), owned,
            &twin_verified));
        CHECK(!VerifyAgainst(twin_verified, twin_intent));
        // Duplicate input.
        const Transaction duplicate =
            Spend(parent, 0, {TxOutput(300000, owned), TxOutput(0, marker)}, true);
        CHECK(!Authenticate(
            Proposal(duplicate, {parent, parent}, owned, 400000, 300000, 100000, 300000), owned));

        // Hidden output: parent authentication succeeds, original output intent fails.
        const Transaction hidden =
            Spend(parent, 0,
                  {TxOutput(100000, owned), TxOutput(1, std::vector<uint8_t>{0x51}),
                   TxOutput(0, marker)});
        offline_signing::VerifiedPrepared hidden_verified;
        CHECK(Authenticate(Proposal(hidden, {parent}, owned, 200000, 100001, 99999, 100000), owned,
                           &hidden_verified));
        ...
        const Transaction redirected =
            Spend(parent, 0, {TxOutput(100000, std::vector<uint8_t>{0x51}), TxOutput(0, marker)});
        offline_signing::VerifiedPrepared redirected_verified;
        CHECK(Authenticate(Proposal(redirected, {parent}, owned, 200000, 100000, 100000, 0), owned,
                           &redirected_verified));
        ...
        CHECK(Authenticate(Proposal(omitted, {exact_fee_parent}, owned, 100000, 0, 100000, 0),
        ...
        const Transaction fee_high =
            Spend(fee_parent, 0, {TxOutput(100000, owned), TxOutput(0, marker)});
        CHECK(!Authenticate(Proposal(fee_high, {fee_parent}, owned, 200001, 100000, 100001, 100000),
                            owned));
        ...
        // Negative fee.
        const Transaction negative_fee =
            Spend(parent, 0, {TxOutput(200001, owned), TxOutput(0, marker)});
        CHECK(!Authenticate(Proposal(negative_fee, {parent}, owned, 200000, 200001, 0, 200001),
                            owned));

        // Output and input overflow attempts are rejected before wraparound.
        const Transaction output_overflow = Spend(
            parent, 0, {TxOutput(UINT64_MAX, owned), TxOutput(1, owned), TxOutput(0, marker)});
        CHECK(!Authenticate(Proposal(output_overflow, {parent}, owned, 200000, 0, 0, 0), owned));
        ...
        TxInput a;
        a.prev_tx_hash = huge_parent.GetTxID();
        a.prev_out_index = 0;
        TxInput b;
        b.prev_tx_hash = tiny_parent.GetTxID();
        b.prev_out_index = 0;
        input_overflow.inputs = {a, b};
        input_overflow.outputs = {TxOutput(0, marker)};
        CHECK(!Authenticate(Proposal(input_overflow, {huge_parent, tiny_parent}, owned, 0, 0, 0, 0),
                            owned));

        // Altered operation marker.
        const Transaction altered =
            Spend(parent, 0,
                  {TxOutput(100000, owned), TxOutput(0, BuildOpReturnScript("VELD_ANCHOR|abcd"))});
        offline_signing::VerifiedPrepared altered_verified;
        CHECK(Authenticate(Proposal(altered, {parent}, owned, 200000, 100000, 100000, 100000),
        ...
        std::cout << "PASS offline_signer_auth_tests checks=" << g_checks << "\n";
        return 0;
    } catch (const std::exception& e) {
        std::cerr << "FAIL offline_signer_auth_tests: " << e.what() << "\n";
        return 1;
    }
}
