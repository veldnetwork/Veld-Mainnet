...
def main():
    ...
    args = p.parse_args()
    directory = args.completed_run.resolve()
    ...
    report = {
        ...
        "new_test_height": 3360,
        "new_production_height": 0,
        "checks": {},
    }
    checks = report["checks"]
    nodes = []
    try:
        a = Node(binary, directory / ("node-boundary-a" + suffix))
        nodes.append(a)
        parent = a.control("import_history", name="prefix-history.bin")
        b = Node(binary, directory / ("node-boundary-b" + suffix))
        nodes.append(b)
        ...
        a.control(
            ...
            wallet=1,
            title=title,
            description="This ordinary vote must survive the later parser and archive migration.",
        )
        for wallet in range(1, 6):
            a.control("endorse", wallet=wallet, sponsor=0, height=3357)
        a.generate(1)
        wait_equal(a, b)
        ...
        a.control(
            ...
            wallet=3,
            title=new_title,
            description="UTF-8 | text: fields remain independently framed after the separate migration.",
        )
        a.control("endorse", wallet=4, sponsor=0, height=3359)
        a.generate(1)
        migration = wait_equal(a, b)
        ...
        report["migration_state"] = migration
        fork = Node(binary, directory / ("node-boundary-fork" + suffix))
        nodes.append(fork)
        assert (
            fork.control("import_history", name="prefix-history.bin")["digest"] == parent["digest"]
        )
        ...
        assert a.control("fa_request", request_id=request_id) == expected_request
        assert (
            a.rpc("getpeginfo")["supply_sats"] == 2400000
            and a.rpc("getpeginfo")["reserve_accounting_holds"]
        )
        ...
        b.stop()
        fork.stop()
        restarted = Node(binary, a.directory, "-after-reorg-restart")
