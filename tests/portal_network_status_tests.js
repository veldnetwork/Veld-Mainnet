...
const source = fs.readFileSync(
    process.argv[2] || path.join(__dirname, '../src/veld-miner-portal.py'),
    'utf8',
);
const html = source.includes('PORTAL_HTML = r"""')
    ? source.split('PORTAL_HTML = r"""')[1].split('"""')[0]
    : source;
...
vm.runInContext(
    ['esc', 'metric', 'inboundMapping', 'topologyGraph', 'topologyRoles', 'network']
        .map(extract)
        .join('\n'),
    context,
);
...
const nodes = [1, 2, 3]
    .map((i) => ({ id: id(i), role: 'fleet', role_index: i, tip_state: 'exact' }))
    .concat(
        [1, 2, 3, 4, 5, 6].map((i) => ({
            id: id(i + 3),
            role: 'miner',
...
const edges = nodes
    .slice(1)
    .map((peer) => ({ first: nodes[0].id, second: peer.id, confirmed: peer.role === 'fleet' }));
const snapshot = {
...
function checkFullNetwork(output) {
    assert.equal(
        (output.match(/aria-label="Sanitized Veld peer topology"/g) || []).length,
        1,
        'Exactly one full network graph',
    );
    assert.equal(
        (output.match(/<g class="peer /g) || []).length,
        9,
        'All reported network identities remain visible',
    );
    assert.equal(
        (output.match(/<path class="edge /g) || []).length,
        8,
        'All reported links remain visible',
    );
    ...
    assert.doesNotMatch(
        output,
        /Your connections|Network overview|This miner|network-overview|aria-expanded|Node 01/,
    );
}
...
const actualNode = {
    ...
    topology: {
        ...snapshot.topology,
        nodes: [...nodes, { id: id(10), role: 'node', role_index: 1, tip_state: 'exact' }],
        eligible_nodes: 10,
    },
};
...
assert.match(output, /<b>6<\/b><span>Miners/);
assert.equal(
    context.topologyRoles({ nodes: [...nodes, nodes[0]] }).fleet,
    3,
    'Duplicate identities do not inflate role counts',
);

assert.equal(context.inboundMapping(device, { ...snapshot, port_mapped: true }).value, 'Mapped');
assert.equal(
    context.inboundMapping(device, { ...snapshot, port_mapped: undefined }).value,
    'Not reported',
);
assert.match(
    context.inboundMapping({ ...device, inbound: 1 }, snapshot).detail,
    /Inbound peers are connected/,
);
assert.equal(context.inboundMapping({ ...device, online: false }, snapshot).value, 'Not current');
assert.equal(
    context.inboundMapping(device, { ...snapshot, process_running: false }).value,
    'Not running',
);
assert.doesNotMatch(
    context.inboundMapping({ ...device, peers: 0 }, { ...snapshot, outbound: 0 }).detail,
    /^Outbound peers are connected/,
);
assert.match(
    context.network(device, {}),
    /Network map appears when this client reports topology data/,
);
output = context.topologyGraph({
    nodes: [{ id: id(11), role: '<img src=x onerror=alert(1)>', tip_state: '<script>' }],
    edges: [],
});
...
console.log(
    'PASS portal network: single full graph, all identities and links, miner labels, genuine node roles, refresh, machine switch, mapping and escaping',
);
