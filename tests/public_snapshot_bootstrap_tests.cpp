...
std::string CanonicalManifest() {
    ...
    return std::string("format=VELD_SNAPSHOT_MANIFEST_V2\n") + "network=veld-public-mainnet-v2\n" +
           ...
           "archive_file=veld-chain-snapshot-h223-" + archive_hash.substr(0, 16) + ".tar.gz\n" +
           "genesis=" + veld::HashToHex(veld::CreateGenesisBlock().GetHash()) +
           "\nanchor_height=1\nanchor_hash=" + std::string(veld::SNAPSHOT_LAUNCH_ANCHOR_HASH) +
...
int main() {
    ...
    Check(IsCanonicalArchiveName(manifest), "archive name binds height and digest");

    Check(!ParseSnapshotManifest("height=8377\ntip_hash=" + std::string(64, '0') +
                                 "\nsha256=" + std::string(64, '1') +
                                 "\ngenesis=" + HashToHex(CreateGenesisBlock().GetHash()) +
                                 "\npublished_at=2026-09-02T14:03:08Z\n")
               .syntax_valid,
          "legacy public v1 manifest is rejected");
    Check(!ParseSnapshotManifest(canonical.substr(0, canonical.size() - 1)).syntax_valid,
    ...
    reordered.replace(network, state - network, "state_digest=v8\n");
    Check(!ParseSnapshotManifest(reordered).syntax_valid, "reordered fields are rejected");
    std::string whitespace = canonical;
    whitespace.replace(whitespace.find("height=223"), 10, "height= 223");
    Check(!ParseSnapshotManifest(whitespace).syntax_valid, "hidden whitespace is rejected");
    std::string carriage = canonical;
    carriage.insert(carriage.find('\n'), 1, '\r');
    Check(!ParseSnapshotManifest(carriage).syntax_valid, "carriage returns are rejected");
    std::string leading_zero = canonical;
    leading_zero.replace(leading_zero.find("height=223"), 10, "height=0223");
    Check(!ParseSnapshotManifest(leading_zero).syntax_valid, "noncanonical height is rejected");
    ...
    Check(!ManifestMatchesCompiledPublicChain(wrong, &error), "wrong network is rejected");
    wrong = manifest;
    ...
    wrong = manifest;
    wrong.sha256[0] = 'A';
    Check(!ManifestMatchesCompiledPublicChain(wrong, &error), "noncanonical digest is rejected");

    Check(SafeArchiveMemberName("db/blocks/MANIFEST-000001"),
          "canonical LevelDB manifest member accepted");
    Check(SafeArchiveMemberName("db/utxo/000123.ldb"), "canonical LevelDB table member accepted");
    Check(!SafeArchiveMemberName("../db/blocks/CURRENT"), "parent traversal rejected");
    Check(!SafeArchiveMemberName("db/blocks/../../escape"), "nested traversal rejected");
    Check(!SafeArchiveMemberName("db/blocks/link target"), "whitespace member rejected");
    Check(!SafeArchiveMemberName("db/other/CURRENT"), "unexpected database rejected");
    Check(!SafeArchiveMemberName("db/blocks/CURRENT/extra"), "unexpected member depth rejected");
    Check(!SafeArchiveMemberName("network.identity"),
          "archive cannot supply the local network identity");

    const auto suffix = std::to_string(std::chrono::steady_clock::now().time_since_epoch().count());
    const auto root =
        std::filesystem::temp_directory_path() / ("veld-public-snapshot-tests-" + suffix);
    std::filesystem::create_directories(root);
    const auto names = root / "names";
    const auto details = root / "details";
    const std::string members = "db/\ndb/blocks/\ndb/blocks/CURRENT\n"
                                "db/blocks/MANIFEST-000001\ndb/index/\ndb/index/CURRENT\n"
                                "db/index/MANIFEST-000001\ndb/utxo/\ndb/utxo/CURRENT\n"
                                "db/utxo/MANIFEST-000001\n";
    const std::string types = "drwx------\ndrwx------\n-rw-------\n-rw-------\n"
                              "drwx------\n-rw-------\n-rw-------\ndrwx------\n"
                              "-rw-------\n-rw-------\n";
    Write(names, members);
    Write(details, types);
    Check(ValidateArchiveListings(names, details, &error), "canonical archive listing accepted");
    Write(names, members + "db/blocks/CURRENT\n");
    Write(details, types + "-rw-------\n");
    Check(!ValidateArchiveListings(names, details, &error), "duplicate archive member rejected");
    Write(names, members);
    std::string link_types = types;
    link_types[link_types.find("-rw-------")] = 'l';
    Write(details, link_types);
    Check(!ValidateArchiveListings(names, details, &error), "archive symlink member rejected");
    ...
    Check(!ValidateExtractedLevelDbTree(extracted, &error), "unexpected extracted file rejected");
    std::filesystem::remove(extracted / "db" / "blocks" / "unexpected.bin");
    std::filesystem::create_hard_link(extracted / "db" / "blocks" / "CURRENT",
                                      extracted / "db" / "blocks" / "000001.ldb");
    Check(!ValidateExtractedLevelDbTree(extracted, &error), "external hardlink identity rejected");
    ...
    Check(channel::secure_file::Read((identity_extract / "network.identity").string(),
                                     identity_bytes, &error, 4096,
    ...
    for (const char* command : {MessageType::TX, MessageType::GETDATA, MessageType::GETBLOCKS,
                                MessageType::MEMPOOL, MessageType::SOLUTION, MessageType::FINVOTE,
                                MessageType::PUNCHREQ, MessageType::ONIONADV}) {
    ...
    Check(!cleanup_error && !std::filesystem::exists(root), "snapshot fixtures cleaned up");
