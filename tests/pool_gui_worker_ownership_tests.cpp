...
BOOL CALLBACK Edits(HWND window,LPARAM arg){
    ...
    if(std::wstring(type)==L"Edit")reinterpret_cast<std::vector<HWND>*>(arg)->push_back(window);
    return TRUE;
}
int Parent(const std::filesystem::path& root,const std::filesystem::path& worker){
    HWND window=CreateWindowExW(0,L"STATIC",L"Owned worker fixture",WS_OVERLAPPEDWINDOW,
    ...
    Check(window!=nullptr,"parent fixture window");
    {
        ...
        std::vector<HWND> edits;EnumChildWindows(window,Edits,reinterpret_cast<LPARAM>(&edits));
        ...
        {std::ofstream ready(root/"ready");ready<<"ready\n";}
        for(int i=0;i<300 && !std::filesystem::exists(root/"finish");++i)Sleep(100);
    }
...
HANDLE Child(DWORD parent,const std::filesystem::path& expected){
    ...
    PROCESSENTRY32W entry{};entry.dwSize=sizeof(entry);HANDLE child=nullptr;
    if(Process32FirstW(snapshot,&entry))do{
        if(entry.th32ParentProcessID!=parent)continue;
        HANDLE candidate=OpenProcess(SYNCHRONIZE|PROCESS_QUERY_LIMITED_INFORMATION|PROCESS_TERMINATE,FALSE,entry.th32ProcessID);
        ...
        if(QueryFullProcessImageNameW(candidate,0,path,&count) &&
           std::filesystem::equivalent(expected,path)){
            Check(child==nullptr,"exactly one owned worker");child=candidate;
        }else CloseHandle(candidate);
    }while(Process32NextW(snapshot,&entry));
    CloseHandle(snapshot);return child;
}
std::wstring Read(const std::filesystem::path& path){
    std::ifstream input(path);std::string value((std::istreambuf_iterator<char>(input)),{});
    return std::wstring(value.begin(),value.end());
}
}
int wmain(int argc,wchar_t** argv)try{
    if(argc==4 && std::wstring(argv[1])==L"--parent")return Parent(argv[2],argv[3]);
    Check(argc==2,"actual worker path required");const std::filesystem::path worker=std::filesystem::absolute(argv[1]);
    Check(std::filesystem::is_regular_file(worker),"actual worker exists");
    wchar_t self[32768]{};Check(GetModuleFileNameW(nullptr,self,32768)>0,"test executable identity");
    const auto root=std::filesystem::temp_directory_path()/(L"veld-job-owner-"+std::to_wstring(GetCurrentProcessId()));
    Check(!std::filesystem::exists(root),"fresh disposable test root");
    bool all=true;
    for(bool crash:{true,false}){
        const auto profile=root/(crash?L"crash":L"normal");std::string error;
        Check(veld::channel::secure_file::EnsurePrivateDirectory(profile.string(),&error),"private fixture profile");
        std::wstring command=L"\""+std::wstring(self)+L"\" --parent \""+profile.wstring()+L"\" \""+worker.wstring()+L"\"";
        std::vector<wchar_t> buffer(command.begin(),command.end());buffer.push_back(0);
        STARTUPINFOW start{};start.cb=sizeof(start);PROCESS_INFORMATION proc{};
        Check(CreateProcessW(self,buffer.data(),nullptr,nullptr,FALSE,CREATE_NO_WINDOW,nullptr,nullptr,&start,&proc),"spawn actual pool parent");
        CloseHandle(proc.hThread);
        for(int i=0;i<100 && !std::filesystem::exists(profile/"ready") && WaitForSingleObject(proc.hProcess,0)==WAIT_TIMEOUT;++i)Sleep(100);
        ...
        if(!child_stopped){TerminateProcess(child,9);WaitForSingleObject(child,5000);}
        if(!parent_stopped)TerminateProcess(proc.hProcess,9);
        CloseHandle(child);CloseHandle(proc.hProcess);
        const bool preserved=Read(profile/"client.json")==config && Read(profile/"resume.request")==L"resume\n";
        std::cout<<(parent_stopped&&child_stopped&&preserved?"PASS ":"FAIL ")
                 <<(crash?"parent crash":"normal parent exit")<<": worker stopped="<<child_stopped
