    ...
    try {
        ...
        while (in.peek() != std::char_traits<char>::eof()) {
            ...
            if (selected.count(height)) {
                ...
                std::cout << "{\"height\":" << height << ",\"pow_hash\":\"" << HashToHex(hash)
                          << "\"}" << std::endl;
            }
            ++height;
        }
        if (height < (separate ? 3361u : 2882u))
            throw std::runtime_error("history does not cross the migration");
        return 0;
