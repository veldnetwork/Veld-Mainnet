...
int main(int argc, char** argv) {
    ...
    RealKeyPair miner;
    miner.script_override = AddressToScript(address);
    if (miner.script_override.size() != 25) {
        node.Stop();
        return 2;
    }
    asert_qualification::candidate_time.store(1767225780);
    auto initial = node.MineBlocks(miner, 1, 0);
    if (initial.size() != 1 || !initial[0].success) {
        node.Stop();
        return 3;
    }
    const auto request =
        std::string(
            "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"getaddresshistory\",\"params\":[\"") +
        address + "\",\"50\"]}";
    std::string first;
    armed.store(true);
    ...
    std::vector<MineBlockResult> blocks;
    std::atomic<bool> committed{false};
    std::thread producer([&] {
        blocks = node.MineBlocks(miner, 1, 0);
    ...
    node.Stop();
    if (!success) {
        std::cerr << "FAIL history remains unavailable after canonical publication\n";
        return 1;
    }
