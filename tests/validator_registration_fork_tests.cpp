...
static Block Operation(const RealKeyPair& key, uint64_t height, const std::string& action,
                       uint64_t bond = 0) {
    Block block;
    block.height = height;
    Transaction tx;
    ...
    const std::string public_key =
        BytesToHex(std::vector<uint8_t>(key.public_key.begin(), key.public_key.end()));
    tx.outputs.emplace_back(0, BuildOpReturnScript(std::string(ValidatorRegistry::VAL_PREFIX) +
                                                   action + "|" + public_key));
    if (bond)
        tx.outputs.emplace_back(bond, AddressToScript(STAKE_VAULT_ADDRESS));
...
int main() {
    ...
    for (uint64_t height : {H - 1, H, H + 1}) {
        for (uint64_t total : {uint64_t{0}, VALIDATOR_UNLOCK_STAKED - 1, VALIDATOR_UNLOCK_STAKED,
                               VALIDATOR_UNLOCK_STAKED + 1}) {
            ValidatorRegistry boundary;
            boundary.SetTotalStaked(total);
            const bool permitted = height >= H || total >= VALIDATOR_UNLOCK_STAKED;
            assert(boundary.RegistrationPreparationError(public_key, key.address, height, total)
                       .empty() == permitted);
            assert(boundary.IsValidatorSystemActive(height) == permitted);
            assert(boundary.ProcessBlock(Operation(key, height, "REGISTER", MIN_VALIDATOR_STAKE),
                                         zero_stake));
    ...
    assert(!registry.ProcessBlock(Operation(key, dereg_height + 1, "REGISTER", MIN_VALIDATOR_STAKE),
                                  zero_stake));
    ...
    assert(
        registry.ProcessBlock(Operation(key, H - 1, "REGISTER", MIN_VALIDATOR_STAKE), zero_stake));
