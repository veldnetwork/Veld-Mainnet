...
async function checkWorker(file, prefix) {
    ...
    const context = vm.createContext({
        ...
        caches: {
            keys: async () => [prefix + 'old', 'unrelated-cache'],
            delete: async (key) => {
                deleted.push(key);
            },
            open: () => {
                throw Error('Must not open or read saved pages');
            },
            match: () => {
                throw Error('Must not return an old response');
            },
        },
    ...
    let promise;
    handlers.fetch({
        request: { method: 'GET', destination: 'document' },
        respondWith: (p) => {
            promise = p;
        },
    });
    ...
    handlers.fetch({
        request: { method: 'POST' },
        respondWith: () => {
            throw Error('Do not intercept signing requests');
        },
    });
...
async function checkNavigation() {
    ...
    for (const route of ['/', '/blocks', '/mempool', '/mining', '/rules', '/liquidity']) {
        ...
        assert.equal(
            context.window.__veldExplorerSoftTxNav,
            true,
            'Legacy inline partial navigation is disabled',
        );
...
(async () => {
    ...
    console.log(
        'PASS: current route navigation; fresh document/script/style/data reads; 429 preserved; offline rejects saved responses; only app caches removed; POST requests untouched',
    );
})().catch((error) => {
    console.error(error);
    process.exitCode = 1;
