...
function fixture() {
    ...
    const document = {
        ...
        querySelector: (selector) =>
            descendants(list).find(
                (node) =>
                    selector === '.bk-badges[data-block-hash="' + node.dataset.blockHash + '"]' &&
                    node.className === 'bk-badges',
            ),
    };
    ...
    const context = vm.createContext({
        ...
        Error,
        AbortController,
        shortHash: (hash) => hash.slice(0, 10),
        shortAddr: (address) => address.slice(0, 10),
        setTimeout: (fn, delay) => {
            const id = ++timerId;
    ...
    function batch(height, headerAge = 180, tipHash) {
        const blocks = Array.from({ length: Math.min(10, height + 1) }, (_, index) => ({
            height: height - index,
            hash: hash(height - index),
            prev_hash: hash(height - index - 1),
            time: Math.floor(now / 1000) - headerAge - index * 180,
            tx_count: 1,
            size_kb: 2.5,
            miner: 'VExampleMinerAddress',
        }));
    ...
    return {
        ...
        advance,
        load: (height, tipHash) =>
            window.veldExplorerDashboard.load(height, tipHash || hash(height)),
        ages: () =>
            document
                .querySelectorAll('#blocks-list [data-block-time]')
...
(async () => {
    ...
    assert.deepEqual(
        catchup.ages().slice(0, 7),
        ['3 m', '6 m', '9 m', '12 m', '15 m', '18 m', '21 m'],
    ...
    assert.equal(
        f.ages()[0],
        '0 s',
        'An old mining timestamp must not make a new arrival look delayed',
    );
    ...
    f.load(102);
    assert.equal(f.requests.length, 3, 'Failed refresh must respect backoff');
    ...
    await f.advance(2000);
    f.load(103);
    const malformed = f.batch(103);
    malformed.blocks[1].hash = 'f'.repeat(64);
    await f.respond(f.requests[5], malformed);
    assert(
        f.list.textContent.includes('temporarily unavailable'),
        'Disconnected pages must be rejected',
    );
    ...
    for (const height of [101, 107]) {
        ...
        assert.equal(
            resumed.ages()[0],
            '3 m',
            'PWA resume must establish a timestamp baseline even for one missed block',
        );
        ...
        assert.equal(
            resumed.ages()[0],
            '0 s',
            'Continuous live arrival timing must recover after resume',
        );
    }
    ...
    assert.equal(
        suspended.ages()[0],
        '3 m',
        'A polling gap must rebaseline even without a visibility event',
    );
    ...
    interrupted.events.visibilitychange();
    interrupted.document.hidden = false;
    interrupted.events.visibilitychange();
    await interrupted.respond(interrupted.requests[1], interrupted.batch(101));
    assert.equal(
        interrupted.list.children[0].href,
        '/block/height/100',
        'A request started before suspension cannot paint after resume',
    );
    assert(
        !interrupted.list.textContent.includes('temporarily unavailable'),
        'Suspension must not display a fetch failure',
    );
    interrupted.load(101);
    ...
    offline.load(101);
    offline.requests[1].reject(new Error('offline'));
    await flush();
    await offline.advance(2000);
    offline.load(101);
    await offline.respond(offline.requests[2], offline.batch(101));
    assert.equal(
        offline.ages()[0],
        '3 m',
        'An arrival missed during a failed request cannot be timed from reconnection',
    );
    ...
    assert.equal(
        restored.ages()[0],
        '3 m',
        'Restoring an unchanged page must discard stale arrival clocks',
    );

    console.log(
        'PASS Explorer live arrival ages, catch-up batches, PWA resume, polling gaps, retries, deadlines and reorg checks',
    );
})().catch((error) => {
    console.error(error);
    process.exitCode = 1;
