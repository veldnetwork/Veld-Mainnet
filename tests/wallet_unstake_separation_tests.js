...
const names = ['_veldStakeFormAddress', 'doUnstake', 'planIncrementalUnstake',
  'doIncrementalUnstake', 'doDeregisterValidator'];
const owner = 'fixture-owner', token = 'inert'.padEnd(64, '_'), generation = 17;
const flush = async () => { for (let i = 0; i < 40; ++i) await Promise.resolve(); };

function fixture() {
  const elements = new Map(), calls = [], prompts = [], timers = [], locks = new Set();
  const element = id => {
  ...
  const context = vm.createContext({
    currentAddr:owner, window:{}, UNSTAKE_BATCH_THRESHOLD_VELD:2000,
    ...
    updateKsIndicator(){}, loadStakingPage(){}, onStakeAddrChange(){}, loadValidatorsPage(){},
    fmt:(value, places) => Number(value).toFixed(places), escHtml:String,
    setTimeout:(fn, ms) => { timers.push({fn, ms}); return timers.length; },
    sleepMs:async ms => { calls.push({sleep:ms}); },
    confirm:text => { prompts.push(text); return true; },
    rpc:async (method, params) => {
      calls.push({method, params});
      assert.equal(method, 'getstakehistory');
      return [{type:'stake', active:true, unlock_at:40000, block:1000, amount_veld:1000},
...
(async () => {
  ...
  assert(extractFunction(source, 'doDeregisterValidator').includes("'preparederegistervalidator'"));
  ++checks;
  ...
  assert.deepEqual(signed.map(call => call.signed), ['prepareunstake', 'prepareunstake']);
  assert.deepEqual(signed.map(call => Array.from(call.args[1])), [[owner, '1000'], [owner, '1500']]);
  assert(signed.every(call => call.args[9] === generation));
  assert.deepEqual(f.calls.filter(call => call.sleep).map(call => call.sleep), [65000]);
  ...
  ++checks;
  console.log('PASS ' + checks + ' unstake and validator-exit controls; inert signatures and no network');
