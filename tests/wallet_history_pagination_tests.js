...
function fixture(handler) {
    ...
    const ctx = vm.createContext({
        ...
        historyIsConsolidation: async () => false,
        renderTokenHistory() {},
        escHtml: (s) =>
            String(s).replace(
                /[&<>"']/g,
        ...
        fmt: (n, d) => Number(n || 0).toFixed(d),
        shortHash: (s) => s.slice(0, 8),
        rpc: async (method, params) => {
            calls.push({ method, params });
            assert(
                ['getaddresshistory', 'gettokenhistory', 'getbalance'].includes(method),
...
function history(rows) {
    return (params) => {
        const start = Number(params[2] || 0),
            entries = rows.slice(start, start + 50);
...
(async () => {
    ...
    const f = fixture(
        history([...Array.from({ length: 151 }, (_, n) => reward(n + 1)), cleanup, sent]),
    );
    ...
    assert.deepEqual(
        f.calls.filter((c) => c.method === 'getaddresshistory').map((c) => c.params[2] || ''),
        ['', '50', '100', '150'],
    );
    ...
    for (const failure of [
        ...
        { entries: [], has_more: true, next_cursor: 'next' },
        { entries: [sent], has_more: true, next_cursor: '50' },
        { entries: [], has_more: 'yes', next_cursor: '' },
    ]) {
    ...
    const large = fixture(
        history([...Array.from({ length: 1001 }, (_, n) => reward(n + 1)), sent]),
    );
    ...
    assert.equal(
        large.calls.filter((c) => c.method === 'getaddresshistory').length,
        20,
        'Auto refresh does not reset partial browsing',
    );
    ...
    let resolveA;
    const race = fixture((params) =>
        params[0] === 'wallet-A'
            ? new Promise((r) => {
                  resolveA = r;
              })
            : { entries: [sent], has_more: false, next_cursor: '' },
    );
    const first = race.ctx.loadHistory();
    const repeated = race.ctx.loadHistory();
    assert.equal(
        race.calls.filter((c) => c.method === 'getaddresshistory').length,
        1,
        'Coalesce same-address loads',
    );
    ...
    assert.equal(
        race.calls.filter((c) => c.method === 'getaddresshistory' && c.params[0] === 'wallet-A')
            .length,
        1,
    );
    ...
    const pending = typing.ctx.loadHistory();
    typing.element('h-addr').value = 'wallet-B';
    resolveTyping({ entries: [sent], has_more: false, next_cursor: '' });
    await pending;
    assert.equal(typing.ctx.historyLoadState.busy, false);
    typing.element('h-addr').value = '';
    await typing.ctx.loadHistory();
    assert.equal(typing.ctx.historyLoadState, null);
    assert.equal(typing.ctx.historyData.length, 0);

    // Duplicate page-boundary rows never double count the transfer or its fee.
    const duplicate = fixture((params, n) =>
        n === 1
            ? { entries: [sent], has_more: true, next_cursor: 'next' }
            : { entries: [sent, cleanup], has_more: false, next_cursor: '' },
    );
    ...
    classified.ctx.historyFilter = 'received';
    await classified.ctx.loadHistory();
    ...
    classified.ctx.historyFilter = 'transfers';
    classified.ctx.renderHistory();
    assert.match(classified.element('h-list').innerHTML, /Received/);
    assert.doesNotMatch(
        classified.element('h-list').innerHTML,
        /Staking Reward|Co-Mine Payout|Validator Rewards/,
    );
    console.log(
        'PASS wallet history pagination: older sends, all fee rows, bounded cursor batches, explicit partial/error states, deduplication, refresh coalescing and address races',
    );
})().catch((e) => {
    console.error(e);
    process.exitCode = 1;
