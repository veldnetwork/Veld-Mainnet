...
def main():
    ...
    for role in ('source', 'fresh'):
        ...
        for process in psutil.process_iter(['exe', 'cmdline']):
            try:
                ...
                if (
                    process.info['exe']
                    and Path(process.info['exe']).resolve() == binary
                    and len(cmd) == 4
                    and Path(cmd[1]).resolve() == directory
                ):
    ...
    report = {
        'status': 'observing',
        'scope': 'passive observation of two owned loopback test nodes',
    ...
    while True:
        ...
        for role, (process, port, directory) in targets.items():
            ...
            try:
                ...
                assert not value.get('error')
                row.update(
                    rpc_seconds=round(time.monotonic() - started, 6),
                    height=value['result']['blocks'],
                    tip=value['result']['best_block_hash'],
                    ibd_complete=value['result']['ibd_complete'],
                )
        ...
        save()
        if time.monotonic() - start > 32400:
            report['status'] = 'observation_deadline'
            break
        time.sleep(60)
