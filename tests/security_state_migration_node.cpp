...
Value Json(const std::string& text) {
    ...
    if (!parser.Parse(out, error))
        throw std::runtime_error("test JSON: " + error);
    return out;
}
const Value& Field(const Value& value, const std::string& name) {
    const auto* out = value.Get(name);
    if (!out)
        throw std::runtime_error("test missing field: " + name);
    return *out;
}
uint64_t Number(const Value& value) {
    uint64_t out = 0;
    if (!ParseCanonicalUint64Text(value.text, out))
        throw std::runtime_error("test integer");
    return out;
}
std::string Call(VeldNode& node, const std::string& method,
                 const std::vector<std::string>& params = {}) {
    std::vector<std::string> args;
    for (const auto& value : params)
        args.push_back(JsonBuilder::String(value));
    const std::string wire = JsonBuilder::Object({{"jsonrpc", JsonBuilder::String("2.0")},
                                                  {"method", JsonBuilder::String(method)},
                                                  {"params", JsonBuilder::Array(args)},
                                                  {"id", "1"}});
...
Value Result(VeldNode& node, const std::string& method,
             const std::vector<std::string>& params = {}) {
    return Field(Json(Call(node, method, params)), "result");
}
std::string SignAndSend(VeldNode& node, const RealKeyPair& key, const std::string& method,
                        const std::vector<std::string>& params,
    ...
    for (uint32_t i = 0; i < tx.inputs.size(); ++i) {
        const auto prev =
            node.GetChain().GetUTXO(tx.inputs[i].prev_tx_hash, tx.inputs[i].prev_out_index);
#ifdef VELD_FIRST_ACTIVATION_NETWORK
        const auto pool = node.GetAmm().GetPool("VELD:btcVELD");
        const bool pool_method =
            method == "prepareammadd" || method == "prepareammswap" || method == "prepareammremove";
        if (pool_method && i == 0 && prev && pool.utxo_valid &&
            tx.inputs[i].prev_tx_hash == pool.pool_txid &&
            tx.inputs[i].prev_out_index == pool.pool_vout &&
        ...
        signatures.push_back(
            BuildScriptSig(key.private_key, key.public_key, tx, i, own).script_sig);
    }
    for (size_t i = 0; i < signatures.size(); ++i)
        tx.inputs[i].script_sig = signatures[i];
...
int main(int argc, char** argv) {
    try {
        compat::InitNetwork();
        if (argc != 4)
            throw std::runtime_error("use DATA_DIR P2P_PORT RPC_PORT");
        ...
        const uint16_t p2p = uint16_t(std::stoul(argv[2])),
                       rpc_port = uint16_t(std::stoul(argv[3]));
        if (p2p < 20000 || rpc_port < 20000 || p2p == rpc_port)
            throw std::runtime_error("test ports");
...
#ifdef VELD_FIRST_ACTIVATION_NETWORK
            const auto seed_path = directory.parent_path() / "disposable-identities" /
                                   ("wallet-" + std::to_string(i) + ".seed");
            if (std::filesystem::file_size(seed_path) != 32)
                throw std::runtime_error("disposable wallet seed length");
            RealKeyPair wallet;
            std::ifstream seed(seed_path, std::ios::binary);
            seed.read(reinterpret_cast<char*>(wallet.private_key.data()), 32);
            if (!seed)
                throw std::runtime_error("disposable wallet unavailable");
            wallet.public_key = DerivePublicKey(wallet.private_key);
            wallet.address = PubKeyToAddress(wallet.public_key, false);
            if (i == 0 && wallet.address != BTCVELD_ISSUER_ADDRESS)
                throw std::runtime_error("disposable launch authority mismatch");
...
#endif
        RealKeyPair miner;
        miner.private_key = wallets[0].private_key;
        miner.public_key = wallets[0].public_key;
        miner.testnet = true;
        miner.address = PubKeyToAddress(miner.public_key, true);
        ...
        if (!node.BindGenerationIdentity(miner, &error))
            throw std::runtime_error("test miner: " + error);
        ...
        if (!compat::SecureRandom(token_bytes.data(), token_bytes.size()))
            throw std::runtime_error("test randomness");
        ...
        if (!http.Start())
            throw std::runtime_error("test RPC listener unavailable");
        std::vector<std::string> addresses;
        for (const auto& w : wallets)
            addresses.push_back(JsonBuilder::String(w.address));
        std::cout << "TEST_READY "
                  << JsonBuilder::Object({{"height", JsonBuilder::Number(node.GetChain().Height())},
                                          {"digest", JsonBuilder::String(Digest(node))},
                                          {"addresses", JsonBuilder::Array(addresses)},
                                          {"explorer_port", JsonBuilder::Number(uint64_t(
                                                                node.TestLocalExplorerPort()))}})
                  << std::endl;
        std::string line;
        while (std::getline(std::cin, line)) {
            try {
                ...
                if (index >= wallets.size())
                    throw std::runtime_error("test wallet index");
                const auto& key = wallets[index];
                std::string response;
#ifdef VELD_FIRST_ACTIVATION_NETWORK
                if (op.rfind("fa_", 0) == 0)
                    response = first_activation.Handle(op, command);
                else
#endif
                    if (op == "status") {
                    ...
                    const auto admission = node.EvaluateWorkAdmission(
                        work_admission::Path::SynchronousGeneration, subject);
                    response = JsonBuilder::Object(
                        {{"height", JsonBuilder::Number(node.GetChain().Height())},
                         {"digest", JsonBuilder::String(Digest(node))},
                         {"tip",
                          JsonBuilder::String(HashToHex(node.GetChain().TipCopy().GetHash()))},
                         {"active_validators",
                          JsonBuilder::Number(
                              uint64_t(node.GetValidators().GetActiveValidatorCount()))},
                         {"ready_peer_ips",
                          JsonBuilder::Number(uint64_t(peer_view.distinct_version_ips))},
                         {"admission",
                          JsonBuilder::String(work_admission::RefusalName(admission.refusal))},
                         {"admission_allowed", JsonBuilder::Bool(admission.allowed)},
                         {"admission_generation",
                          JsonBuilder::Number(
                              admission.binding ? admission.binding->validation_generation : 0)}});
                } else if (op == "synchronize") {
                    const auto peer_view = node.GetPeerHeightView();
                    const bool at_tip = IsInitialDownloadAtTip(
                        true, node.GetChain().Height(), node.GetChain().IsEmpty(),
                    ...
                    if (!at_tip || !node.ChainFullyValidated())
                        throw std::runtime_error("test chain has not synchronized");
                    node.SetIBDComplete(true);
                    node.SyncTCPIBDFlag();
                    if (!node.IsIBDComplete())
                        throw std::runtime_error("node refused synchronized state");
                    ...
                    if (op == "export_history") {
                        if (std::filesystem::exists(path))
                            throw std::runtime_error("test history already exists");
                        ...
                        for (uint64_t h = 0; h <= tip; ++h) {
                            ...
                            if (raw.size() > MAX_BLOCK_SIZE)
                                throw std::runtime_error("test history block size");
                            const uint32_t length = static_cast<uint32_t>(raw.size());
                            for (unsigned i = 0; i < 4; ++i)
                                out.put(char(length >> (i * 8)));
                            out.write(reinterpret_cast<const char*>(raw.data()), raw.size());
                            if (!out)
                                throw std::runtime_error("test history write failed");
                        }
                        out.flush();
                        if (!out)
                            throw std::runtime_error("test history flush failed");
                        response =
                            JsonBuilder::Object({{"exported_height", JsonBuilder::Number(tip)}});
                    } else {
                        std::ifstream in(path, std::ios::binary);
                        char magic[8]{};
                        in.read(magic, 8);
                        if (!in || std::string(magic, 8) != "VLDMIG01")
                            throw std::runtime_error("test history header");
                        uint64_t height = 0;
                        while (in.peek() != std::char_traits<char>::eof()) {
                            if (height > 4096)
                                throw std::runtime_error("test history height bound");
                            uint32_t length = 0;
                            for (unsigned i = 0; i < 4; ++i) {
                                const int b = in.get();
                                if (b < 0)
                                    throw std::runtime_error("test history length");
                                length |= uint32_t(b) << (i * 8);
                            }
                            if (length == 0 || length > MAX_BLOCK_SIZE)
                                throw std::runtime_error("test history body bound");
                            std::vector<uint8_t> raw(length);
                            in.read(reinterpret_cast<char*>(raw.data()), length);
                            Block block;
                            if (!in || Block::Deserialize(raw, 0, block) != raw.size() ||
                                block.Serialize() != raw)
                                throw std::runtime_error("test history block encoding");
                            if (height <= node.GetChain().Height()) {
                                if (node.GetChain().GetBlock(height).Serialize() != raw)
                                    throw std::runtime_error("test history prefix mismatch");
                            } else {
                                ...
                                const auto result = node.GetChainMut().AddBlockDirect(
                                    block, false, false, false,
                                    mining::PowAdmissionContext::Internal());
                                if (!result.IsAccepted())
                                    throw std::runtime_error(
                                        "test local history replay refused at " +
                                        std::to_string(height));
                            }
                            ++height;
                        }
                        response = JsonBuilder::Object(
                            {{"imported_height", JsonBuilder::Number(node.GetChain().Height())},
                             {"digest", JsonBuilder::String(Digest(node))}});
                    }
                } else if (op == "fund") {
                    const size_t to = size_t(Number(Field(command, "to")));
                    if (to >= wallets.size())
                        throw std::runtime_error("test recipient index");
                    response = SignAndSend(
                        node, key, "preparerawtransaction",
                        {key.address, wallets[to].address, Field(command, "amount").text});
                } else if (op == "register") {
                    response = SignAndSend(
                        node, key, "prepareregistervalidator",
                        {key.address, BytesToHex(key.public_key.data(), key.public_key.size())});
                } else if (op == "stake") {
                    response = SignAndSend(node, key, "preparestake",
                                           {key.address, Field(command, "amount").text});
                } else if (op == "propose") {
                    ...
                    else if (type_name == "protocol_upgrade")
                        type = ProposalType::PROTOCOL_UPGRADE;
                    else
                        throw std::runtime_error("unsupported test proposal type");
                    const auto challenge = GovernanceEngine::BuildProposalChallenge(
                        type, key.address, title, description, height, true, framed);
                    const auto signature = BytesToHex(Sign(key.private_key, Hash256d(challenge)));
                    response =
                        SignAndSend(node, key, "preparegovproposal",
                                    {key.address, type_name, title, description,
                                     BytesToHex(key.public_key.data(), key.public_key.size()),
                                     signature, std::to_string(height)});
                } else if (op == "vote") {
                    const auto proposal = Number(Field(command, "proposal"));
                    const auto height = node.GetChain().Height();
                    const std::string identity = Field(command, "identity").text;
                    const auto challenge = GovernanceEngine::BuildVoteChallenge(
                        proposal, VoteChoice::YES, height, identity);
                    const auto signature = BytesToHex(Sign(key.private_key, Hash256d(challenge)));
                    std::vector<std::string> params{
                        key.address, std::to_string(proposal),
                        "yes",       BytesToHex(key.public_key.data(), key.public_key.size()),
                        signature,   std::to_string(height)};
                    if (!identity.empty())
                    ...
                    const auto grant =
                        Result(node, "getworkadmission",
                               {"validator_endorsement", std::to_string(height), hash});
                    const std::string binding = Field(grant, "binding").text;
                    const std::string signing_token = Field(grant, "signing_token").text;
                    try {
                        (void)Result(node, "beginworksigning",
                                     {"validator_endorsement", binding, signing_token});
                        if (!endorsement_journal.record(journal_key, hash))
                            throw std::runtime_error("test endorsement journal write failed");
                        const auto message =
                            ValidatorRegistry::BuildEndorseMessage(height, HexToHash(hash));
                        const auto signature = BytesToHex(Sign(key.private_key, message));
                        const auto wire = ValidatorRegistry::BuildEndorseOp(
                            height, hash, signature, key.address,
                            NextInclusionHeight(node.GetChain().Height()));
                        const auto* sponsor_field = command.Get("sponsor");
                        const size_t sponsor =
                            sponsor_field ? size_t(Number(*sponsor_field)) : index;
                        if (sponsor >= wallets.size())
                            throw std::runtime_error("test sponsor index");
                        response =
                            SignAndSend(node, wallets[sponsor], "preparerawop",
                                        {wallets[sponsor].address, wire}, {binding, signing_token});
                    } catch (...) {
                        try {
                            (void)Result(node, "cancelworksigning", {signing_token});
                        } catch (...) {
                        }
                        throw;
                    }
                ...
                std::cout << "TEST_RESULT "
                          << JsonBuilder::Object({{"test_error", JsonBuilder::String(e.what())}})
                          << std::endl;
            }
        }
        http.Stop();
