...
std::vector<uint8_t> Journal(const std::vector<fq::SignedVote>& votes) {
    ...
    const auto checksum = state_digest::sha256_domain(fq::DOMAIN_EQUIVOCATION_JOURNAL, out);
...
int main() {
    try {
        ...
        const auto absent =
            [&](const fq::SignedVote&,
                const fq::SignedVote&) -> std::optional<fq::ValidatedEquivocationEvidence> {
            ++membership_checks;
            return std::nullopt;
        };
        const auto member_present = [&](const fq::SignedVote& x, const fq::SignedVote& y) {
            ++membership_checks;
            return fq::ValidateEquivocationPairForRetainedMember(x, y, 0, a.set_root, member, 20,
                                                                 fq::NETWORK_ID, genesis);
        };
        ...
        Check(restore(wire, absent, boundary + 1),
              "expired signed journal blocks recovery after membership ages out");
        ...
        Check(!restore(wire, absent, boundary + 1, fq::NETWORK_ID ^ 1),
              "wrong chain signature accepted");
        auto bad = a;
        bad.signature[0] ^= 1;
        ...
        bad = b;
        bad.target.height += 20;
        sign(bad);
        Check(!restore(Journal({a, bad}), absent, boundary + 1),
              "different target heights skipped");
        Check(!restore(Journal({b, a}), absent, boundary + 1),
              "noncanonical pair ordering skipped");
        Check(!restore(Journal({a, b, a, b}), absent, boundary + 1),
              "duplicate expired rows skipped");
        auto corrupt = wire;
        corrupt.back() ^= 1;
