...
int main() {
    ...
    for (const std::string bad :
         {"", "127.000.000.001", "[::1]", "fe80::1%3", "192.0.2.1,198.51.100.1", "not-an-ip"})
        check(!CanonicalIp(bad, canonical));

    std::array<uint8_t, 32> token{};
    check(DecodeToken("000102030405060708090a0b0c0d0e0f"
    ...
    check(result.accepted && !result.forwarded && result.identity == "peer:127.0.0.1");

    headers[kAuthorizationHeader] = "VeldProxy v1=000102030405060708090a0b0c0d0e0f"
                                    "101112131415161718191a1b1c1d1e1f";
    headers[kClientIpHeader] = "2001:0db8::1";
    result = Resolve(config, "127.0.0.1", headers);
    check(result.accepted && result.forwarded && result.identity == "client:2001:db8::1");
    ...
    const auto directory =
        std::filesystem::temp_directory_path() / ("veld-proxy-fixture-" + std::to_string(nonce));
    std::string error;
    check(veld::channel::secure_file::EnsurePrivateDirectory(directory.string(), &error));
    const auto token_path = directory / "proxy.token";
    const std::string encoded = "000102030405060708090a0b0c0d0e0f"
                                "101112131415161718191a1b1c1d1e1f\n";
    check(veld::channel::secure_file::AtomicWriteNew(
        token_path.string(), reinterpret_cast<const uint8_t*>(encoded.data()), encoded.size(),
        &error, true));
    Configuration loaded;
    check(Configure(loaded, "127.0.0.1", token_path.string(), &error));
    check(loaded.enabled && loaded.peer == "127.0.0.1" && ConstantTimeEqual(loaded.token, token));
    std::error_code cleanup_error;
    std::filesystem::remove_all(directory, cleanup_error);
    check(!cleanup_error);

    std::cout << "PASS trusted_proxy_tests checks=" << checks
