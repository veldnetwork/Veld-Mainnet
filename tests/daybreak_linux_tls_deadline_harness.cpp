...
#define main daybreak_unreferenced_desktop_main
...
int main(int argc, char** argv) {
    ...
    if (port_value == 0 || port_value > 65535 || deadline_ms == 0 ||
        deadline_ms > 5000) {
    ...
    if (mode == "plaintext") {
        ...
        const std::string result = proxy_rpc(
            "http://example.com:80", body, "");
        const auto elapsed =
            std::chrono::duration_cast<std::chrono::milliseconds>(
                DesktopTlsClock::now() - started).count();
    ...
    const auto started = DesktopTlsClock::now();
    const std::string result = proxy_rpc_https_until(
        endpoint, body, "",
