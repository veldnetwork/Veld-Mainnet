...
def run(
    argv: list[str], env: dict[str, str], timeout: int = 120
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="veld-offline-signer-") as tmp:
        ...
        passphrase = "Veld-Test-Offline-Process-Test-Only-31!"
        env = os.environ.copy()
        ...
        identity_digest = (
            (fixture_dir / "canonical.identity-digest.txt").read_text(encoding="ascii").strip()
        )
        ...
        valid_rows = [
            row.split("\t")
            for row in (fixture_dir / "valid-operations.tsv")
        ...
        for (
            ...
            assert verified.returncode == 0 and "PASS signed-inputs=" in verified.stdout, (
        ...
        for suffix, unapproved in (
            ...
            if suffix == "nested":
                nested = {
                    "signing_intent": json.loads(
                        (fixture_dir / "canonical.intent.json").read_text(encoding="utf-8")
                    )
                }
                unapproved.write_text(json.dumps(nested, separators=(",", ":")), encoding="utf-8")
        ...
        signing_json = json.dumps(approved_shape["signing_intent"], separators=(",", ":"))
        ...
        escaped_member = approved_text.replace('"signing_intent"', '"signing_int\\u0065nt"', 1)
        document_cases = {
        ...
        alternate = run(
            [
                ...
                "--intent",
                str(fixture_dir / "alternate_vout.intent.json"),
                "--out",
                str(alternate_out),
            ],
            env,
    ...
    print(f"PASS offline_signer_process_tests checks={checks} cases={len(rows)}")
    return 0
