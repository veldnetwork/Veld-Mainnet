...
unknown = dict(
    schema=1,
    local_status_valid=False,
    mining_status_valid=False,
    peer_status_valid=False,
...
known = dict(
    unknown,
    local_status_valid=True,
    mining_status_valid=True,
    ...
    daemon=dict(
        ...
        network_magic=0x56454C44,
        protocol_height=0,
        asert_height=0,
        migration_height=0,
        security_height=2880,
        verification_height=12,
        verification_target=64,
        ibd_complete=False,
...
for change in (
    {'version': '3.1.1\nspoof'},
    {'network_magic': 2**32},
    {'executable_sha256': 'keyfile'},
    {'ibd_complete': 1},
    {'profile': '<script>'},
):
    v = copy.deepcopy(known)
    v['daemon'].update(change)
    rejects(v)
...
with tempfile.TemporaryDirectory(prefix='veld-portal-diagnostics-') as temp:
    ...
    with closing(sqlite3.connect(path)) as db:
        db.execute(
            'CREATE TABLE samples(id INTEGER PRIMARY KEY,device_id INTEGER NOT NULL,captured_at INTEGER NOT NULL,height INTEGER NOT NULL,hashrate REAL NOT NULL,peers INTEGER NOT NULL,inbound INTEGER NOT NULL,mining_active INTEGER NOT NULL)'
        )
    ...
    with patch.object(p.time, 'time', return_value=100):
        store.report(token, report)
    report['snapshot']['diagnostics'] = unknown
    with patch.object(p.time, 'time', return_value=101):
        store.report(token, report)
    report['warning'] = 'Local node status is unavailable.'
    with patch.object(p.time, 'time', return_value=102):
        store.report(token, report)
    report['snapshot']['diagnostics'] = dict(
        unknown, process_running=False, last_exit_at=103, last_exit_code=75
    )
    with patch.object(p.time, 'time', return_value=103):
        store.report(token, report)
    # Reopen the database to establish persisted, not just in-memory, evidence.
    store = p.PortalStore(path)
    with store.database() as db:
        rows = [
            p.present_sample(dict(r))
            for r in db.execute('SELECT * FROM samples WHERE device_id<>99 ORDER BY id')
        ]
    check(len(rows) == 4)
    check(
        rows[0]['hashrate'] == 0 and rows[0]['diagnostics']['daemon']['verification_height'] == 12
    )
    check(rows[1]['hashrate'] is None and rows[1]['height'] is None)
    check(rows[2]['warning'] == 'Local node status is unavailable.')
    check(
        rows[3]['diagnostics']['last_exit_code'] == 75
        and rows[3]['diagnostics']['process_running'] is False
    )
print(
    f'PASS portal diagnostics: {checks} checks; SQLite migration, unknown readings, mixed versions, warning and exit transitions'
)
