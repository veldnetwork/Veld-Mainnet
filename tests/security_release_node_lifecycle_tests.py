...
class StandaloneNode:
    def __init__(self, binary, directory, p2p, rpc, peer, env, phase):
        ...
        self.env = env
        self.binary = binary
        self.log = (directory.parent / (directory.name + '-' + phase + '.log')).open(
            'w', encoding='utf-8'
        )
        self.command = [
            str(binary),
        ...
        try:
            ...
            while time.monotonic() < deadline:
                assert self.process.poll() is None, (
                    'standalone node exited during startup; inspect its local log'
                )
...
def main():
    ...
    run = args.completed_run.resolve()
    out = run / 'release-qualification'
    full_sync = args.scenario == 'full-sync'
    result = out / (
        'standalone-full-sync-lifecycle-results.json'
        if full_sync
        else 'standalone-node-lifecycle-results.json'
    )
    assert not result.exists()
    source_backup = run / (
        'sync-recovery-cold-backup-3' if full_sync else 'upgrade-recovery-cold-backup-3'
    )
    if full_sync:
    ...
    report = {
        'status': 'running',
        'checks': {},
        'scope': 'actual src/veld-node.cpp main loop with isolated regtest profile',
    ...
    def converged(a, b, timeout=180):
        ...
        while time.monotonic() < deadline:
            aa = a.rpc('getblockchaininfo')
            bb = b.rpc('getblockchaininfo')
            if (
                aa['ibd_complete']
                and bb['ibd_complete']
                and aa['blocks'] == bb['blocks']
                and aa['best_block_hash'] == bb['best_block_hash']
                and a.rpc('getnetworkinfo')['connections'] > 0
                and b.rpc('getnetworkinfo')['connections'] > 0
            ):
                return aa
            time.sleep(1)
    ...
    try:
        for role in ('a', 'b'):
            directory = run / (
                ('node-standalone-full-sync-' if full_sync else 'node-standalone-lifecycle-') + role
            )
            assert not directory.exists()
            private_copy(source_backup, directory, run)
            directories.append(directory)
        a = StandaloneNode(binary, directories[0], ports[0], ports[1], ports[2], env, 'initial')
        nodes.append(a)
        b = StandaloneNode(binary, directories[1], ports[2], ports[3], ports[0], env, 'initial')
        nodes.append(b)
        state = converged(a, b)
        assert state['blocks'] == 3360 and state['txindex_enabled']
            ...
            report['samples'].append(
                {
                    ...
                    'height': state['blocks'],
                    'tip': state['best_block_hash'],
                    'ibd_complete': state['ibd_complete'],
                    'a_connections': a.rpc('getnetworkinfo')['connections'],
                    'b_connections': b.rpc('getnetworkinfo')['connections'],
                }
            )
            time.sleep(10)
        report['checks']['connected_synced_operation_survives_peer_freshness_interval'] = True
        b.stop()
        b = StandaloneNode(binary, directories[1], ports[2], ports[3], ports[0], env, 'restarted')
        nodes.append(b)
        ...
        report['final_state'] = {
            key: restarted[key]
            for key in ('blocks', 'best_block_hash', 'ibd_complete', 'txindex_enabled')
        }
        report['status'] = 'integration_pass'
        print('PASS standalone node lifecycle checks=' + str(len(report['checks'])), flush=True)
