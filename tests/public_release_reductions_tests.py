...
clearnet_launcher = (ROOT / "pkg/Start Mining (Clearnet).bat").read_text(encoding="utf-8")
...
address_history = (ROOT / "include/core/address_history.h").read_text(encoding="utf-8")

check("VELD_ENABLE_DIAGNOSTIC_TX_HISTORY" in constants, "missing public/history profile interlock")
check("VELD_ENABLE_SNAPSHOT_BOOTSTRAP" in constants, "missing snapshot storage-profile interlock")
check(
    "VELD_ENABLE_UPNP" in constants or "VELD_ENABLE_UPNP" in nat,
    "missing public/UPnP profile interlock",
)
check(
    'methods_["gettxhistory"]' not in rpc and "gettxhistory" not in rpc,
    "legacy gettxhistory scanner/source remains in the RPC registry",
)
check(
    'methods_["getearnings"]' not in rpc and "getearnings" not in rpc,
...
check(
    '"gettxhistory"' not in allowlist, "hosted/public desktop allow-list still exposes gettxhistory"
)
check('methods_["getaddresshistory"]' in rpc, "bounded indexed address-history RPC is missing")
check('"getaddresshistory"' in allowlist, "hosted desktop does not expose bounded indexed history")
check(
    "rpc('gettxhistory'" not in ui and 'rpc("gettxhistory"' not in ui,
    "desktop still contains a gettxhistory RPC call",
)
check(
    "getearnings" not in desktop and "getearnings" not in ui and "getearnings" not in wallet_cli,
    "desktop or wallet still retains the unbounded earnings/history method",
)
history_helper = ui[
    ui.index("function publicAddressHistory(address, limit)") : ui.index(
        "function rpc(method, params)"
    )
]
check(
    "rpc('getaddresshistory'" in history_helper
    and "Math.min(50" in history_helper
    and "gettxhistory" not in history_helper
    and "setTimeout" not in history_helper
    and "setInterval" not in history_helper,
    "desktop history helper is not bound to the capped indexed RPC",
)
earnings_helper = ui[
    ui.index("function loadEarningsPage(addr) {") : ui.index("function loadPayoutEndorsements() {")
]
check(
    "publicAddressHistory(addr, 50)" in earnings_helper
    and "gettxhistory" not in earnings_helper
    and "setTimeout" not in earnings_helper
    and "setInterval" not in earnings_helper,
    "desktop earnings view is not using bounded indexed history",
)
check(
    'method == "gettxhistory"' not in wallet_cli and 'method":"gettxhistory' not in wallet_cli,
    "wallet CLI still tells users to call unavailable history",
)
check(
    "txhistory" not in explorer.lower(), "legacy explorer txhistory route/scanner remains in source"
)
check(
    'resource == "addresshistory"' in explorer and 'limit > 50' in explorer,
    "explorer indexed-history route is missing its hard page cap",
)
check(
    "MAX_PAGE_SIZE = 50" in address_history
    and "IterateFrom(prefix, cursor" in address_history
    and "block_loader"
    not in address_history[address_history.index("inline std::optional<Page> ReadPage") :],
    "address-history query can exceed its row cap or load block bodies",
)

check("PublicSnapshotDatadirRefusal" in node, "legacy public snapshot-marker boundary disappeared")
check(".snapshot-fast-start-revoked" in node, "public startup omits the snapshot revocation marker")
check(
    "--snapshot-bootstrap" in node_main
    and "--no-snapshot" in node_main
    and "--full-ibd" in node_main,
    "public CLI does not expose the bounded snapshot/full-IBD choice",
)
check(
    "#if defined(VELD_ENABLE_SNAPSHOT_BOOTSTRAP)" in rpc and 'methods_["dumpsnapshot"]' in rpc,
    "snapshot RPC is not compile-profile gated",
)
check(
    "MaybePreferSnapshotAtStartup" not in node_main and "TryAutoSnapshotBootstrap" not in node,
    "retired mirror-driven snapshot entrypoint returned",
)
check(
    '#if defined(VELD_ENABLE_SNAPSHOT_BOOTSTRAP)\n#include "snapshot_manifest.h"' in node
    and "SetBackgroundValidationOnly" in node
    and "SnapshotValidationBase" in node,
    "generic snapshot implementation is not behind its explicit profile",
)
check(
    "#if defined(VELD_ENABLE_SNAPSHOT_BOOTSTRAP)\n    background_validation_base =" in node_main
    and "#if defined(VELD_ENABLE_SNAPSHOT_BOOTSTRAP)\n        if (node.SnapshotBackgroundVerificationFailed())"
    in node_main,
...
check(
    "#if defined(VELD_ENABLE_SNAPSHOT_BOOTSTRAP)" in snapshot_impl_prefix[-700:],
    "snapshot validate/download/promote entrypoints remain in public preprocessing",
)
check(
    "--bootstrap-only" not in launcher and "--snapshot-bootstrap" in launcher,
    "shipped Windows launcher does not select signed snapshot bootstrap",
)
check(
    'data-mode="snapshot"' in portal and 'payload["mode"] not in {' in portal,
    "portal does not offer the bounded snapshot/full synchronization modes",
)
check(
    'L"Snapshot bootstrap: available' in gui and 'L" --snapshot-bootstrap"' in gui,
    "GUI does not expose signed snapshot bootstrap",
)
check(
    "A snapshot is an availability optimization" in explorer
    and "independent genesis IBD" in explorer,
    "Explorer rules omit the public snapshot trust boundary",
)
check(
    "defined(VELD_PUBLIC_TESTNET) || defined(VELD_PUBLIC_RELEASE)" in node_main,
...
check(
    start_body.index("ReadIndependentValidationRequirement_")
    < start_body.index("tcp_server_ = std::make_unique"),
    "snapshot validation requirement is not loaded before service construction",
)

check("#if defined(VELD_ENABLE_UPNP)" in nat, "UPnP implementation is not compile-profile gated")
check(
    "TryUpnp(localip" in nat
    and nat.index("#if defined(VELD_ENABLE_UPNP)", nat.index("void RunLoop"))
    < nat.index("TryUpnp(localip", nat.index("void RunLoop")),
    "worker can still call UPnP without the opt-in profile",
)
check(
    'arg == "--upnp"' in node_main
    and '<< " is not supported in the "' in node_main
    and '<< "Veld " << CLIENT_VERSION << " public release' in node_main,
    "public CLI does not explicitly reject --upnp",
)
check(
    "NAT-PMP/PCP" in node_main and "NAT-PMP/PCP/UPnP" not in node_main,
    "public help still advertises UPnP",
)
check(
    "UPnP" not in clearnet_launcher and "NAT-PMP/PCP" in clearnet_launcher,
    "clearnet launcher still advertises public UPnP",
)
check(
    'arg.find("snapshot")' in node_main and 'arg.rfind("--upnp=", 0)' in node_main,
    "public pre-scan does not reject snapshot/UPnP assignment aliases",
)
check(
    "veld-node: unknown option '" in node_main, "node parser still silently ignores unknown options"
)
...
for controller, text in (("linux", linux), ("windows", windows)):
    ...
    check(
        "node)" in text and "-DVELD_ENABLE_SNAPSHOT_BOOTSTRAP" in text,
...
if binary_value:
    ...
    check(
        help_result.returncode == 0 and "Usage: veld-node" in help_result.stdout,
        "ordinary public help probe failed",
    )
    deployment = run_node("--deployment-info")
    check(
        deployment.returncode == 0
        and '"profile_id":"veld-public-mainnet-v2"' in deployment.stdout
        and '"snapshot_bootstrap_compiled":true' in deployment.stdout,
    ...
    for control in ("--snapshot-bootstrap", "--no-snapshot", "--full-ibd"):
        ...
        check(
            controlled.returncode == 0
            and '"snapshot_bootstrap_compiled":true' in controlled.stdout,
    ...
    with tempfile.TemporaryDirectory(prefix="veld-public-full-ibd-") as datadir:
        full_ibd = run_node("--full-ibd", "--reindex-canonical", "--datadir", datadir)
        check(
            full_ibd.returncode == 1
            and "offline reindex requires an existing" in full_ibd.stderr
            and "unknown option" not in full_ibd.stderr
            and "snapshot bootstrap" not in full_ibd.stderr,
...
print(f"PASS public_release_reductions_tests checks={checks}")
