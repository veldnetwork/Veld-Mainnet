...
def main():
    ...
    result_path = build / 'governance-results.json'
    ...
    identities = subprocess.run(
        [
            str(build / 'release-identities.exe'),
            str(run),
            str(run),
            str(run / 'governance-private'),
        ],
        env=env,
        capture_output=True,
        text=True,
    ...
    try:
        node = Node(binary, run / 'node-governance-ten')
        nodes.append(node)
        ...
        for i in range(8, 11):
            node.control('fund', to=i, amount='60')
            node.generate(1)
            node.control('register', wallet=i)
            node.generate(1)
        assert node.control('status')['active_validators'] == 10
        report['checks']['ten_validators_have_real_mined_utxo_bonds'] = True
        for i in range(1, 11):
            node.control('endorse', wallet=i, sponsor=0, height=node.control('status')['height'])
        node.generate(1)
        ...
        node.generate(1)
        pr = proposal(node, title)
        assert pr['type'] == 'protocol_upgrade', pr
        for i in range(1, 10):
            node.control('vote', wallet=i, proposal=pr['id'], identity=pr['vote_identity'])
        node.generate(1)
        nine = proposal(node, title)
        ...
        node.generate(1)
        ten = proposal(node, title)
        locked_height = node.control('status')['height']
        ...
        report['nine_votes'] = nine
        report['timelocked'] = ten
        save()
        locked = node.control('status')
        node.stop()
        node = Node(binary, run / 'node-governance-ten', '-timelock-restart')
        nodes.append(node)
