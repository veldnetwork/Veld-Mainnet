...
function setup() {
    ...
    const original = [
        { id: 11, name: 'First machine' },
        { id: 22, name: 'Second machine' },
    ];
    const calls = [],
        notices = [];
    ...
    vm.runInContext(
        'let pairingBusy=false;\n' +
            ['openAddMachine', 'closeAddMachine', 'claimMachine', 'refresh']
                .map(extract)
                .join('\n'),
        context,
    );
...
(async () => {
    ...
    assert.equal(
        f.node('add-pair-code').value,
        ' abcd-efgh ',
        'Background refresh preserves input',
    );
    assert.equal(f.context.selected, 22);
    await f.context.claimMachine(event());
    assert.deepEqual(
        Array.from(f.context.devices, (d) => d.id),
        [11, 22, 33],
    );
    ...
    assert.deepEqual(
        Array.from(bad.context.devices, (d) => d.id),
        [11, 22],
    );
    ...
    const busy = setup();
    let resolve;
    busy.state.gate = new Promise((done) => {
        resolve = done;
    });
    ...
    await pending;
    assert.equal(
        busy.calls.filter((c) => c.method === 'POST').length,
        1,
        'Double-submit does not duplicate claims',
    );
    ...
    assert.equal(
        retry.node('pair-dialog').open,
        false,
        'A successful claim is not submitted twice after a refresh failure',
    );
    ...
    console.log(
        'PASS pairing: existing devices, refresh preservation, success selection, errors, retry, double-submit, cancel, and PWA entry',
    );
})().catch((error) => {
    console.error(error);
    process.exitCode = 1;
