#!/usr/bin/env python3
"""Strict process watchdog for work-admission deadlock.

Regression mode requires completion with matching state before the deadline.
A separately selected negative-control mode requires the fixture to reach
WORK_ADMISSION_DEADLOCK_ARMED and then fail or time out. Windows requires the observed timeout exactly, while platforms whose
...
def main() -> int:
    ...
    armed = "WORK_ADMISSION_DEADLOCK_ARMED verified_peers=2 height=0" in stdout
    records = [
        line[len("WORK_ADMISSION_DEADLOCK_JSON ") :]
        for line in stdout.splitlines()
        if line.startswith("WORK_ADMISSION_DEADLOCK_JSON ")
    ]
    if args.expect in {"timeout", "fail-or-timeout"}:
        ...
        disposition = "regression_pass"

    summary = {
        "schema": "VELD_WORK_ADMISSION_DEADLOCK_PROCESS_V1",
    ...
    print(
        "PASS work_admission_deadlock_process_tests "
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"FAIL work_admission_deadlock_process_tests: {exc}",
            file=sys.stderr,
        )
        raise SystemExit(1)
