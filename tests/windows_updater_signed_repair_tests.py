...
def main():
    ...
    for version, expected in PACKAGES.items():
        ...
        request = urllib.request.Request(
            ...
            headers={
                ...
                'User-Agent': 'VELD-Updater-Qualification/3.1.7',
            },
        )
    ...
    env['PATH'] = os.pathsep.join(
        map(str, (system_root / 'System32', system_root, powershell.parent))
    )
    subprocess.run(
        [
            str(powershell),
    ...
    result = json.loads(
        (root / 'repaired client/veld-data/repair-test-result.json').read_text(encoding='utf-8-sig')
    )
