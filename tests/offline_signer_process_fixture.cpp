...
constexpr const char* TOKEN_RECIPIENT =
...
std::vector<uint8_t> OpenReserveProof() {
    const std::vector<uint8_t> parent = BitcoinTx(
        {{TaggedHash("process-open-funding"), 0}},
        {{MINT_SATS + 1'000, {0x51}}});
    reserve::Claim claim;
    ...
    claim.bitcoin_tx = BitcoinTx(
        {{BitcoinTxid(parent), 0}},
        {{MINT_SATS, BtcVeldCustodySpk()},
         {0, reserve::detail::OpenAuthScript(
             claim.prior_commitment, TOKEN_RECIPIENT, claim.mint_amount)}});
...
std::string Proposal(const Transaction& tx,
                     ...
                     uint64_t total_input, uint64_t total_output,
                     uint64_t fee, uint64_t change,
                     bool omit_parent = false,
    ...
    for (size_t i = 0; i < tx.inputs.size(); ++i) {
        ...
        const auto& claimed_script = i == 0 && first_claimed_script
            ? *first_claimed_script : owned;
        inputs += "{\"index\":" + std::to_string(i) +
            ",\"sighash_hex\":" + Quote(BytesToHex(ComputeSighash(
                tx, static_cast<uint32_t>(i), owned))) +
            ",\"prev_script_hex\":" + Quote(BytesToHex(claimed_script)) +
            ",\"value\":" + std::to_string(claim);
        if (!(omit_parent && i == 0))
            inputs += ",\"parent_tx_hex\":" +
                Quote(BytesToHex(parent.Serialize()));
        inputs += "}";
    }
    inputs += "]";
    std::string proposal = "{\"unsigned_tx_hex\":" + Quote(BytesToHex(tx.Serialize())) +
        ",\"inputs\":" + inputs +
    ...
    if (!reserve_state_hex.empty() || !reserve_supply_sats.empty())
        proposal += ",\"reserve_prior_state_hex\":" +
            Quote(reserve_state_hex) +
...
std::string IntentJson(const offline_signing::Intent& intent) {
    return "{\"version\":" + Quote(intent.version) +
        ...
        ",\"expected_change_destination\":" +
            Quote(intent.expected_change_destination) +
        ...
        ",\"source_transactions_digest\":" +
            Quote(intent.source_transactions_digest) +
        ",\"complete_output_digest\":" +
            Quote(intent.complete_output_digest) +
        ",\"operation_identity_digest\":" +
            Quote(intent.operation_identity_digest) +
...
int VerifySigned(const std::string& address, const fs::path& prepared_path,
    ...
    btc_buy::StrictJsonParser parser(
        prepared_json, offline_signing::kMaxPreparedJsonBytes, true);
    offline_signing::VerifiedPrepared verified;
    if (!parser.Parse(prepared_root, error) ||
        !offline_signing::AuthenticatePrepared(
            prepared_root, owned, verified, error))
        throw std::runtime_error("prepared verification failed: " + error);
    std::string signed_hex = Read(signed_path);
    while (!signed_hex.empty() &&
           (signed_hex.back() == '\r' || signed_hex.back() == '\n'))
        signed_hex.pop_back();
    std::vector<uint8_t> signed_raw;
    if (!offline_signing::DecodeLowerHex(
            signed_hex, offline_signing::kMaxUnsignedTransactionBytes,
            signed_raw))
        throw std::runtime_error("signed artifact is not canonical hex");
    Transaction signed_tx;
    if (Transaction::Deserialize(signed_raw, 0, signed_tx) !=
            signed_raw.size() || signed_tx.Serialize() != signed_raw ||
        signed_tx.inputs.size() != verified.tx.inputs.size())
        throw std::runtime_error("signed transaction is noncanonical");
    Transaction unsigned_tx = signed_tx;
    for (auto& input : unsigned_tx.inputs) input.script_sig.clear();
    ...
    for (size_t i = 0; i < signed_tx.inputs.size(); ++i) {
        ...
        if (!pqc::ParseScriptSig(signed_tx.inputs[i].script_sig,
                                 encoded_signature, pubkey) ||
            encoded_signature.size() != 3311U ||
            encoded_signature.front() != SCHEME_ID_MLDSA65 ||
        ...
        const Secp256k1SigDER raw_signature(
            encoded_signature.begin() + 1, encoded_signature.end() - 1);
        if (pubkey_script != owned ||
            !Verify(pubkey,
                    ComputeSighash(verified.tx, static_cast<uint32_t>(i),
                                   verified.prev_scripts[i]),
                    raw_signature))
...
int main(int argc, char** argv) {
    try {
        ...
        std::string family;
        if (!BtcVeldRelayPayloadShape(identity, family) ||
            family != "VELD_BHDR|")
        ...
        const Transaction valid = Spend(
            parent, 0, {TxOutput(100000, owned), TxOutput(0, marker)});
        const auto intent = offline_signing::MakeIntent(
            valid, {parent.Serialize()}, "VELD_BHDR|", "", 0,
            address, 100000, identity);
        ...
        valid_operations.push_back({
            ...
            Proposal(valid, {parent}, owned,
                     200000, 100000, 100000, 100000)});

        const std::string anchor_identity = ValidAnchorIdentity();
        const Transaction anchor_tx = Spend(parent, 0, {
            TxOutput(100000, owned),
            TxOutput(0, BuildOpReturnScript(anchor_identity))});
        valid_operations.push_back({
            "anchor", "sign-op", "VELD_ANCHOR|", "", 0,
            anchor_identity,
            Proposal(anchor_tx, {parent}, owned,
                     200000, 100000, 100000, 100000)});
        ...
        const Transaction reserve_tx = Spend(parent, 0, {
            TxOutput(100000, owned),
            TxOutput(0, BuildOpReturnScript(reserve_identity))});
        valid_operations.push_back({
            "rsv1", "sign-op", reserve::PUBLIC_CARRIER_PREFIX, "", 0,
            reserve_identity,
            Proposal(reserve_tx, {parent}, owned,
                     200000, 100000, 100000, 100000)});

        const std::string mint_identity =
            std::string("VELD_TOKEN|MINT|btcVELD|") + address + "|" +
            TOKEN_RECIPIENT + "|" + std::to_string(MINT_SATS) + "|" +
            reserve::ISSUER_MEMO_PREFIX + BytesToHex(reserve_proof);
        const Transaction mint_tx = Spend(parent, 0, {
            TxOutput(100000, owned),
            TxOutput(0, BuildOpReturnScript(mint_identity))});
        const reserve::State empty_reserve_state{};
        valid_operations.push_back({
            "rtp1_mint", "sign-tx", "BTCVELD_MINT", TOKEN_RECIPIENT,
            MINT_SATS, mint_identity,
            Proposal(mint_tx, {parent}, owned,
                     200000, 100000, 100000, 100000, false, UINT64_MAX,
                     nullptr, BytesToHex(reserve::EncodeState(
                         empty_reserve_state)), "0")});

        const std::string allocation_id = c1reserve::AllocationId(1);
        const std::string allocation_commitment(64, '4');
        const std::string c1_memo = c1reserve::EncodeMemo(
            allocation_id, allocation_commitment);
        const std::string c1_identity =
            std::string("VELD_TOKEN|RESERVE|btcVELD|") + address + "|" +
            TOKEN_RECIPIENT + "|" + std::to_string(MINT_SATS) + "|" +
            c1_memo;
        const Transaction c1_tx = Spend(parent, 0, {
            TxOutput(100000, owned),
            TxOutput(0, BuildOpReturnScript(c1_identity))});
        valid_operations.push_back({
            "c1_reserve", "sign-tx", "BTCVELD_C1_RESERVE",
            TOKEN_RECIPIENT, MINT_SATS, c1_identity,
            Proposal(c1_tx, {parent}, owned,
                     200000, 100000, 100000, 100000)});

        std::ofstream valid_manifest(
            output / "valid-operations.tsv", std::ios::binary | std::ios::trunc);
        ...
        for (const auto& operation : valid_operations) {
            const std::string prepared_name =
                "valid-" + operation.name + ".prepared.json";
        ...
        cases.push_back({"canonical", Proposal(
            valid, {parent}, owned, 200000, 100000, 100000, 100000), true});
        cases.push_back({"wrong_value", Proposal(
            valid, {parent}, owned, 200000, 100000, 100000, 100000,
            false, 199999), false});
        cases.push_back({"missing_parent", Proposal(
            valid, {parent}, owned, 200000, 100000, 100000, 100000,
            true), false});
        const Transaction different = Parent(
            200000, owned, "different-parent");
        cases.push_back({"parent_txid_mismatch", Proposal(
            valid, {different}, owned, 200000, 100000, 100000, 100000),
            false});
        const Transaction wrong_vout = Spend(
            parent, 1, {TxOutput(100000, owned), TxOutput(0, marker)});
        cases.push_back({"wrong_vout", Proposal(
            wrong_vout, {parent}, owned, 200000, 100000, 100000, 100000),
            false});

        Transaction duplicate = valid;
        duplicate.inputs.push_back(duplicate.inputs.front());
        cases.push_back({"duplicate_input", Proposal(
            duplicate, {parent, parent}, owned,
            400000, 300000, 100000, 300000), false});

        const Transaction hidden = Spend(parent, 0, {
            TxOutput(99999, owned), TxOutput(1, std::vector<uint8_t>{0x51}),
            TxOutput(0, marker)});
        cases.push_back({"hidden_output", Proposal(
            hidden, {parent}, owned, 200000, 100000, 100000, 100000),
            false});
        const Transaction split_change = Spend(parent, 0, {
            TxOutput(50000, owned), TxOutput(50000, owned),
            TxOutput(0, marker)});
        cases.push_back({"split_change", Proposal(
            split_change, {parent}, owned,
            200000, 100000, 100000, 100000), false});
        const Transaction marker_before_change = Spend(parent, 0, {
            TxOutput(0, marker), TxOutput(100000, owned)});
        cases.push_back({"marker_before_change", Proposal(
            marker_before_change, {parent}, owned,
            200000, 100000, 100000, 100000), false});
        std::vector<uint8_t> redirected_script(25, 0x42);
        redirected_script[0] = 0x76; redirected_script[1] = 0xa9;
        redirected_script[2] = 0x14; redirected_script[23] = 0x88;
        redirected_script[24] = 0xac;
        const Transaction redirected = Spend(parent, 0, {
            TxOutput(100000, redirected_script), TxOutput(0, marker)});
        cases.push_back({"redirected_change", Proposal(
            redirected, {parent}, owned, 200000, 100000, 100000, 0), false});
        cases.push_back({"wrong_claimed_script", Proposal(
            valid, {parent}, owned, 200000, 100000, 100000, 100000,
            false, UINT64_MAX, &redirected_script), false});
        const Transaction foreign_parent = Parent(
            200000, redirected_script, "foreign-script-parent");
        const Transaction foreign_spend = Spend(
            foreign_parent, 0,
            {TxOutput(100000, owned), TxOutput(0, marker)});
        cases.push_back({"parent_script_not_owned", Proposal(
            foreign_spend, {foreign_parent}, owned,
            200000, 100000, 100000, 100000), false});
        const Transaction exact_fee_parent = Parent(
            100000, owned, "omitted-change-parent");
        const Transaction omitted = Spend(
            exact_fee_parent, 0, {TxOutput(0, marker)});
        cases.push_back({"omitted_change", Proposal(
            omitted, {exact_fee_parent}, owned, 100000, 0, 100000, 0),
            false});
        const Transaction high_parent = Parent(
            200001, owned, "fee-high-parent");
        const Transaction fee_high = Spend(
            high_parent, 0,
            {TxOutput(100000, owned), TxOutput(0, marker)});
        cases.push_back({"fee_above_cap", Proposal(
            fee_high, {high_parent}, owned,
            200001, 100000, 100001, 100000), false});
        const std::string alternate_identity = ValidAnchorIdentity();
        std::string alternate_family;
        if (!BtcVeldRelayPayloadShape(
                alternate_identity, alternate_family) ||
        ...
        const Transaction altered = Spend(parent, 0, {
            TxOutput(100000, owned),
            TxOutput(0, BuildOpReturnScript(alternate_identity))});
        cases.push_back({"altered_marker", Proposal(
            altered, {parent}, owned, 200000, 100000, 100000, 100000),
            false});
        ...
        auto add_malformed_marker = [&](const std::string& name,
                                        std::vector<uint8_t> script) {
            const Transaction candidate = Spend(parent, 0, {
                TxOutput(100000, owned), TxOutput(0, std::move(script))});
            cases.push_back({name, Proposal(
                candidate, {parent}, owned,
                200000, 100000, 100000, 100000), false});
        };
        add_malformed_marker("marker_truncated_pushdata1", {0x6a, 0x4c});
        add_malformed_marker("marker_truncated_pushdata2", {0x6a, 0x4d});
        add_malformed_marker("marker_truncated_pushdata2_length",
                             {0x6a, 0x4d, 0x00});
        std::vector<uint8_t> declared_too_long{0x6a, 0x4c, 76};
        declared_too_long.insert(declared_too_long.end(), 75U, 'A');
        add_malformed_marker("marker_declared_length_too_long",
                             std::move(declared_too_long));
        std::vector<uint8_t> declared_too_short{0x6a, 0x4c, 76};
        declared_too_short.insert(declared_too_short.end(), 77U, 'A');
        add_malformed_marker("marker_declared_length_too_short",
                             std::move(declared_too_short));
        add_malformed_marker("marker_direct_push_trailing",
                             {0x6a, 0x01, 'A', 'B'});
        ...
        cases.push_back({"alternate_valid_vout", Proposal(
            twin_switched, {twin_parent}, owned,
            200000, 100000, 100000, 100000), false});

        std::ofstream manifest(output / "cases.tsv",
                               std::ios::binary | std::ios::trunc);
        if (!manifest.good()) throw std::runtime_error("cannot write manifest");
        for (const auto& test : cases) {
            Write(output / (test.name + ".prepared.json"), test.proposal);
            const std::string intent_name =
                test.name == "alternate_valid_vout"
                    ? "alternate_vout.intent.json"
                    : "canonical.intent.json";
            manifest << test.name << '\t' << test.name << ".prepared.json"
                     << '\t' << intent_name << '\t'
