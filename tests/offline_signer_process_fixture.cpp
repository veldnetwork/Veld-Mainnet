    ...
    try {
        if (argc == 2 && std::string(argv[1]) == "--rtp1-open-auth") {
            ...
            std::cout << "{\"custody_script_hex\":\"" << BytesToHex(BtcVeldCustodySpk())
                      ...
                      << "\",\"recipient\":\"" << TOKEN_RECIPIENT << "\",\"sats\":" << MINT_SATS
                      << "}\n";
            return 0;
        }
        if (argc == 4 && std::string(argv[1]) == "--rtp1-core-template") {
            ...
            if (owned.size() != 25)
                throw std::runtime_error("invalid fixture issuer");
            ...
            auto field = [&](const char* name) -> const btc_buy::JsonValue& {
                ...
                if (!value)
                    throw std::runtime_error("missing fixture proof field");
                return *value;
            };
            ...
            auto hash = [&](const btc_buy::JsonValue& value) {
                ...
                if (result.size() != 32)
                    throw std::runtime_error("fixture proof hash");
                Hash256 digest{};
            ...
            claim.exact_commitment =
                reserve::detail::OpenDepositCommitment(0, MINT_SATS, TOKEN_RECIPIENT);
            claim.has_nullifier_proof = true;
            claim.nullifier_proof = btcnull::EmptyProof();
            const auto proof = reserve::EncodeProof(claim);
            if (proof.empty())
                throw std::runtime_error("fixture proof encoding");
            ...
            const Transaction tx =
                Spend(parent, 0,
                      {TxOutput(100000, owned),
                       TxOutput(0, BuildOpReturnScript("VELD_TOKEN|MINT|" +
                                                       std::string(BTCVELD_TOKEN_ID) + "|" +
                                                       issuer + "|" + TOKEN_RECIPIENT + "|" +
                                                       std::to_string(MINT_SATS) + "|" + memo))});
            const reserve::State empty{};
            std::string from, to, parsed_memo;
            uint64_t amount = 0;
            const BtcVeldReserveMintPolicyContext context{empty, 0};
            const auto refusal =
                BtcVeldMintTemplatePolicy(tx, owned, from, to, amount, parsed_memo, &context);
            if (!refusal.empty())
                throw std::runtime_error("Core fixture mint policy: " + refusal);
            std::cout << Proposal(tx, {parent}, owned, 200000, 100000, 100000, 100000, false,
                                  UINT64_MAX, nullptr, BytesToHex(reserve::EncodeState(empty)), "0")
                      << "\n";
            return 0;
        }
