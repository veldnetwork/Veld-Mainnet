...
def observations(tip=4163):
    return [
        dict(
            ...
            state_tip='b' * 64,
            state_digest='c' * 64,
            hashes={'2800': FIXTURE['hash'], '4000': 'a' * 64},
        )
...
class PolicyTests(unittest.TestCase):
    def test_interval_stays_beyond_reorganization_limit(self):
        ...
        with self.assertRaises(ValueError):
            bind_candidate(4100, 'a' * 64, observations())
    ...
    def test_rejects_noncanonical_feed_shapes(self):
        for value in (
            ...
            None,
            {},
            [dict(FIXTURE, height=True)],
            [dict(FIXTURE, signed_at=True)],
            [dict(FIXTURE, signed_at=int(time.time()) + 1000)],
            [dict(FIXTURE, hash='A' * 64)],
            [dict(FIXTURE, sig='00')],
            [dict(FIXTURE, extra=1)],
            [FIXTURE, FIXTURE],
        ):
            with self.subTest(value_type=type(value).__name__), self.assertRaises(ValueError):
                parse(json.dumps(value).encode())
        with self.assertRaises(ValueError):
            parse(b'{"checkpoints":[],"checkpoints":[]}')
    ...
    def test_rejects_history_replacement_or_removal(self):
        for after in ([record()], [dict(FIXTURE, hash='d' * 64), record()], [FIXTURE]):
            with self.assertRaises(ValueError):
                append_only([FIXTURE], after)

    def test_fleet_failure_modes(self):
        changes = [
            ('unix_time', time.time() - 120),
            ('genesis', '0' * 64),
            ('ibd_complete', False),
        ...
        for key, value in changes:
            ...
            with self.subTest(field=key), self.assertRaises(ValueError):
                qualify(rows, [2800])
        with self.assertRaises(ValueError):
            qualify(observations()[:2])

    def test_historical_hash_disagreement(self):
        rows = observations()
        rows[1]['hashes']['4000'] = 'f' * 64
        with self.assertRaises(ValueError):
            bind_candidate(4000, 'a' * 64, rows)
...
class StoreTests(unittest.TestCase):
    def setUp(self):
        ...
        self.feed = self.root / 'checkpoints.json'
        self.before = encode([FIXTURE])
        self.feed.write_bytes(self.before)

        def verifier(raw):
            if any(cp['sig'] == '0' * 6618 for cp in parse(raw)):
                raise ValueError('Signature rejected by verifier')
    ...
    def install(self, transaction='a' * 32, new=None, base=None):
        return self.store.install(
            dict(
                ...
                base_sha256=base or sha(self.before),
                document_b64=base64.b64encode(new or encode([FIXTURE, record()])).decode(),
            )
        )
    ...
    def test_rollback_refuses_unrelated_change(self):
        self.install()
        self.feed.write_bytes(encode([FIXTURE, record(4100)]))
        with self.assertRaises(ValueError):
            self.store.finish('a' * 32, False)
    ...
    def test_corrupt_backup_cannot_be_restored(self):
        self.install()
        (self.store.state / ('feed-' + sha(self.before) + '.json')).write_bytes(b'corrupt')
        with self.assertRaises(ValueError):
            self.store.finish('a' * 32, False)
    ...
    def test_stale_publisher_becomes_unhealthy(self):
        status = self.store.checked(4163)
        status['last_checked_at'] = int(time.time()) - 4 * 3600
    ...
    def controller(self):
        ...
        class FakePublisher(Publisher):
            def __init__(self):
                self.config = dict(publisher_host='n3')
                self.state = parent.root / 'signer'
            ...
            def gateway(self, host, action, **values):
                if action == 'read':
                    return parent.store.read()
                if action == 'install':
                    return parent.store.install(values)
                if action in ('commit', 'rollback'):
                    return parent.store.finish(values['transaction'], action == 'commit')
                if action == 'checked':
                    return parent.store.checked(values['tip'])
                raise AssertionError(action)

            def public_feed(self):
                if self.fail_readback and parent.store.read()['pending']:
                    return b'[]'
