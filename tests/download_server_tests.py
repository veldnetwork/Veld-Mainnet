...
SOURCE = ROOT / "tests" / "bounded_download_tests.cpp"
checks = 0
...
def fetch(
    client: Path, server: OneRequestServer, destination: Path, maximum: int, deadline_ms: int
) -> tuple[int, int, int, int]:
    run = subprocess.run(
        [
            str(client),
            "--fetch",
            f"http://127.0.0.1:{server.port}/fixture",
            str(destination),
            str(maximum),
            str(deadline_ms),
        ],
...
def main() -> None:
    ...
    check(
        "RunProcessToBoundedFile" in process and "ByteLimitExceeded" in process,
        "parent transfer-boundary primitive missing",
    )
    check(
        "kMaxCheckpointDocumentBytes" in checkpoints
        and "json.size() > kMaxCheckpointDocumentBytes" in checkpoints,
        "checkpoint parser does not share transfer cap",
    )
    check(
        "std::string obj = json.substr" not in checkpoints
        and "const std::string_view obj" in checkpoints,
        "checkpoint parser retains a complete object copy",
    )
    check(
        "end - pos > max_length" in checkpoints,
        "checkpoint parser copies oversized fields before validation",
    )
    check(
        '"--max-filesize"' in node
        and '"--max-redirs", "0"' in node
        and '"--location"' in node
        and '"--proto", "=https"' in node,
        "checkpoint curl boundary/redirect/TLS policy missing",
    )
    check(
        "body.resize" in node
        and "oss << in.rdbuf()" not in node[node.index("size_t LoadCheckpointsFromUrl()") :],
        "checkpoint loader retains a second complete copy",
    )
    check(
        "TryAutoSnapshotBootstrap" not in node
        and "MaybePreferSnapshotAtStartup" not in node
        and "VELD_SNAPSHOT_MIRRORS" not in public_snapshot
        and '"https://veld.network/downloads/"' in public_snapshot
        and "latest.txt" not in public_snapshot,
        "snapshot acquisition is not pinned to the official fixed endpoint",
    )
    check(
        "RunProcessToBoundedFile" in public_snapshot
        and "PUBLIC_SNAPSHOT_MAX_ARCHIVE_BYTES" in public_snapshot
        and "ValidateArchiveListings" in public_snapshot
        and "ValidateExtractedLevelDbTree" in public_snapshot,
        "snapshot transfer/extraction boundaries are incomplete",
    )
    check(
        "ValidateStoredChainOnly" in node and "VELD_ENABLE_SNAPSHOT_BOOTSTRAP" in node,
        "local non-public snapshot validation fixture was not preserved",
    )
    check(
        "Invoke-WebRequest" not in updater and "Expand-Archive" not in updater,
    ...
    for required in (
        ...
        "MaxReleaseExpandedBytes",
        "Expand-BoundedReleaseArchive",
        "CancellationTokenSource",
    ):
        check(required in updater, f"updater boundary missing {required}")
    check(
        "Get-BoundedHttpsFile" in tor_setup
        and "ResponseHeadersRead" in tor_setup
        and "AllowAutoRedirect = $false" in tor_setup
        and "TOR_MAX_ARCHIVE_BYTES" in tor_setup
        and "FileMode]::CreateNew" in tor_setup,
        "Tor bundle transfer is not bounded at the response stream",
    )
    check(
        "Invoke-WebRequest" not in tor_setup
        and "Invoke-WebRequest" not in mining_launcher
        and "Invoke-WebRequest" not in validator_launcher,
        "a shipped launcher retains an unbounded live download",
    )
    check(
        "tor-setup.ps1" in updater and "tor-watchdog.ps1" in updater,
    ...
    check(
        tor_setup_hash in mining_launcher and tor_setup_hash in validator_launcher,
        "launchers do not pin the exact packaged Tor setup helper",
    )

    with tempfile.TemporaryDirectory(prefix="veld-test-download-") as temporary:
        temp = Path(temporary)
        binary = temp / (
            "bounded_download_tests.exe" if os.name == "nt" else "bounded_download_tests"
        )
        ...
        unit = subprocess.run([str(binary)], cwd=ROOT, text=True, capture_output=True, timeout=20)
        check(unit.returncode == 0, unit.stderr)
        check(
            "PASS bounded_download_tests checks=21" in unit.stdout,
        ...
        with OneRequestServer(oversized_length) as server:
            status, exit_code, received, exists = fetch(binary, server, path, maximum, 2000)
        check(
            status == 8 and exit_code != 0,
            "oversized Content-Length was not rejected by curl preflight",
        )
        check(
            received == 0 and exists == 0 and not path.exists(),
        ...
        with OneRequestServer(oversized_chunked) as server:
            status, exit_code, received, exists = fetch(binary, server, path, maximum, 2000)
        check(status in (5, 8) and exit_code != 0, "oversized chunked body was accepted")
        check(exists == 0 and not path.exists(), "oversized chunked partial file survived")
        ...
        with OneRequestServer(slow_stream) as server:
            ...
            status, _exit_code, _received, exists = fetch(binary, server, path, maximum, 150)
        ...
        with OneRequestServer(redirect) as server:
            status, exit_code, _received, exists = fetch(binary, server, path, maximum, 2000)
        check(
            server.requests == 1 and status == 8 and exit_code != 0,
        ...
        with OneRequestServer(malformed_length) as server:
            status, exit_code, _received, exists = fetch(binary, server, path, maximum, 2000)
        check(
            status == 8 and exit_code != 0 and exists == 0 and not path.exists(),
        ...
        with OneRequestServer(exact_response) as server:
            status, exit_code, received, exists = fetch(binary, server, path, maximum, 2000)
        check(
            (status, exit_code, received, exists) == (0, 0, maximum, 1),
            "exact maximum response did not succeed",
        )
        check(path.read_bytes() == exact_body, "exact response bytes changed")
        check(
            hashlib.sha256(path.read_bytes()).digest() == hashlib.sha256(exact_body).digest(),
            "exact response digest mismatch",
        )

    print(f"PASS download_server_tests checks={checks}")
