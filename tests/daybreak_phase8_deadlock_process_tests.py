#!/usr/bin/env python3
"""Strict process watchdog for PHASE8-DEADLOCK-01.

The same fixture is compiled against the failed and remediated sources. The
failed candidate must reach PHASE8_DEADLOCK_ARMED and then fail or time out;
...
def main() -> int:
    ...
    armed = "PHASE8_DEADLOCK_ARMED verified_peers=2 height=0" in stdout
    records = [
        line[len("PHASE8_DEADLOCK_JSON ") :]
        for line in stdout.splitlines()
        if line.startswith("PHASE8_DEADLOCK_JSON ")
    ]
    ...
    summary = {
        "schema": "VELD_PHASE8_DEADLOCK_PROCESS_V1",
    ...
    print(
        "PASS daybreak_phase8_deadlock_process_tests "
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"FAIL daybreak_phase8_deadlock_process_tests: {exc}",
            file=sys.stderr,
        )
        raise SystemExit(1)
