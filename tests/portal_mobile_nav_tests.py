...
require(
    "const CACHE='veld-portal-shell-v23';", "PWA shell cache advances with remote pool controls"
)
require(
    'id="mobile-nav" aria-label="Primary" hidden', "mobile nav starts hidden until session recovery"
)
require('$("mobile-nav").hidden=!d', "mobile nav visibility follows paired-machine availability")
require(
    '$("app-view").classList.toggle("unpaired",!d)',
    "pair screen receives the unpaired layout state",
)
require(
    '.app.unpaired .main{padding-bottom:20px!important}',
    "pair screen does not reserve navbar spacing",
)
...
require(
    "#mobile-nav{display:grid!important;grid-template-columns:repeat(5,minmax(0,1fr));position:fixed;bottom:0;left:0;right:0;",
    "mobile nav divides the available viewport into five equal tabs",
)
require(
    "transform:translateZ(0);will-change:transform",
    "mobile nav uses the wallet compositor behavior",
)
require(
    "#mobile-nav .mob-tab{display:flex;flex-direction:column;align-items:center;justify-content:center;",
...
require(
    ...
    "navigation keeps compact bottom clearance without a full extra safe-area strip",
)
require(
    ".main{padding-bottom:calc(var(--portal-nav-total) + 18px)!important}",
    "page content clears the complete navbar",
)
...
require(
    '.app.unpaired .unpaired-more{display:inline-flex',
    "More remains accessible before the first machine is paired",
)

print(
    "PASS portal mobile navigation: equal tabs, safe-area spacing, five routes, logout in More, and unpaired access"
)
