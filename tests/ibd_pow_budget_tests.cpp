...
int main() {
    ...
    for (unsigned i = 0; i < 8; ++i) {
        ...
        auto charge = lease->TakeForwardCharge();lease.reset();
        Check(!charge.CreditValidatedForwardBlock(1000 - i), "replay and reorg heights never refill");
    }
    Check(!source.TryAcquire(ExpensivePowUse::PeerBlock), "unproductive work exhausts eight-start limit");
    Check(source.TryAcquire(ExpensivePowUse::InternalMine).has_value(), "internal lane remains usable");
    ...
    for (const auto use : {ExpensivePowUse::PeerNms, ExpensivePowUse::PeerReorg,
        ...
        auto lease = other.TryAcquire(use);Check(lease.has_value(), "external lane initial admission");
        auto charge = lease->TakeForwardCharge();lease.reset();
        Check(!charge.CreditValidatedForwardBlock(2000), "other external lanes cannot mint forward credit");
    ...
    Check(!dropped.TryAcquire(ExpensivePowUse::PeerBlock), "failed validation and abandoned receipts stay charged");

    ExpensivePowBudget moving(1, 1, 1min);
    auto lease = moving.TryAcquire(ExpensivePowUse::PeerBlock);
    auto moved_lease = std::move(*lease);lease.reset();
    Check(!lease.has_value() && moving.Stats().in_flight == 1, "moving keeps one active slot");
    auto charge = moved_lease.TakeForwardCharge();
    Check(!moved_lease.TakeForwardCharge().CreditValidatedForwardBlock(1), "lease produces one receipt only");
    ...
    Check(!moving.TryAcquire(ExpensivePowUse::PeerBlock), "returning rate credit cannot release a held slot");
    moved_lease = {};
    Check(moving.TryAcquire(ExpensivePowUse::PeerBlock).has_value(), "slot and rate accounting remain independent");

    ExpensivePowBudget rolling(1, 1, 20ms);
    auto old = rolling.TryAcquire(ExpensivePowUse::PeerBlock);
    auto old_charge = old->TakeForwardCharge();old.reset();
    std::this_thread::sleep_for(30ms);
    auto fresh = rolling.TryAcquire(ExpensivePowUse::PeerBlock);
    Check(fresh.has_value(), "window renews normally");fresh.reset();
