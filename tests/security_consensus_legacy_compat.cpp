...
int main() {
    ...
    ValidatorRecord rec;
    rec.pubkey_hex = std::string(3904, '1');
    rec.address = "fixture";
    rec.bond_custodial = true;
    rec.bond_units = MIN_VALIDATOR_STAKE;
    rec.registered_height = 10;
    rec.slashed = true;
    rec.active = false;
    rec.slashed_at_height = 30;
    rec.last_finality_vote_height = 25;
    ...
    p.proposer = "fixture";
    p.title = "Ordinary";
    p.description = "Canonical legacy control";
    p.created_at = 10;
    p.expires_at = 100;
    ...
    gs.next_id = 2;
    gs.proposals[1] = p;
    gs.last_vote_block["fixture:1"] = 20;
    governance.RestoreState(gs);
    finality.record.target.height = 20;
    finality.record.target.hash.fill(2);
    finality.record.carrier.height = 25;
    finality.record.carrier.hash.fill(3);
    std::cout << HashToHex(governance.GovernanceDigest()) << '\n'
              << HashToHex(validators.ValidatorsDigest()) << '\n'
              << HashToHex(finality.Digest()) << '\n'
    ...
    Block before_upgrade;
    before_upgrade.height = 1919;
    if (!validators.ProcessBlock(before_upgrade, [](const auto&) { return uint64_t{0}; }))
        return 1;
