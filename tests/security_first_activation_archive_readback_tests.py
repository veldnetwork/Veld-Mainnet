...
a = p.parse_args()
directory = a.completed_run.resolve()
...
try:
    ...
    assert (
        ...
        and state["hot_requests"] == 0
        and state["hot_payout_identities"] == 0
    ), state
    ...
    result["checks"] = {
        "complete_replay_digest_matches": True,
        "terminal_request_removed_from_hot_map": True,
        "consumed_payout_removed_from_hot_set": True,
        "authenticated_archive_serves_complete_terminal_request": True,
    }
