...
struct BitcoinHistory {
    ...
    Hash256 MineOn(const Hash256& parent, const Hash256& merkle) {
        ...
        for (uint32_t nonce = 0;; ++nonce) {
            ...
            if (nonce == UINT32_MAX)
                throw std::runtime_error("Veld Bitcoin header nonce exhausted");
        }
        CHECK(mined);
...
struct TestLedger {
    ...
    Hash256 CompleteDigest() const {
        ...
        return state_digest::sha256_domain(
            "VELD/TEST/PURPOSE_BUILT_COMPLETE_STATE/v1", body);
    }
...
Transaction ReserveCarrier(const TransitionFixture& fixture) {
    Transaction tx;
    tx.inputs.push_back(TxInput::Coinbase("test-reserve-carrier"));
...
Transaction IssuerReserveCarrier(const TransitionFixture& fixture,
    ...
    Transaction tx;
    tx.inputs.push_back(TxInput::Coinbase("test-reserve-issuer"));
    TokenOpData op;
    op.action = "MINT";
...
Transaction RawCarrier(const std::string& payload) {
    Transaction tx;
    tx.inputs.push_back(TxInput::Coinbase("test-raw-carrier"));
    tx.outputs.emplace_back(0, BuildOpReturnScript(payload));
    return tx;
}
...
Transaction SignedTokenCarrier(const TokenOpData& op,
    ...
    Transaction tx;
    TxInput input;
    input.prev_tx_hash = TaggedHash("test-token-input-" + nonce);
...
Hash256 CompleteNodeEnvelope(
    ...
    const Hash256 zero = TaggedHash("test-empty-component");
