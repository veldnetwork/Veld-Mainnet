...
vm.runInContext(
    extract('bvDisplayPoolPrice') + '\n' + extract('_veldAssertOpReturnExact'),
    context,
);
...
for (const missing of [undefined, null, 0]) {
    assert.throws(
        () => context._veldAssertOpReturnExact({ unsigned_tx_hex: '' }, missing),
        /missing local/,
    );
}
...
const govIntent = context._veldGovernanceVoteIntent(
    govInfo,
    { id: 1, vote_identity: voteIdentity },
    '1',
    'yes',
);
...
assert.throws(
    () => context._veldGovernanceVoteIntent(govInfo, { id: 1 }, '1', 'yes'),
    /Refresh proposals/,
);
assert.throws(
    () => context._veldGovernanceVoteIntent({ ...govInfo, gov_vote_version: 3 }, null, '1', 'yes'),
    /Unsupported/,
);
const legacyIntent = context._veldGovernanceVoteIntent(
    { ...govInfo, blocks: 1918, gov_vote_version: 1 },
    null,
    '1',
    'yes',
);
...
(async () => {
    const result = await context._veldRecentSearch(
        'fixture',
        () => ++progressUpdates,
        () => true,
    );
