...
int main(int argc, char** argv) {
    ...
    try {
        if (argc != 2)
            throw std::runtime_error("provide the existing hash vectors");
        mining::VeldDatasetLightKat();
        std::ifstream input(argv[1]);
        if (!input)
            throw std::runtime_error("hash vectors unavailable");
        uint64_t height;
        ...
        while (input >> height >> raw >> expected) {
            ...
            for (size_t i = 0; i < raw.size(); i += 2) {
                ...
                if (used != 2 || value > 255)
                    throw std::runtime_error("vector hex");
            ...
            std::cout << "{\"vector\":" << vectors << ",\"height\":" << height
                      << ",\"verification_microseconds\":" << elapsed << ",\"hash\":\""
                      << HashToHex(verified) << "\"}" << std::endl;
        }
