...
check(
    'const std::string operational_path = datadir + "/miner.key";' in bundle,
    "bundle must retain the operational miner.key name",
)
check(bundle.count("AtomicWriteNew(") == 2, "both identity names must use no-overwrite publication")
check(
    bundle.index("portable_path, encrypted") < bundle.index("operational_path, encrypted"),
    "portable recovery copy must be published before operational activation",
)
check(
    "operational_bytes.size() != encrypted.size()" in bundle,
    "operational keyfile needs exact-size readback",
)
check(
    "portable_bytes.size() != encrypted.size()" in bundle,
    "portable keyfile needs exact-size readback",
)
check(
    "std::equal(encrypted.begin(), encrypted.end()," in bundle,
    "bundle must prove byte identity after publication",
)
check("SecureZero(value.data(), value.size())" in bundle, "encrypted staging bytes must be wiped")
check("SecureZero(first.data(), first.size())" in bundle, "operational readback must be wiped")
check("SecureZero(second.data(), second.size())" in bundle, "portable readback must be wiped")
...
check(
    "_wiz_create_portable_key_bundle(" in create_flow,
    "the production create command must use the bundle writer",
)
check(
    "CreatedPrivateKeyWiper" in create_flow,
    "generated private-key bytes must be wiped on every exit",
)
check("keyfile=" in create_flow, "successful creation must report the non-secret portable path")
...
check(
    "wallet_crypto::DecryptWallet(" in node_import,
    "node import must accept the canonical encrypted envelope",
)
check(
    "_wiz_parse_key_record(decrypted, imported, testnet)" in node_import,
    "node import must authenticate the complete key/address record",
)
check(
    "authenticated_browser_address != imported_address" in node_import,
    "node import must reject wallet key/address mismatches",
)

check(
    "binary miner.key is expected to fail textual JSON parsing" in WALLET,
    "wallet import must retain binary miner.key/.veld-keys handling",
)
check(
    "decryptMinerKeyfile(pendingEncBytes, pass)" in WALLET,
    "wallet import must decrypt the canonical node envelope",
)
check(
    "_veldParseAndBindKeyPayload(plaintext)" in WALLET,
    "wallet import must rederive and bind the imported address",
)
check(
    "localStorage.setItem('veld_ks', browserKeystore)" in WALLET,
    "wallet import must persist only a re-encrypted browser keystore",
)

check(
    "imported into both Veld Wallet and Veld Node" in GUI,
    "GUI must explain the portable keyfile's two supported import targets",
)
check("using the same passphrase" in GUI, "GUI must explain passphrase continuity")
...
check(
    "GenerateKeyPair(" not in ensure_flow,
    "portable-key migration must never generate a second wallet",
)
check(
    ensure_flow.count("AtomicWriteNew(") == 1,
    "portable-key migration must publish at most one new file",
)
check(
    "existing portable keyfile differs from miner.key" in ensure_flow,
    "a conflicting portable keyfile must fail closed",
)
check(
    "portable keyfile readback differs from miner.key" in ensure_flow,
    "a newly published portable keyfile needs exact-byte readback",
)
check("portable_exists" in ensure_flow, "portable-key migration must be idempotent")
check(
    "_wiz_is_encrypted(operational_path)" in ensure_flow,
...
check(
    startup_call > NODE.index("Mining wallet unlocked."),
    "portable-key migration must run only after successful sign-in",
)
check(
    "_wiz_ensure_portable_keyfile(" in NODE[startup_call : startup_call + 1400],
