...
int main(int argc, char** argv) try {
    ...
    Check(sb::WriteAddressIbdReceipt(root.string(), signer, payout, 101, tip, &error),
          "address receipt writes");
    sb::FullIbdReceipt receipt;
    Check(sb::VerifyAddressIbdReceipt(root.string(), signer.public_key, payout, receipt, &error),
          "address receipt verifies");
    Check(receipt.height == 101 && receipt.tip_hash == tip && receipt.miner_address == payout,
          "receipt identity and tip exact");
    Check(!sb::VerifyAddressIbdReceipt(root.string(), signer.public_key, other, receipt, &error),
          "payout switch refuses reuse");
    Check(!sb::VerifyAddressIbdReceipt(root.string(), GenerateKeyPair(false).public_key, payout,
                                       receipt, &error),
          "other validation key refused");
    ...
    Check(!sb::VerifyAddressIbdReceipt(root.string(), signer.public_key, payout, receipt, &error),
          "other network identity refused");
    Write(root / "network.identity", "private-mainnet-profile\n");
    ...
    tampered.replace(value, std::string("validated_height=101\n").size(), "validated_height=102\n");
    Write(root / sb::ADDRESS_RECEIPT_FILENAME, tampered);
    Check(!sb::VerifyAddressIbdReceipt(root.string(), signer.public_key, payout, receipt, &error),
          "tampered signed height refused");
    Write(root / sb::ADDRESS_RECEIPT_FILENAME, std::string(original->begin(), original->end()));
    Check(sb::VerifyAddressIbdReceipt(root.string(), signer.public_key, payout, receipt, &error),
          "restored exact receipt verifies");
    const auto testnet_signer = GenerateKeyPair(true);
    const auto testnet_payout = GenerateKeyPair(true).address;
    Check(sb::WriteAddressIbdReceipt(root.string(), testnet_signer, testnet_payout, 102, tip,
                                     &error, true),
          "testnet address receipt writes");
    Check(sb::VerifyAddressIbdReceipt(root.string(), testnet_signer.public_key, testnet_payout,
                                      receipt, &error, false, true),
    ...
    Check(fs::exists(root / sb::ADDRESS_RECEIPT_FILENAME), "recovery keeps signed receipt");
