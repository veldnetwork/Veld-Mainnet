...
Hash256 Tag(unsigned value) {
    return Hash256d(std::to_string(value));
}
std::string Hex(const auto& bytes) {
    return BytesToHex(std::vector<uint8_t>(bytes.begin(), bytes.end()));
}
...
struct MemoryArchive {
    ...
    void Wire(SignerBondCovenant& covenant) {
        covenant.SetArchiveBackend(Read(), Write());
    }
    void Wire(RedeemArchive& archive) {
        archive.SetBackend(Read(), Write());
    }
};
RedeemRequest Request(unsigned id) {
    ...
    r.amount_sats = 20;
    r.dest_spk = {0x51};
    r.request_height = 2870;
    r.deadline_height = 2885;
    r.veld_recipient = "ordinary-recipient";
    r.status = ReqStatus::LOCKED_IN;
...
void ArchiveChecks(const std::filesystem::path& directory) {
    ...
    std::vector<unsigned> order(192);
    std::iota(order.begin(), order.end(), 1);
    for (unsigned i : order)
        Check(archive.Insert(root, Tag(i), std::to_string(i)), "archive insert");
    ...
    for (unsigned i : order)
        archive.Insert(second, Tag(i), std::to_string(i));
    ...
    Check(!archive.Get(root, Tag(1000)) && archive.Get(fork, Tag(1000)) == "fork",
          "archive fork isolation");
    ...
    std::filesystem::create_directories(directory);
    {
        ...
        disk.SetBackend([&](const std::string& h) { return store.Get(h); },
                        [&](const RedeemArchive::Records& records) {
                            db::WriteBatch batch;
                            for (const auto& [h, value] : records)
                                batch.Put(h, value);
                            return store.Write(batch);
                        });
        for (unsigned i : order)
            disk.Insert(persisted, Tag(i), std::to_string(i));
    ...
    }
    {
        ...
        for (unsigned i : order)
            Check(disk.Get(persisted, Tag(i)) == std::to_string(i), "archive reopen");
    }
}
void CovenantChecks() {
    ...
    MemoryArchive memory;
    SignerBondCovenant cov;
    memory.Wire(cov);
    Check(cov.Register("ordinary-signer", 100) && cov.Activate("ordinary-signer"),
          "legacy signer control");
    ...
    Check(
        cov.MarkAuthorizedReservePayout(paid.request_id, Tag(102), paid.amount_sats, paid.dest_spk),
        "ordinary fulfilled request");
    ...
    Check(!cov.Register("new-signer", 1) && !cov.Activate("ordinary-signer"),
          "legacy registration retired");
    Check(cov.GetCopy(paid.request_id)->status == ReqStatus::FULFILLED, "archived paid lookup");
    Check(cov.GetCopy(defaulted.request_id)->status == ReqStatus::DEFAULTED,
          "archived default lookup");
    Check(cov.GetCopy(open.request_id)->request_commitment == open.request_commitment,
          "open commitment preserved");
    Check(cov.SnapshotState().requests.size() == 1 && cov.SnapshotState().consumed_payouts.empty(),
          "terminal history leaves hot state");
    Check(cov.IsConsumedPayout(Tag(102)) && cov.IsConsumedFraudSpend(Tag(103)),
          "consumed identities retained");
    ...
    Check(verdict.slash && verdict.compensate_sats == open.amount_sats,
          "unpaid principal independent of retired bond");
    cov.ApplySlash(verdict);
    cov.SetStatus(open.request_id, ReqStatus::DEFAULTED);
    cov.FinishBlock();
    Check(cov.SnapshotState().signers.empty() && cov.SnapshotState().requests.empty(),
          "settlement-only locks released");
    Check(cov.GetCopy(open.request_id)->status == ReqStatus::DEFAULTED,
          "settled legacy lock archived");
    const Hash256 settled_digest = cov.Digest();
    cov.RestoreState(migrated);
    Check(cov.Digest() == migrated_digest &&
              cov.GetCopy(open.request_id)->status == ReqStatus::LOCKED_IN,
          "candidate rollback restores open liability");
    cov.SetStatus(open.request_id, ReqStatus::DEFAULTED);
    cov.FinishBlock();
    Check(cov.Digest() == settled_digest, "settlement reapplication deterministic");
    cov.RestoreState(parent);
    cov.BeginBlock(H);
...
void GovernanceAndEndorsements() {
    ...
    GovernanceEngine gov(chain, staking);
    gov.SetValidators(&registry);
    Check(!gov.GovernanceBondGateOpen(), "governance locked at zero validators");
    for (size_t i = 0; i < keys.size(); ++i) {
        const auto& key = keys[i];
        const std::string pk = Hex(key.public_key);
        ValidatorRecord r;
        r.pubkey_hex = pk;
        r.address = key.address;
        r.active = true;
        r.bond_custodial = true;
        r.bond_units = MIN_VALIDATOR_STAKE;
        r.registered_height = 1;
        vs.validators[pk] = r;
        vs.address_to_pubkey[r.address] = pk;
    ...
    const std::string challenge = GovernanceEngine::BuildProposalChallenge(
        type, key.address, title, description, H - 1, true, true);
    const std::string signature = Sig(key, challenge);
    const std::string marker = GovernanceEngine::BuildFramedProposalOp(
        type, key.address, title, description, H - 1, Hex(key.public_key), signature);
    ...
    Check(proposal.title == title && proposal.description == description,
          "proposal UTF-8 fields preserved");
    const auto identity = GovernanceEngine::VoteIdentity(proposal);
    const std::string vote_challenge =
        GovernanceEngine::BuildVoteChallenge(proposal.id, VoteChoice::YES, H, identity);
    const auto vote_signature = Sig(key, vote_challenge);
    const std::string vote =
        GovernanceEngine::BuildVoteOp(proposal.id, key.address, VoteChoice::YES, H,
                                      Hex(key.public_key), vote_signature, identity);
    ...
    gov.RestoreState(state);
    gov.ProcessBlock(Carrier(H + 2, "ordinary"), H + 2, false);
    Check(gov.SnapshotState().proposals.at(proposal.id).expires_at == expiry,
          "new migration does not restart expiry");
    Check(gov.SnapshotState().proposals.at(proposal.id).votes_yes == 1,
          "new migration preserves votes");
    gov.RestoreState(before);
    gov.ProcessBlock(Carrier(H, marker), H, false);
    gov.ProcessBlock(Carrier(H + 1, vote), H + 1, false);
    Check(gov.GovernanceDigest() == voted_digest, "proposal/vote replay digest");
    Check(
        GovernanceEngine::BuildProposalChallenge(type, key.address, "A|B", "C", H, true, true) !=
            GovernanceEngine::BuildProposalChallenge(type, key.address, "A", "B|C", H, true, true),
        "field framing is unambiguous");

    const Hash256 target = Tag(888);
    const std::string endorse_sig =
        Hex(Sign(key.private_key, ValidatorRegistry::BuildEndorseMessage(H - 1, target)));
    const std::string current =
        ValidatorRegistry::BuildEndorseOp(H - 1, HashToHex(target), endorse_sig, key.address, H);
    EndorsementWire wire;
    Check(ParseEndorsementWire(current, wire) && wire.attributed && wire.address == key.address,
          "new endorsement producer/parser agreement");
    Block endorsed = Carrier(H, current);
    registry.RestoreState(vs);
    Check(registry.ProcessBlock(endorsed, [](const auto&) { return uint64_t{0}; }),
          "attributed endorsement process");
    Check(registry.HasAcceptedEndorsement(key.address, H - 1, HashToHex(target), endorse_sig),
          "ordinary attributed endorsement accepted");
    ...
    Check(rpc_detail::ExtractValidatorEndorsementTarget(endorsed.transactions[0], extracted) &&
              extracted && extracted->height == H - 1 && extracted->hash == target,
          "RPC work admission target remains bound");
    ...
    Check(copied.SnapshotState().state_migration_active &&
              copied.ValidatorsDigest() == endorsed_digest,
          "registry copy preserves migration state");
    registry.RestoreState(vs);
    ...
    Check(ValidatorRegistry::HasValidRegisterMultiplicity(bounded),
          "declared endorsement work bound");
    bounded.transactions.push_back(endorsed.transactions[0]);
    Check(!ValidatorRegistry::HasValidRegisterMultiplicity(bounded),
          "excess work rejected before processing");
}
...
void DefaultCompensationChecks() {
    ...
    token_parent.tokens[BTCVELD_TOKEN_ID] = {BTCVELD_TOKEN_ID, "btcVELD", recipient.address, 8,
                                             "BTC"};
    ...
    reserve.reserve_txid = Tag(901);
    reserve.reserve_vout = 0;
    reserve.reserve_value_sats = 50000;
    reserve.reserve_bitcoin_block = Tag(902);
    ...
    MemoryArchive memory;
    SignerBondCovenant covenant;
    memory.Wire(covenant);
    tokens.SetBtcVeldRedeemCovenant(&covenant);
    auto request = Request(903);
    request.amount_sats = 10000;
    request.veld_recipient = recipient.address;
    request.dest_spk = recipient.GetP2PKHScript();
    request.deadline_height = H + 2;
    request.request_commitment = SignerBondCovenant::RequestCommitment(request);
    covenant.AddRequest(request, true);
    covenant.BeginBlock(H);
    ...
    auto settle = [&]() {
        ...
        Check(verdict.slash && verdict.compensate_sats == 10000,
              "full default liability survives authority retirement");
        covenant.ApplySlash(verdict);
        Check(tokens.CompensateMint(verdict.compensate_to, verdict.compensate_sats, H + 3,
                                    "migration-default-fixture"),
              "real token compensation credit");
        covenant.SetStatus(request.request_id, ReqStatus::DEFAULTED);
        covenant.FinishBlock();
    };
    settle();
    Check(tokens.GetSupply(BTCVELD_TOKEN_ID) == 50000 &&
              tokens.GetBalance(BTCVELD_TOKEN_ID, recipient.address) == 50000,
          "exact owed principal credited");
    Check(tokens.GetBtcVeldReserveState().AccountingHolds(50000) &&
              tokens.GetBtcVeldReserveState().open_redemption_principal == 0,
          "settlement preserves reserve accounting");
    Check(covenant.SnapshotState().requests.empty() &&
              covenant.GetCopy(request.request_id)->status == ReqStatus::DEFAULTED,
          "settled compensation archived");
    Check(!tokens.CompensateMint(recipient.address, 10000, H + 4, "already-settled"),
          "closed principal cannot compensate twice");
    const auto settled_token = tokens.Digest(), settled_covenant = covenant.Digest();
    tokens.RestoreState(token_parent);
    covenant.RestoreState(covenant_parent);
    Check(tokens.Digest() == token_digest && covenant.Digest() == covenant_digest,
          "combined token and covenant rollback");
    settle();
    Check(tokens.Digest() == settled_token && covenant.Digest() == settled_covenant,
          "combined compensation replay");
}
}
int main(int argc, char** argv) {
    try {
        if (argc != 2)
            throw std::runtime_error("supply a fresh fixture directory");
        const auto path = std::filesystem::absolute(argv[1]);
        if (std::filesystem::exists(path))
            throw std::runtime_error("fixture directory must be fresh");
        ArchiveChecks(path);
        CovenantChecks();
        GovernanceAndEndorsements();
        DefaultCompensationChecks();
        std::cout << "PASS security_state_migration_tests checks=" << checks
                  << " activation=2880\n";
        return 0;
