...
int main() {
    ...
    for (const auto* reason : {"connected", "unspecified", "remote_eof", "node_stop"}) {
        const std::string raw =
            std::string("{\"event\":\"connection\",\"reason\":\"") + reason + "\"}";
    ...
    for (const auto* reason :
        ...
        assert(
            DisplayLogLine(std::string("{\"event\":\"connection\",\"reason\":\"") + reason + "\"}")
                .find("[network WARN]") == 0);
    }
    for (const auto* raw : {"{\"event\":\"connection_diagnostics_lost\",\"failed\":2}",
                            "{\"event\":\"connection\",\"reason\":\"future_error\"}",
                            "{\"event\":\"connection\",\"reason\":42}"})
        assert(DisplayLogLine(raw) == raw);
