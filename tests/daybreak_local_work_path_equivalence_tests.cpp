...
TemplateResponse ParseTemplate(const std::string& response) {
    ...
    Check(error && error->kind == btc_buy::JsonValue::Kind::Null &&
    ...
    Check(block_hex && work_binding && work_token && work_ttl_ms &&
              ...
              work_token->text.size() == 64 &&
              !work_ttl_ms->text.empty() &&
    ...
    Check(consumed == bytes.size() && consumed > 0 &&
    ...
    Check(binding.has_value() &&
              work_admission::EncodeBinding(*binding) == work_binding->text,
          "template work binding has one canonical encoding");
    block.height = binding->subject.height;
    return TemplateResponse{std::move(block), *binding,
                            work_token->text,
...
void PublishPeer(VeldNode& node,
                 const std::shared_ptr<net::Connection>& peer,
...
PeerPair InstallPeers(VeldNode& node, uint64_t height,
                      const Hash256& tip) {
    auto& server = node.TestWorkAdmissionProcessServer();
    server.TestSetPeerHeightClock(100);
    PeerPair peers{MakeConnection("10.82.0.1", 38201),
                   MakeConnection("10.82.0.2", 38202)};
    ...
    Check(view.work_sequencer_wired && view.work_view_stable &&
              ...
              server.TestVerifiedPeerEvidenceCount() == 2,
...
void RefreshPeers(VeldNode& node, const PeerPair& peers) {
    ...
    Check(view.verified_height == tip.height &&
              view.outbound_sync_height == tip.height &&
              view.distinct_version_ips == 2,
...
void SolveTemplate(Block& block) {
    ...
    for (uint64_t nonce = 0; nonce < 5'000'000; ++nonce) {
        ...
        const Hash256 hash = mining::VeldHash(
            block.header.Serialize(), block.height);
        if (hash < target) {
...
void WaitUntilWallClockExceedsMtp(const Blockchain& chain) {
    ...
    throw std::runtime_error(
        "wall clock did not advance beyond parent MTP within 3 seconds");
}
...
ChainView Observe(const VeldNode& node) {
    return ChainView{node.GetChain().Height(),
                     node.GetChain().TipCopy().GetHash(),
                     node.GetChain().TotalSupplyUnits(),
                     node.ConsensusStateDigest()};
}

bool Same(const ChainView& a, const ChainView& b) {
    return a.height == b.height && a.tip == b.tip &&
           a.supply == b.supply && a.state == b.state;
}
...
int main(int argc, char** argv) {
    try {
        ...
        Check(node.GetChainMut().AddBlockDirect(
        ...
        auto peers = InstallPeers(
            node, 0, node.GetChain().TipCopy().GetHash());
        ...
        Check(subject.has_value(),
              "canonical block-production subject is available");
        std::optional<work_admission::Binding> common;
        for (const auto path : {
                 work_admission::Path::InternalMining,
                 work_admission::Path::GetBlockTemplate,
                 work_admission::Path::SubmitBlock,
        ...
        const auto stale_result = node.GetChainMut().AddBlockDirect(
            stale_candidate.block, false, false, false, stale_context);
        Check(stale_result.IsDeferred() &&
                  Same(before_stale, Observe(node)) &&
        ...
        PublishPeer(node, peers.first, 0,
                    node.GetChain().TipCopy().GetHash());
        Check(node.TestWorkAdmissionProcessServer().GetPeerHeightView().
                  distinct_version_ips == 2,
        ...
        Check(synchronous.size() == 1 && synchronous[0].success &&
                  node.GetChain().Height() == 1 &&
        ...
        const std::string template_response = node.TestHandleWorkAdmissionRpc(
            MakeRpcRequest("getblocktemplate", {miner.address}));
        auto templ = ParseTemplate(template_response);
        Check(templ.binding.subject.target_hash ==
                  templ.block.header.GetTemplateWorkIdentity() &&
        ...
        Check(!RpcSucceeded(node.TestHandleWorkAdmissionRpc(
                  MakeRpcRequest("submitblock", {block_hex}))) &&
        ...
        Check(!RpcSucceeded(node.TestHandleWorkAdmissionRpc(
                  MakeRpcRequest("submitblock", {
                      block_hex, work_admission::EncodeBinding(missing),
                      templ.token}))) &&
                  Blockchain::TestAddBlockDirectCalls() == 0,
        ...
        Check(!RpcSucceeded(node.TestHandleWorkAdmissionRpc(
                  MakeRpcRequest("submitblock", {
                      block_hex, work_admission::EncodeBinding(mismatched),
                      templ.token}))) &&
                  Blockchain::TestAddBlockDirectCalls() == 0 &&
        ...
        forged_binding.subject.target_hash =
            alternate.header.GetTemplateWorkIdentity();
        SolveTemplate(alternate);
        std::string attacker_token(64, 'a');
        if (attacker_token == templ.token) attacker_token.assign(64, 'b');
        node.TestResetWorkAdmissionProcessCounters();
        Blockchain::TestResetAddBlockDirectCalls();
        const std::string random_token_refusal =
            node.TestHandleWorkAdmissionRpc(
                MakeRpcRequest("submitblock", {
                    BytesToHex(alternate.Serialize()),
                    work_admission::EncodeBinding(forged_binding),
                    attacker_token}));
        Check(!RpcSucceeded(random_token_refusal) &&
                  random_token_refusal.find(attacker_token) ==
                      std::string::npos &&
                  Blockchain::TestAddBlockDirectCalls() == 0 &&
                  node.TestWorkDurableWriterCalls() == 0 &&
                  node.TestWorkBlockBroadcastCalls() == 0 &&
                  node.TestCachedTemplateCount() == 1 &&
                  Same(before_refusals, Observe(node)),
              "caller-forged identity and random token are rejected before every sink without token reflection");
        node.TestResetWorkAdmissionProcessCounters();
        Blockchain::TestResetAddBlockDirectCalls();
        const std::string genuine_token_mismatch =
            node.TestHandleWorkAdmissionRpc(
                MakeRpcRequest("submitblock", {
                    BytesToHex(alternate.Serialize()),
                    work_admission::EncodeBinding(forged_binding),
        ...
        Check(!RpcSucceeded(genuine_token_mismatch) &&
                  genuine_token_mismatch.find(templ.token) ==
                      std::string::npos &&
                  Blockchain::TestAddBlockDirectCalls() == 0 &&
                  node.TestWorkDurableWriterCalls() == 0 &&
                  node.TestWorkBlockBroadcastCalls() == 0 &&
                  node.TestCachedTemplateCount() == 1 &&
                  Same(before_refusals, Observe(node)),
              "caller-forged exact template identity is rejected before every chain sink without burning the genuine token");

        SolveTemplate(templ.block);
        Check(templ.block.header.GetTemplateWorkIdentity() ==
                  templ.binding.subject.target_hash,
              "solving changes only nonce and preserves template identity");
        Blockchain::TestResetAddBlockDirectCalls();
        const std::string accepted = node.TestHandleWorkAdmissionRpc(
            MakeRpcRequest("submitblock", {
                BytesToHex(templ.block.Serialize()),
                work_admission::EncodeBinding(templ.binding),
                templ.token}));
        Check(RpcSucceeded(accepted) &&
                  Blockchain::TestAddBlockDirectCalls() == 1 &&
                  node.GetChain().Height() == 2 &&
        ...
        Check(!RpcSucceeded(node.TestHandleWorkAdmissionRpc(
                  MakeRpcRequest("submitblock", {
                      ...
                      templ.token}))) &&
                  Blockchain::TestAddBlockDirectCalls() == 0 &&
                  Same(after_submit, Observe(node)),
              "replayed consumed template cannot reach AddBlockDirect");

        RefreshPeers(node, peers);
        WaitUntilWallClockExceedsMtp(node.GetChain());
        auto next_template = ParseTemplate(node.TestHandleWorkAdmissionRpc(
            MakeRpcRequest("getblocktemplate", {miner.address})));
        Blockchain::TestResetAddBlockDirectCalls();
        Check(!RpcSucceeded(node.TestHandleWorkAdmissionRpc(
                  MakeRpcRequest("submitblock", {
                      BytesToHex(next_template.block.Serialize()),
                      work_admission::EncodeBinding(templ.binding),
                      next_template.token}))) &&
                  Blockchain::TestAddBlockDirectCalls() == 0 &&
                  Same(after_submit, Observe(node)),
        ...
        const std::string simultaneous_request = MakeRpcRequest(
            "submitblock", {
                BytesToHex(next_template.block.Serialize()),
        ...
        auto simultaneous_start =
            simultaneous_start_promise.get_future().share();
        ...
        std::thread submit_a([&] {
            try {
                ...
                simultaneous_a = node.TestHandleWorkAdmissionRpc(
        ...
        std::thread submit_b([&] {
            try {
                simultaneous_start.wait();
                simultaneous_b = node.TestHandleWorkAdmissionRpc(
                    simultaneous_request);
            } catch (...) {
        ...
        Check(!simultaneous_failure_a && !simultaneous_failure_b &&
                  (RpcSucceeded(simultaneous_a) !=
                   RpcSucceeded(simultaneous_b)) &&
                  Blockchain::TestAddBlockDirectCalls() == 1 &&
                  node.TestWorkDurableWriterCalls() == 1 &&
                  node.TestWorkBlockBroadcastCalls() == 1 &&
                  node.GetChain().Height() == 3 &&
                  node.TestCachedTemplateCount() == 0 &&
                  node.TestWorkAdmissionCoordinatorSnapshot().active_leases ==
                      0,
              "simultaneous submitblock attempts consume one token and reach one canonical/durable/broadcast sink");
        ...
        const auto internal_subject =
            node.TestCurrentBlockProductionSubject();
        const auto internal_binding = internal_subject
            ? node.TestEvaluateWorkAdmission(
                  work_admission::Path::InternalMining, *internal_subject)
            : work_admission::Decision{};
        auto internal = MineOnly(
            node.GetChainMut(), node.GetMempoolMut(), miner);
        Check(internal.success && internal_binding.allowed &&
        ...
        Blockchain::TestResetAddBlockDirectCalls();
        Check(node.GetChainMut().AddBlockDirect(
                  internal.block, false, false, false,
                  internal_context).IsAccepted() &&
                  Blockchain::TestAddBlockDirectCalls() == 1 &&
                  node.GetChain().Height() == 4,
        ...
        peers.second->Close();
        node.Stop();
        std::cout << "PASS daybreak_local_work_path_equivalence_tests checks="
                  << checks
