...
std::string Request(const Transaction& tx,
...
int main() {
    ...
    const std::string payload = "VELD_VALIDATOR|ENDORSE|7|" + hash_text +
        "|" + signature_text;
    const Transaction canonical = EndorsementTx(payload);

    std::optional<rpc_detail::ValidatorEndorsementTarget> target;
    Check(rpc_detail::ExtractValidatorEndorsementTarget(canonical, target) &&
              target && target->height == 7 &&
              HashToHex(target->hash) == hash_text,
          "canonical endorsement target extracted");

    Transaction ordinary = canonical;
    ordinary.outputs[0].script_pubkey =
        BuildOpReturnScript("ordinary metadata");
    target.reset();
    Check(rpc_detail::ExtractValidatorEndorsementTarget(ordinary, target) &&
              !target,
          "ordinary transaction remains unbound");

    Transaction leading_zero = EndorsementTx(
        "VELD_VALIDATOR|ENDORSE|07|" + hash_text + "|" + signature_text);
    Check(!rpc_detail::ExtractValidatorEndorsementTarget(leading_zero, target),
          "noncanonical height rejected");

    Transaction uppercase_hash = EndorsementTx(
        "VELD_VALIDATOR|ENDORSE|7|" + std::string(64, 'A') + "|" +
        signature_text);
    ...
    noncanonical_push.outputs.front().script_pubkey.insert(
        noncanonical_push.outputs.front().script_pubkey.end(),
        payload_bytes.begin(), payload_bytes.end());
    Check(!rpc_detail::ExtractValidatorEndorsementTarget(
              noncanonical_push, target),
    ...
    const std::filesystem::path storage_path =
        ...
        ("veld-daybreak-endorsement-binding-" + suffix);
    {
        Blockchain chain;
        ...
        Check(missing.find("requires its work binding") != std::string::npos &&
                  mempool.IsEmpty(),
              "missing binding refused before mempool admission");

        work_admission::Binding binding;
        binding.subject.purpose =
        ...
        const std::string unwired = rpc.Handle(
            Request(canonical, encoded, token));
        Check(unwired.find("work admission refused") != std::string::npos &&
                  mempool.IsEmpty(),
        ...
        rpc.SetWorkAdmissionFn(
            [&](work_admission::Path path,
                ...
                bool require_prior) {
                ++closed_calls;
                Check(path == work_admission::Path::ValidatorEndorsement &&
                          subject.height == 7 &&
                          subject.target_hash == HexToHash(hash_text) &&
                          prior && *prior == binding && require_prior,
                      "sink passes exact subject and prior binding");
                return work_admission::Decision{
                    false, work_admission::Refusal::RuntimeClosed,
                    std::nullopt};
            });
        const std::string closed = rpc.Handle(
            Request(canonical, encoded, token));
        Check(closed_calls == 1 &&
                  closed.find("work admission refused") != std::string::npos &&
                  mempool.IsEmpty(),
              "closed predicate emits no cached artifact");

        const std::string ordinary_extra =
            rpc.Handle(Request(ordinary, encoded, token));
        Check(ordinary_extra.find("Usage: sendrawtransaction") !=
                  std::string::npos,
              "ordinary transaction cannot smuggle a work binding");

        const std::string begin_unwired = rpc.Handle(MakeRpcRequest(
            "beginworksigning",
            {"validator_endorsement", encoded, token}));
        Check(begin_unwired.find("activation unavailable") !=
                  std::string::npos,
              "remote signing start fails closed when unwired");

        size_t begin_calls = 0;
        rpc.SetBeginRemoteSigningFn(
            [&](work_admission::Path path, const std::string& got_binding,
                const std::string& got_token) {
                ++begin_calls;
                Check(path == work_admission::Path::ValidatorEndorsement &&
                          got_binding == encoded && got_token == token,
        ...
        const std::string begun = rpc.Handle(MakeRpcRequest(
            "beginworksigning",
            {"validator_endorsement", encoded, token}));
        ...
        const std::string replay = rpc.Handle(MakeRpcRequest(
            "beginworksigning",
            {"validator_endorsement", encoded, token}));
        Check(begin_calls == 2 &&
                  replay.find("token_consumed") != std::string::npos,
        ...
        const std::string cancelled = rpc.Handle(MakeRpcRequest(
            "cancelworksigning", {token}));
        Check(cancel_calls == 1 &&
                  cancelled.find("\"released\":true") !=
                      std::string::npos,
    ...
    Check(!cleanup_error && !std::filesystem::exists(storage_path),
          "fixture storage removed");

    std::cout << "PASS daybreak_validator_endorsement_binding_tests checks="
              << checks << "\n";
    return 0;
}
