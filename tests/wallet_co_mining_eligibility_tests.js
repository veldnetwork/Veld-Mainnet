...
async function exercise(stake, policy, failed = false) {
    const elements = Object.fromEntries(
        ['cm-elig-card', 'cm-elig-body', 'cm-minstake'].map((id) => [
            id,
...
(async () => {
    for (const ordinary of [1000, 500]) {
        for (const stake of [0, 499, 500, 999.99999999, 1000, 1001]) {
            const r = await exercise(stake, {
                min_stake_veld: ordinary,
                co_mining_min_stake_veld: 1000,
            });
    ...
    for (const policy of [
        ...
        {},
        { min_stake_veld: 500 },
        { co_mining_min_stake_veld: 0 },
        { co_mining_min_stake_veld: '1000' },
        { co_mining_min_stake_veld: NaN },
        { co_mining_min_stake_veld: Infinity },
    ]) {
