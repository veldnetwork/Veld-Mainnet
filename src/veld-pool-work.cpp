...
uint64_t Number(const std::string& text, int base) {
    ...
    if (base == 10 && text != std::to_string(value)) throw std::runtime_error("integer encoding");
    return value;
}
...
int main(int argc,char** argv) {
    ...
    while (std::cin.getline(frame, sizeof(frame))) {
        ...
        try {
            if (line.size() > 512) throw std::runtime_error("request limit");
            ...
            BlockHeader header;
            if (!header.Deserialize(bytes)) throw std::runtime_error("header");
                ...
                for (uint64_t offset = 0; offset < count; ++offset) {
                    ...
                    if (proof < accounting || proof < network.bytes ||
                        ...
                        std::cout << "SHARE " << std::hex << std::setw(16) << std::setfill('0')
                                  << header.nonce << ' ' << HashToHex(proof) << std::dec << std::endl;
                    }
                }
