...
static bool _wiz_save_key_encrypted(const std::string& key_file, const veld::Secp256k1PrivKey& priv,
    ...
    std::vector<uint8_t> encrypted;
    if (!_wiz_encrypt_key_record(priv, passphrase, testnet, encrypted))
        return false;
    std::string error;
...
static bool _wiz_load_key_encrypted(const std::string& key_file, const std::string& passphrase,
    ...
    try {
        ...
        if (!veld::wallet_crypto::IsCurrentWalletEnvelope(data) &&
            !_wiz_save_key_encrypted(key_file, kp.private_key, passphrase, testnet)) {
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        const bool mining_flag = arg == "--mine" || arg.rfind("--mine=", 0) == 0 ||
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--wallet") {
            ...
            std::cerr << "       The flag exists only for test-chain rescue and is compiled out "
                         "for mainnet.\n";
            return 78;
        }
#endif
...
#ifdef VELD_FLEET_NO_MINE
            std::cerr << "veld-node: --threads is a mining option and is unavailable in a "
                         "VELD_FLEET_NO_MINE build\n";
            return 78;
#else
            if (i + 1 >= argc) {
            ...
            opt_mining_threads = static_cast<unsigned>(parse_uint_arg("--threads", argv[++i]));
#endif
            ...
            std::cout
                ...
                << "  --mine               Enable mining\n"
#endif
                << "  --endorse            Endorse blocks as a registered validator without mining "
                   "(lighter; no 1 GB dataset)\n"
                << "  --reachable          Auto-open the P2P port via NAT-PMP/PCP so peers can "
                   "dial you (publishes your IP; no-op on a public IP)\n"
                << "  --tor                Be reachable as a Tor v3 .onion (zero IP exposure; "
                   "needs a local Tor daemon with ControlPort)\n"
                << "  --tor-data-dir DIR   Required with --tor; trusted owner-only Tor directory "
                   "containing control_auth_cookie\n"
                << "  --tor-only           Route ALL traffic through Tor (onion-only, real IP "
                   "never exposed); reads .onion from <datadir>/tor/hs/hostname\n"
#ifndef VELD_PUBLIC_TESTNET
                << "  --verify-release M S Verify release manifest M against signature S using the "
                   "pinned release key; exit 0=valid (used by the updater)\n"
#endif
                << "  --deployment-info    Print compiled role/transport/genesis identity without "
                   "opening a datadir; exit\n"
                << "  --verify-compiled-genesis  Verify the compiled memory-hard genesis PoW and "
                   "exit\n"
#ifndef VELD_FLEET_NO_MINE
                << "  --threads <N>        Parallel mining threads (default: estimated physical "
                   "cores minus one)\n"
#endif
...
#ifndef VELD_PUBLIC_TESTNET
#ifdef VELD_FLEET_NO_MINE
                << "  --import-miner-key <file>  Validate and install an encrypted validator "
                   "identity\n"
                << "  --create-miner-key  Create encrypted miner.key and portable .veld-keys "
                   "files, then exit\n"
#else
                << "  --import-miner-key <file>  Validate an encrypted .veld-keys file and "
                   "atomically install it as this datadir's mining/validator identity\n"
                << "  --create-miner-key  Create encrypted miner.key and portable .veld-keys "
                   "files, then exit\n"
#endif
#endif
                ...
                << "  --fleet-anchor <IPv4> Configure and dial one authoritative outbound clock "
                   "anchor. Repeatable.\n"
                << "                       Exact canonical IPv4 literals only; hostnames and ports "
                   "are rejected.\n"
                << "                       Also accepts VELD_FLEET_ANCHOR_IPS (comma-separated "
                   "exact IPv4 list).\n"
                << "  --txindex            Maintain a txid->height index for O(1) "
                   "getrawtransaction\n"
                << "                       (off by default; non-consensus; one-time backfill on "
                   "enable)\n"
#ifdef VELD_PUBLIC_MAINNET
                << "  --snapshot-bootstrap Use the official signed snapshot when it is newer, then "
                   "independently validate from genesis in the background (default).\n"
                << "  --full-ibd           Refuse snapshot import and perform ordinary peer IBD.\n"
                << "  --no-snapshot        Alias for --full-ibd.\n"
#else
                << "  --full-ibd           Refuse snapshot bootstrap and perform ordinary peer "
                   "IBD.\n"
#endif
                << "  --verify-pow         Background re-verify PoW of the ordinary on-disk "
                   "chain.\n"
                << "  --reindex-canonical  Offline recovery: rebuild the canonical index from "
                   "retained\n"
                << "                       block bodies, preserve the prior index, and exit\n"
                << "  --trust <ip>         Trust an IP — exempt from per-IP cap, never banned. "
                   "Repeatable.\n"
                << "                       Also accepts VELD_TRUSTED_IPS env var (comma-separated "
                   "list).\n"
                << "                       Use for multi-device home-NAT setups (PC+laptop one WAN "
                   "IP).\n"
                << "  --setup              Force the 4-option login wizard (same as every-launch "
                   "default)\n"
#ifdef VELD_FLEET_NO_MINE
                << "  --no-prompt, --auto  Skip the validator identity wizard (automation only; "
                   "needs miner.key to exist)\n\n";
#else
                << "  --no-prompt, --auto  Skip the login wizard (automation only; needs miner.key "
                   "to exist)\n\n";
#endif
            ...
            std::cout << "  Two-node developer example:\n"
                      ...
                      << "    Node B:  veld-node --regtest --rpcport 8335 --p2pport 28334 "
...
#ifdef VELD_FLEET_NO_MINE
        std::cerr << "veld-node: regtest auto-mining is unavailable in a VELD_FLEET_NO_MINE build; "
                     "pass --nomine\n";
        return 78;
#else
        opt_mine = true;
    ...
    if (background_validation_base) {
        try {
            ...
            if (opt_tor_only && background_chainstate->GetTCPServer()) {
                background_chainstate->GetTCPServer()->EnableTorOnly(tor_only_onion);
            }
            ...
            if (_wiz_is_encrypted(pool_kp_file)) {
                ...
                if (!g_passphrase.empty() && g_passphrase.size() >= 8) {
                    ...
                    if (_wiz_save_key_encrypted(pool_kp_file, priv, g_passphrase,
                        ...
                        std::cerr << RED
                                  << "  [FATAL] Failed to encrypt pool.key. Refusing to run with "
                                     "plaintext.\n"
                                  << RESET;
                        return 1;
                    }
                    ...
                    std::cerr
                        << RED
                        << "\n  FATAL: pool.key is in PLAINTEXT and no VELD_VAULT_PASSPHRASE is "
                           "set.\n"
        ...
        if (std::filesystem::exists(ep_kp_file)) {
            ...
            if (!_wiz_is_encrypted(ep_kp_file)) {
                std::cerr << RED << "\n  FATAL: endorsement_pool.key is in PLAINTEXT.\n"
                          << "  Delete it and restart with VELD_VAULT_PASSPHRASE set to "
                             "auto-generate a new encrypted key.\n\n"
                          << RESET;
                return 1;
            }
            if (g_passphrase.empty())
            ...
            if (ep_kp.address != ENDORSEMENT_POOL_ADDRESS) {
                std::cerr << RED << "\n  FATAL: endorsement_pool.key address mismatch.\n"
                          ...
                          << "  Update ENDORSEMENT_POOL_ADDRESS in constants.h or delete the key "
                             "file.\n\n"
                          << RESET;
                return 1;
            }
...
#endif
    if (opt_mine || opt_endorse || bind_existing_regtest_generation_identity) {
        if (std::filesystem::exists(kp_file)) {
            if (_wiz_is_encrypted(kp_file)) {
                ...
                if (!pass.empty() && pass.size() >= 8) {
                    ...
                    std::cout << YEL
                              << "  [WARN] --allow-plaintext-miner-key set — continuing with "
                                 "plaintext (INSECURE).\n"
                              << RESET;
                } else {
                    ...
                    if (ipass.empty() || !_wiz_save_key_encrypted(kp_file, miner_kp.private_key,
                                                                  ipass, config.IsTestNetwork())) {
            ...
            if (!saved) {
                std::cerr << RED << "  FATAL: could not atomically persist owner-only miner.key.\n"
                          << RESET;
                return 1;
            }
        ...
        if (opt_regtest) {
            ...
            if (!node.BindGenerationIdentity(miner_kp, &generation_identity_error)) {
                std::cerr << RED
        ...
        if (opt_mine) {
            ...
            unsigned physical_est =
                (hw_threads >= 4 && (hw_threads % 2) == 0) ? (hw_threads / 2) : hw_threads;
            ...
            if (veld::DiagVerbose().load())
                std::cout << "  Threads: " << mine_threads << " of ~" << physical_est
                          << " physical / " << hw_threads << " logical (" << tune_reason << ")\n";
#ifdef VELD_REGTEST_FIXED_DIFF
    ...
    while (!g_shutdown) {
                ...
                if (node.SnapshotQuarantineOnly()) {
                    ...
                    if (fleet_validation_key_ready) {
                        receipt_written = snapshot_bootstrap::WriteFleetIbdReceipt(
                            opt_datadir, fleet_validation_kp, validated_height, validated_tip,
                            &receipt_error);
                    }
#else
                    if (miner_key_ready) {
                        receipt_written = snapshot_bootstrap::WriteFullIbdReceipt(
                            opt_datadir, miner_kp, validated_height, validated_tip, &receipt_error);
                    }
#endif
                    if (!receipt_written) {
...
#ifdef VELD_FLEET_NO_MINE
                        receipt_written = snapshot_bootstrap::WriteFleetIbdReceipt(
                            opt_datadir, fleet_validation_kp, cur_height, validated_tip,
                            &receipt_error);
#else
                        receipt_written = snapshot_bootstrap::WriteFullIbdReceipt(
                            opt_datadir, miner_kp, cur_height, validated_tip, &receipt_error);
#endif
                        if (receipt_written) {
                            full_ibd_receipt_valid = true;
                            ...
                            full_ibd_receipt.tip_hash = validated_tip;
#ifdef VELD_FLEET_NO_MINE
                            full_ibd_receipt.miner_address = fleet_validation_kp.address;
#else
        ...
        if (receipt_refresh_role_ready && at_tip && stable_ticks >= 3 && node.IsIBDComplete() &&
            node.ChainFullyValidated() && (first_receipt_needed || receipt_refresh_due)) {
            std::string receipt_error;
            std::string validated_tip;
            try {
                validated_tip = HashToHex(node.GetChain().Tip().GetHash());
...
#ifdef VELD_FLEET_NO_MINE
            receipt_written = snapshot_bootstrap::WriteFleetIbdReceipt(
                opt_datadir, fleet_validation_kp, cur_height, validated_tip, &receipt_error);
#else
            receipt_written = snapshot_bootstrap::WriteFullIbdReceipt(
                opt_datadir, miner_kp, cur_height, validated_tip, &receipt_error);
#endif
    ...
    !defined(VELD_ENABLE_SNAPSHOT_BOOTSTRAP)
                ...
                if (extra_key_path && extra_key_path[0]) {
                    ...
                    if (_wiz_is_encrypted(extra_key_path)) {
                        ...
                        if (_wiz_load_key_encrypted(extra_key_path, pass, extra_kp,
                            ...
                            std::cerr << RED
                                      << "  [validator] FATAL: failed to decrypt extra endorser "
                                         "key — wrong passphrase?"
                                      << RESET << "\n";
                            veld::WipeString(pass);
                            if (!extra_optional) {
                                std::cerr << RED
                                          << "  [validator] Set VELD_EXTRA_ENDORSER_KEY_OPTIONAL=1 "
                                             "to tolerate this and continue without the extra key."
                                          << RESET << "\n";
                                return 1;
                            }
                        }
                        ...
                        std::cerr << RED
                                  << "  [validator] FATAL: VELD_EXTRA_ENDORSER_KEY must point to "
                                     "an ENCRYPTED key file. Refusing to load plaintext key."
                                  << RESET << "\n";
                        if (!extra_optional)
                            return 1;
                    }
        ...
        if ((opt_mine || opt_endorse) && cur_height > last_endorsed_height + 1 && cur_height > 2 &&
            ...
            for (uint64_t eh = start_h; eh <= end_h; ++eh) {
                ...
                for (auto& [epriv, epub] : endorser_keys) {
                    ...
                    std::string eop =
                        ValidatorRegistry::BuildEndorseOp(eh, HashToHex(ehash), BytesToHex(esig));
    ...
    std::thread watchdog([shutdown_start, SHUTDOWN_DEADLINE_SEC, shutdown_exit_code]() {
        std::this_thread::sleep_for(std::chrono::seconds(SHUTDOWN_DEADLINE_SEC));
        auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(
                           std::chrono::steady_clock::now() - shutdown_start)
                           .count();
        std::cerr << "\n  [shutdown-watchdog] Stop() did not return in " << elapsed
                  << "s — force-exiting (LevelDB WAL is durable,\n"
