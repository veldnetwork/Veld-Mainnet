...
void PrintStatus(const VeldNode& node, const std::string& miner_addr, bool mining,
                 bool endorse_mode) {
    ...
    std::cout << GRAY << "[" << timebuf << "] " << RESET << "height=" << BOLD
              << node.GetChain().Height() << RESET << "  supply=" << GOLD << std::fixed
              << std::setprecision(2) << node.GetChain().TotalSupplyVeld() << " VELD" << RESET
              << "  mempool=" << node.GetMempool().Size() << "  phase="
              << (node.GetChain().TotalSupplyUnits() < node.GetChain().GetStakingActivationUnits()
                      ? "bootstrap"
                      : CYAN "standard" RESET)
              << ((!mining && !endorse_mode) ? (GRAY "  [fleet]" RESET)
                  : (!node.IsIBDComplete() || !node.ChainFullyValidated()) ? (GRAY "  [sync]" RESET)
                  : mining && node.IsMining()
                      ? (node.IsAddressRegisteredValidator(miner_addr)
                             ? ("  [" GREEN "mining" RESET "+" PURPLE "endorsing" RESET "]")
                             : (GREEN "  [mining]" RESET))
                  : mining ? (GRAY "  [mining stopped]" RESET)
  ...
  public:
    WindowsGuiShutdownEvent() {
        const std::wstring name =
            L"Local\\VeldNodeShutdown-" + std::to_wstring(GetCurrentProcessId());
...
static bool _wiz_resolve_keyfile_input(const std::string& entered, std::string& resolved,
    ...
    if (ec || !fs::is_regular_file(parsed, ec) || ec)
        return false;
...
static bool _wiz_read_private_bytes(const std::string& path, std::vector<uint8_t>& out,
                                    size_t max_size, std::string* error = nullptr) {
    return veld::channel::secure_file::Read(path, out, error, max_size,
                                            /*require_private_parent=*/true) ==
           veld::channel::secure_file::ReadResult::Ok;
}

static bool _wiz_read_private_text(const std::string& path, std::string& out, size_t max_size,
...
#ifndef VELD_PUBLIC_TESTNET
static bool _wiz_read_explicit_import(const std::string& path, std::string& out,
                                      std::string* error = nullptr) {
    veld::WipeString(out);
    std::vector<uint8_t> bytes;
...
static std::string _wiz_prompt_passphrase(bool confirm) {
    ...
    if (confirm) {
        ...
        std::cout << "(hidden)\n";
        bool match =
            veld::ConstantTimeEqualsString(pass.data(), pass.size(), pass2.data(), pass2.size());
        if (!match) {
            veld::WipeString(pass);
...
static std::vector<uint8_t> _wiz_b64_decode(const std::string& s) {
    ...
    if (!veld::compat::DecodeBase64Canonical(s, out, MAX_BROWSER_KEYSTORE_FIELD))
        return {};
    return out;
}
...
static std::string _wiz_json_str(const std::string& src, const std::string& key) {
    ...
    while (i < src.size()) {
        ...
        if (src[i] == '"')
            return src.substr(pos + 1, i - pos - 1);
        ++i;
    }
    return "";
}

static bool _wiz_json_u32(const std::string& src, const std::string& key, uint32_t& value) {
    std::string needle = "\"" + key + "\"";
    auto pos = src.find(needle);
    if (pos == std::string::npos)
        return false;
...
static bool _wiz_looks_like_walletui_keystore(const std::string& content) {
    if (content.find("\"keystore\"") == std::string::npos)
        return false;
    if (content.find("\\\"salt\\\"") == std::string::npos)
        return false;
    if (content.find("\\\"iv\\\"") == std::string::npos)
        return false;
...
static std::string _wiz_decrypt_walletui_keystore(const std::string& content,
    ...
    if (version == veld::wallet_crypto::BROWSER_KEYSTORE_VERSION_V3) {
        ...
        if (_wiz_json_str(inner, "name") != "scrypt" ||
            _wiz_json_str(inner, "cipher") != "AES-256-GCM" || !_wiz_json_u32(inner, "n", n) ||
            !_wiz_json_u32(inner, "r", r) || !_wiz_json_u32(inner, "p", p) ||
    ...
    if (salt_b64.empty() || iv_b64.empty() || ct_b64.empty())
        return "";

    auto salt = _wiz_b64_decode(salt_b64);
    auto iv = _wiz_b64_decode(iv_b64);
    auto ct = _wiz_b64_decode(ct_b64);
    if (salt.size() != 16 || iv.size() != 12 || ct.size() < 17)
        return "";

    std::string plaintext;
    if (!veld::wallet_crypto::DecryptBrowserKeystoreCiphertext(version, passphrase, salt, iv, ct,
                                                               plaintext))
        return "";

    std::string priv_hex, authenticated_address;
    if (!veld::wallet_crypto::ParseBrowserKeystoreIdentity(plaintext, priv_hex,
                                                           authenticated_address)) {
        veld::WipeString(plaintext);
        return "";
    }
    if (out_address)
        *out_address = std::move(authenticated_address);
    veld::WipeString(plaintext);
    return priv_hex;
}
#endif
...
static bool _wiz_is_encrypted(const std::string& file) {
    ...
    if (!_wiz_read_private_text(file, content, WIZ_MAX_OPERATIONAL_SECRET_BYTES, &error))
        return false;
...
static bool _wiz_encrypt_key_record(const veld::Secp256k1PrivKey& priv,
                                    const std::string& passphrase, bool testnet,
                                    std::vector<uint8_t>& encrypted, std::string* address = nullptr,
    ...
    std::string plaintext = veld::operational_key::FormatRecord(priv_hex, pub_hex, addr);
...
static bool _wiz_save_key_encrypted(const std::string& key_file, const veld::Secp256k1PrivKey& priv,
                                    const std::string& passphrase, bool testnet = false,
                                    const std::string& preserved_address = {}) {
    std::vector<uint8_t> encrypted;
    if (!_wiz_encrypt_key_record(priv, passphrase, testnet, encrypted, nullptr, preserved_address))
        return false;
    std::string error;
    const bool ok = veld::channel::secure_file::AtomicWrite(key_file, encrypted, &error,
                                                            /*require_private_parent=*/true);
    if (!encrypted.empty())
...
#ifndef VELD_PUBLIC_TESTNET
static bool _wiz_create_portable_key_bundle(const std::string& datadir,
                                            const veld::Secp256k1PrivKey& private_key,
                                            const std::string& passphrase, bool testnet,
                                            std::string& address, std::string& portable_path,
                                            std::string& error) {
    address.clear();
    portable_path.clear();
    error.clear();
    ...
    try {
        if (!_wiz_encrypt_key_record(private_key, passphrase, testnet, encrypted, &address)) {
    ...
    portable_path = datadir + "/veld-wallet-" + address.substr(0, 8) + ".veld-keys";
    ...
    if (!veld::channel::secure_file::AtomicWriteNew(portable_path, encrypted, &error,
                                                    /*require_private_parent=*/true)) {
        return false;
    }
    if (!veld::channel::secure_file::AtomicWriteNew(operational_path, encrypted, &error,
                                                    /*require_private_parent=*/true)) {
    ...
    if (!_wiz_read_private_bytes(operational_path, operational_bytes,
                                 WIZ_MAX_OPERATIONAL_SECRET_BYTES, &read_error) ||
        !_wiz_read_private_bytes(portable_path, portable_bytes, WIZ_MAX_OPERATIONAL_SECRET_BYTES,
                                 &read_error) ||
        operational_bytes.size() != encrypted.size() || portable_bytes.size() != encrypted.size() ||
        !std::equal(encrypted.begin(), encrypted.end(), operational_bytes.begin()) ||
        !std::equal(encrypted.begin(), encrypted.end(), portable_bytes.begin())) {
...
static bool _wiz_ensure_portable_keyfile(const std::string& datadir, const std::string& address,
    ...
    const bool portable_exists = std::filesystem::exists(portable_path, exists_error);
    if (exists_error) {
        error = "cannot determine portable keyfile state";
        return false;
    }
    if (portable_exists) {
        if (!_wiz_read_private_bytes(portable_path, portable_bytes,
                                     WIZ_MAX_OPERATIONAL_SECRET_BYTES, &error)) {
            return false;
        }
    ...
    if (!veld::channel::secure_file::AtomicWriteNew(portable_path, operational_bytes, &error,
                                                    /*require_private_parent=*/true)) {
        return false;
    }
    if (!_wiz_read_private_bytes(portable_path, portable_bytes, WIZ_MAX_OPERATIONAL_SECRET_BYTES,
                                 &error) ||
        portable_bytes.size() != operational_bytes.size() ||
        !std::equal(operational_bytes.begin(), operational_bytes.end(), portable_bytes.begin())) {
...
static bool _wiz_parse_key_record(std::string_view content, veld::RealKeyPair& kp,
                                  bool testnet = false) {
    veld::operational_key::RecordFields fields;
    if (!veld::operational_key::ParseRecord(content, fields))
        return false;
    ...
    try {
        ...
        if (pub_hex.size() != pub.size() * 2 ||
            !veld::PublicKeyOwnsAddress(pub, stored_addr, testnet))
            return false;
        ...
        for (size_t i = 0; i < pub.size(); ++i) {
            ...
            if (hi < 0 || lo < 0 || static_cast<uint8_t>((hi << 4) | lo) != pub[i])
                return false;
        }
...
#ifndef VELD_PUBLIC_TESTNET
static bool _wiz_import_encrypted_keyfile(const std::string& source_path,
                                          const std::string& destination_path,
                                          const std::string& passphrase, bool testnet,
                                          std::string& imported_address, std::string& error) {
    ...
    if (browser_keystore) {
        std::string private_hex =
            _wiz_decrypt_walletui_keystore(content, passphrase, &authenticated_browser_address);
        ...
        try {
            ...
            std::string decrypted = veld::wallet_crypto::DecryptWallet(encrypted, passphrase);
            ...
            if (_wiz_parse_key_record(decrypted, imported, testnet)) {
                ...
                veld::compat::SecureZero(imported.private_key.data(), imported.private_key.size());
    ...
    try {
        ...
        if (browser_keystore)
            imported_address = authenticated_browser_address;
    ...
    if (browser_keystore && (authenticated_browser_address.empty() ||
                             authenticated_browser_address != imported_address)) {
        error = "authenticated key/address mismatch";
        imported_address.clear();
        return false;
    }
    if (!_wiz_save_key_encrypted(destination_path, private_key, passphrase, testnet,
                                 imported_address)) {
...
static bool _wiz_load_key_encrypted(const std::string& key_file, const std::string& passphrase,
                                    veld::RealKeyPair& kp, bool testnet = false) {
    std::vector<uint8_t> data;
    if (!_wiz_read_private_bytes(key_file, data, WIZ_MAX_OPERATIONAL_SECRET_BYTES))
        return false;
    try {
        ...
        if (!_wiz_parse_key_record(plaintext, kp, testnet))
            return false;
        ...
        if (!veld::wallet_crypto::IsCurrentWalletEnvelope(data) &&
            !_wiz_save_key_encrypted(key_file, kp.private_key, passphrase, testnet, kp.address)) {
...
static bool _wiz_save_key(const std::string& key_file, const veld::Secp256k1PrivKey& priv,
                          bool testnet = false) {
...
static bool _wiz_load_plain_key(const std::string& path, veld::RealKeyPair& kp,
                                bool testnet = false) {
    std::string content, error;
    if (!_wiz_read_private_text(path, content, WIZ_MAX_OPERATIONAL_SECRET_BYTES, &error))
        return false;
    struct TextWiper {
        std::string& value;
        ~TextWiper() {
...
static bool _wiz_load_rpc_token(const std::string& path, const std::string& passphrase,
                                std::string& token) {
    token.clear();
    std::vector<uint8_t> data;
    ...
    while (first < last && (token[first] == ' ' || token[first] == '\t'))
        ++first;
...
static bool _wiz_save_rpc_token(const std::string& path, const std::string& token,
                                const std::string& passphrase, std::string* error = nullptr) {
    if (token.size() != 64 || passphrase.empty())
        return false;
...
static bool _wiz_shred_file(const std::string& path) {
    ...
    ok = ::GetFileInformationByHandle(h, &info) &&
         ...
         info.nNumberOfLinks == 1 && ::GetFileSizeEx(h, &size) && size.QuadPart >= 0 &&
         static_cast<unsigned long long>(size.QuadPart) <= WIZ_MAX_OPERATIONAL_SECRET_BYTES;
    for (int pass = 0; ok && pass < 3; ++pass) {
        ...
        while (ok && left != 0) {
            const DWORD want = static_cast<DWORD>(std::min<unsigned long long>(buf.size(), left));
            DWORD wrote = 0;
            ok = ::WriteFile(h, buf.data(), want, &wrote, nullptr) && wrote == want;
            left -= wrote;
        }
        if (ok)
    ...
    ok = ::fstat(fd, &st) == 0 && S_ISREG(st.st_mode) && st.st_uid == ::geteuid() &&
         ...
         static_cast<unsigned long long>(st.st_size) <= WIZ_MAX_OPERATIONAL_SECRET_BYTES;
    for (int pass = 0; ok && pass < 3; ++pass) {
        ok = veld::compat::SecureRandom(buf.data(), buf.size()) && ::lseek(fd, 0, SEEK_SET) == 0;
        off_t left = st.st_size;
        while (ok && left > 0) {
            const size_t want =
                static_cast<size_t>(std::min<off_t>(static_cast<off_t>(buf.size()), left));
...
static void _wiz_offer_shred_disabled_keys(const std::string& datadir) {
    ...
    for (auto& ent : fs::directory_iterator(datadir, ec)) {
        ...
        if (n.rfind("miner.key.disabled.", 0) == 0 || n.rfind("validator.key.disabled.", 0) == 0) {
...
static int run_mining_wizard(const std::string& datadir, bool testnet = false) {
    ...
    if (!veld::channel::secure_file::EnsurePrivateDirectory(datadir, &dir_error)) {
        std::cerr << "\n  [setup] Refusing unsafe data directory: " << dir_error << "\n\n";
        return 1;
    }
#ifdef VELD_PUBLIC_TESTNET
    ...
    if (choice == "2") {
        ...
        if (!_wiz_read_explicit_import(path, content, &import_error)) {
            std::cerr << "\n  ERROR: refusing key import " << path << ": " << import_error
                      << "\n\n";
            return run_mining_wizard(datadir, testnet);
        }
        struct ImportWiper {
        ...
        if (browser_keystore) {
            ...
            veld::compat::ConsoleEchoOn();
            std::cout << "(hidden)\n";
            priv_hex = _wiz_decrypt_walletui_keystore(content, pass, &authenticated_browser_addr);
            if (priv_hex.empty()) {
                veld::WipeString(pass);
            ...
            if (priv_hex.empty()) {
                std::cout
                    ...
                    << std::flush;
                veld::compat::ConsoleEchoOff();
                if (!std::getline(std::cin, priv_hex)) {
                    veld::compat::ConsoleEchoOn();
                    return 1;
                }
                veld::compat::ConsoleEchoOn();
        ...
        const auto derived_addr = veld::PubKeyToAddress(veld::DerivePublicKey(priv), testnet);
        if (!authenticated_browser_addr.empty() && authenticated_browser_addr != derived_addr) {
        ...
        if (!g_passphrase.empty()) {
            save_ok = _wiz_save_key_encrypted(key_file, priv, g_passphrase, testnet);
        } else {
    ...
    if (choice == "4") {
        ...
        std::string pub_hex = hex(kp.public_key.data(), static_cast<int>(kp.public_key.size()));
        std::cout << "\n"
#ifdef VELD_PUBLIC_TESTNET
        ...
        if (!_wiz_save_key_encrypted(key_file, kp.private_key, pass, testnet)) {
            veld::WipeString(pass);
            veld::compat::SecureZero(kp.private_key.data(), 32);
            veld::WipeString(priv_hex);
            std::cerr << "\n  ERROR: write failed\n\n";
        ...
        // with the operator's passphrase; withholding it protected nothing.
        {
            ...
            std::string keys_file =
                datadir + "/veld-wallet-" + kp.address.substr(0, 8) + ".veld-keys";
            auto enc = veld::wallet_crypto::EncryptWallet(plaintext, pass);
            std::string write_error;
            const bool keyfile_ok =
                veld::channel::secure_file::AtomicWrite(keys_file, enc, &write_error,
                                                        /*require_private_parent=*/true);
            if (keyfile_ok) {
                std::cout << "  Keyfile:    " << keys_file << "\n";
                std::cout << "              (encrypted — import into wallet.veld.network)\n";
            } else {
                std::cerr << "  ERROR: secure keyfile write failed: " << write_error << "\n";
            }
            if (!enc.empty())
...
#include <windows.h>
#include <dbghelp.h>
typedef BOOL(WINAPI* MiniDumpWriteDump_t)(HANDLE hProc, DWORD pid, HANDLE hFile, ULONG dumpType,
                                          PVOID exceptionParam, PVOID userStreamParam,
                                          PVOID callbackParam);
static LONG WINAPI VeldCrashFilter_(EXCEPTION_POINTERS* ep) {
    ...
    HANDLE fh = CreateFileA(
        path, GENERIC_WRITE, 0, nullptr, CREATE_NEW,
        FILE_ATTRIBUTE_NORMAL | FILE_FLAG_WRITE_THROUGH | FILE_FLAG_OPEN_REPARSE_POINT, nullptr);
...
static void PrintDeploymentInfoJson() {
    ...
    std::cout << "VELD_DEPLOYMENT_INFO_V1_JSON {"
              ...
              << "\"miner_ui_port\":" << veld::CompiledPublicMinerUiPort() << ","
              ...
              << "\"amm_flat_fee_activation_height\":"
              << veld::BTCVELD_AMM_FLAT_FEE_ACTIVATION_HEIGHT << ","
              << "\"amm_flat_fee_bps\":" << veld::BTCVELD_AMM_FLAT_FEE_BPS << ","
              << "\"sha384_destination_activation_height\":"
              << veld::SHA384_DESTINATION_ACTIVATION_HEIGHT << ","
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        const bool mining_flag = arg == "--address-only" || arg.rfind("--address-only=", 0) == 0 ||
                                 arg == "--mine" || arg.rfind("--mine=", 0) == 0 ||
                                 arg == "--miner" || arg.rfind("--miner=", 0) == 0 ||
                                 arg == "--threads" || arg.rfind("--threads=", 0) == 0;
        if (mining_flag) {
            std::cerr << "veld-node: " << arg << " is unavailable in a VELD_FLEET_NO_MINE build\n";
            return 78;
        }
    }
#endif
    ...
    for (int i = 1; i < argc; ++i)
        if (std::string_view(argv[i]) == "--address-only")
            requested_address_only = true;
    if (requested_address_only) {
        for (int i = 1; i < argc; ++i) {
            ...
            if (flag == "--wallet" || flag == "--setup" || flag == "--endorse" ||
                ...
                std::cerr
                    << "veld-node: --address-only cannot be combined with wallet/signing operations\n";
                return 2;
            }
        }
    }
    ...
    for (int i = 1; i < argc; ++i) {
...
#if defined(VELD_ENABLE_SNAPSHOT_BOOTSTRAP)
        const bool allowed_snapshot_control = arg == "--snapshot-bootstrap" ||
                                              arg == "--no-snapshot" || arg == "--full-ibd" ||
                                              arg == "--verify-snapshot";
#else
        ...
        const bool snapshot_control =
            (arg.rfind("--", 0) == 0 && arg.find("snapshot") != std::string_view::npos) ||
            arg == "--bootstrap-only" || arg.rfind("--bootstrap-only=", 0) == 0;
        if (snapshot_control && !allowed_snapshot_control) {
            std::cerr << "veld-node: FATAL: " << arg
                      << " is not an accepted public snapshot control\n";
            return 2;
        }
        if (arg == "--upnp" || arg.rfind("--upnp=", 0) == 0 || arg.rfind("--upnp-", 0) == 0) {
            std::cerr << "veld-node: FATAL: " << arg << " is not supported in the "
    ...
    if (argc >= 2 && std::string(argv[1]) == "--verify-release") {
        ...
        auto rd = [](const char* p, size_t ceiling, bool& ok) -> std::vector<uint8_t> {
            ...
            std::string error;
            ok = veld::channel::secure_file::ReadExplicitImport(p, d, &error, ceiling) ==
                 veld::channel::secure_file::ReadResult::Ok;
            return d;
        };
        ...
        if (!veld::LoadPinnedReleasePubkey(pub)) {
            std::cerr << "verify-release: bad pinned key\n";
            return 2;
        }
        ...
        if (!iok || !sok) {
            std::cerr << "verify-release: cannot read manifest/sig\n";
            return 2;
        }
    ...
    if (argc >= 2 && std::string(argv[1]) == "--verify-snapshot") {
        ...
        std::vector<uint8_t> sig = rd(argv[3], 64u * 1024u, sok);
        if (!iok || !sok) {
            std::cerr << "verify-snapshot: cannot read manifest/sig\n";
            return 2;
        }
        ...
        if (ok) {
            ...
            ok = compiled.size() == 64 && (m.genesis == compiled || m.genesis == compiled_be);
        }
        std::cout << (ok ? "RELEASE-SIGNATURE-VALID" : "RELEASE-SIGNATURE-INVALID") << "\n";
        return ok ? 0 : 1;
    }
    ...
    if (argc == 2 && std::string(argv[1]) == "--verify-compiled-genesis") {
        ...
        bits_stream << "0x" << std::hex << std::setw(8) << std::setfill('0') << genesis.header.bits;
    ...
    auto parse_uint_arg = [](const char* flag, const char* val) -> uint64_t {
        try {
            ...
            std::cerr << "veld-node: bad value for " << flag << " '" << val << "': " << e.what()
                      << "\n";
            std::exit(2);
        }
    };
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--wallet") {
            ...
            opt_mine = true;
#endif
        } else if (arg == "--address-only") {
            opt_address_only = true;
        } else if (arg == "--endorse") {
            opt_endorse = true;
            ...
            opt_mine = true;
#endif
        } else if (arg == "--no-prompt" || arg == "--auto") {
            opt_no_prompt = true;
        } else if (arg == "--print-rpc-token") {
            opt_print_rpc_token = true;
            opt_no_prompt = true;
        } else if (arg == "--nomine") {
            opt_nomine = true;
            ...
            opt_full_ibd = true;
            opt_snapshot_bootstrap = false;
        } else if (arg == "--verify-pow") {
            opt_verify_pow = true;
        } else if (arg == "--reindex-canonical") {
            opt_reindex_canonical = true;
        } else if (arg == "--rpcport" && i + 1 < argc) {
            opt_rpc_port = (uint16_t)parse_uint_arg("--rpcport", argv[++i]);
        } else if (arg == "--p2pport" && i + 1 < argc) {
            opt_p2p_port = (uint16_t)parse_uint_arg("--p2pport", argv[++i]);
        } else if (arg == "--connect" && i + 1 < argc) {
            opt_connect.push_back(argv[++i]);
        } else if (arg == "--trust" && i + 1 < argc) {
            opt_trusted_ips.push_back(argv[++i]);
        ...
        } else if (arg == "--allow-plaintext-miner-key") {
            opt_allow_plaintext_miner_key = true;
        }
#ifndef VELD_MAINNET_POW
        else if (arg == "--allow-legacy-replay-below" && i + 1 < argc) {
            opt_allow_legacy_replay_below =
                parse_uint_arg("--allow-legacy-replay-below", argv[++i]);
        }
#else
        else if (arg == "--allow-legacy-replay-below") {
            std::cerr << "ERROR: --allow-legacy-replay-below is forbidden in mainnet builds.\n";
            std::cerr
                << "       The flag exists only for test-chain rescue and is compiled out for mainnet.\n";
            return 78;
        }
#endif
        else if (arg == "--threads") {
#ifdef VELD_FLEET_NO_MINE
            std::cerr
                << "veld-node: --threads is a mining option and is unavailable in a VELD_FLEET_NO_MINE build\n";
            return 78;
#else
            if (i + 1 >= argc) {
            ...
            std::cout
                << "  --regtest            Regtest mode (isolated transport + datadir)\n"
    ...
    if (opt_tor && (!opt_tor_data_directory_set || opt_tor_data_directory.empty())) {
    ...
    if (!opt_regtest) {
        std::cerr << "veld-node: FATAL: this l3-regtest-fixed-difficulty-v1 artifact "
    ...
    if (opt_regtest) {
        std::cerr << "veld-node: FATAL: --regtest requires an artifact compiled with "
    ...
    if (opt_fleet_anchor_ips.empty() && config.kind == NetworkKind::Mainnet && !opt_tor_only) {
    ...
    if (!CompiledRoleAllowsPort(requested_p2p_port, CompiledPublicP2PPort()) ||
        ...
        std::cerr << "veld-node: FATAL: PUBLIC TESTNET ports are immutable; "
                  ...
                  << " rpc=" << CompiledPublicRpcPort() << ", requested p2p=" << requested_p2p_port
                  << " rpc=" << opt_rpc_port << "\n";
        return 2;
    }
    ...
    for (const auto& peer : opt_connect) {
        ...
        const std::string host = colon == std::string::npos ? peer : peer.substr(0, colon);
        ...
        if (colon != std::string::npos) {
            try {
                ...
                valid = valid && consumed == peer.size() - colon - 1 && parsed <= 65535;
...
#ifdef VELD_FLEET_NO_MINE
        std::cerr
            << "veld-node: regtest auto-mining is unavailable in a VELD_FLEET_NO_MINE build; pass --nomine\n";
        return 78;
#else
        opt_mine = true;
#endif
    ...
    if (opt_address_only) {
        if (!mining::ValidAddressOnlyDestination(opt_miner_addr, config.IsTestNetwork()) ||
            ...
            std::cerr
                << "veld-node: --address-only requires --miner with a valid payout address for this network\n";
            return 2;
        }
    ...
    if (!veld::channel::secure_file::EnsurePrivateDirectory(opt_datadir, &datadir_error)) {
        std::cerr << "  FATAL: data directory is not a real owner-only directory: " << datadir_error
                  << "\n";
        return 2;
    }
#ifdef VELD_PUBLIC_RELEASE
    std::string identity_error;
    if (!veld::ValidateOrCreatePublicNetworkIdentity(opt_datadir, &identity_error)) {
    ...
    if (opt_create_miner_key && !opt_import_miner_key.empty()) {
        std::cerr
            << "veld-node: --create-miner-key and --import-miner-key are mutually exclusive\n";
        return 2;
    }
    if (opt_create_miner_key) {
        ...
        std::string policy_error;
        if (!veld::wallet_crypto::ValidateNewPassphrase(passphrase, &policy_error)) {
            std::cerr << "veld-node: " << policy_error << "\n";
            return 1;
        }
        veld::RealKeyPair created = veld::GenerateKeyPair(config.IsTestNetwork());
        struct CreatedPrivateKeyWiper {
            veld::RealKeyPair& value;
            ~CreatedPrivateKeyWiper() {
                veld::compat::SecureZero(value.private_key.data(), value.private_key.size());
            }
        } wipe_created{created};
        std::string address;
        std::string portable_path;
        std::string create_error;
        if (!_wiz_create_portable_key_bundle(opt_datadir, created.private_key, passphrase,
        ...
        std::cout << "MINER-KEY-CREATE-COMPLETE address=" << address << " keyfile=" << portable_path
                  << "\n";
        return 0;
    }
    ...
    if (!opt_import_miner_key.empty()) {
        ...
        const std::string destination = opt_datadir + "/miner.key";
        if (!_wiz_import_encrypted_keyfile(opt_import_miner_key, destination, passphrase,
                                           config.IsTestNetwork(), imported_address,
                                           import_error)) {
            std::cerr << "veld-node: keyfile import failed: " << import_error << "\n";
            return 1;
        }
        std::cout << "MINER-KEY-IMPORT-COMPLETE address=" << imported_address << "\n";
        return 0;
    }
#endif
    ...
    if (opt_reindex_canonical) {
        ...
        if (!veld::db::VeldDB::RebuildCanonicalIndexOffline(opt_datadir + "/db", result,
                                                            &recovery_error)) {
            std::cerr << "veld-node: FATAL: " << recovery_error << "\n";
            return 1;
        }
        std::cout << "CANONICAL-REINDEX-COMPLETE height=" << result.height
                  << " tip=" << result.tip_hash << " scanned=" << result.scanned_blocks
    ...
    auto admit_testnet_lease = [&]() -> bool {
        if (testnet_lease_local_now > 0)
            return true;
        const int64_t local_now = public_testnet::CurrentUnixTime();
        if (!public_testnet::CompiledRuntimeLimits(local_now, testnet_runtime_limits,
                                                   &testnet_runtime_error)) {
            std::cerr << "veld-node: FATAL: public-testnet lease refusal: " << testnet_runtime_error
                      << "\n";
            return false;
        }
    ...
    if (opt_print_rpc_token) {
        ...
        if (!admit_testnet_lease())
            return 78;
#endif
        std::string pass = opt_address_only ? g_passphrase : _wiz_ask_passphrase();
        std::string tf_path =
            opt_datadir + (opt_address_only ? "/address-only-rpc.token" : "/rpc.token");
        std::string tok;
    ...
    std::cout << "  Mode:    "
              << (opt_mine ? GREEN "Mining (starts after sync)" RESET
                           : (opt_endorse ? GREEN "Validator" RESET : GRAY "Node" RESET))
              << "\n";
    if (veld::DiagVerbose().load()) {
        ...
        if (opt_endorse && !opt_mine)
            std::cout << "  Endorsing:  " << GREEN "enabled" RESET
                      << GRAY " (validator endorse-only)" RESET << "\n";
        else if (opt_mine)
    ...
    if (opt_tor_only) {
        ...
        // route ALL traffic through Tor's SOCKS5 (zero clearnet IP exposure).
        {
            ...
            if (veld::channel::secure_file::Read(opt_datadir + "/tor/hs/hostname", hostname,
                                                 &hostname_error, 4096,
                                                 /*require_private_parent=*/true) ==
                veld::channel::secure_file::ReadResult::Ok) {
        ...
        std::cout << CYAN << "  [tor] --tor-only: reachable as " << tor_only_onion
                  << " — outbound P2P uses Tor; HTTPS oracle and checkpoint fetches are disabled."
                  << RESET << "\n";
    }
    ...
    try {
        ...
        std::cerr << "  [snapshot] local chain inspection failed: " << e.what() << "\n";
        return 76;
    }
    ...
    if (opt_snapshot_bootstrap && !opt_regtest &&
        !std::filesystem::exists(std::filesystem::path(opt_datadir) /
                                 "db/.snapshot-consensus-replay-required")) {
        uint64_t local_height = 0;
        snapshot_recovery::Floors inherited_floors;
        try {
            inherited_floors = snapshot_recovery::CaptureLocalState(opt_datadir, config.magic,
            ...
            if (any_existing) {
                ...
                if (!tip)
                    throw std::runtime_error("existing chain has no exact durable tip");
                local_height = tip->height;
            }
        } catch (const std::exception& e) {
            std::cerr << "  [snapshot] existing chain preflight failed: " << e.what() << "\n";
            return 1;
        }

        snapshot_bootstrap::PreparedPublicSnapshot prepared;
        std::string snapshot_error;
        if (snapshot_bootstrap::PreparePublicSnapshot(opt_datadir, local_height, prepared,
                                                      &snapshot_error)) {
            ...
            try {
                const std::string candidate_root = (prepared.scratch_root / "extract").string();
                {
                    ...
                    candidate.PrepareSnapshotCandidateReplay(prepared.manifest.height,
                                                             prepared.manifest.tip_hash);
                    candidate.ValidateStoredChainOnly(prepared.manifest.height,
                                                      prepared.manifest.tip_hash,
                                                      /*verify_historical_pow=*/false);
                    const Block anchor =
                        candidate.GetChain().GetBlock(prepared.manifest.anchor_height);
                    if (HashToHex(anchor.GetHash()) != prepared.manifest.anchor_hash) {
                        throw std::runtime_error(
                            "snapshot launch-chain anchor does not match replay");
                    }
                    validation.consensus_state_sha256 = HashToHex(candidate.ConsensusStateDigest());
                    if (!candidate.AnchorSecurityImportSatisfied())
                        throw std::runtime_error(
                            "snapshot does not reconstruct every retained local security floor");
                ...
                std::cout << "  [snapshot] authenticated snapshot h=" << prepared.manifest.height
                          << " installed; services remain quarantined while "
                             "the same chain is independently rebuilt from genesis.\n";
            } catch (const std::exception& e) {
                std::error_code cleanup_error;
                if (!snapshot_recovery::Exists(std::filesystem::path(opt_datadir) /
                                               snapshot_recovery::IMPORT)) {
            ...
            std::cerr << "  [snapshot] official snapshot not used: " << snapshot_error
                      << "; continuing with ordinary peer IBD.\n";
        }
    }
#endif
    ...
    std::optional<VeldNode::SnapshotValidationBase> background_validation_base;
    ...
    if (std::filesystem::exists(std::filesystem::path(opt_datadir) /
                                "db/.snapshot-consensus-replay-required")) {
        if (!opt_full_ibd) {
            std::string handoff_error;
            installed_snapshot_handoff =
                snapshot_bootstrap::VerifyInstalledSnapshotHandoff(opt_datadir, &handoff_error);
    ...
    const auto persist_current_receipt = [&](std::string* error) {
        if (opt_address_only) {
            if (!address_validation_key_ready) {
                if (error)
                    *error = "address-only validation identity is unavailable";
                return false;
            }
            return node.PersistAddressOnlyIbdReceiptAtTip(address_validation_kp, opt_miner_addr,
                                                          full_ibd_receipt, error);
        }
#ifdef VELD_FLEET_NO_MINE
        if (!fleet_validation_key_ready) {
            if (error)
                *error = "fleet identity is unavailable";
            return false;
        }
        return node.PersistFullIbdReceiptAtTip(fleet_validation_kp, true, full_ibd_receipt, error);
#else
        if (!miner_key_ready) {
            if (error)
                *error = "node identity is unavailable";
            return false;
        }
        return node.PersistFullIbdReceiptAtTip(miner_kp, false, full_ibd_receipt, error);
#endif
    };
#endif
#if defined(VELD_ENABLE_SNAPSHOT_BOOTSTRAP)
    ...
    if (!opt_regtest) {
        ...
        if (opt_address_only) {
            const std::string key_file = snapshot_bootstrap::AddressKeyPath(opt_datadir);
            if (std::filesystem::exists(key_file)) {
                address_validation_key_ready = _wiz_load_key_encrypted(
                    key_file, g_passphrase, address_validation_kp, config.IsTestNetwork());
                if (!address_validation_key_ready) {
                ...
                address_validation_key_ready =
                    _wiz_save_key_encrypted(key_file, address_validation_kp.private_key,
            ...
            const bool receipt_present =
                std::filesystem::exists(snapshot_bootstrap::AddressReceiptPath(opt_datadir));
            if (receipt_present) {
                full_ibd_receipt_valid = snapshot_bootstrap::VerifyAddressIbdReceipt(
                    opt_datadir, address_validation_kp.public_key, opt_miner_addr, full_ibd_receipt,
                    &receipt_error, false, config.IsTestNetwork());
                if (!full_ibd_receipt_valid) {
                    std::cerr << "  [snapshot] invalid address-only full-IBD "
                                 "receipt: "
                              << receipt_error << "; fast restart remains locked.\n";
                }
            }
            if (full_ibd_receipt_valid) {
...
#if defined(VELD_FLEET_NO_MINE) && defined(VELD_ENABLE_SNAPSHOT_BOOTSTRAP)
            const std::string fleet_key_file = snapshot_bootstrap::FleetKeyPath(opt_datadir);
            if (std::filesystem::exists(fleet_key_file)) {
                if (g_passphrase.empty()) {
                    std::string env_pass = _wiz_ask_passphrase();
                    if (!env_pass.empty())
                        g_passphrase = env_pass;
                }
                fleet_validation_key_ready = _wiz_load_key_encrypted(
                    fleet_key_file, g_passphrase, fleet_validation_kp, config.IsTestNetwork());
                if (!fleet_validation_key_ready) {
                    std::cerr << "  [snapshot] cannot decrypt the fleet validation "
                                 "key; recovery remains locked.\n";
                    return 1;
                }
                full_ibd_receipt_valid = snapshot_bootstrap::VerifyFleetIbdReceipt(
                    opt_datadir, fleet_validation_kp.public_key, full_ibd_receipt, &receipt_error);
            }
            const bool receipt_present =
                std::filesystem::exists(snapshot_bootstrap::FleetReceiptPath(opt_datadir));
#else
            const bool receipt_present =
                std::filesystem::exists(snapshot_bootstrap::ReceiptPath(opt_datadir));
            if (receipt_present) {
                ...
                if (std::filesystem::exists(miner_key_file)) {
                    if (_wiz_is_encrypted(miner_key_file)) {
                        ...
                        miner_key_ready = _wiz_load_key_encrypted(miner_key_file, g_passphrase,
                                                                  miner_kp, config.IsTestNetwork());
                    } else {
                        miner_key_ready =
                            _wiz_load_plain_key(miner_key_file, miner_kp, config.IsTestNetwork());
                    }
                }
                if (!miner_key_ready) {
                ...
                full_ibd_receipt_valid = snapshot_bootstrap::VerifyFullIbdReceipt(
                    opt_datadir, miner_kp.public_key, full_ibd_receipt, &receipt_error);
            }
#endif
            if (full_ibd_receipt_valid) {
#ifdef VELD_FLEET_NO_MINE
                ...
                std::cerr << "  [snapshot] invalid full-IBD receipt: " << receipt_error
    ...
    if (installed_snapshot_handoff && !opt_full_ibd) {
        node.SetSnapshotFastStartEligible(true, installed_snapshot_handoff->height,
                                          installed_snapshot_handoff->tip_hash);
        node.SetSnapshotQuarantineOnly(true);
    } else {
        node.SetSnapshotFastStartEligible(
            full_ibd_receipt_valid, full_ibd_receipt_valid ? full_ibd_receipt.height : 0,
    ...
    try {
        ...
        if (!public_testnet::BindOrVerifySession(
                ...
                std::max(testnet_lease_local_now, public_testnet::CurrentUnixTime()),
                &testnet_runtime_error)) {
            throw std::runtime_error("public-testnet session refusal: " + testnet_runtime_error);
        }
    } catch (const std::exception& e) {
        std::cerr << "veld-node: FATAL: public-testnet runtime refusal: " << e.what() << "\n";
        return 78;
    }
#endif
    if (opt_p2p_port)
        node.SetP2PPort(opt_p2p_port);
...
#else
        uint64_t activation = opt_staking_supply > 0 ? opt_staking_supply * VELD_UNITS
                                                     : config.bootstrap_phase_end_units;
        node.SetStakingActivation(activation);
        if (opt_staking_supply > 0) {
            std::cout << GRAY << "  Staking activates at: " << opt_staking_supply
                      << " VELD (bringup override)\n"
                      << RESET;
            ...
            std::cout << GRAY << "  Staking activates at: " << (double)activation / VELD_UNITS
                      << " VELD (config)\n"
                      << RESET;
#endif
    }
    ...
    if (opt_mine)
        veld::net::g_suppress_sync.store(true);
    ...
    try {
        ...
        if (opt_tor_only)
            node.ConfigureTorOnly(tor_only_onion);
        ...
        std::cerr << "  [FATAL] Offline startup qualification failed: " << e.what() << "\n";
        return 1;
    } catch (...) {
    ...
    if (!miner_kp.address.empty() && node.IsAddressRegisteredValidator(miner_kp.address)) {
        advertised_services |= veld::MessageType::NODE_VALIDATOR;
        node.SetAdvertisedServices(advertised_services);
    }
    ...
    if (opt_reachable && node.GetTCPServer()
        ...
        if (veld::DiagVerbose().load())
            std::cout << CYAN << "  [nat] automatic inbound mapping enabled" << RESET << "\n";
    ...
    if (opt_tor && node.GetTCPServer()) {
        std::cout << CYAN
                  << "  [tor] --tor: publishing a v3 .onion hidden service "
    ...
    for (size_t anchor_index = 0; anchor_index < opt_fleet_anchor_ips.size(); ++anchor_index) {
        ...
        if (node.AddFleetAnchorIp(ip)) {
            if (veld::DiagVerbose().load())
                std::cout << GREEN << "  [network] " << RESET << "Seed node " << (anchor_index + 1)
                          << " connected endpoint=" << ip << ":" << config.port << "\n";
        } else {
            std::cerr << YEL << "  [network] WARN: " << RESET << "Seed node " << (anchor_index + 1)
    ...
    if (opt_verify_pow)
        node.ForcePowVerification();

    if (opt_connect.empty() && !opt_regtest) {
        std::this_thread::sleep_for(std::chrono::milliseconds(500));
        const auto seeds = opt_tor_only ? veld::seeder::SeedNodeClient::GetTorBootstrap()
        ...
        for (const auto& seed : seeds) {
            if (node.ConnectTo(seed, config.port)) {
                ++connected;
                if (veld::DiagVerbose().load())
                    std::cout << "  [peer] " << seed << ":" << config.port << " connected\n";
            } else {
                if (veld::DiagVerbose().load())
                    std::cout << "  [peer] " << seed << ":" << config.port << " unavailable\n";
            }
        }
        if (connected == 0) {
            std::cout << "  Peer discovery is active; initial connections are still pending.\n";
        } else {
            std::cout << GREEN << "  Peers:   " << RESET << connected << " connected\n";
        }
    }

    {
        ...
        for (const auto& ip : opt_trusted_ips) {
            ...
            if (veld::DiagVerbose().load())
                std::cout << GREEN << "  [trusted-ip] " << RESET << ip << " added\n";
        }
    }
    ...
    if (!opt_connect.empty()) {
        ...
        for (const auto& peer : opt_connect) {
            ...
            if (node.ConnectTo(host, port))
                ...
                std::cout << RED << "  [ERR] " << RESET << "Could not connect to " << host << ":"
                          << port << "\n";
        }
    }
    ...
    if (background_validation_base) {
        try {
            const std::string background_dir = opt_datadir + "/background-ibd";
            background_chainstate = std::make_unique<VeldNode>(config, background_dir);
            ...
            background_chainstate->GetChainMut().SetLocalValidationCeiling(
                background_validation_base->height);
#if defined(VELD_MAINNET_POW) && defined(VELD_PUBLIC_RELEASE)
            background_chainstate->SetStakingActivation(STAKING_ACTIVATION_SUPPLY);
#else
            background_chainstate->SetStakingActivation(config.bootstrap_phase_end_units);
#endif
            ...
            node.SetIndependentValidationProgress(background_chainstate->GetChain().Height());

            if (!opt_connect.empty()) {
                background_validation_peers = opt_connect;
            } else if (!opt_regtest) {
                background_validation_peers =
                    opt_tor_only ? veld::seeder::SeedNodeClient::GetTorBootstrap()
                                 : veld::seeder::SeedNodeClient::GetHardcodedBootstrap();
            }
            ...
            if (background_chainstate)
                background_chainstate->Stop();
            node.RejectIndependentBackgroundValidation("background-chainstate-start-failed");
        ...
        opt_datadir + (opt_address_only ? "/address-only-rpc.token" : "/rpc.token");
    {
        ...
        if (opt_address_only && std::filesystem::exists(token_file) && rpc_token.size() != 64)
            throw std::runtime_error(
                "address-only RPC credential cannot decrypt the existing token; refusing replacement");
        if (rpc_token.size() != 64) {
            ...
            if (g_passphrase.empty()) {
                throw std::runtime_error("Cannot issue RPC token without a wallet passphrase. "
            ...
            if (!_wiz_save_rpc_token(token_file, rpc_token, g_passphrase, &token_error))
                throw std::runtime_error("cannot atomically write owner-only RPC token: " +
                                         token_error);
            if (veld::DiagVerbose().load())
...
#if defined(VELD_FLEET_NO_MINE) && defined(VELD_ENABLE_SNAPSHOT_BOOTSTRAP)
        const std::string fleet_key_file = snapshot_bootstrap::FleetKeyPath(opt_datadir);
        if (!fleet_validation_key_ready && std::filesystem::exists(fleet_key_file)) {
            fleet_validation_key_ready = _wiz_load_key_encrypted(
                fleet_key_file, g_passphrase, fleet_validation_kp, config.IsTestNetwork());
            if (!fleet_validation_key_ready) {
                throw std::runtime_error("cannot decrypt the fleet validation key");
            }
        } else if (!fleet_validation_key_ready) {
            fleet_validation_kp = GenerateKeyPair(config.IsTestNetwork());
            fleet_validation_key_ready =
                _wiz_save_key_encrypted(fleet_key_file, fleet_validation_kp.private_key,
                                        g_passphrase, config.IsTestNetwork());
            if (!fleet_validation_key_ready) {
                throw std::runtime_error("cannot create the encrypted fleet validation key");
            }
        }
#endif
    }
...
#ifdef VELD_PUBLIC_TESTNET
            if (srv->Start(
                    [&node]() noexcept { return node.PublicTestnetRestartAuthorityFreshNow(); })) {
#else
            if (srv->Start()) {
#endif
                if (p != opt_rpc_port) {
                    std::cout << CYAN << "  [INFO] " << RESET << "RPC port " << opt_rpc_port
    ...
    if (rpc_http_owned) {
        ...
        try {
            ...
            (void)veld::channel::secure_file::AtomicWriteText(port_file, port_text, &port_error,
                                                              /*require_private_parent=*/true);
        } catch (...) {
        }
    }
        ...
        std::cout << RED << "  [WARN] " << RESET
                  << "Could not bind immutable PUBLIC TESTNET RPC port " << opt_rpc_port
                  << " — wallet/explorer cannot connect. Free the port and restart.\n";
#else
        std::cout << RED << "  [WARN] " << RESET << "Could not bind RPC on any of " << opt_rpc_port
                  << ", " << (opt_rpc_port + 10) << ", " << (opt_rpc_port + 20) << ", "
                  << (opt_rpc_port + 30) << ", " << (opt_rpc_port + 40)
    ...
    if (!opt_address_only) {
        ...
        if (std::filesystem::exists(pool_kp_file)) {
            ...
            if (_wiz_is_encrypted(pool_kp_file)) {
                ...
                if (g_shutdown.load())
                    return 0;
                if (!_wiz_load_key_encrypted(pool_kp_file, g_passphrase, pool_kp,
                ...
                if (!_wiz_load_plain_key(pool_kp_file, pool_kp, config.IsTestNetwork())) {
                    std::cerr << RED << "  FATAL: pool.key is unsafe, malformed, or inconsistent.\n"
                              << RESET;
                    return 1;
                }
                node.SetPoolKeypair(pool_kp);
                ...
                if (!g_passphrase.empty() && g_passphrase.size() >= 8) {
                    ...
                    if (_wiz_save_key_encrypted(pool_kp_file, priv, g_passphrase,
                                                config.IsTestNetwork())) {
                        std::cout << GRAY << "  Pool key:   " << RESET
                                  << "auto-upgraded plaintext → encrypted\n";
                    } else {
                        std::cerr
                            << RED
                            << "  [FATAL] Failed to encrypt pool.key. Refusing to run with plaintext.\n"
                            << RESET;
                        return 1;
                    }
                } else {
    ...
    if (!opt_address_only) {
        ...
        if (std::filesystem::exists(ep_kp_file)) {
            ...
            if (g_passphrase.empty() || g_passphrase.size() < 8) {
                std::cerr
                    << RED
                    << "  [WARN] endorsement_pool.key missing and no passphrase to auto-generate.\n"
                ...
                try {
                    ...
                    std::cerr << RED << "  Secure endorsement key generation failed: " << e.what()
                              << "\n"
                              << RESET;
                    return 1;
                }
...
#ifndef VELD_FLEET_NO_MINE
    bind_existing_regtest_generation_identity = opt_regtest && std::filesystem::exists(kp_file);
#if defined(VELD_ENABLE_SNAPSHOT_BOOTSTRAP)
    bind_node_validation_identity = !opt_regtest;
#endif
#endif
    if (opt_address_only || opt_mine || opt_endorse || bind_existing_regtest_generation_identity ||
        bind_node_validation_identity) {
        if (opt_address_only) {
            miner_kp.private_key.fill(0);
            miner_kp.public_key.fill(0);
            miner_kp.address = opt_miner_addr;
            miner_kp.testnet = config.IsTestNetwork();
            miner_kp.script_override = AddressToScript(opt_miner_addr);
            std::cout
                << "  Address-only mode: payout only; wallet and validator signing disabled.\n";
        } else {
            if (std::filesystem::exists(kp_file)) {
                if (_wiz_is_encrypted(kp_file)) {
                    if (!miner_key_ready) {
                        ...
                        if (g_shutdown.load())
                            return 0;
                        miner_key_ready = _wiz_load_key_encrypted(kp_file, g_passphrase, miner_kp,
                                                                  config.IsTestNetwork());
                        if (!miner_key_ready) {
                            if (g_shutdown.load())
                                return 0;
                    ...
                    std::cout << GREEN
                              << (opt_mine || opt_endorse ? "  Mining wallet unlocked.\n"
                    ...
                    if (!miner_key_ready) {
                        miner_key_ready =
                            _wiz_load_plain_key(kp_file, miner_kp, config.IsTestNetwork());
                    }
                    if (!miner_key_ready) {
                        std::cerr << RED
                    ...
                    if (!pass.empty() && pass.size() >= 8) {
                        if (!_wiz_save_key_encrypted(kp_file, miner_kp.private_key, pass,
                                                     config.IsTestNetwork())) {
                        ...
                        std::cout
                            << YEL
                            << "  [WARN] --allow-plaintext-miner-key set — continuing with plaintext (INSECURE).\n"
                            << RESET;
                    } else {
                        ...
                        if (yn != "y" && yn != "Y") {
                            std::cerr
                                << RED << "  FATAL: refusing to run with plaintext miner.key.\n"
                        ...
                        if (ipass.empty() ||
                            !_wiz_save_key_encrypted(kp_file, miner_kp.private_key, ipass,
                                                     config.IsTestNetwork())) {
                ...
                if (g_passphrase.empty()) {
                    std::string pass = _wiz_prompt_passphrase(true);
                    if (!pass.empty()) {
                        saved = _wiz_save_key_encrypted(kp_file, miner_kp.private_key, pass,
                                                        config.IsTestNetwork());
                        if (saved)
                            g_passphrase = pass;
                        veld::WipeString(pass);
                    ...
                    saved = _wiz_save_key_encrypted(kp_file, miner_kp.private_key, g_passphrase,
                                                    config.IsTestNetwork());
                }
                if (!saved) {
                    std::cerr << RED
            ...
            if (miner_key_ready && !g_passphrase.empty() &&
                ...
                if (portable_created) {
                    std::cout << GREEN << "  Portable wallet keyfile created: " << portable_path
                              << "\n"
                              << RESET;
                }
            }
            ...
            if (!opt_miner_addr.empty() &&
                ...
                if (override_script.empty()) {
                    std::cout << RED << "  [ERR] Invalid --miner address: " << opt_miner_addr
                              << RESET << "\n";
                } else {
                    miner_kp.script_override = override_script;
                    miner_kp.address = opt_miner_addr;
                    std::cout << "  Mining to:  " << GOLD << opt_miner_addr << RESET
                              << "  (--miner override)\n";
                }
            }
        ...
        if (opt_regtest) {
            ...
            if (!(opt_address_only
                      ? node.BindAddressGenerationIdentity(opt_miner_addr,
        ...
        if (opt_mine) {
            ...
            if (veld::DiagVerbose().load())
                std::cout << "  Threads: " << mine_threads << " of "
                          << (capacity.topology_detected ? "" : "~") << capacity.physical
                          << " physical / " << capacity.logical << " logical (" << tune_reason
                          << ")\n";
#ifdef VELD_REGTEST_FIXED_DIFF
            ...
            std::cout << CYAN
                      << "  [Endorse-only: endorsing blocks as a validator; mining disabled]\n"
                      << RESET;
        } else if (opt_regtest) {
            std::cout << CYAN
            ...
            std::cout << CYAN << "  [Node validation identity ready; mining disabled]\n" << RESET;
        }
    }
    ...
    std::thread heartbeat_thread([&] {
        while (!g_shutdown.load()) {
            ...
            std::time_t t = std::time(nullptr);
            if (veld::DiagVerbose().load())
                veld::vcerr() << "  [hb] ts=" << t << " height=" << node.GetChain().Height()
                              << " peers=" << node.ConnectedPeers()
    ...
    while (!g_shutdown) {
        ...
        if (!opt_datadir.empty()) {
            auto peers = node.GetTCPServer() ? node.GetTCPServer()->GetPeerInfoList()
            ...
            const bool snapshot_fast_start_eligible = node.SnapshotFastStartEligible();
            const auto validation_base = node.IndependentValidationBase();
            const uint64_t verification_height =
                validation_base ? node.IndependentValidationProgress() : local_height;
            const uint64_t verification_target =
                validation_base ? validation_base->height : local_height;
#else
            ...
            status << std::fixed << std::setprecision(2)
                   << "{\"mining_configured\":" << (node.IsMiningConfigured() ? "true" : "false")
                   << ",\"mining_ready\":" << (node.IsMiningReady() ? "true" : "false")
                   << ",\"mining_active\":" << (node.IsMiningActive() ? "true" : "false")
                   ...
                   << ",\"full_ibd\":" << (node.IsFullIbd() ? "true" : "false")
                   << ",\"work_state\":\"" << node.GetMiningWorkStateName() << "\""
                   ...
                   << ",\"miner_address\":\"" << miner_kp.address << "\",\"known_peer_count\":"
                   << (node.GetTCPServer() ? node.GetTCPServer()->GetKnownPeerCount() : 0)
                   << ",\"topology_id\":"
                   << (node.GetTCPServer() ? node.GetTCPServer()->TopologyId() : 0)
                   << ",\"topology_role\":\""
                   << (node.GetTCPServer() ? node.GetTCPServer()->TopologyRole() : "node") << "\""
                   << ",\"port_mapped\":"
                   << ((node.GetTCPServer() && node.GetTCPServer()->PortMapped()) ? "true"
                                                                                  : "false")
                   << ",\"peer_details\":[";
            for (size_t peer_index = 0; peer_index < peers.size(); ++peer_index) {
                ...
                const uint64_t peer_height = have_tip ? tip_it->second->height : 0;
                const int64_t peer_tip_age =
                    have_tip ? now_seconds - tip_it->second->updated_at : -1;
                const int64_t lag_blocks = have_tip ? static_cast<int64_t>(local_height) -
                                                          static_cast<int64_t>(peer_height)
                                                    : 0;
                const bool exact_tip = have_tip && have_local_tip && peer_height == local_height &&
                                       tip_it->second->hash == local_tip;
                if (peer_index != 0)
                    status << ',';
                const bool peer_identified = peer.node_id != 0;
                const uint64_t topology_peer_id =
                    peer_identified ? peer.node_id : static_cast<uint64_t>(peer_index + 1);
                status << "{\"id\":" << topology_peer_id << ",\"connection_id\":\""
                       << peer.connection_id << "\""
                       << ",\"identified\":" << (peer_identified ? "true" : "false")
                       << ",\"role_index\":" << peer.role_index
                       << ",\"inbound\":" << (peer.inbound ? "true" : "false") << ",\"role\":\""
                       << peer.role << "\""
                       << ",\"services\":" << peer.services
                       << ",\"exact_tip\":" << (exact_tip ? "true" : "false")
                       << ",\"bytes_sent\":" << peer.bytes_sent
                       << ",\"bytes_recv\":" << peer.bytes_recv
                       << ",\"peer_height\":" << peer_height
                       << ",\"peer_tip_age_s\":" << peer_tip_age << ",\"lag_blocks\":" << lag_blocks
                       << '}';
            }
            status << "],\"daemon\":{\"version\":\"" << CLIENT_VERSION << "\",\"profile\":\""
                   << DEPLOYMENT_PROFILE_ID << "\",\"pid\":"
#ifdef _WIN32
                   << GetCurrentProcessId()
#else
                   << getpid()
                   ...
                   << (node.ChainFullyValidated() ? "true" : "false") << "}}";
            std::string status_error;
            (void)veld::channel::secure_file::AtomicWriteText(opt_datadir + "/gui-status.json",
                                                              status.str(), &status_error, true);
        }
        ...
        if (background_chainstate && background_validation_base) {
            if (background_chainstate->FailStopRequired() ||
                ...
                node.RejectIndependentBackgroundValidation("background-chainstate-runtime-failed");
                std::cerr << "  [snapshot] FATAL: independent background "
            ...
            const auto observation = background_chainstate->BackgroundValidationResult();
            if (observation.passed_target || observation.reached) {
                ...
                frozen.height = background_chainstate->GetChain().Height();
                if (!background_chainstate->GetChain().IsEmpty()) {
                    frozen.tip_hash = background_chainstate->GetChain().TipCopy().GetHash();
                }
                if (frozen.height == background_validation_base->height) {
                    frozen.state_digest = background_chainstate->ConsensusStateDigest();
                    frozen.reached = true;
                } else {
                    frozen.passed_target = true;
                }
                std::string validation_error;
                if (!node.FinalizeIndependentBackgroundValidation(frozen, &validation_error)) {
                    node.RejectIndependentBackgroundValidation("background-chainstate-mismatch");
                    std::cerr << "  [snapshot] FATAL: independent background "
                ...
                if (node.SnapshotQuarantineOnly()) {
                    ...
                    if (!receipt_written) {
                        std::cerr << "  [snapshot] FATAL: independent IBD "
                                  ...
                                  << (receipt_error.empty() ? "role identity is unavailable"
                                                            : receipt_error)
                                  << "\n";
                        snapshot_verification_exit = true;
                        g_shutdown.store(true);
                ...
                const uint64_t background_height = background_chainstate->GetChain().Height();
                node.SetIndependentValidationProgress(background_height);
                if (background_height != background_last_reported_height &&
                    (background_height <= 10 || background_height % 100 == 0 || tick % 60 == 0)) {
                    background_last_reported_height = background_height;
                    std::cout << "  [background-ibd] verified " << background_height << "/"
                              << background_validation_base->height << " from genesis\n";
                    std::cout.flush();
                }
                if (tick % 30 == 0) {
                    for (const auto& peer : background_validation_peers) {
                        ...
                        const std::string host =
                            colon == std::string::npos ? peer : peer.substr(0, colon);
                        uint16_t port = config.port;
                        if (colon != std::string::npos) {
                            try {
                                port = static_cast<uint16_t>(std::stoul(peer.substr(colon + 1)));
        ...
        if (!opt_fleet_anchor_ips.empty() && (tick % 30) == 0) {
            for (size_t anchor_index = 0; anchor_index < opt_fleet_anchor_ips.size();
                ...
                const auto& ip = opt_fleet_anchor_ips[anchor_index];
                if (!node.AddFleetAnchorIp(ip)) {
                    std::cerr << YEL << "  [network] WARN: " << RESET << "Seed node "
                              << (anchor_index + 1) << " is temporarily unavailable; retrying";
                    if (veld::DiagVerbose().load())
                        std::cerr << " endpoint=" << ip << ":" << config.port;
                    std::cerr << "\n";
                }
            ...
            if (now_ms - last_seed_redial_ms >= 30000) {
                ...
                if (connected_seeds < seeds.size()) {
                    std::cout << "  [seed-watchdog] connected seeds=" << connected_seeds << "/"
                              << seeds.size() << " (" << (seeds.size() - connected_seeds)
                              << " missing) — re-dialing dropped seed(s)\n";
                    std::cout.flush();
                }
                for (const auto& seed : seeds) {
                    const std::string key = seed + ":" + std::to_string(config.port);
        ...
        bool at_tip = IsInitialDownloadAtTip(
            opt_regtest, cur_height, node.GetChain().IsEmpty(), peer_heights.distinct_version_ips,
        ...
        if (at_tip && cur_height > 0) {
            ...
            try {
                our_tip_hash = node.GetChain().Tip().GetHash();
            } catch (...) {
            }
            int agree = 0, disagree = 0, fresh_tips = 0;
            for (const auto& t : tips) {
                if (now_s - t.updated_at > 180)
                    continue;
                if (::veld::HashIsZero(t.hash))
                    continue;
                ++fresh_tips;
...
#if defined(VELD_ENABLE_SNAPSHOT_BOOTSTRAP)
                    bool receipt_role_ready =
                        opt_address_only ? address_validation_key_ready : miner_key_ready;
#ifdef VELD_FLEET_NO_MINE
                    receipt_role_ready = fleet_validation_key_ready;
#endif
                    if (receipt_role_ready && node.ChainFullyValidated() &&
                        (!full_ibd_receipt_valid || cur_height > full_ibd_receipt.height)) {
...
#endif
                    std::cout << (node.ChainFullyValidated() ? "  [IBD complete] height="
                                                             : "  [network sync complete] height=")
                              << cur_height;
        ...
        bool receipt_refresh_role_ready =
            opt_address_only ? address_validation_key_ready : miner_key_ready;
#ifdef VELD_FLEET_NO_MINE
        ...
        const bool receipt_refresh_due =
            full_ibd_receipt_valid && cur_height > full_ibd_receipt.height &&
            cur_height - full_ibd_receipt.height >= FULL_IBD_RECEIPT_REFRESH_BLOCKS &&
            (tick % 60) == 0;
        if (receipt_refresh_role_ready && at_tip && stable_ticks >= 3 && node.IsIBDComplete() &&
            node.ChainFullyValidated() &&
        ...
        if (node.IsIBDComplete() && peer_best > cur_height &&
            peer_best - cur_height > STALE_TIP_TOLERANCE) {
            ++stale_tip_secs;
            if (stale_tip_secs >= STALE_TIP_GRACE_SECS) {
                std::cout << "  [re-IBD] peer tip=" << peer_best << " ahead of ours=" << cur_height
                          << " for " << stale_tip_secs << "s — re-entering IBD to catch up\n";
                std::cout.flush();
        ...
        constexpr uint64_t DEEP_FORK_THRESHOLD = ::veld::MAX_REORG_DEPTH + 50;
        ...
        if (node.IsIBDComplete() && peer_best > cur_height &&
            ...
            if (deep_fork_secs % 30 == 0) {
                std::cerr << "  [deep-fork-watch] peer_best=" << peer_best << " cur=" << cur_height
                          << " gap=" << (peer_best - cur_height) << " (> MAX_REORG_DEPTH+50); "
    ...
    !defined(VELD_ENABLE_SNAPSHOT_BOOTSTRAP)
                ...
                std::cerr << "  [deep-fork-watch] public-profile grace expired "
                          << "(cur=" << cur_height << " peer_best=" << peer_best
        ...
        if (peer_best > cur_height && peer_best > 0) {
            if (stuck_anchor_height == UINT64_MAX) {
                ...
                ++stuck_secs;
                if (stuck_secs % 60 == 0) {
                    std::cerr << "  [stuck-watch] cur=" << cur_height << " peer_best=" << peer_best
                              << " gap=" << (peer_best - cur_height) << " no-advance for "
                              << stuck_secs << "s; bounded peer recovery in "
                    ...
                    if (stuck_secs == 180) {
                        ...
                        std::cerr << "  [stuck-watch] in-process recovery: cleared " << n
                                  << " reject-cache entries (escalation @180s)\n";
                        std::cerr.flush();
                    }
                    if (stuck_secs == 240) {
                        size_t n = node.ClearOrphanPool();
                        std::cerr << "  [stuck-watch] in-process recovery: cleared " << n
                                  << " orphan-pool entries (escalation @240s)\n";
                        std::cerr.flush();
                        node.TriggerTipReconcile();
                    }
                }
                if (stuck_secs >= STUCK_GRACE_SECS) {
                    uint64_t gap = (peer_best > cur_height) ? (peer_best - cur_height) : 0;
                    std::cerr << "  [stuck-watch] no chain advance for " << stuck_secs
                              << "s while peers advanced "
                              << "(cur=" << cur_height << " peer_best=" << peer_best
                              << " gap=" << gap << ").\n";
                    std::cerr.flush();
                    const char* disable_restart = std::getenv("VELD_DISABLE_STUCK_RESTART");
                    bool restart_suppressed =
                        disable_restart && (*disable_restart == '1' || *disable_restart == 'y' ||
                ...
                if (fresh_tips >= 2 && disagree >= 1 && agree <= disagree &&
                    ...
                    if (diverged_anchor_height == UINT64_MAX ||
                        ...
                        if (diverged_stuck_secs >= STUCK_GRACE_SECS) {
                            std::cerr << "  [diverged-tip] no advance for " << diverged_stuck_secs
                                      << "s while peers hold a"
                                      << " tip we can't fetch (cur=" << cur_height
                                      << " agree=" << agree << " disagree=" << disagree
                                      << "). Restarting to resync.\n";
                            std::cerr.flush();
                            const char* disable_restart = std::getenv("VELD_DISABLE_STUCK_RESTART");
                            bool restart_suppressed =
                                disable_restart &&
        ...
        if (!opt_address_only && !endorser_keys_loaded) {
            ...
            // additional signing keys.
            {
                ...
                if (extra_key_path && extra_key_path[0]) {
                    ...
                    if (_wiz_is_encrypted(extra_key_path)) {
                        ...
                        if (_wiz_load_key_encrypted(extra_key_path, pass, extra_kp,
                                                    config.IsTestNetwork())) {
                            std::string wpk_hex;
                            static const char* hx = "0123456789abcdef";
                            for (int i = 0; i < 1952; ++i) {
                            ...
                            std::cout << "  [validator] Loaded extra endorser key "
                                      << extra_kp.address << "\n";
                        } else {
                            std::cerr
                                << RED
                                << "  [validator] FATAL: failed to decrypt extra endorser key — wrong passphrase?"
                                << RESET << "\n";
                            veld::WipeString(pass);
                            if (!extra_optional) {
                                std::cerr
                                    << RED
                                    << "  [validator] Set VELD_EXTRA_ENDORSER_KEY_OPTIONAL=1 to tolerate this and continue without the extra key."
                                    << RESET << "\n";
                                return 1;
                            }
                        }
                        ...
                        std::cerr
                            << RED
                            << "  [validator] FATAL: VELD_EXTRA_ENDORSER_KEY must point to an ENCRYPTED key file. Refusing to load plaintext key."
                            << RESET << "\n";
                        if (!extra_optional)
                            return 1;
                    }
            ...
            if (veld::DiagVerbose().load()) {
                for (auto& [epriv, epub] : endorser_keys) {
                    ...
                    std::cout << "  [validator] address=" << addr << " pubkey=" << preview
        ...
        if (!opt_address_only && (opt_mine || opt_endorse) &&
            cur_height > last_endorsed_height + 1 && cur_height > 2 && node.IsIBDComplete() &&
            node.ChainFullyValidated()) {
            ...
            uint64_t start_h =
                (end_h > ENDORSE_WINDOW_BLOCKS) ? end_h - ENDORSE_WINDOW_BLOCKS + 1 : 1;
            {
                std::lock_guard<std::mutex> lk(local_attempt_ts_mu_);
                uint64_t prune_below =
                    (cur_height > PRUNE_HEIGHT_LOOKBACK) ? cur_height - PRUNE_HEIGHT_LOOKBACK : 0;
                for (auto it = local_attempt_ts_.begin(); it != local_attempt_ts_.end();) {
                    size_t colon = it->first.find(':');
                    if (colon == std::string::npos) {
                        it = local_attempt_ts_.erase(it);
                        continue;
                    }
                    uint64_t h = 0;
                    try {
                        h = std::stoull(it->first.substr(0, colon));
                    } catch (...) {
                        h = 0;
                    }
                    if (h < prune_below)
            ...
            int total_unregistered = 0, total_already = 0, total_no_coins = 0, total_mp_reject = 0;
            ...
            for (uint64_t eh = start_h; eh <= end_h; ++eh) {
                ...
                for (auto& [epriv, epub] : endorser_keys) {
                    if (!node.GetValidators().IsRegistered(epub)) {
                        ++total_unregistered;
                        continue;
                    }
                    bool already = false;
                    for (auto& e : existing)
                        if (e.pubkey_hex == epub) {
                            already = true;
                            break;
                        }
                    if (already) {
                    ...
                    std::string attempt_key = std::to_string(eh) + ":" + epub.substr(0, 32) + ":" +
                    ...
                    work_admission::Subject endorse_subject;
                    {
                        ...
                        endorse_subject.purpose = work_admission::Purpose::ValidatorEndorsement;
                    ...
                    auto endorsement_permit =
                        node.AcquireLocalValidatorEndorsementPermit(endorse_subject);
                    ...
                    auto coins = SelectCoins(node.GetChain(), ekp_script, 0, fee,
                                             node.GetMempoolMut().GetSpentOutputs());
                    if (!coins.sufficient) {
                        coins = SelectCoins(node.GetChain(), miner_kp.GetP2PKHScript(), 0, fee,
                                            node.GetMempool().GetSpentOutputs());
                        if (coins.sufficient)
                            ekp_script = miner_kp.GetP2PKHScript();
                    }
                    if (!coins.sufficient) {
                    ...
                    if (coins.change_amount > 0)
                        tx.outputs.push_back(TxOutput(coins.change_amount, ekp_script));
                    tx.outputs.push_back(TxOutput(0, eop_script));
                    RealKeyPair& sign_kp =
                        (ekp_script == miner_kp.GetP2PKHScript()) ? miner_kp : ekp;
                    if (!endorsement_permit->IsLive()) {
                        ++total_mp_reject;
                        last_mp_reject = "WORK_ADMISSION_STALE";
                        continue;
                    ...
                    const auto sink = node.SubmitLocalValidatorEndorsement(
                    ...
                    if (sink.accepted) {
                        ...
                        ++total_mp_reject;
                        last_mp_reject = sink.deferred ? "WORK_ADMISSION_DEFERRED" : "INVALID";
                    }
                }
                if (round_endorsed > 0) {
            ...
            if (total_endorsed > 0) {
                ...
                std::cout << " (" << total_endorsed << " emit" << (total_endorsed > 1 ? "s)" : ")")
                          << "\n";
                std::cout.flush();
            } else if (log_this_round) {
                if (veld::DiagVerbose().load())
                    veld::vcerr() << "  [endorse-diag] window=" << start_h << "-" << end_h
                                  << " keys=" << endorser_keys.size()
                                  << " unregistered=" << total_unregistered
                                  << " already=" << total_already << " no-coins=" << total_no_coins
                                  << " mp-reject=" << total_mp_reject
                                  << (total_mp_reject ? std::string("(") + last_mp_reject + ")"
                                                      : std::string(""))
                                  << "\n";
                std::cerr.flush();
            }
        ...
        if (tick > 0 && tick % 30 == 0) {
            ...
            if (stale_mp + orphan_mp + expired_mp > 0) {
                std::cerr << "  [mempool-maint] removed stale=" << stale_mp
                          << " orphan=" << orphan_mp << " expired=" << expired_mp << "\n";
                std::cerr.flush();
            }
        ...
        static const uint32_t rotation_offset_s =
            (uint32_t)((std::chrono::steady_clock::now().time_since_epoch().count() /
                        1000000000ULL) %
                       3600);
        if (node.IsIBDComplete() && tick > (long long)rotation_offset_s &&
    ...
    const int shutdown_exit_code =
        testnet_expiry_exit ? 78
                            : ((snapshot_activation_restart || snapshot_recovery_restart)
                                   ? 75
                                   : (snapshot_verification_exit ? 76 : (fail_stop_exit ? 75 : 0)));
#else
    const int shutdown_exit_code = testnet_expiry_exit ? 78 : (fail_stop_exit ? 75 : 0);
#endif

    constexpr int SHUTDOWN_DEADLINE_SEC = 20;
    std::atomic<const char*> shutdown_phase{"RPC service"};
    std::jthread watchdog([shutdown_start, SHUTDOWN_DEADLINE_SEC, shutdown_exit_code,
                           &shutdown_phase](std::stop_token stop) {
        std::mutex mutex;
        std::condition_variable_any wake;
        std::unique_lock lock(mutex);
        (void)wake.wait_for(lock, stop, std::chrono::seconds(SHUTDOWN_DEADLINE_SEC),
                            [] { return false; });
        ...
        auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(
                           ...
                           .count();
        std::cerr << "\n  [shutdown-watchdog] Stop() did not return in " << elapsed << "s during "
                  << shutdown_phase.load()
