...
bool ParseMonitoringReply(const std::string& body, MonitoringReply& out) {
    ...
    if (command && command->kind == veld::btc_buy::JsonValue::Kind::Object) {
        ...
        if (parsed.command.action == "mining.enabled" || parsed.command.action == "privacy.tor" ||
...
std::string CanonicalPortalCommandPayload(const MonitoringReply::Command& command) {
    ...
    if (command.action == "mining.workers" && command.workers >= 1 && command.workers <= 256)
...
unsigned PresetThreadCount(int preset) {
    ...
    if (preset == 0)
        return std::max(1u, (physical + 3u) / 4u);
    if (preset == 2)
        return physical;
    ...
    if (message == WM_COMMAND && LOWORD(wp) == IDOK) {
        ...
        if (!prompt || size <= 0 || size > 1024) {
            if (prompt) {
                ...
                SetDlgItemTextW(dialog, IDC_VELD_PASSPHRASE_LABEL, L"PASSPHRASE REQUIRED");
...
enum class Page { Overview, Blockchain, Mining, Workers, Explorer, Network, Logs, Settings };
...
class NodeGuiApp {
  public:
    ...
    int Run(int show) {
        ...
        hwnd_ = CreateWindowExW(0, wc.lpszClassName, L"Veld Node", WS_OVERLAPPEDWINDOW | WS_VSCROLL,
                                window_x_, window_y_,
  ...
  private:
    ...
    void SetPageScroll(int requested) {
        RECT client{};
        GetClientRect(hwnd_, &client);
        page_scroll_offsets_[PageIndex()] = std::clamp(requested, 0, MaxPageScroll(client));
    ...
    bool IsInteractivePoint(POINT point) {
        const RECT* sidebar_controls[] = {&overview_nav_, &blockchain_nav_, &mining_nav_,
                                          &workers_nav_,  &explorer_nav_,   &network_nav_,
                                          &logs_nav_,     &settings_nav_,   &follow_x_link_};
        ...
        if (page_ == Page::Overview) {
            ...
            if (inside_any({&reachable_toggle_, &tor_toggle_, &reference_toggle_,
                            &remote_monitor_toggle_, &open_monitor_portal_button_,
                            &copy_monitor_code_button_, &mining_mode_toggle_, &open_data_button_,
                            &import_key_button_, &create_key_button_, &preset_eco_button_,
                            &preset_balanced_button_, &preset_max_button_, &preset_minus_button_,
                            &preset_plus_button_, &check_update_button_, &view_changelog_button_}))
                return true;
        }
        return false;
    }
        ...
        while (std::getline(input, line)) {
            if (line == "reachable=0")
                ...
                if (parse_int("mining_threads", 1, 256, threads))
                    ...
                    } else if (parse_int("window_height", 760, 4320, window_height_)) {
    ...
    void DrawOverview(HDC dc, const RECT& client, const LiveState& live) {
        ...
        DrawTextAt(dc,
                   mining_enabled_
                       ...
                       : L"Validation, relay, local explorer, and automatic validator "
                         L"endorsements; CPU mining is off.",
    ...
    void DrawBlockchain(HDC dc, const RECT& client, const LiveState& live) {
        ...
        DrawSyncChoice(dc, snapshot_card_, L"Signed snapshot",
                       live.process_running ? L"Use on the next start; services stay locked while "
                                              L"genesis validation catches up."
                                            : L"Start quickly from an official signed snapshot and "
                                              L"validate it independently in the background.",
                       !full_ibd_choice_, true);
        ...
        const std::wstring detail =
            L"Signed snapshots are replayed before use and remain quarantined from RPC, inbound "
            L"peers, mining, and wallet access until a separate genesis IBD matches the exact tip "
            L"and state digest. Full IBD remains available.";
    ...
    void DrawMining(HDC dc, const RECT& client, const LiveState& live) {
        DrawPageHeader(dc, client, L"Mining", L"VeldHash performance and work admission.", live);
        const int left = S(292);
        const int right = client.right - S(34);
        const int gap = S(16);
        const int top = S(130);
    ...
    void DrawWorkers(HDC dc, const RECT& client, const LiveState& live) {
        ...
        DrawTextAt(dc,
                   L"Per-worker rate is an estimate from the measured aggregate; the node does not "
                   L"report hardware temperature, fan, or power data.",
        ...
        const std::wstring worker_state =
            active ? L"Hashing" : MiningStateLabel(live.mining.work_state);
        ...
        for (uint64_t i = 0; i < shown; ++i) {
            ...
            DrawTextAt(dc, L"CPU " + std::to_wstring(i + 1), cell(2, 20, y1, y2), font_body_,
                       C_TEXT);
            DrawTextAt(dc, worker_state, cell(20, 42, y1, y2), font_body_,
    ...
    void DrawDirectPeerTopology(HDC dc, RECT card, const LiveState& live) {
        ...
        DrawTextAt(dc,
                   L"Direct sessions shown live · network reports join automatically · no "
                   L"addresses exposed",
    ...
    void DrawSettings(HDC dc, const RECT& client, const LiveState& live) {
        ...
        DrawSettingRow(
            ...
            reachable_toggle_);
        DrawSettingRow(dc,
                       {left, rows_top + 2 * (row_h + gap), right, rows_top + 3 * row_h + 2 * gap},
                       L"Show public height reference",
                       L"Uses explorer.veld.network only to estimate visual sync progress. It is "
                       L"never a consensus or security input.",
        ...
        RECT monitor_detail{monitor.left + S(22), monitor.top + S(43), monitor.right - S(390),
                            monitor.bottom - S(10)};
        ...
        if (!monitor_status.pair_code.empty()) {
            copy_monitor_code_button_ = {monitor.right - S(320), monitor.top + S(48),
                                         monitor.right - S(190), monitor.top + S(87)};
            DrawButton(dc, copy_monitor_code_button_, L"Copy code", true);
        } else {
            copy_monitor_code_button_ = {};
        }
        reset_monitor_pairing_button_ = {monitor.right - S(480), monitor.top + S(48),
        ...
        DrawTextAt(
            dc,
            L"Updates require the pinned release signature and exact signed package hash, then "
            L"install atomically with rollback. Wallet secrets never enter the updater.",
    ...
    void OnClick(int x, int y) {
        ...
        if (PtInRect(&overview_nav_, p))
            ...
            if (MessageBoxW(hwnd_,
                ...
                {
                    std::lock_guard<std::mutex> lock(remote_monitor_mutex_);
                    remote_monitor_status_.detail = L"Requesting a new one-time pairing code...";
                    remote_monitor_status_.report_ok = false;
                }
                worker_cv_.notify_all();
            }
    ...
    bool BeginUpdateProcess(UpdateOperation operation) {
        ...
        if (installing && !StopServicesForUpdate())
            return false;
        std::error_code ec;
        std::filesystem::create_directories(state_dir_, ec);
    ...
    void OnTorSetupComplete(DWORD exit_code) {
        ...
        if (exit_code != 0) {
            MessageBoxW(hwnd_,
                        L"Tor could not be prepared. Review the local app log, or select Clearnet "
                        L"in Settings.",
                        L"Veld Node", MB_OK | MB_ICONERROR);
            return;
        }
    ...
    void CreateIdentity() {
        ...
        MessageBoxW(
            hwnd_,
            L"A new mining identity was created. The same encrypted identity was saved as "
            L"miner.key and as a portable .veld-keys file in the Veld data folder. The .veld-keys "
            L"file can be imported into both Veld Wallet and Veld Node using the same passphrase.",
    ...
    void ImportKeyfile() {
        ...
        if (std::filesystem::exists(destination) &&
            MessageBoxW(
                hwnd_,
                L"Replace the current mining and validator identity?\n\nThe existing encrypted "
                L"miner.key will be replaced only if the selected keyfile and passphrase validate.",
        ...
        if (!RunProtectedNodeUtility(arguments, passphrase)) {
            MessageBoxW(
                hwnd_,
                L"The keyfile was not imported. The passphrase may be wrong, or the selected file "
                L"is not a valid Veld keyfile. The existing identity was not replaced.",
                L"Veld Node", MB_OK | MB_ICONERROR);
            return;
        }
        ClearSessionPassphrase();
        MessageBoxW(hwnd_,
                    L"The mining identity was imported. It will also endorse automatically when "
                    L"registered as a validator.",
                    L"Veld Node", MB_OK | MB_ICONINFORMATION);
    }
    ...
    void OpenTrustedWallet() {
        ...
        if (opened <= 32) {
            MessageBoxW(hwnd_,
                        L"The Veld Wallet website could not be opened. Visit "
                        L"https://wallet.veld.network/ in your browser.",
    ...
    void StartNode() {
        ...
        if (!std::filesystem::is_regular_file(data_dir_ / L"miner.key")) {
            page_ = Page::Settings;
            MessageBoxW(hwnd_,
                        L"Create a node identity or import a Veld .veld-keys file in Settings "
                        L"before starting the node.",
                        L"Veld Node", MB_OK | MB_ICONINFORMATION);
            return;
        }
        if (tor_choice_) {
    ...
    void StartNodeEngine() {
        ...
        if (verified_node == INVALID_HANDLE_VALUE) {
            ...
            CloseHandle(nul);
            MessageBoxW(hwnd_,
                        L"The node engine does not match this signed Veld Node app. Reinstall the "
                        L"complete current package.",
    ...
    std::wstring RemoteCommandDescription(const MonitoringReply::Command& command) const {
        ...
        if (command.action == "sync.mode")
            return command.mode == "full" ? L"Use full initial block download on the next start"
                                          : std::wstring{};
        return {};
    }

    bool ConfirmRemoteCommand(const MonitoringReply::Command& command, bool new_trust) const {
        ...
        if (new_trust) {
            ...
            message += L"This is a new portal pairing identity. Approving will "
                       L"also trust command key " +
                       fingerprint + L" for this pairing.\n\n";
        }
    ...
    bool AuthorizeRemoteCommand(const MonitoringReply::Command& command, std::string& rejection) {
        ...
        if (!SavePortalTrust(state_dir_ / L"remote-trust.dat", next)) {
            MessageBoxW(hwnd_,
                        ...
                        L"be saved securely.",
                        L"Veld Portal command blocked", MB_OK | MB_ICONERROR);
        ...
        if (command.action == "node.start") {
            if (live.process_running) {
                ...
                AcknowledgeRemoteCommand(command.id, "local_confirmation",
                                         "Start requested. A passphrase is required locally only "
                                         "when this app session is locked");
            }
    ...
    void ReportRemoteStatus(const LiveState& live, std::string& token) {
        ...
        if (!response.ok) {
            ...
            remote_monitor_status_.report_ok = false;
            remote_monitor_status_.detail = L"Portal unavailable. Retrying automatically.";
            return;
        }
        ...
        if (!reply.paired) {
            ...
            if (trusted.Valid() && trusted.device_id == reply.device_id &&
                ...
                connection_detail = L"The portal command key changed. Remote control is blocked "
                                    L"until you re-pair locally.";
                ...
                connection_detail = L"Signed control is enrolled. The first action will ask you to "
                                    L"trust this pairing locally.";
            }
        }
        {
            std::lock_guard<std::mutex> lock(remote_monitor_mutex_);
    ...
    bool ResetRemotePairing(std::string& token) {
        ...
        if (exists_error || (trust_exists && !DeleteFileW(trust_path.c_str()))) {
            std::lock_guard<std::mutex> lock(remote_monitor_mutex_);
            remote_monitor_status_.report_ok = false;
            remote_monitor_status_.detail = L"Local portal trust could not be cleared securely.";
            return false;
        }
        {
            std::lock_guard<std::mutex> lock(remote_trust_mutex_);
    ...
    void PollLoop() {
        ...
        while (!stop_worker_.load()) {
            ...
            const HttpResult local = HttpGetJson(L"127.0.0.1", 8080, L"/api/stats", false, 900);
            if (local.ok) {
                std::string error;
