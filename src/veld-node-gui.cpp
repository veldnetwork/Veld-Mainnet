...
#endif
#if defined(VELD_POOL_GUI_QUALIFICATION) &&                                                        \
    (!defined(VELD_GUI_TEST_INSTANCE) || defined(VELD_PUBLIC_RELEASE))
...
std::filesystem::path SystemPowerShellPath() {
    ...
    const UINT length =
        GetSystemDirectoryW(system_directory, static_cast<UINT>(sizeof(system_directory) /
                                                                sizeof(system_directory[0])));
    if (length == 0 || length >= sizeof(system_directory) / sizeof(system_directory[0]))
        return {};
    const std::filesystem::path powershell = std::filesystem::path(system_directory) /
                                             L"WindowsPowerShell" / L"v1.0" / L"powershell.exe";
    std::error_code ec;
    return std::filesystem::is_regular_file(powershell, ec) ? powershell : std::filesystem::path{};
}

HttpResult HttpGetJson(const wchar_t* host, INTERNET_PORT port, const wchar_t* path, bool secure,
                       DWORD timeout_ms) {
    HttpResult out;
    HINTERNET session =
        WinHttpOpen(L"VeldNodeGUI/1.0",
    ...
    const BOOL sent = WinHttpSendRequest(request, accept, static_cast<DWORD>(-1L),
...
HttpResult HttpPostJson(const wchar_t* host, INTERNET_PORT port, const wchar_t* path,
                        const std::string& body, const std::string& bearer, DWORD timeout_ms) {
    HttpResult out;
    if (body.empty() || body.size() > 32U * 1024U || bearer.empty() || bearer.size() > 128) {
        out.error = L"Invalid monitoring request";
        return out;
    }
    HINTERNET session = WinHttpOpen(L"VeldNodeGUI/1.0", WINHTTP_ACCESS_TYPE_AUTOMATIC_PROXY,
                                    WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0);
    if (!session) {
    ...
    if (!WinHttpSetOption(request, WINHTTP_OPTION_REDIRECT_POLICY, &redirect_policy,
...
std::string NewDeviceToken() {
    ...
    CryptBinaryToStringA(random, sizeof(random), CRYPT_STRING_BASE64 | CRYPT_STRING_NOCRLF, nullptr,
                         &encoded_size);
    std::string encoded(encoded_size, '\0');
    if (encoded_size == 0 ||
        !CryptBinaryToStringA(random, sizeof(random), CRYPT_STRING_BASE64 | CRYPT_STRING_NOCRLF,
...
DATA_BLOB MonitoringEntropy() {
    static BYTE entropy[] = {0x56, 0x65, 0x6c, 0x64, 0x4e, 0x6f, 0x64, 0x65,
                             0x52, 0x65, 0x6d, 0x6f, 0x74, 0x65, 0x56, 0x31};
    return DATA_BLOB{static_cast<DWORD>(sizeof(entropy)), entropy};
}

bool SaveDeviceToken(const std::filesystem::path& path, const std::string& token) {
    ...
    if (!CryptProtectData(&plain, L"Veld remote monitor credential", &entropy, nullptr, nullptr,
                          CRYPTPROTECT_UI_FORBIDDEN, &protected_blob))
        return false;
    const bool written = veld::node_gui::WriteStateFile(path, protected_blob.pbData,
                                                        protected_blob.cbData, nullptr, false);
...
std::string LoadDeviceToken(const std::filesystem::path& path) {
    ...
    if (input.gcount() != size)
        return {};
    DATA_BLOB protected_blob{static_cast<DWORD>(protected_bytes.size()), protected_bytes.data()};
    DATA_BLOB plain{};
    DATA_BLOB entropy = MonitoringEntropy();
...
bool Base64UrlDecodeExact(const std::string& encoded, size_t expected_size,
    ...
    if (!CryptStringToBinaryA(padded.c_str(), static_cast<DWORD>(padded.size()),
                              ...
                              nullptr, nullptr) ||
        size != expected_size || Base64UrlEncode(decoded.data(), decoded.size()) != encoded) {
        decoded.clear();
        return false;
    }
    return true;
...
HANDLE OpenVerifiedTrustedFile(const std::filesystem::path& path, const std::string& expected) {
    if (expected.size() != 64 ||
    ...
    if (read_total != static_cast<uint64_t>(size.QuadPart) ||
        !veld::compat::ConstantTimeEqual(LowerHex(digest.data(), digest.size()), expected)) {
        CloseHandle(file);
        return INVALID_HANDLE_VALUE;
    }
...
std::string PortalCommandKeyId(const std::string& x, const std::string& y) {
    veld::SHA256 hasher;
...
bool IsPortalCommandKey(const std::string& x, const std::string& y, const std::string& id) {
    std::vector<uint8_t> coordinate;
    return IsPortalKeyId(id) && Base64UrlDecodeExact(x, 32, coordinate) &&
           Base64UrlDecodeExact(y, 32, coordinate) && PortalCommandKeyId(x, y) == id;
}
...
struct PortalTrustState {
    ...
    bool Valid() const {
        return device_id != 0 && IsPortalCommandKey(key_x, key_y, key_id);
    }
};

DATA_BLOB PortalTrustEntropy() {
    static BYTE entropy[] = {0x56, 0x65, 0x6c, 0x64, 0x50, 0x6f, 0x72, 0x74, 0x61,
                             0x6c, 0x54, 0x72, 0x75, 0x73, 0x74, 0x56, 0x33};
    return DATA_BLOB{static_cast<DWORD>(sizeof(entropy)), entropy};
}

bool SavePortalTrust(const std::filesystem::path& path, const PortalTrustState& state) {
    if (!state.Valid())
        return false;
    ...
    if (!CryptProtectData(&plain, L"Veld portal command trust", &entropy, nullptr, nullptr,
                          CRYPTPROTECT_UI_FORBIDDEN, &protected_blob)) {
        SecureZeroMemory(text.data(), text.size());
        return false;
    }
    const bool written =
        veld::node_gui::WriteStateFile(path, protected_blob.pbData, protected_blob.cbData);
    SecureZeroMemory(text.data(), text.size());
    if (protected_blob.pbData) {
        SecureZeroMemory(protected_blob.pbData, protected_blob.cbData);
        LocalFree(protected_blob.pbData);
...
bool LoadPortalTrust(const std::filesystem::path& path, PortalTrustState& state) {
    state = {};
    std::ifstream input(path, std::ios::binary);
    if (!input)
        return false;
    ...
    const bool current = lines.size() == 7 && lines[0] == "VELD_PORTAL_TRUST_V2" &&
                         (lines[6] == "0" || lines[6] == "1");
    const bool ok = (legacy || current) && parse_number(lines[1], parsed.device_id) &&
                    parsed.device_id != 0 && parse_number(lines[2], parsed.last_sequence);
    if (ok) {
...
bool VerifyPortalCommandSignature(const std::string& key_x, const std::string& key_y,
                                  const std::string& envelope, const std::string& signature_text) {
    std::vector<uint8_t> x, y, signature;
    if (!Base64UrlDecodeExact(key_x, 32, x) || !Base64UrlDecodeExact(key_y, 32, y) ||
        !Base64UrlDecodeExact(signature_text, 64, signature))
        return false;
    ...
    if (BCryptOpenAlgorithmProvider(&algorithm, BCRYPT_ECDSA_P256_ALGORITHM, nullptr, 0) == 0) {
        if (BCryptImportKeyPair(algorithm, nullptr, BCRYPT_ECCPUBLIC_BLOB, &key, blob.data(),
                                static_cast<ULONG>(blob.size()), 0) == 0) {
            verified = BCryptVerifySignature(key, nullptr, const_cast<PUCHAR>(digest.data()),
...
bool ParseMonitoringReply(const std::string& body, MonitoringReply& out) {
    ...
    std::string error;
    veld::btc_buy::StrictJsonParser parser(body, 16U * 1024U, /*reject_escaped_object_keys=*/true);
    if (!parser.Parse(root, error) || root.kind != veld::btc_buy::JsonValue::Kind::Object)
        return false;
    MonitoringReply parsed;
    if (root.object.size() != 8 ||
        !veld::node_gui::ParseUint(root.Get("portal_protocol"), parsed.protocol) ||
        (parsed.protocol != 3 && parsed.protocol != 4) ||
        !veld::node_gui::ParseUint(root.Get("device_id"), parsed.device_id) ||
        parsed.device_id == 0 || !veld::node_gui::ParseBool(root.Get("paired"), parsed.paired) ||
        !veld::node_gui::ParseUint(root.Get("pair_expires"), parsed.pair_expires) ||
        !veld::node_gui::ParseUint(root.Get("report_interval"), parsed.report_interval) ||
        parsed.report_interval < 5 || parsed.report_interval > 300)
        return false;
    const auto* pair_code = root.Get("pair_code");
    if (!parsed.paired) {
        if (!pair_code || pair_code->kind != veld::btc_buy::JsonValue::Kind::String ||
            pair_code->text.size() != 9 || pair_code->text[4] != '-')
            return false;
        for (size_t i = 0; i < pair_code->text.size(); ++i) {
            if (i == 4)
                continue;
            if (std::string("23456789ABCDEFGHJKLMNPQRSTUVWXYZ").find(pair_code->text[i]) ==
                std::string::npos)
                return false;
        }
        parsed.pair_code = pair_code->text;
    } else if (!pair_code || pair_code->kind != veld::btc_buy::JsonValue::Kind::Null) {
        return false;
    }
    const auto* command_key = root.Get("command_key");
    if (command_key && command_key->kind == veld::btc_buy::JsonValue::Kind::Object) {
        if (command_key->object.size() != 3)
            return false;
        const auto* id = command_key->Get("id");
        const auto* x = command_key->Get("x");
        const auto* y = command_key->Get("y");
        if (!id || !x || !y || id->kind != veld::btc_buy::JsonValue::Kind::String ||
            x->kind != veld::btc_buy::JsonValue::Kind::String ||
            y->kind != veld::btc_buy::JsonValue::Kind::String ||
            !IsPortalCommandKey(x->text, y->text, id->text))
            return false;
        parsed.command_key_present = true;
        parsed.command_key_id = id->text;
        parsed.command_key_x = x->text;
        parsed.command_key_y = y->text;
    } else if (!command_key || command_key->kind != veld::btc_buy::JsonValue::Kind::Null) {
        return false;
    }
    const auto* command = root.Get("command");
    if (command && command->kind == veld::btc_buy::JsonValue::Kind::Object) {
        if (!parsed.paired || !parsed.command_key_present || command->object.size() != 9)
            return false;
        ...
        if (!veld::node_gui::ParseUint(command->Get("id"), parsed.command.id) ||
            parsed.command.id == 0 ||
            !veld::node_gui::ParseUint(command->Get("sequence"), parsed.command.sequence) ||
            parsed.command.sequence == 0 ||
            !veld::node_gui::ParseUint(command->Get("issued_at"), parsed.command.issued_at) ||
            !veld::node_gui::ParseUint(command->Get("expires_at"), parsed.command.expires_at) ||
            !action || action->kind != veld::btc_buy::JsonValue::Kind::String || !payload ||
            payload->kind != veld::btc_buy::JsonValue::Kind::Object || !nonce ||
            nonce->kind != veld::btc_buy::JsonValue::Kind::String || !key_id ||
            key_id->kind != veld::btc_buy::JsonValue::Kind::String || !signature ||
            signature->kind != veld::btc_buy::JsonValue::Kind::String)
            return false;
        std::vector<uint8_t> decoded;
        if (nonce->text.size() != 22 || !Base64UrlDecodeExact(nonce->text, 16, decoded) ||
            signature->text.size() != 86 || !Base64UrlDecodeExact(signature->text, 64, decoded) ||
            key_id->text != parsed.command_key_id)
            return false;
        ...
        if (parsed.command.action == "node.signin") {
            if (payload->object.size() != 5)
                return false;
            const auto read_field = [&](const char* name, std::string& value) {
                const auto* field = payload->Get(name);
                if (!field || field->kind != veld::btc_buy::JsonValue::Kind::String)
                    return false;
                value = field->text;
                return true;
            };
            auto& unlock = parsed.command.unlock;
            if (!read_field("ciphertext", unlock.ciphertext) ||
                !read_field("identity", unlock.identity) || !read_field("iv", unlock.iv) ||
                !read_field("key_id", unlock.key_id) ||
                !read_field("wrapped_key", unlock.wrapped_key) || !unlock.Valid())
                return false;
            ...
            if (payload->object.size() != 1)
                return false;
            if (!veld::node_gui::ParseBool(payload->Get("enabled"), parsed.command.enabled))
                return false;
        } else if (parsed.command.action == "mining.workers") {
            if (payload->object.size() != 1)
                return false;
            if (!veld::node_gui::ParseUint(payload->Get("workers"), parsed.command.workers) ||
            ...
            if (payload->object.size() != 1)
                return false;
            if (!mode || mode->kind != veld::btc_buy::JsonValue::Kind::String ||
                (mode->text != "full" && mode->text != "snapshot"))
                return false;
            parsed.command.mode = mode->text;
        } else if (parsed.command.action != "node.start" && parsed.command.action != "node.stop" &&
                   parsed.command.action != "pool.start" && parsed.command.action != "pool.stop" &&
    ...
    } else if (command && command->kind != veld::btc_buy::JsonValue::Kind::Null) {
        return false;
    }
    out = std::move(parsed);
    return true;
...
std::string CanonicalPortalCommandPayload(const MonitoringReply::Command& command) {
    if (command.action == "node.signin")
        return command.unlock.Canonical();
    if (command.action == "node.start" || command.action == "node.stop" ||
        command.action == "pool.start" || command.action == "pool.stop" ||
        command.action == "updates.check" || command.action == "updates.install")
        return "{}";
    if (command.action == "mining.enabled" || command.action == "privacy.tor" ||
        command.action == "network.reachable" || command.action == "display.reference" ||
        command.action == "updates.automatic")
        return std::string("{\"enabled\":") + (command.enabled ? "true}" : "false}");
    if (command.action == "mining.workers" && veld::mining::ValidWorkerCount(command.workers))
        return "{\"workers\":" + std::to_string(command.workers) + "}";
    if (command.action == "sync.mode" && (command.mode == "full" || command.mode == "snapshot"))
        return "{\"mode\":\"" + command.mode + "\"}";
    return {};
}

bool EvaluatePortalPairing(const MonitoringReply& reply, const PortalTrustState& current,
    ...
    if (!reply.paired || !reply.command_key_present || reply.device_id == 0 ||
        !IsPortalCommandKey(reply.command_key_x, reply.command_key_y, reply.command_key_id))
        return false;
    if (current.Valid() &&
        (current.device_id != reply.device_id || current.key_id != reply.command_key_id ||
         current.key_x != reply.command_key_x || current.key_y != reply.command_key_y)) {
...
PortalCommandTrustVerdict EvaluatePortalCommandTrust(const MonitoringReply::Command& command,
    ...
    if (!command.present || command.id == 0 || command.device_id == 0 || command.sequence == 0 ||
        command.issued_at == 0 || !Base64UrlDecodeExact(command.nonce, 16, nonce) ||
    ...
    if (envelope.empty())
        return PortalCommandTrustVerdict::Reject;
    if ((command.issued_at > now && command.issued_at - now > 120) || command.expires_at <= now ||
        command.expires_at <= command.issued_at || command.expires_at - command.issued_at < 30 ||
        command.expires_at - command.issued_at > 180)
        return PortalCommandTrustVerdict::Reject;
    if (!VerifyPortalCommandSignature(command.key_x, command.key_y, envelope, command.signature))
        return PortalCommandTrustVerdict::Reject;

    const bool same_pairing = current.Valid() && current.device_id == command.device_id;
    if (same_pairing && (current.key_id != command.key_id || current.key_x != command.key_x ||
...
std::vector<std::wstring> ReadLogTail(const std::filesystem::path& path, size_t max_lines = 28) {
    ...
    while (cursor <= body.size()) {
        ...
        std::string line =
            body.substr(cursor, end == std::string::npos ? std::string::npos : end - cursor);
        if (!line.empty() && line.back() == '\r')
            line.pop_back();
        std::string clean;
        clean.reserve(line.size());
        for (size_t i = 0; i < line.size(); ++i) {
            if (static_cast<unsigned char>(line[i]) == 0x1b && i + 1 < line.size() &&
                line[i + 1] == '[') {
                i += 2;
                while (i < line.size()) {
        ...
        if (!line.empty() && wide.empty())
            wide = L"[unreadable log line]";
        if (!wide.empty()) {
...
std::string ReadTextBounded(const std::filesystem::path& path, size_t max_bytes = 64U * 1024U) {
    std::ifstream input(path, std::ios::binary);
    if (!input)
        return {};
    input.seekg(0, std::ios::end);
...
std::string ReadLastUpdateFailure(const std::filesystem::path& root) {
    ...
    if (!parser.Parse(value, error) || value.kind != veld::btc_buy::JsonValue::Kind::Object)
        return {};
    const auto* status = value.Get("status");
    const auto* message = value.Get("message");
    if (!status || !message || status->kind != veld::btc_buy::JsonValue::Kind::String ||
        message->kind != veld::btc_buy::JsonValue::Kind::String ||
        (status->text != "failed" && status->text != "recovery-required"))
        return {};
...
std::wstring FormatAge(uint64_t unix_timestamp) {
    ...
    if (unix_timestamp > now + 5)
        return L"Clock ahead";
    return FormatUptime(now > unix_timestamp ? now - unix_timestamp : 0) + L" ago";
}
...
std::wstring FormatHashrate(double hashes_per_second) {
    std::wostringstream out;
    out << std::fixed << std::setprecision(hashes_per_second >= 1000.0 ? 1 : 2);
...
std::wstring MiningStateLabel(const std::string& state) {
    ...
    if (state == "waiting-for-anchor")
        return L"Waiting for anchor";
    if (state == "work-admission")
        return L"Checking peer work";
    if (state == "clock-drift")
        return L"Clock correction needed";
    if (state == "genesis-mismatch")
        return L"Genesis mismatch";
    if (state == "waiting-for-peer-tip")
        return L"Waiting for fresh peer tips";
    if (state == "below-peer-floor")
        return L"Waiting for peers";
    if (state == "propagation")
        return L"Waiting for block relay";
    if (state == "reorg-rebuild")
        return L"Rebuilding chain state";
    if (state == "error")
        return L"Mining error";
    return L"Stopped";
}

std::wstring MiningStateDetail(const std::string& state) {
    if (state == "hashing")
        return L"VeldHash workers are advancing live work.";
    if (state == "synchronizing")
        return L"Mining starts after the local chain is fully validated.";
    if (state == "waiting-for-anchor")
        return L"No configured network anchor is currently connected.";
    if (state == "clock-drift")
        return L"Local time differs too far from the anchor median.";
    if (state == "genesis-mismatch")
        return L"Connected peers do not match this build's genesis.";
    if (state == "waiting-for-peer-tip")
        return L"Fresh canonical peer-tip confirmation is required.";
    if (state == "below-peer-floor")
        return L"The configured peer floor is not currently satisfied.";
    if (state == "propagation")
        return L"Mining is paused until the current tip is acknowledged.";
    if (state == "reorg-rebuild")
        return L"Derived chain state is being rebuilt safely.";
    if (state == "error")
        return L"Review Logs for the node-reported mining error.";
    if (state == "work-admission")
        return L"The node is waiting for safe canonical work admission.";
...
bool CopyWideText(HWND owner, const std::wstring& value) {
    if (value.empty() || !OpenClipboard(owner))
        return false;
    EmptyClipboard();
...
bool ReadBoundedRegularFile(const std::filesystem::path& path, uint64_t max_size,
    ...
    BYTE extra = 0;
    DWORD extra_count = 0;
    const bool exact = ReadFile(file, &extra, 1, &extra_count, nullptr) && extra_count == 0;
    CloseHandle(file);
    if (!exact)
        bytes.clear();
    return exact;
...
bool IsSafePackageRelativePath(const std::string& path) {
    ...
    for (size_t i = 0; i <= path.size(); ++i) {
        if (i != path.size() && path[i] != '/') {
            ...
            if (value < 0x20 || value > 0x7e || path[i] == '\\' || path[i] == ':' ||
                path[i] == '*' || path[i] == '?' || path[i] == '"' || path[i] == '<' ||
                path[i] == '>' || path[i] == '|')
                return false;
            continue;
        }
        const std::string segment = path.substr(segment_start, i - segment_start);
...
bool VerifySignedPackage(const std::filesystem::path& module, std::wstring& error) {
    error = L"The signed Veld package could not be verified.";
    if (module.empty())
        return false;
    const bool launched_from_bin = LowerAscii(module.parent_path().filename().string()) == "bin";
    const std::filesystem::path root =
        launched_from_bin ? module.parent_path().parent_path() : module.parent_path();
    const std::filesystem::path manifest_path = root / L"SHA256SUMS.txt";
    const std::filesystem::path signature_path = root / L"SHA256SUMS.txt.sig";
    ...
    for (std::string line; std::getline(input, line);)
        lines.push_back(std::move(line));
    if (lines.size() < 3 || lines.size() > 64 || lines[0] != "# veld-release-manifest-v1" ||
        lines[1] != std::string("# release-version=") + veld::CLIENT_VERSION) {
    ...
    std::unordered_set<std::string> required{"bin/veld-node.exe", "bin/veld-wallet.exe",
                                             "bin/veld-pool-client.exe"};
    const std::string current_relative =
        launched_from_bin ? "bin/" + module.filename().string() : module.filename().string();
    required.insert(LowerAscii(current_relative));
    for (size_t i = 2; i < lines.size(); ++i) {
        const std::string& line = lines[i];
        if (line.size() < 67 || line[64] != ' ' || line[65] != '*' ||
            line.substr(0, 64).find_first_not_of("0123456789abcdef") != std::string::npos) {
        ...
        const std::filesystem::path file_path = root / std::filesystem::path(Utf8ToWide(relative));
        HANDLE verified = OpenVerifiedTrustedFile(file_path, line.substr(0, 64));
...
std::filesystem::path GuiStateDirectory() {
    ...
    if (required > 1 && required < 32768) {
        ...
        const DWORD written = GetEnvironmentVariableW(L"LOCALAPPDATA", raw.data(), required);
...
INT_PTR CALLBACK PassphraseDialogProc(HWND dialog, UINT message, WPARAM wp, LPARAM lp) {
    auto* prompt = reinterpret_cast<PassphrasePrompt*>(GetWindowLongPtrW(dialog, GWLP_USERDATA));
    if (message == WM_INITDIALOG) {
        prompt = reinterpret_cast<PassphrasePrompt*>(lp);
        SetWindowLongPtrW(dialog, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(prompt));
        SetWindowTextW(dialog, prompt->title.c_str());
        const BOOL dark = TRUE;
        DwmSetWindowAttribute(dialog, static_cast<DWMWINDOWATTRIBUTE>(20), &dark, sizeof(dark));
        ...
        prompt->brand_font = CreateFontW(
            -10, 0, 0, 0, FW_SEMIBOLD, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
            CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, DEFAULT_PITCH | FF_DONTCARE, L"Consolas");
        prompt->title_font = CreateFontW(
            -20, 0, 0, 0, FW_SEMIBOLD, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
            CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, DEFAULT_PITCH | FF_DONTCARE, L"Consolas");
        prompt->body_font = CreateFontW(
            -14, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
            CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, DEFAULT_PITCH | FF_DONTCARE, L"Segoe UI");
        prompt->label_font = CreateFontW(
            -11, 0, 0, 0, FW_SEMIBOLD, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
            CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, DEFAULT_PITCH | FF_DONTCARE, L"Consolas");
        prompt->input_font = CreateFontW(
        ...
        SetDlgItemTextW(dialog, IDC_VELD_PASSPHRASE_TITLE, prompt->title.c_str());
        SetDlgItemTextW(dialog, IDC_VELD_PASSPHRASE_PROMPT, prompt->prompt.c_str());
        ...
        SendDlgItemMessageW(dialog, IDC_VELD_PASSPHRASE, EM_SETLIMITTEXT, 1024, 0);
        ...
        SendDlgItemMessageW(dialog, IDC_VELD_PASSPHRASE, EM_SETMARGINS,
                            EC_LEFTMARGIN | EC_RIGHTMARGIN, MAKELPARAM(11, 54));
        if (!prompt->secret) {
        ...
        MapWindowPoints(HWND_DESKTOP, dialog, reinterpret_cast<POINT*>(&prompt->field_outline), 2);
        const int edit_width = prompt->field_outline.right - prompt->field_outline.left;
        const int edit_height = prompt->field_outline.bottom - prompt->field_outline.top;
        ...
        const int centered_height =
            std::min(edit_height - 4, static_cast<int>(input_metrics.tmHeight) + 8);
        SetWindowPos(edit, nullptr, prompt->field_outline.left,
                     prompt->field_outline.top + (edit_height - centered_height) / 2, edit_width,
                     std::max(1, centered_height), SWP_NOZORDER | SWP_NOACTIVATE);
        RECT edit_rect{};
        GetClientRect(edit, &edit_rect);
        SetWindowRgn(edit,
                     CreateRoundRectRgn(edit_rect.left, edit_rect.top, edit_rect.right + 1,
                                        edit_rect.bottom + 1, 12, 12),
                     TRUE);
        SendMessageW(dialog, DM_SETDEFID, IDOK, 0);
        RECT window{};
        GetWindowRect(dialog, &window);
        SetWindowRgn(dialog,
                     CreateRoundRectRgn(0, 0, window.right - window.left + 1,
                                        window.bottom - window.top + 1, 18, 18),
                     TRUE);
        ...
        if (owner && GetWindowRect(owner, &owner_rect)) {
            ...
            SetWindowPos(dialog, nullptr,
                         ...
                         owner_rect.top + (owner_rect.bottom - owner_rect.top - height) / 2, 0, 0,
    ...
    if (message == WM_CTLCOLORSTATIC && prompt && prompt->background) {
        ...
        SetTextColor(dc, id == IDC_VELD_PASSPHRASE_TITLE
                             ? C_TEXT
                             : (id == IDC_VELD_PASSPHRASE_BRAND
                                    ? C_MUTED
    ...
    if (message == WM_CTLCOLOREDIT && prompt && prompt->field_background) {
        ...
        return reinterpret_cast<INT_PTR>(prompt->missing_value ? prompt->error_background
                                                               : prompt->field_background);
    }
    if (message == WM_PAINT && prompt) {
        ...
        RoundRect(dc, client.left, client.top, client.right - 1, client.bottom - 1, 18, 18);

        RECT field = prompt->field_outline;
        HBRUSH field_brush =
            prompt->missing_value ? prompt->error_background : prompt->field_background;
    ...
    if (message == WM_DRAWITEM) {
        ...
        if (item && (item->CtlID == IDOK || item->CtlID == IDCANCEL ||
            ...
            const COLORREF fill_color = reveal    ? C_BG
                                        : primary ? (pressed ? RGB(66, 75, 69) : RGB(48, 56, 51))
            ...
            HPEN border =
                CreatePen(PS_SOLID, 1, reveal ? C_BG : (primary ? RGB(91, 104, 95) : C_BORDER));
            ...
            SetTextColor(item->hDC, primary ? C_TEXT : (reveal ? C_MUTED : C_SUBTEXT));
    ...
    if (message == WM_COMMAND && LOWORD(wp) == IDC_VELD_PASSPHRASE_REVEAL &&
        ...
        HWND edit = GetDlgItem(dialog, IDC_VELD_PASSPHRASE);
        SendMessageW(edit, EM_SETPASSWORDCHAR, prompt->revealed ? 0 : 0x25CF, 0);
        SetDlgItemTextW(dialog, IDC_VELD_PASSPHRASE_REVEAL, prompt->revealed ? L"Hide" : L"Show");
        InvalidateRect(edit, nullptr, TRUE);
        InvalidateRect(GetDlgItem(dialog, IDC_VELD_PASSPHRASE_REVEAL), nullptr, TRUE);
        SetFocus(edit);
        return TRUE;
    }
    if (message == WM_COMMAND && LOWORD(wp) == IDOK) {
        const int size = GetWindowTextLengthW(GetDlgItem(dialog, IDC_VELD_PASSPHRASE));
        if (!prompt || size <= 0 || size > 1024) {
            if (prompt) {
                prompt->missing_value = true;
                SetDlgItemTextW(dialog, IDC_VELD_PASSPHRASE_LABEL,
                                prompt->secret ? L"PASSPHRASE REQUIRED"
                                               : L"PAYOUT ADDRESS REQUIRED");
        ...
        GetDlgItemTextW(dialog, IDC_VELD_PASSPHRASE, prompt->value.data(), size + 1);
    ...
    if (message == WM_COMMAND && LOWORD(wp) == IDC_VELD_PASSPHRASE && prompt) {
        ...
        if (HIWORD(wp) == EN_CHANGE && prompt->missing_value) {
            ...
            SetDlgItemTextW(dialog, IDC_VELD_PASSPHRASE_LABEL,
                            prompt->secret ? L"PASSPHRASE" : L"PAYOUT ADDRESS");
            RedrawWindow(dialog, nullptr, nullptr,
                         RDW_INVALIDATE | RDW_UPDATENOW | RDW_ALLCHILDREN);
            return TRUE;
        }
    }
    if ((message == WM_COMMAND && LOWORD(wp) == IDCANCEL) || message == WM_CLOSE) {
        EndDialog(dialog, IDCANCEL);
        return TRUE;
    }
    if (message == WM_DESTROY && prompt) {
        if (prompt->background)
            DeleteObject(prompt->background);
        if (prompt->field_background)
            DeleteObject(prompt->field_background);
        if (prompt->error_background)
            DeleteObject(prompt->error_background);
        for (HFONT font : {prompt->brand_font, prompt->title_font, prompt->body_font,
                           prompt->label_font, prompt->input_font}) {
            if (font)
                DeleteObject(font);
        }
        prompt->background = prompt->field_background = prompt->error_background = nullptr;
    ...
    if (Process32FirstW(snap, &entry)) {
        do {
            if (_wcsicmp(entry.szExeFile, L"veld-node.exe") == 0) {
                ...
                const bool matches =
                    process && QueryFullProcessImageNameW(process, 0, image, &size) &&
...
bool RequestGuiNodeShutdown(DWORD pid) {
    if (pid == 0)
        return true;
    const std::wstring name = L"Local\\VeldNodeShutdown-" + std::to_wstring(pid);
...
enum class Page { Overview, Blockchain, Mining, Workers, Explorer, Network, Logs, Settings, Pool };

enum class UpdateOperation { None, Check, Install };
...
class NodeGuiApp {
  public:
    ...
    explicit NodeGuiApp(HINSTANCE instance) : instance_(instance) {
        ...
        const auto packaged_node = module.parent_path() / L"bin" / L"veld-node.exe";
        node_path_ = std::filesystem::is_regular_file(packaged_node)
                         ? packaged_node
                         : module.parent_path() / L"veld-node.exe";
        const auto packaged_wallet = module.parent_path() / L"bin" / L"veld-wallet.exe";
        wallet_path_ = std::filesystem::is_regular_file(packaged_wallet)
                           ? packaged_wallet
                           : module.parent_path() / L"veld-wallet.exe";
        ...
        LoadRemoteTrust();
        LoadSettings();
        if (!windows_startup_invocation_)
            LoadUpdateResume();
        const auto prior_update_failure = ReadLastUpdateFailure(InstallRoot());
        if (!prior_update_failure.empty()) {
            update_status_ =
                L"Previous update failed · " + Utf8ToWide(prior_update_failure.substr(0, 160));
    ...
    ~NodeGuiApp() {
        ...
        if (!wallet_signer_token_.empty()) {
            SecureZeroMemory(wallet_signer_token_.data(), wallet_signer_token_.size());
    ...
    int Run(int show) {
        ...
        hwnd_ = CreateWindowExW(WS_EX_COMPOSITED, wc.lpszClassName, L"Veld Node",
                                WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN, window_x_, window_y_,
                                window_width_ > 0 ? window_width_ : desired.right - desired.left,
                                window_height_ > 0 ? window_height_ : desired.bottom - desired.top,
                                nullptr, nullptr, instance_, this);
        if (!hwnd_)
            return 2;
        if (!page_scrollbar_.Create(hwnd_))
            return 2;
        const auto module = ModulePath();
        auto pool_binary = module.parent_path() / L"bin" / L"veld-pool-client.exe";
        if (!std::filesystem::is_regular_file(pool_binary))
            pool_binary = module.parent_path() / L"veld-pool-client.exe";
        pool_panel_ = std::make_unique<veld::node_gui::PoolPanel>(
            hwnd_, state_dir_ / L"pool-client", pool_binary, mining_thread_count_, [this] {
                ...
                if (!VerifySignedPackage(ModulePath(), error)) {
                    throw std::runtime_error(
                        "Signed pool package verification failed. Reinstall the verified client package.");
                }
#endif
                return true;
            });
        if (windows_startup_invocation_)
            pool_panel_->SuppressAutomaticResume();
  ...
  private:
    ...
    std::chrono::steady_clock::time_point next_auto_update_{std::chrono::steady_clock::now() +
    ...
    int MaxPageScroll(const RECT& client) const {
        return std::max(0, PageContentHeight(client) - static_cast<int>(client.bottom));
    }
    ...
    POINT ContentPoint(POINT point) {
        ...
        if (point.x >= S(252))
            point.y += CurrentPageScroll(client);
        return point;
    }
    ...
    void SetPageScroll(int requested) {
        ...
        if (next == page_scroll_offsets_[PageIndex()])
            return;
    ...
    size_t NetworkHoverTarget(POINT point) const {
        for (size_t i = 0; i < network_peer_rects_.size(); ++i) {
            if (PtInRect(&network_peer_rects_[i], point))
                return i + 1;
        }
        for (size_t i = 0; i < network_topology_rects_.size(); ++i) {
            if (PtInRect(&network_topology_rects_[i], point))
                return i + 1;
        }
        return 0;
    }
    ...
    RECT NetworkTooltipRect(const RECT& anchor, int width, int height) const {
        ...
        left = std::clamp(left, bounds_left, std::max(bounds_left, bounds_right - width));
        int top = anchor.top + ((anchor.bottom - anchor.top) - height) / 2;
        top = std::clamp(top, bounds_top, std::max(bounds_top, bounds_bottom - height));
        return RECT{left, top, left + width, top + height};
    }

    bool IsInteractivePoint(POINT point) {
        const RECT* sidebar_controls[] = {
            &overview_nav_, &blockchain_nav_, &mining_nav_,   &workers_nav_, &explorer_nav_,
            &network_nav_,  &logs_nav_,       &settings_nav_, &pool_nav_,    &follow_x_link_};
        for (const RECT* control : sidebar_controls) {
            if (control && PtInRect(control, point))
                return true;
        }
        point = ContentPoint(point);
        if (PtInRect(&action_button_, point))
            return true;
        ...
        if (page_ == Page::Overview) {
            ...
            if (inside_any({&full_ibd_card_, &snapshot_card_}))
                return true;
        } else if (page_ == Page::Mining) {
            if (PtInRect(&address_settings_button_, point))
                return true;
            ...
            if (inside_any({&address_mode_toggle_,
                            &address_edit_button_,
                            &startup_toggle_,
                            &startup_unlock_toggle_,
                            &tray_toggle_,
                            &startup_mode_buttons_[0],
                            &startup_mode_buttons_[1],
                            &startup_mode_buttons_[2],
                            &startup_mode_buttons_[3],
                            &reachable_toggle_,
                            &tor_toggle_,
                            &reference_toggle_,
                            &remote_monitor_toggle_,
    ...
    void ApplyDarkFrame() {
        ...
        DwmSetWindowAttribute(hwnd_, static_cast<DWMWINDOWATTRIBUTE>(20), &enabled,
                              sizeof(enabled));
        DwmSetWindowAttribute(hwnd_, static_cast<DWMWINDOWATTRIBUTE>(35), &caption,
                              sizeof(caption));
        DwmSetWindowAttribute(hwnd_, static_cast<DWMWINDOWATTRIBUTE>(34), &border, sizeof(border));
        DwmSetWindowAttribute(hwnd_, static_cast<DWMWINDOWATTRIBUTE>(36), &text, sizeof(text));
    }

    void NormalizeSavedWindowBounds() {
        if (window_x_ == CW_USEDEFAULT || window_y_ == CW_USEDEFAULT)
            return;
        RECT desired{window_x_, window_y_, window_x_ + window_width_, window_y_ + window_height_};
        ...
        window_x_ = std::clamp<int>(window_x_, info.rcWork.left, info.rcWork.right - window_width_);
        window_y_ =
            std::clamp<int>(window_y_, info.rcWork.top, info.rcWork.bottom - window_height_);
    }
    ...
    void SaveWindowPlacement() {
        ...
        if (normal.right - normal.left >= 1050 && normal.bottom - normal.top >= 760) {
    ...
    void ShowTrayMenu() {
        ...
        std::wstring status =
            stopping ? L"Status: stopping safely"
        ...
        const std::wstring toggle_label =
            stopping ? L"Stopping..."
                     : (live.process_running ? L"Stop node"
                                             : (mining_enabled_ ? L"Start mining" : L"Start node"));
        AppendMenuW(menu, stopping ? MF_GRAYED : MF_STRING, ID_TRAY_TOGGLE_NODE,
                    toggle_label.c_str());
    ...
    static LRESULT CALLBACK WndProcThunk(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
        NodeGuiApp* self = reinterpret_cast<NodeGuiApp*>(GetWindowLongPtrW(hwnd, GWLP_USERDATA));
        if (msg == WM_NCCREATE) {
    ...
    LRESULT WndProc(UINT msg, WPARAM wp, LPARAM lp) {
        if (taskbar_created_message_ && msg == taskbar_created_message_) {
            ...
            if (!tray_added_ && !IsWindowVisible(hwnd_))
        ...
        case WM_VSCROLL: {
            ...
            case SB_LINEDOWN:
                ...
                break;
            case SB_PAGEUP:
                requested -= std::max(1L, client.bottom);
                break;
            case SB_PAGEDOWN:
                requested += std::max(1L, client.bottom);
                break;
            ...
            case SB_TOP:
                ...
                break;
            case SB_BOTTOM:
                requested = MaxPageScroll(client);
                break;
            default:
                return 0;
            }
        ...
        case WM_MOUSEMOVE: {
            ...
            if (next_hover.x != hover_point_.x || next_hover.y != hover_point_.y) {
                const POINT prior_hover = hover_point_;
                const size_t prior_network_target = NetworkHoverTarget(ContentPoint(prior_hover));
                const size_t next_network_target = NetworkHoverTarget(ContentPoint(next_hover));
                hover_point_ = next_hover;
                if (page_ == Page::Network &&
                    (PtInRect(&network_graph_rect_, ContentPoint(prior_hover)) ||
                     PtInRect(&network_graph_rect_, ContentPoint(next_hover)))) {
        ...
        case WM_SETCURSOR:
            if (LOWORD(lp) == HTCLIENT) {
                ...
                SetCursor(LoadCursorW(nullptr, IsInteractivePoint(cursor) ? IDC_HAND : IDC_ARROW));
                return TRUE;
            }
        ...
        case WM_EXITSIZEMOVE:
            ...
            return 0;
        case WM_COMMAND:
            if (pool_panel_ && pool_panel_->Command(wp))
                return 0;
            if (LOWORD(wp) == ID_TRAY_RESTORE)
                RestoreFromTray();
            else if (LOWORD(wp) == ID_TRAY_TOGGLE_NODE)
                ToggleNode();
        ...
        case WM_DRAWITEM:
            if (pool_panel_ && lp &&
                pool_panel_->Draw(*reinterpret_cast<const DRAWITEMSTRUCT*>(lp)))
                return TRUE;
            return DefWindowProcW(hwnd_, msg, wp, lp);
        ...
        case WM_CTLCOLOREDIT:
            if (pool_panel_) {
                const auto brush =
                    pool_panel_->Color(reinterpret_cast<HDC>(wp), reinterpret_cast<HWND>(lp));
                if (brush)
        ...
        case WM_TRAY_ICON:
            ...
            return 0;
        case WM_KEYDOWN:
            if (wp == VK_UP || wp == VK_DOWN || wp == VK_PRIOR || wp == VK_NEXT || wp == VK_HOME ||
                wp == VK_END) {
                RECT client{};
                GetClientRect(hwnd_, &client);
                int requested = CurrentPageScroll(client);
    ...
    void LoadSettings() {
        ...
        if (!input) {
            ...
            settings_load_failed_ =
                std::filesystem::exists(state_dir_ / L"node-gui.conf", error) || bool(error);
            return;
        }
        std::string line;
        while (std::getline(input, line)) {
            ...
            if (line == "startup=1")
                startup_enabled_ = true;
            else if (line == "startup=0")
                startup_enabled_ = false;
            else if (line == "startup_unlock=1")
                startup_unlock_enabled_ = true;
            else if (line == "startup_unlock=0")
                startup_unlock_enabled_ = false;
            else if (line == "minimize_to_tray=1")
                minimize_to_tray_ = true;
            else if (line == "minimize_to_tray=0")
                minimize_to_tray_ = false;
            else if (line.rfind("startup_mode=", 0) == 0)
                startup_mode_ = veld::node_gui::ParseStartupMode(line.substr(13));
            else if (line == "auto_update=1")
                auto_update_enabled_ = true;
            else if (line == "auto_update=0")
                auto_update_enabled_ = false;
            else if (line == "reachable=0")
                reachable_choice_ = false;
            else if (line == "reachable=1")
                reachable_choice_ = true;
            else if (line == "tor=0")
                tor_choice_ = false;
            else if (line == "tor=1")
                tor_choice_ = true;
            else if (line == "address_only=1")
                address_only_ = true;
            else if (line == "address_only=0")
                address_only_ = false;
            ...
            } else if (line == "mining=0")
                mining_enabled_ = false;
            else if (line == "mining=1")
                mining_enabled_ = true;
            else if (line == "mining_preset=eco")
                mining_preset_ = 0;
            else if (line == "mining_preset=balanced")
                mining_preset_ = 1;
            else if (line == "mining_preset=maximum")
                mining_preset_ = 2;
            else if (line == "mining_preset=custom")
                mining_preset_ = 3;
                ...
                auto parse_int = [&](const char* key, int minimum, int maximum, int& target) {
                ...
                if (parse_int("mining_threads", 1, 256, threads))
                    ...
                    int topology_role_index = static_cast<int>(topology_role_index_);
                    if (parse_int("topology_role_index", 0, 999, topology_role_index)) {
                        topology_role_index_ = static_cast<uint32_t>(topology_role_index);
                    } else if (parse_int("chain_range_minutes", 5, 60, chain_range_minutes_)) {
                    } else if (parse_int("rate_range_minutes", 5, 60, rate_range_minutes_)) {
                    } else if (parse_int("window_x", -32000, 32000, window_x_)) {
                    } else if (parse_int("window_y", -32000, 32000, window_y_)) {
                    } else if (parse_int("window_width", 1050, 7680, window_width_)) {
                    } else if (parse_int("window_height", 640, 4320, window_height_)) {
        ...
        if (chain_range_minutes_ != 5 && chain_range_minutes_ != 15 && chain_range_minutes_ != 60)
            chain_range_minutes_ = 15;
        if (rate_range_minutes_ != 5 && rate_range_minutes_ != 15 && rate_range_minutes_ != 60)
            rate_range_minutes_ = 15;
    }
    ...
    bool SaveSettings(bool show_error = true) {
        ...
        output << "startup=" << (startup_enabled_ ? 1 : 0) << "\n"
               ...
               << "reference_display=" << (reference_display_enabled_.load() ? 1 : 0) << "\n"
               << "remote_monitoring=" << (remote_monitoring_enabled_.load() ? 1 : 0) << "\n"
               << "mining_preset="
               << (mining_preset_ == 0
                       ? "eco"
    ...
    void DestroyFonts() {
        for (HFONT* f : {&font_brand_, &font_title_, &font_heading_, &font_body_, &font_small_,
                         &font_number_, &font_log_}) {
            if (*f)
                DeleteObject(*f);
            *f = nullptr;
        }
    ...
    void DrawTextAt(HDC dc, const std::wstring& text, RECT rect, HFONT font, COLORREF color,
    ...
    HFONT FitSingleLineFont(HDC dc, const std::wstring& text, RECT rect,
        ...
        const int available = std::max<int>(0, static_cast<int>(rect.right - rect.left));
        ...
        for (HFONT candidate : candidates) {
            ...
            SIZE measured{};
            if (GetTextExtentPoint32W(dc, text.c_str(), static_cast<int>(text.size()), &measured) &&
    ...
    void FillRound(HDC dc, RECT r, COLORREF fill, COLORREF border, int radius = 6) {
    ...
    void DrawLogo(HDC dc, int cx, int cy, int size) {
        ...
        POINT outer[5] = {{cx, cy - half + S(7)},
                          {cx + half - S(7), cy},
                          {cx, cy + half - S(7)},
                          {cx - half + S(7), cy},
                          {cx, cy - half + S(7)}};
        POINT inner[5] = {{cx, cy - half / 2},
                          {cx + half / 2, cy},
                          {cx, cy + half / 2},
                          {cx - half / 2, cy},
                          {cx, cy - half / 2}};
    ...
    void DrawButton(HDC dc, RECT r, const std::wstring& label, bool enabled) {
        ...
        FillRound(dc, r, enabled ? (hovered ? C_BUTTON_HOVER : C_BUTTON) : RGB(27, 30, 28),
                  enabled ? (hovered ? RGB(111, 121, 114) : C_BUTTON_BORDER) : C_BORDER_SOFT, 5);
    ...
    void Paint() {
        ...
        if (pool_panel_)
            pool_panel_->Show(page_ == Page::Pool, S(292), S(145) - page_scroll,
                              std::max(S(340), static_cast<int>(client.right) - S(332)), dpi_,
                              font_body_);
        PAINTSTRUCT ps{};
        ...
        reference_toggle_ = {};
        mining_mode_toggle_ = {};
        address_mode_toggle_ = {};
        address_edit_button_ = {};
        address_settings_button_ = {};
        open_data_button_ = {};
        import_key_button_ = {};
        create_key_button_ = {};
        open_wallet_button_ = {};
        ...
        auto_update_toggle_ = {};
        view_changelog_button_ = {};
        for (RECT& button : chain_range_buttons_)
            button = {};
        for (RECT& button : rate_range_buttons_)
            button = {};
        chain_plot_rect_ = {};
        rate_plot_rect_ = {};
        network_graph_rect_ = {};
        ...
        switch (page_) {
        case Page::Overview:
            DrawOverview(dc, page_client, live);
            break;
        case Page::Blockchain:
            DrawBlockchain(dc, page_client, live);
            break;
        case Page::Mining:
            DrawMining(dc, page_client, live);
            break;
        case Page::Workers:
            DrawWorkers(dc, page_client, live);
            break;
        case Page::Explorer:
            DrawExplorer(dc, page_client, live);
            break;
        case Page::Network:
            DrawNetwork(dc, page_client, live);
            break;
        case Page::Logs:
            DrawLogs(dc, page_client, live);
            break;
        case Page::Settings:
            DrawSettings(dc, page_client, live);
            break;
        case Page::Pool: {
            ...
            DrawTextAt(
                dc, L"Mine with a pool using your payout address. Keep control of your own wallet.",
                note, font_body_, C_SUBTEXT);
            action_button_ = {};
            break;
        }
        }
    ...
    void DrawSidebar(HDC dc, const RECT& client, const LiveState& live) {
        ...
        DrawNav(dc, blockchain_nav_, L"Blockchain", page_ == Page::Blockchain);
        ...
        RECT version{S(28), client.bottom - S(110), width - S(22), client.bottom - S(82)};
        DrawTextAt(dc, std::wstring(L"Version ") + Utf8ToWide(veld::CLIENT_VERSION), version,
                   font_small_, C_MUTED);
        follow_x_link_ = {S(20), client.bottom - S(64), width - S(20), client.bottom - S(30)};
        const bool follow_hovered = PtInRect(&follow_x_link_, hover_point_);
        DrawTextAt(dc, L"Follow on X @VeldNetwork", follow_x_link_, font_body_,
                   follow_hovered ? C_TEXT : C_SUBTEXT,
                   DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
    }

    void DrawNav(HDC dc, const RECT& r, const std::wstring& label, bool active,
                 bool enabled = true) {
        if (active) {
        ...
        DrawTextAt(dc, label, text, font_body_, active ? C_TEXT : (enabled ? C_SUBTEXT : C_MUTED));
    }

    void DrawPageHeader(HDC dc, const RECT& client, const std::wstring& title,
                        const std::wstring& subtitle, const LiveState& live) {
        ...
        action_button_ = {client.right - button_w - S(34), S(35), client.right - S(34), S(79)};
    ...
    void DrawOverview(HDC dc, const RECT& client, const LiveState& live) {
        ...
        RECT running_pill{sync.right - S(112), sync.top + S(14), sync.right - S(18),
                          sync.top + S(42)};
        const bool stopping = stopping_node_.load() && live.process_running;
        FillRound(dc, running_pill, live.local_online && !stopping ? RGB(29, 39, 31) : C_PANEL_ALT,
        ...
        DrawTextAt(dc,
                   ...
                   running_pill, font_small_, live.local_online && !stopping ? C_TEXT : C_SUBTEXT,
                   DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);

        const uint64_t target = live.reference_online ? live.reference.height : live.local.height;
        const auto progress = veld::node_gui::ComputeSyncProgress(live.local.height, target);
        ...
        RECT state_r{sync.left + S(102), sync.top + S(18), running_pill.left - S(10),
                     sync.top + S(58)};
        DrawTextAt(dc, state_text, state_r, font_heading_, synced ? C_GREEN : C_TEXT);
        RECT height_r{sync.left + S(102), sync.top + S(56), sync.right - S(20), sync.top + S(91)};
        DrawTextAt(dc,
                   stopping ? L"Closing local services"
                            : (live.local_online ? L"Block " + FormatUnsigned(live.local.height)
        ...
        DrawTextAt(dc,
                   stopping ? L"The node is stopping safely"
                            : (attention.empty()
                                   ? (synced ? L"Best chain tip verified"
                                             : L"Consensus validation remains inside veld-node.exe")
                                   : L"Needs attention · " + attention),
                   verify_r, font_small_, stopping || attention.empty() ? C_SUBTEXT : C_WARN,
                   DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX | DT_END_ELLIPSIS);

        RECT mode_label{mode.left + S(24), mode.top + S(14), mode.right - S(20), mode.top + S(42)};
        DrawTextAt(dc, L"Node role", mode_label, font_small_, C_SUBTEXT);
        RECT mode_title{mode.left + S(24), mode.top + S(40), mode.right - S(20), mode.top + S(75)};
        DrawTextAt(dc, mining_enabled_ ? L"Mining node" : L"Full node", mode_title, font_heading_,
                   C_TEXT);
        RECT mode_sub{mode.left + S(24), mode.top + S(78), mode.right - S(20), mode.bottom - S(14)};
        DrawTextAt(
            dc,
            mining_enabled_
            ...
            mode_sub, font_small_, C_SUBTEXT, DT_LEFT | DT_WORDBREAK | DT_NOPREFIX);

        const int cards_top = sync.bottom + S(18);
        const int card_w = (total_w - 3 * gap) / 4;
        DrawMetricCard(dc, {left, cards_top, left + card_w, cards_top + S(124)}, L"Connections",
                       live.local_online ? FormatUnsigned(live.local.peers) : L"—",
                       L"direct P2P sessions");
        DrawMetricCard(
            dc, {left + card_w + gap, cards_top, left + 2 * card_w + gap, cards_top + S(124)},
            L"Uptime", live.process_running ? FormatUptime(live.uptime_seconds) : L"—",
            L"node process");
        DrawMetricCard(
            dc,
            {left + 2 * (card_w + gap), cards_top, left + 3 * card_w + 2 * gap, cards_top + S(124)},
            L"Version", Utf8ToWide(veld::CLIENT_VERSION), L"client build");
        DrawMetricCard(dc, {left + 3 * (card_w + gap), cards_top, right, cards_top + S(124)},
        ...
        RECT graph_title{graph.left + S(22), graph.top + S(13), graph.right, graph.top + S(48)};
        DrawTextAt(dc, L"Chain activity", graph_title, font_body_, C_TEXT);
        DrawRangeSelector(dc, graph.right - S(22), graph.top + S(13), chain_range_minutes_,
                          chain_range_buttons_);
        DrawHistory(
            dc, {graph.left + S(24), graph.top + S(55), graph.right - S(24), graph.bottom - S(24)});

        RECT strip{left, client.bottom - S(94), right, client.bottom - S(26)};
        FillRound(dc, strip, C_PANEL, C_BORDER);
        DrawStripItem(dc, {strip.left, strip.top, strip.left + total_w / 3, strip.bottom}, L"RPC",
                      L"Local only", live.process_running ? C_GREEN : C_MUTED);
        DrawStripItem(
            dc, {strip.left + total_w / 3, strip.top, strip.left + 2 * total_w / 3, strip.bottom},
            L"P2P", live.local_online && live.local.peers > 0 ? L"Connected" : L"Waiting",
        ...
        const bool snapshot_supported =
            live.mining_status_online && live.mining.snapshot_bootstrap_compiled;
        const std::wstring snapshot_status =
            !live.process_running
                ...
                : (!live.mining_status_online
                       ...
                       : (!snapshot_supported
                              ...
                              : (live.snapshot_eligible
                                     ? L"Eligible"
                                     : (live.mining.full_ibd ? L"IBD required" : L"Validating"))));
        const bool snapshot_good = snapshot_selected || live.snapshot_eligible;
        DrawStripItem(dc, {strip.left + 2 * total_w / 3, strip.top, strip.right, strip.bottom},
                      L"Snapshot", snapshot_status, snapshot_good ? C_GREEN : C_MUTED);
    }
    ...
    void DrawMetricCard(HDC dc, RECT r, const std::wstring& label, const std::wstring& value,
                        const std::wstring& sub, COLORREF value_color = C_TEXT) {
        ...
        DrawTextAt(dc, value, value_r, value_font, value == L"—" ? C_MUTED : value_color);
        RECT sub_r{r.left + S(18), r.top + S(86), r.right - S(12), r.bottom - S(8)};
        DrawTextAt(dc, sub, sub_r, font_small_, C_MUTED, DT_LEFT | DT_WORDBREAK | DT_NOPREFIX);
    }

    void DrawRangeSelector(HDC dc, int right, int top, int selected, RECT (&buttons)[3]) {
        ...
        for (int i = 2; i >= 0; --i) {
            buttons[i] = {right - (3 - i) * width - (2 - i) * gap, top,
                          right - (2 - i) * width - (2 - i) * gap, top + S(28)};
            const bool hovered = PtInRect(&buttons[i], hover_point_);
            FillRound(dc, buttons[i],
                      selected == VALUES[i] ? C_PANEL_ALT : (hovered ? RGB(24, 28, 25) : C_BG),
    ...
    void DrawHistory(HDC dc, RECT r) {
        ...
        const auto cutoff =
            std::chrono::steady_clock::now() - std::chrono::minutes(chain_range_minutes_);
        ...
        if (max_h == min_h)
            max_h = min_h + 1;
        RECT plot{r.left + S(76), r.top + S(34), r.right - S(8), r.bottom - S(42)};
        ...
        for (int i = 0; i <= 3; ++i) {
            ...
            const uint64_t value = max_h - (max_h - min_h) * static_cast<uint64_t>(i) / 3;
        ...
        const std::wstring span_label =
            L"-" + FormatUnsigned(static_cast<uint64_t>(chain_range_minutes_)) + L" min";
        RECT x_left{plot.left, plot.bottom + S(4), plot.left + S(90), plot.bottom + S(28)};
        RECT x_right{plot.right - S(70), plot.bottom + S(4), plot.right, plot.bottom + S(28)};
        DrawTextAt(dc, span_label, x_left, font_small_, C_SUBTEXT);
        DrawTextAt(dc, L"now", x_right, font_small_, C_SUBTEXT,
                   DT_RIGHT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
        RECT x_def{plot.left + S(104), plot.bottom + S(4), plot.right - S(86), plot.bottom + S(28)};
        DrawTextAt(dc, L"Observation time", x_def, font_small_, C_MUTED,
                   DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
        if (history.size() < 2) {
            DrawTextAt(dc, L"Activity appears as new blocks arrive.", plot, font_small_, C_MUTED,
                       DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
            return;
        }
        HPEN line = CreatePen(PS_SOLID, S(2), RGB(211, 216, 212));
        SelectObject(dc, line);
        const int64_t span_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
        ...
        for (size_t i = 0; i < history.size(); ++i) {
            const int64_t elapsed =
                std::chrono::duration_cast<std::chrono::milliseconds>(history[i].at - cutoff)
                    .count();
            const int x =
                plot.left + static_cast<int>((plot.right - plot.left) *
                                             std::clamp<int64_t>(elapsed, 0, span_ms) / span_ms);
            const double ratio =
                static_cast<double>(history[i].height - min_h) / static_cast<double>(max_h - min_h);
            const int y = plot.bottom - static_cast<int>((plot.bottom - plot.top) * ratio);
            if (i == 0)
        ...
        if (PtInRect(&plot, hover_point_) && nearest_distance <= S(32)) {
            ...
            DrawTextAt(dc, L"Block " + FormatUnsigned(history[nearest_index].height), tip,
                       font_small_, C_TEXT, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
        }
    }
    ...
    void DrawRateHistory(HDC dc, RECT r) {
        ...
        const auto cutoff =
            std::chrono::steady_clock::now() - std::chrono::minutes(rate_range_minutes_);
        while (history.size() > 1 && history.front().at < cutoff)
            history.pop_front();
        double max_rate = 1.0;
        ...
        const double average_rate =
            history.empty() ? 0.0 : total_rate / static_cast<double>(history.size());
        RECT plot{r.left + S(86), r.top + S(34), r.right - S(8), r.bottom - S(42)};
        ...
        DeleteObject(grid);
        const std::wstring span_label =
            L"-" + FormatUnsigned(static_cast<uint64_t>(rate_range_minutes_)) + L" min";
        RECT x_left{plot.left, plot.bottom + S(4), plot.left + S(90), plot.bottom + S(28)};
        RECT x_right{plot.right - S(70), plot.bottom + S(4), plot.right, plot.bottom + S(28)};
        DrawTextAt(dc, span_label, x_left, font_small_, C_SUBTEXT);
        DrawTextAt(dc, L"now", x_right, font_small_, C_SUBTEXT,
        ...
        DrawTextAt(dc, L"Observation time", x_def, font_small_, C_MUTED,
                   DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
        if (history.size() < 2) {
            DrawTextAt(dc, L"Hashrate history appears while mining is active.", plot, font_small_,
                       C_MUTED, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
            return;
        }
        HPEN line = CreatePen(PS_SOLID, S(2), RGB(211, 216, 212));
        SelectObject(dc, line);
        ...
        for (size_t i = 0; i < history.size(); ++i) {
            ...
            const int y = plot.bottom -
                          static_cast<int>((plot.bottom - plot.top) * history[i].rate / max_rate);
            if (i == 0)
                MoveToEx(dc, x, y, nullptr);
            else
                LineTo(dc, x, y);
        ...
        DrawTextAt(
            dc,
            L"Average " + FormatHashrate(average_rate) + L"   Peak " + FormatHashrate(peak_rate),
            summary, font_small_, C_SUBTEXT, DT_RIGHT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
        if (PtInRect(&plot, hover_point_) && nearest_distance <= S(32)) {
            HPEN guide = CreatePen(PS_DOT, S(1), C_MUTED);
            HGDIOBJ old = SelectObject(dc, guide);
            MoveToEx(dc, nearest_x, plot.top, nullptr);
            ...
            RECT tip{std::min<int>(nearest_x + S(10), plot.right - S(180)),
                     ...
                     std::max<int>(plot.top, nearest_y - S(42)) + S(34)};
            FillRound(dc, tip, C_PANEL_ALT, C_BORDER, 5);
            DrawTextAt(dc, FormatHashrate(history[nearest_index].rate), tip, font_small_, C_TEXT,
                       DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
        }
    }

    void DrawStripItem(HDC dc, RECT r, const std::wstring& label, const std::wstring& value,
    ...
    std::wstring NeedsAttention(const LiveState& live) {
        if (stopping_node_.load())
            return {};
        if (live.process_running && !live.local_online && live.uptime_seconds > 30)
            return L"local status has not opened";
    ...
    double RecentBlocksPerMinute() {
        ...
        while (first + 1 != history_.end() && first->at < now - std::chrono::minutes(2))
            ++first;
        const auto& last = history_.back();
        const double seconds = std::chrono::duration<double>(last.at - first->at).count();
        if (seconds <= 0.0 || last.height <= first->height)
            return 0.0;
    ...
    void DrawBlockchain(HDC dc, const RECT& client, const LiveState& live) {
        DrawPageHeader(dc, client, L"Blockchain", L"Verify and follow the Veld main chain.", live);
        const int left = S(292);
        const int right = client.right - S(34);
        const int top = S(132);
        const uint64_t reference =
            live.reference_online ? live.reference.height : live.local.height;
        const auto progress = veld::node_gui::ComputeSyncProgress(live.local.height, reference);
        const std::wstring state_text =
            !live.process_running
                ? L"Ready to start"
                : (!live.local_online
                       ? L"Opening chain state"
        ...
        DrawTextAt(dc, progress.target_known ? FormatPercent(progress.percent) : L"—", percent,
                   font_heading_, progress.complete ? C_GREEN : C_TEXT,
                   DT_RIGHT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
        RECT count{left, top + S(44), right, top + S(73)};
        std::wstring count_text =
            live.local_online ? FormatUnsigned(live.local.height) + L" of " +
        ...
        if (live.local_online) {
            ...
            if (blocks_per_minute > 0.0 && !progress.complete) {
                ...
                speed << std::fixed << std::setprecision(1) << blocks_per_minute
                      << L" blocks/min · ETA "
                      << FormatEta(
                             static_cast<uint64_t>(progress.remaining / blocks_per_minute * 60.0));
        ...
        DrawSegmentedProgress(dc, {left, top + S(82), right, top + S(98)}, progress.percent,
                              progress.complete);
        ...
        const int total_w = right - left;
        const int card_w = (total_w - 3 * gap) / 4;
        DrawMetricCard(dc, {left, cards_top, left + card_w, cards_top + S(112)}, L"Local height",
                       live.local_online ? FormatUnsigned(live.local.height) : L"—", L"validated");
        DrawMetricCard(
            dc, {left + card_w + gap, cards_top, left + 2 * card_w + gap, cards_top + S(112)},
            L"Network reference",
            live.reference_online ? FormatUnsigned(live.reference.height) : L"—", L"display only");
        DrawMetricCard(
            dc,
            {left + 2 * (card_w + gap), cards_top, left + 3 * card_w + 2 * gap, cards_top + S(112)},
            L"Remaining", progress.target_known ? FormatUnsigned(progress.remaining) : L"—",
            L"blocks");
        DrawMetricCard(dc, {left + 3 * (card_w + gap), cards_top, right, cards_top + S(112)},
                       L"Peer sources", live.local_online ? FormatUnsigned(live.local.peers) : L"—",
                       L"connected");
        ...
        snapshot_card_ = {left + choice_w + gap, choice_top, right, choice_top + S(120)};
        DrawSyncChoice(dc, full_ibd_card_, L"Full IBD",
                       ...
                       full_ibd_choice_, true);
        DrawSyncChoice(
            dc, snapshot_card_, L"Signed snapshot",
            live.process_running
        ...
        RECT verify_title{verify.left + S(22), verify.top + S(12), verify.right - S(20),
                          verify.top + S(45)};
        DrawTextAt(dc, L"Historical verification", verify_title, font_body_, C_TEXT);
        RECT verify_text{verify.left + S(22), verify.top + S(46), verify.right - S(22),
                         verify.bottom - S(15)};
    ...
    void DrawSegmentedProgress(HDC dc, RECT r, double percent, bool complete) {
        ...
        for (int i = 0; i < SEGMENTS; ++i) {
            RECT seg{r.left + i * (width + gap), r.top, r.left + i * (width + gap) + width,
                     r.bottom};
            const COLORREF color =
                i < filled ? (complete && i == SEGMENTS - 1 ? C_GREEN : RGB(210, 215, 211))
                           : RGB(41, 46, 43);
            HBRUSH brush = CreateSolidBrush(color);
    ...
    void DrawStages(HDC dc, int left, int right, int y, const LiveState& live,
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            DrawStatusDot(dc, r.left + S(16), y + S(21), active ? C_GREEN : C_MUTED);
            r.left += S(31);
            DrawTextAt(dc, labels[i], r, font_small_, active ? C_TEXT : C_MUTED);
        }
    }

    void DrawSyncChoice(HDC dc, RECT r, const std::wstring& title, const std::wstring& description,
                        bool selected, bool enabled) {
        const bool hovered = enabled && PtInRect(&r, hover_point_);
        FillRound(dc, r, selected ? C_PANEL_ALT : (hovered ? RGB(20, 24, 21) : C_PANEL),
                  selected ? RGB(145, 153, 148) : (hovered ? RGB(78, 86, 80) : C_BORDER));
        HPEN pen = CreatePen(PS_SOLID, S(2), selected ? C_TEXT : C_MUTED);
        HGDIOBJ old_pen = SelectObject(dc, pen);
        SelectObject(dc, GetStockObject(HOLLOW_BRUSH));
        Ellipse(dc, r.left + S(20), r.top + S(22), r.left + S(42), r.top + S(44));
        if (selected) {
            HBRUSH dot = CreateSolidBrush(C_TEXT);
            HGDIOBJ old_b = SelectObject(dc, dot);
            Ellipse(dc, r.left + S(26), r.top + S(28), r.left + S(36), r.top + S(38));
            SelectObject(dc, old_b);
            DeleteObject(dot);
        }
        SelectObject(dc, old_pen);
    ...
    void DrawMining(HDC dc, const RECT& client, const LiveState& live) {
        DrawPageHeader(dc, client, L"Mining",
                       address_only_
                           ? L"Solo mining to your payout address. No wallet unlock needed."
                           : L"Solo mining with your local wallet identity.",
                       live);
        ...
        const bool mining_configured =
            live.mining_status_online ? live.mining.mining_configured : mining_enabled_.load();
        const bool active = live.mining_status_online && live.mining.mining_active;
        ...
        const std::wstring state_detail =
            !mining_configured
                ? L"CPU mining disabled"
                : (live.mining_status_online ? MiningStateDetail(live.mining.work_state)
                                             : L"waiting for node status");
        DrawMetricCard(dc, {left, top, left + card_w, top + S(124)}, L"Mining status", state,
                       state_detail, active ? C_GREEN : C_TEXT);
        DrawMetricCard(dc, {left + card_w + gap, top, left + 2 * card_w + gap, top + S(124)},
                       L"Total hashrate",
                       live.mining_status_online && mining_configured
                           ? FormatHashrate(live.mining.hashrate)
                           : L"—",
                       L"measured by the node", C_TEXT);
        DrawMetricCard(
            dc, {left + 2 * (card_w + gap), top, left + 3 * card_w + 2 * gap, top + S(124)},
            L"CPU workers",
            live.mining_status_online && mining_configured ? FormatUnsigned(live.mining.threads)
                                                           : L"0",
            L"VeldHash threads", C_TEXT);
        DrawMetricCard(
            dc, {left + 3 * (card_w + gap), top, right, top + S(124)}, L"Accepted blocks",
            live.mining_status_online ? FormatUnsigned(live.mining.blocks_mined_session) : L"—",
            L"this node session", C_TEXT);

        RECT graph{left, top + S(142), right, client.bottom - S(208)};
        FillRound(dc, graph, C_PANEL, C_BORDER);
        RECT graph_title{graph.left + S(22), graph.top + S(12), graph.right - S(22),
                         graph.top + S(44)};
        DrawTextAt(dc, L"Hashrate history", graph_title, font_body_, C_TEXT);
        DrawRangeSelector(dc, graph.right - S(22), graph.top + S(12), rate_range_minutes_,
                          rate_range_buttons_);
        RECT graph_value{graph.left + S(22), graph.top + S(40), graph.right - S(22),
                         graph.top + S(74)};
        DrawTextAt(dc,
                   live.mining_status_online && mining_configured
                       ? FormatHashrate(live.mining.hashrate)
        ...
        DrawRateHistory(
            dc, {graph.left + S(24), graph.top + S(80), graph.right - S(24), graph.bottom - S(22)});

        RECT safety{left, client.bottom - S(190), right, client.bottom - S(26)};
        FillRound(dc, safety, C_PANEL_ALT, C_BORDER);
        RECT safety_title{safety.left + S(22), safety.top + S(12), safety.right - S(20),
                          safety.top + S(43)};
        DrawTextAt(dc, L"Work admission", safety_title, font_heading_, C_TEXT);
        const int col = (safety.right - safety.left) / 4;
        DrawStripItem(dc, {safety.left, safety.top + S(48), safety.left + col, safety.bottom},
                      L"Chain", live.local_online ? L"Validated" : L"Waiting",
                      live.local_online ? C_GREEN : C_MUTED);
        DrawStripItem(dc,
                      {safety.left + col, safety.top + S(48), safety.left + 2 * col, safety.bottom},
                      L"Peers", live.local.peers > 0 ? L"Connected" : L"Waiting",
                      live.local.peers > 0 ? C_GREEN : C_MUTED);
        DrawStripItem(
            dc, {safety.left + 2 * col, safety.top + S(48), safety.left + 3 * col, safety.bottom},
            L"Hashes this session",
            live.mining_status_online && mining_configured
                ? FormatUnsigned(live.mining.total_hashes)
                : L"—",
            C_SUBTEXT);
        DrawStripItem(dc, {safety.left + 3 * col, safety.top + S(48), safety.right, safety.bottom},
                      L"Validator", address_only_ ? L"Off: address only" : L"Auto when registered",
                      C_SUBTEXT);
    }

    void DrawWorkers(HDC dc, const RECT& client, const LiveState& live) {
        DrawPageHeader(dc, client, L"Workers", L"CPU mining threads owned by this node.", live);
        const int left = S(292);
        const int right = client.right - S(34);
        const int top = S(130);
        RECT table{left, top, right, client.bottom - S(26)};
        FillRound(dc, table, C_PANEL, C_BORDER);
        RECT title{table.left + S(22), table.top + S(12), table.right - S(22), table.top + S(44)};
        const uint64_t thread_count =
            live.mining_status_online && live.mining.mining_configured ? live.mining.threads : 0;
        DrawTextAt(dc, L"Local VeldHash workers", title, font_heading_, C_TEXT);
        RECT note{table.left + S(22), table.top + S(44), table.right - S(22), table.top + S(74)};
        DrawTextAt(
            dc,
        ...
        RECT header{table.left + S(18), header_y, table.right - S(18), header_y + S(38)};
        ...
        DrawTextAt(dc, L"WORKER", cell(2, 20, header.top, header.bottom), font_small_, C_SUBTEXT);
        DrawTextAt(dc, L"STATE", cell(20, 42, header.top, header.bottom), font_small_, C_SUBTEXT);
        DrawTextAt(dc, L"EST. RATE", cell(42, 66, header.top, header.bottom), font_small_,
                   C_SUBTEXT);
        DrawTextAt(dc, L"ALGORITHM", cell(66, 84, header.top, header.bottom), font_small_,
                   C_SUBTEXT);
        DrawTextAt(dc, L"ACTIVITY", cell(84, 98, header.top, header.bottom), font_small_,
                   C_SUBTEXT);

        if (thread_count == 0) {
            RECT empty{table.left + S(22), header.bottom + S(20), table.right - S(22),
                       table.bottom - S(20)};
            DrawTextAt(dc,
                       live.process_running ? L"CPU mining is disabled for this node."
        ...
        const std::wstring worker_state = active ? L"Hashing"
                                                 : (live.mining.work_state == "clock-drift"
                                                        ? L"Check PC clock"
                                                        : MiningStateLabel(live.mining.work_state));
        const double per_worker =
            thread_count > 0 ? live.mining.hashrate / static_cast<double>(thread_count) : 0.0;
        for (uint64_t i = 0; i < shown; ++i) {
            ...
            DrawTextAt(dc, L"CPU " + std::to_wstring(i + 1), cell(2, 20, y1, y2), font_body_,
                       C_TEXT);
            RECT state_cell = cell(20, 42, y1, y2);
            state_cell.right -= S(10);
            DrawTextAt(dc, worker_state, state_cell,
                       FitSingleLineFont(dc, worker_state, state_cell, {font_body_, font_small_}),
                       active ? C_GREEN : C_SUBTEXT,
                       DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX | DT_END_ELLIPSIS);
            DrawTextAt(dc, FormatHashrate(per_worker), cell(42, 66, y1, y2), font_body_, C_TEXT);
            DrawTextAt(dc, L"VeldHash", cell(66, 84, y1, y2), font_body_, C_SUBTEXT);
            DrawTextAt(dc, active ? L"Live" : L"Waiting", cell(84, 98, y1, y2), font_body_,
                       active ? C_GREEN : C_MUTED);
        }
        if (thread_count > shown) {
            RECT more{header.left, table.bottom - S(34), header.right, table.bottom - S(8)};
            DrawTextAt(dc, L"+ " + FormatUnsigned(thread_count - shown) + L" additional workers",
                       more, font_small_, C_SUBTEXT,
                       DT_RIGHT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
        }
    }

    void DrawExplorer(HDC dc, const RECT& client, const LiveState& live) {
        DrawPageHeader(dc, client, L"Explorer", L"Inspect the chain served by this node.", live);
        const int left = S(292);
        const int right = client.right - S(34);
        const int gap = S(16);
        const int top = S(130);
        const int total_w = right - left;
        const int card_w = (total_w - 3 * gap) / 4;
        DrawMetricCard(dc, {left, top, left + card_w, top + S(124)}, L"Block height",
                       live.local_online ? FormatUnsigned(live.local.height) : L"—",
                       L"local canonical tip", C_TEXT);
        DrawMetricCard(dc, {left + card_w + gap, top, left + 2 * card_w + gap, top + S(124)},
                       L"Direct peers", live.local_online ? FormatUnsigned(live.local.peers) : L"—",
                       L"this node's P2P sessions", C_TEXT);
        DrawMetricCard(
            dc, {left + 2 * (card_w + gap), top, left + 3 * card_w + 2 * gap, top + S(124)},
            L"Mempool", live.local_online ? FormatUnsigned(live.local.mempool_size) : L"—",
            L"transactions", C_TEXT);
        DrawMetricCard(dc, {left + 3 * (card_w + gap), top, right, top + S(124)}, L"Supply",
                       live.local_online ? FormatVeldNumber(live.local.supply_veld) : L"—",
        ...
        RECT ws_title{workspace.left + S(24), workspace.top + S(22), workspace.right - S(24),
                      workspace.top + S(60)};
        DrawTextAt(dc, L"Locally verified chain", ws_title, font_heading_, C_TEXT);
        ...
        RECT services{chain.right + inner_gap, inner_top, inner_right, inner_bottom};
        FillRound(dc, chain, C_PANEL_ALT, C_BORDER_SOFT, 5);
        FillRound(dc, services, C_PANEL_ALT, C_BORDER_SOFT, 5);

        RECT chain_title{chain.left + S(20), chain.top + S(12), chain.right - S(20),
                         chain.top + S(44)};
        DrawTextAt(dc, L"Canonical tip and recent blocks", chain_title, font_body_, C_TEXT);
        RECT chain_detail{chain.left + S(20), chain.top + S(38), chain.right - S(20),
                          chain.top + S(61)};
        DrawTextAt(dc,
                   live.local_online
                       ? (full_ibd_choice_ ? L"Consensus verified locally from genesis"
        ...
        auto draw_chain_row = [&](int row, const std::wstring& label, const std::wstring& value) {
            const int y = chain.top + S(66) + row * S(34);
            RECT label_r{chain.left + S(20), y, chain.left + label_w, y + S(26)};
            RECT value_r{chain.left + label_w, y, chain.right - S(20), y + S(26)};
            DrawTextAt(dc, label, label_r, font_small_, C_MUTED);
            DrawTextAt(dc, value, value_r, font_log_, C_SUBTEXT,
                       DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX | DT_END_ELLIPSIS);
        };
        draw_chain_row(0, L"Height", live.local_online ? FormatUnsigned(live.local.height) : L"—");
        ...
        draw_chain_row(3, L"Chain data", live.local_online ? FormatBytes(live.chain_bytes) : L"—");
        ...
        RECT feed_title{chain.left + S(20), feed_top + S(10), chain.right - S(20),
                        feed_top + S(38)};
        ...
        for (size_t i = 0; i < feed_count; ++i) {
            ...
            DrawTextAt(dc, L"#" + FormatUnsigned(block.height), height_r, font_body_, C_TEXT);
            std::wstring winner = Utf8ToWide(block.winner);
            if (winner.size() > 20)
                winner = winner.substr(0, 9) + L"…" + winner.substr(winner.size() - 7);
            RECT winner_r{chain.left + S(116), y, chain.right - S(96), y + S(26)};
            DrawTextAt(dc, winner.empty() ? L"Winner unavailable" : L"Won by " + winner, winner_r,
            ...
            DrawTextAt(dc, FormatAge(block.timestamp), age_r, font_small_, C_MUTED,
                       DT_RIGHT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
            RECT detail_r{chain.left + S(20), y + S(25), chain.right - S(20), y + S(49)};
            DrawTextAt(dc,
                       FormatUnsigned(block.transaction_count) + L" tx · " +
                           FormatVeld(block.reward_veld) + L" reward",
                       detail_r, font_small_, C_MUTED);
        }
        if (feed_count == 0) {
            RECT empty{chain.left + S(20), feed_top + S(48), chain.right - S(20), feed_top + S(82)};
            DrawTextAt(dc, L"Waiting for locally verified block details.", empty, font_small_,
                       C_MUTED);
        }

        RECT service_title{services.left + S(20), services.top + S(12), services.right - S(20),
                           services.top + S(44)};
        DrawTextAt(dc, L"Services", service_title, font_body_, C_TEXT);
        auto draw_service = [&](int row, const std::wstring& label, const std::wstring& value) {
            const int y = services.top + S(54) + row * S(43);
            RECT label_r{services.left + S(20), y, services.left + S(116), y + S(28)};
            RECT value_r{services.left + S(116), y, services.right - S(20), y + S(28)};
            DrawTextAt(dc, label, label_r, font_small_, C_MUTED);
            DrawTextAt(dc, value, value_r, font_small_, C_SUBTEXT,
                       DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX | DT_END_ELLIPSIS);
        };
        draw_service(0, L"Explorer", L"explorer.veld.network");
        draw_service(1, L"RPC", live.process_running ? L"Local only · token secured" : L"Stopped");
        draw_service(2, L"P2P",
                     live.process_running
        ...
        const int button_w = (services.right - services.left - S(40) - button_gap) / 2;
        open_explorer_button_ = {services.left + S(20), services.bottom - S(64),
                                 services.left + S(20) + button_w, services.bottom - S(20)};
        open_wallet_button_ = {open_explorer_button_.right + button_gap, services.bottom - S(64),
                               services.right - S(20), services.bottom - S(20)};
    ...
    void DrawNetworkSphere(HDC dc, int cx, int cy, int radius, COLORREF base, COLORREF border) {
        ...
        for (int ring = radius; ring >= 2; --ring) {
            ...
            if (ring > radius * 2 / 3)
                color = MixColor(color, RGB(196, 205, 199), (ring - radius * 2 / 3) * 18 / radius);
            HBRUSH brush = CreateSolidBrush(color);
            HGDIOBJ old_brush = SelectObject(dc, brush);
            const int highlight = (radius - ring) / 3;
            Ellipse(dc, cx - ring - highlight, cy - ring - highlight, cx + ring - highlight,
                    cy + ring - highlight);
            SelectObject(dc, old_brush);
            DeleteObject(brush);
        }
    ...
    void DrawDirectPeerTopology(HDC dc, RECT card, const LiveState& live) {
        ...
        const std::wstring session_summary =
            live.local_online ? FormatUnsigned(live.local.peers) + L" direct · report pending"
                              : L"Offline";
        const int summary_w = S(172);
        RECT summary{card.right - summary_w - S(18), card.top + S(13), card.right - S(18),
                     card.top + S(43)};
        FillRound(dc, summary, C_PANEL_ALT, C_BORDER_SOFT, 5);
        DrawTextAt(dc, session_summary, summary, font_small_, C_SUBTEXT,
                   DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
        RECT title{card.left + S(20), card.top + S(12), summary.left - S(12), card.top + S(44)};
        DrawTextAt(dc, L"Peer topology", title, font_heading_, C_TEXT);
        RECT subtitle{card.left + S(20), card.top + S(42), card.right - S(20), card.top + S(66)};
        DrawTextAt(
            dc,
            L"Direct sessions shown live · network reports join automatically · no addresses exposed",
            subtitle, font_small_, C_MUTED);

        RECT plot{card.left + S(18), card.top + S(70), card.right - S(18), card.bottom - S(43)};
        ...
        auto role_color = [](const std::string& role) -> COLORREF {
            if (role == "fleet")
                return RGB(111, 139, 153);
            if (role == "validator")
                return RGB(143, 111, 189);
            if (role == "mesh" || role == "miner" || role == "node")
                return RGB(92, 145, 124);
            return RGB(92, 145, 124);
        };
        std::vector<veld::node_gui::PeerSummary> fallback_peers;
        const std::vector<veld::node_gui::PeerSummary>* displayed_peers = &live.peer_details;
        const bool using_fallback =
            !live.peer_details_online && live.local_online && live.local.peers > 0;
        if (using_fallback) {
            const size_t count = static_cast<size_t>(std::min<uint64_t>(live.local.peers, 16));
        ...
        if (!live.local_online) {
            DrawTextAt(dc, L"Start the node to view live peer topology.", plot, font_body_, C_MUTED,
                       DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
        } else if (displayed_peers->empty()) {
            DrawTextAt(dc, L"No peers are currently connected.", plot, font_body_, C_MUTED,
                       DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
        } else {
            ...
            const int radius_x =
                std::max<int>(S(85), static_cast<int>(plot.right - plot.left - 2 * node_w) / 2);
            const int radius_y =
                std::max<int>(S(55), static_cast<int>(plot.bottom - plot.top - 2 * node_h) / 2);
            ...
            auto place_layer = [&](const std::vector<size_t>& layer, double scale, double phase) {
                if (layer.empty())
                    return;
                const double step = 2.0 * PI / static_cast<double>(layer.size());
                for (size_t ordinal = 0; ordinal < layer.size(); ++ordinal) {
                    const double angle = phase + step * static_cast<double>(ordinal);
            ...
            for (size_t i = 0; i < shown; ++i) {
                RECT node{centers[i].x - node_w / 2, centers[i].y - node_h / 2,
                          centers[i].x + node_w / 2, centers[i].y + node_h / 2};
            ...
            for (size_t i = 0; i < shown; ++i) {
                ...
                const COLORREF base_line_color =
                    using_fallback
                        ? C_BORDER
                        : (agreed ? MixColor(role_color(peer.role), C_TEXT, 10) : C_WARN);
                const COLORREF line_color =
                    highlighted ? MixColor(role_color(peer.role), C_TEXT, 45) : base_line_color;
                HPEN underlay = CreatePen(PS_SOLID, highlighted ? S(7) : S(3), RGB(19, 24, 21));
                ...
                HPEN line = CreatePen(peer.inbound ? PS_DASH : PS_SOLID, highlighted ? S(3) : S(1),
                                      line_color);
                ...
                const int marker_r = highlighted ? S(5) : S(peer.inbound ? 4 : 3);
                HPEN marker_pen = CreatePen(PS_SOLID, S(1), line_color);
                HBRUSH marker_brush = CreateSolidBrush(peer.inbound ? RGB(11, 14, 12) : line_color);
                HGDIOBJ old_marker_pen = SelectObject(dc, marker_pen);
                HGDIOBJ old_marker_brush = SelectObject(dc, marker_brush);
                Ellipse(dc, marker_x - marker_r, marker_y - marker_r, marker_x + marker_r,
                        marker_y + marker_r);
            ...
            for (int halo : {57, 50}) {
                HPEN halo_pen =
                    CreatePen(PS_SOLID, S(1), halo == 57 ? RGB(40, 48, 43) : RGB(58, 70, 63));
                HGDIOBJ old_halo_pen = SelectObject(dc, halo_pen);
                HGDIOBJ old_halo_brush = SelectObject(dc, GetStockObject(HOLLOW_BRUSH));
                Ellipse(dc, cx - S(halo), cy - S(halo), cx + S(halo), cy + S(halo));
                SelectObject(dc, old_halo_brush);
                SelectObject(dc, old_halo_pen);
                DeleteObject(halo_pen);
            }
            DrawNetworkSphere(
                dc, cx, cy, S(43), RGB(43, 53, 47),
                using_fallback ? C_SUBTEXT : (live.exact_tip_peers ? RGB(151, 177, 158) : C_WARN));
            ...
            DrawTextAt(dc,
                       using_fallback ? L"Direct peers"
                                      : (live.exact_tip_peers ? L"Synchronized" : L"Checking tips"),
                       center_state, font_small_,
                       using_fallback ? C_SUBTEXT : (live.exact_tip_peers ? C_GREEN : C_WARN),
                       DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
            ...
            for (size_t i = 0; i < shown; ++i) {
                ...
                const COLORREF peer_role_color = using_fallback ? C_SUBTEXT : role_color(peer.role);
                const COLORREF sphere_base = using_fallback ? RGB(42, 48, 44)
                                             : peer.exact_tip
                                                 ? MixColor(peer_role_color, RGB(10, 13, 11), 62)
                                                 : RGB(76, 58, 35);
                const COLORREF sphere_border =
                    hovered ? C_TEXT : (peer.exact_tip ? peer_role_color : C_WARN);
                if (hovered) {
                    HPEN hover_pen =
                        CreatePen(PS_SOLID, S(1), MixColor(peer_role_color, C_TEXT, 35));
                    HGDIOBJ old_hover_pen = SelectObject(dc, hover_pen);
                    HGDIOBJ old_hover_brush = SelectObject(dc, GetStockObject(HOLLOW_BRUSH));
                    Ellipse(dc, centers[i].x - S(28), centers[i].y - S(38), centers[i].x + S(28),
                            centers[i].y + S(18));
                    SelectObject(dc, old_hover_brush);
                    SelectObject(dc, old_hover_pen);
                    DeleteObject(hover_pen);
                }
                DrawNetworkSphere(dc, centers[i].x, centers[i].y - S(10), hovered ? S(24) : S(22),
                                  sphere_base, using_fallback ? C_SUBTEXT : sphere_border);
                RECT direction{centers[i].x - S(22), centers[i].y - S(21), centers[i].x + S(22),
                               centers[i].y + S(1)};
                DrawTextAt(dc, using_fallback ? L"PEER" : (peer.inbound ? L"IN" : L"OUT"),
                           direction, font_small_, C_TEXT,
                           DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
                const bool place_copy_above = centers[i].y < cy - S(18);
                RECT label =
                    place_copy_above
                        ? RECT{node.left, centers[i].y - S(49), node.right, centers[i].y - S(31)}
                        : RECT{node.left, centers[i].y + S(13), node.right, centers[i].y + S(33)};
                const std::wstring peer_label =
                    role_label(peer.role) + L" " +
                    (role_number < 10 ? std::wstring(L"0") : std::wstring()) +
                    FormatUnsigned(role_number);
                DrawTextAt(dc, peer_label, label,
                           FitSingleLineFont(dc, peer_label, label, {font_small_, font_log_}),
                           C_TEXT, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
                RECT detail =
                    place_copy_above
                        ? RECT{node.left, centers[i].y - S(67), node.right, centers[i].y - S(48)}
                        : RECT{node.left, centers[i].y + S(30), node.right, centers[i].y + S(49)};
                DrawTextAt(dc,
                           (using_fallback ? std::wstring(L"Direct")
                                           : std::wstring(peer.inbound ? L"In" : L"Out")) +
                               L" · " + (tor_choice_ ? L"Tor" : L"TCP"),
            ...
            if (displayed_peers->size() > shown) {
                RECT more{plot.right - S(120), plot.bottom - S(24), plot.right, plot.bottom};
                DrawTextAt(dc, L"+" + FormatUnsigned(displayed_peers->size() - shown) + L" more",
                           more, font_small_, C_MUTED,
                           DT_RIGHT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
            }
        }

        const int legend_y = card.bottom - S(27);
        auto legend_line = [&](int x, bool inbound, COLORREF color, const wchar_t* label) {
            ...
            HBRUSH marker_brush = inbound ? static_cast<HBRUSH>(GetStockObject(HOLLOW_BRUSH))
                                          : CreateSolidBrush(color);
            HGDIOBJ old_marker_pen = SelectObject(dc, marker_pen);
            HGDIOBJ old_marker_brush = SelectObject(dc, marker_brush);
            Ellipse(dc, marker_x - marker_r, legend_y - marker_r, marker_x + marker_r,
                    legend_y + marker_r);
            SelectObject(dc, old_marker_brush);
            SelectObject(dc, old_marker_pen);
            if (!inbound)
        ...
        if (using_fallback) {
            ...
            RECT detail{card.left + S(168), legend_y - S(13), card.right - S(20), legend_y + S(13)};
            DrawTextAt(dc, L"Direction and tip metadata unavailable", detail, font_small_, C_MUTED,
                       DT_RIGHT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
        } else {
        ...
        for (size_t i = 0; i < network_peer_rects_.size(); ++i) {
            if (!PtInRect(&network_peer_rects_[i], hover_point_))
                continue;
            const auto& peer = live.peer_details[network_peer_indices_[i]];
            std::wstring status =
                peer.peer_tip_age_s < 0
                    ? L"Tip unavailable"
                    : (peer.exact_tip
            ...
            const RECT tip = NetworkTooltipRect(network_peer_rects_[i], S(270), S(88));
            FillRound(dc, tip, RGB(24, 28, 25), RGB(92, 102, 96), 6);
            RECT tip_title{tip.left + S(10), tip.top + S(5), tip.right - S(10), tip.top + S(28)};
            DrawTextAt(dc, role_label(peer.role) + L" · " + status, tip_title, font_small_, C_TEXT);
            RECT tip_detail{tip.left + S(10), tip.top + S(27), tip.right - S(10),
                            tip.bottom - S(5)};
            DrawTextAt(dc,
                       std::wstring(peer.inbound ? L"Inbound" : L"Outbound") +
                           L" TCP session\nReceived " + FormatBytes(peer.bytes_recv) + L" · sent " +
                           FormatBytes(peer.bytes_sent),
                       tip_detail, font_small_, C_MUTED, DT_LEFT | DT_WORDBREAK | DT_NOPREFIX);
            break;
        }
    }
    ...
    void DrawReportedNetworkTopology(HDC dc, RECT card, const LiveState& live) {
        FillRound(dc, card, C_PANEL, C_BORDER);
        const std::wstring coverage =
            live.topology_online ? FormatUnsigned(live.local.peers) + L" direct · " +
        ...
        RECT title{card.left + S(20), card.top + S(12), summary.left - S(12), card.top + S(44)};
        DrawTextAt(dc, L"Peer topology", title, font_heading_, C_TEXT);
        RECT subtitle{card.left + S(20), card.top + S(42), card.right - S(20), card.top + S(78)};
        DrawTextAt(dc,
                   L"Public links. Dotted means one-sided reporting, not inbound "
                   L"direction. No addresses exposed.",
                   subtitle, font_small_, C_MUTED, DT_LEFT | DT_WORDBREAK | DT_NOPREFIX);

        RECT plot{card.left + S(18), card.top + S(82), card.right - S(18), card.bottom - S(62)};
        FillRound(dc, plot, RGB(11, 14, 12), C_BORDER_SOFT, 7);
        InflateRect(&plot, -S(12), -S(10));
        network_graph_rect_ = plot;
        if (!live.topology_online) {
            RECT message{plot.left + S(24), plot.top + S(24), plot.right - S(24),
                         plot.bottom - S(24)};
            DrawTextAt(
                dc,
                L"A fresh network report is not available. Direct sessions remain available above.",
                message, font_body_, C_MUTED, DT_CENTER | DT_VCENTER | DT_WORDBREAK | DT_NOPREFIX);
            return;
        }
        if (live.topology.nodes.empty()) {
            DrawTextAt(dc, L"No network connections have been reported yet.", plot, font_body_,
                       C_MUTED, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
            return;
        }
        ...
        const int cy = (plot.top + plot.bottom) / 2;
        const int radius_x =
            std::max<int>(S(95), static_cast<int>((plot.right - plot.left) / 2) - S(40));
        const int radius_y =
            std::max<int>(S(60), static_cast<int>((plot.bottom - plot.top) / 2) - S(36));
        constexpr double PI = 3.14159265358979323846;
        ...
        std::unordered_map<uint64_t, const veld::node_gui::PeerSummary*> direct_peers;
        ...
        auto sort_layer = [&](std::vector<size_t>& layer, bool local_first) {
            std::stable_sort(layer.begin(), layer.end(), [&](size_t lhs, size_t rhs) {
                const auto& a = live.topology.nodes[lhs];
                const auto& b = live.topology.nodes[rhs];
                if (local_first) {
                    const bool a_local = a.anonymous_id == live.local_topology_id;
                    const bool b_local = b.anonymous_id == live.local_topology_id;
                    if (a_local != b_local)
                        return a_local;
                }
                if (a.role_index != b.role_index)
        ...
        if (!fleet_nodes.empty())
            draw_orbit(0.54, MixColor(role_color("fleet"), RGB(11, 14, 12), 72));
        if (!validator_nodes.empty())
            draw_orbit(0.77, MixColor(role_color("validator"), RGB(11, 14, 12), 72));
        if (!operator_nodes.empty())
            draw_orbit(1.0, MixColor(role_color("node"), RGB(11, 14, 12), 72));
        ...
        for (size_t i = 0; i < shown; ++i) {
            ...
            if (position == positions.end())
                continue;
            RECT hit{position->second.x - S(40), position->second.y - S(30),
                     position->second.x + S(40), position->second.y + S(39)};
        ...
        auto draw_edge_path = [&](const POINT& from, const POINT& to, uint64_t first_id,
                                  uint64_t second_id) {
            ...
            const double center_distance = std::sqrt(center_dx * center_dx + center_dy * center_dy);
            ...
            const double midpoint_x = (static_cast<double>(from.x) + to.x) / 2.0;
            const double midpoint_y = (static_cast<double>(from.y) + to.y) / 2.0;
            ...
            if (minus_distance > plus_distance || (std::abs(minus_distance - plus_distance) < 1.0 &&
        ...
        for (const auto& edge : live.topology.edges) {
            ...
            if (first == positions.end() || second == positions.end())
                continue;
            const bool direct =
                live.local_topology_id != 0 &&
                (edge.first == live.local_topology_id || edge.second == live.local_topology_id);
            const bool highlighted =
                hovered_topology_id != 0 &&
                (edge.first == hovered_topology_id || edge.second == hovered_topology_id);
            const auto first_role = node_roles.find(edge.first);
            const auto second_role = node_roles.find(edge.second);
            const COLORREF first_color =
                role_color(first_role == node_roles.end() ? "node" : first_role->second);
            const COLORREF second_color =
                role_color(second_role == node_roles.end() ? "node" : second_role->second);
            COLORREF edge_color =
                first_color == second_color ? first_color : MixColor(first_color, second_color, 50);
            ...
            const bool tip_differs =
                (first_peer != direct_peers.end() && first_peer->second->peer_tip_age_s >= 0 &&
                 !first_peer->second->exact_tip) ||
                (second_peer != direct_peers.end() && second_peer->second->peer_tip_age_s >= 0 &&
                 !second_peer->second->exact_tip) ||
                (first_peer == direct_peers.end() && first_tip != node_tip_states.end() &&
                 first_tip->second == "differs") ||
                (second_peer == direct_peers.end() && second_tip != node_tip_states.end() &&
            ...
            if (highlighted) {
                ...
                const COLORREF hovered_color =
                    role_color(hovered_role == node_roles.end() ? "node" : hovered_role->second);
            ...
            if (edge.confirmed) {
                HPEN underlay = CreatePen(PS_SOLID, highlighted ? S(7) : (direct ? S(4) : S(3)),
                                          RGB(16, 20, 17));
                HGDIOBJ old_underlay = SelectObject(dc, underlay);
                draw_edge_path(first->second, second->second, edge.first, edge.second);
                SelectObject(dc, old_underlay);
                DeleteObject(underlay);
            }
            HPEN line = CreatePen(
                edge.confirmed ? PS_SOLID : PS_DOT,
                edge.confirmed ? (highlighted ? S(4) : (direct ? S(2) : S(1))) : S(1), edge_color);
        ...
        for (int ring = 3; ring >= 1; --ring) {
            ...
            HPEN center_ring =
                CreatePen(PS_SOLID, S(1), MixColor(center_color, RGB(11, 14, 12), 45 + ring * 10));
            HGDIOBJ old_ring = SelectObject(dc, center_ring);
            HGDIOBJ old_ring_brush = SelectObject(dc, GetStockObject(HOLLOW_BRUSH));
            Ellipse(dc, cx - ring_radius, cy - ring_radius, cx + ring_radius, cy + ring_radius);
        ...
        RECT center_title{cx - S(34), cy - S(12), cx + S(34), cy + S(4)};
        DrawTextAt(dc, L"VELD", center_title, font_small_, C_TEXT,
                   DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
        RECT center_subtitle{cx - S(38), cy + S(2), cx + S(38), cy + S(17)};
        ...
        std::unordered_map<std::string, std::unordered_set<uint32_t>> used_role_ordinals;
        display_ordinals.reserve(shown);
        for (size_t i = 0; i < shown; ++i) {
            const auto& node = live.topology.nodes[i];
            if (node.role_index == 0)
                continue;
        ...
        for (size_t i = 0; i < shown; ++i) {
            const auto& node = live.topology.nodes[i];
            if (display_ordinals.count(node.anonymous_id) != 0)
                continue;
            auto& used = used_role_ordinals[node.role];
            uint32_t ordinal = 1;
            while (used.count(ordinal) != 0)
                ++ordinal;
            used.insert(ordinal);
        ...
        for (size_t i = 0; i < shown; ++i) {
            ...
            const bool tip_unavailable =
                peer_status != direct_peers.end()
                    ? peer_status->second->peer_tip_age_s < 0
                    : node.tip_state == "unavailable" || node.tip_state == "stale";
            const bool tip_differs =
                peer_status != direct_peers.end()
                    ? peer_status->second->peer_tip_age_s >= 0 && !peer_status->second->exact_tip
                    : node.tip_state == "differs";
            const COLORREF status_color =
                tip_differs ? C_WARN : (tip_unavailable ? C_MUTED : color);
            const int radius = direct ? S(18) : S(15);
            RECT hit{center.x - S(40), center.y - S(30), center.x + S(40), center.y + S(39)};
            const bool hovered = PtInRect(&hit, hover_point_);
            const bool connected_to_hover = hover_neighbors.count(node.anonymous_id) != 0;
            const bool unrelated_to_hover =
                hovered_topology_id != 0 && !hovered && !connected_to_hover;
            if (direct) {
                const COLORREF ring_color = connected_to_hover ? MixColor(status_color, C_TEXT, 38)
                                                               : MixColor(status_color, C_TEXT, 18);
                HPEN ring = CreatePen(PS_SOLID, connected_to_hover ? S(2) : S(1), ring_color);
            ...
            DrawNetworkSphere(dc, center.x, center.y, hovered ? radius + S(2) : radius,
                              MixColor(status_color, RGB(10, 13, 11), unrelated_to_hover ? 82 : 62),
                              (hovered || connected_to_hover)
                                  ? MixColor(status_color, C_TEXT, 46)
                                  : (unrelated_to_hover
                                         ? MixColor(status_color, RGB(10, 13, 11), 62)
                                         : status_color));
            ...
            RECT label_rect{center.x - S(45), center.y + S(18), center.x + S(45), center.y + S(38)};
            DrawTextAt(dc, label, label_rect, font_small_, tip_differs ? C_WARN : C_TEXT,
                       DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
            network_topology_rects_.push_back(hit);
            network_topology_indices_.push_back(i);
        }
        if (live.topology.nodes.size() > shown) {
            RECT more{plot.right - S(130), plot.bottom - S(22), plot.right, plot.bottom};
            DrawTextAt(dc, L"+" + FormatUnsigned(live.topology.nodes.size() - shown) + L" more",
                       more, font_small_, C_MUTED,
                       DT_RIGHT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
        }
        ...
        auto legend = [&](int x, int label_right, int y, bool dotted, COLORREF color,
                          const wchar_t* text) {
            HPEN pen = CreatePen(PS_SOLID, S(1), color);
            HGDIOBJ old = SelectObject(dc, pen);
            if (dotted) {
        ...
        legend(legend_left, legend_left + legend_col, relationship_y, false, C_MUTED,
               L"Seen by both");
        legend(legend_left + legend_col, legend_left + 2 * legend_col, relationship_y, true,
               C_MUTED, L"One-sided");
        legend(legend_left + 2 * legend_col, legend_right, relationship_y, false,
               MixColor(C_TEXT, role_color("node"), 48), L"Direct session");
        ...
        Ellipse(dc, status_x, status_y - S(4), status_x + S(8), status_y + S(4));
        SelectObject(dc, old_warning_pen);
        SelectObject(dc, old_warning_brush);
        DeleteObject(warning_brush);
        RECT status_label{status_x + S(16), status_y - S(10), legend_left + legend_col,
                          status_y + S(10)};
        ...
        Ellipse(dc, unknown_x, status_y - S(4), unknown_x + S(8), status_y + S(4));
        SelectObject(dc, old_unknown_pen);
        SelectObject(dc, old_unknown_brush);
        DeleteObject(unknown_brush);
        RECT unknown_label{unknown_x + S(16), status_y - S(10), legend_right, status_y + S(10)};
        DrawTextAt(dc, L"Status unavailable", unknown_label, font_small_, C_MUTED);

        for (size_t i = 0; i < network_topology_rects_.size(); ++i) {
            if (!PtInRect(&network_topology_rects_[i], hover_point_))
                continue;
            ...
            if (direct) {
                for (const auto& peer : live.peer_details) {
                    if (peer.identified && peer.anonymous_id == node.anonymous_id) {
                        direct_peer = &peer;
                        break;
                    }
                }
            }
            const RECT tip = NetworkTooltipRect(network_topology_rects_[i], S(260), S(76));
            FillRound(dc, tip, RGB(24, 28, 25), RGB(92, 102, 96), 6);
            RECT tip_title{tip.left + S(10), tip.top + S(6), tip.right - S(10), tip.top + S(29)};
            DrawTextAt(dc,
                       role_label(node.role) +
                           (direct_peer
                                ? std::wstring(direct_peer->inbound ? L" · inbound direct session"
                                                                    : L" · outbound direct session")
                                : (direct ? L" · this node" : L" · network report")),
                       tip_title, font_small_, C_TEXT);
            RECT tip_detail{tip.left + S(10), tip.top + S(29), tip.right - S(10),
                            tip.bottom - S(5)};
            if (direct_peer) {
                const std::wstring tip_state =
                    direct_peer->exact_tip
                        ? L"Exact tip agreement"
                        : (direct_peer->lag_blocks > 0
                               ? FormatUnsigned(static_cast<uint64_t>(direct_peer->lag_blocks)) +
                                     L" blocks behind"
                               : (direct_peer->lag_blocks < 0
                                      ? FormatUnsigned(
                                            static_cast<uint64_t>(-direct_peer->lag_blocks)) +
                                            L" blocks ahead"
                                      : (direct_peer->peer_tip_age_s < 0
                                             ? L"Tip unavailable"
                                             : L"Different tip at the same height")));
                DrawTextAt(dc,
                           tip_state + L"\nReceived " + FormatBytes(direct_peer->bytes_recv) +
                               L" · sent " + FormatBytes(direct_peer->bytes_sent),
                           tip_detail, font_small_, C_MUTED, DT_LEFT | DT_WORDBREAK | DT_NOPREFIX);
            } else {
                const std::wstring state =
                    node.tip_state == "exact"
                        ...
                        : (node.tip_state == "differs"
                               ...
                               : (node.tip_state == "stale" ? L"Tip report is stale"
                                                            : L"Tip status unavailable"));
                DrawTextAt(dc, state + L"\nLast reported " + FormatAge(node.updated_at), tip_detail,
                           font_small_, C_MUTED);
            }
            break;
        }
    ...
    void DrawUnifiedNetworkTopology(HDC dc, RECT card, const LiveState& live) {
        // A public aggregate adds context, but it is never a prerequisite for
        // displaying the node's authoritative direct sessions.
        if (!live.topology_online || live.topology.nodes.empty() || live.local_topology_id == 0) {
        ...
        auto normalize_role = [](const std::string& role) {
            if (role == "fleet" || role == "miner" || role == "validator" || role == "node")
                return role;
        ...
        auto replace_topology_id = [&](uint64_t previous, uint64_t current) {
            ...
            for (auto& edge : merged.topology.edges) {
                if (edge.first == previous)
                    edge.first = current;
                if (edge.second == previous)
                    edge.second = current;
            }
        };

        auto node_for_id = [&](uint64_t id) -> veld::node_gui::TopologyNode* {
            for (auto& node : merged.topology.nodes) {
                if (node.anonymous_id == id)
                    return &node;
            }
            return nullptr;
        };
        auto candidate_ids = [&](const std::string& role, uint32_t role_index,
            ...
            for (const auto& node : merged.topology.nodes) {
                if (node.role != role || claimed.count(node.anonymous_id) != 0)
                    continue;
                if (role_index != 0 && node.role_index != role_index)
                    continue;
                candidates.push_back(node.anonymous_id);
            }
            std::sort(candidates.begin(), candidates.end(), [&](uint64_t lhs, uint64_t rhs) {
        ...
        auto update_from_peer = [&](uint64_t id, const veld::node_gui::PeerSummary& peer) {
            auto* node = node_for_id(id);
            if (!node)
                return;
            node->updated_at = merged.topology.generated_at;
            node->role = peer.role;
            if (peer.role_index != 0)
                node->role_index = peer.role_index;
            node->tip_state =
                peer.peer_tip_age_s < 0 ? "unavailable" : (peer.exact_tip ? "exact" : "differs");
        };
        auto update_local = [&]() {
        ...
        for (const auto& peer : merged.peer_details) {
            ...
            if (peer.role_index == 0)
                continue;
            const auto candidates = candidate_ids(peer.role, peer.role_index, claimed);
        ...
        if (local_id_present) {
            ...
            const auto candidates = candidate_ids(local_role, topology_role_index_, claimed);
            if (candidates.size() == 1) {
                replace_topology_id(candidates.front(), merged.local_topology_id);
                claimed.insert(merged.local_topology_id);
                update_local();
        ...
        if (!local_id_present) {
            ...
            for (const auto& peer : merged.peer_details) {
                if (!peer.identified || peer.anonymous_id == 0 || peer.role != local_role ||
                    claimed.count(peer.anonymous_id) != 0)
                    continue;
                ++unmatched_same_role;
            }
            if (local_candidates.size() > unmatched_same_role) {
                replace_topology_id(local_candidates.front(), merged.local_topology_id);
                claimed.insert(merged.local_topology_id);
                update_local();
                local_id_present = true;
            }
        ...
        for (const auto& peer : merged.peer_details) {
            if (!peer.identified || peer.anonymous_id == 0 || claimed.count(peer.anonymous_id) != 0)
                continue;
        ...
        for (const auto& peer : merged.peer_details) {
            if (!peer.identified || peer.anonymous_id == 0)
                continue;
            if (node_ids.insert(peer.anonymous_id).second && merged.topology.nodes.size() < 256) {
                veld::node_gui::TopologyNode node;
                node.anonymous_id = peer.anonymous_id;
                node.updated_at = observed_at;
                node.role_index = peer.role_index;
                node.role = peer.role;
                node.tip_state = peer.peer_tip_age_s < 0 ? "unavailable"
        ...
        for (const auto& edge : merged.topology.edges) {
            if (edge.first == 0 || edge.second == 0 || edge.first == edge.second ||
                node_ids.count(edge.first) == 0 || node_ids.count(edge.second) == 0)
                continue;
            ...
            if (existing == normalized_edge_indices.end()) {
                normalized_edge_indices.emplace(key, normalized_edges.size());
        ...
        merged.topology.eligible_nodes =
            std::max<uint64_t>(merged.topology.eligible_nodes, merged.topology.nodes.size());
        DrawReportedNetworkTopology(dc, card, merged);
    }

    void DrawNetwork(HDC dc, const RECT& client, const LiveState& live) {
        DrawPageHeader(dc, client, L"Network", L"Network-wide map and direct peer health.", live);
        const int left = S(292);
        const int right = client.right - S(34);
        const int gap = S(14);
        const int top = S(132);
        ...
        DrawMetricCard(dc, {left, top, left + card_w, top + metrics_h}, L"Direct peers",
                       live.local_online ? FormatUnsigned(live.local.peers) : L"—",
                       L"this node's P2P sessions");
        DrawMetricCard(dc, {left + card_w + gap, top, left + 2 * card_w + gap, top + metrics_h},
                       L"Inbound",
                       live.peer_details_online ? FormatUnsigned(live.inbound_peers) : L"—",
                       live.inbound_peers > 0
                           ? L"remote-initiated · reachable"
                           : (reachable_choice_ ? L"awaiting remote peer" : L"outbound-only mode"));
        DrawMetricCard(
            dc, {left + 2 * (card_w + gap), top, left + 3 * card_w + 2 * gap, top + metrics_h},
            L"Outbound", live.peer_details_online ? FormatUnsigned(live.outbound_peers) : L"—",
            L"locally dialed");
        DrawMetricCard(dc, {left + 3 * (card_w + gap), top, right, top + metrics_h},
                       L"Exact tip agreement",
                       live.peer_details_online ? FormatUnsigned(live.exact_tip_peers) + L" / " +
                                                      FormatUnsigned(live.peer_details.size())
                                                : L"—",
                       L"peer-announced tips", live.exact_tip_peers ? C_GREEN : C_TEXT);
        ...
        RECT health_title{health.left + S(18), health.top + S(12), health.right - S(18),
                          health.top + S(44)};
        DrawTextAt(dc, L"Connection health", health_title,
                   ...
                   C_TEXT);
        const bool healthy =
            live.peer_details_online && !live.peer_details.empty() && live.exact_tip_peers > 0;
        const bool waiting = !live.peer_details_online || live.peer_details.empty();
        RECT health_state{health.left + S(18), health.top + S(50), health.right - S(18),
                          health.top + S(82)};
        DrawTextAt(dc, waiting ? L"Waiting" : (healthy ? L"Healthy" : L"Check peer tips"),
                   health_state, font_heading_, waiting ? C_MUTED : (healthy ? C_GREEN : C_WARN));
        auto health_row = [&](int row, const std::wstring& label, const std::wstring& value) {
            ...
            RECT label_r{health.left + S(18), y, health.right - S(18), y + S(18)};
            DrawTextAt(dc, label, label_r, font_small_, C_MUTED);
            RECT value_r{health.left + S(18), y + S(18), health.right - S(18), y + S(40)};
            DrawTextAt(dc, value, value_r,
                       FitSingleLineFont(dc, value, value_r, {font_body_, font_small_}), C_TEXT);
        };
        health_row(0, L"Tip agreement",
                   live.peer_details_online ? FormatUnsigned(live.exact_tip_peers) + L" of " +
                                                  FormatUnsigned(live.peer_details.size())
                                            : L"Unavailable");
        health_row(1, L"Traffic in",
                   live.peer_details_online ? FormatByteRate(live.peer_recv_rate) : L"Unavailable");
        health_row(2, L"Traffic out",
                   live.peer_details_online ? FormatByteRate(live.peer_send_rate) : L"Unavailable");
        const std::wstring transport =
            tor_choice_
                ? L"Tor only"
                : (live.port_mapped ? L"Clearnet · automatic mapping"
                                    : (live.inbound_peers > 0
                                           ? L"Clearnet · inbound verified"
        ...
        auto distribution_row = [&](int row, const std::wstring& label, const std::wstring& value) {
            const int y = distribution.top + S(46 + row * 27);
            RECT label_r{distribution.left + S(18), y, distribution.right - S(72), y + S(24)};
            RECT value_r{distribution.right - S(72), y, distribution.right - S(18), y + S(24)};
            DrawTextAt(dc, label, label_r, font_small_, C_SUBTEXT);
        ...
        distribution_row(0, L"Inbound / outbound",
                         live.peer_details_online ? FormatUnsigned(live.inbound_peers) + L" / " +
                                                        FormatUnsigned(live.outbound_peers)
                                                  : L"—");
        distribution_row(1, L"Known peer endpoints",
                         live.peer_details_online ? FormatUnsigned(live.known_peer_count) : L"—");
        distribution_row(2, L"Data received",
                         live.peer_details_online ? FormatBytes(live.peer_bytes_recv) : L"—");
        distribution_row(3, L"Data sent",
                         live.peer_details_online ? FormatBytes(live.peer_bytes_sent) : L"—");

        RECT events_title{events.left + S(18), events.top + S(11), events.right - S(18),
                          events.top + S(40)};
        DrawTextAt(dc, L"Recent network events", events_title, font_heading_, C_TEXT);
        if (live.network_events.empty()) {
            RECT empty{events.left + S(18), events.top + S(48), events.right - S(18),
                       events.bottom - S(12)};
            DrawTextAt(dc, L"No connection changes during this session.", empty, font_small_,
                       C_MUTED);
        } else {
            ...
            for (size_t i = 0; i < shown; ++i) {
                ...
                RECT text{events.left + S(18), y, events.right - S(92), y + S(25)};
                DrawTextAt(dc, event.text, text, font_small_, event.warning ? C_WARN : C_SUBTEXT,
                           DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX | DT_END_ELLIPSIS);
                RECT age{events.right - S(92), y, events.right - S(18), y + S(25)};
                DrawTextAt(dc, FormatAge(event.timestamp), age, font_small_, C_MUTED,
                           DT_RIGHT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
            }
        }
    }
    ...
    std::wstring BuildRedactedDiagnostics(const LiveState& live) const {
        ...
        out << L"Veld Node diagnostics\r\n"
            ...
            << L"Network: Mainnet\r\n"
            << L"Node process: " << (live.process_running ? L"running" : L"stopped") << L"\r\n"
            << L"Local status: " << (live.local_online ? L"online" : L"offline") << L"\r\n"
            ...
            << L"Sync mode: "
            << (full_ibd_choice_ ? L"full IBD"
                                 : L"signed snapshot with independent genesis validation")
            << L"\r\n"
            << L"Snapshot bootstrap: available\r\n"
            << L"Transport: "
            << (tor_choice_ ? L"Tor only"
                            : (reachable_choice_ ? L"Clearnet reachable" : L"Clearnet outbound"))
            << L"\r\n"
            << L"Mining configured: " << (mining_enabled_ ? L"yes" : L"no") << L"\r\n"
            << L"Mining state: "
            << (live.mining_status_online ? MiningStateLabel(live.mining.work_state)
                                          : L"unavailable")
            << L"\r\n"
            << L"Hashrate: "
            << (live.mining_status_online ? FormatHashrate(live.mining.hashrate) : L"unavailable")
            << L"\r\n"
            << L"Workers: "
            << (live.mining_status_online ? FormatUnsigned(live.mining.threads)
                                          : FormatUnsigned(mining_thread_count_))
            << L"\r\n";
        return out.str();
    }
    ...
    void DrawLogs(HDC dc, const RECT& client, const LiveState& live) {
        ...
        RECT heading{panel.left + S(22), panel.top + S(14), panel.right - S(380),
                     panel.top + S(48)};
        DrawTextAt(dc, L"node-gui-node.log", heading, font_heading_, C_TEXT);
        RECT path{panel.left + S(22), panel.top + S(48), panel.right - S(380), panel.top + S(78)};
        DrawTextAt(dc, (state_dir_ / L"node-gui-node.log").wstring(), path, font_small_, C_MUTED,
                   DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX | DT_END_ELLIPSIS);
        copy_diagnostics_button_ = {panel.right - S(354), panel.top + S(20), panel.right - S(192),
                                    panel.top + S(64)};
        const bool copied = std::chrono::steady_clock::now() < diagnostics_copied_until_;
        DrawButton(dc, copy_diagnostics_button_,
                   copied ? L"Diagnostics copied" : L"Copy diagnostics", true);
        open_log_button_ = {panel.right - S(184), panel.top + S(20), panel.right - S(22),
                            panel.top + S(64)};
        DrawButton(dc, open_log_button_, L"Open log folder", std::filesystem::exists(state_dir_));

        RECT separator{panel.left + S(22), panel.top + S(88), panel.right - S(22),
                       panel.top + S(89)};
        HBRUSH line = CreateSolidBrush(C_BORDER_SOFT);
        FillRect(dc, &separator, line);
        DeleteObject(line);

        RECT log_area{panel.left + S(22), panel.top + S(104), panel.right - S(22),
                      panel.bottom - S(54)};
        if (live.log_lines.empty()) {
            ...
            const int visible =
                std::max(1, static_cast<int>((log_area.bottom - log_area.top) / line_h));
            const size_t first = live.log_lines.size() > static_cast<size_t>(visible)
                                     ? live.log_lines.size() - static_cast<size_t>(visible)
                                     : 0;
            int y = log_area.top;
        ...
        RECT note{panel.left + S(22), panel.bottom - S(45), panel.right - S(22),
                  panel.bottom - S(14)};
        DrawTextAt(dc, L"Local file | bounded view | refreshes automatically", note, font_small_,
                   C_MUTED);
    }

    void DrawToggle(HDC dc, RECT r, bool on, bool enabled) {
        FillRound(dc, r, on ? RGB(50, 62, 52) : RGB(31, 35, 32), enabled ? C_BORDER : C_BORDER_SOFT,
                  14);
        ...
        Ellipse(dc, cx - diameter / 2, cy - diameter / 2, cx + diameter / 2, cy + diameter / 2);
        SelectObject(dc, old_p);
        SelectObject(dc, old_b);
        DeleteObject(pen);
        DeleteObject(brush);
    ...
    void DrawSettingRow(HDC dc, RECT row, const std::wstring& title, const std::wstring& detail,
                        bool value, bool enabled, RECT& toggle) {
        FillRound(dc, row, C_PANEL, C_BORDER);
        RECT title_r{row.left + S(22), row.top + S(8), row.right - S(100), row.top + S(35)};
        DrawTextAt(dc, title, title_r, font_heading_, enabled ? C_TEXT : C_MUTED,
                   DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX | DT_END_ELLIPSIS);
        RECT detail_r{row.left + S(22), row.top + S(35), row.right - S(100), row.bottom - S(6)};
        DrawTextAt(dc, detail, detail_r, font_small_, enabled ? C_SUBTEXT : C_MUTED,
                   DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX | DT_END_ELLIPSIS);
        toggle = {row.right - S(78), row.top + S(22), row.right - S(22), row.top + S(50)};
        DrawToggle(dc, toggle, value, enabled);
    }

    void DrawPresetButton(HDC dc, RECT r, const std::wstring& label, bool selected) {
        const bool hovered = PtInRect(&r, hover_point_);
        FillRound(dc, r, selected ? C_PANEL_ALT : (hovered ? RGB(24, 28, 25) : C_BG),
                  selected ? RGB(112, 121, 115) : (hovered ? C_BORDER : C_BORDER_SOFT), 5);
    ...
    void DrawSettings(HDC dc, const RECT& client, const LiveState& live) {
        DrawPageHeader(dc, client, L"Settings", L"Local app preferences and paths.", live);
        const int left = S(292);
        const int right = client.right - S(34);
        ...
        DrawTextAt(dc, L"Choose what Veld starts after you sign in to Windows.", detail,
                   font_small_, C_SUBTEXT);
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            DrawPresetButton(dc, startup_mode_buttons_[i], modes[i],
                             static_cast<int>(startup_mode_) == i);
        }
        const bool unlock_mode = !address_only_ && startup_enabled_ &&
                                 (startup_mode_ == veld::node_gui::StartupMode::Solo ||
                                  startup_mode_ == veld::node_gui::StartupMode::Node);
        DrawSettingRow(
            dc, {left, start + S(140), right, start + S(218)}, L"Remember startup unlock",
            L"Optional: Windows protects the unlock for this account. Otherwise sign in when Veld opens.",
            startup_unlock_enabled_, unlock_mode || startup_unlock_enabled_,
            startup_unlock_toggle_);
        DrawSettingRow(
            dc, {left, start + S(226), right, start + S(304)}, L"Minimize to system tray",
        ...
        const bool can_change = !live.process_running && !tor_preparing_.load() &&
                                ...
                                !(pool_panel_ && pool_panel_->Running());
        ...
        DrawTextAt(
            dc,
            L"Mine to a payout address without a local wallet. Your wallet stays elsewhere; validator signing is off.",
            address_detail, font_small_, C_SUBTEXT);
        address_mode_toggle_ = {right - S(78), address_top + S(18), right - S(22),
                                address_top + S(46)};
        ...
        DrawTextAt(dc, address.empty() ? L"No payout address saved" : Utf8ToWide(address),
                   address_value, font_small_, C_TEXT,
                   DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX | DT_END_ELLIPSIS);
        address_edit_button_ = {right - S(168), address_top + S(86), right - S(22),
                                address_top + S(125)};
        ...
        RECT mining_title{left + S(22), top + S(12), right - S(100), top + S(43)};
        DrawTextAt(dc, L"CPU mining", mining_title, font_heading_, C_TEXT);
        RECT mining_detail{left + S(22), top + S(39), right - S(100), top + S(68)};
        DrawTextAt(dc,
                   live.process_running
        ...
        mining_mode_toggle_ = {right - S(78), top + S(22), right - S(22), top + S(50)};
        DrawToggle(dc, mining_mode_toggle_, mining_enabled_, true);
        const int preset_top = top + S(78);
        const int preset_w = S(122);
        preset_eco_button_ = {left + S(22), preset_top, left + S(22) + preset_w,
                              preset_top + S(30)};
        preset_balanced_button_ = {preset_eco_button_.right + S(7), preset_top,
                                   preset_eco_button_.right + S(7) + preset_w, preset_top + S(30)};
        preset_max_button_ = {preset_balanced_button_.right + S(7), preset_top,
                              preset_balanced_button_.right + S(7) + preset_w, preset_top + S(30)};
        DrawPresetButton(dc, preset_eco_button_, PresetCapacityLabel(0), mining_preset_ == 0);
        DrawPresetButton(dc, preset_balanced_button_, PresetCapacityLabel(1), mining_preset_ == 1);
        DrawPresetButton(dc, preset_max_button_, PresetCapacityLabel(2), mining_preset_ == 2);
        preset_plus_button_ = {right - S(52), preset_top, right - S(22), preset_top + S(30)};
        preset_minus_button_ = {right - S(184), preset_top, right - S(154), preset_top + S(30)};
        DrawPresetButton(dc, preset_minus_button_, L"-", false);
        DrawPresetButton(dc, preset_plus_button_, L"+", false);
        RECT thread_value{preset_minus_button_.right, preset_top, preset_plus_button_.left,
                          preset_top + S(30)};
        DrawTextAt(dc, FormatUnsigned(mining_thread_count_) + L" workers", thread_value,
                   font_small_, C_TEXT, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);

        const int rows_top = top + mining_row_h + gap;
        DrawSettingRow(
            dc, {left, rows_top, right, rows_top + row_h}, L"Tor-only privacy",
            live.process_running
            ...
            tor_choice_, !live.process_running && !tor_preparing_.load(), tor_toggle_);
        DrawSettingRow(
            dc, {left, rows_top + row_h + gap, right, rows_top + 2 * row_h + gap},
            ...
            reachable_choice_, !live.process_running && !tor_choice_ && !tor_preparing_.load(),
            reachable_toggle_);
        DrawSettingRow(
            dc, {left, rows_top + 2 * (row_h + gap), right, rows_top + 3 * row_h + 2 * gap},
        ...
        RECT monitor_title{monitor.left + S(22), monitor.top + S(12), monitor.right - S(390),
                           monitor.top + S(41)};
        DrawTextAt(dc, L"Remote access", monitor_title, font_heading_, C_TEXT);
        ...
        DrawTextAt(dc, monitor_text, monitor_detail, font_small_,
                   ...
                   DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX | DT_END_ELLIPSIS);
        remote_monitor_toggle_ = {monitor.right - S(78), monitor.top + S(11), monitor.right - S(22),
                                  monitor.top + S(39)};
        DrawToggle(dc, remote_monitor_toggle_, remote_monitoring_enabled_.load(), true);
        open_monitor_portal_button_ = {monitor.right - S(180), monitor.top + S(48),
                                       monitor.right - S(22), monitor.top + S(87)};
        DrawButton(dc, open_monitor_portal_button_, L"Open portal", true);
        if (!monitor_status.pair_code.empty()) {
            copy_monitor_code_button_ = {monitor.right - S(480), monitor.top + S(48),
        ...
        DrawButton(dc, reset_monitor_pairing_button_, L"New pair code",
                   remote_monitoring_enabled_.load() && monitor_status.credential_ready);

        const int paths_top = monitor.bottom + gap;
        RECT paths{left, paths_top, right, paths_top + S(144)};
        FillRound(dc, paths, C_PANEL, C_BORDER);
        RECT paths_title{paths.left + S(22), paths.top + S(14), paths.right - S(200),
                         paths.top + S(48)};
        DrawTextAt(dc, L"Local paths", paths_title, font_heading_, C_TEXT);
        RECT data_label{paths.left + S(22), paths.top + S(54), paths.left + S(150),
                        paths.top + S(80)};
        DrawTextAt(dc, L"Data directory", data_label, font_small_, C_MUTED);
        RECT data_value{paths.left + S(150), paths.top + S(54), paths.right - S(210),
                        paths.top + S(80)};
        const std::wstring data_name =
            data_dir_.filename().empty() ? data_dir_.wstring() : data_dir_.filename().wstring();
        DrawTextAt(dc, data_name, data_value, font_small_, C_SUBTEXT,
                   DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX | DT_END_ELLIPSIS);
        RECT node_label{paths.left + S(22), paths.top + S(92), paths.left + S(150),
                        paths.top + S(118)};
        DrawTextAt(dc, L"Node binary", node_label, font_small_, C_MUTED);
        RECT node_value{paths.left + S(150), paths.top + S(92), paths.right - S(210),
                        paths.top + S(118)};
        const std::wstring node_name =
            node_path_.filename().empty() ? node_path_.wstring() : node_path_.filename().wstring();
        DrawTextAt(dc, node_name, node_value, font_small_, C_SUBTEXT,
                   DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX | DT_END_ELLIPSIS);
        const int button_w = S(142);
        create_key_button_ = {paths.right - 3 * button_w - S(42), paths.top + S(61),
                              paths.right - 2 * button_w - S(34), paths.top + S(105)};
        ...
        import_key_button_ = {paths.right - 2 * button_w - S(26), paths.top + S(61),
                              paths.right - button_w - S(18), paths.top + S(105)};
        DrawButton(dc, import_key_button_, L"Import keyfile",
                   !address_only_ && !live.process_running && !tor_preparing_.load());
        open_data_button_ = {paths.right - button_w - S(10), paths.top + S(61), paths.right - S(10),
                             paths.top + S(105)};
        DrawButton(dc, open_data_button_, L"Open data folder", std::filesystem::exists(data_dir_));
        RECT paths_note{paths.left + S(22), paths.bottom - S(39), paths.right - S(22),
                        paths.bottom - S(12)};
        DrawTextAt(dc, L"Preferences contain no credentials or wallet material.", paths_note,
                   font_small_, C_MUTED);
        ...
        RECT release_title{release.left + S(22), release.top + S(7), release.right - S(350),
                           release.top + S(32)};
        DrawTextAt(dc, L"Release and updates", release_title, font_heading_, C_TEXT);
        RECT release_version{release.left + S(22), release.top + S(34), release.right - S(350),
                             release.top + S(58)};
        DrawTextAt(dc, L"Installed: " + Utf8ToWide(veld::CLIENT_VERSION), release_version,
                   font_body_, C_SUBTEXT);
        RECT release_status{release.left + S(22), release.top + S(59), release.right - S(350),
                            release.top + S(82)};
        ...
        DrawTextAt(dc, update_busy ? update_progress : update_status_, release_status, font_small_,
                   update_busy ? C_SUBTEXT : update_status_color_);
        check_update_button_ = {release.right - S(324), release.top + S(18), release.right - S(176),
                                release.top + S(62)};
        view_changelog_button_ = {release.right - S(164), release.top + S(18),
                                  release.right - S(20), release.top + S(62)};
        const wchar_t* update_action =
            update_operation == UpdateOperation::Check
                ? L"Checking..."
        ...
        RECT security_note{release.left + S(22), release.top + S(86), release.right - S(22),
                           release.top + S(126)};
        DrawTextAt(
            dc,
            ...
            security_note, font_small_, C_MUTED, DT_LEFT | DT_WORDBREAK | DT_NOPREFIX);
        RECT automatic_label{release.left + S(22), release.top + S(136), release.right - S(100),
                             release.top + S(162)};
        DrawTextAt(dc, L"Automatic updates", automatic_label, font_body_, C_TEXT);
        auto_update_toggle_ = {release.right - S(78), release.top + S(138), release.right - S(26),
                               release.top + S(164)};
        DrawToggle(dc, auto_update_toggle_, auto_update_enabled_, !update_busy);
        RECT automatic_note{release.left + S(22), release.top + S(169), release.right - S(22),
                            release.bottom - S(10)};
        DrawTextAt(
            dc,
            L"Check hourly for signed updates while Veld is open. Resume mining with your saved workers without signing in again after an update.",
            automatic_note, font_small_, C_MUTED, DT_LEFT | DT_WORDBREAK | DT_NOPREFIX);
    }

    void OnClick(int x, int y) {
        POINT p = ContentPoint(POINT{x, y});
        if (PtInRect(&overview_nav_, p))
            page_ = Page::Overview;
        else if (PtInRect(&blockchain_nav_, p))
            page_ = Page::Blockchain;
        else if (PtInRect(&mining_nav_, p))
            page_ = Page::Mining;
        else if (PtInRect(&workers_nav_, p))
            page_ = Page::Workers;
        else if (PtInRect(&explorer_nav_, p))
            page_ = Page::Explorer;
        else if (PtInRect(&network_nav_, p))
            page_ = Page::Network;
        else if (PtInRect(&logs_nav_, p))
            page_ = Page::Logs;
        else if (PtInRect(&settings_nav_, p))
            page_ = Page::Settings;
        else if (PtInRect(&pool_nav_, p))
            page_ = Page::Pool;
        else if (PtInRect(&follow_x_link_, p)) {
            ShellExecuteW(hwnd_, L"open", L"https://x.com/VeldNetwork", nullptr, nullptr,
                          SW_SHOWNORMAL);
        } else if (PtInRect(&action_button_, p))
            ToggleNode();
        else if (page_ == Page::Overview &&
                 (PtInRect(&chain_range_buttons_[0], p) || PtInRect(&chain_range_buttons_[1], p) ||
                  PtInRect(&chain_range_buttons_[2], p))) {
            chain_range_minutes_ = PtInRect(&chain_range_buttons_[0], p)
                                       ? 5
                                       : (PtInRect(&chain_range_buttons_[1], p) ? 15 : 60);
            SaveSettings();
        } else if (page_ == Page::Mining &&
                   (PtInRect(&rate_range_buttons_[0], p) || PtInRect(&rate_range_buttons_[1], p) ||
                    PtInRect(&rate_range_buttons_[2], p))) {
            rate_range_minutes_ = PtInRect(&rate_range_buttons_[0], p)
                                      ? 5
            ...
            sync_choice_explicit_ = true;
            SaveSettings();
        } else if (page_ == Page::Blockchain && PtInRect(&snapshot_card_, p)) {
            full_ibd_choice_ = false;
            sync_choice_explicit_ = true;
            SaveSettings();
        } else if (page_ == Page::Logs && PtInRect(&copy_diagnostics_button_, p)) {
            ...
            if (std::filesystem::exists(state_dir_))
                ShellExecuteW(hwnd_, L"open", state_dir_.c_str(), nullptr, nullptr, SW_SHOWNORMAL);
        } else if (page_ == Page::Explorer && PtInRect(&open_explorer_button_, p)) {
            ShellExecuteW(hwnd_, L"open", L"https://explorer.veld.network/", nullptr, nullptr,
                          SW_SHOWNORMAL);
        } else if (page_ == Page::Explorer && PtInRect(&open_wallet_button_, p)) {
            OpenTrustedWallet();
            ...
            if (!live.process_running && !FindNodeProcess(node_path_) && !tor_preparing_.load() &&
                update_operation_.load() == UpdateOperation::None &&
                !(pool_panel_ && pool_panel_->Running())) {
                if (PtInRect(&address_edit_button_, p) ||
                    (!address_only_ && !ValidSoloAddress(AddressPayout())))
                    ChangeSoloAddress();
                else {
                    const bool previous = address_only_.load();
                    address_only_ = !previous;
                    if (!SaveSettings())
                        address_only_ = previous;
                    else {
                        ClearSessionPassphrase();
                        session_unlock_confirmed_ = false;
                    }
                }
            }
            ...
            if (!address_only_ || startup_unlock_enabled_)
                ChangeStartupUnlock();
        } else if (page_ == Page::Settings && PtInRect(&tray_toggle_, p)) {
            minimize_to_tray_ = !minimize_to_tray_;
            if (!SaveSettings())
                minimize_to_tray_ = !minimize_to_tray_;
        } else if (page_ == Page::Settings && (PtInRect(&startup_mode_buttons_[0], p) ||
                                               PtInRect(&startup_mode_buttons_[1], p) ||
                                               PtInRect(&startup_mode_buttons_[2], p) ||
                                               PtInRect(&startup_mode_buttons_[3], p))) {
            const auto old = startup_mode_;
            for (int i = 0; i < 4; ++i)
                if (PtInRect(&startup_mode_buttons_[i], p))
                    startup_mode_ = static_cast<veld::node_gui::StartupMode>(i);
            if (!SaveSettings())
                startup_mode_ = old;
        } else if (page_ == Page::Settings && PtInRect(&mining_mode_toggle_, p)) {
            mining_enabled_ = !mining_enabled_;
            SaveSettings();
        } else if (page_ == Page::Settings && PtInRect(&preset_eco_button_, p)) {
            mining_preset_ = 0;
            mining_thread_count_ = PresetThreadCount(0);
            SaveSettings();
        } else if (page_ == Page::Settings && PtInRect(&preset_balanced_button_, p)) {
            mining_preset_ = 1;
            mining_thread_count_ = PresetThreadCount(1);
            SaveSettings();
        } else if (page_ == Page::Settings && PtInRect(&preset_max_button_, p)) {
            mining_preset_ = 2;
            mining_thread_count_ = PresetThreadCount(2);
            SaveSettings();
        } else if (page_ == Page::Settings && PtInRect(&preset_minus_button_, p)) {
            mining_preset_ = 3;
            mining_thread_count_ = std::max(1u, mining_thread_count_ - 1);
            SaveSettings();
        } else if (page_ == Page::Settings && PtInRect(&preset_plus_button_, p)) {
            mining_preset_ = 3;
        ...
        } else if (page_ == Page::Settings && PtInRect(&reachable_toggle_, p)) {
            const auto live = SnapshotState();
            if (!live.process_running && !tor_choice_ && !tor_preparing_.load()) {
                reachable_choice_ = !reachable_choice_;
                SaveSettings();
            }
        } else if (page_ == Page::Settings && PtInRect(&reference_toggle_, p)) {
            reference_display_enabled_.store(!reference_display_enabled_.load());
            SaveSettings();
            worker_cv_.notify_all();
        } else if (page_ == Page::Settings && PtInRect(&remote_monitor_toggle_, p)) {
            const bool enabled = !remote_monitoring_enabled_.load();
            remote_monitoring_enabled_.store(enabled);
            {
                std::lock_guard<std::mutex> lock(remote_monitor_mutex_);
                remote_monitor_status_.detail =
                    enabled ? L"Connecting to the secure portal..."
            ...
            SaveSettings();
            worker_cv_.notify_all();
        } else if (page_ == Page::Settings && PtInRect(&open_monitor_portal_button_, p)) {
            ShellExecuteW(hwnd_, L"open", L"https://portal.veld.network/", nullptr, nullptr,
                          SW_SHOWNORMAL);
        } else if (page_ == Page::Settings && PtInRect(&copy_monitor_code_button_, p)) {
            std::wstring code;
            {
                std::lock_guard<std::mutex> lock(remote_monitor_mutex_);
            ...
            if (!code.empty())
                CopyWideText(hwnd_, code);
        } else if (page_ == Page::Settings && PtInRect(&reset_monitor_pairing_button_, p) &&
        ...
        } else if (page_ == Page::Settings && PtInRect(&auto_update_toggle_, p)) {
        ...
        } else if (page_ == Page::Settings && PtInRect(&check_update_button_, p)) {
            if (update_operation_.load() == UpdateOperation::None) {
        ...
        } else if (page_ == Page::Settings && PtInRect(&view_changelog_button_, p)) {
            OpenChangelog();
        } else if (page_ == Page::Settings && PtInRect(&open_data_button_, p)) {
            if (std::filesystem::exists(data_dir_))
                ShellExecuteW(hwnd_, L"open", data_dir_.c_str(), nullptr, nullptr, SW_SHOWNORMAL);
        } else if (page_ == Page::Settings && PtInRect(&import_key_button_, p)) {
            const auto live = SnapshotState();
            if (!address_only_ && !live.process_running && !tor_preparing_.load())
                ImportKeyfile();
        } else if (page_ == Page::Settings && PtInRect(&create_key_button_, p)) {
            const auto live = SnapshotState();
            if (!address_only_ && !live.process_running && !tor_preparing_.load() &&
                !std::filesystem::exists(data_dir_ / L"miner.key"))
                CreateIdentity();
    ...
    void ChangeSoloAddress() {
        PassphrasePrompt request{L"Solo payout address",
                                 L"Paste the address that should receive your solo mining rewards.",
                                 Utf8ToWide(AddressPayout())};
        request.secret = false;
        if (DialogBoxParamW(instance_, MAKEINTRESOURCEW(IDD_VELD_PASSPHRASE), hwnd_,
                            PassphraseDialogProc, reinterpret_cast<LPARAM>(&request)) != IDOK)
            return;
        const auto address = WideToUtf8(request.value);
        if (!ValidSoloAddress(address)) {
            MessageBoxW(hwnd_, L"Enter a valid Veld payout address for this network.",
                        L"Invalid payout address", MB_OK | MB_ICONWARNING);
            return;
        }
        const auto confirmation =
            L"Send solo mining rewards to:\n\n" + request.value +
        ...
        if (MessageBoxW(hwnd_, confirmation.c_str(), L"Confirm payout address",
                        MB_OKCANCEL | MB_ICONINFORMATION) != IDOK)
            return;
        const auto previous = AddressPayout();
        const bool old_mode = address_only_.load();
        SetAddressPayout(address);
        address_only_ = true;
        if (!SaveSettings()) {
            SetAddressPayout(previous);
            address_only_ = old_mode;
            return;
        }
        ClearSessionPassphrase();
        session_unlock_confirmed_ = false;
    ...
    bool AskPassphrase(const std::wstring& title, const std::wstring& prompt, std::wstring& value) {
        PassphrasePrompt request{title, prompt, {}};
        const INT_PTR result =
            DialogBoxParamW(instance_, MAKEINTRESOURCEW(IDD_VELD_PASSPHRASE), hwnd_,
                            PassphraseDialogProc, reinterpret_cast<LPARAM>(&request));
        if (result != IDOK || request.value.empty()) {
            if (!request.value.empty())
                SecureZeroMemory(request.value.data(), request.value.size() * sizeof(wchar_t));
            return false;
        }
    ...
    std::string RemoteIdentityFingerprint() const {
        if (address_only_) {
            const auto address = AddressPayout();
            return ValidSoloAddress(address)
                       ? veld::node_gui::PortalDigest("address-only|" + address)
                       : std::string{};
        }
    ...
    static bool UnattendedPortalAction(const std::string& action) {
        return action == "node.start" || action == "node.stop" || action == "node.signin" ||
               action == "pool.start" || action == "pool.stop" || action == "updates.check" ||
               action == "updates.install" || action == "updates.automatic";
    }
    ...
    bool StoreSessionPassphrase(const std::wstring& passphrase) {
        ...
        const size_t protected_bytes =
            ((clear_bytes + CRYPTPROTECTMEMORY_BLOCK_SIZE - 1) / CRYPTPROTECTMEMORY_BLOCK_SIZE) *
            CRYPTPROTECTMEMORY_BLOCK_SIZE;
        std::vector<BYTE> protected_value(protected_bytes, 0);
        std::memcpy(protected_value.data(), passphrase.c_str(), clear_bytes);
        if (!CryptProtectMemory(protected_value.data(), static_cast<DWORD>(protected_value.size()),
    ...
    bool LoadSessionPassphrase(std::wstring& passphrase) {
        ...
        {
            std::lock_guard<std::mutex> lock(session_passphrase_mutex_);
            if (protected_session_passphrase_.empty() || protected_session_passphrase_chars_ == 0)
                return false;
            clear_value = protected_session_passphrase_;
            chars = protected_session_passphrase_chars_;
        }
        if (!CryptUnprotectMemory(clear_value.data(), static_cast<DWORD>(clear_value.size()),
                                  CRYPTPROTECTMEMORY_SAME_PROCESS)) {
    ...
    std::vector<wchar_t> ChildEnvironmentWithValue(const wchar_t* key, const std::wstring& value) {
        if (!key || !*key || !wcschr(key, L'='))
            return {};
        ...
        const auto position = std::lower_bound(entries.begin(), entries.end(), key,
    ...
    std::vector<wchar_t> ChildEnvironmentWithPassphrase(const std::wstring& passphrase) {
        return ChildEnvironmentWithValue(L"VELD_VAULT_PASSPHRASE=", passphrase);
    }

    std::vector<wchar_t> ChildEnvironmentWithSignerToken(const std::string& token) {
        return ChildEnvironmentWithValue(L"VELD_LOCAL_SIGNER_TOKEN=", Utf8ToWide(token));
    }
    ...
    bool StopProcessForUpdate(DWORD pid) {
        ...
        const std::wstring actual = std::filesystem::path(image_path).lexically_normal().wstring();
        ...
        DWORD wait_result = signaled ? WaitForSingleObject(process, 25000) : WAIT_TIMEOUT;
        ...
        if (wait_result == WAIT_TIMEOUT && TerminateProcess(process, ERROR_PROCESS_ABORTED)) {
    ...
    std::filesystem::path UpdateResumePath() const {
        ...
        return state_dir_ /
               (L"update-resume-" + Utf8ToWide(veld::node_gui::PortalDigest(WideToUtf8(context))) +
                L".dat");
    }
    ...
    bool PrepareUpdateResume() {
        ...
        if (!HasSessionUnlock() || !session_unlock_confirmed_.load() ||
            !LoadSessionPassphrase(resume.passphrase))
            return false;
        std::error_code error;
        ...
        const auto target = ReadTextBounded(
            root / L".veld-update-transaction" / L"stage" / L"SHA256SUMS.txt", 8 * 1024 * 1024);
        if (previous.empty() || target.empty())
            return false;
        ...
        return veld::node_gui::SaveUpdateResume(
            UpdateResumePath(), veld::node_gui::UpdateInstallContext(root), resume, resume.created);
    }

    void LoadUpdateResume() {
        veld::node_gui::UpdateResume resume;
        if (!veld::node_gui::ConsumeUpdateResume(
                UpdateResumePath(), veld::node_gui::UpdateInstallContext(InstallRoot()), resume,
        ...
        if (manifest.empty() || !parser.Parse(value, error))
            return;
        const auto* status = value.Get("status");
        WIN32_FILE_ATTRIBUTE_DATA attributes{};
        if (!status || status->kind != veld::btc_buy::JsonValue::Kind::String ||
            !GetFileAttributesExW(receipt_path.c_str(), GetFileExInfoStandard, &attributes))
            return;
        ...
        if (written < resume.created ||
            !((status->text == "installed" && digest == resume.target_manifest) ||
              (status->text == "failed" && digest == resume.previous_manifest)))
            return;
        std::error_code ec;
        if (std::filesystem::exists(root / L".veld-update-transaction", ec) || ec ||
            settings_load_failed_)
            return;
        ...
        if (ec)
            return;
        const auto identity =
            address_only_ ? (ValidSoloAddress(AddressPayout()) ? "address-only|" + AddressPayout()
                                                               : std::string{})
                          : ReadTextBounded(restored / L"miner.key", 65536);
        if (identity.empty() ||
            Utf8ToWide(veld::node_gui::PortalDigest(identity)) != resume.identity)
            return;
        if (!StoreSessionPassphrase(resume.passphrase))
            return;
        data_dir_ = restored;
    ...
    std::wstring StartupCredentialTarget() const {
        ...
        if (error || data.empty() || install.empty())
            return {};
        CharLowerBuffW(data.data(), static_cast<DWORD>(data.size()));
        return L"Veld/StartupUnlock/v1/" +
               Utf8ToWide(veld::node_gui::PortalDigest(WideToUtf8(install + L"|" + data)));
    }
    ...
    void ChangeWindowsStartup() {
        ...
        if (!veld::node_gui::ReadStartupCommand(previous) || command.empty() ||
            ...
            MessageBoxW(
                hwnd_,
                L"Windows startup could not be saved. Check installation permissions and path length.",
                L"Windows startup", MB_OK | MB_ICONERROR);
            return;
        }
        ...
        if (!SaveSettings()) {
            ...
            if (!veld::node_gui::WriteStartupCommand(previous))
                MessageBoxW(
                    hwnd_, L"Windows startup needs attention. Review Veld in Windows Startup apps.",
                    L"Windows startup", MB_OK | MB_ICONERROR);
        }
    }
    ...
    void ChangeStartupUnlock() {
        if (startup_unlock_enabled_) {
            if (!veld::node_gui::RemoveStartupUnlock(StartupCredentialTarget())) {
                MessageBoxW(hwnd_,
                            L"Windows could not remove the saved startup unlock. Please try again.",
                            L"Startup unlock", MB_OK | MB_ICONERROR);
                return;
            }
            startup_unlock_enabled_ = false;
            SaveSettings();
        ...
        if (address_only_ || !startup_enabled_ ||
            (startup_mode_ != veld::node_gui::StartupMode::Solo &&
             startup_mode_ != veld::node_gui::StartupMode::Node))
            return;
        ...
        if (!session_unlock_confirmed_.load() || !LoadSessionPassphrase(secret.passphrase)) {
            MessageBoxW(
                hwnd_,
                L"Start your node and unlock it once, then enable this option. Pool mining does not need a saved unlock.",
                L"Startup unlock", MB_OK | MB_ICONINFORMATION);
            return;
        }
        if (MessageBoxW(
                hwnd_,
                L"Save this node's unlock in Windows Credential Manager for automatic startup? Programs running as your Windows account can use this saved unlock. Turning this option off removes it.",
                L"Remember startup unlock", MB_YESNO | MB_DEFBUTTON2 | MB_ICONQUESTION) != IDYES)
            return;
        if (!veld::node_gui::SaveStartupUnlock(StartupCredentialTarget(),
                                               Utf8ToWide(RemoteIdentityFingerprint()),
                                               secret.passphrase)) {
            MessageBoxW(
                hwnd_,
                L"Windows could not save the startup unlock. Veld will ask you to sign in at startup.",
                L"Startup unlock", MB_OK | MB_ICONERROR);
            return;
        }
        startup_unlock_enabled_ = true;
        if (!SaveSettings()) {
            startup_unlock_enabled_ = false;
            veld::node_gui::RemoveStartupUnlock(StartupCredentialTarget());
        }
    }

    void TickWindowsStartup() {
        if (startup_handled_ || !pool_panel_)
            return;
        startup_handled_ = true;
        if (!windows_startup_invocation_ || !startup_enabled_ || settings_load_failed_)
            return;
        std::wstring registered;
        const auto expected = veld::node_gui::StartupCommand(ModulePath(), data_dir_);
        if (expected.empty() || !veld::node_gui::ReadStartupCommand(registered) ||
            registered != expected)
            return;
        if (startup_mode_ == veld::node_gui::StartupMode::App)
            return;
        if (startup_mode_ == veld::node_gui::StartupMode::Pool) {
            page_ = Page::Pool;
            std::string error;
            if (!pool_panel_->RemoteStart(error))
                MessageBoxW(hwnd_, Utf8ToWide(error).c_str(), L"Pool startup",
                            MB_OK | MB_ICONINFORMATION);
            return;
        }
        if (SnapshotState().process_running || FindNodeProcess(node_path_))
            return;
        const bool previous = mining_enabled_.load();
        mining_enabled_ = startup_mode_ == veld::node_gui::StartupMode::Solo;
        if (!SaveSettings()) {
            mining_enabled_ = previous;
            return;
        }
        if (startup_unlock_enabled_ && !address_only_) {
            veld::node_gui::UpdateResume secret;
            if (veld::node_gui::ReadStartupUnlock(StartupCredentialTarget(),
                                                  Utf8ToWide(RemoteIdentityFingerprint()),
                                                  secret.passphrase))
    ...
    void TickAutomaticUpdates(
        ...
        if (update_operation_.load() != UpdateOperation::None || tor_preparing_.load() ||
            stopping_node_.load())
            return;
        if (update_resume_pending_) {
            update_resume_pending_ = false;
            if (HasSessionUnlock() && FindNodeProcess(node_path_) == 0)
                StartNode();
        }
        if (!auto_update_enabled_)
            return;
        if (!automatic_install_pending_ && now < next_auto_update_)
            return;
        next_auto_update_ = now + AUTO_UPDATE_INTERVAL;
        if (automatic_install_pending_) {
            automatic_install_pending_ = false;
            if (FindNodeProcess(node_path_) != 0 &&
    ...
    bool BeginUpdateProcess(UpdateOperation operation) {
        ...
        if (installing && !SaveSettings(!automatic_update_operation_))
            return false;
        std::error_code ec;
        std::filesystem::create_directories(state_dir_, ec);
        ...
        const auto output_path =
            state_dir_ / (installing ? L"update-install.log" : L"update-check.log");
        HANDLE output =
            CreateFileW(output_path.c_str(), GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE,
        ...
        if (output == INVALID_HANDLE_VALUE || nul == INVALID_HANDLE_VALUE) {
            ...
            if (nul != INVALID_HANDLE_VALUE)
                CloseHandle(nul);
            update_status_ =
                installing ? L"Update install could not start" : L"Update check could not start";
            update_status_color_ = RGB(231, 126, 126);
            return false;
        }
        ...
        const auto restart_directory =
            std::filesystem::weakly_canonical(data_dir_, directory_error);
        auto environment =
            ChildEnvironmentWithValue(L"VELD_UPDATE_NODE_DATA_DIR=", restart_directory.wstring());
        ...
        const BOOL created =
            CreateProcessW(powershell.c_str(), mutable_command.data(), nullptr, nullptr, TRUE,
        ...
        update_status_ = installing ? L"Downloading and verifying signed update…"
                                    : L"Checking signed release feed…";
        update_status_color_ = C_SUBTEXT;
        worker_cv_.notify_all();
        InvalidateRect(hwnd_, nullptr, FALSE);
    ...
    void OnUpdateProcessComplete(DWORD exit_code) {
        const UpdateOperation operation = update_operation_.exchange(UpdateOperation::None);
        ...
        if (operation == UpdateOperation::Install) {
            ...
            if (exit_code == 4) {
                ...
                if (FindNodeProcess(node_path_) != 0 && !PrepareUpdateResume()) {
                    update_status_ =
                        L"Update postponed · sign in to preserve mining through restart";
                    update_status_color_ = C_WARN;
                    return;
                }
                ...
                const std::string output =
                    ReadTextBounded(state_dir_ / L"update-install.log", 128U * 1024U);
                std::string detail;
                ...
                for (const char* marker : {"[update] FAILED:", "[update] ABORT:"}) {
                    ...
                    if (at != std::string::npos &&
                        ...
                        last_marker = at;
                        const size_t begin =
                            output.find_first_not_of(" \t", at + std::strlen(marker));
                        if (begin != std::string::npos)
                            detail =
                                output.substr(begin, output.find_first_of("\r\n", begin) - begin);
                    }
                }
                if (detail.empty())
                    detail =
                        "The update helper exited with code " + std::to_string(exit_code) + ".";
                if (detail.size() > 1200)
                    detail.resize(1200);
                ...
                if (!automatic)
                    MessageBoxW(hwnd_, message.c_str(), L"Veld update could not finish",
                                MB_OK | MB_ICONERROR);
            }
            return;
        }

        const std::string output = ReadTextBounded(state_dir_ / L"update-check.log", 128U * 1024U);
        if (exit_code == 0) {
            update_available_ = false;
            update_status_ =
                L"Latest signed release verified · " + Utf8ToWide(veld::CLIENT_VERSION);
            update_status_color_ = C_TEXT;
        } else if (exit_code == 2) {
            std::string version;
    ...
    void OpenChangelog() {
        ...
        if (std::filesystem::is_regular_file(local)) {
            ShellExecuteW(hwnd_, L"open", local.c_str(), nullptr, local.parent_path().c_str(),
                          SW_SHOWNORMAL);
        } else {
            ShellExecuteW(hwnd_, L"open", L"https://veld.network/downloads/CHANGES.txt", nullptr,
    ...
    bool OpenChildLogHandles(HANDLE& log, HANDLE& nul) {
        ...
        inherit.nLength = sizeof(inherit);
        inherit.bInheritHandle = TRUE;
        log = CreateFileW(LogPath().c_str(), FILE_APPEND_DATA, FILE_SHARE_READ | FILE_SHARE_WRITE,
    ...
    void BeginTorSetup() {
        const auto install_root = node_path_.parent_path().filename() == L"bin"
                                      ? node_path_.parent_path().parent_path()
                                      : node_path_.parent_path();
        const auto helper = install_root / L"tor-setup.ps1";
        if (!std::filesystem::is_regular_file(helper)) {
            MessageBoxW(hwnd_, L"The signed Tor setup helper is missing from this package.",
                        L"Veld Node", MB_OK | MB_ICONERROR);
            return;
        }
        const auto powershell = SystemPowerShellPath();
        if (powershell.empty()) {
            MessageBoxW(hwnd_, L"Windows PowerShell is unavailable.", L"Veld Node",
                        MB_OK | MB_ICONERROR);
            return;
        }
        ...
        if (!OpenChildLogHandles(log, nul)) {
            MessageBoxW(hwnd_, L"The local app log could not be opened.", L"Veld Node",
                        MB_OK | MB_ICONERROR);
            return;
        }
        std::wstring command = L"\"" + powershell.wstring() +
        ...
        CloseHandle(nul);
        if (!created) {
            MessageBoxW(hwnd_, L"Tor preparation could not be started.", L"Veld Node",
                        MB_OK | MB_ICONERROR);
            return;
        }
        CloseHandle(pi.hThread);
    ...
    bool RunProtectedNodeUtility(const std::wstring& arguments, std::wstring& passphrase) {
        ...
        if (!OpenChildLogHandles(log, nul))
            return false;
        std::wstring command = L"\"" + node_path_.wstring() + L"\" " + arguments;
        std::vector<wchar_t> mutable_command(command.begin(), command.end());
        mutable_command.push_back(L'\0');
        ...
        si.hStdError = log;
        PROCESS_INFORMATION pi{};
        HANDLE verified_node = OpenVerifiedTrustedFile(node_path_, VELD_TRUSTED_NODE_SHA256);
        ...
        const BOOL created =
            CreateProcessW(node_path_.c_str(), mutable_command.data(), nullptr, nullptr, TRUE,
                           CREATE_NO_WINDOW | CREATE_UNICODE_ENVIRONMENT, environment.data(),
    ...
    void CreateIdentity() {
        if (std::filesystem::exists(data_dir_ / L"miner.key"))
            return;
        ...
        if (!AskPassphrase(L"Create mining identity",
                           ...
                           passphrase))
            return;
        if (!AskPassphrase(L"Confirm mining identity", L"Enter the same passphrase again.",
                           confirmation)) {
            SecureZeroMemory(passphrase.data(), passphrase.size() * sizeof(wchar_t));
            return;
        }
        const bool matches = passphrase.size() == confirmation.size() &&
                             std::equal(passphrase.begin(), passphrase.end(), confirmation.begin());
        SecureZeroMemory(confirmation.data(), confirmation.size() * sizeof(wchar_t));
        confirmation.clear();
        if (!matches) {
            SecureZeroMemory(passphrase.data(), passphrase.size() * sizeof(wchar_t));
            MessageBoxW(hwnd_, L"The passphrases did not match.", L"Veld Node",
        ...
        const bool policy_ok =
            veld::wallet_crypto::ValidateNewPassphrase(passphrase_utf8, &policy_error);
        SecureZeroMemory(passphrase_utf8.data(), passphrase_utf8.size());
        passphrase_utf8.clear();
        if (!policy_ok) {
            SecureZeroMemory(passphrase.data(), passphrase.size() * sizeof(wchar_t));
            passphrase.clear();
            MessageBoxW(hwnd_, Utf8ToWide(policy_error).c_str(), L"Veld Node",
                        MB_OK | MB_ICONWARNING);
            return;
        }
        const std::wstring arguments =
            L"--datadir \"" + data_dir_.wstring() + L"\" --create-miner-key";
    ...
    void ImportKeyfile() {
        ...
        if (!AskPassphrase(L"Import Veld keyfile",
                           L"Enter the passphrase for the selected .veld-keys file.", passphrase))
            return;
    ...
    void OpenTrustedWallet() {
        const auto opened = reinterpret_cast<INT_PTR>(ShellExecuteW(
            hwnd_, L"open", L"https://wallet.veld.network/", nullptr, nullptr, SW_SHOWNORMAL));
        if (opened <= 32) {
            MessageBoxW(
                hwnd_,
    ...
    void ToggleNode() {
        if (tor_preparing_.load() || stopping_node_.load() ||
            update_operation_.load() != UpdateOperation::None)
            return;
        const LiveState live = SnapshotState();
    ...
    void StartNode() {
#ifdef VELD_POOL_GUI_QUALIFICATION
        MessageBoxW(hwnd_, L"This isolated test bundle only runs the pool worker.",
                    L"Pool qualification", MB_OK);
        return;
#endif
        if (pool_panel_ && pool_panel_->Running()) {
            MessageBoxW(hwnd_, L"Stop pool mining before starting solo mining.", L"Veld Node",
                        MB_OK | MB_ICONINFORMATION);
            return;
        }
        ...
        if (address_only_ && !ValidSoloAddress(AddressPayout())) {
            ...
            MessageBoxW(
                hwnd_,
                L"Set a valid payout address in Settings before starting address-only mining.",
    ...
    void ObserveOwnedNodeExit() {
        ...
        if (stopping_process_ && WaitForSingleObject(stopping_process_, 0) == WAIT_OBJECT_0) {
            CloseHandle(stopping_process_);
        ...
        const auto action =
            veld::node_client::ClassifyExit(code, operator_stop, recovery_restarts_);
    ...
    std::wstring NodeStartCommand() const {
        ...
        if (address_only_) {
            ...
            command += (mining_enabled_ ? L" --mine" : L" --endorse");
    ...
    void StartNodeEngine() {
        if (!std::filesystem::is_regular_file(node_path_)) {
            const std::wstring message =
                L"The signed node binary was not found beside this app:\n\n" + node_path_.wstring();
            MessageBoxW(hwnd_, message.c_str(), L"Veld Node", MB_OK | MB_ICONERROR);
            return;
        }
        std::error_code ec;
        std::filesystem::create_directories(data_dir_, ec);
        if (ec) {
            MessageBoxW(hwnd_, L"The Veld data directory could not be created.", L"Veld Node",
                        MB_OK | MB_ICONERROR);
            return;
        }
        HANDLE log = INVALID_HANDLE_VALUE;
        ...
        if (address_only_) {
            ...
            if (!veld::mining::AddressOnlyRpcSecret(data_dir_, local_secret, &error)) {
                CloseHandle(log);
                CloseHandle(nul);
                MessageBoxW(hwnd_, Utf8ToWide(error).c_str(), L"Local node access",
                            MB_OK | MB_ICONERROR);
                return;
            }
            if (!passphrase.empty())
                SecureZeroMemory(passphrase.data(), passphrase.size() * sizeof(wchar_t));
            passphrase = Utf8ToWide(local_secret);
            veld::WipeString(local_secret);
        ...
        if (!reused_session_unlock &&
            !AskPassphrase(L"Unlock node identity", L"Enter the passphrase for this identity.",
                           passphrase)) {
            CloseHandle(log);
            CloseHandle(nul);
            return;
        ...
        if (environment.empty()) {
            ...
            CloseHandle(log);
            CloseHandle(nul);
            MessageBoxW(hwnd_, L"A child-process environment could not be created.", L"Veld Node",
                        MB_OK | MB_ICONERROR);
            return;
        }
        ...
        CloseHandle(nul);
        if (!created) {
            MessageBoxW(hwnd_, L"The node process could not be started.", L"Veld Node",
                        MB_OK | MB_ICONERROR);
            return;
        }
        CloseHandle(pi.hThread);
    ...
    std::wstring RemoteCommandDescription(const MonitoringReply::Command& command) const {
        if (command.action == "node.signin")
            return L"Sign in to this node and start mining";
        if (command.action == "node.start")
            return L"Start the Veld node";
        if (command.action == "node.stop")
            return L"Stop the Veld node gracefully";
        ...
        if (command.action == "mining.enabled")
            return command.enabled ? L"Enable CPU mining for the next app-managed start"
                                   : L"Disable CPU mining for the next app-managed start";
        if (command.action == "mining.workers")
            return L"Set the CPU worker count to " + std::to_wstring(command.workers);
        if (command.action == "display.reference")
            return command.enabled ? L"Show the public height reference"
                                   : L"Hide the public height reference";
        if (command.action == "privacy.tor")
            return command.enabled ? L"Use Tor-only peer transport on the next start"
                                   : L"Disable Tor-only peer transport on the next start";
        if (command.action == "network.reachable")
            return command.enabled ? L"Attempt inbound P2P reachability on the next start"
    ...
    bool ConfirmRemoteCommand(const MonitoringReply::Command& command) const {
        ...
        if (action.empty())
            return false;
        std::wstring message =
            L"The Veld Portal requests this exact action:\n\n" + action + L"\n\n";
        message += L"The command is signed and time-limited. No action will be "
                   L"taken unless you choose Yes.";
        return MessageBoxW(hwnd_, message.c_str(), L"Approve Veld Portal action",
    ...
    void LoadRemoteTrust() {
        ...
        portal_trust_load_failed_ = error || (exists && !LoadPortalTrust(path, remote_trust_));
        error.clear();
        const bool reset_pending =
            std::filesystem::exists(state_dir_ / L"remote-pair-reset.dat", error);
    ...
    bool CompleteRemotePairReset() {
        ...
        if (error || (exists && !DeleteFileW(trust_path.c_str())))
            return false;
        if (!DeleteFileW((state_dir_ / L"remote-pair-reset.dat").c_str()))
            return false;
        remote_trust_ = {};
    ...
    bool EnrollPortalControl(const MonitoringReply& reply, std::string& rejection) {
        ...
        if (!EvaluatePortalPairing(reply, remote_trust_, enrolled, rejection))
            return false;
    ...
    bool AuthorizeRemoteCommand(const MonitoringReply::Command& command, std::string& rejection) {
        if (!remote_monitoring_enabled_.load() || portal_pair_reset_requested_.load()) {
            rejection = "Remote access is disabled or pairing is being reset";
            return false;
        }
        ...
        const PortalCommandTrustVerdict verdict = EvaluatePortalCommandTrust(
            command, current, static_cast<uint64_t>(std::time(nullptr)), next, rejection);
        if (verdict == PortalCommandTrustVerdict::Reject)
            return false;
        ...
        if (!remote_monitoring_enabled_.load() || portal_pair_reset_requested_.load() ||
            !portal_control_ready_.load() || portal_trust_load_failed_ ||
            !remote_trust_.unattended ||
            EvaluatePortalCommandTrust(command, remote_trust_,
                                       static_cast<uint64_t>(std::time(nullptr)), next,
                                       rejection) != PortalCommandTrustVerdict::ExistingPairing) {
            if (rejection.empty())
                rejection = "Remote access changed while approval was pending";
            return false;
        }
    ...
    void ExecuteRemoteCommand() {
        ...
        if (command.action == "pool.start" || command.action == "pool.stop") {
            if (!pool_panel_)
                reject("Pool controls are unavailable in this client");
                ...
                if (starting ? pool_panel_->RemoteStart(failure)
                             : pool_panel_->Stop(true, &failure))
                    ...
                    reject(failure.empty() ? "Pool command could not be completed" : failure);
                pool_panel_->Tick();
                std::lock_guard<std::mutex> lock(pool_monitor_mutex_);
                pool_monitor_json_ = pool_panel_->MonitoringJson();
            }
            ...
            if (pool_panel_ && pool_panel_->Running()) {
                ...
                if (command.action == "node.signin" && !address_only_) {
                    std::wstring passphrase;
                    ready =
                        remote_unlock_key_.Decrypt(command.unlock, command.device_id, command.nonce,
                                                   RemoteIdentityFingerprint(), passphrase) &&
                ...
                if (!ready) {
                    reject(command.action == "node.signin"
                               ? "Encrypted sign-in could not be verified. Refresh and retry."
                               : "This node is locked. Use Sign in & mine in the portal.");
                } else {
                    StartNode();
                    if (owned_pid_.load() != 0 || tor_preparing_.load())
                        complete(
                            "Node start requested; mining begins after identity and sync checks");
                    else
                        reject("Node could not start. Check its current status and logs.");
                }
            }
        } else if (command.action == "node.stop") {
            if (!live.process_running)
                complete("Node is already stopped");
            else if (tor_preparing_.load() || update_operation_.load() != UpdateOperation::None) {
            ...
            if (update_operation_.load() != UpdateOperation::None) {
                ...
                if (SaveSettings(false)) {
                    automatic_install_pending_ = false;
                    next_auto_update_ = std::chrono::steady_clock::now() + std::chrono::seconds(5);
                    complete(command.enabled ? "Automatic signed updates enabled"
            ...
            if (SaveSettings())
                ...
                reject("Mining preference could not be saved");
            ...
            if (SaveSettings())
                ...
                reject("Worker count could not be saved");
            ...
            if (saved)
                ...
                reject("Public height display preference could not be saved");
            ...
            if (live.process_running || tor_preparing_.load())
                ...
                if (SaveSettings())
                    ...
                    reject("Tor preference could not be saved");
            }
            ...
            if (live.process_running || tor_choice_ || tor_preparing_.load())
                ...
                if (SaveSettings())
                    ...
                    reject("Inbound reachability preference could not be saved");
            }
            ...
            if (address_only_ && command.mode != "full")
                ...
                if (SaveSettings())
                    ...
                    reject("Synchronization preference could not be saved");
            }
        } else {
    ...
    std::string EnsureMonitoringToken(std::string& cached) {
        ...
        if (std::filesystem::exists(path, credential_error) || credential_error)
            return {};
    ...
    std::string BuildMonitoringReport(const LiveState& live, uint64_t& included_ack) {
        std::string pool_monitor_json;
        {
            std::lock_guard<std::mutex> lock(pool_monitor_mutex_);
            pool_monitor_json = pool_monitor_json_;
        }
        const uint64_t height = live.local_online ? live.local.height : 0;
        const uint64_t reference_height = live.reference_online ? live.reference.height : height;
        const uint64_t sync_lag = reference_height > height ? reference_height - height : 0;
        ...
        const double hashrate = live.mining_status_online && std::isfinite(live.mining.hashrate) &&
                                        live.mining.hashrate >= 0.0
                                    ? live.mining.hashrate
                                    : 0.0;
        ...
        if (live.process_running) {
            if (!live.local_online || !live.mining_status_online) {
                ...
                state = "Mining";
            } else if (sync_lag > 0 || live.mining.work_state.find("sync") != std::string::npos) {
                state = "Syncing";
            } else {
                state = "Paused";
            }
        ...
        const size_t topology_node_count =
            std::min<size_t>(64, live.topology_online ? live.topology.nodes.size() : 0);
        ...
        if (!address_only_ && !identity.empty() &&
            remote_unlock_key_.Ensure(state_dir_ / L"remote-unlock.dat"))
            unlock_json = remote_unlock_key_.PublicJson(identity);
        bool control_ready = false;
        {
            std::lock_guard<std::mutex> lock(remote_trust_mutex_);
            control_ready =
                portal_control_ready_.load() && remote_trust_.Valid() && remote_trust_.unattended;
        }
        std::ostringstream json;
        json << '{' << "\"portal_protocol\":4"
             << ",\"name\":\"" << JsonEscape(WideToUtf8(MonitoringMachineName()))
             << "\",\"version\":\"" << JsonEscape(veld::CLIENT_VERSION)
             << "\",\"height\":" << height << ",\"sync_lag\":" << sync_lag
             << ",\"hashrate\":" << std::fixed << std::setprecision(3) << hashrate
             << ",\"workers\":" << (live.mining_status_online ? live.mining.threads : 0)
             << ",\"peers\":" << peers << ",\"inbound\":" << live.inbound_peers
             << ",\"blocks\":" << (live.mining_status_online ? live.mining.blocks_mined_session : 0)
             << ",\"mining_state\":\"" << state << "\",\"warning\":\"" << JsonEscape(warning)
             << "\""
             << ",\"snapshot\":{\"remote_control\":" << (control_ready ? "true" : "false")
             << ",\"pool_remote_control\":true"
             << ",\"pool\":" << pool_monitor_json << ",\"pairing_control\":true"
             << ",\"automatic_updates\":" << (auto_update_enabled_.load() ? "true" : "false")
             << ",\"identity_unlocked\":"
             << (!address_only_ && HasSessionUnlock() ? "true" : "false")
             << ",\"unlock_key\":" << unlock_json << ",\"diagnostics\":{\"schema\":1"
             ...
             << ",\"status_updated_at\":"
             << (live.mining_status_online ? std::to_string(live.mining.updated_at) : "null")
             << ",\"process_running\":" << (live.process_running ? "true" : "false")
             << ",\"last_exit_at\":" << last_node_exit_at_.load() << ",\"last_exit_code\":"
             << (last_node_exit_at_.load() ? std::to_string(last_node_exit_code_.load()) : "null")
             << ",\"work_state\":"
             << (live.mining_status_online ? "\"" + JsonEscape(live.mining.work_state) + "\""
                                           : "null")
             << ",\"total_hashes\":"
             << (live.mining_status_online ? std::to_string(live.mining.total_hashes) : "null")
             << ",\"mining_ready\":"
             << (live.mining_status_online ? (live.mining.mining_ready ? "true" : "false") : "null")
             << ",\"daemon\":";
        if (live.mining_status_online && live.mining.daemon_identity_known &&
            ...
            json << "{\"version\":\"" << JsonEscape(d.daemon_version) << "\",\"profile\":\""
                 << JsonEscape(d.daemon_profile) << "\",\"executable_sha256\":\""
                 << live.daemon_executable_sha256
        ...
        json << "},"
             << "\"process_running\":" << (live.process_running ? "true" : "false")
             << ",\"snapshot_eligible\":" << (live.snapshot_eligible ? "true" : "false")
             << ",\"full_ibd\":"
             << ((live.mining_status_online ? live.mining.full_ibd : full_ibd_choice_.load())
                     ? "true"
                     : "false")
             ...
             << ",\"mining_active\":"
             << (live.mining_status_online && live.mining.mining_active ? "true" : "false")
             << ",\"mining_ready\":"
             << (live.mining_status_online && live.mining.mining_ready ? "true" : "false")
             ...
             << ",\"chain_bytes\":" << live.chain_bytes << ",\"outbound\":" << live.outbound_peers
             ...
             << ",\"configured_workers\":" << mining_thread_count_ << ",\"peer_roles\":{"
             << "\"fleet\":" << peer_roles["fleet"] << ",\"node\":" << peer_roles["node"]
             << ",\"miner\":" << peer_roles["miner"] << ",\"validator\":" << peer_roles["validator"]
             << ",\"unknown\":" << peer_roles["unknown"] << "}"
             << ",\"topology\":{"
             << "\"generated_at\":" << (live.topology_online ? live.topology.generated_at : 0)
             << ",\"reporting_nodes\":"
             << (live.topology_online ? live.topology.reporting_nodes : 0)
             << ",\"eligible_nodes\":" << (live.topology_online ? live.topology.eligible_nodes : 0)
             << ",\"local_id\":" << (live.topology_online ? live.local_topology_id : 0)
             << ",\"nodes\":[";
        for (size_t i = 0; i < topology_node_count; ++i) {
            const auto& node = live.topology.nodes[i];
            if (i != 0)
                json << ',';
            json << "{\"id\":" << node.anonymous_id << ",\"role\":\"" << JsonEscape(node.role)
                 << "\",\"role_index\":" << node.role_index << ",\"tip_state\":\""
                 << JsonEscape(node.tip_state) << "\",\"updated_at\":" << node.updated_at << '}';
        }
        json << "],\"edges\":[";
        size_t topology_edge_count = 0;
        for (const auto& edge : live.topology.edges) {
            if (topology_edge_count >= 192 || topology_ids.find(edge.first) == topology_ids.end() ||
                topology_ids.find(edge.second) == topology_ids.end())
                continue;
            if (topology_edge_count != 0)
                json << ',';
            json << "{\"first\":" << edge.first << ",\"second\":" << edge.second
                 << ",\"confirmed\":" << (edge.confirmed ? "true" : "false") << '}';
            ++topology_edge_count;
        }
        json << "]}"
             << ",\"recent_blocks\":[";
        const size_t first_block =
            live.recent_blocks.size() > 8 ? live.recent_blocks.size() - 8 : 0;
        for (size_t i = first_block; i < live.recent_blocks.size(); ++i) {
            const auto& block = live.recent_blocks[i];
            if (i != first_block)
                json << ',';
            json << "{\"height\":" << block.height << ",\"timestamp\":" << block.timestamp
                 << ",\"tx_count\":" << block.transaction_count << ",\"reward\":" << std::fixed
                 << std::setprecision(8) << block.reward_veld << ",\"hash\":\""
                 << JsonEscape(block.hash) << "\",\"winner\":\"" << JsonEscape(block.winner)
                 << "\"}";
        }
        json << "],\"events\":[";
        const size_t first_event =
            live.network_events.size() > 20 ? live.network_events.size() - 20 : 0;
        ...
        json << "]}";
        if (included_ack != 0) {
            json << ",\"ack_id\":" << included_ack << ",\"ack_status\":\"" << JsonEscape(ack_status)
    ...
    void ReportRemoteStatus(const LiveState& live, std::string& token) {
        ...
        const HttpResult response =
            HttpPostJson(L"portal.veld.network", INTERNET_DEFAULT_HTTPS_PORT,
        ...
        if (reply.command.present && !control_enrolled) {
            ...
            if (dispatch)
                PostMessageW(hwnd_, WM_REMOTE_COMMAND, 0, 0);
        }
        ...
        }
        {
            ...
            remote_monitor_status_.last_report = static_cast<uint64_t>(std::time(nullptr));
    ...
    bool ResetRemotePairing(std::string& token) {
        if (EnsureMonitoringToken(token).empty()) {
            ...
            remote_monitor_status_.credential_ready = false;
            remote_monitor_status_.report_ok = false;
            remote_monitor_status_.detail = L"The protected device credential could not be loaded.";
            return false;
        }
    ...
    void PollLoop() {
        ...
        while (!stop_worker_.load()) {
            ...
            {
                std::lock_guard<std::mutex> lock(update_process_mutex_);
                if (update_process_ && WaitForSingleObject(update_process_, 0) == WAIT_OBJECT_0) {
            ...
            next.uptime_seconds = next.process_running ? ProcessUptime(next.pid) : 0;
            if (next.pid != hashed_pid || next.uptime_seconds < hashed_uptime) {
                hashed_pid = next.pid;
                running_executable_hash.clear();
                HANDLE process =
                    next.pid ? OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, FALSE, next.pid)
                             : nullptr;
                wchar_t image[32768]{};
                DWORD size = static_cast<DWORD>(std::size(image));
            ...
            const HttpResult local =
                next.process_running ? HttpGetJson(L"127.0.0.1", 8080, L"/api/stats", false, 900)
                                     : HttpResult{};
            if (local.ok) {
                std::string error;
                next.local_online = veld::node_gui::ParseStats(local.body, next.local, error);
                if (!next.local_online)
                    next.local_error = Utf8ToWide(error);
            } else {
                next.local_error = local.error;
            }
            if (next.local_online &&
                (cached_blocks.empty() || cached_blocks.front().height != next.local.height)) {
                ...
                for (uint64_t offset = 0; offset < count; ++offset) {
                    ...
                    const std::wstring path = L"/api/v1/block/" + std::to_wstring(height);
                    const HttpResult block =
                        HttpGetJson(L"127.0.0.1", 8080, path.c_str(), false, 650);
                    if (!block.ok)
                        break;
                    veld::node_gui::BlockSummary parsed;
                    std::string error;
                    if (!veld::node_gui::ParseBlockSummary(block.body, parsed, error))
                        break;
                    refreshed.push_back(std::move(parsed));
                }
                if (!refreshed.empty())
                    cached_blocks = std::move(refreshed);
            }
            if (!next.process_running)
                cached_blocks.clear();
            next.recent_blocks = cached_blocks;

            const std::string mining_body = ReadTextBounded(data_dir_ / L"gui-status.json");
            bool gui_status_fresh = false;
            if (!mining_body.empty()) {
                std::string error;
                veld::node_gui::MiningStats parsed;
                if (veld::node_gui::ParseMiningStats(mining_body, parsed, error)) {
                    const uint64_t now_seconds = static_cast<uint64_t>(std::time(nullptr));
            ...
            if (gui_status_fresh) {
                ...
                std::string error;
                if (veld::node_gui::ParseGuiPeerStatus(mining_body, parsed_peers, error,
                    ...
                    if (previous_peer_sample.time_since_epoch().count() != 0) {
                        const double seconds =
                            std::chrono::duration<double>(sample_now - previous_peer_sample)
                                .count();
                        if (seconds > 0.0 && next.peer_bytes_sent >= previous_peer_bytes_sent) {
                            next.peer_send_rate = static_cast<double>(next.peer_bytes_sent -
                                                                      previous_peer_bytes_sent) /
                                                  seconds;
                        }
                        if (seconds > 0.0 && next.peer_bytes_recv >= previous_peer_bytes_recv) {
                            next.peer_recv_rate = static_cast<double>(next.peer_bytes_recv -
                                                                      previous_peer_bytes_recv) /
                                                  seconds;
                        }
                    }

                    auto add_event = [&](std::wstring text, bool warning = false) {
                        cached_network_events.push_front(
                            {static_cast<uint64_t>(std::time(nullptr)), std::move(text), warning});
                    ...
                    if (!peer_sample_initialized) {
                        add_event(L"Peer topology loaded - " + FormatUnsigned(peer_count) +
                                  L" connections");
                    } else {
                        if (peer_count > previous_peer_count) {
                            const uint64_t connected = peer_count - previous_peer_count;
                            add_event(FormatUnsigned(connected) +
                                      (connected == 1 ? L" peer connected" : L" peers connected"));
                        } else if (peer_count < previous_peer_count) {
                            const uint64_t disconnected = previous_peer_count - peer_count;
                            add_event(FormatUnsigned(disconnected) + (disconnected == 1
                                                                          ? L" peer disconnected"
                                                                          : L" peers disconnected"),
                                      peer_count == 0);
                        }
                        if (previous_exact_tip_peers == 0 && next.exact_tip_peers > 0) {
                            add_event(L"Exact peer-tip agreement restored");
                        } else if (previous_exact_tip_peers > 0 && next.exact_tip_peers == 0) {
            ...
            next.network_events.assign(cached_network_events.begin(), cached_network_events.end());
            const auto now = std::chrono::steady_clock::now();
            ...
            if (!reference_display_enabled_.load()) {
                ...
                last_reference = now;
                cached_reference_ok = false;
                const HttpResult remote =
                    HttpGetJson(L"explorer.veld.network", INTERNET_DEFAULT_HTTPS_PORT,
            ...
            if (last_topology.time_since_epoch().count() == 0 ||
                ...
                if (remote.ok) {
                    ...
                    if (veld::node_gui::ParseTopologySnapshot(remote.body, parsed, error)) {
                        const uint64_t now_seconds = static_cast<uint64_t>(std::time(nullptr));
...

            {
                ...
                if (next.mining_status_online) {
                    ...
                    while (rate_history_.size() > 2400)
                        rate_history_.pop_front();
                }
            }
            const DWORD managed_pid = owned_pid_.load();
            if (managed_pid != 0 && next.pid == managed_pid && next.process_running &&
                next.local_online)
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int show) {
    ...
    if (!VerifySignedPackage(ModulePath(), package_error)) {
        ...
        MessageBoxW(nullptr, message.c_str(), L"Veld package blocked", MB_OK | MB_ICONERROR);
    ...
    if (wcsstr(GetCommandLineW(), L"--qa-passphrase") != nullptr) {
        PassphrasePrompt request{
            L"Unlock node identity", L"Enter the passphrase for this identity.", {}};
        DialogBoxParamW(instance, MAKEINTRESOURCEW(IDD_VELD_PASSPHRASE), nullptr,
                        PassphraseDialogProc, reinterpret_cast<LPARAM>(&request));
        SecureZeroMemory(request.value.data(), request.value.size() * sizeof(wchar_t));
        return 0;
    }
    ...
    if (!singleton || GetLastError() == ERROR_ALREADY_EXISTS) {
        if (!singleton || !veld::node_gui::RestoreExistingWindow())
            MessageBoxW(nullptr,
                        L"Veld Node is starting or is not responding. "
                        L"Try opening it again in a moment.",
                        L"Veld Node", MB_OK | MB_ICONINFORMATION);
        if (singleton)
