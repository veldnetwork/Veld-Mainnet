...
static int CmdNew(const std::string& out_path, bool testnet, LockedSeed* seed_opt,
                  SensitiveString& pass, bool sha384 = false) {
    ...
    auto addr =
        sha384 ? veld::PubKeyToSha384Address(pub, testnet) : veld::PubKeyToAddress(pub, testnet);
...
static std::vector<uint8_t> ReadSigningInput(const std::string& path, size_t maximum, bool& ok) {
    std::vector<uint8_t> bytes;
    ok = veld::channel::secure_file::ReadExplicitImport(path, bytes, nullptr, maximum) ==
...
static int CmdSignTx(const std::string& keyfile, const std::string& prepared,
    ...
    std::vector<uint8_t> pbytes =
        ReadSigningInput(prepared, offline_signing::kMaxPreparedJsonBytes, pok);
    if (!pok) {
        veld::compat::SecureZero(priv.data(), priv.size());
...
static int CmdDecodeMint(const std::string& issuer_p2pkh_hex, const std::string& tx_hex,
    ...
    if (inspect_backing) {
        ...
        const auto classified = btcveld::reserve::ClassifyBitcoinReserveSpend(
            reserve_context.prior_state, reserve_context.circulating_supply, claim.bitcoin_tx,
            claim.direct_parents, BtcVeldCustodySpk(),
        ...
        std::cout << "{\"version\":1,\"operation\":\""
                  ...
                  << "\",\"sats\":" << sats << ",\"recipient\":\"" << to << "\",\"issuer\":\""
                  << from << "\",\"custody_script_hex\":\"" << BytesToHex(BtcVeldCustodySpk())
...
static int CmdVerifySignedCarrierStdin() {
    ...
    for (auto& input : stripped.inputs)
        input.script_sig.clear();
    if (stripped.Serialize() != original)
        return fail("differs from the reserved template");
    ...
    std::cout << "{\"version\":1,\"genesis_hash\":\"" << HashToHex(CreateGenesisBlock().GetHash())
              << "\",\"unsigned_tx_sha256\":\"" << HashToHex(unsigned_hash) << "\",\"txid\":\""
              << HashToHex(tx.GetTxID()) << "\",\"signed_tx_sha256\":\"" << HashToHex(signed_hash)
              << "\",\"input_count\":" << tx.inputs.size() << ",\"issuer_script_hex\":\""
              << script_hex << "\"}\n";
    return 0;
}
...
static int CmdPrepareSigningStdin() {
    ...
    if (!offline_signing::ParseStringField(request, "unsigned_tx_hex", unsigned_hex) ||
        ...
        !offline_signing::ParseStringField(request, "issuer_script_hex", script_hex) ||
        !offline_signing::DecodeLowerHex(script_hex, 25, script) || script.size() != 25 ||
        script[0] != 0x76 || script[1] != 0xa9 || script[2] != 0x14 || script[23] != 0x88 ||
        script[24] != 0xac || !parents || parents->kind != btc_buy::JsonValue::Kind::Array)
        return fail("fields are invalid");
    Transaction tx;
    if (Transaction::Deserialize(raw, 0, tx) != raw.size() || tx.Serialize() != raw ||
        tx.inputs.empty() || tx.inputs.size() > 180 || tx.outputs.empty() ||
        parents->array.size() != tx.inputs.size())
        return fail("transaction is invalid");
    ...
    for (size_t i = 0; i < tx.inputs.size(); ++i) {
        ...
        if (item.kind != btc_buy::JsonValue::Kind::String || item.string_had_escape ||
            !offline_signing::DecodeLowerHex(item.text, offline_signing::kMaxParentTransactionBytes,
                                             parent_raw) ||
            ...
            tx.inputs[i].prev_out_index >= parent.outputs.size())
            return fail("parent is invalid");
        const auto& prevout = parent.outputs[tx.inputs[i].prev_out_index];
        if (prevout.script_pubkey != script || total_input > MAX_SUPPLY_UNITS ||
            prevout.value > MAX_SUPPLY_UNITS - total_input)
            return fail("parent ownership or value differs");
        total_input += prevout.value;
        if (i)
            inputs += ',';
        inputs += "{\"index\":" + std::to_string(i) + ",\"sighash_hex\":\"" +
                  BytesToHex(ComputeSighash(tx, static_cast<uint32_t>(i), script)) +
                  "\",\"prev_script_hex\":\"" + script_hex +
                  "\",\"value\":" + std::to_string(prevout.value) + ",\"parent_tx_hex\":\"" +
                  item.text + "\"}";
    }
    inputs += ']';
    for (const auto& output : tx.outputs) {
    ...
    btc_buy::StrictJsonParser prepared_parser(prepared, offline_signing::kMaxPreparedJsonBytes,
                                              true);
    offline_signing::VerifiedPrepared verified;
    std::string identity;
...
static int CmdDecodeMintStdin(bool inspect_backing = false) {
    ...
    auto text = [&](const char* name, size_t limit) -> std::string {
        ...
        if (!value || value->kind != btc_buy::JsonValue::Kind::String || value->string_had_escape ||
            value->text.empty() || value->text.size() > limit)
            return {};
        std::vector<uint8_t> bytes;
        if (!offline_signing::DecodeLowerHex(value->text, limit / 2, bytes))
            return {};
        return value->text;
    };
...
int main(int argc, char** argv) {
    ...
    if (cmd == "authorize-intent") {
        ...
        const bool relay =
            request.operation_type == "VELD_BHDR|" || request.operation_type == "VELD_ANCHOR|" ||
#if defined(VELD_CUSTODY_AUTHORITY_PROPOSAL)
            request.operation_type == "VELD_CIA1|" || request.operation_type == "VELD_CST1|" ||
#endif
    ...
    if (cmd == "rotate-pass") {
        if (argc < 3) {
            std::cerr
                << "Usage: VELD_OLD_PASSPHRASE=... VELD_NEW_PASSPHRASE=... veld-keygen rotate-pass FILE\n";
            return 2;
        }
        ...
        std::string policy_error;
        if (!veld::wallet_crypto::ValidateNewPassphrase(new_pass.value, &policy_error)) {
            std::cerr << "error: " << policy_error << "\n";
            return 2;
        }
        ...
        SensitiveString plaintext;
        try {
            plaintext.value = veld::wallet_crypto::DecryptWallet(data, old_pass.value);
        } catch (const std::exception& e) {
        ...
        try {
            blob = veld::wallet_crypto::EncryptWallet(plaintext.value, new_pass.value);
        } catch (const std::exception& e) {
        ...
        if (second != std::string::npos) {
            ...
            addr = plaintext.value.substr(second + 1, end == std::string::npos ? std::string::npos
                                                                               : end - second - 1);
        }
        plaintext.Clear();
        std::string write_error;
        if (!secure_file::AtomicWrite(path, blob, &write_error, /*require_private_parent=*/true)) {
