...
#error "PUBLIC TESTNET forbids veld-keygen external/release signing utilities; use node/desktop wallet generation"
#endif
...
static bool ParseCanonicalU64Arg(const std::string& text, uint64_t& out) {
    ...
    const auto parsed = std::from_chars(
        text.data(), text.data() + text.size(), out, 10);
...
static bool ReadSeedWithoutArgv(SensitiveString& seed_hex,
                                std::string& error) {
    ExactEchoGuard echo;
    bool terminal = false;
    if (!echo.DisableIfTerminal(terminal, error)) return false;
    if (!terminal) {
...
static bool ReadSeedFromProtectedHandle(uint64_t raw_handle,
    ...
    const HANDLE handle = reinterpret_cast<HANDLE>(
        static_cast<uintptr_t>(raw_handle));
    DWORD inherit_flags = 0;
    if (handle == INVALID_HANDLE_VALUE ||
        !::GetHandleInformation(handle, &inherit_flags) ||
        (inherit_flags & HANDLE_FLAG_INHERIT) == 0 ||
    ...
    for (;;) {
        DWORD count = 0;
        if (!::ReadFile(handle, buffer, static_cast<DWORD>(sizeof(buffer)),
                        &count, nullptr)) {
...
static int CmdNew(const std::string& out_path, bool testnet,
    ...
    try {
        ...
        std::cerr << "error: ML-DSA public-key derivation failed: "
                  << e.what() << "\n";
        return 1;
    }
    ...
    plaintext.value.reserve(seed->bytes.size() * 2U + 1U +
                            pub.size() * 2U + 1U + addr.size() + 1U);
    ...
    try {
        encrypted = veld::wallet_crypto::EncryptWallet(plaintext.value,
                                                        pass.value);
    } catch (const std::exception& e) {
        plaintext.Clear();
        pass.Clear();
    ...
    if (!veld::channel::secure_file::AtomicWriteNew(
            out_path, encrypted, &write_error,
            /*require_private_parent=*/true)) {
        std::cerr << "error: secure no-overwrite key write failed: "
                  << write_error << "\n";
        return 1;
    }

    std::cout << "address: " << addr << "\n"
              << "file:    " << out_path << " (" << encrypted.size()
              << " bytes, encrypted)\n";
    return 0;
}
...
static int CmdShow(const std::string& path, bool full_pubkey = false) {
    ...
    std::cout << "address: " << addr << "\n"
              << "pubkey:  " << pub_hex.substr(0, 32) << "..." << pub_hex.substr(pub_hex.size() - 32)
              << " (1952 bytes)\n";
    if (full_pubkey) {
...
static int CmdSignRelease(const std::string& keyfile, const std::string& input,
    ...
    if (!fs::exists(keyfile)) { std::cerr << "error: no such key file: " << keyfile << "\n"; return 2; }
    bool kok = false;
    std::vector<uint8_t> kdata = ReadAllBytes(keyfile, kok);
    if (!kok) { std::cerr << "error: cannot read key file\n"; return 1; }
    std::string pass = ReadPassphrase("decrypting release key");
    std::string plaintext;
    try { plaintext = veld::wallet_crypto::DecryptWallet(kdata, pass); }
    catch (const std::exception& e) {
        veld::compat::SecureZero(pass.data(), pass.size());
        std::cerr << "error: decrypt failed: " << e.what() << "\n"; return 1;
    }
    ...
    std::istringstream ss(plaintext);
    std::string priv_hex, pub_hex, addr;
    std::getline(ss, priv_hex); std::getline(ss, pub_hex); std::getline(ss, addr);
    veld::compat::SecureZero(plaintext.data(), plaintext.size());
    ...
    if (!iok) { veld::compat::SecureZero(seed.data(), 32); std::cerr << "error: cannot read input: " << input << "\n"; return 1; }
    veld::Hash256 h = veld::Hash256d(in);
    veld::Secp256k1SigDER sig;
    try { sig = veld::Sign(seed, h); }
    catch (const std::exception& e) {
    ...
    if (!fp) { std::cerr << "error: cannot write sig: " << outsig << "\n"; return 1; }
    if (std::fwrite(sig.data(), 1, sig.size(), fp) != sig.size()) { std::fclose(fp); std::cerr << "error: short write\n"; return 1; }
    std::fclose(fp);
    std::cout << "signed " << input << " -> " << outsig << " (" << sig.size() << " bytes, ML-DSA-65)\n";
...
static int CmdVerifyRelease(const std::string& pubarg, const std::string& input,
    ...
    if (!pub_hex.empty() && pub_hex[0] == '@') {
        ...
        if (!pf.good()) { std::cerr << "error: cannot read pubkey file\n"; return 1; }
        std::getline(pf, pub_hex);
    }
    while (!pub_hex.empty() && (pub_hex.back()=='\n'||pub_hex.back()=='\r'||pub_hex.back()==' '))
        pub_hex.pop_back();
    if (pub_hex.size() != 1952*2) { std::cerr << "error: pubkey must be 3904 hex chars\n"; return 1; }
    veld::Secp256k1PubKey pub{};
    for (size_t i = 0; i < 1952; ++i) pub[i] = (uint8_t)std::stoul(pub_hex.substr(i*2,2), nullptr, 16);
    bool iok=false, sok=false;
    std::vector<uint8_t> in  = ReadAllBytes(input, iok);
    std::vector<uint8_t> sig = ReadAllBytes(sigfile, sok);
    if (!iok || !sok) { std::cerr << "error: cannot read input/sig\n"; return 1; }
    veld::Hash256 h = veld::Hash256d(in);
...
static bool ParseReserveMintPolicyContext(
        const std::string& state_hex, const std::string& supply_text,
    ...
    for (const char c : supply_text) {
        ...
        if (supply > (UINT64_MAX - digit) / 10) return false;
    ...
    if (encoded.empty() || state_hex != BytesToHex(encoded) ||
        !btcveld::reserve::DecodeState(
            encoded.data(), encoded.size(), state) ||
...
static bool WritePrivateFile(const std::string& path,
...
static int CmdSignTx(const std::string& keyfile, const std::string& prepared,
                     ...
                     const std::string& outpath, bool relay_op = false,
                     const IntentAuthorizationRequest* authorize = nullptr) {
    using namespace veld;
    if (!fs::exists(keyfile))  { std::cerr << "error: no such key file: " << keyfile << "\n"; return 2; }
    if (!fs::exists(prepared)) { std::cerr << "error: no such prepared file: " << prepared << "\n"; return 2; }
    ...
    if (!authorize) {
        ...
        if (fs::equivalent(prepared, intent_path, same_error) && !same_error) {
            std::cerr << "error: prepared transaction and detached intent must be different files\n";
            return 2;
        }
    }
    ...
    veld::compat::SecureZero(priv_hex.data(), priv_hex.size());
    Secp256k1PrivKey priv{}; std::copy(seedarr.begin(), seedarr.end(), priv.begin());
    veld::compat::SecureZero(seedarr.data(), seedarr.size());
    std::vector<uint8_t> pub_bytes;
    if (!offline_signing::DecodeLowerHex(pub_hex, 1952U, pub_bytes) ||
        pub_bytes.size() != 1952U) {
    ...
    btc_buy::StrictJsonParser prep_parser(
        prep, offline_signing::kMaxPreparedJsonBytes,
    ...
    our_p2pkh.push_back(0x88); our_p2pkh.push_back(0xAC);

    offline_signing::VerifiedPrepared verified;
    if (!offline_signing::AuthenticatePrepared(
            prep_root, our_p2pkh, verified, parse_error)) {
        veld::compat::SecureZero(priv.data(), priv.size());
        std::cerr << "REFUSED (parent authentication): " << parse_error
                  << ". Nothing was signed.\n";
        return 1;
    }
    if (!offline_signing::VerifyExactFeesOnlyEnvelope(
            verified, our_p2pkh, parse_error)) {
        veld::compat::SecureZero(priv.data(), priv.size());
        std::cerr << "REFUSED (output envelope): " << parse_error
                  << ". Nothing was signed.\n";
        return 1;
    }
    ...
    if (!offline_signing::ExtractCanonicalOperationIdentity(
            tx.outputs.back(), operation_identity, parse_error)) {
        veld::compat::SecureZero(priv.data(), priv.size());
        std::cerr << "REFUSED (operation marker): " << parse_error
                  << ". Nothing was signed.\n";
        return 1;
    }

    const auto* proposal =
        offline_signing::SelectObject(prep_root, "prepared_transaction");
    if (!proposal) {
        veld::compat::SecureZero(priv.data(), priv.size());
    ...
    if (reserve_state || reserve_supply) {
        uint64_t supply = 0;
        if (!reserve_state ||
            reserve_state->kind != btc_buy::JsonValue::Kind::String ||
    ...
    if (!reserve_prior_state_hex.empty() ||
        !reserve_prior_supply_sats.empty()) {
        if (!ParseReserveMintPolicyContext(
    ...
    for (size_t i = 0; i < tx.outputs.size(); ++i) {
        ...
        if (!s.empty() && s[0] == 0x6A) {
            ...
            if (ascii.size() > 256)
                ascii = ascii.substr(0, 256) + "... (" +
                        std::to_string(operation_identity.size()) +
                        " payload bytes)";
            ...
            kind = "P2PKH";
            detail = (s == our_p2pkh) ? "change back to this address"
                                      : "-> hash160 " + HexOf(&s[3], 20);
        } else {
            kind = "script"; detail = HexOf(s.data(), s.size());
        }
        std::cerr << "    [" << i << "] " << tx.outputs[i].value
                  << " base units (" << std::fixed << std::setprecision(8)
                  << ((double)tx.outputs[i].value / 100000000.0) << " VELD)  "
                  << kind << "  " << detail << "\n";
    }
    ...
    if (relay_op) {
        ...
        const std::string refusal =
            BtcVeldRelayTemplatePolicy(tx, our_p2pkh, family);
        if (!refusal.empty()) {
            veld::compat::SecureZero(priv.data(), priv.size());
            std::cerr << "REFUSED (relay policy): " << refusal
                      << ". Nothing was signed.\n";
            return 1;
        }
        ...
        intended_amount = 0;
        std::cerr << "  [policy] canonical fees-only relay verified: "
                  << family << "\n\n";
    } else {
        std::string mint_from, mint_to, mint_memo; uint64_t mint_sats = 0;
        std::string mint_refusal = BtcVeldMintTemplatePolicy(
            tx, our_p2pkh, mint_from, mint_to, mint_sats, mint_memo,
            reserve_context_ptr);
        if (mint_refusal.empty()) {
            if (mint_from != addr)
                mint_refusal = "MINT 'from' (" + mint_from +
                    ") != this issuer key (" + addr + ")";
            if (mint_refusal.empty())
        ...
        if (!mint_refusal.empty()) {
            ...
            std::string reserve_refusal =
                BtcVeldC1ReservationTemplatePolicy(
                    tx, our_p2pkh, c1);
            if (reserve_refusal.empty() && c1.from != addr)
                reserve_refusal =
                    c1.action + " 'from' differs from this issuer key";
            ...
            std::cerr << "  [policy] canonical " << c1.action
                      << " C1 carrier verified: " << c1.sats
                      << " sats btcVELD -> " << c1.to
                      << " allocation=" << c1.allocation_id << "\n\n";
    ...
    if (authorize) {
        ...
        intent = offline_signing::MakeIntent(
            verified.tx, verified.parent_raw, operation_type,
            intended_recipient, intended_amount, addr, expected_change,
            operation_identity);
        ...
        if (!offline_signing::VerifyIntent(
                intent, verified, operation_type, intended_recipient,
                intended_amount, addr, expected_change, operation_identity,
                parse_error)) {
            veld::compat::SecureZero(priv.data(), priv.size());
        ...
        btc_buy::StrictJsonParser intent_parser(
            intent_json, 64U * 1024U,
            /*reject_escaped_object_keys=*/true);
        offline_signing::IntentAuthorization authorization;
        if (!intent_parser.Parse(intent_root, parse_error) ||
            !offline_signing::ParseIntent(
                intent_root, intent, parse_error) ||
            !offline_signing::ParseIntentAuthorization(
                intent_root, authorization, parse_error) ||
            !offline_signing::VerifyIntentAuthorization(
                intent, authorization, pub, parse_error) ||
            !offline_signing::VerifyIntent(
                intent, verified, operation_type, intended_recipient,
                intended_amount, addr, expected_change, operation_identity,
                parse_error)) {
            veld::compat::SecureZero(priv.data(), priv.size());
            std::cerr << "REFUSED (intent): " << parse_error
                      << ". Nothing was signed.\n";
            return 1;
        }
    }
    ...
    if (authorize) {
        const std::string pubkey_digest =
            offline_signing::AuthorizerPubkeyDigest(pub);
        Secp256k1SigDER authorization_signature;
        try {
            authorization_signature = Sign(
                priv, offline_signing::IntentAuthorizationHash(
                    intent, pubkey_digest));
        } catch (const std::exception& e) {
            veld::compat::SecureZero(priv.data(), priv.size());
            std::cerr << "error: intent authorization failed: "
                      << e.what() << "\n";
            return 1;
        }
        veld::compat::SecureZero(priv.data(), priv.size());
        ...
        const std::string document =
            ...
            ",\"complete_output_digest\":" +
                q(intent.complete_output_digest) +
            ...
            ",\"authorizer_pubkey_digest\":" + q(pubkey_digest) +
            ",\"signature_hex\":" +
                q(HexOf(authorization_signature.data(),
                        authorization_signature.size())) + "}}";
        const std::vector<uint8_t> document_bytes(
            document.begin(), document.end());
        if (!WritePrivateFile(outpath, document_bytes,
                              "authorized signing intent"))
            return 1;
    ...
    for (uint32_t i = 0; i < tx.inputs.size(); ++i) {
        SignedInput si = BuildScriptSig(
            priv, pub, tx, i, verified.prev_scripts[i]);
    ...
    if (outpath.empty()) {
        ...
        if (!of.good()) { std::cerr << "error: cannot write " << outpath << "\n"; return 1; }
        of << signed_hex << "\n";
...
static int CmdDecodeMint(const std::string& issuer_p2pkh_hex,
    ...
    auto hex2b = [](const std::string& h, bool& ok) {
        ...
        auto hc = [](char c) -> int { if (c>='0'&&c<='9') return c-'0'; if (c>='a'&&c<='f') return c-'a'+10; if (c>='A'&&c<='F') return c-'A'+10; return -1; };
        for (size_t i = 0; ok && i + 1 < h.size(); i += 2) { int a=hc(h[i]), b=hc(h[i+1]); if (a<0||b<0){ok=false;break;} o.push_back((uint8_t)((a<<4)|b)); }
        return o;
    };
    bool ok1=false, ok2=false;
    std::vector<uint8_t> issuer_p2pkh = hex2b(issuer_p2pkh_hex, ok1);
    std::vector<uint8_t> raw = hex2b(tx_hex, ok2);
    if (!ok1 || issuer_p2pkh.size() != 25) { std::cerr << "error: issuer_p2pkh_hex must be a 25-byte P2PKH\n"; return 2; }
    ...
    if (!reserve_state_hex.empty() || !reserve_supply_sats.empty()) {
        if (!ParseReserveMintPolicyContext(
                reserve_state_hex, reserve_supply_sats, reserve_context)) {
...
static int CmdDecodeC1Reservation(const std::string& issuer_p2pkh_hex,
                                  const std::string& tx_hex) {
    using namespace veld;
    auto hex2b = [](const std::string& h, bool& ok) {
    ...
    const std::vector<uint8_t> issuer_p2pkh =
        hex2b(issuer_p2pkh_hex, script_ok);
    ...
    const std::string refusal = BtcVeldC1ReservationTemplatePolicy(
        tx, issuer_p2pkh, result);
    if (!refusal.empty()) {
    ...
    auto optional_json_string = [&](const std::string& value) {
        return value.empty() ? std::string("null")
                             : std::string("\"") + esc(value) + "\"";
    };
    std::cout << "{\"action\":\"" << result.action
              << "\",\"from\":\"" << esc(result.from) << "\",\"to\":\""
              << esc(result.to) << "\",\"sats\":" << result.sats
              << ",\"allocation_id\":\"" << result.allocation_id
              << "\",\"allocation_commitment\":\""
              << result.allocation_commitment
              ...
              << ",\"fund_outpoint\":"
              << optional_json_string(result.fund_outpoint)
              << ",\"funding_proof_hex\":"
              << optional_json_string(result.funding_proof_hex)
...
static bool ReadPrivateFile(const std::string& path, size_t max_size,
                            std::vector<uint8_t>& out,
                            const char* label) {
    std::string error;
    ...
    if (result != secure_file::ReadResult::Ok) {
        std::cerr << "error: cannot securely read " << label << " " << path
                  << ": " << (result == secure_file::ReadResult::NotFound
                                   ? "not found" : error)
                  << "\n";
        return false;
    }
...
static bool WritePrivateFile(const std::string& path,
    ...
    if (!secure_file::AtomicWriteNew(path, bytes, &error,
                                     /*require_private_parent=*/true)) {
        std::cerr << "error: cannot publish " << label << " without overwrite: "
                  << error << "\n";
        return false;
    }
    return true;
...
static int CmdExportPublicKeyRaw(const std::string& keyfile,
    ...
    std::vector<uint8_t> encrypted;
    if (!ReadPrivateFile(keyfile, 1024u * 1024u, encrypted,
                         "encrypted key")) return 1;
    ...
    SensitiveString plaintext;
    try {
        plaintext.value = veld::wallet_crypto::DecryptWallet(encrypted,
                                                              pass.value);
    } catch (const std::exception& e) {
        std::cerr << "error: decrypt failed: " << e.what() << "\n";
        return 1;
    ...
    if (first == std::string::npos || second == std::string::npos ||
        third == std::string::npos || third + 1 != plaintext.value.size()) {
        std::cerr << "error: key file format is not canonical\n";
        return 1;
    }
    SensitiveString seed_hex(plaintext.value.substr(0, first));
    const std::string stored_pub_hex = plaintext.value.substr(
        first + 1, second - first - 1);
    const std::string address = plaintext.value.substr(
    ...
    if (stored_pub.size() != derived.size() ||
        veld::BytesToHex(stored_pub) != stored_pub_hex) {
...
static void PrintUsage() {
    std::cerr <<
        "veld-keygen — post-quantum (ML-DSA-65) keypair tool\n"
        "\n"
        "Usage:\n"
...
int main(int argc, char** argv) {
    ...
    if (cmd == "new" || cmd == "from-seed") {
        ...
        for (int i = first_opt; i < argc; ++i) {
            ...
            if (a == "--out" && i + 1 < argc) { out_path = argv[++i]; }
                ...
                std::cerr << "error: unknown or incomplete option at argument "
                          << i << " (argument value suppressed)\n";
                return 2;
            }
        }
        ...
        if (cmd == "from-seed") {
            ...
            const bool input_ok = have_seed_input_handle
                ? ReadSeedFromProtectedHandle(seed_input_handle, seed_hex,
                                              input_error)
    ...
    if (cmd == "show") {
        ...
        for (int i = 3; i < argc; ++i) {
            if (std::string(argv[i]) == "--full-pubkey-hex") full_pubkey = true;
        }
    ...
    if (cmd == "sign-release") {
        if (argc < 5) { std::cerr << "Usage: veld-keygen sign-release <keyfile> <input> <outsig>\n"; return 2; }
        return CmdSignRelease(argv[2], argv[3], argv[4]);
    }

    if (cmd == "verify-release") {
        if (argc < 5) { std::cerr << "Usage: veld-keygen verify-release <pubhex|@file> <input> <sigfile>\n"; return 2; }
    ...
    if (cmd == "authorize-intent") {
        ...
        std::string output;
        bool have_operation = false, have_recipient = false,
             have_amount = false, have_change = false,
             have_identity = false, have_absolute = false,
             have_rate = false, have_output = false;
        ...
        for (int i = 4; i < argc; ++i) {
            ...
            if (option == "--operation-type" && !have_operation) {
                ...
                ok = take(i, value) &&
                     ParseCanonicalU64Arg(value, request.intended_amount);
                have_amount = ok;
            } else if (option == "--change-destination" && !have_change) {
                ok = take(i, request.expected_change_destination);
                have_change = ok;
            } else if (option == "--operation-identity-digest" &&
                       !have_identity) {
                ok = take(i, request.operation_identity_digest) &&
                     veld::btc_buy::IsLowerHex(
                         request.operation_identity_digest, 64);
                have_identity = ok;
            } else if (option == "--maximum-absolute-fee" &&
                       !have_absolute) {
                ok = take(i, value) && ParseCanonicalU64Arg(
                    value, request.maximum_absolute_fee);
                have_absolute = ok;
            } else if (option == "--maximum-fee-rate" && !have_rate) {
                ok = take(i, value) && ParseCanonicalU64Arg(
                    value, request.maximum_fee_rate);
                have_rate = ok;
        ...
        if (!have_operation || !have_recipient || !have_amount ||
            !have_change || !have_identity || !have_absolute || !have_rate ||
            !have_output || request.operation_type.empty() || output.empty()) {
    ...
    if (cmd == "sign-tx" || cmd == "sign-op") {
        ...
        for (int i = 4; i < argc; ++i) {
            ...
            else if (a == "--intent" && i + 1 < argc) intent = argv[++i];
            else {
    ...
    if (cmd == "rotate-pass") {
        if (argc < 3) {
            std::cerr << "Usage: VELD_OLD_PASSPHRASE=... VELD_NEW_PASSPHRASE=... veld-keygen rotate-pass FILE\n";
            return 2;
        }
