...
int main(int argc, char** argv) {
    ...
    try {
        if (pool::PrintBuildIdentity(argc, argv, "pool-payout-signer", true))
            return 0;
        ...
        Json intent;
        std::string error;
        Require(btc_buy::StrictJsonParser(frame.data(), 65536, true).Parse(intent, error),
                "request JSON");
        ...
        const auto& inputs = *intent.Get("inputs");
        const auto& recipients = *intent.Get("recipients");
        Require(inputs.kind == Json::Kind::Array && inputs.array.size() >= 2 &&
                    inputs.array.size() <= 64,
                "input bound");
        Require(recipients.kind == Json::Kind::Array && !recipients.array.empty() &&
                    recipients.array.size() <= 128,
                "recipient bound");
        ...
        Require(Text(*intent.Get("pool_script")) == BytesToHex(pool_script) &&
                    Text(*intent.Get("fee_script")) == BytesToHex(fee_script),
        ...
        for (const auto& input : inputs.array) {
            ...
            if (role == "fees")
                fee_in = Add(fee_in, amount);
            else
                pool_in = Add(pool_in, amount);
            fee_input.push_back(role == "fees");
            TxInput in;
            in.prev_tx_hash = HexToHash(txid);
            in.prev_out_index = index;
        ...
        for (const auto& recipient : recipients.array) {
            ...
            Require((script.size() == 25 || IsSha384KeyScript(script)) && script != pool_script &&
                        script != fee_script && destinations.insert(address).second && amount > 0,
                    "recipient policy");
            paid = Add(paid, amount);
            tx.outputs.emplace_back(amount, script);
        }
        Require(pool_in >= paid && fee_in >= fee, "separate funding insufficient");
        if (pool_in > paid)
            tx.outputs.emplace_back(pool_in - paid, pool_script);
        if (fee_in > fee)
            tx.outputs.emplace_back(fee_in - fee, fee_script);
        Require(!tx.HasDustOutput(DUST_THRESHOLD_UNITS),
                "payout or change below native dust limit");
