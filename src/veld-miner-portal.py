...
<head>
  ...
  <style>
    .connection-map{margin-top:12px;border:1px solid var(--soft);border-radius:8px;background:#090c0a;overflow:hidden}.connection-map svg{display:block;width:100%;max-height:390px}.connection-map .link{stroke:#6d927c;stroke-width:2;fill:none}.connection-map circle{fill:#15211a;stroke:#76a48c;stroke-width:2}.connection-map .miner circle{fill:#182d14;stroke:var(--green)}.connection-map .fleet circle{fill:#172832;stroke:#76a3bd}.connection-map .validator circle{fill:#251d31;stroke:#ac8bd1}.connection-map .unknown circle{fill:#1b211e;stroke:var(--muted)}.connection-map text{fill:var(--text);font:600 18px ui-monospace,Consolas,monospace}.connection-map .count{font-size:24px}.connection-map .muted{fill:var(--muted);font-size:15px}.network-note{color:var(--muted);margin:12px 0 0;line-height:1.5}.network-machine{overflow-wrap:anywhere}.network-overview-toggle{margin-top:18px}.network-overview{margin-top:18px;border-top:1px solid var(--soft);padding-top:18px}
...
<script>
const $=id=>document.getElementById(id);let csrf="",devices=[],selected=0,page="overview",installPrompt=null,networkOverviewOpen=false;
...
function directNetworkState(d,s){
  const count=value=>Number.isInteger(value)&&value>=0&&value<=10000?value:0;
  const fresh=d.online===true,running=fresh&&s.process_running===true;
  const role=s.mining_active===true||s.mining_enabled===true?'miner':'node';
  const peers=running?count(d.peers):0,reported=s.peer_roles||{};
  let groups=['fleet','miner','validator','node','unknown'].map(role=>({role,count:count(reported[role])}));
  const classified=groups.reduce((sum,group)=>sum+group.count,0);
  // Role counts describe direct sessions. Never identify an anonymous public
  // vertex by guessing from its role, ordinal, or a rounded 64-bit ID.
  if(classified>peers)groups=[{role:'unknown',count:peers}];
  else groups.find(group=>group.role==='unknown').count+=peers-classified;
  return {fresh,running,role,peers,groups:groups.filter(group=>group.count>0)};
}
...
function directTopologyGraph(d,s){
  const state=directNetworkState(d,s),localLabel=state.role==='miner'?'This miner':'This node';
  if(!state.fresh)return `<div class="chart empty">Waiting for a fresh report from ${esc(d.name)}.</div>`;
  const labels={fleet:'Fleet peers',miner:'Miner peers',validator:'Validator peers',node:'Node peers',unknown:'Other peers'};
  const cx=210,cy=165,groups=state.groups,positions=groups.map((group,index)=>{const angle=-Math.PI/2+2*Math.PI*index/groups.length;return {group,x:cx+130*Math.cos(angle),y:cy+118*Math.sin(angle)}});
  const links=positions.map(({group,x,y})=>`<path class="link" d="M ${cx} ${cy} L ${x} ${y}"><title>${n(group.count)} reported direct ${group.count===1?'connection':'connections'}</title></path>`).join('');
  const peers=positions.map(({group,x,y})=>`<g class="${group.role}"><circle cx="${x}" cy="${y}" r="24"/><text class="count" x="${x}" y="${y+6}" text-anchor="middle">${n(group.count)}</text><text x="${x}" y="${y+43}" text-anchor="middle">${labels[group.role]}</text></g>`).join('');
  const status=!state.running?'Stopped':s.mining_active===true?'Hashing':state.peers?'Connected':'No peers';
  return `<div class="connection-map"><svg viewBox="0 0 420 330" role="img" aria-label="${esc(d.name)}: ${localLabel.toLowerCase()}, ${n(state.peers)} reported direct peers">${links}${peers}<g class="${state.role}"><title>${esc(d.name)} · ${localLabel.toLowerCase()} · ${status}</title><circle cx="${cx}" cy="${cy}" r="62"/><text x="${cx}" y="${cy-3}" text-anchor="middle">${localLabel}</text><text class="muted" x="${cx}" y="${cy+16}" text-anchor="middle">${status}</text></g></svg></div>`;
}
...
function network(d,s){
  const topology=s.topology||{},roles=topologyRoles(topology,s.peer_roles||{}),state=directNetworkState(d,s),mapping=inboundMapping(d,s);
  ...
  const publicOverview=networkOverviewOpen?`<div class="network-overview"><h2>Network overview</h2><p class="network-note">${coverage}. Roles and links are reported by other machines. These anonymous identities are separate from your selected machine's direct connections.</p>${topologyGraph(topology)}<h2>Network peer classes</h2><div class="kv"><div><b>${n(roles.fleet)}</b><span>Fleet</span></div><div><b>${n(roles.node)}</b><span>Nodes</span></div><div><b>${n(roles.miner)}</b><span>Miners</span></div><div><b>${n(roles.validator)}</b><span>Validators</span></div></div></div>`:'';
  return `<div class="cards network-cards">${metric(state.fresh?n(state.peers):'Not current',"Direct peers",state.peers?'green':'')}${metric(state.running?n(d.inbound):'—',"Inbound")}${metric(state.running?n(s.outbound):'—',"Outbound")}${metric(state.running?`${n(s.exact_tip)} / ${n(d.peers)}`:'—',"Exact tip agreement")}${metric(mapping.value,"Automatic port mapping")}${metric(s.tor?'Tor only':'Clearnet',"Transport")}</div><p class="network-note">${mapping.detail}</p><section class="section"><div class="section-head"><div><h2>Your connections</h2><p class="network-machine">${esc(d.name)} · ${state.role==='miner'?'Miner':'Node'}</p><p>Your machine and its direct peers, grouped by role.</p></div><span class="pill">${n(state.peers)} direct</span></div>${directTopologyGraph(d,s)}${nodeCount?`<button type="button" class="button ghost network-overview-toggle" data-network-overview aria-expanded="${networkOverviewOpen}">${networkOverviewOpen?'Hide':'Show'} network overview</button>${publicOverview}`:''}</section>`;
}
document.addEventListener('click',event=>{if(event.target instanceof Element&&event.target.closest('[data-network-overview]')){networkOverviewOpen=!networkOverviewOpen;render()}});
