...
int main(int argc, char** argv) {
    ...
    try {
        if(pool::PrintBuildIdentity(argc,argv,"pool-identity-signer",true))return 0;
        ...
        Json request, policy; std::string error;
        Require(btc_buy::StrictJsonParser(frame.data(),65536,true).Parse(request,error), "request JSON");
        ...
        Require(channel::secure_file::Read(argv[2],raw_policy,&error,1024*1024,true)==
                channel::secure_file::ReadResult::Ok,"protected input manifest unavailable");
        const std::string policy_text(raw_policy.begin(),raw_policy.end());
        Require(btc_buy::StrictJsonParser(policy_text,1024*1024,true).Parse(policy,error),"input policy JSON");
        ...
        Require(Text(*request.Get("chain"))==BytesToHex(genesis) &&
                Text(*policy.Get("chain"))==BytesToHex(genesis),"wrong chain");
        const auto action=Text(*request.Get("action"));
        Require(action=="nms" || action=="stake" || action=="consolidate","operation not permitted");
        ...
        Require(allowed.kind==Json::Kind::Array && allowed.array.size()<=10000,"authorized input bound");
        Require(inputs.kind==Json::Kind::Array && !inputs.array.empty() && inputs.array.size()<=64,"input bound");
        ...
        auto coin=[](const Json& input) {
            ...
            return std::pair{txid,static_cast<uint32_t>(Amount(*input.Get("vout"),UINT32_MAX-1))};
        };
        for (const auto& input:allowed.array) {
            const auto point=coin(input);
            const auto amount=Amount(*input.Get("units"));
            Require(amount>0 && authorizations.emplace(point,amount).second,"authorization duplicate");
        }
        Transaction tx;uint64_t total=0;
        std::set<std::pair<std::string,uint32_t>> used;
        for (const auto& input:inputs.array) {
            const auto point=coin(input);const auto amount=Amount(*input.Get("units"));
            Require(authorizations.contains(point) && authorizations.at(point)==amount &&
                    used.insert(point).second,"funding not explicitly operator-authorized");
            total=Add(total,amount);TxInput in;in.prev_tx_hash=HexToHash(point.first);
            in.prev_out_index=point.second;tx.inputs.push_back(in);
        }
        Require(total>MIN_TX_FEE,"operator funds insufficient");
        const auto key=Key(argv[1]);const auto script=key.GetP2PKHScript();
        ...
        if (action=="nms") {
            Require(stake==0 && tier==0 && inputs.array.size()==1 && height%COMINE_WINDOW_BLOCKS!=0,
                    "near-miss operation policy");
            const auto encoded=Text(*request.Get("header"));
            Require(encoded.size()==176 && encoded.find_first_not_of("0123456789abcdef")==std::string::npos,"header encoding");
            const auto bytes=HexToBytes(encoded);BlockHeader header;
            ...
            Require(mining::g_veldhash_last_dataset_ok() && IsNmsProofInRange(proof,target),"genuine near miss required");
            ...
            if(total>2*MIN_TX_FEE)tx.outputs.emplace_back(total-2*MIN_TX_FEE,script);
        } else {
            ...
            if(action=="stake") {
                ...
                if(total>stake+MIN_TX_FEE)tx.outputs.emplace_back(total-stake-MIN_TX_FEE,script);
                const auto marker=StakingLedger::BuildLockOp(ScriptToAddress(script),stake,height-1,static_cast<uint8_t>(tier));
                Require(marker.size()<=255,"stake marker bound");operation.push_back(0x6a);
                if(marker.size()>75)operation.push_back(0x4c);
                operation.push_back(static_cast<uint8_t>(marker.size()));
                operation.insert(operation.end(),marker.begin(),marker.end());
            } else {
                Require(stake==0 && tier==0 && inputs.array.size()>=2,"consolidation policy");
                tx.outputs.emplace_back(total-MIN_TX_FEE,script);
            }
        }
        if(!operation.empty())tx.outputs.emplace_back(0,operation);
        Require(!tx.HasDustOutput(DUST_THRESHOLD_UNITS),"operator change below native dust policy");
        std::vector<std::vector<uint8_t>> signatures;
        for(uint32_t i=0;i<tx.inputs.size();++i)signatures.push_back(key.SignInput(tx,i,script).script_sig);
        ...
        for(uint32_t i=0;i<tx.inputs.size();++i) {
            ScriptInterpreter verifier;Require(verifier.Execute(tx.inputs[i].script_sig,script,tx,i),"signature verification");
        }
        const auto raw=tx.Serialize();Require(raw.size()<=1024*1024 && tx.IsValid(),"signed transaction bounds");
