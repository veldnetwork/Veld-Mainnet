...
def main():
    ...
    for parent in (*path.parents,path):require(not parent.is_symlink(),'linked credential path refused')
    metadata=path.parent.stat()
    require(stat.S_ISDIR(metadata.st_mode) and metadata.st_uid==os.geteuid() and metadata.st_mode&0o077==0,
    ...
    print('Operator credential saved. Existing sessions are invalidated. No pool policy or wallet key changed.')
