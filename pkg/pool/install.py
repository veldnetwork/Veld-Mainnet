...
def checked_name(value):
    if not re.fullmatch(r'[a-z_][a-z0-9_-]{0,31}',value):raise ValueError('invalid service user')
    return value
...
def verify(package):
    ...
    for directory in ('bin','lib','setup','third-party'):
        ...
        if folder.is_symlink() or not folder.is_dir():raise ValueError('artifact directory missing or linked')
        if any(path.is_symlink() for path in folder.rglob('*')):raise ValueError('artifact symlink refused')
    manifest=package/'pool-sha256.txt'
    lines=manifest.read_text().splitlines();seen=set()
    for line in lines:
        digest,name=line.split('  ',1);path=package/name
        if not re.fullmatch('[a-f0-9]{64}',digest) or name in seen or Path(name).is_absolute() or '..' in Path(name).parts:
            raise ValueError('invalid artifact manifest')
        seen.add(name)
        if path.is_symlink() or not path.resolve().is_relative_to(package):raise ValueError('artifact path escapes package')
        if not path.is_file() or hashlib.sha256(path.read_bytes()).hexdigest()!=digest:raise ValueError('artifact hash mismatch: '+name)
    expected={path.relative_to(package).as_posix() for directory in ('bin','lib','setup','third-party')
              for path in (package/directory).rglob('*') if path.is_file() and '__pycache__' not in path.parts}
    if seen!=expected:raise ValueError('artifact manifest is incomplete')
...
def stage(package,prefix,config,state,core,gateway):
    ...
    for path in prefix.rglob('*'):
        if path.is_symlink():raise ValueError('symlink not permitted in installed artifact')
