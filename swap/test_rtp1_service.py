...
def fixture():
    ...
    native = dict(
        base.response, unsigned_tx_sha256=hashlib.sha256(bytes.fromhex(unsigned)).hexdigest()
    )
    facts = {
        "version": 1,
        "operation": "DEPOSIT",
        ...
        "bitcoin_block": "55" * 32,
        "reserve_vout": 0,
        "reserve_value_sats": 120000,
        "deposit_outpoint": "66" * 32 + ":0",
        "exact_commitment": "55" * 32,
        "sats": 20000,
        "issuer": base.account,
        "recipient": base.account,
        "custody_script_hex": config["custody_script_hex"],
        "direct_parent_txids": ["aa" * 32, "66" * 32],
    }
    bitcoin = {
        "genesis_hash": config["bitcoin_genesis"],
        "network": "regtest",
        "confirmations": 144,
        "reserve_outpoint": "55" * 32 + ":0",
        "reserve_value_sats": 120000,
        "custody_script_hex": config["custody_script_hex"],
    }
    body = {
    ...
    evidence = dict(
        ...
        evidence_sha256=hashlib.sha256(
            b"VELD/RTP1/BACKING/v1\x00" + service.canonical(body)
        ).hexdigest(),
    )
...
class Rtp1ServiceTests(unittest.TestCase):
    ...
    def journal(self, role, state=None):
        ...
        return service.Journal(
            state or service.empty_journal(role, self.config, "bb" * 32),
            self.config,
            role,
            persist,
            self.verify,
    ...
    def reserve(self, request):
        return service.witness_reserve(
            self.witness, request, lambda req: self.evidence, self.sign_receipt
        )

    def commit(self, request, signed):
        return service.witness_commit(
            self.witness,
            request,
            signed,
            lambda req, retained, signed: (self.events.append("retained-decode") or True),
        )
    ...
    def mint(self, **overrides):
        kwargs = dict(
            inspect=lambda req: self.evidence,
            reserve=self.reserve,
            verify_fresh=lambda req, retained: None,
            sign_or_recover=self.sign,
            commit=self.commit,
            halt=lambda: None,
            receipt_verify=self.verify,
    ...
    def test_restart_replays_exact_bytes_without_fresh_issuance(self):
        ...
        self.assertEqual(
            self.mint(inspect=fail, verify_fresh=fail, sign_or_recover=fail), self.signed
        )
    ...
    def test_signing_uncertainty_keeps_both_reservations(self):
        def uncertain(*args):
            raise RuntimeError("exact output missing")

        with self.assertRaises(RuntimeError):
            self.mint(sign_or_recover=uncertain)
        for role in ("issuer", "witness"):
    ...
    def test_witness_rollback_refuses_at_key_boundary(self):
        with self.assertRaises(ValueError):
            self.mint(witness_snapshot=lambda req: {})
    ...
    def test_stale_or_different_context_prevents_signing(self):
        def stale(*args):
            raise ValueError("stale reserve")

        with self.assertRaises(ValueError):
            self.mint(verify_fresh=stale)
        self.assertEqual(self.sign_calls, 0)
    ...
    def test_wrong_commit_ack_never_marks_issuer_committed(self):
        with self.assertRaises(ValueError):
            self.mint(commit=lambda req, signed: {"committed": True})
        self.assertFalse(self.saved["issuer"]["records"][0]["committed"])
    ...
    def test_different_signed_bytes_refused_after_commit(self):
        ...
        self.assertEqual(
            service.unsigned_template_from_signed_hex(changed), self.request["unsigned_tx_hex"]
        )
        with self.assertRaises(ValueError):
            self.commit(self.request, changed)

    def test_invalid_signature_receipt_refuses_before_key(self):
        with self.assertRaises(ValueError):
            self.mint(receipt_verify=lambda *args: False)
        self.assertEqual(self.sign_calls, 0)
    ...
    def test_immutable_receipt_verification_is_cached_only_within_this_journal(self):
        ...
        with self.assertRaises(ValueError):
            service.Journal(state, self.config, "issuer", lambda value: None, lambda *args: False)

    def test_mutated_claim_not_accepted_as_idempotent_retry(self):
        self.reserve(self.request)
        changed = dict(self.request, sats=20001)
        with self.assertRaises(ValueError):
            self.witness.find(changed)
    ...
    def test_missing_or_altered_journal_head_refuses(self):
        ...
        altered["records"] = []
        with self.assertRaises(ValueError):
            self.journal("witness", altered)

    def test_schema_and_integer_types_strict(self):
        for altered in (
            dict(self.request, extra=1),
            dict(self.request, sats=True),
            dict(self.request, unsigned_tx_hex="AA"),
        ):
            with self.subTest(altered=altered), self.assertRaises(ValueError):
                service.request_identity(altered)

    def test_production_and_wrong_policy_do_not_activate(self):
        for config in (
            dict(self.config, bitcoin_network="main"),
            dict(self.config, minimum_confirmations=143),
    ...
    def test_conflicting_reserve_edge_never_has_two_receipts(self):
        ...
        request = dict(
            self.request, unsigned_tx_hex=self.request["unsigned_tx_hex"][:-8] + "01000000"
        )
