...
if __name__ == "__main__":
    ...
    unknown = [arg for arg in sys.argv[1:] if arg != "--initialize" and not arg.startswith("/")]
    if unknown or len(config_paths) > 1 or sys.argv[1:].count("--initialize") > 1:
        raise SystemExit("usage: veld_wt_allocate.py [absolute-config-path] [--initialize]")
