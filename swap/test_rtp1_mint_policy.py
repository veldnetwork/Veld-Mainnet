...
class Rtp1MintPolicyTests(unittest.TestCase):
    def setUp(self):
        self.raw = "00"
        self.chain = {
            "profile_id": "isolated-fixture",
            "consensus_build_profile": "fixture",
            "disposable": True,
            "external_value": False,
            "fixed_difficulty_regtest": True,
            "genesis_hash": bytes(range(1, 33)).hex(),
            "launch_block_hash": "22" * 32,
        }
        self.account = "V" + "1" * 30
        self.expected = {
        ...
        self.response.update(
            {
                ...
                "reserve_prior_state_hex": "ab" * 200,
                "reserve_prior_supply_sats": 100000,
                "custody_descriptor_sha256": "33" * 32,
                "custody_manifest_sha256": "44" * 32,
            }
        )
    ...
    def validate(self, response=None, **updates):
        ...
        return policy.validate_inspection(
            self.response if response is None else response, self.raw, **expected
        )
    ...
    def test_native_decoder_result_binds_exact_inspected_proof(self):
        ...
        with mock.patch.object(policy, "run_bounded_subprocess", return_value=result) as run:
            actual = policy.decode_inspected_mint(
                "fixture-keygen",
                "76a914" + "11" * 20 + "88ac",
                self.raw,
                self.response,
        ...
        self.assertEqual(
            request["reserve_prior_state_hex"], self.response["reserve_prior_state_hex"]
        )
    ...
    def test_native_decoder_failure_is_not_an_authorization(self):
        ...
        with (
            mock.patch.object(policy, "run_bounded_subprocess", return_value=result),
            self.assertRaises(ValueError),
        ):
            policy.decode_inspected_mint(
                "fixture-keygen",
                "76a914" + "11" * 20 + "88ac",
                self.raw,
                self.response,
    ...
    def test_predicted_signed_size_refuses_before_issuance(self):
        ...
        with (
            mock.patch.object(
                ...
                return_value=SimpleNamespace(returncode=0, stdout=json.dumps(decoded)),
            ),
