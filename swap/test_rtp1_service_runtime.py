...
class RuntimeBoundaryTests(unittest.TestCase):
    ...
    def test_reserved_request_is_counted_exactly_once_before_and_after_reservation(self):
        ...
        rows = [
            {"request": other, "confirmation": None},
        ...
        with mock.patch.object(runtime, "beat_gate") as gate:
            for reserved in (False, True):
                if reserved:
                    rows.append({"request": self.request, "confirmation": None})
                runtime.reserved_mint_gate(
                    self.config, "rpc", "beat", journal, self.request, "verify"
                )
    ...
    def test_reserved_request_cannot_reuse_backing_committed_elsewhere(self):
        ...
        rpc.call.side_effect = (
            lambda method, params: {"supply_sats": 0, "tip": 100, "tip_hash": "ab" * 32}
        ...
        journal = mock.Mock(state={"records": rows})
        runtime.reserved_mint_gate(
            self.config, rpc, beat, journal, self.request, lambda *args: True
        )
    ...
    def test_heartbeat_signature_excludes_only_receiver_timestamps(self):
        ...
        rpc = mock.Mock()
        rpc.call.side_effect = (
            lambda method, params: view if method == "getbtcveldsupply" else "ab" * 32
        )
    ...
    def test_invalid_expired_or_insufficient_heartbeat_refuses(self):
        ...
        beat = solvency.stamp_heartbeat(payload, time.time())
        rpc = mock.Mock()
        for candidate, verify, used in (
            (beat, lambda *args: False, 0),
            (dict(beat, expires_at=1), lambda *args: True, 0),
            (beat, lambda *args: True, 20000),
        ):
    ...
    def test_bitcoin_argument_types_are_canonical_json(self):
        if os.name != "posix":
            self.skipTest("existing witness service is POSIX")
        import veld_wt_reserve as witness

        cfg = {
            "cli_base": ["/fixture/bitcoin-cli"],
            "wallet": "w",
            "confirmations": 144,
        ...
        with mock.patch.object(
            witness, "run_bounded_subprocess", return_value=mock.Mock(returncode=0, stdout="{}")
        ) as launch:
    ...
    def test_empty_migration_is_durable_and_originals_unchanged(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            documents = {
                "mint": {
                    "signed": [],
                    "mint_accounting": {"version": 2, "pending": [], "confirmed": []},
                },
                "c1": issuer._empty_c1_signer_state(),
                "leases": issuer._empty_prevout_journal(),
            }
            for label, path in paths.items():
                issuer.save_signer_state_durable(documents[label], path)
            originals = {label: Path(path).read_bytes() for label, path in paths.items()}
            marker = runtime.initialize_empty_migration(
                str(root), "issuer", self.config, paths, issuer.save_signer_state_durable
            )
            cfg = {
                "rtp1_service": self.config,
                "veld_rpc": {"expected_chain": self.config["expected_chain"]},
            }
            loaded = runtime.load_activated_state(
                str(root),
                "issuer",
                cfg,
                {k: p for k, p in paths.items() if k != "leases"},
                issuer.save_signer_state_durable,
                lambda *args: True,
            )
            ...
            with self.assertRaises(ValueError):
                runtime.initialize_empty_migration(
                    str(root), "issuer", self.config, paths, issuer.save_signer_state_durable
                )
            Path(paths["mint"]).write_bytes(originals["mint"] + b" ")
            with self.assertRaises(ValueError):
                runtime.load_activated_state(
                    str(root),
                    "issuer",
                    cfg,
                    {k: p for k, p in paths.items() if k != "leases"},
    ...
    def test_existing_funded_or_signed_c1_state_cannot_migrate(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            documents = {
                ...
                "c1": dict(
                    issuer._empty_c1_signer_state(),
                    records={"funded": {"signed_tx_hex": self.signed}},
                ),
                "leases": issuer._empty_prevout_journal(),
            }
            for label, path in paths.items():
    ...
    def test_partial_migration_never_auto_adopts_or_overwrites(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaises(ValueError):
                runtime.initialize_empty_migration(
                    str(root), "issuer", self.config, {}, issuer.save_signer_state_durable
                )
    ...
    def test_confirmation_requires_exact_native_effect_and_both_branches(self):
        row = {
            ...
            "receipt": {"intent": {"native": self.evidence["native"]}},
            "evidence": self.evidence,
        }
        view = {"supply_sats": 20000, "tip": 3000, "tip_hash": "ab" * 32}
        status = {
            "outpoint": self.evidence["native"]["deposit_outpoint"],
            "minted": True,
            "consumed": True,
            "accepted_effect_kind": "MINT",
            "accepted_txid": lifecycle.signed_identity(
                self.signed, self.request["unsigned_tx_hex"]
            ),
            "tip": 3000,
            "tip_hash": "ab" * 32,
            "accepted_block_height": 2980,
            "accepted_block_hash": "cd" * 32,
        }
        ...
        def native(method, params):
            ...
            if method == "getpeginfo":
                return {"final_height": 2980}
            if method == "getbtcveldmintstatus":
                return dict(status)
            if method == "getblockhash":
                return "ab" * 32 if params == [3000] else "cd" * 32
        ...
        def bitcoin(method, *args):
            if method == "getblockhash":
                return self.config["bitcoin_genesis"] if args == (0,) else "55" * 32
            if method == "getblockchaininfo":
                return dict(info)
            if method == "getblockheader":
                return {"height": 100, "confirmations": 401}
            raise AssertionError(method)
        ...
        with mock.patch.object(runtime, "verify_expected_chain"):
            self.assertEqual(
                runtime.confirmation(self.config, rpc, btc, row),
                {"height": 2980, "hash": "cd" * 32},
            )
            status["accepted_txid"] = "ef" * 32
            self.assertIsNone(runtime.confirmation(self.config, rpc, btc, row))
            status["accepted_txid"] = lifecycle.signed_identity(
                self.signed, self.request["unsigned_tx_hex"]
            )
    ...
    def test_status_and_commit_reconcile_before_releasing_cached_state(self):
        ...
        cfg = {
            "rtp1_service": self.config,
            "state_dir": "/fixture",
            "rtp1_witness_public_key_file": "/fixture/public",
        }
        ...
        for action in ("rtp1_status", "rtp1_commit"):
            ...
            if action == "rtp1_commit":
                envelope["signed_tx_hex"] = self.signed
            journal.reconcile.side_effect = lambda callback: events.append(
                ("reconcile", callback({}))
            )
            with (
                mock.patch.object(runtime, "verifier"),
                mock.patch.object(runtime, "verify_expected_chain"),
                ...
                mock.patch(
                    "veld_signerd.issuer_p2pkh_from_address",
                    return_value="76a914" + "11" * 20 + "88ac",
                ),
                mock.patch.object(
                    lifecycle,
                    "witness_status",
                    side_effect=lambda *args: events.append(("status", None)),
                ),
                mock.patch.object(
                    lifecycle,
                    "witness_commit",
                    side_effect=lambda *args: events.append(("commit", None)),
                ),
            ):
