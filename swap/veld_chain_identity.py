...
def parse_expected_chain(value):
    ...
    for field in _TEXT_FIELDS:
        if (
            ...
            or not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._-]{0,127}", value[field])
            or value[field].startswith("REPLACE_")
        ):
...
def verify_expected_chain(call, expected):
    ...
    if (
        type(compiled) is not str
        or type(genesis) is not str
        or compiled != expected["genesis_hash"]
        or genesis != compiled
    ):
