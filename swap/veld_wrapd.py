...
from swapd_http_security import (
...
def load_configuration(path):
    ...
    try:
        return strict_json_object(read_owner_file(path, MAX_CONFIG_BYTES, "wrap configuration"))
...
def _cors_origin(value, production):
    ...
    if (
        ...
        or parsed.query
        or parsed.fragment
        or value != "%s://%s" % (parsed.scheme, parsed.netloc)
    ):
        raise RuntimeError(
...
def validate_c1_policy(policy):
    ...
    if (
        ...
        or not HEX64_RE.fullmatch(previous)
        or not isinstance(public_range, list)
        or len(public_range) != 2
        or type(public_range[0]) is not int
        or type(public_range[1]) is not int
        or public_range[0] != C1_PUBLIC_RANGE_START
        or public_range[1] < C1_INITIAL_PUBLIC_RANGE_END
        or public_range[1] > 1_000_000
    ...
    if sequence == 1 and (previous != "0" * 64 or public_range[1] != C1_INITIAL_PUBLIC_RANGE_END):
...
def require_supported_allocation_range(policy):
    ...
    if (
        ...
        and validated["public_descriptor_range"][1] != C1_INITIAL_PUBLIC_RANGE_END
    ):
        raise RuntimeError(
...
def load_signed_c1_policy(conf):
    ...
    signature_path = _absolute_config_path(conf, "public_capacity_config_signature_file")
    public_key_path = _absolute_config_path(conf, "public_capacity_config_public_key_file")
    ...
    signature_raw = read_owner_file(signature_path, 16 * 1024, "signed C1 policy signature")
    public_key_raw = read_owner_file(public_key_path, 16 * 1024, "signed C1 policy public key")
    try:
    ...
    if (
        ...
        or not MLDSA65_SIGNATURE_RE.fullmatch(signature or "")
...
def validate_configuration(conf):
    ...
    if production and unsafe_development:
        raise RuntimeError("development_only_allow_unsafe_allocator must be false in production")
    if production:
        required_policy = {
            "public_capacity_policy_version",
            "rl_per_ip_per_min",
            "rl_global_per_min",
            "allocation_store",
            "allow_initial_allocation_store_creation",
            "allocation_witness_command",
            "allocation_checkpoint_command",
            "admission_verifier",
        ...
        if missing:
            raise RuntimeError(
                "production public-capacity policy is incomplete: %s" % ",".join(missing)
            )
        if conf.get("public_capacity_policy_version") != 2:
            raise RuntimeError("public_capacity_policy_version must be the JSON integer 2")
    ...
    if not isinstance(wallet, str) or not re.fullmatch(r"[A-Za-z0-9_.-]{1,128}", wallet):
        raise RuntimeError("wallet must be a bounded Bitcoin Core wallet name")
    allocation_store = conf.get("allocation_store", "/var/lib/veld-wrapd/allocations.json")
    if (
        not isinstance(allocation_store, str)
        or not allocation_store
        or (production and not os.path.isabs(allocation_store))
    ):
        raise RuntimeError("production allocation_store must be one absolute non-empty path")
    ...
    if production:
        checkpoint_command = _command_argv(checkpoint_command, "allocation_checkpoint_command")
        for name in (
            "admission_verifier",
            "public_capacity_config_file",
            "public_capacity_config_signature_file",
        ...
        checkpoint_command = _command_argv(checkpoint_command, "allocation_checkpoint_command")
    consensus_reservation_command = conf.get("consensus_reservation_command")
    if production:
    ...
    return {
        ...
        "cli_base": _command_argv(
            conf.get("cli_base", ["/usr/bin/sudo", "-u", "bitcoin", "/usr/bin/bitcoin-cli"]),
            "cli_base",
        ),
        "wallet": wallet,
        "bind": bind,
        ...
        "max_wrap_sats": bounded_json_int(
            conf.get("max_wrap_sats", 1_000_000_000), "max_wrap_sats", 1, MAX_MONEY_SATS
        ),
        ...
        "http_request_read_timeout_s": bounded_json_int(
            conf.get("http_request_read_timeout_s", 15), "http_request_read_timeout_s", 1, 60
        ),
        ...
        "allocation_store_max_bytes": bounded_json_int(
            C1_JOURNAL_MAX_BYTES if production else conf.get("allocation_store_max_bytes", 1048576),
            "allocation_store_max_bytes",
            65536,
            C1_JOURNAL_MAX_BYTES,
        ),
        ...
        "admission_pow_max_age_s": bounded_json_int(
            C1_ADMISSION_EPOCH_SECONDS if production else conf.get("admission_pow_max_age_s", 300),
            "admission_pow_max_age_s",
            30,
            900,
        ),
        "allocation_witness_command": allocation_witness_command,
...
DEVELOPMENT_ONLY_ALLOW_UNSAFE_ALLOCATOR = SETTINGS["development_only_allow_unsafe_allocator"]
...
ALLOW_INITIAL_ALLOCATION_STORE_CREATION = SETTINGS["allow_initial_allocation_store_creation"]
...
CONSENSUS_RESERVATION_COMMAND = SETTINGS["consensus_reservation_command"]
...
def _warn(message, error):
    ...
    print("[veld_wrapd] %s: %s" % (message, detail), file=sys.stderr, flush=True)
...
def _run_bounded_command(argv, stdout_limit, stderr_limit, timeout_s, description):
    ...
    process = subprocess.Popen(
        argv, stdout=subprocess.PIPE, stderr=subprocess.PIPE, close_fds=True, start_new_session=True
    )
    ...
    readers = [
        threading.Thread(target=drain, args=(process.stdout, "stdout", stdout_limit), daemon=True),
        threading.Thread(target=drain, args=(process.stderr, "stderr", stderr_limit), daemon=True),
    ]
    ...
    if overflow:
        raise RuntimeError("%s %s exceeds its byte limit" % (description, overflow[0]))
...
def _btc_output(method, *args):
    ...
    cmd = (
        list(CLI_BASE)
        + (["-rpcwallet=%s" % WALLET] if WALLET else [])
        + [method]
        + [str(arg) for arg in args]
    )
    code, stdout, stderr = _run_bounded_command(
        cmd, MAX_CLI_STDOUT, MAX_CLI_STDERR, 30, "bitcoin-cli %s" % method
    )
    if code != 0:
        try:
...
def _btc_json(method, *args):
    return load_bounded_json_response(
        io.BytesIO(_btc_output(method, *args)), MAX_CLI_STDOUT, "bitcoin-cli %s response" % method
    )
...
def _load_veld_token(rpc):
    ...
    if token_cmd is not None:
        ...
        code, stdout, _stderr = _run_bounded_command(
            command, MAX_TOKEN_BYTES, MAX_CLI_STDERR, 30, "Veld RPC token command"
        )
        if code != 0:
            raise RuntimeError("Veld RPC token command failed")
        raw = stdout
    elif isinstance(token_file, str) and token_file:
        raw = read_bounded_secret_file(token_file, MAX_TOKEN_BYTES, "Veld RPC token")
    else:
...
def _veld(method, params=None):
    ...
    with open_rpc_request(request, timeout=20) as rpc_response:
        response = load_bounded_json_response(rpc_response, 32 * 1024 * 1024, "Veld RPC response")
...
def _configure_production():
    ...
    if (
        ...
        or type(peg.get("issuer_max_per_mint_sats")) is not int
        or peg["issuer_max_per_mint_sats"] < _C1_POLICY["minimum_allocation_sats"]
    ):
        raise RuntimeError("C1 owner minimum exceeds compiled issuer_max_per_mint_sats")
    if (
        type(peg.get("issuer_static_custody_cap_sats")) is not int
        or peg["issuer_static_custody_cap_sats"] != _C1_POLICY["consensus_custody_ceiling_sats"]
    ):
        raise RuntimeError("compiled issuer custody ceiling differs from signed C1 policy")
    _custody = custody_binding.load_manifest(
        ...
        expected_consensus_manifest_sha256=CONF.get("custody_consensus_manifest_sha256"),
    )
    ...
    if (
        ...
        or entries[0].get("desc") != _custody["descriptor"]
    ...
    required_range = [C1_OPERATOR_RANGE[0], _C1_POLICY["public_descriptor_range"][1]]
    if _custody.get("range") != required_range or entries[0].get("range") != required_range:
        raise RuntimeError(
            "C1 public range requires a ceremony-pinned custody manifest 0-%d" % required_range[1]
        )
    custody_binding.verify_core_derivation(lambda method, *args: _btc_json(method, *args), _custody)
    ...
    _ALLOCATOR = WrapAllocationJournal(
        ...
        policy=_C1_POLICY,
        policy_sha256=_C1_POLICY_SHA256,
        ...
        sequence_status=lambda allocation_id_value: _veld(
            "getbtcveldc1reservation", [allocation_id_value]
        ),
...
def _verify_descriptor_allocator_separation(entry, binding):
    ...
    public_end = (
        999
        if legacy_fixture
        else _C1_POLICY["public_descriptor_range"][1]
        if _C1_POLICY
        else C1_INITIAL_PUBLIC_RANGE_END
    )
    if legacy_fixture:
        ...
        raise RuntimeError("C1 custody pool descriptor must be inactive/non-cursor")
    ...
    if not legacy_fixture:
        for index in (public_start, (public_start + public_end) // 2, public_end):
            derived = _btc_json(
                "deriveaddresses", binding["descriptor"], "[%d,%d]" % (index, index)
            )
            if (
                ...
                raise RuntimeError("cannot derive pre-issued custody pool probe")
            probe = _btc_json("getaddressinfo", derived[0])
            if (
                not isinstance(probe, dict)
                or probe.get("scriptPubKey") != binding["script_pubkeys"][index]
                or probe.get("iswatchonly") is not True
                or probe.get("solvable") is not True
            ):
                raise RuntimeError("custody descriptor pool is not fully watched/solvable")
...
def _canonical_wrap_request_body(request):
    ...
    if (
        ...
        or not isinstance(veld_address, str)
        or not VELD_ADDR_RE.fullmatch(veld_address)
        or type(amount_sats) is not int
        or not 1 <= amount_sats <= MAX_MONEY_SATS
    ):
        raise ValueError("wrap admission request body is malformed")
    return b"\x00".join(
        (
            b"VELD-WRAP-ADMISSION-BODY-v2",
            request_id.encode("ascii"),
            veld_address.encode("ascii"),
            str(amount_sats).encode("ascii"),
        )
    )
...
def _verify_admission_pow(request, now=None, signature_verifier=None):
    ...
    if (
        not PRODUCTION
        and _C1_POLICY is None
        and isinstance(request, dict)
        and "admission_timestamp" in request
    ):
        ...
        if (
            ...
            or not re.fullmatch(r"[0-9a-f]{16}", nonce)
        ):
        ...
        if not _leading_zero_bits(hashlib.sha256(message).digest(), ADMISSION_POW_BITS):
    ...
    if (
        ...
        or not MLDSA65_PUBLIC_KEY_RE.fullmatch(public_key)
        or not isinstance(signature, str)
        or not MLDSA65_SIGNATURE_RE.fullmatch(signature)
    ):
    ...
    digest = hashlib.sha256(bytes.fromhex(beacon) + bytes.fromhex(nonce) + request_hash).digest()
    if not has_leading_zero_bits(digest, _C1_POLICY["pow_bits"]):
        raise ValueError("wrap admission proof does not meet difficulty")
    message = (
        b"VELD-WRAP-ADMISSION-v2\x00" + bytes.fromhex(beacon) + bytes.fromhex(nonce) + request_hash
    )
    verifier = signature_verifier
    if verifier is None:
        verifier_path = _absolute_config_path(CONF, "admission_verifier")
        verifier = lambda key, msg, sig: verify_mldsa65_with_keygen(key, msg, sig, verifier_path)
...
def _principal_hash(source):
    ...
    return hashlib.sha256(
        b"VELD-WRAP-ADMISSION-PRINCIPAL-v1\x00" + source.encode("ascii", "strict")
    ).hexdigest()


def _call_allocation_witness(action, record):
    """Call the independent allocation authority with policy-hash parity."""
    if action not in ("preflight_allocation", "register_allocation", "verify_allocation"):
    ...
    completed = run_bounded_subprocess(
        list(ALLOCATION_WITNESS_COMMAND),
        input_text=request,
        timeout=45,
        stdout_max=64 * 1024,
        stderr_max=64 * 1024,
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            "allocation witness refused/unavailable: " + completed.stderr.strip()[:240]
        )
    try:
    ...
    if action in ("register_allocation", "verify_allocation"):
        required |= {"expires_at", "deposit_observed_at", "funded_reserved_at", "minted_at"}
    if (
        ...
        or response.get("descriptor_index") != record["descriptor_index"]
        or response.get("commitment_blind") != record["commitment_blind"]
        or response.get("consensus_allocation_id") != record["consensus_allocation_id"]
        or response.get("authorized") is not True
        or type(response.get("idempotent")) is not bool
        or response.get("capacity_policy_sha256") != _C1_POLICY_SHA256
        or (
            action in ("register_allocation", "verify_allocation")
            and (
                response.get("expires_at") != record["expires_at"]
                or (
                    response.get("deposit_observed_at") is not None
                    and type(response.get("deposit_observed_at")) is not int
                )
                or (
                    response.get("funded_reserved_at") is not None
                    and type(response.get("funded_reserved_at")) is not int
                )
                or (
                    response.get("minted_at") is not None
                    and type(response.get("minted_at")) is not int
                )
            )
        )
        or (action == "verify_allocation" and response.get("idempotent") is not True)
    ):
        raise RuntimeError("allocation witness policy hash/acknowledgement is not exact")
    return response
...
def _ensure_consensus_reservation(record):
    ...
    completed = run_bounded_subprocess(
        list(CONSENSUS_RESERVATION_COMMAND),
        input_text=request,
        timeout=120,
        stdout_max=64 * 1024,
        stderr_max=64 * 1024,
    ...
    try:
        acknowledgement = strict_json_object(completed.stdout.encode("utf-8"))
    except Exception as exc:
        raise RuntimeError(
            "consensus reservation coordinator acknowledgement is malformed"
        ) from exc
    expected_ack = {
        "version",
        "allocation_id",
    ...
    if (
        ...
        or set(acknowledgement) != expected_ack
        or acknowledgement.get("version") != 3
        or acknowledgement.get("allocation_id") != record["consensus_allocation_id"]
        or any(
            type(acknowledgement.get(key)) is not bool
            for key in (
                "found",
                "active",
                "retired",
        ...
        or any(
            type(acknowledgement.get(key)) is not int or acknowledgement[key] < 0
            for key in ("last_sequence", "confirmations", "required_confirmations")
        )
        or acknowledgement["required_confirmations"] != C1_RESERVATION_FINALITY_DEPTH
        or (
            acknowledgement["funded"]
            and (
                not isinstance(acknowledgement["funding_outpoint"], str)
                or not re.fullmatch(
                    r"[0-9a-f]{64}:(?:0|[1-9][0-9]{0,9})", acknowledgement["funding_outpoint"]
                )
            )
        )
        or (not acknowledgement["funded"] and acknowledgement["funding_outpoint"] is not None)
    ):
        raise RuntimeError("consensus reservation coordinator acknowledgement is not exact")
    status = _veld("getbtcveldc1reservation", [record["consensus_allocation_id"]])
    if not isinstance(status, dict) or type(status.get("retired")) is not bool:
    ...
    if found and (
        status.get("allocation_id") != record["consensus_allocation_id"]
        ...
        or status["expires_height"] - status["created_height"] + 1 != C1_RESERVATION_LIFETIME_BLOCKS
        or type(status.get("confirmations")) is not int
        or status.get("required_confirmations") != C1_RESERVATION_FINALITY_DEPTH
    ):
        raise RuntimeError("C1 consensus reservation differs from the allocation journal")
    if found and status.get("exposed") is True:
        for field in (
            ...
            if type(status.get(field)) is not int or status[field] < 0:
                raise RuntimeError("C1 consensus funding-window status is malformed")
        if not (
    ...
    if status.get("funded") is True:
        if (
            not isinstance(status.get("funding_outpoint"), str)
            or not re.fullmatch(r"[0-9a-f]{64}:(?:0|[1-9][0-9]{0,9})", status["funding_outpoint"])
    ...
    ready = (
        ...
        and status.get("exposed") is True
        and type(status.get("exposed_height")) is int
        and status.get("exposure_canonical_depth_reached") is True
        and status["exposure_confirmations"] >= C1_RESERVATION_FINALITY_DEPTH
    )
    if not ready:
        ...
        public_status = {
            ...
            "last_sequence": (
                status.get("last_sequence") if type(status.get("last_sequence")) is int else 0
            ),
            "confirmations": (
                status.get("confirmations") if type(status.get("confirmations")) is int else 0
            ),
    ...
    result["recommended_send_cutoff_height"] = status["recommended_send_cutoff_height"]
    return result
...
def _open_journal_parent(path, create=False):
    ...
    try:
        ...
        if (
            ...
            raise RuntimeError("allocation journal directory must be owner-only")
        return name, dfd
    except Exception:
        os.close(dfd)
        raise
...
class WrapAllocationJournal:
    ...
    def __init__(
        ...
        *,
        ...
        if self.legacy_fixture:
            ...
            public_end = 999
            if (
                type(reserve_indices) is not int
                or type(per_principal) is not int
                or type(per_destination) is not int
            ):
            ...
            if (
                ...
                or not HEX64_RE.fullmatch(policy_sha256)
                or not callable(preflight_allocation)
                or not callable(ensure_consensus_reservation)
                or not callable(sequence_status)
                or max_bytes != self.policy["journal_max_bytes"]
            ):
        ...
        if (
            ...
            or (self.legacy_fixture and not public_start <= initial_next <= public_end)
            or (not self.legacy_fixture and initial_next is not None)
            or not callable(register_allocation)
            or (sequence_status is not None and not callable(sequence_status))
    ...
    def _load_or_initialize(self):
        try:
            ...
            if not self.allow_initialize:
                raise RuntimeError(
                    "allocation journal is absent; one-shot initialization approval is required"
                )
            self._save()
        ...
        try:
            try:
                fd = os.open(
                    name, os.O_RDONLY | os.O_NOFOLLOW | getattr(os, "O_CLOEXEC", 0), dir_fd=dfd
                )
            except FileNotFoundError:
                os.close(dfd)
                if not self.allow_initialize:
            ...
            if self.allow_initialize:
                raise RuntimeError(
                    "allow_initial_allocation_store_creation must be false after the journal exists"
                )
            info = os.fstat(fd)
            if (
        ...
        if (
            ...
            or (self.legacy_fixture and type(document.get("initial_next_index")) is not int)
        ...
        if not self.legacy_fixture:
            ...
            if document.get("pool_range_start") != self.public_start:
                raise RuntimeError("allocation journal pool range start is inconsistent")
            if stored_sequence == self.policy_sequence:
                ...
                self.policy_sequence != stored_sequence + 1
                or self.policy["previous_policy_sha256"] != stored_hash
                or self.public_end < stored_end
            ):
            ...
            for number, event in enumerate(self.events, 1):
                if (
                    ...
                    or not HEX64_RE.fullmatch(event["event_sha256"])
                ):
                ...
                previous = claimed
        self.initial_next = document["initial_next_index"] if self.legacy_fixture else None
        seen_indices = set()
        seen_blinds = set()
        reserved = 0
        for record in document["records"]:
            required_record_keys = self.RECORD_KEYS if self.legacy_fixture else self.C1_RECORD_KEYS
            ...
            if (
                ...
                or (self.legacy_fixture and not self.initial_next <= index <= self.public_end)
                or (not self.legacy_fixture and not self.public_start <= index <= self.public_end)
                or index in seen_indices
                or not isinstance(record["btc_address"], str)
                or not 14 <= len(record["btc_address"]) <= 100
                or record["script_pubkey"] != self.binding["script_pubkeys"][index]
                or record["state"] not in ("reserved", "allocated", "issued")
            ):
                raise RuntimeError("allocation journal record is invalid")
            if not self.legacy_fixture and (
                type(record["admitted_at"]) is not int
                or type(record["expires_at"]) is not int
                or record["expires_at"] - record["admitted_at"]
                != self.policy["unfunded_expiry_seconds"]
                or not isinstance(record["capacity_policy_sha256"], str)
                or not HEX64_RE.fullmatch(record["capacity_policy_sha256"])
                or not isinstance(record["commitment_blind"], str)
                or not HEX64_RE.fullmatch(record["commitment_blind"])
                or int(record["commitment_blind"], 16) == 0
                or record["commitment_blind"] in seen_blinds
                or not isinstance(record["consensus_allocation_id"], str)
                or not ALLOCATION_REQUEST_RE.fullmatch(record["consensus_allocation_id"])
                or int(record["consensus_allocation_id"], 16) == 0
                or int(record["consensus_allocation_id"], 16) > self.last_consensus_sequence
            ):
    ...
    def _ordered_records(self):
        if self.legacy_fixture:
            return sorted(self.records.values(), key=lambda record: record["descriptor_index"])
        return sorted(
            self.records.values(), key=lambda record: int(record["consensus_allocation_id"], 16)
        )
    ...
    def _append_event(self, action, request_id, now=None):
        ...
        core = {
            ...
            "previous_event_sha256": (self.events[-1]["event_sha256"] if self.events else "0" * 64),
        }
        core["event_sha256"] = hashlib.sha256(
    ...
    def _admission_capacity_available(self):
        if self.legacy_fixture:
            return True
        row_limit = self.max_rows * (100 - self.policy["lifecycle_reserve_percent"]) // 100
        byte_limit = self.max_bytes * (100 - self.policy["lifecycle_reserve_percent"]) // 100
        raw = (
            json.dumps(self._document(), sort_keys=True, separators=(",", ":"), allow_nan=False)
            + "\n"
        ).encode("utf-8")
    ...
    def _validate_record_sequence(self):
        ...
        if self.legacy_fixture:
            expected = list(range(self.initial_next, self.initial_next + len(ordered)))
            if indices != expected or (indices and indices[-1] > self.public_end):
                raise RuntimeError("allocation journal descriptor history is not contiguous")
        else:
            sequences = [int(record["consensus_allocation_id"], 16) for record in ordered]
            if (
                sequences != sorted(set(sequences))
                or any(sequence > self.last_consensus_sequence for sequence in sequences)
                or len(indices) != len(set(indices))
                or any(not self.public_start <= index <= self.public_end for index in indices)
            ):
                raise RuntimeError("allocation sequence or private descriptor mapping is invalid")
        reserved = [record for record in ordered if record["state"] != "issued"]
        if reserved and reserved != ordered[-1:]:
            raise RuntimeError("allocation journal pending intent is not the final index")
    ...
    def _validate_record_addresses(self):
        ...
        if not ordered:
            return
        first = ordered[0]["descriptor_index"] if self.legacy_fixture else self.public_start
        last = ordered[-1]["descriptor_index"] if self.legacy_fixture else self.public_end
        derived = _btc_json(
            "deriveaddresses", self.binding["descriptor"], "[%d,%d]" % (first, last)
        )
        if (
            not isinstance(derived, list)
            or len(derived) != last - first + 1
            or any(
                not isinstance(address, str) or not 14 <= len(address) <= 100 for address in derived
            )
        ):
            raise RuntimeError("cannot rederive allocation journal address history")
        for record in ordered:
            if record["btc_address"] != derived[record["descriptor_index"] - first]:
                raise RuntimeError("allocation journal address differs from descriptor index")
    ...
    def _save(self):
        ...
        if not self.legacy_fixture and len(self.records) + len(self.events) > self.max_rows:
        ...
        fd = -1
        published = False
        tmp_name = "%s.%d.%s.tmp" % (name, os.getpid(), os.urandom(16).hex())
        try:
            fd = os.open(
                tmp_name,
            ...
            if not published:
                raise _JournalNotPublishedError(
                    "allocation journal update was not published"
                ) from exc
    ...
    def _checkpoint(self, action):
        if action not in ("store", "verify") or ALLOCATION_CHECKPOINT_COMMAND is None:
        ...
        request = json.dumps(
            {
                ...
                "last_descriptor_index": (
                    max(record["descriptor_index"] for record in self.records.values())
                    if self.records
                    else self.public_start - 1
                ),
            },
            sort_keys=True,
            separators=(",", ":"),
        ...
        completed = run_bounded_subprocess(
            list(ALLOCATION_CHECKPOINT_COMMAND),
            input_text=request,
            timeout=45,
            stdout_max=64 * 1024,
            stderr_max=64 * 1024,
        ...
        try:
            ...
            raise RuntimeError("allocation checkpoint acknowledgement is invalid") from exc
        if (
            ...
            or answer.get("capacity_policy_sha256") != self.policy_sha256
            or answer.get("journal_sha256") != digest
            or answer.get("last_consensus_sequence") != self.last_consensus_sequence
            or type(answer.get("monotonic_sequence")) is not int
            or answer["monotonic_sequence"] < 1
        ):
            raise RuntimeError("allocation checkpoint authority refused/inexact acknowledgement")
    ...
    def _descriptor_entry(self):
        ...
        if (
            ...
            or not self.public_start <= next_index <= self.public_end + 1
        ):
    ...
    def _assert_core_cursor(self):
        ...
        if next_index not in self._expected_core_next():
            raise RuntimeError("custody allocator cursor differs from durable journal history")
    ...
    def _lifecycle_result(self, record, response, observed):
        ...
        if (
            not isinstance(response, dict)
            or response.get("expires_at") != record["expires_at"]
            or (
                response.get("deposit_observed_at") is not None
                and type(response.get("deposit_observed_at")) is not int
            )
            ...
            or (
                response.get("minted_at") is not None and type(response.get("minted_at")) is not int
            )
        ):
        ...
        if (funded_reserved_at is not None and deposit_observed_at is None) or (
        ...
        try:
            reservation = (
                self.ensure_consensus_reservation(record)
                if self.ensure_consensus_reservation is not None
                else None
            )
        except PendingConsensusReservation as pending:
            if (
                deposit_observed_at is None
                and observed >= record["expires_at"]
                and pending.status.get("exposed") is not True
            ):
                ...
                if pending.status.get("retired") is True and not any(
                    event["action"] == "expired" and event["request_id"] == record["request_id"]
            ...
            raise
        if reservation is None and deposit_observed_at is None and observed >= record["expires_at"]:
            raise ExpiredAllocation(record["expires_at"])
        ...
        if reservation is not None:
            ...
            send_window_open = tip <= reservation["recommended_send_cutoff_height"]
            canonical_funded = reservation.get("funded") is True
            if funded_reserved_at is not None and not canonical_funded:
                raise RuntimeError("allocation witness funded state is absent from consensus")
        result = dict(record)
        lifecycle = (
            "FUNDED_RESERVED"
            if canonical_funded
            else "FUNDING_OBSERVED"
            if deposit_observed_at is not None
            else "UNFUNDED"
            if send_window_open
            else "RECOVERY_ONLY"
        )
        result.update(
            {
                ...
                "minted_at": minted_at,
                "send_allowed": (
                    deposit_observed_at is None and not canonical_funded and send_window_open
                ),
                "lifecycle": lifecycle,
            }
        )
        if reservation is not None:
            result.update(
                {
                    "consensus_reservation_created_height": reservation["created_height"],
                    "consensus_reservation_exposed_height": reservation["exposed_height"],
                    "consensus_reservation_confirmations": reservation["confirmations"],
                    "send_starts_height": reservation["send_starts_height"],
                    "recommended_send_cutoff_height": reservation["recommended_send_cutoff_height"],
                    "funding_accepts_through_height": reservation["funding_accepts_through_height"],
                    "funding_expires_height": reservation["funding_expires_height"],
    ...
    def _finish_reserved(self, record, observed):
        ...
        if not self.legacy_fixture and record["state"] == "reserved":
            ...
            if next_index == index:
                returned = _btc("getnewaddress", record["veld_address"], "bech32m")
                if returned != record["btc_address"]:
                    raise RuntimeError("Core advanced a different custody descriptor address")
                next_index = self._descriptor_entry()
                if next_index != index + 1:
                    raise RuntimeError("Core did not advance the reserved custody descriptor index")
            elif next_index != index + 1:
                raise RuntimeError("custody allocator advanced outside the durable intent")
        elif self.legacy_fixture and next_index != index + 1:
            raise RuntimeError("allocated custody address differs from the Core cursor")
        info = _btc_json("getaddressinfo", record["btc_address"])
        if (
            not isinstance(info, dict)
            or info.get("scriptPubKey") != record["script_pubkey"]
            or (
                not self.legacy_fixture
                and (info.get("iswatchonly") is not True or info.get("solvable") is not True)
            )
    ...
    def _verify_issued_record(self, record, observed):
        ...
        if (
            ...
            raise RuntimeError("issued custody address ownership/label proof failed")
    ...
    def _synchronize_consensus_sequence(self):
        ...
        if (
            ...
            or status.get("allocation_id") != query_id
            or type(status.get("last_sequence")) is not int
            or not 0 <= status["last_sequence"] <= C1_UINT64_MAX
        ):
        ...
        if chain_sequence > self.last_consensus_sequence:
            raise RuntimeError("C1 chain sequence is ahead of the durable allocator journal")
        if chain_sequence < self.last_consensus_sequence:
            pending = next(
                (
                    record
                    for record in self.records.values()
                    if int(record["consensus_allocation_id"], 16) == self.last_consensus_sequence
                ),
                None,
            )
            if pending is None:
                raise RuntimeError("C1 unresolved sequence has no durable allocation intent")
            ...
            if (
                not isinstance(status, dict)
                or status.get("allocation_id") != query_id
                or status.get("last_sequence") != self.last_consensus_sequence
            ):
                raise RuntimeError("prior C1 allocation sequence is still unresolved")

    def allocate(self, request_id, principal, veld_address, amount_sats, now=None):
        if not ALLOCATION_REQUEST_RE.fullmatch(request_id or "") or not PRINCIPAL_HASH_RE.fullmatch(
            principal or ""
        ):
        ...
        with self.lock:
            ...
            if existing is not None:
                ...
                if (
                    existing["veld_address"] != veld_address
                ...
                if not self.legacy_fixture and admitted_at >= existing["expires_at"]:
                    raise ExpiredAllocation(existing["expires_at"])
                return self._finish_reserved(existing, admitted_at)
            if any(record["state"] != "issued" for record in self.records.values()):
                raise RuntimeError("a prior custody allocation needs exact retry/reconciliation")
            self._synchronize_consensus_sequence()
            if self.legacy_fixture:
                if (
                    sum(record["principal_hash"] == principal for record in self.records.values())
                    >= self.per_principal
                ):
                    raise RuntimeError("principal custody-address quota is full")
                if (
                    sum(record["veld_address"] == veld_address for record in self.records.values())
                ...
                if (
                    sum(
                        record["admitted_at"] // self.policy["epoch_seconds"] == epoch
                        for record in self.records.values()
                    )
                    >= self.policy["global_allocations_per_epoch"]
                ):
                    raise RuntimeError("AT_CAPACITY: C1 epoch allocation cap reached")
                if (
                    sum(record["admitted_at"] // 86_400 == day for record in self.records.values())
                    >= self.policy["global_allocations_per_day"]
                ):
                    raise RuntimeError("AT_CAPACITY: C1 daily allocation cap reached")
                live = [
                    record for record in self.records.values() if admitted_at < record["expires_at"]
                ]
                principal_sats = sum(
                    record["amount_sats"]
                    for record in live
            ...
            public_end = 999 - self.reserve_indices if self.legacy_fixture else self.public_end
            if self.legacy_fixture:
                index = cursor
            else:
                used = {record["descriptor_index"] for record in self.records.values()}
                remaining = [
                    index
                    for index in range(self.public_start, self.public_end + 1)
                    if index not in used
                ]
                if not remaining:
                    raise RuntimeError("AT_CAPACITY: public custody descriptor range exhausted")
                index = remaining[secrets.randbelow(len(remaining))]
            if index > public_end or (
                not self.legacy_fixture and not _c1_public_index_allowed(index, self.policy)
            ):
                raise RuntimeError(
                    (
                        "public custody allocator reserve threshold reached"
                        if self.legacy_fixture
                        else "AT_CAPACITY: public custody descriptor range exhausted"
                    )
                )
            derived = _btc_json(
                "deriveaddresses", self.binding["descriptor"], "[%d,%d]" % (index, index)
            )
            if (
                not isinstance(derived, list)
                or len(derived) != 1
            ...
            if not self.legacy_fixture:
                if self.last_consensus_sequence == C1_UINT64_MAX:
                    raise RuntimeError("AT_CAPACITY: C1 consensus allocation sequence exhausted")
                consensus_sequence = self.last_consensus_sequence + 1
                used_blinds = {row["commitment_blind"] for row in self.records.values()}
                commitment_blind = secrets.token_hex(32)
                while int(commitment_blind, 16) == 0 or commitment_blind in used_blinds:
                    commitment_blind = secrets.token_hex(32)
                record.update(
                    {
                        ...
                        "consensus_allocation_id": allocation_id(consensus_sequence),
                        "admitted_at": admitted_at,
                        "expires_at": admitted_at + self.policy["unfunded_expiry_seconds"],
                        "capacity_policy_sha256": self.policy_sha256,
                    }
                )
    ...
    def has_request(self, request_id):
        if not isinstance(request_id, str) or not ALLOCATION_REQUEST_RE.fullmatch(request_id):
            return False
        with self.lock:
        ...
        with self.lock:
            if refresh:
                ...
                if ordered and ordered[-1]["state"] != "issued":
                    raise RuntimeError("final custody allocation is not independently witnessed")
                if ordered:
                    self.register_allocation(dict(ordered[-1]), True)
            issued = sum(record["state"] == "issued" for record in self.records.values())
            pending_witness = sum(
                record["state"] == "allocated" for record in self.records.values()
            )
            public_end = 999 - self.reserve_indices if self.legacy_fixture else self.public_end
            answer = {
                ...
                "public_indices_remaining": (
                    ...
                    if self.legacy_fixture
                        ...
                        - self.public_start
                        + 1
                        - len({record["descriptor_index"] for record in self.records.values()})
                    )
                ),
                "reserve_threshold_reached": (
                    self.last_next > public_end
                    if self.legacy_fixture
                    else len(self.records) >= public_end - self.public_start + 1
                ),
            }
            if not self.legacy_fixture:
                answer.update(
...
def _verify_production_identity():
    if PRODUCTION:
        ...
        if (
            ...
            or supply_before.get("tip") != tip
            or not isinstance(supply_before.get("tip_hash"), str)
            or not HEX64_RE.fullmatch(supply_before["tip_hash"])
            or tip_hash_before != supply_before["tip_hash"]
            or tip_hash_after != tip_hash_before
            or any(peg.get(name) != peg_after.get(name) for name in fields)
            or peg.get("supply_sats") != supply_before["supply_sats"]
            or type(effective) is not int
            or effective < 0
            or type(reserved) is not int
            or reserved < 0
            or supply_before["supply_sats"] > effective
            or reserved > effective - supply_before["supply_sats"]
            or peg.get("issuer_mint_headroom_sats")
            != max(0, effective - supply_before["supply_sats"] - reserved)
        ):
...
def _bitcoin_freshness(now=None):
    ...
    if (
        ...
        or not isinstance(chain.get("bestblockhash"), str)
        or not HEX64_RE.fullmatch(chain["bestblockhash"])
    ):
        raise RuntimeError("Bitcoin Core freshness snapshot is incoherent")
    header = _btc_json("getblockheader", chain["bestblockhash"])
    if (
        not isinstance(header, dict)
        or header.get("hash") != chain["bestblockhash"]
        or type(header.get("time")) is not int
        or header["time"] > observed + 120
    ):
...
def _public_allocation_gate(peg, freshness):
    ...
    if (
        ...
        return "PEG_LOCKED"
    if not isinstance(freshness, dict) or freshness.get("phase") == "RECOVERY_ONLY":
...
class AtomicDualWindowLimiter:
    ...
    def _prune_hits(self, hits, cutoff):
        if hits and hits[-1] <= cutoff:
            ...
            return
        work = 0
        while hits and hits[0] <= cutoff and work < self.maintenance_budget:
            hits.popleft()
            work += 1
    ...
    def allow(self, source):
        ...
        with self.lock:
            ...
            if (self.global_hits and self.global_hits[0] <= cutoff) or (hits and hits[0] <= cutoff):
                return False
            if len(self.global_hits) >= self.global_limit or (
                hits is not None and len(hits) >= self.per_source
            ):
                return False
            if hits is None:
    ...
    def stored_hits(self):
        with self.lock:
            return len(self.global_hits) + sum(len(hits) for hits in self.sources.values())


_RATE_LIMITER = AtomicDualWindowLimiter(RL_PER_IP_PER_MIN, RL_GLOBAL_PER_MIN)
...
def _max_wrap_sats(peg):
    if not PRODUCTION:
        return bounded_json_int(CONFIG_MAX_WRAP_SATS, "max_wrap_sats", 1, MAX_MONEY_SATS)
    ...
    compatibility_max = bounded_json_int(
        peg.get("issuer_max_per_mint_sats"), "issuer_max_per_mint_sats", 1, MAX_MONEY_SATS
    )
    ...
    headroom = bounded_json_int(
        peg.get("issuer_mint_headroom_sats"), "issuer_mint_headroom_sats", 0, MAX_MONEY_SATS
    )
...
def _confirmations(peg):
    ...
    return bounded_json_int(
        peg.get("spv_k_btc") if isinstance(peg, dict) else None, "spv_k_btc", 1, 1_000_000
    )
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _send(self, code, obj):
        body = (
            b""
            if code == 204
            else json.dumps(obj, separators=(",", ":"), allow_nan=False).encode("utf-8")
        )
    ...
    def _read_json_body(self):
        ...
        if transfer:
            ...
            return None
        if len(lengths) != 1:
            self._send(411 if not lengths else 400, {"error": "one content length is required"})
            return None
        ...
        if length <= 0 or length > MAX_BODY:
            self._send(413 if length > MAX_BODY else 400, {"error": "bad body length"})
            return None
        if (
            len(content_types) != 1
    ...
    def do_OPTIONS(self):
        ...
        if (
            target is None
            or target.query
            or target.path not in ("/health", "/spk", "/wrap", "/admission")
        ):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/health" and not parsed.query:
            ...
            if not _rate_ok(source):
                return self._send(429, {"ok": False, "error": "rate limited — try again shortly"})
            try:
                ...
                return self._send(
                    200,
                    {
                        ...
                        "min_wrap_sats": (C1_MIN_ALLOCATION_SATS if PRODUCTION else 1),
                        ...
                        "public_allocation_status": _public_allocation_gate(peg, freshness),
                        "consensus_capacity_reservation": "C1R1_C1E1_C1C1_C1F1_MNP2",
                        "allocator": (_ALLOCATOR.capacity(refresh=True) if _ALLOCATOR else None),
                    },
                )
            except Exception as e:
                _warn("health check failed", e)
                return self._send(503, {"ok": False, "error": "backend health check failed"})
        ...
        if parsed.path == "/spk":
            ...
            if (
                not isinstance(info, dict)
                or info.get("isvalid") is not True
            ...
            if not isinstance(address_type, str) or len(address_type) > 32:
                address_type = "?"
            return self._send(200, {"address": addr, "scriptPubKey": script, "type": address_type})
        return self._send(404, {"error": "not found"})
    ...
    def do_POST(self):
        ...
        try:
            ...
            recorded_retry = PRODUCTION and _allocation_recorded(req.get("request_id")) is True
            if PRODUCTION and admission_status != "OPEN" and not recorded_retry:
                return self._send(
                    503,
                    {
                        ...
                        "bitcoin_freshness": freshness,
                        "request_recorded": _allocation_recorded(req.get("request_id")),
                    },
                )
            ...
            if PRODUCTION:
                ...
                allocation = _ALLOCATOR.allocate(req["request_id"], principal, veld, amount)
                ...
                if admission_status == "OPEN" and allocation.get("send_allowed") is True:
                    response_peg = _verify_production_identity()
                    response_freshness = _bitcoin_freshness()
                    admission_status = _public_allocation_gate(response_peg, response_freshness)
                ...
                if (
                    type(allocation.get("send_allowed")) is not bool
                    or allocation.get("lifecycle")
                    not in (
                        "UNFUNDED",
                        "FUNDING_OBSERVED",
                        "FUNDED_RESERVED",
                    ...
                    or type(allocation.get("expires_at")) is not int
                ):
                    raise RuntimeError("custody allocation lifecycle is unavailable")
                if not allocation["send_allowed"]:
                    return self._send(
                        200,
                        {
                            ...
                            "deposit_observed_at": allocation.get("deposit_observed_at"),
                            "funded_reserved_at": allocation.get("funded_reserved_at"),
                            ...
                            "send_starts_height": allocation.get("send_starts_height"),
                            ...
                            "funding_expires_height": allocation.get("funding_expires_height"),
                            "current_height": allocation.get("current_height"),
                            "capacity_warning": C1_CONSENSUS_CAPACITY_WARNING,
                            "note": (
                                "deposit already observed; do not send again"
                                if allocation["lifecycle"]
                                in ("FUNDING_OBSERVED", "FUNDED_RESERVED", "MINTED")
                ...
                script = (
                    address_info.get("scriptPubKey") if isinstance(address_info, dict) else None
                )
                if (
            ...
            return self._send(
                429 if at_capacity else 503,
                {
                    "error": ("AT_CAPACITY" if at_capacity else "custody wallet unavailable"),
        ...
        return self._send(
            200,
            {
                ...
                "max_sats": max_sats,
                "confirmations": _confirmations(peg),
                "lifecycle": (allocation["lifecycle"] if PRODUCTION else "UNFUNDED"),
                "send_allowed": True,
                "consensus_capacity_reserved": (
                    allocation.get("consensus_capacity_reserved") is True if PRODUCTION else False
                ),
                "consensus_reservation_created_height": (
                    allocation.get("consensus_reservation_created_height") if PRODUCTION else None
                ),
                "consensus_reservation_exposed_height": (
                    allocation.get("consensus_reservation_exposed_height") if PRODUCTION else None
                ),
                "send_starts_height": (
                    allocation.get("send_starts_height") if PRODUCTION else None
                ),
                "recommended_send_cutoff_height": (
                    allocation.get("recommended_send_cutoff_height") if PRODUCTION else None
                ),
                "funding_accepts_through_height": (
                    allocation.get("funding_accepts_through_height") if PRODUCTION else None
                ),
                "funding_expires_height": (
                    allocation.get("funding_expires_height") if PRODUCTION else None
                ),
                "current_height": (allocation.get("current_height") if PRODUCTION else None),
                "capacity_warning": C1_CONSENSUS_CAPACITY_WARNING,
                "note": (
                    "send exactly amount_sats before the recommended cutoff; "
...
def main():
    ...
    with BoundedThreadingTCPServer(
        ...
        request_read_timeout_s=HTTP_REQUEST_READ_TIMEOUT_S,
    ) as srv:
        srv.serve_forever()
