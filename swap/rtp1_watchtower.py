...
def verify_wallet_descriptor(result, binding):
    ...
    if (
        entry.get("desc") != binding["descriptor"]
        or entry.get("active") is not False
        or entry.get("internal", False) is not False
        or type(covered) is not list
        or len(covered) != 2
        or any(type(value) is not int for value in covered)
        or covered[0] != 0
        or not binding["range"][1] <= covered[1] <= 1_000_000
    ):
...
def read_snapshot(config, veld, bitcoin, scripts):
    ...
    if (
        ...
        or peg.get("reserve_semantics") != "rolling-outpoint-v1"
        or peg.get("reserve_status") not in {"EMPTY", "ACTIVE"}
        or peg.get("reserve_accounting_holds") is not True
        or peg.get("tip") != height
        or peg.get("supply_sats") != amount
        or type(peg.get("tip")) is not int
        or type(peg.get("supply_sats")) is not int
        or peg.get("custody_descriptor_sha256") != config["custody_descriptor_sha256"]
        or peg.get("custody_manifest_sha256") != config["custody_manifest_sha256"]
        or peg.get("issuer") != config["issuer"]
        or peg.get("token_id") != "btcVELD"
    ):
        raise ValueError("native reserve and supply are inconsistent or frozen")
    principal = _uint(
        peg.get("open_redemption_principal_sats"), MAX_BTC_SATS, "unsettled redemption principal"
    )
    ...
    for row in rows:
        ...
        if (
            ...
            or coin.get("bestblock") != before[1]
            or type(coin.get("confirmations")) is not int
            or coin["confirmations"] != confirmations
            or type(coin.get("scriptPubKey")) is not dict
            or coin["scriptPubKey"].get("hex") != row["scriptPubKey"]
            or _sats(coin.get("value")) != value
        ):
    ...
    if (
        _bitcoin_frame(bitcoin, config["bitcoin_network"], config["bitcoin_genesis"]) != before
        or veld("getbtcveldsupply", []) != supply
        or veld("getblockhash", [height]) != tip
    ):
    ...
    return {
        ...
        "open_redemption_principal_sats": principal,
        "tip": height,
        "tip_hash": tip,
        "bitcoin_height": before[0],
        "bitcoin_hash": before[1],
        "utxo_count": len(seen),
    }
...
class Rtp1Watchtower(Watchtower):
    def _core(self, method, *params):
        return self.btc.call(
            method,
            *(
                value if type(value) is str else json.dumps(value, allow_nan=False)
                for value in params
            ),
        )
    ...
    def __init__(self, cfg):
        ...
        if (
            cfg.get("production") is not True
            or cfg.get("veld_rpc", {}).get("expected_chain") != self.rtp1["expected_chain"]
        ):
            raise ValueError(
                "RTP1 watchtower requires its strict entry point and exact disposable chain pins"
            )
        ...
        if self.receiver_command is not None:
            ...
            if (
                ...
                or any(
                    type(arg) is not str or not arg or len(arg) > 4096 or "\x00" in arg
                    for arg in argv
                )
                or not os.path.isabs(argv[0])
            ):
                raise ValueError(
                    "local disposable heartbeat receiver requires bounded absolute argv"
                )
    ...
    def _configure_production_custody(self, btc_cfg, cfg):
        if (
            self.btc_api
            or self.btc is None
            or not btc_cfg.get("wallet")
            or self.custody_addresses
            or self.custody_desc_file
        ):
            raise ValueError(
                "RTP1 requires exactly one independently configured watch-only Core wallet"
            )
        binding = custody.load_manifest(
            cfg.get("custody_spk_manifest_file"),
            self.rtp1["custody_descriptor_sha256"],
            self.rtp1["custody_manifest_sha256"],
            expected_spv_spk_hex=self.rtp1["custody_script_hex"],
            network="regtest",
        )
    ...
    def _production_bitcoin_chain_identity(self):
        frame = _bitcoin_frame(
            lambda method, params: self._core(method, *params),
            "regtest",
            self.rtp1["bitcoin_genesis"],
        )
    ...
    def read_production_backing_snapshot(self, supply, tip, tip_hash):
        ...
        snapshot = read_snapshot(
            ...
            self.veld.rpc,
            lambda method, params: self._core(method, *params),
            self.production_binding["script_pubkeys"],
        )
        if (snapshot["supply_sats"], snapshot["tip"], snapshot["tip_hash"]) != (
            supply,
            tip,
            tip_hash,
        ):
    ...
    def _push(self, payload):
        ...
        result = run_bounded_subprocess(
            ...
            stdout_max=65536,
            stderr_max=65536,
            description="disposable heartbeat receiver",
        )
