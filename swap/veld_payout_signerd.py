...
CONFIG = os.environ.get(
    "VELD_PAYOUT_SIGNER_CONFIG", os.path.join(HERE, "payout-signer-config.json")
)
...
def same_obligation(a, b):
    a = rd.normalize_redeem(a)
    b = rd.normalize_redeem(b)
    return all(
        a[k] == b[k]
        for k in (
            "burn_txid",
...
def _custody_policy(cfg):
    ...
    consensus_manifest_hash = cfg.get("custody_consensus_manifest_sha256", manifest_hash)
    range_valid = (
        isinstance(configured_range, list)
        and len(configured_range) == 2
        and configured_range[0] == 0
        and type(configured_range[1]) is int
        and (configured_range[1] == 999 or 10999 <= configured_range[1] <= 1_000_000)
    )
    expected_count = configured_range[1] + 1 if range_valid else -1
    if (
        not isinstance(descriptor_hash, str)
        or not re.fullmatch(r"[0-9a-f]{64}", descriptor_hash)
        or not isinstance(manifest_hash, str)
        or not re.fullmatch(r"[0-9a-f]{64}", manifest_hash)
        or not isinstance(consensus_manifest_hash, str)
        or not re.fullmatch(r"[0-9a-f]{64}", consensus_manifest_hash)
        or not range_valid
        or not isinstance(configured, list)
        or len(configured) != expected_count
        or len(set(configured)) != expected_count
        or any(
            not isinstance(x, str) or not re.fullmatch(r"5120[0-9a-f]{64}", x) for x in configured
        )
    ):
...
def _verify_operational_custody_derivation(cfg, btc):
    ...
    if (
        ...
        or not descriptor.startswith("tr(")
        or hashlib.sha256(descriptor.encode("utf-8")).hexdigest() != policy["descriptor_sha256"]
    ):
    ...
    operational_bytes = (json.dumps(document, sort_keys=True, separators=(",", ":")) + "\n").encode(
        "utf-8"
    )
    ...
    consensus_bytes = (json.dumps(consensus, sort_keys=True, separators=(",", ":")) + "\n").encode(
        "utf-8"
    )
    if hashlib.sha256(consensus_bytes).hexdigest() != policy["consensus_manifest_sha256"]:
    ...
    matches = (
        [
            ...
            and entry.get("desc") == descriptor
            and entry.get("internal", False) is not True
            and entry.get("range") == policy["range"]
        ]
...
def _validate_unsigned_psbt_policy(decoded_psbt, decoded_tx):
    ...
    if (
        ...
        or not isinstance(tx_inputs, list)
        or len(inputs) != len(tx_inputs)
    ):
...
def _validate_fresh_burn_authority(veld, cfg, observed):
    ...
    try:
        tip_before = _wire_uint(veld.call("getblockcount", []), "Veld tip height")
        tip_hash_before = veld.call("getblockhash", [tip_before])
        if not isinstance(tip_hash_before, str) or not rd.TXID_RE.fullmatch(tip_hash_before):
        ...
        final_height = _wire_uint(
            peg.get("final_height") if isinstance(peg, dict) else None, "Veld final height"
        )
        ...
        if not canonical:
            refuse(why)
        tip_after = _wire_uint(veld.call("getblockcount", []), "Veld tip height")
    ...
    if tip_before - observed["burn_height"] <= rd.K_VELD or observed["burn_height"] > final_height:
...
def sign(req, cfg, store, veld, btc):
    ...
    if not isinstance(supplied_inputs, list) or any(
        not isinstance(x, dict) or type(x.get("vout")) is not int for x in supplied_inputs
    ):
    ...
    fee = req.get("fee_sats", -1)
    change = req.get("change_sats", -1)
    ...
    if type(fee) is not int or type(change) is not int or change < 0 or fee < 0 or fee > max_fee:
    ...
    rd.validate_payout_tx(
        btc, raw, observed, resolved, fee, change, change_spk, require_unsigned=True
    )
    ...
    try:
        store.commit_signing_proposal(rid, raw, [(x["txid"], x["vout"]) for x in resolved])
...
def main():
    try:
        ...
        cfg = load_bounded_json_file(
            os.path.abspath(CONFIG), MAX_CONFIG_BYTES, "payout signer configuration"
        )
        ...
        if "native_custody" in cfg:
            ...
            result = native_custody_service.dispatch(
                req,
                cfg,
                rd.VeldRpc(cfg["veld_rpc"]),
                rd.Btc(cfg["cli_base"], cfg["wallet"]),
