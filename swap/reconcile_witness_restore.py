...
def _merge_source(inventory, receipt, keygen, pubkey, source, txid=None, signed_digest=None):
    ...
    if entry is None:
        ...
        current_rank = (entry["receipt"]["beat_seq"], entry["receipt"]["reserved_at"])
    ...
    if txid is not None:
        if (
            ...
            or not sol.HASH256_RE.fullmatch(txid)
            or not isinstance(signed_digest, str)
            or not sol.HASH256_RE.fullmatch(signed_digest)
        ):
...
def validate_receipt_archive(archive):
    ...
    if (
        ...
        and archive.get("version") == 1
        and archive.get("kind") == "VELD_MINT_RECEIPT_ARCHIVE"
        and set(archive) == {"version", "kind", "entries"}
        and isinstance(archive["entries"], list)
    ):
    ...
    if (
        ...
        or set(archive) != required
        or archive.get("version") != witness.TERMINAL_ARCHIVE_VERSION
        or archive.get("kind") != witness.TERMINAL_ARCHIVE_KIND
    ):
    ...
    for field in ("txid", "signed_tx_sha256", "block_hash", "finalized_at_tip_hash"):
        if not isinstance(archive.get(field), str) or not sol.HASH256_RE.fullmatch(archive[field]):
    ...
    if (
        ...
        or height < 0
        or tip < height
        or tip - height + 1 <= witness.MAX_REORG_DEPTH
    ):
...
def validate_terminal_log_bytes(data):
    ...
    for sequence, line in enumerate(lines[1:], 1):
        ...
        try:
            record = witness.strict_json_loads(line.encode("utf-8"), "restored terminal tombstone")
...
def collect_inventory(
    ...
    for ledger_id, ledger in ledgers:
        ...
        for index, item in enumerate(ledger.get("reservations", [])):
            ...
            _merge_source(
                inventory, receipt, keygen, pubkey, "%s:%d" % (ledger_id, index), txid, digest
            )
    ...
    for signer_id, state in signer_states:
        ...
        try:
            ...
            fail("signer state %s is not current/reconciled: %s" % (signer_id, error))
        signed_by_request = {}
        ...
        for index, record in enumerate(signed):
            ...
            if (
                ...
                or receipt.get("sats") != record.get("sats")
                or receipt.get("recipient") != record.get("to")
            ):
        ...
        for field in ("pending", "confirmed"):
            for index, row in enumerate(accounting[field]):
                ...
                entry = _merge_source(
                    inventory, receipt, keygen, pubkey, "%s:%s:%d" % (signer_id, field, index)
                )
    ...
    for archive_id, archive in receipt_archives or []:
        ...
        if archive.get("kind") == witness.TERMINAL_ARCHIVE_KIND:
            digest = hashlib.sha256(witness._canonical_json_bytes(archive)).hexdigest()
        ...
        for index, item in enumerate(archive["entries"]):
            ...
            _merge_source(
                inventory, receipt, keygen, pubkey, "%s:%d" % (archive_id, index), txid, digest
            )
    ...
    for index, tombstone in enumerate(terminal_tombstones or []):
        ...
        if (
            ...
            != tuple(tombstone[field] for field in witness.TOMBSTONE_IDENTITY_FIELDS)
            or witness._receipt_full_sha256(receipt) != tombstone["receipt_sha256"]
        ):
            fail("terminal tombstone differs from its exact full receipt")
        if (
            entry.get("txid") != tombstone["txid"]
            or entry.get("signed_tx_sha256") != tombstone["signed_tx_sha256"]
        ):
    ...
    for rid, entry in inventory.items():
        ...
        if receipt.get("allocation_verified") is True:
            ...
            if (
                allocation_request in allocation_requests
                and allocation_requests[allocation_request] != rid
            ):
...
def validate_restored_ledger(ledger):
    ...
    if (
        ...
        and isinstance(ledger.get("reservations"), list)
    ):
        ledger = {
    ...
    for index, entry in enumerate(ledger["reservations"]):
        ...
        if status in ("confirmed", "final"):
            if (
                txid is None
                or not isinstance(entry.get("block_height"), int)
                or isinstance(entry.get("block_height"), bool)
...
def validate_artifact(
    artifact,
    restored_sha,
    restored_terminal_sha,
    signer_hashes,
    inventory,
    terminal_tombstones=None,
    snapshot_hashes=None,
    archive_hashes=None,
    approval_policy_sha=None,
    beat_pubkey_sha=None,
):
    if (
    ...
    if beat_pubkey_sha is not None and artifact.get("beat_pubkey_sha256") != beat_pubkey_sha:
    ...
    if (
        ...
        or artifact.get("signer_state_sha256s") != expected_hashes
    ):
    ...
    if (
        len(expected_snapshots) != len(set(expected_snapshots))
        or artifact.get("ledger_snapshot_sha256s") != expected_snapshots
    ):
        fail("artifact is not bound to the exact immutable ledger snapshots")
    if (
        len(expected_archives) != len(set(expected_archives))
        or artifact.get("receipt_archive_sha256s") != expected_archives
    ):
    ...
    quiesced = signerd._accounting_sats(wait.get("quiesced_height"), "quiesced_height")
    observed = signerd._accounting_sats(wait.get("observed_height"), "observed_height")
    if (
        observed != tip_height
        or observed - quiesced <= witness.MAX_REORG_DEPTH
        or wait.get("observed_tip_hash") != tip_hash
    ):
    ...
    if (
        ...
        or not sol.IDENTIFIER_RE.fullmatch(issuer_id)
        or not isinstance(witness_id, str)
        or not sol.IDENTIFIER_RE.fullmatch(witness_id)
    ):
    ...
    return {
        ...
        "terminal_head_sha256": (
            terminals[-1]["record_sha256"] if terminals else witness.ZERO_HASH
        ),
        "reservations": result,
    }
...
def _write_exact_backup(path, payload):
    if os.path.exists(path):
        if (
            legacy_reconcile.regular_bytes(path, private=True, max_bytes=256 * 1024 * 1024)
            != payload
        ):
...
def _atomic_apply_locked(paths, restored_bytes, restored_terminal_bytes, reconciled, artifact_sha):
    ...
    terminal_backup = paths["terminal"] + ".pre-reconciliation." + terminal_sha + ".jsonl"
    desired = _ledger_payload(reconciled)
    current = legacy_reconcile.regular_bytes(ledger_path, private=True, max_bytes=64 * 1024 * 1024)
    ...
    audit_marker = os.path.join(os.path.dirname(marker), "WITNESS_RECONCILED." + artifact_sha)
    ...
    dfd = os.open(os.path.dirname(marker), os.O_RDONLY | getattr(os, "O_DIRECTORY", 0))
    try:
        os.fsync(dfd)
    finally:
        os.close(dfd)
...
def atomic_apply(paths, restored_bytes, restored_terminal_bytes, reconciled, artifact_sha):
    ...
    with witness.exclusive_witness_state_lock(paths):
        return _atomic_apply_locked(
            paths, restored_bytes, restored_terminal_bytes, reconciled, artifact_sha
        )
...
def main():
    ...
    expected_restored_sha = (
        artifact.get("restored_ledger_sha256") if isinstance(artifact, dict) else None
    )
    ...
    expected_head = (
        terminal_tombstones[-1]["record_sha256"] if terminal_tombstones else witness.ZERO_HASH
    )
    if (
        restored_json["terminal_count"] != len(terminal_tombstones)
        or restored_json["terminal_head_sha256"] != expected_head
    ):
    ...
    for index, path in enumerate(args.ledger_snapshot):
        ...
        ledger_snapshots.append(("ledger-snapshot-%d" % index, validate_restored_ledger(snapshot)))
    ...
    for index, path in enumerate(args.receipt_archive):
        ...
        receipt_archives.append(("receipt-archive-%d" % index, validate_receipt_archive(archive)))
    ...
    inventory = collect_inventory(
        ...
        receipt_archives=receipt_archives,
    ...
    operators, statement_sha = legacy_reconcile.verify_approvals(artifact, policy, keygen)
