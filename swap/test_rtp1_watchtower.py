...
class Rtp1WatchtowerTests(unittest.TestCase):
    def test_core_keypool_extension_preserves_pinned_authority(self):
        ...
        for end in (999, 1000, 1000000):
            observer.verify_wallet_descriptor(
                {"descriptors": [{"desc": "pinned", "active": False, "range": [0, end]}]}, binding
            )
    ...
    def test_disposable_observer_may_require_more_burial_but_never_less(self):
        ...
        with (
            mock.patch.object(observer, "verify_expected_chain"),
            mock.patch.object(observer.custody, "verify_peg_identity"),
        ):
            for depth in (3, 144):
                instance.veld.rpc.return_value = {
    ...
    def setUp(self):
        ...
        self.scripts = [self.config["custody_script_hex"]] + [
            "5120" + format(i, "064x") for i in range(999)
        ]
        ...
        self.peg = dict(
            ...
            reserve_accounting_holds=True,
            open_redemption_principal_sats=20000,
            reserve_value_sats=125000,
            reserve_surplus_sats=5000,
            issuer=self.config["issuer"],
            token_id="btcVELD",
            custody_descriptor_sha256=self.config["custody_descriptor_sha256"],
            custody_manifest_sha256=self.config["custody_manifest_sha256"],
        )
        self.frame = {
            "chain": "regtest",
        ...
        self.coin = {
            "bestblock": "bb" * 32,
            "confirmations": 144,
            "scriptPubKey": {"hex": self.scripts[0]},
            "value": 0.002,
        }
    ...
    def veld(self, method, params):
        ...
        return copy.deepcopy(
            {
                "getbtcveldsupply": self.native,
                "getpeginfo": self.peg,
                "getblockhash": self.native["tip_hash"],
            }[method]
        )
    ...
    def bitcoin(self, method, params):
        ...
        if method == "getblockhash":
            return self.config["bitcoin_genesis"] if params == [0] else self.frame["bestblockhash"]
        return copy.deepcopy(
            {"getblockchaininfo": self.frame, "listunspent": self.rows, "gettxout": self.coin}[
                method
            ]
        )
    ...
    def read(self, **changes):
        with mock.patch.object(observer, "verify_expected_chain") as pinned:
            result = observer.read_snapshot(
                dict(self.config, **changes), self.veld, self.bitcoin, self.scripts
            )
    ...
    def test_frozen_or_incoherent_native_reserve_refuses(self):
        for field, value in (
            ("reserve_status", "FROZEN"),
            ("reserve_accounting_holds", 1),
            ("reserve_value_sats", 124999),
            ("open_redemption_principal_sats", True),
            ("custody_manifest_sha256", "dd" * 32),
            ("tip", 2999),
        ):
    ...
    def test_bitcoin_frame_change_refuses_snapshot(self):
        ...
        def moving(method, params):
            ...
            if method == "gettxout":
                self.frame["bestblockhash"] = "dd" * 32
            return result
    ...
    def test_live_profile_never_activates_this_adapter(self):
        ...
        with self.assertRaises(ValueError):
            self.read(bitcoin_network="main")
