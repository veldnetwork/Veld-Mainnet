...
SIGNER_CONFIG = os.environ.get("VELD_SIGNER_CONFIG",
...
def emit_unsigned_carrier_abandoned(error):
    sys.stdout.write(json.dumps(
        error.answer, sort_keys=True, separators=(",", ":"),
        allow_nan=False) + "\n")
    raise SystemExit(75)
...
def _secure_regular(path, private=False, exact_mode=None, executable=False):
    ...
    if exact_mode is not None and mode != exact_mode:
        raise ValueError("%s mode is %04o, expected %04o" %
                         (path, mode, exact_mode))
...
def _secure_service_directory(path):
    ...
    if (not stat.S_ISDIR(info.st_mode) or stat.S_ISLNK(info.st_mode) or
        ...
        raise ValueError(
            "signer service directory must be root/service-owned and non-writable")
    return path
...
def _secure_text(path, private=False, exact_mode=None, max_bytes=16 * 1024 * 1024):
    ...
    try:
        ...
        if (not stat.S_ISREG(opened.st_mode) or opened.st_nlink != 1 or
                opened.st_dev != info.st_dev or opened.st_ino != info.st_ino or
                opened.st_uid != info.st_uid or
                stat.S_IMODE(opened.st_mode) != stat.S_IMODE(info.st_mode)):
    ...
    def __init__(self, cfg):
        ...
        if cfg.get("token_cmd"):
            ...
            token = read_bounded_secret_file(
                cfg["token_file"], 4096,
                "trusted Veld RPC token").decode("ascii").strip()
        if not token:
    ...
    def call(self, method, params=None):
        ...
        with open_rpc_request(req, timeout=15) as resp:
            env = load_bounded_json_response(
                resp, 32 * 1024 * 1024, "trusted Veld RPC response")
        ...
        if isinstance(result, str):
            try:
                result = strict_json_loads(
                    result, "nested trusted Veld RPC result")
            except ValueError:
                pass
        return result
...
def parse_unsigned_tx_inputs(unsigned_tx_hex):
    ...
    for _ in range(count):
        ...
        txid = raw[pos:pos + 32].hex(); pos += 32
        vout = int.from_bytes(raw[pos:pos + 4], "little"); pos += 4
...
def resolve_mint_prevouts(unsigned_tx_hex, rpc, issuer_script_hex,
                          min_confirmations=DEFAULT_MIN_PREVOUT_CONFIRMATIONS):
...
def enforce_exact_mint_fee(total_in, total_out,
                           expected=EXPECTED_MINT_FEE_UNITS):
...
def validate_compiled_mint_policy(rpc, issuer, *, completion=False):
    ...
    try:
        values = [peg.get(name) for name in (
            "issuer_max_per_mint_sats", "issuer_static_custody_cap_sats",
            "issuer_effective_custody_cap_sats", "issuer_mint_headroom_sats")]
    ...
    if (MAX_WINDOW_SATS <= 0 or MAX_WINDOW_SATS > static_cap or
            effective_cap < 0 or effective_cap > static_cap or
            headroom < 0 or headroom > effective_cap):
...
def _exact_effect_locator(status, prefix):
    ...
    return (isinstance(txid, str) and re.fullmatch(r"[0-9a-f]{64}", txid) and
            type(height) is int and height >= 0 and
            isinstance(block_hash, str) and
            re.fullmatch(r"[0-9a-f]{64}", block_hash) and
            type(tx_index) is int and tx_index >= 0 and
            type(marker_vout) is int and marker_vout >= 0)
...
def validate_fresh_mint_boundary(rpc, issuer, sats, deposit_outpoint,
                                 proof_hex, retry_txid=None,
    ...
    """
    if (type(sats) is not int or sats <= 0 or
            not BTC_OUTPOINT_RE.fullmatch(deposit_outpoint or "") or
            ((reservation_allocation_id is None) !=
             isinstance(proof_hex, str)) or
            (proof_hex is not None and
             not re.fullmatch(r"[0-9a-f]+", proof_hex)) or
            (retry_txid is not None and
             (not isinstance(retry_txid, str) or
              not re.fullmatch(r"[0-9a-f]{64}", retry_txid))) or
            (reservation_allocation_id is not None and
             (not isinstance(reservation_allocation_id, str) or
              not CONSENSUS_ALLOCATION_RE.fullmatch(
                  reservation_allocation_id) or
              int(reservation_allocation_id, 16) == 0)) or
            (recipient is not None and
             (not isinstance(recipient, str) or
              not VELD_ADDR_RE.fullmatch(recipient))) or
            (reservation_script_pubkey is not None and
             (not isinstance(reservation_script_pubkey, str) or
              not re.fullmatch(r"5120[0-9a-f]{64}",
                               reservation_script_pubkey))) or
            (reservation_commitment_blind is not None and
             (not isinstance(reservation_commitment_blind, str) or
              not re.fullmatch(r"[0-9a-f]{64}",
                               reservation_commitment_blind) or
              int(reservation_commitment_blind, 16) == 0)) or
            ((reservation_allocation_id is None) !=
             (reservation_script_pubkey is None)) or
            ((reservation_allocation_id is None) !=
             (reservation_commitment_blind is None))):
        raise ValueError("mint boundary identity is malformed")
    completion = reservation_allocation_id is not None
    peg = validate_compiled_mint_policy(
        rpc, issuer, completion=completion)
    ...
    peg_after = validate_compiled_mint_policy(
        rpc, issuer, completion=completion)
    capacity_fields = (
        "tip", "supply_sats", "issuer_effective_custody_cap_sats",
        "issuer_reserved_sats", "issuer_mint_headroom_sats",
    ...
    if (not isinstance(supply_before, dict) or
            not isinstance(supply_after, dict) or
            ...
            type(supply_before.get("tip")) is not int or
            supply_before["tip"] < 0 or
            not isinstance(supply_before.get("tip_hash"), str) or
            not re.fullmatch(r"[0-9a-f]{64}", supply_before["tip_hash"])):
    ...
    if (peg.get("tip") != supply_before["tip"] or
            peg.get("supply_sats") != supply_sats or
            type(headroom) is not int or headroom != expected_headroom):
        raise ValueError("getpeginfo capacity differs from coherent supply tuple")
    if (not isinstance(status, dict) or
            status.get("outpoint") != deposit_outpoint or
            ...
            not re.fullmatch(r"[0-9a-f]{64}", status["root"]) or
            ...
            not re.fullmatch(r"[0-9a-f]{64}", status["tip_hash"]) or
            type(peg.get("tip")) is not int or
            peg["tip"] != status["tip"] or
            status["tip_hash"] != supply_before["tip_hash"]):
    ...
    if consumed:
        ...
        if (status["minted"] or not _null_locator(status, "accepted") or
                not _null_locator(status, "consumer") or
                not _null_locator(status, "credit") or
                any(status.get(name) is not None for name in accepted_metadata)):
            raise ValueError(
                "unconsumed mint-nullifier unexpectedly has effect metadata")

    if reservation_allocation_id is not None:
        if not consumed:
            raise ValueError("C1 MNP2 requires a canonical C1_FUND consumer")
        if (status.get("c1_allocation_id") != reservation_allocation_id or
                not _exact_effect_locator(status, "consumer")):
        ...
        if not status["minted"]:
            if (status.get("accepted_effect_kind") != "C1_FUND" or
                    not _null_locator(status, "credit")):
                raise ValueError("C1 outpoint is not in the funded-only state")
        else:
            if (status.get("accepted_effect_kind") != "C1_MINT" or
                    not _exact_effect_locator(status, "credit") or
                    retry_txid is None or
                    status.get("credit_txid") != retry_txid or
                    status.get("accepted_txid") != retry_txid):
                raise ValueError("C1 outpoint was minted by another carrier")
            return {"effect_confirmed_retry": True, "peg": peg,
                    "status": status}

        reservation = rpc.call(
            "getbtcveldc1reservation", [reservation_allocation_id])
        if (not isinstance(reservation, dict) or
                reservation.get("allocation_id") !=
                    reservation_allocation_id or
                reservation.get("tip") != supply_before["tip"] or
                reservation.get("found") is not True or
                reservation.get("active") is not True or
                reservation.get("exposed") is not True or
                reservation.get("funded") is not True or
                reservation.get("funding_outpoint") != deposit_outpoint or
                reservation.get("recipient") != recipient or
                reservation.get("amount_sats") != sats or
                reservation.get("allocation_commitment") !=
                    allocation_commitment(
                        reservation_allocation_id, recipient, sats,
                        reservation_script_pubkey,
                        reservation_commitment_blind)):
            raise ValueError(
                "matching funded C1 consensus reservation is unavailable")
    else:
        if consumed:
            if (status.get("minted") is not True or
                    status.get("accepted_effect_kind") != "MINT" or
                    retry_txid is None or
                    status.get("accepted_txid") != retry_txid):
                raise ValueError(
                    "BTC deposit outpoint is already consumed by another mint")
            return {"effect_confirmed_retry": True, "peg": peg,
                    "status": status}
        if status["proof_hex"] != proof_hex:
            raise ValueError("unsigned MNP1 proof differs from fresh trusted status")
    if reservation_allocation_id is None and (
            type(headroom) is not int or sats > headroom):
        raise ValueError(
            "mint amount exceeds fresh issuer_mint_headroom_sats")
...
def mint_params_from_tx(unsigned_tx_hex, issuer, issuer_p2pkh_hex):
    ...
    r = run_bounded_subprocess(
        ...
        input_text="", timeout=30, stdout_max=MAX_CHILD_OUTPUT_BYTES,
        stderr_max=1024 * 1024, description="decode-mint")
    if r.returncode != 0:
    ...
    if (type(sats) is not int or type(total_out) is not int or
            total_out < 0 or total_out > MAX_ACCOUNTING_SATS):
    ...
    if proof is not None:
        ...
        if len(proof) != 32 + sum(
                byte.bit_count() for byte in proof[:32]) * 32:
            refuse("MINT MNP1 proof bitmap/hash count is non-canonical")
    elif (int(reservation_allocation_id, 16) == 0 or
          int(reservation_commitment_blind, 16) == 0):
    ...
    return (to, sats, total_out, outpoint, proof_hex,
            reservation_allocation_id, reservation_script_pubkey,
            reservation_commitment_blind)
...
def validate_mint_allocation_claim(value, recipient, sats, deposit_outpoint):
    ...
    required = {
        ...
        "descriptor_index", "capacity_policy_sha256", "commitment_blind",
    ...
    if (not isinstance(request_id, str) or
            not re.fullmatch(r"[0-9a-f]{32}", request_id) or
            ...
            not CONSENSUS_ALLOCATION_RE.fullmatch(consensus_allocation_id) or
            int(consensus_allocation_id, 16) == 0 or
            not isinstance(outpoint, str) or
            not BTC_OUTPOINT_RE.fullmatch(outpoint) or
            outpoint != deposit_outpoint or
            type(descriptor_index) is not int or
            type(range_start) is not int or range_start != 1000 or
            type(range_end) is not int or not 10999 <= range_end <= 1_000_000 or
            not range_start <= descriptor_index <= range_end or
            not isinstance(policy_sha256, str) or
            not sol.HASH256_RE.fullmatch(policy_sha256) or
            not isinstance(recipient, str) or
            not VELD_ADDR_RE.fullmatch(recipient) or
...
def _accounting_sats(value, field, allow_zero=True):
    if (isinstance(value, bool) or not isinstance(value, int) or value < 0 or
            value > MAX_ACCOUNTING_SATS or (not allow_zero and value == 0)):
        raise ValueError("%s is not a bounded %sinteger" %
...
def _validate_signed_audit_records(state):
    ...
    for index, record in enumerate(records):
        ...
        if (not isinstance(request_id, str) or
                not re.fullmatch(r"[0-9a-f]{64}", request_id) or
                request_id in seen):
            raise ValueError(
                "signed audit record %d has an invalid/duplicate request_id" % index)
        ...
        if (not isinstance(signed_hex, str) or len(signed_hex) % 2 or
                len(signed_hex) > MAX_CHILD_OUTPUT_BYTES * 2 or
        ...
        sats = _accounting_sats(
            record.get("sats"), "signed[%d].sats" % index, allow_zero=False)
        signed_at = _accounting_sats(
            record.get("at"), "signed[%d].at" % index)
        calculated = hashlib.sha256(
            hashlib.sha256(bytes.fromhex(signed_hex)).digest()).hexdigest()
        if calculated != txid:
            raise ValueError(
                "signed audit record %d does not bind its bytes to the recorded txid" % index)
        normalized = {
            "request_id": request_id, "txid": txid,
            "signed_tx_hex": signed_hex.lower(), "sats": sats,
            "to": recipient, "at": signed_at,
        }
        ...
        if receipt is not None:
            ...
            if not ok:
                raise ValueError(
                    "signed audit record %d witness receipt: %s" % (index, why))
            if (receipt["request_id"] != request_id or
                    receipt["sats"] != sats or
            ...
            if not isinstance(committed, bool):
                raise ValueError(
                    "signed audit record %d witness_committed is not boolean" % index)
            normalized["witness_receipt"] = receipt
            normalized["witness_committed"] = committed
        elif "witness_committed" in record:
            raise ValueError(
                "signed audit record %d commits a missing witness receipt" % index)
...
def _legacy_pending_entries(state):
    ...
    for i, record in enumerate(records):
        ...
        sats = _accounting_sats(record.get("sats"),
                                "legacy signed[%d].sats" % i, allow_zero=False)
        ...
        if not isinstance(request_id, str) or not re.fullmatch(r"[0-9a-f]{64}", request_id):
            request_id = "legacy-" + hashlib.sha256(
                (str(i) + ":" + material).encode()).hexdigest()
        entry = {
...
def _validate_mint_entries(entries, field, require_txid=False):
    ...
    for i, entry in enumerate(entries):
        ...
        if (not isinstance(request_id, str) or not request_id or
                len(request_id) > 128 or request_id in seen):
            raise ValueError("%s mint %d has an invalid/duplicate request_id" % (field, i))
        sats = _accounting_sats(entry.get("sats"),
                                "%s[%d].sats" % (field, i), allow_zero=False)
        txid = entry.get("txid")
        if txid is not None and (not isinstance(txid, str) or
                                 not re.fullmatch(r"[0-9a-f]{64}", txid)):
        ...
        total += sats; seen.add(request_id)
        normalized = {"request_id": request_id, "sats": sats,
                      "signed_at": _accounting_sats(
                          entry.get("signed_at", 0),
                          "%s[%d].signed_at" % (field, i))}
        if txid is not None:
        ...
        if receipt is not None:
            ...
            if (receipt["request_id"] != request_id or
                    receipt["sats"] != sats):
                raise ValueError("%s mint %d witness receipt does not bind entry" %
                                 (field, i))
            normalized["witness_receipt"] = receipt
            committed = entry.get("witness_committed", False)
            if not isinstance(committed, bool):
                raise ValueError("%s mint %d witness_committed is not boolean" %
                                 (field, i))
            normalized["witness_committed"] = committed
        clean.append(normalized)
    return clean, total
...
def _initialize_mint_accounting(state):
    ...
    if migrated:
        ...
        if state.get("signed") or "total_signed" in state:
            raise ValueError(
                "legacy signer state requires offline mint-state reconciliation")
        accounting = {"version": MINT_ACCOUNTING_VERSION,
                      "pending": [], "confirmed": []}
    ...
    pending, ignored_pending_total = _validate_mint_entries(
        accounting.get("pending"), "pending")
    ...
    audit_by_id = {
        ...
        for record in _validate_signed_audit_records(state)
    }
    for entry in pending + confirmed:
        ...
        if (audit is None or audit["txid"] != entry["txid"] or
                audit["sats"] != entry["sats"]):
            raise ValueError(
                "signed outstanding mint is missing/mismatched in audit journal")
        entry_receipt = entry.get("witness_receipt")
        audit_receipt = audit.get("witness_receipt")
        if ((entry_receipt or None) != (audit_receipt or None) or
                entry.get("witness_committed", False) !=
                audit.get("witness_committed", False)):
            raise ValueError(
                "outstanding mint witness state differs from audit journal")
    return accounting, migrated


def _copy_accounting_entry(entry):
    keys = ("request_id", "sats", "signed_at", "txid",
            "witness_receipt", "witness_committed")
    ...
    try:
        ...
        if (isinstance(receipt, dict) and
            ...
            if (not isinstance(supply_before, dict) or
                    supply_before != supply_after or
                    set(supply_before) != {"supply_sats", "tip", "tip_hash"} or
                    supply_before.get("supply_sats") != hb.get("supply_sats") or
                    supply_before.get("tip") != hb.get("tip") or
                    supply_before.get("tip_hash") != hb.get("tip_hash") or
                    not isinstance(status, dict) or
                    status.get("outpoint") != outpoint or
                    type(status.get("consumed")) is not bool or
                    status.get("proof_version") != "MNP1" or
                    status.get("tip") != hb.get("tip") or
                    status.get("tip_hash") != supply_before["tip_hash"]):
                return None
            if not status["consumed"]:
                return None
            if (status.get("accepted_txid") != txid or
                    type(status.get("accepted_block_height")) is not int or
                    status["accepted_block_height"] < 0 or
            ...
            heartbeat_tip = _accounting_sats(
                hb.get("tip"), "heartbeat tip")
            if height > heartbeat_tip or rpc.call(
                    "getblockhash", [height]) != block_hash:
                return None
            return {"confirmations": heartbeat_tip - height + 1,
        ...
        if reported_confirmations < snapshot_confirmations:
            return None
        return {"confirmations": snapshot_confirmations, "block_height": height,
...
def reconcile_mint_accounting(state, hb, rpc=None):
    ...
    for entry in confirmed:
        ...
        if info is None:
            ...
            kept.update({"block_height": info["block_height"],
    ...
    for entry in pending + reorged:
        ...
        if info is None:
            ...
            watched = dict(entry)
            watched.update({"block_height": info["block_height"],
                            "block_hash": info["block_hash"]})
...
def prune_finalized_signed_audits(state, keep_request_id=None, now=None):
    ...
    outstanding = {
        ...
        for entry in accounting[field]
    }
    cutoff = (time.time() if now is None else float(now)) - WINDOW_SECS
    records = _validate_signed_audit_records(state)
    kept = [
        record for record in records
        if (record["request_id"] == keep_request_id or
            record["request_id"] in outstanding or
            record["at"] >= cutoff)
    ]
...
def record_signed_mint(state, request_id, txid, signed_tx_hex,
                       sats, recipient, signed_at=None):
    ...
    calculated = hashlib.sha256(
        hashlib.sha256(bytes.fromhex(signed_tx_hex)).digest()).hexdigest()
    if calculated != txid:
    ...
    now = (int(time.time()) if signed_at is None else
           _accounting_sats(signed_at, "signed mint timestamp"))
    ...
    if reservation is None:
        ...
        pending.append({"request_id": request_id, "txid": txid,
                        "sats": sats, "signed_at": now})
    else:
    ...
    accounting["pending"] = pending
    audit = {"request_id": request_id, "txid": txid,
             "signed_tx_hex": signed_tx_hex.lower(), "sats": sats,
             "to": recipient, "at": now}
...
def load_signer_state(path=None):
    ...
    if not os.path.lexists(path):
        raise FileNotFoundError(
            "signer authority state is missing; never auto-initialize")
    state = strict_json_loads(
        _secure_text(os.path.abspath(path), private=True,
                     max_bytes=SIGNER_STATE_MAX_BYTES), "signer state")
    if not isinstance(state, dict):
...
def save_signer_state_durable(state, path=None,
                              max_bytes=SIGNER_STATE_MAX_BYTES):
    ...
    if (not stat.S_ISDIR(dir_info.st_mode) or stat.S_ISLNK(dir_info.st_mode) or
            dir_info.st_uid not in (0, os.geteuid()) or
            stat.S_IMODE(dir_info.st_mode) & 0o022):
        raise ValueError(
...
def direct_mint_prevout_owner_id(deposit_outpoint):
    if (not BTC_OUTPOINT_RE.fullmatch(str(deposit_outpoint)) or
            int(deposit_outpoint.rsplit(":", 1)[1]) > 0xffffffff):
...
def c1_prevout_owner_id(allocation_id, phase, funding_sha256=None):
    if (not CONSENSUS_ALLOCATION_RE.fullmatch(str(allocation_id)) or
            phase not in ("RESERVE", "EXPOSE", "CANCEL", "FUND") or
            ((phase == "FUND") != (funding_sha256 is not None)) or
            (funding_sha256 is not None and
             not re.fullmatch(r"[0-9a-f]{64}", str(funding_sha256)))):
...
def _parse_prevout_owner_id(owner_id):
    ...
    direct = re.fullmatch(
        r"mint-direct:([0-9a-f]{64}:(?:0|[1-9][0-9]{0,9}))",
        str(owner_id))
...
def _empty_prevout_journal():
    return {"version": PREVOUT_JOURNAL_VERSION, "owners": {},
            "terminal_observations": {}, "scan_cursor": None,
            "cleanup_staging_ids": []}


def _validate_prevout_journal(state):
    if (not isinstance(state, dict) or set(state) != {
            "version", "owners", "terminal_observations", "scan_cursor",
            "cleanup_staging_ids"} or
            state.get("version") != PREVOUT_JOURNAL_VERSION or
            not isinstance(state.get("owners"), dict) or
            len(state["owners"]) > PREVOUT_JOURNAL_MAX_OWNERS or
            not isinstance(state.get("terminal_observations"), dict) or
            len(state["terminal_observations"]) > PREVOUT_JOURNAL_MAX_OWNERS or
            not isinstance(state.get("cleanup_staging_ids"), list) or
            len(state["cleanup_staging_ids"]) > PREVOUT_JOURNAL_MAX_OWNERS or
            len(set(state["cleanup_staging_ids"])) !=
                len(state["cleanup_staging_ids"]) or
            any(not re.fullmatch(r"[0-9a-f]{64}", str(staging_id))
                for staging_id in state["cleanup_staging_ids"]) or
            (state.get("scan_cursor") is not None and
             not isinstance(state["scan_cursor"], str))):
    ...
    for owner_id, owner in state["owners"].items():
        ...
        if (not isinstance(owner, dict) or set(owner) != {
                "capability", "allocation_id", "deposit_outpoint", "active",
                "revoked_unsigned_sha256",
                "revoked_conflicting_input_outpoints"} or
                owner.get("capability") != capability or
                owner.get("allocation_id") != allocation_id or
                ((capability in ("mint", "mint-direct")) !=
                 isinstance(owner.get("deposit_outpoint"), str)) or
                (owner.get("deposit_outpoint") is not None and
                 (not BTC_OUTPOINT_RE.fullmatch(owner["deposit_outpoint"]) or
                  int(owner["deposit_outpoint"].rsplit(":", 1)[1]) >
                      0xffffffff)) or
                not isinstance(owner.get("revoked_unsigned_sha256"), list) or
                len(owner["revoked_unsigned_sha256"]) >
                    PREVOUT_MAX_REVOKED_PER_OWNER or
                len(set(owner["revoked_unsigned_sha256"])) !=
                    len(owner["revoked_unsigned_sha256"]) or
                any(not re.fullmatch(r"[0-9a-f]{64}", str(item))
                    for item in owner["revoked_unsigned_sha256"]) or
                not isinstance(
                    owner.get("revoked_conflicting_input_outpoints"), dict) or
                set(owner["revoked_conflicting_input_outpoints"]) !=
                    set(owner["revoked_unsigned_sha256"])):
            raise ValueError("issuer prevout owner record is invalid")
        for revoked_hash, conflicts in \
                owner["revoked_conflicting_input_outpoints"].items():
            ...
            if (not isinstance(conflicts, list) or
                    len(conflicts) > PREVOUT_MAX_REVOKED_PER_OWNER or
                    conflicts != sorted(set(conflicts)) or
                    any(not BTC_OUTPOINT_RE.fullmatch(str(outpoint)) or
                        int(outpoint.rsplit(":", 1)[1]) > 0xffffffff
                        for outpoint in conflicts)):
                raise ValueError(
                    "issuer prevout revocation conflict set is invalid")
        active = owner["active"]
        if active is None:
            continue
        if (not isinstance(active, dict) or set(active) != {
                "unsigned_sha256", "input_outpoints", "txid", "created_at",
                "signing_state", "staging_id"} or
                not re.fullmatch(r"[0-9a-f]{64}",
                                 str(active.get("unsigned_sha256"))) or
                active["unsigned_sha256"] in
                    owner["revoked_unsigned_sha256"] or
                not isinstance(active.get("input_outpoints"), list) or
                not active["input_outpoints"] or
                len(active["input_outpoints"]) > 10_000 or
                len(set(active["input_outpoints"])) !=
                    len(active["input_outpoints"]) or
                any(not BTC_OUTPOINT_RE.fullmatch(str(outpoint)) or
                    int(outpoint.rsplit(":", 1)[1]) > 0xffffffff
                    for outpoint in active["input_outpoints"]) or
                (active.get("txid") is not None and
                 not re.fullmatch(r"[0-9a-f]{64}", str(active["txid"]))) or
                active.get("signing_state") not in
                    ("LEASED", "SIGNING", "SIGNED") or
                ((active.get("signing_state") == "SIGNED") !=
                 (active.get("txid") is not None)) or
                not re.fullmatch(r"[0-9a-f]{64}",
                                 str(active.get("staging_id"))) or
                active["staging_id"] != hashlib.sha256(
                    (owner_id + "\x00" + active["unsigned_sha256"]).encode(
                        "utf-8")).hexdigest() or
                type(active.get("created_at")) is not int or
                active["created_at"] < 0):
            raise ValueError("issuer prevout active lease is invalid")
        for outpoint in active["input_outpoints"]:
            if outpoint in occupied:
                raise ValueError(
                    "issuer prevout is leased by more than one carrier")
            occupied[outpoint] = owner_id
    for allocation_id, observation in state["terminal_observations"].items():
        if (not CONSENSUS_ALLOCATION_RE.fullmatch(str(allocation_id)) or
                not isinstance(observation, dict) or set(observation) != {
                    "tip", "tip_hash"} or
                type(observation.get("tip")) is not int or
                observation["tip"] < 0 or
                not re.fullmatch(r"[0-9a-f]{64}",
                                 str(observation.get("tip_hash")))):
...
def load_prevout_journal():
    if not os.path.lexists(PREVOUT_STATEF):
        raise FileNotFoundError(
            "issuer prevout journal is missing; never auto-initialize")
    state = strict_json_loads(_secure_text(
        PREVOUT_STATEF, private=True, max_bytes=PREVOUT_JOURNAL_MAX_BYTES),
...
def save_prevout_journal(state):
    ...
    return save_signer_state_durable(
        state, PREVOUT_STATEF, max_bytes=PREVOUT_JOURNAL_MAX_BYTES)
...
def _unsigned_abandonment(owner_id, owner, unsigned_sha256):
    conflicts = owner["revoked_conflicting_input_outpoints"].get(
        unsigned_sha256)
    ...
    return UnsignedCarrierAbandoned({
        ...
        "capability": owner["capability"], "owner_id": owner_id,
...
def _promote_legacy_revocation_inputs(state, owner, unsigned_sha256,
                                      input_outpoints):
    ...
    if not 1 <= len(conservative) <= PREVOUT_MAX_REVOKED_PER_OWNER:
        raise ValueError(
            "legacy revoked carrier input set requires operator reconciliation")
    owner["revoked_conflicting_input_outpoints"][
        unsigned_sha256] = conservative
    save_prevout_journal(state)


def _bind_prevout_owner(state, owner_id, capability, allocation_id,
                        deposit_outpoint):
    parsed_capability, parsed_allocation = _parse_prevout_owner_id(owner_id)
    if (capability != parsed_capability or allocation_id != parsed_allocation or
            ((capability in ("mint", "mint-direct")) !=
             isinstance(deposit_outpoint, str)) or
            (deposit_outpoint is not None and
             (not BTC_OUTPOINT_RE.fullmatch(deposit_outpoint) or
              int(deposit_outpoint.rsplit(":", 1)[1]) > 0xffffffff))):
    ...
    if owner is None:
        ...
        owner = {"capability": capability, "allocation_id": allocation_id,
                 "deposit_outpoint": deposit_outpoint, "active": None,
                 "revoked_unsigned_sha256": [],
                 "revoked_conflicting_input_outpoints": {}}
        state["owners"][owner_id] = owner
    elif (owner["capability"] != capability or
          owner["allocation_id"] != allocation_id or
          owner["deposit_outpoint"] != deposit_outpoint):
        raise ValueError("issuer prevout owner identity changed")
    return owner


def reject_conflicting_issuer_prevouts(state, owner_id, capability,
    ...
    owner = _bind_prevout_owner(
        state, owner_id, capability, allocation_id, deposit_outpoint)
    if unsigned_sha256 in owner["revoked_unsigned_sha256"]:
        _promote_legacy_revocation_inputs(
            state, owner, unsigned_sha256, input_outpoints)
        raise _unsigned_abandonment(owner_id, owner, unsigned_sha256)
    active = owner["active"]
    if active is not None:
        if (active["unsigned_sha256"] != unsigned_sha256 or
                active["input_outpoints"] != input_outpoints):
            raise ValueError(
                "issuer prevout owner already has another exact carrier")
        return False
    conflicts = _conflicting_issuer_prevouts(
        state, owner_id, input_outpoints)
    if not conflicts:
...
def reserve_issuer_prevouts(state, owner_id, capability, allocation_id,
                            deposit_outpoint, unsigned_tx_hex):
    ...
    conflicts = _conflicting_issuer_prevouts(
        state, owner_id, input_outpoints)
    if conflicts:
        if (len(owner["revoked_unsigned_sha256"]) >=
                PREVOUT_MAX_REVOKED_PER_OWNER):
            raise ValueError("issuer prevout revocation ceiling reached")
        owner["revoked_unsigned_sha256"].append(unsigned_sha256)
        owner["revoked_conflicting_input_outpoints"][unsigned_sha256] = conflicts
        save_prevout_journal(state)
    ...
    active = {"unsigned_sha256": unsigned_sha256,
              "input_outpoints": input_outpoints, "txid": None,
              "created_at": int(time.time()), "signing_state": "LEASED",
              "staging_id": hashlib.sha256(
                  (owner_id + "\x00" + unsigned_sha256).encode(
                      "utf-8")).hexdigest()}
    owner["active"] = active
    save_prevout_journal(state)
    return active, True
...
def mark_issuer_prevout_signature(state, owner_id, unsigned_tx_hex, txid):
    ...
    unsigned_sha256 = hashlib.sha256(
        bytes.fromhex(unsigned_tx_hex)).hexdigest()
    if (owner is None or owner["active"] is None or
            owner["active"]["unsigned_sha256"] != unsigned_sha256 or
            owner["active"]["input_outpoints"] !=
                _unsigned_input_outpoints(unsigned_tx_hex) or
            not re.fullmatch(r"[0-9a-f]{64}", str(txid))):
...
def _signing_stage_paths(staging_id):
    ...
    return (os.path.join(directory, staging_id + ".prepared.json"),
...
def sign_or_recover_staged_carrier(prevout_state, owner_id,
    ...
    if not os.path.lexists(signed_path):
        if not started_here:
            raise ValueError(
                "indeterminate prior issuer signing attempt; exact output missing")
        ...
        passphrase = None
        if run.returncode != 0:
            raise ValueError(description + " refused: " +
                             run.stderr.strip()[:200])
    signed_hex = _secure_text(
        os.path.abspath(signed_path), private=True,
        ...
        max_bytes=maximum_signed_hex_bytes + 1).strip()
    if (not re.fullmatch(r"[0-9a-fA-F]+", signed_hex) or
            len(signed_hex) > maximum_signed_hex_bytes or
            sol.unsigned_template_from_signed_hex(signed_hex) !=
                unsigned_tx_hex):
...
def _coherent_c1_terminal_status(rpc, allocation_id):
    ...
    if (not isinstance(supply_before, dict) or
            ...
            supply_before["tip"] < 0 or
            not re.fullmatch(r"[0-9a-f]{64}",
                             str(supply_before.get("tip_hash"))) or
            not isinstance(status, dict) or
            status.get("allocation_id") != allocation_id or
            type(status.get("retired")) is not bool or
            status.get("tip") != supply_before["tip"]):
...
def _direct_mint_lease_is_final(rpc, owner):
    ...
    if (not isinstance(supply_before, dict) or
            ...
            supply_before != supply_after or
            not isinstance(status, dict) or
            status.get("outpoint") != owner["deposit_outpoint"] or
            status.get("tip") != supply_before.get("tip") or
            status.get("tip_hash") != supply_before.get("tip_hash") or
            type(status.get("consumed")) is not bool):
        raise ValueError("direct mint lease snapshot is incoherent")
    if not status["consumed"]:
        return False
    if (status.get("accepted_txid") != active["txid"] or
            status.get("accepted_effect_kind") != "MINT" or
            type(status.get("accepted_block_height")) is not int or
            status["accepted_block_height"] < 0 or
            not re.fullmatch(r"[0-9a-f]{64}",
                             str(status.get("accepted_block_hash"))) or
            supply_before["tip"] < status["accepted_block_height"] or
            supply_before["tip"] - status["accepted_block_height"] + 1 <=
                MAX_REORG_DEPTH or
            rpc.call("getblockhash", [status["accepted_block_height"]]) !=
                status["accepted_block_hash"]):
        return False
    return True
    ...
    for owner_id, allocation_id in selected:
        ...
        if observed is None:
            state["terminal_observations"][allocation_id] = {
                "tip": supply["tip"], "tip_hash": supply["tip_hash"]}
            changed = True
            continue
        observed_hash = rpc.call("getblockhash", [observed["tip"]])
        if (observed_hash != observed["tip_hash"] or
                supply["tip"] < observed["tip"]):
        ...
        if supply["tip"] - observed["tip"] + 1 <= MAX_REORG_DEPTH:
            continue
        for stale_owner in [
                key for key, owner in state["owners"].items()
                if owner["allocation_id"] == allocation_id]:
            active = state["owners"][stale_owner]["active"]
            if active is not None:
                if active["staging_id"] not in state["cleanup_staging_ids"]:
                    state["cleanup_staging_ids"].append(
                        active["staging_id"])
...
def validate_mint_authority_config(cfg, issuer):
    ...
    if (not isinstance(signer_id, str) or not sol.IDENTIFIER_RE.fullmatch(signer_id) or
            active_id != signer_id):
    ...
    if policy.get("wrap_allocation_witness_required") is not True:
        raise ValueError(
            "mint_authority.wrap_allocation_witness_required must be true")
    try:
        marker = _secure_text(
            ACTIVE_MINT_SIGNERF, private=True, exact_mode=0o600,
            max_bytes=1024).strip()
    except Exception as e:
    ...
    if (not isinstance(command, list) or not command or
            any(not isinstance(x, str) or not x or "\x00" in x
                for x in command) or not os.path.isabs(command[0])):
...
def _call_witness(command, payload):
    ...
    run = run_bounded_subprocess(
        command, input_text=encoded, timeout=45,
        stdout_max=MAX_CHILD_OUTPUT_BYTES, stderr_max=1024 * 1024,
        description="reservation witness")
    if run.returncode != 0:
        raise ValueError("reservation witness unavailable/refused: " +
...
def verify_reservation_receipt(receipt, issuer, witness, request_id,
                               unsigned_tx_sha256, sats, recipient, heartbeat,
                               allocation):
    ok, why = sol.validate_reservation_receipt(receipt)
    if not ok:
    ...
    expected = {
        ...
        "allocation_public_descriptor_range_start":
            allocation["public_descriptor_range_start"],
        "allocation_public_descriptor_range_end":
            allocation["public_descriptor_range_end"],
    }
...
def reserve_witness_headroom(witness, issuer, request_id, unsigned_tx_sha256,
                             unsigned_tx_hex, sats, recipient, heartbeat,
                             allocation):
    payload = {
        "action": "reserve", "issuer_id": issuer,
        "request_id": request_id, "unsigned_tx_sha256": unsigned_tx_sha256,
        "unsigned_tx_hex": unsigned_tx_hex,
        "sats": sats, "recipient": recipient,
        "allocation": allocation,
        "beat_seq": heartbeat["seq"], "tip": heartbeat["tip"],
        "tip_hash": heartbeat["tip_hash"],
    }
    answer = _call_witness(witness["command"], payload)
    return verify_reservation_receipt(
        answer.get("receipt"), issuer, witness, request_id,
        unsigned_tx_sha256, sats, recipient, heartbeat, allocation)
...
def record_mint_reservation(state, receipt, signed_at=None):
    ...
    for entry in pending:
        if entry["request_id"] != request_id:
            continue
        if (entry["sats"] != receipt["sats"] or
                entry.get("witness_receipt", {}).get("reservation_id") !=
    ...
    pending.append({"request_id": request_id, "sats": sats,
                    "signed_at": int(time.time() if signed_at is None else signed_at),
                    "witness_receipt": receipt, "witness_committed": False})
    accounting["pending"] = pending
    return True
...
def commit_witness_transaction(witness, state, request_id, txid, signed_tx_hex):
    ...
    answer = _call_witness(witness["command"], {
        "action": "commit", "reservation_id": rid, "txid": txid,
        "signed_tx_hex": signed_tx_hex})
    if (answer.get("reservation_id") != rid or answer.get("txid") != txid or
            answer.get("committed") is not True):
    ...
    for field in ("pending", "confirmed"):
        for entry in accounting.get(field, []):
            if not entry.get("witness_receipt"):
                raise ValueError(
                    "unwitnessed outstanding mint requires offline reconciliation")
...
def validate_c1_allocation_claim(value):
    required = {
        ...
        "expires_at", "capacity_policy_sha256", "commitment_blind",
        "consensus_allocation_id",
    }
    if not isinstance(value, dict) or set(value) != required:
        raise ValueError("C1 allocation claim schema is not canonical")
    if (not isinstance(value["request_id"], str) or
            not re.fullmatch(r"[0-9a-f]{32}", value["request_id"]) or
            not isinstance(value["principal_hash"], str) or
            not re.fullmatch(r"[0-9a-f]{64}", value["principal_hash"]) or
            not isinstance(value["veld_address"], str) or
            not VELD_ADDR_RE.fullmatch(value["veld_address"]) or
            type(value["amount_sats"]) is not int or
            not 10_000 <= value["amount_sats"] <= MAX_SINGLE_SATS or
            type(value["descriptor_index"]) is not int or
            not 1000 <= value["descriptor_index"] <= 10_999 or
            not isinstance(value["btc_address"], str) or
            not re.fullmatch(r"bc1p[023456789ac-hj-np-z]{58}",
                             value["btc_address"]) or
            not isinstance(value["script_pubkey"], str) or
            not re.fullmatch(r"5120[0-9a-f]{64}", value["script_pubkey"]) or
            not isinstance(value["commitment_blind"], str) or
            not re.fullmatch(r"[0-9a-f]{64}", value["commitment_blind"]) or
            int(value["commitment_blind"], 16) == 0 or
            not isinstance(value["consensus_allocation_id"], str) or
            not CONSENSUS_ALLOCATION_RE.fullmatch(
                value["consensus_allocation_id"]) or
            int(value["consensus_allocation_id"], 16) == 0 or
            type(value["admitted_at"]) is not int or value["admitted_at"] < 0 or
            type(value["expires_at"]) is not int or
            value["expires_at"] - value["admitted_at"] != 7 * 24 * 60 * 60 or
            not isinstance(value["capacity_policy_sha256"], str) or
            not re.fullmatch(r"[0-9a-f]{64}",
...
def decode_c1_reservation(unsigned_tx_hex, issuer, issuer_script_hex):
    run = run_bounded_subprocess(
        ...
        input_text="", timeout=30, stdout_max=MAX_CHILD_OUTPUT_BYTES,
        stderr_max=1024 * 1024, description="decode-c1-reservation")
    if run.returncode != 0:
        raise ValueError("C1 carrier policy: " +
                         (run.stderr.strip()[:200] or "decoder refused"))
    decoded = strict_json_loads(run.stdout, "decode-c1-reservation response")
    required = {"action", "from", "to", "sats", "allocation_id",
                "allocation_commitment", "fund_script_pubkey_hex",
                "fund_commitment_blind_hex", "fund_outpoint",
                "funding_proof_hex", "total_out_sats", "num_inputs"}
    if (not isinstance(decoded, dict) or set(decoded) != required or
            decoded.get("action") not in
                ("RESERVE", "EXPOSE", "CANCEL", "FUND") or
            decoded.get("from") != issuer or
            type(decoded.get("sats")) is not int or
            type(decoded.get("total_out_sats")) is not int or
            type(decoded.get("num_inputs")) is not int or
            decoded["num_inputs"] < 1):
        raise ValueError("decoded C1 carrier is malformed")
    fund_fields = ("fund_script_pubkey_hex", "fund_commitment_blind_hex",
                   "fund_outpoint", "funding_proof_hex")
    if ((decoded["action"] == "FUND" and
         any(not isinstance(decoded.get(field), str) for field in fund_fields)) or
            (decoded["action"] != "FUND" and
             any(decoded.get(field) is not None for field in fund_fields))):
...
def verify_c1_allocation_witness(cfg, allocation):
    ...
    if (not isinstance(policy, dict) or
            set(policy) != {"enabled", "allocation_witness_command",
                            "durable_archive_command"} or
    ...
    fields = {key: allocation[key] for key in (
        "request_id", "principal_hash", "veld_address", "amount_sats",
        "descriptor_index", "btc_address", "script_pubkey", "admitted_at",
    ...
    completed = run_bounded_subprocess(
        list(command), input_text=request, timeout=45,
        stdout_max=64 * 1024, stderr_max=64 * 1024,
        description="independent C1 allocation witness")
    if completed.returncode != 0:
        raise ValueError("C1 allocation witness refused: " +
                         completed.stderr.strip()[:200])
    answer = strict_json_loads(completed.stdout, "C1 allocation witness response")
    required_answer = {
        "version", "action", "request_id", "descriptor_index", "authorized",
        "idempotent", "capacity_policy_sha256", "expires_at",
        "commitment_blind", "consensus_allocation_id",
        "deposit_observed_at", "funded_reserved_at", "minted_at",
    }
    if (not isinstance(answer, dict) or set(answer) != required_answer or
            answer.get("version") != 4 or
            answer.get("action") != "verify_allocation" or
            answer.get("request_id") != allocation["request_id"] or
            answer.get("descriptor_index") != allocation["descriptor_index"] or
            answer.get("commitment_blind") != allocation["commitment_blind"] or
            answer.get("consensus_allocation_id") !=
                allocation["consensus_allocation_id"] or
            answer.get("authorized") is not True or
            answer.get("idempotent") is not True or
            answer.get("capacity_policy_sha256") !=
                allocation["capacity_policy_sha256"] or
            answer.get("expires_at") != allocation["expires_at"] or
            (answer.get("deposit_observed_at") is not None and
             type(answer.get("deposit_observed_at")) is not int) or
            (answer.get("funded_reserved_at") is not None and
             type(answer.get("funded_reserved_at")) is not int) or
            (answer.get("minted_at") is not None and
             type(answer.get("minted_at")) is not int)):
        raise ValueError("C1 allocation witness acknowledgement is not exact")
    return answer


def validate_c1_signing_boundary(rpc, issuer, allocation, phase,
                                 funding=None, retry_txid=None):
    completion = phase in ("CANCEL", "FUND")
    peg_before = validate_compiled_mint_policy(
        rpc, issuer, completion=completion)
    allocation_id = allocation["consensus_allocation_id"]
    status = rpc.call("getbtcveldc1reservation", [allocation_id])
    peg_after = validate_compiled_mint_policy(
        rpc, issuer, completion=completion)
    fields = ("tip", "supply_sats", "issuer_effective_custody_cap_sats",
              "issuer_reserved_sats", "issuer_mint_headroom_sats")
    ...
    if (not isinstance(status, dict) or
            status.get("allocation_id") != allocation_id or
            type(status.get("found")) is not bool or
            type(status.get("retired")) is not bool or
            type(status.get("last_sequence")) is not int or
            type(status.get("sequence_history_count")) is not int or
            not 0 <= status.get("last_sequence", -1) <= (1 << 64) - 1 or
            not 0 <= status.get("sequence_history_count", -1) <=
                (1 << 64) - 1 or
            status["sequence_history_count"] != status["last_sequence"]):
        raise ValueError("C1 reservation status is malformed")
    if (type(status.get("tip")) is not int or status["tip"] < 0 or
            type(peg_before.get("tip")) is not int or
            status["tip"] != peg_before["tip"] or
            status["tip"] != peg_after.get("tip")):
    ...
    if found and (status.get("recipient") != allocation["veld_address"] or
                  status.get("amount_sats") != allocation["amount_sats"] or
                  status.get("allocation_commitment") != allocation_commitment(
                      allocation_id, allocation["veld_address"],
                      allocation["amount_sats"], allocation["script_pubkey"],
    ...
    if phase == "RESERVE":
        if found:
            ...
            if (status["retired"] or
                    int(allocation_id, 16) != status["last_sequence"] + 1):
        ...
        if (not found or status.get("active") is not True or
                (status.get("exposed") is True and retry_txid is None) or
                (status.get("exposed") is not True and
                 (status.get("reserve_canonical_depth_reached") is not True or
                  type(status.get("reserve_confirmations")) is not int or
                  status["reserve_confirmations"] < C1_FINALITY_DEPTH))):
            raise ValueError("C1R1 is not an exact canonical exposure authority")
    elif phase == "CANCEL":
        if (found or status["retired"] or
                int(allocation_id, 16) != status["last_sequence"] + 1):
            raise ValueError("C1 cancellation is not the exact next missing sequence")
    elif phase == "FUND":
        if (not isinstance(funding, dict) or set(funding) != {
                "outpoint", "proof_hex", "proof_parent_root",
                "proof_parent_count"} or
                not BTC_OUTPOINT_RE.fullmatch(str(funding.get("outpoint", ""))) or
                not isinstance(funding.get("proof_hex"), str) or
                not re.fullmatch(r"[0-9a-f]+", funding["proof_hex"]) or
                not isinstance(funding.get("proof_parent_root"), str) or
                not re.fullmatch(r"[0-9a-f]{64}",
                                 funding["proof_parent_root"]) or
                type(funding.get("proof_parent_count")) is not int or
                funding["proof_parent_count"] < 0):
            raise ValueError("C1 funding boundary object is malformed")
        if (not found or status.get("active") is not True or
                status.get("exposed") is not True or
                status.get("exposure_canonical_depth_reached") is not True):
            raise ValueError("C1 funding lacks a deep exposed reservation")
        if status.get("funded") is True:
            if (retry_txid is None or
                    status.get("funding_outpoint") != funding["outpoint"]):
                raise ValueError("C1 reservation is already funded differently")
            mint_status = rpc.call(
                "getbtcveldmintstatus", [funding["outpoint"]])
            if (not isinstance(mint_status, dict) or
                    mint_status.get("tip") != status["tip"] or
                    ...
                    mint_status.get("c1_allocation_id") != allocation_id or
                    mint_status.get("consumer_txid") != retry_txid):
        ...
        if (type(status.get("funding_starts_height")) is not int or
                type(status.get("funding_accepts_through_height")) is not int or
                target_height < status["funding_starts_height"] or
                target_height > status["funding_accepts_through_height"]):
        ...
        if (not isinstance(mint_status, dict) or
                mint_status.get("outpoint") != funding["outpoint"] or
                mint_status.get("tip") != status["tip"] or
                mint_status.get("consumed") is not False or
                mint_status.get("minted") is not False or
                mint_status.get("root") != funding["proof_parent_root"] or
                mint_status.get("count") != funding["proof_parent_count"] or
                not isinstance(mint_status.get("proof_hex"), str) or
                not funding["proof_hex"].endswith(mint_status["proof_hex"])):
...
def _empty_c1_signer_state():
    return {"version": 3, "records": {}, "terminal_observations": {},
            "tombstones": {}, "compaction_cursor": None}
...
def _c1_stage_metadata_valid(key, record):
    ...
    if (not re.fullmatch(r"[0-9a-f]{64}",
                         str(record.get("allocation_sha256"))) or
            not re.fullmatch(r"[0-9a-f]{64}",
                             str(record.get("unsigned_sha256"))) or
            not re.fullmatch(r"[0-9a-f]{64}", str(record.get("txid"))) or
            type(record.get("signed_at")) is not int or
            record["signed_at"] < 0 or
            ((match.group(2) == "FUND") !=
             (isinstance(record.get("funding_sha256"), str) and
              isinstance(record.get("semantic_sha256"), str))) or
            (record.get("funding_sha256") is not None and
             (not re.fullmatch(r"[0-9a-f]{64}", record["funding_sha256"]) or
              record["funding_sha256"] != match.group(3))) or
            (record.get("semantic_sha256") is not None and
             not re.fullmatch(r"[0-9a-f]{64}", record["semantic_sha256"]))):
        return False
    return True
...
def _validate_c1_cache_record(key, record):
    ...
    common = {"kind", "allocation_sha256", "unsigned_sha256", "txid",
              "signed_at", "funding_sha256", "semantic_sha256"}
    with_inputs = common | {"input_outpoints"}
    if "input_outpoints" in record and (
            not isinstance(record["input_outpoints"], list) or
            not record["input_outpoints"] or
            len(record["input_outpoints"]) > 10_000 or
            len(set(record["input_outpoints"])) !=
                len(record["input_outpoints"]) or
            any(not BTC_OUTPOINT_RE.fullmatch(str(outpoint)) or
                int(str(outpoint).rsplit(":", 1)[1]) > 0xffffffff
    ...
    if record.get("kind") == "raw":
        ...
        if (not isinstance(signed_hex, str) or
                not re.fullmatch(r"[0-9a-f]+", signed_hex) or
        ...
        return
    durable = with_inputs | {
        "signed_tx_sha256", "archive_record_sha256", "archive_id",
        "archived_at", "coordinator_sequence", "coordinator_state_sha256",
    }
    legacy_durable = durable - {"input_outpoints"}
    if (record.get("kind") != "durable" or
            set(record) not in (durable, legacy_durable) or
            not re.fullmatch(r"[0-9a-f]{64}",
                             str(record.get("signed_tx_sha256"))) or
            not re.fullmatch(r"[0-9a-f]{64}",
                             str(record.get("archive_record_sha256"))) or
            not isinstance(record.get("archive_id"), str) or
            not 1 <= len(record["archive_id"]) <= 256 or
            type(record.get("archived_at")) is not int or
            record["archived_at"] < 0 or
            type(record.get("coordinator_sequence")) is not int or
            record["coordinator_sequence"] <= 0 or
            not re.fullmatch(r"[0-9a-f]{64}",
                             str(record.get("coordinator_state_sha256")))):
...
def load_c1_signer_state():
    if not os.path.lexists(C1_STATEF):
        raise FileNotFoundError(
            "C1 signer authority state is missing; never auto-initialize")
    state = strict_json_loads(
        _secure_text(C1_STATEF, private=True, max_bytes=C1_STATE_MAX_BYTES),
        "C1 signer state")
    ...
    if (isinstance(state, dict) and set(state) == {"version", "records"} and
            state.get("version") == 2 and isinstance(state.get("records"), dict)):
    ...
    if (isinstance(state, dict) and state.get("version") == 3 and
            set(state) == {"version", "records", "terminal_observations",
                           "tombstones"}):
        state["compaction_cursor"] = None
        migrated_on_read = True
    if (not isinstance(state, dict) or set(state) != {
            "version", "records", "terminal_observations", "tombstones",
            "compaction_cursor"} or
            state.get("version") != 3 or
            not isinstance(state.get("records"), dict) or
            not isinstance(state.get("terminal_observations"), dict) or
            not isinstance(state.get("tombstones"), dict) or
            (state.get("compaction_cursor") is not None and
             not CONSENSUS_ALLOCATION_RE.fullmatch(
                 str(state.get("compaction_cursor")))) or
            len(state["records"]) > C1_MAX_CACHE_ROWS or
            len(state["terminal_observations"]) > 10_000 or
            len(state["tombstones"]) > 10_000):
        raise ValueError("C1 signer state schema is invalid")
    for record in state["records"].values():
        if record.get("kind") == "raw" and "input_outpoints" not in record:
            unsigned = sol.unsigned_template_from_signed_hex(
                record.get("signed_tx_hex"))
    ...
    for allocation_id, observed_tip in state["terminal_observations"].items():
        if (not CONSENSUS_ALLOCATION_RE.fullmatch(str(allocation_id)) or
                type(observed_tip) is not int or observed_tip < 0):
            raise ValueError("C1 terminal observation is invalid")
    for allocation_id, tombstone in state["tombstones"].items():
        if (not CONSENSUS_ALLOCATION_RE.fullmatch(str(allocation_id)) or
                not isinstance(tombstone, dict) or set(tombstone) != {
                    "allocation_sha256", "terminal_tip", "bundle_sha256",
                    "archive_id", "archived_at"} or
                not re.fullmatch(r"[0-9a-f]{64}",
                                 str(tombstone.get("allocation_sha256"))) or
                type(tombstone.get("terminal_tip")) is not int or
                tombstone["terminal_tip"] < 0 or
                not re.fullmatch(r"[0-9a-f]{64}",
                                 str(tombstone.get("bundle_sha256"))) or
                not isinstance(tombstone.get("archive_id"), str) or
                not 1 <= len(tombstone["archive_id"]) <= 256 or
                type(tombstone.get("archived_at")) is not int or
                tombstone["archived_at"] < 0):
...
def save_c1_signer_state(state):
    ...
    return save_signer_state_durable(
        state, C1_STATEF, max_bytes=C1_STATE_MAX_BYTES)
...
def require_c1_signer_lifecycle_capacity(state, reserve_bytes=None):
    ...
    if (type(reserve_bytes) is not int or reserve_bytes < 0 or
            c1_signer_state_size(state) + reserve_bytes >
                C1_STATE_RESERVE_BYTES):
        raise ValueError("C1 signer lifecycle admission reserve is exhausted")


def _c1_archive_command(cfg):
    policy = cfg.get("c1_reservation_authority")
    command = (policy.get("durable_archive_command")
...
def _archive_c1_payload(cfg, allocation_id, action, payload):
    ...
    request = {
        "version": 1, "action": action, "allocation_id": allocation_id,
        "archived_at": archived_at, "record_sha256": record_sha256,
        "record": payload,
    }
    completed = run_bounded_subprocess(
        _c1_archive_command(cfg),
    ...
    if completed.returncode != 0:
        raise RuntimeError("C1 signer archive refused: " +
                           completed.stderr.strip()[:240])
    answer = strict_json_loads(
        completed.stdout, "C1 signer archive acknowledgement")
    required = {"version", "archived", "allocation_id", "record_sha256",
                "readback_sha256", "archive_id", "object_lock_mode",
                "retention_until"}
    if (not isinstance(answer, dict) or set(answer) != required or
            answer.get("version") != 1 or answer.get("archived") is not True or
            answer.get("allocation_id") != allocation_id or
            answer.get("record_sha256") != record_sha256 or
            answer.get("readback_sha256") != record_sha256 or
            answer.get("object_lock_mode") != "COMPLIANCE" or
            type(answer.get("retention_until")) is not int or
            answer["retention_until"] <
                archived_at + C1_ARCHIVE_RETENTION_SECONDS or
            not isinstance(answer.get("archive_id"), str) or
            not 1 <= len(answer["archive_id"]) <= 256):
        raise RuntimeError(
            "C1 signer archive acknowledgement is not exact/durable")
    return record_sha256, answer["archive_id"], archived_at


def _read_archived_c1_cache_record(cfg, allocation_id, key, durable):
    request = {
        "version": 1, "action": "read_c1_signer_record",
    ...
    completed = run_bounded_subprocess(
        ...
        input_text=json.dumps(request, sort_keys=True, separators=(",", ":"),
                              allow_nan=False),
        timeout=120, stdout_max=8 * 1024 * 1024,
        stderr_max=64 * 1024,
        description="C1 signer COMPLIANCE archive readback")
    if completed.returncode != 0:
        raise RuntimeError("C1 signer archive readback refused: " +
                           completed.stderr.strip()[:240])
    answer = strict_json_loads(
        completed.stdout, "C1 signer archive readback response")
    expected = {"version", "found", "allocation_id", "archive_id",
                "record_sha256", "record"}
    if (not isinstance(answer, dict) or set(answer) != expected or
            answer.get("version") != 1 or answer.get("found") is not True or
            answer.get("allocation_id") != allocation_id or
            answer.get("archive_id") != durable["archive_id"] or
            answer.get("record_sha256") !=
                durable["archive_record_sha256"] or
            not isinstance(answer.get("record"), dict) or
            hashlib.sha256(json.dumps(
                answer["record"], sort_keys=True, separators=(",", ":"),
                allow_nan=False).encode("utf-8")).hexdigest() !=
                durable["archive_record_sha256"]):
        raise RuntimeError("C1 signer archive readback is not exact")
    payload = answer["record"]
    if (set(payload) != {"cache_key", "cache_record",
                         "coordinator_receipt"} or
            payload.get("cache_key") != key or
            not isinstance(payload.get("cache_record"), dict)):
    ...
    if (raw["allocation_sha256"] != durable["allocation_sha256"] or
            raw["unsigned_sha256"] != durable["unsigned_sha256"] or
            raw["txid"] != durable["txid"]):
...
def acknowledge_c1_signature_durable(req, cfg):
    required = {
        "version", "action", "allocation_id", "phase", "funding_sha256",
        "allocation_sha256", "unsigned_sha256", "signed_tx_sha256", "txid",
        "coordinator_sequence", "coordinator_state_sha256",
    }
    if (not isinstance(req, dict) or set(req) != required or
            req.get("version") != 1 or
            req.get("action") != "ack_c1_signature_durable" or
            not CONSENSUS_ALLOCATION_RE.fullmatch(
                str(req.get("allocation_id", ""))) or
            req.get("phase") not in ("RESERVE", "EXPOSE", "CANCEL", "FUND") or
            ((req.get("phase") == "FUND") !=
             isinstance(req.get("funding_sha256"), str)) or
            (req.get("funding_sha256") is not None and
             not re.fullmatch(r"[0-9a-f]{64}", req["funding_sha256"])) or
            any(not re.fullmatch(r"[0-9a-f]{64}", str(req.get(field, "")))
                for field in ("allocation_sha256", "unsigned_sha256",
                              "signed_tx_sha256", "txid",
                              "coordinator_state_sha256")) or
            type(req.get("coordinator_sequence")) is not int or
            req["coordinator_sequence"] <= 0):
    ...
    if (record["allocation_sha256"] != req["allocation_sha256"] or
            record["unsigned_sha256"] != req["unsigned_sha256"] or
            record["txid"] != req["txid"]):
        raise ValueError("C1 durable receipt differs from local signature")
    if record["kind"] == "durable":
        if (record["signed_tx_sha256"] != req["signed_tx_sha256"] or
                record["coordinator_sequence"] != req["coordinator_sequence"] or
                record["coordinator_state_sha256"] !=
                    req["coordinator_state_sha256"]):
            raise ValueError("C1 durable receipt replay changed")
        return {"version": 1, "action": "c1_signature_durable",
                "allocation_id": req["allocation_id"], "phase": req["phase"],
                "funding_sha256": req["funding_sha256"],
                "record_sha256": record["archive_record_sha256"],
                "archive_id": record["archive_id"], "idempotent": True}
    ...
    payload = {"cache_key": key, "cache_record": record,
               "coordinator_receipt": req}
    ...
    return {"version": 1, "action": "c1_signature_durable",
            "allocation_id": req["allocation_id"], "phase": req["phase"],
            "funding_sha256": req["funding_sha256"],
            "record_sha256": record_sha256, "archive_id": archive_id,
            "idempotent": False}
...
def compact_c1_signer_state(rpc, cfg, state, scan_limit=1):
    ...
    allocation_ids = sorted({key.split(":", 1)[0]
    ...
    for allocation_id in selected:
        ...
        if (not isinstance(status, dict) or
                status.get("allocation_id") != allocation_id or
                type(status.get("retired")) is not bool or
                type(status.get("tip")) is not int or status["tip"] < 0):
        ...
        payload = {"allocation_id": allocation_id,
                   ...
                   "terminal_tip": status["tip"], "records": records}
...
def _upgrade_prevout_journal_v1(legacy):
    if (not isinstance(legacy, dict) or set(legacy) != {
            "version", "owners", "terminal_observations", "scan_cursor"} or
            legacy.get("version") != 1 or
            not isinstance(legacy.get("owners"), dict) or
            not isinstance(legacy.get("terminal_observations"), dict)):
    ...
    for owner_id, owner in legacy["owners"].items():
        ...
        if (not isinstance(owner, dict) or set(owner) != {
                "capability", "allocation_id", "deposit_outpoint", "active",
                "revoked_unsigned_sha256"} or
                owner.get("capability") != capability or
                owner.get("allocation_id") != allocation_id):
        ...
        converted["revoked_conflicting_input_outpoints"] = {
            unsigned_hash: []
            for unsigned_hash in owner["revoked_unsigned_sha256"]}
        if owner["active"] is not None:
            ...
            active = dict(active)
            active["signing_state"] = (
                "SIGNED" if active["txid"] is not None else "SIGNING")
            active["staging_id"] = hashlib.sha256(
                (owner_id + "\x00" + active["unsigned_sha256"]).encode(
                    "utf-8")).hexdigest()
...
def _upgrade_prevout_journal_v2(legacy):
    ...
    upgraded["owners"] = {
        owner_id: dict(
            ...
            revoked_conflicting_input_outpoints={
                unsigned_hash: []
                for unsigned_hash in owner.get(
                    "revoked_unsigned_sha256", [])})
        for owner_id, owner in legacy["owners"].items()}
    return _validate_prevout_journal(upgraded)
...
def _upgrade_prevout_journal_v3(legacy):
    if (not isinstance(legacy, dict) or set(legacy) != {
            "version", "owners", "terminal_observations", "scan_cursor",
            "cleanup_staging_ids"} or legacy.get("version") != 3):
...
def _merge_reconciled_signed_lease(state, owner_id, capability,
                                   allocation_id, deposit_outpoint,
                                   unsigned_sha256, input_outpoints, txid,
                                   occupied=None):
    owner = _bind_prevout_owner(
        state, owner_id, capability, allocation_id, deposit_outpoint)
    if (not re.fullmatch(r"[0-9a-f]{64}", str(unsigned_sha256)) or
            not isinstance(input_outpoints, list) or not input_outpoints or
            not re.fullmatch(r"[0-9a-f]{64}", str(txid))):
    ...
    conflict = next((occupied[outpoint] for outpoint in input_outpoints
                     if outpoint in occupied and
                     occupied[outpoint] != owner_id), None)
    if conflict is not None:
        raise ValueError(
            "existing signed issuer carriers overlap fee inputs: %s / %s" %
            (conflict, owner_id))
    active = owner["active"]
    if active is not None:
        if (active["unsigned_sha256"] != unsigned_sha256 or
                active["input_outpoints"] != input_outpoints or
                active["txid"] not in (None, txid)):
    ...
    owner["active"] = {
        "unsigned_sha256": unsigned_sha256,
        "input_outpoints": list(input_outpoints), "txid": txid,
        "created_at": int(time.time()), "signing_state": "SIGNED",
        "staging_id": hashlib.sha256(
            (owner_id + "\x00" + unsigned_sha256).encode(
                "utf-8")).hexdigest(),
    }
...
def _reject_orphan_signing_stages(state):
    ...
    referenced = {owner["active"]["staging_id"]
...
def _cleanup_reconciled_signed_stages(state, confirmed_staging_ids):
    ...
    """
    if (not isinstance(confirmed_staging_ids, set) or
            any(not re.fullmatch(r"[0-9a-f]{64}", str(staging_id))
                for staging_id in confirmed_staging_ids)):
    ...
    signed_ids = {
        ...
        for owner in state["owners"].values()
        if owner["active"] is not None and
        owner["active"]["signing_state"] == "SIGNED" and
...
def load_or_reconcile_prevout_journal(cfg, issuer, issuer_script_hex):
    ...
    if existed:
        raw = strict_json_loads(_secure_text(
            PREVOUT_STATEF, private=True,
            max_bytes=PREVOUT_JOURNAL_MAX_BYTES), "issuer prevout journal")
        ...
        raise FileNotFoundError(
            "issuer prevout journal is missing; never reconstruct a partial "
            "authority restore")
    ...
    for record in _validate_signed_audit_records(mint_state):
        unsigned = sol.unsigned_template_from_signed_hex(
            record["signed_tx_hex"])
        if (hashlib.sha256(bytes.fromhex(unsigned)).hexdigest() !=
                record["request_id"]):
            raise ValueError("signed mint cache unsigned hash differs")
        (_recipient, _sats, _total_out, deposit_outpoint, _proof,
         allocation_id, _script, _blind) = mint_params_from_tx(
            unsigned, issuer, issuer_script_hex)
        ...
        _merge_reconciled_signed_lease(
            state, owner_id, capability, allocation_id, deposit_outpoint,
            record["request_id"], _unsigned_input_outpoints(unsigned),
            record["txid"], occupied)
        confirmed_staging_ids.add(
            state["owners"][owner_id]["active"]["staging_id"])
    ...
    for key, record in c1_state["records"].items():
        ...
        if "input_outpoints" not in source:
            source = _read_archived_c1_cache_record(
                cfg, allocation_id, key, record)
            record["input_outpoints"] = list(source["input_outpoints"])
            c1_upgraded = True
        _merge_reconciled_signed_lease(
            state, "c1:" + key, "c1-reservation", allocation_id, None,
            source["unsigned_sha256"], source["input_outpoints"],
            source["txid"], occupied)
        confirmed_staging_ids.add(
            state["owners"]["c1:" + key]["active"]["staging_id"])
...
def handle_c1_reservation_request(req, issuer, issuer_script_hex, cfg):
    if (not isinstance(req, dict) or set(req) != {
            "version", "action", "phase", "allocation", "funding",
            "unsigned_tx_hex"} or
            req.get("version") != 2 or
            req.get("action") != "sign_c1_reservation" or
            req.get("phase") not in ("RESERVE", "EXPOSE", "CANCEL", "FUND") or
            not isinstance(req.get("unsigned_tx_hex"), str) or
            not re.fullmatch(r"[0-9a-f]+", req["unsigned_tx_hex"]) or
            len(req["unsigned_tx_hex"]) > 2 * C1_MAX_SIGNED_TX_BYTES):
        raise ValueError("C1 signer request schema is not canonical")
    phase = req["phase"]
    allocation = validate_c1_allocation_claim(req["allocation"])
    funding = req["funding"]
    funding_required = {"outpoint", "proof_hex", "proof_parent_root",
                        "proof_parent_count"}
    if phase == "FUND":
        if (not isinstance(funding, dict) or set(funding) != funding_required or
                not isinstance(funding.get("outpoint"), str) or
                not BTC_OUTPOINT_RE.fullmatch(funding["outpoint"]) or
                int(funding["outpoint"].rsplit(":", 1)[1]) > 0xffffffff or
                not isinstance(funding.get("proof_hex"), str) or
                not re.fullmatch(r"[0-9a-f]+", funding["proof_hex"]) or
                len(funding["proof_hex"]) % 2 or
                not 174 <= len(funding["proof_hex"]) <= 38_000 or
                not isinstance(funding.get("proof_parent_root"), str) or
                not re.fullmatch(r"[0-9a-f]{64}",
                                 funding["proof_parent_root"]) or
                type(funding.get("proof_parent_count")) is not int or
                not 0 <= funding["proof_parent_count"] <= (1 << 64) - 1):
    ...
    decoded = decode_c1_reservation(
        req["unsigned_tx_hex"], issuer, issuer_script_hex)
    expected_commitment = allocation_commitment(
        allocation["consensus_allocation_id"], allocation["veld_address"],
        allocation["amount_sats"], allocation["script_pubkey"],
        allocation["commitment_blind"])
    if (decoded["action"] != phase or
            decoded["to"] != allocation["veld_address"] or
            decoded["sats"] != allocation["amount_sats"] or
            decoded["allocation_id"] !=
                allocation["consensus_allocation_id"] or
            decoded["allocation_commitment"] != expected_commitment or
            (phase == "FUND" and
             (decoded["fund_script_pubkey_hex"] !=
                  allocation["script_pubkey"] or
              decoded["fund_commitment_blind_hex"] !=
                  allocation["commitment_blind"] or
              decoded["fund_outpoint"] != funding["outpoint"] or
              decoded["funding_proof_hex"] != funding["proof_hex"]))):
    ...
    funding_sha = (hashlib.sha256(json.dumps(
        funding, sort_keys=True, separators=(",", ":"), allow_nan=False
    ).encode("utf-8")).hexdigest() if funding is not None else None)
    semantic_sha = None
    ...
    prevout_state = load_or_reconcile_prevout_journal(
        cfg, issuer, issuer_script_hex)
    maintain_prevout_journal(rpc, prevout_state)
    reject_conflicting_issuer_prevouts(
        prevout_state, prevout_owner_id, "c1-reservation",
        allocation["consensus_allocation_id"], None,
        req["unsigned_tx_hex"])
    allocation_sha = hashlib.sha256(json.dumps(
        allocation, sort_keys=True, separators=(",", ":"),
        allow_nan=False).encode("utf-8")).hexdigest()
    unsigned_sha = hashlib.sha256(bytes.fromhex(
        req["unsigned_tx_hex"])).hexdigest()
    cached = state["records"].get(key)
    if cached is not None and (cached["allocation_sha256"] != allocation_sha or
                               cached["unsigned_sha256"] != unsigned_sha):
        raise ValueError("C1 signer idempotency key was reused with changed bytes")
    if cached is not None and cached["kind"] == "durable":
        raise ValueError(
            "exact C1 signature replay is delegated to durable coordinator state")
    if phase == "FUND" and cached is None:
        prefix = allocation["consensus_allocation_id"] + ":FUND:"
        prior = [record for other_key, record in state["records"].items()
    ...
    validate_c1_signing_boundary(
        rpc, issuer, allocation, phase, funding=funding,
        retry_txid=(cached["txid"] if cached is not None else None))
    if cached is None:
        min_confs = cfg.get("min_prevout_confirmations",
                            DEFAULT_MIN_PREVOUT_CONFIRMATIONS)
    ...
    if cached is None:
        ...
        state["records"][key] = {
            ...
            "signed_tx_hex": signed_hex, "txid": signed_txid,
            "signed_at": int(time.time()),
            "funding_sha256": funding_sha,
            "semantic_sha256": semantic_sha,
            "input_outpoints": _unsigned_input_outpoints(
                req["unsigned_tx_hex"]),
        }
        save_c1_signer_state(state)
    ...
    return {"version": 2, "action": "signed_c1_reservation",
            "phase": phase, "request_id": allocation["request_id"],
            "allocation_id": allocation["consensus_allocation_id"],
...
def watchtower_gate(sats_tx, state, rpc=None, is_retry=False):
    ...
    if not _trusted_marker_exists(WT_REQUIRED, "watchtower-required"):
        ...
        if _trusted_marker_exists(CAPS_ONLY_OK, "caps-only-ok"):
            ...
            return 0, None
        refuse("solvency watchtower not armed and no caps-only-ok opt-out present "
               "(fail-safe): create 'watchtower-required' (+ run the watchtower) for "
               "production, or 'caps-only-ok' for dev/regtest")
    try:
        hb = strict_json_loads(read_bounded_regular_file(
            os.path.abspath(HEARTBEATF), 64 * 1024,
            "signer heartbeat", private=True), "signer heartbeat")
    except FileNotFoundError:
    ...
    if is_retry:
        ...
        if allow and used > int(hb["headroom_sats"]):
            allow, why = False, ("outstanding signed mints %d exceed watchtower headroom %d" %
                                 (used, int(hb["headroom_sats"])))
    else:
        allow, why = sol.heartbeat_gate(hb, time.time(), used, sats_tx)
    if not allow:
        refuse("watchtower: " + why)
    _log("WT-GATE ok headroom=%s outstanding=%d mint=%d retry=%s migrated=%s tip=%s"
         % (hb.get("headroom_sats"), used, 0 if is_retry else sats_tx,
            is_retry, migrated, hb.get("tip_hash", "")[:16]))
    return used, hb
...
def load_signer_configuration():
    ...
    if (not isinstance(marker, str) or not os.path.isabs(marker) or
            ...
            os.path.basename(marker) != "signer-authority-state.json"):
        raise ValueError(
...
def require_signer_authority_state_set(cfg):
    ...
    for path in expected["state_files"].values():
        if not os.path.lexists(path):
            raise ValueError(
                "activated signer authority state set is incomplete: " + path)
...
def main(capability="mint"):
    ...
    (to_tx, sats_tx, total_out, deposit_outpoint, nullifier_proof_hex,
     consensus_reservation_id,
     consensus_reservation_script_pubkey,
     consensus_reservation_commitment_blind) = mint_params_from_tx(
        utx, issuer, prev_script_hex)
    ...
    try:
        retry_record, incremental_sats = classify_mint_request(
            state, request_id, sats_tx, to_tx)
        window_used = window_signed_sats(state)
        if retry_record is not None and not retry_record.get("witness_receipt"):
            raise ValueError(
                "pre-witness cached signature requires offline reconciliation")
    except Exception as e:
    ...
    try:
        ...
        if allocation_claim is None:
            raise ValueError(
                "independent wrap allocation claim is required")
        validate_fresh_mint_boundary(
            rpc, issuer, sats_tx, deposit_outpoint, nullifier_proof_hex,
            retry_record.get("txid") if retry_record is not None else None,
            consensus_reservation_id, to_tx,
            allocation_claim["script_pubkey"],
            allocation_claim["commitment_blind"])
        require_witnessed_outstanding(state)
    except Exception as e:
        refuse("trusted RPC setup: " + str(e)[:240])
    prevout_owner_id = mint_prevout_owner_id(
        allocation_claim["consensus_allocation_id"])
    try:
        prevout_state = load_or_reconcile_prevout_journal(
            cfg, issuer, prev_script_hex)
        maintain_prevout_journal(rpc, prevout_state)
        reject_conflicting_issuer_prevouts(
            prevout_state, prevout_owner_id, "mint",
            allocation_claim["consensus_allocation_id"], deposit_outpoint, utx)
    except UnsignedCarrierAbandoned as e:
        emit_unsigned_carrier_abandoned(e)
    except Exception as e:
    ...
    if retry_record is None:
        try:
            ...
            if type(min_confs) is not int or not (1 <= min_confs <= 1_000_000):
                raise ValueError(
                    "min_prevout_confirmations must be an exact bounded JSON integer")
            total_in, resolved_inputs = resolve_mint_prevouts(
                utx, rpc, prev_script_hex, min_confs)
    ...
    ignored_used, heartbeat = watchtower_gate(
        sats_tx, state, rpc=rpc, is_retry=capacity_reserved)
    if heartbeat is None:
        refuse("shared reservation witness is mandatory outside explicit unit helpers")

    try:
        if prune_finalized_signed_audits(
                state, keep_request_id=request_id):
    ...
    if retry_record is None:
        try:
            receipt = reserve_witness_headroom(
                witness, issuer, request_id, request_id, utx,
                sats_tx, to_tx, heartbeat, allocation_claim)
    ...
    signed_txid = hashlib.sha256(
        hashlib.sha256(bytes.fromhex(signed)).digest()).hexdigest()
    try:
        record_signed_mint(state, request_id, signed_txid, signed,
                           sats_tx, to_tx)
        save_signer_state_durable(state)
    except Exception as e:
    ...
    try:
        ...
        if not entry or not entry.get("witness_committed"):
            commit_witness_transaction(
                witness, state, request_id, signed_txid, signed)
            save_signer_state_durable(state)
    except Exception as e:
    ...
    try:
        mark_issuer_prevout_signature(
            prevout_state, prevout_owner_id, utx, signed_txid)
    except Exception as e:
    ...
    _log(f"{'REPLAYED' if retry_record is not None else 'SIGNED'} txid={signed_txid} "
...
if __name__ == "__main__":
    if sys.argv[1:] == ["--initialize-authority-state"]:
        ...
        sys.stdout.write(json.dumps(
            initialized, sort_keys=True, separators=(",", ":"),
            allow_nan=False) + "\n")
    elif (len(sys.argv) == 3 and sys.argv[1] == "--capability" and
