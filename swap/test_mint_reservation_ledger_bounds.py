...
def c1_receipt(signature_bytes=7000):
    ...
    return {
        ...
        "unsigned_tx_sha256": request_id,
        "reservation_id": sol.reservation_id(
            ISSUER, request_id, request_id, 10_000, RECIPIENT,
            outpoint, allocation_id),
        "sats": 10_000,
        "recipient": RECIPIENT,
        "beat_seq": 9,
        "tip": 100,
    ...
    def call(self, method, params=None):
        if method == "getrawtransaction":
            ...
            return {"txid": self.txid,
                    ...
                    "block_height": self.height, "block_hash": BLOCK_HASH}
...
class ReservationLedgerBoundsTests(unittest.TestCase):
    def setUp(self):
        ...
        witness._persist_ledger(
            self.paths, ledger, witness._reservation_ledger_policy(self.cfg))
    ...
    def beat(self, tip):
        return sol.stamp_heartbeat(sol.build_beat_payload(
            1000, 0, 0, tip, TIP_HASH, tip, 120), int(time.time()))
    ...
    def test_strict_depth_archive_then_compact_and_terminal_commit_replay(self):
        ...
        with mock.patch.object(witness, "_archive_terminal") as archive:
            ...
            self.assertEqual(witness._reconcile(
                ledger, self.beat(198), FakeRpc(self.txid, tip=198),
                self.cfg, self.paths), 0)
            ...
            self.assertEqual(witness._reconcile(
                ledger, self.beat(199), FakeRpc(self.txid),
                self.cfg, self.paths), 0)
        ...
        self.assertLess(Path(self.paths["terminal"]).stat().st_size,
                        len(self.receipt["sig"]) // 2)

        decoded = mock.Mock(returncode=0, stdout=json.dumps({
            "from": ISSUER, "to": RECIPIENT, "sats": 1}), stderr="")
        with mock.patch.object(witness.subprocess, "run", return_value=decoded):
            answer = witness.handle_commit({
                ...
                "txid": self.txid, "signed_tx_hex": self.signed,
            }, self.cfg, self.paths, restarted)
        ...
        with mock.patch.object(witness.subprocess, "run", return_value=decoded), \
            ...
            witness.handle_commit({
                ...
                "txid": hashlib.sha256(hashlib.sha256(
                    bytes.fromhex(mutated)).digest()).hexdigest(),
    ...
    def test_c1_deep_terminal_worm_archive_removes_local_row_without_tombstone(self):
        ...
        with mock.patch.object(witness, "_archive_terminal") as archive, \
             self.assertRaises(SystemExit):
            witness._compact_terminal(
                self.paths, ledger, entry, self.beat(198), self.cfg)
        archive.assert_not_called()
        ...
        with mock.patch.object(witness, "_archive_terminal") as archive:
            witness._compact_terminal(
                self.paths, ledger, entry, self.beat(199), self.cfg)
        archive.assert_called_once()
        archived = archive.call_args.args[0]
        ...
        self.assertEqual(
            Path(self.paths["terminal"]).read_text(),
        ...
        decoded = mock.Mock(returncode=0, stdout=json.dumps({
            "from": ISSUER, "to": RECIPIENT, "sats": 10_000}), stderr="")
        with mock.patch.object(witness.subprocess, "run", return_value=decoded), \
             self.assertRaises(SystemExit):
            witness.handle_commit({
                "reservation_id": c1["reservation_id"],
                "txid": entry["txid"], "signed_tx_hex": signed,
            }, self.cfg, self.paths, restarted)
    ...
    def test_archive_or_space_failure_never_discards_active_receipt(self):
        for label, archive_side_effect, free_bytes in (
            ...
            with self.subTest(label=label):
                ...
                free = (mock.patch.object(witness, "_state_free_bytes",
                                          return_value=free_bytes)
                        if free_bytes is not None else mock.patch.object(
    ...
    def test_append_before_checkpoint_crash_is_exactly_recoverable(self):
        ...
        entry.update({"status": "final", "block_height": 99,
                      "block_hash": BLOCK_HASH})
        beat = self.beat(199)
        archive_entry = witness._terminal_archive_entry(entry, beat)
        archive_sha = hashlib.sha256(
            witness._canonical_json_bytes(archive_entry)).hexdigest()
        ...
        witness._append_tombstone(
            self.paths, ledger, record,
            witness._reservation_ledger_policy(self.cfg))
        ...
        witness._persist_ledger(
            self.paths, recovered, witness._reservation_ledger_policy(self.cfg))
    ...
    def test_tamper_truncation_and_unbound_tail_fail_closed(self):
        ...
        archive_entry = witness._terminal_archive_entry(entry, beat)
        record = witness._make_tombstone(
            entry, beat, 1, witness.ZERO_HASH,
        ...
        path.write_bytes(original[:-1]); path.chmod(0o600)
        with self.assertRaises(SystemExit):
            witness._load_ledger(self.paths["ledger"], self.cfg)
        path.write_bytes(original.replace(b'"sats":1', b'"sats":2', 1)); path.chmod(0o600)
        with self.assertRaises(SystemExit):
            witness._load_ledger(self.paths["ledger"], self.cfg)
    ...
    def test_production_archive_hook_requires_absolute_safe_executable(self):
        ...
        undersized = configured(["/archive-hook"])
        undersized["reservation_ledger"]["max_active_bytes"] = (
            256 * 1024 * 1024 - 1)
        with self.assertRaises(SystemExit):
            witness._reservation_ledger_policy(undersized, production=True)
        ...
        with self.assertRaises(SystemExit):
            witness._reservation_ledger_policy(
                configured(["relative-hook"]), production=True)
        with self.assertRaises(SystemExit):
            witness._reservation_ledger_policy(
        ...
        hook.chmod(0o700)
        accepted = witness._reservation_ledger_policy(
            configured([str(hook)]), production=True)
        ...
        with self.assertRaises(SystemExit):
            witness._reservation_ledger_policy(
                configured([str(link)]), production=True)

    def test_archive_hook_ack_is_exact_and_digest_bound(self):
        entry = {"version": 1, "kind": witness.TERMINAL_ARCHIVE_KIND,
                 "receipt": self.receipt, "txid": self.txid,
                 "signed_tx_sha256": self.signed_digest,
                 "block_height": 99, "block_hash": BLOCK_HASH,
                 "finalized_at_tip": 199, "finalized_at_tip_hash": TIP_HASH}
        ...
        good_answer = {
            "version": 1, "stored": True, "durable": True,
            "entry_sha256": digest, "readback_sha256": digest,
        ...
        with mock.patch.object(witness, "run_bounded_subprocess",
                               return_value=good) as run, \
             mock.patch.object(witness.time, "time", return_value=now):
            self.assertEqual(
                witness._archive_terminal(entry, digest, policy), good_answer)
        ...
        for label, answer in mutations.items():
            with self.subTest(label), mock.patch.object(
                    witness, "run_bounded_subprocess", return_value=mock.Mock(
