...
class FeedVeld:
    ...
    def call(self, method, params=None):
        ...
        cursor = (params or [""])[0]
        rows = [{
            "txid": x["burn_txid"], "vout": x["opreturn_vout"],
            "from": x["redeemer"], "to": "",
            "amount_sats": x["amount_sats"],
            "block": x["burn_height"], "block_hash": x["burn_block_hash"],
            "memo": x["dest_btc_addr"], "token": "btcVELD",
            "is_mint": False, "is_burn": False, "is_redeem": True,
        } for x in self.records]
        return {"tip": 60, "tip_hash": "aa" * 32,
                "final_height": 60, "cursor": cursor,
...
class SigningVeld:
    def call(self, method, params=None):
        ...
        if method == "getpeginfo":
            return {"active": True, "spv_active": True, "final_height": 200,
...
class SigningBtc:
    ...
    def call(self, method, *args):
        if method == "decoderawtransaction":
            raw = args[0]
            txin = {"txid": self.INPUT_TXID, "vout": 0,
                    "sequence": self.tx_sequence,
                    "scriptSig": {"hex": self.tx_script_sig}}
            if self.tx_witness is not None:
                txin["txinwitness"] = self.tx_witness
            return {"txid": ("bc" * 32 if raw == "aa" else "de" * 32),
                    "version": self.tx_version, "locktime": self.tx_locktime,
                    "vin": [txin],
                    "vout": [
        ...
        if method == "gettxout":
            return {"value": rd.btc_str(BURN["amount_sats"] + 1000),
                    "confirmations": 12,
...
class CanonicalC1Veld:
    ...
    def __init__(self, operator_spk=SigningBtc.CUSTODY_SPK, credit_depth=101,
                 phase="C1_MINT", race_after_first_status=False):
    ...
    def _status(self, outpoint):
        ...
        answer = {
            ...
            "root": "99" * 32, "count": 1, "tip": self.tip,
            "tip_hash": self.TIP_HASH,
            "accepted_txid": None, "accepted_block_height": None,
            "accepted_block_hash": None, "accepted_tx_index": None,
            "accepted_marker_vout": None, "accepted_effect_kind": None,
            "c1_allocation_id": None,
            "consumer_txid": None, "consumer_block_height": None,
            "consumer_block_hash": None, "consumer_tx_index": None,
        ...
        if phase == "C1_MINT":
            answer.update({
                ...
                "accepted_tx_index": 2, "accepted_marker_vout": 0,
                ...
                "credit_tx_index": 2, "credit_marker_vout": 0,
            })
        return answer

    def call(self, method, params=None):
        if method == "getbtcveldsupply":
            return {"supply_sats": 77_000, "tip": self.tip,
                    "tip_hash": self.TIP_HASH}
        ...
        if method == "getpeginfo":
            return {
                "active": True, "spv_active": True, "final_height": self.tip,
                "custody_descriptor_sha256": "ab" * 32,
                "custody_manifest_sha256": "cd" * 32,
                "custody_descriptor_range": [0, 999],
                "spv_custody_descriptor_index": 0,
...
class PayoutSignerPolicyTests(unittest.TestCase):
    ...
    def test_coordinator_requires_durable_observation_quorum(self):
        cfg = {"mode": "threshold_psbt", "threshold": 2, "signers": [
        ...
        def ok_run(cmd, **kwargs):
            ...
            return subprocess.CompletedProcess(cmd, 0,
                stdout=json.dumps({"signer_id": sid, "observed": [rid]}), stderr="")
    ...
    def test_coordinator_holds_when_observation_quorum_is_missing(self):
        ...
        def one_only(cmd, **kwargs):
            if cmd[0] == "signer-one":
                return subprocess.CompletedProcess(cmd, 0,
                    stdout=json.dumps({"signer_id": "s1", "observed": [rid]}), stderr="")
    ...
    def test_observation_backlog_over_10000_is_chunked_for_every_signer(self):
        ...
        def ok_run(cmd, **kwargs):
            ...
            sid = {"signer-one": "s1", "signer-two": "s2",
                   "signer-three": "s3"}[cmd[0]]
    ...
    def test_fetch_walks_all_bounded_rpc_pages(self):
        ...
        class PagedFeed:
            ...
            def call(self, method, params=None):
                ...
                return {
                    "tip": 1000, "tip_hash": "ab" * 32,
                    "final_height": 999, "cursor": cursor,
                    "page_limit": 512,
    ...
    def test_fetch_restarts_on_same_height_tip_hash_change(self):
        ...
        class SameHeightReorgFeed:
            ...
            def call(self, method, params=None):
                ...
                tip_hash = ("11" * 32 if self.attempt == 1 and not cursor
                            else "22" * 32)
                rows = [{
                    "txid": x["burn_txid"], "vout": x["opreturn_vout"],
                    "from": x["redeemer"], "to": "",
    ...
    def test_independent_signer_checks_exact_tx_then_commits_first_proposal(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            cfg = {"signer_id": "s1",
                   ...
                   "custody_script_pubkeys": scripts,
                   "btc_input_confirmations": 6, "max_payout_fee_sats": 2000}
            base = {
                "redeem_id": rd.redeem_id(BURN), "burn_txid": BURN["burn_txid"],
                "opreturn_vout": BURN["opreturn_vout"], "amount_sats": BURN["amount_sats"],
                "burn_height": BURN["burn_height"], "burn_block_hash": BURN["burn_block_hash"],
                "dest_spk": BURN["dest_btc_addr"], "raw_tx_hex": "aa", "psbt": "psbt-a",
                "inputs": [{"txid": btc.INPUT_TXID, "vout": 0, "value_sats": BURN["amount_sats"] + 1000}],
                "fee_sats": 1000, "change_sats": 0, "change_spk": "",
            ...
            alternate = dict(base); alternate.update({"raw_tx_hex": "bb", "psbt": "psbt-b"})
            with self.assertRaises(SystemExit) as cm:
    ...
    def test_operational_manifest_and_public_boundary_scripts_are_enforced(self):
        ...
        class BoundaryBtc:
            ...
            def call(self, method, *args):
                if method == "listdescriptors":
                    return {"descriptors": [{
                        "desc": descriptor, "active": False,
                        "internal": False, "range": [0, 10999],
                    }]}
                if method == "gettxout":
                    return {"value": "0.00001000", "confirmations": 12,
                            "scriptPubKey": {"hex": self.script}}
                raise AssertionError(method)

        with mock.patch.object(
                ps.custody_binding, "verify_core_derivation") as verify:
            policy = ps._verify_operational_custody_derivation(
                cfg, BoundaryBtc(scripts[1000]))
        self.assertEqual(policy["range"], [0, 10999])
        self.assertEqual(verify.call_args.args[1]["script_pubkeys"][10999],
                         scripts[10999])
        ...
        for index in (1000, 10999):
            with self.subTest(index=index):
                resolved = ps._resolved_inputs(
                    BoundaryBtc(scripts[index]), decoded, cfg,
                    CanonicalC1Veld(scripts[0]))
        ...
        self.assertFalse(ps._change_script_allowed(
            cfg, 1, "5120" + "ff" * 32))

        tampered = dict(cfg)
        tampered["custody_manifest_sha256"] = "00" * 32
        with mock.patch.object(
                ps.custody_binding, "verify_core_derivation"):
            with self.assertRaises(SystemExit):
                ps._verify_operational_custody_derivation(
                    tampered, BoundaryBtc(scripts[1000]))
    ...
    def test_public_c1_depth_100_refused_and_101_accepted(self):
        ...
        item = {"txid": SigningBtc.INPUT_TXID, "vout": 0,
        ...
        self.assertTrue(rd.validate_payout_custody_inputs(
        ...
        self.assertTrue(rd.validate_payout_custody_inputs(None, [{
            **item, "script_pubkey_hex": scripts[999]}], policy))
    ...
    def test_public_c1_reorg_and_restart_are_rechecked(self):
        ...
        self.assertTrue(rd.validate_payout_custody_inputs(
            veld, [item], ps._custody_policy(cfg)))
        ...
        with self.assertRaisesRegex(RuntimeError, "no longer canonical"):
            rd.validate_payout_custody_inputs(
                veld, [item], ps._custody_policy(dict(cfg)))
    ...
    def test_public_c1_race_before_key_use_is_refused(self):
        ...
        veld = CanonicalC1Veld(
            scripts[0], credit_depth=101, race_after_first_status=True)
        request = {
            ...
            "raw_tx_hex": "aa", "psbt": "psbt-a",
            "inputs": [{"txid": btc.INPUT_TXID, "vout": 0,
    ...
    def test_builder_skips_pending_public_input_and_uses_deep_c1_mint(self):
        ...
        class BuilderBtc:
            ...
            def call(self, method, *args):
                ...
                if method == "listunspent":
                    return [
                        {"txid": pending_txid, "vout": 0,
                         "amount": "0.00050000", "confirmations": 12,
                         "spendable": True, "solvable": True,
                         "scriptPubKey": scripts[1000]},
                        {"txid": accepted_txid, "vout": 0,
                         "amount": "0.00050000", "confirmations": 12,
                         "spendable": True, "solvable": True,
                         "scriptPubKey": scripts[1001]},
                        {"txid": operator_txid, "vout": 0,
                         "amount": "0.00050000", "confirmations": 12,
                         "spendable": True, "solvable": True,
                ...
                if method == "decoderawtransaction":
                    ...
                    for destination, value in self.outputs.items():
                        if destination == "data":
                            outputs.append({
                                "value": "0.00000000",
                                "scriptPubKey": {
                                    "hex": rd.payout_marker_script(BURN)}})
                        else:
                            outputs.append({
                                "value": value,
                                "scriptPubKey": {"hex": (
                    ...
                    return {
                        "txid": "ee" * 32, "version": 2, "locktime": 0,
                        "vin": [{**item, "sequence": 0xffffffff,
                                 "scriptSig": {"hex": ""}}
        ...
        raw, selected, _fee, _change, change_spk = rd.build_payout(
            btc, BURN, "change", {
                ...
                "feerate_sat_vb": 1,
            }, veld=BuilderVeld(scripts[0]), custody_policy=policy)
        self.assertEqual(raw, "aa")
        self.assertEqual(change_spk, scripts[999])
        self.assertEqual([(x["txid"], x["vout"]) for x in selected],
                         [(operator_txid, 0), (accepted_txid, 0)])

    def test_coordinator_rechecks_public_c1_after_quorum_before_broadcast(self):
        scripts = full_custody_scripts()
        policy = ps._custody_policy(public_custody_cfg(scripts))
        selected = [{"txid": SigningBtc.INPUT_TXID, "vout": 0,
                     "value_sats": BURN["amount_sats"] + 1000,
                     "script_pubkey_hex": scripts[1000]}]
        ...
        coordinator = rd.RedeemCoordinator(
            {"change_addr": "change", "payout_signing": {
                "mode": "threshold_psbt"}}, store, object(), veld)
        ...
        with mock.patch.object(rd, "load_payout_custody_policy",
                               return_value=policy), \
             mock.patch.object(rd, "find_existing_payout", return_value=None), \
             mock.patch.object(rd, "verify_burn_canonical",
                               return_value=(True, "canonical")), \
             mock.patch.object(rd, "build_payout", return_value=(
                 "aa", selected, 1000, 0, scripts[999])), \
    ...
    def test_signer_refuses_weak_sighash_and_preloaded_signature_psbts(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            for sighash in (
                    "NONE", "SINGLE", "ALL|ANYONECANPAY",
                    "SINGLE|ANYONECANPAY", 1):
            ...
            for field in ("partial_signatures", "taproot_key_path_sig",
    ...
    def test_legacy_proposal_is_backfilled_before_new_signing(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            store.db.execute(
                "UPDATE obligations SET status='broadcasting',raw_tx_hex='aa' WHERE rid=?",
                (rid,))
            ps.backfill_legacy_input_commitments(store, SigningBtc())
            rows = store.db.execute(
                "SELECT txid,vout,rid FROM signing_input_commitments").fetchall()
