...
def validate_descriptor_policy(descriptor, *, network="main"):
    ...
    if (
        len(parts) != 2
        or not parts[0].startswith(prefix)
        or not parts[0].endswith("))")
        or len(parts[1]) != 8
        or any(char not in BECH32_CHARSET for char in parts[1])
    ):
    ...
    for expression in expressions:
        ...
        if (
            len(raw) != 82
            or raw[:4] != key_version
            or raw[4] != 3
            or raw[9:13] != bytes.fromhex("80000000")
            or raw[45] not in (2, 3)
...
def load_manifest(
    ...
    expected_spv_spk_hex=None,
    require_absolute=True,
    expected_range_end=999,
    ...
    *,
    ...
    descriptor_hash = _hex64(expected_descriptor_sha256, "custody descriptor SHA-256")
    manifest_hash = _hex64(expected_manifest_sha256, "custody manifest SHA-256")
    if type(expected_range_end) is not int or not 999 <= expected_range_end <= 1_000_000:
        raise RuntimeError("custody manifest range end is outside [999,1000000]")
    consensus_manifest_hash = (
        manifest_hash
        if expected_consensus_manifest_sha256 is None
        else _hex64(expected_consensus_manifest_sha256, "consensus custody manifest SHA-256")
    )
    try:
        raw = read_bounded_regular_file(
            os.path.abspath(path), MAX_MANIFEST_BYTES, "custody SPK manifest"
        )
    ...
    if (
        ...
        or "multi_a(3," not in descriptor
        or re.search(r"(?:xprv|tprv|yprv|zprv|uprv|vprv)", descriptor, re.I)
        or hashlib.sha256(descriptor.encode("utf-8")).hexdigest() != descriptor_hash
        or document.get("descriptor_sha256") != descriptor_hash
        or document.get("range") != [0, expected_range_end]
        or not isinstance(scripts, list)
        or len(scripts) != expected_range_end + 1
        or len(set(scripts)) != expected_range_end + 1
        or any(not isinstance(item, str) or not P2TR_SPK_RE.fullmatch(item) for item in scripts)
    ):
    ...
    if expected_range_end == 999:
        if consensus_manifest_hash != manifest_hash:
            raise RuntimeError("legacy operational and consensus manifest identities differ")
    else:
        prefix = dict(document)
        prefix["range"] = [0, 999]
        prefix["script_pubkeys"] = scripts[:1000]
        prefix_bytes = (json.dumps(prefix, sort_keys=True, separators=(",", ":")) + "\n").encode(
            "utf-8"
        )
    ...
    if expected_spv_spk_hex is not None:
        if not isinstance(expected_spv_spk_hex, str) or not P2TR_SPK_RE.fullmatch(
            expected_spv_spk_hex
        ):
...
def verify_peg_identity(peg, binding, *, network="main"):
    ...
    validate_descriptor_policy(
        binding.get("descriptor") if isinstance(binding, dict) else None, network=network
    )
    ...
    if peg.get("custody_manifest_sha256") != binding["consensus_manifest_sha256"]:
...
def _bech32m_spk(address, expected_hrp="bc"):
    ...
    if (
        ...
        or not address.startswith(expected_hrp + "1")
    ):
    ...
    expanded = (
        [ord(char) >> 5 for char in address[:split]]
        + [0]
...
def verify_core_derivation(call, binding, *, expected_hrp="bc"):
    ...
    validate_descriptor_policy(
        binding.get("descriptor") if isinstance(binding, dict) else None,
    ...
    if (
        ...
        or not 999 <= custody_range[1] <= 1_000_000
    ):
        raise RuntimeError("custody derivation range is invalid")
    count = custody_range[1] + 1
    addresses = call("deriveaddresses", binding["descriptor"], "[0,%d]" % custody_range[1])
    if not isinstance(addresses, list) or len(addresses) != count or len(set(addresses)) != count:
        raise RuntimeError("Bitcoin Core did not derive %d unique custody addresses" % count)
