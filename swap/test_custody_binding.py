...
class CustodyBindingTests(unittest.TestCase):
    ...
    def _write(self, document):
        self.path.write_text(json.dumps(document, sort_keys=True, separators=(",", ":")) + "\n")
    ...
    def test_c1_extended_manifest_preserves_compiled_prefix_identity(self):
        ...
        found = self._load(range_end=10999, consensus_hash=consensus_hash)
        self.assertEqual(found["range"], [0, 10999])
        self.assertEqual(found["consensus_range"], [0, 999])
        for index in (0, 999, 1000, 10999):
            self.assertEqual(found["script_pubkeys"][index], self.scripts[index])
        ...
        with mock.patch.object(binding, "_bech32m_spk", side_effect=self.scripts):
            binding.verify_core_derivation(derive, found)
        self.assertEqual(calls, [("deriveaddresses", self.descriptor, "[0,10999]")])
