...
def fake_receipt(core, cfg):
    result = dict(core); result["sig_alg"] = "mldsa65"; result["sig"] = "00"
    return result
...
class ReservationWitnessTests(unittest.TestCase):
    def setUp(self):
        ...
        witness.atomic_write(
            self.paths["ledger"],
            json.dumps(witness._empty_ledger(),
                       sort_keys=True, separators=(",", ":")) + "\n")
        witness.atomic_write(
            self.paths["terminal"], witness._empty_terminal_log_text())
        self.rpc = FakeRpc()
        self.beat = sol.stamp_heartbeat(sol.build_beat_payload(
            5, 0, 0, 100, TIP_HASH, 7, 120), int(time.time()))
        self.cfg = {
            "issuer_id": ISSUER, "witness_id": "witness-1",
            "issuer_p2pkh_hex": "76a914" + "22" * 20 + "88ac",
            "keygen": "/fake/keygen", "state_dir": str(self.state),
    ...
    def request(self, tag=1, sats=1, recipient=RECIPIENT):
        ...
        return {"action": "reserve", "issuer_id": ISSUER,
                "request_id": digest, "unsigned_tx_sha256": digest,
                "unsigned_tx_hex": raw, "sats": sats, "recipient": recipient,
                "beat_seq": 7, "tip": 100, "tip_hash": TIP_HASH}

    def decoded(self, sats=1, recipient=RECIPIENT):
        return subprocess.CompletedProcess([], 0, stdout=json.dumps({
            "from": ISSUER, "to": recipient, "sats": sats}), stderr="")

    def reserve(self, ledger, request, decoded=None):
        with mock.patch.object(witness, "_sign_receipt", side_effect=fake_receipt), \
             mock.patch.object(witness.subprocess, "run",
                               return_value=decoded or self.decoded()):
    ...
    def test_exact_idempotency_mutation_and_durable_crash_charge(self):
        ...
        shrunk = sol.stamp_heartbeat(sol.build_beat_payload(
            0, 0, 0, 100, TIP_HASH, 8, 120), int(time.time()))
        old_beat = self.beat; self.beat = shrunk
        shrunk_request = self.request(); shrunk_request["beat_seq"] = 8
    ...
    def test_reserve_independently_derives_amount_recipient_and_hash(self):
        ...
        with self.assertRaises(SystemExit):
            self.reserve(ledger, self.request(recipient=RECIPIENT),
                         self.decoded(recipient="Vdifferent"))
        wrong_hash = self.request(); wrong_hash["request_id"] = "aa" * 32
        with self.assertRaises(SystemExit):
    ...
    def test_commit_binds_signed_bytes_unsigned_template_and_mint_fields(self):
        ...
        with mock.patch.object(witness.subprocess, "run", return_value=self.decoded()):
            answer = witness.handle_commit({"reservation_id":
                ledger["reservations"][0]["receipt"]["reservation_id"],
                "txid": txid, "signed_tx_hex": signed}, self.cfg,
                self.paths, ledger)
        ...
        with self.assertRaises(SystemExit):
            witness.handle_commit({"reservation_id": answer["reservation_id"],
                "txid": old_txid, "signed_tx_hex": unrelated}, self.cfg,
                self.paths, restored)
        with mock.patch.object(witness.subprocess, "run",
                               return_value=self.decoded(sats=999)):
            ...
            other_txid = hashlib.sha256(hashlib.sha256(
                bytes.fromhex(other_signed)).digest()).hexdigest()
            with self.assertRaises(SystemExit):
                witness.handle_commit({"reservation_id":
                    other_ledger["reservations"][0]["receipt"]["reservation_id"],
                    "txid": other_txid, "signed_tx_hex": other_signed}, self.cfg,
                    self.paths, other_ledger)

    def test_confirmation_requires_exact_block_hash_and_reorg_restores_charge(self):
        ledger = witness._load_ledger(self.paths["ledger"])
        request = self.request(); self.reserve(ledger, request)
        signed = signed_tx(request["unsigned_tx_hex"])
        txid = hashlib.sha256(hashlib.sha256(bytes.fromhex(signed)).digest()).hexdigest()
        ledger["reservations"][0]["txid"] = txid
        ledger["reservations"][0]["signed_tx_sha256"] = hashlib.sha256(
            bytes.fromhex(signed)).hexdigest()
        self.rpc.transactions[txid] = {"txid": txid, "confirmations": 2,
            "block_height": 99, "block_hash": "22" * 32}
    ...
    def test_confirmation_depth_is_derived_from_signed_beat_tip(self):
        ...
        entry["txid"] = txid
        entry["signed_tx_sha256"] = hashlib.sha256(
            bytes.fromhex(signed)).hexdigest()
        self.rpc.transactions[txid] = {
            ...
            "confirmations": 10_000, "block_height": 99,
            "block_hash": "22" * 32,
        }
        self.rpc.block_hashes[99] = "22" * 32
        self.assertEqual(witness._reconcile(ledger, self.beat, self.rpc), 0)
        self.assertEqual(entry["status"], "confirmed")
        info = signerd._canonical_mint_confirmation(
            self.rpc, {"txid": txid}, self.beat)
        ...
        entry.pop("block_height", None); entry.pop("block_hash", None)
        self.assertEqual(witness._reconcile(ledger, self.beat, self.rpc), 1)
        self.assertIsNone(signerd._canonical_mint_confirmation(
            self.rpc, {"txid": txid}, self.beat))
    ...
    def test_stale_restore_symlink_oversize_and_witness_down_fail_closed(self):
        ...
        beat_path.write_text(json.dumps(self.beat)); beat_path.chmod(0o600)
        stale = dict(self.beat); stale["expires_at"] = time.time() - 1
        beat_path.write_text(json.dumps(stale))
        with self.assertRaises(SystemExit):
            witness._load_beat(str(beat_path), time.time())
        beat_path.unlink(); beat_path.symlink_to("/etc/passwd")
        with self.assertRaises(SystemExit):
            witness._load_beat(str(beat_path), time.time())
        ...
        oversized = subprocess.run(
            ...
            input="x" * (witness.MAX_STDIN + 1), text=True,
