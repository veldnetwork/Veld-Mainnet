...
def read_protected_file(path, maximum, description, *, private=False):
    ...
    if (
        ...
        or "\x00" in path
        or any(not p or p.endswith((".", " ")) or ":" in p for p in tail[1:].split("\\"))
    ):
    ...
    get_info = api(kernel, "GetFileInformationByHandle", w.BOOL, w.HANDLE, ctypes.POINTER(FileInfo))
    ...
    read_file = api(
        kernel, "ReadFile", w.BOOL, w.HANDLE, ptr, w.DWORD, ctypes.POINTER(w.DWORD), ptr
    )
    ...
    sid_text = api(advapi, "ConvertSidToStringSidW", w.BOOL, ptr, ctypes.POINTER(ptr))
    ...
    try:
        ...
        if (
            file_type(handle) != 1
            or not get_info(handle, ctypes.byref(before))
            or before.attributes & (0x10 | 0x400)
            or before.links != 1
        ):
        ...
        if (
            not count
            or count >= len(name)
            or os.path.normcase(name.value) != os.path.normcase("\\\\?\\" + path)
        ):
        ...
        for index in range(acl.count):
            ...
            owner_access |= principal == current_sid and bool(
                entry.mask & (1 | 0x80000000 | 0x10000000)
            )
        ...
        while remaining:
            ...
            actual = w.DWORD()
            if (
                not read_file(handle, buffer, len(buffer), ctypes.byref(actual), None)
                or not actual.value
            ):
        ...
        after = FileInfo()
        if (
            not get_info(handle, ctypes.byref(after))
            or any(
            ...
            or bytes(before.written) != bytes(after.written)
        ):
