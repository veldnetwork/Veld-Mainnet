...
HASH_FIELDS = ("domain_hash", "deployment_policy_hash", "descriptor_hash", "manifest_hash",
    "signing_policy_hash", "bitcoin_genesis_hash", "domain_genesis_hash")
INSPECTION_HASHES = ("genesis_hash", "bitcoin_genesis_hash", "domain_hash", "deployment_policy_hash",
    "descriptor_hash", "manifest_hash", "signing_policy_hash", "burn_id", "intent_commitment",
    "bitcoin_txid", "unsigned_tx_sha256", "reserve_txid", "authorization_block", "final_block",
    "finality_certificate", "tip_hash")
INSPECTION_NUMBERS = ("epoch", "fee_sats", "reserve_vout", "principal_sats",
    "authorization_height", "final_height", "tip")
...
def configuration(cfg):
    ...
    fields = {"version", "expected_chain", "epoch", "member_index", "member_key",
        "minimum_bitcoin_confirmations", "max_fee_sats", "state_path", "manifest_path", "custody_manifest_sha256", *HASH_FIELDS}
    ...
    for field in (*HASH_FIELDS, "member_key", "custody_manifest_sha256"):
        if type(value[field]) is not str or not HASH.fullmatch(value[field]) or value[field] == "0" * 64:
            raise ValueError("native custody has an invalid identity pin")
    for field, low, high in (("epoch", 1, (1 << 63) - 1), ("member_index", 0, 4),
            ("minimum_bitcoin_confirmations", 144, 1000000), ("max_fee_sats", 1, 100000000)):
    ...
    if type(cfg.get("signer_id")) is not str or not re.fullmatch(r"[A-Za-z0-9._-]{1,128}", cfg["signer_id"]):
...
class CommitmentStore:
    ...
    def __init__(self, cfg, *, initialize=False):
        ...
        self.policy_id = hashlib.sha256(canonical({"policy": self.policy, "signer_id": cfg["signer_id"]}).encode()).hexdigest()
        parent = rd._ensure_secure_directory(os.path.dirname(path), "native custody state directory", create=False, private=True)
        if initialize:
        ...
        try:
            ...
            rows = self.db.execute("SELECT policy,final_height,final_block FROM identity").fetchall()
            if len(rows) != 1 or rows[0][0] != self.policy_id:
                raise ValueError("native custody authority is absent, changed or partially initialized")
    ...
    def commit(self, observation, raw, native):
        ...
        try:
            old_height, old_hash = self.db.execute("SELECT final_height,final_block FROM identity").fetchone()
            if old_height and (o["final_height"] < old_height or native("getblockhash", [old_height]) != old_hash):
                raise ValueError("native custody finalized history was rewound or changed")
            peers = self.db.execute("SELECT burn FROM commitments WHERE reserve_txid=? AND reserve_vout=?",
                (o["reserve_txid"], o["reserve_vout"])).fetchall()
            if any(row[0] != o["burn_id"] for row in peers):
                raise ValueError("reserve edge already belongs to another retained obligation")
            if o["kind"] == "PAYOUT" and self.db.execute("SELECT 1 FROM commitments WHERE burn=? AND kind='RETIREMENT'", (o["burn_id"],)).fetchone():
                raise ValueError("retirement permanently disables fresh payout signatures")
            existing = self.db.execute("SELECT intent,raw,psbt FROM commitments WHERE burn=? AND kind=?", (o["burn_id"], o["kind"])).fetchone()
            if existing:
                if existing[:2] != (o["intent_commitment"], raw):
                    raise ValueError("native custody proposal differs from retained signed authority")
            else:
                if self.db.execute("SELECT count(*) FROM commitments").fetchone()[0] >= MAX_ROWS:
                ...
                self.db.execute("INSERT INTO commitments VALUES (?,?,?,?,?,?,?,NULL)",
                    (o["burn_id"], o["kind"], o["intent_commitment"], raw, o["reserve_txid"], o["reserve_vout"], canonical(o)))
            self.db.execute("UPDATE identity SET final_height=?,final_block=?", (o["final_height"], o["final_block"]))
    ...
    def retain(self, observation, psbt):
        ...
        try:
            ...
            row = self.db.execute("SELECT psbt FROM commitments WHERE burn=? AND kind=? AND intent=?", key).fetchone()
            ...
            self.db.execute("UPDATE commitments SET psbt=? WHERE burn=? AND kind=? AND intent=?", (psbt, *key))
            self.db.execute("COMMIT")
        except BaseException:
            self.db.execute("ROLLBACK")
            raise
...
def inspect(native, policy, wire, raw):
    ...
    fields = {"version", "native_authorized", "kind", "destination_script_hex", "refund_address", *INSPECTION_HASHES, *INSPECTION_NUMBERS}
    if type(o) is not dict or set(o) != fields or type(o["version"]) is not int or o["version"] != 1 or o["native_authorized"] is not True:
    ...
    if (o["epoch"] != policy["epoch"] or not 0 < o["fee_sats"] <= policy["max_fee_sats"] or
            not o["principal_sats"] or not 0 < o["authorization_height"] <= o["final_height"] <= o["tip"] or
            o["reserve_vout"] > 0xffffffff or
            o["unsigned_tx_sha256"] != hashlib.sha256(bytes.fromhex(raw)).hexdigest() or
            type(o["destination_script_hex"]) is not str or not re.fullmatch(r"(?:[0-9a-f]{2}){1,80}", o["destination_script_hex"]) or
            type(o["refund_address"]) is not str or not re.fullmatch(r"V[1-9A-HJ-NP-Za-km-z]{25,49}", o["refund_address"])):
        raise ValueError("native custody inspection fails exact transaction or finality binding")
    for height, block in (("authorization_height", "authorization_block"), ("final_height", "final_block"), ("tip", "tip_hash")):
...
def bitcoin_context(core, policy, binding, observed, raw):
    ...
    if (type(chain) is not dict or chain.get("chain") != "regtest" or chain.get("initialblockdownload") is not False or
            core("getblockhash", 0) != bytes.fromhex(policy["bitcoin_genesis_hash"])[::-1].hex()):
        raise ValueError("independent Bitcoin chain is unavailable or mismatched")
    decoded = core("decoderawtransaction", raw)
    if type(decoded) is not dict or decoded.get("txid") != bytes.fromhex(observed["bitcoin_txid"])[::-1].hex():
    ...
    for index, item in enumerate(inputs):
        if type(item) is not dict or type(item.get("txid")) is not str or not HASH.fullmatch(item["txid"]) or type(item.get("vout")) is not int or not 0 <= item["vout"] <= 0xffffffff:
            raise ValueError("Bitcoin prevout identity is malformed")
        key = (item["txid"], item["vout"])
        if key in seen: raise ValueError("Bitcoin transaction repeats an input")
        seen.add(key)
        coin = core("gettxout", *key, True)
        if (type(coin) is not dict or coin.get("bestblock") != chain.get("bestblockhash") or
                type(coin.get("confirmations")) is not int or coin["confirmations"] < policy["minimum_bitcoin_confirmations"] or
                type(coin.get("scriptPubKey")) is not dict):
    ...
    if type(outputs) is not list or not 1 <= len(outputs) <= 8 or any(type(row) is not dict for row in outputs):
        raise ValueError("Bitcoin outputs are malformed")
    if len(reserve) != 1 or total - sum(rd.btc_to_sats(row["value"]) for row in outputs) != observed["fee_sats"]:
        raise ValueError("Bitcoin inputs or fee differ from native intent")
    after = core("getblockchaininfo")
    if type(after) is not dict or any(after.get(key) != chain.get(key) for key in ("chain", "blocks", "bestblockhash")) or after.get("initialblockdownload") is not False:
...
def refresh_authority(native, policy, wire, raw, previous):
    ...
    if (any(current[field] != previous[field] for field in previous if field not in moving) or
            current["final_height"] < previous["final_height"] or
            native("getblockhash", [previous["final_height"]]) != previous["final_block"]):
...
def sign(request, cfg, store, native, core):
    ...
    if type(request) is not dict or set(request) != {"action", "authorization_hex", "raw_tx_hex"} or request["action"] != "native_sign":
    ...
    for value, bound in ((wire, MAX_AUTH_BYTES), (raw, MAX_RAW_BYTES)):
        if type(value) is not str or not 0 < len(value) <= 2 * bound or not re.fullmatch(r"(?:[0-9a-f]{2})+", value):
            raise ValueError("native custody request exceeds its canonical wire bound")
    binding = custody.load_manifest(policy["manifest_path"], policy["descriptor_hash"], policy["custody_manifest_sha256"], network="regtest")
    ...
    if type(answer) is not dict: raise ValueError("custody wallet could not prepare the exact PSBT")
    ...
    if type(answer) is not dict: raise ValueError("custody wallet did not return a signing result")
...
def validate_partial(psbt, decoded, reserve_index, policy, *, signed):
    if type(psbt) is not dict or type(psbt.get("tx")) is not dict or psbt["tx"].get("txid") != decoded["txid"]:
    ...
    for index, item in enumerate(inputs):
        if type(item) is not dict or item.get("sighash", "ALL") != "ALL" or any(item.get(key) for key in (
                "partial_signatures", "taproot_key_path_sig", "final_scriptSig", "final_scriptwitness")):
        ...
        if index != reserve_index or not signed:
            if signatures: raise ValueError("custody PSBT includes unexpected signatures")
        elif (type(signatures) is not list or len(signatures) != 1 or type(signatures[0]) is not dict or
                set(signatures[0]) != {"pubkey", "leaf_hash", "sig"} or signatures[0]["pubkey"] != policy["member_key"] or
                type(signatures[0]["sig"]) is not str or not re.fullmatch(r"[0-9a-f]{128}01", signatures[0]["sig"])):
            raise ValueError("custody wallet must contribute exactly its one ALL-bound member signature")
        if index != reserve_index: continue
        leaves = item.get("taproot_scripts")
        if type(leaves) is not list or len(leaves) != 1 or type(leaves[0]) is not dict or leaves[0].get("leaf_ver") != 192:
        ...
        if leaf[-2:] != b"\x53\x9c" or any(leaf[i * 34] != 32 or leaf[i * 34 + 33] != (0xac if i == 0 else 0xba) for i in range(5)):
...
def dispatch(request, cfg, native, bitcoin, *, initialize=False):
    ...
    try:
        ...
        def core(method, *params):
            return bitcoin.call(method, *(value if type(value) is str else canonical(value) for value in params), deadline=deadline)
...
def collect(request, cfg, native, bitcoin):
    ...
    if type(signing) is not dict or signing.get("threshold") != 3 or type(signing.get("threshold")) is not int:
        raise ValueError("native custody collection requires exactly three of five")
    members = signing.get("signers")
    if type(members) is not list or len(members) != 5 or any(type(row) is not dict or set(row) != {"id", "command", "member_index", "member_key"} for row in members):
        raise ValueError("native custody collection requires five independently pinned members")
    if (any(type(row["member_index"]) is not int for row in members) or
            sorted(row["member_index"] for row in members) != list(range(5)) or
            any(type(row["id"]) is not str or not re.fullmatch(r"[A-Za-z0-9._-]{1,128}", row["id"]) or
                type(row["member_key"]) is not str or not HASH.fullmatch(row["member_key"]) or
                type(row["command"]) is not list or not 1 <= len(row["command"]) <= 16 or
                any(type(arg) is not str or not arg or "\0" in arg for arg in row["command"]) for row in members) or
            len({row["id"] for row in members}) != 5 or len({row["member_key"] for row in members}) != 5 or
            len({canonical(row["command"]) for row in members}) != 5):
        raise ValueError("native custody identities, keys or command paths are invalid or repeated")
    timeout = rd._bounded_config_int(signing.get("collection_timeout_secs", 600), "native collection deadline", 1, 1800)
    ...
    if type(prepared) is not dict: raise ValueError("external fee wallet could not prepare PSBT")
    psbt = rd._checked_partial_psbt(prepared.get("psbt"))
    info = core("decodepsbt", psbt)
    if type(info) is not dict or type(info.get("inputs")) is not list or len(info["inputs"]) != len(decoded["vin"]):
        raise ValueError("external fee wallet returned malformed input maps")
    forbidden = ("partial_signatures", "taproot_key_path_sig", "taproot_script_path_sigs", "final_scriptSig", "final_scriptwitness")
    if any(type(row) is not dict or any(row.get(key) for key in forbidden) for row in info["inputs"]):
    ...
    if type(sponsored) is not dict: raise ValueError("external fee sponsor produced no PSBT")
    sponsored = rd._checked_partial_psbt(sponsored.get("psbt"))
    info = core("decodepsbt", sponsored)
    if (type(info) is not dict or type(info.get("tx")) is not dict or info["tx"].get("txid") != decoded["txid"] or
            type(info.get("inputs")) is not list or len(info["inputs"]) != len(decoded["vin"]) or
            type(info["inputs"][reserve_index]) is not dict or any(info["inputs"][reserve_index].get(key) for key in forbidden)):
    ...
    for member in members:
        ...
        try:
            result = rd.run_bounded_subprocess(member["command"], input_text=canonical(request),
                timeout=min(remaining, 180), stdout_max=rd.MAX_SIGNER_OUTPUT_BYTES,
                stderr_max=1024 * 1024, description="native custody member")
            if result.returncode: continue
            response = rd.strict_json_loads(result.stdout, "native custody contribution")
            if type(response) is not dict or set(response) != {"signer_id", "psbt"} or response["signer_id"] != member["id"]: continue
            partial = rd._checked_partial_psbt(response["psbt"])
            member_policy = dict(policy, member_index=member["member_index"], member_key=member["member_key"])
            validate_partial(core("decodepsbt", partial), decoded, reserve_index, member_policy, signed=True)
        ...
        for indices in itertools.combinations(range(len(partials)), 3):
            if indices in attempted or time.monotonic() >= deadline: continue
            attempted.add(indices)
            try:
                combined = rd._checked_partial_psbt(core("combinepsbt", [sponsored, *[partials[i][1] for i in indices]]))
                final = core("finalizepsbt", combined)
                if type(final) is not dict or final.get("complete") is not True: continue
                signed = final.get("hex")
                if type(signed) is not str or len(signed) > 2 * rd.MAX_SIGNER_OUTPUT_BYTES or not re.fullmatch(r"(?:[0-9a-f]{2})+", signed): continue
                transaction = core("decoderawtransaction", signed)
                if (type(transaction) is not dict or transaction.get("txid") != decoded["txid"] or
                        type(transaction.get("vin")) is not list or len(transaction["vin"]) != len(decoded["vin"]) or
                        not rd._has_exact_custody_witness({"vin":[transaction["vin"][reserve_index]]})): continue
                acceptance = core("testmempoolaccept", [signed])
                if type(acceptance) is not list or len(acceptance) != 1 or type(acceptance[0]) is not dict or acceptance[0].get("txid") != decoded["txid"] or acceptance[0].get("allowed") is not True: continue
                ...
                continue
    raise RuntimeError("no usable three-of-five native custody quorum before the bounded collection ended")
