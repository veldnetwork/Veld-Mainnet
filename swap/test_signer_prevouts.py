...
class SignerPrevoutTests(unittest.TestCase):
    @staticmethod
    def boundary_rpc(headroom=62_500, proof="00" * 32, consumed=False, accepted_txid=None):
        outpoint = TXID_A + ":0"
            ...
            def call(self, method, params=None):
                if method == "getpeginfo":
                    return {
                        ...
                        "supply_sats": 100_000 - headroom,
                        "tip": 20,
                    }
                if method == "getbtcveldsupply":
                    return {"supply_sats": 100_000 - headroom, "tip": 20, "tip_hash": "22" * 32}
                if method == "getbtcveldmintstatus":
                    answer = {
                        ...
                        "accepted_effect_kind": "MINT" if consumed else None,
    ...
    def test_exact_inputs_resolved_and_caller_metadata_has_no_role(self):
        ...
        rpc = FakeRpc(
            {(TXID_A, 3): utxo(TXID_A, 3, 400_000), (TXID_B, 7): utxo(TXID_B, 7, 700_000)}
        )
        ...
        self.assertEqual([(x["txid"], x["vout"]) for x in got], [(TXID_A, 3), (TXID_B, 7)])
        self.assertEqual(rpc.calls, [("gettxout", [TXID_A, 3]), ("gettxout", [TXID_B, 7])])
    ...
    def test_fresh_headroom_and_exact_mnp1_proof_are_final_boundaries(self):
        ...
        accepted = signer.validate_fresh_mint_boundary(
            self.boundary_rpc(headroom=50_000), "Vissuer", 50_000, outpoint, "00" * 32
        )
        self.assertFalse(accepted["effect_confirmed_retry"])
        with self.assertRaisesRegex(ValueError, "headroom"):
            signer.validate_fresh_mint_boundary(
                self.boundary_rpc(headroom=50_000), "Vissuer", 62_500, outpoint, "00" * 32
            )
        with self.assertRaisesRegex(ValueError, "proof differs"):
            signer.validate_fresh_mint_boundary(
                self.boundary_rpc(proof="11" * 32), "Vissuer", 10_000, outpoint, "00" * 32
            )
    ...
    def test_same_height_a_to_b_supply_mix_is_rejected(self):
        ...
        def mixed(method, params=None):
            ...
            if method == "getbtcveldsupply":
                calls += 1
                if calls == 2:
                    return {"supply_sats": 37_500, "tip": 20, "tip_hash": "99" * 32}
            return original(method, params)

        rpc.call = mixed
        with self.assertRaisesRegex(ValueError, "supply tuple"):
            signer.validate_fresh_mint_boundary(rpc, "Vissuer", 10_000, TXID_A + ":0", "00" * 32)
    ...
    def test_mnp2_reservation_tip_must_match_coherent_supply_tip(self):
        ...
        with self.assertRaisesRegex(ValueError, "funded C1"):
            signer.validate_fresh_mint_boundary(
                ...
                recipient=recipient,
                reservation_script_pubkey=script,
