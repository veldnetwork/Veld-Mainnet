...
def partition_custody_policy(binding):
    ...
    range_valid = (
        isinstance(custody_range, list)
        and len(custody_range) == 2
        and custody_range[0] == 0
        and type(custody_range[1]) is int
        and (
            custody_range[1] == OPERATOR_CUSTODY_RANGE_END or 10999 <= custody_range[1] <= 1_000_000
        )
    )
    expected_count = custody_range[1] + 1 if range_valid else -1
    if (
        not range_valid
        or not isinstance(scripts, (list, tuple))
        or len(scripts) != expected_count
        or len(set(scripts)) != expected_count
        or any(
            not isinstance(script, str) or not P2TR_SPK_RE.fullmatch(script) for script in scripts
        )
    ):
    ...
    answer.update(
        {
            ...
            "operator_script_pubkeys": frozenset(scripts[: OPERATOR_CUSTODY_RANGE_END + 1]),
            "public_script_pubkeys": frozenset(scripts[OPERATOR_CUSTODY_RANGE_END + 1 :]),
        }
    )
    return answer
...
def load_payout_custody_policy(cfg):
    ...
    if (
        ...
        or configured_range[0] != 0
        or type(configured_range[1]) is not int
    ):
...
def _coherent_supply_snapshot(veld):
    ...
    if (
        ...
        or set(snapshot) != {"supply_sats", "tip", "tip_hash"}
        or type(snapshot.get("supply_sats")) is not int
        or not (0 <= snapshot["supply_sats"] <= (1 << 63) - 1)
        or type(snapshot.get("tip")) is not int
        or not (0 <= snapshot["tip"] <= (1 << 63) - 1)
        or not isinstance(snapshot.get("tip_hash"), str)
        or not TXID_RE.fullmatch(snapshot["tip_hash"])
    ):
...
def _mint_locator(status, prefix, tip):
    ...
    if (
        ...
        or not (0 <= tx_index <= 0xFFFFFFFF)
        or type(marker_vout) is not int
        or not (0 <= marker_vout <= 0xFFFFFFFF)
    ):
...
def validate_payout_custody_inputs(veld, inputs, custody_policy):
    ...
    for item in inputs:
        ...
        if (
            ...
            or not P2TR_SPK_RE.fullmatch(script)
            or (txid, vout) in seen
        ):
    ...
    for outpoint, _item in public:
        ...
        if (
            ...
            or set(status) != MINT_STATUS_KEYS
        ...
        if (
            status["consumed"] is not True
            or status["minted"] is not True
            or status.get("accepted_effect_kind") != "C1_MINT"
        ):
        ...
        if (
            ...
            or not C1_ALLOCATION_ID_RE.fullmatch(allocation_id)
        ...
        if before["tip"] - credit[1] <= MAX_REORG_DEPTH:
            raise PublicC1MintNotAccepted(
                "public C1_MINT credit has not cleared MAX_REORG_DEPTH: " + outpoint
            )
...
def clears_veld_reorg_horizon(tip, burn_height):
    ...
    if (
        isinstance(tip, bool)
        or not isinstance(tip, int)
        or isinstance(burn_height, bool)
        or not isinstance(burn_height, int)
    ):
...
def btc_str(sats):
    if isinstance(sats, bool) or not isinstance(sats, int) or sats < 0 or sats > (1 << 63) - 1:
...
def _signer_timeout(signing_cfg):
    return _bounded_config_int(
        signing_cfg.get("signer_timeout_secs", 90), "payout_signing.signer_timeout_secs", 1, 300
    )
...
def _ensure_secure_directory(path, description, *, create, private):
    ...
    if (
        ...
        raise RuntimeError(
            "%s must be a root/service-owned non-symlink %sdirectory" % (description, qualifier)
        )
    return path
...
def _token_from_cfg(rpc):
    ...
    if rpc.get("token_cmd"):
        ...
        out = run_bounded_subprocess(
            cmd, timeout=30, stdout_max=4096, stderr_max=64 * 1024, description="rpc token_cmd"
        )
        if out.returncode != 0:
        ...
        token = (
            read_bounded_secret_file(rpc["token_file"], 4096, "Veld RPC token")
            .decode("ascii")
            .strip()
        )
    if not token:
...
class VeldRpc:
    ...
    def call(self, method, params=None, *, deadline=None):
        ...
        with open_rpc_request(req, timeout=timeout) as resp:
            env = load_bounded_json_response(resp, 32 * 1024 * 1024, "Veld RPC response")
...
def normalize_redeem(r):
    ...
    if (
        out["amount_sats"] <= 0
        or out["amount_sats"] > (1 << 63) - 1
        or out["burn_height"] < 0
        or out["burn_height"] > (1 << 63) - 1
    ):
    ...
    if not HEX_RE.fullmatch(out["dest_btc_addr"]) or len(out["dest_btc_addr"]) > 100:
...
def payout_marker(r):
    ...
    return (
        PAYOUT_MARKER_MAGIC + bytes.fromhex(r["burn_txid"]) + r["opreturn_vout"].to_bytes(4, "big")
    )
    ...
    for _attempt in range(3):
        ...
        for _page_number in range(MAX_REDEEM_PAGES):
            ...
            if not isinstance(tip_hash, str) or not TXID_RE.fullmatch(tip_hash):
                raise RuntimeError("getbtcveldredeems omitted a canonical lowercase tip_hash")
            ...
            if snapshot_tip is None:
                snapshot_tip, snapshot_tip_hash, snapshot_final = (tip, tip_hash, final_height)
                snapshot_authority = authority
            elif (
                tip != snapshot_tip
                or tip_hash != snapshot_tip_hash
                or final_height != snapshot_final
                or authority != snapshot_authority
            ):
                changed = True
                break
            if result.get("cursor") != cursor:
                raise RuntimeError("getbtcveldredeems did not echo the requested cursor")
            if result.get("page_limit") != page_limit:
                raise RuntimeError("getbtcveldredeems did not honor the requested page limit")
            ...
            for x in page:
                ...
                if (
                    x.get("is_redeem") is not True
                    or x.get("is_burn") is not False
                    or x.get("is_mint") is not False
                    or x.get("token") != "btcVELD"
                ):
                    raise RuntimeError("getbtcveldredeems returned a non-canonical redeem row")
                ...
                if record["burn_height"] > tip:
                    raise RuntimeError("getbtcveldredeems returned a row above its snapshot tip")
                ...
                if previous_order_key is not None and order_key <= previous_order_key:
                    raise RuntimeError("getbtcveldredeems rows are not in canonical index order")
            ...
            if has_more and len(page) != page_limit:
                raise RuntimeError("getbtcveldredeems advertised a short interior page")
...
class ObligationStore:
    ...
    def __init__(self, path):
        ...
        _ensure_secure_directory(
            directory, "redemption authority directory", create=True, private=False
        )
        ...
        try:
            for source in records:
                ...
                if old:
                    ...
                    changed.append(r)
            self.db.execute(
                "INSERT OR REPLACE INTO meta(key,value) VALUES('last_tip',?)", (str(tip),)
            )
            self.db.execute(
                "INSERT OR REPLACE INTO meta(key,value) VALUES('last_final_height',?)",
    ...
    def open_records(self):
        rows = self.db.execute(
            ...
            "WHERE status != 'paid' ORDER BY burn_height,burn_txid,vout"
        ).fetchall()
    ...
    def all_records(self):
        rows = self.db.execute(
            "SELECT rid,burn_txid,vout,redeemer,amount_sats,dest_spk,burn_height,"
            "burn_block_hash,status,payout_txid,raw_tx_hex,note FROM obligations "
            "ORDER BY burn_height,burn_txid,vout"
        ).fetchall()
        return [self._row(x) for x in rows]
    ...
    def transition(self, rid, status, payout_txid=None, raw_tx_hex=None, note=None):
        ...
        try:
            cur = self.db.execute(
                "SELECT status,payout_txid,raw_tx_hex FROM obligations WHERE rid=?", (rid,)
            ).fetchone()
            if not cur:
    ...
    def commit_signing_proposal(self, rid, raw_tx_hex, inputs):
        ...
        try:
            cur = self.db.execute(
                "SELECT status,raw_tx_hex FROM obligations WHERE rid=?", (rid,)
            ).fetchone()
            if not cur:
                raise RuntimeError("unknown redemption id")
            proposal = self.db.execute(
                "SELECT raw_tx_hex FROM signing_proposals WHERE rid=?", (rid,)
            ).fetchone()
            ...
            bound = set(
                self.db.execute(
                    "SELECT txid,vout FROM signing_input_commitments WHERE rid=?", (rid,)
                ).fetchall()
            )
...
def spk_to_address(btc, spk_hex):
    ...
    if isinstance(d, dict):
        ...
        addr = (
            d.get("address")
            or (d.get("addresses") or [None])[0]
            or seg.get("address")
            or (seg.get("addresses") or [None])[0]
        )
    if not addr:
...
def validate_payout_tx(
    btc, raw_hex, r, selected, fee_sats, change_sats, change_spk_hex, require_unsigned=False
):
    ...
    if (
        ...
        or d["version"] != 2
        or type(d.get("locktime")) is not int
        or d["locktime"] != 0
    ):
    ...
    if (
        ...
        or not vin
        or any(not isinstance(item, dict) for item in vin)
        or not isinstance(vout, list)
        or any(not isinstance(item, dict) for item in vout)
    ):
    ...
    for txin in vin:
        ...
        if require_unsigned:
            ...
            if (
                not isinstance(script_sig, dict)
                or script_sig.get("hex", "") != ""
                or txin.get("txinwitness")
            ):
                raise RuntimeError("unsigned payout contains signature/witness material")
    if any(
        not isinstance(x.get("txid"), str)
        or not TXID_RE.fullmatch(x["txid"])
        or type(x.get("vout")) is not int
        or not (0 <= x["vout"] <= 0xFFFFFFFF)
        for x in vin
    ):
    ...
    if (
        ...
        or any(
            ...
            or not isinstance(x.get("txid"), str)
            or not TXID_RE.fullmatch(x["txid"])
            or type(x.get("vout")) is not int
            or not (0 <= x["vout"] <= 0xFFFFFFFF)
            or type(x.get("value_sats")) is not int
            or not (0 <= x["value_sats"] <= (1 << 63) - 1)
            for x in selected
        )
    ):
...
def build_payout(btc, r, change_addr, cfg, veld=None, custody_policy=None):
    ...
    r = normalize_redeem(r)
    policy = (
        partition_custody_policy(custody_policy)
        if custody_policy is not None
        else load_payout_custody_policy(cfg)
    )
    ...
    if (
        any(type(value) is not int for value in (minconf, input_vbytes, feerate))
        or not (1 <= minconf <= 1_000_000)
        or not (1 <= input_vbytes <= 1_000_000)
        or not (1 <= feerate <= 1_000_000)
    ):
        raise RuntimeError("invalid payout confirmation/vbytes/feerate policy")
    source_utxos = btc.call("listunspent", minconf, 9999999)
    if (
        not isinstance(source_utxos, list)
        or len(source_utxos) > 10_000_000
        or any(not isinstance(u, dict) for u in source_utxos)
    ):
    ...
    for u in source_utxos:
        ...
        if not isinstance(script, str) or not P2TR_SPK_RE.fullmatch(script.lower()):
    ...
    if 0 < change < DUST_SATS:
        fee += change
        change = 0  # dust is deliberately added to fee
    ...
    validate_payout_tx(btc, raw, r, selected, fee, change, change_spk, require_unsigned=True)
...
def _wallet_transactions(btc):
    ...
    if (
        ...
        or len(transactions) + len(removed) > 10_000_000
    ):
...
def find_existing_payout(btc, r):
    ...
    for txid in _wallet_transactions(btc):
        ...
        if (
            ...
            or info.get("txid", txid) != txid
            or not isinstance(info.get("hex"), str)
            or not re.fullmatch(r"[0-9a-fA-F]+", info["hex"])
            or len(info["hex"]) % 2
        ):
        ...
        if (
            ...
            or type(abandoned) is not bool
            or not isinstance(conflicts, list)
            or any(
                not isinstance(value, str) or not TXID_RE.fullmatch(value) for value in conflicts
            )
        ):
...
def guard_fresh_authority(store, btc, tip):
    ...
    for r in store.open_records():
        if clears_veld_reorg_horizon(int(tip), int(r["burn_height"])) and not find_existing_payout(
            btc, r
        ):
            raise RuntimeError(
                "fresh redemption authority found a mature unmarked burn; "
...
def _has_exact_custody_witness(transaction):
    ...
    for item in inputs:
        ...
        if (
            ...
            or len(witness) != 7
            or any(not isinstance(value, str) for value in witness)
        ):
            return False
        try:
        ...
        for slot in range(5):
            offset = slot * 34
            if script[offset] != 32 or script[offset + 33] != (0xAC if slot == 0 else 0xBA):
                return False
        ...
        for value in witness[:5]:
            if value and (
                not HEX_RE.fullmatch(value)
                or not (len(value) == 128 or (len(value) == 130 and value.endswith("01")))
            ):
                return False
    return True
...
def sign_payout(btc, raw, proposal, signing_cfg):
    ...
    if any(
        ...
        or not x
        or any(not isinstance(arg, str) or not arg or "\x00" in arg for arg in x)
        for x in commands
    ):
    ...
    total_timeout = _bounded_config_int(
        ...
        "payout_signing.collection_timeout_secs",
        1,
        1800,
    )
    ...
    if (
        ...
        or not isinstance(expected.get("txid"), str)
        or not TXID_RE.fullmatch(expected["txid"])
    ):
    ...
    if (
        ...
        or not isinstance(unsigned.get("tx"), dict)
        or unsigned["tx"].get("txid") != expected["txid"]
        or not isinstance(unsigned.get("inputs"), list)
        or not unsigned["inputs"]
    ):
    ...
    for signer_cfg in signers:
        ...
        if (
            ...
            or set(answer) != {"signer_id", "psbt"}
            or answer.get("signer_id") != signer_cfg["id"]
        ):
            continue
        try:
            ...
            if (
                ...
                or not isinstance(decoded.get("tx"), dict)
                or decoded["tx"].get("txid") != expected["txid"]
                or not isinstance(decoded.get("inputs"), list)
                or len(decoded["inputs"]) != len(unsigned["inputs"])
                or any(not isinstance(item, dict) for item in decoded["inputs"])
            ):
                continue
        ...
        for size in range(threshold, len(partials) + 1):
            for indices in itertools.combinations(range(len(partials)), size):
                ...
                try:
                    combined = _checked_partial_psbt(
                        call(
                            "combinepsbt",
                            json.dumps([partials[i][1] for i in indices], separators=(",", ":")),
                        )
                    )
                    ...
                    if (
                        ...
                        or not isinstance(final.get("hex"), str)
                        or len(final["hex"]) > 2 * MAX_SIGNER_OUTPUT_BYTES
                        or not HEX_RE.fullmatch(final["hex"])
                    ):
                        continue
                    transaction = call("decoderawtransaction", final["hex"])
                    if (
                        not isinstance(transaction, dict)
                        or transaction.get("txid") != expected["txid"]
                        or not _has_exact_custody_witness(transaction)
                    ):
                        continue
                    ...
                    if (
                        ...
                        or acceptance[0].get("txid") != expected["txid"]
...
def replicate_observations(signing_cfg, records):
    ...
    for batch_no, batch in enumerate(batches, 1):
        payload = json.dumps(
            {"action": "observe", "records": batch}, sort_keys=True, separators=(",", ":")
        )
        ack = []
        ...
        for s in signers:
            ...
            if out.returncode != 0:
                continue
            try:
                answer = strict_json_loads(out.stdout, "threshold observation response")
            except (ValueError, RecursionError):
                continue
            if (
                not isinstance(answer, dict)
                or set(answer) != {"signer_id", "observed"}
                or answer.get("signer_id") != s.get("id")
                or not isinstance(answer.get("observed"), list)
                or len(answer["observed"]) != len(want)
                or any(not isinstance(item, str) for item in answer["observed"])
            ):
                continue
    ...
    def reconcile_bitcoin_authority(self):
        ...
        for r in self.store.all_records():
            ...
            if existing:
                ...
                status = (
                    "paid"
                    if (existing["confirmations"] > 0 or r["status"] == "paid")
                    else "broadcasting"
                )
                self.store.transition(
                    r["rid"],
                    status,
                    payout_txid=existing["txid"],
        ...
        for r in self.store.open_records():
            ...
            if existing:
                ...
                self.store.transition(
                    ...
                    payout_txid=existing["txid"],
                    raw_tx_hex=existing["hex"],
                    note="Bitcoin marker found",
                )
                print(
                    "  %s: existing Bitcoin payout %s - no re-pay"
                    % (rid[:16], existing["txid"][:16])
                )
                continue
            depth = int(tip) - int(r["burn_height"])
            if depth <= K_VELD:
                print("  %s: depth %d <= MAX_REORG_DEPTH %d - HOLD" % (rid[:16], depth, K_VELD))
                continue
            if final_height is None:
                self.store.transition(rid, "held", note="node omitted final_height")
                print("  %s: no final_height - HOLD" % rid[:16])
                continue
            if int(r["burn_height"]) > int(final_height):
                print("  %s: burn above final_height - HOLD" % rid[:16])
                continue
            canonical, why = verify_burn_canonical(self.veld, r)
            if not canonical:
                self.store.transition(rid, "orphaned", note=why)
                print("  %s: %s - NO PAY" % (rid[:16], why))
                continue
            ...
            if find_existing_payout(self.btc, r):
                print("  %s: payout appeared during build - NO PAY" % rid[:16])
                continue
            proposal = {
                "version": 1,
                "redeem_id": rid,
                "burn_txid": r["burn_txid"],
                "opreturn_vout": r["opreturn_vout"],
                "burn_height": r["burn_height"],
                "burn_block_hash": r["burn_block_hash"],
                "dest_spk": r["dest_btc_addr"],
                "amount_sats": r["amount_sats"],
                "inputs": [
                    {"txid": x["txid"], "vout": x["vout"], "value_sats": x["value_sats"]}
                    for x in selected
                ],
                "fee_sats": fee,
                "change_sats": change,
            ...
            validate_payout_custody_inputs(self.veld, selected, custody_policy)
            self.store.transition(
                rid,
                "broadcasting",
            ...
            if find_existing_payout(self.btc, r):
                print("  %s: payout appeared before broadcast - NO PAY" % rid[:16])
                continue
            try:
                ...
                txid = existing["txid"]
            self.store.transition(
                rid, "broadcasting", payout_txid=txid, raw_tx_hex=signed_hex, note="broadcast"
            )
            print(
                "  %s: BROADCAST %d sats btc_txid=%s approvals=%d fee=%d change=%d"
...
def _authority_path(cfg, state, live):
    ...
    if not path:
        if live:
            raise RuntimeError(
                "live redemption requires authority_db on independent durable storage"
            )
        path = os.path.join(state, "redemption_authority.sqlite3")
    path = os.path.abspath(path)
    state = os.path.abspath(state)
    if "allow_local_authority_dev" in cfg and type(cfg["allow_local_authority_dev"]) is not bool:
        raise RuntimeError("allow_local_authority_dev must be a JSON boolean")
    if (
        live
        and os.path.commonpath([path, state]) == state
        and cfg.get("allow_local_authority_dev") is not True
    ):
...
def main():
    ...
    cfg = load_bounded_json_file(
        os.path.abspath(sys.argv[1]), MAX_CONFIG_BYTES, "redemption coordinator configuration"
    )
    if not isinstance(cfg, dict):
    ...
    if "native_custody" in cfg:
        ...
        if sys.argv[2:] == ["--prepare-native-settlement"]:
            ...
            answer = native_custody_settlement.prepare(
                request, cfg, VeldRpc(cfg["veld_rpc"]), Btc(cfg["cli_base"], cfg["wallet"])
            )
            sys.stdout.write(json.dumps(answer, sort_keys=True, separators=(",", ":")) + "\n")
            return
        signed, members = native_custody_service.collect(
            request, cfg, VeldRpc(cfg["veld_rpc"]), Btc(cfg["cli_base"], cfg["wallet"])
        )
        sys.stdout.write(
            json.dumps(
                {"signed_tx_hex": signed, "contributors": members, "broadcast": False},
                sort_keys=True,
                separators=(",", ":"),
            )
            + "\n"
    ...
    _ensure_secure_directory(state, "redemption state directory", create=True, private=True)
    if "veld_rpc" in cfg and cfg["veld_rpc"] is not None and not isinstance(cfg["veld_rpc"], dict):
    ...
    if live:
        ...
        if (
            type(tip) is not int
            or type(final_height) is not int
            or tip < 0
            or final_height < 0
            or final_height > tip
        ):
        ...
        if os.path.lexists(path):
            offline = load_bounded_json_file(
                ...
                MAX_OFFLINE_REDEEMS_BYTES,
                "offline pending redemptions",
                private=True,
            )
    ...
    print(
        "[redeemd] tip=%d final_height=%s observed=%d durable_open=%d"
