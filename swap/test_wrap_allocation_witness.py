...
class FakeVeldRpc:
    ...
    def call(self, method, params=None):
        ...
        if method == "getbtcveldsupply":
            return {"supply_sats": self.supply_sats, "tip": self.tip, "tip_hash": self.tip_hash}
        if method == "getbtcveldc1reservation":
            return {
                ...
                "allocation_commitment": witness.allocation_commitment(
                    self.allocation_id, self.recipient, self.sats, self.script, self.blind
                ),
                "tip": self.tip,
            }
...
class FakePolicyRpc:
    def __init__(self, compiled_k=6, best_height=899_000, mint_live=True, completion_live=True):
    ...
    def call(self, method, params=None):
        if method == "getpeginfo":
            return {
                ...
                "peg_unlocked": True,
                "mint_live": self.mint_live,
                "completion_live": self.completion_live,
            }
        if method == "getbtcheaderinfo":
            return {"spv_active": True, "best_height": self.best_height, "k_btc": self.compiled_k}
        raise AssertionError(method)
...
class FakePublicPolicyRpc:
    def call(self, method, params=None):
        ...
        if method == "getbtcveldsupply":
            return {"supply_sats": 0, "tip": 100, "tip_hash": "55" * 32}
        if method == "getblockhash":
            return "55" * 32
        raise AssertionError(method)
...
class WrapAllocationWitnessTests(unittest.TestCase):
    def setUp(self):
        ...
        empty = witness._empty_allocation_ledger(self.cfg, self.policy, self.policy_hash)
        witness.atomic_write(
        ...
        self.checkpoint_patch = mock.patch.object(witness, "_checkpoint_allocation_ledger")
    ...
    def register(self, allocation=None):
        ledger = witness._load_allocation_ledger(
            self.paths["allocations"], self.cfg, self.binding, self.policy, self.policy_hash
        )
    ...
    def test_c1_public_allocation_reaches_final_deposit_validation(self):
        ...
        request = {
            ...
            "sats": 100_000,
            "recipient": RECIPIENT,
            "allocation": dict(claim),
            "beat_seq": 1,
            "tip": 10,
            "tip_hash": TIP_HASH,
        }
        beat = sol.stamp_heartbeat(
            sol.build_beat_payload(1_000_000, 0, 0, 10, TIP_HASH, 1, 120), now
        )
        ...
        with (
            mock.patch.object(
                witness,
                "_sign_receipt",
                side_effect=lambda core, cfg: {**core, "sig_alg": "mldsa65", "sig": "00"},
            ),
        ...
        self.assertEqual(second["receipt"]["allocation_capacity_policy_sha256"], "bb" * 32)
        ...
        register_req = {
            ...
            "action": "register_allocation",
            "capacity_policy_sha256": "aa" * 32,
            "allocation": {key: record[key] for key in witness.ALLOCATION_FIELDS},
        }
        with self.assertRaises(SystemExit):
        ...
        self.assertEqual(retry["funded_reserved_at"], funded["funded_reserved_at"])
    ...
    def claim(self):
        return {
            ...
            "public_descriptor_range_start": 1000,
            "public_descriptor_range_end": 10999,
            "consensus_allocation_id": self.allocation["consensus_allocation_id"],
    ...
    def request(self, tag=1):
        ...
        return {
            ...
            "sats": 62_500,
            "recipient": RECIPIENT,
            "allocation": self.claim(),
            "beat_seq": 1,
            "tip": 10,
            "tip_hash": TIP_HASH,
        }

    def decoded(self, recipient=RECIPIENT, sats=62_500, outpoint=OUTPOINT):
            ...
            stdout=json.dumps(
                {
                    ...
                    % (
                        ...
                        self.script,
                        self.allocation["commitment_blind"],
                        outpoint,
                    ),
                }
            ),
    ...
    def test_forced_reservation_entrypoint_is_unconditionally_production_only(self):
        ...
        for cfg in ({}, {"production": False}, {"production": 1}, {"production": "true"}, None):
        ...
        gate_at = source.index("_require_production_reservation_service(cfg)", main_at)
        ledger_at = source.index("ledger = _load_ledger", main_at)
        self.assertLess(gate_at, ledger_at)
        self.assertNotIn(
            'if cfg.get("production") is True:',
            source[
                source.index('elif req.get("action") == "reserve":', main_at) : ledger_at + 2500
            ],
        )
    ...
    def test_forced_command_rechecks_compiled_confirmation_and_spv_policy(self):
        with mock.patch.object(witness.custody_binding, "verify_peg_identity"):
            ...
            for rpc, cfg in (
                ...
                (FakePolicyRpc(completion_live=False), {"btc": {"confirmations": 6}}),
                (FakePolicyRpc(), {"btc": {"confirmations": 5}}),
                (FakePolicyRpc(), {"btc": {"confirmations": True}}),
            ):
                with self.subTest(rpc=rpc.__dict__, cfg=cfg), self.assertRaises(SystemExit):
                    witness._verify_forced_command_peg_policy(rpc, cfg, self.binding)

    def test_production_bitcoin_policy_is_exact_not_operator_tunable(self):
        good = {
            "cli_base": ["bitcoin-cli"],
            "wallet": "custody-watch",
            "confirmations": 6,
            "tip_age_alert_secs": 3600,
            "max_tip_age_secs": 7200,
        }
        btc = witness.BitcoinCli(good)
        self.assertEqual((btc.tip_age_alert_secs, btc.max_tip_age_secs), (3600, 7200))
        for field, value in (
    ...
    def test_c5_five_point_boundaries_cover_allocation_and_mint_reserve(self):
        ...
        for age, phase in expected.items():
            ...
            with self.subTest(path="public-allocation", age=age):
                ...
                with (
                    mock.patch.object(witness, "VeldRpc", return_value=FakePublicPolicyRpc()),
        ...
        now = int(time.time())
        beat = sol.stamp_heartbeat(sol.build_beat_payload(100_000, 0, 0, 10, TIP_HASH, 1, 120), now)
        ledger = witness._empty_ledger()
        ...
        with (
            ...
            mock.patch.object(witness.subprocess, "run", return_value=self.decoded()),
        ):
            first = witness.handle_reserve(
                request,
                self.cfg,
                ...
                ledger,
                beat,
                FakeVeldRpc(script=self.script),
                allocations,
                fresh,
                self.binding,
                899_000,
        ...
        self.assertEqual(first["receipt"]["reservation_id"], replay["receipt"]["reservation_id"])
    ...
    def test_public_allocation_rejects_same_height_mixed_capacity_snapshot(self):
        ...
        with (
            mock.patch.object(witness, "VeldRpc", return_value=MovingSupplyRpc()),
            mock.patch.object(witness, "BitcoinCli", return_value=FakeBitcoin(self.script)),
            mock.patch.object(
                witness,
                "refuse",
                side_effect=lambda message: (_ for _ in ()).throw(RuntimeError(message)),
            ),
        ):
            with self.assertRaisesRegex(RuntimeError, "capacity tuple changed/is incoherent"):
                witness._verify_public_allocation_backend(
                    cfg, dict(witness.C1_POLICY_EXACT), now=100_000
                )
    ...
    def test_registration_is_contiguous_immutable_and_idempotent(self):
        ...
        skipped.update(
            {
                ...
                "btc_address": bech32m_address(self.binding["script_pubkeys"][1001]),
        ...
        absent["btc_address"] = bech32m_address(self.binding["script_pubkeys"][1002])
    ...
    def test_reservation_requires_registered_confirmed_exact_bitcoin_output(self):
        ...
        btc = FakeBitcoin(self.script)
        beat = sol.stamp_heartbeat(
            sol.build_beat_payload(100_000, 0, 0, 10, TIP_HASH, 1, 120), int(time.time())
        )
        ledger = witness._empty_ledger()
        with (
            mock.patch.object(
        ...
        cases.append(("underconfirmed", self.request(), underconfirmed, self.decoded()))
        ...
        reorg = FakeBitcoin(self.script)
        reorg.after = "55" * 32
        cases.append(("tip-race", self.request(), reorg, self.decoded()))
        ibd = FakeBitcoin(self.script)
        ibd.ibd = True
        cases.append(("ibd", self.request(), ibd, self.decoded()))
        header_lag = FakeBitcoin(self.script)
        header_lag.headers += 1
        ...
        behind_spv = FakeBitcoin(self.script)
        behind_spv.blocks = 898_999
        behind_spv.headers = 898_999
        cases.append(("behind-veld-spv", self.request(), behind_spv, self.decoded()))
    ...
    def test_proofless_mnp2_waits_for_canonical_c1_fund(self):
        ...
        with (
            ...
            witness.handle_reserve(
                ...
                witness._empty_ledger(),
                beat,
                UnfundedRpc(script=self.script),
                allocations,
                FakeBitcoin(self.script),
                self.binding,
                899_000,
        ...
        self.assertIsNotNone(allocations["allocations"][0]["deposit_observed_at"])
        self.assertIsNone(allocations["allocations"][0]["funded_reserved_at"])
        ...
        decoded["memo"] += ";00"
        stale = subprocess.CompletedProcess([], 0, stdout=json.dumps(decoded), stderr="")
        with (
            mock.patch.object(witness.subprocess, "run", return_value=stale),
            self.assertRaises(SystemExit),
        ):
    ...
    def test_funded_reorg_restart_refund_and_proofless_mnp2_recover(self):
        ...
        with (
            mock.patch.object(witness.subprocess, "run", return_value=self.decoded()),
            mock.patch.object(
                witness, "_sign_receipt", side_effect=RuntimeError("simulated signer loss")
            ),
        ...
        self.assertEqual(witness._load_ledger(self.paths["ledger"], self.cfg)["reservations"], [])
        ...
        with (
            ...
            witness.handle_reserve(
                ...
                beat,
                ReorgedFundingRpc(script=self.script),
                restarted,
                bitcoin,
                self.binding,
                899_000,
        ...
        self.assertEqual(after_reorg["events"][-1]["action"], "funded_reservation_reverted")
        ...
        restarted_reservations = witness._load_ledger(self.paths["ledger"], self.cfg)
        signed_receipt = lambda core, cfg: {**core, "sig_alg": "mldsa65", "sig": "00"}
        with (
            mock.patch.object(witness.subprocess, "run", return_value=self.decoded()),
            mock.patch.object(witness, "_sign_receipt", side_effect=signed_receipt),
        ):
        ...
        self.assertEqual(
            recovered["receipt"]["allocation_request_id"],
            self.allocation["consensus_allocation_id"],
        )
        final = self.registered_ledger()
        self.assertEqual(final["allocations"][0]["funding_outpoint"], OUTPOINT)
        self.assertEqual(
            [event["action"] for event in final["events"]].count("funded_reservation_observed"), 2
        )
        self.assertEqual(final["events"][-1]["action"], "funded_reservation_observed")
        ...
        self.assertEqual(
            signed_state["reservations"][0]["receipt"]["reservation_id"],
            recovered["receipt"]["reservation_id"],
        )
        after_signed_reorg = self.registered_ledger()
        self.assertEqual(
            after_signed_reorg["allocations"][0]["deposit_observed_at"], deposit_observed_at
        )
        self.assertIsNone(after_signed_reorg["allocations"][0]["funded_reserved_at"])
        self.assertIsNone(after_signed_reorg["allocations"][0]["funding_outpoint"])
        self.assertEqual(
            len(witness._load_ledger(self.paths["ledger"], self.cfg)["reservations"]), 1
        )
        ...
        self.assertTrue(replay["idempotent"])
        self.assertEqual(
            replay["receipt"]["reservation_id"], recovered["receipt"]["reservation_id"]
        )
        ...
        self.assertEqual(after_mint_restart["events"][-1]["action"], "mint_effect_observed")
        self.assertEqual(
            after_mint_restart["events"][-1]["request_id"], self.allocation["request_id"]
        )
        self.assertNotEqual(
            after_mint_restart["events"][-1]["request_id"],
            self.allocation["consensus_allocation_id"],
        )
    ...
    def test_one_bitcoin_outpoint_and_allocation_bind_one_immutable_mint_template(self):
        ...
        allocations = self.registered_ledger()
        beat = sol.stamp_heartbeat(
            sol.build_beat_payload(200_000, 0, 0, 10, TIP_HASH, 1, 120), int(time.time())
        )
        ledger = witness._empty_ledger()
        with (
            mock.patch.object(
    ...
    def test_c1_retirement_requires_exact_effect_and_checkpoints_allocation(self):
        ...
        class EffectRpc:
            ...
            def call(self, method, params=None):
                ...
                if method == "getbtcveldsupply":
                    return {"supply_sats": 0, "tip": 100, "tip_hash": TIP_HASH}
                if method == "getbtcveldmintstatus":
                    answer = {
                        "outpoint": OUTPOINT,
                        ...
                        answer.update(
                            {
                                ...
                                "accepted_effect_kind": (
                                    "C1_MINT" if self.phase == "C1_MINT" else "C1_FUND"
                                ),
        ...
        rpc.phase = "C1_MINT"
        with (
            mock.patch.object(witness, "atomic_write") as write,
            ...
            self.assertEqual(authority["events"][-1]["action"], "mint_effect_observed")
            self.assertEqual(authority["events"][-1]["request_id"], allocation["request_id"])
            write.assert_called()
            ...
            self.assertEqual(entry["status"], "reserved")
            self.assertEqual(authority["events"][-1]["action"], "mint_effect_reverted")
            self.assertEqual(authority["events"][-1]["request_id"], allocation["request_id"])
        self.assertEqual(len(ledger["reservations"]), 1)
