...
def blob(value, maximum):
    if type(value) is not str or not 0 < len(value) <= 2 * maximum or not re.fullmatch(r"(?:[0-9a-f]{2})+", value):
...
def reserve_network_binding(policy):
    ...
    return hashlib.sha256(b"VELD/BTCVELD/RESERVE_NETWORK/v1" + vector(genesis.encode()) +
        vector(profile.encode()) + vector(b"btcveld-reserve-outpoint-v1")).digest()


def merkle_branch(txids, wanted, header):
    if (type(txids) is not list or not 1 <= len(txids) <= 100000 or
            any(type(txid) is not str or not authority.HASH.fullmatch(txid) for txid in txids) or
            len(set(txids)) != len(txids) or txids.count(wanted) != 1 or len(header) != 80):
    ...
    while len(rows) > 1:
        if len(rows) % 2: rows.append(rows[-1])
...
def prepare(request, cfg, native, bitcoin):
    ...
    if type(request) is not dict or set(request) != {"action", "authorization_hex", "raw_tx_hex", "fund_address"} or request["action"] != "native_settlement":
    ...
    def core(method, *params):
        return bitcoin.call(method, *(v if type(v) is str else authority.canonical(v) for v in params), deadline=deadline)
    ...
    binding = authority.custody.load_manifest(policy["manifest_path"], policy["descriptor_hash"],
        policy["custody_manifest_sha256"], network="regtest")
    ...
    if (type(peg) is not dict or peg.get("reserve_accounting_holds") is not True or
            peg.get("reserve_status") != "ACTIVE" or peg.get("reserve_txid") != observed["reserve_txid"] or
            peg.get("reserve_vout") != observed["reserve_vout"]):
    ...
    if (type(chain) is not dict or chain.get("chain") != "regtest" or chain.get("initialblockdownload") is not False or
            core("getblockhash", 0) != hash_bytes(policy["bitcoin_genesis_hash"])[::-1].hex()):
    ...
    if type(transaction) is not dict or transaction.get("txid") != txid or integer(transaction.get("confirmations")) < policy["minimum_bitcoin_confirmations"]:
    ...
    if (type(block) is not dict or block.get("hash") != block_hash or
            core("getblockhash", integer(block.get("height"))) != block_hash or
            integer(block.get("confirmations")) < policy["minimum_bitcoin_confirmations"]):
    ...
    for authorized, (parent_hash, vout) in zip(parents, parsed["inputs"]):
        parent_raw = blob(core("getrawtransaction", parent_hash[::-1].hex(), False), 4 * 1024 * 1024)
        parent = parse_and_strip_bitcoin_tx(parent_raw)
        bound_parent = parse_and_strip_bitcoin_tx(authorized)
        if (digest(parent["legacy"]) != parent_hash or not vout < len(parent["outputs"]) or
                bound_parent["legacy"] != parent["legacy"]):
            raise ValueError("settlement parent does not match the exact consumed output")
    if len(stripped) > 12000 or sum(map(len, parents)) > 10000 or any(len(p) > 12000 for p in parents):
    ...
    if new_value < 0 or (new_value and (len(successors) != 1 or successors[0]["value_sats"] != new_value)) or (not new_value and successors):
    ...
    proof = (b"RTP1" + bytes([4 if payout else 3]) + network + hash_bytes(peg.get("reserve_transition_commitment")) +
        hash_bytes(peg["reserve_txid"]) + struct.pack("<IQQ", integer(peg["reserve_vout"], 0xffffffff), prior_value,
            integer(peg.get("reserve_transition_count"))) + (hash_bytes(observed["bitcoin_txid"]) if new_value else bytes(32)) +
        struct.pack("<IQ", new_vout, new_value) + hash_bytes(observed["bitcoin_txid"]) + hash_bytes(block_hash)[::-1] +
        struct.pack("<IB", directions, len(branch)) + b"".join(branch) + (wire[133:165] if payout else bytes(32)) +
        struct.pack("<Q", 0) + vector(stripped) + struct.pack("<H", len(parents)) + b"".join(vector(p) for p in parents) + vector(b""))
    ...
    if (authority.inspect(native, policy, wire.hex(), raw.hex()) != observed or native("getpeginfo", []) != peg or
            core("getblockhash", block["height"]) != block_hash):
        raise ValueError("settlement authority, reserve or Bitcoin inclusion changed during preparation")
    ...
    return {"version": 1, "bitcoin_txid": txid, "bitcoin_block": block_hash,
        "burn_id": observed["burn_id"], "kind": observed["kind"], "settlement_hex": settlement.hex(),
        "proof_hex": proof.hex(), "prepared": prepared, "broadcast": False}
