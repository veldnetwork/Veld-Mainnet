...
def t_headroom_math():
    check("headroom = custody-liability-margin",
...
def t_build_beat():
    p = sol.build_beat_payload(10 * BTC, 6 * BTC, BTC, tip=100,
    ...
    burn_pending = sol.build_beat_payload(
        10 * BTC, 6 * BTC, BTC, tip=100, tip_hash=TIP_HASH, seq=2,
        ttl_secs=120, backing_liability_sats=7 * BTC)
    check("pending burn remains in signed backing liability",
          burn_pending["backing_liability_sats"] == 7 * BTC and
          burn_pending["headroom_sats"] == 2 * BTC and
          sol.validate_beat_payload(burn_pending)[0])
    # cannot build a beat while insolvent
    raised = False
    try:
        sol.build_beat_payload(5 * BTC, 6 * BTC, 0, tip=1,
                               tip_hash=TIP_HASH, seq=1, ttl_secs=120)
    except ValueError:
        raised = True
    ...
    try:
        sol.build_beat_payload(
            10 * BTC, 6 * BTC, 0, tip=1, tip_hash=TIP_HASH, seq=1,
            ttl_secs=120, backing_liability_sats=5 * BTC)
...
def t_payload_validation():
    ...
    bad_ver = dict(good); bad_ver["v"] = 999
    check("wrong version rejected", not sol.validate_beat_payload(bad_ver)[0])

    lied = dict(good); lied["headroom_sats"] = 9 * BTC   # claim more than custody-supply
    check("inconsistent headroom rejected", not sol.validate_beat_payload(lied)[0])

    missing_liability = dict(good); missing_liability.pop("backing_liability_sats")
    check("heartbeat missing complete liability rejected",
          not sol.validate_beat_payload(missing_liability)[0])

    below_supply = dict(good); below_supply["backing_liability_sats"] = 5 * BTC
    below_supply["headroom_sats"] = 4 * BTC
    check("heartbeat liability below supply rejected",
          not sol.validate_beat_payload(below_supply)[0])

    badttl = dict(good); badttl["ttl_secs"] = 10 ** 9
    check("oversized ttl rejected", not sol.validate_beat_payload(badttl)[0])

    bad_hash = dict(good); bad_hash["tip_hash"] = "AA" * 32
    check("non-canonical tip hash rejected", not sol.validate_beat_payload(bad_hash)[0])

    for label, value in (("bool", True), ("string", "10"),
                         ("float", 10.0), ("int64 overflow", 1 << 63)):
        malformed = dict(good); malformed["custody_sats"] = value
        check("%s integer field rejected" % label,
              not sol.validate_beat_payload(malformed)[0])

    negative_supply = dict(good)
    negative_supply["supply_sats"] = -5 * BTC
    negative_supply["headroom_sats"] = (
        negative_supply["custody_sats"] - negative_supply["supply_sats"] -
        negative_supply["margin_sats"])
    check("negative supply cannot fabricate positive headroom",
...
def t_gate_fail_closed():
    ...
    check("mint within headroom allowed",
          sol.heartbeat_gate(beat, now + 1, 0, 3 * BTC)[0])
    ...
    check("expired heartbeat denies",
          not sol.heartbeat_gate(beat, now + 121, 0, 1)[0])

    # mint exceeds headroom -> deny (custody-box compromise trying to over-mint)
    check("over-headroom mint denies",
          not sol.heartbeat_gate(beat, now + 1, 0, 5 * BTC)[0])

    # in-flight window already consumed the headroom -> deny the next one
    check("window in-flight charged against headroom",
          not sol.heartbeat_gate(beat, now + 1, 4 * BTC, 1 * BTC)[0])

    # exactly at the boundary (window_signed + mint == headroom) is allowed;
    # one sat over is not.
    check("exact boundary allowed",
          sol.heartbeat_gate(beat, now + 1, 2 * BTC, 2 * BTC)[0])
    check("one sat over boundary denied",
          not sol.heartbeat_gate(beat, now + 1, 2 * BTC, 2 * BTC + 1)[0])

    # forged on-disk heartbeat (negative headroom) -> deny
    forged = dict(beat); forged["headroom_sats"] = -1
    check("forged negative-headroom heartbeat denies",
          not sol.heartbeat_gate(forged, now + 1, 0, 1)[0])

    # non-positive mint -> deny
    check("non-positive mint denies",
          not sol.heartbeat_gate(beat, now + 1, 0, 0)[0])
...
def t_receiver_clock_stamp():
    ...
    p2 = dict(p); p2["ttl_secs"] = 10_000
    hb2 = sol.stamp_heartbeat(p2, 500.0)
    check("stamp clamps ttl to MAX_TTL_SECS",
          hb2["expires_at"] == 500.0 + sol.MAX_TTL_SECS)


if __name__ == "__main__":
    for fn in (t_headroom_math, t_build_beat, t_payload_validation,
               t_gate_fail_closed, t_receiver_clock_stamp):
        print(fn.__name__)
        fn()
    print()
    if FAILS:
