...
CONFIG = os.environ.get(
    "VELD_WITNESS_CONFIG",
    _CONFIG_ARGS[0] if len(_CONFIG_ARGS) == 1 else os.path.join(HERE, "watchtowerd.conf.json"),
)
...
C1_POLICY_KEYS = {
    ...
    "principal_lifetime_sats",
    "destination_lifetime_sats",
    "consensus_custody_ceiling_sats",
    "minimum_allocation_sats",
    "unfunded_expiry_seconds",
    "operator_descriptor_range",
    "public_descriptor_range",
    "journal_max_rows",
    "journal_max_bytes",
    "lifecycle_reserve_percent",
    "bitcoin_freshness_alert_seconds",
    "bitcoin_recovery_only_seconds",
    "peg_unlock_required",
    "archive_ack_required",
    "archive_authority_id",
}
C1_POLICY_EXACT = {
    "version": 1,
    "pow_bits": 24,
    "epoch_seconds": 600,
    "global_allocations_per_epoch": 5,
    "global_allocations_per_day": 200,
...
def _require_exact_c5_btc_policy(btc):
    ...
    if (
        getattr(btc, "tip_age_alert_secs", None) != C5_TIP_ALERT_SECONDS
        or getattr(btc, "max_tip_age_secs", None) != C5_TIP_HARD_SECONDS
    ):
        refuse(
            "production Bitcoin freshness policy must be exactly "
...
def _empty_terminal_log_text():
    return json.dumps(TERMINAL_LOG_HEADER, sort_keys=True, separators=(",", ":")) + "\n"
...
class VeldRpc:
    def __init__(self, cfg):
        ...
        if cfg.get("token_cmd"):
            ...
            token = (
                read_bounded_secret_file(cfg["token_file"], 4096, "Veld RPC token")
                .decode("ascii")
                .strip()
            )
    ...
    def call(self, method, params=None):
        ...
        with open_rpc_request(request, timeout=15) as response:
            envelope = load_bounded_json_response(response, 32 * 1024 * 1024, "Veld RPC response")
...
class BitcoinCli:
    ...
    def __init__(self, cfg):
        ...
        if (
            ...
            or not cli
            or len(cli) > 64
            or any(not isinstance(item, str) or not item or "\x00" in item for item in cli)
            or not isinstance(wallet, str)
            or not wallet
            or len(wallet) > 128
            or type(confirmations) is not int
            or not 1 <= confirmations <= 1_000_000
            or tip_age_alert_secs != C5_TIP_ALERT_SECONDS
            or max_tip_age_secs != C5_TIP_HARD_SECONDS
        ):
    ...
    def call(self, method, *args):
        if not isinstance(method, str) or not re.fullmatch(r"[a-z][a-z0-9]{0,63}", method):
        ...
        run = run_bounded_subprocess(
            ...
            + [
                ...
                if value is None or type(value) in (bool, list, dict, float)
                else str(value)
                for value in args
            ],
            timeout=60,
            stdout_max=32 * 1024 * 1024,
            stderr_max=1024 * 1024,
            description="bitcoin-cli %s" % method,
        )
        if run.returncode != 0:
            raise RuntimeError("bitcoin-cli %s: %s" % (method, run.stderr.strip()[:500]))
...
def _paths(cfg):
    ...
    return {
        ...
        "terminal": os.path.join(state_dir, "mint-reservation-tombstones.jsonl"),
        "allocations": os.path.join(state_dir, "wrap-allocation-authority.json"),
...
def _reservation_ledger_policy(cfg=None, production=False):
    ...
    """
    if not production and (not isinstance(cfg, dict) or "reservation_ledger" not in cfg):
        return {
            "max_active_rows": 999,
    ...
    if production and (
        policy["max_active_rows"] < PRODUCTION_ACTIVE_RESERVATION_ROWS
        or policy["max_active_bytes"] < PRODUCTION_ACTIVE_RESERVATION_BYTES
    ):
        refuse(
            "production reservation ledger cannot represent all 10,000 minimum-size active leases"
        )
...
def _validate_active_entry(entry, index):
    ...
    allowed_entry = {"receipt", "status", "txid", "block_height", "block_hash", "signed_tx_sha256"}
    ...
    txid = entry.get("txid")
    if txid is not None and (not isinstance(txid, str) or not sol.HASH256_RE.fullmatch(txid)):
    ...
    if status in ("confirmed", "final"):
        if (
            txid is None
            or type(entry.get("block_height")) is not int
            or entry["block_height"] < 0
            or not isinstance(entry.get("block_hash"), str)
            or not sol.HASH256_RE.fullmatch(entry["block_hash"])
        ):
...
def _validate_tombstone(record, expected_sequence, expected_previous):
    ...
    if (
        ...
        or tombstone_version not in (TOMBSTONE_VERSION, C1_TOMBSTONE_VERSION)
        or record.get("kind") != TOMBSTONE_KIND
        or record.get("sequence") != expected_sequence
        or record.get("prev_record_sha256") != expected_previous
    ):
    ...
    for field in (
        ...
        if not isinstance(record.get(field), str) or not sol.HASH256_RE.fullmatch(record[field]):
    ...
    if (
        not isinstance(record.get("issuer_id"), str)
        or not sol.IDENTIFIER_RE.fullmatch(record["issuer_id"])
        or not isinstance(record.get("witness_id"), str)
        or not sol.IDENTIFIER_RE.fullmatch(record["witness_id"])
        or type(record.get("sats")) is not int
        or not 1 <= record["sats"] <= sol.MAX_WIRE_INT
        or not isinstance(record.get("recipient"), str)
        or not 1 <= len(record["recipient"]) <= 128
        or type(record.get("allocation_verified")) is not bool
    ):
    ...
    if record["allocation_verified"]:
        if tombstone_version == TOMBSTONE_VERSION:
            descriptor_allowed = (
                type(allocation_values[1]) is int and 1 <= allocation_values[1] <= 999
            )
        else:
            policy_sha256 = record.get("allocation_capacity_policy_sha256")
            range_start = record.get("allocation_public_descriptor_range_start")
            range_end = record.get("allocation_public_descriptor_range_end")
            descriptor_allowed = (
                isinstance(policy_sha256, str)
                and sol.HASH256_RE.fullmatch(policy_sha256) is not None
                and type(range_start) is int
                and range_start == 1000
                and type(range_end) is int
                and 10999 <= range_end <= 1_000_000
                and type(allocation_values[1]) is int
                and range_start <= allocation_values[1] <= range_end
            )
        if (
            not isinstance(allocation_values[0], str)
            or not ALLOCATION_REQUEST_RE.fullmatch(allocation_values[0])
            or not descriptor_allowed
            or not isinstance(allocation_values[2], str)
            or not re.fullmatch(r"bc1p[023456789ac-hj-np-z]{58}", allocation_values[2])
            or not isinstance(allocation_values[3], str)
            or not re.fullmatch(r"5120[0-9a-f]{64}", allocation_values[3])
            or not isinstance(allocation_values[4], str)
            or not BTC_OUTPOINT_RE.fullmatch(allocation_values[4])
        ):
    ...
    if (
        ...
        or type(tip) is not int
        or height < 0
        or tip < height
        or tip - height + 1 <= MAX_REORG_DEPTH
    ):
...
def _same_tombstone_identity(receipt, tombstone):
    ...
    return (
        all(receipt.get(field) == tombstone.get(field) for field in fields)
        and _receipt_full_sha256(receipt) == tombstone["receipt_sha256"]
    )
...
def _load_terminal_log(path, policy):
    ...
    text, byte_count = _read_text_nofollow_bounded(
        path, policy["max_terminal_bytes"], "terminal reservation tombstone log", exact_mode=0o600
    )
    ...
    try:
        header = strict_json_loads(lines[0].encode("utf-8"), "terminal tombstone log header")
    ...
    for offset, line in enumerate(lines[1:], 1):
        ...
        try:
            record = strict_json_loads(line.encode("utf-8"), "terminal reservation tombstone")
        except Exception as error:
...
def _load_ledger(path, cfg=None):
    ...
    try:
        ...
        text, ignored_size = _read_text_nofollow_bounded(
            path, policy["max_active_bytes"], "active reservation ledger", exact_mode=0o600
        )
    ...
    if (
        ...
        or not isinstance(ledger.get("reservations"), list)
        or type(ledger.get("terminal_count")) is not int
        or not isinstance(ledger.get("terminal_head_sha256"), str)
        or not sol.HASH256_RE.fullmatch(ledger["terminal_head_sha256"])
        or set(ledger) != {"version", "terminal_count", "terminal_head_sha256", "reservations"}
    ):
        refuse("reservation ledger schema is invalid")
    if (
        ledger["terminal_count"] < 0
        or ledger["terminal_count"] > policy["max_terminal_rows"]
        or len(ledger["reservations"]) > policy["max_active_rows"]
    ):
    ...
    terminal_path = os.path.join(
        os.path.dirname(os.path.abspath(path)), "mint-reservation-tombstones.jsonl"
    )
    ...
    active_by_id = {entry["receipt"]["reservation_id"]: entry for entry in ledger["reservations"]}
    for record in tail:
    ...
    ledger["terminal_head_sha256"] = terminals[-1]["record_sha256"] if terminals else ZERO_HASH
...
def _validate_c1_policy(policy):
    ...
    if (
        ...
        or not isinstance(public_range, list)
        or len(public_range) != 2
        or public_range[0] != 1000
        or type(public_range[1]) is not int
        or not 10999 <= public_range[1] <= 1_000_000
        or not isinstance(authority, str)
        or not 8 <= len(authority) <= 256
        or authority.startswith("REPLACE_")
    ):
...
def _c1_public_index_allowed(index, policy):
    return (
        ...
        and isinstance(policy.get("public_descriptor_range"), list)
        and policy["public_descriptor_range"][0] <= index <= policy["public_descriptor_range"][1]
    )
...
def _c1_receipt_policy_refresh_allowed(old, c1_policy, policy_sha256):
    ...
    if (
        ...
        or old.get("v") != sol.C1_RESERVATION_VERSION
        or not isinstance(c1_policy, dict)
    ...
    if (
        ...
        or old_start != current_range[0]
        or type(old_end) is not int
        or type(index) is not int
        or not old_start <= index <= old_end
        or old_end > current_range[1]
    ):
        return False
    if old_hash == policy_sha256:
        return old_end == current_range[1]
    return old_hash == c1_policy.get("previous_policy_sha256") and old_end < current_range[1]
...
def _load_signed_c1_policy(cfg):
    ...
    try:
        raw = read_bounded_regular_file(paths[0], 1024 * 1024, "signed C1 capacity policy")
        signature = (
            read_bounded_regular_file(paths[1], 16 * 1024, "signed C1 capacity policy signature")
            .decode("ascii", "strict")
            .strip()
        )
        public_key = (
            read_bounded_regular_file(paths[2], 16 * 1024, "signed C1 capacity policy public key")
            .decode("ascii", "strict")
            .strip()
        )
        if (
            not re.fullmatch(r"[0-9a-f]{3904}", public_key)
            or not re.fullmatch(r"[0-9a-f]{6618}", signature)
            or not verify_mldsa65_with_keygen(public_key, raw, signature, verifier)
        ):
...
def _allocation_policy(cfg, *, require_c1=False):
    ...
    if (
        not require_c1
        and isinstance(policy, dict)
        and set(policy) == {"version", "initial_descriptor_index", "allow_initial_ledger_creation"}
        and policy.get("version") == 1
        and policy.get("initial_descriptor_index") == 1
        and type(policy.get("allow_initial_ledger_creation")) is bool
    ):
        return dict(policy)
    if (
        ...
        != {
            "version",
            "initial_descriptor_index",
            "allow_initial_ledger_creation",
            "checkpoint_command",
        }
        or policy.get("version") != 2
        or policy.get("initial_descriptor_index") != 1000
        or type(policy.get("allow_initial_ledger_creation")) is not bool
        or not isinstance(policy.get("checkpoint_command"), list)
        or not policy["checkpoint_command"]
        or any(
            not isinstance(value, str) or not value or "\x00" in value
            for value in policy["checkpoint_command"]
        )
        or not os.path.isabs(policy["checkpoint_command"][0])
    ):
...
def _require_supported_allocation_range(c1_policy):
    ...
    if (
        C1_ALLOCATION_ROTATION_PROTOCOL_VERSION != 1
        and validated["public_descriptor_range"][1] != 10999
    ):
        refuse(
...
def _validate_registered_allocation(record, binding, c1_policy=None):
    fields = (
        LEGACY_REGISTERED_ALLOCATION_FIELDS if c1_policy is None else REGISTERED_ALLOCATION_FIELDS
    )
    ...
    if (
        ...
        or not 1 <= sats <= sol.MAX_WIRE_INT
        or type(index) is not int
        or not index_start <= index <= index_end
        or (c1_policy is not None and not _c1_public_index_allowed(index, c1_policy))
        ...
        or not 0 <= registered_at <= sol.MAX_WIRE_INT
        or not isinstance(binding, dict)
        or binding.get("range") != expected_range
        or not isinstance(binding.get("script_pubkeys"), tuple)
        or script != binding["script_pubkeys"][index]
    ):
        refuse("registered wrap allocation fields are invalid")
    if c1_policy is not None and (
        sats < c1_policy["minimum_allocation_sats"]
        ...
        or not ALLOCATION_REQUEST_RE.fullmatch(consensus_allocation_id)
        or int(consensus_allocation_id, 16) == 0
        or type(record.get("admitted_at")) is not int
        or type(record.get("expires_at")) is not int
        or record["expires_at"] - record["admitted_at"] != c1_policy["unfunded_expiry_seconds"]
        or (
            record.get("deposit_observed_at") is not None
            and (
                type(record["deposit_observed_at"]) is not int
                or record["deposit_observed_at"] < record["admitted_at"]
            )
        )
        or (
            record.get("funded_reserved_at") is not None
            and (
                type(record["funded_reserved_at"]) is not int
                or record["deposit_observed_at"] is None
                or record["funded_reserved_at"] < record["deposit_observed_at"]
                or not isinstance(funding_outpoint, str)
                or not BTC_OUTPOINT_RE.fullmatch(funding_outpoint)
                or int(funding_outpoint.rsplit(":", 1)[1]) > 0xFFFFFFFF
            )
        )
        or (record.get("funded_reserved_at") is None and funding_outpoint is not None)
        or (
            record.get("minted_at") is not None
            and (
                type(record["minted_at"]) is not int
                or record["funded_reserved_at"] is None
                or record["minted_at"] < record["funded_reserved_at"]
            )
        )
    ):
...
def _load_allocation_ledger(path, cfg, binding, c1_policy=None, policy_sha256=None):
    if not os.path.lexists(path):
    ...
    try:
        ...
        ledger = strict_json_loads(_read_text_nofollow(path), "wrap allocation authority ledger")
    except Exception as e:
    ...
    if (
        ...
        or ledger.get("initial_descriptor_index") != authority["initial_descriptor_index"]
        or not isinstance(ledger.get("allocations"), list)
        or len(ledger["allocations"]) > (999 if legacy else 100_000)
    ):
    ...
    if not legacy:
        ...
        if (
            ...
            or stored_end < 10999
            or type(last_consensus_sequence) is not int
            or not 0 <= last_consensus_sequence <= C1_UINT64_MAX
            or not isinstance(events, list)
            or len(ledger["allocations"]) + len(events) > c1_policy["journal_max_rows"]
        ):
            refuse("wrap allocation authority C1 ledger is malformed")
        if stored_sequence == c1_policy["record_sequence"]:
            if (
                stored_hash != policy_sha256
                or stored_end != c1_policy["public_descriptor_range"][1]
            ):
                refuse("allocation witness signed-config hash mismatch")
        elif (
            c1_policy["record_sequence"] != stored_sequence + 1
            or c1_policy["previous_policy_sha256"] != stored_hash
            or c1_policy["public_descriptor_range"][1] < stored_end
        ):
    ...
    for offset, record in enumerate(ledger["allocations"]):
        ...
        consensus_sequence = int(record["consensus_allocation_id"], 16) if not legacy else 0
        if (
            (legacy and record["descriptor_index"] != expected)
            or (
                not legacy
                and (
                    consensus_sequence == 0
                    or consensus_sequence > ledger["last_consensus_sequence"]
                )
            )
            or record["request_id"] in seen_requests
            or record["btc_address"] in seen_addresses
            or record["descriptor_index"] in seen_indices
            or (
                not legacy
                and (
                    record["commitment_blind"] in seen_blinds
                    or record["consensus_allocation_id"] in seen_allocation_ids
                )
            )
        ):
    ...
    if not legacy:
        by_request = {record["request_id"]: record for record in ledger["allocations"]}
        registered, deposits, funded_reserved, minted = set(), set(), set(), set()
        for event in ledger["events"]:
            if (
                not isinstance(event, dict)
                or set(event) != {"action", "at", "request_id"}
                or event.get("action")
                not in (
                    "registered",
                    "deposit_observed",
            ...
            if event["action"] == "registered":
                if event["request_id"] in registered or event["at"] != record["registered_at"]:
                ...
                if event["request_id"] in funded_reserved or record["deposit_observed_at"] is None:
                ...
                if event["request_id"] in minted or record["funded_reserved_at"] is None:
        ...
        if (
            registered != set(by_request)
            or deposits != expected_deposits
            or funded_reserved != expected_funded_reserved
            or minted != expected_minted
        ):
    ...
    if policy_advanced:
        ...
        ledger["public_descriptor_range_end"] = c1_policy["public_descriptor_range"][1]
...
def _activate_allocation_policy_update(cfg, paths, ledger, policy_sha256):
    ...
    _checkpoint_allocation_ledger(cfg, ledger, policy_sha256, "store")
    return True


def _c1_capacity_reason(
    ledger, candidate, policy, now, issuer_headroom_sats=None, issuer_reserved_sats=None
):
    ...
    if (
        sum(row["admitted_at"] // policy["epoch_seconds"] == epoch for row in ledger["allocations"])
        >= policy["global_allocations_per_epoch"]
    ):
        return "C1 epoch allocation cap reached"
    if (
        sum(row["admitted_at"] // 86_400 == day for row in ledger["allocations"])
        >= policy["global_allocations_per_day"]
    ):
        return "C1 daily allocation cap reached"
    live = [
        row
        for row in ledger["allocations"]
        if row.get("deposit_observed_at") is not None or now < row["expires_at"]
    ]
    if issuer_headroom_sats is not None:
        if (
            type(issuer_headroom_sats) is not int
            or issuer_headroom_sats < 0
            or type(issuer_reserved_sats) is not int
            or issuer_reserved_sats < 0
        ):
        ...
        local_reserved = sum(row["amount_sats"] for row in live if row.get("minted_at") is None)
        offchain_pending = max(0, local_reserved - issuer_reserved_sats)
        if (
            offchain_pending > issuer_headroom_sats
            or candidate["amount_sats"] > issuer_headroom_sats - offchain_pending
        ):
            return "active C1 allocations exceed fresh issuer mint headroom"
    principal = sum(
        row["amount_sats"] for row in live if row["principal_hash"] == candidate["principal_hash"]
    )
    ...
    destination = sum(
        row["amount_sats"] for row in live if row["veld_address"] == candidate["veld_address"]
    )
    if principal + candidate["amount_sats"] > policy["principal_lifetime_sats"]:
        return "principal lifetime sat ceiling reached"
    if destination + candidate["amount_sats"] > policy["destination_lifetime_sats"]:
...
def _c1_storage_allows(row_count, byte_count, policy, *, new_admission):
    ...
    if new_admission:
        ...
        return (
            row_count < policy["journal_max_rows"] * percent // 100
            and byte_count < policy["journal_max_bytes"] * percent // 100
        )
    return row_count < policy["journal_max_rows"] and byte_count < policy["journal_max_bytes"]
...
def _checkpoint_allocation_ledger(cfg, ledger, policy_sha256, action):
    ...
    request = json.dumps(
        {
            ...
            "last_descriptor_index": (
                max(row["descriptor_index"] for row in ledger["allocations"])
                if ledger["allocations"]
                else 999
            ),
        },
        sort_keys=True,
    ...
    if (
        ...
        or answer.get("action") != action
        or answer.get("capacity_policy_sha256") != policy_sha256
        or answer.get("ledger_sha256") != digest
        or answer.get("last_consensus_sequence") != ledger["last_consensus_sequence"]
        or type(answer.get("monotonic_sequence")) is not int
        or answer["monotonic_sequence"] < 1
    ):
        refuse("allocation checkpoint authority refused or returned an inexact ack")
    return answer


def handle_register_allocation(
    req,
    cfg,
    paths,
    ledger,
    ...
    if (
        ...
        or set(req) != wrapper_keys
        or req.get("version") != version
        or req.get("action") not in actions
        or not isinstance(req.get("allocation"), dict)
        or set(req["allocation"]) != request_fields
        or (not legacy and req.get("capacity_policy_sha256") != policy_sha256)
    ):
    ...
    if existing is not None:
        if any(existing[field] != candidate[field] for field in request_fields):
        ...
        if not legacy and not verify_only:
            ...
            active_unminted = sum(
                row["amount_sats"]
                for row in ledger["allocations"]
                if (row.get("deposit_observed_at") is not None or observed < row["expires_at"])
                and row.get("minted_at") is None
            )
            ...
            offchain_pending = max(0, active_unminted - issuer_reserved_sats)
        ...
        if not legacy:
            if (
                candidate["admitted_at"] > observed + 120
                or observed - candidate["admitted_at"] > 900
            ):
        ...
        if any(
            record["descriptor_index"] == candidate["descriptor_index"]
            or record["btc_address"] == candidate["btc_address"]
            or (
                not legacy
                and (
                    record["commitment_blind"] == candidate["commitment_blind"]
                    or record["consensus_allocation_id"] == candidate["consensus_allocation_id"]
                )
            )
            for record in ledger["allocations"]
        ):
        ...
        if legacy:
            expected = ledger["initial_descriptor_index"] + len(ledger["allocations"])
            if candidate["descriptor_index"] != expected:
                refuse("wrap allocation registration is not the next contiguous index")
        elif ledger["last_consensus_sequence"] == C1_UINT64_MAX or candidate[
            "consensus_allocation_id"
        ] != "%032x" % (ledger["last_consensus_sequence"] + 1):
        ...
        if preflight:
            ...
            if not legacy:
                ...
                ledger["last_consensus_sequence"] = int(candidate["consensus_allocation_id"], 16)
...
def _verify_allocation_deposit(
    claim,
    recipient,
    sats,
    outpoint,
    ...
    claim_fields = {"request_id", "btc_address", "script_pubkey", "deposit_outpoint"}
    ...
    if (
        ...
        or not isinstance(address, str)
        or not isinstance(script, str)
        or claim.get("deposit_outpoint") != outpoint
    ...
    if c1_policy is not None:
        if (
            claim.get("descriptor_index") is None
            or claim.get("capacity_policy_sha256") != allocation_policy_sha256
            or claim.get("public_descriptor_range_start") != c1_policy["public_descriptor_range"][0]
            or claim.get("public_descriptor_range_end") != c1_policy["public_descriptor_range"][1]
            or not isinstance(claim.get("consensus_allocation_id"), str)
            or not ALLOCATION_REQUEST_RE.fullmatch(claim["consensus_allocation_id"])
            or int(claim["consensus_allocation_id"], 16) == 0
            or not isinstance(claim.get("commitment_blind"), str)
            or not re.fullmatch(r"[0-9a-f]{64}", claim["commitment_blind"])
            or int(claim["commitment_blind"], 16) == 0
            or not _c1_public_index_allowed(claim.get("descriptor_index"), c1_policy)
        ):
            refuse("mint allocation claim differs from signed C1 policy")
    record = next(
        (entry for entry in allocations["allocations"] if entry["request_id"] == request_id), None
    )
    if (
        record is None
        or record["btc_address"] != address
        or record["script_pubkey"] != script
        or record["veld_address"] != recipient
        or record["amount_sats"] != sats
        or (
            c1_policy is not None
            and (
                record["descriptor_index"] != claim["descriptor_index"]
                or record["consensus_allocation_id"] != claim["consensus_allocation_id"]
                or record["commitment_blind"] != claim["commitment_blind"]
            )
        )
    ):
    ...
    if (
        ...
        and record.get("deposit_observed_at") is None
        and observed_now >= record["expires_at"]
    ):
    ...
    if (
        ...
        or not sol.HASH256_RE.fullmatch(before)
        or after != before
        ...
        or blocks < spv_height
        or headers != blocks
        or chain.get("bestblockhash") != before
        or not isinstance(best_header, dict)
        or best_header.get("hash") != before
        or type(header_time) is not int
        or header_time > observed_now + C5_TIP_HARD_SECONDS
    ...
    if (
        ...
        or confirmations < btc.confirmations
        or not isinstance(script_info, dict)
        or script_info.get("hex") != record["script_pubkey"]
        or _btc_to_sats(output.get("value")) != record["amount_sats"]
    ):
...
def _verify_c1_funded_reservation(record, outpoint, beat, rpc):
    ...
    if record.get("funded_reserved_at") is not None and record.get("funding_outpoint") != outpoint:
    ...
    if (
        ...
        or supply_before.get("supply_sats") != beat.get("supply_sats")
        or supply_before.get("tip") != beat.get("tip")
        or supply_before.get("tip_hash") != beat.get("tip_hash")
        or not isinstance(status, dict)
        or status.get("allocation_id") != allocation_id_value
        or status.get("found") is not True
        ...
        or status.get("funded") is not True
        or status.get("funding_outpoint") != outpoint
        or status.get("recipient") != record["veld_address"]
        or status.get("amount_sats") != record["amount_sats"]
        or status.get("allocation_commitment") != expected_commitment
        or status.get("tip") != beat.get("tip")
        or not isinstance(mint_status, dict)
        or mint_status.get("outpoint") != outpoint
        or mint_status.get("consumed") is not True
        or mint_status.get("minted") is not False
        or mint_status.get("proof_version") != "MNP1"
        or mint_status.get("accepted_effect_kind") != "C1_FUND"
        or mint_status.get("c1_allocation_id") != allocation_id_value
        or mint_status.get("tip") != beat.get("tip")
        or mint_status.get("tip_hash") != beat.get("tip_hash")
    ):
    ...
    if (
        type(height) is not int
        or not 0 <= height <= beat["tip"]
        or not isinstance(block_hash, str)
        or not sol.HASH256_RE.fullmatch(block_hash)
        or not isinstance(mint_status.get("consumer_txid"), str)
        or not sol.HASH256_RE.fullmatch(mint_status["consumer_txid"])
        or type(mint_status.get("consumer_tx_index")) is not int
        or mint_status["consumer_tx_index"] < 0
        or type(mint_status.get("consumer_marker_vout")) is not int
        or mint_status["consumer_marker_vout"] < 0
    ):
...
def _verify_forced_command_peg_policy(rpc, cfg, binding):
    ...
    try:
        ...
        if peg.get("peg_unlocked") is not True or peg.get("completion_live") is not True:
            raise ValueError(
                "canonical launch/liveness gate does not permit exact C1 mint completion"
            )
        compiled_k = sol._wire_uint(peg.get("spv_k_btc"), "compiled spv_k_btc", positive=True)
        btc_cfg = cfg.get("btc")
        configured_k = btc_cfg.get("confirmations") if isinstance(btc_cfg, dict) else None
        if type(configured_k) is not int or configured_k != compiled_k:
            raise ValueError("witness Bitcoin confirmations differ from compiled spv_k_btc")
        relay = rpc.call("getbtcheaderinfo")
        if (
            not isinstance(relay, dict)
            or relay.get("spv_active") is not True
            or type(relay.get("best_height")) is not int
            or relay["best_height"] < 1
            or relay.get("k_btc") != compiled_k
        ):
...
def _verify_public_allocation_backend(cfg, c1_policy, now=None, rpc=None):
    ...
    if (
        type(peg.get("issuer_max_per_mint_sats")) is not int
        or peg["issuer_max_per_mint_sats"] < c1_policy["minimum_allocation_sats"]
        or type(peg.get("issuer_static_custody_cap_sats")) is not int
        or peg["issuer_static_custody_cap_sats"] != c1_policy["consensus_custody_ceiling_sats"]
        or type(peg.get("issuer_effective_custody_cap_sats")) is not int
        or peg["issuer_effective_custody_cap_sats"] < 0
        or type(peg.get("issuer_mint_headroom_sats")) is not int
        or peg["issuer_mint_headroom_sats"] < 0
        or type(peg.get("issuer_reserved_sats")) is not int
        or peg["issuer_reserved_sats"] < 0
        or type(peg.get("supply_sats")) is not int
        or peg["supply_sats"] < 0
    ...
    if (
        peg["supply_sats"] > peg["issuer_effective_custody_cap_sats"]
        or peg["issuer_reserved_sats"]
    ...
    expected_headroom = (
        peg["issuer_effective_custody_cap_sats"] - peg["supply_sats"] - peg["issuer_reserved_sats"]
    )
    if (
        not isinstance(supply_before, dict)
        or supply_before != supply_after
        or set(supply_before) != {"supply_sats", "tip", "tip_hash"}
        or supply_before.get("supply_sats") != peg["supply_sats"]
        or supply_before.get("tip") != peg["tip"]
        or not isinstance(supply_before.get("tip_hash"), str)
        or not sol.HASH256_RE.fullmatch(supply_before["tip_hash"])
        or any(peg.get(name) != peg_after.get(name) for name in capacity_fields)
        or peg["issuer_mint_headroom_sats"] != expected_headroom
        or not isinstance(before_veld_tip, str)
        or not sol.HASH256_RE.fullmatch(before_veld_tip)
        or before_veld_tip != supply_before["tip_hash"]
        or after_veld_tip != before_veld_tip
    ):
    ...
    if (
        ...
        or chain.get("chain") != "main"
        or chain.get("initialblockdownload") is not False
        or type(chain.get("blocks")) is not int
        or chain.get("headers") != chain.get("blocks")
        or chain.get("bestblockhash") != before
        or not isinstance(header, dict)
        or header.get("hash") != before
        or type(header.get("time")) is not int
    ):
...
def _canonical_confirmation(entry, beat, rpc):
    ...
    if not txid:
        return None
    receipt = entry.get("receipt")
    c1 = isinstance(receipt, dict) and receipt.get("v") == sol.C1_RESERVATION_VERSION
    if c1:
        ...
        if (
            ...
            or len(status["proof_hex"]) % 2 != 0
            or not isinstance(status.get("root"), str)
            or not sol.HASH256_RE.fullmatch(status["root"])
            or type(status.get("count")) is not int
            or not 0 <= status["count"] <= C1_UINT64_MAX
            or type(status.get("tip")) is not int
            or status.get("tip") != beat["tip"]
        ...
        if status["consumed"] is False:
            if status["minted"] is not False or any(
                status.get(name) is not None
                for name in (
                    "accepted_txid",
            ...
            return None
        if (
            status.get("c1_allocation_id") != receipt.get("allocation_request_id")
            or not isinstance(status.get("accepted_txid"), str)
            or not sol.HASH256_RE.fullmatch(status["accepted_txid"])
            or type(status.get("accepted_block_height")) is not int
            or not 0 <= status["accepted_block_height"] <= beat["tip"]
            or not isinstance(status.get("accepted_block_hash"), str)
            or not sol.HASH256_RE.fullmatch(status["accepted_block_hash"])
            or type(status.get("accepted_tx_index")) is not int
            or status["accepted_tx_index"] < 0
            or type(status.get("accepted_marker_vout")) is not int
            or status["accepted_marker_vout"] < 0
            or status.get("consumer_txid") is None
            or type(status.get("consumer_block_height")) is not int
            or not 0 <= status["consumer_block_height"] <= beat["tip"]
            or not isinstance(status.get("consumer_block_hash"), str)
            or not sol.HASH256_RE.fullmatch(status["consumer_block_hash"])
            or type(status.get("consumer_tx_index")) is not int
            or status["consumer_tx_index"] < 0
            or type(status.get("consumer_marker_vout")) is not int
            or status["consumer_marker_vout"] < 0
        ):
            refuse("C1 deposit was consumed by a different/invalid C1F1; HOLD")
        ...
        if status["minted"] is False:
            if (
                status.get("accepted_effect_kind") != "C1_FUND"
                or status.get("accepted_txid") != status.get("consumer_txid")
                or status.get("accepted_block_height") != status.get("consumer_block_height")
                or status.get("accepted_block_hash") != status.get("consumer_block_hash")
                or status.get("accepted_tx_index") != status.get("consumer_tx_index")
                or status.get("accepted_marker_vout") != status.get("consumer_marker_vout")
                or any(
                    status.get(name) is not None
                    for name in (
                        "credit_txid",
        ...
        if (
            ...
            or status.get("credit_txid") != txid
            or status.get("accepted_txid") != status.get("credit_txid")
            or status.get("accepted_block_height") != status.get("credit_block_height")
            or status.get("accepted_block_hash") != status.get("credit_block_hash")
            or status.get("accepted_tx_index") != status.get("credit_tx_index")
            or status.get("accepted_marker_vout") != status.get("credit_marker_vout")
            or type(status.get("credit_block_height")) is not int
            or status["credit_block_height"] < 0
            or not isinstance(status.get("credit_block_hash"), str)
            or not sol.HASH256_RE.fullmatch(status["credit_block_hash"])
            or type(status.get("credit_tx_index")) is not int
            or status["credit_tx_index"] < 0
            or type(status.get("credit_marker_vout")) is not int
            or status["credit_marker_vout"] < 0
        ):
        ...
        try:
            if (
                rpc.call("getblockhash", [status["consumer_block_height"]])
                != status["consumer_block_hash"]
            ):
...
def _checkpoint_allocation_effect(
    cfg, paths, allocation_ledger, policy_sha256, receipt, present, now, c1_policy=None
):
    ...
    record = next(
        (
            row
            for row in allocation_ledger["allocations"]
            if row.get("consensus_allocation_id") == consensus_allocation_id
        ),
        None,
    )
    if (
        record is None
        or record.get("funded_reserved_at") is None
        or record.get("amount_sats") != receipt.get("sats")
        or record.get("veld_address") != receipt.get("recipient")
        or record.get("btc_address") != receipt.get("allocation_btc_address")
        or record.get("script_pubkey") != receipt.get("allocation_script_pubkey")
    ):
    ...
    allocation_ledger["events"].append(
        {
            "action": ("mint_effect_observed" if present else "mint_effect_reverted"),
    ...
    raw = (json.dumps(allocation_ledger, sort_keys=True, separators=(",", ":")) + "\n").encode(
        "utf-8"
    )
    policy = c1_policy
    if policy is None:
    ...
    if not _c1_storage_allows(
        len(allocation_ledger["allocations"]) + len(allocation_ledger["events"]),
        len(raw),
        policy,
        new_admission=False,
    ):
    ...
    _checkpoint_allocation_ledger(cfg, allocation_ledger, policy_sha256, "store")
    return True


def _checkpoint_funded_reservation(
    cfg, paths, allocation_ledger, policy_sha256, record, outpoint, present, now, c1_policy=None
):
    ...
    if (
        not isinstance(record, dict)
        or not any(row is record for row in allocation_ledger.get("allocations", []))
        or not isinstance(outpoint, str)
        or not BTC_OUTPOINT_RE.fullmatch(outpoint)
    ...
    allocation_ledger["events"].append(
        {
            "action": ("funded_reservation_observed" if present else "funded_reservation_reverted"),
            "at": now,
            "request_id": record["request_id"],
        }
    )
...
def _reconcile_c1_funded_reservation(
    cfg, paths, allocation_ledger, policy_sha256, record, beat, rpc, now, c1_policy=None
):
    ...
    if record.get("minted_at") is not None:
        ...
        return False
    if not isinstance(old_outpoint, str) or not BTC_OUTPOINT_RE.fullmatch(old_outpoint):
        refuse("C1 funded allocation lacks its persisted outpoint")
    try:
        supply_before = rpc.call("getbtcveldsupply")
        status = rpc.call("getbtcveldc1reservation", [record["consensus_allocation_id"]])
    ...
    if (
        ...
        or supply_before.get("tip_hash") != beat.get("tip_hash")
        or not isinstance(status, dict)
        or status.get("allocation_id") != record["consensus_allocation_id"]
        or status.get("found") is not True
        or status.get("active") is not True
        or status.get("retired") is not False
        or status.get("sequence") != sequence
        or type(status.get("last_sequence")) is not int
        or status["last_sequence"] < sequence
        or status.get("recipient") != record["veld_address"]
        or status.get("amount_sats") != record["amount_sats"]
        or status.get("allocation_commitment") != expected_commitment
        or status.get("exposed") is not True
        or status.get("exposure_canonical_depth_reached") is not True
        or type(status.get("exposure_confirmations")) is not int
        or status["exposure_confirmations"] < MAX_REORG_DEPTH + 1
        or status.get("required_confirmations") != MAX_REORG_DEPTH + 1
        or status.get("canonical_depth_reached") is not True
        or status.get("tip") != beat.get("tip")
        or not isinstance(mint_status, dict)
        or mint_status.get("outpoint") != old_outpoint
        or mint_status.get("proof_version") != "MNP1"
        or not isinstance(mint_status.get("proof_hex"), str)
        or not re.fullmatch(r"[0-9a-f]+", mint_status["proof_hex"])
        or len(mint_status["proof_hex"]) % 2 != 0
        or not isinstance(mint_status.get("root"), str)
        or not sol.HASH256_RE.fullmatch(mint_status["root"])
        or type(mint_status.get("count")) is not int
        or not 0 <= mint_status["count"] <= C1_UINT64_MAX
        or mint_status.get("tip") != beat.get("tip")
        or mint_status.get("tip_hash") != beat.get("tip_hash")
    ):
    ...
    if status.get("funded") is True:
        ...
        if (
            status.get("funding_outpoint") != old_outpoint
            or type(status.get("funding_confirmations")) is not int
            or status["funding_confirmations"] < 1
            or type(status.get("funding_canonical_depth_reached")) is not bool
            or mint_status.get("consumed") is not True
            or mint_status.get("minted") is not False
            or mint_status.get("accepted_effect_kind") != "C1_FUND"
            or mint_status.get("c1_allocation_id") != record["consensus_allocation_id"]
            or any(
                not isinstance(mint_status.get(name), str)
                or not sol.HASH256_RE.fullmatch(mint_status[name])
                for name in locator_names
            )
            or mint_status.get("accepted_txid") != mint_status.get("consumer_txid")
            or mint_status.get("accepted_block_height") != mint_status.get("consumer_block_height")
            or mint_status.get("accepted_block_hash") != mint_status.get("consumer_block_hash")
            or mint_status.get("accepted_tx_index") != mint_status.get("consumer_tx_index")
            or mint_status.get("accepted_marker_vout") != mint_status.get("consumer_marker_vout")
            or any(
                mint_status.get(name) is not None
                for name in (
                    "credit_txid",
                    "credit_block_height",
                    "credit_block_hash",
        ...
        try:
            if rpc.call("getblockhash", [height]) != mint_status["consumer_block_hash"]:
    ...
    if (
        status.get("funded") is not False
        or status.get("funding_outpoint") is not None
        or status.get("funding_confirmations") is not None
        or status.get("funding_canonical_depth_reached") is not False
        or mint_status.get("consumed") is not False
        or mint_status.get("minted") is not False
        or any(mint_status.get(name) is not None for name in null_locators)
    ):
        refuse("C1 funding rollback is not an exact unconsumed transition")
    return _checkpoint_funded_reservation(
        cfg, paths, allocation_ledger, policy_sha256, record, old_outpoint, False, now, c1_policy
    )
...
def _snapshot_text(ledger, policy):
    ...
    text = json.dumps(_snapshot_object(ledger), sort_keys=True, separators=(",", ":")) + "\n"
...
def _archive_terminal(entry, entry_sha256, policy):
    ...
    minimum_retention_until = observed_at + TERMINAL_ARCHIVE_RETENTION_SECONDS
    request = {
        "version": TERMINAL_ARCHIVE_VERSION,
    ...
    run = run_bounded_subprocess(
        command,
        input_text=_canonical_json_bytes(request).decode("utf-8") + "\n",
        timeout=60,
        stdout_max=64 * 1024,
        stderr_max=64 * 1024,
    ...
    if (
        ...
        or answer.get("version") != TERMINAL_ARCHIVE_VERSION
        or answer.get("stored") is not True
        or answer.get("durable") is not True
        or answer.get("entry_sha256") != entry_sha256
        or answer.get("readback_sha256") != entry_sha256
        or not isinstance(answer.get("archive_id"), str)
        or not 1 <= len(answer["archive_id"]) <= 512
        or answer.get("object_lock_mode") != "COMPLIANCE"
        or type(answer.get("retention_until")) is not int
        or answer["retention_until"] < minimum_retention_until
    ):
...
def _make_tombstone(entry, beat, sequence, previous, archive_sha256):
    ...
    identity_fields = C1_TOMBSTONE_IDENTITY_FIELDS if c1 else TOMBSTONE_IDENTITY_FIELDS
    record = {
...
def _compact_terminal(paths, ledger, entry, beat, cfg):
    ...
    c1 = receipt.get("v") == sol.C1_RESERVATION_VERSION
    if (
        entry.get("status") != "final"
        or type(entry.get("block_height")) is not int
        or entry["block_height"] < 0
        or not isinstance(entry.get("block_hash"), str)
        or not sol.HASH256_RE.fullmatch(entry["block_hash"])
        or type(beat.get("tip")) is not int
        or beat["tip"] < entry["block_height"]
        or beat["tip"] - entry["block_height"] + 1 <= MAX_REORG_DEPTH
        or not isinstance(beat.get("tip_hash"), str)
        or not sol.HASH256_RE.fullmatch(beat["tip_hash"])
    ):
    ...
    prospective["reservations"] = [row for row in ledger["reservations"] if row is not entry]
    ...
    record = _make_tombstone(
        entry, beat, ledger["terminal_count"] + 1, ledger["terminal_head_sha256"], archive_sha
    )
    line = _canonical_json_bytes(record) + b"\n"
    if (
        ledger["terminal_count"] >= policy["max_terminal_rows"]
        or ledger["_terminal_size"] + len(line) > policy["max_terminal_bytes"]
    ):
    ...
    for entry in list(ledger["reservations"]):
        ...
        if c1 and confirmation is not None:
            if (
                cfg is None
                or paths is None
                or allocation_ledger is None
                or not isinstance(allocation_policy_sha256, str)
            ):
            ...
            and allocation_ledger is not None
            and any(
                row.get("consensus_allocation_id") == receipt.get("allocation_request_id")
                and row.get("minted_at") is not None
...
def _sign_receipt(core, cfg):
    ...
    if passfile:
        ...
        passphrase = (
            read_bounded_secret_file(passfile, 4096, "witness receipt passphrase")
            .decode("utf-8")
            .strip()
        )
    ...
    with tempfile.TemporaryDirectory(prefix="veld-reservation-sign-") as td:
        ...
        run = run_bounded_subprocess(
            ...
            stderr_max=64 * 1024,
            env=environment,
            description="witness receipt signer",
        )
        if run.returncode != 0:
            refuse("reservation receipt signing failed")
        sig = read_bounded_regular_file(signature, 3309, "witness receipt signature")
    if len(sig) != 3309:
...
def _validate_request(req, cfg, beat):
    ...
    if (
        req.get("beat_seq") != beat["seq"]
        or req.get("tip") != beat["tip"]
        or req.get("tip_hash") != beat["tip_hash"]
    ):
...
def _decode_mint_template(unsigned_hex, cfg):
    ...
    for field, minimum, maximum in (("total_out_sats", 0, (1 << 63) - 1), ("num_inputs", 1, 65535)):
        if field in decoded:
            value = decoded[field]
            if type(value) is not int or not (minimum <= value <= maximum):
...
def _derive_unsigned_mint(req, cfg):
    ...
    if cfg.get("production") is True:
        ...
        if (
            match.group(1) != allocation.get("consensus_allocation_id")
            or match.group(2) != allocation.get("script_pubkey")
            or match.group(3) != allocation.get("commitment_blind")
        ):
...
def handle_reserve(
    ...
    if cfg.get("production") is True:
        ...
        if c1_policy is not None:
            ...
            if changed:
                raw = (
                    json.dumps(allocation_ledger, sort_keys=True, separators=(",", ":")) + "\n"
                ).encode()
                if not _c1_storage_allows(
                    len(allocation_ledger["allocations"]) + len(allocation_ledger["events"]),
                    len(raw),
            ...
            _verify_c1_funded_reservation(verified_allocation, outpoint, beat, rpc)
        ...
        if getattr(btc, "last_c5_phase", None) == "RECOVERY_ONLY" and prior_reservation is None:
            refuse(
                "RECOVERY_ONLY: new mint reservations are halted; "
    ...
    allocation_request_id = (
        ...
        else (verified_allocation["request_id"] if verified_allocation is not None else None)
    )
    rid = sol.reservation_id(
        issuer_id,
        req["request_id"],
        req["unsigned_tx_sha256"],
        sats,
        recipient,
        outpoint,
        allocation_request_id,
    ...
    existing = next(
        (x for x in ledger["reservations"] if x["receipt"]["reservation_id"] == rid), None
    )
    terminal = next((x for x in ledger.get("_terminals", []) if x["reservation_id"] == rid), None)
    if terminal is not None:
        refuse("reservation is already terminal; recover/replay its exact signed bytes")
    same_request = next(
        (x for x in ledger["reservations"] if x["receipt"]["request_id"] == req["request_id"]), None
    )
    terminal_request = next(
        (x for x in ledger.get("_terminals", []) if x["request_id"] == req["request_id"]), None
    )
    ...
    if verified_allocation is not None:
        same_outpoint = next(
            (x for x in ledger["reservations"] if x["receipt"].get("deposit_outpoint") == outpoint),
            None,
        )
        terminal_outpoint = next(
            (x for x in ledger.get("_terminals", []) if x.get("deposit_outpoint") == outpoint), None
        )
        ...
        same_allocation = next(
            (
                x
                for x in ledger["reservations"]
                if x["receipt"].get("allocation_request_id") == allocation_request_id
            ),
            None,
        )
        terminal_allocation = next(
            (
                x
                for x in ledger.get("_terminals", [])
                if x.get("allocation_request_id") == allocation_request_id
            ),
            None,
        )
        if same_allocation is not None and same_allocation["receipt"]["reservation_id"] != rid:
    ...
    core = {
        "v": (sol.C1_RESERVATION_VERSION if c1_receipt else sol.RESERVATION_VERSION),
        ...
        "allocation_descriptor_index": (
            verified_allocation["descriptor_index"] if verified_allocation is not None else None
        ),
        "allocation_btc_address": (
            verified_allocation["btc_address"] if verified_allocation is not None else None
        ),
        "allocation_script_pubkey": (
            verified_allocation["script_pubkey"] if verified_allocation is not None else None
        ),
    ...
    if c1_receipt:
        if (
            ...
            or not sol.HASH256_RE.fullmatch(allocation_policy_sha256)
            or not _c1_public_index_allowed(verified_allocation["descriptor_index"], c1_policy)
        ):
            refuse("C1 reservation policy/index binding is unavailable")
        core.update(
            {
                "allocation_capacity_policy_sha256": allocation_policy_sha256,
                "allocation_public_descriptor_range_start": c1_policy["public_descriptor_range"][0],
                "allocation_public_descriptor_range_end": c1_policy["public_descriptor_range"][1],
            }
        )
...
def handle_commit(req, cfg, paths, ledger):
    ...
    entry = next((x for x in ledger["reservations"] if x["receipt"]["reservation_id"] == rid), None)
    terminal = next((x for x in ledger.get("_terminals", []) if x["reservation_id"] == rid), None)
    ...
    if unsigned_sha256 != receipt["unsigned_tx_sha256"] or unsigned_sha256 != receipt["request_id"]:
    ...
    if (
        decoded.get("from") != receipt["issuer_id"]
        or decoded.get("to") != receipt["recipient"]
        or decoded.get("sats") != receipt["sats"]
    ):
    ...
    if receipt.get("allocation_verified") is True:
        ...
        if (
            match is None
            or match.group(1) != receipt.get("allocation_request_id")
            or match.group(2) != receipt.get("allocation_script_pubkey")
            or match.group(4) != receipt.get("deposit_outpoint")
        ):
            refuse("committed mint outpoint differs from reservation receipt")
    signed_digest = hashlib.sha256(signed_bytes).hexdigest()
    if terminal is not None:
        if terminal["txid"] != txid or terminal["signed_tx_sha256"] != signed_digest:
            refuse("terminal reservation is bound to different exact signed bytes")
        return {"reservation_id": rid, "txid": txid, "committed": True, "terminal": True}
...
def main():
    ...
    try:
        ...
        cfg = strict_json_loads(_read_text_nofollow(config_path), "witness configuration")
    except Exception as e:
    ...
    with exclusive_witness_state_lock(paths):
        ...
        if req.get("action") == "commit":
            ...
            allocation_ledger = _load_allocation_ledger(
                paths["allocations"], cfg, binding, c1_policy, allocation_policy_sha256
            )
            if not _activate_allocation_policy_update(
                cfg, paths, allocation_ledger, allocation_policy_sha256
            ):
                _checkpoint_allocation_ledger(
                    cfg, allocation_ledger, allocation_policy_sha256, "verify"
                )
...
def allocation_main(initialize=False):
    ...
    with exclusive_witness_state_lock(paths):
        ...
        if initialize:
            ...
            if not _activate_allocation_policy_update(cfg, paths, ledger, policy_sha256):
                _checkpoint_allocation_ledger(cfg, ledger, policy_sha256, "verify")
            backend = None
            if req.get("action") in ("preflight_allocation", "register_allocation"):
                observed = int(time.time())
                beat = _load_beat(paths["beat"], observed)
                if not _tip_matches(beat, rpc):
                    refuse("fresh beat tip is not canonical before allocation reconciliation")
                backend = _verify_public_allocation_backend(cfg, c1_policy, now=observed, rpc=rpc)
                if (
                    backend["veld_tip"] != beat.get("tip")
                    or backend["veld_tip_hash"] != beat.get("tip_hash")
                    or backend["supply_sats"] != beat.get("supply_sats")
                ):
                    refuse("signed beat and allocation capacity tuple differ")
                reservation_policy = _reservation_ledger_policy(cfg, production=True)
                ...
                _persist_ledger(paths, reservations, reservation_policy)
                if time.time() > float(beat["expires_at"]) or not _tip_matches(beat, rpc):
            ...
            answer = handle_register_allocation(
                ...
                c1_policy,
                policy_sha256,
                issuer_headroom_sats=(
                    backend["issuer_headroom_sats"] if backend is not None else None
                ),
                issuer_reserved_sats=(
                    backend["issuer_reserved_sats"] if backend is not None else None
                ),
            )
            if backend is not None and (
                time.time() > float(beat["expires_at"]) or not _tip_matches(beat, rpc)
            ):
