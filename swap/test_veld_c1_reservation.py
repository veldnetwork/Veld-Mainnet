...
class WrapLifecycleTests(unittest.TestCase):
    ...
    def test_confirmation_wait_consumes_none_of_seven_day_send_window(self):
        ...
        record["state"] = "issued"
        status = {
            "created_height": 1, "exposed_height": 102,
            "confirmations": 101, "tip": 202, "funded": False,
        ...
        result = journal._lifecycle_result(
            record, self.response(record), record["expires_at"] + 1)
        ...
        for tip, allowed in ((3_359, True), (3_360, True),
                             (3_361, False), (3_461, False),
                             (3_561, False)):
            with self.subTest(tip=tip):
                boundary = dict(status, tip=tip)
                outcome = self.journal(
                    lambda _record, value=boundary: value)._lifecycle_result(
                        record, self.response(record), record["expires_at"] + 2)
                self.assertIs(outcome["send_allowed"], allowed)
                self.assertEqual(
                    outcome["lifecycle"],
                    "UNFUNDED" if allowed else "RECOVERY_ONLY")
    ...
    def test_pre_exposure_intent_can_expire_but_exposed_pending_cannot(self):
        ...
        pending_unexposed = wrapd.PendingConsensusReservation({
            "found": False, "active": False, "retired": True,
            "exposed": False,
            "confirmations": 50, "required_confirmations": 101})
        cancelled = self.journal(lambda _record: (_ for _ in ()).throw(
            pending_unexposed))
        cancelled.events = [{"action": "expired",
                             "request_id": record["request_id"]}]
        with self.assertRaises(wrapd.ExpiredAllocation):
            cancelled._lifecycle_result(
        ...
        with self.assertRaises(wrapd.PendingConsensusReservation):
            self.journal(lambda _record: (_ for _ in ()).throw(
                pending_exposed))._lifecycle_result(
                    record, self.response(record), record["expires_at"])

    def test_pending_public_status_never_contains_address_or_script(self):
        record = allocation(1)
        completed = subprocess.CompletedProcess(
            [], 75, stdout=json.dumps({
                "version": 3,
                "allocation_id": record["consensus_allocation_id"],
                "found": False, "active": False, "retired": False,
                "last_sequence": 0, "exposed": False, "funded": False,
                "funding_outpoint": None, "confirmations": 0,
        ...
        node_status = {
            "allocation_id": record["consensus_allocation_id"],
            "found": False, "active": False, "retired": False,
            "exposed": False, "tip": 1, "confirmations": 0,
            "funded": False, "funding_outpoint": None,
            "required_confirmations": 101,
            "canonical_depth_reached": False,
        }
        with mock.patch.object(wrapd, "PRODUCTION", True), \
                mock.patch.object(wrapd, "CONSENSUS_RESERVATION_COMMAND",
                                  ("/usr/bin/coordinator",)), \
...
class CoordinatorDurabilityTests(unittest.TestCase):
    ...
    def test_lost_reply_and_restart_replay_only_exact_signed_bytes(self):
        signed = "00"
        stage = {
            "unsigned_tx_hex": "00", "unsigned_sha256": hashlib.sha256(
                b"\x00").hexdigest(), "signed_tx_hex": signed,
            "txid": coordinator.txid(signed), "broadcast_attempts": 0,
        ...
        record = {"kind": "active", "allocation_sha256": "aa" * 32,
                  "request_id": REQUEST_ID, "terminal_observed_tip": None,
                  "RESERVE": stage, "EXPOSE": None, "CANCEL": None,
                  "FUND": []}
        cfg = {"state_file": "/unused",
               "reservation_signer_command": ("/usr/bin/signer",)}
        ...
        with mock.patch.object(coordinator, "persist"), \
            ...
            self.assertIs(coordinator.prepare_stage(
                mock.Mock(), cfg, allocation(), "RESERVE", record, state), stage)
            signer.assert_not_called()
            rpc = mock.Mock()
            rpc.call.side_effect = [TimeoutError("lost reply"),
        ...
        self.assertEqual([call.args[1] for call in rpc.call.call_args_list],
                         [[signed], [signed]])
    ...
    def test_signer_mutation_and_broadcast_txid_mismatch_are_fatal(self):
        ...
        cfg = {"state_file": "/unused",
               "reservation_signer_command": ("/usr/bin/signer",)}
        state = {"version": 3, "sequence": 1, "records": {}}
        answer = {"version": 2, "action": "signed_c1_reservation",
                  "phase": "RESERVE", "request_id": REQUEST_ID,
                  "allocation_id": "0" * 31 + "1",
                  "signed_tx_hex": "00", "txid": coordinator.txid("00")}
        completed = subprocess.CompletedProcess(
            [], 0, stdout=json.dumps(answer), stderr="")
        with mock.patch.object(coordinator, "persist"), \
                mock.patch.object(coordinator, "run_bounded_subprocess",
                                  return_value=completed), \
                mock.patch.object(
                    coordinator.solvency, "unsigned_template_from_signed_hex",
                    return_value="ff"):
            with self.assertRaisesRegex(ValueError, "does not bind"):
                coordinator.prepare_stage(
                    mock.Mock(), cfg, allocation(), "RESERVE", record, state)
