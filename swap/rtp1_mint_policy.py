...
def _template_bytes(unsigned_tx_hex):
    if (
        type(unsigned_tx_hex) is not str
        or not unsigned_tx_hex
        or len(unsigned_tx_hex) > 256 * 1024
        or len(unsigned_tx_hex) % 2
        or not re.fullmatch(r"[0-9a-f]+", unsigned_tx_hex)
    ):
...
def validate_inspection(
    ...
    *,
    ...
    if (
        value["candidate_height"] != value["tip"] + 1
        or not 0 < value["final_height"] <= value["tip"]
        or value["fee_units"] != 100000
    ):
        raise ValueError("RTP1 inspection frame or fee is inconsistent")
    if (
        value["genesis_hash"] != expected["genesis_hash"]
        or value["issuer"] != issuer
        or value["recipient"] != recipient
        or value["sats"] != sats
        or value["custody_descriptor_sha256"] != descriptor
        or value["custody_manifest_sha256"] != manifest
        or value["unsigned_tx_sha256"] != hashlib.sha256(raw).hexdigest()
    ):
    ...
    if (
        ...
        or not 2 <= len(state) <= 4096
        or len(state) % 2
        or not re.fullmatch(r"[0-9a-f]+", state)
    ):
...
def decode_inspected_mint(keygen, issuer_script, unsigned_tx_hex, inspection, **policy):
    ...
    if (
        decoded["from"] != inspection["issuer"]
        or decoded["to"] != inspection["recipient"]
        or decoded["sats"] != inspection["sats"]
        or not 1 <= decoded["num_inputs"] <= 4096
    ):
    ...
    memo = decoded["memo"]
    if (
        type(memo) is not str
        or not memo.startswith("RTP1:")
        or not 2 <= len(memo) - 5 <= 256 * 1024
        or (len(memo) - 5) % 2
        or not re.fullmatch(r"[0-9a-f]+", memo[5:])
        or hashlib.sha256(bytes.fromhex(memo[5:])).hexdigest() != inspection["proof_sha256"]
    ):
