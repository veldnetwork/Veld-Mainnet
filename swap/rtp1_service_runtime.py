...
def initialize_empty_migration(directory, role, config, legacy_paths, persist):
    ...
    if role not in {"issuer", "witness"} or any(
        os.path.lexists(p) for p in (marker_path, journal_path)
    ):
    ...
    if role == "issuer":
        ...
        if lifecycle.canonical(documents["mint"]) not in {
            lifecycle.canonical(value) for value in empty_mints
        }:
        ...
        if documents["mint"] != {
            "version": 3,
            "terminal_count": 0,
            "terminal_head_sha256": lifecycle.ZERO,
            "reservations": [],
        }:
        ...
        if (
            ...
            or set(allocation) != fields
            or type(allocation["version"]) is not int
            or allocation["version"] != 5
            or type(allocation["last_consensus_sequence"]) is not int
            or allocation["last_consensus_sequence"] != 0
            or allocation["allocations"] != []
            or allocation["events"] != []
        ):
            raise ValueError("existing allocation authority requires independent reconciliation")
    marker = {
        "version": 1,
        "role": role,
        "policy_id": lifecycle.digest(config),
        "legacy_files": hashes,
    }
...
def initialize_issuer(service):
    ...
    with os.fdopen(fd, "a+") as lock:
        ...
        return initialize_empty_migration(
            service.HERE,
            "issuer",
            cfg["rtp1_service"],
            {"mint": service.STATEF, "c1": service.C1_STATEF, "leases": service.PREVOUT_STATEF},
            lambda value, path: service.save_signer_state_durable(
                value, path, max_bytes=lifecycle.MAX_BYTES
            ),
        )
...
def initialize_witness(service):
    ...
    cfg = strict_json_loads(
        read_bounded_regular_file(config_path, 1024 * 1024, "witness configuration", private=True),
    ...
    with service.exclusive_witness_state_lock(paths):
        ...
        return initialize_empty_migration(
            ...
            {
                "mint": paths["ledger"],
                "allocations": paths["allocations"],
                "terminal": paths["terminal"],
            },
            lambda value, path: service.atomic_write(
                path, lifecycle.canonical(value).decode("utf-8") + "\n"
            ),
        )
...
def load_activated_state(directory, role, cfg, legacy_paths, persist, verify):
    ...
    marker = strict_json_loads(
        read_bounded_regular_file(marker_path, 16384, "RTP1 migration inventory", private=True),
        "RTP1 migration inventory",
    )
    if (
        type(marker) is not dict
        or set(marker) != {"version", "role", "policy_id", "legacy_files"}
        or type(marker["version"]) is not int
        or marker["version"] != 1
        or marker["role"] != role
        or marker["policy_id"] != lifecycle.digest(config)
        or type(marker["legacy_files"]) is not dict
        or set(marker["legacy_files"]) != set(legacy_paths)
    ):
    ...
    for label, path in legacy_paths.items():
        ...
        if hashlib.sha256(raw).hexdigest() != marker["legacy_files"][label]:
            raise ValueError(
                "legacy authority changed after RTP1 migration; reconciliation required"
            )
    path = os.path.join(directory, "rtp1-" + role + ".json")
    state = strict_json_loads(
        read_bounded_regular_file(
            path, lifecycle.MAX_BYTES, "RTP1 authority journal", private=True
        ),
        "RTP1 authority journal",
    )
    ...
    return lifecycle.Journal(state, config, role, lambda value: persist(value, path), verify)
...
def observation(config, rpc, btc, keygen, script, request):
    ...
    result = inspect_independent_backing(
        rpc.call,
        lambda method, params: btc.call(method, *params),
        keygen,
        script,
        request["unsigned_tx_hex"],
        mint_policy=policy,
        bitcoin_policy={
            "expected_network": config["bitcoin_network"],
            "expected_genesis": config["bitcoin_genesis"],
            "custody_script_hex": config["custody_script_hex"],
            "min_confirmations": config["minimum_confirmations"],
        },
    )
...
def confirmation(config, rpc, btc, row):
    ...
    try:
        ...
        if (
            ...
            or type(peg.get("final_height")) is not int
            or not 0 < peg["final_height"] <= before["tip"]
        ):
            return None
        request = row["request"]
        ...
        if (
            ...
            or status.get("minted") is not True
            or status.get("consumed") is not True
            or status.get("accepted_effect_kind") != "MINT"
            or status.get("accepted_txid") != txid
            or status.get("tip") != before["tip"]
        ...
        if (
            ...
            or not 0 < height <= peg["final_height"]
            or rpc.call("getblockhash", [height]) != block
        ):
            return None
        raw = rpc.call("gettransaction", [txid, height])
        if (
            type(raw) is not dict
            or raw.get("txid") != txid
            or type(raw.get("block_height")) is not int
            or raw["block_height"] != height
            or raw.get("raw_hex") != row["signed_tx_hex"]
            or raw.get("block_hash") != block
        ):
            return None
        ...
        if (
            ...
            or header["confirmations"] < config["minimum_confirmations"]
            or btc.call("getblockhash", header["height"]) != facts["bitcoin_block"]
        ):
            return None
        if (
            btc.call("getblockchaininfo") != info
            or rpc.call("getbtcveldsupply", []) != before
            or rpc.call("getblockhash", [before["tip"]]) != before["tip_hash"]
        ):
            return None
...
def beat_gate(config, rpc, beat, outstanding, amount, verify):
    ...
    signed_beat = {
        ...
        if key not in {"issued_at", "received_at", "expires_at"}
    }
    ...
    if (
        ...
        or view.get("supply_sats") != beat["supply_sats"]
        or view.get("tip") != beat["tip"]
        or view.get("tip_hash") != beat["tip_hash"]
        or rpc.call("getblockhash", [beat["tip"]]) != beat["tip_hash"]
    ):
...
def reserved_mint_gate(config, rpc, beat, journal, request, verify):
    ...
    outstanding = sum(
        row["request"]["sats"]
        for row in journal.state["records"]
...
def check_descriptor(config, cfg, rpc, btc):
    binding = custody.load_manifest(
        ...
        expected_spv_spk_hex=config["custody_script_hex"],
        network=config["bitcoin_network"],
    )
    custody.verify_peg_identity(
        rpc.call("getpeginfo", []), binding, network=config["bitcoin_network"]
    )
    descriptor = btc.call("getdescriptorinfo", binding["descriptor"])
    if (
        type(descriptor) is not dict
        or descriptor.get("descriptor") != binding["descriptor"]
        or descriptor.get("hasprivatekeys") is not False
        or descriptor.get("issolvable") is not True
    ):
...
def verifier(keygen, public_key):
    pinned = (
        read_bounded_regular_file(public_key, 4096, "independent witness public key")
        .decode("ascii")
        .strip()
    )
...
def verify_committed_carrier(keygen, script, config, request, retained, signed):
    ...
    result = run_bounded_subprocess(
        [keygen, "verify-signed-carrier-stdin"],
        input_text=lifecycle.canonical(
            {
                "issuer_script_hex": script,
        ...
        stdout_max=4096,
        stderr_max=65536,
        description="RTP1 committed carrier verifier",
    )
    ...
    expected = {
        "version": 1,
        "genesis_hash": config["expected_chain"]["genesis_hash"],
        "unsigned_tx_sha256": lifecycle.request_identity(request),
        "txid": txid,
        "signed_tx_sha256": hashlib.sha256(bytes.fromhex(signed)).hexdigest(),
        "input_count": decoded["num_inputs"],
        "issuer_script_hex": script,
    }
    if (
        type(answer) is not dict
        or answer != expected
        or type(answer.get("version")) is not int
        or type(answer.get("input_count")) is not int
    ):
...
def issuer_request(service, request, cfg, issuer, script):
    ...
    def gate(req, retained=None):
        ...
        if retained is not None and lifecycle.intent_from_evidence(
            config, req, fresh
        ) != lifecycle.intent_from_evidence(config, req, retained):
        ...
        beat = strict_json_loads(
            read_bounded_regular_file(
                service.HEARTBEATF, 65536, "issuer solvency heartbeat", private=True
            ),
            "issuer solvency heartbeat",
        )
        reserved_mint_gate(config, rpc, beat, journal, req, verify)
        return fresh

    def call(action, req, **extra):
        return service._call_witness(
            witness["command"], {"action": action, "request": req, **extra}
        )

    def sign(req, retained, revalidate):
        owner = service.direct_mint_prevout_owner_id(retained["native"]["deposit_outpoint"])
        service.reserve_issuer_prevouts(
            lease,
            owner,
            "mint-direct",
            None,
            retained["native"]["deposit_outpoint"],
            req["unsigned_tx_hex"],
        )

        def evidence():
            total, inputs = service.resolve_mint_prevouts(req["unsigned_tx_hex"], rpc, script)
            return service.prepare_evidence(
                service.KEYGEN,
                rpc.call,
                rpc.expected_chain,
                req["unsigned_tx_hex"],
                ...
                amount=req["sats"],
                reserve_context={
                    key: retained["native"][key]
                    for key in ("reserve_prior_state_hex", "reserve_prior_supply_sats")
                },
            )

        return service.sign_or_recover_staged_carrier(
            lease,
            owner,
            req["unsigned_tx_hex"],
            script,
    ...
    signed = lifecycle.issuer_mint(
        journal,
        request,
        inspect=gate,
        reserve=lambda req: call("rtp1_reserve", req),
        verify_fresh=gate,
        sign_or_recover=sign,
        commit=lambda req, signed: call("rtp1_commit", req, signed_tx_hex=signed),
        halt=halt,
        receipt_verify=verify,
        witness_snapshot=lambda req: call("rtp1_status", req),
    ...
    service.reserve_issuer_prevouts(
        ...
        "mint-direct",
        None,
        row["evidence"]["native"]["deposit_outpoint"],
        request["unsigned_tx_hex"],
    )
    service.mark_issuer_prevout_signature(
        lease,
        owner,
        request["unsigned_tx_hex"],
...
def witness_request(service, envelope, cfg, paths, rpc):
    ...
    if (
        ...
        or action not in {"rtp1_reserve", "rtp1_commit", "rtp1_status"}
    ):
    ...
    if action == "rtp1_commit":
        return lifecycle.witness_commit(
            journal,
            request,
            envelope["signed_tx_hex"],
            lambda req, retained, signed: verify_committed_carrier(
                keygen, script, config, req, retained, signed
            ),
        )
    ...
    return lifecycle.witness_reserve(
        journal, request, inspect, lambda core: service._sign_receipt(core, cfg)
    )
