...
class WrapConfigurationTests(unittest.TestCase):
    def test_configuration_is_absolute_owner_only_bounded_strict_json(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            path.chmod(0o600)
            self.assertEqual(wrapd.load_configuration(str(path)), {"production": False})
            with self.assertRaisesRegex(RuntimeError, "absolute"):
                wrapd.load_configuration("wrapd.json")

            path.write_text('{"production":false,"production":true}', encoding="utf-8")
    ...
    def test_exact_scalars_loopback_command_and_production_cors(self):
        ...
        self.assertIsNone(wrapd.validate_configuration(no_cors)["cors_origin"])
    ...
    def test_development_allocator_needs_conspicuous_main_guard(self):
        with (
            mock.patch.object(wrapd, "PRODUCTION", False),
            mock.patch.object(wrapd, "DEVELOPMENT_ONLY_ALLOW_UNSAFE_ALLOCATOR", False),
        ):
    ...
    def test_allocation_witness_ack_is_exact_and_verify_is_nonmutating(self):
        ...
        with (
            mock.patch.object(wrapd, "ALLOCATION_WITNESS_COMMAND", ("/usr/bin/ssh", "w")),
            mock.patch.object(wrapd, "_C1_POLICY_SHA256", "cc" * 32),
            mock.patch.object(
                wrapd,
                "run_bounded_subprocess",
                return_value=completed("register_allocation", False),
            ) as run,
        ):
            self.assertFalse(wrapd._register_allocation_with_witness(record)["idempotent"])
        ...
        with (
            ...
            mock.patch.object(wrapd, "_C1_POLICY_SHA256", "cc" * 32),
            mock.patch.object(
                wrapd, "run_bounded_subprocess", return_value=completed("verify_allocation", True)
            ),
        ):
            self.assertTrue(
                wrapd._register_allocation_with_witness(record, verify_only=True)["idempotent"]
            )
        ...
        with (
            ...
            mock.patch.object(wrapd, "_C1_POLICY_SHA256", "cc" * 32),
            mock.patch.object(
                wrapd, "run_bounded_subprocess", return_value=completed("verify_allocation", False)
            ),
        ):
            with self.assertRaisesRegex(RuntimeError, "acknowledgement"):
                wrapd._register_allocation_with_witness(record, verify_only=True)
...
class AtomicDualWindowLimiterTests(unittest.TestCase):
    def test_limits_are_atomic_bounded_and_rejections_do_not_grow_state(self):
        ...
        limiter = wrapd.AtomicDualWindowLimiter(
            2, 3, capacity=2, maintenance_budget=1, clock=lambda: now[0]
        )
    ...
    def test_concurrent_requests_share_one_global_window(self):
        limiter = wrapd.AtomicDualWindowLimiter(50, 20, clock=lambda: 10.0)
        ...
        workers = [threading.Thread(target=hit, args=(index,)) for index in range(50)]
...
class CustodyAllocatorSeparationTests(unittest.TestCase):
    ...
    def test_index_zero_is_reserved_and_unlabelled_for_issuer_allocation(self):
        with mock.patch.object(wrapd, "_btc_json", side_effect=self.backend([])):
            wrapd._verify_descriptor_allocator_separation(self.entry, self.binding)
        with mock.patch.object(
            wrapd, "_btc_json", side_effect=self.backend([{"name": "SPV-RESERVED"}])
        ):
            wrapd._verify_descriptor_allocator_separation(self.entry, self.binding)
    ...
    def test_zero_exhausted_or_recipient_labelled_allocator_fails_closed(self):
        for next_index in (0, 1000, True, "1"):
            ...
            with (
                self.subTest(next_index=next_index),
                mock.patch.object(wrapd, "_btc_json", side_effect=self.backend([])),
                self.assertRaisesRegex(RuntimeError, r"(?:\[1,999\]|malformed)"),
            ):
                wrapd._verify_descriptor_allocator_separation(entry, self.binding)

        veld = "V" + "1" * 25
        with (
            mock.patch.object(wrapd, "_btc_json", side_effect=self.backend([veld])),
    ...
    def test_index_zero_script_and_label_metadata_are_exact(self):
        bad_backends = (
            lambda method, *_args: (
                ["bc1p" + "q" * 58]
                if method == "deriveaddresses"
                else {"scriptPubKey": "5120" + "22" * 32, "labels": []}
            ),
            self.backend(None),
    ...
    def test_command_output_and_json_are_strictly_bounded(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "stdout exceeds"):
            wrapd._run_bounded_command(
                [sys.executable, "-c", "print('x'*10000)"], 32, 32, 5, "test"
            )
        with mock.patch.object(wrapd, "_btc_output", return_value=b'{"ok":1,"ok":2}'):
...
class WrapHttpBoundaryTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        cls.server = wrapd.BoundedThreadingTCPServer(
            ("127.0.0.1", 0), wrapd.Handler, max_workers=8, request_read_timeout_s=3
        )
    ...
    def test_post_requires_exact_target_framing_content_type_and_json_schema(self):
        ...
        status, _, _ = self.request("POST", "/wrap", body, {"Content-Type": "text/plain"})
        self.assertEqual(status, 415)

        duplicate = b'{"veld_address":"V1111111111111111111111111","amount_sats":1,"amount_sats":2}'
        status, _, _ = self.request(
            "POST", "/wrap", duplicate, {"Content-Type": "application/json"}
        )
        self.assertEqual(status, 400)

        extra = b'{"veld_address":"V1111111111111111111111111","amount_sats":1,"extra":true}'
        status, _, _ = self.request("POST", "/wrap", extra, {"Content-Type": "application/json"})
        self.assertEqual(status, 400)

        status, _, _ = self.request(
            "POST", "/wrap?ignored=1", body, {"Content-Type": "application/json"}
        )
    ...
    def test_development_only_one_sat_fixture_uses_bounded_backend_parsers(self):
        ...
        with (
            mock.patch.object(wrapd, "_btc", return_value=btc) as btc_call,
            mock.patch.object(wrapd, "_btc_json", side_effect=btc_json),
        ):
            status, headers, payload = self.request(
                "POST",
                "/wrap",
                json.dumps({"veld_address": veld, "amount_sats": 1}).encode("utf-8"),
                {
    ...
    def test_production_http_enforces_owner_wrap_range_and_reports_health(self):
        ...
        def veld_rpc(method, params=None):
            ...
            if method == "getbtcveldsupply":
                return {"supply_sats": peg["supply_sats"], "tip": peg["tip"], "tip_hash": "55" * 32}
            if method == "getblockhash":
                return "55" * 32
            if method == "validateaddress":
                address = params[0]
                return {"isvalid": True, "address": address, "network": "mainnet"}
            raise AssertionError(method)
        ...
        def production_body(amount):
            return {
                ...
                "admission_beacon": wrapd._ADMISSION_BEACON.current()["beacon"],
        ...
        with (
            ...
            mock.patch.object(wrapd, "has_leading_zero_bits", return_value=True) as pow_check,
            mock.patch.object(
                wrapd, "verify_mldsa65_with_keygen", return_value=True
            ) as signature_check,
        ):
            ...
            self.assertEqual(health["consensus_capacity_reservation"], "C1R1_C1E1_C1C1_C1F1_MNP2")
            ...
            peg.update({"supply_sats": 100_000, "issuer_mint_headroom_sats": 0})
            allocator.has_request.side_effect = lambda request_id: (request_id == "%032x" % 10_000)
            allocator.allocate.side_effect = lambda request_id, principal, address, amount: {
                "request_id": request_id,
                "principal_hash": principal,
                "veld_address": address,
    ...
    def test_open_to_stalled_allocation_never_reveals_btc_address(self):
        ...
        with (
            ...
            mock.patch.object(
                wrapd, "_bitcoin_freshness", return_value={"phase": "FRESH", "tip_age_seconds": 0}
            ),
            mock.patch.object(wrapd, "_verify_admission_pow", return_value="aa" * 32),
            mock.patch.object(wrapd, "_validate_veld_destination", return_value=True),
        ):
            stalled_status, _, stalled = self.request(
                "POST",
                "/wrap",
                json.dumps(body("11" * 16)).encode(),
                {"Content-Type": "application/json"},
            )
            open_status, _, opened = self.request(
                "POST",
                "/wrap",
                json.dumps(body("22" * 16)).encode(),
                {"Content-Type": "application/json"},
            )
    ...
    def test_proxy_attribution_and_query_shape_fail_closed(self):
        ...
        self.assertEqual(status, 400)
        status, _, _ = self.request("GET", "/spk?address=a&address=b")
        self.assertEqual(status, 400)
