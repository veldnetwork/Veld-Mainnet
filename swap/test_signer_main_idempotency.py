...
class SignerMainIdempotencyTests(unittest.TestCase):
    def test_active_authority_and_state_paths_reject_unsafe_files(self):
        issuer = "Vissuer"
        policy = {"mint_authority": {
            "topology": "single-active-shared-witness",
            "signer_id": "mint-primary", "active_signer_id": "mint-primary",
            "issuer_id": issuer,
        ...
        with tempfile.TemporaryDirectory(prefix="signer-file-policy-") as td_name:
            td = Path(td_name)
            marker = td / "active-mint-signer"; marker.write_text("mint-primary\n")
            marker.chmod(0o600)
            pubkey = td / "beat.pub"; pubkey.write_text("public")
            state = td / "state.json"; state.write_text('{"signed":[]}\n')
            state.chmod(0o600)
            with mock.patch.multiple(
                    sd, ACTIVE_MINT_SIGNERF=str(marker), BEAT_PUBKEY=str(pubkey)):
                ...
                marker.unlink(); marker.write_text("mint-primary\n"); marker.chmod(0o600)
                pubkey.unlink(); pubkey.symlink_to("/etc/passwd")
                with self.assertRaisesRegex(ValueError, "non-symlink"):
                    sd.validate_mint_authority_config(policy, issuer)

                state.unlink(); state.symlink_to("/etc/passwd")
                with self.assertRaisesRegex(ValueError, "non-symlink"):
    ...
    def test_cached_request_replays_exact_bytes_without_spent_prevout_lookup(self):
        ...
        receipt_core = {
            ...
            "reservation_id": sol.reservation_id(
                issuer, request_id, request_id, sats, recipient,
                deposit_outpoint, allocation["consensus_allocation_id"]),
            "sats": sats, "recipient": recipient, "beat_seq": 1,
            "tip": 10, "tip_hash": "11" * 32, "headroom_sats": sats,
            ...
            "deposit_outpoint": deposit_outpoint,
            "reserved_at": 1, "sig_alg": "mldsa65", "sig": "00",
        }
        state = {
            "signed": [{"request_id": request_id, "txid": txid,
                        "signed_tx_hex": signed_hex, "sats": sats,
                        "to": recipient, "at": 1,
                        "witness_receipt": receipt_core,
                        "witness_committed": True}],
            "mint_accounting": {
                "version": sd.MINT_ACCOUNTING_VERSION,
                "pending": [{"request_id": request_id, "txid": txid,
                             "sats": sats, "signed_at": 1,
                             "witness_receipt": receipt_core,
                             "witness_committed": True}],
                "confirmed": [],
            },
        ...
        with tempfile.TemporaryDirectory(prefix="signer-main-retry-") as td:
            td = Path(td)
            paths = {name: str(td / name) for name in
                     ("veld-keygen", "issuer.key", "passphrase.txt",
                      "issuer-address.txt", "signer-state.json", "HALT",
                      "signer.log", "signer-heartbeat.json",
                      "watchtower-required", "caps-only-ok",
                      "issuer-prevout-leases.json")}
            for name in ("veld-keygen", "issuer.key", "passphrase.txt",
                         "issuer-address.txt"):
                (td / name).write_text("fixture")
            (td / "veld-keygen").chmod(0o700)
            (td / "issuer.key").chmod(0o600)
            (td / "passphrase.txt").chmod(0o600)
            config = td / "signer-config.json"; config.write_text("{}")
            config.chmod(0o600)

            fake_rpc = object()
            saved = []
            stdin = io.StringIO(json.dumps({"unsigned_tx_hex": unsigned_hex,
                                             "recipient": recipient,
                                             "sats": sats,
                                             "allocation": allocation}))
            stdout = io.StringIO(); stderr = io.StringIO()
            replacements = {
                "HERE": str(td), "KEYGEN": paths["veld-keygen"],
                "KEYFILE": paths["issuer.key"], "PASSFILE": paths["passphrase.txt"],
                "ADDRFILE": paths["issuer-address.txt"],
                "STATEF": paths["signer-state.json"], "HALTF": paths["HALT"],
                "LOGF": paths["signer.log"], "HEARTBEATF": paths["signer-heartbeat.json"],
                "WT_REQUIRED": paths["watchtower-required"],
                "CAPS_ONLY_OK": paths["caps-only-ok"], "SIGNER_CONFIG": str(config),
            ...
            with mock.patch.multiple(sd, **replacements), \
                mock.patch.object(sd, "load_signer_configuration",
                                  return_value={}), \
                ...
                mock.patch.object(sd, "issuer_addr", return_value=issuer), \
                 ...
                 mock.patch.object(sd, "mint_params_from_tx",
                                   return_value=(recipient, sats, 99999,
                                                 deposit_outpoint,
                                                 None,
                                                 allocation["consensus_allocation_id"],
                                                 allocation["script_pubkey"],
                                                 allocation["commitment_blind"])), \
                 mock.patch.object(sd, "load_signer_state",
                                   return_value=copy.deepcopy(state)), \
                 mock.patch.object(sd, "load_or_reconcile_prevout_journal",
                                   return_value=sd._empty_prevout_journal()), \
                 mock.patch.object(sd, "save_signer_state_durable",
                                   side_effect=save_state), \
                 mock.patch.object(sd, "TrustedVeldRpc", return_value=fake_rpc), \
                 mock.patch.object(sd, "validate_mint_authority_config",
                                   return_value=({}, {"witness_id": "witness-1",
                                                     "command": ["witness"]})), \
                 mock.patch.object(sd, "validate_fresh_mint_boundary",
                                   return_value={}), \
                 mock.patch.object(sd, "resolve_mint_prevouts") as resolve, \
                 mock.patch.object(sd, "watchtower_gate",
                                   return_value=(0, {"seq": 1})) as gate, \
    ...
    def test_new_signature_is_reserved_and_fsynced_before_signing_and_commit(self):
        ...
        heartbeat = {"seq": 9, "tip": 20, "tip_hash": "22" * 32,
                     "headroom_sats": sats, "expires_at": 9999999999}
        receipt = {
            "v": sol.C1_RESERVATION_VERSION, "kind": sol.RESERVATION_KIND,
            "issuer_id": issuer, "witness_id": "witness-1",
            "request_id": request_id, "unsigned_tx_sha256": request_id,
            "reservation_id": sol.reservation_id(
                issuer, request_id, request_id, sats, recipient,
                deposit_outpoint, allocation["consensus_allocation_id"]),
            "sats": sats, "recipient": recipient, "beat_seq": 9,
            "tip": 20, "tip_hash": "22" * 32, "headroom_sats": sats,
            "allocation_verified": True,
            "allocation_request_id": allocation["consensus_allocation_id"],
            "allocation_descriptor_index": allocation["descriptor_index"],
            "allocation_btc_address": allocation["btc_address"],
        ...
        with tempfile.TemporaryDirectory(prefix="signer-main-order-") as td_name:
            ...
            def durable_save(value, path=None,
                             max_bytes=sd.SIGNER_STATE_MAX_BYTES):
                if path == paths["issuer-prevout-leases.json"]:
                    active = next(iter(value["owners"].values()))["active"]
                    events.append(
                        "fsync-prevout-txid" if active["txid"] else
                        ("fsync-prevout-signing"
                         if active["signing_state"] == "SIGNING" else
                         "fsync-prevout-lease"))
                    return original_save(value, path, max_bytes)
            ...
            def sign_process(argv, **ignored):
                ...
                out_path.write_text(signed_hex); out_path.chmod(0o600)
            ...
            stdin = io.StringIO(json.dumps({"unsigned_tx_hex": unsigned_hex,
                                             ...
                                             "sats": sats,
                                             "allocation": allocation}))
            stdout = OrderedOutput(); stderr = io.StringIO()
            replacements = {
                "HERE": str(td), "KEYGEN": paths["veld-keygen"],
                "KEYFILE": paths["issuer.key"], "PASSFILE": paths["passphrase.txt"],
                "ADDRFILE": paths["issuer-address.txt"],
            ...
            with ExitStack() as stack:
                ...
                stack.enter_context(mock.patch.object(
                    sd, "load_signer_configuration", return_value={}))
                stack.enter_context(mock.patch.object(
                    sd, "require_signer_authority_state_set"))
                stack.enter_context(mock.patch.object(
                    sd, "issuer_addr", return_value=issuer))
                stack.enter_context(mock.patch.object(
                    sd, "issuer_p2pkh_from_address", return_value="00" * 25))
                stack.enter_context(mock.patch.object(
                    sd, "mint_params_from_tx",
                    return_value=(recipient, sats, 1000,
                                  deposit_outpoint,
                                  None,
                                  allocation["consensus_allocation_id"],
                                  allocation["script_pubkey"],
                                  allocation["commitment_blind"])))
                stack.enter_context(mock.patch.object(
                    sd, "TrustedVeldRpc", return_value=object()))
                stack.enter_context(mock.patch.object(
                    sd, "load_or_reconcile_prevout_journal",
                    return_value=sd._empty_prevout_journal()))
                stack.enter_context(mock.patch.object(
                    sd, "validate_mint_authority_config",
                    return_value=({}, {"witness_id": "witness-1",
                                      "command": ["witness"]})))
                stack.enter_context(mock.patch.object(
                    sd, "validate_fresh_mint_boundary", return_value={}))
                stack.enter_context(mock.patch.object(
                    sd, "resolve_mint_prevouts",
                    return_value=(101000, [{"txid": "x"}])))
                stack.enter_context(mock.patch.object(
                    sd, "watchtower_gate", return_value=(0, heartbeat)))
                stack.enter_context(mock.patch.object(
                    sd, "reserve_witness_headroom", side_effect=reserve))
                stack.enter_context(mock.patch.object(
                    sd, "commit_witness_transaction", side_effect=commit))
                stack.enter_context(mock.patch.object(
                    sd.sol, "unsigned_template_from_signed_hex",
                    return_value=unsigned_hex))
                stack.enter_context(mock.patch.object(
                    sd, "save_signer_state_durable", side_effect=durable_save))
                stack.enter_context(mock.patch.object(
                    sd.subprocess, "run", side_effect=sign_process))
            ...
            self.assertEqual(stdout.getvalue(), signed_hex + "\n")
            self.assertEqual(events, ["fsync-prevout-lease", "reserve",
                                      "fsync-reservation", "fsync-prepared",
                                      "fsync-prevout-signing", "sign",
                                      "fsync-signed-bytes", "commit",
                                      "fsync-commit", "fsync-prevout-txid",
                                      "stdout"])
