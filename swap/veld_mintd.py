...
from veld_spvmint import (
    MAX_BITCOIN_TX_BYTES,
    MAX_BLOCK_TXIDS,
    be_to_le,
    build_merkle_branch,
    dsha,
    fold_merkle,
    parse_and_strip_bitcoin_tx,
)  # noqa: E402
...
def production_allocation_settings(cfg):
    ...
    if missing:
        raise RuntimeError("production allocation policy is incomplete: %s" % ",".join(missing))
    path = cfg["allocation_store"]
    if not isinstance(path, str) or not path or not os.path.isabs(path):
        raise RuntimeError("production allocation_store must be an absolute path")
    maximum = _config_int(
        cfg["allocation_store_max_bytes"],
        "allocation_store_max_bytes",
        65536,
        MAX_ALLOCATION_STORE_BYTES,
    )
    ...
    if type(range_end) is not int or not 10999 <= range_end <= 1_000_000:
        raise RuntimeError("public_descriptor_range_end must be an integer in [10999,1000000]")
    consensus_manifest_hash = cfg["custody_consensus_manifest_sha256"]
    if not isinstance(consensus_manifest_hash, str) or not re.fullmatch(
        r"[0-9a-f]{64}", consensus_manifest_hash
    ):
        raise RuntimeError("custody_consensus_manifest_sha256 must be canonical 64-hex")
    ...
    if (
        ...
        raise RuntimeError("consensus_reservation_command must be an absolute bounded argv")
    return (path, maximum, policy_hash, range_end, consensus_manifest_hash, tuple(command))
    ...
    def __init__(self, url, token_file=None, token_cmd=None):
        ...
        if token_cmd:
            if (
                ...
                or not token_cmd
                or any(
                    not isinstance(item, str) or not item or "\x00" in item for item in token_cmd
                )
            ):
            ...
            self.token = (
                read_bounded_secret_file(token_file, 4096, "Veld RPC token").decode("ascii").strip()
            )
...
def _validate_ledger(document):
    ...
    for oid, record in document.items():
        if (
            ...
            or not OUTPOINT_RE.fullmatch(oid)
            or int(oid.rsplit(":", 1)[1]) > 0xFFFFFFFF
        ):
        ...
        if status in ("reprepare", "prepared", "signed") and "excluded_issuer_prevouts" in record:
            validate_issuer_prevout_exclusions(record["excluded_issuer_prevouts"])
        ...
        if status in ("prepared", "signed") and (
            not isinstance(unsigned_hex, str)
            or not re.fullmatch(r"[0-9a-fA-F]+", unsigned_hex)
            or len(unsigned_hex) > 2 * MAX_SIGNED_TX_BYTES
            or not isinstance(record.get("inputs"), list)
            or record.get("signer_allocation") is not None
            and not isinstance(record.get("signer_allocation"), dict)
        ):
        ...
        if status in ("signed", "broadcast", "effect_confirmed", "hold") and (
            ...
            or len(signed_hex) > 2 * MAX_SIGNED_TX_BYTES
        ):
            raise RuntimeError("mint ledger signed transaction is malformed")
        schemas = {
            "reprepare": common | {"excluded_issuer_prevouts"},
            "prepared": common | {"unsigned_tx_hex", "inputs", "signer_allocation"},
            "signed": common | {"unsigned_tx_hex", "inputs", "signer_allocation", "signed_tx_hex"},
            "broadcast": common | {"signed_tx_hex", "veld_txid", "broadcast_at"},
            "hold": common | {"signed_tx_hex", "veld_txid", "broadcast_at", "hold_reason"},
        ...
        if status in ("prepared", "signed"):
            allowed_schemas += (schemas[status] | {"excluded_issuer_prevouts"},)
        ...
        if status in ("broadcast", "hold", "effect_confirmed"):
            if (
                ...
                or type(record.get("broadcast_at")) is not int
                or record["broadcast_at"] < record["at"]
            ):
                raise RuntimeError("mint ledger broadcast identity is malformed")
        if status == "hold" and (
            not isinstance(record.get("hold_reason"), str)
            or not 1 <= len(record["hold_reason"]) <= 512
        ):
            raise RuntimeError("mint ledger HOLD reason is malformed")
        if status == "effect_confirmed" and (
            type(record.get("effect_at")) is not int
            or record["effect_at"] < record["broadcast_at"]
            or type(record.get("accepted_block_height")) is not int
            or record["accepted_block_height"] < 0
            or not isinstance(record.get("accepted_block_hash"), str)
            or not TXID_RE.fullmatch(record["accepted_block_hash"])
        ):
...
def load_ledger(path):
    if not os.path.lexists(path):
        return {}
    document = _validate_ledger(
        load_bounded_json_file(os.path.abspath(path), MAX_LEDGER_BYTES, "mint ledger")
    )
...
def load_issued_wrap_allocations(path, maximum, custody, expected_policy_sha256=None):
    ...
    if not isinstance(path, str) or not os.path.isabs(path):
        raise RuntimeError("production allocation_store must be an absolute path")
    if type(maximum) is not int or maximum < 65536 or maximum > MAX_ALLOCATION_STORE_BYTES:
    ...
    if (
        not isinstance(custody_range, list)
        or len(custody_range) != 2
        or custody_range[0] != 0
        or type(custody_range[1]) is not int
        or (c1 and not 10999 <= custody_range[1] <= 1_000_000)
        or (not c1 and custody_range[1] != 999)
    ):
    ...
    if (
        ...
        or custody.get("range") != [0, range_end]
        or not isinstance(custody.get("script_pubkeys"), tuple)
        or len(custody["script_pubkeys"]) != range_end + 1
    ):
        raise RuntimeError("custody manifest is unavailable for allocation binding")
    document = load_bounded_json_file(path, maximum, "wrap allocation journal", private=True)
    ...
    if (
        ...
        or (
            ...
            and (
                ...
                or not initial_min <= document["initial_next_index"] <= range_end
            )
        )
        or not isinstance(document.get("records"), list)
        or len(document["records"]) > (100_000 if c1 else 999)
        or (
            c1
            and (
                document.get("capacity_policy_sha256") != expected_policy_sha256
                or type(document.get("capacity_policy_sequence")) is not int
                or document["capacity_policy_sequence"] < 1
                or document.get("pool_range_start") != 1000
                or document.get("public_descriptor_range_end") != range_end
                or type(document.get("last_consensus_sequence")) is not int
                or not 0 <= document["last_consensus_sequence"] <= (1 << 64) - 1
                or not isinstance(document.get("events"), list)
                or len(document["records"]) + len(document["events"]) > 100_000
            )
        )
    ):
        raise RuntimeError("wrap allocation journal schema is invalid")
    initial = document["initial_next_index"] if not c1 else document["pool_range_start"]
    if c1:
        previous_event = "0" * 64
    ...
    for record in document["records"]:
        ...
        if (
            ...
            or not 1 <= amount <= MAX_MONEY_SATS
            or type(index) is not int
            or not initial <= index <= range_end
            or index in seen_indices
            or not isinstance(address, str)
            or not 14 <= len(address) <= 100
            or address in seen_addresses
            or script != custody["script_pubkeys"][index]
            or state not in ("reserved", "allocated", "issued")
        ):
            raise RuntimeError("wrap allocation record is invalid")
        if c1 and (
            type(record.get("admitted_at")) is not int
            or type(record.get("expires_at")) is not int
            or record["expires_at"] - record["admitted_at"] != 604_800
            or not isinstance(record.get("capacity_policy_sha256"), str)
            or not re.fullmatch(r"[0-9a-f]{64}", record["capacity_policy_sha256"])
            or not isinstance(record.get("commitment_blind"), str)
            or not re.fullmatch(r"[0-9a-f]{64}", record["commitment_blind"])
            or int(record["commitment_blind"], 16) == 0
            or record["commitment_blind"] in seen_blinds
            or not isinstance(record.get("consensus_allocation_id"), str)
            or not re.fullmatch(r"0{16}[0-9a-f]{16}", record["consensus_allocation_id"])
            or int(record["consensus_allocation_id"], 16) == 0
            or int(record["consensus_allocation_id"], 16) > document["last_consensus_sequence"]
            or record["consensus_allocation_id"] in seen_consensus_ids
        ):
            raise RuntimeError("wrap allocation C1 metadata is invalid")
        try:
            address_script = custody_binding._bech32m_spk(address)
        except RuntimeError as exc:
            raise RuntimeError("wrap allocation address is not canonical custody P2TR") from exc
        if address_script != script:
            raise RuntimeError("wrap allocation address does not match its custody script")
    ...
    records.sort(
        key=(lambda record: int(record["consensus_allocation_id"], 16))
        if c1
        else (lambda record: record["descriptor_index"])
    )
    if (
        (
            (not c1)
            and [record["descriptor_index"] for record in records]
            != list(range(initial, initial + len(records)))
        )
        or (records and max(record["descriptor_index"] for record in records) > range_end)
        or len(reserved) > 1
        or (reserved and reserved != records[-1:])
    ):
        raise RuntimeError("wrap allocation descriptor history is not contiguous")
    return {
        record["btc_address"]: dict(record) for record in records if record["state"] == "issued"
    }
...
class RemoteSigner:
    ...
    def sign(self, unsigned_tx_hex, inputs, recipient, sats, allocation=None):
        ...
        req = json.dumps(
            {"unsigned_tx_hex": unsigned_tx_hex, "recipient": recipient, "sats": int(sats)}
        )
        ...
        cmd = [
            ...
            "-o",
            "UserKnownHostsFile=/var/lib/veld-peg/.ssh/known_hosts",
        ]
        if self.ssh_key:
            cmd += ["-i", self.ssh_key]
        ...
        out = run_bounded_subprocess(
            ...
            input_text=req,
            timeout=60,
            stdout_max=MAX_SIGNED_TX_BYTES,
            stderr_max=1024 * 1024,
            description="remote mint signer",
        )
        if out.returncode == 75 and allocation is not None:
            try:
                answer = strict_json_loads(out.stdout, "remote signer unsigned abandonment")
            except Exception as exc:
                raise RuntimeError("remote signer returned a malformed abandonment") from exc
            expected = {
                "version",
                "action",
                "capability",
            ...
            try:
                conflicts = validate_issuer_prevout_exclusions(
                    answer.get("conflicting_input_outpoints") if isinstance(answer, dict) else None,
                    candidate_inputs=unsigned_input_outpoints(unsigned_tx_hex),
                )
            except Exception as exc:
                raise RuntimeError(
            ...
            if (
                ...
                or answer.get("capability") != "mint"
                or answer.get("owner_id") != "mint:" + allocation_id
                or answer.get("allocation_id") != allocation_id
                or answer.get("deposit_outpoint") != allocation["deposit_outpoint"]
                or answer.get("unsigned_sha256") != unsigned_sha256
                or answer.get("reason") != "issuer_prevout_conflict"
            ):
                raise RuntimeError("remote signer abandonment does not bind this exact carrier")
...
class Minter:
    def __init__(self, cfg):
        ...
        if not isinstance(self.issuer_addr, str) or not VELD_ADDR_RE.fullmatch(self.issuer_addr):
            raise RuntimeError("issuer_addr is malformed")
        sc = cfg["signer"]  # {"ssh_target", "ssh_key"?, "remote_cmd"?}
        self.signer = RemoteSigner(
            sc["ssh_target"],
            sc.get("ssh_key", ""),
            sc.get("remote_cmd", "veld_signerd --capability mint"),
        )
        # tunables (config overrides; conservative defaults)
        self.K_BTC = _config_int(
            cfg.get("k_btc_confirmations", 6), "k_btc_confirmations", 1, 1_000_000
        )
        self.MAX_SINGLE_SATS = _config_int(
            cfg.get("max_single_mint_sats", 5 * SATS), "max_single_mint_sats", 1, (1 << 63) - 1
        )  # dev default only
        self.MAX_WINDOW_SATS = _config_int(
            cfg.get("max_window_mint_sats", 25 * SATS), "max_window_mint_sats", 1, (1 << 63) - 1
        )  # dev default only
        self.WINDOW_SECS = _config_int(cfg.get("window_secs", 3600), "window_secs", 1, 31_536_000)
        self.RECONCILE_TOL_SATS = _config_int(
            cfg.get("reconcile_tolerance_sats", 0), "reconcile_tolerance_sats", 0, (1 << 63) - 1
        )
        ...
        if self.production:
            self.custody = custody_binding.load_manifest(
                ...
                expected_consensus_manifest_sha256=(self.custody_consensus_manifest_sha256),
            )
    ...
    def trip_halt(self, reason):
        ...
        try:
            ...
            pass
        warn(
            f"*** HALT tripped: {reason} - all minting stopped until {self.halt_path} is removed ***"
        )
    ...
    def _verify_production_identity(self, *, completion=False):
        ...
        if (
            ...
            or peg.get(required_gate) is not True
        ):
            raise RuntimeError(
        ...
        if (
            self.MAX_WINDOW_SATS <= 0
            or self.MAX_WINDOW_SATS > issuer_static_cap
            or issuer_effective_cap > issuer_static_cap
            or issuer_effective_cap < 0
            or issuer_reserved < 0
            or issuer_reserved > issuer_static_cap
            or issuer_headroom < 0
            or issuer_headroom > issuer_effective_cap
        ):
        ...
        if (
            ...
            or not TXID_RE.fullmatch(supply_before["tip_hash"])
            ...
            or issuer_headroom
            != max(0, issuer_effective_cap - supply_before["supply_sats"] - issuer_reserved)
        ):
            raise RuntimeError("production issuer capacity/supply tuple changed or is incoherent")
    ...
    def _configure_production(self):
        ...
        if (
            ...
            or len(entries) != 1
            or entries[0].get("desc") != self.custody["descriptor"]
            or entries[0].get("internal") is not False
            or entries[0].get("active") is not False
            or entries[0].get("range") != self.custody["range"]
        ):
    ...
    def _require_current_issued_allocation(self, deposit):
        ...
        if (
            allocation is None
            or allocation.get("request_id") != request_id
            or allocation.get("btc_address") != address
            or allocation.get("script_pubkey") != script
            or allocation.get("veld_address") != recipient
            or allocation.get("amount_sats") != sats
        ):
            raise RuntimeError("deposit is not exactly bound to a current issued wrap allocation")
        return allocation

    def _require_canonical_c1_reservation(
        self, allocation, peg, outpoint=None, require_funded=False
    ):
        ...
        if (
            ...
            or status["exposure_confirmations"] < 101
            or status.get("recipient") != allocation.get("veld_address")
            or status.get("amount_sats") != amount
            or status.get("allocation_commitment")
            != allocation_commitment(
                allocation_id,
                allocation.get("veld_address"),
                amount,
                allocation.get("script_pubkey"),
                allocation.get("commitment_blind"),
            )
            or status.get("tip") != peg.get("tip")
            or type(status.get("funded")) is not bool
            or (require_funded and status.get("funded") is not True)
            or (require_funded and status.get("funding_outpoint") != outpoint)
            or (
                not require_funded
                and status.get("funded") is True
                and outpoint is not None
                and status.get("funding_outpoint") != outpoint
            )
            or type(supply) is not int
            or type(static_cap) is not int
            or type(reserved) is not int
            or type(amount) is not int
            or reserved < amount
            or supply < 0
            or amount <= 0
            or supply > static_cap - amount
        ):
            raise RuntimeError("matching canonical exposed C1 capacity is unavailable")
        return status
    ...
    def _build_c1_funding_proof(self, deposit, allocation):
        ...
        confirmations = verbose.get("confirmations") if isinstance(verbose, dict) else None
        if (
            not isinstance(block_hash, str)
            or not TXID_RE.fullmatch(block_hash)
            or type(confirmations) is not int
            or confirmations < self.K_BTC + 1
        ...
        if (
            ...
            or header["height"] < 0
            or type(header.get("confirmations")) is not int
            or header["confirmations"] < self.K_BTC + 1
        ):
        ...
        if (
            ...
            or relay.get("k_btc") != self.K_BTC
            or type(relay.get("best_height")) is not int
            or relay["best_height"] < block_height + self.K_BTC
        ):
        ...
        if (
            ...
            or block.get("hash") not in (None, block_hash)
            or block.get("height") not in (None, block_height)
        ):
        ...
        if (
            ...
            or not TXID_RE.fullmatch(merkle_root)
            or root != be_to_le(merkle_root)
            or fold_merkle(be_to_le(txid), branch, directions) != root
        ):
        ...
        if (
            ...
            or len(raw_hex) > MAX_BITCOIN_TX_BYTES * 2
        ):
        ...
        exact_outputs = [
            ...
            if row["script"] == exact_script and row["value_sats"] == allocation["amount_sats"]
        ]
        if (
            len(exact_outputs) != 1
            or exact_outputs[0]["vout"] != vout
            or deposit.get("sats") != allocation["amount_sats"]
        ):
        ...
        if (
            nullifier["consumed"]
            or nullifier["minted"]
            or not isinstance(nullifier.get("proof_hex"), str)
            or not re.fullmatch(r"[0-9a-f]+", nullifier["proof_hex"])
            or not isinstance(nullifier.get("root"), str)
            or not TXID_RE.fullmatch(nullifier["root"])
            or type(nullifier.get("count")) is not int
            or nullifier["count"] < 0
        ):
            raise RuntimeError("CFP1 parent nullifier state is unavailable")
        proof = (
            b"CFP1"
            + be_to_le(block_hash)
            + struct.pack("<I", directions)
            + bytes([len(branch)])
            + b"".join(branch)
        ...
        if (
            fresh["consumed"]
            or fresh.get("root") != nullifier["root"]
            or fresh.get("count") != nullifier["count"]
            or fresh.get("proof_hex") != nullifier["proof_hex"]
        ):
    ...
    def _ensure_c1_funded(self, allocation, funding):
        ...
        completed = run_bounded_subprocess(
            list(self.consensus_reservation_command),
            input_text=request,
            timeout=120,
            stdout_max=64 * 1024,
            stderr_max=64 * 1024,
        ...
        if completed.returncode not in (0, 75):
            raise RuntimeError("C1 funding coordinator refused: " + completed.stderr.strip()[:240])
        answer = strict_json_loads(completed.stdout, "C1 funding coordinator response")
        expected = {
            "version",
            "allocation_id",
            "found",
        ...
        if (
            ...
            or any(
                type(answer.get(key)) is not int or answer[key] < 0
                for key in ("last_sequence", "confirmations", "required_confirmations")
            )
            or answer["required_confirmations"] != 101
            or (answer["funded"] and answer.get("funding_outpoint") != funding["outpoint"])
            or (not answer["funded"] and answer.get("funding_outpoint") is not None)
        ):
    ...
    def custody_confirmed_sats(self):
        ...
        if self.production:
            return sum(
                btc_to_sats(row.get("amount"))
                for row in self._production_wallet_rows()
                if (type(row.get("confirmations")) is int and row["confirmations"] >= self.K_BTC)
            )
    ...
    def window_minted_sats(self):
        ...
        return sum(
            ...
            if r.get("status") in ("signed", "broadcast", "effect_confirmed", "hold", "minted")
            and r.get("at", 0) >= cutoff
        )
    ...
    def mint_outpoint_status(self, outpoint):
        ...
        supply_before = (
            self.veld.rpc("getbtcveldsupply") if getattr(self, "production", False) else None
        )
        ...
        if getattr(self, "production", False) and (
            ...
            or supply_before != supply_after
            or set(supply_before) != {"supply_sats", "tip", "tip_hash"}
            or status["tip"] != supply_before.get("tip")
            or status["tip_hash"] != supply_before.get("tip_hash")
        ):
            raise RuntimeError("mint effect status changed across coherent supply tuple")

        def exact_locator(prefix):
            return (
                isinstance(status.get(prefix + "_txid"), str)
                and TXID_RE.fullmatch(status[prefix + "_txid"])
                and type(status.get(prefix + "_block_height")) is int
                and status[prefix + "_block_height"] >= 0
                and isinstance(status.get(prefix + "_block_hash"), str)
                and TXID_RE.fullmatch(status[prefix + "_block_hash"])
                and type(status.get(prefix + "_tx_index")) is int
                and status[prefix + "_tx_index"] >= 0
                and type(status.get(prefix + "_marker_vout")) is int
                and status[prefix + "_marker_vout"] >= 0
            )
        ...
        if status["consumed"]:
            if not exact_locator("accepted") or status.get("accepted_effect_kind") not in (
                "MINT",
                "C1_FUND",
                "C1_MINT",
            ):
                raise RuntimeError("consumed mint outpoint lacks an exact accepted effect locator")
            if status.get("accepted_effect_kind") in ("C1_FUND", "C1_MINT"):
                if (
                    not isinstance(status.get("c1_allocation_id"), str)
                    or not re.fullmatch(r"0{16}[0-9a-f]{16}", status["c1_allocation_id"])
                    or not exact_locator("consumer")
                ):
                    raise RuntimeError("C1 mint status lacks its FUND consumer")
            elif status.get("c1_allocation_id") is not None or not null_locator("consumer"):
            ...
            status["minted"]
            or not null_locator("accepted")
            or not null_locator("consumer")
            or not null_locator("credit")
            or status.get("accepted_effect_kind") is not None
            or status.get("c1_allocation_id") is not None
        ):
            raise RuntimeError("unconsumed mint outpoint unexpectedly has effect metadata")
        return status
    ...
    def exact_mint_effect(self, outpoint, expected_txid):
        ...
        if not status["minted"]:
            return None
        if (
            status.get("credit_txid") != expected_txid
            or status.get("accepted_txid") != expected_txid
            or status.get("accepted_effect_kind") not in ("MINT", "C1_MINT")
        ):
            raise RuntimeError("deposit outpoint was consumed by a different accepted mint txid")
        return status
    ...
    def _prune_settled_ledger(self):
        ...
        for outpoint, record in self.ledger.items():
            ...
            if record.get("status") == "minted":
                ...
                if not getattr(self, "production", False) and self.mint_outpoint_consumed(outpoint):
                    removable.append(outpoint)
                continue
            effect = self.exact_mint_effect(outpoint, record["veld_txid"])
            if (
                effect is not None
                and effect["tip"] - effect["accepted_block_height"] + 1 > MAX_REORG_DEPTH
            ):
                removable.append(outpoint)
        if not removable:
            return 0
    ...
    def _advance_unresolved(self, outpoint, record):
        ...
        if status == "minting":
            if not self.production:
                ...
                if self.mint_outpoint_consumed(outpoint):
                    ...
                    self._save_record()
                    return True
                self.trip_halt(f"{outpoint}: legacy mint intent requires manual review")
                return False
            raise RuntimeError("legacy production mint intent requires offline reconciliation")
        if status == "minted":
            if not self.production:
        ...
        if expected_txid is not None:
            ...
            if effect is not None:
                if (
                    ...
                    and record.get("accepted_block_height") == effect["accepted_block_height"]
                    and record.get("accepted_block_hash") == effect["accepted_block_hash"]
                ):
                ...
                self._save_record()
                log(f"{outpoint}: exact accepted mint effect confirmed txid={expected_txid}")
                return True
        ...
        if status == "prepared":
            ...
            try:
                ...
                try:
                    ...
                    raise
                warn(
                    f"{outpoint}: signer revoked conflicting unsigned fee "
        ...
        if status == "signed":
            ...
            calculated_txid = hashlib.sha256(
                hashlib.sha256(bytes.fromhex(signed)).digest()
            ).hexdigest()
            ...
            if txid != calculated_txid:
                raise RuntimeError("sendrawtransaction txid differs from exact signed carrier")
            record.clear()
            record.update(
                {
                    "status": "broadcast",
            ...
            self._save_record()
            log(
                f"{outpoint}: broadcast exact mint carrier txid={txid}; "
        ...
        if status in ("broadcast", "hold"):
            ...
            try:
                ...
                record["status"] = "hold"
                record["hold_reason"] = "exact carrier absent/unrebroadcastable: " + str(exc)[:400]
                self._save_record()
                warn(
                    f"{outpoint}: HOLD exact carrier; operator should inspect/rebroadcast "
    ...
    def eligible_deposits(self):
        ...
        utxos = (
            self._production_wallet_rows()
            if self.production
            else self.btc.call("listunspent", self.K_BTC, 9999999)
        )
        ...
        for u in utxos:
            ...
            if (
                not isinstance(u.get("txid"), str)
                or not TXID_RE.fullmatch(u["txid"])
                or type(u.get("vout")) is not int
                or not (0 <= u["vout"] <= 0xFFFFFFFF)
            ):
                continue
            ...
            if self.production:
                ...
                allocation = allocations.get(address) if isinstance(address, str) else None
                if (
                    allocation is None
                    or not isinstance(script, str)
                    or allocation["btc_address"] != address
                    or allocation["script_pubkey"] != script
                    or allocation["veld_address"] != label
                    or allocation["amount_sats"] != sats
                ):
                    warn(
                        "%s: custody output does not exactly match an issued "
                        "wrap allocation - HOLD" % outpoint_id(u["txid"], u["vout"])
                    )
                    continue
                deposit.update(
                    {
    ...
    def process_one(self, d):
        ...
        if rec:
            if rec.get("status") != "reprepare":
                ...
                return
            if rec.get("recipient") != d.get("recipient") or rec.get("sats") != d.get("sats"):
                raise RuntimeError("replacement deposit differs from durable mint owner")
            exclusions = validate_issuer_prevout_exclusions(rec.get("excluded_issuer_prevouts"))
            replacing = True
        ...
        unresolved = [
            other
            for other, row in self.ledger.items()
            if other != oid and row.get("status") not in ("effect_confirmed", "minted")
        ]
        if unresolved:
        ...
        if not VELD_ADDR_RE.match(d["recipient"]):
            warn(f"{oid}: bad recipient label {d['recipient']!r} - skip")
            return

        # Gate 5: finality
        if d["confs"] < self.K_BTC:
            log(f"{oid}: confs {d['confs']} < K_BTC {self.K_BTC} - HOLD (not final)")
            return

        # Gate 6: amount bound
        if not (0 < d["sats"] <= self.MAX_SINGLE_SATS):
            self.trip_halt(f"{oid}: amount {d['sats']} outside (0, {self.MAX_SINGLE_SATS}]")
            return
        ...
        if (
            self.production
            and initial_mint_status["consumed"]
            and initial_mint_status.get("accepted_effect_kind") != "C1_FUND"
        ):
            self.trip_halt(f"{oid}: outpoint consumed outside the expected C1_FUND lifecycle")
            return
        if self.production:
            try:
                self._verify_production_identity(completion=True)
        ...
        if self.window_minted_sats() + d["sats"] > self.MAX_WINDOW_SATS:
            self.trip_halt(
                f"{oid}: window cap - {self.window_minted_sats()}+{d['sats']} > {self.MAX_WINDOW_SATS}"
            )
            return
        ...
        if self.production:
            try:
                ...
                self.trip_halt(
                    "%s: wrap allocation binding unavailable/mismatched: %s" % (oid, str(exc)[:160])
                )
                return

            if not initial_mint_status["consumed"]:
                try:
                    funding = self._build_c1_funding_proof(d, current_allocation)
                    ...
                    late_peg = self._verify_production_identity(completion=True)
                    reservation = self._require_canonical_c1_reservation(
                        current_allocation, late_peg, outpoint=oid, require_funded=True
                    )
                except Exception as exc:
                    warn(f"{oid}: C1F1 funding lifecycle HOLD: {str(exc)[:240]}")
                    return
            if (
                initial_mint_status.get("consumed") is not True
                or initial_mint_status.get("minted") is not False
                or initial_mint_status.get("accepted_effect_kind") != "C1_FUND"
                or initial_mint_status.get("c1_allocation_id")
        ...
        mint_params = [self.issuer_addr, d["recipient"], str(d["sats"]), deposit_outpoint]
        if self.production:
            mint_params.extend(
                [
                    current_allocation["consensus_allocation_id"],
        ...
        if (
            ...
            or not isinstance(prep.get("unsigned_tx_hex"), str)
            or not isinstance(prep.get("inputs"), list)
            or prep.get("excluded_issuer_prevouts") != exclusions
            or (
                self.production
                and (
                    prep.get("c1_allocation_id") != current_allocation["consensus_allocation_id"]
                    or prep.get("recipient") != d["recipient"]
                    or prep.get("mint_sats") != d["sats"]
                    or prep.get("deposit_outpoint") != deposit_outpoint
                    or prep.get("c1_script_pubkey_hex") != current_allocation["script_pubkey"]
                    or prep.get("c1_commitment_blind_hex") != current_allocation["commitment_blind"]
                )
            )
        ):
            raise RuntimeError("preparetokenmint returned a malformed transaction template")
        if set(exclusions).intersection(unsigned_input_outpoints(prep["unsigned_tx_hex"])):
            raise RuntimeError("preparetokenmint reused an excluded issuer prevout")
        ...
        if self.production:
            signer_allocation = {
                ...
                "consensus_allocation_id": current_allocation["consensus_allocation_id"],
    ...
    def run_once(self):
        if self.halted():
            warn(f"HALT present ({self.halt_path}) - not minting. Remove it to resume.")
            return
        if self.production and any(
            record.get("status") in ("minting", "minted") for record in self.ledger.values()
        ):
            warn(
                "legacy production mint ledger requires offline exact-effect "
        ...
        for oid in sorted(
            oid for oid, record in self.ledger.items() if record.get("status") == "effect_confirmed"
        ):
            try:
                if not self._advance_unresolved(oid, self.ledger[oid]):
                    return
            except Exception as exc:
                warn(f"{oid}: settled mint-effect reconciliation HOLD: {str(exc)[:240]}")
                return
        unfinished = sorted(
            oid
            for oid, record in self.ledger.items()
            if record.get("status") not in ("effect_confirmed", "minted")
        )
        if unfinished:
            if len(unfinished) > 1:
                warn(
                    "multiple unresolved mint carriers require operator review; "
        ...
        log(
            f"[mintd] issuer={self.issuer_addr} K_BTC={self.K_BTC} eligible_deposits={len(deps)} "
        ...
        for d in deps:
            ...
            try:
                ...
                warn(
                    f"{outpoint_id(d['txid'], d['vout'])}: {type(e).__name__}: {e} - left unminted"
                )
                record = self.ledger.get(outpoint_id(d["txid"], d["vout"]))
                if isinstance(record, dict):
                    warn(
                        "%s: exact carrier remains in durable %s state"
                        % (outpoint_id(d["txid"], d["vout"]), record.get("status"))
                    )
            if self.halted():
                warn("HALT became active during this pass - stopping immediately")
                break
