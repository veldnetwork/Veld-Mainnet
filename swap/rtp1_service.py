...
def configuration(value):
    ...
    if (
        pins["disposable"] is not True
        or pins["external_value"] is not False
        or value["bitcoin_network"] != "regtest"
    ):
    ...
    if (
        ...
        or not re.fullmatch(r"V[1-9A-HJ-NP-Za-km-z]{25,49}", value["issuer"])
        or type(value["witness_id"]) is not str
        or not re.fullmatch(r"[A-Za-z0-9._-]{1,128}", value["witness_id"])
        or type(value["custody_script_hex"]) is not str
        or not re.fullmatch(r"5120[0-9a-f]{64}", value["custody_script_hex"])
        or type(value["minimum_confirmations"]) is not int
        or not 144 <= value["minimum_confirmations"] <= 1000000
    ):
...
def mint_policy(config, request):
    return {
        "expected_chain": config["expected_chain"],
...
def request_identity(request):
    if (
        ...
        or set(request) != {"action", "unsigned_tx_hex", "recipient", "sats"}
        or request["action"] != "rtp1_mint"
    ):
        raise ValueError("RTP1 mint request schema is invalid")
    raw = _template_bytes(request["unsigned_tx_hex"])
    if (
        type(request["recipient"]) is not str
        or not re.fullmatch(r"V[1-9A-HJ-NP-Za-km-z]{25,49}", request["recipient"])
        or type(request["sats"]) is not int
        or not 0 < request["sats"] <= 1000000000
    ):
...
def intent_from_evidence(config, request, evidence):
    ...
    if (
        type(evidence["version"]) is not int
        or evidence["version"] != 1
        or evidence["kind"] != "rtp1-independent-backing"
        or evidence["evidence_sha256"]
        != hashlib.sha256(b"VELD/RTP1/BACKING/v1\x00" + canonical(body)).hexdigest()
    ):
        raise ValueError("independent backing evidence binding is invalid")
    native = validate_inspection(
        evidence["native"], request["unsigned_tx_hex"], **mint_policy(config, request)
    )
    facts = validate_backing_facts(evidence["transition"])
    bitcoin = evidence["bitcoin"]
    if (
        type(bitcoin) is not dict
        or bitcoin.get("genesis_hash") != config["bitcoin_genesis"]
        or bitcoin.get("network") != config["bitcoin_network"]
        or type(bitcoin.get("confirmations")) is not int
        or bitcoin["confirmations"] < config["minimum_confirmations"]
        or facts["custody_script_hex"] != config["custody_script_hex"]
        or facts["sats"] != native["sats"]
        or facts["recipient"] != native["recipient"]
        or facts["issuer"] != native["issuer"]
        or facts["deposit_outpoint"] != native["deposit_outpoint"]
        or facts["bitcoin_txid"] != bytes.fromhex(native["bitcoin_txid"])[::-1].hex()
        or facts["bitcoin_block"] != bytes.fromhex(native["bitcoin_block"])[::-1].hex()
        or bitcoin.get("reserve_outpoint")
        != facts["bitcoin_txid"] + ":" + str(facts["reserve_vout"])
        or bitcoin.get("reserve_value_sats") != facts["reserve_value_sats"]
        or bitcoin.get("custody_script_hex") != facts["custody_script_hex"]
    ):
...
def validate_receipt(receipt, config, request, evidence, verify):
    ...
    if (
        ...
        or receipt["kind"] != "VELD_RTP1_RESERVATION"
        or receipt["policy_id"] != digest(config)
        or receipt["witness_id"] != config["witness_id"]
        or receipt["request_id"] != rid
        or type(receipt["sequence"]) is not int
        or not 1 <= receipt["sequence"] <= MAX_ROWS
        or receipt["intent"] != intent_from_evidence(config, request, evidence)
        or receipt["sig_alg"] != "mldsa65"
        or type(receipt["sig"]) is not str
        or not re.fullmatch(r"[0-9a-f]{6618}", receipt["sig"])
    ):
...
def empty_journal(role, config, migration):
    ...
    return {
        "version": 1,
        "kind": "VELD_RTP1_JOURNAL",
        "role": role,
        "policy_id": digest(config),
        "migration": migration,
        "records": [],
        "head": ZERO,
    }
...
def signed_identity(signed, unsigned):
    if (
        ...
        or not 2 <= len(signed) <= 2 * MAX_SIGNED_BYTES
        or len(signed) % 2
        or not re.fullmatch(r"[0-9a-f]+", signed)
        or unsigned_template_from_signed_hex(signed) != unsigned
    ):
...
class Journal:
    ...
    def validate(self):
        ...
        if (
            type(s) is not dict
            or set(s) != {"version", "kind", "role", "policy_id", "migration", "records", "head"}
            or type(s["version"]) is not int
            or s["version"] != 1
            or s["kind"] != "VELD_RTP1_JOURNAL"
            or s["role"] != self.role
            or s["policy_id"] != digest(self.config)
            or type(s["records"]) is not list
            or len(s["records"]) > MAX_ROWS
            or len(canonical(s)) > MAX_BYTES
        ):
        ...
        for seq, row in enumerate(s["records"], 1):
            ...
            receipt = validate_receipt(
                row["receipt"], self.config, row["request"], row["evidence"], self.verify_receipt
            )
            ...
            if (
                rid in requests
                or deposit in deposits
                or edge in edges
                or receipt["sequence"] != seq
                or receipt["previous"] != head
            ):
            ...
            if type(row["committed"]) is not bool or (
                row["committed"] and row["signed_tx_hex"] is None
            ):
                raise ValueError("RTP1 committed liability has no exact signed bytes")
            conf = row["confirmation"]
            if conf is not None:
                if (
                    type(conf) is not dict
                    or set(conf) != {"height", "hash"}
                    or type(conf["height"]) is not int
                    or conf["height"] <= 0
                    or not row["committed"]
                ):
    ...
    def commit(self, request, signed, committed):
        ...
        return {
            "version": 1,
            "kind": "VELD_RTP1_COMMIT",
            "policy_id": digest(self.config),
            "request_id": request_identity(request),
            "receipt_sha256": digest(row["receipt"]),
...
def witness_reserve(journal, request, inspect, sign_receipt):
    ...
    core = {
        "version": 1,
        "kind": "VELD_RTP1_RESERVATION",
        "policy_id": digest(journal.config),
        "witness_id": journal.config["witness_id"],
        "request_id": request_identity(request),
        "sequence": len(journal.state["records"]) + 1,
        "previous": journal.state["head"],
        "intent": intent,
    }
...
def check_witness_status(snapshot, row):
    if (
        ...
        or set(snapshot) != {"receipt", "committed", "signed_tx_hex"}
        or snapshot["receipt"] != row["receipt"]
        or type(snapshot["committed"]) is not bool
        or (snapshot["signed_tx_hex"] is not None) != snapshot["committed"]
    ):
