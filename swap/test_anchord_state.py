...
class AnchorInfoVeld:
    def __init__(self, high_water=0, final_height=100,
                 anchor_active=True, finality_active=True):
    ...
    def rpc(self, method, params=None):
        if method == "getanchorinfo":
            return {"anchor_active": self.anchor_active,
                    ...
                    "high_water": self.high_water, "k_btc": 6}
        raise AssertionError(method)
...
class TemplateBtc:
    ...
    def call(self, method, *args):
        ...
        if method == "getaddressinfo":
            return {"scriptPubKey": fund_spk, "ismine": True,
                    "solvable": True, "iswatchonly": False}
        if method == "listunspent":
            return [{"txid": "22" * 32, "vout": 7,
        ...
        if method == "decoderawtransaction":
            ...
            change = next(iter(outputs[1].values()))
            return {"txid": self.txid, "vsize": self.vsize,
                    "vin": [{"txid": "22" * 32, "vout": 7}],
                    "vout": [
                        {"value": Decimal(0), "scriptPubKey": {
                            "hex": (b"\x6a" + bytes([len(data)]) + data).hex()}},
...
class FreshnessBtc:
    ...
    def call(self, method, *args):
        if method == "getblockchaininfo":
            return {"chain": self.chain, "initialblockdownload": self.ibd,
                    "blocks": self.blocks, "headers": self.headers,
                    "bestblockhash": self.best}
        if method == "getblockheader":
            self.assert_args = args
            return {"hash": self.best, "height": self.blocks,
                    "confirmations": 1, "time": 100_000 - self.age}
        raise AssertionError(method)
...
class AnchorStateTests(unittest.TestCase):
    ...
    def test_bitcoin_tip_freshness_rejects_wrong_chain_ibd_and_header_lag(self):
        for btc in (FreshnessBtc(1, chain="test"),
                    FreshnessBtc(1, ibd=True),
                    FreshnessBtc(1, headers=101)):
            a = object.__new__(anchord.Anchor)
            a.production = True; a.btc = btc
    ...
    def make_anchor(self, td):
        ...
        a.state = {"anchors": {"10": {
            "status": "committed", "btc_txid": "ab" * 32,
            "veld_hash": "cd" * 32}}}
    ...
    def test_no_bitcoin_spend_before_real_validator_finality(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            a.btc_feerate_sat_vb = lambda: self.fail(
                "pre-finality Bitcoin fee quote attempted")
    ...
    def test_new_anchor_target_is_capped_at_validator_final_height(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            a.veld_block_hash = lambda height: (
                self.assertEqual(height, 90) or "44" * 32)
            prepared = []
            a.prepare_btc_commit = lambda height, vhash, fee, rate: (
                prepared.append(height) or {
                    "veld_hash": vhash, "btc_txid": "ab" * 32,
                    "btc_tx_hex": "0100000000", "status": "prepared",
                    "at": 1, "fee_sats": fee, "feerate_sat_vb": str(rate)})
    ...
    def test_inconsistent_active_rpc_without_finality_fails_closed(self):
        a = object.__new__(anchord.Anchor)
        a.veld = AnchorInfoVeld(final_height=0, anchor_active=True,
    ...
    def test_prepare_builds_exact_change_fee_and_legacy_opreturn(self):
        a = object.__new__(anchord.Anchor)
        a.btc = TemplateBtc(); a.btc_fund_addr = "1LegacyAddress"
        a.max_fee_sats = 20_000; a.max_vsize = 400
        a.fee_ceiling_svb = Decimal(50)
    ...
    def test_prepare_rejects_actual_signed_vsize_above_hard_ceiling(self):
        a = object.__new__(anchord.Anchor)
        a.btc = TemplateBtc(vsize=401); a.btc_fund_addr = "1LegacyAddress"
        a.max_fee_sats = 20_000; a.max_vsize = 400
        a.fee_ceiling_svb = Decimal(50)
    ...
    def test_prepared_record_is_fsynced_before_first_broadcast_and_resumes_exactly(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            a.production = True; a.k_btc = 6
            a.veld_tip = lambda: (100, True)
            a.btc_tip_age = lambda: 0
            a.tip_age_alert_secs = 3600; a.max_tip_age_secs = 7200
            a.relay_resubmit_secs = 3600; a.min_spacing = 480
            a.deadline_blocks = 960; a.cfg = {}
            a.fee_cheap_svb = Decimal(5); a.fee_ceiling_svb = Decimal(50)
            a.btc_feerate_sat_vb = lambda: Decimal(1)
            a.commit_fee_sats = lambda _: 400
            ...
            a.prepare_btc_commit = lambda *args: {
                "veld_hash": "44" * 32, "btc_txid": txid, "btc_tx_hex": raw,
                "status": "prepared", "at": 1, "fee_sats": 400,
                "feerate_sat_vb": "1"}
            ...
            with self.assertRaisesRegex(RuntimeError, "lost send"):
                a.run_once()
            self.assertEqual(anchord.load_anchor_state(a.state_path)["anchors"]["100"]["status"],
                             "prepared")
    ...
    def test_c5_boundaries_gate_only_new_anchor_admission(self):
        with tempfile.TemporaryDirectory() as td:
            for age, admitted in ((3599, True), (3600, True), (7199, True),
                                  (7200, False), (7201, False)):
                a = self.make_anchor(td)
                a.state = {"anchors": {}}
...
class AnchorProductionConfigTests(unittest.TestCase):
    def setUp(self):
        ...
        self.bitcoin_dir = self.root / "bitcoin"; self.bitcoin_dir.mkdir(mode=0o700)
        self.veld_dir = self.root / "veld"; self.veld_dir.mkdir(mode=0o700)
        ...
        self.keyfile.write_bytes(b"encrypted-test-key"); os.chmod(self.keyfile, 0o600)
    ...
    def config(self):
        return {
            "production": True, "state_dir": str(self.state),
            "cli_base": [str(self.bitcoin), "-datadir=" + str(self.bitcoin_dir)],
            "btc_wallet": "veld-anchor-fees", "btc_rpc_timeout": 30,
            "veld_rpc": {"url": "http://127.0.0.1:8334",
                         "token_cmd": [str(self.node), "--datadir=" + str(self.veld_dir),
                                       "--print-rpc-token"]},
            ...
            "k_btc": 6, "relay_resubmit_timeout_secs": 3600,
            "anchor_tx_vsize": 400, "fee_conf_target": 6,
            "fee_fallback_sat_vb": 20, "max_fee_sats": 20000,
            "anchor_deadline_blocks": 960,
            "anchor_min_spacing_blocks": 480,
            "fee_cheap_sat_vb": 5, "fee_ceiling_sat_vb": 50,
    ...
    def test_production_policy_is_pinned_and_anchor_lag_is_retired(self):
        for field, bad in (("k_btc", 5),
            ...
            cfg = self.config(); cfg[field] = bad
            with self.assertRaises(RuntimeError, msg=field):
                anchord.validate_config(cfg)
        cfg = self.config(); cfg["anchor_lag"] = 2
    ...
    def test_state_dir_and_state_file_fail_closed(self):
        ...
        os.chmod(path, 0o600); os.chmod(self.state, 0o755)
    ...
    def test_shipped_service_config_and_runbook_are_launch_bound(self):
        ...
        for marker in ("User=veldanchor", "StateDirectory=veld-anchord",
                       "StateDirectoryMode=0700", "ProtectSystem=strict",
                       "UMask=0077", "--loop 30"):
        ...
        approved = {
            ...
            "max_fee_sats": 20000, "anchor_deadline_blocks": 960,
            "anchor_min_spacing_blocks": 480, "fee_cheap_sat_vb": 5,
            "fee_ceiling_sat_vb": 50, "tip_age_alert_secs": 3600,
        ...
        for marker in ("production launch policy", "prepared", "fsync",
                       "getanchorinfo.high_water", "actual signed vsize",
                       "recovery-only",
