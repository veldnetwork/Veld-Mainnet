...
def row(at=0, amount=10_000, principal="p", destination="Vdest", *, funded=None, expires=604800):
    return {
        "admitted_at": at,
        "amount_sats": amount,
        "principal_hash": principal,
        "veld_address": destination,
        "deposit_observed_at": funded,
        "funded_reserved_at": funded,
        "expires_at": expires,
    }
...
class C1PolicyTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as root:
            ...
            with self.assertRaises(SystemExit):
                witness._load_allocation_ledger(
                    str(path),
                    cfg,
                    {"range": [0, 11000], "script_pubkeys": tuple("" for _ in range(11001))},
                    raised,
                    new_hash,
                )
            with self.assertRaisesRegex(RuntimeError, "C1_RANGE_ROTATION_REQUIRED"):
    ...
    def test_minimum_and_sat_ceiling_boundaries(self):
        ...
        self.assertIn("minimum", witness._c1_capacity_reason(ledger, below, approved, 0))
        self.assertIsNone(witness._c1_capacity_reason(ledger, candidate, approved, 0))

        for total, rejected in (
            (999_999_999, False),
            (1_000_000_000, False),
            (1_000_000_001, True),
        ):
            prior = total - 10_000
            principal_ledger = {
                "allocations": [row(amount=prior, principal="candidate", destination="Vother")]
            }
            reason = witness._c1_capacity_reason(principal_ledger, candidate, approved, 0)
            self.assertEqual(reason is not None, rejected, (total, reason))
            destination_candidate = dict(candidate, principal_hash="different")
            destination_ledger = {
                "allocations": [row(amount=prior, principal="other", destination="Vnew")]
            }
    ...
    def test_fresh_headroom_charges_active_nonminted_allocations_once(self):
        approved = policy()
        first = dict(row(amount=62_500, principal="p1", destination="V1"), minted_at=None)
        second = dict(row(amount=62_500, principal="p2", destination="V2"), minted_at=None)
        self.assertIn(
            "headroom",
            witness._c1_capacity_reason(
                {"allocations": [first]},
                second,
                approved,
                0,
        ...
        self.assertIsNone(
            witness._c1_capacity_reason(
                {"allocations": [first]},
                dict(row(amount=10_000, principal="p3", destination="V3"), minted_at=None),
                approved,
                0,
                issuer_headroom_sats=37_500,
        ...
        first["minted_at"] = 10
        residual = dict(row(amount=10_000, principal="p3", destination="V3"), minted_at=None)
        self.assertIsNone(
            witness._c1_capacity_reason(
                {"allocations": [first]},
                residual,
    ...
    def test_epoch_and_day_boundaries(self):
        approved = policy()
        candidate = row(at=60_000, principal="candidate", destination="Vnew", expires=700_000)
        for total, rejected in ((4, False), (5, False), (6, True)):
            existing = total - 1
            ledger = {
                "allocations": [
                    row(at=60_000, principal="p%d" % i, destination="V%d" % i, expires=700_000)
                    for i in range(existing)
                ]
            }
            reason = witness._c1_capacity_reason(ledger, candidate, approved, 60_000)
            self.assertEqual(reason is not None, rejected, (total, reason))

        for total, rejected in ((199, False), (200, False), (201, True)):
            existing = total - 1
            rows = [
                row(at=(i // 4) * 600, principal="p%d" % i, destination="V%d" % i, expires=700_000)
                for i in range(existing)
            ]
            candidate_day = row(
                at=80_000, principal="candidate", destination="Vnew", expires=700_000
            )
            reason = witness._c1_capacity_reason(
    ...
    def test_day_seven_releases_unfunded_quota_but_not_funded_quota(self):
        approved = policy()
        candidate = row(at=604_800, principal="same", destination="Vnew", expires=1_209_600)
        expired = row(amount=1_000_000_000, principal="same", destination="Vold", expires=604_800)
        self.assertIsNone(
            witness._c1_capacity_reason({"allocations": [expired]}, candidate, approved, 604_800)
        )
        funded = dict(expired, deposit_observed_at=604_799, funded_reserved_at=604_799)
        self.assertIn(
            "principal",
            witness._c1_capacity_reason({"allocations": [funded]}, candidate, approved, 604_800),
        )
    ...
    def test_storage_reserve_boundaries(self):
        ...
        self.assertTrue(
            witness._c1_storage_allows(
                89_999, max_bytes * 9 // 10 - 1, approved, new_admission=True
            )
        )
        self.assertFalse(witness._c1_storage_allows(90_000, 1, approved, new_admission=True))
        self.assertTrue(
            witness._c1_storage_allows(95_000, max_bytes * 95 // 100, approved, new_admission=False)
        )
        self.assertFalse(witness._c1_storage_allows(100_000, 1, approved, new_admission=False))

    def test_current_beacon_signature_tuple_and_rotation(self):
        authority = swap_admission.AdmissionBeacon(random_bytes=lambda count: b"r" * count)
        request = {
            "request_id": "aa" * 16,
            "veld_address": "V" + "1" * 33,
            "amount_sats": 10_000,
        ...
        with (
            ...
            principal = wrapd._verify_admission_pow(
                request, now=1, signature_verifier=lambda key, message, signature: True
            )
            self.assertEqual(
                principal,
                __import__("hashlib")
                .sha256(bytes.fromhex(request["admission_public_key"]))
                .hexdigest(),
            )
            with self.assertRaisesRegex(ValueError, "not current"):
                wrapd._verify_admission_pow(
                    request, now=600, signature_verifier=lambda key, message, signature: True
                )
    ...
    def test_c5_and_peg_unlock_phases_fail_closed(self):
        ...
        self.assertEqual(wrapd._public_allocation_gate({}, {"phase": "FRESH"}), "PEG_LOCKED")
        peg = {"peg_unlocked": True, "mint_live": True}
        for age in (3_599, 3_600, 7_199):
            self.assertEqual(wrapd._public_allocation_gate(peg, observed[age]), "OPEN")
        for age in (7_200, 7_201):
            self.assertEqual(wrapd._public_allocation_gate(peg, observed[age]), "RECOVERY_ONLY")
    ...
    def test_policy_hash_mismatch_and_worm_ack_fail_closed(self):
        ...
        completed = subprocess.CompletedProcess([], 0, stdout=json.dumps(wrong), stderr="")
        with (
            mock.patch.object(wrapd, "ALLOCATION_WITNESS_COMMAND", ("/bin/x",)),
            mock.patch.object(wrapd, "_C1_POLICY_SHA256", "a" * 64),
        ...
        ledger = {"last_consensus_sequence": 0, "allocations": [], "events": []}
        bad_ack = subprocess.CompletedProcess([], 0, stdout=json.dumps({"version": 1}), stderr="")
        with (
            mock.patch.object(witness, "run_bounded_subprocess", return_value=bad_ack),
    ...
    def test_real_allocator_uses_public_range_and_five_per_epoch(self):
        ...
        binding = {"descriptor": "tr(c1)#test", "range": [0, 10999], "script_pubkeys": scripts}
        labels = {}
        chain_sequence = [0]
        ...
        def btc_json(method, *args):
            ...
            if method == "deriveaddresses":
                first, last = [int(value) for value in args[1].strip("[]").split(",")]
                return [address(index) for index in range(first, last + 1)]
            if method == "getaddressinfo":
                index = int(args[0][-8:])
                return {
                    "scriptPubKey": scripts[index],
                    "iswatchonly": True,
                    "solvable": True,
                    "labels": ([{"name": labels[args[0]]}] if args[0] in labels else []),
                }
            raise AssertionError(method)
        ...
        def checkpoint(*args, **kwargs):
            ...
            answer = {
                ...
                "last_consensus_sequence": request["last_consensus_sequence"],
                "monotonic_sequence": 1,
            }
            return subprocess.CompletedProcess([], 0, stdout=json.dumps(answer), stderr="")
        ...
        def sequence_status(allocation_id_value):
            return {"allocation_id": allocation_id_value, "last_sequence": chain_sequence[0]}

        with (
            tempfile.TemporaryDirectory() as directory,
            mock.patch.object(wrapd, "_btc_json", side_effect=btc_json),
            mock.patch.object(wrapd, "_btc", side_effect=btc_text),
            mock.patch.object(wrapd, "ALLOCATION_CHECKPOINT_COMMAND", ("/bin/checkpoint",)),
            mock.patch.object(wrapd, "run_bounded_subprocess", side_effect=checkpoint),
        ):
            journal = wrapd.WrapAllocationJournal(
                str(Path(directory) / "c1.json"),
                binding,
                None,
            ...
            for index in range(5):
                ...
                self.assertEqual(result["consensus_allocation_id"], "%032x" % (index + 1))
                self.assertIs(result["send_allowed"], True)
            with self.assertRaisesRegex(RuntimeError, "epoch"):
                journal.allocate("f" * 32, "e" * 64, "V" + "Z" * 25, 100_000)
            ...
            next_result = restarted.allocate("6" * 32, "6" * 64, "V" + "Y" * 25, 10_000)
            self.assertEqual(next_result["consensus_allocation_id"], "%032x" % 6)
