...
def heartbeat(supply=6 * BTC, custody=None, tip=100, tip_hash=TIP_HASH):
    ...
    return sol.stamp_heartbeat(sol.build_beat_payload(
        custody, supply, 0, tip, tip_hash, 1, 120), time.time())
...
def part_a():
    ...
    try:
        for f in ("veld_wt_recv.py", "veld_peg_solvency.py",
                  "rpc_url_policy.py"):
            shutil.copy(os.path.join(HERE, f), os.path.join(d, f))
        hbf = os.path.join(d, "signer-heartbeat.json")
        haltf = os.path.join(d, "HALT")

        payload = sol.build_beat_payload(10 * BTC, 6 * BTC, 0, tip=100,
                                         tip_hash=TIP_HASH, seq=1, ttl_secs=120)
        ...
        check("receiver stamped expiry from its clock",
              "expires_at" in hb and hb["expires_at"] > time.time())
        check("gate accepts mint within headroom",
              sol.heartbeat_gate(hb, time.time(), 0, 3 * BTC)[0])
        check("gate denies mint over headroom",
              not sol.heartbeat_gate(hb, time.time(), 0, 5 * BTC)[0])
        ...
        check("replay cannot refresh receiver-stamped expiry",
              open(hbf).read() == first_heartbeat)

        # Two forced-command processes racing the same next sequence serialize on
        # the durable lock; exactly one may publish it.
        payload2 = dict(payload); payload2["seq"] = 2
        ...
        check("concurrent duplicate beat has exactly one winner",
              sorted([p1.returncode, p2.returncode]) == [0, 2])
        check("durable sequence advanced exactly once",
              json.load(open(hbf))["seq"] == 2 and
              open(os.path.join(d, "watchtower-last-seq")).read().strip() == "2")
        ...
        payload3 = dict(payload); payload3["seq"] = 3
        ...
        check("oversized payload cannot advance heartbeat or durable sequence",
              open(hbf).read() == before_oversize and
              open(os.path.join(d, "watchtower-last-seq")).read().strip() == "2")
        ...
        lied = dict(payload); lied["headroom_sats"] = 9 * BTC
...
def part_b():
    ...
    try:
        ...
        beat = sol.stamp_heartbeat(
            sol.build_beat_payload(10 * BTC, 6 * BTC, 0, 100,
                                   TIP_HASH, 1, 120), time.time())
        ...
        state2 = {
            ...
            "mint_accounting": {
                ...
                "pending": [{"request_id": "f" * 64, "sats": 4 * BTC,
...
def part_c():
    ...
    check("legacy refusal does not silently mutate/delete audit rows",
    ...
    signed1 = "01" * 80; txid1 = txid_for(signed1)
    sd.record_signed_mint(state, "a" * 64, txid1, signed1,
                          2 * BTC, RECIPIENT, signed_at=20)
    used, _ = sd.reconcile_mint_accounting(state, hb, rpc)
    check("new signature remains outstanding before confirmation", used == 2 * BTC)
    rpc.transactions[txid1] = {
        "txid": txid1, "confirmations": 2, "block_height": 99,
        "block_hash": MINT_BLOCK_HASH,
    }
    used, _ = sd.reconcile_mint_accounting(
        state, heartbeat(supply=12 * BTC), rpc)
    check("exact canonical tx confirmation permits a back-to-back mint", used == 0)

    signed2 = "02" * 80; txid2 = txid_for(signed2)
    sd.record_signed_mint(state, "b" * 64, txid2, signed2,
                          3 * BTC, RECIPIENT, signed_at=21)
    used, _ = sd.reconcile_mint_accounting(
        state, heartbeat(supply=12 * BTC), rpc)
    check("back-to-back mint is charged exactly once", used == 3 * BTC)
    cached, incremental = sd.classify_mint_request(
        state, "b" * 64, 3 * BTC, RECIPIENT)
    check("retry adds zero rolling-window/headroom capacity",
          incremental == 0)
    check("retry replays byte-identical signed tx and txid",
          cached["signed_tx_hex"] == signed2 and
          txid_for(cached["signed_tx_hex"]) == cached["txid"] == txid2)
    inserted = sd.record_signed_mint(
        state, "b" * 64, txid2, signed2,
        3 * BTC, RECIPIENT, signed_at=22)
    used, _ = sd.reconcile_mint_accounting(
        state, heartbeat(supply=12 * BTC), rpc)
    ...
    used, _ = sd.reconcile_mint_accounting(
        state, heartbeat(supply=7 * BTC), rpc)
    used, _ = sd.reconcile_mint_accounting(
        state, heartbeat(supply=12 * BTC), rpc)
    ...
    used, _ = sd.reconcile_mint_accounting(
        state, heartbeat(supply=10 * BTC), rpc)
    check("pre-finality mint reorg returns exact tx to pending", used == 5 * BTC)
    rpc.transactions[txid1] = {
        "txid": txid1, "confirmations": sd.MAX_REORG_DEPTH,
        "block_height": 1, "block_hash": MINT_BLOCK_HASH,
    }
    used, _ = sd.reconcile_mint_accounting(
        state, heartbeat(supply=12 * BTC), rpc)
    check("mint at MAX_REORG_DEPTH remains under reorg watch",
          used == 3 * BTC and len(state["mint_accounting"]["confirmed"]) == 1)
    rpc.transactions[txid1]["confirmations"] = sd.MAX_REORG_DEPTH + 1
    used, _ = sd.reconcile_mint_accounting(
        state, heartbeat(supply=12 * BTC, tip=101,
                         tip_hash=MINT_BLOCK_HASH), rpc)
    check("mint deeper than MAX_REORG_DEPTH is pruned from reorg watch",
          used == 3 * BTC and not state["mint_accounting"]["confirmed"])

    constants = open(os.path.join(HERE, "..", "include", "core", "constants.h")).read()
    check("signer MAX_REORG_DEPTH matches consensus constant",
          ("MAX_REORG_DEPTH                = %d" % sd.MAX_REORG_DEPTH) in constants)
    import veld_wt_reserve as reserve
    check("witness MAX_REORG_DEPTH matches signer and consensus",
          reserve.MAX_REORG_DEPTH == sd.MAX_REORG_DEPTH and
          ("MAX_REORG_DEPTH                = %d" % reserve.MAX_REORG_DEPTH) in constants)
    ...
    for i in range(501):
        ...
        sd.record_signed_mint(volume, request_id, txid_for(signed_hex), signed_hex,
                              1, RECIPIENT, signed_at=int(time.time()))
    cached, increment = sd.classify_mint_request(
        volume, first_request[0], 1, RECIPIENT)
    check(">500 tiny mints remain fully counted in rolling window",
          len(volume["signed"]) == 501 and sd.window_signed_sats(volume) == 501)
    check("oldest of >500 requests retains exact replay bytes forever",
          increment == 0 and cached["signed_tx_hex"] == first_request[1] and
          cached["txid"] == first_request[2])
    ...
    try:
        ...
        signed3 = "03" * 80; txid3 = txid_for(signed3)
        sd.record_signed_mint(persisted, "c" * 64, txid3, signed3,
                              25, RECIPIENT, signed_at=30)
        ...
        outstanding, _ = sd.reconcile_mint_accounting(
            restarted, heartbeat(supply=100), rpc)
        ...
        check("durable signer state is owner-only",
...
def part_d():
    ...
    wt.veld = SupplyRpc({"supply_sats": 123, "tip": 456,
                         "tip_hash": TIP_HASH})
    check("tip-bound supply object is accepted exactly",
          wt.read_supply() == (123, 456, TIP_HASH))
