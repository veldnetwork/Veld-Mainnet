...
def receipt(tag, sats, recipient):
    ...
    return {
        "v": sol.RESERVATION_VERSION, "kind": sol.RESERVATION_KIND,
        "issuer_id": ISSUER, "witness_id": WITNESS,
        "request_id": request_id, "unsigned_tx_sha256": request_id,
        "reservation_id": sol.reservation_id(
            ISSUER, request_id, request_id, sats, recipient, None, None),
        "sats": sats, "recipient": recipient, "beat_seq": tag,
        "tip": 100, "tip_hash": "11" * 32, "headroom_sats": 1000,
        "allocation_verified": False, "allocation_request_id": None,
        "allocation_descriptor_index": None, "allocation_btc_address": None,
        "allocation_script_pubkey": None, "deposit_outpoint": None,
        "reserved_at": tag, "sig_alg": "mldsa65", "sig": "00",
    }
...
def c1_receipt(descriptor_index, range_end=10999):
    ...
    outpoint = "22" * 32 + ":0"
    return {
        "v": sol.C1_RESERVATION_VERSION, "kind": sol.RESERVATION_KIND,
        "issuer_id": ISSUER, "witness_id": WITNESS,
        "request_id": request_id, "unsigned_tx_sha256": request_id,
        "reservation_id": sol.reservation_id(
            ISSUER, request_id, request_id, 100_000, ADDR1,
            outpoint, allocation_request_id),
        "sats": 100_000, "recipient": ADDR1, "beat_seq": 9,
        "tip": 250, "tip_hash": "11" * 32, "headroom_sats": 1_000_000,
        ...
        "reserved_at": 9, "sig_alg": "mldsa65", "sig": "00",
    }
...
class WitnessRestoreReconciliationTests(unittest.TestCase):
    def setUp(self):
        ...
        self.pubkey = self.root / "beat.pub"; self.pubkey.write_text("fixture")
        self.pub_a = self.root / "a.pub"; self.pub_a.write_text("a")
        self.pub_b = self.root / "b.pub"; self.pub_b.write_text("b")
        self.policy = {"operators": [
            {"operator_id": "op-a", "pubkey_file": str(self.pub_a)},
            {"operator_id": "op-b", "pubkey_file": str(self.pub_b)},
        ]}
        self.policy_bytes = (json.dumps(
        ...
        ledger_path.write_bytes(self.ledger_bytes); ledger_path.chmod(0o600)
        self.terminal_bytes = witness._empty_terminal_log_text().encode("utf-8")
        terminal_path = Path(self.paths["terminal"])
        terminal_path.write_bytes(self.terminal_bytes); terminal_path.chmod(0o600)
        ...
        self.r2_txid = txid
        self.signer_state = {
            "signed": [{"request_id": self.r2["request_id"], "txid": txid,
                        "signed_tx_hex": signed, "sats": 20, "to": ADDR2,
                        "at": 1, "witness_receipt": self.r2,
                        "witness_committed": True}],
            "mint_accounting": {"version": signer.MINT_ACCOUNTING_VERSION,
                "pending": [{"request_id": self.r2["request_id"], "txid": txid,
                    "sats": 20, "signed_at": 1, "witness_receipt": self.r2,
                    "witness_committed": True}], "confirmed": []},
        }
        self.signer_path = self.root / "signer-state.json"
        self.signer_bytes = (json.dumps(
            self.signer_state, sort_keys=True, separators=(",", ":")) + "\n").encode()
        self.signer_path.write_bytes(self.signer_bytes); self.signer_path.chmod(0o600)
        self.artifact = {
            "version": restore.ARTIFACT_VERSION, "kind": restore.ARTIFACT_KIND,
            "restored_ledger_sha256": hashlib.sha256(self.ledger_bytes).hexdigest(),
            "restored_terminal_log_sha256": hashlib.sha256(
                self.terminal_bytes).hexdigest(),
            ...
            "ledger_snapshot_sha256s": [], "receipt_archive_sha256s": [],
            "source_inventory_complete": True,
            "issuer_id": ISSUER, "witness_id": WITNESS,
            "canonical_tip": {"height": 250, "hash": "22" * 32},
            "supply_sats": 30, "custody_sats": 100, "margin_sats": 10,
            "wait_evidence": {"signer_halted": True, "minter_halted": True,
                "witness_halted": True, "mint_mempool_empty": True,
                "minter_queue_empty": True, "quiesced_height": 100,
                "observed_height": 250, "observed_tip_hash": "22" * 32},
            "reservations": [
                {"reservation_id": self.r1["reservation_id"],
                 "disposition": "outstanding"},
                {"reservation_id": self.r2["reservation_id"],
                 "disposition": "canonical_final", "txid": txid,
                 "block_height": 100, "block_hash": "33" * 32},
            ],
        }
        statement_sha = hashlib.sha256(
            legacy.canonical_statement(self.artifact)).hexdigest()
        self.artifact["approvals"] = [
            {"operator_id": "op-a", "statement_sha256": statement_sha,
             "signature_hex": "00"},
            {"operator_id": "op-b", "statement_sha256": statement_sha,
             "signature_hex": "01"},
        ]
    ...
    def test_c1_receipt_policy_boundaries_restart_and_tombstone_reload(self):
        ...
        signer_allocation = {
            ...
            "consensus_allocation_id":
                active_receipt["allocation_request_id"],
            ...
            "capacity_policy_sha256":
                active_receipt["allocation_capacity_policy_sha256"],
        ...
        with mock.patch.object(signer, "_secure_text", return_value="00"), \
             mock.patch.object(signer, "run_bounded_subprocess",
                               return_value=mock.Mock(returncode=0)):
            signer.verify_reservation_receipt(
                active_receipt, ISSUER, {"witness_id": WITNESS},
                active_receipt["request_id"],
                active_receipt["unsigned_tx_sha256"],
                active_receipt["sats"], active_receipt["recipient"],
                heartbeat, signer_allocation)
            for field, value in (
                ...
                with self.subTest(signer_tamper=field):
                    ...
                    with self.assertRaises(ValueError):
                        signer.verify_reservation_receipt(
                            tampered, ISSUER, {"witness_id": WITNESS},
                            tampered["request_id"],
                            tampered["unsigned_tx_sha256"],
                            tampered["sats"], tampered["recipient"],
                            heartbeat, signer_allocation)

        ledger = witness._empty_ledger()
        ledger["reservations"] = [
            {"receipt": active_receipt, "status": "reserved"}]
        witness.atomic_write(
            self.paths["ledger"], json.dumps(
                ledger, sort_keys=True, separators=(",", ":")) + "\n")
        restarted = witness._load_ledger(self.paths["ledger"])
        self.assertEqual(
            restarted["reservations"][0]["receipt"]
            ["allocation_descriptor_index"], 10999)

        entry = {
            "receipt": active_receipt, "status": "final",
            "txid": "33" * 32, "signed_tx_sha256": "44" * 32,
            "block_height": 100, "block_hash": "55" * 32,
        }
        beat = {"tip": 250, "tip_hash": "66" * 32}
        tombstone = witness._make_tombstone(
            entry, beat, 1, witness.ZERO_HASH, "77" * 32)
        ...
        tampered_tombstone["record_sha256"] = hashlib.sha256(
            witness._tombstone_payload(tampered_tombstone)).hexdigest()
        self.assertFalse(witness._same_tombstone_identity(
            active_receipt, tampered_tombstone))
        terminal_text = (witness._empty_terminal_log_text() +
    ...
    def test_union_two_approved_finality_and_atomic_apply(self):
        ...
        operators, ignored = legacy.verify_approvals(
            self.artifact, self.policy, str(self.keygen))
        ...
        backup, terminal_backup, audit = restore.atomic_apply(
            self.paths, self.ledger_bytes, self.terminal_bytes,
            result, artifact_sha)
    ...
    def test_exact_depth_missing_union_and_insolvency_refuse(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "strictly beyond"):
            restore.validate_artifact(
                bad, hashlib.sha256(self.ledger_bytes).hexdigest(),
                hashlib.sha256(self.terminal_bytes).hexdigest(),
                [hashlib.sha256(self.signer_bytes).hexdigest()], inventory, [])
        bad = copy.deepcopy(self.artifact); bad["reservations"] = bad["reservations"][:1]
        with self.assertRaisesRegex(RuntimeError, "exact receipt union"):
            restore.validate_artifact(
                bad, hashlib.sha256(self.ledger_bytes).hexdigest(),
                hashlib.sha256(self.terminal_bytes).hexdigest(),
                [hashlib.sha256(self.signer_bytes).hexdigest()], inventory, [])
        bad = copy.deepcopy(self.artifact); bad["custody_sats"] = 39
    ...
    def test_signed_bytes_conflicts_and_malformed_ledger_refuse(self):
        ...
        legacy_v2 = {"version": 2, "reservations": copy.deepcopy(
            self.ledger["reservations"])}
    ...
    def test_immutable_snapshot_and_receipt_archive_extend_union(self):
        ...
        snapshot["reservations"] = [
            {"receipt": r3, "status": "reserved"}]
        archive = {"version": 1, "kind": "VELD_MINT_RECEIPT_ARCHIVE",
                   "entries": [{"receipt": r4}]}
        inventory = restore.collect_inventory(
            self.ledger, [("mint-primary", self.signer_state)],
            str(self.keygen), str(self.pubkey),
            ledger_snapshots=[("worm-ledger", snapshot)],
            receipt_archives=[("worm-receipts", archive)])
        self.assertEqual(set(inventory), {
            self.r1["reservation_id"], self.r2["reservation_id"],
            r3["reservation_id"], r4["reservation_id"]})
    ...
    def test_terminal_tombstone_requires_exact_worm_object_and_stays_compact(self):
        ...
        signed_bytes = bytes.fromhex(signed)
        txid = hashlib.sha256(hashlib.sha256(signed_bytes).digest()).hexdigest()
        entry = {"receipt": self.r1, "status": "final", "txid": txid,
        ...
        archive_sha = hashlib.sha256(
            witness._canonical_json_bytes(terminal_archive)).hexdigest()
        tombstone = witness._make_tombstone(
            entry, beat, 1, witness.ZERO_HASH, archive_sha)
        terminal_bytes = (witness._empty_terminal_log_text() + json.dumps(
            tombstone, sort_keys=True, separators=(",", ":")) + "\n").encode()
        ...
        with self.assertRaisesRegex(RuntimeError, "full receipt"):
            restore.collect_inventory(
                ledger, [], str(self.keygen), str(self.pubkey),
                terminal_tombstones=terminals)
        inventory = restore.collect_inventory(
            ledger, [], str(self.keygen), str(self.pubkey),
            receipt_archives=[("worm-terminal", terminal_archive)],
            terminal_tombstones=terminals)
        self.assertEqual(inventory[self.r1["reservation_id"]]["tombstone"],
                         tombstone)
        ...
        artifact.update({
            ...
            "reservations": [{"reservation_id": self.r1["reservation_id"],
                              "disposition": "canonical_final", "txid": txid,
                              "block_height": 100, "block_hash": "33" * 32}],
        })
        result = restore.validate_artifact(
            artifact, hashlib.sha256(ledger_bytes).hexdigest(),
            hashlib.sha256(terminal_bytes).hexdigest(), [], inventory, terminals)
