...
class MintStateReconciliationTests(unittest.TestCase):
    def setUp(self):
        ...
        self.state.write_bytes(self.old_bytes); self.state.chmod(0o600)
        (self.root / "HALT").write_text("operator halt\n")
        entries = signer._legacy_pending_entries(self.old)
        self.artifact = {
            "version": 1, "kind": reconcile.ARTIFACT_KIND,
            "old_state_sha256": hashlib.sha256(self.old_bytes).hexdigest(),
            "canonical_tip": {"height": 250, "hash": "11" * 32},
            "supply_sats": 100, "custody_sats": 200, "margin_sats": 10,
            "wait_evidence": {"signer_halted": True, "minter_halted": True,
                "mint_mempool_empty": True, "minter_queue_empty": True,
                "witness_restore_halt_armed": True, "quiesced_height": 100,
                "observed_height": 250, "observed_tip_hash": "11" * 32},
            "resolved_requests": [{"request_id": entry["request_id"],
                "sats": entry["sats"], "disposition": "canonical_final",
                "txid": ("%02x" % (index + 1)) * 32,
                "block_height": 100 + index, "block_hash": "22" * 32}
                for index, entry in enumerate(entries)],
            "omitted_journal_floor": {"sats": 0,
                "disposition": "resolved_inventory", "evidence_sha256": "33" * 32},
        }
        statement_sha = hashlib.sha256(
            reconcile.canonical_statement(self.artifact)).hexdigest()
    ...
    def validate(self, artifact=None):
        ...
        operators, ignored = reconcile.verify_approvals(
            artifact, self.policy, str(self.keygen))
        return reconcile.transform(self.old, artifact, "44" * 32,
                                   operators, inventory)
    ...
    def test_two_approved_transform_retains_tombstones_and_atomic_backup(self):
        ...
        self.assertEqual(applied["legacy_reconciliation"]["operator_ids"],
                         ["op-a", "op-b"])

    def test_hash_wait_approval_and_unresolved_fail_closed(self):
        bad = copy.deepcopy(self.artifact); bad["old_state_sha256"] = "00" * 32
        with self.assertRaisesRegex(RuntimeError, "exact old"):
            reconcile.validate_evidence(
        ...
        with self.assertRaisesRegex(RuntimeError, "strictly beyond"):
            reconcile.validate_evidence(
                bad, hashlib.sha256(self.old_bytes).hexdigest(), self.old)
        bad = copy.deepcopy(self.artifact); bad["approvals"] = bad["approvals"][:1]
        ...
        with self.assertRaisesRegex(RuntimeError, "final or provably invalidated"):
            inventory = reconcile.validate_evidence(
                bad, hashlib.sha256(self.old_bytes).hexdigest(), self.old)
            reconcile.transform(self.old, bad, "44" * 32,
                                ["op-a", "op-b"], inventory)
