...
class CanonicalParentTests(unittest.TestCase):
    def setUp(self):
        ...
        self.block = "12" * 32
        self.answer = {"txid": self.txid, "block_height": 100,
            "block_hash": self.block, "raw_hex": self.raw}
        self.calls = []

    def call(self, method, params):
        self.calls.append((method, params))
        if method == "getblockhash": return self.block
        if method == "gettransaction": return dict(self.answer)
    ...
    def test_exact_old_parent_is_fetched_without_recent_history_search(self):
        ...
        self.assertEqual(self.calls, [("getblockhash", [100]),
            ("gettransaction", [self.txid, 100]), ("getblockhash", [100])])

    def test_parent_location_and_raw_identity_are_independently_bound(self):
        for field, value in (("txid", "ab" * 32), ("block_height", 101),
                ("block_height", True), ("block_hash", "34" * 32), ("raw_hex", "abcd00")):
    ...
    def test_reorganization_during_fetch_is_refused(self):
        def changed(method, params):
            ...
            if method == "gettransaction": self.block = "34" * 32
            return result
