...
def staged_evidence(unsigned):
    return {
        "version": 2,
        "prepared": {"unsigned_tx_hex": unsigned},
        "authorization": {
            "operation_type": "BTCVELD_C1_RESERVE",
            "recipient": RECIPIENT,
            "amount": 10000,
...
def reservation_status(
    ...
    *,
    ...
    return {
        ...
        "exposure_canonical_depth_reached": exposed and exposure_confirmations >= 101,
...
def unconsumed_status(root="55" * 32, count=7, proof_hex="00" * 32, tip=200):
    return {
        "outpoint": OUTPOINT,
        "consumed": False,
        "minted": False,
...
class SignerBoundaryTests(unittest.TestCase):
    ...
    def boundary(self, status, phase, funding=None, retry_txid=None, mint=None, claim=None):
        with mock.patch.object(
            signerd,
            "validate_compiled_mint_policy",
            side_effect=[self.peg(status["tip"]), self.peg(status["tip"])],
        ):
            return signerd.validate_c1_signing_boundary(
                ...
                phase,
                funding=funding,
                retry_txid=retry_txid,
            )

    def test_exposure_depth_100_101_and_funding_window_edges(self):
        with self.assertRaisesRegex(ValueError, "exposure authority"):
            self.boundary(reservation_status(exposed=False, reserve_confirmations=100), "EXPOSE")
        self.assertTrue(
            self.boundary(reservation_status(exposed=False, reserve_confirmations=101), "EXPOSE")
        )

        funding = self.funding()
        with self.assertRaisesRegex(ValueError, "outside"):
            self.boundary(reservation_status(tip=199), "FUND", funding)
        self.assertTrue(self.boundary(reservation_status(tip=200), "FUND", funding))
        self.assertTrue(self.boundary(reservation_status(tip=299), "FUND", funding))
        with self.assertRaisesRegex(ValueError, "outside"):
    ...
    def test_fund_parent_change_and_depth_100_reorg_refuse(self):
        ...
        with self.assertRaisesRegex(ValueError, "proof parent"):
            self.boundary(
                reservation_status(), "FUND", funding, mint=unconsumed_status(root="66" * 32)
            )
        with self.assertRaisesRegex(ValueError, "deep exposed"):
            self.boundary(reservation_status(exposure_confirmations=100), "FUND", funding)
    ...
    def test_every_c1_phase_binds_status_tip_to_both_capacity_samples(self):
            ...
            with (
                ...
                with self.assertRaisesRegex(ValueError, "tip differs"):
                    signerd.validate_c1_signing_boundary(
                        Rpc(reservation_status(tip=201)),
                        ISSUER,
                        allocation(),
                        phase,
        ...
        with mock.patch.object(
            signerd, "validate_compiled_mint_policy", side_effect=[self.peg(200), self.peg(201)]
        ):
            with self.assertRaisesRegex(ValueError, "capacity changed"):
                signerd.validate_c1_signing_boundary(
                    Rpc(reservation_status(tip=200)), ISSUER, allocation(), "EXPOSE"
                )
        ...
        for phase in ("RESERVE", "CANCEL"):
            for sequence, accepted in ((1, False), (2, True), (3, False)):
                with self.subTest(phase=phase, sequence=sequence):
                    status = reservation_status(sequence=sequence, last_sequence=1, found=False)
                    if accepted:
                        self.assertTrue(self.boundary(status, phase, claim=allocation(sequence)))
                    else:
                        with self.assertRaisesRegex(ValueError, "next"):
                            self.boundary(status, phase, claim=allocation(sequence))

        for phase in ("EXPOSE", "FUND"):
            for sequence, accepted in ((1, True), (2, True), (3, False)):
                with self.subTest(phase=phase, sequence=sequence):
                    status = reservation_status(
                        sequence=sequence, last_sequence=2, exposed=(phase == "FUND")
                    )
                    funding = self.funding() if phase == "FUND" else None
                    if accepted:
                        self.assertTrue(
                            self.boundary(
                                status, phase, funding=funding, claim=allocation(sequence)
                            )
                        )
                    else:
    ...
    def test_proofless_mnp2_decoding_binds_blind(self):
        decoded = {
            ...
            "memo": "MNP2;%s;%s;%s;%s" % (ALLOCATION_ID, SCRIPT, BLIND, OUTPOINT),
        }
        completed = subprocess.CompletedProcess([], 0, stdout=json.dumps(decoded), stderr="")
        with mock.patch.object(signerd, "run_bounded_subprocess", return_value=completed):
            result = signerd.mint_params_from_tx("00", ISSUER, "00")
        self.assertEqual(result[3:], (OUTPOINT, None, ALLOCATION_ID, SCRIPT, BLIND))

    def test_only_cancel_fund_and_mnp2_use_completion_gate(self):
        cases = {
            "RESERVE": (reservation_status(exposed=False, found=False, last_sequence=0), None),
            "EXPOSE": (reservation_status(exposed=False), None),
            "CANCEL": (reservation_status(exposed=False, found=False, last_sequence=0), None),
        ...
        for phase, (status, funding) in cases.items():
            with (
                ...
                signerd.validate_c1_signing_boundary(
                    Rpc(status), ISSUER, allocation(), phase, funding=funding
                )
                expected = phase in ("CANCEL", "FUND")
                self.assertEqual(
                    [call.kwargs.get("completion") for call in gate.call_args_list],
                    [expected, expected],
                )
...
class CompletionExceptionServiceTests(unittest.TestCase):
    ...
    def peg(*, unlocked=True, mint_live=True, completion_live=True):
        return {
            "active": True,
            "peg_unlocked": unlocked,
            "mint_live": mint_live,
            "completion_live": completion_live,
            "issuer": ISSUER,
            "issuer_max_per_mint_sats": signerd.MAX_SINGLE_SATS,
            "issuer_static_custody_cap_sats": max(signerd.MAX_WINDOW_SATS, 100_000),
            "issuer_effective_custody_cap_sats": 100_000,
            "issuer_mint_headroom_sats": 90_000,
        }
    ...
    def test_later_stall_opens_completion_but_not_new_mint_admission(self):
        stalled = self.PegRpc(self.peg(mint_live=False, completion_live=True))
        with self.assertRaisesRegex(ValueError, "minting closed"):
            signerd.validate_compiled_mint_policy(stalled, ISSUER, completion=False)
        self.assertEqual(
            signerd.validate_compiled_mint_policy(stalled, ISSUER, completion=True)[
                "completion_live"
            ],
            True,
        )
    ...
    def test_prelaunch_rejects_both_admission_and_completion(self):
        prelaunch = self.PegRpc(self.peg(unlocked=False, mint_live=False, completion_live=False))
        for completion in (False, True):
            with self.subTest(completion=completion), self.assertRaises(ValueError):
                signerd.validate_compiled_mint_policy(prelaunch, ISSUER, completion=completion)
    ...
    def test_mint_receipt_binds_consensus_id_not_private_request_id(self):
        ...
        heartbeat = {"seq": 3, "tip": 20, "tip_hash": "55" * 32, "headroom_sats": 10_000}
        receipt = {
            "issuer_id": ISSUER,
            "witness_id": "witness-1",
            "request_id": "66" * 32,
            "unsigned_tx_sha256": "77" * 32,
            "sats": 10_000,
            "recipient": RECIPIENT,
            "beat_seq": 3,
            "tip": 20,
        ...
        with (
            mock.patch.object(
                signerd.sol, "validate_reservation_receipt", return_value=(True, "ok")
            ),
            mock.patch.object(signerd, "_secure_text", return_value="00"),
            mock.patch.object(
                signerd, "run_bounded_subprocess", return_value=mock.Mock(returncode=0)
            ),
        ):
            self.assertIs(
...
class Cfp1ProducerTests(unittest.TestCase):
    ...
    def test_builds_exact_cfp1_and_rechecks_parent(self):
        ...
        class Bitcoin:
            def call(self, method, *args):
                if method == "getrawtransaction" and args[1] == "true":
                    return {"txid": txid, "blockhash": block_hash, "confirmations": 7}
                if method == "getrawtransaction":
                    return raw.hex()
                if method == "getblockheader":
                    return {"hash": block_hash, "height": 100, "confirmations": 7}
                if method == "getblockhash":
                    return block_hash
                if method == "getblock":
                    return {"hash": block_hash, "height": 100, "merkleroot": txid, "tx": [txid]}
                raise AssertionError("unexpected Bitcoin RPC " + method)

        class Veld:
            def rpc(self, method, params=None):
                if method == "getbtcheaderinfo":
                    return {"spv_active": True, "k_btc": 6, "best_height": 106}
        ...
        daemon.mint_outpoint_status = mock.Mock(side_effect=[dict(parent), dict(parent)])
        claim = allocation()
        ...
        daemon.mint_outpoint_status = mock.Mock(side_effect=[dict(parent), changed])
...
class TerminalArchiveTests(unittest.TestCase):
    ...
    def test_worm_ack_precedes_terminal_local_deletion(self):
        record = self.record()
        state = {"version": 3, "sequence": 0, "records": {ALLOCATION_ID: record}}
        ...
        digest = hashlib.sha256(reservationd.canonical(record).encode()).hexdigest()
        answer = {
            "version": 1,
            "archived": True,
            "allocation_id": ALLOCATION_ID,
        ...
        with (
            mock.patch.object(reservationd, "run_bounded_subprocess", return_value=completed),
    ...
    def test_archive_refusal_retains_full_record(self):
        ...
        state = {"version": 3, "sequence": 0, "records": {ALLOCATION_ID: record}}
        completed = subprocess.CompletedProcess([], 9, stdout="", stderr="no quorum")
        with mock.patch.object(reservationd, "run_bounded_subprocess", return_value=completed):
            with self.assertRaisesRegex(RuntimeError, "archive refused"):
                reservationd.compact_terminal(
                    mock.Mock(),
                    {"terminal_archive_command": ("/archive",)},
                    state,
                    ALLOCATION_ID,
                    record,
                    {"tip": 101},
    ...
    def test_terminal_observation_reorg_clear_is_transactional(self):
        ...
        state = {"version": 3, "sequence": 7, "records": {ALLOCATION_ID: record}}
        with mock.patch.object(reservationd, "persist") as persist:
            self.assertTrue(reservationd.clear_terminal_observation({}, state, record))
            persist.assert_called_once()
        self.assertIsNone(record["terminal_observed_tip"])

        record["terminal_observed_tip"] = 200
        with mock.patch.object(
            reservationd, "persist", side_effect=RuntimeError("checkpoint down")
        ):
...
class SignerCacheDurabilityTests(unittest.TestCase):
    def raw_record(self):
        ...
        return {
            "kind": "raw",
            "allocation_sha256": "66" * 32,
            "unsigned_sha256": "77" * 32,
    ...
    def cfg(self):
        return {
            "c1_reservation_authority": {
                "enabled": True,
                "allocation_witness_command": [REAL_TRUE],
    ...
    def test_coordinator_receipt_archives_before_dropping_raw_bytes(self):
        ...
        with (
            mock.patch.object(signerd, "load_c1_signer_state", return_value=state),
            mock.patch.object(signerd, "save_c1_signer_state") as save,
            mock.patch.object(signerd, "run_bounded_subprocess", side_effect=archive),
        ):
            answer = signerd.acknowledge_c1_signature_durable(self.receipt(record), self.cfg())
    ...
    def test_weak_worm_ack_never_drops_only_raw_copy(self):
        ...
        with (
            mock.patch.object(signerd, "load_c1_signer_state", return_value=state),
            mock.patch.object(signerd, "save_c1_signer_state") as save,
            mock.patch.object(signerd, "run_bounded_subprocess", return_value=weak),
        ):
            with self.assertRaisesRegex(RuntimeError, "not exact/durable"):
                signerd.acknowledge_c1_signature_durable(self.receipt(record), self.cfg())
    ...
    def durable_record(self):
        ...
        return {
            "kind": "durable",
            "allocation_sha256": raw["allocation_sha256"],
            "unsigned_sha256": raw["unsigned_sha256"],
            "signed_tx_sha256": hashlib.sha256(bytes.fromhex(raw["signed_tx_hex"])).hexdigest(),
            "txid": raw["txid"],
    ...
    def test_terminal_compaction_waits_101_and_resets_on_rollback(self):
        ...
        class Rpc:
            ...
            def call(self, method, params=None):
                return {"allocation_id": ALLOCATION_ID, "retired": self.retired, "tip": self.tip}

        with mock.patch.object(signerd, "save_c1_signer_state"):
            self.assertEqual(signerd.compact_c1_signer_state(Rpc(True, 100), self.cfg(), state), 0)
            self.assertEqual(state["terminal_observations"][ALLOCATION_ID], 100)
            self.assertEqual(signerd.compact_c1_signer_state(Rpc(False, 101), self.cfg(), state), 0)
            self.assertNotIn(ALLOCATION_ID, state["terminal_observations"])
            self.assertIn(key, state["records"])
            self.assertEqual(signerd.compact_c1_signer_state(Rpc(True, 200), self.cfg(), state), 0)
            with mock.patch.object(
                signerd,
                "run_bounded_subprocess",
                side_effect=lambda *a, **kw: self.archive_result(mock.Mock(kwargs=kw)),
            ):
                self.assertEqual(
                    signerd.compact_c1_signer_state(Rpc(True, 300), self.cfg(), state), 1
                )
    ...
    def test_state_roundtrip_and_symmetric_size_ceiling(self):
        ...
        for sequence in range(2, 513):
            state["records"]["%032x:RESERVE" % sequence] = self.durable_record()
...
class CapacityReserveTests(unittest.TestCase):
    def test_new_lifecycle_uses_90_percent_reserve_completion_uses_hard_cap(self):
        ...
        with mock.patch.object(signerd, "C1_STATE_RESERVE_BYTES", signer_size + 10):
        ...
        with mock.patch.object(reservationd, "STATE_ADMISSION_MAX", projected):
            ...
            with self.assertRaisesRegex(ValueError, "admission reserve"):
                reservationd.require_lifecycle_admission_capacity(coordinator_state, 1)

    def test_cached_exposure_never_rebroadcasts_while_mint_gate_closed(self):
        stage = {"signed_tx_hex": "00"}
        rpc = mock.Mock()
        with (
            mock.patch.object(reservationd, "mint_lifecycle_live", return_value=False),
            mock.patch.object(reservationd, "broadcast_exact") as broadcast,
        ):
            self.assertFalse(reservationd.rebroadcast_cached_exposure_if_live(rpc, {}, {}, stage))
            broadcast.assert_not_called()
        with (
            mock.patch.object(reservationd, "mint_lifecycle_live", return_value=True),
            mock.patch.object(reservationd, "broadcast_exact", return_value=True) as broadcast,
        ):
            self.assertTrue(reservationd.rebroadcast_cached_exposure_if_live(rpc, {}, {}, stage))
    ...
    def test_hard_cap_failure_rolls_back_in_memory_lifecycle_mutation(self):
        ...
        state = {"version": 3, "sequence": 4, "records": {ALLOCATION_ID: record}}
        prepared = {
            "allocation_id": ALLOCATION_ID,
            "action": "EXPOSE",
            "recipient": RECIPIENT,
        ...
        with mock.patch.object(reservationd, "persist", side_effect=ValueError("byte ceiling")):
            with self.assertRaisesRegex(ValueError, "byte ceiling"):
                reservationd.prepare_stage(rpc, {"issuer": ISSUER}, claim, "EXPOSE", record, state)
    ...
    def test_deleted_terminal_is_not_recreated_and_missing_active_fails(self):
        ...
        with mock.patch.object(reservationd, "persist") as persist:
            self.assertIsNone(reservationd.bind_or_admit_record({}, state, allocation(), retired))
        ...
        with self.assertRaisesRegex(ValueError, "exactly next"):
            SignerBoundaryTests().boundary(
                reservation_status(found=False, retired=True, last_sequence=1),
                "RESERVE",
                claim=allocation(),
            )
...
class CrossCapabilityPrevoutLeaseTests(unittest.TestCase):
    def test_conflict_is_durably_revoked_and_fresh_unsigned_can_replace_it(self):
        ...
        with mock.patch.object(signerd, "save_prevout_journal") as save:
            signerd.reserve_issuer_prevouts(
                state, c1_owner, "c1-reservation", ALLOCATION_ID, None, c1_unsigned
            )
            with self.assertRaises(signerd.UnsignedCarrierAbandoned) as caught:
                signerd.reject_conflicting_issuer_prevouts(
                    state, mint_owner, "mint", ALLOCATION_ID, OUTPOINT, mint_conflict
                )
            ...
            self.assertEqual(
                answer["unsigned_sha256"], hashlib.sha256(bytes.fromhex(mint_conflict)).hexdigest()
            )
            self.assertEqual(answer["conflicting_input_outpoints"], ["91" * 32 + ":0"])
            ...
            signerd.reject_conflicting_issuer_prevouts(
                state, mint_owner, "mint", ALLOCATION_ID, OUTPOINT, mint_fresh
            )
            ...
            signerd.mark_issuer_prevout_signature(state, mint_owner, mint_fresh, "ab" * 32)
            self.assertEqual(state["owners"][mint_owner]["active"]["txid"], "ab" * 32)
    ...
    def test_minter_discards_only_exact_exit_75_unsigned_attestation(self):
        ...
        daemon.signer.sign.side_effect = mintd.UnsignedMintCarrierAbandoned(["91" * 32 + ":0"])
    ...
    def test_coordinator_replaces_only_signer_revoked_unsigned_stage(self):
        ...
        completed = subprocess.CompletedProcess([], 75, stdout=json.dumps(answer), stderr="")
        ...
        with (
            ...
            with self.assertRaisesRegex(RuntimeError, "fresh preparation"):
                reservationd.prepare_stage(mock.Mock(), cfg, claim, "RESERVE", record, state)
            persist.assert_called_once()
        self.assertIs(record["RESERVE"], stage)
        self.assertTrue(stage["replacement_required"])
        self.assertEqual(stage["excluded_issuer_prevouts"], ["91" * 32 + ":0"])

        stage = exact_stage
        record["RESERVE"] = stage
        refused = subprocess.CompletedProcess([], 2, stdout="", stderr="no")
        with (
            mock.patch.object(reservationd, "run_bounded_subprocess", return_value=refused),
    ...
    def test_stale_fund_variant_restarts_with_persisted_exclusion_suffix(self):
        ...
        stage = {
            ...
            "unsigned_sha256": hashlib.sha256(bytes.fromhex(old_unsigned)).hexdigest(),
            "signed_tx_hex": None,
            "txid": None,
            "broadcast_attempts": 0,
            "last_broadcast_at": None,
            "funding_sha256": funding_sha,
            "semantic_sha256": reservationd._funding_semantic_sha256(claim, funding),
        ...
        with (
            ...
            with self.assertRaisesRegex(RuntimeError, "fresh preparation"):
                reservationd.prepare_stage(
                    mock.Mock(), cfg, claim, "FUND", record, state, funding=funding
                )
        ...
        with (
            ...
            mock.patch.object(reservationd, "persist", return_value="bb" * 32) as persist,
            mock.patch.object(reservationd, "acknowledge_signer_durable", return_value=durable),
        ):
            result = reservationd.prepare_stage(
                rpc, cfg, claim, "FUND", record, state, funding=funding
            )
        params = rpc.call.call_args.args[1]
        self.assertEqual(params[-1], "issuer-prevout-exclusions-v1:" + "91" * 32 + ":0")
        self.assertFalse(result["replacement_required"])
        self.assertEqual(result["excluded_issuer_prevouts"], ["91" * 32 + ":0"])
    ...
    def test_leases_release_only_after_hash_anchored_terminal_depth_101(self):
        ...
        with mock.patch.object(signerd, "save_prevout_journal"):
            signerd.reserve_issuer_prevouts(
                state, owner, "c1-reservation", ALLOCATION_ID, None, unsigned_proposal("93" * 32)
            )
        ...
        class Rpc:
            ...
            def call(self, method, params=None):
                if method == "getbtcveldsupply":
                    return {"supply_sats": 0, "tip": self.tip, "tip_hash": self.tip_hash}
                if method == "getbtcveldc1reservation":
                    return {"allocation_id": ALLOCATION_ID, "retired": True, "tip": self.tip}
                if method == "getblockhash":
    ...
    def test_signing_crash_recovers_exact_stage_and_never_resigns_ambiguity(self):
        ...
        with tempfile.TemporaryDirectory(prefix="issuer-stage-") as directory:
            ...
            with (
                mock.patch.multiple(
                    signerd,
                    PREVOUT_STATEF=str(journal_path),
                    SIGNING_STAGE_DIR=str(stage_dir),
                    KEYFILE=str(key),
                    PASSFILE=str(password),
                    KEYGEN=str(keygen),
                ),
                mock.patch.object(
                    signerd.sol, "unsigned_template_from_signed_hex", return_value=unsigned
                ),
                mock.patch.object(
                    signerd, "run_bounded_subprocess", side_effect=key_operation
                ) as run,
            ):
                signerd.reserve_issuer_prevouts(
                    state, owner, "c1-reservation", ALLOCATION_ID, None, unsigned
                )
                self.assertEqual(
                    signerd.sign_or_recover_staged_carrier(
                        ...
                        "00" * 25,
                        "test",
                        build_evidence=lambda: staged_evidence(unsigned),
                        revalidate=lambda: None,
                    ),
                    signed,
                )
                self.assertEqual(state["owners"][owner]["active"]["signing_state"], "SIGNING")
    ...
    def test_staged_signature_accepts_exact_hex_ceiling_plus_newline(self):
        ...
        with tempfile.TemporaryDirectory(prefix="issuer-stage-boundary-") as directory:
            ...
            with (
                ...
                mock.patch.object(signerd, "run_bounded_subprocess", side_effect=key_operation),
            ):
                signerd.reserve_issuer_prevouts(
                    state, owner, "c1-reservation", ALLOCATION_ID, None, unsigned
                )
    ...
    def test_initialized_empty_journal_reconciles_mint_and_archived_c1_union(self):
        ...
        mint_signed = "bc" * 80
        mint_txid = hashlib.sha256(hashlib.sha256(bytes.fromhex(mint_signed)).digest()).hexdigest()
        mint_request = hashlib.sha256(bytes.fromhex(mint_unsigned)).hexdigest()
        mint_state = {
            "signed": [
                {
                    "request_id": mint_request,
                    "txid": mint_txid,
                    "signed_tx_hex": mint_signed,
                    "sats": 10_000,
                    "to": RECIPIENT,
                    "at": 1,
                }
        ...
        archived["txid"] = hashlib.sha256(
            hashlib.sha256(bytes.fromhex(archived["signed_tx_hex"])).digest()
        ).hexdigest()
        ...
        with tempfile.TemporaryDirectory(prefix="lease-reconcile-") as directory:
            ...
            with (
                mock.patch.multiple(
                    signerd,
                    PREVOUT_STATEF=journal,
                    SIGNING_STAGE_DIR=str(Path(directory) / ".staging"),
                ),
                mock.patch.object(signerd, "load_signer_state", return_value=mint_state),
                mock.patch.object(signerd, "load_c1_signer_state", return_value=c1_state),
                mock.patch.object(
                    signerd.sol, "unsigned_template_from_signed_hex", return_value=mint_unsigned
                ),
                mock.patch.object(
                    signerd,
                    "mint_params_from_tx",
    ...
    def test_terminal_gc_never_deletes_stage_before_lease_removal_fsync(self):
        ...
        state = signerd._empty_prevout_journal()
        with (
            tempfile.TemporaryDirectory(prefix="gc-order-") as directory,
            mock.patch.multiple(
                signerd,
            ...
            signerd.reserve_issuer_prevouts(
                state, owner_id, "c1-reservation", ALLOCATION_ID, None, unsigned
            )
            active = state["owners"][owner_id]["active"]
            active["signing_state"] = "SIGNING"
            prepared, signed = signerd._signing_stage_paths(active["staging_id"])
            Path(prepared).write_text("prepared")
            Path(prepared).chmod(0o600)
            Path(signed).write_text("signed")
            Path(signed).chmod(0o600)
            state["terminal_observations"][ALLOCATION_ID] = {"tip": 100, "tip_hash": "10" * 32}

            class Rpc:
                def call(self, method, params=None):
                    if method == "getbtcveldsupply":
                        return {"supply_sats": 0, "tip": 200, "tip_hash": "20" * 32}
                    if method == "getbtcveldc1reservation":
                        return {"allocation_id": ALLOCATION_ID, "retired": True, "tip": 200}
                    if method == "getblockhash":
                        return "10" * 32
                    raise AssertionError(method)
            ...
            with mock.patch.object(signerd, "save_prevout_journal", side_effect=fail_removal_save):
                with self.assertRaisesRegex(RuntimeError, "checkpoint down"):
    ...
    def test_unconfirmed_signed_stage_survives_partial_primary_restore(self):
        ...
        state = signerd._empty_prevout_journal()
        with (
            tempfile.TemporaryDirectory(prefix="stage-restore-") as directory,
            mock.patch.multiple(
                signerd,
                PREVOUT_STATEF=str(Path(directory) / "leases.json"),
                SIGNING_STAGE_DIR=str(Path(directory) / ".staging"),
            ...
            self.assertEqual(signerd._cleanup_reconciled_signed_stages(state, set()), 0)
            self.assertTrue(Path(prepared).exists())
            self.assertTrue(Path(signed).exists())
            self.assertEqual(
                signerd._cleanup_reconciled_signed_stages(state, {active["staging_id"]}), 1
            )
    ...
    def test_v3_revocation_promotes_full_candidate_inputs_before_ack(self):
        ...
        with mock.patch.object(signerd, "save_prevout_journal") as save:
            with self.assertRaises(signerd.UnsignedCarrierAbandoned) as caught:
                signerd.reject_conflicting_issuer_prevouts(
                    state, owner_id, "mint", ALLOCATION_ID, OUTPOINT, unsigned
                )
        self.assertEqual(caught.exception.answer["conflicting_input_outpoints"], ["99" * 32 + ":0"])
        save.assert_called_once_with(state)
...
class SignerAuthorityInitializationTests(unittest.TestCase):
    def test_one_shot_complete_init_and_partial_delete_fail_closed(self):
        with tempfile.TemporaryDirectory(prefix="signer-authority-init-") as directory:
            ...
            Path(paths["SIGNER_CONFIG"]).write_text(
                json.dumps(
                    {
            ...
            with (
                ...
                mock.patch.object(signerd, "issuer_addr", return_value=ISSUER),
                mock.patch.object(signerd, "issuer_p2pkh_from_address", return_value="00" * 25),
            ):
                document = signerd.initialize_signer_authority_state()
                self.assertEqual(
                    document,
                    signerd._authority_state_marker_document(signerd.load_signer_configuration()),
                )
                self.assertEqual(signerd.load_signer_state(), {"signed": []})
                self.assertEqual(signerd.load_c1_signer_state(), signerd._empty_c1_signer_state())
                self.assertEqual(signerd.load_prevout_journal(), signerd._empty_prevout_journal())
                ...
                self.assertEqual(signerd.initialize_signer_authority_state(), document)
    ...
    def test_activated_set_reparses_every_member_and_reconciles(self):
        ...
        for member, malformed in corruptions.items():
            with (
                ...
                tempfile.TemporaryDirectory(prefix="signer-authority-revalidate-") as directory,
            ):
                root = Path(directory)
                marker = root / "signer-authority-state.json"
                paths = {
        ...
        with tempfile.TemporaryDirectory(prefix="signer-authority-reconcile-") as directory:
            root = Path(directory)
            marker = root / "signer-authority-state.json"
            paths = {
                "STATEF": str(root / "signer-state.json"),
            ...
            with (
                ...
                with mock.patch.object(
                    ...
                    with self.assertRaisesRegex(ValueError, "cross-state mismatch"):
                        signerd.initialize_signer_authority_state()
    ...
    def test_complete_pre_marker_upgrade_requires_chain_reconciliation(self):
        with tempfile.TemporaryDirectory(prefix="signer-authority-adopt-") as directory:
            ...
            Path(paths["SIGNER_CONFIG"]).chmod(0o600)
            with (
                mock.patch.multiple(signerd, HERE=str(root), **paths),
                mock.patch.object(signerd, "issuer_addr", return_value=ISSUER),
                mock.patch.object(signerd, "issuer_p2pkh_from_address", return_value="00" * 25),
            ):
                ...
                before = {
                    ...
                    for path in (paths["STATEF"], paths["C1_STATEF"], paths["PREVOUT_STATEF"])
                }
    ...
    def test_missing_prevout_member_is_never_reconstructed_from_other_caches(self):
        with (
            tempfile.TemporaryDirectory(prefix="missing-prevout-") as directory,
            mock.patch.multiple(
                signerd,
            ...
            with self.assertRaisesRegex(FileNotFoundError, "partial"):
                signerd.load_or_reconcile_prevout_journal({}, ISSUER, "00" * 25)
...
class GlobalCoordinatorMaintenanceTests(unittest.TestCase):
    ...
    def test_fair_reaper_compacts_every_terminal_without_client_retry(self):
        ...
        class Rpc:
            def call(self, method, params=None):
                self.last = params[0]
                return {"allocation_id": params[0], "retired": True, "tip": 500}

        visited = []

        def compact(_rpc, _cfg, actual, allocation_id, _record, _status, allow_archive=True):
        ...
        with (
            mock.patch.object(reservationd, "persist"),
            mock.patch.object(reservationd, "compact_terminal", side_effect=compact),
        ):
            while state["records"]:
