...
CONFIG = os.environ.get(
    "VELD_C1_RESERVATIOND_CONFIG",
    os.path.join(HERE, "c1-reservationd-config.json"))
...
def exact_argv(value, name):
    if (not isinstance(value, list) or not value or
            any(not isinstance(part, str) or not part or "\x00" in part
                for part in value) or not os.path.isabs(value[0])):
...
class Rpc:
    def __init__(self, cfg):
        ...
        if "token_file" in cfg:
            token = read_bounded_secret_file(
                cfg["token_file"], 4096, "coordinator RPC token")
            try:
            ...
            out = run_bounded_subprocess(
                list(command), timeout=30, stdout_max=4096,
                stderr_max=64 * 1024, description="coordinator RPC token_cmd")
            if out.returncode != 0:
    ...
    def call(self, method, params=None):
        body = canonical({"jsonrpc": "2.0", "id": 1, "method": method,
        ...
        if not isinstance(envelope, dict) or envelope.get("error"):
            raise RuntimeError("Veld RPC %s failed: %s" %
                               (method, envelope.get("error")
                                if isinstance(envelope, dict) else "bad envelope"))
...
def validate_allocation(value):
    keys = {"request_id", "principal_hash", "veld_address", "amount_sats", "descriptor_index",
            "btc_address", "script_pubkey", "admitted_at", "expires_at",
    ...
    if (not isinstance(value["request_id"], str) or
            not REQ_RE.fullmatch(value["request_id"]) or
            not isinstance(value["principal_hash"], str) or
            not HASH_RE.fullmatch(value["principal_hash"]) or
            not isinstance(value["veld_address"], str) or
            not VELD_RE.fullmatch(value["veld_address"]) or
            type(value["amount_sats"]) is not int or
            not MIN_SATS <= value["amount_sats"] <= MAX_SATS or
            ...
            not 14 <= len(value["btc_address"]) <= 100 or
            not isinstance(value["script_pubkey"], str) or
            not P2TR_RE.fullmatch(value["script_pubkey"]) or
            not isinstance(value["commitment_blind"], str) or
            not HASH_RE.fullmatch(value["commitment_blind"]) or
            not isinstance(value["consensus_allocation_id"], str) or
            not ALLOCATION_ID_RE.fullmatch(value["consensus_allocation_id"]) or
            ...
            not HASH_RE.fullmatch(value["capacity_policy_sha256"])):
...
def validate_funding(value):
    if value is None:
        return None
    required = {"outpoint", "proof_hex", "proof_parent_root",
                "proof_parent_count"}
    if (not isinstance(value, dict) or set(value) != required or
            not isinstance(value.get("outpoint"), str) or
            not OUTPOINT_RE.fullmatch(value["outpoint"]) or
            int(value["outpoint"].rsplit(":", 1)[1]) > 0xffffffff or
            not isinstance(value.get("proof_hex"), str) or
            not HEX_RE.fullmatch(value["proof_hex"]) or
            len(value["proof_hex"]) % 2 != 0 or
            not 174 <= len(value["proof_hex"]) <= 38_000 or
            not isinstance(value.get("proof_parent_root"), str) or
            not HASH_RE.fullmatch(value["proof_parent_root"]) or
            type(value.get("proof_parent_count")) is not int or
            not 0 <= value["proof_parent_count"] <= (1 << 64) - 1):
...
def validate_status(status, allocation):
    ...
    for key in ("found", "active", "retired", "exposed", "funded",
                "canonical_depth_reached", "reserve_canonical_depth_reached",
    ...
    for key in ("tip", "confirmations", "required_confirmations",
                "sequence", "last_sequence", "sequence_history_count"):
        if type(status.get(key)) is not int or status[key] < 0:
            raise ValueError("reservation RPC status %s is not an integer" % key)
    if (status.get("allocation_id") !=
            allocation["consensus_allocation_id"] or
            status["sequence"] != int(
                allocation["consensus_allocation_id"], 16) or
            status["sequence_history_count"] != status["last_sequence"] or
            status["required_confirmations"] != FINALITY_DEPTH):
    ...
    if status["found"]:
        if (status.get("recipient") != allocation["veld_address"] or
        ...
        if status["exposed"]:
            for key in ("exposed_height", "funding_starts_height",
        ...
        if status["funded"]:
            if (not status["exposed"] or
                    not isinstance(status.get("funding_outpoint"), str) or
                    not OUTPOINT_RE.fullmatch(status["funding_outpoint"]) or
                    type(status.get("funded_height")) is not int or
                    type(status.get("funding_confirmations")) is not int):
...
def validate_funding_parent(rpc, funding):
    ...
    if (not isinstance(status, dict) or
            status.get("outpoint") != funding["outpoint"] or
            status.get("consumed") is not False or
            status.get("minted") is not False or
            ...
            not HASH_RE.fullmatch(status["root"]) or
            type(status.get("count")) is not int or status["count"] < 0 or
            status["root"] != funding["proof_parent_root"] or
            status["count"] != funding["proof_parent_count"] or
            not funding["proof_hex"].endswith(status["proof_hex"])):
        raise ValueError("CFP1 parent differs from the fresh mint-nullifier root")
    for name in ("accepted_txid", "accepted_block_height",
...
def read_config():
    cfg = strict_json_loads(read_bounded_regular_file(
        os.path.abspath(CONFIG), 1024 * 1024,
        "C1 reservation coordinator config", private=True),
        "C1 reservation coordinator config")
    required = {"version", "state_file", "issuer", "veld_rpc",
                "reservation_signer_command", "checkpoint_command",
    ...
    if (not isinstance(cfg["state_file"], str) or
            not os.path.isabs(cfg["state_file"]) or
            type(cfg["allow_initial_state_creation"]) is not bool or
            not isinstance(cfg["issuer"], str) or
            not VELD_RE.fullmatch(cfg["issuer"])):
    ...
    cfg["checkpoint_command"] = exact_argv(
        cfg["checkpoint_command"], "checkpoint_command")
...
def load_state(path):
    ...
    state = strict_json_loads(read_bounded_regular_file(
        path, STATE_MAX, "C1 coordinator state", private=True),
        "C1 coordinator state")
    v3 = (isinstance(state, dict) and state.get("version") == 3 and
          set(state) == {"version", "sequence", "records"})
    v4 = (isinstance(state, dict) and state.get("version") == 4 and
          set(state) == {"version", "sequence", "records",
                         "compaction_cursor"})
    if (not (v3 or v4) or not isinstance(state.get("records"), dict) or
            type(state.get("sequence")) is not int or
            not 0 <= state["sequence"] <= (1 << 63) - 1 or
            len(state["records"]) > MAX_RECORDS or
            (v4 and state["compaction_cursor"] is not None and
             (not isinstance(state["compaction_cursor"], str) or
              not ALLOCATION_ID_RE.fullmatch(state["compaction_cursor"])))):
    ...
    for allocation_id, record in state["records"].items():
        if (not isinstance(allocation_id, str) or
                not ALLOCATION_ID_RE.fullmatch(allocation_id) or
                int(allocation_id, 16) == 0 or
                not isinstance(record, dict)):
            raise ValueError("C1 coordinator record identity is invalid")
        if record.get("kind") == "terminal":
            if (set(record) != {"kind", "allocation_sha256", "request_id",
                                "terminal_tip", "record_sha256", "archive_id",
                                "archived_at"} or
                    not HASH_RE.fullmatch(str(record.get("allocation_sha256"))) or
                    not REQ_RE.fullmatch(str(record.get("request_id"))) or
                    type(record.get("terminal_tip")) is not int or
                    record["terminal_tip"] < 0 or
                    not HASH_RE.fullmatch(str(record.get("record_sha256"))) or
            ...
            continue
        if (record.get("kind") != "active" or
                set(record) != {"kind", "allocation_sha256", "request_id",
                               "terminal_observed_tip", "RESERVE", "EXPOSE",
                               "CANCEL", "FUND"} or
                not HASH_RE.fullmatch(str(record["allocation_sha256"])) or
                not isinstance(record["request_id"], str) or
                not REQ_RE.fullmatch(record["request_id"]) or
                (record["terminal_observed_tip"] is not None and
                 (type(record["terminal_observed_tip"]) is not int or
                  record["terminal_observed_tip"] < 0)) or
                not isinstance(record["FUND"], list) or
                len(record["FUND"]) > MAX_FUND_VARIANTS):
        ...
        for stage in stages:
            if stage is None:
                continue
            base_stage_keys = {
                    "unsigned_tx_hex", "unsigned_sha256", "signed_tx_hex",
                    "txid", "broadcast_attempts", "last_broadcast_at",
                    "funding_sha256", "semantic_sha256", "funding_outpoint",
                    "created_at", "signer_archive_record_sha256",
                    "signer_archive_id", "signer_receipt_sequence",
                    "signer_receipt_state_sha256"}
            exclusion_keys = {
                "excluded_issuer_prevouts", "replacement_required"}
            if (not isinstance(stage, dict) or
                    set(stage) not in (
                        base_stage_keys, base_stage_keys | exclusion_keys)):
            ...
            if (type(replacement_required) is not bool or
                    (replacement_required and
                     (stage["signed_tx_hex"] is not None or
                      stage["txid"] is not None or
                      stage["signer_archive_record_sha256"] is not None))):
                raise ValueError("C1 coordinator replacement state is invalid")
            if (
                    not HEX_RE.fullmatch(str(stage["unsigned_tx_hex"])) or
                    not HASH_RE.fullmatch(str(stage["unsigned_sha256"])) or
                    hashlib.sha256(bytes.fromhex(stage["unsigned_tx_hex"])).hexdigest()
                        != stage["unsigned_sha256"] or
                    (stage["signed_tx_hex"] is not None and
                     (not HEX_RE.fullmatch(str(stage["signed_tx_hex"])) or
                      txid(stage["signed_tx_hex"]) != stage["txid"])) or
                    (stage["signed_tx_hex"] is None and stage["txid"] is not None) or
                    type(stage["broadcast_attempts"]) is not int or
                    stage["broadcast_attempts"] < 0 or
                    (stage["last_broadcast_at"] is not None and
                     type(stage["last_broadcast_at"]) is not int) or
                    type(stage["created_at"]) is not int or
                    stage["created_at"] < 0 or
                    ((stage["signer_archive_record_sha256"] is None) !=
                     (stage["signer_archive_id"] is None)) or
                    ((stage["signer_archive_record_sha256"] is None) !=
                     (stage["signer_receipt_sequence"] is None)) or
                    ((stage["signer_archive_record_sha256"] is None) !=
                     (stage["signer_receipt_state_sha256"] is None)) or
                    (stage["signer_archive_record_sha256"] is not None and
                     (not HASH_RE.fullmatch(
                          str(stage["signer_archive_record_sha256"])) or
                      not isinstance(stage["signer_archive_id"], str) or
                      not 1 <= len(stage["signer_archive_id"]) <= 256 or
                      type(stage["signer_receipt_sequence"]) is not int or
                      stage["signer_receipt_sequence"] <= 0 or
                      not HASH_RE.fullmatch(
                          str(stage["signer_receipt_state_sha256"])))) or
                    (stage["funding_sha256"] is not None and
                     not HASH_RE.fullmatch(str(stage["funding_sha256"]))) or
                    (stage["semantic_sha256"] is not None and
                     not HASH_RE.fullmatch(str(stage["semantic_sha256"]))) or
                    (stage["funding_outpoint"] is not None and
                     not OUTPOINT_RE.fullmatch(str(stage["funding_outpoint"])))):
                raise ValueError("C1 coordinator stage is invalid")
            is_fund = stage in record["FUND"]
            if is_fund != (stage["funding_sha256"] is not None and
                           stage["semantic_sha256"] is not None and
...
def checkpoint_state(cfg, state, digest):
    request = canonical({
        "version": 1, "action": "store_c1_coordinator_checkpoint",
        "sequence": state["sequence"], "state_sha256": digest,
        "record_count": len(state["records"]),
    })
    completed = run_bounded_subprocess(
        list(cfg["checkpoint_command"]), input_text=request, timeout=45,
        stdout_max=64 * 1024, stderr_max=64 * 1024,
        description="C1 coordinator monotonic checkpoint")
    if completed.returncode != 0:
        raise RuntimeError("C1 checkpoint refused/unavailable: " +
                           completed.stderr.strip()[:240])
    answer = strict_json_loads(completed.stdout, "C1 checkpoint response")
    if (not isinstance(answer, dict) or set(answer) != {
            "version", "stored", "sequence", "state_sha256"} or
            answer.get("version") != 1 or answer.get("stored") is not True or
            answer.get("sequence") != state["sequence"] or
            answer.get("state_sha256") != digest):
...
def require_lifecycle_admission_capacity(state, reserve_bytes=0):
    if (type(reserve_bytes) is not int or reserve_bytes < 0 or
            projected_state_size(state) + reserve_bytes > STATE_ADMISSION_MAX):
        raise ValueError(
            "C1 coordinator lifecycle admission reserve is exhausted")
...
def bind_or_admit_record(cfg, state, allocation, status):
    ...
    allocation_hash = hashlib.sha256(
        canonical(allocation).encode("utf-8")).hexdigest()
    ...
    if record is None:
        ...
        if status["found"]:
            raise ValueError(
                "active C1 reservation is missing durable coordinator state")
        if len(state["records"]) >= MAX_RECORDS:
            raise ValueError("coordinator record ceiling reached")
        record = {"kind": "active", "allocation_sha256": allocation_hash,
        ...
        require_lifecycle_admission_capacity(
            candidate, 2 * MAX_CARRIER_BYTES + 2048)
        ...
        try:
            ...
            raise
    elif (record["allocation_sha256"] != allocation_hash or
          record["request_id"] != allocation["request_id"]):
        raise ValueError("consensus allocation id is already bound differently")
    if any(other_id != allocation_id and
           other["request_id"] == allocation["request_id"]
...
def load_or_initialize_state(cfg):
    ...
    if cfg["allow_initial_state_creation"]:
        raise ValueError(
            "allow_initial_state_creation must be false after initialization")
    state = load_state(cfg["state_file"])
    checkpoint_state(
        cfg, state,
        hashlib.sha256((canonical(state) + "\n").encode("utf-8")).hexdigest())
    if state["version"] == 3:
        ...
        persist(cfg, state)
    legacy_terminals = [allocation_id for allocation_id, record in
                        state["records"].items()
                        if record.get("kind") == "terminal"]
    if legacy_terminals:
...
def _fund_variant(record, funding):
    ...
    return next((stage for stage in record["FUND"]
                 if stage["funding_sha256"] == digest), None)


def acknowledge_signer_durable(cfg, allocation, phase, stage, state,
                               state_digest):
    request = {
        "version": 1, "action": "ack_c1_signature_durable",
        "allocation_id": allocation["consensus_allocation_id"],
        "phase": phase, "funding_sha256": stage["funding_sha256"],
        "allocation_sha256": hashlib.sha256(
            canonical(allocation).encode("utf-8")).hexdigest(),
        "unsigned_sha256": stage["unsigned_sha256"],
        "signed_tx_sha256": hashlib.sha256(
            bytes.fromhex(stage["signed_tx_hex"])).hexdigest(),
        "txid": stage["txid"], "coordinator_sequence": state["sequence"],
        "coordinator_state_sha256": state_digest,
    }
    completed = run_bounded_subprocess(
        list(cfg["reservation_signer_command"]),
        input_text=canonical(request), timeout=120,
        stdout_max=64 * 1024, stderr_max=64 * 1024,
        description="isolated C1 signer durable receipt")
    if completed.returncode != 0:
        raise RuntimeError("isolated C1 signer durable receipt refused: " +
                           completed.stderr.strip()[:240])
    answer = strict_json_loads(
        completed.stdout, "isolated C1 signer durable response")
    expected = {"version", "action", "allocation_id", "phase",
                "funding_sha256", "record_sha256", "archive_id",
                "idempotent"}
    if (not isinstance(answer, dict) or set(answer) != expected or
            answer.get("version") != 1 or
            answer.get("action") != "c1_signature_durable" or
            answer.get("allocation_id") !=
                allocation["consensus_allocation_id"] or
            answer.get("phase") != phase or
            answer.get("funding_sha256") != stage["funding_sha256"] or
            not HASH_RE.fullmatch(str(answer.get("record_sha256"))) or
            not isinstance(answer.get("archive_id"), str) or
            not 1 <= len(answer["archive_id"]) <= 256 or
            type(answer.get("idempotent")) is not bool):
...
def prepare_stage(rpc, cfg, allocation, phase, record, state, funding=None):
    if phase == "FUND":
        ...
        stage = record[phase]
    replacing = (stage is not None and
                 stage.get("replacement_required", False) is True)
    if stage is None or replacing:
        exclusions = validate_issuer_prevout_exclusions(
            stage.get("excluded_issuer_prevouts", []) if replacing else [])
        params = [
            cfg["issuer"], allocation["veld_address"],
            ...
            allocation_commitment(
                ...
                allocation["commitment_blind"]), phase]
        if phase == "FUND":
            params.extend([allocation["script_pubkey"],
                           allocation["commitment_blind"],
                           funding["outpoint"], funding["proof_hex"]])
        ...
        if (not isinstance(prepared, dict) or
                prepared.get("allocation_id") !=
                    allocation["consensus_allocation_id"] or
                prepared.get("action") != phase or
                prepared.get("recipient") != allocation["veld_address"] or
                prepared.get("amount_sats") != allocation["amount_sats"] or
                prepared.get("allocation_commitment") != allocation_commitment(
                    allocation["consensus_allocation_id"],
                    allocation["veld_address"], allocation["amount_sats"],
                    allocation["script_pubkey"],
                    allocation["commitment_blind"]) or
                prepared.get("required_confirmations") != FINALITY_DEPTH or
                prepared.get("lifetime_blocks") != LIFETIME_BLOCKS or
                prepared.get("fee") != 100_000 or
                prepared.get("excluded_issuer_prevouts") != exclusions or
                not isinstance(prepared.get("unsigned_tx_hex"), str) or
                not HEX_RE.fullmatch(prepared["unsigned_tx_hex"]) or
                len(prepared["unsigned_tx_hex"]) > 2 * MAX_CARRIER_BYTES):
        ...
        if set(exclusions).intersection(unsigned_input_outpoints(unsigned)):
            raise ValueError(
                "prepared C1 carrier reused an excluded issuer prevout")
        replacement = {
            ...
            "funding_sha256": (_funding_sha256(funding)
                               if funding is not None else None),
            "semantic_sha256": (_funding_semantic_sha256(allocation, funding)
                                if funding is not None else None),
            "funding_outpoint": (funding["outpoint"]
                                 if funding is not None else None),
            "created_at": (stage["created_at"] if replacing
                           else int(time.time())),
    ...
    if stage["signed_tx_hex"] is None:
        request = {"version": 2, "action": "sign_c1_reservation",
                   "phase": phase, "allocation": allocation,
                   "funding": funding,
                   "unsigned_tx_hex": stage["unsigned_tx_hex"]}
        signed = run_bounded_subprocess(
            list(cfg["reservation_signer_command"]),
            input_text=canonical(request), timeout=90,
            stdout_max=8 * 1024 * 1024, stderr_max=1024 * 1024,
            description="isolated C1 reservation signer")
        if signed.returncode == 75:
            answer = strict_json_loads(
                signed.stdout, "isolated C1 unsigned abandonment")
            owner_id = ("c1:" + allocation["consensus_allocation_id"] +
                        ":" + phase +
                        ((":" + stage["funding_sha256"])
                         if phase == "FUND" else ""))
            ...
            if not isinstance(answer, dict):
                raise RuntimeError(
                    "isolated C1 signer abandonment is not exact")
            try:
                conflicts = validate_issuer_prevout_exclusions(
                    ...
                    candidate_inputs=unsigned_input_outpoints(
                        stage["unsigned_tx_hex"]))
            except (TypeError, ValueError) as exc:
                raise RuntimeError(
                    "isolated C1 signer abandonment is not exact") from exc
            if (set(answer) != expected or
                    answer.get("version") != 1 or
                    answer.get("action") != "unsigned_carrier_abandoned" or
                    answer.get("capability") != "c1-reservation" or
                    answer.get("owner_id") != owner_id or
                    answer.get("allocation_id") !=
                        allocation["consensus_allocation_id"] or
                    answer.get("deposit_outpoint") is not None or
                    answer.get("unsigned_sha256") !=
                        stage["unsigned_sha256"] or
            ...
            exclusions = sorted(set(
                stage.get("excluded_issuer_prevouts", []) + conflicts))
        ...
        if signed.returncode != 0:
            raise RuntimeError("isolated C1 signer refused: " +
                               signed.stderr.strip()[:240])
        answer = strict_json_loads(signed.stdout, "isolated C1 signer response")
        expected = {"version", "action", "phase", "request_id",
                    "allocation_id",
                    "signed_tx_hex", "txid"}
        if (not isinstance(answer, dict) or set(answer) != expected or
                answer.get("version") != 2 or
                answer.get("action") != "signed_c1_reservation" or
                answer.get("phase") != phase or
                answer.get("request_id") != allocation["request_id"] or
                answer.get("allocation_id") !=
                    allocation["consensus_allocation_id"] or
                not isinstance(answer.get("signed_tx_hex"), str) or
                not HEX_RE.fullmatch(answer["signed_tx_hex"]) or
                len(answer["signed_tx_hex"]) > 2 * MAX_CARRIER_BYTES or
                txid(answer["signed_tx_hex"]) != answer.get("txid")):
            raise ValueError("isolated C1 signer acknowledgement is not exact")
        try:
            reconstructed = solvency.unsigned_template_from_signed_hex(
                answer["signed_tx_hex"])
        except Exception as exc:
            raise ValueError("isolated C1 signature is not decodable") from exc
        if reconstructed != stage["unsigned_tx_hex"]:
            raise ValueError(
                "isolated C1 signature does not bind the stored unsigned carrier")
    ...
    if stage["signer_archive_record_sha256"] is None:
        ...
        durable = acknowledge_signer_durable(
            cfg, allocation, phase, stage, state, state_digest)
...
def mint_lifecycle_live(rpc):
    ...
    return (isinstance(peg, dict) and peg.get("active") is True and
            peg.get("peg_unlocked") is True and
            peg.get("mint_live") is True)
...
def completion_lifecycle_live(rpc):
    ...
    return (isinstance(peg, dict) and peg.get("active") is True and
            peg.get("peg_unlocked") is True and
            peg.get("completion_live") is True)


def rebroadcast_cached_exposure_if_live(rpc, cfg, state, stage):
    if (stage is None or not stage.get("signed_tx_hex") or
            not mint_lifecycle_live(rpc)):
        return False
...
def compact_terminal(rpc, cfg, state, allocation_id, record, status,
                     allow_archive=True):
    ...
    for outpoint in funded_outpoints:
        ...
        if mint_status.get("consumed") is True:
            if (mint_status.get("minted") is not True or
                    mint_status.get("accepted_effect_kind") != "C1_MINT" or
                    mint_status.get("c1_allocation_id") != allocation_id or
                    type(mint_status.get("credit_block_height")) is not int or
                    mint_status["credit_block_height"] > tip or
                    tip - mint_status["credit_block_height"] + 1 <= 100):
                return False
            deep_terminal = True
    ...
    completed = run_bounded_subprocess(
        list(cfg["terminal_archive_command"]), input_text=archive_request,
        timeout=120, stdout_max=64 * 1024, stderr_max=64 * 1024,
        description="C1 terminal WORM archive")
    if completed.returncode != 0:
        raise RuntimeError("C1 terminal archive refused: " +
                           completed.stderr.strip()[:240])
    acknowledgement = strict_json_loads(
        completed.stdout, "C1 terminal archive acknowledgement")
    if (not isinstance(acknowledgement, dict) or
            set(acknowledgement) != {"version", "archived", "allocation_id",
                                     "record_sha256", "readback_sha256",
                                     "archive_id", "object_lock_mode",
                                     "retention_until"} or
            acknowledgement.get("version") != 1 or
            acknowledgement.get("archived") is not True or
            acknowledgement.get("allocation_id") != allocation_id or
            acknowledgement.get("record_sha256") != record_sha256 or
            acknowledgement.get("readback_sha256") != record_sha256 or
            acknowledgement.get("object_lock_mode") != "COMPLIANCE" or
            type(acknowledgement.get("retention_until")) is not int or
            acknowledgement["retention_until"] <
                archived_at + ARCHIVE_RETENTION_SECONDS or
            not isinstance(acknowledgement.get("archive_id"), str) or
            not 1 <= len(acknowledgement["archive_id"]) <= 256):
...
def maintain_terminal_records(rpc, cfg, state, scan_limit=8,
                              archive_limit=1):
    """Bounded fair reaper independent of per-allocation client retries."""
    if (type(scan_limit) is not int or not 1 <= scan_limit <= 256 or
            type(archive_limit) is not int or
            not 0 <= archive_limit <= scan_limit):
    ...
    if cursor is not None:
        ...
        if start == len(allocation_ids):
            start = 0
    selected = [allocation_ids[(start + offset) % len(allocation_ids)]
    ...
    for allocation_id in selected:
        ...
        if status["retired"]:
            if compact_terminal(
                    rpc, cfg, state, allocation_id, record, status,
...
def emit(status, code):
    public = {"version": 3, "allocation_id": status["allocation_id"],
              "found": status["found"], "active": status["active"],
...
def main():
    try:
        ...
        if (not isinstance(request, dict) or
                set(request) != {"version", "action", "allocation", "funding"} or
                request.get("version") != 2 or
                request.get("action") != "ensure_c1_lifecycle"):
        ...
        status = validate_status(
            rpc.call("getbtcveldc1reservation", [allocation_id]), allocation)
        ...
        if record["kind"] == "terminal":
            if not status["retired"]:
                raise ValueError(
                    "archived terminal sequence reappeared; deep-reorg incident")
        ...
        if status["exposed"]:
            exposed_ready = (status["active"] and
                             status["exposure_canonical_depth_reached"] and
                             status["exposure_confirmations"] >= FINALITY_DEPTH)
            if not exposed_ready:
                rebroadcast_cached_exposure_if_live(
                    rpc, cfg, state, record["EXPOSE"])
                emit(status, 75)
            ...
            if (target_height < status["funding_starts_height"] or
            ...
            for prior in record["FUND"]:
                if (prior["funding_sha256"] != funding_digest and
                        prior["signed_tx_hex"] and in_mempool(rpc, prior)):
                    ...
                    emit(status, 75)
            stage = prepare_stage(
                rpc, cfg, allocation, "FUND", record, state, funding=funding)
        ...
        if int(time.time()) >= allocation["expires_at"]:
            if not status["found"]:
                if not completion_lifecycle_live(rpc):
                    emit(status, 75)
                stage = prepare_stage(
                    rpc, cfg, allocation, "CANCEL", record, state)
                broadcast_exact(rpc, cfg, state, stage)
            emit(status, 75)
        ...
        if (not status["active"] or status["confirmations"] < FINALITY_DEPTH or
                not status["canonical_depth_reached"]):
            if (record["RESERVE"] is not None and
                    record["RESERVE"]["signed_tx_hex"] and
                    mint_lifecycle_live(rpc)):
...
def maintenance_main():
    try:
        ...
        lock = acquire_state_lock(cfg)
        state = load_or_initialize_state(cfg)
        result = maintain_terminal_records(
            rpc, cfg, state, scan_limit=32, archive_limit=1)
        sys.stdout.write(canonical({
            "version": 1, "maintained": True,
