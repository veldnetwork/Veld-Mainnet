...
def fixture(directory):
    ...
    policy.update(
        ...
        member_index=0,
        member_key=h(99),
        minimum_bitcoin_confirmations=144,
        max_fee_sats=1000,
        state_path=str(directory / "native.db"),
        manifest_path=str(directory / "manifest.json"),
        custody_manifest_sha256=h(98),
    )
    cfg = {
        "native_custody": policy,
        "veld_rpc": {"expected_chain": pins},
        "signer_id": "member-one",
    }
    raw = "010203"
    ...
    observed.update(
        ...
        authorization_height=10,
        final_height=20,
        tip=22,
        destination_script_hex="0014" + "12" * 20,
        refund_address="V" + "1" * 33,
    ...
    def native(method, params):
        ...
        if method == "getcompiledgenesis":
            return pins["genesis_hash"]
        if method == "getblockcount":
            return observed["tip"]
        if method == "inspectcustodyintent":
            return copy.deepcopy(observed)
        if method == "getblockhash":
            return {
                0: pins["genesis_hash"],
                1: pins["launch_block_hash"],
                observed["authorization_height"]: observed["authorization_block"],
                observed["final_height"]: observed["final_block"],
                observed["tip"]: observed["tip_hash"],
            }[params[0]]
...
class PolicyTests(unittest.TestCase):
    ...
    def test_exact_finalized_observation(self):
        self.assertEqual(
            service.inspect(self.native, self.cfg["native_custody"], "00", self.raw), self.observed
        )
    ...
    def test_finality_can_advance_only_above_the_same_canonical_prefix(self):
        ...
        self.assertEqual(
            service.refresh_authority(native, self.cfg['native_custody'], '00', self.raw, old),
            self.observed,
        )
    ...
    def test_advancing_tip_never_allows_another_reserve_intent_or_refund(self):
        ...
        for field in ('reserve_txid', 'intent_commitment', 'burn_id', 'authorization_block'):
            ...
            with self.subTest(field=field), self.assertRaises(ValueError):
                service.refresh_authority(
                    self.native, self.cfg['native_custody'], '00', self.raw, old
                )
            self.observed.clear()
        ...
        for field in service.HASH_FIELDS:
            with self.subTest(field=field):
                ...
                with self.assertRaises(ValueError):
                    service.inspect(self.native, policy, "00", self.raw)
        ...
        for field in service.INSPECTION_NUMBERS:
            for value in (True, 1.0, "1", -1, 1 << 64):
                ...
                with self.subTest(field=field, value=value):
                    ...
                    with self.assertRaises(ValueError):
                        service.inspect(self.native, self.cfg["native_custody"], "00", self.raw)
                self.observed[field] = old

    def test_changed_transaction_refused(self):
        with self.assertRaises(ValueError):
            service.inspect(self.native, self.cfg["native_custody"], "00", "010204")
    ...
    def test_schema_confusion_refused(self):
        for value in (None, [], True, dict(self.observed, extra=1)):
            ...
            with self.assertRaises(ValueError):
                service.inspect(bad, self.cfg["native_custody"], "00", self.raw)

    def test_noncanonical_finality_refused(self):
        def wrong(method, params):
            if method == "getblockhash" and params == [20]:
                return "ff" * 32
            return self.native(method, params)

        with self.assertRaises(ValueError):
            service.inspect(wrong, self.cfg["native_custody"], "00", self.raw)
...
@unittest.skipUnless(
    os.name == "posix", "POSIX authority store; managed Windows integration remains separate"
)
class StoreTests(unittest.TestCase):
    ...
    def test_durable_commit_and_original_response(self):
        ...
        try:
            self.assertEqual(
                store.commit(self.observed, self.raw, self.native), "original-response"
            )
        finally:
            store.close()
    ...
    def test_conflicting_intent_and_reserve_burn_refused(self):
        ...
        try:
            ...
            for field in ("intent_commitment", "burn_id"):
                ...
                with self.assertRaises(ValueError):
                    store.commit(altered, self.raw, self.native)
    ...
    def test_retirement_retains_original_and_disables_payout(self):
        ...
        try:
            ...
            with self.assertRaises(ValueError):
                store.commit(self.observed, self.raw, self.native)
            self.assertEqual(
                store.db.execute("SELECT psbt FROM commitments WHERE kind='PAYOUT'").fetchone()[0],
                "original",
            )
        finally:
            store.close()
    ...
    def test_finalized_checkpoint_rollback_refused(self):
        ...
        try:
            store.commit(self.observed, self.raw, self.native)
            for altered, native in (
                (dict(self.observed, final_height=19), self.native),
                (self.observed, lambda *_: "ff" * 32),
            ):
                with self.assertRaises(ValueError):
                    store.commit(altered, self.raw, native)
            self.assertEqual(
                store.db.execute("SELECT final_height FROM identity").fetchone()[0], 20
            )
        finally:
            store.close()
    ...
    def test_retained_signature_cannot_be_replaced(self):
        ...
        try:
            ...
            store.retain(self.observed, "original")
            with self.assertRaises(ValueError):
                store.retain(self.observed, "replacement")
        finally:
            store.close()
