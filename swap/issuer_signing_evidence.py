...
def _canonical_hex(value, maximum, name):
    if (type(value) is not str or not 2 <= len(value) <= 2 * maximum or
            len(value) % 2 or not re.fullmatch(r"[0-9a-f]+", value)):
...
def prepare_evidence(keygen, call, expected_chain, unsigned_tx_hex, issuer_script,
    ...
    run = run_bounded_subprocess([keygen, "prepare-signing-stdin"],
        input_text=json.dumps(payload, separators=(",", ":"), allow_nan=False), timeout=60,
        stdout_max=MAX_EVIDENCE_BYTES, stderr_max=64 * 1024, description="issuer evidence builder")
    if run.returncode:
    ...
    if (type(answer) is not dict or set(answer) != {"version", "genesis_hash", "signature_network",
            "operation_identity_digest", "prepared"} or type(answer["version"]) is not int or
            answer["version"] != 1 or answer["genesis_hash"] != expected["genesis_hash"] or
            answer["signature_network"] not in {"mainnet", "testnet"} or
            not _HASH.fullmatch(str(answer["operation_identity_digest"]))):
        raise ValueError("native signing evidence identity is invalid")
    prepared = answer["prepared"]
    if (type(prepared) is not dict or set(prepared) != {"unsigned_tx_hex", "inputs", "total_input",
            "total_output", "fee", "change"} or prepared["unsigned_tx_hex"] != unsigned_tx_hex or
            type(prepared["inputs"]) is not list or len(prepared["inputs"]) != len(resolved_inputs)):
    ...
    for i, (meta, resolved, parent) in enumerate(zip(prepared["inputs"], resolved_inputs, parents)):
        if (type(meta) is not dict or set(meta) != {"index", "sighash_hex", "prev_script_hex", "value", "parent_tx_hex"} or
                type(meta["index"]) is not int or meta["index"] != i or
                type(meta["value"]) is not int or meta["value"] != resolved.get("value_units") or
                meta["prev_script_hex"] != issuer_script or resolved.get("script_pubkey_hex") != issuer_script or
                meta["parent_tx_hex"] != parent or not _HASH.fullmatch(str(meta["sighash_hex"]))):
    ...
    if (prepared["fee"] != 100000 or prepared["total_input"] != sum(row["value_units"] for row in resolved_inputs) or
            prepared["total_input"] != prepared["total_output"] + prepared["fee"] or
            prepared["change"] != prepared["total_output"]):
        raise ValueError("native signing evidence fee differs from issuer policy")
    if reserve_context is not None:
        if type(reserve_context) is not dict or set(reserve_context) != {"reserve_prior_state_hex", "reserve_prior_supply_sats"}:
    ...
    verify_expected_chain(call, expected)
    return {"version": 2, "prepared": prepared, "authorization": {
        "operation_type": operation_type, "recipient": recipient, "amount": amount,
        "change_destination": issuer, "operation_identity_digest": answer["operation_identity_digest"],
