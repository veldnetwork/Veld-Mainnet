...
def _validate_production_rpc(rpc):
    ...
    if bool(cmd) == bool(token_file):
        raise RuntimeError("production relay requires exactly one RPC token source")
    if token_file:
        if token_file != "/run/veld-btcrelayd/rpc.token":
            raise RuntimeError("production relay token_file must be its exact private runtime file")
...
def validate_config(cfg):
    ...
    if production:
        datadir_args = [v.split("=", 1)[1] for v in cli[1:] if v.startswith("-datadir=")]
    ...
    _strict_int(
        cfg.get("max_headers_per_op", MAX_HEADERS_PER_OP),
        "max_headers_per_op",
        1,
        MAX_HEADERS_PER_OP,
    )
    _strict_int(
        cfg.get("resubmit_timeout_secs", 600),
        "resubmit_timeout_secs",
        MIN_RESUBMIT_SECS,
        MAX_RESUBMIT_SECS,
    )
...
def load_relay_state(path):
    ...
    if not isinstance(obj, dict) or not {"submitted_tip", "submitted_at"} <= set(obj) <= allowed:
    ...
    state = {"submitted_tip": tip, "submitted_at": float(submitted_at), "reorg_rewind": rewind}
    ...
    if all(present):
        start = _strict_int(obj["submitted_start"], "relay_state.submitted_start", 1, 100000000)
        if start > tip:
        ...
        state.update(
            {"submitted_start": start, "submitted_txid": txid, "submitted_tip_hash": tip_hash}
        )
    return state
...
class Relay:
    def __init__(self, cfg):
        ...
        # tunables
        self.checkpoint_height = int(
            cfg.get("checkpoint_height", 0)
        )  # compiled BtcVeldCheckpoint height
        ...
        self.resubmit_secs = int(
            cfg.get("resubmit_timeout_secs", 600)
        )  # in-flight -> assume dropped
    ...
    def post_batch(self, first_h, headers):
        ...
        if (
            not isinstance(first_h, int)
            or first_h <= 0
            or not headers
            or len(headers) > self.max_per_op
            or len(headers) > 255
            or any(not isinstance(header, bytes) or len(header) != 80 for header in headers)
        ):
    ...
    def submitted_batch_is_still_pending(self, submitted, btc_tip):
        ...
        if (
            ...
            or not re.fullmatch(r"[0-9a-f]{64}", tip_hash)
    ...
    def run_once(self):
        ...
        if not info.get("spv_active"):
            log("[relay] SPV dormant (spv_active=false) - nothing to relay")
            return
        best = _strict_int(info.get("best_height"), "consensus best_height", 0, 100000000)
        btc_tip = _strict_int(self.btc.call("getblockcount"), "Bitcoin best height", 0, 100000000)
        ...
        if in_flight and (time.time() - submitted_at) < self.resubmit_secs:
            log(
                f"[relay] waiting for in-flight frontier {best + 1}..{submitted} "
        ...
        if in_flight and self.submitted_batch_is_still_pending(submitted, btc_tip):
            ...
            log(
                f"[relay] submitted frontier {self.state['submitted_start']}..{submitted} "
        ...
        if start > btc_tip:
            if rewind != int(self.state.get("reorg_rewind", 0)):
                ...
                save_json(self.state_path, self.state)
            log(
                f"[relay] up to date: consensus_best={best} btc_tip={btc_tip} "
        ...
        log(
            f"[relay] consensus_best={best} btc_tip={btc_tip} rewind={rewind} "
...
def main():
    ...
    try:
        cfg = validate_config(
            _read_private_json(os.path.abspath(sys.argv[1]), "relay config", MAX_CONFIG_BYTES)
        )
