...
def marker_script(row, txid=None, vout=None):
    ...
    data = (liability.PAYOUT_MARKER_MAGIC + bytes.fromhex(burn_txid) +
            int(burn_vout).to_bytes(4, "big"))
    return (bytes([0x6a, len(data)]) + data).hex()


class VeldFeed:
    def __init__(self, rows, short_interior=False, duplicate_second=False,
                 loop_cursor=False, fail_cursor=None, authority_rows=None):
        self.rows = list(rows)
        self.authority_rows = list(
            self.rows if authority_rows is None else authority_rows)
    ...
    def call(self, method, params=None):
        ...
        if self.duplicate_second and cursor != "" and page:
            page[0] = self.rows[0]
        next_cursor = (cursor if self.loop_cursor and has_more else
                       "page-%d" % (start + len(page)))
        return {
            "tip": TIP,
            "tip_hash": TIP_HASH,
            "final_height": TIP,
...
class BitcoinWallet:
    ...
    def add_payout(self, row, confirmations=K_BTC, amount=None,
                   destination=None, marker=None, txid=None, outgoing=True,
                   abandoned=False, conflicts=None):
    ...
    def call(self, method, *args):
        ...
        if method == "getblockchaininfo":
            return {"chain": self.chain, "blocks": self.height,
                    "bestblockhash": self.best, "initialblockdownload": False}
        if method == "listsinceblock":
            return {"transactions": [{"txid": txid}
...
def records(rows):
    return liability.read_canonical_redeems(
        VeldFeed(rows).call, TIP, TIP_HASH, TOKEN)


class RedeemLiabilityTests(unittest.TestCase):
    def test_redeem_commitment_cross_language_vector(self):
        row = {
            "txid": "01" * 32, "vout": 7, "token": "btcVELD",
            "from": "Vfixture", "to": "", "amount_sats": 123456789,
            "block": 42, "memo": "0014" + "22" * 20,
    ...
    def test_exact_k_confirmed_payout_releases_only_its_liability(self):
        row = redeem_row(amount=100)
        wallet = BitcoinWallet(); wallet.add_payout(row, confirmations=K_BTC)
        payouts = liability.scan_outgoing_payouts(wallet.call)
        self.assertEqual(liability.backing_liability(
            900, records([row]), payouts, K_BTC), (900, 0, 0, 1))
    ...
    def test_unconfirmed_wrong_destination_amount_and_marker_fail_closed(self):
        ...
        unconfirmed = BitcoinWallet(); unconfirmed.add_payout(
            row, confirmations=K_BTC - 1)
        self.assertEqual(liability.backing_liability(
            900, normalized, liability.scan_outgoing_payouts(unconfirmed.call),
            K_BTC), (1000, 100, 1, 0))

        wrong_amount = BitcoinWallet(); wrong_amount.add_payout(row, amount=99)
        with self.assertRaisesRegex(RuntimeError, "wrong destination or amount"):
            liability.backing_liability(
                900, normalized,
                liability.scan_outgoing_payouts(wrong_amount.call), K_BTC)

        wrong_destination = BitcoinWallet(); wrong_destination.add_payout(
            row, destination="0014" + "ff" * 20)
        with self.assertRaisesRegex(RuntimeError, "wrong destination or amount"):
            liability.backing_liability(
                900, normalized,
                liability.scan_outgoing_payouts(wrong_destination.call), K_BTC)

        # A well-formed marker for a different burn cannot release this burn.
        wrong_marker = BitcoinWallet(); wrong_marker.add_payout(
            row, marker=marker_script(row, txid="ff" * 32))
        self.assertEqual(liability.backing_liability(
            900, normalized,
            liability.scan_outgoing_payouts(wrong_marker.call), K_BTC),
            (1000, 100, 1, 0))

        partial = BitcoinWallet(); partial.add_payout(
            row, marker=(b"\x6a\x05VLDR\x01").hex())
    ...
    def test_bitcoin_reorg_restores_liability(self):
        row = redeem_row(amount=100)
        wallet = BitcoinWallet(); txid = wallet.add_payout(row, confirmations=K_BTC)
        normalized = records([row])
    ...
    def test_veld_reorg_disappearance_and_restart_recompute_from_authority(self):
        ...
        self.assertEqual(liability.backing_liability(
            1000, records([]), payouts, K_BTC), (1000, 0, 0, 0))
    ...
    def test_snapshot_mismatch_network_mismatch_and_read_failures(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "simulated Veld"):
            liability.read_canonical_redeems(
                failing_feed.call, TIP, TIP_HASH, TOKEN)

        wallet = BitcoinWallet()
        self.assertEqual(liability.bitcoin_chain_identity(wallet.call),
                         (wallet.height, wallet.best))
        wallet.chain = "test"
        with self.assertRaisesRegex(RuntimeError, "not mainnet"):
            liability.bitcoin_chain_identity(wallet.call)
        wallet.chain = "main"; wallet.fail_method = "listsinceblock"
