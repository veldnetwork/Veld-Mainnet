...
def allocation_commitment(
    request_id, recipient, amount_sats, script_pubkey_hex, commitment_blind_hex
):
    """Return the exact C++ c1reserve::AllocationCommitment hex value."""
    if (
        not isinstance(request_id, str)
        or not REQUEST_RE.fullmatch(request_id)
        or int(request_id, 16) == 0
        or not isinstance(recipient, str)
        or not VELD_RE.fullmatch(recipient)
        or type(amount_sats) is not int
        or amount_sats <= 0
        or amount_sats > (1 << 63) - 1
        or not isinstance(script_pubkey_hex, str)
        or not P2TR_RE.fullmatch(script_pubkey_hex)
        or not isinstance(commitment_blind_hex, str)
        or not COMMITMENT_RE.fullmatch(commitment_blind_hex)
        or int(commitment_blind_hex, 16) == 0
    ):
        raise ValueError("C1 allocation commitment identity is malformed")
    body = (
        _length_prefixed(request_id)
        + _length_prefixed(recipient)
