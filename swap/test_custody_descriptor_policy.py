...
DESCRIPTOR = json.loads((Path(__file__).parent / "fixtures" /
...
class CustodyDescriptorPolicyTests(unittest.TestCase):
    ...
    def test_only_fixed_nums_key_and_three_of_five_tree_are_permitted(self):
        ...
        for count in (4, 6):
            ...
            descriptor = ("tr(" + binding.CUSTODY_NUMS_KEY + ",multi_a(3," +
                          ",".join(selected) + "))#sat2slz5")
    ...
    def test_key_origin_and_derivation_are_exact(self):
        for old, new in (("/86h/0h/0h]", "/86h/1h/0h]"),
                         ("/86h/0h/0h]", "/84h/0h/0h]"),
                         ("/0/*", "/1/*"), ("/0/*", "/0/0")):
